...
> ⚠️ **两个基石后来都被真分析器推翻了，这一节的排序不再成立。**
> `TYPE:MISSING` 验证后只兑现 **5 个**平凡函数且全部多解（见本文第 10 节）；
> `CALL:user-function` 用内联验证是 **0**，而且是结构性的 0
> （见 `.ai/FINDINGS-2026-08-26-call-lever.md`）。
> 下面这一小节保留原文，是为了留住**镜像投影会往哪个方向偏**的记录——
> 六次推翻，六次全部偏乐观。
...
**（上面这段已作废。）** 那个 +44 的两半边分别验成了 5（且全部多解）和 0。
「不要先做 str 方法白名单」这半句仍然成立，但理由变了：不是因为有更好的选项，
而是因为**没有更好的选项**。
...
### 后记：那个「唯一的基石」当天也验成了 0

写下上面这段之后，同日又做了两轮验证，都是「不预测，验证」：

| 提案 | 本文/镜像说 | 真分析器说 | 证据 |
|---|---|---|---|
| 扩规范类型（bytes / `T\|None` / `list[str]` / `-> None`） | 有收益 | **0** | 本文第 7 节 |
| 补类型注解（`TYPE:MISSING`） | 11 | **5，全部多解** | 本文第 10 节 |
| 跨单元调用纯度证明（`CALL:user-function`） | +18 / 基石 | **0（结构性）** | `FINDINGS-2026-08-26-call-lever.md` |
| 换 profile 到类方法（25.7% 的语料） | 最大的一块 | **21，20 个是 `return <字面量>`** | `FINDINGS-2026-08-26-method-profile.md` |

调用杠杆为什么是**结构性**的 0：纯度证明要放行一个调用者，前提是被调方本身可提升；
全语料唯一可提升的函数 `humanize::_gettext_noop` **有 0 个调用点**。
先做它就是先建一座通向空地的桥。

**三次独立测量撞在同一堵墙上——类型墙**（内联后 87%、方法提升后 93.6%、
类型面自身上界 12）。**没有任何单点改动能把这条线从 0.1% 量级抬起来。**

真正的选择因此不在「再放宽一条规则」这个层面：**要么接受这条线的定位是
「窄子集上的可证明转换」而不是「通用转换」，要么把 `self` 的对象类型接进类型系统——
后者不是放宽 profile，是另一个工程。** 这个判断有四组实测数据托着，不是意见。

**方法论上真正该带走的一条**：本文里每一个由镜像推出来的数，后来被真分析器裁决时
**无一例外地变小**（`list[str]` 13→0、body_clean 32→13、类型面上界 31→12、
calls_only 27→18、注解 11→5、调用 18→0）。**六次，方向一次都没反过。**
手写近似永远往「更宽松」漂，因为漏一条规则＝少一个拒绝。
**凡是要发布的数字，必须由真分析器裁决。**
