...
> **本表的"真实运行证据"一列是第二轮的数字，后面几轮改过其中三个**，正文没有回改，
> 因为这份文档是按轮次追加的记录，不是当前状态快照。当前值：
> 系统 ETA P50 **215.25h → 56.648h**（真实遥测校准，§8.2）；
> 置信度 **0.52 → 0.60**（证据上限 0.65，§9.2/§10.9）；
> 费用一栏已换成**已核验列表价**，但占比未经校准、对长任务高估（§10.4/§10.4.1）。
> **要当前状态请看 §10、§11；本节只说明每个 Skill 落在哪。**

| Skill | 状态 | 实现 | 测试 | 真实运行证据（第二轮，部分已被后续轮次取代） |
| --- | --- | --- | --- | --- |
...
## 3. 真实仓库上跑过的命令（2026-08-19，第二轮）

> **这一节的数字是第二轮那次运行的**，保留原样以便复算。后续轮次改变了其中几项，
> 每处都在下面就地标了注记。当前状态见 §10、§11。
...
> **三处已被后续轮次取代：**
> - **系统 wall-clock 215.25h → 56.648h**：接入真实 pytest 日志后，验证类任务被高估
>   2.6–3.8 倍（§8.2）。人工基线与加速比也随之变化。
> - **置信度 0.52 → 0.60**：证据推导出的上限升到 0.65 之后才提的，且写在 seed 里
>   而不是产物里（§9.2、§10.9）。
> - **上面那个 Token 结构（63/24/7/3.3/2.1）是假设，不是实测。** 真实会话实测是
>   `cached_input` **98.8%**、`input` **0.0005%**，而且随任务长度变化（§10.4、§10.4.1）。
>   Token **总量**没变，但**费用**因此对长任务高估数倍。
...
- **设备侧没有 pytest。** 158 项单测在云端 Python 3.11 执行；设备侧只跑了 CLI 端到端。
  （当前是 **291 项**，且 CI 在 Python 3.10 + 3.12 上都跑，见 §9.9。）
