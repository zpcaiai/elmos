...
`/repositories` 可从 GitHub、Gitee 或允许列表内的通用 HTTPS Git 服务建立精确提交工作区，读取/修改受控文本后，一键交接到 `/generation`。生成端重新校验同一租户/Actor、工作区完整性和文件 SHA-256，将最多八个仓库文件作为 `repository-file` 来源绑定到审批摘要，再生成、真实验证和归档完整项目；仓库脚本不会在导入时执行，推送、PR、合并与云部署仍保持独立授权和 `NOT_RUN`。完整软硬件要求、配置及逐步操作见 [`docs/GIT_REPOSITORY_WORKSPACES.md`](docs/GIT_REPOSITORY_WORKSPACES.md)。
