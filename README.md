...
附属能力的实测覆盖率分为两项：SQL 方言转写对当前 76 个真实迁移文件的自动转写候选已提升到 **1128/1485 = 76.0%**，
...
四目标 emitter reachability 在默认无 namespace profile 下为 **325/1128 = 28.8%**，显式源默认命名空间映射
`{"": "dbo"}` profile 下为 **375/1128 = 33.2%**；这些仍是保守上界，不代表实库执行或认证，
