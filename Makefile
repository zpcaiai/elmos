...
.PHONY: test-engine test-engines-check test-engines-list test-engines-all certification-key-hygiene-check ethan-external-certification-controls

verify: business-line-contracts backend dotnet python frontend sql-transpiler sql-dialect component-dialect web
business-line-contracts: ethan-external-certification-controls model-catalog-check makefile-portability-check test-engines-check chinadb-commercial-migration-skills
...
ethan-external-certification-controls: certification-key-hygiene-check
	python3 -m unittest discover -s tests/production-readiness -p 'test_ethan_external_exchange.py'
test-engines-check:
