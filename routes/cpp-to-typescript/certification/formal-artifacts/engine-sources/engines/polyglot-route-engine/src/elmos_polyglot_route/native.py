...
_JAVASCRIPT_ANALYZER_SHA256 = "50baa7a855d4b50899da32003f164444db60eb64454ce596d6a6f4e9f1c06a56"
_JAVASCRIPT_ANALYZER_BYTES = 27_569
...
def _run_trusted_go_analyzer(
    ...
    """Run the Go analyzer and promote only its exact missing-function result.

    ``go run`` reports a normal analyzer rejection through a non-zero process
    exit, so ``_run`` wraps it as ``NATIVE_ANALYZER_FAILED``.  The route-level
    negative contract, however, is language-independent and requires the exact
    ``FUNCTION_NOT_FOUND:<requested-name>`` value.  Deriving the sole allowed
    value from the validated command prevents arbitrary stderr, a different
    function name, or a forged executable prefix from becoming a semantic
    decision.
    """

    if toolchain.language != "go" or helper != ENGINE_ROOT / "native" / "go" / "analyzer.go":
    ...
    if not source.is_absolute():
        raise RouteError("GO_ANALYZER_COMMAND_SHAPE_INVALID")
    if len(arguments) == 3 and arguments[2] != "--emitted-target":
        raise RouteError("GO_ANALYZER_COMMAND_SHAPE_INVALID")
    if selector == "--inventory":
        if len(arguments) != 2:
            raise RouteError("GO_ANALYZER_COMMAND_SHAPE_INVALID")
        promotable = None
    elif selector.startswith("--functions="):
        if len(arguments) == 3 and arguments[2] != "--emitted-target":
            raise RouteError("GO_ANALYZER_COMMAND_SHAPE_INVALID")
        names = selector.removeprefix("--functions=").split(",")
        if not names or any(re.fullmatch(r"[A-Za-z_][A-Za-z0-9_]*", name) is None for name in names):
            raise RouteError("GO_ANALYZER_COMMAND_SHAPE_INVALID")
        promotable = None
    else:
        if re.fullmatch(r"[A-Za-z_][A-Za-z0-9_]*", selector) is None:
            raise RouteError("GO_ANALYZER_COMMAND_SHAPE_INVALID")
        promotable = f"FUNCTION_NOT_FOUND:{selector}"
    ...
    try:
        ...
        if promotable is not None:
            wrapped_prefix = f"NATIVE_ANALYZER_FAILED:{toolchain.executable}:"
            if str(error) in {
                f"{wrapped_prefix}{promotable}",
                f"{wrapped_prefix}{promotable}\nexit status 2",
            }:
                raise RouteError(promotable) from error
        raise
...
def _run_trusted_rust_analyzer(
    ...
    """Run the Rust analyzer with exact missing-function error promotion."""

    cargo = toolchain.auxiliary
    expected_package = ENGINE_ROOT / "native" / "rust"
    if toolchain.language != "rust" or cargo is None or package != expected_package:
    ...
    try:
        ...
        if promotable is not None and str(error) == (
            f"NATIVE_ANALYZER_FAILED:{cargo}:{promotable}"
        ):
            raise RouteError(promotable) from error
        raise
    ...
    *,
    ...
    try:
        if language == "java":
            ...
            missing_function_errors = frozenset(
                f"FUNCTION_NOT_FOUND:{name}" for name in function_names
            )

            def promote(reason: str) -> RouteError | None:
                if reason in missing_function_errors:
            ...
            if toolchain.auxiliary is None:
                return None
            cargo = toolchain.auxiliary
            document = _run_trusted_rust_analyzer(
                toolchain,
                package,
                [str(resolved), selector, *(["--emitted-target"] if emitted_target else [])],
            )
            missing_function_errors = frozenset(
                f"FUNCTION_NOT_FOUND:{name}" for name in function_names
            )
...
def analyze(
    ...
    *,
    ...
    if language == "java":
        ...
        package = ENGINE_ROOT / "native" / "rust"
        value = _run_trusted_rust_analyzer(
            toolchain,
            package,
            [str(source), function_name, *(["--emitted-target"] if emitted_target else [])],
        )
