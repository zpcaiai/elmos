...
# Routes whose persisted local evidence is complete enough for the
# authoritative per-route gate.  The immutable core 30 are replay-verified and
# the 60 admitted direct routes are executable.  The V3 66 remain active in the
# declared matrix but research-only/NOT_RUN, so including them here would turn
# an intentional evidence boundary into a structurally impossible CI gate.
LOCAL_EVIDENCE_ROUTE_KEYS = (*CORE_ROUTE_KEYS, *EXECUTABLE_DIRECT_ROUTE_KEYS)
if len(LOCAL_EVIDENCE_ROUTE_KEYS) != 90:
    raise RuntimeError("LOCAL_EVIDENCE_ROUTE_COUNT_DRIFT")
if set(LOCAL_EVIDENCE_ROUTE_KEYS) & set(V3_EXACT_ROUTE_KEYS):
    raise RuntimeError("LOCAL_EVIDENCE_ROUTES_INCLUDE_RESEARCH_ONLY_ROUTE")
if set(LOCAL_EVIDENCE_ROUTE_KEYS) | set(V3_EXACT_ROUTE_KEYS) != set(
    COMPLETE_ROUTE_KEYS
):
    raise RuntimeError("LOCAL_EVIDENCE_ROUTE_PARTITION_INCOMPLETE")
