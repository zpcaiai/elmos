...
Names exceeding Codex's 64-character limit use deterministic SHA-256 aliases
recorded as `source_name` mappings in `manifest.json`. Every Skill includes an
`agents/openai.yaml` interface that invokes its normalized `$skill-name`.
