---
name: b74-powershell-project-profile-detector
description: "Use when ELMOS must inspect and model Powershell Project Profile Detector for Bash/POSIX shell and PowerShell scripts, modules, error handling, arguments, processes, remoting, secrets, signing, linting, and tests, preserving ownership, compatibility, security, reproducibility, and evidence boundaries. Triggers: Powershell Project Profile Detector, Batch 74, Bash/POSIX shell and PowerShell scripts, modules, error handling, arguments, processes, remoting, secrets, signing, linting, and tests."
metadata:
  id: PG328
  batch: 74
  version: 1.0.0
  ...
  category: "Bash Shell and PowerShell Operations Pack"
---

# PG328 — Powershell Project Profile Detector

## Objective

Inspect and model Powershell Project Profile Detector for Bash/POSIX shell and PowerShell scripts, modules, error handling, arguments, processes, remoting, secrets, signing, linting, and tests, preserving ownership, compatibility, security, reproducibility, and evidence boundaries.
...
Use for `powershell-project-profile-detector` tasks involving Bash/POSIX shell and PowerShell scripts, modules, error handling, arguments, processes, remoting, secrets, signing, linting, and tests.
...
- Inspect source markers such as `*.sh`, `*.bash`, `*.zsh`, `*.ps1`, `*.psm1`, `*.psd1` and resolve effective includes, generated files, workspaces, modules, targets, and entrypoints.
...
Upstream dependencies: `PG183`, `PG184`, `PG185`, `PG191`, `PG320`, `PG327`.
...
- `powershell_project_profile_detector.json`
- `powershell_project_profile_detector_artifacts/`
- `powershell_project_profile_detector_verification.json`
...
- Preferred tools: `bash/sh/zsh`, `shellcheck`, `shfmt`, `bats`, `pwsh`, `PSScriptAnalyzer`, `Pester`. Record alternate tools, versions, and rationale.
...
- Do not classify technology from file extensions alone.
- Separate generated, vendored, test, example, and production assets.
- Record evidence and confidence for every detected boundary.
- Quote expansions and treat filenames/PATH as untrusted.
- Test traps, pipelines, signals, temporary files, and cleanup.
- Avoid eval unless independently justified.
- Distinguish Windows PowerShell and PowerShell 7.
- Handle native exit codes and PowerShell error records.
- Use ShouldProcess for consequential operations.
