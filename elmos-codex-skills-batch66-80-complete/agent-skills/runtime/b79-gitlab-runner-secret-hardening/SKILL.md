---
name: b79-gitlab-runner-secret-hardening
description: "Use when ELMOS must enforce and test GitLab Runner Secret Hardening for GitHub Actions, GitLab CI, Jenkins pipelines, reusable components, runners, permissions, OIDC, caches, environments, releases, provenance, and cross-system certification, preserving ownership, compatibility, security, reproducibility, and evidence boundaries. Triggers: GitLab Runner Secret Hardening, Batch 79, GitHub Actions, GitLab CI, Jenkins pipelines, reusable components, runners, permissions, OIDC, caches, environments, releases, provenance, and cross-system certification."
metadata:
  id: PG398
  batch: 79
  version: 1.0.0
  ...
  category: "GitHub Actions GitLab CI and Jenkins Pipeline Pack"
---

# PG398 — GitLab Runner Secret Hardening

## Objective

Enforce and test GitLab Runner Secret Hardening for GitHub Actions, GitLab CI, Jenkins pipelines, reusable components, runners, permissions, OIDC, caches, environments, releases, provenance, and cross-system certification, preserving ownership, compatibility, security, reproducibility, and evidence boundaries.
...
Use for `gitlab-runner-secret-hardening` tasks involving GitHub Actions, GitLab CI, Jenkins pipelines, reusable components, runners, permissions, OIDC, caches, environments, releases, provenance, and cross-system certification.
...
- Inspect source markers such as `.github/workflows/*.yml`, `.gitlab-ci.yml`, `gitlab-ci/*.yml`, `Jenkinsfile`, `vars/*.groovy` and resolve effective includes, generated files, workspaces, modules, targets, and entrypoints.
...
Upstream dependencies: `PG183`, `PG184`, `PG185`, `PG191`, `PG390`, `PG397`.
...
- `gitlab_runner_secret_hardening_plan.json`
- `gitlab_runner_secret_hardening_artifacts/`
- `gitlab_runner_secret_hardening_verification.json`
...
- Preferred tools: `GitHub Actions`, `GitLab CI`, `Jenkins`, `actionlint`, `yamllint`, `container runners`, `OIDC/cloud CLIs`. Record alternate tools, versions, and rationale.
...
- Use references, workload identity, or secret stores rather than plaintext.
- Verify redaction before evidence persistence.
- Test rotation and missing-secret failure behavior.
- Resolve all includes and rules.
- Protect runners, variables, environments, and dynamic pipelines.
- Test review-app cleanup and deployment serialization.
...
```bash
gitlab-ci-lint <config>
```

```bash
jenkinsfile-runner <options>
```
