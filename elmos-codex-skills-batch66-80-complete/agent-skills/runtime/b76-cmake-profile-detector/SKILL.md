---
name: b76-cmake-profile-detector
description: "Use when ELMOS must inspect and model CMake Profile Detector for Makefile and CMake build graphs plus Nginx reverse proxy, TLS, rate limiting, routing, and configuration certification, preserving ownership, compatibility, security, reproducibility, and evidence boundaries. Triggers: CMake Profile Detector, Batch 76, Makefile and CMake build graphs plus Nginx reverse proxy, TLS, rate limiting, routing, and configuration certification."
metadata:
  id: PG355
  batch: 76
  version: 1.0.0
  ...
  category: "Makefile CMake and Nginx Build and Proxy Pack"
---

# PG355 — CMake Profile Detector

## Objective

Inspect and model CMake Profile Detector for Makefile and CMake build graphs plus Nginx reverse proxy, TLS, rate limiting, routing, and configuration certification, preserving ownership, compatibility, security, reproducibility, and evidence boundaries.
...
Use for `cmake-profile-detector` tasks involving Makefile and CMake build graphs plus Nginx reverse proxy, TLS, rate limiting, routing, and configuration certification.
...
- Inspect source markers such as `Makefile`, `*.mk`, `CMakeLists.txt`, `CMakePresets.json`, `nginx.conf`, `conf.d/*.conf` and resolve effective includes, generated files, workspaces, modules, targets, and entrypoints.
...
Upstream dependencies: `PG183`, `PG184`, `PG185`, `PG191`, `PG350`, `PG354`.
...
- `cmake_profile_detector.json`
- `cmake_profile_detector_artifacts/`
- `cmake_profile_detector_verification.json`
...
- Preferred tools: `make`, `cmake`, `ninja`, `nginx`, `configuration linters`. Record alternate tools, versions, and rationale.
...
- Use target-scoped properties and out-of-source builds.
- Test install/export consumers and presets.
- Avoid directory-global compiler/link flags.
- Declare phony targets and safe clean roots.
- Test parallel and incremental rebuild behavior.
- Keep local and CI targets aligned.
