---
name: b73-swift-package-dependency-reproducer
description: "Use when ELMOS must reproduce deterministically Swift Package Dependency Reproducer for Swift, SwiftUI, Xcode, SwiftPM, state, networking, persistence, concurrency, Objective-C interoperability, widgets, accessibility, XCTest, and signing, preserving ownership, compatibility, security, reproducibility, and evidence boundaries. Triggers: Swift Package Dependency Reproducer, Batch 73, Swift, SwiftUI, Xcode, SwiftPM, state, networking, persistence, concurrency, Objective-C interoperability, widgets, accessibility, XCTest, and signing."
metadata:
  id: PG310
  batch: 73
  version: 1.0.0
  ...
  category: "Swift and Apple Native Project Pack"
---

# PG310 — Swift Package Dependency Reproducer

## Objective

Reproduce deterministically Swift Package Dependency Reproducer for Swift, SwiftUI, Xcode, SwiftPM, state, networking, persistence, concurrency, Objective-C interoperability, widgets, accessibility, XCTest, and signing, preserving ownership, compatibility, security, reproducibility, and evidence boundaries.
...
Use for `swift-package-dependency-reproducer` tasks involving Swift, SwiftUI, Xcode, SwiftPM, state, networking, persistence, concurrency, Objective-C interoperability, widgets, accessibility, XCTest, and signing.
...
- Inspect source markers such as `Package.swift`, `*.xcodeproj`, `*.xcworkspace`, `Sources/`, `Tests/`, `*.entitlements`, `Info.plist` and resolve effective includes, generated files, workspaces, modules, targets, and entrypoints.
...
Upstream dependencies: `PG183`, `PG184`, `PG185`, `PG191`, `PG308`, `PG309`.
...
- `swift_package_dependency_reproducer_plan.json`
- `swift_package_dependency_reproducer_artifacts/`
- `swift_package_dependency_reproducer_verification.json`
...
- Preferred tools: `swift`, `xcodebuild`, `Swift Package Manager`, `swift-format/swiftlint`, `XCTest`, `simctl`. Record alternate tools, versions, and rationale.
...
- Use frozen or locked dependency resolution.
- Do not execute untrusted lifecycle/build scripts before policy review.
- Report every lockfile or dependency graph change.
- Record Xcode, SDK, deployment target, scheme, entitlement, and signing assumptions.
- Distinguish simulator and device evidence.
- Test strict concurrency and lifecycle behavior.
