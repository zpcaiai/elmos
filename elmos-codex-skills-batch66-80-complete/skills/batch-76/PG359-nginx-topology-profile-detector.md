---
name: b76-nginx-topology-profile-detector
description: "Use when ELMOS must inspect and model Nginx Topology Profile Detector for Makefile and CMake build graphs plus Nginx reverse proxy, TLS, rate limiting, routing, and configuration certification, preserving ownership, compatibility, security, reproducibility, and evidence boundaries. Triggers: Nginx Topology Profile Detector, Batch 76, Makefile and CMake build graphs plus Nginx reverse proxy, TLS, rate limiting, routing, and configuration certification."
metadata:
  id: PG359
  batch: 76
  version: 1.0.0
...
# PG359 — Nginx Topology Profile Detector

## Objective

Inspect and model Nginx Topology Profile Detector for Makefile and CMake build graphs plus Nginx reverse proxy, TLS, rate limiting, routing, and configuration certification, preserving ownership, compatibility, security, reproducibility, and evidence boundaries.
...
Use for `nginx-topology-profile-detector` tasks involving Makefile and CMake build graphs plus Nginx reverse proxy, TLS, rate limiting, routing, and configuration certification.
...
Upstream dependencies: `PG183`, `PG184`, `PG185`, `PG191`, `PG350`, `PG358`.
...
- `nginx_topology_profile_detector.json`
- `nginx_topology_profile_detector_artifacts/`
- `nginx_topology_profile_detector_verification.json`
...
- Resolve the effective include tree and location precedence.
- Test TLS, client IP trust, timeouts, WebSocket/gRPC, caching, and upstream failure.
