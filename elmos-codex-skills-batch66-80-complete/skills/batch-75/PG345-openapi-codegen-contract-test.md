---
name: b75-openapi-codegen-contract-test
description: "Use when ELMOS must implement and validate OpenAPI Codegen Contract Test for SQL migrations, queries, transactions, fixtures, performance and security plus OpenAPI, Protocol Buffers, GraphQL, code generation, and compatibility, preserving ownership, compatibility, security, reproducibility, and evidence boundaries. Triggers: OpenAPI Codegen Contract Test, Batch 75, SQL migrations, queries, transactions, fixtures, performance and security plus OpenAPI, Protocol Buffers, GraphQL, code generation, and compatibility."
metadata:
  id: PG345
  batch: 75
  version: 1.0.0
  ...
  category: "SQL and API Contract Language Pack"
---

# PG345 — OpenAPI Codegen Contract Test

## Objective

Implement and validate OpenAPI Codegen Contract Test for SQL migrations, queries, transactions, fixtures, performance and security plus OpenAPI, Protocol Buffers, GraphQL, code generation, and compatibility, preserving ownership, compatibility, security, reproducibility, and evidence boundaries.
...
Use for `openapi-codegen-contract-test` tasks involving SQL migrations, queries, transactions, fixtures, performance and security plus OpenAPI, Protocol Buffers, GraphQL, code generation, and compatibility.
...
- Inspect source markers such as `*.sql`, `openapi*.yaml/json`, `*.proto`, `*.graphql`, `*.graphqls` and resolve effective includes, generated files, workspaces, modules, targets, and entrypoints.
...
Upstream dependencies: `PG183`, `PG184`, `PG185`, `PG191`, `PG334`, `PG344`.
...
- `openapi_codegen_contract_test_plan.json`
- `openapi_codegen_contract_test_artifacts/`
- `openapi_codegen_contract_test_verification.json`
...
- Preferred tools: `database clients`, `migration tools`, `SQL linters`, `Spectral/Redocly`, `protoc/buf`, `GraphQL validators/codegen`. Record alternate tools, versions, and rationale.
...
- Tests must be isolated, deterministic, and failure-artifact preserving.
- Do not hide deterministic failures with retries.
- Include unit, integration, and negative paths.
- Classify breaking, dangerous, additive, and documentation-only changes.
- Round-trip representative wire fixtures.
- Preserve stable operation/message/field identities.
- Require stable operationId values and explicit per-operation security.
- Validate examples against schemas.
- Run breaking-change analysis against a baseline.
