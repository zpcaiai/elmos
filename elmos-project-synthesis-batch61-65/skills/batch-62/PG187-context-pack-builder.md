...
status: proposed
summary: Builds minimal, source-grounded context packs for each specialist agent.
depends_on: ["PG004", "PG009", "PG182"]
capabilities:
  - workspace:read
...
# PG187 — Context Pack Builder

## 1. Objective

Improve accuracy while reducing context overload, leakage, and stale assumptions.
...
- Validate and consume agent_task.
- Validate and consume artifact_graph.
- Validate and consume repository_index.
- Validate and consume policy_context.
- Validate and consume context_budget.
- Produce and validate context_pack.
- Produce and validate source_map.
- Produce and validate omission_report.
- Produce and validate context_fingerprint.
...
1. Identify authoritative upstream objects and relevant repository symbols.
2. Retrieve bounded contiguous source and specification context.
3. Prioritize approved facts, current versions, and direct dependencies.
4. Exclude unrelated tenant, secret, stale, and superseded content.
5. Publish source mapping and intentional omissions.
...
- `MISSING_AUTHORITATIVE_CONTEXT` — stop unsafe continuation and emit a structured diagnostic.
- `CONTEXT_OVERFLOW` — stop unsafe continuation and emit a structured diagnostic.
- `CROSS_TENANT_LEAK` — stop unsafe continuation and emit a structured diagnostic.
- `STALE_ARTIFACT_INCLUDED` — stop unsafe continuation and emit a structured diagnostic.
...
- Reject or escalate `missing_authoritative_context` without inventing success.
- Reject or escalate `context_overflow` without inventing success.
- Reject or escalate `cross_tenant_leak` without inventing success.
- Reject or escalate `stale_artifact_included` without inventing success.
...
- Every material fact in the pack has a source.
- Context stays within budget without omitting required dependencies.
- Superseded and cross-tenant content is excluded.
