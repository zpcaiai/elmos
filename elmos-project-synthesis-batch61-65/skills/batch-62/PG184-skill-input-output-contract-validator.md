---
id: PG184
name: skill-input-output-contract-validator
version: 1.0.0
batch: 62
engine: elmos.project-synthesis
schema: elmos.project-synthesis.v1
status: proposed
summary: Validates Skill manifests, schemas, preconditions, outputs, evidence, and version compatibility.
depends_on: ["PG002", "PG003", "PG007", "PG183"]
capabilities:
  - workspace:read
...
# PG184 — Skill Input Output Contract Validator

## 1. Objective

Prevent malformed or falsely compatible Skills from entering agent workflows.
...
- Validate and consume skill_manifest.
- Validate and consume input_artifacts.
- Validate and consume expected_outputs.
- Validate and consume platform_contracts.
- Produce and validate contract_validation.
- Produce and validate adapter_requirements.
- Produce and validate blocking_findings.
- Produce and validate compatibility_evidence.
...
1. Validate manifest structure, identifiers, versions, capabilities, and dependencies.
2. Validate input schema, semantic preconditions, approval state, and provenance.
3. Validate output schema and evidence obligations.
4. Check backward compatibility and required adapters.
5. Block execution on critical contract ambiguity.
...
- `MANIFEST_INVALID` — stop unsafe continuation and emit a structured diagnostic.
- `INPUT_SEMANTIC_GAP` — stop unsafe continuation and emit a structured diagnostic.
- `OUTPUT_CONTRACT_MISMATCH` — stop unsafe continuation and emit a structured diagnostic.
- `VERSION_INCOMPATIBILITY` — stop unsafe continuation and emit a structured diagnostic.
...
- Reject or escalate `manifest_invalid` without inventing success.
- Reject or escalate `input_semantic_gap` without inventing success.
- Reject or escalate `output_contract_mismatch` without inventing success.
- Reject or escalate `version_incompatibility` without inventing success.
...
- Every invoked Skill passes contract validation.
- Adapters are explicit and versioned.
- Missing evidence obligations are blocking.
