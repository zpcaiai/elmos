---
id: PG188
name: domain-pack-sdk
version: 1.0.0
batch: 62
engine: elmos.project-synthesis
schema: elmos.project-synthesis.v1
status: proposed
summary: Defines packaging, extension, validation, testing, and publication contracts for Domain Packs.
depends_on: ["PG065", "PG184"]
capabilities:
  - workspace:read
...
# PG188 — Domain Pack Sdk

## 1. Objective

Allow reusable domain intelligence without hard-coding industries into the core engine.
...
- Validate and consume domain_pack_definition.
- Validate and consume platform_schemas.
- Validate and consume skill_registry.
- Validate and consume template_registry.
- Produce and validate domain_pack_manifest.
- Produce and validate extension_contracts.
- Produce and validate validation_suite.
- Produce and validate sdk_artifacts.
...
1. Define pack metadata, glossary, entities, rules, workflows, permissions, integrations, tests, templates, and policies.
2. Define extension points and override precedence.
3. Generate validation and compatibility tests.
4. Create authoring, linting, and local-preview SDK assets.
5. Publish signed pack artifacts.
...
- `DOMAIN_PACK_SCHEMA_INVALID` — stop unsafe continuation and emit a structured diagnostic.
- `UNSAFE_OVERRIDE` — stop unsafe continuation and emit a structured diagnostic.
- `CORE_CONTRACT_COLLISION` — stop unsafe continuation and emit a structured diagnostic.
- `UNSIGNED_PACK` — stop unsafe continuation and emit a structured diagnostic.
...
- Reject or escalate `domain_pack_schema_invalid` without inventing success.
- Reject or escalate `unsafe_override` without inventing success.
- Reject or escalate `core_contract_collision` without inventing success.
- Reject or escalate `unsigned_pack` without inventing success.
...
- A Domain Pack can be installed without modifying core engine code.
- Overrides cannot weaken constitutional or tenant policy.
- Pack compatibility is machine-testable.
