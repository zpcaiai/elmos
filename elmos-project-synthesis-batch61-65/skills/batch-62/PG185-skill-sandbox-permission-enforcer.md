---
id: PG185
name: skill-sandbox-permission-enforcer
version: 1.0.0
batch: 62
engine: elmos.project-synthesis
schema: elmos.project-synthesis.v1
status: proposed
summary: Enforces process, filesystem, network, repository, secret, deployment, and data capabilities for Skills and agents.
depends_on: ["PG008", "PG147", "PG184"]
capabilities:
  - workspace:read
...
# PG185 — Skill Sandbox Permission Enforcer

## 1. Objective

Apply least privilege at execution time, not only in prompt instructions.
...
- Validate and consume sandbox_profile.
- Validate and consume policy_context.
- Produce and validate sandbox_configuration.
- Produce and validate runtime_decisions.
- Produce and validate violations.
- Produce and validate permission_evidence.
...
1. Compile declared capabilities into sandbox controls.
2. Mount only approved files and directories.
3. Restrict process execution, network destinations, registries, secrets, and cloud identities.
4. Monitor attempted violations and terminate unsafe actions.
5. Record effective permissions and runtime evidence.
...
- `SANDBOX_ESCAPE` — stop unsafe continuation and emit a structured diagnostic.
- `UNDECLARED_NETWORK` — stop unsafe continuation and emit a structured diagnostic.
- `SECRET_SCOPE_VIOLATION` — stop unsafe continuation and emit a structured diagnostic.
- `REPOSITORY_WRITE_VIOLATION` — stop unsafe continuation and emit a structured diagnostic.
...
- Reject or escalate `sandbox_escape` without inventing success.
- Reject or escalate `undeclared_network` without inventing success.
- Reject or escalate `secret_scope_violation` without inventing success.
- Reject or escalate `repository_write_violation` without inventing success.
...
- Effective permissions are no broader than declared grants.
- Violations are blocked and audited.
- Sensitive credentials are short-lived and scope-bound.
