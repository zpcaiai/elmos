---
id: PG210
name: ai-agent-rag-domain-pack
version: 1.0.0
batch: 64
engine: elmos.project-synthesis
schema: elmos.project-synthesis.v1
status: proposed
summary: Provides knowledge source, ingestion, retrieval, prompt, tool, agent, memory, evaluation, and safety domain models.
depends_on: ["PG092", "PG188", "PG202"]
capabilities:
  - workspace:read
...
# PG210 — Ai Agent Rag Domain Pack

## 1. Objective

Accelerate governed Agentic and RAG application synthesis.
...
- Validate and consume ai_requirements.
- Validate and consume model_and_data_policy.
- Produce and validate agent_rag_pack.
- Produce and validate tool_contracts.
- Produce and validate evaluation_suites.
- Produce and validate safety_controls.
...
1. Model sources, documents, chunks, embeddings, indexes, retrieval, prompts, models, agents, tools, memory, runs, and evaluations.
2. Define ingestion, retrieval, generation, tool-call, approval, and feedback workflows.
3. Define prompt-injection, data-boundary, tool-permission, citation, and hallucination controls.
4. Provide model routing and cost policies.
5. Provide adversarial retrieval, tool misuse, and evaluation scenarios.
...
- `PROMPT_INJECTION` — stop unsafe continuation and emit a structured diagnostic.
- `CROSS_TENANT_RETRIEVAL` — stop unsafe continuation and emit a structured diagnostic.
- `UNSAFE_TOOL_CALL` — stop unsafe continuation and emit a structured diagnostic.
- `UNCITED_HIGH_RISK_CLAIM` — stop unsafe continuation and emit a structured diagnostic.
...
- Reject or escalate `prompt_injection` without inventing success.
- Reject or escalate `cross_tenant_retrieval` without inventing success.
- Reject or escalate `unsafe_tool_call` without inventing success.
- Reject or escalate `uncited_high_risk_claim` without inventing success.
...
- Retrieval enforces tenant and source permissions.
- High-risk outputs include verification or human approval.
