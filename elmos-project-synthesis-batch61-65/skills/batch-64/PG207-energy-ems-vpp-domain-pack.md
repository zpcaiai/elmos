---
id: PG207
name: energy-ems-vpp-domain-pack
version: 1.0.0
batch: 64
engine: elmos.project-synthesis
schema: elmos.project-synthesis.v1
status: proposed
summary: Provides assets, meters, forecasts, dispatch, constraints, tariffs, markets, settlement, and flexibility models.
...
# PG207 — Energy Ems Vpp Domain Pack

## 1. Objective

Accelerate energy management and virtual power plant systems with auditable optimization boundaries.
...
- Validate and consume energy_requirements.
- Validate and consume market_and_grid_rules.
- Produce and validate energy_domain_pack.
- Produce and validate dispatch_workflows.
- Produce and validate market_integrations.
...
1. Model sites, devices, meters, forecasts, schedules, bids, constraints, dispatch, events, and settlements.
2. Define command acknowledgment, override, fallback, and safety constraints.
3. Define tariff, market, weather, grid, and device integration contracts.
4. Provide forecast error, communications loss, conflicting dispatch, and reconciliation tests.
5. Publish SLO and audit requirements.
...
- `CONSTRAINT_VIOLATION` — stop unsafe continuation and emit a structured diagnostic.
- `UNSAFE_DISPATCH` — stop unsafe continuation and emit a structured diagnostic.
- `METER_DATA_GAP` — stop unsafe continuation and emit a structured diagnostic.
- `SETTLEMENT_MISMATCH` — stop unsafe continuation and emit a structured diagnostic.
...
- Reject or escalate `constraint_violation` without inventing success.
- Reject or escalate `unsafe_dispatch` without inventing success.
- Reject or escalate `meter_data_gap` without inventing success.
- Reject or escalate `settlement_mismatch` without inventing success.
...
- Dispatch respects hard device and grid constraints.
- Manual override and communications-loss behavior are explicit.
- Settlement inputs are traceable and reconcilable.
