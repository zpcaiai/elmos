---
id: PG214
name: architecture-review-ui
version: 1.0.0
batch: 65
engine: elmos.project-synthesis
schema: elmos.project-synthesis.v1
status: proposed
summary: Provides interactive architecture diagrams, ADRs, tradeoffs, risks, threats, deployment, and approval views.
depends_on: ["PG054", "PG179"]
capabilities:
  - workspace:read
...
# PG214 — Architecture Review Ui

## 1. Objective

Make architecture planning and accountability understandable to engineers and stakeholders.
...
- Validate and consume adr_catalog.
- Validate and consume tradeoff_matrix.
- Validate and consume threat_model.
- Validate and consume approval_state.
- Produce and validate architecture_review_views.
- Produce and validate diagram_models.
- Produce and validate review_workflows.
...
1. Render context, container, module, data, communication, trust, and deployment views.
2. Link diagram elements to requirements and ADRs.
3. Show alternatives, tradeoffs, risks, experiments, and unresolved decisions.
4. Support scoped comments and approval.
5. Generate authorization, accessibility, and stale-data tests.
...
- `DIAGRAM_MODEL_DRIFT` — stop unsafe continuation and emit a structured diagnostic.
- `ADR_LINK_MISSING` — stop unsafe continuation and emit a structured diagnostic.
- `RISK_HIDDEN` — stop unsafe continuation and emit a structured diagnostic.
- `UNAUTHORIZED_APPROVAL` — stop unsafe continuation and emit a structured diagnostic.
...
- Reject or escalate `diagram_model_drift` without inventing success.
- Reject or escalate `adr_link_missing` without inventing success.
- Reject or escalate `risk_hidden` without inventing success.
- Reject or escalate `unauthorized_approval` without inventing success.
...
- Architecture views derive from authoritative models.
- Critical risks and open decisions remain visible.
- Approvals bind to the exact baseline.
