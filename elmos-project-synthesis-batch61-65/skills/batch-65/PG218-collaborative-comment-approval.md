---
id: PG218
name: collaborative-comment-approval
version: 1.0.0
batch: 65
engine: elmos.project-synthesis
schema: elmos.project-synthesis.v1
status: proposed
summary: Provides reusable comments, mentions, assignments, decisions, approvals, exceptions, and notification workflows.
depends_on: ["PG179", "PG213", "PG214", "PG215"]
capabilities:
  - workspace:read
...
# PG218 — Collaborative Comment Approval

## 1. Objective

Create consistent collaboration semantics across all product workspaces.
...
- Validate and consume workspace_objects.
- Validate and consume identity_directory.
- Validate and consume approval_matrix.
- Validate and consume notification_policy.
- Produce and validate collaboration_model.
- Produce and validate comment_threads.
- Produce and validate approval_actions.
- Produce and validate notification_events.
...
1. Attach threaded comments and decisions to immutable or versioned objects.
2. Support mentions, assignments, due dates, and resolution.
3. Support scoped approval, rejection, exception, revocation, and expiry.
4. Generate notifications without leaking inaccessible content.
5. Maintain append-only decision history.
...
- `COMMENT_ORPHANED` — stop unsafe continuation and emit a structured diagnostic.
- `NOTIFICATION_INFORMATION_LEAK` — stop unsafe continuation and emit a structured diagnostic.
- `APPROVAL_SCOPE_WRONG` — stop unsafe continuation and emit a structured diagnostic.
- `DECISION_HISTORY_MUTATED` — stop unsafe continuation and emit a structured diagnostic.
...
- Reject or escalate `comment_orphaned` without inventing success.
- Reject or escalate `notification_information_leak` without inventing success.
- Reject or escalate `approval_scope_wrong` without inventing success.
- Reject or escalate `decision_history_mutated` without inventing success.
...
- Comments retain object and version context.
- Notifications reveal no unauthorized content.
- Decision history is immutable and auditable.
