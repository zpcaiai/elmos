---
id: PG216
name: build-run-console-ui
version: 1.0.0
batch: 65
engine: elmos.project-synthesis
schema: elmos.project-synthesis.v1
status: proposed
summary: Provides live and historical build, test, startup, smoke, repair, and environment evidence views.
depends_on: ["PG147", "PG158", "PG181"]
capabilities:
  - workspace:read
...
# PG216 — Build Run Console Ui

## 1. Objective

Make execution state and failure diagnosis transparent without exposing secrets.
...
- Validate and consume run_events.
- Validate and consume execution_evidence.
- Validate and consume diagnostics.
- Validate and consume repair_history.
- Validate and consume environment_matrix.
- Produce and validate run_console_views.
- Produce and validate log_stream_contract.
- Produce and validate diagnostic_actions.
...
1. Render stage, environment, duration, status, and evidence.
2. Stream structured redacted logs and trace links.
3. Show failure classification, root-cause hypotheses, and repair attempts.
4. Support pause, cancel, retry, approve, and download-diagnostic actions according to policy.
5. Generate authorization, redaction, reconnect, and long-run tests.
...
- `SECRET_IN_CONSOLE` — stop unsafe continuation and emit a structured diagnostic.
- `STAGE_STATUS_MISMATCH` — stop unsafe continuation and emit a structured diagnostic.
- `DUPLICATE_RETRY_ACTION` — stop unsafe continuation and emit a structured diagnostic.
- `REPAIR_HISTORY_HIDDEN` — stop unsafe continuation and emit a structured diagnostic.
...
- Reject or escalate `secret_in_console` without inventing success.
- Reject or escalate `stage_status_mismatch` without inventing success.
- Reject or escalate `duplicate_retry_action` without inventing success.
- Reject or escalate `repair_history_hidden` without inventing success.
...
- Console output is redacted and source-linked.
- Actions respect run state and capabilities.
- Repair attempts and remaining limits are visible.
