---
id: PG220
name: generation-usage-metering
version: 1.0.0
batch: 65
engine: elmos.project-synthesis
schema: elmos.project-synthesis.v1
status: proposed
summary: Meters model, tool, runner, storage, network, artifact, and support usage by tenant, project, run, and feature.
depends_on: ["PG186", "PG219"]
capabilities:
  - workspace:read
...
# PG220 — Generation Usage Metering

## 1. Objective

Support cost control, chargeback, billing, capacity planning, and product analytics.
...
- Validate and consume provider_usage.
- Validate and consume runner_metrics.
- Validate and consume storage_metrics.
- Validate and consume pricing_catalog.
- Produce and validate usage_ledger.
- Produce and validate cost_allocation.
- Produce and validate usage_summaries.
- Produce and validate anomaly_findings.
...
1. Normalize usage records from models, tools, runners, storage, and network.
2. Deduplicate retried or replayed provider events.
3. Allocate cost to tenant, workspace, project, run, Skill, and feature.
4. Apply pricing and included-usage rules with versioning.
5. Detect anomalous or missing metering.
...
- `DOUBLE_METERING` — stop unsafe continuation and emit a structured diagnostic.
- `USAGE_EVENT_LOSS` — stop unsafe continuation and emit a structured diagnostic.
- `PRICING_VERSION_ERROR` — stop unsafe continuation and emit a structured diagnostic.
- `CROSS_TENANT_ALLOCATION` — stop unsafe continuation and emit a structured diagnostic.
...
- Reject or escalate `double_metering` without inventing success.
- Reject or escalate `usage_event_loss` without inventing success.
- Reject or escalate `pricing_version_error` without inventing success.
- Reject or escalate `cross_tenant_allocation` without inventing success.
...
- Usage ledger is append-only and reconcilable.
- Retries do not double-charge.
- Reported costs identify pricing version and coverage.
