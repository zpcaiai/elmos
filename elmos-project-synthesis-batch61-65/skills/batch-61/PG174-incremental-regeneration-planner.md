---
id: PG174
name: incremental-regeneration-planner
version: 1.0.0
batch: 61
engine: elmos.project-synthesis
schema: elmos.project-synthesis.v1
status: proposed
summary: Creates the smallest safe regeneration plan for an approved change request.
depends_on: ["PG032", "PG042", "PG073", "PG173"]
capabilities:
  - workspace:read
...
# PG174 — Incremental Regeneration Planner

## 1. Objective

Limit generated changes, validation cost, and review scope while preserving correctness.
...
- Validate and consume approved_change_baseline.
- Validate and consume requirements_impact.
- Validate and consume artifact_graph.
- Validate and consume ownership_manifest.
- Validate and consume current_generation_manifest.
- Produce and validate regeneration_plan.
- Produce and validate affected_units.
- Produce and validate validation_scope.
- Produce and validate approval_gates.
...
1. Compute transitive impact from changed specification nodes.
2. Partition affected artifacts into regenerate, merge, preserve, revalidate, and untouched sets.
3. Minimize change scope without breaking dependency closure.
4. Select required language, integration, test, build, and deployment skills.
5. Publish ordered generation units and approval gates.
...
- `UNDER_SCOPED_PLAN` — stop unsafe continuation and emit a structured diagnostic.
- `OVERBROAD_REGENERATION` — stop unsafe continuation and emit a structured diagnostic.
- `DEPENDENCY_CLOSURE_GAP` — stop unsafe continuation and emit a structured diagnostic.
- `APPROVAL_GATE_MISSING` — stop unsafe continuation and emit a structured diagnostic.
...
- Reject or escalate `under_scoped_plan` without inventing success.
- Reject or escalate `overbroad_regeneration` without inventing success.
- Reject or escalate `dependency_closure_gap` without inventing success.
- Reject or escalate `approval_gate_missing` without inventing success.
...
- Every affected artifact has a planned disposition.
- Untouched areas are provably outside the impact closure.
- Breaking changes trigger required approval and compatibility work.
