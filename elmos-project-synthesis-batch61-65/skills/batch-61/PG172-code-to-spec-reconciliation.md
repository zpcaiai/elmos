---
id: PG172
name: code-to-spec-reconciliation
version: 1.0.0
batch: 61
engine: elmos.project-synthesis
schema: elmos.project-synthesis.v1
status: proposed
summary: Proposes controlled specification updates when valid repository behavior is not represented upstream.
depends_on: ["PG071", "PG171"]
capabilities:
  - workspace:read
...
# PG172 — Code To Spec Reconciliation

## 1. Objective

Prevent valuable user-authored behavior from being erased or left undocumented.
...
- Validate and consume drift_findings.
- Validate and consume repository_behavior_model.
- Validate and consume current_specifications.
- Validate and consume ownership_manifest.
- Produce and validate reconciliation_proposals.
- Produce and validate spec_patch_candidates.
...
1. Filter drift to behavior plausibly intended and owned by users.
2. Infer candidate requirement, rule, API, data, workflow, and architecture updates with explicit confidence.
3. Separate factual observation from proposed intent.
4. Generate reviewable upstream patches and impact analysis.
5. Apply only approved reconciliation changes to new baselines.
...
- `INVENTED_INTENT` — stop unsafe continuation and emit a structured diagnostic.
- `OWNERSHIP_CONFLICT` — stop unsafe continuation and emit a structured diagnostic.
- `UNSAFE_AUTO_ACCEPT` — stop unsafe continuation and emit a structured diagnostic.
- `PROPOSAL_SCOPE_LEAK` — stop unsafe continuation and emit a structured diagnostic.
...
- Reject or escalate `invented_intent` without inventing success.
- Reject or escalate `ownership_conflict` without inventing success.
- Reject or escalate `unsafe_auto_accept` without inventing success.
- Reject or escalate `proposal_scope_leak` without inventing success.
...
- No inferred intent is auto-approved.
- Approved user behavior receives new upstream identifiers and trace links.
- Rejected reconciliation leaves user-owned code preserved.
