...
status: proposed
summary: Detects repository structures and runtime topology that violate the approved architecture baseline.
depends_on: ["PG045", "PG047", "PG050", "PG053", "PG171"]
capabilities:
  - workspace:read
...
# PG176 — Architecture Drift Detector

## 1. Objective

Keep generated and evolved projects aligned with module, dependency, data, trust, and deployment decisions.
...
- Validate and consume deployment_snapshot.
- Validate and consume dependency_graph.
- Validate and consume telemetry_topology.
- Produce and validate architecture_drift_findings.
- Produce and validate forbidden_edges.
- Produce and validate topology_delta.
- Produce and validate remediation_options.
...
1. Compare code and project dependency graphs with allowed module edges.
2. Compare data ownership and writes with the approved source-of-truth plan.
3. Compare trust boundaries, service topology, and runtime communication.
4. Distinguish deliberate approved evolution from accidental drift.
5. Generate remediation or architecture-change proposals.
...
- `DEPENDENCY_DRIFT` — stop unsafe continuation and emit a structured diagnostic.
- `DATA_OWNER_VIOLATION` — stop unsafe continuation and emit a structured diagnostic.
- `TRUST_BOUNDARY_BYPASS` — stop unsafe continuation and emit a structured diagnostic.
- `RUNTIME_TOPOLOGY_MISMATCH` — stop unsafe continuation and emit a structured diagnostic.
...
- Reject or escalate `dependency_drift` without inventing success.
- Reject or escalate `data_owner_violation` without inventing success.
- Reject or escalate `trust_boundary_bypass` without inventing success.
- Reject or escalate `runtime_topology_mismatch` without inventing success.
...
- Forbidden dependencies are detected with symbol-level evidence.
- Unauthorized cross-owner writes are blocking findings.
- Approved architecture changes supersede rather than mutate history.
