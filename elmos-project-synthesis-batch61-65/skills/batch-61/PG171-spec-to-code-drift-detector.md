...
status: proposed
summary: Detects divergence between approved specifications and current repository behavior.
depends_on: ["PG032", "PG042", "PG064", "PG076", "PG158"]
capabilities:
  - workspace:read
...
# PG171 — Spec To Code Drift Detector

## 1. Objective

Identify missing, obsolete, or contradictory implementation before change planning.
...
- Validate and consume active_requirements_baseline.
- Validate and consume architecture_baseline.
- Validate and consume project_blueprint.
- Validate and consume repository_snapshot.
- Validate and consume generation_manifest.
- Produce and validate drift_findings.
- Produce and validate coverage_delta.
- Produce and validate severity_map.
- Produce and validate reconciliation_candidates.
...
1. Load immutable baselines and the current repository snapshot.
2. Map files, symbols, routes, schemas, migrations, tests, and deployment assets to Artifact Graph nodes.
3. Detect missing implementations, undocumented behavior, stale generated artifacts, and policy drift.
4. Classify expected user extensions separately from unauthorized divergence.
5. Publish evidence-backed findings and bounded reconciliation candidates.
...
- `FALSE_DRIFT` — stop unsafe continuation and emit a structured diagnostic.
- `MISSED_BEHAVIOR` — stop unsafe continuation and emit a structured diagnostic.
- `LINEAGE_GAP` — stop unsafe continuation and emit a structured diagnostic.
- `USER_EXTENSION_MISCLASSIFIED` — stop unsafe continuation and emit a structured diagnostic.
...
- Reject or escalate `false_drift` without inventing success.
- Reject or escalate `missed_behavior` without inventing success.
- Reject or escalate `lineage_gap` without inventing success.
- Reject or escalate `user_extension_misclassified` without inventing success.
...
- Every critical finding cites both expected and observed evidence.
- User-owned extensions are not labeled as drift without contract evidence.
- Unresolved critical drift blocks incremental regeneration.
