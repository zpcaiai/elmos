---
id: PG201
name: failure-recovery-chaos-eval
version: 1.0.0
batch: 63
engine: elmos.project-synthesis
schema: elmos.project-synthesis.v1
status: proposed
summary: Evaluates recovery from model, tool, runner, network, registry, storage, broker, and approval interruptions.
depends_on: ["PG149", "PG150", "PG181", "PG191"]
capabilities:
  - workspace:read
...
# PG201 — Failure Recovery Chaos Eval

## 1. Objective

Certify durable workflow behavior under realistic failures.
...
- Validate and consume benchmark_runs.
- Validate and consume fault_profiles.
- Validate and consume orchestrator.
- Validate and consume sandbox_and_runners.
- Produce and validate recovery_score.
- Produce and validate duplicate_side_effect_findings.
- Produce and validate checkpoint_evidence.
- Produce and validate operator_actions.
...
- `recovery_score`
- `duplicate_side_effect_findings`
- `checkpoint_evidence`
- `operator_actions`
...
1. Inject failures at defined workflow checkpoints.
2. Test retry, compensation, pause, resume, cancellation, and approval expiry.
3. Detect duplicate writes, commits, migrations, deployments, and notifications.
4. Verify terminal diagnostic quality.
5. Publish recovery time and residual manual actions.
...
- `DUPLICATE_SIDE_EFFECT` — stop unsafe continuation and emit a structured diagnostic.
- `CHECKPOINT_CORRUPTION` — stop unsafe continuation and emit a structured diagnostic.
- `UNSAFE_COMPENSATION` — stop unsafe continuation and emit a structured diagnostic.
- `SILENT_PARTIAL_SUCCESS` — stop unsafe continuation and emit a structured diagnostic.
...
- Reject or escalate `duplicate_side_effect` without inventing success.
- Reject or escalate `checkpoint_corruption` without inventing success.
- Reject or escalate `unsafe_compensation` without inventing success.
- Reject or escalate `silent_partial_success` without inventing success.
...
- Failures do not produce false delivered state.
- Completed side effects are not duplicated after resume.
- Operator-required actions are explicit and bounded.
