---
id: PG197
name: traceability-completeness-eval
version: 1.0.0
batch: 63
engine: elmos.project-synthesis
schema: elmos.project-synthesis.v1
status: proposed
summary: Evaluates end-to-end lineage from source requirements through code, tests, evidence, releases, and change history.
depends_on: ["PG042", "PG076", "PG158", "PG169"]
capabilities:
  - workspace:read
...
# PG197 — Traceability Completeness Eval

## 1. Objective

Certify that delivery claims can be audited and impact-analyzed.
...
- Validate and consume test_evidence.
- Validate and consume release_manifest.
- Produce and validate traceability_score.
- Produce and validate orphan_findings.
- Produce and validate broken_path_findings.
- Produce and validate auditability_report.
...
1. Validate required node types and edge semantics.
2. Trace each must-have requirement to implementation, verification, evidence, and delivery.
3. Trace generated artifacts back to approved sources and tools.
4. Detect orphan, stale, contradictory, or circular lineage.
5. Publish completeness by release slice.
...
- `ORPHAN_REQUIREMENT` — stop unsafe continuation and emit a structured diagnostic.
- `ORPHAN_ARTIFACT` — stop unsafe continuation and emit a structured diagnostic.
- `EVIDENCE_PATH_BROKEN` — stop unsafe continuation and emit a structured diagnostic.
- `STALE_TRACE_LINK` — stop unsafe continuation and emit a structured diagnostic.
...
- Reject or escalate `orphan_artifact` without inventing success.
- Reject or escalate `evidence_path_broken` without inventing success.
- Reject or escalate `stale_trace_link` without inventing success.
...
- Every must-have requirement has a complete delivery path.
- Every managed artifact has approved provenance.
- Broken critical paths fail certification.
