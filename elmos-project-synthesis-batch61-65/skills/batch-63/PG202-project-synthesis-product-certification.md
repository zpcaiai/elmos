---
id: PG202
name: project-synthesis-product-certification
version: 1.0.0
batch: 63
engine: elmos.project-synthesis
schema: elmos.project-synthesis.v1
status: proposed
summary: Combines benchmark and policy evidence into a versioned product certification decision.
depends_on: ["PG192", "PG193", "PG194", "PG195", "PG196", "PG197", "PG198", "PG199", "PG200", "PG201"]
capabilities:
  - workspace:read
...
# PG202 — Project Synthesis Product Certification

## 1. Objective

Declare exactly which profiles, environments, risks, and limits the release supports.
...
- Validate and consume all_eval_results.
- Validate and consume certification_policy.
- Validate and consume known_issues.
- Validate and consume support_matrix.
- Produce and validate certification_decision.
- Produce and validate certified_scope.
- Produce and validate limitations.
- Produce and validate signed_certificate.
...
1. Validate completeness and independence of evaluation evidence.
2. Apply mandatory thresholds and non-compensable critical failures.
3. Define certified languages, profiles, domain packs, environments, and scale limits.
4. List exclusions, known issues, and expiry conditions.
5. Sign and publish the release certificate.
...
- `EVIDENCE_INCOMPLETE` — stop unsafe continuation and emit a structured diagnostic.
- `CRITICAL_FAILURE_AVERAGED_OUT` — stop unsafe continuation and emit a structured diagnostic.
- `SCOPE_OVERCLAIM` — stop unsafe continuation and emit a structured diagnostic.
- `CERTIFICATE_STALE` — stop unsafe continuation and emit a structured diagnostic.
...
- Reject or escalate `evidence_incomplete` without inventing success.
- Reject or escalate `critical_failure_averaged_out` without inventing success.
- Reject or escalate `scope_overclaim` without inventing success.
- Reject or escalate `certificate_stale` without inventing success.
...
- Critical failure cannot be offset by aggregate score.
- Certified scope is explicit and no broader than evidence.
- Certification expires when material dependencies or policies change.
