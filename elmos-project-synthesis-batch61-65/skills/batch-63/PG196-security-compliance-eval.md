---
id: PG196
name: security-compliance-eval
version: 1.0.0
batch: 63
engine: elmos.project-synthesis
schema: elmos.project-synthesis.v1
status: proposed
summary: Evaluates threat controls, authorization, tenant isolation, supply chain, secrets, data handling, and compliance evidence.
depends_on: ["PG050", "PG145", "PG162", "PG191"]
capabilities:
  - workspace:read
...
# PG196 — Security Compliance Eval

## 1. Objective

Certify security claims with adversarial and policy-based validation.
...
- Validate and consume generated_system.
- Validate and consume threat_model.
- Validate and consume security_tests.
- Validate and consume delivery_artifacts.
- Validate and consume compliance_profile.
- Produce and validate security_eval_score.
- Produce and validate critical_findings.
- Produce and validate control_coverage.
- Produce and validate compliance_evidence.
...
1. Map threats and policy obligations to implemented controls and tests.
2. Run authorization, tenant isolation, injection, secret, dependency, image, and configuration checks.
3. Evaluate evidence, exceptions, owners, and expiry.
4. Test fail-closed behavior under missing configuration.
5. Publish residual risk and certification decision.
...
- `CRITICAL_VULNERABILITY` — stop unsafe continuation and emit a structured diagnostic.
- `CROSS_TENANT_ACCESS` — stop unsafe continuation and emit a structured diagnostic.
- `SECRET_EXPOSURE` — stop unsafe continuation and emit a structured diagnostic.
- `CONTROL_EVIDENCE_MISSING` — stop unsafe continuation and emit a structured diagnostic.
...
- Reject or escalate `cross_tenant_access` without inventing success.
- Reject or escalate `secret_exposure` without inventing success.
- Reject or escalate `control_evidence_missing` without inventing success.
