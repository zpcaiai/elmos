---
id: PG195
name: build-run-success-eval
version: 1.0.0
batch: 63
engine: elmos.project-synthesis
schema: elmos.project-synthesis.v1
status: proposed
summary: Evaluates clean setup, build, migration, startup, health, smoke, shutdown, and evidence reproducibility.
depends_on: ["PG147", "PG156", "PG158", "PG191"]
capabilities:
  - workspace:read
...
# PG195 — Build Run Success Eval

## 1. Objective

Certify that generated projects actually run from a clean environment.
...
- Validate and consume execution_recipe.
- Validate and consume certification_environment.
- Produce and validate runtime_eval_score.
- Produce and validate execution_evidence.
- Produce and validate reproducibility_report.
- Produce and validate failure_findings.
...
1. Provision an isolated clean environment.
2. Execute documented install, build, migration, test, startup, health, smoke, and shutdown steps.
3. Repeat configured runs to detect hidden local state.
4. Validate evidence completeness and fingerprints.
5. Publish pass/fail by certified environment.
...
- `CLEAN_BUILD_FAILURE` — stop unsafe continuation and emit a structured diagnostic.
- `STARTUP_FAILURE` — stop unsafe continuation and emit a structured diagnostic.
- `SMOKE_FAILURE` — stop unsafe continuation and emit a structured diagnostic.
- `NONREPRODUCIBLE_RUN` — stop unsafe continuation and emit a structured diagnostic.
...
- Reject or escalate `clean_build_failure` without inventing success.
- Reject or escalate `startup_failure` without inventing success.
- Reject or escalate `smoke_failure` without inventing success.
- Reject or escalate `nonreproducible_run` without inventing success.
...
- No local undeclared dependency is required.
- Required execution stages succeed in certified environments.
- Results are reproducible from the recorded recipe.
