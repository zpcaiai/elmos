---
id: PG191
name: golden-project-benchmark-suite
version: 1.0.0
batch: 63
engine: elmos.project-synthesis
schema: elmos.project-synthesis.v1
status: proposed
summary: Defines representative benchmark projects across languages, architectures, and risk levels.
depends_on: ["PG077", "PG089", "PG101", "PG124", "PG190"]
capabilities:
  - workspace:read
...
# PG191 — Golden Project Benchmark Suite

## 1. Objective

Provide stable end-to-end workloads for Project Synthesis certification.
...
- Validate and consume supported_profiles.
- Validate and consume domain_pack_catalog.
- Validate and consume risk_model.
- Validate and consume certification_policy.
- Produce and validate benchmark_catalog.
- Produce and validate golden_inputs.
- Produce and validate expected_invariants.
- Produce and validate benchmark_versions.
...
1. Select Java, Python, .NET, worker, full-stack, event, multi-tenant, and AI-backed scenarios.
2. Create approved requirements, architecture, Blueprint, expected behavior, and negative cases.
3. Include clean generation and incremental change scenarios.
4. Version fixtures and external dependencies.
5. Publish benchmark scoring and reproducibility rules.
...
- `BENCHMARK_TOO_EASY` — stop unsafe continuation and emit a structured diagnostic.
- `EXPECTED_OUTPUT_OVERFIT` — stop unsafe continuation and emit a structured diagnostic.
- `FIXTURE_DRIFT` — stop unsafe continuation and emit a structured diagnostic.
- `MISSING_RISK_COVERAGE` — stop unsafe continuation and emit a structured diagnostic.
...
- Reject or escalate `benchmark_too_easy` without inventing success.
- Reject or escalate `expected_output_overfit` without inventing success.
- Reject or escalate `fixture_drift` without inventing success.
- Reject or escalate `missing_risk_coverage` without inventing success.
...
- Benchmarks cover all certified Golden Paths.
- Expected outcomes focus on behavior and invariants rather than exact source text.
