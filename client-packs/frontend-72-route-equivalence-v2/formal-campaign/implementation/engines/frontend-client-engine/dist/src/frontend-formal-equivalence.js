...
import { accessSync, constants as fsConstants, existsSync, lstatSync, mkdirSync, readFileSync, readdirSync, realpathSync, writeFileSync, } from "node:fs";
import { basename, dirname, join, relative, resolve, sep } from "node:path";
import ts from "typescript";
import { navigationSourceSpec, } from "./bounded-navigation-source.js";
...
import { renderTargetProject } from "./project-templates.js";
...
const lockedZ3BinaryDigests = new Set([
...
const lockedZ3IdentityProbeAttempts = 3;
const lockedZ3VersionByIdentity = new Map();
...
export function frontendFormalDigest(value) {
    ...
    if (ts.isArrayLiteralExpression(expression)) {
        const items = expression.elements.map((element, index) => {
            if (ts.isSpreadElement(element))
                throw new Error("spread is outside bounded navigation grammar");
    ...
    if (ts.isObjectLiteralExpression(expression)) {
        ...
        for (const member of expression.properties) {
            if (!ts.isPropertyAssignment(member))
                throw new Error("non-property member is outside bounded navigation grammar");
            const key = propertyName(member.name, source);
            if (Object.hasOwn(result, key))
                throw new Error(`duplicate bounded navigation property: ${key}`);
...
function validateBoundedModel(value) {
    if (!value || typeof value !== "object" || Array.isArray(value))
        throw new Error("bounded navigation model must be an object");
    const model = value;
    const exactRoot = ["fallback", "navigation", "profile", "projectTitle", "render", "routes", "schemaVersion"];
    if (Object.keys(model).sort().join("|") !== exactRoot.join("|"))
        throw new Error("bounded navigation root shape drifted");
    if (model.schemaVersion !== "1.0" || model.profile !== "bounded-navigation-v1")
        throw new Error("bounded navigation identity drifted");
    ...
    if (!navigation || Object.keys(navigation).join("|") !== "label" || navigation.label !== "主要导航")
        throw new Error("navigation contract drifted");
    if (!render || Object.keys(render).sort().join("|") !== "headingLevel|mainRole" || render.mainRole !== "main" || render.headingLevel !== 1)
        throw new Error("render contract drifted");
    if (!fallback || Object.keys(fallback).join("|") !== "strategy" || fallback.strategy !== "FIRST_DECLARED_ROUTE")
        throw new Error("fallback contract drifted");
    if (!Array.isArray(model.routes) || model.routes.length === 0)
        throw new Error("bounded navigation routes are required");
    const ids = new Set();
    const paths = new Set();
    for (const [index, raw] of model.routes.entries()) {
        ...
        if (Object.keys(route).sort().join("|") !== fields.join("|"))
            throw new Error(`route ${index} shape drifted`);
        for (const field of ["id", "path", "title", "text"])
            assertString(route[field], `route ${index}.${field}`);
        if (typeof route.requiresAuth !== "boolean" || typeof route.deepLink !== "boolean")
            throw new Error(`route ${index} flags drifted`);
        if (ids.has(route.id) || paths.has(route.path))
            throw new Error("bounded navigation route identity is duplicated");
        ids.add(route.id);
        paths.add(route.path);
    }
    return value;
}
function tsRelift(profile, sourcePath, text) {
    ...
    if (parseDiagnostics.length > 0)
        throw new Error(`navigation source parse failed for ${profile}`);
    let initializer;
    source.forEachChild(node => {
        if (!ts.isVariableStatement(node))
            return;
        for (const declaration of node.declarationList.declarations) {
            if (ts.isIdentifier(declaration.name) && declaration.name.text === "ELMOS_BOUNDED_NAVIGATION")
                initializer = declaration.initializer;
        }
    });
    if (!initializer)
        throw new Error("ELMOS_BOUNDED_NAVIGATION declaration is missing");
...
function resolvePointer(value, pointer) {
    ...
    if (!pointer.startsWith("/"))
        throw new Error(`invalid RFC6901 pointer: ${pointer}`);
    let current = value;
    for (const raw of pointer.slice(1).split("/")) {
        const key = raw.replaceAll("~1", "/").replaceAll("~0", "~");
        if (Array.isArray(current)) {
            if (!/^(?:0|[1-9][0-9]*)$/.test(key))
                throw new Error(`invalid array pointer: ${pointer}`);
...
function astFacts(source, root = source) {
    ...
    const visit = (node) => {
        ...
        if (ts.isPropertyAccessExpression(node))
            properties.add(node.getText(source));
        if (ts.isElementAccessExpression(node))
            elements.add(node.getText(source));
        if (ts.isCallExpression(node))
            calls.add(node.expression.getText(source));
        if (ts.isStringLiteral(node) || ts.isNoSubstitutionTemplateLiteral(node))
            strings.push(node.text);
        if (ts.isJsxAttribute(node))
            jsxAttributes.add(node.name.getText(source));
        if (ts.isJsxOpeningElement(node) || ts.isJsxSelfClosingElement(node))
            jsxTags.add(node.tagName.getText(source));
        node.forEachChild(visit);
    };
    visit(root);
    return { imports, identifiers, properties, elements, calls, strings, jsx_attributes: jsxAttributes, jsx_tags: jsxTags };
}
function requireImport(facts, moduleName, names) {
    const imported = facts.imports.get(moduleName);
    if (!imported || names.some(name => !imported.has(name)))
        throw new Error(`consumer AST import binding drifted: ${moduleName}:${names.join(",")}`);
}
function requireFacts(facts, required) {
    for (const [name, actual, expected] of [
        ...
        for (const item of expected)
            if (!actual.has(item))
                throw new Error(`consumer AST ${name} binding drifted: ${item}`);
    }
    for (const value of required.string_contains ?? []) {
        if (!facts.strings.some(item => item.includes(value)))
            throw new Error(`consumer AST template/string binding drifted: ${value}`);
    }
}
function requireTopLevelExpression(source, prefix, required = {}) {
    const statement = source.statements.find(candidate => ts.isExpressionStatement(candidate) && candidate.expression.getText(source).startsWith(prefix));
    if (!statement || !ts.isExpressionStatement(statement))
        throw new Error(`reachable top-level entry expression is missing: ${source.fileName}:${prefix}`);
    requireFacts(astFacts(source, statement.expression), required);
}
function requireDirectRoutesIdentity(source, variableName, expectedText) {
...
function namedFunction(source, name) {
    ...
    if (!declaration || !ts.isFunctionDeclaration(declaration) || !declaration.body)
        throw new Error(`reachable consumer function is missing: ${name}`);
    return declaration;
}
function requireRouteAlias(files, path, moduleName) {
    const text = files[path];
    if (text === undefined)
        throw new Error(`route alias source is missing: ${path}`);
...
function scriptBlock(path, text) {
    const matches = [...text.matchAll(/<script(?:\s[^>]*)?>([\s\S]*?)<\/script>/g)];
    if (matches.length !== 1 || matches[0]?.[1] === undefined)
        throw new Error(`bounded component requires exactly one script block: ${path}`);
...
function requireCodeTokens(files, path, tokens) {
    const text = files[path];
    if (text === undefined)
        throw new Error(`consumer source is missing: ${path}`);
    const executable = codeWithoutComments(text);
    for (const token of tokens)
        if (!executable.includes(token))
            throw new Error(`bounded consumer grammar drifted in ${path}: ${token}`);
}
function requireEntryReachability(profile, files) {
    const parse = (path) => {
        const text = files[path];
        if (text === undefined)
            throw new Error(`entry source is missing: ${path}`);
...
export function boundedNavigationGeneratedConsumerPaths(profile) {
    return generatedConsumerPaths[profile];
}
function generatedProjectName(profile, files) {
    const manifest = files["package.json"];
    if (manifest === undefined)
        return `formal-${profile.replaceAll("-", "")}`;
    let parsed;
    try {
...
function expectedConsumerContext(profile, model, files) {
    const sourceProfile = uiTargetProfiles().find(candidate => candidate.id !== profile);
    if (!sourceProfile)
        throw new Error("source profile for consumer grammar is unavailable");
...
export function expectedBoundedNavigationConsumerFiles(profile, model, files) {
    return renderTargetProject(expectedConsumerContext(profile, model, files));
}
function validateGeneratedConsumerGrammar(profile, model, files) {
    ...
    for (const path of generatedConsumerPaths[profile]) {
        if (files[path] === undefined)
            throw new Error(`reachable generated consumer is missing: ${profile}:${path}`);
...
function rejectDuplicateRouteTables(files, contractPath) {
            ...
            const walk = (node) => {
                if (ts.isObjectLiteralExpression(node)) {
                    ...
                    for (const member of node.properties) {
                        ...
                        let key;
                        try {
                            key = propertyName(member.name, source);
                        }
                        catch {
                            continue;
                        }
                        const value = unwrapExpression(member.initializer);
                        if (ts.isStringLiteral(value) || value.kind === ts.SyntaxKind.TrueKeyword || value.kind === ts.SyntaxKind.FalseKeyword)
                            literalFields.add(key);
                    }
                    if ([...routeKeys].every(key => literalFields.has(key)))
                        throw new Error(`duplicate literal route table is forbidden: ${path}`);
                }
                node.forEachChild(walk);
            };
            walk(source);
        }
        if (path.endsWith(".dart") && /GeneratedRoute\s*\(\s*["']/.test(text))
            throw new Error(`duplicate Dart route table is forbidden: ${path}`);
    }
}
function validateConsumerBinding(profile, files, contractPath) {
    ...
    switch (profile) {
        ...
        case "jquery": {
            ...
            if (!called)
                throw new Error("jQuery render consumer is not reachable from the entry");
            break;
        }
        case "svelte": {
            requireRouteAlias(files, "src/routes.ts", "./elmos-bounded-navigation");
...
function validateContractConsumer(profile, source) {
    ...
    if (profile === "flutter") {
        ...
        for (const token of tokens)
            if (!executable.includes(token))
                throw new Error(`bounded selector/observer dataflow drifted: ${token}`);
        return;
    }
...
export function reliftBoundedNavigationProject(profile, files) {
    const spec = navigationSourceSpec(profile);
    const text = files[spec.sourcePath];
    if (text === undefined)
        throw new Error(`navigation source is missing: ${spec.sourcePath}`);
...
export function canonicalBoundedNavigationModel(request) {
    ...
    return {
        ...
        routes: valid.uiIr.routes.map(route => {
            ...
            if (!component)
                throw new Error(`canonical route component is missing: ${route.componentId}`);
            return {
                id: route.id,
                path: route.path,
...
export function observeBoundedNavigationModel(model, interpreter) {
    const first = model.routes[0];
    if (!first)
        throw new Error("behavior domain requires a first route");
    const observation = (traceId, operation, inputPath, selected, resolution) => ({
...
function behaviorComparable(observations) {
    return observations.map(({ trace_id: _traceId, ...rest }) => rest);
}
function independentObserve(model) {
    const inputs = [
        { operation: "INITIAL_RENDER", path: null },
        ...model.routes.map(route => ({ operation: "SELECT_DECLARED_PATH", path: route.path })),
        { operation: "SELECT_UNKNOWN_PATH", path: "/__elmos_unknown_route__" },
    ];
    const first = model.routes.at(0);
    if (!first)
        throw new Error("reference behavior requires a route");
...
function flatten(value, pointer = "", output = {}) {
    if (Array.isArray(value)) {
        ...
        for (const key of keys)
            flatten(value[key], `${pointer}/${pointerEscape(key)}`, output);
    }
    else
...
export function buildFrontendSmt2(canonicalModel, sourceModel, targetModel, canonicalBehavior, independentBehavior, sourceBehavior, targetBehavior, formalInputDigest = "UNBOUND_FORMAL_INPUT") {
    const datasets = [
    ...
    const disequalities = [];
    const semanticModels = [canonicalModel, sourceModel, targetModel, canonicalModel];
    const prefixes = ["canonical", "source", "target", "reference"];
    const literal = (value, sort) => {
        if (sort === "String")
    ...
    const selector = (model, field, sort) => {
        const first = model.routes[0];
    ...
    for (const [name, field, sort] of symbolicFields) {
        ...
        disequalities.push(`(not (= (canonical_${name} event input_path) (source_${name} event input_path)))`, `(not (= (canonical_${name} event input_path) (target_${name} event input_path)))`, `(not (= (canonical_${name} event input_path) (reference_${name} event input_path)))`);
    }
    const renderFields = [
    ...
    for (const [name, sort, get] of renderFields) {
        for (const [index, prefix] of prefixes.entries())
            lines.push(`(define-fun ${prefix}_${name} () ${sort} ${literal(get(semanticModels[index]), sort)})`);
        disequalities.push(`(not (= canonical_${name} source_${name}))`, `(not (= canonical_${name} target_${name}))`, `(not (= canonical_${name} reference_${name}))`);
    }
...
export function runFrontendSolver(smt2, options = {}) {
    const command = options.command ?? process.env.ELMOS_FRONTEND_Z3 ?? "z3";
    const timeout = options.timeout_ms ?? 10_000;
    const rejected = (outcome, reason, realpath = null, binaryDigest = null, version = "UNKNOWN") => ({
        schema_version: "1.0",
        solver: command,
    ...
    if (basename(binaryPath) !== "z3")
        return rejected("ERROR", "solver executable identity is not Z3", binaryPath, binaryDigest);
    if (!lockedZ3BinaryDigests.has(binaryDigest))
        return rejected("ERROR", "solver binary digest is not the locked Z3 4.16.0 artifact", binaryPath, binaryDigest);
    ...
    if (solverVersion === undefined) {
        ...
        for (let attempt = 0; attempt < lockedZ3IdentityProbeAttempts; attempt += 1) {
            ...
            if (solverVersion === lockedZ3Version) {
                lockedZ3VersionByIdentity.set(identityKey, solverVersion);
                break;
            }
        }
    }
...
function makeProfileBundles(options) {
    ...
    for (const profile of uiTargetProfiles()) {
        ...
        if (options.tamper?.profile_id === profile.id) {
            ...
            if (current === undefined || !current.includes(options.tamper.find))
                throw new Error("requested tamper target was not found");
...
function routeChunks(source, target) {
    ...
    if (pointers.join("|") !== collectPointers(target.model).sort().join("|"))
        throw new Error("semantic pointer sets diverged");
...
export function materializeFrontendFormalCampaign(outputDirectory, options = {}) {
    const output = resolve(outputDirectory);
    if (existsSync(output) && readdirSync(output).length > 0)
        throw new Error("frontend formal output directory must be absent or empty");
    ...
    for (const route of uiConversionRoutes()) {
        ...
        const status = !semanticEqual || !behaviorEqual || !chunkEqual
            ? "REFUTED"
            : solver.proof_status;
        const solverResult = {
            ...solver,
    ...
    const visit = (directory) => {
        for (const name of readdirSync(directory).sort()) {
            ...
            if (metadata.isSymbolicLink())
                throw new Error(`symbolic links are forbidden in formal artifacts: ${path}`);
            if (metadata.isDirectory())
                visit(path);
            else if (metadata.isFile())
                output.push(relative(root, path).split(sep).join("/"));
            else
                throw new Error(`non-regular formal artifact is forbidden: ${path}`);
        }
    };
    visit(root);
    return output;
...
function safeCampaignFile(root, relativePath, name) {
    ...
    if (!actual.startsWith(`${rootReal}${sep}`))
        throw new Error(`${name} resolves outside campaign root`);
    return actual;
}
function exactObjectKeys(value, expected, name) {
    ...
    if (actual.join("|") !== wanted.join("|"))
        throw new Error(`${name} keys drifted`);
}
export function verifyFrontendFormalCampaign(outputDirectory) {
    const output = resolve(outputDirectory);
    const errors = [];
    try {
        ...
        if (campaign.schema_version !== "1.0" || campaign.kind !== "frontend-formal-route-campaign"
            ...
            || canonical(campaign.assumptions) !== canonical(proofAssumptions))
            errors.push("campaign identity or proof boundary drifted");
        const profiles = campaign.profiles;
        const verifiedProfiles = new Map();
        const seenProfileIds = new Set();
        if (!Array.isArray(profiles) || profiles.length !== 9)
            errors.push("campaign must bind nine profiles");
        else
            for (const raw of profiles) {
                ...
                try {
                    ...
                    if (!uiTargetProfiles().some(candidate => candidate.id === id) || seenProfileIds.has(id))
                        throw new Error("profile identity is unknown or duplicated");
                    ...
                    if (frontendFormalDigest(base) !== manifestDigest || manifestDigest !== profile.manifest_digest
                        ...
                        || manifest.digest_scope !== "sorted UTF-8 project files keyed by POSIX relative path")
                        throw new Error("manifest identity or digest drifted");
                    ...
                    if (computedProjectDigest !== profile.project_digest || computedProjectDigest !== manifest.project_digest)
                        throw new Error("project digest drifted");
                    ...
                    if (manifest.file_count !== expectedFileRows.length || canonical(manifest.files) !== canonical(expectedFileRows))
                        throw new Error("manifest file inventory drifted");
                    const relift = reliftBoundedNavigationProject(id, diskFiles);
                    if (relift.model_digest !== profile.relift_model_digest)
                        throw new Error("profile relift model digest drifted");
        ...
        if (seenProfileIds.size !== exactIds.size || [...exactIds].some(id => !seenProfileIds.has(id)))
            errors.push("campaign profile closure is incomplete");
        const routes = campaign.routes;
        const seenRouteIds = new Set();
        const seenPairs = new Set();
        const solverReplayCache = new Map();
        if (!Array.isArray(routes) || routes.length !== 72)
            errors.push("campaign must bind 72 directed routes");
        else
            for (const raw of routes) {
                ...
                try {
                    ...
                    if (routeId !== pair || sourceId === targetId || seenRouteIds.has(routeId) || seenPairs.has(pair))
                    ...
                    if (!source || !target || route.source_project_digest !== source.project_digest || route.target_project_digest !== target.project_digest)
                        throw new Error("route project binding drifted");
                    ...
                    if (route.evidence_path !== `${routeRoot}/layered-result.json` || route.formal_input_path !== `${routeRoot}/formal-input.json`
                        || route.solver_result_path !== `${routeRoot}/solver-result.json`)
                        throw new Error("route artifact path is non-canonical");
                    ...
                    if (bytesDigest(formalBytes) !== route.formal_input_digest || artifactBytes(formal) !== formalBytes)
                        throw new Error("formal input bytes or digest drifted");
                    ...
                    if (artifactBytes(layered) !== layeredBytes || layered.status !== route.status || route.layered_result !== route.status
                        ...
                        || canonical(layered.assumptions) !== canonical(proofAssumptions))
                        throw new Error("layered result drifted");
                    const links = layered.links;
                    if (!links || typeof links !== "object")
                        throw new Error("layered links are missing");
                    ...
                    for (const [key, value] of Object.entries(expectedPaths))
                        if (links[key] !== value)
                            throw new Error(`${key} is non-canonical`);
                    if (links.formal_input_digest !== route.formal_input_digest)
                        throw new Error("layered formal digest drifted");
                    const readLinkedJson = (pathKey, digestKey) => {
                        const path = safeCampaignFile(output, links[pathKey], `${routeId}.${pathKey}`);
                        const bytes = readFileSync(path, "utf8");
                        const value = JSON.parse(bytes);
                        if (artifactBytes(value) !== bytes || bytesDigest(bytes) !== links[digestKey])
                            throw new Error(`${digestKey} bytes drifted`);
                        return { bytes, value };
                    };
                    ...
                    if (canonical(sourceModelArtifact.value) !== canonical(source.relift) || canonical(targetModelArtifact.value) !== canonical(target.relift))
                        throw new Error("source/target re-lift artifact drifted");
                    ...
                    if (formal.schema_version !== "1.0" || formal.kind !== "frontend-bounded-navigation-formal-input"
                        ...
                        || canonical(formal.assumptions) !== canonical(proofAssumptions))
                        throw new Error("formal semantic/model binding drifted");
                    const tuple = formal.tuple;
                    if (!tuple || tuple.source_profile !== sourceId || tuple.target_profile !== targetId
                        ...
                        || tuple.target_framework_version !== uiTargetProfile(targetId).frameworkVersion)
                        throw new Error("formal exact tuple drifted");
                    const canonicalBehavior = observeBoundedNavigationModel(canonicalModel, "canonical");
                    const independentBehavior = independentObserve(canonicalModel);
                    const sourceBehavior = observeBoundedNavigationModel(source.relift.model, "source");
                    const targetBehavior = observeBoundedNavigationModel(target.relift.model, "target");
                    ...
                    if (canonical(behaviorArtifact.value) !== canonical(expectedBehavior) || formal.behavior_digest !== links.behavior_digest
                        || formal.behavior_equal !== behaviorEqual)
                        throw new Error("behavior evidence drifted");
                    ...
                    if (canonical(chunkArtifact.value) !== canonical(expectedChunks) || formal.chunk_digest !== links.chunks_digest
                        || formal.chunk_equal !== chunkEqual)
                        throw new Error("chunk mapping/span/hash evidence drifted");
                    ...
                    if (smt2 !== expectedSmt2 || bytesDigest(smt2) !== links.smt2_digest)
                        throw new Error("SMT2 bytes or symbolic encoding drifted");
                    ...
                    const expectedStatus = !semanticEqual || !behaviorEqual || !chunkEqual ? "REFUTED" : proofStatus;
                    if (route.status !== expectedStatus || solver.schema_version !== "1.0" || solver.route_id !== routeId
                        ...
                        || solver.smt2_digest !== links.smt2_digest)
                        throw new Error("solver result/status linkage drifted");
                    if (solver.identity_status === "VERIFIED") {
                        ...
                        if (solver.solver !== binaryPath || basename(binaryPath) !== "z3" || realpathSync(binaryPath) !== binaryPath
                            ...
                            || solver.stderr !== "" || !["unsat\n", "sat\n", "unknown\n"].includes(String(solver.stdout)))
                            throw new Error("locked solver identity/execution drifted");
                        const options = solver.options;
                        if (!options || canonical(options.args) !== canonical(["-in"]) || typeof options.timeout_ms !== "number")
                            throw new Error("solver options drifted");
                        const environment = solver.environment;
                        if (!environment || environment.platform !== process.platform || environment.arch !== process.arch || environment.node_version !== process.version)
                            throw new Error("solver environment drifted");
                        ...
                        const replay = solverReplayCache.get(replayKey)
                            ?? runFrontendSolver(smt2, { command: binaryPath, timeout_ms: options.timeout_ms });
                        ...
                        if (replay.identity_status !== "VERIFIED" || replay.outcome !== solverOutcome || replay.stdout !== solver.stdout
                            || replay.exit_code !== solver.exit_code || replay.solver_binary_sha256 !== solver.solver_binary_sha256)
                            throw new Error("solver replay diverged");
                    }
                    else if (route.status !== "NOT_PROVED" || solver.stdout !== "" || solver.exit_code !== null)
                        throw new Error("unverified solver identity cannot support a proof result");
                    ...
                    if (canonical(compositionArtifact.value) !== canonical(expectedComposition))
                        throw new Error("composition evidence drifted");
                    const layers = layered.layers;
                    if (layers)
                        exactObjectKeys(layers, ["emitted_source_relift", "emitted_target_relift", "semantic", "chunk", "behavior", "smt_solver", "framework_native_build", "framework_native_runtime", "independent_external_verification"], `${routeId}.layers`);
                    if (!layers || layers.emitted_source_relift !== "PASSED" || layers.emitted_target_relift !== "PASSED"
                        ...
                        || layers.independent_external_verification !== "NOT_RUN")
                        throw new Error("layered status evidence drifted");
                }
                catch (error) {
        ...
        if (seenRouteIds.size !== expectedPairs.size || [...expectedPairs].some(routeId => !seenRouteIds.has(routeId)))
        ...
        if (canonical(campaign.source_liftings) !== canonical(expectedSourceLiftings) || canonical(campaign.target_lowerings) !== canonical(expectedTargetLowerings))
            errors.push("campaign lifting/lowering closure drifted");
        if (Array.isArray(routes)) {
            ...
            if (canonical(campaign.counts) !== canonical(expectedCounts))
                errors.push("campaign proof counts drifted");
        }
    }
    catch (error) {
...
//# sourceMappingURL=frontend-formal-equivalence.js.map