...
export function navigationSourceSpec(profile) {
    switch (profile) {
        case "vue2":
...
export function attachBoundedNavigationSource(profile, model, files) {
    if (model.routes.length === 0)
        throw new Error("bounded navigation requires at least one route");
    const spec = navigationSourceSpec(profile);
    if (files[spec.entryPath] === undefined)
        throw new Error(`bounded navigation entry is missing: ${spec.entryPath}`);
...
//# sourceMappingURL=bounded-navigation-source.js.map