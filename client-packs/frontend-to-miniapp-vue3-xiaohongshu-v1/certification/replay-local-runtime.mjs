...
const sourceRootRelative = "client-packs/frontend-to-miniapp-vue3-xiaohongshu-v1/source-snapshots/vue3-todo-v1.0.1";
...
const implementationEntries = [
  ...
  {
    path: "client-packs/frontend-to-miniapp-vue3-xiaohongshu-v1/certification/replay-local-runtime.mjs",
...
assert.equal(targetProfile.framework, "xiaohongshu-native-miniapp-candidate", "target framework drift");
...
const evidenceControlPaths = [
  "client-packs/frontend-to-miniapp-vue3-xiaohongshu-v1/pack.json",
  "client-packs/frontend-to-miniapp-vue3-xiaohongshu-v1/route-matrix.json",
  "client-packs/frontend-to-miniapp-vue3-xiaohongshu-v1/source-snapshots/manifest.json",
  "client-packs/frontend-to-miniapp-vue3-xiaohongshu-v1/target-profile/profile.json",
  "client-packs/frontend-to-miniapp-vue3-xiaohongshu-v1/acceptance/acceptance-profile.json",
  "client-packs/frontend-to-miniapp-vue3-xiaohongshu-v1/ui-ir/model.json",
  "client-packs/frontend-to-miniapp-vue3-xiaohongshu-v1/transformations/vue3-todo-to-xiaohongshu-native.json",
  "client-packs/frontend-to-miniapp-vue3-xiaohongshu-v1/certification/source-build-evidence.json",
  "client-packs/frontend-to-miniapp-vue3-xiaohongshu-v1/certification/external-evidence-status.json",
  "client-packs/frontend-to-miniapp-vue3-xiaohongshu-v1/certification/certification.json",
...
const transformationEntry = evidenceControlEntries.find(entry => entry.path.endsWith("/transformations/vue3-todo-to-xiaohongshu-native.json"));
...
assert.equal(reviewUiIr.pack_key, "frontend-to-miniapp-vue3-xiaohongshu-v1", "review UI IR pack drift");
...
const expectedTransformationContracts = new Map([
  ...
  ["component.app-shell", "pages/index/index.xhsml contains the source-traced application shell"],
  ["component.todo-input", "pages/index/index.xhsml input value binds text and bindinput targets handleInput0"],
  ["component.add-button", "pages/index/index.xhsml button disabled binds !canSubmit0 and bindtap targets handleSubmit0"],
  ["component.todo-list", "pages/index/index.xhsml iterates itemsRender with __elmosKey and pages/index/index.js derives each key from item plus source index"],
  ["effect.add-todo", "pages/index/index.js trims text appends one value persists application items and clears text"],
  ["token.app-shell", "pages/index/index.css scopes .app-shell with the deterministic scope class and preserves max-width auto margins and padding"],
...
const request = {
  schemaVersion: "1.0",
  requestId: "conv-vue3-todo-xiaohongshu",
...
const templateEntry = one(Object.entries(project.files).filter(([path]) => path.endsWith(".xhsml")), "generated XHSML");
const styleEntry = one(Object.entries(project.files).filter(([path]) => path.endsWith(".css") && path.startsWith("pages/")), "generated page CSS");
...
const stylePlan = one(run.plan.styles.filter(item => item.platform === "xiaohongshu"), "Xiaohongshu style plan");
const appShellRule = one(stylePlan.rules.filter(rule => rule.styleId === appShellStyle.id), "app shell style rule");
assert.deepEqual(appManifest.pages, ["pages/index/index"], "route.home must be the only emitted Xiaohongshu page");
...
assert.ok(styleEntry[1].includes(exactAppShellRule), "generated CSS must contain the exact scoped app shell rule");
assert.ok(templateLines.has(`<view class="elmos-node app-shell ${appShellStyle.scopeClass}" data-source-node="${appShell.id}" role="main">`), "generated XHSML app shell opening tag drift");
assert.ok(templateLines.has(`<input class="elmos-control" data-source-node="${todoInput.id}" aria-label="Todo text" value="{{text}}" bindinput="handleInput0" />`), "generated XHSML todo input contract drift");
assert.ok(templateLines.has(`<button class="elmos-control" data-source-node="${addButton.id}" disabled="{{!canSubmit0}}" bindtap="handleSubmit0">Add</button>`), "generated XHSML add button contract drift");
assert.ok(templateLines.has('<block xhs:for="{{itemsRender}}" xhs:for-item="item" xhs:key="__elmosKey">'), "generated XHSML todo iteration contract drift");
assert.ok(templateLines.has(`<view class="elmos-list-item" data-source-node="${todoList.id}" role="listitem"><text>{{item.value}}</text></view>`), "generated XHSML todo item contract drift");
...
const observed = {
  schema_version: 1,
  evidence_key: "vue3-todo-xiaohongshu-local-runtime-2026-08-20",
  ...
  official_xiaohongshu_build_preview_device_upload_review_release: "NOT_RUN",
  replay: "pnpm --dir engines/frontend-client-engine run build && node client-packs/frontend-to-miniapp-vue3-xiaohongshu-v1/certification/replay-local-runtime.mjs --check",
};
