...
try { var precisionIcons = ["route", "layers", "code", "workflow", "database", "test", "shield", "spark"]; } catch(e) {}
try { var precisionMaturityCounts = precisionMigrationSummary.maturityCounts; } catch(e) {}
try { var precisionExecutableCount = ["ADAPTER_DECLARED", "ADAPTER_CONTRACT_PASSED", "LOCAL_EXECUTED", "HOLDOUT_PASSED", "EXTERNAL_VERIFIED", "CERTIFIED"]
    .reduce((total, maturity) => total + (precisionMaturityCounts[maturity] ?? 0), 0); } catch(e) {}
try { var precisionRanges = precisionMigrationPhases.map((phase, index) => ({
