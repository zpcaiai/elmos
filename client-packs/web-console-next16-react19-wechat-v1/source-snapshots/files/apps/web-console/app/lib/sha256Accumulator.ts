...
/** Small incremental SHA-256 used to verify bounded downloads without a second full ArrayBuffer. */
export class Sha256Accumulator {
  ...
  update(bytes: Uint8Array): this {
    if (this.finalized) throw new Error("SHA256_ALREADY_FINALIZED");
    this.totalBytes += bytes.byteLength;
    if (!Number.isSafeInteger(this.totalBytes)) throw new Error("SHA256_INPUT_TOO_LARGE");
    let offset = 0;
    while (offset < bytes.byteLength) {
      const count = Math.min(64 - this.buffered, bytes.byteLength - offset);
      this.buffer.set(bytes.subarray(offset, offset + count), this.buffered);
      this.buffered += count;
      offset += count;
      if (this.buffered === 64) {
  ...
  private compress(chunk: Uint8Array): void {
    ...
    for (let index = 16; index < 64; index += 1) {
      const prior = this.words[index - 15];
      const beforePrior = this.words[index - 2];
      const sigma0 = rotateRight(prior, 7) ^ rotateRight(prior, 18) ^ (prior >>> 3);
      const sigma1 = rotateRight(beforePrior, 17) ^ rotateRight(beforePrior, 19) ^ (beforePrior >>> 10);
    ...
    for (let index = 0; index < 64; index += 1) {
      ...
      const first = (h + sigma1 + choice + ROUND_CONSTANTS[index] + this.words[index]) >>> 0;
