...
const capabilities = {
  ...
  routes: [
    ...
    },
    {
      ...
      targetJava: "21",
      recipeId: "io.elmos.openrewrite.SpringBoot1_5ToBoot3_5_16Java21",
      evidenceStatus: "NOT_RUN",
      verifiedSourceSpringBoot: "",
      verifiedSourceJava: "",
      notes: "Executable preparation recipe is available; exact runtime and external evidence remain NOT_RUN.",
    },
    {
...
test("Spring 真实旅程 UI 可完成导入、证据查看、下载、启动、健康检查与停止", async ({
  ...
  await expect(
    catalog.getByRole("row").filter({ hasText: "Boot 3.5.16" }).getByRole("cell", { name: "NOT_RUN · Spring Boot" }),
  ).toHaveCount(1);
  await expect(page.getByText("NOT_RUN 路线需要 Runner 显式开启实验执行", { exact: false })).toBeVisible();
  await expect(
