...
- Every generated route now includes an exact runnable target contract and can be published through the create-only FRT materializer, which attaches and validates a portable Batch 46 smoke pack before atomically exposing the output. Real local smoke runs passed 6/6 required checks and the sole Batch 46 runnable gate for React, Vue 2, Vue 3, Flutter web and WeChat DevTools. The ArkUI smoke path executes its bounded hvigor/hdc adapter but remains `NOT_RUN` on this host.
...
- 29 local protocol tests exercise typed parameter rejection, accepted and adversarial signed records for all nine consolidated external checks, every explicit qualification adapter, recomputed observation tamper detection, privacy-minimized iOS physical-device recording, and the repository gate, including the required runnable-route check. Three additional materializer tests execute a generated route and its sole runnable gate. These fixtures validate code paths only and are never copied into external evidence.
