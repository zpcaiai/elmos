...
-- V44 also imported a specification-only content projection at this name.
-- Preserve that append-only relation and its rows before the production runtime
-- claims artifact.content_objects for content-addressed storage.  An unknown
-- shape is a hard stop: silently renaming it would hide schema drift or data
-- loss at the migration boundary.
DO $$
DECLARE
    has_imported_shape boolean;
BEGIN
    IF to_regclass('artifact.content_objects') IS NOT NULL THEN
        ...
        IF has_imported_shape THEN
            ...
            COMMENT ON TABLE artifact.specification_imported_content_objects IS
                'Append-only V44 specification projection retained during the V77 production runtime expansion.';
        ELSE
            RAISE EXCEPTION
