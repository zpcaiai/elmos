...
from elmos_openhands.errors import ContractViolation, IdempotencyConflict, LeaseLost, TenantIsolationError
...
from elmos_openhands.sandbox import InMemorySecretBroker, KubernetesSandboxBackend, MountSpec, NetworkPolicy, ProductionSandboxProvider, SandboxExecRequest, SandboxExecResult, SandboxQuotas, SandboxSpec, SandboxStats
from elmos_openhands.workspace import IsolationClass, LocalWorkspaceProvider, WorkspaceRequest
from elmos_openhands.workspace_api import FileRange, GitOperation, LocalWorkspaceApi, PatchOperation, PatchSet, WorkspaceMutationStore
...
class FakePostgresCursor:
    ...
    def execute(self, query, params=()):
        ...
        elif normalized.startswith("SELECT * FROM oh_execution_events"):
            self.current = None
        elif normalized.startswith("SELECT seq,digest FROM oh_execution_events"):
            self.current = None
        else:
            self.current = None
