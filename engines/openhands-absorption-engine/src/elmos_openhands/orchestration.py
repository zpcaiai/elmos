...
class DagPlan:
    ...
    @classmethod
    def create(cls, identity: Identity, version: int, nodes: Iterable[ChildAgentContract], reason: str) -> "DagPlan":
        values = tuple(nodes)
        ...
        if len(graph) != len(values) or any(dependency not in graph for deps in graph.values() for dependency in deps) or _cycle(graph):
            raise ContractViolation("DAG plan has duplicate, absent or cyclic dependencies")
        body = {"identity": identity.scope(), "version": version, "nodes": [_contract_dict(node) for node in values], "reason": reason}
...
class TemporalDagOrchestrator:
    ...
    async def start(self, plan: DagPlan, *, manifest_digest: str, idempotency_key: str) -> TemporalStartResult:
        ...
        payload = {
            "schema_version": "1.0", "identity": _identity_dict(plan.identity), "plan": _plan_dict(plan),
            "manifest_digest": manifest_digest, "idempotency_key": idempotency_key,
        }
        try:
            handle = await self.client.start_workflow(
                "ElmosTaskWorkflow", payload, id=workflow_id, task_queue=self.task_queue,
                id_reuse_policy="REJECT_DUPLICATE", execution_timeout=timedelta(days=7),
                run_timeout=timedelta(days=1), task_timeout=timedelta(seconds=10),
                memo={"tenant_id": plan.identity.tenant_id, "project_id": plan.identity.project_id, "plan_digest": plan.digest},
                search_attributes={"TenantId": [plan.identity.tenant_id], "ProjectId": [plan.identity.project_id]},
            )
            ...
            if type(error).__name__ not in {"WorkflowAlreadyStartedError", "WorkflowExecutionAlreadyStartedError"}:
                raise
            ...
            if not isinstance(status, Mapping) or any(status.get(key) != value for key, value in expected.items()):
                raise ContractViolation("Temporal workflow identity collision or conflicting duplicate start") from error
            handle = existing
        temporal_run_id = str(getattr(handle, "first_execution_run_id", None) or getattr(handle, "run_id", "unknown"))
        return TemporalStartResult(workflow_id, temporal_run_id, plan.digest)

    async def amend(self, workflow_id: str, plan: DagPlan, *, expected_version: int, actor: str, idempotency_key: str) -> None:
        ...
        result = await handle.execute_update("amend_plan", {"expected_version": expected_version, "plan": _plan_dict(plan), "actor": actor, "idempotency_key": idempotency_key}, id=idempotency_key)
        if not isinstance(result, Mapping) or result.get("status") != "applied" or int(result.get("version", -1)) != plan.version:
    ...
    async def cancel(self, workflow_id: str, *, actor: str, reason: str, idempotency_key: str) -> None:
        ...
        handle = self.client.get_workflow_handle(workflow_id)
        await handle.signal("request_cancel", {"actor": actor, "reason": reason, "idempotency_key": idempotency_key})
...
def build_temporal_workflow_definitions() -> tuple[type[Any], type[Any]]:
    ...
    retry = RetryPolicy(initial_interval=timedelta(seconds=1), maximum_interval=timedelta(seconds=30), maximum_attempts=5, non_retryable_error_types=["ContractViolation", "PolicyDenied"])
    ...
    class ElmosChildAgentWorkflow:
        ...
        async def run(self, payload: Mapping[str, Any]) -> Mapping[str, Any]:
            ...
            value = await workflow.execute_activity(
                "RunChildAgentActivity", payload, start_to_close_timeout=timedelta(hours=2),
                heartbeat_timeout=timedelta(seconds=30), retry_policy=retry,
            )
            if not isinstance(value, Mapping):
                raise RuntimeError("child activity returned a non-object")
            return dict(value)
    ...
    class ElmosTaskWorkflow:
        ...
        async def amend_plan(self, payload: Mapping[str, Any]) -> Mapping[str, Any]:
            ...
            for node_id in self.running_nodes | self.completed_nodes:
                if node_id not in incoming_nodes or digest_of(incoming_nodes[node_id]) != digest_of(current_nodes[node_id]):
        ...
        def runtime_status(self) -> Mapping[str, Any]:
            return {"phase": self.phase, "plan_version": self.plan_version, "cancel_requested": self.cancel_requested, "running_nodes": sorted(self.running_nodes), "completed_nodes": sorted(self.completed_nodes), "identity": self.start_identity, "plan_digest": self.start_plan_digest, "manifest_digest": self.start_manifest_digest, "start_idempotency_key": self.start_idempotency_key}
        ...
        async def run(self, payload: Mapping[str, Any]) -> Mapping[str, Any]:
            ...
            if resume_state is None:
                ...
                workspace = await workflow.execute_activity("BootstrapWorkspaceActivity", payload, start_to_close_timeout=timedelta(minutes=15), heartbeat_timeout=timedelta(seconds=30), retry_policy=retry)
                await workflow.execute_activity("BuildRepoIntelligenceActivity", {**dict(payload), "workspace": workspace}, start_to_close_timeout=timedelta(minutes=30), heartbeat_timeout=timedelta(seconds=30), retry_policy=retry)
                completed: dict[str, Mapping[str, Any]] = {}
            else:
                if not isinstance(resume_state, Mapping) or "workspace" not in resume_state or not isinstance(resume_state.get("completed"), Mapping):
                ...
                for node_id, result in dict(resume_state["completed"]).items():
                    if not isinstance(result, Mapping):
                        raise RuntimeError("Temporal continuation contains an invalid child result")
            ...
            while not self.cancel_requested:
                if materialized_version != self.plan_version:
                    ...
                    pending = {node_id: node for node_id, node in current_nodes.items() if node_id not in self.completed_nodes}
                    materialized_version = self.plan_version
                if not pending:
                    break
                ready = [node for node in pending.values() if all(dep in completed for dep in node.get("dependencies", []))]
                if not ready:
                ...
                try:
                    results = await asyncio.gather(*[
                        workflow.execute_child_workflow(
                            ElmosChildAgentWorkflow.run, {"parent": {**dict(payload), "plan": self.plan}, "node": node, "workspace": workspace},
                            id=f"{workflow.info().workflow_id}:{node['node_id']}:v{self.plan_version}",
                            task_queue=workflow.info().task_queue, cancellation_type=workflow.ChildWorkflowCancellationType.WAIT_CANCELLATION_COMPLETED,
                        ) for node in ready
                    ])
                except Exception as error:
                    self.running_nodes.clear()
                    self.phase = "compensating"
                    compensation = await workflow.execute_activity("CompensateTaskActivity", {"payload": payload, "completed": completed, "failure_type": type(error).__name__}, start_to_close_timeout=timedelta(minutes=30), retry_policy=retry)
                    return {"status": "failed", "completed": completed, "failure_type": type(error).__name__, "compensation": compensation}
                self.running_nodes.clear()
                for node, result in zip(ready, results):
                    if result.get("status") != "succeeded":
                        self.phase = "compensating"
                        compensation = await workflow.execute_activity("CompensateTaskActivity", {"payload": payload, "completed": completed, "failed_node": node["node_id"]}, start_to_close_timeout=timedelta(minutes=30), retry_policy=retry)
                        return {"status": "failed", "completed": completed, "failed_node": node["node_id"], "compensation": compensation}
                ...
                if pending and len(completed) - continuation_start >= 100:
                    workflow.continue_as_new({**dict(payload), "plan": self.plan, "resume_state": {"workspace": workspace, "completed": completed}})
            if self.cancel_requested:
                self.phase = "cancelled"
                compensation = await workflow.execute_activity("CompensateTaskActivity", {"payload": payload, "completed": completed, "cancel": self.cancel_reason}, start_to_close_timeout=timedelta(minutes=30), retry_policy=retry)
                return {"status": "cancelled", "completed": completed, "compensation": compensation}
            try:
                self.phase = "integration"
                integrated = await workflow.execute_activity("IntegrateResultsActivity", {"payload": payload, "completed": completed}, start_to_close_timeout=timedelta(hours=1), heartbeat_timeout=timedelta(seconds=30), retry_policy=retry)
                self.phase = "verification"
                verified = await workflow.execute_activity("VerifyTaskActivity", {"payload": payload, "integrated": integrated}, start_to_close_timeout=timedelta(hours=2), heartbeat_timeout=timedelta(seconds=30), retry_policy=retry)
                self.phase = "finalizing"
                result = await workflow.execute_activity("FinalizeEvidenceActivity", {"payload": payload, "integrated": integrated, "verified": verified}, start_to_close_timeout=timedelta(minutes=30), retry_policy=retry)
            except Exception as error:
                self.phase = "compensating"
                compensation = await workflow.execute_activity("CompensateTaskActivity", {"payload": payload, "completed": completed, "failure_type": type(error).__name__}, start_to_close_timeout=timedelta(minutes=30), retry_policy=retry)
                return {"status": "failed", "completed": completed, "failure_type": type(error).__name__, "compensation": compensation}
            ...
            if not isinstance(result, Mapping):
                raise RuntimeError("evidence finalization returned a non-object")
            return dict(result)
...
class DagControlStore:
    ...
    def save_plan(self, plan: DagPlan, *, actor: str) -> None:
        ...
        with self._transaction():
            ...
            if existing is not None:
                if (existing["digest"], existing["body"], existing["actor"], existing["reason"]) == (plan.digest, encoded, actor, plan.reason):
                    return
                raise ContractViolation("DAG plan version is immutable")
            previous = self._connection.execute("SELECT MAX(version) FROM dag_plans WHERE tenant_id=? AND project_id=? AND task_id=? AND run_id=? AND scope_node_id=?", plan.identity.scope()).fetchone()[0]
            ...
            self._connection.execute("INSERT INTO dag_plans VALUES(?,?,?,?,?,?,?,?,?,?)", (*plan.identity.scope(), plan.version, plan.digest, encoded, actor, plan.reason))
            for node in plan.nodes:
                self._connection.execute("INSERT INTO dag_budget(tenant_id,project_id,task_id,run_id,scope_node_id,node_id,reserved_micros) VALUES(?,?,?,?,?,?,?) ON CONFLICT(tenant_id,run_id,scope_node_id,node_id) DO UPDATE SET reserved_micros=excluded.reserved_micros", (*plan.identity.scope(), node.node_id, node.budget_micros))

    def consume_budget(self, identity: Identity, node_id: str, cost_micros: int, *, global_limit_micros: int, idempotency_key: str) -> None:
        if cost_micros < 0 or global_limit_micros < 0 or not idempotency_key or len(idempotency_key.encode("utf-8")) > 256:
        ...
        with self._transaction():
            ...
            if prior is not None:
                if (prior["node_id"], int(prior["cost_micros"]), int(prior["global_limit_micros"])) != (node_id, cost_micros, global_limit_micros):
                    raise ContractViolation("DAG budget idempotency key was reused with different content")
                return
            row = self._connection.execute("SELECT reserved_micros,used_micros FROM dag_budget WHERE tenant_id=? AND project_id=? AND task_id=? AND run_id=? AND scope_node_id=? AND node_id=?", (*identity.scope(), node_id)).fetchone()
            total = int(self._connection.execute("SELECT COALESCE(SUM(used_micros),0) FROM dag_budget WHERE tenant_id=? AND project_id=? AND task_id=? AND run_id=? AND scope_node_id=?", identity.scope()).fetchone()[0])
            if row is None or int(row["used_micros"]) + cost_micros > int(row["reserved_micros"]) or total + cost_micros > global_limit_micros:
                raise BudgetExceeded("DAG node or global budget exceeded")
            self._connection.execute("UPDATE dag_budget SET used_micros=used_micros+? WHERE tenant_id=? AND project_id=? AND task_id=? AND run_id=? AND scope_node_id=? AND node_id=?", (cost_micros, *identity.scope(), node_id))
            self._connection.execute("INSERT INTO dag_budget_mutations VALUES(?,?,?,?,?,?,?,?,?)", (*identity.scope(), idempotency_key, node_id, cost_micros, global_limit_micros))
    ...
    def lock_resource(self, identity: Identity, resource: str, owner_node: str) -> None:
        ...
        with self._lock:
            self._assert_run_scope(identity)
            row = self._connection.execute("SELECT owner_node FROM workspace_locks WHERE tenant_id=? AND project_id=? AND task_id=? AND run_id=? AND scope_node_id=? AND resource=?", (*identity.scope(), resource)).fetchone()
            if row is not None and row["owner_node"] != owner_node:
                raise ContractViolation("workspace resource is locked by another node")
            if row is not None:
                return
            self._connection.execute("INSERT INTO workspace_locks(tenant_id,project_id,task_id,run_id,scope_node_id,resource,owner_node) VALUES(?,?,?,?,?,?,?) ON CONFLICT(tenant_id,run_id,scope_node_id,resource) DO UPDATE SET version=version+1", (*identity.scope(), resource, owner_node))

    def unlock_resource(self, identity: Identity, resource: str, owner_node: str) -> None:
        with self._lock:
            self._assert_run_scope(identity)
            self._connection.execute("DELETE FROM workspace_locks WHERE tenant_id=? AND project_id=? AND task_id=? AND run_id=? AND scope_node_id=? AND resource=? AND owner_node=?", (*identity.scope(), resource, owner_node))

    def register_compensation(self, identity: Identity, node_id: str, operation: str, *, idempotency_key: str) -> int:
        ...
        with self._transaction():
            ...
            sequence = int(self._connection.execute("SELECT COALESCE(MAX(sequence),-1)+1 FROM dag_compensations WHERE tenant_id=? AND project_id=? AND task_id=? AND run_id=? AND scope_node_id=? AND node_id=?", (*identity.scope(), node_id)).fetchone()[0])
            self._connection.execute("INSERT INTO dag_compensations(tenant_id,project_id,task_id,run_id,scope_node_id,node_id,sequence,operation,idempotency_key,state,receipt) VALUES(?,?,?,?,?,?,?,?,?,'pending',NULL)", (*identity.scope(), node_id, sequence, operation, idempotency_key))
            return sequence

    def compensate(self, identity: Identity, callbacks: Mapping[str, Callable[[str, str], str]]) -> tuple[str, ...]:
        self._assert_run_scope(identity)
        rows = self._connection.execute("SELECT * FROM dag_compensations WHERE tenant_id=? AND project_id=? AND task_id=? AND run_id=? AND scope_node_id=? AND state IN ('pending','executing','unknown') ORDER BY sequence DESC,node_id DESC", identity.scope()).fetchall()
        receipts: list[str] = []
        for row in rows:
            callback = callbacks.get(row["operation"])
            if callback is None:
                raise NotConfigured("compensation callback is not registered: " + row["operation"])
            self._connection.execute("UPDATE dag_compensations SET state='executing' WHERE tenant_id=? AND project_id=? AND task_id=? AND run_id=? AND scope_node_id=? AND node_id=? AND sequence=? AND state IN ('pending','executing','unknown')", (*identity.scope(), row["node_id"], row["sequence"]))
            try:
                receipt = callback(row["node_id"], row["idempotency_key"])
            except Exception:
                self._connection.execute("UPDATE dag_compensations SET state='unknown' WHERE tenant_id=? AND project_id=? AND task_id=? AND run_id=? AND scope_node_id=? AND node_id=? AND sequence=? AND state='executing'", (*identity.scope(), row["node_id"], row["sequence"]))
                raise
            if not receipt:
                self._connection.execute("UPDATE dag_compensations SET state='unknown' WHERE tenant_id=? AND project_id=? AND task_id=? AND run_id=? AND scope_node_id=? AND node_id=? AND sequence=? AND state='executing'", (*identity.scope(), row["node_id"], row["sequence"]))
                raise ContractViolation("compensation callback returned no receipt")
            self._connection.execute("UPDATE dag_compensations SET state='completed',receipt=? WHERE tenant_id=? AND project_id=? AND task_id=? AND run_id=? AND scope_node_id=? AND node_id=? AND sequence=? AND state='executing'", (receipt, *identity.scope(), row["node_id"], row["sequence"]))
...
class LocalDagExecutor:
    ...
    def run(self, identity: Identity, worker: Callable[[AgentNode], str], *, owner: str = "local-dag") -> tuple[AgentNode, ...]:
        ...
        while True:
            ...
            for node in batch:
                ...
                try:
                    ...
                    raise
                completed.append(self.dag.complete(identity, node.node_id, owner, claimed.fencing_token or "", result_ref))
...
class RepositoryIntegrator(Protocol):
    ...
    def apply_resolution(self, integration_branch: str, resolution: Mapping[str, Any]) -> Mapping[str, Any]: ...
...
class SemanticMergeCoordinator:
    ...
    def integrate(
        ...
        *,
        ...
        resolve: Callable[[tuple[str, ...], tuple[str, ...], tuple[MergeInput, ...]], Mapping[str, Any]] | None,
        ...
        try:
            if (file_conflicts or symbol_conflicts) and resolve is None:
                ...
                return MergeResult("blocked", branch, None, file_conflicts, symbol_conflicts, (), digest_of({"branch": branch, "files": file_conflicts, "symbols": symbol_conflicts, "status": "blocked"}))
            ...
            self.repository.rollback(branch, base_revision)
            raise
        body = {"branch": branch, "revision": revision, "files": file_conflicts, "symbols": symbol_conflicts, "receipts": receipts, "status": "succeeded"}
        return MergeResult("succeeded", branch, revision, file_conflicts, symbol_conflicts, tuple(receipts), digest_of(body))


class ProviderFallbackPolicy:
    def choose(self, contract: ChildAgentContract, *, attempted: Iterable[str], available: Mapping[str, Mapping[str, Any]]) -> str:
...
def _contract_dict(node: ChildAgentContract) -> dict[str, Any]:
    return {**asdict(node), "workspace_strategy": node.workspace_strategy.value, "input_refs": list(node.input_refs), "dependencies": list(node.dependencies), "provider_preferences": list(node.provider_preferences), "output_contract": dict(node.output_contract)}


def _plan_dict(plan: DagPlan) -> dict[str, Any]:
    return {"identity": _identity_dict(plan.identity), "version": plan.version, "nodes": [_contract_dict(node) for node in plan.nodes], "reason": plan.reason, "digest": plan.digest}
...
def _validate_temporal_plan_payload(value: Any) -> dict[str, Any]:
    ...
    if set(identity) != identity_fields or any(not isinstance(identity.get(name), str) or not identity.get(name) for name in identity_fields - {"agent_id"}):
    ...
    if version < 1 or not isinstance(reason, str) or not reason.strip() or not isinstance(declared_digest, str):
        raise ContractViolation("Temporal DAG metadata is invalid")
    node_fields = {"node_id", "task", "input_refs", "output_contract", "dependencies", "workspace_strategy", "provider_preferences", "budget_micros", "max_attempts", "compensation"}
    ...
    for raw_node in nodes:
        ...
        dependencies, inputs, preferences = node.get("dependencies"), node.get("input_refs"), node.get("provider_preferences")
        ...
        if not isinstance(dependencies, (list, tuple)) or not all(isinstance(item, str) for item in dependencies):
            raise ContractViolation("Temporal child-agent dependencies are invalid")
        if not isinstance(inputs, (list, tuple)) or not all(isinstance(item, str) and item.startswith(("sha256:", "git:", "artifact:")) for item in inputs):
            raise ContractViolation("Temporal child-agent input references are invalid")
        if not isinstance(preferences, (list, tuple)) or not all(isinstance(item, str) and item for item in preferences):
        ...
        if not isinstance(node.get("budget_micros"), int) or int(node["budget_micros"]) < 0 or not isinstance(node.get("max_attempts"), int) or int(node["max_attempts"]) < 1:
    ...
    if len(graph) != len(normalized_nodes) or any(dependency not in graph for dependencies in graph.values() for dependency in dependencies) or _cycle(graph):
        raise ContractViolation("Temporal DAG has duplicate, absent or cyclic dependencies")
    normalized_identity = {name: identity.get(name) for name in ("tenant_id", "project_id", "task_id", "run_id", "node_id", "agent_id")}
    digest_body = {"identity": tuple(normalized_identity[name] for name in ("tenant_id", "project_id", "task_id", "run_id", "node_id")), "version": version, "nodes": normalized_nodes, "reason": reason}
    if digest_of(digest_body) != declared_digest:
        raise ContractViolation("Temporal DAG digest is invalid")
    return {"identity": normalized_identity, "version": version, "nodes": normalized_nodes, "reason": reason, "digest": declared_digest}
