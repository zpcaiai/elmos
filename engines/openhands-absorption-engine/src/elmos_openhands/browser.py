        ...
        try:
            for step in scenario.steps:
                ...
                if result.get("ok") is False:
                    status = "fail"
                    break
        except Exception as error:  # noqa: BLE001 - convert driver failures into bounded evidence
            status = "error"
            ...
            try:
                for step in scenario.cleanup:
                    driver.execute(step)
            except Exception as error:  # noqa: BLE001 - cleanup failures must remain observable
                status = "error"
            ...
            except Exception as error:  # noqa: BLE001 - capture failures must remain observable
                status = "error"
