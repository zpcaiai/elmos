...
class DurableWorkerRegistry:
    ...
    def register(self, worker_id: str, *, region: str, residency: Iterable[str], capabilities: Iterable[str], capacity: WorkerCapacity, deployment_version: str, now: float | None = None) -> DurableWorkerLease:
        ...
        now = time.time() if now is None else now
        with self._lock:
            row = self._connection.execute("SELECT epoch FROM runtime_workers WHERE worker_id=?", (worker_id,)).fetchone()
            ...
            self._connection.execute(
                ...
                (worker_id, region, json.dumps(sorted(set(residency))), json.dumps(sorted(set(capabilities))), json.dumps(asdict(capacity), sort_keys=True), token, epoch, now + self.lease_seconds, deployment_version),
            )
    ...
    def heartbeat(self, lease: DurableWorkerLease, *, now: float | None = None) -> DurableWorkerLease:
        now = time.time() if now is None else now
        with self._lock:
            updated = self._connection.execute("UPDATE runtime_workers SET expires_at=? WHERE worker_id=? AND fencing_token=? AND epoch=? AND expires_at>?", (now + self.lease_seconds, lease.worker_id, lease.fencing_token, lease.epoch, now)).rowcount
        if updated != 1:
    ...
    def drain(self, lease: DurableWorkerLease) -> DurableWorkerLease:
        updated = self._connection.execute("UPDATE runtime_workers SET draining=1 WHERE worker_id=? AND fencing_token=? AND epoch=?", (lease.worker_id, lease.fencing_token, lease.epoch)).rowcount
        if updated != 1:
            raise LeaseLost("durable worker drain lost fencing ownership")
        return self.get(lease.worker_id)

    def place(self, identity: Identity, *, region: str, residency: str, required_capabilities: Iterable[str], now: float | None = None) -> tuple[str, DurableWorkerLease]:
        now = time.time() if now is None else now
        required = set(required_capabilities)
        with self._lock:
            self.recover_stale(now=now)
            rows = self._connection.execute("SELECT * FROM runtime_workers WHERE region=? AND draining=0 AND expires_at>? ORDER BY used_slots ASC,worker_id ASC", (region, now)).fetchall()
            candidates = [self._lease(row) for row in rows]
            candidates = [worker for worker in candidates if residency in worker.residency and required.issubset(worker.capabilities) and worker.used_slots < worker.capacity.slots]
            if not candidates:
                raise LeaseLost("no durable worker satisfies capacity/residency constraints")
            worker = candidates[0]
            assignment_id = new_id()
            self._connection.execute("UPDATE runtime_workers SET used_slots=used_slots+1 WHERE worker_id=? AND fencing_token=? AND used_slots<?", (worker.worker_id, worker.fencing_token, worker.capacity.slots))
            try:
                self._connection.execute("INSERT INTO worker_assignments VALUES(?,?,?,?,?,?,?,?, 'active',?)", (assignment_id, *identity.scope(), worker.worker_id, worker.fencing_token, now))
            except sqlite3.IntegrityError:
                self._connection.execute("UPDATE runtime_workers SET used_slots=MAX(0,used_slots-1) WHERE worker_id=?", (worker.worker_id,))
                row = self._connection.execute("SELECT assignment_id,worker_id,project_id,task_id FROM worker_assignments WHERE tenant_id=? AND run_id=? AND node_id=? AND state='active'", (identity.tenant_id, identity.run_id, identity.node_id)).fetchone()
                if row is None:
                    raise
    ...
    def release(self, identity: Identity, assignment_id: str) -> None:
        with self._lock:
            row = self._connection.execute("SELECT * FROM worker_assignments WHERE assignment_id=? AND tenant_id=? AND project_id=? AND task_id=? AND run_id=? AND node_id=? AND state='active'", (assignment_id, *identity.scope())).fetchone()
            if row is None:
                return
            self._connection.execute("UPDATE worker_assignments SET state='released' WHERE assignment_id=?", (assignment_id,))
            self._connection.execute("UPDATE runtime_workers SET used_slots=MAX(0,used_slots-1) WHERE worker_id=?", (row["worker_id"],))

    def recover_stale(self, *, now: float | None = None) -> tuple[str, ...]:
        now = time.time() if now is None else now
        rows = self._connection.execute("SELECT worker_id FROM runtime_workers WHERE expires_at<=?", (now,)).fetchall()
        stale = tuple(row["worker_id"] for row in rows)
        for worker_id in stale:
            self._connection.execute("UPDATE worker_assignments SET state='stale' WHERE worker_id=? AND state='active'", (worker_id,))
            self._connection.execute("UPDATE runtime_workers SET used_slots=0,draining=1 WHERE worker_id=?", (worker_id,))
        return stale

    def assignments(self, worker_id: str, *, active_only: bool = True) -> int:
        state = " AND state='active'" if active_only else ""
        return int(self._connection.execute("SELECT COUNT(*) FROM worker_assignments WHERE worker_id=?" + state, (worker_id,)).fetchone()[0])

    def get(self, worker_id: str) -> DurableWorkerLease:
        row = self._connection.execute("SELECT * FROM runtime_workers WHERE worker_id=?", (worker_id,)).fetchone()
        if row is None:
    ...
    def _lease(row: sqlite3.Row) -> DurableWorkerLease:
        return DurableWorkerLease(row["worker_id"], row["region"], frozenset(json.loads(row["residency_json"])), frozenset(json.loads(row["capabilities_json"])), WorkerCapacity(**json.loads(row["capacity_json"])), int(row["used_slots"]), row["fencing_token"], int(row["epoch"]), float(row["expires_at"]), bool(row["draining"]), row["deployment_version"])
...
class TenantQuota:
    ...
    def __post_init__(self) -> None:
        if min(self.concurrency, self.tokens, self.cpu_minutes, self.storage_bytes, self.cost_micros, self.queue_depth) < 0:
...
class DurableAdmissionController:
    ...
    def set_quota(self, tenant_id: str, quota: TenantQuota) -> None:
        with self._transaction():
            self._connection.execute("INSERT INTO tenant_quotas VALUES(?,?) ON CONFLICT(tenant_id) DO UPDATE SET quota_json=excluded.quota_json", (tenant_id, json.dumps(asdict(quota), sort_keys=True)))
    ...
    def admit(self, identity: Identity, *, window: str) -> AdmissionLease:
        ...
        with self._transaction():
            ...
            self._connection.execute("INSERT OR IGNORE INTO tenant_usage(tenant_id,window) VALUES(?,?)", (identity.tenant_id, window))
            usage = self._connection.execute("SELECT * FROM tenant_usage WHERE tenant_id=? AND window=?", (identity.tenant_id, window)).fetchone()
            if int(usage["active"]) >= quota.concurrency:
                ...
                self._connection.execute("UPDATE tenant_usage SET queued=queued+1 WHERE tenant_id=? AND window=?", (identity.tenant_id, window))
                state = "queued"
            else:
                self._connection.execute("UPDATE tenant_usage SET active=active+1 WHERE tenant_id=? AND window=?", (identity.tenant_id, window))
                state = "active"
            lease = AdmissionLease(new_id(), identity, window, state)
            self._connection.execute("INSERT INTO admissions VALUES(?,?,?,?,?,?,?,?,?)", (lease.admission_id, *identity.scope(), identity.agent_id, window, state))
            return lease
    ...
    def promote(self, lease: AdmissionLease) -> AdmissionLease:
        with self._transaction():
            ...
            usage = self._connection.execute("SELECT * FROM tenant_usage WHERE tenant_id=? AND window=?", (lease.identity.tenant_id, lease.window)).fetchone()
            if lease.state != "queued" or row["state"] != "queued" or usage is None or int(usage["active"]) >= quota.concurrency:
                raise LeaseLost("queued admission cannot be promoted")
            self._connection.execute("UPDATE tenant_usage SET queued=MAX(0,queued-1),active=active+1 WHERE tenant_id=? AND window=?", (lease.identity.tenant_id, lease.window))
            updated = self._connection.execute("UPDATE admissions SET state='active' WHERE admission_id=? AND state='queued'", (lease.admission_id,)).rowcount
            if updated != 1:
    ...
    def consume(self, lease: AdmissionLease, *, tokens: int = 0, cpu_minutes: float = 0, storage_bytes: int = 0, cost_micros: int = 0) -> None:
        ...
        with self._transaction():
            ...
            usage = self._connection.execute("SELECT * FROM tenant_usage WHERE tenant_id=? AND window=?", (lease.identity.tenant_id, lease.window)).fetchone()
            requested = {"tokens": int(usage["tokens"]) + tokens, "cpu_minutes": float(usage["cpu_minutes"]) + cpu_minutes, "storage_bytes": int(usage["storage_bytes"]) + storage_bytes, "cost_micros": int(usage["cost_micros"]) + cost_micros}
            if requested["tokens"] > quota.tokens or requested["cpu_minutes"] > quota.cpu_minutes or requested["storage_bytes"] > quota.storage_bytes or requested["cost_micros"] > quota.cost_micros:
                raise BudgetExceeded("tenant multidimensional quota exceeded")
            self._connection.execute("UPDATE tenant_usage SET tokens=?,cpu_minutes=?,storage_bytes=?,cost_micros=? WHERE tenant_id=? AND window=?", (*requested.values(), lease.identity.tenant_id, lease.window))
    ...
    def release(self, lease: AdmissionLease) -> None:
        with self._transaction():
            ...
            if row["state"] == "released":
                return
            column = "active" if row["state"] == "active" else "queued"
            self._connection.execute(f"UPDATE tenant_usage SET {column}=MAX(0,{column}-1) WHERE tenant_id=? AND window=?", (lease.identity.tenant_id, lease.window))
            self._connection.execute("UPDATE admissions SET state='released' WHERE admission_id=?", (lease.admission_id,))

    def _admission(self, lease: AdmissionLease) -> sqlite3.Row:
        row = self._connection.execute("SELECT * FROM admissions WHERE admission_id=?", (lease.admission_id,)).fetchone()
        if row is None:
    ...
    def _quota(self, tenant_id: str) -> TenantQuota:
        row = self._connection.execute("SELECT quota_json FROM tenant_quotas WHERE tenant_id=?", (tenant_id,)).fetchone()
        if row is None:
...
class ResumableEventStream(EventStream):
    ...
    def cursor(self, identity: Identity, *, after_seq: int) -> SignedEventCursor:
        ...
        return SignedEventCursor(identity.tenant_id, identity.project_id, identity.task_id, identity.run_id, identity.node_id, after_seq, head, signature)

    def resume(self, identity: Identity, cursor: SignedEventCursor, *, limit: int = 100) -> tuple[Event, ...]:
        if (cursor.tenant_id, cursor.project_id, cursor.task_id, cursor.run_id, cursor.node_id) != identity.scope():
        ...
        events = self.ledger.events(identity.tenant_id, identity.run_id, limit=100_000)
        actual = "genesis" if cursor.after_seq == -1 else (str(events[cursor.after_seq].digest) if cursor.after_seq < len(events) else "missing")
