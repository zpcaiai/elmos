...
class SandboxWarmPool:
    ...
    def __init__(self, backend: Any, database: str = ":memory:", *, policy: WarmPoolPolicy = WarmPoolPolicy()) -> None:
...
def _subprocess_runner(command: list[str], timeout: float) -> subprocess.CompletedProcess[bytes]:
    try:
        return subprocess.run(command, stdin=subprocess.DEVNULL, stdout=subprocess.PIPE, stderr=subprocess.PIPE, timeout=timeout, check=False)
