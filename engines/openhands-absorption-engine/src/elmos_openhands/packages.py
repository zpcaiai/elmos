...
from .models import canonical_json, digest_of, utc_now, validate_id
...
class Ed25519PackageSigner:
    ...
    def sign(self, digest: str, publisher: str) -> str:
        ...
        value = key.sign(f"{publisher}:{digest}".encode())
    ...
    def verify(self, digest: str, publisher: str, signature: str) -> bool:
        ...
        try:
            ...
            key.verify(value, f"{publisher}:{digest}".encode())
        except Exception:  # noqa: BLE001 - malformed signatures are untrusted package input
            return False
        return True
