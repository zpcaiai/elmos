...
from .providers import ProviderAdapter, ProviderRequest, ProviderRouter, RouteConstraints
...
class AgentRuntime:
    def __init__(self, ledger: EventLedger, provider: ProviderAdapter | ProviderRouter, gateway: ToolGateway, gates: CompletionGateEngine | None = None, *, owner: str = "runtime", lease_seconds: float = 60.0) -> None:
        self.ledger = ledger
    ...
    def run_turn(self, request: RuntimeTurnInput, *, now: float | None = None) -> RuntimeTurnResult:
        ...
        try:
            ...
            try:
                provider_request = ProviderRequest(request.identity, request.manifest.model, dict(request.context), request.tool_schemas, request.checkpoint)
            ...
            self._check_budget(request.budget, projected_usage, len(projection["actions"]) + (1 if provider_response.action else 0), now, time.time())
            ...
            if provider_response.completion is not None:
                ...
                checkpoint = self._checkpoint(request, lease, provider_response.usage, {"phase": "completion-proposed"})
            ...
            observation = self.gateway.execute(request.identity, action, context, approved_by=request.approval)
            if self.gates is not None:
            ...
            checkpoint = self._checkpoint(request, lease, provider_response.usage, {"phase": "observation", "action_id": action.action_id, "status": observation.status.value})
    ...
    def _checkpoint(self, request: RuntimeTurnInput, lease: FencedLease, usage: Usage, state: dict[str, Any]) -> str:
        checkpoint_state = {"runtime": state, "provider": {"provider": self._provider_name()}, "usage": usage.as_dict()}
        return self.ledger.save_checkpoint(request.identity, event_seq=self._last_seq(request.identity), manifest_hash=request.manifest.digest, state=checkpoint_state, workspace_ref=None if request.workspace is None else request.workspace.workspace_id, context_fingerprint=str(request.context.get("fingerprint", "")))

    def _next_turn(self, identity: Identity) -> int:
        return self._last_seq(identity) + 1

    def _decide(self, request: ProviderRequest, constraints: RouteConstraints | None):
