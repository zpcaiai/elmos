...
def main() -> int:
    ...
    _validate_project_metadata(errors)
...
def _validate_project_metadata(errors: list[str]) -> None:
    try:
        project = tomllib.loads(PYPROJECT.read_text(encoding="utf-8"))
        dependencies = project.get("project", {}).get("dependencies", [])
    except (OSError, tomllib.TOMLDecodeError) as error:
        errors.append(f"project metadata is invalid: {error}")
        return
    if "cryptography==50.0.1" not in dependencies:
        errors.append("project metadata must pin the core Ed25519 verifier dependency")
