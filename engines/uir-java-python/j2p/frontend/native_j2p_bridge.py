"""Python ctypes bridge to the native Rust Java AST/CST parser."""
...
def get_native_lib():
    ...
    try:
        ...
        # void* elmos_j2p_parse_summary(char* source)
        lib.elmos_j2p_parse_summary.argtypes = [ctypes.c_char_p]
        lib.elmos_j2p_parse_summary.restype = ctypes.c_void_p
...
def native_parse_java_summary(source_code: str) -> Optional[Dict[str, Any]]:
    lib = get_native_lib()
    if lib is None:
        return None

    source_bytes = source_code.encode("utf-8")
    res_ptr = lib.elmos_j2p_parse_summary(source_bytes)
    if not res_ptr:
        return None
