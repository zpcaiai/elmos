        ...
        *,
        ...
        for field_name, source_field in source_fields.items():
            ...
            if target_field is None:
                # Removing an optional request field widens the accepted
                # request contract; removing a response field still breaks a
                # client that consumes it.  The previous implementation
                # treated both directions as breaking.
                request_optional = channel == "request" and not source_field["required"]
                changes.append(
                    ContractDiffItem(
                        endpoint=endpoint,
                        category="FIELD_REMOVED",
                        severity="NON_BREAKING" if request_optional else "BREAKING",
