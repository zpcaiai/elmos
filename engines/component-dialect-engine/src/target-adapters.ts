...
export const TARGET_ADAPTERS: { [K in Framework]: TargetAdapterDescriptor } = {
  ...
  vue2: { id: "vue2-options-component-adapter", targetFramework: "vue2", categoryModes: commonWebModes, requiredRuntime: "BROWSER", syntaxEvidence: "AVAILABLE_HERE", runtimeEvidence: "EXTERNAL_RUNTIME_REQUIRED", emit: (ir) => ({ emitted: emitVue2(componentFor(ir)), emittedFiles: null, notes: ["Vue 2 compatibility output is statically validated here; an authorized Vue 2 runtime is required for behavioral evidence", ...(componentFor(ir).props.some((p) => p.kind === "callback" && p.paramType !== undefined) ? ["Vue 2 has no typed emit declaration; callback payload types are not representable"] : [])] }) },
