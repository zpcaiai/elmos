...
class FlutterAstParser(BaseAstParser):
    ...
    def parse(self, source_code: str) -> UniversalModule:
        ...
        widget_regex = re.compile(
            r"class\s+([A-Za-z_][A-Za-z0-9_]*)\s+extends\s+(StatelessWidget|StatefulWidget)\s*\{([^}]+)\}", re.DOTALL
        )
        ...
        for match in state_regex.finditer(source_code):
            ...
            build_match = re.search(
                r"Widget\s+build\s*\([^)]*\)\s*\{.*?return\s+([A-Za-z0-9_]+)\s*\((.*?)\);", state_body, re.DOTALL
            )
            if build_match:
