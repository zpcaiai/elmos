...
class JavaAstParser(BaseAstParser):
    ...
    def _extract_class_blocks(self, source: str) -> list[tuple[str, list[UniversalAnnotation], str | None, bool, str]]:
        ...
        while pos < len(source):
            ...
            body = (
                source[open_brace_idx + 1 : close_brace_idx] if close_brace_idx != -1 else source[open_brace_idx + 1 :]
            )
    ...
    def _parse_class_body(
        ...
        while pos < len(body):
            ...
            if type_or_name == cls_name or name_or_paren == cls_name:
                constructors.append(
                    UniversalConstructor(
                        params=params, visibility=visibility, body=self._parse_statements_simple(m_body_str)
                    )
                )
            else:
