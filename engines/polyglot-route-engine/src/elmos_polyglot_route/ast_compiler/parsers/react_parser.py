...
class ReactAstParser(BaseAstParser):
    ...
    def parse(self, source_code: str) -> UniversalModule:
        ...
        for match in re.finditer(
            r'import\s+(?:\{[^}]+\}|\*\s+as\s+[A-Za-z0-9_]+|[A-Za-z0-9_]+)\s+from\s+["\']([^"\']+)["\']', source_code
        ):
        ...
        for match in comp_regex.finditer(source_code):
            ...
            state_regex = re.compile(
                r"const\s*\[([A-Za-z0-9_]+),\s*set([A-Za-z0-9_]+)\]\s*=\s*useState(?:<([^>]+)>)?\(([^)]*)\);"
            )
            ...
            jsx_match = re.search(
                r"return\s*\(\s*(<[A-Za-z0-9_]+[^>]*>.*?</[A-Za-z0-9_]+>|<[A-Za-z0-9_]+[^/>]*/>)\s*\);",
                body_str,
                re.DOTALL,
            )
            if jsx_match:
