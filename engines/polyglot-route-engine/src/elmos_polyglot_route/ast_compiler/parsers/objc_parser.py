...
class ObjCAstParser(BaseAstParser):
    ...
    def parse(self, source_code: str) -> UniversalModule:
        ...
        for match in interface_regex.finditer(source_code):
            ...
            u_class = UniversalClass(name=c_name, super_class=s_name.strip() if s_name else "NSObject")
            if protocols:
                u_class.interfaces = [p.strip() for p in protocols.split(",")]

            # Extract @property (nonatomic, copy/strong/assign) Type *name;
            prop_regex = re.compile(r"@property\s*\([^)]*\)\s*([A-Za-z0-9_]+(?:\s*\*)?)\s*([A-Za-z_][A-Za-z0-9_]*);")
        ...
        impl_regex = re.compile(r"@implementation\s+([A-Za-z_][A-Za-z0-9_]*)\s*(.*?)\s*@end", re.DOTALL)
        for match in impl_regex.finditer(source_code):
            ...
            method_regex = re.compile(r"([-+])\s*\(([^)]+)\)\s*([A-Za-z0-9_:]+)\s*\{([^}]+)\}", re.DOTALL)
            for m_match in method_regex.finditer(body):
                ...
                u_method = UniversalMethod(name=m_name, return_type=ret_type, is_static=is_static)
