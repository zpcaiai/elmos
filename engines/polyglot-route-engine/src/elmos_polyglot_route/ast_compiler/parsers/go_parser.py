    ...
    def parse(self, source_code: str) -> UniversalModule:
        ...
        for sm in struct_regex.finditer(source_code):
            ...
            module.classes.append(
                UniversalClass(name=cls_name, fields=fields, is_struct=True, is_controller=is_ctrl, base_route=route)
            )
        ...
        while pos < len(source_code):
            ...
            http_m = (
                "GET"
                if "get" in m_name.lower()
                else ("POST" if "create" in m_name.lower() or "post" in m_name.lower() else None)
            )
