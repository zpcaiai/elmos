    ...
    def run_cpp_sanitizer(self, cpp_code: str, sanitizer: SanitizerType = SanitizerType.ASAN) -> SanitizerResult:
        ...
        if not self.clangpp:
            return SanitizerResult(
                ...
                language="cpp",
                exit_code=0,
                stdout="Clang++ not available, skipped",
                stderr="",
            )
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            if comp_res.returncode != 0:
                return SanitizerResult(
                    ...
                    error_summary="C++ Compilation failed",
                )

            run_res = subprocess.run([str(bin_file)], capture_output=True, text=True, timeout=10)
            passed = (
                (run_res.returncode == 0)
                and ("AddressSanitizer" not in run_res.stderr)
                and ("runtime error:" not in run_res.stderr)
            )
            return SanitizerResult(
                passed=passed,
                sanitizer=sanitizer,
    ...
    def run_swift_sanitizer(self, swift_code: str) -> SanitizerResult:
        ...
        if not self.swiftc:
            return SanitizerResult(
                ...
                language="swift",
                exit_code=0,
                stdout="swiftc not available, skipped",
                stderr="",
            )
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            if comp_res.returncode != 0:
                return SanitizerResult(
                    ...
                    stdout=comp_res.stdout,
                    stderr=comp_res.stderr,
                    error_summary="Swift compilation failed",
                )
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            if comp_res.returncode != 0:
                return SanitizerResult(
                    ...
                    stdout=comp_res.stdout,
                    stderr=comp_res.stderr,
                    error_summary="ObjC compilation failed",
                )
    ...
    def run_rust_sanitizer(self, rust_code: str, sanitizer: SanitizerType = SanitizerType.ASAN) -> SanitizerResult:
        ...
        if not self.rustc:
            return SanitizerResult(
                ...
                language="rust",
                exit_code=0,
                stdout="rustc not available, skipped",
                stderr="",
            )
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            if comp_res.returncode != 0:
                return SanitizerResult(
                    ...
                    stdout=comp_res.stdout,
                    stderr=comp_res.stderr,
                    error_summary="rustc compilation failed",
                )
    ...
    def run_go_race_sanitizer(self, go_code: str) -> SanitizerResult:
        ...
        if not self.go:
            return SanitizerResult(
                ...
                language="go",
                exit_code=0,
                stdout="go not available, skipped",
                stderr="",
            )
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            try:
                run_res = subprocess.run(
                    [self.go, "run", "-race", str(src_file)], cwd=tmpdir, capture_output=True, text=True, timeout=60
                )
