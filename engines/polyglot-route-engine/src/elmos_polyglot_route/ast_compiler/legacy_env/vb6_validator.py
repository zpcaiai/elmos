...
class Vb6StrictSemanticValidator:
    ...
    def validate(cls, source_code: str) -> tuple[int, list[Vb6Diagnostic]]:
        ...
        for idx, line in enumerate(lines, start=1):
            ...
            decl_match = re.match(
                r"^(?:Public|Private|Dim|Static)\s+([A-Za-z_][A-Za-z0-9_]*)(?:\s+As\s+([A-Za-z0-9_]+))?",
                stripped,
                re.IGNORECASE,
            )
            if decl_match:
            ...
            sub_match = re.match(
                r"^(?:Public|Private)?\s*(?:Function|Sub)\s+([A-Za-z_][A-Za-z0-9_]*)\s*\((.*?)\)(?:\s+As\s+([A-Za-z0-9_]+))?",
                stripped,
                re.IGNORECASE,
            )
            if sub_match:
                ...
                if params_str:
                    for param in params_str.split(","):
                        param_match = re.search(
                            r"([A-Za-z_][A-Za-z0-9_]*)(?:\s+As\s+([A-Za-z0-9_]+))?", param.strip(), re.IGNORECASE
                        )
                        if param_match:
