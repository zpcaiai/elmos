...
from ..ir import UniversalAnnotation, UniversalClass, UniversalModule
...
class FrameworkLowering:
    ...
    def lower_class(cls, c: UniversalClass, target_language: str) -> None:
        if not c.is_controller and not any(
            "controller" in a.name.lower() or "router" in a.name.lower() for a in c.annotations
        ):
            return
