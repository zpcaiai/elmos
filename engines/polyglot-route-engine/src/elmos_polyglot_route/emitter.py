...
#: Swift traps on Int overflow and on both integer division errors by default,
...
def emit(ir: SemanticIR, target: Language) -> EmittedFile:
    ...
    if target == "rust":
        body = "\n\n".join([*helpers, functions])
        return EmittedFile("migrated.rs", body + "\n")
    if target == "cpp":
    ...
    if target == "swift":
        body = "\n\n".join([*helpers, functions])
        return EmittedFile("migrated.swift", body + "\n")
    body = "\n\n".join([*helpers, functions])
    return EmittedFile("migrated.ts", f"{body}\n")
