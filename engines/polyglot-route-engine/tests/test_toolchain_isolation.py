...
def test_homebrew_route_bundle_profiles_are_exact_and_fail_closed() -> None:
    ...
    refreshed_hosted = toolchains._select_homebrew_route_bundle_profile(
    ...
    assert refreshed_hosted.profile_id == "github-macos26-20260907.0351.1"
    assert all(getattr(refreshed_hosted, field) == getattr(current_hosted, field) for field in dotnet_fields)
    assert refreshed_hosted.php_tree_sha256 == ("ca33ea07e927e25416bc906af465ba6713824e3e5af66fb974f319e92c43d6d9")
...
@pytest.mark.parametrize(
    ...
    (
        ...
        toolchains._HOMEBREW_ROUTE_REFRESHED_HOSTED_PROFILE,
    ),
