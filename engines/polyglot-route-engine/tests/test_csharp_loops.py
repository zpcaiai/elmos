"""Lifting while and for loops from C# source code.

Verifies that C# while loops (`while (cond) { ... }`) and monotonic for loops
...
def test_csharp_for_loop_lifts_default_step(tmp_path: Path) -> None:
    source = _source(
        tmp_path,
        "        long total = 0;\n"
        "        for (long i = 0; i < n; i++) {\n"
...
def test_csharp_for_loop_lifts_custom_step(tmp_path: Path) -> None:
    source = _source(
        tmp_path,
        "        long total = 0;\n"
...
def test_csharp_rejects_non_monotonic_cond(tmp_path: Path) -> None:
    source = _source(
        tmp_path,
        "        long total = 0;\n"
        "        for (long i = 0; i != n; i++) {\n"
    ...
    with pytest.raises(RouteError, match="CSHARP_FOR_CONDITION_NON_MONOTONIC"):
        analyze(source, "csharp", "subject")


def test_csharp_rejects_non_monotonic_update(tmp_path: Path) -> None:
    source = _source(
        tmp_path,
        "        long total = 0;\n"
        "        for (long i = 0; i < n; i--) {\n"
    ...
    with pytest.raises(RouteError, match="CSHARP_FOR_UPDATE_NON_MONOTONIC"):
        analyze(source, "csharp", "subject")


def test_csharp_loop_emits_to_all_targets(tmp_path: Path) -> None:
    source = _source(
        tmp_path,
        "        long total = 0;\n"
