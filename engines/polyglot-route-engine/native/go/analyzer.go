...
func expression(expr ast.Expr) map[string]any {
	...
	case *ast.BinaryExpr:
		...
		return map[string]any{
			...
			"left": expression(value.X), "right": expression(value.Y),
		}
	default:
...
func statements(block *ast.BlockStmt) []map[string]any {
	...
	for _, raw := range block.List {
		...
		case *ast.ReturnStmt:
			...
			result = append(result, map[string]any{"kind": "return", "expression": expression(statement.Results[0])})
		case *ast.IfStmt:
			...
			if statement.Else != nil {
				...
				elseBody = statements(elseBlock)
			}
			result = append(result, map[string]any{
				"kind": "if", "condition": expression(statement.Cond),
				"then": statements(statement.Body), "else": elseBody,
			})
		default:
...
func main() {
	...
	if len(arguments) != 2 {
		fail("USAGE:analyzer SOURCE FUNCTION")
	}
	...
	payload := map[string]any{
		...
		"functions": []map[string]any{{
			...
			"body":        statements(candidate.Body),
		}},
		"diagnostics": []string{},
	}
