"""Pure standard library unit tests for commercial capability expansion local algorithms.

Zero external dependencies: uses unittest.TestCase instead of pytest.
"""
...
class LocalAlgorithmsTest(unittest.TestCase):
    def test_provenance_validates_closure_dag_and_is_order_invariant(self) -> None:
        ...
        self.assertEqual(result["topological_order"], ("runtime", "lib", "app"))
        with self.assertRaises(ContractError) as cycle:
            validate_provenance_bindings(bindings, {"app": ["lib"], "lib": ["app"], "runtime": []})
        self.assertEqual(cycle.exception.code, "PROVENANCE_CYCLE")
        with patch.object(algorithms, "MAX_EDGES", 1):
            with self.assertRaises(ContractError) as limited:
                validate_provenance_bindings(bindings, dependencies)
            self.assertEqual(limited.exception.code, "LOCAL_INPUT_LIMIT")

    def test_provenance_large_fanout_uses_heap_without_reordering_the_ready_set(self) -> None:
        ...
        with patch.object(algorithms, "deque", NoSortDeque):
            result = validate_provenance_bindings(bindings, dependencies)
        self.assertEqual(result["binding_count"], 2_000)
        self.assertEqual(result["topological_order"], tuple(sorted(bindings)))

    def test_progressive_disclosure_filters_scope_budgets_and_is_order_invariant(self) -> None:
        ...
        self.assertEqual(result, reordered)
        self.assertEqual(result["selected_skill_ids"], ("db-small",))
        self.assertEqual(result["deferred_skill_ids"], ("db-large",))
        self.assertNotIn("other-tenant", result["selected_skill_ids"])
        bad = deepcopy(records)
        bad[0]["permissions"] = ["admin"]
        filtered = progressive_disclosure(bad, _context(), ["database"], 8)
        self.assertEqual(filtered["selected_skill_ids"], ("db-large",))
        self.assertNotIn("db-small", filtered["selected_skill_ids"])
        with patch.object(algorithms, "MAX_ITEMS", 1):
            with self.assertRaises(ContractError) as limited:
                progressive_disclosure(records, _context(), ["database"], 8)
            self.assertEqual(limited.exception.code, "LOCAL_INPUT_LIMIT")

    def test_progressive_disclosure_rejects_unbounded_query_term_scans(self) -> None:
    ...
    def test_router_enforces_hard_constraints_deterministically(self) -> None:
        candidates = _router_candidates()
        result = route_constrained_candidate(candidates, _router_constraints())
        self.assertEqual(result, route_constrained_candidate(list(reversed(candidates)), _router_constraints()))
        self.assertEqual(result["selected_id"], "safe")
        with self.assertRaises(ContractError) as infeasible:
            route_constrained_candidate(candidates, {**_router_constraints(), "min_proof": 5})
        self.assertEqual(infeasible.exception.code, "NO_FEASIBLE_CANDIDATE")
        with patch.object(algorithms, "MAX_ITEMS", 1):
            with self.assertRaises(ContractError) as limited:
                route_constrained_candidate(candidates, _router_constraints())
            self.assertEqual(limited.exception.code, "LOCAL_INPUT_LIMIT")

    def test_typed_graph_closure_handles_chains_cycles_order_and_limits(self) -> None:
        ...
        self.assertEqual(result, reordered)
        self.assertEqual(result["affected_nodes"], ("api", "database", "events", "service"))
        cyclic = [*_edges(), {"source": "database", "target": "api"}]
        self.assertEqual(typed_graph_closure(nodes, cyclic, ["api"])["affected_count"], 4)
        with self.assertRaises(ContractError) as dangling:
            typed_graph_closure(nodes, [*_edges(), {"source": "api", "target": "missing"}], ["api"])
        self.assertEqual(dangling.exception.code, "INVALID_GRAPH")
        with patch.object(algorithms, "MAX_ITEMS", 2):
            with self.assertRaises(ContractError) as limited:
                typed_graph_closure(nodes, _edges(), ["api"])
            self.assertEqual(limited.exception.code, "LOCAL_INPUT_LIMIT")

    def test_affected_tests_exposes_uncovered_critical_and_is_order_invariant(self) -> None:
        nodes = ["api", "service", "database", "events"]
        ...
        self.assertEqual(result, reordered)
        self.assertEqual(result["confidence"], "INCOMPLETE")
        self.assertEqual(result["uncovered_critical_nodes"], ("database",))
        with self.assertRaises(ContractError) as invalid:
            select_affected_tests(nodes, _edges(), ["api"], {"missing": ["test"]}, [])
        self.assertEqual(invalid.exception.code, "INVALID_GRAPH")
        with patch.object(algorithms, "MAX_ITEMS", 2):
            with self.assertRaises(ContractError) as limited:
                select_affected_tests(nodes, _edges(), ["api"], coverage, ["database"])
            self.assertEqual(limited.exception.code, "LOCAL_INPUT_LIMIT")

    def test_dependency_slice_is_closed_all_or_fail_and_order_invariant(self) -> None:
        ...
        self.assertEqual(result, reordered)
        self.assertEqual(result["selected_nodes"], ("app", "lib", "runtime"))
        with self.assertRaises(ContractError) as budget:
            dependency_closed_slice(nodes, edges, ["app"], costs, 5)
        self.assertEqual(budget.exception.code, "SLICE_BUDGET_EXCEEDED")
        with patch.object(algorithms, "MAX_ITEMS", 2):
            with self.assertRaises(ContractError) as limited:
                dependency_closed_slice(nodes, edges, ["app"], costs, 6)
            self.assertEqual(limited.exception.code, "LOCAL_INPUT_LIMIT")

    def test_risk_is_monotonic_order_invariant_and_bounded(self) -> None:
        ...
        self.assertEqual(high, reordered)
        self.assertIsInstance(high["score"], int)
        self.assertIsInstance(low["score"], int)
        self.assertGreaterEqual(high["score"], low["score"])
        with self.assertRaises(ContractError) as invalid:
            classify_monotonic_risk(["api"], ["db"], [], [], 0, "1")
        self.assertEqual(invalid.exception.code, "INVALID_RISK_INPUT")
        with patch.object(algorithms, "MAX_ITEMS", 1):
            with self.assertRaises(ContractError) as limited:
                classify_monotonic_risk(["api", "db"], [], [], [], 0, "1")
            self.assertEqual(limited.exception.code, "LOCAL_INPUT_LIMIT")

    def test_explainability_ledger_hash_chain_is_order_invariant_and_tamper_evident(self) -> None:
        edits = _ledger_edits()
        result = build_explainability_ledger(edits)
        self.assertEqual(result, build_explainability_ledger(list(reversed(edits))))
        tampered = deepcopy(edits)
        tampered[0]["reason"] = "different reason"
        self.assertNotEqual(build_explainability_ledger(tampered)["ledger_digest"], result["ledger_digest"])
        ...
        self.assertEqual(sequence.exception.code, "INVALID_LEDGER_SEQUENCE")
        with patch.object(algorithms, "MAX_ITEMS", 1):
            with self.assertRaises(ContractError) as limited:
                build_explainability_ledger(edits)
            self.assertEqual(limited.exception.code, "LOCAL_INPUT_LIMIT")

    def test_lineage_closure_is_typed_downstream_order_invariant_and_bounded(self) -> None:
        ...
        self.assertEqual(result, reordered)
        self.assertEqual(result["affected_consumers"], ("clean", "report"))
        invalid_edges = [{"source": "raw", "target": "clean", "kind": "table"}]
        with self.assertRaises(ContractError) as invalid:
            lineage_impact_closure(_lineage_entities(), invalid_edges, ["raw"])
        self.assertEqual(invalid.exception.code, "INVALID_LINEAGE")
        with patch.object(algorithms, "MAX_ITEMS", 2):
            with self.assertRaises(ContractError) as limited:
                lineage_impact_closure(_lineage_entities(), _lineage_edges(), ["raw"])
            self.assertEqual(limited.exception.code, "LOCAL_INPUT_LIMIT")

    def test_keyed_decimal_reconciliation_preserves_duplicates_order_and_limits(self) -> None:
        rows = _source_rows()
        result = reconcile_keyed_rows(rows, list(reversed(rows)), ["id"], ["amount"])
        self.assertTrue(result["equivalent"])
        self.assertEqual(result["duplicate_key_rows"], {"source": 1, "target": 1})
        ...
        self.assertEqual(mismatch["aggregate_deltas"], {"amount": "0.01"})
        self.assertIsInstance(mismatch["mismatched_key_digests"], tuple)
        self.assertEqual(len(mismatch["mismatched_key_digests"]), 1)
        with self.assertRaises(ContractError) as binary_float:
            reconcile_keyed_rows([{"id": 1, "amount": 1.1}], [], ["id"], ["amount"])
        self.assertEqual(binary_float.exception.code, "INVALID_RECONCILIATION_ROW")
        with patch.object(algorithms, "MAX_ITEMS", 2):
            with self.assertRaises(ContractError) as limited:
                reconcile_keyed_rows(rows, rows, ["id"], ["amount"])
            self.assertEqual(limited.exception.code, "LOCAL_INPUT_LIMIT")

    def test_rubric_scorecard_has_hard_mandatory_gates_order_and_limits(self) -> None:
        ...
        self.assertEqual(result, reordered)
        self.assertEqual(result["decision"], "PASS_BOUNDED_LOCAL")
        failed = evaluate_rubric_scorecard({"quality": "0.7", "coverage": "1"}, _rubric())
        self.assertEqual(failed["decision"], "FAIL")
        self.assertEqual(failed["mandatory_failures"], ("quality",))
        with self.assertRaises(ContractError) as missing:
            evaluate_rubric_scorecard({"quality": "0.9"}, _rubric())
        self.assertEqual(missing.exception.code, "INVALID_RUBRIC")
        with patch.object(algorithms, "MAX_ITEMS", 1):
            with self.assertRaises(ContractError) as limited:
                evaluate_rubric_scorecard(observations, _rubric())
            self.assertEqual(limited.exception.code, "LOCAL_INPUT_LIMIT")

    def test_incident_divergence_finds_first_cause_and_rejects_gaps(self) -> None:
        events = _events()
        self.assertTrue(incident_causal_divergence(events, list(reversed(events)))["equivalent"])
        observed = deepcopy(events)
        observed[1]["payload_digest"] = _digest("c")
        result = incident_causal_divergence(events, observed)
        self.assertEqual(result["first_divergence_sequence"], 1)
        ...
        self.assertEqual(gap.exception.code, "INCIDENT_REPLAY_INCONCLUSIVE")
        with patch.object(algorithms, "MAX_ITEMS", 1):
            with self.assertRaises(ContractError) as limited:
                incident_causal_divergence(events, events)
            self.assertEqual(limited.exception.code, "LOCAL_INPUT_LIMIT")

    def test_optimizer_returns_pareto_frontier_without_quality_proof_compensation(self) -> None:
        candidates = _optimizer_candidates()
        result = optimize_cost_latency_quality(candidates, _optimizer_constraints())
        self.assertEqual(result, optimize_cost_latency_quality(list(reversed(candidates)), _optimizer_constraints()))
        self.assertEqual(result["pareto_frontier_ids"], ("balanced", "fast"))
    ...
    def test_optimizer_handles_maximum_tradeoff_frontier_and_identical_metrics(self) -> None:
        ...
        self.assertEqual(result["feasible_count"], candidate_count)
        self.assertEqual(len(result["pareto_frontier_ids"]), candidate_count)
        ...
        self.assertEqual(duplicate_result["pareto_frontier_ids"], ("same-a", "same-b"))

    def test_every_public_algorithm_rejects_noncanonical_total_input(self) -> None:
