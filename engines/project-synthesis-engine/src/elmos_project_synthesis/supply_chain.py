...
def _artifact_cache_candidate(
    ...
    *,
    ...
    if not all(safe_piece.fullmatch(value) for value in (group, artifact, version)):
        raise SupplyChainFailure(f"NATIVE_ARTIFACT_COORDINATE_INVALID:{reference}")
    filenames = (f"{artifact}-{version}.jar", f"{artifact}-{version}.pom")
    ...
    for candidate, cache_root, cache_kind in candidates:
        if candidate.is_symlink() or candidate.stat().st_size <= 0 or candidate.stat().st_size > 512 * 1024 * 1024:
            continue
...
_EXPECTED_TOOLCHAIN_LABELS: dict[str, tuple[str, ...]] = {
    ...
    "go": ("Go 1.25.0",),
    "kotlin": ("OpenJDK 21.0.11", "Gradle 8.14.3"),
    "php": ("PHP 8.4.12",),
...
def _validate_verification_evidence(
    ...
    This rejects hand-written green summaries and empty result lists. It does
    not turn repository-owned local evidence into independent verification;
...
def build_release_manifest(
    ...
    *,
    ...
    if source_repository is None:
        revision = {"commit_sha": None, "tree_sha": None, "worktree_clean": None}
...
def _validate_unsigned_release_manifest(manifest: Mapping[str, Any]) -> None:
    ...
    if (
        ...
        or set(revision) != {"commit_sha", "tree_sha", "worktree_clean"}
...
def verify_release_signature(
    ...
    *,
    ...
    if not all(isinstance(value, dict) for value in (manifest, envelope, trust_root)):
