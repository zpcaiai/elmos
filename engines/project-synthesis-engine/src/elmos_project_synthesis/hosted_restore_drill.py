...
def run_restore_drill(
    *,
    ...
    with postgres_environment(source) as source_environment:
        dumped = _run(
            [postgres_executable(pg_dump), "--no-owner", "--no-privileges"],
    ...
    with postgres_environment(restore) as restore_environment:
        restored = _run(
            [postgres_executable(psql), "-v", "ON_ERROR_STOP=1", "-f", str(dump)],
