...
def postgres_environment(url: str) -> Iterator[dict[str, str]]:
    ...
    with tempfile.NamedTemporaryFile(mode="w", prefix="elmos-pgpass-", delete=False) as pgpass:
...
def read_url_file(path: Path) -> str:
    ...
    info = path.stat()
    if info.st_size < 16 or info.st_size > 4_096 or (os.name == "posix" and info.st_mode & 0o077):
        raise HostedPostgresError("HOSTED_POSTGRES_URL_FILE_UNSAFE")
