...
def _require_owner_only_file(path: Path, *, minimum: int = 32, public: bool = False) -> Path:
    ...
    if info.st_size < minimum or info.st_size > 16_384:
        raise JobTokenError("JOB_TOKEN_KEY_UNSAFE")
    # POSIX permission bits are meaningful only on POSIX filesystems. Windows
    # reports synthesized mode bits even after chmod; path/symlink/size checks
    # still apply there and production deployment must enforce an ACL at the
    # secret mount boundary.
    if public:
...
def _sign(private_key: Path, message: bytes) -> bytes:
    # Windows does not allow OpenSSL to reopen a delete-on-close temporary
    # file while Python still owns the handle. Close first and remove it in a
    # finally block so signing has the same bounded scratch lifecycle on both
    # platforms.
    with tempfile.NamedTemporaryFile(
        dir=private_key.parent, prefix="elmos-job-token-sign-", delete=False
    ) as inbound:
        os.chmod(inbound.name, 0o600)
        inbound.write(message)
        inbound.flush()
        inbound_path = Path(inbound.name)
    try:
...
def _verify_with_sigfile(public_key: Path, message: bytes, signature: bytes, scratch: Path) -> bool:
    ...
    with tempfile.NamedTemporaryFile(dir=scratch, prefix="elmos-job-token-sig-", delete=False) as sig, \
        tempfile.NamedTemporaryFile(dir=scratch, prefix="elmos-job-token-msg-", delete=False) as inbound:
