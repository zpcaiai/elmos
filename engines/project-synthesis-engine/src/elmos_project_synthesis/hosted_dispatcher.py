...
def clamp_limits(requested: dict[str, int]) -> dict[str, int]:
    return {
        "cpu_millis": max(
            100,
            min(int(requested.get("cpu_millis", HARD_CPU_MILLIS)), HARD_CPU_MILLIS),
        ),
        "memory_mib": max(
            64,
            min(int(requested.get("memory_mib", HARD_MEMORY_MIB)), HARD_MEMORY_MIB),
        ),
        "pids": max(8, min(int(requested.get("pids", HARD_PIDS)), HARD_PIDS)),
        "wallclock_seconds": max(
            1,
            min(int(requested.get("wallclock_seconds", HARD_WALLCLOCK)), HARD_WALLCLOCK),
        ),
    }
