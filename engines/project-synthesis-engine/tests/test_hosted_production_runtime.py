...
def test_hosted_postgres_password_never_enters_client_argv(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    url_file = tmp_path / "pg.url"
    url_file.write_text(
        "postgresql://app:secret%3Avalue@db.example:5432/app_db?sslmode=require\n",
        encoding="utf-8",
    )
    os.chmod(url_file, 0o600)
    ...
    def fake_run(command: list[str], **kwargs: object) -> subprocess.CompletedProcess[str]:
        ...
        pgpass = Path(str(environment["PGPASSFILE"]))
        observed["command"] = command
        observed["pgpass"] = pgpass.read_text(encoding="utf-8")
        observed["environment"] = environment
        return subprocess.CompletedProcess(command, 0, "17.5\n", "")

    monkeypatch.setattr("elmos_project_synthesis.hosted_postgres.subprocess.run", fake_run)
    binding = bind_hosted_postgres(url_file, psql=sys.executable)
    assert binding.version == "17.5"
    assert "secret" not in " ".join(observed["command"])  # type: ignore[arg-type]
    assert "secret\\:value" in str(observed["pgpass"])
    assert "PGPASSWORD" not in observed["environment"]  # type: ignore[operator]
