...
function routeData(context: ProjectTemplateContext): readonly Record<string, unknown>[] {
...
function reactTemplate(context: ProjectTemplateContext): Record<string, string> {
  const routes = routeData(context);
  ...
  return {
    ...
    "src/routes.ts": lines(
      ...
      "}",
      `export const routes: readonly GeneratedRoute[] = ${JSON.stringify(routes, null, 2)};`,
    ),
    "src/App.tsx": lines(
      ...
      '  return <main className="content" id="main"><article className="card">',
      ...
      "      {routes.map(route => <NavLink key={route.id} to={route.path}>{route.title}</NavLink>)}",
      "    </nav>",
      "    <Routes>",
...
function vue3Template(context: ProjectTemplateContext): Record<string, string> {
  ...
  return {
    ...
    "src/routes.ts": lines(
      ...
      "  readonly requiresAuth: boolean; readonly deepLink: boolean;",
      "}",
      `export const routes: readonly GeneratedRoute[] = ${JSON.stringify(routeData(context), null, 2)};`,
    ),
    "src/router.ts": lines(
      ...
      "  routes: routes.map(route => ({ path: route.path, component: GeneratedPage, meta: { generatedRoute: route } })),",
      "});",
    ),
    "src/views/GeneratedPage.vue": lines(
      ...
      "</script>",
      "<template><main class=\"content\" id=\"main\"><article class=\"card\">",
    ...
    "src/App.vue": lines(
      ...
      '  <RouterLink v-for="route in routes" :key="route.id" :to="route.path">{{ route.title }}</RouterLink>',
...
function vue2Template(context: ProjectTemplateContext): Record<string, string> {
  ...
  return {
    ...
    "src/routes.js": `export const routes = ${JSON.stringify(routeData(context), null, 2)};\n`,
    "src/router.js": lines(
      'import Vue from "vue";',
      ...
      'import { routes } from "./routes";',
      "Vue.use(VueRouter);",
      "export const router = new VueRouter({ mode: \"history\", routes: routes.map(route => ({ path: route.path, component: GeneratedPage, meta: { generatedRoute: route } })) });",
    ),
    "src/views/GeneratedPage.vue": lines(
      "<script>",
      'import { routes } from "../routes";',
...
function jqueryTemplate(context: ProjectTemplateContext): Record<string, string> {
  return {
    ...
    "src/main.ts": lines(
      ...
      '  $("#main").empty().append(article);',
      "}",
      "",
      'const nav = $("<nav>", { class: "nav", "aria-label": "主要导航" }).append($("<strong>").text(' + js(context.request.title) + "));",
      "for (const route of routes) nav.append($(\"<a>\", { href: route.path }).text(route.title));",
...
function svelteTemplate(context: ProjectTemplateContext): Record<string, string> {
  return {
    ...
    "src/App.svelte": lines(
      ...
      `  <strong>${html(context.request.title)}</strong>`,
      '  {#each routes as route}<a href={route.path} onclick={(event) => navigate(event, route.path)}>{route.title}</a>{/each}',
      '</nav><main class="content" id="main"><article class="card">',
      "  <h1>{page?.title}</h1><p>{page?.text}</p>",
      '  <p class="status" role="status">生成状态：等待真实浏览器与可访问性验证</p>',
      "</article></main></div>",
    ),
...
function angularTemplate(context: ProjectTemplateContext): Record<string, string> {
  const routeRecords = routeData(context).map(route => ({
  ...
  return {
    ...
    "src/routes.ts": lines(
      ...
      `const generated = ${JSON.stringify(routeRecords, null, 2)} as const;`,
      "export const routes: Routes = generated.map(route => ({ path: route.path, component: GeneratedPageComponent, data: route.data }));",
    ),
    "src/app/generated-page.component.ts": lines(
      ...
      '  template: `<main class="content" id="main"><article class="card"><h1>{{ title }}</h1><p>{{ text }}</p><p class="status" role="status">生成状态：等待真实浏览器与可访问性验证</p></article></main>`,',
      "})",
    ...
    "src/app/app.component.ts": lines(
      ...
      `const generatedRoutes = ${JSON.stringify(routeData(context), null, 2)} as const;`,
      "@Component({",
      "  standalone: true, selector: \"app-root\", imports: [RouterLink, RouterOutlet],",
      '  template: `<div class="shell"><nav class="nav" aria-label="主要导航"><strong>' + html(context.request.title) + '</strong>@for (route of routes; track route.id) {<a [routerLink]="route.path">{{ route.title }}</a>}</nav><router-outlet /></div>`,',
      "})",
      "export class AppComponent { readonly routes = generatedRoutes; }",
    ),
    "src/main.ts": lines(
...
function reactNativeTemplate(context: ProjectTemplateContext): Record<string, string> {
  ...
  const screenEntries = screenNames.map(route =>
    `      <Stack.Screen name=${js(route.screen)} component={GeneratedScreen} initialParams={{ title: ${js(route.title)}, text: ${js(route.text)} }} />`);
  const links = Object.fromEntries(screenNames.map(route => [route.screen, route.path.replace(/^\//, "") || "home"]));
  return {
    "package.json": packageJson(
      context,
      {
    ...
    "src/navigation.tsx": lines(
      ...
      "",
      "export interface GeneratedScreenParams { readonly title: string; readonly text: string; }",
      ...
      '  return <ScrollView contentContainerStyle={styles.content}><View accessible accessibilityRole="summary">',
      ...
      "  </View></ScrollView>;",
      "}",
      `const linking: LinkingOptions<RootStackParamList> = { prefixes: [${js(`${context.safeProjectName}://`)}], config: { screens: ${JSON.stringify(links)} } };`,
      "export function GeneratedNavigation() {",
      "  return <NavigationContainer linking={linking}><Stack.Navigator>",
      ...screenEntries,
...
function flutterTemplate(context: ProjectTemplateContext): Record<string, string> {
  const routeRows = context.routes.map(route =>
    `  GeneratedRoute(${js(route.path)}, ${js(route.title)}, ${js(route.text)}),`);
  return {
    "pubspec.yaml": lines(
    ...
    "lib/main.dart": lines(
      ...
      "  const GeneratedRoute(this.path, this.title, this.text);",
      "  final String path;",
      "  final String title;",
      "  final String text;",
      "}",
      ...
      `    return MaterialApp(title: ${js(context.request.title)}, initialRoute: generatedRoutes.first.path,`,
      "      routes: { for (final route in generatedRoutes) route.path: (_) => GeneratedPage(route: route) });",
      "  }",
      "}",
      "",
      ...
      "          child: ListView(",
      "            children: [",
      "              for (final item in generatedRoutes)",
      "                ListTile(",
      "                  title: Text(item.title),",
      "                  onTap: () => Navigator.of(context).pushReplacementNamed(item.path),",
      "                ),",
      "            ],",
      "          ),",
      "        ),",
    ...
    "test/widget_test.dart": lines(
      ...
      `import 'package:${context.safeProjectName.replaceAll("-", "_")}/main.dart';`,
      "void main() {",
      `  test('preserves all generated routes', () { expect(generatedRoutes.length, ${context.routes.length}); expect(generatedRoutes.map((item) => item.path).toSet().length, generatedRoutes.length); });`,
      "}",
    ),
  };
}
...
function harmonyTemplate(context: ProjectTemplateContext): Record<string, string> {
  const routeRows = context.routes.map((route, index) =>
    `  { id: ${js(route.id)}, path: ${js(route.path)}, title: ${js(route.title)}, text: ${js(route.text)}, index: ${index} },`);
  return {
    ...
    "entry/src/main/ets/pages/Index.ets": lines(
      "interface GeneratedRoute { id: string; path: string; title: string; text: string; index: number; }",
      "const GENERATED_ROUTES: GeneratedRoute[] = [",
      ...routeRows,
      "];",
      "@Entry",
      "@Component",
      ...
      "          ForEach(GENERATED_ROUTES, (item: GeneratedRoute) => {",
      "            Button(item.title).width('100%').onClick(() => { this.selected = item.index; })",
      "          }, (item: GeneratedRoute) => item.id)",
      "        }.width('34%').padding(16)",
      "        Column({ space: 16 }) {",
      "          Text(GENERATED_ROUTES[this.selected].title).fontSize(30).fontWeight(FontWeight.Bold)",
      "          Text(GENERATED_ROUTES[this.selected].text).fontSize(18)",
