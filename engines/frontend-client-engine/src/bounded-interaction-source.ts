...
import type { BoundedNavigationRouteSource } from "./bounded-navigation-source.js";
import type { UiFrameworkId, UiProjectGenerationRequestV2 } from "./project-types.js";
...
export type InteractionBlockId = (typeof interactionBlockIds)[number];

export const interactionBlockSymbolMap: Readonly<Record<InteractionBlockId, string>> = {
...
export type InteractionInfluenceClass = "TRANSITION" | "OBSERVABLE_EFFECT" | "DECLARATION_ECHO";
export type InteractionRuntimeInfluenceClass = "FRAMEWORK_OBSERVABLE" | "MODEL_ONLY_NOT_RUNTIME" | "ADAPTER_SEAM_NOT_RUN" | "DECLARATION_ECHO";

export const interactionInfluenceMatrix: Readonly<Record<InteractionBlockId, Readonly<Record<string, InteractionInfluenceClass>>>> = {
  "route-navigation-deeplink-404": {
...
export const interactionRuntimeInfluenceMatrix: Readonly<Record<InteractionBlockId, Readonly<Record<string, InteractionRuntimeInfluenceClass>>>> = Object.fromEntries(
  interactionBlockIds.map(blockId => [blockId, Object.fromEntries(Object.entries(interactionInfluenceMatrix[blockId]).map(([pointer, influence]) => {
...
) as Readonly<Record<InteractionBlockId, Readonly<Record<string, InteractionRuntimeInfluenceClass>>>>;

export function aggregateModelInfluence(blockId: InteractionBlockId): InteractionInfluenceClass {
  const values = Object.values(interactionInfluenceMatrix[blockId]);
  if (values.includes("OBSERVABLE_EFFECT")) return "OBSERVABLE_EFFECT";
  if (values.includes("TRANSITION")) return "TRANSITION";
  return "DECLARATION_ECHO";
}

export function aggregateRuntimeInfluence(blockId: InteractionBlockId): InteractionRuntimeInfluenceClass {
  const runtimeValues = Object.entries(interactionRuntimeInfluenceMatrix[blockId])
    ...
    .map(([, value]) => value);
  if (runtimeValues.length === 0) return "DECLARATION_ECHO";
  if (runtimeValues.includes("MODEL_ONLY_NOT_RUNTIME")) return "MODEL_ONLY_NOT_RUNTIME";
  if (runtimeValues.includes("ADAPTER_SEAM_NOT_RUN")) return "ADAPTER_SEAM_NOT_RUN";
  return "FRAMEWORK_OBSERVABLE";
}

export interface BoundedFrontendInteractionModel {
  readonly schemaVersion: "1.0";
  readonly profile: "bounded-frontend-interaction-v1";
  readonly projectTitle: string;
  readonly navigation: {
    readonly label: "主要导航";
    readonly fallback: "FIRST_DECLARED_ROUTE";
  ...
  readonly formBindingValidation: {
...
export type InteractionEvent = BoundedFrontendInteractionModel["actionEvent"]["acceptedEvents"][number];
...
export interface InteractionInput {
  ...
  readonly lifecycle: "MOUNT" | "ACTIVE" | "UNMOUNT";
  readonly query: string;
  readonly keyboardKey: "NONE" | "Enter";
  readonly authenticated: boolean;
  readonly permissionGranted: boolean;
  readonly tenantId: string;
  readonly resourceTenantId: string;
  readonly networkResult: "NONE" | "SUCCESS" | "ERROR" | "STALE";
  readonly hydration: "NONE" | "MATCH" | "MISMATCH";
  readonly locale: string;
  readonly theme: string;
  readonly viewportWidth: number;
  readonly nativeLifecycle: "FOREGROUND" | "BACKGROUND";
  readonly deepLinkPath: string | null;
  readonly nativePermission: "GRANTED" | "DENIED";
...
export type InteractionScenarioId = (typeof interactionScenarioIds)[number];

export interface InteractionScenario {
  readonly scenarioId: InteractionScenarioId;
  readonly input: InteractionInput;
}

export type InteractionBlockObservation = Readonly<Record<string, string | number | boolean | null>>;
...
export interface InteractionObservation {
  ...
  readonly blocks: Readonly<Record<InteractionBlockId, InteractionBlockObservation>>;
}

export interface InteractionSourceSpec {
  readonly sourcePath: string;
  readonly compatibilityPath: string;
  readonly parser: "TYPESCRIPT_AST" | "DART_BOUNDED_BASE64";
}

export function interactionSourceSpec(profile: UiFrameworkId): InteractionSourceSpec {
  switch (profile) {
    case "vue2":
...
function routeModel(request: UiProjectGenerationRequestV2): readonly BoundedNavigationRouteSource[] {
  ...
  return request.uiIr.routes.map(route => {
    ...
    if (!component) throw new Error(`interaction route component is missing: ${route.componentId}`);
    return {
      id: route.id,
      path: route.path,
...
export function canonicalBoundedFrontendInteractionModel(request: UiProjectGenerationRequestV2): BoundedFrontendInteractionModel {
  const routes = routeModel(request);
  if (routes.length === 0) throw new Error("bounded interaction requires at least one route");
...
export function boundedInteractionScenarios(model: BoundedFrontendInteractionModel): readonly InteractionScenario[] {
...
export function observeBoundedFrontendInteraction(
  model: BoundedFrontendInteractionModel,
  scenarioValue: InteractionScenario,
): InteractionObservation {
  const input = scenarioValue.input;
  const first = model.navigation.routes[0];
  if (!first) throw new Error("bounded interaction observation requires a route");
...
function tsContract(model: BoundedFrontendInteractionModel, javascript: boolean): string {
  ...
  const reducer = javascript ? rawReducer : rawReducer.replace(
    "function observeBoundedFrontendInteraction(model, scenarioValue)",
    "function observeBoundedFrontendInteraction(model: typeof ELMOS_FRONTEND_INTERACTION, scenarioValue: (typeof ELMOS_INTERACTION_SCENARIOS)[number])",
  );
  return [
...
function tsNavigationCompatibility(profile: UiFrameworkId): string {
...
function dartContract(model: BoundedFrontendInteractionModel): string {
...
function dartNavigationCompatibility(): string {
  return [
    "// Direct identity projection for bounded-navigation-v1 compatibility; no second route literal is permitted.",
...
export function interactionContractSource(profile: UiFrameworkId, model: BoundedFrontendInteractionModel): string {
  return profile === "flutter" ? dartContract(model) : tsContract(model, profile === "vue2");
}

export function navigationCompatibilitySource(profile: UiFrameworkId): string {
