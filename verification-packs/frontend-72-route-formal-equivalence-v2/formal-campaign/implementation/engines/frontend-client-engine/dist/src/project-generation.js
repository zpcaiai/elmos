...
import { recommendedExtensionProfiles, uiConversionRoute, uiConversionRoutes, uiTargetProfile, uiTargetProfiles, } from "./project-profiles.js";
import { componentForRoute, renderTargetProject } from "./project-templates.js";
import { attachBoundedNavigationSource } from "./bounded-navigation-source.js";
const frameworkIds = new Set(uiTargetProfiles().map(profile => profile.id));
...
function validateNodeArray(name, nodes, knownIds, allowEmpty) {
    ...
    for (const node of nodes) {
        ...
        if (knownIds.has(node.id))
            throw new Error(`duplicate UI IR id: ${node.id}`);
        knownIds.add(node.id);
    }
}
export function validateUiProjectGenerationRequest(value) {
    if (!value || typeof value !== "object")
        throw new Error("project request is required");
    if (value.schemaVersion !== "1.0")
        throw new Error("unsupported project request schemaVersion");
    ...
    if (!frameworkIds.has(value.targetFramework))
        throw new Error("target framework is unsupported");
    ...
    const sourceProfile = uiTargetProfile(value.source.framework);
    if (value.source.version !== sourceProfile.frameworkVersion) {
        throw new Error(`source.version must equal ${sourceProfile.frameworkVersion} for ${sourceProfile.id}`);
    }
    if (!sourceProfile.platforms.includes(value.source.platform)) {
        throw new Error(`source.platform is not supported by ${sourceProfile.id}`);
    }
        ...
        for (const node of group) {
            for (const reference of node.references) {
                if (!ids.has(reference))
                    throw new Error(`UI IR reference is unresolved: ${reference}`);
            }
        }
    }
    return value;
}
function canonicalBatch32UiIr(request) {
    const node = (value) => ({
        id: value.id,
        kind: value.kind,
        name: value.name,
...
function verificationScript(target, runtimeVersion) {
    if (target === "flutter") {
        return [
            "#!/usr/bin/env bash",
            "set -euo pipefail",
...
function workflow(target, runtimeVersion) {
...
function readme(request, obligations) {
    return [
        `# ${request.title}`,
        "",
...
function commonFiles(request, projectId, obligations) {
    ...
    return {
        ...
        ".github/workflows/generated-ui-quality.yml": workflow(request.targetFramework, profile.nodeVersion ?? profile.runtimeVersion),
    };
}
function obligations(request) {
    const result = [
...
export function generateUiProject(request) {
    ...
    const renderedTargetFiles = renderTargetProject({
        request: valid,
        profile,
        safeProjectName: valid.projectName,
        routes: renderedRoutes,
    });
    const targetFiles = attachBoundedNavigationSource(valid.targetFramework, {
        schemaVersion: "1.0",
        ...
        routes: renderedRoutes.map(item => ({
            id: item.id,
            path: item.path,
            title: item.title,
            text: item.text,
    ...
    for (const required of profile.requiredProjectFiles) {
        if (!(required in files))
            throw new Error(`target template omitted required file: ${required}`);
    }
...
export function uiProjectGenerationCapabilities() {
    return {
        schemaVersion: "1.0",
...
//# sourceMappingURL=project-generation.js.map