...
import { aggregateModelInfluence, aggregateRuntimeInfluence, boundedInteractionScenarios, canonicalBoundedFrontendInteractionModel, interactionBlockIds, interactionBlockSymbolMap, interactionContractSource, interactionInfluenceMatrix, interactionScenarioIds, interactionSourceSpec, navigationCompatibilitySource, observeBoundedFrontendInteraction, } from "./bounded-interaction-source.js";
import { augmentBoundedInteractionFiles, boundedFrontendBlockObserverContracts, boundedFrontendRuntimeActualKeys, boundedInteractionConsumerPaths, boundedInteractionFixtureRequest, boundedInteractionSourceFixtureBytes, generateBoundedInteractionProject, projectBoundedFrontendRuntimeObservation, reduceBoundedFrontendRuntime, requiredRuntimeChannels, } from "./bounded-interaction-project.js";
import { interactionRuntimeInfluenceMatrix } from "./bounded-interaction-source.js";
import { boundedNavigationGeneratedConsumerPaths, expectedBoundedNavigationConsumerFiles, frontendFormalDigest, runFrontendSolver, } from "./frontend-formal-equivalence.js";
...
function collectPointers(value, pointer = "", result = []) {
    result.push(pointer);
    if (Array.isArray(value))
        value.forEach((item, index) => collectPointers(item, `${pointer}/${index}`, result));
    else if (value !== null && typeof value === "object") {
        for (const key of Object.keys(value).sort(codePointCompare)) {
            collectPointers(value[key], `${pointer}/${pointerEscape(key)}`, result);
        }
    }
    return result;
}
function resolvePointer(value, pointer) {
    ...
    for (const raw of pointer.slice(1).split("/")) {
        ...
        if (Array.isArray(current))
            ...
            throw new Error(`unresolved RFC6901 pointer: ${pointer}`);
    }
    return current;
}
...
function parseLiteral(node, source, sourcePath, pointer, spans) {
    ...
    if (ts.isNumericLiteral(expression))
        return record(Number(expression.text));
    ...
    if (ts.isArrayLiteralExpression(expression)) {
        return record(expression.elements.map((element, index) => {
            if (ts.isSpreadElement(element))
                throw new Error("spread is outside bounded interaction grammar");
    ...
    if (ts.isObjectLiteralExpression(expression)) {
        ...
        for (const member of expression.properties) {
            if (!ts.isPropertyAssignment(member))
                throw new Error("non-property member is outside bounded interaction grammar");
            const key = propertyName(member.name);
            if (Object.hasOwn(result, key))
                throw new Error(`duplicate bounded interaction property: ${key}`);
...
function validateModel(value, profile) {
    if (!value || typeof value !== "object" || Array.isArray(value))
        throw new Error("bounded interaction model must be an object");
    const model = value;
    ...
    if (Object.keys(value).sort(codePointCompare).join("|") !== rootKeys.join("|")
        ...
        || typeof model.projectTitle !== "string" || model.projectTitle.length === 0)
        throw new Error("bounded interaction root identity/shape drifted");
    if (!Array.isArray(model.navigation.routes) || model.navigation.routes.length === 0)
        throw new Error("bounded interaction routes are required");
    ...
    for (const [index, route] of model.navigation.routes.entries()) {
        if (!route || Object.keys(route).sort(codePointCompare).join("|") !== "deepLink|id|path|requiresAuth|text|title"
            ...
            || typeof route.requiresAuth !== "boolean" || typeof route.deepLink !== "boolean")
            throw new Error(`bounded interaction route ${index} drifted`);
        if (routeIds.has(route.id) || routePaths.has(route.path))
            throw new Error("bounded interaction route identity is duplicated");
    ...
    for (const key of Object.values(blockModelKeys).filter(key => key !== "navigation")) {
        if (canonical(model[key]) !== canonical(baseline[key]))
            throw new Error(`bounded interaction ${String(key)} profile drifted`);
    }
    if (model.navigation.label !== baseline.navigation.label || model.navigation.fallback !== baseline.navigation.fallback)
        throw new Error("bounded interaction navigation profile drifted");
    return model;
}
function tsRelift(profile, path, text) {
    const kind = path.endsWith(".js") ? ts.ScriptKind.JS : ts.ScriptKind.TS;
    const source = ts.createSourceFile(path, text, ts.ScriptTarget.Latest, true, kind);
    const diagnostics = source.parseDiagnostics ?? [];
    if (diagnostics.length > 0)
        throw new Error(`interaction source parse failed for ${profile}`);
    let initializer;
    for (const statement of source.statements) {
        if (!ts.isVariableStatement(statement))
            continue;
        for (const declaration of statement.declarationList.declarations) {
            if (ts.isIdentifier(declaration.name) && declaration.name.text === "ELMOS_FRONTEND_INTERACTION")
                initializer = declaration.initializer;
        }
    }
    if (!initializer)
        throw new Error("ELMOS_FRONTEND_INTERACTION declaration is missing");
    const spans = {};
    const model = validateModel(parseLiteral(initializer, source, path, "", spans), profile);
    return { model, spans };
}
function dartRelift(profile, path, text) {
    const match = /const String elmosFrontendInteractionBase64 = "([A-Za-z0-9+/=]+)";/.exec(text);
    if (!match || match.index < 0 || !match[1])
        throw new Error("Dart bounded interaction payload is missing");
    const decoded = Buffer.from(match[1], "base64").toString("utf8");
    if (Buffer.from(decoded, "utf8").toString("base64") !== match[1])
        throw new Error("Dart bounded interaction payload is not canonical base64");
...
function navigationProjection(model) {
    return {
...
export function reliftBoundedInteractionProject(profile, files) {
    const spec = interactionSourceSpec(profile);
    const text = files[spec.sourcePath];
    if (text === undefined)
        throw new Error(`interaction source is missing: ${spec.sourcePath}`);
    const parsed = spec.parser === "TYPESCRIPT_AST" ? tsRelift(profile, spec.sourcePath, text) : dartRelift(profile, spec.sourcePath, text);
    if (text !== interactionContractSource(profile, parsed.model))
        throw new Error("bounded interaction reducer/source grammar drifted");
    if (files[spec.compatibilityPath] !== navigationCompatibilitySource(profile))
        throw new Error("bounded navigation compatibility is not the direct identity projection");
    ...
    for (const path of paths) {
        if (files[path] === undefined || files[path] !== expected[path])
            throw new Error(`reachable bounded interaction consumer grammar drifted: ${profile}:${path}`);
    }
    const blockDigests = Object.fromEntries(interactionBlockIds.map(blockId => [blockId, frontendFormalDigest(parsed.model[blockModelKeys[blockId]])]));
    return {
...
export function referenceObserveBoundedInteraction(model, scenario) {
    const input = scenario.input;
    const routes = [...model.navigation.routes];
    const first = routes.at(0);
    if (!first)
        throw new Error("reference reducer requires a route");
    ...
    const allowed = (route) => !route.requiresAuth
        || (input.authenticated && input.permissionGranted && tenantMatch);
    const authorized = allowed(requested);
    ...
    const locale = new Set(model.i18nThemeResponsive.supportedLocales).has(input.locale)
        ? input.locale : model.i18nThemeResponsive.fallbackLocale;
    const theme = new Set(model.i18nThemeResponsive.themes).has(input.theme)
    ...
    const nativeLifecycleKnown = new Set(model.nativePlatform.lifecycleStates).has(input.nativeLifecycle);
    ...
    return {
        ...
        blocks: {
            ...
            "action-event": { event: input.event, keyboardKey: input.keyboardKey, handled: new Set(model.actionEvent.acceptedEvents).has(input.event), action: submitted ? (valid && authorized ? "SUBMIT_ACCEPTED" : model.actionEvent.deniedAction) : input.event },
            ...
            "i18n-theme-responsive": { requestedLocale: input.locale, localeSupported: new Set(model.i18nThemeResponsive.supportedLocales).has(input.locale), locale, requestedTheme: input.theme, themeSupported: new Set(model.i18nThemeResponsive.themes).has(input.theme), theme, viewportWidth: input.viewportWidth, columns: input.viewportWidth <= model.i18nThemeResponsive.compactBreakpoint ? model.i18nThemeResponsive.compactColumns : model.i18nThemeResponsive.wideColumns },
...
function behaviorComparable(values) {
    return values;
}
export function observeInteractionScenarioSet(model, kind) {
...
function smtString(value) {
    return `"${value.replaceAll('"', '""')}"`;
}
function smtLiteral(value, sort) {
    if (sort === "String")
        return smtString(String(value));
    if (sort === "Bool")
        return value ? "true" : "false";
    return String(value);
}
function routeSelector(model, field, sort) {
...
function interactionSymbolicOutputs(model, blockId) {
    ...
    const select = (requested, fallback) => `(ite ${authorized} ${requested} ${fallback})`;
    ...
    switch (blockId) {
        ...
        case "i18n-theme-responsive": {
            ...
            for (const value of [...model.i18nThemeResponsive.supportedLocales].reverse())
                locale = `(ite (= locale_input ${smtString(value)}) ${smtString(value)} ${locale})`;
            let theme = smtString(model.i18nThemeResponsive.defaultTheme);
            for (const value of [...model.i18nThemeResponsive.themes].reverse())
                theme = `(ite (= theme_input ${smtString(value)}) ${smtString(value)} ${theme})`;
            return [
...
export function buildInteractionSmt2(canonicalModel, sourceModel, targetModel, referenceModel, formalInputDigest = "UNBOUND_FORMAL_INPUT") {
    const models = [canonicalModel, sourceModel, targetModel, referenceModel];
    const prefixes = ["canonical", "source", "target", "reference"];
    const lines = [
    ...
    for (const blockId of interactionBlockIds) {
        ...
        if (new Set(outputs[0].map(output => output.name)).size !== outputs[0].length)
            throw new Error(`duplicate SMT output name for ${blockId}`);
        const behaviorDiffs = [];
        for (const [index, output] of outputs[0].entries()) {
            for (const [modelIndex, prefix] of prefixes.entries()) {
                ...
                if (current.name !== output.name || current.sort !== output.sort)
                    throw new Error(`SMT output shape diverged for ${blockId}`);
...
export function buildInteractionVacuitySmt2(formalInputDigest = "UNBOUND_FORMAL_INPUT") {
    const lines = [
...
function primitiveLeafPointers(value) {
    return collectPointers(value).filter(pointer => {
        if (pointer === "" || pointer === "/schemaVersion" || pointer === "/profile" || pointer === "/projectTitle")
            return false;
        const item = resolvePointer(value, pointer);
        return item === null || typeof item !== "object";
    });
}
function alternateLeaf(pointer, value) {
    ...
    if (typeof value === "string") {
        if (pointer.endsWith("/path"))
            return value === "/" ? "/mutant" : `${value}-mutant`;
        if (value === "LIGHT")
            return "DARK";
        if (value === "DARK")
            return "LIGHT";
...
export function mutateInteractionModelAtPointer(model, pointer) {
    if (!primitiveLeafPointers(model).includes(pointer))
        throw new Error(`interaction mutation pointer is not a primitive semantic leaf: ${pointer}`);
    ...
    if (Array.isArray(current))
        current[Number(leaf)] = alternateLeaf(pointer, current[Number(leaf)]);
    else
        current[leaf] = alternateLeaf(pointer, current[leaf]);
    return clone;
}
function pointerBlock(pointer) {
    if (/^\/navigation\/routes\/[0-9]+\/(?:title|text)$/.test(pointer))
        return "component-template-view";
...
const mutationVariantKinds = ["SOURCE_ONLY", "TARGET_ONLY", "REFERENCE_ONLY"];
function selectMutationWitness(counterexamples, blockId) {
    ...
    if (!candidate)
        throw new Error(`bounded interaction block lacks an owned behavior mutant witness: ${blockId}`);
    if (pointerBlock(candidate.pointer) !== blockId)
        throw new Error(`bounded interaction mutation pointer is outside its block: ${blockId}`);
    return candidate;
}
function mutationModels(canonicalModel, mutant, variant) {
    if (variant === "SOURCE_ONLY")
        return [canonicalModel, mutant, canonicalModel, canonicalModel];
    if (variant === "TARGET_ONLY")
        return [canonicalModel, canonicalModel, mutant, canonicalModel];
    return [canonicalModel, canonicalModel, canonicalModel, mutant];
}
function mutationFormalInput(canonicalModel, mutant, blockId, variant, candidate) {
    return {
        schema_version: "1.0",
...
export function interactionLeafCounterexamples(model) {
    const scenarios = boundedInteractionScenarios(model);
...
function makeInteractionBundles(options) {
    ...
    for (const profile of uiTargetProfiles()) {
        ...
        if (options.tamper?.profile_id === profile.id) {
            ...
            if (current === undefined || !current.includes(options.tamper.find))
                throw new Error("requested interaction tamper target was not found");
...
function materializeProject(root, files) {
    for (const [path, content] of Object.entries(files)) {
...
function runtimeDriverContract(profile, model) {
...
function routeRuntimeChannels(source, target) {
    const present = new Set([...requiredRuntimeChannels(source), ...requiredRuntimeChannels(target)]);
    return ["browser", "android", "ios", "harmonyos"].filter(channel => present.has(channel));
}
function routeChunks(source, target) {
    ...
    if (sourcePointers.join("|") !== targetPointers.join("|"))
        throw new Error("interaction semantic pointer sets diverged");
...
function blockBehaviorDigest(observations, blockId) {
...
function solverResultWithLinks(solver, routeId, formalInputDigest, solverInputDigest) {
...
export function materializeFrontendInteractionCampaign(outputDirectory, options = {}) {
    const output = resolve(outputDirectory);
    if (existsSync(output) && readdirSync(output).length > 0)
        throw new Error("frontend interaction output directory must be absent or empty");
    mkdirSync(output, { recursive: true });
    const bundles = makeInteractionBundles(options);
    const firstBundle = bundles.values().next().value;
    if (!firstBundle)
        throw new Error("interaction profile bundles are empty");
    ...
    for (const blockId of interactionBlockIds) {
        ...
        const variants = mutationVariantKinds.map(variant => {
            ...
            const smtPath = `${root}/proof.smt2`;
            const solverPath = `${root}/solver-result.json`;
            ...
            writeArtifact(output, formalInputPath, formalInput);
            writeArtifact(output, smtPath, smt2);
            writeArtifact(output, solverPath, solverArtifact);
    ...
    for (const route of uiConversionRoutes()) {
        ...
        const solver = runFrontendSolver(smt2, options.solver);
        const vacuitySolver = runFrontendSolver(vacuitySmt2, options.solver);
        const smtDigest = bytesDigest(smt2);
        const vacuityDigest = bytesDigest(vacuitySmt2);
        ...
        const vacuitySolverResultDigest = artifactDigest(vacuitySolverResult);
        const status = !semanticEqual || !behaviorEqual || !chunkEqual ? "REFUTED"
        ...
        const blockResults = interactionBlockIds.map(blockId => {
            ...
            const blockStatus = status === "PROVED_UNDER_ASSUMPTIONS" && semanticMutantDetected && behaviorMutantDetected && seededMutationPassed
        ...
        writeArtifact(output, fixedPaths.source_model_path, source.relift);
        writeArtifact(output, fixedPaths.target_model_path, target.relift);
        writeArtifact(output, fixedPaths.behavior_path, behavior);
        writeArtifact(output, fixedPaths.chunks_path, chunkArtifact);
        writeArtifact(output, fixedPaths.formal_input_path, formalInput);
        writeArtifact(output, fixedPaths.smt2_path, smt2);
        writeArtifact(output, fixedPaths.solver_result_path, solverResult);
        writeArtifact(output, fixedPaths.vacuity_input_path, vacuitySmt2);
        writeArtifact(output, fixedPaths.vacuity_solver_result_path, vacuitySolverResult);
        writeArtifact(output, fixedPaths.block_results_path, blockArtifact);
        writeArtifact(output, fixedPaths.composition_path, composition);
        const layeredDigest = writeArtifact(output, fixedPaths.layered_result_path, layered);
        routeRecords.push({
    ...
    const blockCounts = Object.fromEntries(interactionBlockIds.map(blockId => [blockId, Object.fromEntries(["PROVED_UNDER_ASSUMPTIONS", "REFUTED", "NOT_PROVED"].map(status => [status, routeRecords.filter(route => {
                const artifact = JSON.parse(readFileSync(join(output, ...route.block_results_path.split("/")), "utf8"));
...
function exactObjectKeys(value, expected, name) {
    ...
    if (actual.join("|") !== wanted.join("|"))
        throw new Error(`${name} exact keys drifted`);
}
function safeCampaignFile(root, relativePath, name) {
    if (typeof relativePath !== "string")
        throw new Error(`${name} path is not a string`);
    assertSafeRelative(relativePath);
    const destination = resolve(root, ...relativePath.split("/"));
    if (!destination.startsWith(`${root}/`))
        throw new Error(`${name} path escapes campaign root`);
    const metadata = lstatSync(destination);
    if (metadata.isSymbolicLink() || !metadata.isFile())
        throw new Error(`${name} is not a regular artifact file`);
    return destination;
}
function filesBelow(root, prefix = "") {
    ...
    for (const name of readdirSync(join(root, ...prefix.split("/").filter(Boolean))).sort(codePointCompare)) {
        ...
        if (stat.isSymbolicLink())
            throw new Error(`symbolic link is forbidden in project bundle: ${path}`);
        if (stat.isDirectory())
            result.push(...filesBelow(root, path));
        else if (stat.isFile())
            result.push(path);
        else
            throw new Error(`non-regular project entry: ${path}`);
    }
    return result;
}
function readJsonArtifact(root, path, digestValue, name) {
    const bytes = readFileSync(safeCampaignFile(root, path, name), "utf8");
    if (bytesDigest(bytes) !== digestValue)
        throw new Error(`${name} bytes digest drifted`);
    const value = JSON.parse(bytes);
    if (artifactBytes(value) !== bytes)
        throw new Error(`${name} JSON bytes are non-canonical`);
    return { bytes, value };
}
function verifySolverArtifact(value, smt2, formalInputDigest, smtDigest, expectedOutcome, solverOverride, replayCache) {
    if (value.formal_input_digest !== formalInputDigest || value.solver_input_digest !== smtDigest || value.smt2_digest !== smtDigest
        || value.unconditional_proof !== false)
        throw new Error("solver linkage drifted");
    if (expectedOutcome !== undefined && value.outcome !== expectedOutcome)
        throw new Error(`solver outcome is not ${expectedOutcome}`);
    if (value.identity_status === "VERIFIED") {
        ...
        if (typeof recordedRealpath !== "string" || resolve(recordedRealpath) !== recordedRealpath
            ...
            || canonical(value.invocation) !== canonical([recordedRealpath, "-in"]))
            throw new Error("verified solver recorded path/invocation is malformed");
        const recordedOptions = value.options;
        if (!recordedOptions || canonical(recordedOptions.args) !== canonical(["-in"])
            ...
            || recordedOptions.timeout_ms <= 0)
            throw new Error("verified solver recorded options are malformed");
        ...
        try {
            resolvedCommand = realpathSync(resolvedCommand);
        }
        catch { /* runFrontendSolver emits the fail-closed missing identity */ }
        ...
        if (replay.identity_status !== "VERIFIED")
            throw new Error("solver override does not match the locked solver identity");
        ...
        const coreKeys = Object.keys(normalizedReplay);
        const recordedCore = Object.fromEntries(coreKeys.map(key => [key, value[key]]));
        if (canonical(recordedCore) !== canonical(normalizedReplay))
            throw new Error("solver replay or locked identity/options/output diverged");
        replayCache?.set(replayKey, replay);
        return normalizedReplay;
    }
    else if (value.outcome === "UNSAT" || value.outcome === "SAT")
        throw new Error("unverified solver cannot support a solver decision");
    return undefined;
}
export function verifyFrontendInteractionCampaign(outputDirectory, options = {}) {
    const errors = [];
    const output = resolve(outputDirectory);
    const solverReplayCache = new Map();
    try {
        ...
        if (artifactBytes(campaign) !== campaignBytes)
            throw new Error("campaign JSON bytes are non-canonical");
        ...
        if (campaign.schema_version !== "1.0" || campaign.kind !== "frontend-interaction-formal-route-campaign"
            ...
            || campaign.certification !== "NOT_CERTIFIED" || canonical(campaign.assumptions) !== canonical(proofAssumptions))
            throw new Error("campaign identity/boundary drifted");
        const canonicalModel = canonicalBoundedFrontendInteractionModel(boundedInteractionFixtureRequest("react"));
        const scenarioManifest = campaign.scenario_manifest;
        ...
        if (canonical(scenarioArtifact.value) !== canonical(expectedScenario) || scenarioManifest.source_byte_count !== Buffer.byteLength(scenarioArtifact.bytes, "utf8")
            || canonical(scenarioManifest.scenario_ids) !== canonical(interactionScenarioIds) || scenarioManifest.scenario_count !== interactionScenarioIds.length)
            throw new Error("scenario corpus/manifest drifted");
        const mutationLink = campaign.mutation_campaign;
        ...
        if (mutationArtifact.schema_version !== "1.0" || mutationArtifact.kind !== "bounded-interaction-seeded-mutation-campaign"
            || mutationArtifact.proof_profile !== "bounded-frontend-interaction-v1" || mutationArtifact.status !== "PASSED" || mutationLink.status !== "PASSED")
            throw new Error("mutation campaign identity/status drifted");
        const mutations = mutationArtifact.mutations;
        if (!Array.isArray(mutations) || mutations.length !== interactionBlockIds.length)
            throw new Error("mutation campaign block closure drifted");
        const mutationBlockIds = mutations.map(item => item.block_id);
        if (new Set(mutationBlockIds).size !== interactionBlockIds.length
            || interactionBlockIds.some(blockId => !mutationBlockIds.includes(blockId)))
            throw new Error("mutation campaign block identity is duplicated or incomplete");
        const expectedCounterexamples = interactionLeafCounterexamples(canonicalModel);
        for (const blockId of interactionBlockIds) {
            const rows = mutations.filter(item => item.block_id === blockId);
            if (rows.length !== 1)
                throw new Error(`mutation row identity is not unique: ${blockId}`);
            const row = rows[0];
            ...
            if (pointerBlock(pointer) !== blockId || pointer !== candidate.pointer
                ...
                || canonical(row.counterexample_replay) !== canonical(candidate))
                throw new Error(`mutation pointer/witness/symbol drifted: ${blockId}`);
            ...
            if (!Array.isArray(variants) || variants.length !== 3)
                throw new Error(`mutation variants drifted: ${blockId}`);
            const actualKinds = variants.map(value => value.variant);
            if (new Set(actualKinds).size !== mutationVariantKinds.length
                || mutationVariantKinds.some(kind => !actualKinds.includes(kind)))
                throw new Error(`mutation variant identity is duplicated or incomplete: ${blockId}`);
            for (const kind of mutationVariantKinds) {
                const matching = variants.filter(value => value.variant === kind);
                if (matching.length !== 1)
                    throw new Error(`mutation variant is not unique: ${blockId}.${kind}`);
                ...
                if (variant.formal_input_path !== `${variantRoot}/formal-input.json` || variant.smt2_path !== `${variantRoot}/proof.smt2`
                    || variant.solver_result_path !== `${variantRoot}/solver-result.json`)
                    throw new Error(`mutation variant path drifted: ${blockId}.${kind}`);
                ...
                if (canonical(formal) !== canonical(expectedFormal) || variant.formal_input_digest !== artifactDigest(expectedFormal))
                    throw new Error(`mutation formal input reconstruction drifted: ${blockId}.${String(kind)}`);
                const smtPath = safeCampaignFile(output, variant.smt2_path, `${blockId}.${String(kind)}.smt`);
                const smt2 = readFileSync(smtPath, "utf8");
                if (bytesDigest(smt2) !== variant.smt2_digest)
                    throw new Error(`mutation SMT digest drifted: ${blockId}.${String(kind)}`);
                const models = mutationModels(canonicalModel, mutant, kind);
                if (smt2 !== buildInteractionSmt2(...models, String(variant.formal_input_digest)))
                    throw new Error(`mutation SMT reconstruction drifted: ${blockId}.${String(kind)}`);
                ...
                if (!replay)
                    throw new Error(`mutation solver is not replayable: ${blockId}.${kind}`);
                ...
                if (canonical(solver) !== canonical(expectedSolver) || variant.solver_result_digest !== artifactDigest(expectedSolver)
                    || variant.solver_outcome !== "SAT" || variant.replay_status !== "PASSED")
                    throw new Error(`mutation solver/result/status reconstruction drifted: ${blockId}.${kind}`);
            }
            const expectedRowStatus = mutationVariantKinds.every(kind => {
                const variant = variants.find(value => value.variant === kind);
                return variant.solver_outcome === "SAT" && variant.replay_status === "PASSED";
            }) ? "REFUTED_AS_EXPECTED" : "NOT_PROVED";
            if (row.status !== expectedRowStatus)
                throw new Error(`mutation row status drifted: ${blockId}`);
        }
        const verifiedProfiles = new Map();
        const profiles = campaign.profiles;
        if (!Array.isArray(profiles) || profiles.length !== 9)
            throw new Error("campaign profile closure is not nine");
        for (const raw of profiles) {
            ...
            try {
                ...
                if (verifiedProfiles.has(id) || !uiTargetProfiles().some(candidate => candidate.id === id))
                    throw new Error("profile id is invalid or duplicated");
                const exact = uiTargetProfile(id);
                const request = boundedInteractionFixtureRequest(id);
                if (profile.framework_version !== exact.frameworkVersion || canonical(profile.platforms) !== canonical(exact.platforms)
                    ...
                    || profile.source_kind !== "GENERATED_FIXTURE" || profile.target_build !== "NOT_RUN" || profile.target_runtime !== "NOT_RUN")
                    throw new Error("exact profile tuple/path/boundary drifted");
                ...
                if (sourceBytes !== boundedInteractionSourceFixtureBytes(request) || bytesDigest(sourceBytes) !== profile.source_fixture_digest
                    ...
                    || request.uiIr.sourceSnapshotDigest !== profile.source_fixture_digest)
                    throw new Error("generated source fixture provenance drifted");
                ...
                const computedDigest = projectDigest(diskFiles);
                if (computedDigest !== profile.project_digest)
                    throw new Error("profile project digest drifted");
                const relift = reliftBoundedInteractionProject(id, diskFiles);
                if (relift.model_digest !== profile.relift_model_digest || canonical(relift.block_digests) !== canonical(profile.relift_block_digests))
                    throw new Error("profile relift digests drifted");
                const expectedDriver = runtimeDriverContract(id, relift.model);
                if (canonical(profile.runtime_driver_contract) !== canonical(expectedDriver))
                    throw new Error("runtime driver/projection contract drifted");
                ...
                const manifestDigest = manifestArtifact.manifest_digest;
                const { manifest_digest: _ignored, ...manifestBase } = manifestArtifact;
                if (frontendFormalDigest(manifestBase) !== manifestDigest || manifestDigest !== profile.manifest_digest
                    ...
                    || canonical(manifestArtifact.runtime_driver_contract) !== canonical(expectedDriver))
                    throw new Error("profile manifest linkage drifted");
        ...
        if (verifiedProfiles.size !== 9)
            throw new Error("verified profile closure is incomplete");
        const routes = campaign.routes;
        if (!Array.isArray(routes) || routes.length !== 72)
            throw new Error("campaign route closure is not 72");
        ...
        for (const raw of routes) {
            ...
            try {
                ...
                const sourceId = route.source_profile;
                const targetId = route.target_profile;
                if (routeId !== `${sourceId}--to--${targetId}` || sourceId === targetId || seenRoutes.has(routeId))
                    throw new Error("route identity is invalid or duplicated");
                seenRoutes.add(routeId);
                const source = verifiedProfiles.get(sourceId);
                const target = verifiedProfiles.get(targetId);
                if (!source || !target || route.source_project_digest !== source.project_digest || route.target_project_digest !== target.project_digest)
                    throw new Error("route project links drifted");
                ...
                if (canonical(sourceModelArtifact) !== canonical(source.relift) || canonical(targetModelArtifact) !== canonical(target.relift))
                    throw new Error("route source/target model artifact drifted");
                ...
                if (canonical(behaviorArtifact) !== canonical(expectedBehavior) || route.behavior_path !== fixedPaths.behavior_path
                    || route.behavior_digest !== artifactDigest(expectedBehavior))
                    throw new Error("route behavior evidence reconstruction drifted");
                ...
                if (canonical(chunkArtifact) !== canonical(expectedChunkArtifact) || route.chunks_path !== fixedPaths.chunks_path
                    || route.chunks_digest !== artifactDigest(expectedChunkArtifact))
                    throw new Error("route chunk evidence reconstruction drifted");
                ...
                if (canonical(formal) !== canonical(expectedFormal) || route.formal_input_path !== fixedPaths.formal_input_path
                    || route.formal_input_digest !== expectedFormalDigest)
                    throw new Error("route formal input reconstruction drifted");
                ...
                if (route.solver_input_path !== fixedPaths.smt2_path || bytesDigest(smt2) !== route.solver_input_digest
                    || smt2 !== expectedSmt2 || route.solver_input_digest !== expectedSmtDigest)
                    throw new Error("route symbolic SMT reconstruction drifted");
                ...
                if (!solverReplay)
                    throw new Error("route solver result is not replayable");
                ...
                if (canonical(solver) !== canonical(expectedSolver) || route.solver_result_path !== fixedPaths.solver_result_path
                    || route.solver_result_digest !== expectedSolverDigest)
                    throw new Error("route solver result reconstruction drifted");
                const vacuity = readFileSync(safeCampaignFile(output, route.vacuity_input_path, `${routeId}.vacuity`), "utf8");
                const expectedVacuity = buildInteractionVacuitySmt2(expectedFormalDigest);
                const expectedVacuityDigest = bytesDigest(expectedVacuity);
                if (route.vacuity_input_path !== fixedPaths.vacuity_input_path || bytesDigest(vacuity) !== route.vacuity_input_digest
                    || vacuity !== expectedVacuity || route.vacuity_input_digest !== expectedVacuityDigest)
                    throw new Error("route vacuity SMT reconstruction drifted");
                ...
                if (!vacuityReplay)
                    throw new Error("route vacuity solver result is not replayable");
                ...
                if (canonical(vacuitySolver) !== canonical(expectedVacuitySolver) || route.vacuity_solver_result_path !== fixedPaths.vacuity_solver_result_path
                    || route.vacuity_solver_result_digest !== expectedVacuitySolverDigest)
                    throw new Error("route vacuity solver result reconstruction drifted");
                const expectedStatus = !semanticEqual || !behaviorEqual || !chunkEqual ? "REFUTED"
                ...
                const expectedBlockResults = interactionBlockIds.map(blockId => {
                    ...
                    const blockBehaviorEqual = blockBehaviorDigest(canonicalObservations, blockId) === blockBehaviorDigest(referenceObservations, blockId)
                        && blockBehaviorDigest(canonicalObservations, blockId) === blockBehaviorDigest(sourceObservations, blockId)
                        && blockBehaviorDigest(canonicalObservations, blockId) === blockBehaviorDigest(targetObservations, blockId);
                    const seededMutationPassed = mutations.find(row => row.block_id === blockId)?.status === "REFUTED_AS_EXPECTED";
                    const blockStatus = expectedStatus === "PROVED_UNDER_ASSUMPTIONS" && semanticMutantDetected && behaviorMutantDetected && seededMutationPassed
                ...
                if (canonical(blockArtifact) !== canonical(expectedBlockArtifact))
                    throw new Error("route block results reconstruction drifted");
                if (canonical(composition) !== canonical(expectedComposition))
                    throw new Error("route composition reconstruction drifted");
                if (canonical(layered) !== canonical(expectedLayered))
                    throw new Error("route layered result reconstruction drifted");
                if (canonical(route) !== canonical(expectedRoute))
                    throw new Error("campaign route row reconstruction drifted");
            }
            catch (error) {
                errors.push(`${routeId}: ${error instanceof Error ? error.message : String(error)}`);
            }
        }
        if (seenRoutes.size !== 72 || uiConversionRoutes().some(route => !seenRoutes.has(route.routeId)))
            errors.push("campaign directed route closure is incomplete");
        const expectedCounts = Object.fromEntries(["PROVED_UNDER_ASSUMPTIONS", "REFUTED", "NOT_PROVED"].map(status => [status, routes.filter(route => route.status === status).length]));
        if (canonical(campaign.counts) !== canonical(expectedCounts))
            errors.push("campaign route counts drifted");
        if (canonical(campaign.block_counts) !== canonical(expectedBlockCounts))
            errors.push("campaign block counts drifted");
    }
    catch (error) {
...
//# sourceMappingURL=frontend-interaction-formal-equivalence.js.map