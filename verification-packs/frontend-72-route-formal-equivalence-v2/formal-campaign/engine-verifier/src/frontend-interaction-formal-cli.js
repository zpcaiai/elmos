#!/usr/bin/env node
import { materializeFrontendInteractionCampaign, verifyFrontendInteractionCampaign, } from "./frontend-interaction-formal-equivalence.js";
...
function main() {
    ...
    for (let index = 0; index < args.length; index += 1) {
        ...
        if (!allowed.has(value))
            throw new Error(`unknown frontend interaction option: ${value}`);
        if (value !== "--json")
            index += 1;
    }
    ...
    if (!Array.isArray(routes) || routes.some(route => route.status !== "PROVED_UNDER_ASSUMPTIONS"))
        process.exitCode = 2;
}
try {
    main();
...
//# sourceMappingURL=frontend-interaction-formal-cli.js.map