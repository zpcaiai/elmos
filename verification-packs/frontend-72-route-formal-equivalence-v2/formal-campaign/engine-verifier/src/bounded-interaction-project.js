...
import { boundedInteractionScenarios, canonicalBoundedFrontendInteractionModel, interactionBlockIds, interactionContractSource, interactionSourceSpec, navigationCompatibilitySource, } from "./bounded-interaction-source.js";
...
function validateExactBlock(name, value, fields, expected) {
    ...
    if (canonical(semantic) !== canonical(expected))
        throw new Error(`${name} literal contract drifted`);
}
function blockBindings(request) {
 ...
 */
export const boundedFrontendBlockObserverContracts = {
 ...
 */
export function reduceBoundedFrontendRuntime(model, scenario) {
    const input = scenario.input;
    const routes = model.navigation.routes.slice();
    const first = routes[0];
    if (!first)
        throw new Error("runtime interaction reducer requires a route");
    let requested = first;
    for (const route of routes)
        if (route.path === input.routePath)
            requested = route;
    const exactTenant = model.identityPermission.tenantIsolation === "EXACT_TENANT_MATCH"
        && input.tenantId === input.resourceTenantId;
    const canOpen = (route) => !route.requiresAuth
    ...
    if (input.deepLinkPath !== null) {
        nativeTarget = first;
        for (const route of routes)
            if (route.path === input.deepLinkPath)
                nativeTarget = route;
    }
...
export function projectBoundedFrontendRuntimeObservation(observation, channel) {
...
function nodeFromBinding(name, value) {
    return { id: value.id, name, kind: `bounded-interaction-${name}`, references: value.references, sourceRefs: value.sourceRefs };
}
export function interactionV2ToV1Request(request) {
    const byName = Object.fromEntries(blockBindings(request).map(([name, value]) => [name, nodeFromBinding(name, value)]));
    return {
        schemaVersion: "1.0",
...
export function validateUiProjectGenerationRequestV2(request) {
    if (!request || typeof request !== "object" || request.schemaVersion !== "2.0")
        throw new Error("interaction request schemaVersion 2.0 is required");
    ...
    if (!sha256.test(request.uiIr.sourceSnapshotDigest))
        throw new Error("uiIr v2 sourceSnapshotDigest is invalid");
    ...
    for (const [index, view] of request.uiIr.views.entries())
        exactKeys(view, ["id", "name", "kind", "references", "sourceRefs"], `uiIr.views[${index}]`);
    ...
    const ir = request.uiIr;
    validateExactBlock("componentTemplate", ir.componentTemplate, ["componentId", "templateKind", "keyedBy", "titleBinding", "textBinding"], { componentId: "interaction.shell", templateKind: "ROUTE_DETAIL_WITH_INTERACTION_MATRIX", keyedBy: "route.id", titleBinding: "route.title", textBinding: "route.text" });
    validateExactBlock("stateManagement", ir.stateManagement, ["stateId", "initial", "minimum", "maximum", "transition"], { stateId: "bounded.counter", initial: 0, minimum: 0, maximum: 2, transition: "SATURATING_INCREMENT" });
    validateExactBlock("actionEvent", ir.actionEvent, ["acceptedEvents", "deniedAction", "keyboardSubmit"], { acceptedEvents: ["BOOT", "NAVIGATE", "AUTHENTICATE", "SUBMIT", "CANCEL", "HYDRATE", "DISPLAY_CHANGE", "NATIVE_DEEPLINK"], deniedAction: "BLOCK", keyboardSubmit: "Enter" });
    validateExactBlock("effectLifecycle", ir.effectLifecycle, ["mountEffect", "cleanupEffect", "maxExecutionsPerMount", "staleResponsePolicy"], { mountEffect: "LOAD_ON_MOUNT", cleanupEffect: "CANCEL_ON_UNMOUNT", maxExecutionsPerMount: 1, staleResponsePolicy: "IGNORE_AFTER_CANCEL" });
    validateExactBlock("formBindingValidation", ir.formBindingValidation, ["formId", "fieldId", "initialValue", "required", "minimumLength", "validation", "invalidCode"], { formId: "search", fieldId: "query", initialValue: "", required: true, minimumLength: 2, validation: "ON_SUBMIT", invalidCode: "QUERY_TOO_SHORT" });
    validateExactBlock("apiNetwork", ir.apiNetwork, ["operationId", "method", "path", "timeoutMs", "retry", "cacheScope", "cancelOnUnmount"], { operationId: "search", method: "POST", path: "/api/search", timeoutMs: 1000, retry: "NEVER", cacheScope: "TENANT_QUERY", cancelOnUnmount: true });
    validateExactBlock("identityPermission", ir.identityPermission, ["anonymousRole", "authenticatedRole", "requiredPermission", "deniedBehavior", "tenantIsolation", "serverAuthorityRequired"], { anonymousRole: "ANONYMOUS", authenticatedRole: "MEMBER", requiredPermission: "search:execute", deniedBehavior: "HIDE_AND_BLOCK", tenantIsolation: "EXACT_TENANT_MATCH", serverAuthorityRequired: true });
    validateExactBlock("renderingHydration", ir.renderingHydration, ["mode", "hydrationPolicy", "mismatchBehavior", "duplicateEffectsAllowed"], { mode: "HYDRATABLE_CSR", hydrationPolicy: "REQUIRE_MATCH", mismatchBehavior: "RENDER_ERROR", duplicateEffectsAllowed: false });
    validateExactBlock("accessibilityFocus", ir.accessibilityFocus, ["navigationLabel", "mainRole", "headingLevel", "formLabel", "errorRole", "liveRegion", "invalidFocusTarget", "keyboardSubmit"], { navigationLabel: "主要导航", mainRole: "main", headingLevel: 1, formLabel: "搜索", errorRole: "alert", liveRegion: "polite", invalidFocusTarget: "query", keyboardSubmit: "Enter" });
    validateExactBlock("i18nThemeResponsive", ir.i18nThemeResponsive, ["supportedLocales", "fallbackLocale", "themes", "defaultTheme", "compactBreakpoint", "compactColumns", "wideColumns"], { supportedLocales: ["zh-CN", "en-US"], fallbackLocale: "en-US", themes: ["LIGHT", "DARK"], defaultTheme: "LIGHT", compactBreakpoint: 720, compactColumns: 1, wideColumns: 2 });
    validateExactBlock("nativePlatform", ir.nativePlatform, ["boundary", "capability", "lifecycleStates", "permission", "deniedBehavior", "recovery"], { boundary: "ADAPTER", capability: "OPEN_DEEP_LINK", lifecycleStates: ["FOREGROUND", "BACKGROUND"], permission: "DEEPLINK_OPEN", deniedBehavior: "NO_OP_REPORTED", recovery: "FOREGROUND_RETRY" });
    ...
    for (const node of [...request.uiIr.routes, ...request.uiIr.views, ...request.uiIr.components]) {
        if (ids.has(node.id))
            throw new Error(`duplicate UI interaction id: ${node.id}`);
        ids.add(node.id);
    }
    for (const [name, binding] of blockBindings(request)) {
        requireBinding(binding, name);
        if (ids.has(binding.id))
            throw new Error(`duplicate UI interaction id: ${binding.id}`);
        ids.add(binding.id);
    }
    for (const node of [
        ...
        for (const reference of node.references)
            if (!ids.has(reference))
                throw new Error(`UI interaction reference is unresolved: ${reference}`);
        for (const sourceRef of node.sourceRefs) {
            if (typeof sourceRef !== "string" || sourceRef.startsWith("/") || sourceRef.includes("..")
                || !/^[A-Za-z0-9._/-]+:[1-9][0-9]*$/.test(sourceRef))
                throw new Error(`UI interaction sourceRef is unsafe or non-canonical: ${sourceRef}`);
        }
    }
    const exactReferences = (name, actual, expected) => {
        if (canonical(actual) !== canonical(expected))
            throw new Error(`${name} role dependency graph drifted`);
    };
    for (const route of request.uiIr.routes)
        exactReferences(`route ${route.id}`, route.references, [route.componentId, ir.identityPermission.id]);
    for (const component of request.uiIr.components)
        exactReferences(`component ${component.id}`, component.references, [ir.componentTemplate.id, ir.accessibilityFocus.id, ir.i18nThemeResponsive.id]);
...
function runtimeReducerDeclaration(profile) {
    const raw = reduceBoundedFrontendRuntime.toString();
    const untyped = "function reduceBoundedFrontendRuntime(model, scenario)";
    if (!raw.includes(untyped))
        throw new Error("runtime reducer source header drifted");
...
function runtimeProjectionDeclaration(profile) {
    ...
    if (!raw.includes(untyped))
        throw new Error("runtime projection source header drifted");
    const header = profile === "vue2"
...
function tsRuntimeModule(profile) {
...
function reactNativePlatformScaffoldContract() {
...
function flutterPlatformScaffold(projectName) {
    return {
        ".metadata": [
...
function flutterIntegrationTest(projectName) {
    return [
        "import 'dart:convert';",
        "import 'package:flutter/foundation.dart';",
        "import 'package:flutter/material.dart';",
...
function flutterIntegrationDriver() {
    return [
        "import 'dart:convert';",
        "import 'dart:io';",
...
function blockSpecificReactWebConsumer() {
    return [
        'import { FormEvent, useEffect, useRef, useState } from "react";',
        'import { ELMOS_FRONTEND_INTERACTION, ELMOS_INTERACTION_SCENARIOS } from "./elmos-bounded-interaction";',
...
function blockSpecificVue3WebConsumer() {
    return [
        '<script setup lang="ts">',
        'import { onBeforeUnmount, ref } from "vue";',
        'import { ELMOS_FRONTEND_INTERACTION, ELMOS_INTERACTION_SCENARIOS } from "./elmos-bounded-interaction";',
...
function blockSpecificVue2WebConsumer() {
    return [
        "<script>",
        'import { ELMOS_FRONTEND_INTERACTION, ELMOS_INTERACTION_SCENARIOS } from "./elmos-bounded-interaction";',
...
function blockSpecificSvelteWebConsumer() {
    return [
        '<script lang="ts">',
        'import { onDestroy } from "svelte";',
        'import { ELMOS_FRONTEND_INTERACTION, ELMOS_INTERACTION_SCENARIOS } from "./elmos-bounded-interaction";',
...
function blockSpecificAngularWebConsumer() {
    return [
        'import { Component, ElementRef, OnDestroy, ViewChild, signal } from "@angular/core";',
        'import { ELMOS_FRONTEND_INTERACTION, ELMOS_INTERACTION_SCENARIOS } from "./elmos-bounded-interaction";',
        'import { elmosBrowserObserverDeclarations, elmosCreateBrowserRuntimeSession, elmosDeclarationJson, type ElmosBrowserMeasurement, type ElmosRuntimeScenario } from "./elmos-interaction-runtime";',
...
function blockSpecificJqueryWebConsumer() {
    return [
        'import $ from "jquery";',
        'import { ELMOS_FRONTEND_INTERACTION, ELMOS_INTERACTION_SCENARIOS } from "./elmos-bounded-interaction";',
        'import { elmosBrowserObserverDeclarations, elmosCreateBrowserRuntimeSession, elmosDeclarationJson, type ElmosBrowserMeasurement, type ElmosRuntimeScenario } from "./elmos-interaction-runtime";',
...
function blockSpecificNativeRuntimeModule() {
    const contracts = generatedObserverContractLiteral();
    return [
...
function blockSpecificNativeReactConsumer() {
    return [
        'import { useEffect, useRef, useState } from "react";',
...
function blockSpecificDartRuntimeModule() {
    return [
...
function blockSpecificFlutterConsumer() {
    return [
        "import 'dart:convert';",
        "import 'package:flutter/foundation.dart';",
        "import 'package:flutter/material.dart';",
...
function blockSpecificFlutterIntegrationTest(projectName) {
    return [
        "import 'dart:convert';",
        "import 'package:flutter/foundation.dart';",
        "import 'package:flutter/material.dart';",
...
function blockSpecificFlutterIntegrationDriver() {
    return [
        "import 'dart:convert';",
        "import 'dart:io';",
        "import 'package:integration_test/integration_test_driver.dart';",
...
function blockSpecificArkRuntimeModule() {
    const contracts = generatedObserverContractLiteral();
    return [
...
function replaceExact(source, needle, replacement, path) {
    const first = source.indexOf(needle);
    if (first < 0 || source.indexOf(needle, first + needle.length) >= 0)
        throw new Error(`interaction integration point drifted: ${path}`);
    return source.replace(needle, replacement);
}
export function augmentBoundedInteractionFiles(profile, model, baseFiles) {
    ...
    switch (profile) {
        ...
        case "jquery":
            ...
            files["src/elmos-interaction-consumer.ts"] = replaceExact(files["src/elmos-interaction-consumer.ts"], ")); for (const scenario of ELMOS_INTERACTION_SCENARIOS) {", ")); const formElement = form.get(0); if (!formElement) throw new Error('ELMOS form was not created'); formElement.addEventListener('invalid', invalidForm, true); form.on('input.elmos', '#elmos-query', syncValidity); for (const scenario of ELMOS_INTERACTION_SCENARIOS) {", "src/elmos-interaction-consumer.ts");
            files["src/elmos-interaction-consumer.ts"] = replaceExact(files["src/elmos-interaction-consumer.ts"], "const cleanup = () => { session.dispose(); form.off('.elmos'); form.find('button').off('.elmos'); };", "const cleanup = () => { formElement.removeEventListener('invalid', invalidForm, true); session.dispose(); form.off('.elmos'); form.find('button').off('.elmos'); };", "src/elmos-interaction-consumer.ts");
        ...
        case "flutter":
            {
                ...
                if (!nameMatch?.[1])
                    throw new Error("Flutter generated package name is missing");
            ...
            files["lib/main.dart"] = replaceExact(files["lib/main.dart"], "  const GeneratedApp({super.key});", "  const GeneratedApp({this.interactionApiAdapter, this.interactionNativeAdapter, super.key});\n  final ElmosApiAdapter? interactionApiAdapter;\n  final ElmosNativeAdapter? interactionNativeAdapter;", "lib/main.dart");
            files["lib/main.dart"] = replaceExact(files["lib/main.dart"], `    return MaterialApp(title: ${JSON.stringify(model.projectTitle)}, initialRoute: elmosFirstRoute.path,`, `    return MaterialApp(title: ${JSON.stringify(model.projectTitle)}, initialRoute: elmosFirstRoute.path,\n      builder: (context, child) => Column(children: [Expanded(child: child ?? const SizedBox.shrink()), SizedBox(height: 240, child: Material(child: ElmosInteractionPanel(apiAdapter: interactionApiAdapter, nativeAdapter: interactionNativeAdapter)))]),`, "lib/main.dart");
            break;
        case "harmony-arkui":
            files["entry/src/main/ets/elmos-interaction-runtime.ts"] = blockSpecificArkRuntimeModule();
            files["entry/src/main/ets/pages/Index.ets"] = replaceExact(files["entry/src/main/ets/pages/Index.ets"], "import { ELMOS_ROUTES, elmosSelectBoundedRoute } from '../elmos-bounded-navigation';", "import { ELMOS_ROUTES, elmosSelectBoundedRoute } from '../elmos-bounded-navigation';\nimport { ELMOS_INTERACTION_SCENARIOS } from '../elmos-bounded-interaction';\nimport { elmosArkObserverDeclarations } from '../elmos-interaction-runtime';", "entry/src/main/ets/pages/Index.ets");
            files["entry/src/main/ets/pages/Index.ets"] = replaceExact(files["entry/src/main/ets/pages/Index.ets"], "  @State selected: number = 0;", "  @State selected: number = 0;\n  @State interactionDeclarations: string[][] = ELMOS_INTERACTION_SCENARIOS.map(() => []);\n  @State interactionSequences: number[] = ELMOS_INTERACTION_SCENARIOS.map(() => 0);\n  private interactionSequence: number = 0;\n  private dispatchInteraction(index: number): void { this.interactionSequence += 1; this.interactionDeclarations[index] = elmosArkObserverDeclarations(['action-event']).map((declaration: object) => JSON.stringify(declaration)); this.interactionSequences[index] = this.interactionSequence; this.interactionDeclarations = [...this.interactionDeclarations]; this.interactionSequences = [...this.interactionSequences]; }", "entry/src/main/ets/pages/Index.ets");
            files["entry/src/main/ets/pages/Index.ets"] = replaceExact(files["entry/src/main/ets/pages/Index.ets"], "          Text('生成状态：等待 HarmonyOS 真机与无障碍验证').fontColor('#6B4F00')", "          Text('生成状态：等待 HarmonyOS 真机与无障碍验证').fontColor('#6B4F00')\n          ForEach(ELMOS_INTERACTION_SCENARIOS.slice(), (_scenario: object, index: number) => { Column() { Button(String(index)).accessibilityText('action:' + String(index)).onClick(() => { this.dispatchInteraction(index); }); ForEach(this.interactionDeclarations[index], (declaration: string) => { Text(declaration).accessibilityText('scenario:' + String(index) + ':runtime:BLOCK_SPECIFIC_RUNTIME_OBSERVED:state:PARTIAL:sequence:' + String(this.interactionSequences[index]) + ':declaration:' + declaration) }) } })", "entry/src/main/ets/pages/Index.ets");
            break;
    }
    return files;
}
export function boundedInteractionConsumerPaths(profile) {
    switch (profile) {
...
export function generateBoundedInteractionProject(requestValue) {
    ...
    const metadata = JSON.parse(files["elmos.ui-migration.json"]);
...
function binding(id, references) {
    return { id, references, sourceRefs: [`fixtures/bounded-interaction/${id}:1`] };
}
export function boundedInteractionFixtureRequest(target) {
    const source = uiTargetProfiles().find(profile => profile.id !== target);
    if (!source)
        throw new Error("bounded interaction fixture source is unavailable");
    const routeNode = (id, componentId, path, requiresAuth, deepLink) => ({
    ...
    const componentNode = (id, name, text) => ({
    ...
    const sourceRefsFor = (id) => {
        const marker = `"id": ${JSON.stringify(id)}`;
        const offset = initialBytes.indexOf(marker);
        if (offset < 0)
            throw new Error(`generated fixture source span is missing: ${id}`);
        return [`${fixturePath}:${initialBytes.slice(0, offset).split("\n").length}`];
    };
    const rebind = (value) => ({ ...value, sourceRefs: sourceRefsFor(value.id) });
    const rebound = {
        ...draft,
        uiIr: {
            ...draft.uiIr,
...
export function boundedInteractionSourceFixtureArtifact(request) {
...
export function boundedInteractionSourceFixtureBytes(request) {
    return `${JSON.stringify(boundedInteractionSourceFixtureArtifact(request), null, 2)}\n`;
}
export function requiredRuntimeChannels(profile) {
    if (profile === "react-native" || profile === "flutter")
        return ["browser", "android", "ios"];
    if (profile === "harmony-arkui")
        return ["harmonyos"];
    return ["browser"];
}
export function interactionTargetProfile(profile) {
    return uiTargetProfile(profile);
}
//# sourceMappingURL=bounded-interaction-project.js.map