---
name: spring-quality-gates
description: 在正确性后执行性能 smoke/load、CVE/SBOM、SAST/Secret、配置与可观测性 Gate。 Use for this exact ELMOS Spring Golden Route v2 contract; imported specifications are not runtime or certification evidence.
...
metadata:
  ...
  source_id: ELMOS-GR-062
  source_name: spring-quality-gates
  source_path: skills/spring-quality-gates/SKILL.md
  source_sha256: sha256:b2a3ee8e984fa17ac7d565ee18d54cfdb786e0be8d30cab168ff2b6fc6ec3704
  source_contract_sha256: sha256:1423c517a3a59765058cf39967e79a7c70bf2ce06f95424b23598b9f2158a96a
...
在正确性后执行性能 smoke/load、CVE/SBOM、SAST/Secret、配置与可观测性 Gate。
...
- [ ] 定义 `spring-quality-gates` 的 public interface、输入/输出/Error Schema 和版本策略。
- [ ] 实现 `spring-quality-gates` 的 production adapter/service，不把业务状态只保存在 Prompt 或内存对象中。
...
- [ ] 至少生成：quality-gates.json, security-findings.json, performance-report.json。
...
- [ ] 本 Skill 的 required outputs 均由真实目标仓库执行产生：quality-gates.json, security-findings.json, performance-report.json。
...
- `spring-api-diff`
- `spring-db-diff`
- `spring-security-diff`
- `spring-sideeffect-diff`
...
- Machine contract: `references/contract.json` (`sha256:1423c517a3a59765058cf39967e79a7c70bf2ce06f95424b23598b9f2158a96a`).
