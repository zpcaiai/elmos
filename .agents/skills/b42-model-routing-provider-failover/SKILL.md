---
name: b42-model-routing-provider-failover
description: "Implement and verify Batch 42 Skill 1425 for model routing provider failover. Use when work requires \u591a\u6a21\u578b\u8def\u7531\u548cProvider\u6545\u969c\u8f6c\u79fb, typed evidence, negative and holdout validation, and a fail-closed certification decision."
---
...
1. Inspect the current implementation, runtime facts, policies, incidents, decisions, and prior evidence relevant to `b42-model-routing-provider-failover`.
...
4. Implement the smallest production-shaped capability that exercises `多模型路由和Provider故障转移` without bypassing existing control planes.
...
- Capability-specific implementation, tests, run logs, evidence references, and residual-risk records for `b42-model-routing-provider-failover`
