---
name: tst-shell-powershell-injection-portability
description: "Shell与PowerShell注入及可移植性。验证参数、引号、Globbing、管道、退出码、Trap、编码、路径和命令注入防护。 Use for ELMOS Batch 66–80 slightly strict certification tests."
metadata:
  id: X013
...
---
    # X013 — Shell与PowerShell注入及可移植性

    ## Objective

    验证参数、引号、Globbing、管道、退出码、Trap、编码、路径和命令注入防护。 覆盖成功、边界、负向、故障、重放、恢复、篡改和状态冒充。默认不得把未运行、Skipped、Mock-only或陈旧Evidence计为通过。
    ...
    ### X013-01 — Shell与PowerShell注入及可移植性 / 参数引号Globbing与路径

- Priority: `P0`
- Category: `injection`
- Given: 传入空格、换行、通配符、Unicode、前导短横和恶意命令片段
- When: 执行本Case定义的测试、故障注入和证据收集
- Then: 参数保持原子，不触发命令注入、意外Globbing或路径穿越
- Evidence: source-snapshot, environment-manifest, raw-command-log, result-json, artifact-manifest, replay-command

### X013-02 — Shell与PowerShell注入及可移植性 / 退出码Trap与Pipeline失败

- Priority: `P1`
- Category: `error-contract`
- Given: 令Pipeline中间命令失败并发送中断信号
- When: 执行本Case定义的测试、故障注入和证据收集
- Then: 正确传播退出码，Trap只执行一次，临时文件清理
- Evidence: source-snapshot, environment-manifest, raw-command-log, result-json, artifact-manifest, replay-command

### X013-03 — Shell与PowerShell注入及可移植性 / POSIX Bash PowerShell矩阵

- Priority: `P2`
- Category: `portability`
- Given: 在声明Shell与Windows/Linux/macOS路径规则下运行
- When: 执行本Case定义的测试、故障注入和证据收集
- Then: 兼容声明真实，编码与换行不破坏行为
- Evidence: source-snapshot, environment-manifest, raw-command-log, result-json, artifact-manifest, replay-command
