---
name: elmos-vector-store-migration-benchmark-controller
description: Migrate embeddings and metadata among vector stores while preserving ACLs, recall, latency, freshness, deletion and cost envelopes.
version: 4.0.0
...
# Vector Store Migration and Benchmark Controller
...
- `VectorStoreMigrationAndBenchmarkControllerProfile`
- `VectorStoreMigrationAndBenchmarkControllerPlan`
- `VectorStoreMigrationAndBenchmarkControllerRun`
- `VectorStoreMigrationAndBenchmarkControllerDecision`

Required semantic capabilities:

- validate embedding identity and dimensionality
- rebuild or transfer indexes deterministically
- run recall/latency/cost benchmark corpora
- verify ACL and namespace preservation
- shadow queries and gate cutover/rollback
...
- `elmos-ai-rag-retrieval-grounding-certifier` from `elmos-functional-assurance-certification-skills` — optional companion; required only for E4/E5/P05 certification.

## Primary Outputs

- `artifacts/elmos-vector-store-migration-benchmark-controller/profile.yaml`
- `artifacts/elmos-vector-store-migration-benchmark-controller/plan.json`
- `artifacts/elmos-vector-store-migration-benchmark-controller/result.json`
- `artifacts/elmos-vector-store-migration-benchmark-controller/evidence/`
...
- **embedding model changes mid-migration** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **ANN benchmark contaminated** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **ACL metadata omitted** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **deletes lag beyond policy** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **score normalization compared incorrectly** — must have a preventive control, a detective signal, a negative fixture and a response action.
...
- native scenario: validate embedding identity and dimensionality
- native scenario: rebuild or transfer indexes deterministically
- native scenario: run recall/latency/cost benchmark corpora
- native scenario: verify ACL and namespace preservation
- native scenario: shadow queries and gate cutover/rollback
