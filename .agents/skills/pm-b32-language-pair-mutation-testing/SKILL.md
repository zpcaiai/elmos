...
# Language Pair Mutation Testing
## ELMOS runtime binding

- Invoke this repository Skill as `$pm-b32-language-pair-mutation-testing`.
- Immutable source identity: `language-pair-mutation-testing` in `precision-migration-b01-44` (B32).
- Runtime adapter: `differential-test-and-repair`; binding state: `UNAVAILABLE`.
- Resolve and plan with `python3 scripts/precision_migration/runtime.py plan --skill pm-b32-language-pair-mutation-testing`.
...
- Phase: `H 测试、验证与自动修复`
- Version: `1.0.0`
- Tags: advanced-dynamic-validation, batch-32

## Purpose

注入特定语言方向常见错误，验证专用测试和验证器是否能杀死 Mutant。

本 Skill 属于精密语言转换系统。它必须以**可执行证据**为准，不得用模型自评、源码相似度或“能编译”替代语义与行为验证。

## Use when

- 当前任务直接涉及 `language-pair-mutation-testing` 所描述的能力。
...
1. 评估或生成测试
2. 执行本 Skill 的核心职责：注入特定语言方向常见错误，验证专用测试和验证器是否能杀死 Mutant。
3. 同步环境并双运行
4. 比较输出、状态和副作用
5. 搜索最小反例
6. 驱动修复并执行全量回归
...
```yaml
skill: language-pair-mutation-testing
version: 1.0.0
