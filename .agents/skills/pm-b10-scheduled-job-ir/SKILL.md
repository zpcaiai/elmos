...
- Invoke this repository Skill as `$pm-b10-scheduled-job-ir`.
- Immutable source identity: `scheduled-job-ir` in `precision-migration-b01-44` (B10).
- Runtime adapter: `semantic-recovery-and-ir`; binding state: `DECLARED`.
- Resolve and plan with `python3 scripts/precision_migration/runtime.py plan --skill pm-b10-scheduled-job-ir`.
...
1. 定义可观察边界
2. 执行本 Skill 的核心职责：表达调度、并发策略、补偿、重跑、锁和失败恢复。
3. 将源语义归一化到受限模型
4. 保留不可丢失信息
5. 标记近似与不支持语义
