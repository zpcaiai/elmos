---
name: elmos-cross-store-polyglot-persistence-route-planner
description: Select relational, document, key-value, graph, search, vector and lakehouse targets per bounded context using workload, semantics, reversibility and operational evidence.
version: 4.0.0
...
# Cross-Store Polyglot Persistence Route Planner
...
- `CrossStorePolyglotPersistenceRoutePlannerProfile`
- `CrossStorePolyglotPersistenceRoutePlannerPlan`
- `CrossStorePolyglotPersistenceRoutePlannerRun`
- `CrossStorePolyglotPersistenceRoutePlannerDecision`

Required semantic capabilities:

- classify aggregate and query workloads
- score store capabilities and exit risk
- design ownership and consistency boundaries
- plan coexistence and strangler routes
- emit no-fit and external-policy decisions
...
- `artifacts/elmos-cross-store-polyglot-persistence-route-planner/profile.yaml`
- `artifacts/elmos-cross-store-polyglot-persistence-route-planner/plan.json`
- `artifacts/elmos-cross-store-polyglot-persistence-route-planner/result.json`
- `artifacts/elmos-cross-store-polyglot-persistence-route-planner/evidence/`
...
- **technology fashion overrides semantics** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **one source of truth becomes ambiguous** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **cross-store consistency unbounded** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **exit path absent** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **cost model ignores operational burden** — must have a preventive control, a detective signal, a negative fixture and a response action.
...
- native scenario: classify aggregate and query workloads
- native scenario: score store capabilities and exit risk
- native scenario: design ownership and consistency boundaries
- native scenario: plan coexistence and strangler routes
- native scenario: emit no-fit and external-policy decisions
