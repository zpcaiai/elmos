...
将 React 组件、Hook 调用、状态图和副作用转换为可验证的语义模型。

## 何时使用

- 检测到 React/JSX/TSX
- 需要诊断 Hook、路由或状态库转换差异

## 输入

- project-inventory.json
- React 源文件、tsconfig、bundler 配置
- 依赖图
...
1. 用 TypeScript/SWC/Babel AST 解析 JSX/TSX 和类型信息，保留源位置。
2. 识别函数组件、类组件、HOC、render props、Context 和组合模式。
3. 构建 useState/useReducer/useEffect/useMemo/useCallback/useRef 的依赖和执行时序。
4. 提取 React Router 或自定义路由、参数、守卫和懒加载边界。
5. 提取 Redux/Zustand/MobX 等状态图、异步 action 和持久化行为。
6. 识别 Portal、DOM 查询、浏览器事件、CSS-in-JS、Suspense 和 SSR 专属行为。
7. 对动态组件、反射调用和不可静态求值表达式输出风险与测试要求。

## 强制规则

- 遵守 Hook 调用顺序，禁止错误内联
- effect 清理函数必须进入生命周期语义
- Portal/DOM 行为不得静默忽略
...
- Source identity is pinned to `elmos.frontend-to-miniapp.skills` `1.0.0`, Skill `react-to-miniapp-analyzer`, and `sha256:4cf62b580b15e72556b1efb50c83953c78d6dd7f313883dc88751ab88fd8b424`.
