---
name: gr-benchmark-corpus
description: 建立分层规模、技术组合、风险、holdout 和故障注入的代表性仓库语料。 Use for this exact ELMOS Spring Golden Route v2 contract; imported specifications are not runtime or certification evidence.
...
metadata:
  ...
  source_id: ELMOS-GR-073
  source_name: gr-benchmark-corpus
  source_path: skills/gr-benchmark-corpus/SKILL.md
  source_sha256: sha256:8224afc22c0a052e2883b42cbd90c17bc0a6acc55dbc94188261207c0e68dedc
  source_contract_sha256: sha256:e86cb0101f0a5d3dd06a7aebfde0405c902c7e1cd118a57d2dbc245b66c9bfb4
...
建立分层规模、技术组合、风险、holdout 和故障注入的代表性仓库语料。

## Scope and capabilities

- OSS/customer/synthetic policy
- Frozen commits
- License/data approvals
- Train vs holdout
- Case tagging
...
- [ ] 定义 `gr-benchmark-corpus` 的 public interface、输入/输出/Error Schema 和版本策略。
- [ ] 实现 `gr-benchmark-corpus` 的 production adapter/service，不把业务状态只保存在 Prompt 或内存对象中。
...
- [ ] 至少生成：corpus-manifest.json, case-metadata, holdout-partition.json。
...
- [ ] 本 Skill 的 required outputs 均由真实目标仓库执行产生：corpus-manifest.json, case-metadata, holdout-partition.json。
- [ ] 所有 mandatory tests 和 security gates 通过，且无 critical unknown。

## Dependencies

- `repo-benchmark-profile`
...
- Machine contract: `references/contract.json` (`sha256:e86cb0101f0a5d3dd06a7aebfde0405c902c7e1cd118a57d2dbc245b66c9bfb4`).
