...
description: "Implement and verify Batch 43 Skill 1438 for event schema compatibility. Use when work requires Event\u548cSchema\u517c\u5bb9\u6cbb\u7406, typed evidence, negative and holdout validation, and a fail-closed certification decision."
---
...
1. Inspect the current implementation, runtime facts, policies, incidents, decisions, and prior evidence relevant to `b43-event-schema-compatibility`.
...
4. Implement the smallest production-shaped capability that exercises `Event和Schema兼容治理` without bypassing existing control planes.
...
- Capability-specific implementation, tests, run logs, evidence references, and residual-risk records for `b43-event-schema-compatibility`
