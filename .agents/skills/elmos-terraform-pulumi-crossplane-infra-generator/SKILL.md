...
description: Generate portable infrastructure definitions from Deployment IR with state, policy, drift, secret and rollback controls.
version: 4.0.0
...
# Terraform, Pulumi and Crossplane Infrastructure Generator
...
- `TerraformPulumiAndCrossplaneInfrastructureGeneratorProfile`
- `TerraformPulumiAndCrossplaneInfrastructureGeneratorPlan`
- `TerraformPulumiAndCrossplaneInfrastructureGeneratorRun`
- `TerraformPulumiAndCrossplaneInfrastructureGeneratorDecision`

Required semantic capabilities:

- lower Deployment IR to selected IaC target
- separate state and tenant boundaries
- generate policy and cost checks
- plan/import existing resources safely
- test drift, destroy and rollback behavior
...
- `artifacts/elmos-terraform-pulumi-crossplane-infra-generator/profile.yaml`
- `artifacts/elmos-terraform-pulumi-crossplane-infra-generator/plan.json`
- `artifacts/elmos-terraform-pulumi-crossplane-infra-generator/result.json`
- `artifacts/elmos-terraform-pulumi-crossplane-infra-generator/evidence/`
...
- **IaC state leaks secrets** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **resource import adopts wrong tenant** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **destroy plan unsafe** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **provider version mutable** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **generated policy not enforced** — must have a preventive control, a detective signal, a negative fixture and a response action.
...
- native scenario: lower Deployment IR to selected IaC target
- native scenario: separate state and tenant boundaries
- native scenario: generate policy and cost checks
- native scenario: plan/import existing resources safely
- native scenario: test drift, destroy and rollback behavior
