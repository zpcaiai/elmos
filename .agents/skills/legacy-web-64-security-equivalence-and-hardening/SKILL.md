---
name: legacy-web-64-security-equivalence-and-hardening
description: "Repository-owned exact runtime interface for 安全等价与可选硬化验证; bounded semantic analysis and evidence generation for Java legacy web modernization."
metadata:
  source_package: elmos.java-legacy-web.repository-modernization
  source_version: 1.0.0
  source_id: 64-security-equivalence-and-hardening
  source_digest: sha256:1d17a464429325a03360025207095333b7876dbc59717ee83a3befca7ba9cf4b
  phase: verification
  runtime_state: CODE_COMPLETE_LOCAL
  capability_state: LOCAL_EXECUTED
  operation_code: SECURITY_ORACLE_EVALUATED
  runtime_handler_id: legacy-web-handler:64-security-equivalence-and-hardening
---
...
---
id: 64-security-equivalence-and-hardening
title: 安全等价与可选硬化验证
version: 1.0.0
phase: verification
priority: P0
...
验证认证、角色、拒绝路径、CSRF/token、Cookie 和绑定面；将行为保持与显式安全硬化差异分开报告。
...
producerSkill: 64-security-equivalence-and-hardening
...
6. 建立身份/角色矩阵并验证允许、拒绝、匿名、过期 session、CSRF/token 和跳转。
7. 检测 Spring 默认行为造成的状态码、login URL、session fixation、header 变化。
8. hardened 模式的 deliberate delta 单独签字和回归。
9. 安全 scanner 结果不能替代业务授权差分测试。
