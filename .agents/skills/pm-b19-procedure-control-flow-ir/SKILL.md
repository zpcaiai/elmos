---
name: pm-b19-procedure-control-flow-ir
description: "表达存储过程、函数、游标、异常、动态 SQL、临时对象和事务控制流. Precision Migration B19 contract; use for this exact assessment, transformation, validation, repair, evidence, or cutover scope."
---

# Procedure Control Flow Ir
## ELMOS runtime binding

- Invoke this repository Skill as `$pm-b19-procedure-control-flow-ir`.
- Immutable source identity: `procedure-control-flow-ir` in `precision-migration-b01-44` (B19).
- Runtime adapter: `database-and-data-route`; binding state: `UNAVAILABLE`.
- Resolve and plan with `python3 scripts/precision_migration/runtime.py plan --skill pm-b19-procedure-control-flow-ir`.
...
表达存储过程、函数、游标、异常、动态 SQL、临时对象和事务控制流。

本 Skill 属于精密语言转换系统。它必须以**可执行证据**为准，不得用模型自评、源码相似度或“能编译”替代语义与行为验证。

## Use when

- 当前任务直接涉及 `procedure-control-flow-ir` 所描述的能力。
...
1. 盘点对象与专有能力
2. 执行本 Skill 的核心职责：表达存储过程、函数、游标、异常、动态 SQL、临时对象和事务控制流。
...
```yaml
skill: procedure-control-flow-ir
version: 1.0.0
