---
name: elmos-network-egress-data-exfiltration-certifier
description: Certify destination, protocol, payload, DNS, proxy and data-loss controls for every model, tool, connector and generated deployment.
version: 4.0.0
...
# Network Egress and Data-Exfiltration Certifier
...
- `NetworkEgressAndDataExfiltrationCertifierProfile`
- `NetworkEgressAndDataExfiltrationCertifierPlan`
- `NetworkEgressAndDataExfiltrationCertifierRun`
- `NetworkEgressAndDataExfiltrationCertifierDecision`

Required semantic capabilities:

- derive egress allowlist from capabilities
- bind DNS/IP/service identity and TLS policy
- inspect/redact classified payloads
- test covert channels and redirect chains
- record blocked and approved transfer evidence
...
- `elmos-ai-provider-data-residency-controller` from `elmos-ai-capability-enhancement-skills` — required base evidence producer.
- `elmos-ai-prompt-tool-security-redteam` from `elmos-ai-capability-enhancement-skills` — required base evidence producer.

## Primary Outputs

- `artifacts/elmos-network-egress-data-exfiltration-certifier/profile.yaml`
- `artifacts/elmos-network-egress-data-exfiltration-certifier/plan.json`
- `artifacts/elmos-network-egress-data-exfiltration-certifier/result.json`
- `artifacts/elmos-network-egress-data-exfiltration-certifier/evidence/`
...
- **DNS rebinding** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **redirect escapes allowlist** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **encrypted payload bypasses DLP** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **model endpoint receives unnecessary data** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **proxy logs secrets** — must have a preventive control, a detective signal, a negative fixture and a response action.
...
- native scenario: derive egress allowlist from capabilities
- native scenario: bind DNS/IP/service identity and TLS policy
- native scenario: inspect/redact classified payloads
- native scenario: test covert channels and redirect chains
- native scenario: record blocked and approved transfer evidence
