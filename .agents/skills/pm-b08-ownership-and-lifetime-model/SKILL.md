---
name: pm-b08-ownership-and-lifetime-model
description: "统一所有权、借用、别名、可变性、资源生命周期和线程安全语义. Precision Migration B08 contract; use for this exact assessment, transformation, validation, repair, evidence, or cutover scope."
---

# Ownership And Lifetime Model
## ELMOS runtime binding

- Invoke this repository Skill as `$pm-b08-ownership-and-lifetime-model`.
- Immutable source identity: `ownership-and-lifetime-model` in `precision-migration-b01-44` (B08).
- Runtime adapter: `semantic-recovery-and-ir`; binding state: `DECLARED`.
- Resolve and plan with `python3 scripts/precision_migration/runtime.py plan --skill pm-b08-ownership-and-lifetime-model`.
...
- Phase: `C 精密语义表示`
- Version: `1.0.0`
- Tags: batch-08, canonical-type-system

## Purpose

统一所有权、借用、别名、可变性、资源生命周期和线程安全语义。

本 Skill 属于精密语言转换系统。它必须以**可执行证据**为准，不得用模型自评、源码相似度或“能编译”替代语义与行为验证。

## Use when

- 当前任务直接涉及 `ownership-and-lifetime-model` 所描述的能力。
...
1. 定义可观察边界
2. 执行本 Skill 的核心职责：统一所有权、借用、别名、可变性、资源生命周期和线程安全语义。
3. 将源语义归一化到受限模型
4. 保留不可丢失信息
5. 标记近似与不支持语义
...
```yaml
skill: ownership-and-lifetime-model
version: 1.0.0
