---
name: elmos-ai-runaway-loop-economic-dos-guard
description: Enforce step, token, tool, cost, wall-clock, fanout and side-effect budgets while detecting retry storms, delegation
  loops and fallback cascades.
version: 4.0.0
...
# AI Runaway Loop and Economic DoS Guard
...
- `Approval`
- `ResidualRisk`
- `AIRunawayLoopAndEconomicDoSGuardRun`
- `AIRunawayLoopAndEconomicDoSGuardDecision`

Required semantic capabilities:

- Multi-dimensional budget envelope
- Cycle and repeated-state detection
- Retry/fallback/fanout circuit breakers
- Per-tenant and global rate governance
- Graceful stop with checkpoint and reconciliation
...
- `run-budget-contract.json`
- `loop-detection-events.jsonl`
- `circuit-breaker-state.json`
- `economic-dos-report.json`
...
- **economic denial of service** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **recursive delegation** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **unbounded browser crawl** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **budget race** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **circuit breaker bypass** — must have a preventive control, a detective signal, a negative fixture and a response action.
