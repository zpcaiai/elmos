...
# Exception To Result Error Lowering
## ELMOS runtime binding

- Invoke this repository Skill as `$pm-b15-exception-to-result-error-lowering`.
- Immutable source identity: `exception-to-result-error-lowering` in `precision-migration-b01-44` (B15).
- Runtime adapter: `directed-backend-route`; binding state: `UNAVAILABLE`.
- Resolve and plan with `python3 scripts/precision_migration/runtime.py plan --skill pm-b15-exception-to-result-error-lowering`.
...
在异常、Result、error tuple、panic 和错误码之间保持错误路径与清理语义。

本 Skill 属于精密语言转换系统。它必须以**可执行证据**为准，不得用模型自评、源码相似度或“能编译”替代语义与行为验证。

## Use when

- 当前任务直接涉及 `exception-to-result-error-lowering` 所描述的能力。
...
1. 使用语言原生前端恢复语义
2. 执行本 Skill 的核心职责：在异常、Result、error tuple、panic 和错误码之间保持错误路径与清理语义。
...
```yaml
skill: exception-to-result-error-lowering
version: 1.0.0
