---
name: safe-agent-role-switching
description: 运行中切换 Agent/模型只能在安全 provider-turn 边界完成。 Use for this exact ELMOS Spring Golden Route v2 contract; imported specifications are not runtime or certification evidence.
...
metadata:
  ...
  source_id: FOUNDATION-06-safe-agent-role-switching
  source_name: safe-agent-role-switching
  source_path: skills/safe-agent-role-switching/SKILL.md
  source_sha256: sha256:78eed2f48d4d88d7844615d03a19e3a1db00684226a7a1bb94339c634c76f562
  source_contract_sha256: sha256:ad6c81e39043e3865083302b81d0bba5c130b6e9a625abf5a64371a05a892bd9
...
# Safe Agent Role Switching

## Goal

运行中切换 Agent/模型只能在安全 provider-turn 边界完成。

## Use when

在 Elmos 需要实现或升级 **Safe Agent Role Switching** 相关能力时使用；它不是仅供解释的提示词，而是实现、测试与验收契约。

## Capabilities to implement

- 当前 tool call/side effect 不被中途换角色重放
- 下一个 turn 使用新 Agent 的权限和上下文
- 切换事件持久化并可审计
- 禁止通过角色切换绕过既有 deny policy
...
- [ ] 定义 `safe-agent-role-switching` 的 public interface、input/output/error schemas。
- [ ] 建立 `safe-agent-role-switching` 的 capability/feature flags 与版本字段。
...
- [ ] 至少覆盖并验收：当前 tool call/side effect 不被中途换角色重放；以及 下一个 turn 使用新 Agent 的权限和上下文。
...
- Machine contract: `references/contract.json` (`sha256:ad6c81e39043e3865083302b81d0bba5c130b6e9a625abf5a64371a05a892bd9`).
