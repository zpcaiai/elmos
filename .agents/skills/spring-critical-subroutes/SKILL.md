---
name: spring-critical-subroutes
description: 为数据/事务、安全、消息、缓存和定时任务建立独立高风险子路线。 Use for this exact ELMOS Spring Golden Route v2 contract; imported specifications are not runtime or certification evidence.
...
metadata:
  ...
  source_id: ELMOS-GR-014
  source_name: spring-critical-subroutes
  source_path: skills/spring-critical-subroutes/SKILL.md
  source_sha256: sha256:42eed6ac319a679564bc16c1a44ec88744238c8033a2f3f29e20512bf36be32b
  source_contract_sha256: sha256:158f50dd46261c9f22b9339e02955ef2ecd564463399940d303280fd559824e4
...
# Data Security Messaging Subroutes

## Objective

为数据/事务、安全、消息、缓存和定时任务建立独立高风险子路线。

## Scope and capabilities

- JPA/Hibernate/事务
- Filter Chain/OAuth/JWT/method security
- Kafka/Rabbit retry/DLQ
- Redis key/TTL/session
- Scheduler/Batch 副作用
...
- [ ] 定义 `spring-critical-subroutes` 的 public interface、输入/输出/Error Schema 和版本策略。
- [ ] 实现 `spring-critical-subroutes` 的 production adapter/service，不把业务状态只保存在 Prompt 或内存对象中。
...
- [ ] 至少生成：subroute-dags.json, critical-scenarios.json, risk-register.json。
...
- [ ] 本 Skill 的 required outputs 均由真实目标仓库执行产生：subroute-dags.json, critical-scenarios.json, risk-register.json。
...
- Machine contract: `references/contract.json` (`sha256:158f50dd46261c9f22b9339e02955ef2ecd564463399940d303280fd559824e4`).
