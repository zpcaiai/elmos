---
name: elmos-protocol-state-machine-model-checker
description: Compile workflow, MCP/A2A/ACP and service protocols into executable state models and verify reachability, safety, liveness, cancellation and recovery.
version: 4.0.0
...
# Protocol State-Machine Model Checker
...
- `ProtocolStateMachineModelCheckerProfile`
- `ProtocolStateMachineModelCheckerPlan`
- `ProtocolStateMachineModelCheckerRun`
- `ProtocolStateMachineModelCheckerDecision`

Required semantic capabilities:

- translate typed protocol states and guards
- check invalid transitions and unreachable states
- verify bounded liveness and cancellation
- model retries, timeouts and crash recovery
- map counterexamples to executable scenarios
...
- `artifacts/elmos-protocol-state-machine-model-checker/profile.yaml`
- `artifacts/elmos-protocol-state-machine-model-checker/plan.json`
- `artifacts/elmos-protocol-state-machine-model-checker/result.json`
- `artifacts/elmos-protocol-state-machine-model-checker/evidence/`
...
- **state abstraction drops data guard** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **fairness assumption unrealistic** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **state explosion hidden by cutoff** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **counterexample translation wrong** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **protocol version mismatch** — must have a preventive control, a detective signal, a negative fixture and a response action.
...
- native scenario: translate typed protocol states and guards
- native scenario: check invalid transitions and unreachable states
- native scenario: verify bounded liveness and cancellation
- native scenario: model retries, timeouts and crash recovery
- native scenario: map counterexamples to executable scenarios
