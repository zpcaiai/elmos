---
name: scan-generate-edit-lifecycle
description: 为大型迁移固定 scan、generate、edit 三阶段执行顺序。 Use for this exact ELMOS Spring Golden Route v2 contract; imported specifications are not runtime or certification evidence.
...
metadata:
  ...
  source_id: FOUNDATION-02-scan-generate-edit-lifecycle
  source_name: scan-generate-edit-lifecycle
  source_path: skills/scan-generate-edit-lifecycle/SKILL.md
  source_sha256: sha256:931ff761ceb0b01cad294634c7038022620b3329ece5a71b1f2d29d03f3a68fc
  source_contract_sha256: sha256:f85cdb17e2d8a99952d610b63ddce31525acb2eb4c1e1ed40be09c2c0e291a42
...
# Scan → Generate → Edit Lifecycle

## Goal

为大型迁移固定 scan、generate、edit 三阶段执行顺序。

## Use when

在 Elmos 需要实现或升级 **Scan → Generate → Edit Lifecycle** 相关能力时使用；它不是仅供解释的提示词，而是实现、测试与验收契约。

## Capabilities to implement

- scan 构建事实和决策输入
- generate 创建适配器/配置/测试等新文件
- edit 对现有源码做最小修改
- 阶段间有不可变 checkpoint 和审计数据
...
- [ ] 定义 `scan-generate-edit-lifecycle` 的 public interface、input/output/error schemas。
- [ ] 建立 `scan-generate-edit-lifecycle` 的 capability/feature flags 与版本字段。
...
- [ ] 至少覆盖并验收：scan 构建事实和决策输入；以及 generate 创建适配器/配置/测试等新文件。
...
- Machine contract: `references/contract.json` (`sha256:f85cdb17e2d8a99952d610b63ddce31525acb2eb4c1e1ed40be09c2c0e291a42`).
