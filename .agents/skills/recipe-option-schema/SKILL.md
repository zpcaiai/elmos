---
name: recipe-option-schema
description: Recipe 参数成为强类型 schema，而不是散落在 prompt 中。 Use for this exact ELMOS Spring Golden Route v2 contract; imported specifications are not runtime or certification evidence.
...
metadata:
  ...
  source_id: FOUNDATION-02-recipe-option-schema
  source_name: recipe-option-schema
  source_path: skills/recipe-option-schema/SKILL.md
  source_sha256: sha256:943b974d5c385bcada97e54fdcca9391669d7e3995ff7285b4bf97f61355dc62
  source_contract_sha256: sha256:ebd431be4132fb29a10ab4b4f667a2da0db5c3480f6018cf67a5e32cb481a390
...
# Recipe Option Schema

## Goal

Recipe 参数成为强类型 schema，而不是散落在 prompt 中。

## Use when

在 Elmos 需要实现或升级 **Recipe Option Schema** 相关能力时使用；它不是仅供解释的提示词，而是实现、测试与验收契约。

## Capabilities to implement

- 参数包含名称、类型、描述、示例、必填、合法值
- 支持从字段/构造器/配置声明生成 descriptor
- 运行前 schema 验证并 fail-fast
- UI/CLI/Agent 可由同一 schema 自动生成输入
...
- [ ] 定义 `recipe-option-schema` 的 public interface、input/output/error schemas。
- [ ] 建立 `recipe-option-schema` 的 capability/feature flags 与版本字段。
...
- [ ] 至少覆盖并验收：参数包含名称、类型、描述、示例、必填、合法值；以及 支持从字段/构造器/配置声明生成 descriptor。
...
- Machine contract: `references/contract.json` (`sha256:ebd431be4132fb29a10ab4b4f667a2da0db5c3480f6018cf67a5e32cb481a390`).
