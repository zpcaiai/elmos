---
name: etgb-platform-control-plane-validation
description: Validate all Elmos Product Control Plane, Projects, Jobs and Administration capabilities through the production adapter, independent Oracles and fail-closed release evidence. Repository-owned ETGB execution is available through the local runtime; external production evidence remains explicit.
metadata:
  ...
  source_skill: platform-control-plane-validation
...
---
name: platform-control-plane-validation
description: Validate all declared Elmos Product Control Plane, Projects, Jobs and Administration capabilities with executable positive, boundary, adversarial, concurrency, recovery and evidence-backed release tests.
---

# Elmos Product Control Plane, Projects, Jobs and Administration Validation

## Purpose

Use this Skill to plan, execute and certify the `platform-control-plane` product domain. It owns **66 declared capabilities** (33 P0, 27 P1, 6 P2) across 4 execution contexts and four mandatory variants.

## Inputs

- Frozen release candidate and feature-registry revision.
- Tenant, project, environment authority, budget and fencing context.
- Concrete cases from `suites/full-product.jsonl` whose business line is `platform-control-plane`.
...
2. Provision an isolated context for: web-console, public-api, event-stream, admin-console.
...
5. Apply the domain Oracle `control-plane-state-idempotency-observability-and-audit` plus security, state, audit and unsupported-disclosure Oracles.
...
Representative capabilities include: Project Create Read Update Delete, Repository Bind Unbind, Business Line Selection, Source Target Configuration, Model Selection And Auto Selection, Task Create Submit Start, Task State Machine, Task Progress Events, Task Pause Resume Cancel, Task Retry And Compensation, Task Idempotency Key, Task Lease And Fencing. The complete source of truth is `matrices/full-product.yaml`; no prose list may override it.
...
Route execution through `external-control-plane-harness` under `integrations/harness/full-product-adapters.yaml`. An adapter stub or skipped case is implementation progress, not product certification.
