---
name: elmos-trusted-computing-base-minimization-governor
description: Inventory, minimize and version the compilers, solvers, runtimes, policies, signers and assumptions trusted by each certificate.
version: 4.0.0
...
# Trusted Computing Base Minimization Governor
...
- `TrustedComputingBaseMinimizationGovernorProfile`
- `TrustedComputingBaseMinimizationGovernorPlan`
- `TrustedComputingBaseMinimizationGovernorRun`
- `TrustedComputingBaseMinimizationGovernorDecision`

Required semantic capabilities:

- build per-claim TCB graph
- classify trusted, verified and monitored components
- minimize adapters and privileged code
- track vulnerabilities and version drift
- publish TCB and residual assumptions in certificate
...
- `artifacts/elmos-trusted-computing-base-minimization-governor/profile.yaml`
- `artifacts/elmos-trusted-computing-base-minimization-governor/plan.json`
- `artifacts/elmos-trusted-computing-base-minimization-governor/result.json`
- `artifacts/elmos-trusted-computing-base-minimization-governor/evidence/`
...
- **hidden transitive trust** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **generator counted as verifier** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **signer host unmeasured** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **TCB vulnerability not revoking certificate** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **assumption omitted from customer report** — must have a preventive control, a detective signal, a negative fixture and a response action.
...
- native scenario: build per-claim TCB graph
- native scenario: classify trusted, verified and monitored components
- native scenario: minimize adapters and privileged code
- native scenario: track vulnerabilities and version drift
- native scenario: publish TCB and residual assumptions in certificate
