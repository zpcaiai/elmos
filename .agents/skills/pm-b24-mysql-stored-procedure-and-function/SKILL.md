---
name: pm-b24-mysql-stored-procedure-and-function
description: "迁移 MySQL Procedure、Function、变量、游标、Handler 和事务. Precision Migration B24 contract; use for this exact assessment, transformation, validation, repair, evidence, or cutover scope."
---

# Mysql Stored Procedure And Function
## ELMOS runtime binding

- Invoke this repository Skill as `$pm-b24-mysql-stored-procedure-and-function`.
- Immutable source identity: `mysql-stored-procedure-and-function` in `precision-migration-b01-44` (B24).
- Runtime adapter: `database-and-data-route`; binding state: `UNAVAILABLE`.
- Resolve and plan with `python3 scripts/precision_migration/runtime.py plan --skill pm-b24-mysql-stored-procedure-and-function`.
...
- Phase: `G 数据库精密互转`
- Version: `1.0.0`
- Tags: batch-24, mysql-proprietary-pack

## Purpose

迁移 MySQL Procedure、Function、变量、游标、Handler 和事务。

本 Skill 属于精密语言转换系统。它必须以**可执行证据**为准，不得用模型自评、源码相似度或“能编译”替代语义与行为验证。

## Use when

- 当前任务直接涉及 `mysql-stored-procedure-and-function` 所描述的能力。
...
1. 盘点对象与专有能力
2. 执行本 Skill 的核心职责：迁移 MySQL Procedure、Function、变量、游标、Handler 和事务。
...
```yaml
skill: mysql-stored-procedure-and-function
version: 1.0.0
