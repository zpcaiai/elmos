---
name: parser-tck
description: 为每种 parser 建立共享语法/语义/打印兼容性测试套件。 Use for this exact ELMOS Spring Golden Route v2 contract; imported specifications are not runtime or certification evidence.
...
metadata:
  ...
  source_id: FOUNDATION-04-parser-tck
  source_name: parser-tck
  source_path: skills/parser-tck/SKILL.md
  source_sha256: sha256:1b1fc6701386446c4dab22f9ab8e96cdf2496e438946dc1707911001e14d36fc
  source_contract_sha256: sha256:f230185110f97f161dc13f43baed04d8edcc289d5eeaae17e6933f18b7a897fd
...
# Parser Technology Compatibility Kit

## Goal

为每种 parser 建立共享语法/语义/打印兼容性测试套件。

## Use when

在 Elmos 需要实现或升级 **Parser Technology Compatibility Kit** 相关能力时使用；它不是仅供解释的提示词，而是实现、测试与验收契约。

## Capabilities to implement

- 覆盖语言规范和真实项目 corpus
- 同语言不同实现/版本共享测试用例
- 检测 parser/printer/type attribution 回归
- TCK 结果进入能力注册和发布门禁
...
- [ ] 定义 `parser-tck` 的 public interface、input/output/error schemas。
- [ ] 建立 `parser-tck` 的 capability/feature flags 与版本字段。
...
- [ ] 至少覆盖并验收：覆盖语言规范和真实项目 corpus；以及 同语言不同实现/版本共享测试用例。
...
- Machine contract: `references/contract.json` (`sha256:f230185110f97f161dc13f43baed04d8edcc289d5eeaae17e6933f18b7a897fd`).
