---
name: pm-b42-production-shadow-run
description: "复制真实请求到目标系统并隔离副作用，比较响应、意图、Trace 和性能. Precision Migration B42 contract; use for this exact assessment, transformation, validation, repair, evidence, or cutover scope."
---

# Production Shadow Run
## ELMOS runtime binding

- Invoke this repository Skill as `$pm-b42-production-shadow-run`.
- Immutable source identity: `production-shadow-run` in `precision-migration-b01-44` (B42).
- Runtime adapter: `shadow-canary-cutover`; binding state: `UNAVAILABLE`.
- Resolve and plan with `python3 scripts/precision_migration/runtime.py plan --skill pm-b42-production-shadow-run`.
...
复制真实请求到目标系统并隔离副作用，比较响应、意图、Trace 和性能。

本 Skill 属于精密语言转换系统。它必须以**可执行证据**为准，不得用模型自评、源码相似度或“能编译”替代语义与行为验证。

## Use when

- 当前任务直接涉及 `production-shadow-run` 所描述的能力。
...
1. 汇总证据与未解决项
2. 执行本 Skill 的核心职责：复制真实请求到目标系统并隔离副作用，比较响应、意图、Trace 和性能。
3. 执行硬性发布门禁
4. 影子/Canary/渐进切换
5. 监控并自动回滚
6. 沉淀反例、规则和企业交付能力
