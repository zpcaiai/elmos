...
# Generic And Variance Model
## ELMOS runtime binding

- Invoke this repository Skill as `$pm-b08-generic-and-variance-model`.
- Immutable source identity: `generic-and-variance-model` in `precision-migration-b01-44` (B08).
- Runtime adapter: `semantic-recovery-and-ir`; binding state: `UNAVAILABLE`.
- Resolve and plan with `python3 scripts/precision_migration/runtime.py plan --skill pm-b08-generic-and-variance-model`.
...
建模泛型约束、擦除、实例化、协变、逆变和高阶类型差异。

本 Skill 属于精密语言转换系统。它必须以**可执行证据**为准，不得用模型自评、源码相似度或“能编译”替代语义与行为验证。

## Use when

- 当前任务直接涉及 `generic-and-variance-model` 所描述的能力。
...
1. 定义可观察边界
2. 执行本 Skill 的核心职责：建模泛型约束、擦除、实例化、协变、逆变和高阶类型差异。
3. 将源语义归一化到受限模型
4. 保留不可丢失信息
5. 标记近似与不支持语义
...
```yaml
skill: generic-and-variance-model
version: 1.0.0
