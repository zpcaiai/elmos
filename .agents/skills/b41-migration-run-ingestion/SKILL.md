---
name: b41-migration-run-ingestion
description: "Implement and verify Batch 41 Skill 1396 for migration run ingestion. Use when work requires Migration Run\u77e5\u8bc6\u81ea\u52a8\u6444\u53d6, typed evidence, negative and holdout validation, and a fail-closed certification decision."
---
...
## Skill 1396: Migration Run知识自动摄取
...
1. Inspect the current implementation, runtime facts, policies, incidents, decisions, and prior evidence relevant to `b41-migration-run-ingestion`.
...
4. Implement the smallest production-shaped capability that exercises `Migration Run知识自动摄取` without bypassing existing control planes.
...
- Capability-specific implementation, tests, run logs, evidence references, and residual-risk records for `b41-migration-run-ingestion`
