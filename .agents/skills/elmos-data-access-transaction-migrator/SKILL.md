---
name: elmos-data-access-transaction-migrator
description: "Translate persistence, queries, migrations, transactions, retries, caches, and state ownership without silently changing data semantics."
version: 1.0.0
skill_id: ELMOS-POLY-023
layer: transformation
risk: critical
readiness: not-run
dependencies:
  - "elmos-api-contract-migrator"
triggers:
  - "Use when implementing or executing `elmos-data-access-transaction-migrator`."
  - "Use when the current DAG node requires the transformation capability."
outputs:
  - "target-data-layer/"
  - "query-mapping.json"
  - "transaction-mapping.json"
  - "data-migration-plan.json"
---

# Data Access Transaction Migrator

## Objective

Translate persistence, queries, migrations, transactions, retries, caches, and state ownership without silently changing data semantics.
...
1. Map source schema and ORM models to target types with explicit precision, collation, nullability, timezone, and generated-value behavior.
2. Translate queries using semantic tests rather than string substitution.
3. Preserve or intentionally redesign transaction boundaries, isolation, retry, locking, outbox, and idempotency.
4. Generate forward and rollback migrations where reversible.
5. Preserve cache key formats and invalidation behavior during compatibility windows.
6. Create dual-read/write or change-data-capture plans only with loop and ordering controls.
7. Generate dataset fixtures and invariant checks.
8. Block automatic migration of unsupported stored procedures or vendor-specific behavior.
...
- [ ] Golden datasets compare equal under declared tolerances..
- [ ] Transaction failure and retry tests pass..
- [ ] Rollback is tested or marked irreversible with approval..
- [ ] Precision and timezone edge cases are covered..
