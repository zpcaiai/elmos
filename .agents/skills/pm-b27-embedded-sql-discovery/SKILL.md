---
name: pm-b27-embedded-sql-discovery
description: "发现源码、模板、配置、报表和脚本中的嵌入式 SQL 与动态片段. Precision Migration B27 contract; use for this exact assessment, transformation, validation, repair, evidence, or cutover scope."
---

# Embedded Sql Discovery
## ELMOS runtime binding

- Invoke this repository Skill as `$pm-b27-embedded-sql-discovery`.
- Immutable source identity: `embedded-sql-discovery` in `precision-migration-b01-44` (B27).
- Runtime adapter: `database-and-data-route`; binding state: `DECLARED`.
- Resolve and plan with `python3 scripts/precision_migration/runtime.py plan --skill pm-b27-embedded-sql-discovery`.
...
发现源码、模板、配置、报表和脚本中的嵌入式 SQL 与动态片段。

本 Skill 属于精密语言转换系统。它必须以**可执行证据**为准，不得用模型自评、源码相似度或“能编译”替代语义与行为验证。

## Use when

- 当前任务直接涉及 `embedded-sql-discovery` 所描述的能力。
...
1. 盘点对象与专有能力
2. 执行本 Skill 的核心职责：发现源码、模板、配置、报表和脚本中的嵌入式 SQL 与动态片段。
