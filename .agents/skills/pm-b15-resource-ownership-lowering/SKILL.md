...
# Resource Ownership Lowering
## ELMOS runtime binding

- Invoke this repository Skill as `$pm-b15-resource-ownership-lowering`.
- Immutable source identity: `resource-ownership-lowering` in `precision-migration-b01-44` (B15).
- Runtime adapter: `directed-backend-route`; binding state: `DECLARED`.
- Resolve and plan with `python3 scripts/precision_migration/runtime.py plan --skill pm-b15-resource-ownership-lowering`.
...
迁移 GC、RAII、ARC、Ownership、Borrow、Dispose、defer 和上下文管理。

本 Skill 属于精密语言转换系统。它必须以**可执行证据**为准，不得用模型自评、源码相似度或“能编译”替代语义与行为验证。

## Use when

- 当前任务直接涉及 `resource-ownership-lowering` 所描述的能力。
...
1. 使用语言原生前端恢复语义
2. 执行本 Skill 的核心职责：迁移 GC、RAII、ARC、Ownership、Borrow、Dispose、defer 和上下文管理。
...
```yaml
skill: resource-ownership-lowering
version: 1.0.0
