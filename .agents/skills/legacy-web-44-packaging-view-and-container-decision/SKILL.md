---
name: legacy-web-44-packaging-view-and-container-decision
description: "Repository-owned exact runtime interface for 打包、视图与容器决策; bounded semantic analysis and evidence generation for Java legacy web modernization."
metadata:
...
---
id: 44-packaging-view-and-container-decision
title: 打包、视图与容器决策
version: 1.0.0
phase: planning
priority: P0
...
根据 JSP、外置容器、共享类库、JNDI、classloader 和部署约束选择 executable WAR/JAR、容器与视图迁移策略。
...
producerSkill: 44-packaging-view-and-container-decision
...
5. 输出允许差异和回滚设计。
6. 保留 JSP 时优先选择受支持的 WAR 路径，并验证目标容器 Servlet 6.1 兼容性。
7. 分析外置容器共享库/JNDI/classloader 是否可消除或需保留。
8. 区分 executable WAR、traditional WAR、executable JAR 和分离前端。
9. 打包改变必须通过资源、JSP、welcome/error page 和 classpath 验证。
