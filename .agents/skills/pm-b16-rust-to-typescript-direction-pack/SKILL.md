---
name: pm-b16-rust-to-typescript-direction-pack
description: "提供从 Rust 到 TypeScript/Node.js 的仓库级专用转换与验证包，覆盖类型、错误、副作用、并发、框架、依赖和目标惯用实现. Precision Migration B16 contract; use for this exact assessment, transformation, validation, repair, evidence, or cutover scope."
---

# Rust To Typescript Direction Pack
## ELMOS runtime binding

- Invoke this repository Skill as `$pm-b16-rust-to-typescript-direction-pack`.
- Immutable source identity: `rust-to-typescript-direction-pack` in `precision-migration-b01-44` (B16).
- Runtime adapter: `directed-backend-route`; binding state: `UNAVAILABLE`.
- Resolve and plan with `python3 scripts/precision_migration/runtime.py plan --skill pm-b16-rust-to-typescript-direction-pack`.
...
提供从 Rust 到 TypeScript/Node.js 的仓库级专用转换与验证包，覆盖类型、错误、副作用、并发、框架、依赖和目标惯用实现。

本 Skill 属于精密语言转换系统。它必须以**可执行证据**为准，不得用模型自评、源码相似度或“能编译”替代语义与行为验证。

## Use when

- 当前任务直接涉及 `rust-to-typescript-direction-pack` 所描述的能力。
...
1. 使用语言原生前端恢复语义
2. 执行本 Skill 的核心职责：提供从 Rust 到 TypeScript/Node.js 的仓库级专用转换与验证包，覆盖类型、错误、副作用、并发、框架、依赖和目标惯用实现。
...
```yaml
skill: rust-to-typescript-direction-pack
version: 1.0.0
