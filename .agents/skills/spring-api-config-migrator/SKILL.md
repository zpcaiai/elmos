---
name: spring-api-config-migrator
description: 迁移 Java API、注解、properties/yaml、starter 和测试配置，输出最小 Diff。 Use for this exact ELMOS Spring Golden Route v2 contract; imported specifications are not runtime or certification evidence.
...
metadata:
  ...
  source_id: ELMOS-GR-046
  source_name: spring-api-config-migrator
  source_path: skills/spring-api-config-migrator/SKILL.md
  source_sha256: sha256:93ee73e0701eda47c383e46396d2b148acaa5fd1d6dc0c13cf17de6ddf7282fe
  source_contract_sha256: sha256:dbdc580fe784042221ae70fbf1085105ba29288396db91632a39d1dbea6814e9
...
# API & Configuration Migrator

## Objective

迁移 Java API、注解、properties/yaml、starter 和测试配置，输出最小 Diff。
...
- [ ] 定义 `spring-api-config-migrator` 的 public interface、输入/输出/Error Schema 和版本策略。
- [ ] 实现 `spring-api-config-migrator` 的 production adapter/service，不把业务状态只保存在 Prompt 或内存对象中。
...
- [ ] 至少生成：semantic-patches, config-diff.json, unresolved-diagnostics.json。
...
- [ ] 全量 Recipe 最终收敛
- [ ] Agent 只接收剩余业务歧义
- [ ] 本 Skill 的 required outputs 均由真实目标仓库执行产生：semantic-patches, config-diff.json, unresolved-diagnostics.json。
- [ ] 所有 mandatory tests 和 security gates 通过，且无 critical unknown。

## Dependencies

- `spring-build-migrator`
...
- Machine contract: `references/contract.json` (`sha256:dbdc580fe784042221ae70fbf1085105ba29288396db91632a39d1dbea6814e9`).
