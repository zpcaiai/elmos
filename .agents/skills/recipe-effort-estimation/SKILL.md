---
name: recipe-effort-estimation
description: 把每个自动修复的人工替代工作量和自动运行成本量化。 Use for this exact ELMOS Spring Golden Route v2 contract; imported specifications are not runtime or certification evidence.
...
metadata:
  ...
  source_id: FOUNDATION-02-recipe-effort-estimation
  source_name: recipe-effort-estimation
  source_path: skills/recipe-effort-estimation/SKILL.md
  source_sha256: sha256:1c1f7ba2de00f81e2f24a37f0330da2c5a80f31279db2aceb13450a2069afff6
  source_contract_sha256: sha256:1e6c8578aad226b270e53a2debea14035fb6ca9e463e28d849576c155249be02
...
# Recipe Effort Estimation

## Goal

把每个自动修复的人工替代工作量和自动运行成本量化。

## Use when

在 Elmos 需要实现或升级 **Recipe Effort Estimation** 相关能力时使用；它不是仅供解释的提示词，而是实现、测试与验收契约。

## Capabilities to implement

- Recipe 可声明 estimated effort per occurrence
- 按 changeset 聚合潜在人工节省
- 同时记录机器 wall-clock、token、CPU 和模型费用
- 用于 ROI、调度优先级与客户账单解释
...
- [ ] 定义 `recipe-effort-estimation` 的 public interface、input/output/error schemas。
- [ ] 建立 `recipe-effort-estimation` 的 capability/feature flags 与版本字段。
...
- [ ] 至少覆盖并验收：Recipe 可声明 estimated effort per occurrence；以及 按 changeset 聚合潜在人工节省。
...
- Machine contract: `references/contract.json` (`sha256:1e6c8578aad226b270e53a2debea14035fb6ca9e463e28d849576c155249be02`).
