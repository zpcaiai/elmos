---
name: tst-container-image-compose-runtime-hardening
description: "容器镜像与Compose运行加固。验证Rootless、最小权限、健康检查、Secret、网络、SBOM、多架构和确定性镜像。 Use for ELMOS Batch 66–80 slightly strict certification tests."
metadata:
  id: X015
...
---
    # X015 — 容器镜像与Compose运行加固

    ## Objective

    验证Rootless、最小权限、健康检查、Secret、网络、SBOM、多架构和确定性镜像。 覆盖成功、边界、负向、故障、重放、恢复、篡改和状态冒充。默认不得把未运行、Skipped、Mock-only或陈旧Evidence计为通过。
    ...
    ### X015-01 — 容器镜像与Compose运行加固 / 非Root最小权限运行

- Priority: `P0`
- Category: `container-security`
- Given: 以只读RootFS、非Root、Drop Capabilities和受限网络启动
- When: 执行本Case定义的测试、故障注入和证据收集
- Then: 应用可运行且无特权回退或宿主敏感挂载
- Evidence: source-snapshot, environment-manifest, raw-command-log, result-json, artifact-manifest, replay-command

### X015-02 — 容器镜像与Compose运行加固 / Secret与Layer泄漏

- Priority: `P0`
- Category: `secret`
- Given: 扫描Build Context、历史Layer、环境、日志和Compose渲染
- When: 执行本Case定义的测试、故障注入和证据收集
- Then: Secret值不出现在镜像、缓存、日志或配置输出
- Evidence: source-snapshot, environment-manifest, raw-command-log, result-json, artifact-manifest, replay-command

### X015-03 — 容器镜像与Compose运行加固 / 多架构与Compose恢复

- Priority: `P2`
- Category: `runtime`
- Given: 构建多架构镜像并在Compose依赖重启/故障后恢复
- When: 执行本Case定义的测试、故障注入和证据收集
- Then: Health/Dependency/Shutdown行为正确，镜像来源和SBOM可验证
- Evidence: source-snapshot, environment-manifest, raw-command-log, result-json, artifact-manifest, replay-command
