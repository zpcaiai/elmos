...
# Database State Comparator
## ELMOS runtime binding

- Invoke this repository Skill as `$pm-b30-database-state-comparator`.
- Immutable source identity: `database-state-comparator` in `precision-migration-b01-44` (B30).
- Runtime adapter: `differential-test-and-repair`; binding state: `UNAVAILABLE`.
- Resolve and plan with `python3 scripts/precision_migration/runtime.py plan --skill pm-b30-database-state-comparator`.
...
比较相关表、事务、约束、序列、触发器和最终数据库状态。

本 Skill 属于精密语言转换系统。它必须以**可执行证据**为准，不得用模型自评、源码相似度或“能编译”替代语义与行为验证。

## Use when

- 当前任务直接涉及 `database-state-comparator` 所描述的能力。
...
1. 评估或生成测试
2. 执行本 Skill 的核心职责：比较相关表、事务、约束、序列、触发器和最终数据库状态。
3. 同步环境并双运行
4. 比较输出、状态和副作用
5. 搜索最小反例
6. 驱动修复并执行全量回归
...
```yaml
skill: database-state-comparator
version: 1.0.0
