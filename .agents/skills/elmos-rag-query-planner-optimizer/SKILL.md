---
name: elmos-rag-query-planner-optimizer
description: Compile evidence-aware retrieval plans across rewrite, decomposition, hybrid search, filters, rerank, graph traversal,
  context packing and abstention budgets.
version: 4.0.0
...
# RAG Query Planner and Optimizer
...
- **Kernel authorities used:** K3, K4, K6, K7
...
- `DeploymentUnit`
- `ConformanceRun`
- `RAGQueryPlannerAndOptimizerRun`
- `RAGQueryPlannerAndOptimizerDecision`

Required semantic capabilities:

- Typed query intent and decomposition
- Dense/sparse/graph strategy selection
- Filter and ACL pushdown
- Rerank/context packing budget
- Fallback and evidence-insufficient abstention
...
- `retrieval-plan.json`
- `query-plan-trace.json`
- `retrieval-cost-model.json`
- `plan-selection-report.json`
...
- **query expansion exfiltration** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **filter omission** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **retrieval cost explosion** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **reranker poisoning** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **context stuffing** — must have a preventive control, a detective signal, a negative fixture and a response action.
...
- filter pushdown
- budget constraint
- unsupported evidence abstention
