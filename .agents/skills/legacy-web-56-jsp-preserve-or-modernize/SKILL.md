---
name: legacy-web-56-jsp-preserve-or-modernize
description: "Repository-owned exact runtime interface for JSP 保留或现代化; bounded semantic analysis and evidence generation for Java legacy web modernization."
metadata:
  source_package: elmos.java-legacy-web.repository-modernization
  source_version: 1.0.0
  source_id: 56-jsp-preserve-or-modernize
  source_digest: sha256:06938364c3a51f613e1a80a71260d20c438d830d25defcbff07da38f28bc3174
  phase: transformation
  runtime_state: CODE_COMPLETE_LOCAL
  capability_state: LOCAL_EXECUTED
  operation_code: JSP_VIEW_TARGET_GENERATED
  runtime_handler_id: legacy-web-handler:56-jsp-preserve-or-modernize
---
...
---
id: 56-jsp-preserve-or-modernize
title: JSP 保留或现代化
version: 1.0.0
phase: transformation
priority: P0
...
- 44-packaging-view-and-container-decision
produces:
- view-target-changes
idempotent: true
resumable: true
...
默认保留 JSP/Tiles/taglib 并提供 WAR 路径；可在独立波次迁移至 Thymeleaf/React/Vue，保持 view model 契约。
...
producerSkill: 56-jsp-preserve-or-modernize
...
5. 以幂等 change set 发布。
6. preserve 模式保持 URL、view name、model/request/session attributes 和 taglib 依赖。
7. active view 的副作用先搬到显式服务并建立等价测试，再迁移模板。
8. modernize 模式通过 view contract adapter 与独立 E2E 基线执行。
9. 任何 UI 差异不得混入框架等价分数。
