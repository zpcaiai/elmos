---
name: elmos-webhook-delivery-security-reliability-controller
description: Generate and verify webhook signing, replay protection, delivery retries, ordering, endpoint rotation, observability and dead-letter handling.
version: 4.0.0
...
# Webhook Delivery Security and Reliability Controller
...
- `WebhookDeliverySecurityAndReliabilityControllerProfile`
- `WebhookDeliverySecurityAndReliabilityControllerPlan`
- `WebhookDeliverySecurityAndReliabilityControllerRun`
- `WebhookDeliverySecurityAndReliabilityControllerDecision`

Required semantic capabilities:

- generate canonical signature and timestamp verification
- enforce replay and tenant isolation
- implement retry and idempotent receipt
- rotate endpoints and secrets safely
- certify delivery and reconciliation evidence
...
- `artifacts/elmos-webhook-delivery-security-reliability-controller/profile.yaml`
- `artifacts/elmos-webhook-delivery-security-reliability-controller/plan.json`
- `artifacts/elmos-webhook-delivery-security-reliability-controller/result.json`
- `artifacts/elmos-webhook-delivery-security-reliability-controller/evidence/`
...
- **signature canonicalization mismatch** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **replay window too wide** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **secret logged** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **retry causes duplicate business effect** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **endpoint takeover** — must have a preventive control, a detective signal, a negative fixture and a response action.
...
- native scenario: generate canonical signature and timestamp verification
- native scenario: enforce replay and tenant isolation
- native scenario: implement retry and idempotent receipt
- native scenario: rotate endpoints and secrets safely
- native scenario: certify delivery and reconciliation evidence
