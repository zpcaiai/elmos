---
name: reconnect-safe-history
description: 提供 durable replay cursor + live tail，客户端断线后无缝续传。 Use for this exact ELMOS Spring Golden Route v2 contract; imported specifications are not runtime or certification evidence.
...
metadata:
  ...
  source_id: FOUNDATION-07-reconnect-safe-history
  source_name: reconnect-safe-history
  source_path: skills/reconnect-safe-history/SKILL.md
  source_sha256: sha256:735ed6e7e19a20e248907d9286b27bf60476e6538f662344b7485f881185177f
  source_contract_sha256: sha256:77c3b8412501edebe7c490b911006fc9814daac3c4a00e51536b11ef777fc06f
...
提供 durable replay cursor + live tail，客户端断线后无缝续传。

## Use when

在 Elmos 需要实现或升级 **Reconnect-safe History** 相关能力时使用；它不是仅供解释的提示词，而是实现、测试与验收契约。

## Capabilities to implement

- cursor 只由持久事件推进
- 先订阅 wake 再 replay，避免 replay/tail race
- ephemeral token delta 不伪装为 durable cursor
- 有限 history API 支持分页且序列严格递增
...
- [ ] 定义 `reconnect-safe-history` 的 public interface、input/output/error schemas。
- [ ] 建立 `reconnect-safe-history` 的 capability/feature flags 与版本字段。
...
- [ ] 至少覆盖并验收：cursor 只由持久事件推进；以及 先订阅 wake 再 replay，避免 replay/tail race。
...
- Machine contract: `references/contract.json` (`sha256:77c3b8412501edebe7c490b911006fc9814daac3c4a00e51536b11ef777fc06f`).
