---
name: transformation-before-after-tests
description: Recipe 必须用明确 before/after/unchanged fixtures 测试。 Use for this exact ELMOS Spring Golden Route v2 contract; imported specifications are not runtime or certification evidence.
...
metadata:
  ...
  source_id: FOUNDATION-04-transformation-before-after-tests
  source_name: transformation-before-after-tests
  source_path: skills/transformation-before-after-tests/SKILL.md
  source_sha256: sha256:66c654727f5f793bd4c77e6e97fdcc70d5ea10f7d46c4c535aefa7fbf2257043
  source_contract_sha256: sha256:06ec572a7f0f468a644000b619d143e9dfcdc822878b1a13180ba6f8e2b97585
...
# Transformation Before/After Tests

## Goal

Recipe 必须用明确 before/after/unchanged fixtures 测试。

## Use when

在 Elmos 需要实现或升级 **Transformation Before/After Tests** 相关能力时使用；它不是仅供解释的提示词，而是实现、测试与验收契约。

## Capabilities to implement

- 正例、反例、边界、幂等性全覆盖
- 同一 Recipe 二次运行应无变化或满足显式多 cycle 契约
- 异常输入保持源码并输出错误证据
- fixture 可自动生成回归最小化样本
...
- [ ] 定义 `transformation-before-after-tests` 的 public interface、input/output/error schemas。
- [ ] 建立 `transformation-before-after-tests` 的 capability/feature flags 与版本字段。
...
- [ ] 至少覆盖并验收：正例、反例、边界、幂等性全覆盖；以及 同一 Recipe 二次运行应无变化或满足显式多 cycle 契约。
...
- Machine contract: `references/contract.json` (`sha256:06ec572a7f0f468a644000b619d143e9dfcdc822878b1a13180ba6f8e2b97585`).
