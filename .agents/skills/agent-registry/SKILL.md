---
name: agent-registry
description: 把 Agent 作为可配置、可发现、可禁用的一等运行实体。 Use for this exact ELMOS Spring Golden Route v2 contract; imported specifications are not runtime or certification evidence.
...
metadata:
  ...
  source_id: FOUNDATION-06-agent-registry
  source_name: agent-registry
  source_path: skills/agent-registry/SKILL.md
  source_sha256: sha256:237ef61c498f6d4c5a2dc9737121ebc66789dd0127f464bf661c8ae77314316e
  source_contract_sha256: sha256:4070de48b8889ada3addc6fff08c9042fc00edbf3b3d2cafb4b6a12bba5a3341
...
把 Agent 作为可配置、可发现、可禁用的一等运行实体。

## Use when

在 Elmos 需要实现或升级 **Agent Registry** 相关能力时使用；它不是仅供解释的提示词，而是实现、测试与验收契约。

## Capabilities to implement

- Agent 声明 id/description/mode/model/prompt/permissions/limits
- 支持 project/global/managed 来源合并
- 选择前做 capability 与 permission 校验
- Agent 配置变更可热重载但必须进入 Context Epoch
...
- [ ] 定义 `agent-registry` 的 public interface、input/output/error schemas。
- [ ] 建立 `agent-registry` 的 capability/feature flags 与版本字段。
...
- [ ] 至少覆盖并验收：Agent 声明 id/description/mode/model/prompt/permissions/limits；以及 支持 project/global/managed 来源合并。
...
- Machine contract: `references/contract.json` (`sha256:4070de48b8889ada3addc6fff08c9042fc00edbf3b3d2cafb4b6a12bba5a3341`).
