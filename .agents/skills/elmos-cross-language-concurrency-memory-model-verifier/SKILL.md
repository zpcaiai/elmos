---
name: elmos-cross-language-concurrency-memory-model-verifier
description: Verify that generated or migrated code preserves happens-before, atomicity, cancellation, scheduling and shared-state invariants across incompatible language runtimes.
version: 4.0.0
...
# Cross-Language Concurrency and Memory-Model Verifier
...
- `CrossLanguageConcurrencyAndMemoryModelVerifierProfile`
- `CrossLanguageConcurrencyAndMemoryModelVerifierPlan`
- `CrossLanguageConcurrencyAndMemoryModelVerifierRun`
- `CrossLanguageConcurrencyAndMemoryModelVerifierDecision`

Required semantic capabilities:

- compare JMM, .NET, ECMAScript, Go and Rust ordering semantics
- exercise data-race and lost-update litmus tests
- verify async cancellation and structured-concurrency propagation
- check lock-free atomic and visibility mappings
- derive bounded runtime monitors for unprovable schedules
...
- `artifacts/elmos-cross-language-concurrency-memory-model-verifier/profile.yaml`
- `artifacts/elmos-cross-language-concurrency-memory-model-verifier/plan.json`
- `artifacts/elmos-cross-language-concurrency-memory-model-verifier/result.json`
- `artifacts/elmos-cross-language-concurrency-memory-model-verifier/evidence/`
...
- **memory-order weakening** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **hidden scheduler dependence** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **cancellation becomes resource leak** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **race detector false assurance** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **nondeterministic mismatch normalized away** — must have a preventive control, a detective signal, a negative fixture and a response action.
...
- native scenario: compare JMM, .NET, ECMAScript, Go and Rust ordering semantics
- native scenario: exercise data-race and lost-update litmus tests
- native scenario: verify async cancellation and structured-concurrency propagation
- native scenario: check lock-free atomic and visibility mappings
- native scenario: derive bounded runtime monitors for unprovable schedules
