---
name: successful-patch-to-recipe-learning
description: 把经大量验证的 Agent 修复候选抽象为可复用 Recipe。 Use for this exact ELMOS Spring Golden Route v2 contract; imported specifications are not runtime or certification evidence.
...
metadata:
  ...
  source_id: FOUNDATION-10-successful-patch-to-recipe-learning
  source_name: successful-patch-to-recipe-learning
  source_path: skills/successful-patch-to-recipe-learning/SKILL.md
  source_sha256: sha256:461c2b4d50a720b4239482c3a426cb7a9d6daa89ee93f38e589106599c794a30
  source_contract_sha256: sha256:b11eb88d4b518d325cb5a199bd7988ee881583bd933d353adf12531b359b40a3
...
# Successful Patch → Recipe Learning

## Goal

把经大量验证的 Agent 修复候选抽象为可复用 Recipe。

## Use when

在 Elmos 需要实现或升级 **Successful Patch → Recipe Learning** 相关能力时使用；它不是仅供解释的提示词，而是实现、测试与验收契约。

## Capabilities to implement

- 聚类相似 semantic patches 而非纯文本 diff
- 自动生成 pattern/precondition/template 候选
- 必须通过跨仓 corpus、反例、幂等和安全认证
- 未认证候选只作为建议，不自动执行
...
- [ ] 定义 `successful-patch-to-recipe-learning` 的 public interface、input/output/error schemas。
- [ ] 建立 `successful-patch-to-recipe-learning` 的 capability/feature flags 与版本字段。
...
- [ ] 至少覆盖并验收：聚类相似 semantic patches 而非纯文本 diff；以及 自动生成 pattern/precondition/template 候选。
...
- Machine contract: `references/contract.json` (`sha256:b11eb88d4b518d325cb5a199bd7988ee881583bd933d353adf12531b359b40a3`).
