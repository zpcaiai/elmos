...
# Jdbc Ado Odbc Driver Migration
## ELMOS runtime binding

- Invoke this repository Skill as `$pm-b27-jdbc-ado-odbc-driver-migration`.
- Immutable source identity: `jdbc-ado-odbc-driver-migration` in `precision-migration-b01-44` (B27).
- Runtime adapter: `database-and-data-route`; binding state: `DECLARED`.
- Resolve and plan with `python3 scripts/precision_migration/runtime.py plan --skill pm-b27-jdbc-ado-odbc-driver-migration`.
...
迁移 JDBC、ADO.NET、ODBC、驱动配置、连接串、参数和类型绑定。

本 Skill 属于精密语言转换系统。它必须以**可执行证据**为准，不得用模型自评、源码相似度或“能编译”替代语义与行为验证。

## Use when

- 当前任务直接涉及 `jdbc-ado-odbc-driver-migration` 所描述的能力。
...
1. 盘点对象与专有能力
2. 执行本 Skill 的核心职责：迁移 JDBC、ADO.NET、ODBC、驱动配置、连接串、参数和类型绑定。
...
```yaml
skill: jdbc-ado-odbc-driver-migration
version: 1.0.0
