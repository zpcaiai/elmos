...
description: "Implement and verify Batch 44 Skill 1468 for budget quota cost guardrail. Use when work requires Budget\u3001Quota\u548c\u6210\u672cGuardrail, typed evidence, negative and holdout validation, and a fail-closed certification decision."
---

# Budget、Quota和成本Guardrail
...
## Skill 1468: Budget、Quota和成本Guardrail
...
1. Inspect the current implementation, runtime facts, policies, incidents, decisions, and prior evidence relevant to `b44-budget-quota-cost-guardrail`.
...
4. Implement the smallest production-shaped capability that exercises `Budget、Quota和成本Guardrail` without bypassing existing control planes.
...
- Capability-specific implementation, tests, run logs, evidence references, and residual-risk records for `b44-budget-quota-cost-guardrail`
