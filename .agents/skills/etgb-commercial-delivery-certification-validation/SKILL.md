---
name: etgb-commercial-delivery-certification-validation
description: Validate all Commercial Packaging, Evidence, Golden Routes and Customer Acceptance capabilities through the production adapter, independent Oracles and fail-closed release evidence. Repository-owned ETGB execution is available through the local runtime; external production evidence remains explicit.
metadata:
  ...
  source_skill: commercial-delivery-certification-validation
...
---
name: commercial-delivery-certification-validation
description: Validate all declared Commercial Packaging, Evidence, Golden Routes and Customer Acceptance capabilities with executable positive, boundary, adversarial, concurrency, recovery and evidence-backed release tests.
---

# Commercial Packaging, Evidence, Golden Routes and Customer Acceptance Validation

## Purpose

Use this Skill to plan, execute and certify the `commercial-delivery-certification` product domain. It owns **59 declared capabilities** (34 P0, 20 P1, 5 P2) across 4 execution contexts and four mandatory variants.

## Inputs

- Frozen release candidate and feature-registry revision.
- Tenant, project, environment authority, budget and fencing context.
- Concrete cases from `suites/full-product.jsonl` whose business line is `commercial-delivery-certification`.
...
2. Provision an isolated context for: trial, paid-project, enterprise-golden-route, private-deployment.
...
5. Apply the domain Oracle `commercial-delivery-entitlement-evidence-reproducibility-and-acceptance` plus security, state, audit and unsupported-disclosure Oracles.
...
Representative capabilities include: Deliverable Manifest, Source And Artifact Checksums, Download Package Integrity, License And Third Party Notice, Sbom Aibom Vex, Build Provenance And Signature, Immutable Release Candidate, Test Plan And Result Bundle, Raw And Normalized Evidence, Evidence Signature And Chain, Known Limitations And Unsupported List, Manual Intervention Report. The complete source of truth is `matrices/full-product.yaml`; no prose list may override it.
...
Route execution through `external-commercial-certification-harness` under `integrations/harness/full-product-adapters.yaml`. An adapter stub or skipped case is implementation progress, not product certification.
