---
name: elmos-migration-route-planner
description: "Select evidence-backed modernization or conversion routes using risk, cost, reversibility, capability fit, and validation feasibility."
version: 1.0.0
skill_id: ELMOS-POLY-015
layer: planning
risk: critical
readiness: not-run
dependencies:
...
triggers:
  - "Use when implementing or executing `elmos-migration-route-planner`."
...
Select evidence-backed modernization or conversion routes using risk, cost, reversibility, capability fit, and validation feasibility.
...
1. Load source inventory, behavior contracts, Project/Semantic/Framework IR, target profile, and route registry.
2. Generate candidates for in-place modernization, framework upgrade, interop, strangler migration, module conversion, and full rewrite.
3. Score candidates by semantic loss, framework fit, state coupling, testability, security, performance, effort, and rollback.
4. Use route profiles only as defaults; bind every choice to current evidence.
5. Run feasibility probes for uncertain compilers, parsers, libraries, and deployment targets.
6. Present Pareto candidates and preserve rejected options.
7. Require approval for irreversible or high-loss routes.
8. Emit a validation plan before implementation begins.
...
- [ ] High-loss candidates cannot be auto-approved..
- [ ] A selected route references current snapshot and target profile..
- [ ] Rejected alternatives include reasons..
- [ ] Unknown evidence lowers confidence rather than becoming a positive score..
