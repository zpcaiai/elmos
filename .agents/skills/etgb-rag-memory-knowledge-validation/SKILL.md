---
name: etgb-rag-memory-knowledge-validation
description: Validate all RAG, Knowledge Base, Search, Reranking and Memory capabilities through the production adapter, independent Oracles and fail-closed release evidence. Repository-owned ETGB execution is available through the local runtime; external production evidence remains explicit.
metadata:
  ...
  source_skill: rag-memory-knowledge-validation
...
---
name: rag-memory-knowledge-validation
description: Validate all declared RAG, Knowledge Base, Search, Reranking and Memory capabilities with executable positive, boundary, adversarial, concurrency, recovery and evidence-backed release tests.
---

# RAG, Knowledge Base, Search, Reranking and Memory Validation

## Purpose

Use this Skill to plan, execute and certify the `rag-memory-knowledge` product domain. It owns **67 declared capabilities** (37 P0, 25 P1, 5 P2) across 4 execution contexts and four mandatory variants.

## Inputs

- Frozen release candidate and feature-registry revision.
- Tenant, project, environment authority, budget and fencing context.
- Concrete cases from `suites/full-product.jsonl` whose business line is `rag-memory-knowledge`.
...
2. Provision an isolated context for: small-corpus, large-corpus, multi-tenant, incremental-update.
...
5. Apply the domain Oracle `rag-retrieval-faithfulness-citation-memory-and-security` plus security, state, audit and unsupported-disclosure Oracles.
...
Representative capabilities include: Document Parse And Chunk, Chunk Overlap And Boundaries, Embedding Generation, Embedding Model Version Freeze, Vector Index Build, Hybrid Dense Sparse Search, Metadata Filtering, Tenant And Document Acl Filtering, Reranking, Query Rewrite, Multi Query Retrieval, Multi Hop Retrieval. The complete source of truth is `matrices/full-product.yaml`; no prose list may override it.
...
Route execution through `external-rag-memory-harness` under `integrations/harness/full-product-adapters.yaml`. An adapter stub or skipped case is implementation progress, not product certification.
