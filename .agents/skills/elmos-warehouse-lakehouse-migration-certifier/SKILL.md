---
name: elmos-warehouse-lakehouse-migration-certifier
description: Certify semantic, performance, governance and cost migration among warehouses and open table formats using dual execution and workload replay.
version: 4.0.0
...
# Warehouse and Lakehouse Migration Certifier
...
- `WarehouseAndLakehouseMigrationCertifierProfile`
- `WarehouseAndLakehouseMigrationCertifierPlan`
- `WarehouseAndLakehouseMigrationCertifierRun`
- `WarehouseAndLakehouseMigrationCertifierDecision`

Required semantic capabilities:

- translate analytical SQL and UDFs
- replay representative BI/ETL workloads
- verify data and aggregate equivalence
- validate table-feature and engine compatibility
- certify cost, concurrency and rollback
- validate immutable/incremental snapshot plus CDC boundary equivalence
- certify schema evolution, partition specification and table-format metadata compatibility
...
- `elmos-lakehouse-table-format-ir-compiler` from `elmos-ai-capability-enhancement-skills` — required base evidence producer.

## Primary Outputs

- `artifacts/elmos-warehouse-lakehouse-migration-certifier/profile.yaml`
- `artifacts/elmos-warehouse-lakehouse-migration-certifier/plan.json`
- `artifacts/elmos-warehouse-lakehouse-migration-certifier/result.json`
- `artifacts/elmos-warehouse-lakehouse-migration-certifier/evidence/`
...
- **aggregate tolerance hides missing rows** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **warehouse-specific UDF mistranslated** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **table feature unreadable by engine** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **cost based on synthetic scale only** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **rollback snapshots expired** — must have a preventive control, a detective signal, a negative fixture and a response action.
...
- native scenario: translate analytical SQL and UDFs
- native scenario: replay representative BI/ETL workloads
- native scenario: verify data and aggregate equivalence
- native scenario: validate table-feature and engine compatibility
- native scenario: certify cost, concurrency and rollback
- native scenario: validate immutable source snapshot, incremental snapshot and CDC cutover equivalence
- native scenario: certify schema evolution, partition specification and table-format metadata compatibility
