...
- Invoke this repository Skill as `$pm-b41-runtime-evidence-package`.
- Immutable source identity: `runtime-evidence-package` in `precision-migration-b01-44` (B41).
- Runtime adapter: `evidence-release-gate`; binding state: `DECLARED`.
- Resolve and plan with `python3 scripts/precision_migration/runtime.py plan --skill pm-b41-runtime-evidence-package`.
...
打包双运行、Fuzz、Mutation、并发、故障、性能、UI 和真机结果。

本 Skill 属于精密语言转换系统。它必须以**可执行证据**为准，不得用模型自评、源码相似度或“能编译”替代语义与行为验证。

## Use when

- 当前任务直接涉及 `runtime-evidence-package` 所描述的能力。
...
1. 汇总证据与未解决项
2. 执行本 Skill 的核心职责：打包双运行、Fuzz、Mutation、并发、故障、性能、UI 和真机结果。
3. 执行硬性发布门禁
4. 影子/Canary/渐进切换
5. 监控并自动回滚
6. 沉淀反例、规则和企业交付能力
...
```yaml
skill: runtime-evidence-package
version: 1.0.0
