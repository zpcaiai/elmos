...
# Sequence Identity Ir
## ELMOS runtime binding

- Invoke this repository Skill as `$pm-b19-sequence-identity-ir`.
- Immutable source identity: `sequence-identity-ir` in `precision-migration-b01-44` (B19).
- Runtime adapter: `database-and-data-route`; binding state: `DECLARED`.
- Resolve and plan with `python3 scripts/precision_migration/runtime.py plan --skill pm-b19-sequence-identity-ir`.
...
表达 Sequence、Identity、Auto Increment、缓存、回退和并发生成。

本 Skill 属于精密语言转换系统。它必须以**可执行证据**为准，不得用模型自评、源码相似度或“能编译”替代语义与行为验证。

## Use when

- 当前任务直接涉及 `sequence-identity-ir` 所描述的能力。
...
1. 盘点对象与专有能力
2. 执行本 Skill 的核心职责：表达 Sequence、Identity、Auto Increment、缓存、回退和并发生成。
