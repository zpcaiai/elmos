---
name: elmos-wasi-sandbox-capability-certifier
description: Certify filesystem, network, clock, randomness, environment, secret and process capabilities for WASI workloads under deny-by-default runtime policy.
version: 4.0.0
...
# WASI Sandbox and Capability Certifier
...
- `WASISandboxAndCapabilityCertifierProfile`
- `WASISandboxAndCapabilityCertifierPlan`
- `WASISandboxAndCapabilityCertifierRun`
- `WASISandboxAndCapabilityCertifierDecision`

Required semantic capabilities:

- compile explicit WASI capability grants
- deny undeclared filesystem and network access
- virtualize clock and randomness for replay
- verify host-call audit and tenant isolation
- exercise revocation, timeout and escape negative cases
...
- `elmos-wasm-component-wit-portability-compiler` from `elmos-ai-capability-enhancement-skills` — required base evidence producer.
- `elmos-runtime-sandbox-ebpf-behavior-verifier` from `elmos-ai-capability-enhancement-skills` — required base evidence producer.

## Primary Outputs

- `artifacts/elmos-wasi-sandbox-capability-certifier/profile.yaml`
- `artifacts/elmos-wasi-sandbox-capability-certifier/plan.json`
- `artifacts/elmos-wasi-sandbox-capability-certifier/result.json`
- `artifacts/elmos-wasi-sandbox-capability-certifier/evidence/`
...
- **preopened directory escapes scope** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **network capability broadens after resume** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **host secret exposed to guest** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **nondeterminism breaks replay** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **runtime claims isolation without native test** — must have a preventive control, a detective signal, a negative fixture and a response action.
...
- native scenario: compile explicit WASI capability grants
- native scenario: deny undeclared filesystem and network access
- native scenario: virtualize clock and randomness for replay
- native scenario: verify host-call audit and tenant isolation
- native scenario: exercise revocation, timeout and escape negative cases
