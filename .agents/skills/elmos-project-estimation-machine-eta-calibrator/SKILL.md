---
name: elmos-project-estimation-machine-eta-calibrator
description: Estimate and continuously calibrate machine wall-clock, queue, retry, token, compute and verification duration for repository-scale generation and certification.
version: 4.0.0
...
# Project Estimation and Machine ETA Calibrator
...
- `ProjectEstimationAndMachineETACalibratorProfile`
- `ProjectEstimationAndMachineETACalibratorPlan`
- `ProjectEstimationAndMachineETACalibratorRun`
- `ProjectEstimationAndMachineETACalibratorDecision`

Required semantic capabilities:

- extract size, graph, route and risk features
- separate execution, queue and approval time
- calibrate distributions from completed runs
- update ETA after checkpoints and failures
- report confidence and driver decomposition
...
- `elmos-productivity-value-stream-scorecard` from `elmos-functional-assurance-certification-skills` — optional companion; required only for E4/E5/P05 certification.

## Primary Outputs

- `artifacts/elmos-project-estimation-machine-eta-calibrator/profile.yaml`
- `artifacts/elmos-project-estimation-machine-eta-calibrator/plan.json`
- `artifacts/elmos-project-estimation-machine-eta-calibrator/result.json`
- `artifacts/elmos-project-estimation-machine-eta-calibrator/evidence/`
...
- **human time mixed with machine ETA** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **survivorship bias** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **queue capacity assumed constant** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **optimistic estimate used for billing** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **customer-specific data leaks into model** — must have a preventive control, a detective signal, a negative fixture and a response action.
...
- native scenario: extract size, graph, route and risk features
- native scenario: separate execution, queue and approval time
- native scenario: calibrate distributions from completed runs
- native scenario: update ETA after checkpoints and failures
- native scenario: report confidence and driver decomposition
