---
name: elmos-adapter-fortran
description: Implement Fortran language parsing, semantic lowering, framework/runtime
  mapping, target emission and verification for repository-scale modernization.
version: 1.0.0
skill_id: ELMOS-POLY-096
...
triggers:
- Use when implementing or executing `elmos-adapter-fortran`.
- Use when the migration DAG requires `technology-adapter` capability.
- Use when `fortran` is detected as source, target or repository support surface.
outputs:
- adapters/fortran/adapter-manifest.json
- adapters/fortran/semantic-mapping.yaml
- adapters/fortran/verification-profile.yaml
---
...
Implement Fortran language parsing, semantic lowering, framework/runtime mapping, target emission and verification for repository-scale modernization.
...
- Technology ID: `fortran`
- Display: Fortran
- Kind: language
- File/build markers: .f, .for, .f77, .f90, .f95, .f03, .f08
- Build systems: CMake, Make, fpm
- Preferred analyzers: flang/gfortran frontend, fparser, tree-sitter-fortran
- Runtime/framework surface: BLAS/LAPACK, MPI, OpenMP, coarrays
- Native verification: gfortran/flang build, numerical regression, MPI/OpenMP tests

### Semantic risk register

- COMMON/EQUIVALENCE/SAVE.
- implicit typing.
- column-major arrays.
- floating-point reproducibility.
- legacy IO and numerical libraries.
