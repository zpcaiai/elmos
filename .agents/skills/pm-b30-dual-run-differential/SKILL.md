...
- Invoke this repository Skill as `$pm-b30-dual-run-differential`.
- Immutable source identity: `batch-30-dual-run-differential` in `precision-migration-b01-44` (B30).
- Runtime adapter: `differential-test-and-repair`; binding state: `DECLARED`.
- Resolve and plan with `python3 scripts/precision_migration/runtime.py plan --skill pm-b30-dual-run-differential`.
...
- 当任务涉及 **source-target-dual-runner** 时，调用 `../pm-b30-source-target-dual-runner/SKILL.md`。
- 当任务涉及 **input-and-environment-synchronizer** 时，调用 `../pm-b30-input-and-environment-synchronizer/SKILL.md`。
- 当任务涉及 **clock-random-uuid-controller** 时，调用 `../pm-b30-clock-random-uuid-controller/SKILL.md`。
- 当任务涉及 **http-differential-comparator** 时，调用 `../pm-b30-http-differential-comparator/SKILL.md`。
- 当任务涉及 **database-state-comparator** 时，调用 `../pm-b30-database-state-comparator/SKILL.md`。
- 当任务涉及 **message-side-effect-comparator** 时，调用 `../pm-b30-message-side-effect-comparator/SKILL.md`。
- 当任务涉及 **external-call-comparator** 时，调用 `../pm-b30-external-call-comparator/SKILL.md`。
- 当任务涉及 **trace-graph-comparator** 时，调用 `../pm-b30-trace-graph-comparator/SKILL.md`。
- 当任务涉及 **ui-semantic-tree-comparator** 时，调用 `../pm-b30-ui-semantic-tree-comparator/SKILL.md`。
- 当任务涉及 **visual-differential-comparator** 时，调用 `../pm-b30-visual-differential-comparator/SKILL.md`。
- 当任务涉及 **navigation-and-storage-comparator** 时，调用 `../pm-b30-navigation-and-storage-comparator/SKILL.md`。
- 当任务涉及 **performance-differential-comparator** 时，调用 `../pm-b30-performance-differential-comparator/SKILL.md`。
