---
name: schema-validated-tool-io
description: Tool 输入和输出在模型边界强制 schema 校验。 Use for this exact ELMOS Spring Golden Route v2 contract; imported specifications are not runtime or certification evidence.
...
metadata:
  ...
  source_id: FOUNDATION-08-schema-validated-tool-io
  source_name: schema-validated-tool-io
  source_path: skills/schema-validated-tool-io/SKILL.md
  source_sha256: sha256:e728b59d455cf87546b047491fd40b3124f710e1513706abfa5f6976b12de341
  source_contract_sha256: sha256:cf5d2d6b633619dd7864d3c5a8ac4c1d4c65a58066a5dab4328e9fbce6a59a55
...
# Schema-validated Tool I/O

## Goal

Tool 输入和输出在模型边界强制 schema 校验。

## Use when

在 Elmos 需要实现或升级 **Schema-validated Tool I/O** 相关能力时使用；它不是仅供解释的提示词，而是实现、测试与验收契约。

## Capabilities to implement

- 模型参数 decode 失败不执行 handler
- handler 输出 encode 失败标记 tool failure
- 结构化结果与 model-visible text/file 可分别定义
- 错误包含字段级路径而非模糊字符串
...
- [ ] 定义 `schema-validated-tool-io` 的 public interface、input/output/error schemas。
- [ ] 建立 `schema-validated-tool-io` 的 capability/feature flags 与版本字段。
...
- [ ] 至少覆盖并验收：模型参数 decode 失败不执行 handler；以及 handler 输出 encode 失败标记 tool failure。
...
- Machine contract: `references/contract.json` (`sha256:cf5d2d6b633619dd7864d3c5a8ac4c1d4c65a58066a5dab4328e9fbce6a59a55`).
