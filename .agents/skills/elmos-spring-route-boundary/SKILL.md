---
name: elmos-spring-route-boundary
description: 冻结首版 Spring Route 支持矩阵、非目标和定制变更机制。 Use for this exact ELMOS Spring Golden Route v2 contract; imported specifications are not runtime or certification evidence.
...
metadata:
  ...
  source_id: ELMOS-GR-006
  source_name: elmos-spring-route-boundary
  source_path: skills/elmos-spring-route-boundary/SKILL.md
  source_sha256: sha256:2e324ff305256cd1f8cbb2408d17c2639844d3c650e6d3fcdb78bc20305d2f4e
  source_contract_sha256: sha256:f69cf7f423e363043ae7dabb7992fceee7100941027c65bd01e76711b080cc28
...
# Spring Route Boundary

## Objective

冻结首版 Spring Route 支持矩阵、非目标和定制变更机制。

## Scope and capabilities

- Maven 多模块
- MVC/Security/JPA/Hibernate
- PostgreSQL/MySQL/Flyway/Liquibase
- 可选 Redis/Kafka
- EJB/JNI/大量 JSP 等进入 Unsupported Manager
...
- [ ] 定义 `elmos-spring-route-boundary` 的 public interface、输入/输出/Error Schema 和版本策略。
- [ ] 实现 `elmos-spring-route-boundary` 的 production adapter/service，不把业务状态只保存在 Prompt 或内存对象中。
...
- [ ] 至少生成：spring-route-contract.yaml, support-matrix.json, non-goals.json。
...
- [ ] 本 Skill 的 required outputs 均由真实目标仓库执行产生：spring-route-contract.yaml, support-matrix.json, non-goals.json。
- [ ] 所有 mandatory tests 和 security gates 通过，且无 critical unknown。

## Dependencies

- `elmos-golden-route-priority`
...
- Machine contract: `references/contract.json` (`sha256:f69cf7f423e363043ae7dabb7992fceee7100941027c65bd01e76711b080cc28`).
