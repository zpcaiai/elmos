---
name: tst-strict-release-certification-gate
description: "Aggregate all Batch 1–37 functional, negative, security, scale, recovery, holdout, evidence and customer acceptance results into a conservative release decision."
---

# Test Skill T052: Batch 1–37严格最终认证门禁

## Use this skill when

- Codex must implement, expand, execute, or review strict tests for Batch 1–37横向能力.
...
- P0/P1结果必须确定且可重放
- 跨Batch结果使用统一Case与Evidence Schema
- Critical失败不得Waive
...
9. Reject certification unless one external trust anchor verifies a signed request that covers the exact control digests and all 408 result/evidence-manifest digests.
10. Report `BLOCKED` and field evidence `NOT_RUN` when execution or authorization is absent; local engineering checks never count as field certification.
