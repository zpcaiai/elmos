---
name: elmos-model-serving-canary-shadow-rollback-controller
description: Run no-side-effect shadow, bounded canary, traffic splitting, quality/SLO comparison and atomic rollback for model-server changes.
version: 4.0.0
...
# Model Serving Canary, Shadow and Rollback Controller
...
- `ModelServingCanaryShadowAndRollbackControllerProfile`
- `ModelServingCanaryShadowAndRollbackControllerPlan`
- `ModelServingCanaryShadowAndRollbackControllerRun`
- `ModelServingCanaryShadowAndRollbackControllerDecision`

Required semantic capabilities:

- bind candidate model/server/config revision
- mirror traffic with privacy controls
- compare quality, safety, latency and cost
- promote by staged policy gates
- rollback model, cache and routing consistently
...
- `artifacts/elmos-model-serving-canary-shadow-rollback-controller/profile.yaml`
- `artifacts/elmos-model-serving-canary-shadow-rollback-controller/plan.json`
- `artifacts/elmos-model-serving-canary-shadow-rollback-controller/result.json`
- `artifacts/elmos-model-serving-canary-shadow-rollback-controller/evidence/`
...
- **shadow invokes external tool** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **traffic split not sticky** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **candidate sees different context** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **rollback leaves incompatible KV cache** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **metrics lag after damage** — must have a preventive control, a detective signal, a negative fixture and a response action.
...
- native scenario: bind candidate model/server/config revision
- native scenario: mirror traffic with privacy controls
- native scenario: compare quality, safety, latency and cost
- native scenario: promote by staged policy gates
- native scenario: rollback model, cache and routing consistently
