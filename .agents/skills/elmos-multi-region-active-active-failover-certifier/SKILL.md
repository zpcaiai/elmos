---
name: elmos-multi-region-active-active-failover-certifier
description: Certify routing, data replication, session/memory ownership, consistency, failover, failback and regional isolation for AI services.
version: 4.0.0
...
# Multi-Region Active-Active Failover Certifier
...
- `MultiRegionActiveActiveFailoverCertifierProfile`
- `MultiRegionActiveActiveFailoverCertifierPlan`
- `MultiRegionActiveActiveFailoverCertifierRun`
- `MultiRegionActiveActiveFailoverCertifierDecision`

Required semantic capabilities:

- model region topology and failure domains
- verify data, cache and session replication
- exercise active-active conflict handling
- perform failover/failback under load
- confirm residency, RTO/RPO and cost envelope
...
- `artifacts/elmos-multi-region-active-active-failover-certifier/profile.yaml`
- `artifacts/elmos-multi-region-active-active-failover-certifier/plan.json`
- `artifacts/elmos-multi-region-active-active-failover-certifier/result.json`
- `artifacts/elmos-multi-region-active-active-failover-certifier/evidence/`
...
- **split brain** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **session routed without memory** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **failback overwrites newer state** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **regional egress violates policy** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **DNS TTL makes RTO impossible** — must have a preventive control, a detective signal, a negative fixture and a response action.
...
- native scenario: model region topology and failure domains
- native scenario: verify data, cache and session replication
- native scenario: exercise active-active conflict handling
- native scenario: perform failover/failback under load
- native scenario: confirm residency, RTO/RPO and cost envelope
