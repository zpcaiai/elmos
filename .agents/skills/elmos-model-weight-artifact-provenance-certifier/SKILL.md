---
name: elmos-model-weight-artifact-provenance-certifier
description: Certify model weights, tokenizer, adapters, quantization, conversion and serving artifacts from source through registry, build, signature and deployment.
version: 4.0.0
...
# Model Weight and Artifact Provenance Certifier
...
- `ModelWeightAndArtifactProvenanceCertifierProfile`
- `ModelWeightAndArtifactProvenanceCertifierPlan`
- `ModelWeightAndArtifactProvenanceCertifierRun`
- `ModelWeightAndArtifactProvenanceCertifierDecision`

Required semantic capabilities:

- inventory weight, tokenizer, adapter and config digests
- trace conversion, merge, quantization and packaging lineage
- verify signatures, licenses and allowed use
- reproduce selected artifact transformations
- bind serving deployment to certified artifact graph
...
- `elmos-ai-bom-provenance-generator` from `elmos-ai-capability-enhancement-skills` — required base evidence producer.
- `elmos-model-license-usage-restriction-governor` from `elmos-ai-capability-enhancement-skills` — required base evidence producer.

## Primary Outputs

- `artifacts/elmos-model-weight-artifact-provenance-certifier/profile.yaml`
- `artifacts/elmos-model-weight-artifact-provenance-certifier/plan.json`
- `artifacts/elmos-model-weight-artifact-provenance-certifier/result.json`
- `artifacts/elmos-model-weight-artifact-provenance-certifier/evidence/`
...
- **weight file substituted after evaluation** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **tokenizer mismatch** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **adapter merged into wrong base** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **quantization provenance missing** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **license restriction omitted from deployment** — must have a preventive control, a detective signal, a negative fixture and a response action.
...
- native scenario: inventory weight, tokenizer, adapter and config digests
- native scenario: trace conversion, merge, quantization and packaging lineage
- native scenario: verify signatures, licenses and allowed use
- native scenario: reproduce selected artifact transformations
- native scenario: bind serving deployment to certified artifact graph
