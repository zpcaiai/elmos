---
name: elmos-adapter-ims-mfs
description: Implement IMS MFS repository-dsl parsing, semantic lowering, framework/runtime
  mapping, target emission and verification for repository-scale modernization.
version: 1.0.0
skill_id: ELMOS-POLY-104
...
triggers:
- Use when implementing or executing `elmos-adapter-ims-mfs`.
- Use when the migration DAG requires `technology-adapter` capability.
- Use when `ims-mfs` is detected as source, target or repository support surface.
outputs:
- adapters/ims-mfs/adapter-manifest.json
- adapters/ims-mfs/semantic-mapping.yaml
- adapters/ims-mfs/verification-profile.yaml
---
...
Implement IMS MFS repository-dsl parsing, semantic lowering, framework/runtime mapping, target emission and verification for repository-scale modernization.
...
- Technology ID: `ims-mfs`
- Display: IMS MFS
- Kind: repository-dsl
- File/build markers: .mfs
- Build systems: owner runtime/toolchain
- Preferred analyzers: MFS parser
