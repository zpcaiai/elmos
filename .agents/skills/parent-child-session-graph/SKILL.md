---
name: parent-child-session-graph
description: 子 Agent 拥有独立可回放会话，并保留父任务关系。 Use for this exact ELMOS Spring Golden Route v2 contract; imported specifications are not runtime or certification evidence.
...
metadata:
  ...
  source_id: FOUNDATION-06-parent-child-session-graph
  source_name: parent-child-session-graph
  source_path: skills/parent-child-session-graph/SKILL.md
  source_sha256: sha256:b5e89a292bd16c5d3c763d6ce6b3d507835fe92c671c63857d7cb55c60d9fc32
  source_contract_sha256: sha256:ba915ee5147d9ba12a83966fb8f767256edb45f2c6cec721cb585faa531daf3b
...
子 Agent 拥有独立可回放会话，并保留父任务关系。

## Use when

在 Elmos 需要实现或升级 **Parent/Child Session Graph** 相关能力时使用；它不是仅供解释的提示词，而是实现、测试与验收契约。

## Capabilities to implement

- child session 记录 parent/task/delegation contract
- 父 Agent 只接收结构化 summary/artifacts
- 支持跳转查看子 Agent 完整证据
- 取消父任务时按策略传播到子任务
...
- [ ] 定义 `parent-child-session-graph` 的 public interface、input/output/error schemas。
- [ ] 建立 `parent-child-session-graph` 的 capability/feature flags 与版本字段。
...
- [ ] 至少覆盖并验收：child session 记录 parent/task/delegation contract；以及 父 Agent 只接收结构化 summary/artifacts。
...
- Machine contract: `references/contract.json` (`sha256:ba915ee5147d9ba12a83966fb8f767256edb45f2c6cec721cb585faa531daf3b`).
