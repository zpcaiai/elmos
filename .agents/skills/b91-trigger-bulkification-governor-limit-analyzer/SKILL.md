---
name: b91-trigger-bulkification-governor-limit-analyzer
description: "Use when ELMOS must run trigger bulkification governor limit analyzer for Batch 91 Salesforce Apex / SOQL / LWC Pack. Preserve native semantics, safety boundaries, ownership, traceability, and fail-closed evidence; require real vendor toolchain execution for runtime claims."
metadata:
  source_package: "elmos-language-packs-batch81-95-complete"
  source_id: "PG346"
  source_name: "trigger-bulkification-governor-limit-analyzer"
  source_sha256: "sha256:78294a78a9e6feaf5ca5b301d59ba7744076443da2aa2093261b958521bbbd3c"
  batch: 91
  source_engine: "elmos.language-packs"
  source_status: "proposed"
  normalized_namespace: "language-pack"
---
# Batch 91 Language Pack / source PG346 — Trigger Bulkification Governor Limit Analyzer

## 1. Objective

Analyze trigger bulkification governor limit analyzer within the Salesforce Apex / SOQL / LWC Pack, preserving platform semantics, traceability, security, and reproducible evidence.

## 2. Scope

This Skill is part of **Batch 91: Salesforce Apex / SOQL / LWC Pack**.
...
- Process metadata/package definitions.
- Produce Apex/LWC projects.
- Produce bulk-safe triggers.
- Produce Flows.
- Produce Platform Event integrations.
- Produce SFDX packages.
...
2. Process the relevant artifacts for Trigger Bulkification Governor Limit Analyzer using the approved enterprise-crm-platform semantic contracts and toolchain versions.
3. Preserve source provenance and create Artifact Graph links across inputs, outputs, tests, approvals, and evidence.
4. Detect and classify governor-limit breach, sharing/FLS bypass, trigger recursion; never convert uncertainty or unsupported behavior into a silent success.
...
7. Apply the safety boundary: CRUD, field-level security, sharing, tenant boundaries, and governor limits are mandatory correctness properties rather than optional optimizations.
...
- `GOVERNOR_LIMIT_BREACH` — stop unsafe continuation and emit source-linked diagnostic evidence.
- `SHARING_FLS_BYPASS` — stop unsafe continuation and emit source-linked diagnostic evidence.
- `TRIGGER_RECURSION` — stop unsafe continuation and emit source-linked diagnostic evidence.
- `BULKIFICATION_FAILURE` — stop unsafe continuation and emit source-linked diagnostic evidence.
- `METADATA_DEPENDENCY_GAP` — stop unsafe continuation and emit source-linked diagnostic evidence.
- `ORG_SPECIFIC_ASSUMPTION` — stop unsafe continuation and emit source-linked diagnostic evidence.
...
- Apply this Batch safety boundary: **CRUD, field-level security, sharing, tenant boundaries, and governor limits are mandatory correctness properties rather than optional optimizations.**

## 14. Unit Tests

- A representative valid salesforce input produces all declared outputs for trigger-bulkification-governor-limit-analyzer.
...
- At least one batch-specific risk (governor-limit breach) is detected by a negative fixture.

## 15. Integration Tests

- Consume approved Project Synthesis or Modernization baselines and write complete lineage into the Artifact Graph.
- Integrate with at least one representative toolchain from: Salesforce DX, Apex, SOQL/SOSL, LWC.
...
- Reject or block governor-limit breach without weakening the expected result.
- Reject or block sharing/FLS bypass without weakening the expected result.
- Reject or block trigger recursion without weakening the expected result.
- Reject or block bulkification failure without weakening the expected result.
- Reject or block metadata dependency gap without weakening the expected result.
- Reject or block org-specific assumption without weakening the expected result.
