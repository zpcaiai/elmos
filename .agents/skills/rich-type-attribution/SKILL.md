---
name: rich-type-attribution
description: 把类型归因从“可选附加信息”提升为迁移和搜索的基础设施。 Use for this exact ELMOS Spring Golden Route v2 contract; imported specifications are not runtime or certification evidence.
...
metadata:
  ...
  source_id: FOUNDATION-01-rich-type-attribution
  source_name: rich-type-attribution
  source_path: skills/rich-type-attribution/SKILL.md
  source_sha256: sha256:78fb2fd19344a121c9aa77fd521c7cb0116c89c9d810d653796d3b33bd220a26
  source_contract_sha256: sha256:c6b22112af6cc33183df5c3df2bb3141f14d22b8b0a6206315efadefd72d89ac
...
# Rich Type Attribution

## Goal

把类型归因从“可选附加信息”提升为迁移和搜索的基础设施。

## Use when

在 Elmos 需要实现或升级 **Rich Type Attribution** 相关能力时使用；它不是仅供解释的提示词，而是实现、测试与验收契约。

## Capabilities to implement

- 对标识符、调用、字段、泛型、继承等建立稳定类型引用
- 区分未知、未实现、解析失败，禁止已知节点退化成 Unknown
- 支持缺失类型信息时的显式降级策略和置信度
- 向 Recipe/Agent 暴露 type-aware 查询而非纯文本猜测
...
- [ ] 定义 `rich-type-attribution` 的 public interface、input/output/error schemas。
- [ ] 建立 `rich-type-attribution` 的 capability/feature flags 与版本字段。
...
- [ ] 至少覆盖并验收：对标识符、调用、字段、泛型、继承等建立稳定类型引用；以及 区分未知、未实现、解析失败，禁止已知节点退化成 Unknown。
...
- Machine contract: `references/contract.json` (`sha256:c6b22112af6cc33183df5c3df2bb3141f14d22b8b0a6206315efadefd72d89ac`).
