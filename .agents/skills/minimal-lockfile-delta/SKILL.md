---
name: minimal-lockfile-delta
description: 锁文件更新默认采用最小变更，而非全图重新解析。 Use for this exact ELMOS Spring Golden Route v2 contract; imported specifications are not runtime or certification evidence.
...
metadata:
  ...
  source_id: FOUNDATION-04-minimal-lockfile-delta
  source_name: minimal-lockfile-delta
  source_path: skills/minimal-lockfile-delta/SKILL.md
  source_sha256: sha256:e78424ffa48d4c002b14c8593d9fb431c381522d5beb965dbfb2f5a0a4ad7710
  source_contract_sha256: sha256:e17afae3abbbd2b3138ff3ed1f268b30d5d2ef6954534d8b44026b23faa20b7c
...
锁文件更新默认采用最小变更，而非全图重新解析。

## Use when

在 Elmos 需要实现或升级 **Minimal Lockfile Delta** 相关能力时使用；它不是仅供解释的提示词，而是实现、测试与验收契约。

## Capabilities to implement

- 只移动目标包及其依赖约束真正迫使变化的节点
- 未受影响 entry 尽量字节级保持
- 可解释为何某个 transitive pin 被迫改变
- 减少遗留依赖导致的无关失败面
...
- [ ] 定义 `minimal-lockfile-delta` 的 public interface、input/output/error schemas。
- [ ] 建立 `minimal-lockfile-delta` 的 capability/feature flags 与版本字段。
...
- [ ] 至少覆盖并验收：只移动目标包及其依赖约束真正迫使变化的节点；以及 未受影响 entry 尽量字节级保持。
...
- Machine contract: `references/contract.json` (`sha256:e17afae3abbbd2b3138ff3ed1f268b30d5d2ef6954534d8b44026b23faa20b7c`).
