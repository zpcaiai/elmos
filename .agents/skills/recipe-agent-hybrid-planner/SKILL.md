---
name: recipe-agent-hybrid-planner
description: 一个迁移计划可混合确定性 Recipe、Agent coding、Tool、人工审批和验证。 Use for this exact ELMOS Spring Golden Route v2 contract; imported specifications are not runtime or certification evidence.
...
metadata:
  ...
  source_id: FOUNDATION-10-recipe-agent-hybrid-planner
  source_name: recipe-agent-hybrid-planner
  source_path: skills/recipe-agent-hybrid-planner/SKILL.md
  source_sha256: sha256:41a7823438f7ecff446b543a80071bcb8283f6f152e6a3b7294e0d8264021083
  source_contract_sha256: sha256:e3c4afba847cd6f0677f7387b1b02c270ea9f4dac1657a3abe13736db9eb96de
...
# Recipe + Agent Hybrid Planner

## Goal

一个迁移计划可混合确定性 Recipe、Agent coding、Tool、人工审批和验证。

## Use when

在 Elmos 需要实现或升级 **Recipe + Agent Hybrid Planner** 相关能力时使用；它不是仅供解释的提示词，而是实现、测试与验收契约。

## Capabilities to implement

- Planner 输出 typed DAG 而非自然语言清单
- 每节点声明 executor/inputs/outputs/side-effect/rollback
- 确定性节点可缓存和复用
- LLM 节点只接收必要 semantic slice
...
- [ ] 定义 `recipe-agent-hybrid-planner` 的 public interface、input/output/error schemas。
- [ ] 建立 `recipe-agent-hybrid-planner` 的 capability/feature flags 与版本字段。
...
- [ ] 至少覆盖并验收：Planner 输出 typed DAG 而非自然语言清单；以及 每节点声明 executor/inputs/outputs/side-effect/rollback。
...
- Machine contract: `references/contract.json` (`sha256:e3c4afba847cd6f0677f7387b1b02c270ea9f4dac1657a3abe13736db9eb96de`).
