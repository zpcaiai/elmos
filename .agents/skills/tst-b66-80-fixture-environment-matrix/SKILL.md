---
name: tst-b66-80-fixture-environment-matrix
description: "Fixture与环境矩阵。冻结版本、平台、架构、数据库、Runner、设备、集群和代表性Fixture。 Use for ELMOS Batch 66–80 slightly strict certification tests."
metadata:
  id: X003
...
---
    ...
    冻结版本、平台、架构、数据库、Runner、设备、集群和代表性Fixture。 覆盖成功、边界、负向、故障、重放、恢复、篡改和状态冒充。默认不得把未运行、Skipped、Mock-only或陈旧Evidence计为通过。
    ...
    ### X003-01 — Fixture与环境矩阵 / 版本与平台矩阵冻结

- Priority: `P0`
- Category: `environment`
- Given: 提交包含OS、架构、工具链、时区、Locale和依赖服务的环境清单
- When: 执行本Case定义的测试、故障注入和证据收集
- Then: 所有运行绑定环境Hash，缺失关键版本时不得执行认证
- Evidence: source-snapshot, environment-manifest, raw-command-log, result-json, artifact-manifest, replay-command

### X003-02 — Fixture与环境矩阵 / 时间随机端口与网络控制

- Priority: `P1`
- Category: `determinism`
- Given: 重复运行使用受控时钟、Seed、端口和网络
- When: 执行本Case定义的测试、故障注入和证据收集
- Then: 结果可重放且无隐式公网依赖
- Evidence: source-snapshot, environment-manifest, raw-command-log, result-json, artifact-manifest, replay-command

### X003-03 — Fixture与环境矩阵 / 代表性Fixture与Holdout分离

- Priority: `P2`
- Category: `holdout`
- Given: 运行公开Fixture和未用于生成的Holdout
- When: 执行本Case定义的测试、故障注入和证据收集
- Then: Holdout结果独立记录，禁止自动更新Golden
- Evidence: source-snapshot, environment-manifest, raw-command-log, result-json, artifact-manifest, replay-command
