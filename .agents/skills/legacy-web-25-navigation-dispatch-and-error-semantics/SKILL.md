---
name: legacy-web-25-navigation-dispatch-and-error-semantics
description: "Repository-owned exact runtime interface for 导航、分派与错误语义恢复; bounded semantic analysis and evidence generation for Java legacy web modernization."
metadata:
  source_package: elmos.java-legacy-web.repository-modernization
  source_version: 1.0.0
  source_id: 25-navigation-dispatch-and-error-semantics
  source_digest: sha256:e1ed7e2fe3b106c1f1a24dbdfbc5bcc98b09edc8c552787d485c5141751f54b9
  phase: semantic-recovery
  runtime_state: CODE_COMPLETE_LOCAL
  capability_state: LOCAL_EXECUTED
  operation_code: NAVIGATION_SEMANTICS_RECOVERED
  runtime_handler_id: legacy-web-handler:25-navigation-dispatch-and-error-semantics
---
...
---
id: 25-navigation-dispatch-and-error-semantics
title: 导航、分派与错误语义恢复
version: 1.0.0
phase: semantic-recovery
priority: P0
...
统一建模 forward、include、redirect、chain、error dispatch、welcome file、result code、状态码、请求属性和 URL 编码。
...
producerSkill: 25-navigation-dispatch-and-error-semantics
...
6. 以 Navigation Algebra 表示 `forward/include/redirect/chain/error/render/stream/none`。
7. 记录状态码、Location 编码、context path、query string、request attributes、response commit 状态。
8. 计算 global/local exception mapping 和 result 匹配优先级。
9. 任何 forward→redirect 或 include→render 变化均视为默认 P0 mismatch。
