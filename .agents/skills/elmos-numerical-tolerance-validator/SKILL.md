---
name: elmos-numerical-tolerance-validator
description: Validate numerical outputs across representative and adversarial datasets
  using declared absolute/relative/ULP tolerances and performance budgets.
version: 1.0.0
skill_id: ELMOS-POLY-161
layer: validation
risk: high
readiness: not-run
dependencies:
...
triggers:
- Use when implementing or executing `elmos-numerical-tolerance-validator`.
- Use when the migration DAG requires `validation` capability.
outputs:
- validation/elmos-numerical-tolerance-validator/result.json
- validation/elmos-numerical-tolerance-validator/evidence/
---

# Numerical Tolerance Validator

## Objective

Validate numerical outputs across representative and adversarial datasets using declared absolute/relative/ULP tolerances and performance budgets.
