---
name: b39-problem-root-cause-loop
description: "Implement and verify Batch 39 Skill 1357 for problem root cause loop. Use when work requires Problem Management\u548c\u957f\u671fRoot Cause\u95ed\u73af, typed evidence, negative and holdout validation, and a fail-closed certification decision."
---

# Problem Management和长期Root Cause闭环
...
## Skill 1357: Problem Management和长期Root Cause闭环
...
1. Inspect the current implementation, runtime facts, policies, incidents, decisions, and prior evidence relevant to `b39-problem-root-cause-loop`.
...
4. Implement the smallest production-shaped capability that exercises `Problem Management和长期Root Cause闭环` without bypassing existing control planes.
...
- Capability-specific implementation, tests, run logs, evidence references, and residual-risk records for `b39-problem-root-cause-loop`
