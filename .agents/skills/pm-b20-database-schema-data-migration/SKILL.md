...
- Invoke this repository Skill as `$pm-b20-database-schema-data-migration`.
- Immutable source identity: `batch-20-database-schema-data-migration` in `precision-migration-b01-44` (B20).
- Runtime adapter: `database-and-data-route`; binding state: `DECLARED`.
- Resolve and plan with `python3 scripts/precision_migration/runtime.py plan --skill pm-b20-database-schema-data-migration`.
...
- 当任务涉及 **schema-inventory** 时，调用 `../pm-b20-schema-inventory/SKILL.md`。
- 当任务涉及 **data-type-mapping** 时，调用 `../pm-b20-data-type-mapping/SKILL.md`。
- 当任务涉及 **table-and-constraint-converter** 时，调用 `../pm-b20-table-and-constraint-converter/SKILL.md`。
- 当任务涉及 **index-and-partition-converter** 时，调用 `../pm-b20-index-and-partition-converter/SKILL.md`。
- 当任务涉及 **view-and-materialized-view-converter** 时，调用 `../pm-b20-view-and-materialized-view-converter/SKILL.md`。
- 当任务涉及 **sequence-and-identity-converter** 时，调用 `../pm-b20-sequence-and-identity-converter/SKILL.md`。
- 当任务涉及 **data-profile-and-quality-analysis** 时，调用 `../pm-b20-data-profile-and-quality-analysis/SKILL.md`。
- 当任务涉及 **full-load-migration** 时，调用 `../pm-b20-full-load-migration/SKILL.md`。
- 当任务涉及 **incremental-cdc-migration** 时，调用 `../pm-b20-incremental-cdc-migration/SKILL.md`。
- 当任务涉及 **data-checksum-validation** 时，调用 `../pm-b20-data-checksum-validation/SKILL.md`。
- 当任务涉及 **row-level-differential-validation** 时，调用 `../pm-b20-row-level-differential-validation/SKILL.md`。
- 当任务涉及 **cutover-and-rollback-plan** 时，调用 `../pm-b20-cutover-and-rollback-plan/SKILL.md`。
