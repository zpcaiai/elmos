---
name: elmos-openfeature-progressive-delivery-safety-controller
description: Generate and govern feature-flag evaluation, targeting, exposure, shadow, canary, rollback and kill-switch controls for generated AI systems.
version: 4.0.0
...
# OpenFeature Progressive Delivery Safety Controller
...
- `OpenFeatureProgressiveDeliverySafetyControllerProfile`
- `OpenFeatureProgressiveDeliverySafetyControllerPlan`
- `OpenFeatureProgressiveDeliverySafetyControllerRun`
- `OpenFeatureProgressiveDeliverySafetyControllerDecision`

Required semantic capabilities:

- compile provider-neutral flag and context contracts
- bind flag state to exact artifact and policy digests
- enforce tenant/risk cohort targeting
- record exposure and guardrail evidence
- rollback atomically across model, prompt, cache and workflow
...
- `artifacts/elmos-openfeature-progressive-delivery-safety-controller/profile.yaml`
- `artifacts/elmos-openfeature-progressive-delivery-safety-controller/plan.json`
- `artifacts/elmos-openfeature-progressive-delivery-safety-controller/result.json`
- `artifacts/elmos-openfeature-progressive-delivery-safety-controller/evidence/`
...
- **flag default enables unsafe feature** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **evaluation context leaks tenant data** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **control and candidate share state** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **rollback leaves incompatible cache** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **manual override bypasses certification scope** — must have a preventive control, a detective signal, a negative fixture and a response action.
...
- native scenario: compile provider-neutral flag and context contracts
- native scenario: bind flag state to exact artifact and policy digests
- native scenario: enforce tenant/risk cohort targeting
- native scenario: record exposure and guardrail evidence
- native scenario: rollback atomically across model, prompt, cache and workflow
