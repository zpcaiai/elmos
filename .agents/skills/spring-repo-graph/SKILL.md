---
name: spring-repo-graph
description: 构建模块、符号、Bean、Endpoint、数据、事务、安全、消息、配置和测试 typed graph。 Use for this exact ELMOS Spring Golden Route v2 contract; imported specifications are not runtime or certification evidence.
...
metadata:
  ...
  source_id: ELMOS-GR-035
  source_name: spring-repo-graph
  source_path: skills/spring-repo-graph/SKILL.md
  source_sha256: sha256:157b052264bc5fa5b7c633f95264886126f48e2cd093b7a67ad828c5d03c20f0
  source_contract_sha256: sha256:3e242ada2e702bd6550eac4838b951aad6f7b6f63b6a01903e55b56c9be637a4
...
构建模块、符号、Bean、Endpoint、数据、事务、安全、消息、配置和测试 typed graph。
...
- [ ] 定义 `spring-repo-graph` 的 public interface、输入/输出/Error Schema 和版本策略。
- [ ] 实现 `spring-repo-graph` 的 production adapter/service，不把业务状态只保存在 Prompt 或内存对象中。
...
- [ ] 至少生成：repository-graph, graph-manifest.json, completeness.json。
...
- [ ] 本 Skill 的 required outputs 均由真实目标仓库执行产生：repository-graph, graph-manifest.json, completeness.json。
...
- Machine contract: `references/contract.json` (`sha256:3e242ada2e702bd6550eac4838b951aad6f7b6f63b6a01903e55b56c9be637a4`).
