...
- Invoke this repository Skill as `$pm-b27-cross-layer-performance-tuning`.
- Immutable source identity: `cross-layer-performance-tuning` in `precision-migration-b01-44` (B27).
- Runtime adapter: `database-and-data-route`; binding state: `UNAVAILABLE`.
- Resolve and plan with `python3 scripts/precision_migration/runtime.py plan --skill pm-b27-cross-layer-performance-tuning`.
...
联合调优 SQL、索引、ORM、连接池、批处理、缓存和应用调用模式。

本 Skill 属于精密语言转换系统。它必须以**可执行证据**为准，不得用模型自评、源码相似度或“能编译”替代语义与行为验证。

## Use when

- 当前任务直接涉及 `cross-layer-performance-tuning` 所描述的能力。
...
1. 盘点对象与专有能力
2. 执行本 Skill 的核心职责：联合调优 SQL、索引、ORM、连接池、批处理、缓存和应用调用模式。
...
```yaml
skill: cross-layer-performance-tuning
version: 1.0.0
