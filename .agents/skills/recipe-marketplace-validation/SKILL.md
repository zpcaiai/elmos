---
name: recipe-marketplace-validation
description: 目录内容、实现和 bundle 必须双向完整一致。 Use for this exact ELMOS Spring Golden Route v2 contract; imported specifications are not runtime or certification evidence.
...
metadata:
  ...
  source_id: FOUNDATION-03-recipe-marketplace-validation
  source_name: recipe-marketplace-validation
  source_path: skills/recipe-marketplace-validation/SKILL.md
  source_sha256: sha256:8bf5e36ac4326a629c49acab179f046fa5e9148bc67322d5bec4082c7322fa45
  source_contract_sha256: sha256:2ff4f2468ac97bf17ce0128fb045ae903c446c64bcbbc1e2e782ef611161e90b
...
# Recipe Marketplace Validation

## Goal

目录内容、实现和 bundle 必须双向完整一致。

## Use when

在 Elmos 需要实现或升级 **Recipe Marketplace Validation** 相关能力时使用；它不是仅供解释的提示词，而是实现、测试与验收契约。

## Capabilities to implement

- 检测目录存在但实现缺失的 phantom recipe
- 检测实现存在但目录漏登记的 missing recipe
- 校验 displayName/description/schema 规范
- CI 中阻止 stale marketplace 发布
...
- [ ] 定义 `recipe-marketplace-validation` 的 public interface、input/output/error schemas。
- [ ] 建立 `recipe-marketplace-validation` 的 capability/feature flags 与版本字段。
...
- [ ] 至少覆盖并验收：检测目录存在但实现缺失的 phantom recipe；以及 检测实现存在但目录漏登记的 missing recipe。
...
- Machine contract: `references/contract.json` (`sha256:2ff4f2468ac97bf17ce0128fb045ae903c446c64bcbbc1e2e782ef611161e90b`).
