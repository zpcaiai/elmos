---
name: b35-verification-factory
description: Implement and certify a production-shaped advanced verification pack combining property metamorphic mutation fuzz symbolic model contract data security concurrency numeric solver oracle coverage counterexample and assurance-case evidence.
---
...
## Skill 1265: b35-verification-factory

## Use this skill when

- A user asks to add advanced verification beyond ordinary unit and differential testing.
- Several Batch 35 techniques must be coordinated for one production-shaped migrated workload.

## Domain-specific risks and invariants

- A collection of disconnected tools can produce contradictory claims and unrepeatable evidence.
- Unbounded verification can consume unlimited compute without improving decision confidence.

## Workflow

1. Inspect Batch 21-34 contracts, tests, evidence, manifests, known defects, security policies, and production risks; create a verification gap inventory.
2. Confirm accountable business, engineering, security, data, operations, and verification owners plus the exact workload and risk tier.
3. Scaffold the verification pack and define validation profile, oracle registry, support matrix, budgets, and release claims.
4. Implement the minimum production-shaped combination of property, metamorphic, mutation, fuzz, model, contract, data, security, concurrency, numeric, solver, and replay methods required by the profile.
5. Run development and negative corpora, then untouched holdout and representative workloads.
6. Build the assurance case, residual-risk register, and continuous regression hooks.
7. Run the conservative Batch 35 gate and retain only the strongest evidence-supported status.
...
- Coordinate the entire advanced verification lifecycle for one exact migration route, workload, artifact tuple, and approved risk profile.
...
- verification-pack.json, validation-profile.json, oracle-registry.json, assurance-case.json, certification evidence and a conservative gate result.
