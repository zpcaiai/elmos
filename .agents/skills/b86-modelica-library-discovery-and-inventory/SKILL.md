---
name: b86-modelica-library-discovery-and-inventory
description: "Use when ELMOS must run modelica library discovery and inventory for Batch 86 Modelica / FMI / FMU Digital Twin Pack. Preserve native semantics, safety boundaries, ownership, traceability, and fail-closed evidence; require real vendor toolchain execution for runtime claims."
metadata:
  source_package: "elmos-language-packs-batch81-95-complete"
  source_id: "PG283"
  source_name: "modelica-library-discovery-and-inventory"
  source_sha256: "sha256:a1860bd7353b0a68e374be2747acb67bcb1fb35fefee16fea819a607f1919475"
  batch: 86
  source_engine: "elmos.language-packs"
  source_status: "proposed"
  normalized_namespace: "language-pack"
---
# Batch 86 Language Pack / source PG283 — Modelica Library Discovery And Inventory

## 1. Objective

Discover and inventory modelica library discovery and inventory within the Modelica / FMI / FMU Digital Twin Pack, preserving platform semantics, traceability, security, and reproducible evidence.

## 2. Scope

This Skill is part of **Batch 86: Modelica / FMI / FMU Digital Twin Pack**.

### In scope

- Process Modelica classes.
- Process libraries.
- Process equations.
- Process connectors.
- Process experiment annotations.
- Process FMI model descriptions.
- Process FMUs.
- Process telemetry.
- Produce Modelica models.
- Produce FMU artifacts.
- Produce co-simulation plans.
- Produce calibrated parameters.
- Produce simulation regression evidence.
- Produce runtime twin.
...
1. Resolve the estate scope, access boundaries, dialects, versions, repositories, runtime systems, and authoritative source locations.
2. Process the relevant artifacts for Modelica Library Discovery And Inventory using the approved physical-modeling semantic contracts and toolchain versions.
3. Preserve source provenance and create Artifact Graph links across inputs, outputs, tests, approvals, and evidence.
4. Detect and classify unit inconsistency, over/under-determined equations, event chattering; never convert uncertainty or unsupported behavior into a silent success.
...
7. Apply the safety boundary: Simulation predictions must be labelled separately from observed physical state and cannot directly authorize unsafe control actions.
...
- `UNIT_INCONSISTENCY` — stop unsafe continuation and emit source-linked diagnostic evidence.
- `OVER_UNDER_DETERMINED_EQUATIONS` — stop unsafe continuation and emit source-linked diagnostic evidence.
- `EVENT_CHATTERING` — stop unsafe continuation and emit source-linked diagnostic evidence.
- `SOLVER_INSTABILITY` — stop unsafe continuation and emit source-linked diagnostic evidence.
- `FMU_COMPATIBILITY_DRIFT` — stop unsafe continuation and emit source-linked diagnostic evidence.
- `PARAMETER_IDENTIFIABILITY` — stop unsafe continuation and emit source-linked diagnostic evidence.
...
- Apply this Batch safety boundary: **Simulation predictions must be labelled separately from observed physical state and cannot directly authorize unsafe control actions.**

## 14. Unit Tests

- A representative valid modelica-fmi input produces all declared outputs for modelica-library-discovery-and-inventory.
...
- At least one batch-specific risk (unit inconsistency) is detected by a negative fixture.

## 15. Integration Tests

- Consume approved Project Synthesis or Modernization baselines and write complete lineage into the Artifact Graph.
- Integrate with at least one representative toolchain from: OpenModelica, Dymola-compatible models, FMI 2/3 adapters, FMU runtime.
...
- Reject or block unit inconsistency without weakening the expected result.
- Reject or block over/under-determined equations without weakening the expected result.
- Reject or block event chattering without weakening the expected result.
- Reject or block solver instability without weakening the expected result.
- Reject or block FMU compatibility drift without weakening the expected result.
- Reject or block parameter identifiability without weakening the expected result.
