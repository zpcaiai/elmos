---
name: pm-b23-sqlserver-service-broker
description: "迁移 Service Broker 队列、会话、合同、事务和激活. Precision Migration B23 contract; use for this exact assessment, transformation, validation, repair, evidence, or cutover scope."
---

# Sqlserver Service Broker
## ELMOS runtime binding

- Invoke this repository Skill as `$pm-b23-sqlserver-service-broker`.
- Immutable source identity: `sqlserver-service-broker` in `precision-migration-b01-44` (B23).
- Runtime adapter: `database-and-data-route`; binding state: `DECLARED`.
- Resolve and plan with `python3 scripts/precision_migration/runtime.py plan --skill pm-b23-sqlserver-service-broker`.
...
迁移 Service Broker 队列、会话、合同、事务和激活。

本 Skill 属于精密语言转换系统。它必须以**可执行证据**为准，不得用模型自评、源码相似度或“能编译”替代语义与行为验证。

## Use when

- 当前任务直接涉及 `sqlserver-service-broker` 所描述的能力。
...
1. 盘点对象与专有能力
2. 执行本 Skill 的核心职责：迁移 Service Broker 队列、会话、合同、事务和激活。
...
```yaml
skill: sqlserver-service-broker
version: 1.0.0
