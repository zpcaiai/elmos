---
name: elmos-vector-database-index-ir-compiler
description: Compile embedding schema, metric, normalization, filter, index algorithm, consistency, namespace, deletion and recall/latency envelopes across vector stores.
version: 4.0.0
...
# Vector Database and Index IR Compiler
...
- `VectorDatabaseAndIndexIRCompilerProfile`
- `VectorDatabaseAndIndexIRCompilerPlan`
- `VectorDatabaseAndIndexIRCompilerRun`
- `VectorDatabaseAndIndexIRCompilerDecision`

Required semantic capabilities:

- model vector dimensions, metric and normalization
- capture HNSW/IVF/PQ and vendor index parameters
- represent metadata filters and tenant namespaces
- compile update/delete/compaction semantics
- bind recall, latency and cost obligations
...
- `artifacts/elmos-vector-database-index-ir-compiler/profile.yaml`
- `artifacts/elmos-vector-database-index-ir-compiler/plan.json`
- `artifacts/elmos-vector-database-index-ir-compiler/result.json`
- `artifacts/elmos-vector-database-index-ir-compiler/evidence/`
...
- **metric mismatch** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **dimension silently projected** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **tenant namespace collision** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **deleted vectors remain searchable** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **index parameter migration degrades recall** — must have a preventive control, a detective signal, a negative fixture and a response action.
...
- native scenario: model vector dimensions, metric and normalization
- native scenario: capture HNSW/IVF/PQ and vendor index parameters
- native scenario: represent metadata filters and tenant namespaces
- native scenario: compile update/delete/compaction semantics
- native scenario: bind recall, latency and cost obligations
