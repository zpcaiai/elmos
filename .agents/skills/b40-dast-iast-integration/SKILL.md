...
description: "Implement and verify Batch 40 Skill 1375 for dast iast integration. Use when work requires DAST\u548cIAST\u96c6\u6210, typed evidence, negative and holdout validation, and a fail-closed certification decision."
---
...
1. Inspect the current implementation, runtime facts, policies, incidents, decisions, and prior evidence relevant to `b40-dast-iast-integration`.
...
4. Implement the smallest production-shaped capability that exercises `DAST和IAST集成` without bypassing existing control planes.
...
- Capability-specific implementation, tests, run logs, evidence references, and residual-risk records for `b40-dast-iast-integration`
