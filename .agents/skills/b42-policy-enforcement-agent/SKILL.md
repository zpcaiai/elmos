...
description: "Implement and verify Batch 42 Skill 1419 for policy enforcement agent. Use when work requires Policy Enforcement Agent, typed evidence, negative and holdout validation, and a fail-closed certification decision."
---
...
## Skill 1419: Policy Enforcement Agent
...
1. Inspect the current implementation, runtime facts, policies, incidents, decisions, and prior evidence relevant to `b42-policy-enforcement-agent`.
...
4. Implement the smallest production-shaped capability that exercises `Policy Enforcement Agent` without bypassing existing control planes.
...
- Capability-specific implementation, tests, run logs, evidence references, and residual-risk records for `b42-policy-enforcement-agent`
