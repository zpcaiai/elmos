---
name: elmos-gitops-argocd-flux-release-controller
description: Generate signed declarative release, environment promotion, health, rollback and drift reconciliation workflows.
version: 4.0.0
...
# GitOps Argo CD and Flux Release Controller
...
- `GitOpsArgoCDAndFluxReleaseControllerProfile`
- `GitOpsArgoCDAndFluxReleaseControllerPlan`
- `GitOpsArgoCDAndFluxReleaseControllerRun`
- `GitOpsArgoCDAndFluxReleaseControllerDecision`

Required semantic capabilities:

- publish signed immutable manifests
- enforce promotion by evidence gate
- configure health and sync waves
- detect and classify drift
- rollback Git, image, schema and model coherently
...
- `elmos-production-deployment-certifier` from `elmos-functional-assurance-certification-skills` — optional companion; required only for E4/E5/P05 certification.

## Primary Outputs

- `artifacts/elmos-gitops-argocd-flux-release-controller/profile.yaml`
- `artifacts/elmos-gitops-argocd-flux-release-controller/plan.json`
- `artifacts/elmos-gitops-argocd-flux-release-controller/result.json`
- `artifacts/elmos-gitops-argocd-flux-release-controller/evidence/`
...
- **manual cluster change overwritten unsafely** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **Git commit not signed** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **health check ignores migration** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **rollback manifest references deleted image** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **drift auto-heal masks attack** — must have a preventive control, a detective signal, a negative fixture and a response action.
...
- native scenario: publish signed immutable manifests
- native scenario: enforce promotion by evidence gate
- native scenario: configure health and sync waves
- native scenario: detect and classify drift
- native scenario: rollback Git, image, schema and model coherently
