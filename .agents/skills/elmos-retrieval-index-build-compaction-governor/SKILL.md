---
name: elmos-retrieval-index-build-compaction-governor
description: Govern full/incremental index builds, segment merge, compaction, replicas, consistency, resource budgets and rollback across retrieval stores.
version: 4.0.0
...
# Retrieval Index Build and Compaction Governor
...
- `RetrievalIndexBuildAndCompactionGovernorProfile`
- `RetrievalIndexBuildAndCompactionGovernorPlan`
- `RetrievalIndexBuildAndCompactionGovernorRun`
- `RetrievalIndexBuildAndCompactionGovernorDecision`

Required semantic capabilities:

- plan versioned full and incremental builds
- coordinate replicas and online cutover
- verify compaction does not lose ACL/deletes
- bound CPU, memory, IO and duration
- retain rollback index until certification
...
- `artifacts/elmos-retrieval-index-build-compaction-governor/profile.yaml`
- `artifacts/elmos-retrieval-index-build-compaction-governor/plan.json`
- `artifacts/elmos-retrieval-index-build-compaction-governor/result.json`
- `artifacts/elmos-retrieval-index-build-compaction-governor/evidence/`
...
- **partial index published** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **compaction resurrects deleted item** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **replica versions mixed** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **resource surge affects production** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **rollback index garbage-collected** — must have a preventive control, a detective signal, a negative fixture and a response action.
...
- native scenario: plan versioned full and incremental builds
- native scenario: coordinate replicas and online cutover
- native scenario: verify compaction does not lose ACL/deletes
- native scenario: bound CPU, memory, IO and duration
- native scenario: retain rollback index until certification
