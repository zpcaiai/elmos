---
name: architecture-context-extractor
description: 从代码/配置/依赖提取可机器查询的架构图和边界。 Use for this exact ELMOS Spring Golden Route v2 contract; imported specifications are not runtime or certification evidence.
...
metadata:
  ...
  source_id: FOUNDATION-05-architecture-context-extractor
  source_name: architecture-context-extractor
  source_path: skills/architecture-context-extractor/SKILL.md
  source_sha256: sha256:be67c07762b880c5084fdbb74badcf758a1ec101aac5444ca75f9f3c062375ce
  source_contract_sha256: sha256:fa2909b5bad0dacfe0b208e2c27f615057e23de6fd5af099eda58271881a001e
...
从代码/配置/依赖提取可机器查询的架构图和边界。

## Use when

在 Elmos 需要实现或升级 **Architecture Context Extractor** 相关能力时使用；它不是仅供解释的提示词，而是实现、测试与验收契约。

## Capabilities to implement

- 模块、服务、数据库、消息、外部调用形成 typed graph
- 可导出 CALM/JSON/图数据库表示
- 架构变更前后可 diff
- 用于影响分析和迁移规划
...
- [ ] 定义 `architecture-context-extractor` 的 public interface、input/output/error schemas。
- [ ] 建立 `architecture-context-extractor` 的 capability/feature flags 与版本字段。
...
- [ ] 至少覆盖并验收：模块、服务、数据库、消息、外部调用形成 typed graph；以及 可导出 CALM/JSON/图数据库表示。
...
- Machine contract: `references/contract.json` (`sha256:fa2909b5bad0dacfe0b208e2c27f615057e23de6fd5af099eda58271881a001e`).
