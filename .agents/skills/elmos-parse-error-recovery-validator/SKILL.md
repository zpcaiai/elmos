...
triggers:
  - "Use when implementing or executing `elmos-parse-error-recovery-validator`."
  - "Use when a migration/certification DAG requires `frontend-semantics` capability."
outputs:
  - "semantic-assurance/elmos-parse-error-recovery-validator/model.json"
  - "semantic-assurance/elmos-parse-error-recovery-validator/evidence.json"
  - "semantic-assurance/elmos-parse-error-recovery-validator/diagnostics.json"
---

# Parse Error Recovery Validator

## Objective

Validate partial and malformed repositories without allowing parser recovery to silently invent executable semantics.
...
- **generated/broken source classification.**
- **partial AST confidence.**
- **repair-before-transform policy.**
...
- [ ] recovery node cannot enter transform without approval.
