...
# Procedure Symbolic Validator
## ELMOS runtime binding

- Invoke this repository Skill as `$pm-b21-procedure-symbolic-validator`.
- Immutable source identity: `procedure-symbolic-validator` in `precision-migration-b01-44` (B21).
- Runtime adapter: `database-and-data-route`; binding state: `DECLARED`.
- Resolve and plan with `python3 scripts/precision_migration/runtime.py plan --skill pm-b21-procedure-symbolic-validator`.
...
对关键过程切片执行符号或关系验证，寻找源目标反例。

本 Skill 属于精密语言转换系统。它必须以**可执行证据**为准，不得用模型自评、源码相似度或“能编译”替代语义与行为验证。

## Use when

- 当前任务直接涉及 `procedure-symbolic-validator` 所描述的能力。
...
1. 盘点对象与专有能力
2. 执行本 Skill 的核心职责：对关键过程切片执行符号或关系验证，寻找源目标反例。
...
```yaml
skill: procedure-symbolic-validator
version: 1.0.0
