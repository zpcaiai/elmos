---
name: legacy-web-50-deterministic-ast-and-config-rewrite
description: "Repository-owned exact runtime interface for 确定性 AST 与配置改写; bounded semantic analysis and evidence generation for Java legacy web modernization."
metadata:
  source_package: elmos.java-legacy-web.repository-modernization
  source_version: 1.0.0
  source_id: 50-deterministic-ast-and-config-rewrite
  source_digest: sha256:a0d7b0ee77e421695d909a5fcac18c01179ce5dc8a24c6076865ad0b6d0da3bb
  phase: transformation
  runtime_state: CODE_COMPLETE_LOCAL
  capability_state: LOCAL_EXECUTED
  operation_code: DETERMINISTIC_STRUCTURED_REWRITE_COMPILED
  runtime_handler_id: legacy-web-handler:50-deterministic-ast-and-config-rewrite
---
...
---
id: 50-deterministic-ast-and-config-rewrite
title: 确定性 AST 与配置改写
version: 1.0.0
phase: transformation
priority: P0
...
以类型解析、XML/descriptor 模型和 OpenRewrite 风格 recipe 实施可重复改写；LLM 仅处理有证据约束的语义缺口。
...
producerSkill: 50-deterministic-ast-and-config-rewrite
...
5. 以幂等 change set 发布。
6. recipe 前置条件必须匹配 IR/evidence，禁止仅凭文本命中。
7. 改写后执行格式化、imports、符号解析和结构验证。
8. 对 XML 保留注释/顺序中有语义的节点，并输出 structural diff。
9. LLM patch 必须通过 AST parse 和 policy lint。
