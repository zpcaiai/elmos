---
name: spring-sandbox
description: 在隔离 Runner 执行不可信构建，Secret 通过短期最小权限 Broker 注入。 Use for this exact ELMOS Spring Golden Route v2 contract; imported specifications are not runtime or certification evidence.
...
metadata:
  ...
  source_id: ELMOS-GR-056
  source_name: spring-sandbox
  source_path: skills/spring-sandbox/SKILL.md
  source_sha256: sha256:9e9d0835951a6ee58a26ded0d42a49baa9d49c55b557e65e8f9c8776c803b82e
  source_contract_sha256: sha256:83933d28f3938dd0c56776b24cb00adeb7c94f0c0188ac4b91bfbb3bf53305f1
...
在隔离 Runner 执行不可信构建，Secret 通过短期最小权限 Broker 注入。
...
- [ ] 定义 `spring-sandbox` 的 public interface、输入/输出/Error Schema 和版本策略。
- [ ] 实现 `spring-sandbox` 的 production adapter/service，不把业务状态只保存在 Prompt 或内存对象中。
...
- [ ] 至少生成：sandbox-evidence.json, resource-usage.json, security-findings.json。
...
- [ ] 本 Skill 的 required outputs 均由真实目标仓库执行产生：sandbox-evidence.json, resource-usage.json, security-findings.json。
- [ ] 所有 mandatory tests 和 security gates 通过，且无 critical unknown。

## Dependencies

- `spring-human-approval`
...
- Machine contract: `references/contract.json` (`sha256:83933d28f3938dd0c56776b24cb00adeb7c94f0c0188ac4b91bfbb3bf53305f1`).
