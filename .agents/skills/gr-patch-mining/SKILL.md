---
name: gr-patch-mining
description: 从经验证且获授权的 Semantic Patch 聚类重复模式，发现 Recipe candidate。 Use for this exact ELMOS Spring Golden Route v2 contract; imported specifications are not runtime or certification evidence.
...
metadata:
  ...
  source_id: ELMOS-GR-093
  source_name: gr-patch-mining
  source_path: skills/gr-patch-mining/SKILL.md
  source_sha256: sha256:22b29a388648c2e06791b42b42ae51327418afef0abd9e51baab2d9d97d15bab
  source_contract_sha256: sha256:6cb989c4d9b12bd0a0de8045c6e6a4b5daf01af1659c942c94abbeaafb01a64d
...
# Successful Patch Mining

## Objective

从经验证且获授权的 Semantic Patch 聚类重复模式，发现 Recipe candidate。

## Scope and capabilities

- Privacy/license filtering
- Semantic not textual clustering
- Preconditions/template extraction
...
- [ ] 定义 `gr-patch-mining` 的 public interface、输入/输出/Error Schema 和版本策略。
- [ ] 实现 `gr-patch-mining` 的 production adapter/service，不把业务状态只保存在 Prompt 或内存对象中。
...
- [ ] 至少生成：pattern-clusters.json, recipe-candidates。
...
- [ ] 本 Skill 的 required outputs 均由真实目标仓库执行产生：pattern-clusters.json, recipe-candidates。
- [ ] 所有 mandatory tests 和 security gates 通过，且无 critical unknown。

## Dependencies

- `gr-cache`
- `spring-deterministic-share`
...
- Machine contract: `references/contract.json` (`sha256:6cb989c4d9b12bd0a0de8045c6e6a4b5daf01af1659c942c94abbeaafb01a64d`).
