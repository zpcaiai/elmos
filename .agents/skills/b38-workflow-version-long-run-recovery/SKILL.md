---
name: b38-workflow-version-long-run-recovery
description: "Implement and verify Batch 38 Skill 1340 for workflow version long run recovery. Use when work requires Workflow\u7248\u672c\u5e76\u5b58\u548c\u957f\u4efb\u52a1\u6062\u590d, typed evidence, negative and holdout validation, and a fail-closed certification decision."
---
...
1. Inspect the current implementation, runtime facts, policies, incidents, decisions, and prior evidence relevant to `b38-workflow-version-long-run-recovery`.
...
4. Implement the smallest production-shaped capability that exercises `Workflow版本并存和长任务恢复` without bypassing existing control planes.
...
- Capability-specific implementation, tests, run logs, evidence references, and residual-risk records for `b38-workflow-version-long-run-recovery`
