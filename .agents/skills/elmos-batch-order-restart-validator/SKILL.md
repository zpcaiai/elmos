---
name: elmos-batch-order-restart-validator
description: Verify job ordering, return-code branching, dataset lifecycle, checkpoints,
  restart and exactly-once/at-least-once side-effect semantics.
version: 1.0.0
skill_id: ELMOS-POLY-157
layer: validation
risk: high
readiness: not-run
dependencies:
...
triggers:
- Use when implementing or executing `elmos-batch-order-restart-validator`.
- Use when the migration DAG requires `validation` capability.
outputs:
- validation/elmos-batch-order-restart-validator/result.json
- validation/elmos-batch-order-restart-validator/evidence/
---

# Batch Order Restart Validator

## Objective

Verify job ordering, return-code branching, dataset lifecycle, checkpoints, restart and exactly-once/at-least-once side-effect semantics.
