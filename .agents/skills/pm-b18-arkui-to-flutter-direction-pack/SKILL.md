---
name: pm-b18-arkui-to-flutter-direction-pack
description: "提供从 ArkTS/ArkUI 到 Flutter 的前端整库专用转换与验证包，覆盖组件、状态、生命周期、路由、布局、平台能力和真机行为. Precision Migration B18 contract; use for this exact assessment, transformation, validation, repair, evidence, or cutover scope."
---

# Arkui To Flutter Direction Pack
## ELMOS runtime binding

- Invoke this repository Skill as `$pm-b18-arkui-to-flutter-direction-pack`.
- Immutable source identity: `arkui-to-flutter-direction-pack` in `precision-migration-b01-44` (B18).
- Runtime adapter: `frontend-client-route`; binding state: `UNAVAILABLE`.
- Resolve and plan with `python3 scripts/precision_migration/runtime.py plan --skill pm-b18-arkui-to-flutter-direction-pack`.
...
提供从 ArkTS/ArkUI 到 Flutter 的前端整库专用转换与验证包，覆盖组件、状态、生命周期、路由、布局、平台能力和真机行为。

本 Skill 属于精密语言转换系统。它必须以**可执行证据**为准，不得用模型自评、源码相似度或“能编译”替代语义与行为验证。

## Use when

- 当前任务直接涉及 `arkui-to-flutter-direction-pack` 所描述的能力。
...
1. 恢复组件契约和UI状态机
2. 执行本 Skill 的核心职责：提供从 ArkTS/ArkUI 到 Flutter 的前端整库专用转换与验证包，覆盖组件、状态、生命周期、路由、布局、平台能力和真机行为。
3. 映射事件、路由、布局和平台能力
4. 生成目标项目
...
```yaml
skill: arkui-to-flutter-direction-pack
version: 1.0.0
