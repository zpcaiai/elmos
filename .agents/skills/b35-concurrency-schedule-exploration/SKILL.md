---
name: b35-concurrency-schedule-exploration
description: Implement controlled concurrency schedule exploration with deterministic schedulers partial-order reduction barriers seeds and forbidden-outcome detection.
---
...
## Skill 1277: b35-concurrency-schedule-exploration

## Use this skill when

- Concurrency-sensitive behavior requires more than repeated stress.

## Domain-specific risks and invariants

- Nondeterministic schedules are hard to reproduce and may miss rare bugs.

## Workflow

1. Identify synchronization points, shared state, transactions, message ordering, retries, and idempotency keys.
2. Instrument or adapt a deterministic scheduler and define bounded interleaving points.
3. Apply partial-order reduction without removing semantically distinct schedules.
4. Explore normal, duplicate, reorder, timeout, cancel, crash, and restart schedules.
5. Persist exact schedules and outcomes.
6. Shrink forbidden outcomes and replay on source and target.
...
- Explore concurrent requests, transactions, message handlers, retries, locks, atomics, caches, and idempotency paths.
...
- Schedule campaign specs, explored schedules, outcomes, deadlocks, races, minimal schedules, and replay harnesses.
