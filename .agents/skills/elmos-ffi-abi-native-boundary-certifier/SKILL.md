---
name: elmos-ffi-abi-native-boundary-certifier
description: Model and certify calling conventions, layout, ownership, lifetime, error, thread and binary compatibility at JNI, P/Invoke, C ABI, WebAssembly and native-library boundaries.
version: 4.0.0
...
# FFI, ABI and Native Boundary Certifier
...
- `FFIABIAndNativeBoundaryCertifierProfile`
- `FFIABIAndNativeBoundaryCertifierPlan`
- `FFIABIAndNativeBoundaryCertifierRun`
- `FFIABIAndNativeBoundaryCertifierDecision`

Required semantic capabilities:

- fingerprint platform ABI and compiler flags
- verify struct layout, alignment and symbol visibility
- check ownership and lifetime transfer
- exercise callback, exception and thread-affinity boundaries
- certify binary compatibility and rollback
...
- `elmos-language-semantic-profile-compiler` from `elmos-ai-capability-enhancement-skills` — required base evidence producer.

## Primary Outputs

- `artifacts/elmos-ffi-abi-native-boundary-certifier/profile.yaml`
- `artifacts/elmos-ffi-abi-native-boundary-certifier/plan.json`
- `artifacts/elmos-ffi-abi-native-boundary-certifier/result.json`
- `artifacts/elmos-ffi-abi-native-boundary-certifier/evidence/`
...
- **ABI drift** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **use-after-free across FFI** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **exception crosses unsupported boundary** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **architecture-specific layout hidden** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **unsigned native dependency substitution** — must have a preventive control, a detective signal, a negative fixture and a response action.
...
- native scenario: fingerprint platform ABI and compiler flags
- native scenario: verify struct layout, alignment and symbol visibility
- native scenario: check ownership and lifetime transfer
- native scenario: exercise callback, exception and thread-affinity boundaries
- native scenario: certify binary compatibility and rollback
