...
- Invoke this repository Skill as `$pm-b37-proof-agent`.
- Immutable source identity: `proof-agent` in `precision-migration-b01-44` (B37).
- Runtime adapter: `model-routing-and-agent-harness`; binding state: `UNAVAILABLE`.
- Resolve and plan with `python3 scripts/precision_migration/runtime.py plan --skill pm-b37-proof-agent`.
...
1. 计算风险并拆分任务
2. 执行本 Skill 的核心职责：生成证明义务、调用 Leanstral/SMT、解释反例并回写测试。
3. 选择模型与隔离工作区
4. 执行并收集客观反馈
5. 独立审查和升级
6. 持久化状态、证据、成本与审批
