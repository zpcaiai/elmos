...
- Invoke this repository Skill as `$pm-b41-release-gate-engine`.
- Immutable source identity: `release-gate-engine` in `precision-migration-b01-44` (B41).
- Runtime adapter: `evidence-release-gate`; binding state: `DECLARED`.
- Resolve and plan with `python3 scripts/precision_migration/runtime.py plan --skill pm-b41-release-gate-engine`.
...
1. 汇总证据与未解决项
2. 执行本 Skill 的核心职责：以硬门槛而非平均分决定是否允许发布、灰度或要求人工。
3. 执行硬性发布门禁
4. 影子/Canary/渐进切换
5. 监控并自动回滚
6. 沉淀反例、规则和企业交付能力
