---
name: spring-diagnostic-classifier
description: 把 Javac/Maven/Spring 启动错误聚类为 package、signature、type、bean、security、dependency 等根因。 Use for this exact ELMOS Spring Golden Route v2 contract; imported specifications are not runtime or certification evidence.
...
metadata:
  ...
  source_id: ELMOS-GR-049
  source_name: spring-diagnostic-classifier
  source_path: skills/spring-diagnostic-classifier/SKILL.md
  source_sha256: sha256:07606e295d0a08574ecdf05ef0f264d2aae26404b9269aebc91dfaf8e6743b34
  source_contract_sha256: sha256:b0d7c1a46c48f2a8ffa8bc38ef00b77ec6939f30fa3f8943b6eaebd97152a962
...
# Compile Diagnostic Classifier

## Objective

把 Javac/Maven/Spring 启动错误聚类为 package、signature、type、bean、security、dependency 等根因。
...
- [ ] 定义 `spring-diagnostic-classifier` 的 public interface、输入/输出/Error Schema 和版本策略。
- [ ] 实现 `spring-diagnostic-classifier` 的 production adapter/service，不把业务状态只保存在 Prompt 或内存对象中。
...
- [ ] 至少生成：diagnostics.json, root-causes.json, repair-candidates.json。
...
- [ ] 本 Skill 的 required outputs 均由真实目标仓库执行产生：diagnostics.json, root-causes.json, repair-candidates.json。
...
- Machine contract: `references/contract.json` (`sha256:b0d7c1a46c48f2a8ffa8bc38ef00b77ec6939f30fa3f8943b6eaebd97152a962`).
