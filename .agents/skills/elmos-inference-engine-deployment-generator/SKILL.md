---
name: elmos-inference-engine-deployment-generator
description: Generate KServe, Triton, Ray Serve, vLLM, SGLang, TGI and equivalent deployments with exact images, model artifacts, probes, scaling and rollback.
version: 4.0.0
...
# Inference Engine Deployment Generator
...
- `InferenceEngineDeploymentGeneratorProfile`
- `InferenceEngineDeploymentGeneratorPlan`
- `InferenceEngineDeploymentGeneratorRun`
- `InferenceEngineDeploymentGeneratorDecision`

Required semantic capabilities:

- select serving engine from workload profile
- emit model repository and deployment manifests
- configure health, readiness and startup probes
- generate autoscaling, routing and cache policy
- run native inference and rollback tests
...
- `artifacts/elmos-inference-engine-deployment-generator/profile.yaml`
- `artifacts/elmos-inference-engine-deployment-generator/plan.json`
- `artifacts/elmos-inference-engine-deployment-generator/result.json`
- `artifacts/elmos-inference-engine-deployment-generator/evidence/`
...
- **mutable model URI** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **probe validates process not model** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **GPU resource overcommit** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **rollback cannot load old model format** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **server-specific extension leaks portability** — must have a preventive control, a detective signal, a negative fixture and a response action.
...
- native scenario: select serving engine from workload profile
- native scenario: emit model repository and deployment manifests
- native scenario: configure health, readiness and startup probes
- native scenario: generate autoscaling, routing and cache policy
- native scenario: run native inference and rollback tests
