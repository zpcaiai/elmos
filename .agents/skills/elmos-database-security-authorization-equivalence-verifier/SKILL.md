---
name: elmos-database-security-authorization-equivalence-verifier
description: Verify roles, grants, row/column security, masking, encryption, auditing and privileged routines remain equivalent or explicitly strengthened across database routes.
version: 4.0.0
...
# Database Security and Authorization Equivalence Verifier
...
- `DatabaseSecurityAndAuthorizationEquivalenceVerifierProfile`
- `DatabaseSecurityAndAuthorizationEquivalenceVerifierPlan`
- `DatabaseSecurityAndAuthorizationEquivalenceVerifierRun`
- `DatabaseSecurityAndAuthorizationEquivalenceVerifierDecision`

Required semantic capabilities:

- compile roles, grants and ownership into authorization IR
- compare row and column policy behavior with negative identities
- verify masking, encryption and audit semantics
- exercise definer/invoker and privileged routine boundaries
- certify least-privilege cutover and rollback
...
- `artifacts/elmos-database-security-authorization-equivalence-verifier/profile.yaml`
- `artifacts/elmos-database-security-authorization-equivalence-verifier/plan.json`
- `artifacts/elmos-database-security-authorization-equivalence-verifier/result.json`
- `artifacts/elmos-database-security-authorization-equivalence-verifier/evidence/`
...
- **owner privileges expand in target** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **RLS policy omitted** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **security-definer routine escalates access** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **masked column exposed through view** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **audit event lost during dual run** — must have a preventive control, a detective signal, a negative fixture and a response action.
...
- native scenario: compile roles, grants and ownership into authorization IR
- native scenario: compare row and column policy behavior with negative identities
- native scenario: verify masking, encryption and audit semantics
- native scenario: exercise definer/invoker and privileged routine boundaries
- native scenario: certify least-privilege cutover and rollback
