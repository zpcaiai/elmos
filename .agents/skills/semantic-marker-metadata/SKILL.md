---
name: semantic-marker-metadata
description: 用非侵入 Marker 附着构建、Git、框架、搜索和变换证据。 Use for this exact ELMOS Spring Golden Route v2 contract; imported specifications are not runtime or certification evidence.
...
metadata:
  ...
  source_id: FOUNDATION-01-semantic-marker-metadata
  source_name: semantic-marker-metadata
  source_path: skills/semantic-marker-metadata/SKILL.md
  source_sha256: sha256:34e73187d4159ab9a49753c7a2919c175edf90a59d397057c09fedb3a1f320a1
  source_contract_sha256: sha256:da5c26bdcbeef17f1c648accbad511314f63295f5cdec590d725fc1c3ff3ba10
...
# Semantic Marker Metadata

## Goal

用非侵入 Marker 附着构建、Git、框架、搜索和变换证据。

## Use when

在 Elmos 需要实现或升级 **Semantic Marker Metadata** 相关能力时使用；它不是仅供解释的提示词，而是实现、测试与验收契约。

## Capabilities to implement

- Marker 与语法树结构解耦，可增删而不破坏打印
- 记录 Git provenance、构建信息、搜索命中和 Recipe provenance
- 支持序列化、版本化和跨运行比较
- 禁止把执行期元数据硬编码进语言 AST
...
- [ ] 定义 `semantic-marker-metadata` 的 public interface、input/output/error schemas。
- [ ] 建立 `semantic-marker-metadata` 的 capability/feature flags 与版本字段。
...
- [ ] 至少覆盖并验收：Marker 与语法树结构解耦，可增删而不破坏打印；以及 记录 Git provenance、构建信息、搜索命中和 Recipe provenance。
...
- Machine contract: `references/contract.json` (`sha256:da5c26bdcbeef17f1c648accbad511314f63295f5cdec590d725fc1c3ff3ba10`).
