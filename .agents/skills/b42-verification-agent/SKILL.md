...
description: "Implement and verify Batch 42 Skill 1418 for verification agent. Use when work requires Verification Agent, typed evidence, negative and holdout validation, and a fail-closed certification decision."
---
...
1. Inspect the current implementation, runtime facts, policies, incidents, decisions, and prior evidence relevant to `b42-verification-agent`.
...
4. Implement the smallest production-shaped capability that exercises `Verification Agent` without bypassing existing control planes.
...
- Capability-specific implementation, tests, run logs, evidence references, and residual-risk records for `b42-verification-agent`
