---
name: spring-source-profile
description: 识别 Java/Spring/Maven/模块/依赖/插件/运行拓扑并保存证据与置信度。 Use for this exact ELMOS Spring Golden Route v2 contract; imported specifications are not runtime or certification evidence.
...
metadata:
  ...
  source_id: ELMOS-GR-010
  source_name: spring-source-profile
  source_path: skills/spring-source-profile/SKILL.md
  source_sha256: sha256:14f3e8bf1dab7f2c1c77a5c3e51e8537fd11f54ab3cf4c972409618081182d0b
  source_contract_sha256: sha256:5ac768f40713ec85452b6508249bb355dcb6cbabd395c7e6fec3d17a70bc88e3
...
# Spring Source Profile Discovery

## Objective

识别 Java/Spring/Maven/模块/依赖/插件/运行拓扑并保存证据与置信度。

## Scope and capabilities

- effective Maven model
- parent/BOM/profile/plugin/toolchain
- Spring 实际 API 使用
- 模块与运行入口
- 冲突与 unknown
...
- [ ] 定义 `spring-source-profile` 的 public interface、输入/输出/Error Schema 和版本策略。
- [ ] 实现 `spring-source-profile` 的 production adapter/service，不把业务状态只保存在 Prompt 或内存对象中。
...
- [ ] 至少生成：source-profile.json, module-graph.json, capability-evidence.json。
...
- [ ] 本 Skill 的 required outputs 均由真实目标仓库执行产生：source-profile.json, module-graph.json, capability-evidence.json。
- [ ] 所有 mandatory tests 和 security gates 通过，且无 critical unknown。

## Dependencies

- `spring-route-orchestrator`
...
- Machine contract: `references/contract.json` (`sha256:5ac768f40713ec85452b6508249bb355dcb6cbabd395c7e6fec3d17a70bc88e3`).
