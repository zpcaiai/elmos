---
name: pm-b22-oracle-feature-migration-decision
description: "为 Oracle 专有能力输出 DIRECT_MAP、REWRITE、EMULATE、EXTERNALIZE、RETAIN 或 UNSUPPORTED. Precision Migration B22 contract; use for this exact assessment, transformation, validation, repair, evidence, or cutover scope."
---

# Oracle Feature Migration Decision
## ELMOS runtime binding

- Invoke this repository Skill as `$pm-b22-oracle-feature-migration-decision`.
- Immutable source identity: `oracle-feature-migration-decision` in `precision-migration-b01-44` (B22).
- Runtime adapter: `database-and-data-route`; binding state: `UNAVAILABLE`.
- Resolve and plan with `python3 scripts/precision_migration/runtime.py plan --skill pm-b22-oracle-feature-migration-decision`.
...
- Phase: `G 数据库精密互转`
- Version: `1.0.0`
- Tags: batch-22, oracle-proprietary-pack

## Purpose

为 Oracle 专有能力输出 DIRECT_MAP、REWRITE、EMULATE、EXTERNALIZE、RETAIN 或 UNSUPPORTED。

本 Skill 属于精密语言转换系统。它必须以**可执行证据**为准，不得用模型自评、源码相似度或“能编译”替代语义与行为验证。

## Use when

- 当前任务直接涉及 `oracle-feature-migration-decision` 所描述的能力。
...
1. 盘点对象与专有能力
2. 执行本 Skill 的核心职责：为 Oracle 专有能力输出 DIRECT_MAP、REWRITE、EMULATE、EXTERNALIZE、RETAIN 或 UNSUPPORTED。
...
```yaml
skill: oracle-feature-migration-decision
version: 1.0.0
