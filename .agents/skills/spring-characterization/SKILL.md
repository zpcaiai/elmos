---
name: spring-characterization
description: 捕获旧系统 API、数据库、安全、消息、配置、错误、启动和性能行为。 Use for this exact ELMOS Spring Golden Route v2 contract; imported specifications are not runtime or certification evidence.
...
metadata:
  ...
  source_id: ELMOS-GR-034
  source_name: spring-characterization
  source_path: skills/spring-characterization/SKILL.md
  source_sha256: sha256:4d57d79d93eabdb6d604f0236c8f364449e88a0071b287d9c259f9abfc15db82
  source_contract_sha256: sha256:71f7401b3572afce4ea1ddf4e52448776f355f2ea73daa44f535f459c857a117
...
捕获旧系统 API、数据库、安全、消息、配置、错误、启动和性能行为。

## Scope and capabilities

- HTTP/DB/Auth/Event/Job 行为指纹
- PII 脱敏
- P0 覆盖图
- 未知行为显式
...
- [ ] 定义 `spring-characterization` 的 public interface、输入/输出/Error Schema 和版本策略。
- [ ] 实现 `spring-characterization` 的 production adapter/service，不把业务状态只保存在 Prompt 或内存对象中。
...
- [ ] 至少生成：baseline-characterization.json, behavior-fingerprints, coverage-map.json。
...
- [ ] 本 Skill 的 required outputs 均由真实目标仓库执行产生：baseline-characterization.json, behavior-fingerprints, coverage-map.json。
...
- Machine contract: `references/contract.json` (`sha256:71f7401b3572afce4ea1ddf4e52448776f355f2ea73daa44f535f459c857a117`).
