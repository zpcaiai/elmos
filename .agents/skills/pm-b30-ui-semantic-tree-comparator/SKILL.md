---
name: pm-b30-ui-semantic-tree-comparator
description: "比较 Role、Name、State、Value、可见、可用、焦点和组件层级. Precision Migration B30 contract; use for this exact assessment, transformation, validation, repair, evidence, or cutover scope."
---

# Ui Semantic Tree Comparator
## ELMOS runtime binding

- Invoke this repository Skill as `$pm-b30-ui-semantic-tree-comparator`.
- Immutable source identity: `ui-semantic-tree-comparator` in `precision-migration-b01-44` (B30).
- Runtime adapter: `differential-test-and-repair`; binding state: `DECLARED`.
- Resolve and plan with `python3 scripts/precision_migration/runtime.py plan --skill pm-b30-ui-semantic-tree-comparator`.
...
比较 Role、Name、State、Value、可见、可用、焦点和组件层级。

本 Skill 属于精密语言转换系统。它必须以**可执行证据**为准，不得用模型自评、源码相似度或“能编译”替代语义与行为验证。

## Use when

- 当前任务直接涉及 `ui-semantic-tree-comparator` 所描述的能力。
...
1. 评估或生成测试
2. 执行本 Skill 的核心职责：比较 Role、Name、State、Value、可见、可用、焦点和组件层级。
3. 同步环境并双运行
4. 比较输出、状态和副作用
5. 搜索最小反例
6. 驱动修复并执行全量回归
...
```yaml
skill: ui-semantic-tree-comparator
version: 1.0.0
