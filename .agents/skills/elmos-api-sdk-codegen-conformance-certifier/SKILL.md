---
name: elmos-api-sdk-codegen-conformance-certifier
description: Generate and certify multi-language SDKs, examples and documentation from API/workflow contracts with compatibility, auth, error, streaming and retry semantics intact.
version: 4.0.0
...
# API and SDK Code Generation Conformance Certifier
...
- **Kernel authorities used:** K1, K3, K5, K6, K7, K8
...
- `APIAndSDKCodeGenerationConformanceCertifierProfile`
- `APIAndSDKCodeGenerationConformanceCertifierPlan`
- `APIAndSDKCodeGenerationConformanceCertifierRun`
- `APIAndSDKCodeGenerationConformanceCertifierDecision`

Required semantic capabilities:

- generate Java, Python, TypeScript, C#, Go and Rust clients
- verify authentication, pagination, streaming and error models
- run client-server contract and workflow tests
- check source maps, documentation and semantic versioning
- certify rollback and deprecation compatibility
...
- `elmos-openapi-arazzo-workflow-compiler` from `elmos-ai-capability-enhancement-skills` — required base evidence producer.
- `elmos-generated-api-event-schema-evolution-controller` from `elmos-ai-capability-enhancement-skills` — required base evidence producer.

## Primary Outputs

- `artifacts/elmos-api-sdk-codegen-conformance-certifier/profile.yaml`
- `artifacts/elmos-api-sdk-codegen-conformance-certifier/plan.json`
- `artifacts/elmos-api-sdk-codegen-conformance-certifier/result.json`
- `artifacts/elmos-api-sdk-codegen-conformance-certifier/evidence/`
...
- **SDK drops required security scope** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **retry duplicates non-idempotent operation** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **language default loses precision** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **generated docs contradict runtime** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **client version claims compatibility without mixed-version test** — must have a preventive control, a detective signal, a negative fixture and a response action.
...
- native scenario: generate Java, Python, TypeScript, C#, Go and Rust clients
- native scenario: verify authentication, pagination, streaming and error models
- native scenario: run client-server contract and workflow tests
- native scenario: check source maps, documentation and semantic versioning
- native scenario: certify rollback and deprecation compatibility
