---
name: elmos-transaction-locking-equivalence-verifier
description: Verify isolation, lock acquisition, deadlock, savepoint, constraint timing, sequence visibility and concurrent business invariants across database engines.
version: 4.0.0
...
# Transaction and Locking Equivalence Verifier
...
- Generate bounded concurrent schedules
- Observe locks, waits and serialization failures
- Compare commit/rollback and savepoint semantics
- Verify business invariants under concurrency
- Classify unavoidable engine-specific differences
...
- `transaction-equivalence-results.json`
- `concurrency-schedule-corpus/`
- `deadlock-observations.json`
- `transaction-counterexamples/`
...
- **test schedule not reproducible** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **lock wait mistaken for deadlock** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **isolation level alias differs** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **business invariant not checked** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **deadlock retry duplicates side effect** — must have a preventive control, a detective signal, a negative fixture and a response action.
...
- write-skew scenario
- deadlock victim behavior
- savepoint rollback
- deferrable constraint timing
- sequence visibility
