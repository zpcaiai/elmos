---
name: repo-semantic-accounting
description: 统计包/类型/方法/API/依赖/配置/Bean/Security/Entity/Test 的真实变换与执行者。 Use for this exact ELMOS Spring Golden Route v2 contract; imported specifications are not runtime or certification evidence.
...
metadata:
  ...
  source_id: ELMOS-GR-022
  source_name: repo-semantic-accounting
  source_path: skills/repo-semantic-accounting/SKILL.md
  source_sha256: sha256:4a1a13a57f0b5e487c368d3a8f30345b6bbce007427263f79decd755720d7205
  source_contract_sha256: sha256:0735f2e7ff4f5ad66a5d9e7e132af5d9a4ae86b1e7dddbeb9ba07fed922166aa
...
# Semantic Change Accounting

## Objective

统计包/类型/方法/API/依赖/配置/Bean/Security/Entity/Test 的真实变换与执行者。

## Scope and capabilities

- 统一 Semantic Patch taxonomy
- Recipe/Agent/Human/Tool 归因
- 复合操作去重
- automation share
...
- semantic-change-ledger.json
- automation-share.json
...
- [ ] 定义 `repo-semantic-accounting` 的 public interface、输入/输出/Error Schema 和版本策略。
- [ ] 实现 `repo-semantic-accounting` 的 production adapter/service，不把业务状态只保存在 Prompt 或内存对象中。
...
- [ ] 至少生成：semantic-change-ledger.json, automation-share.json。
...
- [ ] 本 Skill 的 required outputs 均由真实目标仓库执行产生：semantic-change-ledger.json, automation-share.json。
- [ ] 所有 mandatory tests 和 security gates 通过，且无 critical unknown。

## Dependencies

- `repo-transform-surface`
...
- Machine contract: `references/contract.json` (`sha256:0735f2e7ff4f5ad66a5d9e7e132af5d9a4ae86b1e7dddbeb9ba07fed922166aa`).
