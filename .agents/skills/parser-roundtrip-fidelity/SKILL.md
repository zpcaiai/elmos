---
name: parser-roundtrip-fidelity
description: 建立 parse→print 的无损往返质量门禁。 Use for this exact ELMOS Spring Golden Route v2 contract; imported specifications are not runtime or certification evidence.
...
metadata:
  ...
  source_id: FOUNDATION-01-parser-roundtrip-fidelity
  source_name: parser-roundtrip-fidelity
  source_path: skills/parser-roundtrip-fidelity/SKILL.md
  source_sha256: sha256:bfee744bc4fe94e0c30804b5f51a11ec4589bf2361258d55c0d16bd5e52b669b
  source_contract_sha256: sha256:b2cc0b585729547db4a8bab3b1542e5c59840d6fa0dc4f31119769688862ad3a
...
建立 parse→print 的无损往返质量门禁。

## Use when

在 Elmos 需要实现或升级 **Parser Round-trip Fidelity** 相关能力时使用；它不是仅供解释的提示词，而是实现、测试与验收契约。

## Capabilities to implement

- 未修改源码 parse/print 后字节或规范化等价
- 保留注释、空白、换行、编码和非语义 token
- 对无法解析片段使用显式 Quark/Unknown 边界而非静默删除
- 每种语言维护 round-trip corpus 与回归用例
...
- [ ] 定义 `parser-roundtrip-fidelity` 的 public interface、input/output/error schemas。
- [ ] 建立 `parser-roundtrip-fidelity` 的 capability/feature flags 与版本字段。
...
- [ ] 至少覆盖并验收：未修改源码 parse/print 后字节或规范化等价；以及 保留注释、空白、换行、编码和非语义 token。
...
- Machine contract: `references/contract.json` (`sha256:b2cc0b585729547db4a8bab3b1542e5c59840d6fa0dc4f31119769688862ad3a`).
