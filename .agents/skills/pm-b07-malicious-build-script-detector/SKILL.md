...
# Malicious Build Script Detector
## ELMOS runtime binding

- Invoke this repository Skill as `$pm-b07-malicious-build-script-detector`.
- Immutable source identity: `malicious-build-script-detector` in `precision-migration-b01-44` (B07).
- Runtime adapter: `semantic-recovery-and-ir`; binding state: `DECLARED`.
- Resolve and plan with `python3 scripts/precision_migration/runtime.py plan --skill pm-b07-malicious-build-script-detector`.
...
- Phase: `B 源码理解与可信执行底座`
- Version: `1.0.0`
- Tags: batch-07, sandbox-security-reproducibility

## Purpose

检测危险安装脚本、宿主访问、凭证读取、持久化、挖矿和供应链行为。

本 Skill 属于精密语言转换系统。它必须以**可执行证据**为准，不得用模型自评、源码相似度或“能编译”替代语义与行为验证。

## Use when

- 当前任务直接涉及 `malicious-build-script-detector` 所描述的能力。
...
1. 发现仓库与环境
2. 执行本 Skill 的核心职责：检测危险安装脚本、宿主访问、凭证读取、持久化、挖矿和供应链行为。
3. 使用原生工具提取语义
4. 建立可复现工具链和沙箱
5. 执行最小验证任务
6. 持久化摘要、哈希和证据
...
```yaml
skill: malicious-build-script-detector
version: 1.0.0
