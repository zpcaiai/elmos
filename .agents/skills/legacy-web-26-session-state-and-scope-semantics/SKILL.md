---
name: legacy-web-26-session-state-and-scope-semantics
description: "Repository-owned exact runtime interface for Session、状态与作用域语义恢复; bounded semantic analysis and evidence generation for Java legacy web modernization."
metadata:
  source_package: elmos.java-legacy-web.repository-modernization
  source_version: 1.0.0
  source_id: 26-session-state-and-scope-semantics
  source_digest: sha256:9f6b3096d96c93459f6e14f52aa1f68a45bfe6698a7d590bbd3c703330e27e3e
  phase: semantic-recovery
  runtime_state: CODE_COMPLETE_LOCAL
  capability_state: LOCAL_EXECUTED
  operation_code: STATE_SCOPE_RECOVERED
  runtime_handler_id: legacy-web-handler:26-session-state-and-scope-semantics
---
...
---
id: 26-session-state-and-scope-semantics
title: Session、状态与作用域语义恢复
version: 1.0.0
phase: semantic-recovery
priority: P0
requires:
...
识别 page/request/action/session/application/thread/static/cache/DB 状态，恢复创建、读取、覆盖、失效、序列化和跨请求序列。
...
producerSkill: 26-session-state-and-scope-semantics
...
6. 为每个状态对象建立 owner、scope、lifetime、serialization、mutation sites、read-after-write 关系。
7. 从日志/trace 挖掘跨请求 sequence，而非只分析单次 handler。
8. 检测 session fixation、cluster replication、不兼容 class version 和并发 tab 竞争。
9. 为 wizard/购物车/登录/flash message 生成状态机。
