...
- Invoke this repository Skill as `$pm-b44-enterprise-reporting`.
- Immutable source identity: `enterprise-reporting` in `precision-migration-b01-44` (B44).
- Runtime adapter: `enterprise-private-commercialization`; binding state: `UNAVAILABLE`.
- Resolve and plan with `python3 scripts/precision_migration/runtime.py plan --skill pm-b44-enterprise-reporting`.
...
生成技术、管理、审计、合规、成本、SLA 和项目组合报告。

本 Skill 属于精密语言转换系统。它必须以**可执行证据**为准，不得用模型自评、源码相似度或“能编译”替代语义与行为验证。

## Use when

- 当前任务直接涉及 `enterprise-reporting` 所描述的能力。
...
1. 汇总证据与未解决项
2. 执行本 Skill 的核心职责：生成技术、管理、审计、合规、成本、SLA 和项目组合报告。
3. 执行硬性发布门禁
4. 影子/Canary/渐进切换
5. 监控并自动回滚
6. 沉淀反例、规则和企业交付能力
