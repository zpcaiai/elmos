---
name: post-crash-continuation-recovery
description: 补强 OpenCode 尚未完全解决的 crash 后 continuation，作为 Elmos 生产级能力。 Use for this exact ELMOS Spring Golden Route v2 contract; imported specifications are not runtime or certification evidence.
...
metadata:
  ...
  source_id: FOUNDATION-08-post-crash-continuation-recovery
  source_name: post-crash-continuation-recovery
  source_path: skills/post-crash-continuation-recovery/SKILL.md
  source_sha256: sha256:3bd5f454939c7178e7531194617e9ad542677455a94c4de4aef0f278b792bde6
  source_contract_sha256: sha256:3b94b8e3baef268f741401fc9439aad15259aac6cc74dd87be9ca12d96b836b5
...
# Post-crash Continuation Recovery

## Goal

补强 OpenCode 尚未完全解决的 crash 后 continuation，作为 Elmos 生产级能力。

## Use when

在 Elmos 需要实现或升级 **Post-crash Continuation Recovery** 相关能力时使用；它不是仅供解释的提示词，而是实现、测试与验收契约。

## Capabilities to implement

- 区分 input promoted、provider dispatched、assistant durable、tool started/settled 等断点
- 恢复算法基于 durable evidence 决定 resume/retry/manual reconcile
- 集群 ownership 使用 lease/fencing token
- 任何歧义副作用进入人工/外部对账路径
...
- [ ] 定义 `post-crash-continuation-recovery` 的 public interface、input/output/error schemas。
- [ ] 建立 `post-crash-continuation-recovery` 的 capability/feature flags 与版本字段。
...
- [ ] 至少覆盖并验收：区分 input promoted、provider dispatched、assistant durable、tool started/settled 等断点；以及 恢复算法基于 durable evidence 决定 resume/retry/manual reconcile。
...
- Machine contract: `references/contract.json` (`sha256:3b94b8e3baef268f741401fc9439aad15259aac6cc74dd87be9ca12d96b836b5`).
