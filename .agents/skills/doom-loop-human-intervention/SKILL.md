---
name: doom-loop-human-intervention
description: 检测 Agent 重复失败/重复 tool call，并在正确时机转入人工决策。 Use for this exact ELMOS Spring Golden Route v2 contract; imported specifications are not runtime or certification evidence.
...
metadata:
  ...
  source_id: FOUNDATION-08-doom-loop-human-intervention
  source_name: doom-loop-human-intervention
  source_path: skills/doom-loop-human-intervention/SKILL.md
  source_sha256: sha256:0c33a99218ebf7ca1c4a2a555b99de920d5628b5760d6d5f90302a3e85f1fd6c
  source_contract_sha256: sha256:c823abb69d471d49bab995c94330457f8d509c7458b61d3f4ed3ff327afa00ec
...
# Doom-loop & Human Intervention

## Goal

检测 Agent 重复失败/重复 tool call，并在正确时机转入人工决策。

## Use when

在 Elmos 需要实现或升级 **Doom-loop & Human Intervention** 相关能力时使用；它不是仅供解释的提示词，而是实现、测试与验收契约。

## Capabilities to implement

- 基于相同错误、相同 patch、相同 tool args 的循环指纹
- 连续无进展达到阈值触发 stop reason
- 允许 question/approval/correction 进入下一 turn
- 不得通过无限换模型掩盖逻辑死循环
...
- [ ] 定义 `doom-loop-human-intervention` 的 public interface、input/output/error schemas。
- [ ] 建立 `doom-loop-human-intervention` 的 capability/feature flags 与版本字段。
...
- [ ] 至少覆盖并验收：基于相同错误、相同 patch、相同 tool args 的循环指纹；以及 连续无进展达到阈值触发 stop reason。
...
- Machine contract: `references/contract.json` (`sha256:c823abb69d471d49bab995c94330457f8d509c7458b61d3f4ed3ff327afa00ec`).
