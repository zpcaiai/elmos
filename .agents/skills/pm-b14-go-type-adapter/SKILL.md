---
name: pm-b14-go-type-adapter
description: "提取并规范化 Go 类型、泛型、可空性、联合类型和类型约束. Precision Migration B14 contract; use for this exact assessment, transformation, validation, repair, evidence, or cutover scope."
---

# Go Type Adapter
## ELMOS runtime binding

- Invoke this repository Skill as `$pm-b14-go-type-adapter`.
- Immutable source identity: `go-type-adapter` in `precision-migration-b01-44` (B14).
- Runtime adapter: `directed-backend-route`; binding state: `DECLARED`.
- Resolve and plan with `python3 scripts/precision_migration/runtime.py plan --skill pm-b14-go-type-adapter`.
...
1. 使用语言原生前端恢复语义
2. 执行本 Skill 的核心职责：提取并规范化 Go 类型、泛型、可空性、联合类型和类型约束。
