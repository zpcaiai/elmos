---
name: durable-tool-settlement
description: Tool call 在执行前投影，结果/失败/中断也持久化后再继续模型。 Use for this exact ELMOS Spring Golden Route v2 contract; imported specifications are not runtime or certification evidence.
...
metadata:
  ...
  source_id: FOUNDATION-07-durable-tool-settlement
  source_name: durable-tool-settlement
  source_path: skills/durable-tool-settlement/SKILL.md
  source_sha256: sha256:a1131fee00fe06a62e55f299682be1299acd1335560e24efca2d9d5f9e15df74
  source_contract_sha256: sha256:d921bec767471c9ad21727985df072dbdd0a1ffda570ca1757dbe3907b021c51
...
Tool call 在执行前投影，结果/失败/中断也持久化后再继续模型。

## Use when

在 Elmos 需要实现或升级 **Durable Tool Settlement** 相关能力时使用；它不是仅供解释的提示词，而是实现、测试与验收契约。

## Capabilities to implement

- tool call durable-before-execute
- settlement 关联 assistant message + tool call identity
- crash 后 running call 标记 interrupted，禁止静默重放
- continuation 只读取已 settlement 的历史
...
- [ ] 定义 `durable-tool-settlement` 的 public interface、input/output/error schemas。
- [ ] 建立 `durable-tool-settlement` 的 capability/feature flags 与版本字段。
...
- [ ] 至少覆盖并验收：tool call durable-before-execute；以及 settlement 关联 assistant message + tool call identity。
...
- Machine contract: `references/contract.json` (`sha256:d921bec767471c9ad21727985df072dbdd0a1ffda570ca1757dbe3907b021c51`).
