---
name: elmos-online-schema-change-backfill-controller
description: Execute expand/contract, online DDL, dual-read/write, throttled backfill, validation and cleanup without violating availability or data invariants.
version: 4.0.0
...
# Online Schema Change and Backfill Controller
...
- `OnlineSchemaChangeAndBackfillControllerProfile`
- `OnlineSchemaChangeAndBackfillControllerPlan`
- `OnlineSchemaChangeAndBackfillControllerRun`
- `OnlineSchemaChangeAndBackfillControllerDecision`

Required semantic capabilities:

- compile expand/contract phases
- throttle resumable backfills under SLO
- verify dual-read/write compatibility
- detect drift and repair missed rows
- gate cleanup on mixed-version retirement
...
- `artifacts/elmos-online-schema-change-backfill-controller/profile.yaml`
- `artifacts/elmos-online-schema-change-backfill-controller/plan.json`
- `artifacts/elmos-online-schema-change-backfill-controller/result.json`
- `artifacts/elmos-online-schema-change-backfill-controller/evidence/`
...
- **backfill overwhelms production** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **old writer corrupts new column** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **cleanup starts before fleet converges** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **resume duplicates effects** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **validation samples miss systematic drift** — must have a preventive control, a detective signal, a negative fixture and a response action.
...
- native scenario: compile expand/contract phases
- native scenario: throttle resumable backfills under SLO
- native scenario: verify dual-read/write compatibility
- native scenario: detect drift and repair missed rows
- native scenario: gate cleanup on mixed-version retirement
