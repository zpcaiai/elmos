---
name: elmos-continuous-control-monitoring-engine
description: Continuously evaluate runtime, security, compliance, quality, SLO and certification controls against current evidence and trigger restriction, incident or recertification.
version: 4.0.0
...
- `ContinuousControlMonitoringEngineProfile`
- `ContinuousControlMonitoringEnginePlan`
- `ContinuousControlMonitoringEngineRun`
- `ContinuousControlMonitoringEngineDecision`

Required semantic capabilities:

- compile controls into executable evidence queries
- evaluate freshness, scope and operating effectiveness
- correlate violations to affected certificates and tenants
- trigger bounded remediation, restriction or recertification
- preserve audit timeline and human accountability
...
- `elmos-runtime-policy-continuous-compliance-monitor` from `elmos-functional-assurance-certification-skills` — optional companion; required only for E4/E5/P05 certification.
- `elmos-certificate-drift-revocation-controller` from `elmos-functional-assurance-certification-skills` — optional companion; required only for E4/E5/P05 certification.

## Primary Outputs

- `artifacts/elmos-continuous-control-monitoring-engine/profile.yaml`
- `artifacts/elmos-continuous-control-monitoring-engine/plan.json`
- `artifacts/elmos-continuous-control-monitoring-engine/result.json`
- `artifacts/elmos-continuous-control-monitoring-engine/evidence/`
...
- **control reports green from stale telemetry** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **monitor can alter its own thresholds** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **cross-tenant evidence aggregation leaks data** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **violation suppressed without approval** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **recertification trigger lost during outage** — must have a preventive control, a detective signal, a negative fixture and a response action.
...
- native scenario: compile controls into executable evidence queries
- native scenario: evaluate freshness, scope and operating effectiveness
- native scenario: correlate violations to affected certificates and tenants
- native scenario: trigger bounded remediation, restriction or recertification
- native scenario: preserve audit timeline and human accountability
