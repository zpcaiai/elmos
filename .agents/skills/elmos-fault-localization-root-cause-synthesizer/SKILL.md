---
name: elmos-fault-localization-root-cause-synthesizer
description: Correlate counterexamples, traces, diffs, changes and dependency graphs to rank root causes and bounded repair locations without becoming the correctness oracle.
version: 4.0.0
...
# Fault Localization and Root-Cause Synthesizer
...
- `FaultLocalizationAndRootCauseSynthesizerProfile`
- `FaultLocalizationAndRootCauseSynthesizerPlan`
- `FaultLocalizationAndRootCauseSynthesizerRun`
- `FaultLocalizationAndRootCauseSynthesizerDecision`

Required semantic capabilities:

- cluster failures by causal signature
- slice change and dependency graph
- rank source/spec/adapter/runtime causes
- propose minimal reproducer and repair boundary
- record confidence and alternative hypotheses
...
- `artifacts/elmos-fault-localization-root-cause-synthesizer/profile.yaml`
- `artifacts/elmos-fault-localization-root-cause-synthesizer/plan.json`
- `artifacts/elmos-fault-localization-root-cause-synthesizer/result.json`
- `artifacts/elmos-fault-localization-root-cause-synthesizer/evidence/`
...
- **correlation presented as causation** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **root cause points to generated symptom** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **LLM explanation suppresses counterevidence** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **repair scope expands silently** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **sensitive trace copied into prompt** — must have a preventive control, a detective signal, a negative fixture and a response action.
...
- native scenario: cluster failures by causal signature
- native scenario: slice change and dependency graph
- native scenario: rank source/spec/adapter/runtime causes
- native scenario: propose minimal reproducer and repair boundary
- native scenario: record confidence and alternative hypotheses
