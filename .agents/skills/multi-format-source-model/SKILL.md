---
name: multi-format-source-model
description: 将代码、构建脚本、配置、数据格式和基础设施文件统一纳入变换图。 Use for this exact ELMOS Spring Golden Route v2 contract; imported specifications are not runtime or certification evidence.
...
metadata:
  ...
  source_id: FOUNDATION-01-multi-format-source-model
  source_name: multi-format-source-model
  source_path: skills/multi-format-source-model/SKILL.md
  source_sha256: sha256:ed1653d9456f95385fe2c6af314589938699e0bdd2dde1ae95664ac3ab976614
  source_contract_sha256: sha256:86e6f464d18670b43cd2abc88bf56d6e322c21d1d68e8b0417c0fbbc67e72fca
...
将代码、构建脚本、配置、数据格式和基础设施文件统一纳入变换图。

## Use when

在 Elmos 需要实现或升级 **Multi-format Source Model** 相关能力时使用；它不是仅供解释的提示词，而是实现、测试与验收契约。

## Capabilities to implement

- 覆盖 Maven/Gradle、JSON/YAML/XML/TOML/HCL/Properties/Protobuf/Docker 等类别
- 配置节点同样支持无损 parse/print 与 Recipe
- 代码变换与 build/config 变换可组成一个事务
- 跨文件引用通过统一 SourceFile ID 和 provenance 关联
...
- [ ] 定义 `multi-format-source-model` 的 public interface、input/output/error schemas。
- [ ] 建立 `multi-format-source-model` 的 capability/feature flags 与版本字段。
...
- [ ] 至少覆盖并验收：覆盖 Maven/Gradle、JSON/YAML/XML/TOML/HCL/Properties/Protobuf/Docker 等类别；以及 配置节点同样支持无损 parse/print 与 Recipe。
...
- Machine contract: `references/contract.json` (`sha256:86e6f464d18670b43cd2abc88bf56d6e322c21d1d68e8b0417c0fbbc67e72fca`).
