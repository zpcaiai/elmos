---
name: elmos-property-test-generator
description: "Derive property-based and metamorphic tests for conversions where finite examples are insufficient."
version: 1.0.0
skill_id: ELMOS-POLY-040
layer: validation
risk: high
readiness: not-run
dependencies:
  - "elmos-contract-test-generator"
triggers:
  - "Use when implementing or executing `elmos-property-test-generator`."
...
Derive property-based and metamorphic tests for conversions where finite examples are insufficient.
...
1. Identify algebraic, ordering, round-trip, serialization, idempotency, monotonicity, conservation, and invariant properties.
2. Define valid and invalid input generators from schemas, types, constraints, and observed domains.
3. Bound generators to avoid unsafe or meaningless states.
4. Run properties against source, target, and differential harnesses.
5. Shrink failures and persist minimal counterexamples.
6. Separate universal properties from assumptions inferred from samples.
7. Promote confirmed counterexamples into regression fixtures.
8. Report generator coverage and rejected inputs.
...
- [ ] Known injected violations are found and shrunk..
- [ ] Generator constraints prevent destructive inputs..
- [ ] Counterexamples are reproducible..
- [ ] Sample-derived assumptions are labeled..
