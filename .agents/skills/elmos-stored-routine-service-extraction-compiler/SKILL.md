---
name: elmos-stored-routine-service-extraction-compiler
description: Extract stored procedures, functions, packages and triggers into governed services while preserving transaction, error, lock, idempotency and side-effect semantics.
version: 4.0.0
...
# Stored Routine to Service Extraction Compiler
...
- `StoredRoutineToServiceExtractionCompilerProfile`
- `StoredRoutineToServiceExtractionCompilerPlan`
- `StoredRoutineToServiceExtractionCompilerRun`
- `StoredRoutineToServiceExtractionCompilerDecision`

Required semantic capabilities:

- classify pure, transactional and side-effecting routine regions
- generate typed service and database contracts
- preserve transaction and lock boundaries
- replace triggers with explicit outbox or workflow where approved
- dual-run routine/service behavior and rollback
...
- `artifacts/elmos-stored-routine-service-extraction-compiler/profile.yaml`
- `artifacts/elmos-stored-routine-service-extraction-compiler/plan.json`
- `artifacts/elmos-stored-routine-service-extraction-compiler/result.json`
- `artifacts/elmos-stored-routine-service-extraction-compiler/evidence/`
...
- **distributed transaction introduced silently** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **trigger ordering lost** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **exception mapping changes retry behavior** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **service call duplicates side effect** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **database session state omitted** — must have a preventive control, a detective signal, a negative fixture and a response action.
...
- native scenario: classify pure, transactional and side-effecting routine regions
- native scenario: generate typed service and database contracts
- native scenario: preserve transaction and lock boundaries
- native scenario: replace triggers with explicit outbox or workflow where approved
- native scenario: dual-run routine/service behavior and rollback
