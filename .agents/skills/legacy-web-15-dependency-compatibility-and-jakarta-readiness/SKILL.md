...
metadata:
  ...
  source_digest: sha256:cba8d95ddaf31ef6ce3fd474b55e4d5d85df9da6d100a2e3c6bb323d29418624
  phase: repository-forensics
  runtime_state: CODE_COMPLETE_LOCAL
  capability_state: LOCAL_EXECUTED
  operation_code: JAKARTA_READINESS_SCORED
  runtime_handler_id: legacy-web-handler:15-dependency-compatibility-and-jakarta-readiness
---
...
---
id: 15-dependency-compatibility-and-jakarta-readiness
title: 依赖兼容性与 Jakarta 就绪度
version: 1.0.0
phase: repository-forensics
priority: P0
requires:
...
分析 javax→jakarta、Servlet 6.1、Spring Framework 7、Boot 4 模块化、第三方库、容器和字节码基线兼容性。
...
producerSkill: 15-dependency-compatibility-and-jakarta-readiness
...
4. 输出有效模型、置信度和未决项。
5. 用最小可复现实验验证高风险推断。
6. 建立每个依赖对 Java 17+/Jakarta EE 11/Servlet 6.1/Spring 7 的兼容状态。
7. 检测仍暴露 javax API、字节码增强、容器 SPI、旧 JAXB/JAX-WS、日志桥和测试插件。
8. 输出 direct replacement、upgrade、adapter、isolate、retire 五类处置。
9. 不得用全局字符串替换代替符号级 namespace 迁移。
