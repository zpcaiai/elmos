---
name: parse-failure-evidence
description: 解析/反序列化失败必须成为可查询的一等证据。 Use for this exact ELMOS Spring Golden Route v2 contract; imported specifications are not runtime or certification evidence.
...
metadata:
  ...
  source_id: FOUNDATION-04-parse-failure-evidence
  source_name: parse-failure-evidence
  source_path: skills/parse-failure-evidence/SKILL.md
  source_sha256: sha256:3d90ac7b3c9ba3236eda5ac6d29beee4f656887f462d20e8f9d2d68b746ebac8
  source_contract_sha256: sha256:f55510d047fc7c0c898a1335dac055e0683265dca03ea3f1fa42ac67df6138e6
...
解析/反序列化失败必须成为可查询的一等证据。

## Use when

在 Elmos 需要实现或升级 **Parse Failure Evidence** 相关能力时使用；它不是仅供解释的提示词，而是实现、测试与验收契约。

## Capabilities to implement

- 保留原始文本、位置、parser/version 和异常链
- 失败节点不被静默删除或自动改写
- 聚合 ParseFailures DataTable
- 可触发 parser fallback 或 Agent 专门修复但需人工/验证边界
...
- [ ] 定义 `parse-failure-evidence` 的 public interface、input/output/error schemas。
- [ ] 建立 `parse-failure-evidence` 的 capability/feature flags 与版本字段。
...
- [ ] 至少覆盖并验收：保留原始文本、位置、parser/version 和异常链；以及 失败节点不被静默删除或自动改写。
...
- Machine contract: `references/contract.json` (`sha256:f55510d047fc7c0c898a1335dac055e0683265dca03ea3f1fa42ac67df6138e6`).
