---
name: b46-capability-dependency-graph
description: 建立前置、运行时、认证、商业和可选依赖图，并支持循环、撤销、过期和升级影响传播。
---

# Skill 1499：跨Batch Capability Dependency Graph
...
建立前置、运行时、认证、商业和可选依赖图，并支持循环、撤销、过期和升级影响传播。
...
- 依赖图必须区分prerequisite/runtime/certification/commercial/optional；撤销和过期必须向消费者传播；循环依赖为阻断。
...
- `contracts/product-kernel/capability-dependency.schema.json`
- `services/control-plane/dependencies/`
- `ui/src/features/dependency-graph/`
