---
name: hierarchical-context-and-repo-cache
description: 把 parser/LST/index/query/summary/provider prompt cache 组合成分层缓存。 Use for this exact ELMOS Spring Golden Route v2 contract; imported specifications are not runtime or certification evidence.
...
metadata:
  ...
  source_id: FOUNDATION-10-hierarchical-context-and-repo-cache
  source_name: hierarchical-context-and-repo-cache
  source_path: skills/hierarchical-context-and-repo-cache/SKILL.md
  source_sha256: sha256:1e48e75e34754dfba67013bfd9395c6366ffc729e7efa68b301dce5886b12912
  source_contract_sha256: sha256:4b9abf36d6664e3ccc8ec4b079eeb84bb8928561458b831f3e43a7d06735a944
...
# Hierarchical Context & Repository Cache

## Goal

把 parser/LST/index/query/summary/provider prompt cache 组合成分层缓存。

## Use when

在 Elmos 需要实现或升级 **Hierarchical Context & Repository Cache** 相关能力时使用；它不是仅供解释的提示词，而是实现、测试与验收契约。

## Capabilities to implement

- key 包含 git tree/file hash、parser/skill/rule/model/context 版本
- 局部文件变化只失效受影响 symbol/dependency slice
- 区分 deterministic cache 与 model cache
- 持续测 hit rate、节省 token、错误复用率
...
- [ ] 定义 `hierarchical-context-and-repo-cache` 的 public interface、input/output/error schemas。
- [ ] 建立 `hierarchical-context-and-repo-cache` 的 capability/feature flags 与版本字段。
...
- [ ] 至少覆盖并验收：key 包含 git tree/file hash、parser/skill/rule/model/context 版本；以及 局部文件变化只失效受影响 symbol/dependency slice。
...
- Machine contract: `references/contract.json` (`sha256:4b9abf36d6664e3ccc8ec4b079eeb84bb8928561458b831f3e43a7d06735a944`).
