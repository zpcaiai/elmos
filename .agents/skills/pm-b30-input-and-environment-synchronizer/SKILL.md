...
# Input And Environment Synchronizer
## ELMOS runtime binding

- Invoke this repository Skill as `$pm-b30-input-and-environment-synchronizer`.
- Immutable source identity: `input-and-environment-synchronizer` in `precision-migration-b01-44` (B30).
- Runtime adapter: `differential-test-and-repair`; binding state: `DECLARED`.
- Resolve and plan with `python3 scripts/precision_migration/runtime.py plan --skill pm-b30-input-and-environment-synchronizer`.
...
同步输入、配置、依赖、数据库、外部返回、Locale、时区和平台状态。

本 Skill 属于精密语言转换系统。它必须以**可执行证据**为准，不得用模型自评、源码相似度或“能编译”替代语义与行为验证。

## Use when

- 当前任务直接涉及 `input-and-environment-synchronizer` 所描述的能力。
...
1. 评估或生成测试
2. 执行本 Skill 的核心职责：同步输入、配置、依赖、数据库、外部返回、Locale、时区和平台状态。
3. 同步环境并双运行
4. 比较输出、状态和副作用
5. 搜索最小反例
6. 驱动修复并执行全量回归
...
```yaml
skill: input-and-environment-synchronizer
version: 1.0.0
