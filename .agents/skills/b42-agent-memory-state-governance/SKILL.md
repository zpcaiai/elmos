...
description: "Implement and verify Batch 42 Skill 1424 for agent memory state governance. Use when work requires Agent Memory\u548c\u72b6\u6001\u6cbb\u7406, typed evidence, negative and holdout validation, and a fail-closed certification decision."
---
...
1. Inspect the current implementation, runtime facts, policies, incidents, decisions, and prior evidence relevant to `b42-agent-memory-state-governance`.
...
4. Implement the smallest production-shaped capability that exercises `Agent Memory和状态治理` without bypassing existing control planes.
...
- Capability-specific implementation, tests, run logs, evidence references, and residual-risk records for `b42-agent-memory-state-governance`
