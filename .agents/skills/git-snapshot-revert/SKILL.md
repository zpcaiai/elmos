---
name: git-snapshot-revert
description: 用独立 Git object/index 体系为 Agent 修改提供高效 checkpoint/diff/restore/revert。 Use for this exact ELMOS Spring Golden Route v2 contract; imported specifications are not runtime or certification evidence.
...
metadata:
  ...
  source_id: FOUNDATION-08-git-snapshot-revert
  source_name: git-snapshot-revert
  source_path: skills/git-snapshot-revert/SKILL.md
  source_sha256: sha256:2cdebeb8620a366eddfdc662928d8edda1bbb0947ff23ad8332d91bd75d5bff7
  source_contract_sha256: sha256:d26255a328f0a104595a21dadf159913e99d728f4345458d638a23e9f1e03eb9
...
用独立 Git object/index 体系为 Agent 修改提供高效 checkpoint/diff/restore/revert。

## Use when

在 Elmos 需要实现或升级 **Git Snapshot & Revert** 相关能力时使用；它不是仅供解释的提示词，而是实现、测试与验收契约。

## Capabilities to implement

- 不污染用户现有分支和 index
- 可 track tree hash、生成 patch、restore、revert、full diff
- 复用原仓 object database/index 降低超大仓首次 hash 成本
- 忽略文件、大文件、symlink/longpath 有明确策略
...
- [ ] 定义 `git-snapshot-revert` 的 public interface、input/output/error schemas。
- [ ] 建立 `git-snapshot-revert` 的 capability/feature flags 与版本字段。
...
- [ ] 至少覆盖并验收：不污染用户现有分支和 index；以及 可 track tree hash、生成 patch、restore、revert、full diff。
...
- Machine contract: `references/contract.json` (`sha256:d26255a328f0a104595a21dadf159913e99d728f4345458d638a23e9f1e03eb9`).
