---
name: elmos-kubernetes-ai-workload-operator-generator
description: Generate controllers/CRDs for agent, retrieval and model-serving workloads with reconciliation, status, finalizers, upgrades and tenancy.
version: 4.0.0
...
# Kubernetes AI Workload Operator Generator
...
- `KubernetesAIWorkloadOperatorGeneratorProfile`
- `KubernetesAIWorkloadOperatorGeneratorPlan`
- `KubernetesAIWorkloadOperatorGeneratorRun`
- `KubernetesAIWorkloadOperatorGeneratorDecision`

Required semantic capabilities:

- define versioned CRDs and status conditions
- implement idempotent reconciliation and finalizers
- manage model/data/artifact dependencies
- enforce tenant, network and resource policy
- test upgrade, failure and deletion semantics
...
- `artifacts/elmos-kubernetes-ai-workload-operator-generator/profile.yaml`
- `artifacts/elmos-kubernetes-ai-workload-operator-generator/plan.json`
- `artifacts/elmos-kubernetes-ai-workload-operator-generator/result.json`
- `artifacts/elmos-kubernetes-ai-workload-operator-generator/evidence/`
...
- **operator loop creates storm** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **finalizer blocks deletion forever** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **status says ready before dependency** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **CRD upgrade loses field** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **controller cluster role overprivileged** — must have a preventive control, a detective signal, a negative fixture and a response action.
...
- native scenario: define versioned CRDs and status conditions
- native scenario: implement idempotent reconciliation and finalizers
- native scenario: manage model/data/artifact dependencies
- native scenario: enforce tenant, network and resource policy
- native scenario: test upgrade, failure and deletion semantics
