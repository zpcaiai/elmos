...
- Invoke this repository Skill as `$pm-b20-data-checksum-validation`.
- Immutable source identity: `data-checksum-validation` in `precision-migration-b01-44` (B20).
- Runtime adapter: `database-and-data-route`; binding state: `DECLARED`.
- Resolve and plan with `python3 scripts/precision_migration/runtime.py plan --skill pm-b20-data-checksum-validation`.
...
- 当前任务直接涉及 `data-checksum-validation` 所描述的能力。
...
1. 盘点对象与专有能力
2. 执行本 Skill 的核心职责：执行表级、分区级、列级和业务聚合校验。
...
```yaml
skill: data-checksum-validation
version: 1.0.0
