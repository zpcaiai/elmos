---
name: elmos-ai-cache-semantic-consistency-verifier
description: Verify prompt, model, tool, retrieval, build and transformation caches against semantic keys, tenant isolation,
  freshness and evidence invalidation.
version: 4.0.0
...
# AI Cache Semantic Consistency Verifier
...
- **Kernel authorities used:** K2, K6, K7
...
- `Approval`
- `ResidualRisk`
- `AICacheSemanticConsistencyVerifierRun`
- `AICacheSemanticConsistencyVerifierDecision`

Required semantic capabilities:

- Semantic cache key construction
- Tenant/policy/model/tool/version partitioning
- Freshness and negative-cache TTL
- Drift/evidence invalidation propagation
- Poisoning and collision detection
...
- `cache-key-contract.json`
- `cache-consistency-results.json`
- `cache-invalidation-events.jsonl`
- `cache-isolation-proof.json`
...
- **cross-tenant cache leak** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **stale unsafe result** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **hash collision abuse** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **poisoned semantic cache** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **policy bypass via hit** — must have a preventive control, a detective signal, a negative fixture and a response action.
...
- tool policy change invalidation
- poisoned entry quarantine
