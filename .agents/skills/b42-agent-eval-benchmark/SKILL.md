...
description: "Implement and verify Batch 42 Skill 1427 for agent eval benchmark. Use when work requires Agent Eval\u548cBenchmark, typed evidence, negative and holdout validation, and a fail-closed certification decision."
---

# Agent Eval和Benchmark
...
## Skill 1427: Agent Eval和Benchmark
...
1. Inspect the current implementation, runtime facts, policies, incidents, decisions, and prior evidence relevant to `b42-agent-eval-benchmark`.
...
4. Implement the smallest production-shaped capability that exercises `Agent Eval和Benchmark` without bypassing existing control planes.
...
- Capability-specific implementation, tests, run logs, evidence references, and residual-risk records for `b42-agent-eval-benchmark`
