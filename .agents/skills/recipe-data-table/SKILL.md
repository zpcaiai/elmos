...
description: Recipe 运行可输出结构化分析表而不把结果塞进日志。 Use for this exact ELMOS Spring Golden Route v2 contract; imported specifications are not runtime or certification evidence.
...
metadata:
  ...
  source_id: FOUNDATION-03-recipe-data-table
  source_name: recipe-data-table
  source_path: skills/recipe-data-table/SKILL.md
  source_sha256: sha256:4ad75458b52deb3c00fbac877b027e35e9221ebf72f42defaabc3f2adc2452f7
  source_contract_sha256: sha256:066e392eb1f4e6059733a13530bf3ab0cc5c8619cbfd3131ed39339694cbefc9
...
Recipe 运行可输出结构化分析表而不把结果塞进日志。

## Use when

在 Elmos 需要实现或升级 **Recipe Data Table** 相关能力时使用；它不是仅供解释的提示词，而是实现、测试与验收契约。

## Capabilities to implement

- 表 schema 和列描述由 Recipe 声明
- 支持 InMemory/CSV/NoOp/自定义 sink
- 数据表与 changeset 同一 run ID 关联
- 适合依赖清单、搜索命中、错误、质量指标等大结果
...
- [ ] 定义 `recipe-data-table` 的 public interface、input/output/error schemas。
- [ ] 建立 `recipe-data-table` 的 capability/feature flags 与版本字段。
...
- [ ] 至少覆盖并验收：表 schema 和列描述由 Recipe 声明；以及 支持 InMemory/CSV/NoOp/自定义 sink。
...
- Machine contract: `references/contract.json` (`sha256:066e392eb1f4e6059733a13530bf3ab0cc5c8619cbfd3131ed39339694cbefc9`).
