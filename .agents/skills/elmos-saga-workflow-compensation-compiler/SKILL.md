---
name: elmos-saga-workflow-compensation-compiler
description: Compile long-running distributed business transactions into durable saga state machines with explicit compensation, irreversible steps and human gates.
version: 4.0.0
...
# Saga Workflow and Compensation Compiler
...
- `SagaWorkflowAndCompensationCompilerProfile`
- `SagaWorkflowAndCompensationCompilerPlan`
- `SagaWorkflowAndCompensationCompilerRun`
- `SagaWorkflowAndCompensationCompilerDecision`

Required semantic capabilities:

- derive saga steps and state ownership
- classify compensable and irreversible effects
- generate timeout, retry and approval transitions
- persist idempotency and reconciliation state
...
- `artifacts/elmos-saga-workflow-compensation-compiler/profile.yaml`
- `artifacts/elmos-saga-workflow-compensation-compiler/plan.json`
- `artifacts/elmos-saga-workflow-compensation-compiler/result.json`
- `artifacts/elmos-saga-workflow-compensation-compiler/evidence/`
...
- **compensation not semantic inverse** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **irreversible step executed before approval** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **timeout races successful completion** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **duplicate compensation** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **saga completion ignores unsettled side effect** — must have a preventive control, a detective signal, a negative fixture and a response action.
...
- native scenario: derive saga steps and state ownership
- native scenario: classify compensable and irreversible effects
- native scenario: generate timeout, retry and approval transitions
- native scenario: persist idempotency and reconciliation state
- native scenario: verify cancellation and partial-failure behavior
