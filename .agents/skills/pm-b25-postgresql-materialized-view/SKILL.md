---
name: pm-b25-postgresql-materialized-view
description: "迁移物化视图、并发刷新、唯一索引和刷新调度. Precision Migration B25 contract; use for this exact assessment, transformation, validation, repair, evidence, or cutover scope."
---
...
迁移物化视图、并发刷新、唯一索引和刷新调度。

本 Skill 属于精密语言转换系统。它必须以**可执行证据**为准，不得用模型自评、源码相似度或“能编译”替代语义与行为验证。

## Use when

- 当前任务直接涉及 `postgresql-materialized-view` 所描述的能力。
...
1. 盘点对象与专有能力
2. 执行本 Skill 的核心职责：迁移物化视图、并发刷新、唯一索引和刷新调度。
...
```yaml
skill: postgresql-materialized-view
version: 1.0.0
