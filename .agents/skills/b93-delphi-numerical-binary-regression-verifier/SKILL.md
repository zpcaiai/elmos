---
name: b93-delphi-numerical-binary-regression-verifier
description: "Use when ELMOS must run delphi numerical binary regression verifier for Batch 93 Delphi / Object Pascal Modernization Pack. Preserve native semantics, safety boundaries, ownership, traceability, and fail-closed evidence; require real vendor toolchain execution for runtime claims."
metadata:
  source_package: "elmos-language-packs-batch81-95-complete"
  source_id: "PG377"
  source_name: "delphi-numerical-binary-regression-verifier"
  source_sha256: "sha256:737ec2684010492c57755fea12e2a42633baff001e9cbf3514294c86a2fbbecf"
  batch: 93
  source_engine: "elmos.language-packs"
  source_status: "proposed"
  normalized_namespace: "language-pack"
---
# Batch 93 Language Pack / source PG377 — Delphi Numerical Binary Regression Verifier

## 1. Objective

Verify delphi numerical binary regression verifier within the Delphi / Object Pascal Modernization Pack, preserving platform semantics, traceability, security, and reproducible evidence.

## 2. Scope

This Skill is part of **Batch 93: Delphi / Object Pascal Modernization Pack**.

### In scope

- Process Object Pascal.
- Process DFM forms.
- Process VCL/FMX.
- Process BPL/DCP packages.
- Process BDE/FireDAC.
- Process COM.
- Process Windows APIs.
- Process threads.
- Produce canonical Delphi IR.
- Produce .NET/C++ services.
- Produce modern desktop/web UI.
- Produce data adapters.
...
2. Process the relevant artifacts for Delphi Numerical Binary Regression Verifier using the approved windows-industrial-legacy semantic contracts and toolchain versions.
3. Preserve source provenance and create Artifact Graph links across inputs, outputs, tests, approvals, and evidence.
4. Detect and classify manual memory lifetime, event ordering, DFM property loss; never convert uncertainty or unsupported behavior into a silent success.
...
7. Apply the safety boundary: Industrial and hardware-connected Delphi systems require target-device and timing validation before replacement.
...
- `MANUAL_MEMORY_LIFETIME` — stop unsafe continuation and emit source-linked diagnostic evidence.
- `EVENT_ORDERING` — stop unsafe continuation and emit source-linked diagnostic evidence.
- `DFM_PROPERTY_LOSS` — stop unsafe continuation and emit source-linked diagnostic evidence.
- `BDE_TRANSACTION_DRIFT` — stop unsafe continuation and emit source-linked diagnostic evidence.
- `PACKAGE_INITIALIZATION_ORDER` — stop unsafe continuation and emit source-linked diagnostic evidence.
- `THREAD_AFFINITY` — stop unsafe continuation and emit source-linked diagnostic evidence.
...
- Apply this Batch safety boundary: **Industrial and hardware-connected Delphi systems require target-device and timing validation before replacement.**

## 14. Unit Tests

- A representative valid delphi-object-pascal input produces all declared outputs for delphi-numerical-binary-regression-verifier.
...
- At least one batch-specific risk (manual memory lifetime) is detected by a negative fixture.

## 15. Integration Tests

- Consume approved Project Synthesis or Modernization baselines and write complete lineage into the Artifact Graph.
- Integrate with at least one representative toolchain from: Delphi/Object Pascal, VCL, FireMonkey, FireDAC.
...
- Reject or block manual memory lifetime without weakening the expected result.
- Reject or block event ordering without weakening the expected result.
- Reject or block DFM property loss without weakening the expected result.
- Reject or block BDE transaction drift without weakening the expected result.
- Reject or block package initialization order without weakening the expected result.
- Reject or block thread affinity without weakening the expected result.
