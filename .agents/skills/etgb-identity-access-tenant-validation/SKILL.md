---
name: etgb-identity-access-tenant-validation
description: Validate all Identity, Access, Organization and Tenant Governance capabilities through the production adapter, independent Oracles and fail-closed release evidence. Repository-owned ETGB execution is available through the local runtime; external production evidence remains explicit.
metadata:
  ...
  source_skill: identity-access-tenant-validation
...
---
name: identity-access-tenant-validation
description: Validate all declared Identity, Access, Organization and Tenant Governance capabilities with executable positive, boundary, adversarial, concurrency, recovery and evidence-backed release tests.
---

# Identity, Access, Organization and Tenant Governance Validation

## Purpose

Use this Skill to plan, execute and certify the `identity-access-tenant` product domain. It owns **53 declared capabilities** (28 P0, 20 P1, 5 P2) across 4 execution contexts and four mandatory variants.

## Inputs

- Frozen release candidate and feature-registry revision.
- Tenant, project, environment authority, budget and fencing context.
- Concrete cases from `suites/full-product.jsonl` whose business line is `identity-access-tenant`.
...
2. Provision an isolated context for: web-console, public-api, admin-console, service-account.
...
5. Apply the domain Oracle `identity-policy-security-session-and-audit` plus security, state, audit and unsupported-disclosure Oracles.
...
Representative capabilities include: Account Registration, Email Verification, Password Login, Password Reset, Logout Session Revocation, Mfa Enrollment And Challenge, Oidc Sso Login, Saml Sso Login, Session Expiry And Rotation, Concurrent Session Policy, Api Key Create Rotate Revoke, Service Account Authentication. The complete source of truth is `matrices/full-product.yaml`; no prose list may override it.
...
Route execution through `external-identity-access-harness` under `integrations/harness/full-product-adapters.yaml`. An adapter stub or skipped case is implementation progress, not product certification.
