---
name: b42-agent-shadow-canary
description: "Implement and verify Batch 42 Skill 1428 for agent shadow canary. Use when work requires Shadow\u3001Canary\u548c\u53d7\u9650\u4e0a\u7ebf, typed evidence, negative and holdout validation, and a fail-closed certification decision."
---
...
1. Inspect the current implementation, runtime facts, policies, incidents, decisions, and prior evidence relevant to `b42-agent-shadow-canary`.
...
4. Implement the smallest production-shaped capability that exercises `Shadow、Canary和受限上线` without bypassing existing control planes.
...
- Capability-specific implementation, tests, run logs, evidence references, and residual-risk records for `b42-agent-shadow-canary`
