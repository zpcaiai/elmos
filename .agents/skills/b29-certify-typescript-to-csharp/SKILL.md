...
## Skill 1157: TypeScript → C#

Create a customer-usable, independently certified TypeScript → C# route. The reverse direction is not covered by this skill.

## Use this skill when

Use when adding, repairing, benchmarking, or certifying the `typescript-to-csharp` route.

## Route-specific semantic hazards

- structural, union, intersection, mapped, and conditional types
- number precision, coercion, NaN, Infinity, and BigInt
- undefined/null/optional properties versus C# nullable semantics
- Promise/event-loop scheduling versus Task and threads
- decorators, metadata, prototypes, and dynamic property access
- module loading, object-key order, and JavaScript runtime APIs

## Workflow

1. Inspect `engines/typescript-engine`, `engines/csharp-engine`, shared PSP/UIR contracts, and any existing `routes/typescript-to-csharp` package.
2. Run `python3 scripts/batch29/scaffold_route.py --source typescript --target csharp` if the route package is absent.
...
- `routes/typescript-to-csharp/route.json`
- `routes/typescript-to-csharp/support-matrix.json`
- `routes/typescript-to-csharp/lowering/`
- `routes/typescript-to-csharp/mappings/`
- `routes/typescript-to-csharp/compat-runtime/manifest.json`
- `routes/typescript-to-csharp/corpus/development/`
- `routes/typescript-to-csharp/corpus/holdout/`
- `routes/typescript-to-csharp/certification/evidence.json`

## Verification

- Run the real TypeScript source compiler/runtime and the real C# target compiler/runtime.
...
- Run `python3 scripts/batch29/validate_route.py routes/typescript-to-csharp` and the route gate.
- Verify critical behavior differences and unknown critical semantics are zero for certified scope.

## Stop and escalate when

- A listed structural, union, intersection, mapped, and conditional types cannot be preserved, adapted, or explicitly blocked.
- A listed number precision, coercion, NaN, Infinity, and BigInt cannot be preserved, adapted, or explicitly blocked.
- A listed undefined/null/optional properties versus C# nullable semantics cannot be preserved, adapted, or explicitly blocked.
