---
name: elmos-devcontainer-nix-hermetic-environment-generator
description: Generate reproducible local and CI development environments with pinned toolchains, services, policies and offline verification.
version: 4.0.0
...
# Devcontainer and Nix Hermetic Environment Generator
...
- `DevcontainerAndNixHermeticEnvironmentGeneratorProfile`
- `DevcontainerAndNixHermeticEnvironmentGeneratorPlan`
- `DevcontainerAndNixHermeticEnvironmentGeneratorRun`
- `DevcontainerAndNixHermeticEnvironmentGeneratorDecision`

Required semantic capabilities:

- emit devcontainer/Nix environment definitions
- pin compilers, package managers and system libraries
- provision local service dependencies
- support offline cache and integrity checks
- verify parity with CI and production profiles
...
- `elmos-hermetic-build-environment-attestation-controller` from `elmos-functional-assurance-certification-skills` — optional companion; required only for E4/E5/P05 certification.

## Primary Outputs

- `artifacts/elmos-devcontainer-nix-hermetic-environment-generator/profile.yaml`
- `artifacts/elmos-devcontainer-nix-hermetic-environment-generator/plan.json`
- `artifacts/elmos-devcontainer-nix-hermetic-environment-generator/result.json`
- `artifacts/elmos-devcontainer-nix-hermetic-environment-generator/evidence/`
...
- **developer environment uses unpinned latest** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **host tool shadows pinned binary** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **secret baked into image** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **local database semantics differ** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **cache not content verified** — must have a preventive control, a detective signal, a negative fixture and a response action.
...
- native scenario: emit devcontainer/Nix environment definitions
- native scenario: pin compilers, package managers and system libraries
- native scenario: provision local service dependencies
- native scenario: support offline cache and integrity checks
- native scenario: verify parity with CI and production profiles
