---
name: elmos-source-target-debuggability-observability-verifier
description: Verify that generated targets retain actionable logs, traces, metrics, stack attribution, correlation, replay and safe debugging comparable to the source system.
version: 4.0.0
...
# Source-to-Target Debuggability and Observability Verifier
...
- `SourceToTargetDebuggabilityAndObservabilityVerifierProfile`
- `SourceToTargetDebuggabilityAndObservabilityVerifierPlan`
- `SourceToTargetDebuggabilityAndObservabilityVerifierRun`
- `SourceToTargetDebuggabilityAndObservabilityVerifierDecision`

Required semantic capabilities:

- compare source and target trace topology
- verify error and stack attribution
- validate correlation IDs across protocols
- exercise record/replay and break-glass debug paths
- measure telemetry cost and cardinality
...
- `artifacts/elmos-source-target-debuggability-observability-verifier/profile.yaml`
- `artifacts/elmos-source-target-debuggability-observability-verifier/plan.json`
- `artifacts/elmos-source-target-debuggability-observability-verifier/result.json`
- `artifacts/elmos-source-target-debuggability-observability-verifier/evidence/`
...
- **telemetry silently drops critical span** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **source map leaks secrets** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **cardinality causes outage** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **debug path bypasses authorization** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **target errors become opaque** — must have a preventive control, a detective signal, a negative fixture and a response action.
...
- native scenario: compare source and target trace topology
- native scenario: verify error and stack attribution
- native scenario: validate correlation IDs across protocols
- native scenario: exercise record/replay and break-glass debug paths
- native scenario: measure telemetry cost and cardinality
