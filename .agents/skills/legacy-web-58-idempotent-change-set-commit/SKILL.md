---
name: legacy-web-58-idempotent-change-set-commit
description: "Repository-owned exact runtime interface for 幂等 Change Set 提交; bounded semantic analysis and evidence generation for Java legacy web modernization."
metadata:
  source_package: elmos.java-legacy-web.repository-modernization
  source_version: 1.0.0
  source_id: 58-idempotent-change-set-commit
  source_digest: sha256:55dd7ec92a930925db07099a8a6068589712451fd0c0ce81fffb92ac806fe11b
  phase: transformation
  runtime_state: CODE_COMPLETE_LOCAL
  capability_state: LOCAL_EXECUTED
  operation_code: CHANGE_SET_STAGING_COMMIT_COMPILED
  runtime_handler_id: legacy-web-handler:58-idempotent-change-set-commit
---
...
---
id: 58-idempotent-change-set-commit
title: 幂等 Change Set 提交
version: 1.0.0
phase: transformation
priority: P0
requires:
...
将改写按 transformation unit 分组，确保重复执行不漂移、冲突可回滚、提交原子化且生成机器可读 manifest。
...
producerSkill: 58-idempotent-change-set-commit
...
5. 以幂等 change set 发布。
6. change set 包含前置 hash、操作、逆操作、依赖、验证命令和产物 hash。
7. 重复 apply 返回 no-op 或同一结果。
8. 冲突时不自动覆盖用户并发修改，生成三方合并任务。
9. 提交按模块/语义单元分组，避免超大不可审查 commit。
