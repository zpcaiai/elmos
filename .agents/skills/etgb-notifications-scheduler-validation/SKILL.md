---
name: etgb-notifications-scheduler-validation
description: Validate all Notifications, Scheduled Jobs and Delivery Reliability capabilities through the production adapter, independent Oracles and fail-closed release evidence. Repository-owned ETGB execution is available through the local runtime; external production evidence remains explicit.
metadata:
  ...
  source_skill: notifications-scheduler-validation
...
---
name: notifications-scheduler-validation
description: Validate all declared Notifications, Scheduled Jobs and Delivery Reliability capabilities with executable positive, boundary, adversarial, concurrency, recovery and evidence-backed release tests.
---

# Notifications, Scheduled Jobs and Delivery Reliability Validation

## Purpose

Use this Skill to plan, execute and certify the `notifications-scheduler` product domain. It owns **46 declared capabilities** (25 P0, 16 P1, 5 P2) across 4 execution contexts and four mandatory variants.

## Inputs

- Frozen release candidate and feature-registry revision.
- Tenant, project, environment authority, budget and fencing context.
- Concrete cases from `suites/full-product.jsonl` whose business line is `notifications-scheduler`.
...
2. Provision an isolated context for: in-app, email, webhook, scheduled-digest.
...
5. Apply the domain Oracle `notification-delivery-idempotency-preference-and-audit` plus security, state, audit and unsupported-disclosure Oracles.
...
Representative capabilities include: Event To Notification Mapping, Notification Tenant And Recipient Authorization, Template Variable Validation, No Secret Or Pii Leak, Delivery Idempotency, Duplicate Event Suppression, Out Of Order Event Policy, Retry And Dead Letter, Delivery Status, User Preferences, Unsubscribe And Mandatory Notification Policy, Timezone Scheduled Delivery. The complete source of truth is `matrices/full-product.yaml`; no prose list may override it.
...
Route execution through `external-notification-scheduler-harness` under `integrations/harness/full-product-adapters.yaml`. An adapter stub or skipped case is implementation progress, not product certification.
