...
# Smt Translation Validator
## ELMOS runtime binding

- Invoke this repository Skill as `$pm-b35-smt-translation-validator`.
- Immutable source identity: `smt-translation-validator` in `precision-migration-b01-44` (B35).
- Runtime adapter: `formal-and-advanced-verification`; binding state: `DECLARED`.
- Resolve and plan with `python3 scripts/precision_migration/runtime.py plan --skill pm-b35-smt-translation-validator`.
...
将源目标 Core IR 编码为 SMT 关系，证明等价或寻找反例。

本 Skill 属于精密语言转换系统。它必须以**可执行证据**为准，不得用模型自评、源码相似度或“能编译”替代语义与行为验证。

## Use when

- 当前任务直接涉及 `smt-translation-validator` 所描述的能力。
...
1. 固定语义与定理陈述
2. 执行本 Skill 的核心职责：将源目标 Core IR 编码为 SMT 关系，证明等价或寻找反例。
3. 生成或编码证明义务
4. 调用Lean/SMT/模型检查
5. 最小化反例并回写测试
6. 由可信内核/求解器签发结果
...
```yaml
skill: smt-translation-validator
version: 1.0.0
