...
description: "Implement and verify Batch 41 Skill 1397 for pattern antipattern extraction. Use when work requires Pattern\u548cAnti-pattern\u63d0\u53d6, typed evidence, negative and holdout validation, and a fail-closed certification decision."
---

# Pattern和Anti-pattern提取
...
## Skill 1397: Pattern和Anti-pattern提取
...
1. Inspect the current implementation, runtime facts, policies, incidents, decisions, and prior evidence relevant to `b41-pattern-antipattern-extraction`.
...
4. Implement the smallest production-shaped capability that exercises `Pattern和Anti-pattern提取` without bypassing existing control planes.
...
- Capability-specific implementation, tests, run logs, evidence references, and residual-risk records for `b41-pattern-antipattern-extraction`
