---
name: pm-b14-typescript-cfg-ssa-adapter
description: "为 TypeScript/Node.js 构建控制流、数据流与 SSA/近似 SSA 表示，支持切片和验证. Precision Migration B14 contract; use for this exact assessment, transformation, validation, repair, evidence, or cutover scope."
---

# Typescript Cfg Ssa Adapter
## ELMOS runtime binding

- Invoke this repository Skill as `$pm-b14-typescript-cfg-ssa-adapter`.
- Immutable source identity: `typescript-cfg-ssa-adapter` in `precision-migration-b01-44` (B14).
- Runtime adapter: `directed-backend-route`; binding state: `UNAVAILABLE`.
- Resolve and plan with `python3 scripts/precision_migration/runtime.py plan --skill pm-b14-typescript-cfg-ssa-adapter`.
...
为 TypeScript/Node.js 构建控制流、数据流与 SSA/近似 SSA 表示，支持切片和验证。

本 Skill 属于精密语言转换系统。它必须以**可执行证据**为准，不得用模型自评、源码相似度或“能编译”替代语义与行为验证。

## Use when

- 当前任务直接涉及 `typescript-cfg-ssa-adapter` 所描述的能力。
...
1. 使用语言原生前端恢复语义
2. 执行本 Skill 的核心职责：为 TypeScript/Node.js 构建控制流、数据流与 SSA/近似 SSA 表示，支持切片和验证。
...
```yaml
skill: typescript-cfg-ssa-adapter
version: 1.0.0
