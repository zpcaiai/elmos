...
description: "Implement and verify Batch 42 Skill 1415 for migration planner agent. Use when work requires Migration Planner Agent, typed evidence, negative and holdout validation, and a fail-closed certification decision."
---
...
## Skill 1415: Migration Planner Agent
...
1. Inspect the current implementation, runtime facts, policies, incidents, decisions, and prior evidence relevant to `b42-migration-planner-agent`.
...
4. Implement the smallest production-shaped capability that exercises `Migration Planner Agent` without bypassing existing control planes.
...
- Capability-specific implementation, tests, run logs, evidence references, and residual-risk records for `b42-migration-planner-agent`
