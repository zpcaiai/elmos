...
# Modernization Option Comparator
## ELMOS runtime binding

- Invoke this repository Skill as `$pm-b02-modernization-option-comparator`.
- Immutable source identity: `modernization-option-comparator` in `precision-migration-b01-44` (B02).
- Runtime adapter: `assessment-and-target-planning`; binding state: `UNAVAILABLE`.
- Resolve and plan with `python3 scripts/precision_migration/runtime.py plan --skill pm-b02-modernization-option-comparator`.
...
比较保留、升级、重新平台化、重构、重写、包装和退役等现代化路径。

本 Skill 属于精密语言转换系统。它必须以**可执行证据**为准，不得用模型自评、源码相似度或“能编译”替代语义与行为验证。

## Use when

- 当前任务直接涉及 `modernization-option-comparator` 所描述的能力。
...
1. 确认决策问题与可比较边界
2. 执行本 Skill 的核心职责：比较保留、升级、重新平台化、重构、重写、包装和退役等现代化路径。
3. 收集可验证证据并标准化
4. 应用评分/比较/试点模型
5. 显式列出未知项和敏感假设
6. 输出推荐、备选与拒绝条件
...
```yaml
skill: modernization-option-comparator
version: 1.0.0
