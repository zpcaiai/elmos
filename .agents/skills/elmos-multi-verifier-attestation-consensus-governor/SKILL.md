---
name: elmos-multi-verifier-attestation-consensus-governor
description: Implement and independently certify multi-verifier attestation consensus governor, including compose hardware, firmware, workload, model and policy verifier results, define quorum, veto, conflict and freshness rules without weakening authoritative failures and record verifier independence, overlap and uncertainty.
version: 4.0.0
...
# Multi-Verifier Attestation Consensus Governor
...
- `MultiVerifierAttestationConsensusGovernorProfile`
- `MultiVerifierAttestationConsensusGovernorRun`
- `MultiVerifierAttestationConsensusGovernorFinding`
- `MultiVerifierAttestationConsensusGovernorDecision`
- `MultiVerifierAttestationConsensusGovernorEvidence`

Required semantic capabilities:

- compose hardware, firmware, workload, model and policy verifier results
- define quorum, veto, conflict and freshness rules without weakening authoritative failures
- record verifier independence, overlap and uncertainty
...
- `elmos-rats-eat-corim-attestation-appraisal-controller`
- `elmos-proof-checker-diversity-cross-validation-governor`

## Primary Outputs

- `artifacts/elmos-multi-verifier-attestation-consensus-governor/profile.yaml`
- `artifacts/elmos-multi-verifier-attestation-consensus-governor/plan.json`
- `artifacts/elmos-multi-verifier-attestation-consensus-governor/result.json`
- `artifacts/elmos-multi-verifier-attestation-consensus-governor/evidence/`
- `artifacts/elmos-multi-verifier-attestation-consensus-governor/assurance-case.json`
...
- **majority overrides critical hardware refutation** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **verifiers share same endorsement source** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **stale component result is combined with fresh evidence** — must have a preventive control, a detective signal, a negative fixture and a response action.
...
- native scenario: compose hardware, firmware, workload, model and policy verifier results
- native scenario: define quorum, veto, conflict and freshness rules without weakening authoritative failures
- native scenario: record verifier independence, overlap and uncertainty
