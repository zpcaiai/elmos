---
name: b43-rolling-mixed-version-upgrade
description: "Implement and verify Batch 43 Skill 1450 for rolling mixed version upgrade. Use when work requires Rolling Upgrade\u548c\u6df7\u5408\u7248\u672c\u8fd0\u884c, typed evidence, negative and holdout validation, and a fail-closed certification decision."
---

# Rolling Upgrade和混合版本运行
...
## Skill 1450: Rolling Upgrade和混合版本运行
...
1. Inspect the current implementation, runtime facts, policies, incidents, decisions, and prior evidence relevant to `b43-rolling-mixed-version-upgrade`.
...
4. Implement the smallest production-shaped capability that exercises `Rolling Upgrade和混合版本运行` without bypassing existing control planes.
...
- Capability-specific implementation, tests, run logs, evidence references, and residual-risk records for `b43-rolling-mixed-version-upgrade`
