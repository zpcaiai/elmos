---
name: elmos-lab-environment-validity-monitor
description: Implement and independently certify laboratory environment and result validity monitor, including continuously monitor environment, dependency, network, data and provider conditions that influence results, apply control charts, check samples and trend rules to validity indicators and quarantine affected results and trigger re-evaluation.
version: 4.0.0
...
# Laboratory Environment and Result Validity Monitor
...
- `LaboratoryEnvironmentAndResultValidityMonitorProfile`
- `LaboratoryEnvironmentAndResultValidityMonitorRun`
- `LaboratoryEnvironmentAndResultValidityMonitorFinding`
- `LaboratoryEnvironmentAndResultValidityMonitorDecision`
- `LaboratoryEnvironmentAndResultValidityMonitorEvidence`

Required semantic capabilities:

- continuously monitor environment, dependency, network, data and provider conditions that influence results
- apply control charts, check samples and trend rules to validity indicators
- quarantine affected results and trigger re-evaluation
...
- `elmos-test-equipment-software-calibration-traceability-governor`
- `elmos-lab-reproducibility-repeatability-certifier`

## Primary Outputs

- `artifacts/elmos-lab-environment-validity-monitor/profile.yaml`
- `artifacts/elmos-lab-environment-validity-monitor/plan.json`
- `artifacts/elmos-lab-environment-validity-monitor/result.json`
- `artifacts/elmos-lab-environment-validity-monitor/evidence/`
- `artifacts/elmos-lab-environment-validity-monitor/assurance-case.json`
...
- **environment drift occurs between periodic checks** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **control sample is not representative of main test** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **invalid run remains in certificate evidence** — must have a preventive control, a detective signal, a negative fixture and a response action.
...
- native scenario: continuously monitor environment, dependency, network, data and provider conditions that influence results
- native scenario: apply control charts, check samples and trend rules to validity indicators
- native scenario: quarantine affected results and trigger re-evaluation
