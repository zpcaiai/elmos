---
name: gr-observability
description: 统一 trace_id/job_id/project_id/run_id，记录队列、阶段、模型、工具、缓存、质量和错误预算。 Use for this exact ELMOS Spring Golden Route v2 contract; imported specifications are not runtime or certification evidence.
...
metadata:
  ...
  source_id: ELMOS-GR-069
  source_name: gr-observability
  source_path: skills/gr-observability/SKILL.md
  source_sha256: sha256:3593fca4cdcdf5ad524773b2c35cc230b1359de22de4cd7f34ace46951c90220
  source_contract_sha256: sha256:ccc74053909ba44cf0cf02918a9664ed44f1aff9c5b2eab1e6605c965904717c
...
统一 trace_id/job_id/project_id/run_id，记录队列、阶段、模型、工具、缓存、质量和错误预算。
...
- [ ] 定义 `gr-observability` 的 public interface、输入/输出/Error Schema 和版本策略。
- [ ] 实现 `gr-observability` 的 production adapter/service，不把业务状态只保存在 Prompt 或内存对象中。
...
- [ ] 至少生成：slo.yaml, dashboard-spec.json, alert-rules.yaml。
...
- [ ] 本 Skill 的 required outputs 均由真实目标仓库执行产生：slo.yaml, dashboard-spec.json, alert-rules.yaml。
...
- Machine contract: `references/contract.json` (`sha256:ccc74053909ba44cf0cf02918a9664ed44f1aff9c5b2eab1e6605c965904717c`).
