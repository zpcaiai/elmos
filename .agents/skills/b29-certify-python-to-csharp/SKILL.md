---
name: b29-certify-python-to-csharp
description: Implement or certify the directed Python-to-C# migration route, including source semantics, target lowering, compatibility strategy, corpus, real builds, behavior evidence, and route manifest. Use only for Python source and C# target work.
---
...
Create a customer-usable, independently certified Python → C# route. The reverse direction is not covered by this skill.

## Use this skill when

Use when adding, repairing, benchmarking, or certifying the `python-to-csharp` route.

## Route-specific semantic hazards

- dynamic attributes, metaclasses, descriptors, and monkey patching
- multiple inheritance and method-resolution order
- duck typing, overload synthesis, and insufficient type evidence
- generators, context managers, decorators, and async iterators
- arbitrary-precision integers, truthiness, and container semantics
- runtime imports, eval/exec, and native extension boundaries

## Workflow

1. Inspect `engines/python-engine`, `engines/csharp-engine`, shared PSP/UIR contracts, and any existing `routes/python-to-csharp` package.
2. Run `python3 scripts/batch29/scaffold_route.py --source python --target csharp` if the route package is absent.
...
- `routes/python-to-csharp/route.json`
- `routes/python-to-csharp/support-matrix.json`
- `routes/python-to-csharp/lowering/`
- `routes/python-to-csharp/mappings/`
- `routes/python-to-csharp/compat-runtime/manifest.json`
- `routes/python-to-csharp/corpus/development/`
- `routes/python-to-csharp/corpus/holdout/`
- `routes/python-to-csharp/certification/evidence.json`

## Verification

- Run the real Python source compiler/runtime and the real C# target compiler/runtime.
...
- Run `python3 scripts/batch29/validate_route.py routes/python-to-csharp` and the route gate.
- Verify critical behavior differences and unknown critical semantics are zero for certified scope.

## Stop and escalate when

- A listed dynamic attributes, metaclasses, descriptors, and monkey patching cannot be preserved, adapted, or explicitly blocked.
- A listed multiple inheritance and method-resolution order cannot be preserved, adapted, or explicitly blocked.
- A listed duck typing, overload synthesis, and insufficient type evidence cannot be preserved, adapted, or explicitly blocked.
