---
name: elmos-gpu-accelerator-capacity-scheduler
description: Schedule heterogeneous GPUs/NPUs, model replicas, batching and preemption under memory, topology, fairness, SLO and cost constraints.
version: 4.0.0
...
# GPU and Accelerator Capacity Scheduler
...
- `GPUAndAcceleratorCapacitySchedulerProfile`
- `GPUAndAcceleratorCapacitySchedulerPlan`
- `GPUAndAcceleratorCapacitySchedulerRun`
- `GPUAndAcceleratorCapacitySchedulerDecision`

Required semantic capabilities:

- model accelerator capabilities and topology
- bin-pack models with memory/KV constraints
- reserve critical tenant capacity
- coordinate preemption and warm pools
- forecast queue, cost and scale-up delay
...
- `artifacts/elmos-gpu-accelerator-capacity-scheduler/profile.yaml`
- `artifacts/elmos-gpu-accelerator-capacity-scheduler/plan.json`
- `artifacts/elmos-gpu-accelerator-capacity-scheduler/result.json`
- `artifacts/elmos-gpu-accelerator-capacity-scheduler/evidence/`
...
- **memory fragmentation ignored** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **tenant starvation** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **preemption corrupts session** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **scale-up ETA optimistic** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **hardware telemetry spoofed** — must have a preventive control, a detective signal, a negative fixture and a response action.
...
- native scenario: model accelerator capabilities and topology
- native scenario: bin-pack models with memory/KV constraints
- native scenario: reserve critical tenant capacity
- native scenario: coordinate preemption and warm pools
- native scenario: forecast queue, cost and scale-up delay
