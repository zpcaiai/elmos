---
name: build-model-intelligence
description: 解析构建工具、依赖图、插件、仓库和语言级别形成统一 Build IR。 Use for this exact ELMOS Spring Golden Route v2 contract; imported specifications are not runtime or certification evidence.
...
metadata:
  ...
  source_id: FOUNDATION-04-build-model-intelligence
  source_name: build-model-intelligence
  source_path: skills/build-model-intelligence/SKILL.md
  source_sha256: sha256:60260a85b96778a0a8589bbb408c869035a2cb9407af576c7152b552d406718a
  source_contract_sha256: sha256:fe6255502a6dc4f138b1defd81f9f1a9549f72500feff7abfa29c3dff902bd08
...
解析构建工具、依赖图、插件、仓库和语言级别形成统一 Build IR。

## Use when

在 Elmos 需要实现或升级 **Build Model Intelligence** 相关能力时使用；它不是仅供解释的提示词，而是实现、测试与验收契约。

## Capabilities to implement

- Maven/Gradle/npm/Python 等 manifest 与源码语义关联
- 区分直接、传递、约束、插件和 BOM
- 识别 repository/index、镜像和凭据需求
- 变换后自动验证构建图一致性
...
- [ ] 定义 `build-model-intelligence` 的 public interface、input/output/error schemas。
- [ ] 建立 `build-model-intelligence` 的 capability/feature flags 与版本字段。
...
- [ ] 至少覆盖并验收：Maven/Gradle/npm/Python 等 manifest 与源码语义关联；以及 区分直接、传递、约束、插件和 BOM。
...
- Machine contract: `references/contract.json` (`sha256:fe6255502a6dc4f138b1defd81f9f1a9549f72500feff7abfa29c3dff902bd08`).
