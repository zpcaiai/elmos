---
name: spring-build-migrator
description: 确定性迁移 POM/BOM/plugin/dependency/starter 并验证 convergence。 Use for this exact ELMOS Spring Golden Route v2 contract; imported specifications are not runtime or certification evidence.
...
metadata:
  ...
  source_id: ELMOS-GR-045
  source_name: spring-build-migrator
  source_path: skills/spring-build-migrator/SKILL.md
  source_sha256: sha256:3efea0b51eaef2c8a15fd1540244d47a3b1ebac074f351e7e7a3df34fe212eaf
  source_contract_sha256: sha256:01faa279741b8f24538e8b39f430c60935966ec054cf3fd994bdc1774b325f4c
...
# Build & Dependency Migrator

## Objective

确定性迁移 POM/BOM/plugin/dependency/starter 并验证 convergence。
...
- [ ] 定义 `spring-build-migrator` 的 public interface、输入/输出/Error Schema 和版本策略。
- [ ] 实现 `spring-build-migrator` 的 production adapter/service，不把业务状态只保存在 Prompt 或内存对象中。
...
- [ ] 至少生成：build-patches, dependency-diff.json, convergence-report.json。
...
- [ ] 全量 Recipe 最终收敛
- [ ] Agent 只接收剩余业务歧义
- [ ] 本 Skill 的 required outputs 均由真实目标仓库执行产生：build-patches, dependency-diff.json, convergence-report.json。
- [ ] 所有 mandatory tests 和 security gates 通过，且无 critical unknown。

## Dependencies

- `spring-license-router`
- `spring-maven-multimodule`
...
- Machine contract: `references/contract.json` (`sha256:01faa279741b8f24538e8b39f430c60935966ec054cf3fd994bdc1774b325f4c`).
