---
name: b95-lua-runtime-version-compatibility-checker
description: "Use when ELMOS must run lua runtime version compatibility checker for Batch 95 Lua / OpenResty / Embedded Scripting Pack. Preserve native semantics, safety boundaries, ownership, traceability, and fail-closed evidence; require real vendor toolchain execution for runtime claims."
metadata:
  source_package: "elmos-language-packs-batch81-95-complete"
  source_id: "PG401"
  source_name: "lua-runtime-version-compatibility-checker"
  source_sha256: "sha256:cbc1dbeff2297ac35ec275f81a7d28187bb1c1430b0903540fd5ec9b1011ca20"
  batch: 95
  source_engine: "elmos.language-packs"
  source_status: "proposed"
  normalized_namespace: "language-pack"
---
# Batch 95 Language Pack / source PG401 — Lua Runtime Version Compatibility Checker

## 1. Objective

Verify lua runtime version compatibility checker within the Lua / OpenResty / Embedded Scripting Pack, preserving platform semantics, traceability, security, and reproducible evidence.

## 2. Scope

This Skill is part of **Batch 95: Lua / OpenResty / Embedded Scripting Pack**.

### In scope

- Process Lua modules.
- Process OpenResty/Nginx configs.
- Process embedded scripts.
- Process plugins.
- Process coroutines.
- Process LuaJIT FFI.
- Process game/server scripts.
- Produce Lua projects.
- Produce OpenResty gateways.
- Produce sandbox policies.
- Produce host bindings.
- Produce deployment assets.
...
2. Process the relevant artifacts for Lua Runtime Version Compatibility Checker using the approved embedded-scripting-gateway semantic contracts and toolchain versions.
3. Preserve source provenance and create Artifact Graph links across inputs, outputs, tests, approvals, and evidence.
4. Detect and classify sandbox escape, event-loop blocking, FFI memory error; never convert uncertainty or unsupported behavior into a silent success.
...
7. Apply the safety boundary: Untrusted Lua and plugin execution must be capability-limited, resource-bounded, and isolated from secrets and host process authority.
...
- `SANDBOX_ESCAPE` — stop unsafe continuation and emit source-linked diagnostic evidence.
- `EVENT_LOOP_BLOCKING` — stop unsafe continuation and emit source-linked diagnostic evidence.
- `FFI_MEMORY_ERROR` — stop unsafe continuation and emit source-linked diagnostic evidence.
- `LUA_LUAJIT_INCOMPATIBILITY` — stop unsafe continuation and emit source-linked diagnostic evidence.
- `SHARED_DICTIONARY_RACE` — stop unsafe continuation and emit source-linked diagnostic evidence.
- `UNTRUSTED_PLUGIN_PRIVILEGE` — stop unsafe continuation and emit source-linked diagnostic evidence.
...
- Apply this Batch safety boundary: **Untrusted Lua and plugin execution must be capability-limited, resource-bounded, and isolated from secrets and host process authority.**

## 14. Unit Tests

- A representative valid lua-openresty input produces all declared outputs for lua-runtime-version-compatibility-checker.
...
- At least one batch-specific risk (sandbox escape) is detected by a negative fixture.

## 15. Integration Tests

- Consume approved Project Synthesis or Modernization baselines and write complete lineage into the Artifact Graph.
- Integrate with at least one representative toolchain from: Lua 5.x, LuaJIT, OpenResty, Nginx.
...
- Reject or block sandbox escape without weakening the expected result.
- Reject or block event-loop blocking without weakening the expected result.
- Reject or block FFI memory error without weakening the expected result.
- Reject or block Lua/LuaJIT incompatibility without weakening the expected result.
- Reject or block shared-dictionary race without weakening the expected result.
- Reject or block untrusted plugin privilege without weakening the expected result.
