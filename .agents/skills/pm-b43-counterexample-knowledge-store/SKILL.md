---
name: pm-b43-counterexample-knowledge-store
description: "存储方向、源码模式、错误目标、最小反例、根因、修复和版本. Precision Migration B43 contract; use for this exact assessment, transformation, validation, repair, evidence, or cutover scope."
---

# Counterexample Knowledge Store
## ELMOS runtime binding

- Invoke this repository Skill as `$pm-b43-counterexample-knowledge-store`.
- Immutable source identity: `counterexample-knowledge-store` in `precision-migration-b01-44` (B43).
- Runtime adapter: `continuous-modernization-learning`; binding state: `DECLARED`.
- Resolve and plan with `python3 scripts/precision_migration/runtime.py plan --skill pm-b43-counterexample-knowledge-store`.
...
- Phase: `L 证据、上线和产品化`
- Version: `1.0.0`
- Tags: batch-43, continuous-modernization-learning

## Purpose

存储方向、源码模式、错误目标、最小反例、根因、修复和版本。

本 Skill 属于精密语言转换系统。它必须以**可执行证据**为准，不得用模型自评、源码相似度或“能编译”替代语义与行为验证。

## Use when

- 当前任务直接涉及 `counterexample-knowledge-store` 所描述的能力。
...
1. 汇总证据与未解决项
2. 执行本 Skill 的核心职责：存储方向、源码模式、错误目标、最小反例、根因、修复和版本。
3. 执行硬性发布门禁
4. 影子/Canary/渐进切换
5. 监控并自动回滚
6. 沉淀反例、规则和企业交付能力
...
```yaml
skill: counterexample-knowledge-store
version: 1.0.0
