---
name: elmos-chaos-experiment-safety-governor
description: Plan and authorize bounded failure injection with blast-radius, abort, observability, side-effect and restoration controls.
version: 4.0.0
...
# Chaos Experiment Safety Governor
...
- `ChaosExperimentSafetyGovernorProfile`
- `ChaosExperimentSafetyGovernorPlan`
- `ChaosExperimentSafetyGovernorRun`
- `ChaosExperimentSafetyGovernorDecision`

Required semantic capabilities:

- compile experiment hypothesis and steady state
- calculate scope and dependency blast radius
- enforce kill switch and time bounds
- verify monitoring before injection
- restore state and reconcile effects
...
- `artifacts/elmos-chaos-experiment-safety-governor/profile.yaml`
- `artifacts/elmos-chaos-experiment-safety-governor/plan.json`
- `artifacts/elmos-chaos-experiment-safety-governor/result.json`
- `artifacts/elmos-chaos-experiment-safety-governor/evidence/`
...
- **chaos runs in wrong tenant** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **abort signal unavailable** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **steady state metric blind** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **experiment corrupts durable data** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **restoration assumed not verified** — must have a preventive control, a detective signal, a negative fixture and a response action.
...
- native scenario: compile experiment hypothesis and steady state
- native scenario: calculate scope and dependency blast radius
- native scenario: enforce kill switch and time bounds
- native scenario: verify monitoring before injection
- native scenario: restore state and reconcile effects
