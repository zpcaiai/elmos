---
name: pm-b24-mysql-json
description: "迁移 JSON 类型、路径、函数、索引和比较语义. Precision Migration B24 contract; use for this exact assessment, transformation, validation, repair, evidence, or cutover scope."
---

# Mysql Json
## ELMOS runtime binding

- Invoke this repository Skill as `$pm-b24-mysql-json`.
- Immutable source identity: `mysql-json` in `precision-migration-b01-44` (B24).
- Runtime adapter: `database-and-data-route`; binding state: `DECLARED`.
- Resolve and plan with `python3 scripts/precision_migration/runtime.py plan --skill pm-b24-mysql-json`.
...
1. 盘点对象与专有能力
2. 执行本 Skill 的核心职责：迁移 JSON 类型、路径、函数、索引和比较语义。
