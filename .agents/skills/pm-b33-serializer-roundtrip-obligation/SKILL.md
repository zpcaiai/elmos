---
name: pm-b33-serializer-roundtrip-obligation
description: "生成编码、解码、规范化和跨语言 Carrier 的 Round-trip 义务. Precision Migration B33 contract; use for this exact assessment, transformation, validation, repair, evidence, or cutover scope."
---

# Serializer Roundtrip Obligation
...
生成编码、解码、规范化和跨语言 Carrier 的 Round-trip 义务。

本 Skill 属于精密语言转换系统。它必须以**可执行证据**为准，不得用模型自评、源码相似度或“能编译”替代语义与行为验证。

## Use when

- 当前任务直接涉及 `serializer-roundtrip-obligation` 所描述的能力。
...
1. 固定语义与定理陈述
2. 执行本 Skill 的核心职责：生成编码、解码、规范化和跨语言 Carrier 的 Round-trip 义务。
3. 生成或编码证明义务
4. 调用Lean/SMT/模型检查
5. 最小化反例并回写测试
6. 由可信内核/求解器签发结果
...
```yaml
skill: serializer-roundtrip-obligation
version: 1.0.0
