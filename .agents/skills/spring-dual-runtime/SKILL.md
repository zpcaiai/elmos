---
name: spring-dual-runtime
description: 在隔离环境同时运行旧系统和目标系统，并对同一场景提供可比较端点。 Use for this exact ELMOS Spring Golden Route v2 contract; imported specifications are not runtime or certification evidence.
...
metadata:
  ...
  source_id: ELMOS-GR-057
  source_name: spring-dual-runtime
  source_path: skills/spring-dual-runtime/SKILL.md
  source_sha256: sha256:8be67178138920d0257edf77cfb620177f018f94d985ee6068b6c1fbe17ffce3
  source_contract_sha256: sha256:26cdd2ba26fbeb7ba67a7239bafd74db934ee8bcbb3e0c5354f02684ba19b8af
...
在隔离环境同时运行旧系统和目标系统，并对同一场景提供可比较端点。
...
- [ ] 定义 `spring-dual-runtime` 的 public interface、输入/输出/Error Schema 和版本策略。
- [ ] 实现 `spring-dual-runtime` 的 production adapter/service，不把业务状态只保存在 Prompt 或内存对象中。
...
- [ ] 至少生成：runtime-manifest.json, health-evidence.json, scenario-endpoints.json。
...
- [ ] 本 Skill 的 required outputs 均由真实目标仓库执行产生：runtime-manifest.json, health-evidence.json, scenario-endpoints.json。
...
- Machine contract: `references/contract.json` (`sha256:26cdd2ba26fbeb7ba67a7239bafd74db934ee8bcbb3e0c5354f02684ba19b8af`).
