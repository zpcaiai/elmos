...
## Skill 1152: Java → TypeScript

Create a customer-usable, independently certified Java → TypeScript route. The reverse direction is not covered by this skill.

## Use this skill when

Use when adding, repairing, benchmarking, or certifying the `java-to-typescript` route.

## Route-specific semantic hazards

- Java integral widths versus JavaScript number/BigInt
- nominal classes versus structural typing
- null versus undefined and optional properties
- threads and blocking APIs versus the event loop
- checked exceptions and synchronous resource lifetime
- annotations, reflection, package/module, and serialization differences

## Workflow

1. Inspect `engines/java-engine`, `engines/typescript-engine`, shared PSP/UIR contracts, and any existing `routes/java-to-typescript` package.
2. Run `python3 scripts/batch29/scaffold_route.py --source java --target typescript` if the route package is absent.
...
- `routes/java-to-typescript/route.json`
- `routes/java-to-typescript/support-matrix.json`
- `routes/java-to-typescript/lowering/`
- `routes/java-to-typescript/mappings/`
- `routes/java-to-typescript/compat-runtime/manifest.json`
- `routes/java-to-typescript/corpus/development/`
- `routes/java-to-typescript/corpus/holdout/`
- `routes/java-to-typescript/certification/evidence.json`

## Verification

- Run the real Java source compiler/runtime and the real TypeScript target compiler/runtime.
...
- Run `python3 scripts/batch29/validate_route.py routes/java-to-typescript` and the route gate.
- Verify critical behavior differences and unknown critical semantics are zero for certified scope.

## Stop and escalate when

- A listed Java integral widths versus JavaScript number/BigInt cannot be preserved, adapted, or explicitly blocked.
- A listed nominal classes versus structural typing cannot be preserved, adapted, or explicitly blocked.
- A listed null versus undefined and optional properties cannot be preserved, adapted, or explicitly blocked.
