---
name: pm-b38-skill-composition-planner
description: "生成满足依赖、顺序、隔离和共享能力的 Skill 组合执行计划. Precision Migration B38 contract; use for this exact assessment, transformation, validation, repair, evidence, or cutover scope."
---

# Skill Composition Planner
## ELMOS runtime binding

- Invoke this repository Skill as `$pm-b38-skill-composition-planner`.
- Immutable source identity: `skill-composition-planner` in `precision-migration-b01-44` (B38).
- Runtime adapter: `skill-and-project-synthesis`; binding state: `UNAVAILABLE`.
- Resolve and plan with `python3 scripts/precision_migration/runtime.py plan --skill pm-b38-skill-composition-planner`.
...
- Version: `1.0.0`
- Tags: batch-38, executable-skill-specification

## Purpose

生成满足依赖、顺序、隔离和共享能力的 Skill 组合执行计划。

本 Skill 属于精密语言转换系统。它必须以**可执行证据**为准，不得用模型自评、源码相似度或“能编译”替代语义与行为验证。

## Use when

- 当前任务直接涉及 `skill-composition-planner` 所描述的能力。
...
2. 执行本 Skill 的核心职责：生成满足依赖、顺序、隔离和共享能力的 Skill 组合执行计划。
3. 编译Skills与架构蓝图
4. 生成/复用全部构件
5. 构建并执行验收/安全/完整性门禁
6. 输出项目、Runbook和证据
...
```yaml
skill: skill-composition-planner
version: 1.0.0
