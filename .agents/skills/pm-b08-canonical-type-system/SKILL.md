...
- Invoke this repository Skill as `$pm-b08-canonical-type-system`.
- Immutable source identity: `batch-08-canonical-type-system` in `precision-migration-b01-44` (B08).
- Runtime adapter: `semantic-recovery-and-ir`; binding state: `DECLARED`.
- Resolve and plan with `python3 scripts/precision_migration/runtime.py plan --skill pm-b08-canonical-type-system`.
...
- 当任务涉及 **canonical-primitive-types** 时，调用 `../pm-b08-canonical-primitive-types/SKILL.md`。
- 当任务涉及 **numeric-semantics-model** 时，调用 `../pm-b08-numeric-semantics-model/SKILL.md`。
- 当任务涉及 **decimal-and-rounding-model** 时，调用 `../pm-b08-decimal-and-rounding-model/SKILL.md`。
- 当任务涉及 **null-missing-undefined-model** 时，调用 `../pm-b08-null-missing-undefined-model/SKILL.md`。
- 当任务涉及 **option-result-union-model** 时，调用 `../pm-b08-option-result-union-model/SKILL.md`。
- 当任务涉及 **generic-and-variance-model** 时，调用 `../pm-b08-generic-and-variance-model/SKILL.md`。
- 当任务涉及 **collection-semantics-model** 时，调用 `../pm-b08-collection-semantics-model/SKILL.md`。
- 当任务涉及 **datetime-timezone-model** 时，调用 `../pm-b08-datetime-timezone-model/SKILL.md`。
- 当任务涉及 **serialization-type-model** 时，调用 `../pm-b08-serialization-type-model/SKILL.md`。
- 当任务涉及 **refinement-type-contracts** 时，调用 `../pm-b08-refinement-type-contracts/SKILL.md`。
- 当任务涉及 **ownership-and-lifetime-model** 时，调用 `../pm-b08-ownership-and-lifetime-model/SKILL.md`。
- 当任务涉及 **gradual-type-recovery** 时，调用 `../pm-b08-gradual-type-recovery/SKILL.md`。
