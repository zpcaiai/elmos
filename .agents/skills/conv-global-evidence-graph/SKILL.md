...
# CONV-006：统一Evidence Graph

## Objective

连接源码、语义、规则、生成代码、测试、审批、Release和生产部署。
...
- Batch 21–45 Evidence类型
- Artifact和Source Map

## Required outputs

- `evidence-graph.schema.json`
- `Evidence Node/Edge Registry`
- Lineage查询接口

## Workflow

1. 定义稳定Node和Edge类型。
2. 统一Digest、Producer、Trust、Retention和Signature。
3. 导入现有Evidence并保留原始引用。
4. 实现从目标Symbol回溯源Commit与认证。
5. 实现过期、撤销和删除传播。
