---
name: etgb-ai-solution-factory-validation
description: Validate all AI-Native Project Factory and Cross-Framework Agent Solution Compiler capabilities through the production adapter, independent Oracles and fail-closed release evidence. Repository-owned ETGB execution is available through the local runtime; external production evidence remains explicit.
metadata:
  ...
  source_skill: ai-solution-factory-validation
...
---
name: ai-solution-factory-validation
description: Validate all declared AI-Native Project Factory and Cross-Framework Agent Solution Compiler capabilities with executable positive, boundary, adversarial, concurrency, recovery and evidence-backed release tests.
---

# AI-Native Project Factory and Cross-Framework Agent Solution Compiler Validation

## Purpose

Use this Skill to plan, execute and certify the `ai-solution-factory` product domain. It owns **62 declared capabilities** (37 P0, 20 P1, 5 P2) across 4 execution contexts and four mandatory variants.

## Inputs

- Frozen release candidate and feature-registry revision.
- Tenant, project, environment authority, budget and fencing context.
- Concrete cases from `suites/full-product.jsonl` whose business line is `ai-solution-factory`.
...
2. Provision an isolated context for: import-existing, greenfield-generate, cross-framework-convert, upgrade-and-roundtrip.
...
5. Apply the domain Oracle `ai-solution-native-conformance-graph-rag-tool-security-and-proof` plus security, state, audit and unsupported-disclosure Oracles.
...
Representative capabilities include: Ai Sir Requirement Compilation, Ai Sir Model Agent Workflow Tool Rag Memory Interaction Security Runtime Assurance, Dify Import Generate Conformance, Universal Rag Generate Conformance, Langchain Import Generate Conformance, Langgraph Import Generate Conformance, Spring Ai Import Generate Conformance, Pi Agent Import Generate Conformance, Deepseek Harness Conformance, Openharness Conformance, Symphony Conformance, Openclaw Conformance. The complete source of truth is `matrices/full-product.yaml`; no prose list may override it.
...
Route execution through `external-ai-solution-factory-harness` under `integrations/harness/full-product-adapters.yaml`. An adapter stub or skipped case is implementation progress, not product certification.
