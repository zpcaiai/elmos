---
name: elmos-adapter-xbase
description: Implement xBase/FoxPro language parsing, semantic lowering, framework/runtime
  mapping, target emission and verification for repository-scale modernization.
version: 1.0.0
skill_id: ELMOS-POLY-098
...
triggers:
- Use when implementing or executing `elmos-adapter-xbase`.
- Use when the migration DAG requires `technology-adapter` capability.
- Use when `xbase` is detected as source, target or repository support surface.
outputs:
- adapters/xbase/adapter-manifest.json
- adapters/xbase/semantic-mapping.yaml
- adapters/xbase/verification-profile.yaml
---
...
Implement xBase/FoxPro language parsing, semantic lowering, framework/runtime mapping, target emission and verification for repository-scale modernization.
...
- Technology ID: `xbase`
- Display: xBase/FoxPro
- Kind: language
- File/build markers: .prg, .scx, .vcx, .frx, .dbf, .cdx
- Build systems: Visual FoxPro/xBase toolchain
- Preferred analyzers: xBase parser, DBF/CDX metadata reader, VFP form/class parser
- Runtime/framework surface: DBF/CDX, VFP forms, OLE/COM
- Native verification: legacy runtime replay, DBF/index comparison, UI/report characterization
...
- macro evaluation.
- DBF index/collation behavior.
- form/report metadata.
