---
name: semantic-patch-and-evidence-ledger
description: 每次修改同时保存文本 diff、语义变化、执行者、验证和原因。 Use for this exact ELMOS Spring Golden Route v2 contract; imported specifications are not runtime or certification evidence.
...
metadata:
  ...
  source_id: FOUNDATION-10-semantic-patch-and-evidence-ledger
  source_name: semantic-patch-and-evidence-ledger
  source_path: skills/semantic-patch-and-evidence-ledger/SKILL.md
  source_sha256: sha256:8d1f0b27484072d84d70f66023f3c37678c696cad584bc457c10561696b6de54
  source_contract_sha256: sha256:59c480f719b46ea6f089583cff793d6bf9bade96c96fd11143190a28d8ce1fda
...
# Semantic Patch & Evidence Ledger

## Goal

每次修改同时保存文本 diff、语义变化、执行者、验证和原因。

## Use when

在 Elmos 需要实现或升级 **Semantic Patch & Evidence Ledger** 相关能力时使用；它不是仅供解释的提示词，而是实现、测试与验收契约。

## Capabilities to implement

- Recipe/Agent/Tool 都生成统一 Patch IR
- 记录 before/after symbol/type/config facts
- patch 可归因、可回滚、可比较
- 无 evidence 的修改不能进入完成态
...
- [ ] 定义 `semantic-patch-and-evidence-ledger` 的 public interface、input/output/error schemas。
- [ ] 建立 `semantic-patch-and-evidence-ledger` 的 capability/feature flags 与版本字段。
...
- [ ] 至少覆盖并验收：Recipe/Agent/Tool 都生成统一 Patch IR；以及 记录 before/after symbol/type/config facts。
...
- Machine contract: `references/contract.json` (`sha256:59c480f719b46ea6f089583cff793d6bf9bade96c96fd11143190a28d8ce1fda`).
