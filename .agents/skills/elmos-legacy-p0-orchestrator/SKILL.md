---
name: elmos-legacy-p0-orchestrator
description: Coordinate P0 legacy repository discovery, semantic lowering, target
  selection, conversion, coexistence, verification, evidence, and cutover.
version: 1.0.0
skill_id: ELMOS-POLY-065
layer: orchestration
risk: critical
readiness: not-run
dependencies:
...
triggers:
- Use when implementing or executing `elmos-legacy-p0-orchestrator`.
- Use when the migration DAG requires `orchestration` capability.
outputs:
- legacy-run.json
- legacy-stage-state.json
- legacy-decision-log.json
---

# Legacy P0 Orchestrator

## Objective

Coordinate P0 legacy repository discovery, semantic lowering, target selection, conversion, coexistence, verification, evidence, and cutover.
