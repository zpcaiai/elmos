...
# Documentation And Runbook Generator
## ELMOS runtime binding

- Invoke this repository Skill as `$pm-b39-documentation-and-runbook-generator`.
- Immutable source identity: `documentation-and-runbook-generator` in `precision-migration-b01-44` (B39).
- Runtime adapter: `skill-and-project-synthesis`; binding state: `UNAVAILABLE`.
- Resolve and plan with `python3 scripts/precision_migration/runtime.py plan --skill pm-b39-documentation-and-runbook-generator`.
...
生成 README、架构、API、操作、故障、恢复和发布 Runbook。

本 Skill 属于精密语言转换系统。它必须以**可执行证据**为准，不得用模型自评、源码相似度或“能编译”替代语义与行为验证。

## Use when

- 当前任务直接涉及 `documentation-and-runbook-generator` 所描述的能力。
...
2. 执行本 Skill 的核心职责：生成 README、架构、API、操作、故障、恢复和发布 Runbook。
3. 编译Skills与架构蓝图
4. 生成/复用全部构件
5. 构建并执行验收/安全/完整性门禁
6. 输出项目、Runbook和证据
...
```yaml
skill: documentation-and-runbook-generator
version: 1.0.0
