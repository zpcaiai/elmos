...
# Formal Runtime Evidence Merger
## ELMOS runtime binding

- Invoke this repository Skill as `$pm-b35-formal-runtime-evidence-merger`.
- Immutable source identity: `formal-runtime-evidence-merger` in `precision-migration-b01-44` (B35).
- Runtime adapter: `formal-and-advanced-verification`; binding state: `UNAVAILABLE`.
- Resolve and plan with `python3 scripts/precision_migration/runtime.py plan --skill pm-b35-formal-runtime-evidence-merger`.
...
合并形式证明覆盖与运行时验证证据，并显式标记假设和未建模部分。

本 Skill 属于精密语言转换系统。它必须以**可执行证据**为准，不得用模型自评、源码相似度或“能编译”替代语义与行为验证。

## Use when

- 当前任务直接涉及 `formal-runtime-evidence-merger` 所描述的能力。
...
1. 固定语义与定理陈述
2. 执行本 Skill 的核心职责：合并形式证明覆盖与运行时验证证据，并显式标记假设和未建模部分。
3. 生成或编码证明义务
4. 调用Lean/SMT/模型检查
5. 最小化反例并回写测试
6. 由可信内核/求解器签发结果
...
```yaml
skill: formal-runtime-evidence-merger
version: 1.0.0
