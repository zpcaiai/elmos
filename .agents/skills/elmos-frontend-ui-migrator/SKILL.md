---
name: elmos-frontend-ui-migrator
description: "Translate web UI structure, component behavior, state, routing, forms, styling, accessibility, assets, and browser contracts."
version: 1.0.0
skill_id: ELMOS-POLY-030
layer: ui-transformation
risk: critical
readiness: not-run
dependencies:
  - "elmos-deployment-runtime-migrator"
triggers:
  - "Use when implementing or executing `elmos-frontend-ui-migrator`."
  - "Use when the current DAG node requires the ui-transformation capability."
outputs:
  - "target-web-ui/"
  - "ui-component-map.json"
  - "state-route-map.json"
  - "visual-regression-plan.json"
---

# Frontend UI Migrator

## Objective

Translate web UI structure, component behavior, state, routing, forms, styling, accessibility, assets, and browser contracts.
...
1. Inventory pages, components, design tokens, state stores, routes, guards, forms, validation, data fetching, i18n, and assets.
2. Map source component and lifecycle semantics to target components and hooks.
3. Separate server, client, shared, and build-time execution where the target supports them.
4. Preserve keyboard, focus, screen-reader, semantic, and responsive behavior.
5. Translate state ownership, cache, optimistic updates, error and loading states.
6. Generate component, integration, accessibility, and end-to-end tests.
7. Use screenshot/golden comparison only with deterministic fixtures and tolerances.
8. Record unsupported browser or platform behavior.

## Implementation Contract

- Behavior, accessibility, state, and navigation outrank superficial visual similarity.
- Golden comparisons use deterministic fixtures and explicit tolerances.
- Server/client and browser/runtime boundaries remain explicit.
...
- [ ] Accessibility checks include keyboard and semantics..
- [ ] State and route deep-link fixtures pass..
- [ ] Visual tolerances are explicit..
