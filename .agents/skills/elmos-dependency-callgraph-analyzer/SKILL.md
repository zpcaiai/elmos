---
name: elmos-dependency-callgraph-analyzer
description: "Build package, symbol, call, data, reflection, dynamic-loading, and external-integration graphs used to scope conversion impact."
version: 1.0.0
skill_id: ELMOS-POLY-006
...
triggers:
  - "Use when implementing or executing `elmos-dependency-callgraph-analyzer`."
  - "Use when the current DAG node requires the semantic-analysis capability."
outputs:
  - "dependency-graph.json"
  - "call-graph.json"
  - "dynamic-edge-register.json"
  - "impact-radius.json"
---

# Dependency Callgraph Analyzer

## Objective

Build package, symbol, call, data, reflection, dynamic-loading, and external-integration graphs used to scope conversion impact.
...
1. Resolve modules, packages, types, functions, methods, fields, imports, inheritance, and implementation edges.
2. Add build-time, runtime, test, generated-code, native, and deployment dependencies.
3. Model reflection, dependency injection, service loaders, dynamic imports, plugin registries, and RPC edges as confidence-scored dynamic edges.
4. Detect cycles, high-centrality nodes, shared kernels, and unstable boundaries.
5. Compute change impact for candidate migration units.
6. Preserve source locations and parser confidence for every edge.
7. Compare static graphs with available runtime traces.
8. Emit migration-order constraints.

## Implementation Contract

- Compiler-grade semantic data is preferred; tolerant parsers are fallback evidence only.
- Every inferred edge or type carries a confidence and source location.
- Dynamic behavior is modeled, not silently ignored.
...
- [ ] Known reflection or dynamic import fixtures appear in the graph..
- [ ] Cycle detection is deterministic..
- [ ] Impact radius distinguishes proven and inferred edges..
- [ ] Unresolved symbols are counted and bounded..
