...
description: "Implement and verify Batch 43 Skill 1451 for security fix backport. Use when work requires \u5b89\u5168\u4fee\u590dBackport, typed evidence, negative and holdout validation, and a fail-closed certification decision."
---
...
1. Inspect the current implementation, runtime facts, policies, incidents, decisions, and prior evidence relevant to `b43-security-fix-backport`.
...
4. Implement the smallest production-shaped capability that exercises `安全修复Backport` without bypassing existing control planes.
...
- Capability-specific implementation, tests, run logs, evidence references, and residual-risk records for `b43-security-fix-backport`
