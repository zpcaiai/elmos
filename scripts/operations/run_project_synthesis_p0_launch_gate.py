...
``PASS``/``production_ready``/``certified`` field as authority.  Production
...
def evaluate_launch_gate(
    ...
    *,
    ...
    """Evaluate all nine mandatory gates and return a derived decision."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    if result.get("decision") == "LOCAL_CONTRACT_VALID" and args.test_mode:
        return 0
    return 0 if result.get("production_ready") is True and result.get("certified") is True else 2
