...
def engine_languages() -> tuple[str, ...]:
    """Read SUPPORTED_LANGUAGES without importing the engine.
    ...
    match = re.search(r"^SUPPORTED_LANGUAGES\s*[:=][^=]*=\s*\(([^)]*)\)", text, re.MULTILINE)
    if match is None:
        match = re.search(r"^SUPPORTED_LANGUAGES\s*=\s*\(([^)]*)\)", text, re.MULTILINE)
    require(match is not None, "ENGINE_SUPPORTED_LANGUAGES_NOT_FOUND")
