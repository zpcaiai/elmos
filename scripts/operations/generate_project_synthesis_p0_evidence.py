...
def _run(command: list[str], cwd: Path, *, timeout: int = 60) -> subprocess.CompletedProcess[str]:
    return subprocess.run(  # noqa: S603
        ...
        timeout=timeout,
        check=False,
        env={**os.environ, "UV_OFFLINE": "1", "UV_FROZEN": "1"},
    )
    ...
    return [
        ...
        (
            ...
            [
                ...
                str(engine),
                "pytest",
                str(engine / "tests" / "test_supply_chain.py"),
                str(engine / "tests" / "test_project_graphs.py"),
                str(engine / "tests" / "test_project_documentation.py"),
            ],
            engine,
            600,
        ),
        (
            "ruff",
            [uv, "run", "--offline", "--frozen", "--project", str(engine), "ruff", "check", "src", "tests"],
            engine,
            300,
        ),
        (
            "mypy",
            [uv, "run", "--offline", "--frozen", "--project", str(engine), "mypy"],
            engine,
            300,
        ),
    ]
