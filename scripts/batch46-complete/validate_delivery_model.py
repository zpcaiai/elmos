...
def validate(path):
 d=load(path)
 if d.get("gross_margin",0)<0.35 or d.get("repeatable_projects",0)<2 or d.get("unit_cost",-1)<=0: fail("delivery model not repeatable/profitable")
 return d
def main():validate(sys.argv[1]);print("delivery model ok")
