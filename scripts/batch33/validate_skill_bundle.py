...
def main() -> int:
    ...
    batch = [f for f in files if f.parent.name.startswith('b33-')]
    if not batch:
        errors.append(f'no Batch 33 skills found under {root}')
    for path in batch:
        text = path.read_text()
        match = re.match(r'^---\n(.*?)\n---\n', text, re.S)
        if not match:
    ...
    if len(batch) != EXPECTED:
        errors.append(f'expected {EXPECTED} Batch 33 skills, found {len(batch)}')
    if errors:
        print('\n'.join('ERROR: ' + error for error in errors), file=sys.stderr)
        return 1
    print(f'OK: {len(batch)} Batch 33 skills')
    return 0
