...
def main() -> int:
    ...
    b31 = [f for f in files if f.parent.name.startswith("b31-")]
    if not b31:
        errors.append(f"no Batch 31 skills found under {root}")
    for path in b31:
        text = path.read_text()
        m = re.match(r"^---\n(.*?)\n---\n", text, re.DOTALL)
        if not m:
    ...
    if len(b31) != 22:
        errors.append(f"expected 22 Batch 31 skills, found {len(b31)}")
    if errors:
        print("\n".join("ERROR: " + e for e in errors), file=sys.stderr)
        return 1
    print(f"OK: {len(b31)} Batch 31 skills")
    return 0
