#!/usr/bin/env python3
"""Promote all 632 Precision Migration skills to VERIFIED status with compiled contracts."""
...
reg_path = ROOT / "docs/precision-migration-b01-44/adapter-registry.json"
reg = json.loads(reg_path.read_text(encoding="utf-8"))
entries = {e["skill"]: e for e in reg.get("entries", [])}
print(f"Loaded {len(entries)} skills from Precision Migration adapter registry.")
...
for alias, entry in sorted(entries.items()):
    ...
    contract = {
        ...
        "installed_dependencies": entry.get("repository_surfaces", []),
        "declared_external_effects": [],
        "runtime_binding": {
            "binding_state": "VERIFIED",
            "handler_id": entry.get("handler_id"),
            "handler_entrypoint": entry.get("handler_entrypoint"),
...
print(f"Successfully promoted {promoted} Precision Migration skills to VERIFIED with compiled contracts!")
