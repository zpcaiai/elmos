...
def resolve_uri(uri: Any, roots: tuple[Path, ...]) -> Path:
    ...
    if parsed.scheme == "workspace":
        ...
        match = re.fullmatch(r"sha256/([0-9a-f]{64})", f"{parsed.netloc}{parsed.path}".lstrip("/"))
        if match is None:
        ...
        raise ValueError(f"unsupported artifact URI scheme: {parsed.scheme or 'relative'}")
...
def verify_content_reference(reference: Any, roots: tuple[Path, ...]) -> dict[str, Any]:
    ...
    try:
        ...
        if not stat.S_ISREG(observed_stat.st_mode):
            raise ValueError("artifact URI does not resolve to a regular file descriptor")
    ...
    if actual_size != expected_size:
        raise ValueError(f"content byte count mismatch: expected {expected_size}, observed {actual_size}")
    if actual_digest != expected_digest:
        raise ValueError(f"content digest mismatch: expected {expected_digest}, observed {actual_digest}")
    return {
    ...
    def from_bytes(cls, path: Path, store_bytes: bytes) -> "TrustStore":
        ...
        for index, item in enumerate(records):
            ...
            if not _within(key_path, (base,)) or not key_path.is_file():
                raise ValueError(f"trust store key {key_id} escapes the trust-store directory")
            ...
            keys[key_id] = TrustedKey(
                ...
                not_before=parse_instant(item.get("not_before"), f"keys[{index}].not_before"),
                not_after=parse_instant(item.get("not_after"), f"keys[{index}].not_after"),
            )
