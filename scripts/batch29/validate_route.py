...
def main() -> int:
    ...
    for d in REQUIRED_DIRS:
        if not (route / d).exists():
            errors.append(f"missing: {route / d}")
    try:
        manifest = load(route / "route.json")
        for k in REQUIRED_ROUTE:
            if k not in manifest:
                errors.append(f"route.json missing key: {k}")
        if manifest.get("status") not in ALLOWED_ROUTE_STATUS:
            errors.append("invalid route status")
        if manifest.get("source", {}).get("language") == manifest.get("target", {}).get("language"):
    ...
    try:
        ...
        for cap in support.get("capabilities", []):
            if cap.get("status") not in ALLOWED_CAP_STATUS:
                errors.append(f"invalid capability status: {cap.get('id')}")
            if cap.get("status") in {"certified", "supported"} and not cap.get("evidence_refs"):
                errors.append(f"{cap.get('status')} capability lacks evidence: {cap.get('id')}")
            if cap.get("status") in {"conditional", "blocked"} and not cap.get("reason"):
                errors.append(f"conditional/blocked capability lacks reason: {cap.get('id')}")
    except Exception as e:
        errors.append(str(e))
    for f in [
