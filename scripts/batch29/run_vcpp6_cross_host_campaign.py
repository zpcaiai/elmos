#!/usr/bin/env python3
"""Prepare, execute and verify digest-bound VC++6 cross-host route campaigns.

The non-VC++6 compiler and the proprietary Microsoft VC++6 SP6 compiler cannot share the exact
...
non-VCPP6 host, then the verifier compares both sides' typed observations.
...
from route_sets import VCPP6_EXACT_ROUTE_KEYS, split_route_key  # noqa: E402
...
def _compact_non_vcpp6_side(local_root: Path) -> None:
    ...
    for child in local_root.iterdir():
        ...
        if child.is_symlink() or child.is_file():
            ...
            raise RouteError("VCPP6_CAMPAIGN_LOCAL_ARTIFACT_UNSAFE")
...
def _binding(path: Path, root: Path) -> dict[str, object]:
    ...
    try:
        ...
        raise RouteError("VCPP6_CAMPAIGN_ARTIFACT_PATH_ESCAPE") from error
    metadata = path.lstat()
    if path.is_symlink() or not stat.S_ISREG(metadata.st_mode):
        raise RouteError("VCPP6_CAMPAIGN_ARTIFACT_UNSAFE")
...
def _read_bound(root: Path, binding: dict[str, Any]) -> bytes:
    ...
    if not isinstance(relative, str) or not relative or Path(relative).is_absolute():
        raise RouteError("VCPP6_CAMPAIGN_ARTIFACT_BINDING_INVALID")
    ...
    if observed != binding:
        raise RouteError("VCPP6_CAMPAIGN_ARTIFACT_DIGEST_MISMATCH")
...
def _fixture(fixtures: Path, corpus: str, language: str) -> tuple[Path, Path, str]:
    ...
    if not source.is_file() or not cases.is_file():
        raise RouteError(f"VCPP6_CAMPAIGN_FIXTURE_MISSING:{corpus}:{language}")
...
def _validate_route_key(route_key: str) -> tuple[Language, Language]:
    if route_key not in VCPP6_EXACT_ROUTE_KEYS:
        raise RouteError(f"VCPP6_CAMPAIGN_ROUTE_NOT_ALLOWED:{route_key}")
...
def prepare(repo: Path, route_key: str, output: Path) -> dict[str, Any]:
    ...
    if output.exists() or output.is_symlink():
        raise RouteError("VCPP6_CAMPAIGN_OUTPUT_ALREADY_EXISTS")
    ...
    for corpus in CORPORA:
        ...
        if not isinstance(cases, list) or not cases:
            raise RouteError(f"VCPP6_CAMPAIGN_CASES_INVALID:{corpus}")
        local_root = corpus_root / "non-vcpp6-side"
        ...
        _compact_non_vcpp6_side(local_root)
        artifacts = [
    ...
    manifest = {
        "schema_version": 1,
        "kind": "elmos.vcpp6-cross-host-campaign-request",
        "route_key": route_key,
        "source_language": source_language,
        "target_language": target_language,
        "corpora": corpus_records,
        "prepared_side_status": "PASSED_LOCAL",
        "windows_vcpp6_side_status": "NOT_RUN",
...
def _load_request(request_root: Path) -> dict[str, Any]:
    ...
    if request_path.is_symlink() or not request_path.is_file():
        raise RouteError("VCPP6_CAMPAIGN_REQUEST_MISSING")
    request = json.loads(request_path.read_text(encoding="utf-8"))
    if (
        not isinstance(request, dict)
        or request.get("kind") != "elmos.vcpp6-cross-host-campaign-request"
        or request.get("windows_vcpp6_side_status") != "NOT_RUN"
        ...
        raise RouteError("VCPP6_CAMPAIGN_REQUEST_INVALID")
    ...
    if (
        ...
        raise RouteError("VCPP6_CAMPAIGN_REQUEST_ROUTE_BINDING_INVALID")
    ...
    if not isinstance(corpora, list) or {
        ...
        raise RouteError("VCPP6_CAMPAIGN_REQUEST_CORPUS_SET_INVALID")
    expected_local_role = "target" if source_language == "vcpp6" else "source"
    for corpus in corpora:
        if (
            ...
            raise RouteError("VCPP6_CAMPAIGN_REQUEST_CORPUS_INVALID")
        for artifact in corpus["artifacts"]:
            if not isinstance(artifact, dict):
                raise RouteError("VCPP6_CAMPAIGN_REQUEST_ARTIFACT_SET_INVALID")
        ...
        if not isinstance(local_report_binding, dict):
            raise RouteError("VCPP6_CAMPAIGN_REQUEST_LOCAL_REPORT_MISSING")
        ...
        if (
            ...
            != (target_language if source_language == "vcpp6" else source_language)
        ):
            raise RouteError("VCPP6_CAMPAIGN_REQUEST_LOCAL_REPORT_INVALID")
    return request
...
def execute_windows(request_root: Path, output: Path) -> dict[str, Any]:
    ...
    for raw in request.get("corpora", []):
        ...
        if target_language == "vcpp6":
            target_binding = next(
                item
                for item in raw["artifacts"]
                if isinstance(item, dict) and str(item.get("path", "")).endswith("migrated.cpp")
            )
            emitted = EmittedFile("migrated.cpp", _read_bound(request_root, target_binding).decode())
            report = validate(
                emitted,
                "vcpp6",
                target_semantic.functions[0],
            ...
            source_binding = next(
                item
                for item in raw["artifacts"]
                if isinstance(item, dict) and str(item.get("path", "")).endswith(".cpp")
            )
        ...
        report_path = run_root / "vcpp6-side-report.json"
    ...
    receipt = {
        "schema_version": 1,
        "kind": "elmos.vcpp6-cross-host-windows-receipt",
        "route_key": request["route_key"],
        "request_sha256": "sha256:"
        + hashlib.sha256((request_root / "campaign-request.json").read_bytes()).hexdigest(),
        "runs": run_records,
        "windows_vcpp6_side_status": "PASSED_LOCAL",
...
def verify(request_root: Path, windows_root: Path, output: Path) -> dict[str, Any]:
    ...
    if receipt_path.is_symlink() or not receipt_path.is_file():
        raise RouteError("VCPP6_CAMPAIGN_WINDOWS_RECEIPT_MISSING")
    ...
    if (
        ...
        or receipt.get("kind") != "elmos.vcpp6-cross-host-windows-receipt"
        or receipt.get("route_key") != request.get("route_key")
        or receipt.get("request_sha256") != expected_request_sha256
        or receipt.get("windows_vcpp6_side_status") != "PASSED_LOCAL"
        ...
        raise RouteError("VCPP6_CAMPAIGN_WINDOWS_RECEIPT_INVALID")
    ...
    for run in receipt.get("runs", []):
        if not isinstance(run, dict) or run.get("status") != "PASSED_LOCAL":
            raise RouteError("VCPP6_CAMPAIGN_WINDOWS_RUN_INVALID")
        corpus = str(run.get("corpus", ""))
        local = local_by_corpus.get(corpus)
        if local is None:
            raise RouteError("VCPP6_CAMPAIGN_CORPUS_MISMATCH")
        ...
        if (
            ...
            raise RouteError("VCPP6_CAMPAIGN_WINDOWS_ARTIFACT_SET_INVALID")
        ...
        if (
            ...
            raise RouteError(f"VCPP6_CAMPAIGN_BEHAVIOR_MISMATCH:{corpus}")
        comparisons.append({"corpus": corpus, "status": "MATCH"})
    if set(item["corpus"] for item in comparisons) != set(CORPORA):
        raise RouteError("VCPP6_CAMPAIGN_CORPUS_SET_INCOMPLETE")
    result = {
        "schema_version": 1,
        "kind": "elmos.vcpp6-cross-host-verification",
        "route_key": request["route_key"],
...
def main() -> int:
    ...
    prepare_parser.add_argument("--route", required=True, choices=VCPP6_EXACT_ROUTE_KEYS)
    ...
    try:
        if args.command == "prepare":
            ...
            prepared: list[str] = []
            reused: list[str] = []
            for route_key in VCPP6_EXACT_ROUTE_KEYS:
                ...
                if route_output.exists():
                    ...
                    if existing.get("route_key") != route_key:
                        raise RouteError(
                            f"VCPP6_CAMPAIGN_RESUME_ROUTE_MISMATCH:{route_key}"
                        )
                    for corpus_name in CORPORA:
                        _compact_non_vcpp6_side(
                            route_output / corpus_name / "non-vcpp6-side"
                        )
            ...
            result = {
                "schema_version": 1,
                "kind": "elmos.vcpp6-cross-host-prepare-set-result",
