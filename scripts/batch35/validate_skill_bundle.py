...
EXPECTED=22
def main():
 root=Path(sys.argv[1] if len(sys.argv)>1 else '.agents/skills'); errors=[]; names={}; batch=sorted(p for p in root.glob('b35-*/SKILL.md'))
 if not batch: errors.append(f'no Batch 35 skills found under {root}')
 for path in batch:
 ...
 if len(batch)!=EXPECTED: errors.append(f'expected {EXPECTED} Batch 35 skills, found {len(batch)}')
 if errors:
  print('\n'.join('ERROR: '+e for e in errors),file=sys.stderr); return 1
 print(f'OK: {len(batch)} Batch 35 skills'); return 0
