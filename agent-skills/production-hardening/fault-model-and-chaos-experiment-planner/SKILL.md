---
name: fault-model-and-chaos-experiment-planner
description: "Use when Batch 10 must derive bounded fault hypotheses and non-production chaos experiments from service dependencies."
---

# Fault Model And Chaos Experiment Planner
...
2. Derive bounded fault hypotheses and non-production chaos experiments from service dependencies.
...
- Block on destructive production scope, unknown blast radius or missing recovery observation.
...
Produce fault models and experiments with blast radius, steady state, abort criteria and kill switch. Bind every object to the immutable artifact and authority evidence. The strongest possible Batch 10 claim is eligibility for progressive delivery; always keep `production_ready=false` and `eligible_for_cutover=false`.
