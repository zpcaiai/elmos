---
name: runtime-resource-profiler
description: "Use when Batch 10 must profile CPU, memory, allocation, GC, thread, connection and I/O behavior under load."
---
...
2. Profile CPU, memory, allocation, GC, thread, connection and I/O behavior under load.
...
- Block on unexplained leaks, unbounded growth or measurements not aligned to workload phases.
...
Produce time-aligned resource profiles and suspected bottleneck evidence. Bind every object to the immutable artifact and authority evidence. The strongest possible Batch 10 claim is eligibility for progressive delivery; always keep `production_ready=false` and `eligible_for_cutover=false`.
