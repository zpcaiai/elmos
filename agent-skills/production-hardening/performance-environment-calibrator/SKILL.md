---
name: performance-environment-calibrator
description: "Use when Batch 10 must calibrate the isolated performance environment, generator capacity and run variance."
---

# Performance Environment Calibrator
...
2. Calibrate the isolated performance environment, generator capacity and run variance.
...
- Block on production access, generator saturation, warmup contamination or excessive variance.
...
Produce calibration evidence with environment hash, noise signals and comparability decisions. Bind every object to the immutable artifact and authority evidence. The strongest possible Batch 10 claim is eligibility for progressive delivery; always keep `production_ready=false` and `eligible_for_cutover=false`.
