---
name: sast-secret-and-code-security-scanner
description: "Use when Batch 10 must run and interpret external SAST, secret and code-security scans for the immutable artifact."
---

# Sast Secret And Code Security Scanner
...
2. Run and interpret external SAST, secret and code-security scans for the immutable artifact.
...
- Block on secret leakage, critical findings, missing scanner provenance or Agent-only dismissal.
...
Produce deduplicated findings with severity, reachability, owner and raw scan evidence. Bind every object to the immutable artifact and authority evidence. The strongest possible Batch 10 claim is eligibility for progressive delivery; always keep `production_ready=false` and `eligible_for_cutover=false`.
