---
name: dependency-failure-injection-validator
description: "Use when Batch 10 must validate behavior under latency, timeout, error, disconnect and dependency unavailability."
---

# Dependency Failure Injection Validator
...
2. Validate behavior under latency, timeout, error, disconnect and dependency unavailability.
...
- Block on production dependency injection, corruption, cascade failure or incomplete recovery.
...
Produce per-dependency failure and recovery evidence. Bind every object to the immutable artifact and authority evidence. The strongest possible Batch 10 claim is eligibility for progressive delivery; always keep `production_ready=false` and `eligible_for_cutover=false`.
