---
name: timeout-retry-circuit-breaker-validator
description: "Use when Batch 10 must validate timeout budgets, retry bounds, jitter, idempotency and circuit-breaker transitions."
---

# Timeout Retry Circuit Breaker Validator
...
2. Validate timeout budgets, retry bounds, jitter, idempotency and circuit-breaker transitions.
...
- Block on retry storm, unbounded retry, duplicate side effect or stuck circuit.
...
Produce bounded retry and circuit state evidence under approved failures. Bind every object to the immutable artifact and authority evidence. The strongest possible Batch 10 claim is eligibility for progressive delivery; always keep `production_ready=false` and `eligible_for_cutover=false`.
