---
name: dynamic-api-and-application-security-validator
description: "Use when Batch 10 must validate runtime API and application security using an isolated DAST authority."
---

# Dynamic Api And Application Security Validator
...
2. Validate runtime API and application security using an isolated DAST authority.
...
- Block on production probing, unsafe payload scope, critical exploitable finding or incomplete coverage.
...
Produce reproducible endpoint findings, attack traces and remediation evidence. Bind every object to the immutable artifact and authority evidence. The strongest possible Batch 10 claim is eligibility for progressive delivery; always keep `production_ready=false` and `eligible_for_cutover=false`.
