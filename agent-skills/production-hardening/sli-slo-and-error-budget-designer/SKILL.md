---
name: sli-slo-and-error-budget-designer
description: "Use when Batch 10 must define calculable service-level indicators, objectives and error-budget burn policy."
---

# Sli Slo And Error Budget Designer
...
2. Define calculable service-level indicators, objectives and error-budget burn policy.
...
- Block on uncalculable SLI, missing critical coverage or objective disconnected from user impact.
...
Produce SLI formulas, SLO windows, burn-rate rules and evidence queries. Bind every object to the immutable artifact and authority evidence. The strongest possible Batch 10 claim is eligibility for progressive delivery; always keep `production_ready=false` and `eligible_for_cutover=false`.
