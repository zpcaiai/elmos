---
name: logs-metrics-and-traces-instrumentation-validator
description: "Use when Batch 10 must validate signal completeness, correctness, redaction and runtime overhead."
---

# Logs Metrics And Traces Instrumentation Validator
...
2. Validate signal completeness, correctness, redaction and runtime overhead.
...
- Block on sensitive telemetry, missing critical signal, high overhead or unbounded cardinality.
...
Produce coverage and overhead evidence for logs, metrics and traces. Bind every object to the immutable artifact and authority evidence. The strongest possible Batch 10 claim is eligibility for progressive delivery; always keep `production_ready=false` and `eligible_for_cutover=false`.
