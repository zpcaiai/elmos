---
name: threat-model-and-attack-surface-builder
description: "Use when Batch 10 must build service threat models from assets, trust boundaries, identities, data and exposed interfaces."
---

# Threat Model And Attack Surface Builder
...
2. Build service threat models from assets, trust boundaries, identities, data and exposed interfaces.
...
- Block on missing critical boundary, unowned threat or attack surface not tied to the tested artifact.
...
Produce versioned threats, abuse cases, mitigations and residual-risk ownership. Bind every object to the immutable artifact and authority evidence. The strongest possible Batch 10 claim is eligibility for progressive delivery; always keep `production_ready=false` and `eligible_for_cutover=false`.
