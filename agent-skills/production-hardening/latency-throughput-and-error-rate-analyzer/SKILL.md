---
name: latency-throughput-and-error-rate-analyzer
description: "Use when Batch 10 must analyze throughput, p50, p95, p99, timeout and error-rate evidence against explicit SLOs."
---

# Latency Throughput And Error Rate Analyzer
...
2. Analyze throughput, p50, p95, p99, timeout and error-rate evidence against explicit SLOs.
...
- Block on aggregates that hide tail failures, incomplete samples or threshold relaxation.
...
Produce per-scenario tail-latency, throughput and error decisions with raw evidence links. Bind every object to the immutable artifact and authority evidence. The strongest possible Batch 10 claim is eligibility for progressive delivery; always keep `production_ready=false` and `eligible_for_cutover=false`.
