---
name: telemetry-semantic-contract-builder
description: "Use when Batch 10 must define stable log, metric, trace and correlation semantics for critical service signals."
---

# Telemetry Semantic Contract Builder
...
2. Define stable log, metric, trace and correlation semantics for critical service signals.
...
- Block on ambiguous units, unbounded labels, sensitive fields or missing critical event semantics.
...
Produce versioned telemetry contracts with ownership, units, labels and redaction rules. Bind every object to the immutable artifact and authority evidence. The strongest possible Batch 10 claim is eligibility for progressive delivery; always keep `production_ready=false` and `eligible_for_cutover=false`.
