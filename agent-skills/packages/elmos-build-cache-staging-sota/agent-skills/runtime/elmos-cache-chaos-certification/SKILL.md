---
name: elmos-cache-chaos-certification
description: Prove crash safety, determinism, security, and operational readiness through replayable fault injection and signed production certification.
version: 1.1.0
...
# Chaos Testing and Production Certification

## Outcome

Prove crash safety, determinism, security, and operational readiness through replayable fault injection and signed production certification.
...
- Dependency skills have passed: **elmos-atomic-file-write-promotion, elmos-checkpoint-resume, elmos-cache-security-provenance, elmos-cache-observability-performance**.
...
- Fault injection is deterministic and records exact boundary and seed.
- Tests validate manifests, published trees, side effects, and external state, not only process exit.
- Certification is scoped to exact stage, schema, toolchain, rule pack, storage profile, platform, trust namespace, and evidence.
- Expired, revoked, forged, stale, or scope-mismatched certificates are rejected.
...
1. Inject process kill, reboot simulation, disk full, inode exhaustion, fsync failure, permission loss, partial write, clock skew, network partition, duplicate delivery, stale lease, corrupted blob, and remote inconsistency.
2. Exercise faults before and after reservation, temp creation, write, fsync, seal, CAS put, metadata commit, checkpoint attach, remote publish, and final tree switch.
3. Compare clean, cached, resumed, remote, and failure-injected output tree digests and behavior evidence.
4. Prove at-most-once side effect or correct compensation.
5. Issue a signed certificate linked to immutable evidence, with limitations, expiry, and revocation hooks.
6. Make every discovered failure path a permanent regression test.
