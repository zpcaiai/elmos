...
5. `docs/research/sota-cache-algorithm-matrix.md`
6. The selected skill and all dependency skills
...
- Never declare one policy universally optimal; replay equal-capacity SIEVE, S3-FIFO, W-TinyLFU, and cost/size-aware baselines on representative traces.
- Optimize avoided compute, model tokens, bytes, and critical-path time in addition to raw hit ratio.
- Keep learning and policy selection off the correctness path and provide an immediate fixed-policy fallback.
- Prefer exact DAG next-use information over speculative prefetch, and account for wasted prefetch bytes.
