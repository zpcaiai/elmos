---
name: elmos-cache-rollout-end-to-end
description: Integrate the subsystem into the complete ELMOS repository-to-converted-project flow and roll it out through staging-first, shadow, read-only, progressive shared write, and certified production reuse.
version: 1.2.0
...
# Rollout and End-to-End Integration

## Outcome

Integrate the subsystem into the complete ELMOS repository-to-converted-project flow and roll it out through staging-first, shadow, read-only, progressive shared write, and certified production reuse.
...
- Executing phase `P7-rollout` of the cache/staging capability DAG.
- Diagnosing correctness, recoverability, cache-hit, storage, or publication problems related to this capability.
- Dependency skills have passed: **elmos-cache-api-cli-contracts, elmos-incremental-conversion-dag, elmos-checkpoint-resume, elmos-generation-conflict-merge, elmos-native-build-cache-adapters, elmos-cache-retention-gc, elmos-cache-chaos-certification**.
...
- The authoritative flow is snapshot, discover, analyze, Semantic IR, plan, generate into staging, seal, build/test, certify, and atomically publish.
- Every skipped computation is justified by a compatible cache hit.
- Every final file is reachable from the sealed tree manifest.
- Interruption at any stage has an explicit recovery or bounded final-failure path.
- Rollout always has feature flags, kill switches, divergence detection, and rollback.
...
0. Integrate the certified adaptive policy orchestrator, DAG-aware prefetch, cost-aware admission, and fixed-policy fallbacks; do not enable learned write decisions before the SOTA certification skill passes.

1. Wire StageContracts into the existing ELMOS orchestrator, Skill Registry, and capability DAG.
2. Resolve cache candidates before scheduling, allocate isolated workspaces, execute misses, and stage every generated file.
3. Checkpoint expensive boundaries, resume failed nodes, merge safely, build/test/behavior-validate, repair, and revalidate.
4. Assemble tree manifest and evidence bundle, then publish atomically.
5. Roll out in staging-only, shadow-compare, local read, shared read, limited write, and production-certified phases.
6. Run local, distributed, offline, crash, security, scale, and representative language-pair acceptance suites.
