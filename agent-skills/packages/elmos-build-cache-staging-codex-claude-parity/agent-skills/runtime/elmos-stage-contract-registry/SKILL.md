---
name: elmos-stage-contract-registry
description: Require every ELMOS stage to declare input/output schemas, determinism, fingerprint dimensions, cache policy, workspace mounts, resources, side effects, and checkpoint semantics.
version: 1.2.0
...
Require every ELMOS stage to declare input/output schemas, determinism, fingerprint dimensions, cache policy, workspace mounts, resources, side effects, and checkpoint semantics.
...
- Dependency skills have passed: **elmos-cache-key-fingerprinting, elmos-intermediate-artifact-manifest**.
...
- A stage cannot execute unless its contract validates.
- Declared outputs are the only files eligible for sandbox export.
- Contract and schema versions participate in cache identity.
- Undeclared environment, filesystem, network, or side-effect dependencies fail hermeticity checks.
...
1. Define the StageContract schema and registry loader.
2. Register discovery, parse, normalize, semantic analysis, IR, mapping, generation, dependency conversion, compile, test, behavior validation, repair, and certification stages.
3. Validate producer/consumer schema compatibility and capability DAG edges.
4. Generate runtime guards, documentation, and key dimensions from the same authoritative contract.
5. Lint for broad invalidation, mutable aliases, undeclared paths, missing validation floors, and non-idempotent side effects.
