---
name: elmos-cache-key-fingerprinting
description: Build canonical, explainable cache keys that include every result-affecting input while excluding irrelevant machine-specific noise.
version: 1.0.0
...
# ActionKey and Stage Fingerprinting

## Outcome

Build canonical, explainable cache keys that include every result-affecting input while excluding irrelevant machine-specific noise.
...
- Dependency skills have passed: **elmos-project-snapshot-merkle, elmos-cache-system-architecture**.
...
- Canonical serialization is typed, sorted, Unicode-normalized, and stable.
- Only declared result-affecting environment values participate in the key.
- LLM-backed stages include immutable model snapshot, prompt template digest, seed, decoding settings, tool versions, and tool-output digests.
- Stage implementation, contract schema, target runtime, compiler flags, rule pack, and dependency locks are explicit dimensions.
...
1. Define StageFingerprintSpec with required, optional, normalized, secret, and excluded dimensions.
2. Canonicalize paths, arrays, maps, numeric values, compiler flags, feature flags, and environment values.
3. Use dependency public-interface digests instead of full implementation digests where correctness permits.
4. Persist a human-readable fingerprint document and exact miss-reason comparison.
5. Add differential tests proving which input changes must or must not alter the ActionKey.
