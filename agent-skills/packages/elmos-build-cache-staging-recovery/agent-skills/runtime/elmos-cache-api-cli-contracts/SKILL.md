---
name: elmos-cache-api-cli-contracts
description: Define stable APIs and commands for cache lookup, blob transfer, workspace lifecycle, staged-file transitions, checkpoints, run recovery, diagnostics, and administration.
version: 1.0.0
...
# Cache API and CLI Contracts

## Outcome

Define stable APIs and commands for cache lookup, blob transfer, workspace lifecycle, staged-file transitions, checkpoints, run recovery, diagnostics, and administration.
...
- Dependency skills have passed: **elmos-cache-system-architecture, elmos-cache-metadata-database**.
...
- Run and workspace mutations require expected version and/or lease epoch.
- Destructive GC or purge operations default to dry-run and emit an auditable plan.
...
1. Define OpenAPI operations for Action Cache lookup/commit, blob existence/upload/download, run creation/state, staged-file seal/promote, checkpoint commit, resume, and atomic publish.
2. Define CLI commands for cache status, inspect, explain-miss, verify, pin, unpin, gc, workspace list/recover, run resume, and doctor.
3. Create typed error codes for digest mismatch, stale lease, invalid transition, validation too low, quarantined entry, conflict, and quota breach.
4. Specify resumable chunk transfer, conditional requests, request tracing, and stable pagination.
5. Build contract tests against local in-memory and production adapters.
