---
name: elmos-content-addressable-storage
description: Implement immutable, deduplicated, integrity-checked storage for AST, Semantic IR, mapping plans, generated files, patches, build outputs, tests, checkpoints, logs, and evidence bundles.
version: 1.0.0
...
Implement immutable, deduplicated, integrity-checked storage for AST, Semantic IR, mapping plans, generated files, patches, build outputs, tests, checkpoints, logs, and evidence bundles.
...
- A canonical digest maps to one byte sequence.
- Writes remain invisible until streaming digest, size checks, fsync, and atomic commit complete.
- CAS objects are immutable; updates create new objects and references.
- Corrupt objects are rejected and quarantined before trusted use.
...
1. Implement sharded local CAS paths with streaming put/get, sidecar metadata, compression metadata, and optional chunk manifests.
2. Use create-if-absent semantics so concurrent identical writers converge without a global lock.
3. Support verified hardlink/reflink materialization with safe copy fallback.
4. Add scrub, corruption quarantine, remote replica repair, and storage accounting.
5. Measure restore cost and support bypass when recomputation is cheaper.
