...
description: "Run FRT-2012 Distributed Worker Runtime for FRT G20 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-2012"
  source_name: "frt-2012-distributed-worker-runtime"
  source_sha256: "sha256:2935a5f733e8f1620e0c29539b1254a791943005d2c1f13e09211485ddda5151"
  batch: "G20"
...
- Runtime catalog key: `FRT-2012` / `frt-2012-distributed-worker-runtime`.
...
# FRT-2012 — Distributed Worker Runtime

## Objective

在G20范围内实现 **Distributed Worker Runtime**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：提供Skill SDK、Runtime、Registry和Marketplace; 实现CLI、API、IDE、Web Console和Multi-Tenant Workspace; Durable Workflow与分布式Worker支持暂停、恢复、取消和幂等。

## When to Use

- 实现或修改与 **Distributed Worker Runtime** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-2012Result`：版本化、带状态和诊断的结构化结果。
- `FRT-2012EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-2012FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-2012CertificateFragment`：供G20批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g20/frt-2012-distributed-worker-runtime/
packages/runtime/g20/frt-2012-distributed-worker-runtime/
services/control-plane/g20/frt-2012-distributed-worker-runtime/
apps/web-console/src/features/g20/frt-2012-distributed-worker-runtime/
apps/admin-console/src/features/g20/frt-2012-distributed-worker-runtime/
tests/g20/frt-2012-distributed-worker-runtime/
```
...
```text
POST /v1/skills/frt-2012/runs
GET  /v1/skills/frt-2012/runs/{run_id}
POST /v1/skills/frt-2012/runs/{run_id}/verify
GET  /v1/skills/frt-2012/runs/{run_id}/findings
GET  /v1/skills/frt-2012/runs/{run_id}/evidence
```
