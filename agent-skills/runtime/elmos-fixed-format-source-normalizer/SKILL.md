---
name: elmos-fixed-format-source-normalizer
description: Normalize fixed-column, sequence-area, continuation, indicator, and card-image
  source forms into reversible logical source while preserving original coordinates.
version: 1.0.0
skill_id: ELMOS-POLY-068
layer: normalization
risk: high
readiness: not-run
dependencies:
- elmos-source-encoding-normalizer
triggers:
- Use when implementing or executing `elmos-fixed-format-source-normalizer`.
- Use when the migration DAG requires `normalization` capability.
outputs:
- fixed-format-map.json
- logical-source/
- roundtrip-map.json
---

# Fixed Format Source Normalizer

## Objective

Normalize fixed-column, sequence-area, continuation, indicator, and card-image source forms into reversible logical source while preserving original coordinates.
