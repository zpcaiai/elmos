...
description: "Run FRT-1916 Semantic Drift Detector for FRT G19 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-1916"
  source_name: "frt-1916-semantic-drift-detector"
  source_sha256: "sha256:40d035ad576cac155b52d4deb89a10d00bb834fb20ab999d9543859e54eebf2a"
  batch: "G19"
...
- Runtime catalog key: `FRT-1916` / `frt-1916-semantic-drift-detector`.
...
# FRT-1916 — Semantic Drift Detector

## Objective

在G19范围内实现 **Semantic Drift Detector**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：从IR、Rule和业务不变量生成Proof Obligation; Leanstral仅提出证明候选，Lean Kernel为可信根; 组合SMT、Model Checking、Symbolic和Differential Evidence。

## When to Use

- 实现或修改与 **Semantic Drift Detector** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-1916Result`：版本化、带状态和诊断的结构化结果。
- `FRT-1916EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-1916FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-1916CertificateFragment`：供G19批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g19/frt-1916-semantic-drift-detector/
packages/runtime/g19/frt-1916-semantic-drift-detector/
services/control-plane/g19/frt-1916-semantic-drift-detector/
apps/web-console/src/features/g19/frt-1916-semantic-drift-detector/
apps/admin-console/src/features/g19/frt-1916-semantic-drift-detector/
tests/g19/frt-1916-semantic-drift-detector/
```
...
```text
POST /v1/skills/frt-1916/runs
GET  /v1/skills/frt-1916/runs/{run_id}
POST /v1/skills/frt-1916/runs/{run_id}/verify
GET  /v1/skills/frt-1916/runs/{run_id}/findings
GET  /v1/skills/frt-1916/runs/{run_id}/evidence
```
