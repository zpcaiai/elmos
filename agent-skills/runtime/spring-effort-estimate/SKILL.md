---
name: spring-effort-estimate
description: 基于 ERSS、RCS、迁移面、Recipe 覆盖、历史数据和验证深度估算机器/人工工作。 Use for this exact ELMOS Spring Golden Route v2 contract; imported specifications are not runtime or certification evidence.
...
metadata:
  ...
  source_id: ELMOS-GR-030
  source_name: spring-effort-estimate
  source_path: skills/spring-effort-estimate/SKILL.md
  source_sha256: sha256:5abedbf710a7b3ea8076b708e0408d0775f0c88cadd23bcd17334d8666fe7c94
  source_contract_sha256: sha256:035d0f14956ee4635e1f06e29ef194a2cd86fec891d893dfc14b15aa70a24ae7
...
基于 ERSS、RCS、迁移面、Recipe 覆盖、历史数据和验证深度估算机器/人工工作。
...
- [ ] 定义 `spring-effort-estimate` 的 public interface、输入/输出/Error Schema 和版本策略。
- [ ] 实现 `spring-effort-estimate` 的 production adapter/service，不把业务状态只保存在 Prompt 或内存对象中。
...
- [ ] 至少生成：effort-estimate.json, assumptions.json。
...
- [ ] 历史失败与迁移回归边界冻结
- [ ] 报价、ETA、假设和风险进入 SOW
- [ ] 本 Skill 的 required outputs 均由真实目标仓库执行产生：effort-estimate.json, assumptions.json。
...
- Machine contract: `references/contract.json` (`sha256:035d0f14956ee4635e1f06e29ef194a2cd86fec891d893dfc14b15aa70a24ae7`).
