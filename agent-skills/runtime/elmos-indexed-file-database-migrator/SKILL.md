---
name: elmos-indexed-file-database-migrator
description: Migrate VSAM/DBF/DDS/indexed files to relational/document/KV stores while
  preserving keys, collation, duplicates, ordering and restart semantics.
version: 1.0.0
skill_id: ELMOS-POLY-110
...
triggers:
- Use when implementing or executing `elmos-indexed-file-database-migrator`.
- Use when the migration DAG requires `data-transformation` capability.
outputs:
- transforms/elmos-indexed-file-database-migrator/plan.json
- transforms/elmos-indexed-file-database-migrator/semantic-loss.json
- transforms/elmos-indexed-file-database-migrator/evidence.json
---

# Indexed File Database Migrator

## Objective

Migrate VSAM/DBF/DDS/indexed files to relational/document/KV stores while preserving keys, collation, duplicates, ordering and restart semantics.
