...
description: "Run FRT-1308 Web State Router and UI Mapping for FRT G13 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-1308"
  source_name: "frt-1308-web-state-router-and-ui-mapping"
  source_sha256: "sha256:40cac7b6ae274e88c6791199f7be26b88f9855b916ee08fb8491561ed675a6ad"
  batch: "G13"
  source_version: "1.0.0"
  source_risk: "critical-by-default"
  source_certificate_family: "WR"
...
- Runtime catalog key: `FRT-1308` / `frt-1308-web-state-router-and-ui-mapping`.
...
# FRT-1308 — Web State Router and UI Mapping

## Objective

在G13范围内实现 **Web State Router and UI Mapping**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：六个方向独立声明版本、能力和Known Differences; 处理Options/Composition/Hooks、Router、Store、SFC/JSX差异; 建立直接路径与三角间接路径差分验证。

## When to Use

- 实现或修改与 **Web State Router and UI Mapping** 对应的Contract、Runtime、API、UI、测试或证据链时。
- 相关代码、Schema、Pack、Policy、环境或依赖变化导致既有Evidence失效时。
- G13 Orchestrator生成该Skill的执行义务时。
...
- `SourceArtifactSet`：G1–G12核心能力、Web源/目标Profiles和Golden Repositories。
- `SkillPolicy`：权限、网络、Sandbox、Toolchain、Evidence和停止规则。

## Outputs

- `FRT-1308Result`：版本化、带状态和诊断的结构化结果。
- `FRT-1308EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-1308FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-1308CertificateFragment`：供G13批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g13/frt-1308-web-state-router-and-ui-mapping/
packages/runtime/g13/frt-1308-web-state-router-and-ui-mapping/
services/control-plane/g13/frt-1308-web-state-router-and-ui-mapping/
apps/web-console/src/features/g13/frt-1308-web-state-router-and-ui-mapping/
apps/admin-console/src/features/g13/frt-1308-web-state-router-and-ui-mapping/
tests/g13/frt-1308-web-state-router-and-ui-mapping/
```
...
```text
POST /v1/skills/frt-1308/runs
GET  /v1/skills/frt-1308/runs/{run_id}
POST /v1/skills/frt-1308/runs/{run_id}/verify
GET  /v1/skills/frt-1308/runs/{run_id}/findings
GET  /v1/skills/frt-1308/runs/{run_id}/evidence
```
...
- The parent G13 Orchestrator can consume the result and independently decide certification.
