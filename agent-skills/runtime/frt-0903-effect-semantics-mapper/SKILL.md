...
description: "Run FRT-0903 Effect Semantics Mapper for FRT G09 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-0903"
  source_name: "frt-0903-effect-semantics-mapper"
  source_sha256: "sha256:44d1ad7bcfedb59fcac6f877b77c37c73b3895d882cc4c0a49c4756c9450267f"
  batch: "G09"
...
- Runtime catalog key: `FRT-0903` / `frt-0903-effect-semantics-mapper`.
...
# FRT-0903 — Effect Semantics Mapper

## Objective

在G09范围内实现 **Effect Semantics Mapper**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：区分Local、Server、Global、Persistent和Derived State; 保持Effect触发、Cleanup、Lifecycle和Evaluation Order; 传播Cancellation并防止Stale Result Commit。

## When to Use

- 实现或修改与 **Effect Semantics Mapper** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-0903Result`：版本化、带状态和诊断的结构化结果。
- `FRT-0903EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-0903FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-0903CertificateFragment`：供G09批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g09/frt-0903-effect-semantics-mapper/
packages/runtime/g09/frt-0903-effect-semantics-mapper/
services/control-plane/g09/frt-0903-effect-semantics-mapper/
apps/web-console/src/features/g09/frt-0903-effect-semantics-mapper/
apps/admin-console/src/features/g09/frt-0903-effect-semantics-mapper/
tests/g09/frt-0903-effect-semantics-mapper/
```
...
```text
POST /v1/skills/frt-0903/runs
GET  /v1/skills/frt-0903/runs/{run_id}
POST /v1/skills/frt-0903/runs/{run_id}/verify
GET  /v1/skills/frt-0903/runs/{run_id}/findings
GET  /v1/skills/frt-0903/runs/{run_id}/evidence
```
