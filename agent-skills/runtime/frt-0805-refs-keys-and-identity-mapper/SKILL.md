...
description: "Run FRT-0805 Refs Keys and Identity Mapper for FRT G08 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-0805"
  source_name: "frt-0805-refs-keys-and-identity-mapper"
  source_sha256: "sha256:d92a539e09437305f781e53b4ca71851596eb889443e1bd791e5f818771197ff"
  batch: "G08"
...
- Runtime catalog key: `FRT-0805` / `frt-0805-refs-keys-and-identity-mapper`.
...
# FRT-0805 — Refs Keys and Identity Mapper

## Objective

在G08范围内实现 **Refs Keys and Identity Mapper**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：保持Props默认值、事件时机、Slot/Children/Builder结构; 保持Key、Ref、Widget Identity和状态保留语义; 正确降低Hooks、Composable、Context和局部状态。

## When to Use

- 实现或修改与 **Refs Keys and Identity Mapper** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-0805Result`：版本化、带状态和诊断的结构化结果。
- `FRT-0805EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-0805FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-0805CertificateFragment`：供G08批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g08/frt-0805-refs-keys-and-identity-mapper/
packages/runtime/g08/frt-0805-refs-keys-and-identity-mapper/
services/control-plane/g08/frt-0805-refs-keys-and-identity-mapper/
apps/web-console/src/features/g08/frt-0805-refs-keys-and-identity-mapper/
apps/admin-console/src/features/g08/frt-0805-refs-keys-and-identity-mapper/
tests/g08/frt-0805-refs-keys-and-identity-mapper/
```
...
```text
POST /v1/skills/frt-0805/runs
GET  /v1/skills/frt-0805/runs/{run_id}
POST /v1/skills/frt-0805/runs/{run_id}/verify
GET  /v1/skills/frt-0805/runs/{run_id}/findings
GET  /v1/skills/frt-0805/runs/{run_id}/evidence
```
