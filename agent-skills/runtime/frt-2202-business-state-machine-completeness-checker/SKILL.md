...
description: "Run FRT-2202 Business State Machine Completeness Checker for FRT G22 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-2202"
  source_name: "frt-2202-business-state-machine-completeness-checker"
  source_sha256: "sha256:7be58b2f6b76c1a1445ca00b24f6bd90e64402d088bfc3d85542ee4660a9055c"
  batch: "G22"
...
- Runtime catalog key: `FRT-2202` / `frt-2202-business-state-machine-completeness-checker`.
...
在G22范围内实现 **Business State Machine Completeness Checker**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：Value Stream 与业务Outcome建模; 状态机可达性、终态、Timeout、Cancel、Unknown、Recovery完整性; 跨域Saga、幂等、补偿和Reconciliation。

## When to Use

- 实现或修改与 **Business State Machine Completeness Checker** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-2202Result`：版本化、带状态和诊断的结构化结果。
- `FRT-2202EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-2202FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-2202CertificateFragment`：供G22批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g22/frt-2202-business-state-machine-completeness-checker/
packages/runtime/g22/frt-2202-business-state-machine-completeness-checker/
services/control-plane/g22/frt-2202-business-state-machine-completeness-checker/
apps/web-console/src/features/g22/frt-2202-business-state-machine-completeness-checker/
apps/admin-console/src/features/g22/frt-2202-business-state-machine-completeness-checker/
tests/g22/frt-2202-business-state-machine-completeness-checker/
```
...
```text
POST /v1/skills/frt-2202/runs
GET  /v1/skills/frt-2202/runs/{run_id}
POST /v1/skills/frt-2202/runs/{run_id}/verify
GET  /v1/skills/frt-2202/runs/{run_id}/findings
GET  /v1/skills/frt-2202/runs/{run_id}/evidence
```
