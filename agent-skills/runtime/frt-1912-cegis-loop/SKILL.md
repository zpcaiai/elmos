...
description: "Run FRT-1912 CEGIS Loop for FRT G19 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-1912"
  source_name: "frt-1912-cegis-loop"
  source_sha256: "sha256:2350bb9137ef7366484f8e9dac8df1284b41122f08325cabac264acc72fcacd9"
  batch: "G19"
...
- Runtime catalog key: `FRT-1912` / `frt-1912-cegis-loop`.
...
在G19范围内实现 **CEGIS Loop**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：从IR、Rule和业务不变量生成Proof Obligation; Leanstral仅提出证明候选，Lean Kernel为可信根; 组合SMT、Model Checking、Symbolic和Differential Evidence。

## When to Use

- 实现或修改与 **CEGIS Loop** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-1912Result`：版本化、带状态和诊断的结构化结果。
- `FRT-1912EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-1912FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-1912CertificateFragment`：供G19批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g19/frt-1912-cegis-loop/
packages/runtime/g19/frt-1912-cegis-loop/
services/control-plane/g19/frt-1912-cegis-loop/
apps/web-console/src/features/g19/frt-1912-cegis-loop/
apps/admin-console/src/features/g19/frt-1912-cegis-loop/
tests/g19/frt-1912-cegis-loop/
```
...
```text
POST /v1/skills/frt-1912/runs
GET  /v1/skills/frt-1912/runs/{run_id}
POST /v1/skills/frt-1912/runs/{run_id}/verify
GET  /v1/skills/frt-1912/runs/{run_id}/findings
GET  /v1/skills/frt-1912/runs/{run_id}/evidence
```
