---
name: b104-disaster-recovery-replay-certification
description: "Use when ELMOS must Prove backup, restore, regional recovery, workflow replay and external-effect reconciliation. Triggers: Disaster Recovery and Replay Certification, Real World Product Certification Pack, DR, backup restore, replay, RPO/RTO."
metadata:
  id: B104-S09
...
# B104-S09 — Disaster Recovery and Replay Certification

## Objective

Prove backup, restore, regional recovery, workflow replay and external-effect reconciliation.
...
Use this Skill when the approved task explicitly requires **Disaster Recovery and Replay Certification** or when the capability graph selects it as part of a minimal dependency closure.
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b104-disaster-recovery-replay-certification/SKILL.md --out /tmp/disaster_recovery_replay_certification.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B104-S09
```
...
- Reject or expose backup without restore.
- Reject or expose duplicate webhook after replay.
- Reject or expose key material unavailable.
