...
# Package State Converter
## ELMOS runtime binding

- Invoke this repository Skill as `$pm-b21-package-state-converter`.
- Immutable source identity: `package-state-converter` in `precision-migration-b01-44` (B21).
- Runtime adapter: `database-and-data-route`; binding state: `UNAVAILABLE`.
- Resolve and plan with `python3 scripts/precision_migration/runtime.py plan --skill pm-b21-package-state-converter`.
...
将 Package 公私有成员、会话状态和初始化转换为 Schema、模块或服务状态。

本 Skill 属于精密语言转换系统。它必须以**可执行证据**为准，不得用模型自评、源码相似度或“能编译”替代语义与行为验证。

## Use when

- 当前任务直接涉及 `package-state-converter` 所描述的能力。
...
1. 盘点对象与专有能力
2. 执行本 Skill 的核心职责：将 Package 公私有成员、会话状态和初始化转换为 Schema、模块或服务状态。
