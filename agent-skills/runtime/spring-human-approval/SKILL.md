---
name: spring-human-approval
description: 对架构、许可、未知语义和高风险副作用实施结构化审批、拒绝和修正。 Use for this exact ELMOS Spring Golden Route v2 contract; imported specifications are not runtime or certification evidence.
...
metadata:
  ...
  source_id: ELMOS-GR-055
  source_name: spring-human-approval
  source_path: skills/spring-human-approval/SKILL.md
  source_sha256: sha256:6b883da9747ace9f1383e5466d75fbaec9b16a1f9830a576b4f1fc406767f814
  source_contract_sha256: sha256:a097c2682e7b4f41615f1a5015515bb09b51ce81f3483ff635edf9be42b86f66
...
对架构、许可、未知语义和高风险副作用实施结构化审批、拒绝和修正。
...
- [ ] 定义 `spring-human-approval` 的 public interface、输入/输出/Error Schema 和版本策略。
- [ ] 实现 `spring-human-approval` 的 production adapter/service，不把业务状态只保存在 Prompt 或内存对象中。
...
- [ ] 至少生成：approval-events, decision.json, handoff-package。
...
- [ ] 本 Skill 的 required outputs 均由真实目标仓库执行产生：approval-events, decision.json, handoff-package。
- [ ] 所有 mandatory tests 和 security gates 通过，且无 critical unknown。

## Dependencies

- `spring-crash-recovery`
...
- Machine contract: `references/contract.json` (`sha256:a097c2682e7b4f41615f1a5015515bb09b51ce81f3483ff635edf9be42b86f66`).
