---
name: evidence-analytics-assurance-cockpit-orchestrator
description: 端到端编排Semantic Layer、Data Quality、Completeness、Risk、Control、Audit、Freshness、Trend、Portfolio、Cockpit、Alert与Forecast。
metadata:
  elmos_batch: "37C"
  elmos_skill_id: "ANA37C001"
  pack_version: "3.7-complete"
---

# ANA37C001 — Evidence Analytics 与 Assurance Cockpit 总编排

## Objective

端到端编排Semantic Layer、Data Quality、Completeness、Risk、Control、Audit、Freshness、Trend、Portfolio、Cockpit、Alert与Forecast。
...
- 生成 docs/assurance/batch-37c-baseline.md 并盘点隐藏公式、平均百分比、Null-to-zero与直接事务表查询。
- 按 Semantic→Quality→Completeness/Risk/Control→Audit/Freshness→Trend/Portfolio→Cockpit/Action→Forecast→Gate 实施。
- 所有高管指标必须使用Certified Source和Immutable Metric Snapshot。
- 最终报告列出数据、统计、控制映射与预测限制。
...
- orchestratorRequiresBatch37AAndB
- dashboardCannotBypassMetricRegistry
- criticalFailuresRemainVisible
- allAnalyticsGatesAreReferenced
