---
name: software-supply-chain-risk-exposure-concentration-and-b-c5de6a01
description: 计算组件、漏洞、Builder、Signer、Provider、依赖集中度、可达性、暴露期与影响范围。
metadata:
  elmos_batch: "37C"
  elmos_skill_id: "ANA37C005"
  pack_version: "3.7-complete"
---

# ANA37C005 — Software Supply-chain Risk、Exposure、Concentration 与 Blast Radius

## Objective

计算组件、漏洞、Builder、Signer、Provider、依赖集中度、可达性、暴露期与影响范围。
...
- 风险维度包括Severity、Exploitability、Reachability、Business Criticality、Trust、Depth、Concentration、Age、Confidence。
- Unknown Reachability与Missing SBOM降低Confidence，不转为零风险。
- 计算Portfolio对相同Component/Builder/Signer/SCM/CI/Runner Release的依赖集中度。
- 通过Evidence/Dependency Graph执行Revoke/Compromise/Vulnerability Blast Radius。
...
- cvssScoresAreNotSummedAsPortfolioRisk
- unknownReachabilityRemainsUnknown
- revokedSignerBlastRadiusFindsAllDependents
- riskAcceptanceRequiresOwnerAndExpiry
