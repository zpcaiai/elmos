---
name: pm-b27-native-query-rewriter
description: "重写原生查询、Hint、锁、分页、函数、JSON、日期和返回类型. Precision Migration B27 contract; use for this exact assessment, transformation, validation, repair, evidence, or cutover scope."
---

# Native Query Rewriter
## ELMOS runtime binding

- Invoke this repository Skill as `$pm-b27-native-query-rewriter`.
- Immutable source identity: `native-query-rewriter` in `precision-migration-b01-44` (B27).
- Runtime adapter: `database-and-data-route`; binding state: `UNAVAILABLE`.
- Resolve and plan with `python3 scripts/precision_migration/runtime.py plan --skill pm-b27-native-query-rewriter`.
...
- Phase: `G 数据库精密互转`
- Version: `1.0.0`
- Tags: application-database-joint-migration, batch-27

## Purpose

重写原生查询、Hint、锁、分页、函数、JSON、日期和返回类型。

本 Skill 属于精密语言转换系统。它必须以**可执行证据**为准，不得用模型自评、源码相似度或“能编译”替代语义与行为验证。

## Use when

- 当前任务直接涉及 `native-query-rewriter` 所描述的能力。
...
1. 盘点对象与专有能力
2. 执行本 Skill 的核心职责：重写原生查询、Hint、锁、分页、函数、JSON、日期和返回类型。
