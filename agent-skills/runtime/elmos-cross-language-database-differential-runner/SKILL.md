---
name: elmos-cross-language-database-differential-runner
description: Run joint source/target application and source/target database scenarios to detect interaction defects that isolated code or SQL tests miss.
version: 4.0.0
...
# Cross-Language and Database Differential Runner
...
- **Kernel authorities used:** K3, K6, K7
...
- Orchestrate four-way source/target application/database runs
- Normalize protocol and database observations
- Compare transaction and tool side effects end to end
- Attribute mismatch to code, ORM, SQL or engine
- Persist replayable counterexamples
...
- `cross-stack-differential-results.json`
- `interaction-mismatch-graph.json`
- `cross-stack-traces/`
- `cross-stack-counterexamples/`
...
- **environment combinations mislabeled** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **crossed run writes shared state** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **ORM cache masks database mismatch** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **trace correlation lost** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **source baseline changes during run** — must have a preventive control, a detective signal, a negative fixture and a response action.
...
- source-app/source-db baseline
- target-app/target-db candidate
- crossed compatibility combinations
- transaction rollback interaction
- concurrent request/data race
- CDC and application cutover
