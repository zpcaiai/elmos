---
name: b99-workflow-versioning-replay
description: "Use when ELMOS must Support deterministic workflow code evolution, replay and migration for in-flight runs. Triggers: Workflow Versioning and Replay, Durable Runtime Kernel Pack, workflow versions, replay, migration markers, compatibility."
metadata:
  id: B99-S10
...
# B99-S10 — Workflow Versioning and Replay

## Objective

Support deterministic workflow code evolution, replay and migration for in-flight runs.
...
Use this Skill when the approved task explicitly requires **Workflow Versioning and Replay** or when the capability graph selects it as part of a minimal dependency closure.
...
- version markers immutable.
- unsupported migration blocks.
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b99-workflow-versioning-replay/SKILL.md --out /tmp/workflow_versioning_replay.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B99-S10
```
...
- Reject or expose nondeterministic replay.
- Reject or expose version marker deletion.
- Reject or expose in-flight schema break.
