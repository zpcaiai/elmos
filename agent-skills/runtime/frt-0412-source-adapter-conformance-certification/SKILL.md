...
description: "Run FRT-0412 Source Adapter Conformance Certification for FRT G04 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-0412"
  source_name: "frt-0412-source-adapter-conformance-certification"
  source_sha256: "sha256:2e6a1cb37e1659f44e6cc5ed10fd5cf72b0f814d927ef448346fb7c2304a4360"
  batch: "G04"
...
- Runtime catalog key: `FRT-0412` / `frt-0412-source-adapter-conformance-certification`.
...
# FRT-0412 — Source Adapter Conformance Certification

## Objective

在G04范围内实现 **Source Adapter Conformance Certification**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：每个Adapter只解析源，不生成目标代码; 恢复组件、状态、事件、生命周期、路由、样式和平台能力; 统一动态语法、宏、装饰器和模板表达式。

## When to Use

- 实现或修改与 **Source Adapter Conformance Certification** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-0412Result`：版本化、带状态和诊断的结构化结果。
- `FRT-0412EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-0412FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-0412CertificateFragment`：供G04批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g04/frt-0412-source-adapter-conformance-certification/
packages/runtime/g04/frt-0412-source-adapter-conformance-certification/
services/control-plane/g04/frt-0412-source-adapter-conformance-certification/
apps/web-console/src/features/g04/frt-0412-source-adapter-conformance-certification/
apps/admin-console/src/features/g04/frt-0412-source-adapter-conformance-certification/
tests/g04/frt-0412-source-adapter-conformance-certification/
```
...
```text
POST /v1/skills/frt-0412/runs
GET  /v1/skills/frt-0412/runs/{run_id}
POST /v1/skills/frt-0412/runs/{run_id}/verify
GET  /v1/skills/frt-0412/runs/{run_id}/findings
GET  /v1/skills/frt-0412/runs/{run_id}/evidence
```
