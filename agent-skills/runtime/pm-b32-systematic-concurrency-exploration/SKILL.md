...
- Invoke this repository Skill as `$pm-b32-systematic-concurrency-exploration`.
- Immutable source identity: `systematic-concurrency-exploration` in `precision-migration-b01-44` (B32).
- Runtime adapter: `differential-test-and-repair`; binding state: `DECLARED`.
- Resolve and plan with `python3 scripts/precision_migration/runtime.py plan --skill pm-b32-systematic-concurrency-exploration`.
...
接管调度点并探索关键线程、任务、Future、goroutine 和消息交错。

本 Skill 属于精密语言转换系统。它必须以**可执行证据**为准，不得用模型自评、源码相似度或“能编译”替代语义与行为验证。

## Use when

- 当前任务直接涉及 `systematic-concurrency-exploration` 所描述的能力。
...
1. 评估或生成测试
2. 执行本 Skill 的核心职责：接管调度点并探索关键线程、任务、Future、goroutine 和消息交错。
3. 同步环境并双运行
4. 比较输出、状态和副作用
5. 搜索最小反例
6. 驱动修复并执行全量回归
...
```yaml
skill: systematic-concurrency-exploration
version: 1.0.0
