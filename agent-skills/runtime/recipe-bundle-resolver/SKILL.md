---
name: recipe-bundle-resolver
description: 通过两阶段 Resolver→Reader 懒加载 Maven/NPM/YAML 等 Recipe bundle。 Use for this exact ELMOS Spring Golden Route v2 contract; imported specifications are not runtime or certification evidence.
...
metadata:
  ...
  source_id: FOUNDATION-03-recipe-bundle-resolver
  source_name: recipe-bundle-resolver
  source_path: skills/recipe-bundle-resolver/SKILL.md
  source_sha256: sha256:c7f0f11c252c99149d82fa574a3685b11a0208887dd4da227e9ac680385a5e74
  source_contract_sha256: sha256:cd678a2da9e21ab646dca0b86f90b2a3c14d27976f311e2ae65157b4a31f2cd8
...
# Recipe Bundle Resolver

## Goal

通过两阶段 Resolver→Reader 懒加载 Maven/NPM/YAML 等 Recipe bundle。

## Use when

在 Elmos 需要实现或升级 **Recipe Bundle Resolver** 相关能力时使用；它不是仅供解释的提示词，而是实现、测试与验收契约。

## Capabilities to implement

- 核心只依赖 ecosystem resolver 接口
- 先 resolve bundle，再 describe/prepare Recipe
- 仅在实际执行时加载实现，降低启动和上下文成本
- 新生态通过插件增加而不修改核心
...
- [ ] 定义 `recipe-bundle-resolver` 的 public interface、input/output/error schemas。
- [ ] 建立 `recipe-bundle-resolver` 的 capability/feature flags 与版本字段。
...
- [ ] 至少覆盖并验收：核心只依赖 ecosystem resolver 接口；以及 先 resolve bundle，再 describe/prepare Recipe。
...
- Machine contract: `references/contract.json` (`sha256:cd678a2da9e21ab646dca0b86f90b2a3c14d27976f311e2ae65157b4a31f2cd8`).
