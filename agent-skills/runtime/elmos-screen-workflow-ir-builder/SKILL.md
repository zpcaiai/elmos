---
name: elmos-screen-workflow-ir-builder
description: Recover terminal, desktop and 4GL screen navigation, state, validation,
  field protection, events, menu/action flows and user-visible error behavior.
version: 1.0.0
skill_id: ELMOS-POLY-075
layer: ir
risk: critical
readiness: not-run
dependencies:
- elmos-legacy-dependency-runtime-graph
triggers:
- Use when implementing or executing `elmos-screen-workflow-ir-builder`.
- Use when the migration DAG requires `ir` capability.
outputs:
- screen-workflow-ir.json
- ui-state-machine.json
---

# Screen Workflow Ir Builder

## Objective

Recover terminal, desktop and 4GL screen navigation, state, validation, field protection, events, menu/action flows and user-visible error behavior.
