...
description: "Run FRT-1608 Flutter Widget State and Navigation Mapper for FRT G16 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-1608"
  source_name: "frt-1608-flutter-widget-state-and-navigation-mapper"
  source_sha256: "sha256:3887f06a43155a01f37780649775a66c8b03436263d6083efc8310461d967276"
  batch: "G16"
  source_version: "1.0.0"
  source_risk: "critical-by-default"
  source_certificate_family: "WF"
...
- Runtime catalog key: `FRT-1608` / `frt-1608-flutter-widget-state-and-navigation-mapper`.
...
# FRT-1608 — Flutter Widget State and Navigation Mapper

## Objective

在G16范围内实现 **Flutter Widget State and Navigation Mapper**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：映射DOM/组件到Widget树而非逐元素复制; 处理Key、BuildContext、Controller、State和Navigator; 处理Plugin、Platform Channel、Web/Mobile能力差异。

## When to Use

- 实现或修改与 **Flutter Widget State and Navigation Mapper** 对应的Contract、Runtime、API、UI、测试或证据链时。
- 相关代码、Schema、Pack、Policy、环境或依赖变化导致既有Evidence失效时。
- G16 Orchestrator生成该Skill的执行义务时。
...
- `SourceArtifactSet`：G1–G12、Flutter/Web Profiles和多端设备Fixture。
- `SkillPolicy`：权限、网络、Sandbox、Toolchain、Evidence和停止规则。

## Outputs

- `FRT-1608Result`：版本化、带状态和诊断的结构化结果。
- `FRT-1608EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-1608FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-1608CertificateFragment`：供G16批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g16/frt-1608-flutter-widget-state-and-navigation-mapper/
packages/runtime/g16/frt-1608-flutter-widget-state-and-navigation-mapper/
services/control-plane/g16/frt-1608-flutter-widget-state-and-navigation-mapper/
apps/web-console/src/features/g16/frt-1608-flutter-widget-state-and-navigation-mapper/
apps/admin-console/src/features/g16/frt-1608-flutter-widget-state-and-navigation-mapper/
tests/g16/frt-1608-flutter-widget-state-and-navigation-mapper/
```
...
```text
POST /v1/skills/frt-1608/runs
GET  /v1/skills/frt-1608/runs/{run_id}
POST /v1/skills/frt-1608/runs/{run_id}/verify
GET  /v1/skills/frt-1608/runs/{run_id}/findings
GET  /v1/skills/frt-1608/runs/{run_id}/evidence
```
...
- The parent G16 Orchestrator can consume the result and independently decide certification.
