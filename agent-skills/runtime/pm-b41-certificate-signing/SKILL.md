...
- Invoke this repository Skill as `$pm-b41-certificate-signing`.
- Immutable source identity: `certificate-signing` in `precision-migration-b01-44` (B41).
- Runtime adapter: `evidence-release-gate`; binding state: `DECLARED`.
- Resolve and plan with `python3 scripts/precision_migration/runtime.py plan --skill pm-b41-certificate-signing`.
...
1. 汇总证据与未解决项
2. 执行本 Skill 的核心职责：对证据、产物、规则、环境和审批生成不可篡改签名。
3. 执行硬性发布门禁
4. 影子/Canary/渐进切换
5. 监控并自动回滚
6. 沉淀反例、规则和企业交付能力
