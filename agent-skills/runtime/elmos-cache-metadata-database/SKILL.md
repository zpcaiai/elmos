---
name: elmos-cache-metadata-database
description: Implement transactional metadata for projects, snapshots, runs, nodes, artifacts, Action Cache entries, staged files, checkpoints, leases, references, events, and outbox jobs.
version: 1.2.0
...
Implement transactional metadata for projects, snapshots, runs, nodes, artifacts, Action Cache entries, staged files, checkpoints, leases, references, events, and outbox jobs.
...
- Large artifact bytes are not stored in relational rows.
- All mutable rows are tenant/project scoped and use optimistic versioning or compare-and-swap transitions.
- A stale worker cannot commit after its lease epoch changes.
- Artifact deletion is reference-aware and never inferred only from one run row.
...
1. Create PostgreSQL migrations and a SQLite-compatible local schema for all authoritative entities.
2. Add unique idempotency constraints for ActionKeys, staged logical paths, checkpoints, event sequence numbers, and external side-effect receipts.
3. Implement repository interfaces and legal state-transition guards.
4. Add outbox records for remote uploads, GC, audit propagation, and recovery scheduling.
5. Benchmark indexes for high-cardinality ActionKeys, staged-file recovery, run history, and artifact graph traversal.
