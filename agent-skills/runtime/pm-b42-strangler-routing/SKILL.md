---
name: pm-b42-strangler-routing
description: "在网关、服务、消息或前端路由层按能力分流旧实现和新实现. Precision Migration B42 contract; use for this exact assessment, transformation, validation, repair, evidence, or cutover scope."
---

# Strangler Routing
## ELMOS runtime binding

- Invoke this repository Skill as `$pm-b42-strangler-routing`.
- Immutable source identity: `strangler-routing` in `precision-migration-b01-44` (B42).
- Runtime adapter: `shadow-canary-cutover`; binding state: `DECLARED`.
- Resolve and plan with `python3 scripts/precision_migration/runtime.py plan --skill pm-b42-strangler-routing`.
...
1. 汇总证据与未解决项
2. 执行本 Skill 的核心职责：在网关、服务、消息或前端路由层按能力分流旧实现和新实现。
3. 执行硬性发布门禁
4. 影子/Canary/渐进切换
5. 监控并自动回滚
6. 沉淀反例、规则和企业交付能力
