---
name: pm-b24-mysql-auto-increment
description: "迁移 AUTO_INCREMENT、步长、偏移、锁模式和复制行为. Precision Migration B24 contract; use for this exact assessment, transformation, validation, repair, evidence, or cutover scope."
---

# Mysql Auto Increment
## ELMOS runtime binding

- Invoke this repository Skill as `$pm-b24-mysql-auto-increment`.
- Immutable source identity: `mysql-auto-increment` in `precision-migration-b01-44` (B24).
- Runtime adapter: `database-and-data-route`; binding state: `DECLARED`.
- Resolve and plan with `python3 scripts/precision_migration/runtime.py plan --skill pm-b24-mysql-auto-increment`.
...
迁移 AUTO_INCREMENT、步长、偏移、锁模式和复制行为。

本 Skill 属于精密语言转换系统。它必须以**可执行证据**为准，不得用模型自评、源码相似度或“能编译”替代语义与行为验证。

## Use when

- 当前任务直接涉及 `mysql-auto-increment` 所描述的能力。
...
1. 盘点对象与专有能力
2. 执行本 Skill 的核心职责：迁移 AUTO_INCREMENT、步长、偏移、锁模式和复制行为。
