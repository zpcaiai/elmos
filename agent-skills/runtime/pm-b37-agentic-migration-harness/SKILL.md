...
- Invoke this repository Skill as `$pm-b37-agentic-migration-harness`.
- Immutable source identity: `batch-37-agentic-migration-harness` in `precision-migration-b01-44` (B37).
- Runtime adapter: `model-routing-and-agent-harness`; binding state: `DECLARED`.
- Resolve and plan with `python3 scripts/precision_migration/runtime.py plan --skill pm-b37-agentic-migration-harness`.
...
- 当任务涉及 **migration-task-decomposer** 时，调用 `../pm-b37-migration-task-decomposer/SKILL.md`。
- 当任务涉及 **semantic-slice-task-planner** 时，调用 `../pm-b37-semantic-slice-task-planner/SKILL.md`。
- 当任务涉及 **multi-agent-worktree-manager** 时，调用 `../pm-b37-multi-agent-worktree-manager/SKILL.md`。
- 当任务涉及 **build-repair-agent** 时，调用 `../pm-b37-build-repair-agent/SKILL.md`。
- 当任务涉及 **test-migration-agent** 时，调用 `../pm-b37-test-migration-agent/SKILL.md`。
- 当任务涉及 **difference-diagnosis-agent** 时，调用 `../pm-b37-difference-diagnosis-agent/SKILL.md`。
- 当任务涉及 **proof-agent** 时，调用 `../pm-b37-proof-agent/SKILL.md`。
- 当任务涉及 **security-review-agent** 时，调用 `../pm-b37-security-review-agent/SKILL.md`。
- 当任务涉及 **human-approval-gate** 时，调用 `../pm-b37-human-approval-gate/SKILL.md`。
- 当任务涉及 **pause-resume-cancel** 时，调用 `../pm-b37-pause-resume-cancel/SKILL.md`。
- 当任务涉及 **retry-and-idempotency** 时，调用 `../pm-b37-retry-and-idempotency/SKILL.md`。
- 当任务涉及 **token-and-resource-governance** 时，调用 `../pm-b37-token-and-resource-governance/SKILL.md`。
