...
description: "Run FRT-2804 Graceful Degradation Runtime for FRT G28 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-2804"
  source_name: "frt-2804-graceful-degradation-runtime"
  source_sha256: "sha256:a968b1549d43b8e9f6b6ac825a06da197b4363e625a624469ba9179b177395b6"
  batch: "G28"
...
- Runtime catalog key: `FRT-2804` / `frt-2804-graceful-degradation-runtime`.
...
在G28范围内实现 **Graceful Degradation Runtime**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：Dependency Failure Model与Common Cause分析; 端到端Deadline、Retry Budget、Circuit Breaker、Bulkhead; Queue Durability、Replay、Stateful Failover、Fence Token。

## When to Use

- 实现或修改与 **Graceful Degradation Runtime** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-2804Result`：版本化、带状态和诊断的结构化结果。
- `FRT-2804EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-2804FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-2804CertificateFragment`：供G28批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g28/frt-2804-graceful-degradation-runtime/
packages/runtime/g28/frt-2804-graceful-degradation-runtime/
services/control-plane/g28/frt-2804-graceful-degradation-runtime/
apps/web-console/src/features/g28/frt-2804-graceful-degradation-runtime/
apps/admin-console/src/features/g28/frt-2804-graceful-degradation-runtime/
tests/g28/frt-2804-graceful-degradation-runtime/
```
...
```text
POST /v1/skills/frt-2804/runs
GET  /v1/skills/frt-2804/runs/{run_id}
POST /v1/skills/frt-2804/runs/{run_id}/verify
GET  /v1/skills/frt-2804/runs/{run_id}/findings
GET  /v1/skills/frt-2804/runs/{run_id}/evidence
```
