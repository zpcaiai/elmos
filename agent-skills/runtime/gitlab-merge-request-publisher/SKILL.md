---
name: gitlab-merge-request-publisher
description: Plan and publish an idempotent GitLab Draft merge request for an evidence-ready delivery. Use only with authorized GitLab credentials.
---
...
Require project authorization, tier/capability detection, base/head SHA, sanitized branch, report, risks, checks and idempotency key.

## Workflow

1. Validate schema version, stable identities, immutable hashes and policy prerequisites.
2. Reconfirm HEAD; create/update one Draft MR; preserve approver policy; publish notes/discussions and artifact links; record provider evidence.
...
gitlab-mr-delivery.json with MR ID, URL, draft state, head SHA, tier and evidence refs.

## Fail-closed rules

Unavailable authorization or API is NOT_RUN; never force push, auto-merge or assume Ultimate features.
