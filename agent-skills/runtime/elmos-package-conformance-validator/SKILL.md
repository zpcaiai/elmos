---
name: package-conformance-validator
description: Use this skill when Elmos must perform package conformance validator as part of the 00-foundation-contracts production
  workflow, with typed contracts, policy enforcement, evidence capture, and rollback.
license: Proprietary-Elmos-Commercial
compatibility: Elmos v3 harness; K0 Cross-Kernel Contracts; policy and evidence services required.
metadata:
  version: 3.0.0
  pack: 00-foundation-contracts
  priority: P0
  exposure: atomic-registry-only
  business-line: platform-foundation
allowed-tools: registry.read schema.validate policy.evaluate artifact.hash
---
# package-conformance-validator

## 能力目标

对整个 Skills Package 执行结构、命名、权限、证据和依赖一致性校验。
...
- 必须门：schema-valid, globally-unique-id, versioned, owner-assigned, rollback-defined；
...
- 业务线：`platform-foundation`；包：`00-foundation-contracts`。
