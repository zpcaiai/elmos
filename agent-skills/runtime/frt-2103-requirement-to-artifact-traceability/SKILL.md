...
description: "Run FRT-2103 Requirement-to-Artifact Traceability for FRT G21 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-2103"
  source_name: "frt-2103-requirement-to-artifact-traceability"
  source_sha256: "sha256:f1ce7be92ec405b82f16f5d16cfee1357a836756c7f1254cf64cbdda9b6bb911"
  batch: "G21"
...
- Runtime catalog key: `FRT-2103` / `frt-2103-requirement-to-artifact-traceability`.
...
在G21范围内实现 **Requirement-to-Artifact Traceability**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：统一 Product Closure Graph; Requirement、Capability、Journey、Command、API、Data、Permission、Test、Monitoring、Runbook、Certificate 双向追踪; 孤儿 UI/API/Command/Event/Data Write 检测。

## When to Use

- 实现或修改与 **Requirement-to-Artifact Traceability** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-2103Result`：版本化、带状态和诊断的结构化结果。
- `FRT-2103EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-2103FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-2103CertificateFragment`：供G21批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g21/frt-2103-requirement-to-artifact-traceability/
packages/runtime/g21/frt-2103-requirement-to-artifact-traceability/
services/control-plane/g21/frt-2103-requirement-to-artifact-traceability/
apps/web-console/src/features/g21/frt-2103-requirement-to-artifact-traceability/
apps/admin-console/src/features/g21/frt-2103-requirement-to-artifact-traceability/
tests/g21/frt-2103-requirement-to-artifact-traceability/
```
...
```text
POST /v1/skills/frt-2103/runs
GET  /v1/skills/frt-2103/runs/{run_id}
POST /v1/skills/frt-2103/runs/{run_id}/verify
GET  /v1/skills/frt-2103/runs/{run_id}/findings
GET  /v1/skills/frt-2103/runs/{run_id}/evidence
```
