---
name: deterministic-first-router
description: 任务先判断能否用 Recipe/语义查询解决，再决定是否调用 LLM。 Use for this exact ELMOS Spring Golden Route v2 contract; imported specifications are not runtime or certification evidence.
...
metadata:
  ...
  source_id: FOUNDATION-10-deterministic-first-router
  source_name: deterministic-first-router
  source_path: skills/deterministic-first-router/SKILL.md
  source_sha256: sha256:8aafc5b5d44a11d4583d7c3e2143f1d59cb36414da873b1e5c19317107919a19
  source_contract_sha256: sha256:a56ca908d5ce8dc3445f5262c106b0167ef3a4870eacadfa43cdd06df1f7299b
...
任务先判断能否用 Recipe/语义查询解决，再决定是否调用 LLM。

## Use when

在 Elmos 需要实现或升级 **Deterministic-first Router** 相关能力时使用；它不是仅供解释的提示词，而是实现、测试与验收契约。

## Capabilities to implement

- 规则命中优先级高于自由生成
- 按 confidence/capability/expected cost 选择 Recipe、Agent 或 Hybrid
- 同类 LLM 成功模式可逐步降低到确定性规则
- 路由结果带可解释证据
...
- [ ] 定义 `deterministic-first-router` 的 public interface、input/output/error schemas。
- [ ] 建立 `deterministic-first-router` 的 capability/feature flags 与版本字段。
...
- [ ] 至少覆盖并验收：规则命中优先级高于自由生成；以及 按 confidence/capability/expected cost 选择 Recipe、Agent 或 Hybrid。
...
- Machine contract: `references/contract.json` (`sha256:a56ca908d5ce8dc3445f5262c106b0167ef3a4870eacadfa43cdd06df1f7299b`).
