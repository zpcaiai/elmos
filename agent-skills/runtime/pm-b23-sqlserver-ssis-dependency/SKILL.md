---
name: pm-b23-sqlserver-ssis-dependency
description: "识别并迁移 SSIS Package、连接、数据流、控制流和调度依赖. Precision Migration B23 contract; use for this exact assessment, transformation, validation, repair, evidence, or cutover scope."
---

# Sqlserver Ssis Dependency
## ELMOS runtime binding

- Invoke this repository Skill as `$pm-b23-sqlserver-ssis-dependency`.
- Immutable source identity: `sqlserver-ssis-dependency` in `precision-migration-b01-44` (B23).
- Runtime adapter: `database-and-data-route`; binding state: `UNAVAILABLE`.
- Resolve and plan with `python3 scripts/precision_migration/runtime.py plan --skill pm-b23-sqlserver-ssis-dependency`.
...
识别并迁移 SSIS Package、连接、数据流、控制流和调度依赖。

本 Skill 属于精密语言转换系统。它必须以**可执行证据**为准，不得用模型自评、源码相似度或“能编译”替代语义与行为验证。

## Use when

- 当前任务直接涉及 `sqlserver-ssis-dependency` 所描述的能力。
...
1. 盘点对象与专有能力
2. 执行本 Skill 的核心职责：识别并迁移 SSIS Package、连接、数据流、控制流和调度依赖。
...
```yaml
skill: sqlserver-ssis-dependency
version: 1.0.0
