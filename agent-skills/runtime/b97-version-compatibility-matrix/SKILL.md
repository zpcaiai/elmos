---
name: b97-version-compatibility-matrix
description: "Use when ELMOS must Model ELMOS Core, Skill, contract, toolchain, route and domain-pack compatibility using exact ranges and evidence. Triggers: Version Compatibility Matrix, Canonical Capability Graph Consolidation Pack, semver compatibility, toolchain constraints, schema compatibility, route compatibility."
metadata:
  id: B97-S08
...
# B97-S08 — Version Compatibility Matrix

## Objective

Model ELMOS Core, Skill, contract, toolchain, route and domain-pack compatibility using exact ranges and evidence.
...
Use this Skill when the approved task explicitly requires **Version Compatibility Matrix** or when the capability graph selects it as part of a minimal dependency closure.
...
- range intersections validated.
- unsupported combinations blocked.
- matrix provenance complete.
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b97-version-compatibility-matrix/SKILL.md --out /tmp/version_compatibility_matrix.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B97-S08
```
...
- Reject or expose wildcard version acceptance.
- Reject or expose downgrade ambiguity.
- Reject or expose conflicting ranges.
