...
description: "Run FRT-1907 Symbolic Execution Adapter for FRT G19 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-1907"
  source_name: "frt-1907-symbolic-execution-adapter"
  source_sha256: "sha256:5f21a99ec141682d67af11afcd7b06f9ec64f30673cd344ac3a495d2504a0c46"
  batch: "G19"
...
- Runtime catalog key: `FRT-1907` / `frt-1907-symbolic-execution-adapter`.
...
# FRT-1907 — Symbolic Execution Adapter

## Objective

在G19范围内实现 **Symbolic Execution Adapter**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：从IR、Rule和业务不变量生成Proof Obligation; Leanstral仅提出证明候选，Lean Kernel为可信根; 组合SMT、Model Checking、Symbolic和Differential Evidence。

## When to Use

- 实现或修改与 **Symbolic Execution Adapter** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-1907Result`：版本化、带状态和诊断的结构化结果。
- `FRT-1907EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-1907FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-1907CertificateFragment`：供G19批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g19/frt-1907-symbolic-execution-adapter/
packages/runtime/g19/frt-1907-symbolic-execution-adapter/
services/control-plane/g19/frt-1907-symbolic-execution-adapter/
apps/web-console/src/features/g19/frt-1907-symbolic-execution-adapter/
apps/admin-console/src/features/g19/frt-1907-symbolic-execution-adapter/
tests/g19/frt-1907-symbolic-execution-adapter/
```
...
```text
POST /v1/skills/frt-1907/runs
GET  /v1/skills/frt-1907/runs/{run_id}
POST /v1/skills/frt-1907/runs/{run_id}/verify
GET  /v1/skills/frt-1907/runs/{run_id}/findings
GET  /v1/skills/frt-1907/runs/{run_id}/evidence
```
