...
metadata:
  ...
  category: "Kotlin JVM Android and Multiplatform Project Pack"
---

# PG251 — Kotlin Coroutine Structured Concurrency

## Objective

Implement and validate Kotlin Coroutine Structured Concurrency for Kotlin JVM, Spring, Ktor, coroutines, data access, Java interoperability, Android Compose, and Kotlin Multiplatform, preserving ownership, compatibility, security, reproducibility, and evidence boundaries.
...
Use for `kotlin-coroutine-structured-concurrency` tasks involving Kotlin JVM, Spring, Ktor, coroutines, data access, Java interoperability, Android Compose, and Kotlin Multiplatform.
...
- Inspect source markers such as `build.gradle.kts`, `settings.gradle.kts`, `gradle/libs.versions.toml`, `src/main/kotlin`, `AndroidManifest.xml` and resolve effective includes, generated files, workspaces, modules, targets, and entrypoints.
...
Upstream dependencies: `PG183`, `PG184`, `PG185`, `PG191`, `PG246`, `PG250`.
...
- `kotlin_coroutine_structured_concurrency_plan.json`
- `kotlin_coroutine_structured_concurrency_artifacts/`
- `kotlin_coroutine_structured_concurrency_verification.json`
...
- Preferred tools: `java`, `kotlinc`, `gradle/gradlew`, `detekt`, `ktlint`, `JUnit`, `Android SDK`. Record alternate tools, versions, and rationale.
...
- Define ownership, cancellation, backpressure, shutdown, and leak policy.
- Run race/deadlock/leak checks where available.
- Do not leave orphan work after cancellation.
- Test coroutine cancellation and dispatcher ownership.
- Contain platform types at Java interop boundaries.
- Record Gradle, plugin, JVM, Android, and KMP target versions.
