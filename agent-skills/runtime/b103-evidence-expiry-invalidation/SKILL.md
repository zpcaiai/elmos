---
name: b103-evidence-expiry-invalidation
description: "Use when ELMOS must Expire and invalidate evidence on time or causal change and propagate revocation to certifications. Triggers: Evidence Expiry and Invalidation, Evidence and Certification Fabric Pack, expiry, causal invalidation, retest, revocation."
metadata:
  id: B103-S09
...
# B103-S09 — Evidence Expiry and Invalidation

## Objective

Expire and invalidate evidence on time or causal change and propagate revocation to certifications.
...
Use this Skill when the approved task explicitly requires **Evidence Expiry and Invalidation** or when the capability graph selects it as part of a minimal dependency closure.
...
- `revocation_events.json`
- `revalidation_plan.json`
...
- certifications revoke transitively.
- expiry clocks trusted.
- retest scope explicit.
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b103-evidence-expiry-invalidation/SKILL.md --out /tmp/evidence_expiry_invalidation.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B103-S09
```
...
- Reject or expose stale evidence retained.
- Reject or expose revocation event dropped.
- Reject or expose clock rollback.
