---
name: b100-runner-attestation-posture
description: "Use when ELMOS must Attest runner software, image, kernel, configuration and policy posture before sensitive tasks. Triggers: Runner Attestation and Posture, Hardened Runner Fabric Pack, attestation, image digest, posture policy, trust decision."
metadata:
  id: B100-S03
...
# B100-S03 — Runner Attestation and Posture

## Objective

Attest runner software, image, kernel, configuration and policy posture before sensitive tasks.
...
Use this Skill when the approved task explicitly requires **Runner Attestation and Posture** or when the capability graph selects it as part of a minimal dependency closure.
...
- posture freshness.
- failed attestation quarantines.
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b100-runner-attestation-posture/SKILL.md --out /tmp/runner_attestation_posture.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B100-S03
```
...
- Reject or expose forged attestation.
- Reject or expose stale posture reuse.
- Reject or expose debug mode allowed.
