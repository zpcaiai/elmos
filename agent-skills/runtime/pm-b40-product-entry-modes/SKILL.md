...
- Invoke this repository Skill as `$pm-b40-product-entry-modes`.
- Immutable source identity: `batch-40-product-entry-modes` in `precision-migration-b01-44` (B40).
- Runtime adapter: `skill-and-project-synthesis`; binding state: `DECLARED`.
- Resolve and plan with `python3 scripts/precision_migration/runtime.py plan --skill pm-b40-product-entry-modes`.
...
- 当任务涉及 **compare-and-repair** 时，调用 `../pm-b40-compare-and-repair/SKILL.md`。
- 当任务涉及 **convert-and-certify** 时，调用 `../pm-b40-convert-and-certify/SKILL.md`。
- 当任务涉及 **generate-from-skills** 时，调用 `../pm-b40-generate-from-skills/SKILL.md`。
- 当任务涉及 **migration-mode-selector** 时，调用 `../pm-b40-migration-mode-selector/SKILL.md`。
- 当任务涉及 **asset-reuse-planner** 时，调用 `../pm-b40-asset-reuse-planner/SKILL.md`。
