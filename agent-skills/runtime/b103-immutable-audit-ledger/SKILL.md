---
name: b103-immutable-audit-ledger
description: "Use when ELMOS must Record actor, decision, mutation, access and certification events in tamper-evident sequence. Triggers: Immutable Audit Ledger, Evidence and Certification Fabric Pack, audit ledger, hash chain, actor identity, access audit."
metadata:
  id: B103-S10
...
# B103-S10 — Immutable Audit Ledger

## Objective

Record actor, decision, mutation, access and certification events in tamper-evident sequence.
...
Use this Skill when the approved task explicitly requires **Immutable Audit Ledger** or when the capability graph selects it as part of a minimal dependency closure.
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b103-immutable-audit-ledger/SKILL.md --out /tmp/immutable_audit_ledger.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B103-S10
```
...
- Reject or expose event deletion.
- Reject or expose backdated entry.
- Reject or expose anonymous approval.
