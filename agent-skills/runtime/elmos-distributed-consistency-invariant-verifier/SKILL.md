---
name: elmos-distributed-consistency-invariant-verifier
description: Verify business invariants across services, stores, caches and event streams under concurrency, partitions, retries and failover.
version: 4.0.0
...
# Distributed Consistency Invariant Verifier
...
- `DistributedConsistencyInvariantVerifierProfile`
- `DistributedConsistencyInvariantVerifierPlan`
- `DistributedConsistencyInvariantVerifierRun`
- `DistributedConsistencyInvariantVerifierDecision`

Required semantic capabilities:

- compile cross-component invariants
- generate concurrent histories and partitions
- check monotonicity and convergence
- compare source/target consistency envelopes
- attach runtime monitors where proof is bounded
...
- `artifacts/elmos-distributed-consistency-invariant-verifier/profile.yaml`
- `artifacts/elmos-distributed-consistency-invariant-verifier/plan.json`
- `artifacts/elmos-distributed-consistency-invariant-verifier/result.json`
- `artifacts/elmos-distributed-consistency-invariant-verifier/evidence/`
...
- **invariant scoped to one store only** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **eventual convergence never bounded** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **cache masks stale authoritative state** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **partition scenario omitted** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **runtime monitor not connected to gate** — must have a preventive control, a detective signal, a negative fixture and a response action.
...
- native scenario: compile cross-component invariants
- native scenario: generate concurrent histories and partitions
- native scenario: check monotonicity and convergence
- native scenario: compare source/target consistency envelopes
- native scenario: attach runtime monitors where proof is bounded
