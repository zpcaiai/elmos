...
description: "Run FRT-0904 Lifecycle Ordering Validator for FRT G09 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-0904"
  source_name: "frt-0904-lifecycle-ordering-validator"
  source_sha256: "sha256:55a2af9c41e098b32b0be63b639068fd432a1dd5d9bc129bba0c7f922af59640"
  batch: "G09"
...
- Runtime catalog key: `FRT-0904` / `frt-0904-lifecycle-ordering-validator`.
...
# FRT-0904 — Lifecycle Ordering Validator

## Objective

在G09范围内实现 **Lifecycle Ordering Validator**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：区分Local、Server、Global、Persistent和Derived State; 保持Effect触发、Cleanup、Lifecycle和Evaluation Order; 传播Cancellation并防止Stale Result Commit。

## When to Use

- 实现或修改与 **Lifecycle Ordering Validator** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-0904Result`：版本化、带状态和诊断的结构化结果。
- `FRT-0904EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-0904FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-0904CertificateFragment`：供G09批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g09/frt-0904-lifecycle-ordering-validator/
packages/runtime/g09/frt-0904-lifecycle-ordering-validator/
services/control-plane/g09/frt-0904-lifecycle-ordering-validator/
apps/web-console/src/features/g09/frt-0904-lifecycle-ordering-validator/
apps/admin-console/src/features/g09/frt-0904-lifecycle-ordering-validator/
tests/g09/frt-0904-lifecycle-ordering-validator/
```
...
```text
POST /v1/skills/frt-0904/runs
GET  /v1/skills/frt-0904/runs/{run_id}
POST /v1/skills/frt-0904/runs/{run_id}/verify
GET  /v1/skills/frt-0904/runs/{run_id}/findings
GET  /v1/skills/frt-0904/runs/{run_id}/evidence
```
