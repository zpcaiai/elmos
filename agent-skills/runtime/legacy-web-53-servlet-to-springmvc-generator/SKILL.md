---
name: legacy-web-53-servlet-to-springmvc-generator
description: "Repository-owned exact runtime interface for Servlet→Spring MVC 生成器; bounded semantic analysis and evidence generation for Java legacy web modernization."
metadata:
  source_package: elmos.java-legacy-web.repository-modernization
  source_version: 1.0.0
  source_id: 53-servlet-to-springmvc-generator
  source_digest: sha256:947607c7823beb1b2729c2f0926aedacf891972dd8abc7b29c2797ba56c7835e
  phase: transformation
  runtime_state: CODE_COMPLETE_LOCAL
  capability_state: LOCAL_EXECUTED
  operation_code: SERVLET_TARGET_GENERATED
  runtime_handler_id: legacy-web-handler:53-servlet-to-springmvc-generator
---

# Servlet→Spring MVC 生成器
...
---
id: 53-servlet-to-springmvc-generator
title: Servlet→Spring MVC 生成器
version: 1.0.0
phase: transformation
priority: P0
requires:
...
转换 HttpServlet、Filter、Listener、程序化注册、异步/流式响应和 descriptor，同时保留 dispatch 与生命周期语义。
...
producerSkill: 53-servlet-to-springmvc-generator
...
5. 以幂等 change set 发布。
6. 不是所有 Servlet 都应变 Controller；低层 streaming/async/protocol servlet 可保留并以 registration bean 托管。
7. Filter 保留 dispatcher type、order、asyncSupported 和 url/servlet-name mapping。
8. Listener 映射到合适生命周期，不把容器 callback 简化为普通 Spring event。
9. RequestDispatcher.include/forward 和 response commit 需专门处理。
