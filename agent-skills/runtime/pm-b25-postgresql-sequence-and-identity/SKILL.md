---
name: pm-b25-postgresql-sequence-and-identity
description: "迁移 Sequence、Identity、缓存、OWNED BY 和事务外取号. Precision Migration B25 contract; use for this exact assessment, transformation, validation, repair, evidence, or cutover scope."
---

# Postgresql Sequence And Identity
## ELMOS runtime binding

- Invoke this repository Skill as `$pm-b25-postgresql-sequence-and-identity`.
- Immutable source identity: `postgresql-sequence-and-identity` in `precision-migration-b01-44` (B25).
- Runtime adapter: `database-and-data-route`; binding state: `UNAVAILABLE`.
- Resolve and plan with `python3 scripts/precision_migration/runtime.py plan --skill pm-b25-postgresql-sequence-and-identity`.
...
迁移 Sequence、Identity、缓存、OWNED BY 和事务外取号。

本 Skill 属于精密语言转换系统。它必须以**可执行证据**为准，不得用模型自评、源码相似度或“能编译”替代语义与行为验证。

## Use when

- 当前任务直接涉及 `postgresql-sequence-and-identity` 所描述的能力。
...
1. 盘点对象与专有能力
2. 执行本 Skill 的核心职责：迁移 Sequence、Identity、缓存、OWNED BY 和事务外取号。
...
```yaml
skill: postgresql-sequence-and-identity
version: 1.0.0
