...
description: "Run FRT-0212 Repository Model Certification for FRT G02 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-0212"
  source_name: "frt-0212-repository-model-certification"
  source_sha256: "sha256:e5fc9edbc20a70ad5d489c0020459581e49194f2e6796f0cd1eebc6047e5f427"
  batch: "G02"
...
- Runtime catalog key: `FRT-0212` / `frt-0212-repository-model-certification`.
...
# FRT-0212 — Repository Model Certification

## Objective

在G02范围内实现 **Repository Model Certification**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：识别Monorepo、Package、Framework、Runtime和Build链; 建立模块、依赖、路由、组件、状态、API、存储和资产清单; 检测动态代码、原生能力、平台限制和迁移风险。

## When to Use

- 实现或修改与 **Repository Model Certification** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-0212Result`：版本化、带状态和诊断的结构化结果。
- `FRT-0212EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-0212FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-0212CertificateFragment`：供G02批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g02/frt-0212-repository-model-certification/
packages/runtime/g02/frt-0212-repository-model-certification/
services/control-plane/g02/frt-0212-repository-model-certification/
apps/web-console/src/features/g02/frt-0212-repository-model-certification/
apps/admin-console/src/features/g02/frt-0212-repository-model-certification/
tests/g02/frt-0212-repository-model-certification/
```
...
```text
POST /v1/skills/frt-0212/runs
GET  /v1/skills/frt-0212/runs/{run_id}
POST /v1/skills/frt-0212/runs/{run_id}/verify
GET  /v1/skills/frt-0212/runs/{run_id}/findings
GET  /v1/skills/frt-0212/runs/{run_id}/evidence
```
