...
# Sqlserver Query Hint And Plan
## ELMOS runtime binding

- Invoke this repository Skill as `$pm-b23-sqlserver-query-hint-and-plan`.
- Immutable source identity: `sqlserver-query-hint-and-plan` in `precision-migration-b01-44` (B23).
- Runtime adapter: `database-and-data-route`; binding state: `UNAVAILABLE`.
- Resolve and plan with `python3 scripts/precision_migration/runtime.py plan --skill pm-b23-sqlserver-query-hint-and-plan`.
...
处理 NOLOCK、UPDLOCK、INDEX、RECOMPILE、Plan Guide 和目标执行计划。

本 Skill 属于精密语言转换系统。它必须以**可执行证据**为准，不得用模型自评、源码相似度或“能编译”替代语义与行为验证。

## Use when

- 当前任务直接涉及 `sqlserver-query-hint-and-plan` 所描述的能力。
...
1. 盘点对象与专有能力
2. 执行本 Skill 的核心职责：处理 NOLOCK、UPDLOCK、INDEX、RECOMPILE、Plan Guide 和目标执行计划。
...
```yaml
skill: sqlserver-query-hint-and-plan
version: 1.0.0
