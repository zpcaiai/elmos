...
description: "Run FRT-2017 Kubernetes Single-Node and Air-Gapped Deployment for FRT G20 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-2017"
  source_name: "frt-2017-kubernetes-single-node-and-air-gapped-deployment"
  source_sha256: "sha256:227ffd7ab444416cccabb9b95303eddb37b620cf403326eca4b54011b4b769a1"
  batch: "G20"
  source_version: "1.0.0"
  source_risk: "critical-by-default"
  source_certificate_family: "PD"
...
- Runtime catalog key: `FRT-2017` / `frt-2017-kubernetes-single-node-and-air-gapped-deployment`.
...
# FRT-2017 — Kubernetes Single-Node and Air-Gapped Deployment

## Objective

在G20范围内实现 **Kubernetes Single-Node and Air-Gapped Deployment**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：提供Skill SDK、Runtime、Registry和Marketplace; 实现CLI、API、IDE、Web Console和Multi-Tenant Workspace; Durable Workflow与分布式Worker支持暂停、恢复、取消和幂等。

## When to Use

- 实现或修改与 **Kubernetes Single-Node and Air-Gapped Deployment** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-2017Result`：版本化、带状态和诊断的结构化结果。
- `FRT-2017EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-2017FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-2017CertificateFragment`：供G20批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g20/frt-2017-kubernetes-single-node-and-air-gapped-deployment/
packages/runtime/g20/frt-2017-kubernetes-single-node-and-air-gapped-deployment/
services/control-plane/g20/frt-2017-kubernetes-single-node-and-air-gapped-deployment/
apps/web-console/src/features/g20/frt-2017-kubernetes-single-node-and-air-gapped-deployment/
apps/admin-console/src/features/g20/frt-2017-kubernetes-single-node-and-air-gapped-deployment/
tests/g20/frt-2017-kubernetes-single-node-and-air-gapped-deployment/
```
...
```text
POST /v1/skills/frt-2017/runs
GET  /v1/skills/frt-2017/runs/{run_id}
POST /v1/skills/frt-2017/runs/{run_id}/verify
GET  /v1/skills/frt-2017/runs/{run_id}/findings
GET  /v1/skills/frt-2017/runs/{run_id}/evidence
```
