---
name: evidence-freshness-expiry-renewal-sla-and-forward-alerting
description: 实现按Evidence/Control类型的Freshness、提前失效、续期、SLA、90/60/30天预警和影响预测。
metadata:
  elmos_batch: "37C"
  elmos_skill_id: "ANA37C008"
  pack_version: "3.7-complete"
---

# ANA37C008 — Evidence Freshness、Expiry、Renewal、SLA 与 Forward Alerting

## Objective

实现按Evidence/Control类型的Freshness、提前失效、续期、SLA、90/60/30天预警和影响预测。
...
- Freshness使用Created/Observed/Effective/Expires、Source Commit、Policy、Control、Signer、Builder、Report Period。
- Staleness原因包括Age、Source/Policy/Scope Change、Revocation、Withdrawal等。
- Renewal类型包括Rerun Scan/SBOM、Renew Assessment、Rotate Signer、Refresh Audit。
- Forecast关联受影响Project、Control、Audit、Owner和Lead Time。
...
- freshnessUsesPolicyNotGlobalAge
- sourceCommitChangeCanStaleEvidenceEarly
- unknownExpiryDoesNotPassFreshness
- resolvedRenewalTriggersReverification
