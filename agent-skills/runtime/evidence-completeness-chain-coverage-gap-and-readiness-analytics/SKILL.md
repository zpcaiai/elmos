---
name: evidence-completeness-chain-coverage-gap-and-readiness-analytics
description: 基于Requirement Profile和End-to-end Evidence Chain计算多维完整度、缺口和项目就绪度。
metadata:
  elmos_batch: "37C"
  elmos_skill_id: "ANA37C004"
  pack_version: "3.7-complete"
---

# ANA37C004 — Evidence Completeness、Chain Coverage、Gap 与 Readiness Analytics

## Objective

基于Requirement Profile和End-to-end Evidence Chain计算多维完整度、缺口和项目就绪度。
...
- 标准Chain覆盖 Source→Build→SBOM→Security→Test→Performance→Delivery→Acceptance。
- Requirement包含Artifact/Predicate/Verification/Freshness/Producer Assurance/Multiplicity/Criticality。
- 覆盖维度为Presence/Integrity/Authenticity/Correlation/Scope/Freshness/Verification/Review。
- Critical Gap强制Blocked，并生成精确Remediation Recommendation。
...
- uploadedFileCountIsNotCompleteness
- presentButUnverifiedEvidenceDoesNotCountVerified
- criticalGapBlocksReadiness
- allStagesMustLinkForEndToEndCoverage
