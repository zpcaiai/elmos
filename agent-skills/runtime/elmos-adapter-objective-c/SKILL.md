---
name: elmos-adapter-objective-c
description: "Implement Objective-C analysis and incremental Swift/Flutter migration while preserving runtime messaging, memory management, UIKit/AppKit lifecycle, and native SDK behavior."
version: 1.0.0
skill_id: ELMOS-POLY-059
...
triggers:
  - "Use when implementing or executing `elmos-adapter-objective-c`."
  - "Use when the current DAG node requires the technology-adapter capability."
  - "Use when `objc` is a source, target, interoperability, or modernization technology."
  - "Use when files or build roots matching .m, .mm, .h are detected."
outputs:
  - "adapters/objc/adapter-manifest.json"
  - "adapters/objc/parser-profile.yaml"
  - "adapters/objc/semantic-mapping.yaml"
  - "adapters/objc/verification-profile.yaml"
---
...
Implement Objective-C analysis and incremental Swift/Flutter migration while preserving runtime messaging, memory management, UIKit/AppKit lifecycle, and native SDK behavior.
...
- Technology registry entry `objc` and the detected native toolchain profile.
...
| Kind | language |
| File/build markers | `.m`, `.mm`, `.h` |
| Build systems | Xcode, xcodebuild, CocoaPods, Bazel |
| Preferred analyzers | Clang AST, libclang, SourceKit, tree-sitter-objc |
| Framework coverage | Foundation, UIKit, AppKit, Core Data |
| Native verification | `xcodebuild test`, `clang static analyzer`, `XCTest` |

### Semantic risk register

- dynamic message dispatch and forwarding.
- categories and method swizzling.
- ARC/MRC and Core Foundation bridging.
- nullability gaps.
- KVC/KVO and notifications.
- blocks and concurrency.
- Objective-C++ and C/C++ ABI.
...
1. Detect Xcode targets/configurations, SDKs, architectures, ARC/MRC, CocoaPods, generated files, bridging headers, and Objective-C++.
2. Use Clang AST/libclang with actual build settings to resolve types, selectors, properties, protocols, categories, blocks, and macros.
3. Lower dynamic messaging, categories, protocols, KVC/KVO, notifications, blocks, errors, ownership, and Core Foundation bridging into Semantic IR.
4. Lower UIKit/AppKit lifecycle, navigation, outlets/actions, Core Data, delegates, and platform services into Framework IR.
5. Identify swizzling, forwarding, runtime lookup, MRC, inline C/C++, and binary SDK boundaries.
6. Plan incremental bridging to Swift or platform-channel wrapping for Flutter.
7. Preserve Objective-C-visible API where mixed-language compatibility is required.
8. Verify with xcodebuild/XCTest, static analyzer, memory tests, UI tests, and device/simulator evidence.
...
- [ ] ARC/MRC and Core Foundation bridge.
- [ ] categories/swizzling/forwarding detection.
- [ ] KVC/KVO/notification behavior.
- [ ] UIKit lifecycle/navigation.
- [ ] Objective-C to Swift bridging ABI.
- [ ] Objective-C++/binary SDK blocker.
