---
name: b98-executable-contract-certification-gate
description: "Use when ELMOS must Certify executable contracts only when compilation, guards, permissions, dependencies, rollback, tests, evidence and signatures are complete. Triggers: Executable Contract Certification Gate, Executable Skill Contract and Registry Pack, contract gate, anti-fake state, independent verification, signed release."
metadata:
  id: B98-S16
  ...
  category: "Executable Skill Contract and Registry Pack"
---

# B98-S16 — Executable Contract Certification Gate

## Objective

Certify executable contracts only when compilation, guards, permissions, dependencies, rollback, tests, evidence and signatures are complete.
...
The objective belongs to **Executable Skill Contract and Registry Pack**, whose system-level purpose is to compile Markdown Skills into signed executable contracts with schemas, permissions, rollback, tests, registry, search, dependency resolution and compatibility governance.

## When to Use

Use this Skill when the approved task explicitly requires **Executable Contract Certification Gate** or when the capability graph selects it as part of a minimal dependency closure.
...
- `contract_certification_result.json`
- `contract_release_manifest.json`
...
- Preferred implementation/tooling context: `Python/Rust/Java compiler implementation`, `JSON Schema 2020-12`, `policy engine`, `signature service`. Alternatives are allowed only when their versions, behavior differences and rationale are recorded.
...
- all mandatory obligations satisfied.
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b98-executable-contract-certification-gate/SKILL.md --out /tmp/executable_contract_certification_gate.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B98-S16
```
...
- Reject or expose unsigned certified contract.
- Reject or expose not-run obligation.
- Reject or expose permission wildcard waiver.
