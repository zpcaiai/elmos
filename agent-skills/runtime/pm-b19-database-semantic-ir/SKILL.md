...
- Invoke this repository Skill as `$pm-b19-database-semantic-ir`.
- Immutable source identity: `batch-19-database-semantic-ir` in `precision-migration-b01-44` (B19).
- Runtime adapter: `database-and-data-route`; binding state: `DECLARED`.
- Resolve and plan with `python3 scripts/precision_migration/runtime.py plan --skill pm-b19-database-semantic-ir`.
...
- 当任务涉及 **database-schema-ir** 时，调用 `../pm-b19-database-schema-ir/SKILL.md`。
- 当任务涉及 **sql-expression-ir** 时，调用 `../pm-b19-sql-expression-ir/SKILL.md`。
- 当任务涉及 **query-plan-ir** 时，调用 `../pm-b19-query-plan-ir/SKILL.md`。
- 当任务涉及 **database-type-system** 时，调用 `../pm-b19-database-type-system/SKILL.md`。
- 当任务涉及 **constraint-and-index-ir** 时，调用 `../pm-b19-constraint-and-index-ir/SKILL.md`。
- 当任务涉及 **transaction-isolation-ir** 时，调用 `../pm-b19-transaction-isolation-ir/SKILL.md`。
- 当任务涉及 **procedure-control-flow-ir** 时，调用 `../pm-b19-procedure-control-flow-ir/SKILL.md`。
- 当任务涉及 **trigger-and-event-ir** 时，调用 `../pm-b19-trigger-and-event-ir/SKILL.md`。
- 当任务涉及 **sequence-identity-ir** 时，调用 `../pm-b19-sequence-identity-ir/SKILL.md`。
- 当任务涉及 **database-security-ir** 时，调用 `../pm-b19-database-security-ir/SKILL.md`。
- 当任务涉及 **replication-and-cdc-ir** 时，调用 `../pm-b19-replication-and-cdc-ir/SKILL.md`。
- 当任务涉及 **database-effect-ir** 时，调用 `../pm-b19-database-effect-ir/SKILL.md`。
