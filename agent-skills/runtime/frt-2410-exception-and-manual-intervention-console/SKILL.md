...
description: "Run FRT-2410 Exception and Manual Intervention Console for FRT G24 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-2410"
  source_name: "frt-2410-exception-and-manual-intervention-console"
  source_sha256: "sha256:9ca0a723e7423d8e6856b4290772799a954588d4d9facfa3f7bff0c423b620e4"
  batch: "G24"
...
- Runtime catalog key: `FRT-2410` / `frt-2410-exception-and-manual-intervention-console`.
...
在G24范围内实现 **Exception and Manual Intervention Console**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：Admin Object × Action × Role × Scope × Environment矩阵; 高风险操作Impact Preview、Reason、Approval、Typed Command、Audit; Manual Case、Data Repair、Reconciliation正式后台流程。

## When to Use

- 实现或修改与 **Exception and Manual Intervention Console** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-2410Result`：版本化、带状态和诊断的结构化结果。
- `FRT-2410EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-2410FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-2410CertificateFragment`：供G24批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g24/frt-2410-exception-and-manual-intervention-console/
packages/runtime/g24/frt-2410-exception-and-manual-intervention-console/
services/control-plane/g24/frt-2410-exception-and-manual-intervention-console/
apps/web-console/src/features/g24/frt-2410-exception-and-manual-intervention-console/
apps/admin-console/src/features/g24/frt-2410-exception-and-manual-intervention-console/
tests/g24/frt-2410-exception-and-manual-intervention-console/
```
...
```text
POST /v1/skills/frt-2410/runs
GET  /v1/skills/frt-2410/runs/{run_id}
POST /v1/skills/frt-2410/runs/{run_id}/verify
GET  /v1/skills/frt-2410/runs/{run_id}/findings
GET  /v1/skills/frt-2410/runs/{run_id}/evidence
```
