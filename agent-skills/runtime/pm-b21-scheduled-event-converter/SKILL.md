...
# Scheduled Event Converter
## ELMOS runtime binding

- Invoke this repository Skill as `$pm-b21-scheduled-event-converter`.
- Immutable source identity: `scheduled-event-converter` in `precision-migration-b01-44` (B21).
- Runtime adapter: `database-and-data-route`; binding state: `DECLARED`.
- Resolve and plan with `python3 scripts/precision_migration/runtime.py plan --skill pm-b21-scheduled-event-converter`.
...
转换数据库 Job、Event、Scheduler 到目标调度器或外部工作流。

本 Skill 属于精密语言转换系统。它必须以**可执行证据**为准，不得用模型自评、源码相似度或“能编译”替代语义与行为验证。

## Use when

- 当前任务直接涉及 `scheduled-event-converter` 所描述的能力。
...
1. 盘点对象与专有能力
2. 执行本 Skill 的核心职责：转换数据库 Job、Event、Scheduler 到目标调度器或外部工作流。
...
```yaml
skill: scheduled-event-converter
version: 1.0.0
