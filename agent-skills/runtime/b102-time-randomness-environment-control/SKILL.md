---
name: b102-time-randomness-environment-control
description: "Use when ELMOS must Control clocks, time zones, random seeds, locale, ports and environment-dependent behavior. Triggers: Time Randomness and Environment Control, Semantic Equivalence Lab Pack, time control, randomness, locale, environment."
metadata:
  id: B102-S10
...
# B102-S10 — Time Randomness and Environment Control

## Objective

Control clocks, time zones, random seeds, locale, ports and environment-dependent behavior.
...
Use this Skill when the approved task explicitly requires **Time Randomness and Environment Control** or when the capability graph selects it as part of a minimal dependency closure.
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b102-time-randomness-environment-control/SKILL.md --out /tmp/time_randomness_environment_control.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B102-S10
```
...
- Reject or expose system clock dependency.
- Reject or expose locale-only pass.
- Reject or expose port collision.
