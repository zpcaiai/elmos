...
## Verify
执行本skill的acceptance.yaml对应真实测试，并检查原始报告而非Agent总结。关联输出：repo-map.json, gap-analysis.md, compatibility-map.json, native-toolchain-inventory.json。
至少演练：未读取源码或缺少数据库；必须观察：相关实现状态必须unknown/NOT_RUN，不能标completed。
