---
name: b100-resource-quota-enforcer
description: "Use when ELMOS must Enforce CPU, memory, disk, inode, process, network and wall-clock budgets against hostile workloads. Triggers: Resource Quota Enforcer, Hardened Runner Fabric Pack, cgroups, disk quotas, process limits, timeouts."
metadata:
  id: B100-S10
...
# B100-S10 — Resource Quota Enforcer

## Objective

Enforce CPU, memory, disk, inode, process, network and wall-clock budgets against hostile workloads.
...
Use this Skill when the approved task explicitly requires **Resource Quota Enforcer** or when the capability graph selects it as part of a minimal dependency closure.
...
- hard limits effective.
- OOM classified.
- quota evidence complete.
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b100-resource-quota-enforcer/SKILL.md --out /tmp/resource_quota_enforcer.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B100-S10
```
...
- Reject or expose fork bomb.
- Reject or expose zip bomb.
- Reject or expose disk sparse-file bypass.
