...
description: "Run FRT-0906 Cancellation Propagation Mapper for FRT G09 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-0906"
  source_name: "frt-0906-cancellation-propagation-mapper"
  source_sha256: "sha256:b28734e0757889ec2669a2f5689314945c53656aed7c972eef99dd08cacc5112"
  batch: "G09"
...
- Runtime catalog key: `FRT-0906` / `frt-0906-cancellation-propagation-mapper`.
...
# FRT-0906 — Cancellation Propagation Mapper

## Objective

在G09范围内实现 **Cancellation Propagation Mapper**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：区分Local、Server、Global、Persistent和Derived State; 保持Effect触发、Cleanup、Lifecycle和Evaluation Order; 传播Cancellation并防止Stale Result Commit。

## When to Use

- 实现或修改与 **Cancellation Propagation Mapper** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-0906Result`：版本化、带状态和诊断的结构化结果。
- `FRT-0906EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-0906FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-0906CertificateFragment`：供G09批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g09/frt-0906-cancellation-propagation-mapper/
packages/runtime/g09/frt-0906-cancellation-propagation-mapper/
services/control-plane/g09/frt-0906-cancellation-propagation-mapper/
apps/web-console/src/features/g09/frt-0906-cancellation-propagation-mapper/
apps/admin-console/src/features/g09/frt-0906-cancellation-propagation-mapper/
tests/g09/frt-0906-cancellation-propagation-mapper/
```
...
```text
POST /v1/skills/frt-0906/runs
GET  /v1/skills/frt-0906/runs/{run_id}
POST /v1/skills/frt-0906/runs/{run_id}/verify
GET  /v1/skills/frt-0906/runs/{run_id}/findings
GET  /v1/skills/frt-0906/runs/{run_id}/evidence
```
