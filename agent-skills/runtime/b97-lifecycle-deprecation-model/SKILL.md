---
name: b97-lifecycle-deprecation-model
description: "Use when ELMOS must Implement proposed, experimental, supported, deprecated, retired and blocked lifecycle states with transition controls. Triggers: Lifecycle and Deprecation Model, Canonical Capability Graph Consolidation Pack, lifecycle states, transition policy, deprecation window, replacement linkage."
metadata:
  id: B97-S07
...
# B97-S07 — Lifecycle and Deprecation Model

## Objective

Implement proposed, experimental, supported, deprecated, retired and blocked lifecycle states with transition controls.
...
Use this Skill when the approved task explicitly requires **Lifecycle and Deprecation Model** or when the capability graph selects it as part of a minimal dependency closure.
...
- replacement links valid.
- retired Skills not selectable.
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b97-lifecycle-deprecation-model/SKILL.md --out /tmp/lifecycle_deprecation_model.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B97-S07
```
...
- Reject or expose reactivating retired Skill without approval.
- Reject or expose missing migration guide.
- Reject or expose expiry bypass.
