...
# Transaction Api Rewriter
## ELMOS runtime binding

- Invoke this repository Skill as `$pm-b27-transaction-api-rewriter`.
- Immutable source identity: `transaction-api-rewriter` in `precision-migration-b01-44` (B27).
- Runtime adapter: `database-and-data-route`; binding state: `DECLARED`.
- Resolve and plan with `python3 scripts/precision_migration/runtime.py plan --skill pm-b27-transaction-api-rewriter`.
...
迁移应用事务 API、传播、隔离、超时、重试和补偿。

本 Skill 属于精密语言转换系统。它必须以**可执行证据**为准，不得用模型自评、源码相似度或“能编译”替代语义与行为验证。

## Use when

- 当前任务直接涉及 `transaction-api-rewriter` 所描述的能力。
...
1. 盘点对象与专有能力
2. 执行本 Skill 的核心职责：迁移应用事务 API、传播、隔离、超时、重试和补偿。
...
```yaml
skill: transaction-api-rewriter
version: 1.0.0
