...
description: "Run FRT-1108 RTL and Bidirectional Text Mapper for FRT G11 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-1108"
  source_name: "frt-1108-rtl-and-bidirectional-text-mapper"
  source_sha256: "sha256:4f19bf229e70f54457eb3fecc1ad675a9ca42fed67c5ac12d0d181ec9b79b1ce"
  batch: "G11"
  source_version: "1.0.0"
  source_risk: "critical-by-default"
  source_certificate_family: "UI"
...
- Runtime catalog key: `FRT-1108` / `frt-1108-rtl-and-bidirectional-text-mapper`.
...
# FRT-1108 — RTL and Bidirectional Text Mapper

## Objective

在G11范围内实现 **RTL and Bidirectional Text Mapper**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：恢复布局约束、Design Token和响应式Size Class; 生成资产、字体、Icon、Theme和动画映射; 保持Locale、Plural、Date/Number和RTL语义。

## When to Use

- 实现或修改与 **RTL and Bidirectional Text Mapper** 对应的Contract、Runtime、API、UI、测试或证据链时。
- 相关代码、Schema、Pack、Policy、环境或依赖变化导致既有Evidence失效时。
- G11 Orchestrator生成该Skill的执行义务时。
...
- `SourceArtifactSet`：UI/Layout/Style IR、Design Assets、Locale Catalog、Target Platform Profile。
- `SkillPolicy`：权限、网络、Sandbox、Toolchain、Evidence和停止规则。

## Outputs

- `FRT-1108Result`：版本化、带状态和诊断的结构化结果。
- `FRT-1108EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-1108FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-1108CertificateFragment`：供G11批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g11/frt-1108-rtl-and-bidirectional-text-mapper/
packages/runtime/g11/frt-1108-rtl-and-bidirectional-text-mapper/
services/control-plane/g11/frt-1108-rtl-and-bidirectional-text-mapper/
apps/web-console/src/features/g11/frt-1108-rtl-and-bidirectional-text-mapper/
apps/admin-console/src/features/g11/frt-1108-rtl-and-bidirectional-text-mapper/
tests/g11/frt-1108-rtl-and-bidirectional-text-mapper/
```
...
```text
POST /v1/skills/frt-1108/runs
GET  /v1/skills/frt-1108/runs/{run_id}
POST /v1/skills/frt-1108/runs/{run_id}/verify
GET  /v1/skills/frt-1108/runs/{run_id}/findings
GET  /v1/skills/frt-1108/runs/{run_id}/evidence
```
...
- The parent G11 Orchestrator can consume the result and independently decide certification.
