...
description: "Run FRT-2021 Billing SLO and Observability for FRT G20 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-2021"
  source_name: "frt-2021-billing-slo-and-observability"
  source_sha256: "sha256:bd50ca9a681313728580d824185ddab04e5585780a3ca42e7140a4d61918d939"
  batch: "G20"
...
- Runtime catalog key: `FRT-2021` / `frt-2021-billing-slo-and-observability`.
...
# FRT-2021 — Billing SLO and Observability

## Objective

在G20范围内实现 **Billing SLO and Observability**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：提供Skill SDK、Runtime、Registry和Marketplace; 实现CLI、API、IDE、Web Console和Multi-Tenant Workspace; Durable Workflow与分布式Worker支持暂停、恢复、取消和幂等。

## When to Use

- 实现或修改与 **Billing SLO and Observability** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-2021Result`：版本化、带状态和诊断的结构化结果。
- `FRT-2021EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-2021FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-2021CertificateFragment`：供G20批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g20/frt-2021-billing-slo-and-observability/
packages/runtime/g20/frt-2021-billing-slo-and-observability/
services/control-plane/g20/frt-2021-billing-slo-and-observability/
apps/web-console/src/features/g20/frt-2021-billing-slo-and-observability/
apps/admin-console/src/features/g20/frt-2021-billing-slo-and-observability/
tests/g20/frt-2021-billing-slo-and-observability/
```
...
```text
POST /v1/skills/frt-2021/runs
GET  /v1/skills/frt-2021/runs/{run_id}
POST /v1/skills/frt-2021/runs/{run_id}/verify
GET  /v1/skills/frt-2021/runs/{run_id}/findings
GET  /v1/skills/frt-2021/runs/{run_id}/evidence
```
