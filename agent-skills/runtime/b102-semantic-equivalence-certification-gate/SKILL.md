---
name: b102-semantic-equivalence-certification-gate
description: "Use when ELMOS must Certify exact route and journey scopes only when authoritative oracles, representative tests and fresh evidence agree. Triggers: Semantic Equivalence Certification Gate, Semantic Equivalence Lab Pack, equivalence gate, journey scope, independent verifier, residual differences."
metadata:
  id: B102-S16
  ...
  category: "Semantic Equivalence Lab Pack"
---

# B102-S16 — Semantic Equivalence Certification Gate

## Objective

Certify exact route and journey scopes only when authoritative oracles, representative tests and fresh evidence agree.
...
The objective belongs to **Semantic Equivalence Lab Pack**, whose system-level purpose is to prove modernization and generation correctness using oracle governance, characterization, golden masters, differential execution, contracts, invariants, concurrency, performance, security, property testing, human review and evidence freshness.

## When to Use

Use this Skill when the approved task explicitly requires **Semantic Equivalence Certification Gate** or when the capability graph selects it as part of a minimal dependency closure.
...
- `equivalence_certification_result.json`
- `accepted_differences.json`
...
- Preferred implementation/tooling context: `test frameworks`, `differential harness`, `property/fuzz tools`, `benchmark tools`, `isolated services`. Alternatives are allowed only when their versions, behavior differences and rationale are recorded.
...
- differences explicitly accepted.
- no LLM-only oracle.
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b102-semantic-equivalence-certification-gate/SKILL.md --out /tmp/semantic_equivalence_certification_gate.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B102-S16
```
...
- Reject or expose averaged pass.
- Reject or expose unreviewed accepted difference.
- Reject or expose stale evidence.
