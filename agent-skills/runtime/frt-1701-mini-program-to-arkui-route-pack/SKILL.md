...
description: "Run FRT-1701 Mini Program to ArkUI Route Pack for FRT G17 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-1701"
  source_name: "frt-1701-mini-program-to-arkui-route-pack"
  source_sha256: "sha256:97e7b07e05a2ddffc5fdd79e161fef82ae2423e90f6bde348bd1141f9bdb14de"
  batch: "G17"
...
- Runtime catalog key: `FRT-1701` / `frt-1701-mini-program-to-arkui-route-pack`.
...
# FRT-1701 — Mini Program to ArkUI Route Pack

## Objective

在G17范围内实现 **Mini Program to ArkUI Route Pack**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：补齐小程序/ArkUI/Flutter六个方向; 统一移动Navigation、Lifecycle、Permission、Storage和Native能力; 验证直接与间接三角路径。

## When to Use

- 实现或修改与 **Mini Program to ArkUI Route Pack** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-1701Result`：版本化、带状态和诊断的结构化结果。
- `FRT-1701EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-1701FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-1701CertificateFragment`：供G17批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g17/frt-1701-mini-program-to-arkui-route-pack/
packages/runtime/g17/frt-1701-mini-program-to-arkui-route-pack/
services/control-plane/g17/frt-1701-mini-program-to-arkui-route-pack/
apps/web-console/src/features/g17/frt-1701-mini-program-to-arkui-route-pack/
apps/admin-console/src/features/g17/frt-1701-mini-program-to-arkui-route-pack/
tests/g17/frt-1701-mini-program-to-arkui-route-pack/
```
...
```text
POST /v1/skills/frt-1701/runs
GET  /v1/skills/frt-1701/runs/{run_id}
POST /v1/skills/frt-1701/runs/{run_id}/verify
GET  /v1/skills/frt-1701/runs/{run_id}/findings
GET  /v1/skills/frt-1701/runs/{run_id}/evidence
```
