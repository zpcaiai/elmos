...
description: "Run FRT-2218 Agent Run-to-Human-Approval Closure Pack for FRT G22 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-2218"
  source_name: "frt-2218-agent-run-to-human-approval-closure-pack"
  source_sha256: "sha256:9fb6caca3597ca37e41fa4593c7f8218abc2cc29290ad988cd271f254afcbbe6"
  batch: "G22"
...
- Runtime catalog key: `FRT-2218` / `frt-2218-agent-run-to-human-approval-closure-pack`.
...
在G22范围内实现 **Agent Run-to-Human-Approval Closure Pack**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：Value Stream 与业务Outcome建模; 状态机可达性、终态、Timeout、Cancel、Unknown、Recovery完整性; 跨域Saga、幂等、补偿和Reconciliation。

## When to Use

- 实现或修改与 **Agent Run-to-Human-Approval Closure Pack** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-2218Result`：版本化、带状态和诊断的结构化结果。
- `FRT-2218EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-2218FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-2218CertificateFragment`：供G22批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g22/frt-2218-agent-run-to-human-approval-closure-pack/
packages/runtime/g22/frt-2218-agent-run-to-human-approval-closure-pack/
services/control-plane/g22/frt-2218-agent-run-to-human-approval-closure-pack/
apps/web-console/src/features/g22/frt-2218-agent-run-to-human-approval-closure-pack/
apps/admin-console/src/features/g22/frt-2218-agent-run-to-human-approval-closure-pack/
tests/g22/frt-2218-agent-run-to-human-approval-closure-pack/
```
...
```text
POST /v1/skills/frt-2218/runs
GET  /v1/skills/frt-2218/runs/{run_id}
POST /v1/skills/frt-2218/runs/{run_id}/verify
GET  /v1/skills/frt-2218/runs/{run_id}/findings
GET  /v1/skills/frt-2218/runs/{run_id}/evidence
```
