...
description: "Run FRT-0111 Safety Constitution and Trust Boundary for FRT G01 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-0111"
  source_name: "frt-0111-safety-constitution-and-trust-boundary"
  source_sha256: "sha256:06865a8b8042364fdf58abcde71bc11e3c8459bc935288a3e127b5f8b8d970da"
  batch: "G01"
...
- Runtime catalog key: `FRT-0111` / `frt-0111-safety-constitution-and-trust-boundary`.
...
# FRT-0111 — Safety Constitution and Trust Boundary

## Objective

在G01范围内实现 **Safety Constitution and Trust Boundary**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：定义系统不可妥协语义与风险等级; 建立可扩展Monorepo与稳定模块边界; 统一Skill Manifest、输入输出Schema、权限、版本与证据协议。

## When to Use

- 实现或修改与 **Safety Constitution and Trust Boundary** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-0111Result`：版本化、带状态和诊断的结构化结果。
- `FRT-0111EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-0111FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-0111CertificateFragment`：供G01批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g01/frt-0111-safety-constitution-and-trust-boundary/
packages/runtime/g01/frt-0111-safety-constitution-and-trust-boundary/
services/control-plane/g01/frt-0111-safety-constitution-and-trust-boundary/
apps/web-console/src/features/g01/frt-0111-safety-constitution-and-trust-boundary/
apps/admin-console/src/features/g01/frt-0111-safety-constitution-and-trust-boundary/
tests/g01/frt-0111-safety-constitution-and-trust-boundary/
```
...
```text
POST /v1/skills/frt-0111/runs
GET  /v1/skills/frt-0111/runs/{run_id}
POST /v1/skills/frt-0111/runs/{run_id}/verify
GET  /v1/skills/frt-0111/runs/{run_id}/findings
GET  /v1/skills/frt-0111/runs/{run_id}/evidence
```
