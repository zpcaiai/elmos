...
- Invoke this repository Skill as `$pm-b36-byok-provider-adapter`.
- Immutable source identity: `byok-provider-adapter` in `precision-migration-b01-44` (B36).
- Runtime adapter: `model-routing-and-agent-harness`; binding state: `DECLARED`.
- Resolve and plan with `python3 scripts/precision_migration/runtime.py plan --skill pm-b36-byok-provider-adapter`.
...
支持客户自带 OpenAI、Anthropic、Gemini、国产或 OpenAI兼容模型凭证。

本 Skill 属于精密语言转换系统。它必须以**可执行证据**为准，不得用模型自评、源码相似度或“能编译”替代语义与行为验证。

## Use when

- 当前任务直接涉及 `byok-provider-adapter` 所描述的能力。
...
1. 计算风险并拆分任务
2. 执行本 Skill 的核心职责：支持客户自带 OpenAI、Anthropic、Gemini、国产或 OpenAI兼容模型凭证。
3. 选择模型与隔离工作区
4. 执行并收集客观反馈
5. 独立审查和升级
6. 持久化状态、证据、成本与审批
