---
name: b97-capability-graph-certification-gate
description: "Use when ELMOS must Certify the canonical graph only when inventory, ontology, identity, dependencies, conflicts, ownership, compatibility and evidence requirements are complete. Triggers: Capability Graph Certification Gate, Canonical Capability Graph Consolidation Pack, graph gate, anti-fabrication, holdout queries, signed snapshot."
metadata:
  id: B97-S16
...
# B97-S16 — Capability Graph Certification Gate

## Objective

Certify the canonical graph only when inventory, ontology, identity, dependencies, conflicts, ownership, compatibility and evidence requirements are complete.
...
Use this Skill when the approved task explicitly requires **Capability Graph Certification Gate** or when the capability graph selects it as part of a minimal dependency closure.
...
- `graph_certification_result.json`
- `signed_graph_snapshot.json`
...
- no unresolved P0 graph defects.
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b97-capability-graph-certification-gate/SKILL.md --out /tmp/capability_graph_certification_gate.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B97-S16
```
...
- Reject or expose fabricated certified state.
- Reject or expose unresolved cycle waiver.
- Reject or expose unsigned graph snapshot.
