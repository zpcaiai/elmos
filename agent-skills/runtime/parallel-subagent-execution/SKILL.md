...
description: 把互不依赖的搜索、分析、测试任务并行化，但结果确定性归并。 Use for this exact ELMOS Spring Golden Route v2 contract; imported specifications are not runtime or certification evidence.
...
metadata:
  ...
  source_id: FOUNDATION-06-parallel-subagent-execution
  source_name: parallel-subagent-execution
  source_path: skills/parallel-subagent-execution/SKILL.md
  source_sha256: sha256:fb15bbbeb49e997b7d2f731bc8adb2d603a3b16e8c7dee233e12fbe5e4a44bd3
  source_contract_sha256: sha256:1789a2bac805c037dcb11019ce50b1ab1b7023afef6f8eabf2aab9f0f9678218
...
把互不依赖的搜索、分析、测试任务并行化，但结果确定性归并。

## Use when

在 Elmos 需要实现或升级 **Parallel Subagent Execution** 相关能力时使用；它不是仅供解释的提示词，而是实现、测试与验收契约。

## Capabilities to implement

- Planner 显式标记 dependency-free subtasks
- 并发有租户/项目/模型/工具多级上限
- 结果按 task ID/依赖拓扑稳定合并
- 任何共享副作用必须串行或幂等
...
- [ ] 定义 `parallel-subagent-execution` 的 public interface、input/output/error schemas。
- [ ] 建立 `parallel-subagent-execution` 的 capability/feature flags 与版本字段。
...
- [ ] 至少覆盖并验收：Planner 显式标记 dependency-free subtasks；以及 并发有租户/项目/模型/工具多级上限。
...
- Machine contract: `references/contract.json` (`sha256:1789a2bac805c037dcb11019ce50b1ab1b7023afef6f8eabf2aab9f0f9678218`).
