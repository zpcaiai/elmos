...
description: "Run FRT-2712 Cache Performance and Stampede Validator for FRT G27 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-2712"
  source_name: "frt-2712-cache-performance-and-stampede-validator"
  source_sha256: "sha256:efd5b55831237ae4da57de5f03306ec2bdb29ea984d15ff5c788e42938caef4c"
  batch: "G27"
...
- Runtime catalog key: `FRT-2712` / `frt-2712-cache-performance-and-stampede-validator`.
...
在G27范围内实现 **Cache Performance and Stampede Validator**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：业务化Workload Model; 并发线性化点、Race、Lost Update、Idempotency Under Load; Load/Stress/Spike/Soak与Recovery。

## When to Use

- 实现或修改与 **Cache Performance and Stampede Validator** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-2712Result`：版本化、带状态和诊断的结构化结果。
- `FRT-2712EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-2712FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-2712CertificateFragment`：供G27批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g27/frt-2712-cache-performance-and-stampede-validator/
packages/runtime/g27/frt-2712-cache-performance-and-stampede-validator/
services/control-plane/g27/frt-2712-cache-performance-and-stampede-validator/
apps/web-console/src/features/g27/frt-2712-cache-performance-and-stampede-validator/
apps/admin-console/src/features/g27/frt-2712-cache-performance-and-stampede-validator/
tests/g27/frt-2712-cache-performance-and-stampede-validator/
```
...
```text
POST /v1/skills/frt-2712/runs
GET  /v1/skills/frt-2712/runs/{run_id}
POST /v1/skills/frt-2712/runs/{run_id}/verify
GET  /v1/skills/frt-2712/runs/{run_id}/findings
GET  /v1/skills/frt-2712/runs/{run_id}/evidence
```
