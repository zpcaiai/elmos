...
description: "Run FRT-2311 Data Repair Workflow for FRT G23 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-2311"
  source_name: "frt-2311-data-repair-workflow"
  source_sha256: "sha256:141b7cd6f2c07be76a46884260e0ef4a8a3cc0743d2050fefcee1be2490d18dc"
  batch: "G23"
...
- Runtime catalog key: `FRT-2311` / `frt-2311-data-repair-workflow`.
...
在G23范围内实现 **Data Repair Workflow**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：字段级Data Contract、Authority和Owner; Source-to-Sink与字段级Lineage; Write→Transaction→Outbox→Event→Projection→Cache→Read完整路径。

## When to Use

- 实现或修改与 **Data Repair Workflow** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-2311Result`：版本化、带状态和诊断的结构化结果。
- `FRT-2311EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-2311FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-2311CertificateFragment`：供G23批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g23/frt-2311-data-repair-workflow/
packages/runtime/g23/frt-2311-data-repair-workflow/
services/control-plane/g23/frt-2311-data-repair-workflow/
apps/web-console/src/features/g23/frt-2311-data-repair-workflow/
apps/admin-console/src/features/g23/frt-2311-data-repair-workflow/
tests/g23/frt-2311-data-repair-workflow/
```
...
```text
POST /v1/skills/frt-2311/runs
GET  /v1/skills/frt-2311/runs/{run_id}
POST /v1/skills/frt-2311/runs/{run_id}/verify
GET  /v1/skills/frt-2311/runs/{run_id}/findings
GET  /v1/skills/frt-2311/runs/{run_id}/evidence
```
