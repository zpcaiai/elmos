...
description: "Run FRT-0202 Framework and Version Detector for FRT G02 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-0202"
  source_name: "frt-0202-framework-and-version-detector"
  source_sha256: "sha256:6f8426047c39870ea90dc479e7ea21997bf1d21204ea0c9c5d04dfb3d89d4547"
  batch: "G02"
...
- Runtime catalog key: `FRT-0202` / `frt-0202-framework-and-version-detector`.
...
# FRT-0202 — Framework and Version Detector

## Objective

在G02范围内实现 **Framework and Version Detector**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：识别Monorepo、Package、Framework、Runtime和Build链; 建立模块、依赖、路由、组件、状态、API、存储和资产清单; 检测动态代码、原生能力、平台限制和迁移风险。

## When to Use

- 实现或修改与 **Framework and Version Detector** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-0202Result`：版本化、带状态和诊断的结构化结果。
- `FRT-0202EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-0202FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-0202CertificateFragment`：供G02批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g02/frt-0202-framework-and-version-detector/
packages/runtime/g02/frt-0202-framework-and-version-detector/
services/control-plane/g02/frt-0202-framework-and-version-detector/
apps/web-console/src/features/g02/frt-0202-framework-and-version-detector/
apps/admin-console/src/features/g02/frt-0202-framework-and-version-detector/
tests/g02/frt-0202-framework-and-version-detector/
```
...
```text
POST /v1/skills/frt-0202/runs
GET  /v1/skills/frt-0202/runs/{run_id}
POST /v1/skills/frt-0202/runs/{run_id}/verify
GET  /v1/skills/frt-0202/runs/{run_id}/findings
GET  /v1/skills/frt-0202/runs/{run_id}/evidence
```
