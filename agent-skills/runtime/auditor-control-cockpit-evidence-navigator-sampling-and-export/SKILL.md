---
name: auditor-control-cockpit-evidence-navigator-sampling-and-export
description: 实现Framework→Control→Implementation→Assessment→Evidence→Finding→POA&M导航、抽样和不可变导出。
metadata:
  elmos_batch: "37C"
  elmos_skill_id: "ANA37C012"
  pack_version: "3.7-complete"
---

# ANA37C012 — Auditor Control Cockpit、Evidence Navigator、Sampling 与 Export

## Objective

实现Framework→Control→Implementation→Assessment→Evidence→Finding→POA&M导航、抽样和不可变导出。
...
- Control View显示Text/Version、Applicability、Responsibility、Implementation、Requirements、Evidence、Verification、Assessment、Freshness、Findings、POA&M。
- Evidence Navigator显示Digest、Producer、Subject、Scope、Commit、Retention和Lineage。
- Sampling支持Random/Stratified/Risk-based/Systematic并保存Seed和Population。
- Auditor Note是独立Claim，不修改Assessment；Export为Scoped Evidence Pack。
...
- auditorSeesOnlyEngagementScope
- sampleSeedReproducesSamePopulationSelection
- auditorNoteDoesNotModifyAssessment
- exportPreservesControlEvidenceLinks
