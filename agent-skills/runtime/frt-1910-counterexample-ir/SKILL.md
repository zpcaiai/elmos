...
description: "Run FRT-1910 Counterexample IR for FRT G19 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-1910"
  source_name: "frt-1910-counterexample-ir"
  source_sha256: "sha256:03244168df0c43514531b63dd3cc6085310b8ae448f404469216c9ab0816c1e3"
  batch: "G19"
...
- Runtime catalog key: `FRT-1910` / `frt-1910-counterexample-ir`.
...
# FRT-1910 — Counterexample IR

## Objective

在G19范围内实现 **Counterexample IR**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：从IR、Rule和业务不变量生成Proof Obligation; Leanstral仅提出证明候选，Lean Kernel为可信根; 组合SMT、Model Checking、Symbolic和Differential Evidence。

## When to Use

- 实现或修改与 **Counterexample IR** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-1910Result`：版本化、带状态和诊断的结构化结果。
- `FRT-1910EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-1910FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-1910CertificateFragment`：供G19批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g19/frt-1910-counterexample-ir/
packages/runtime/g19/frt-1910-counterexample-ir/
services/control-plane/g19/frt-1910-counterexample-ir/
apps/web-console/src/features/g19/frt-1910-counterexample-ir/
apps/admin-console/src/features/g19/frt-1910-counterexample-ir/
tests/g19/frt-1910-counterexample-ir/
```
...
```text
POST /v1/skills/frt-1910/runs
GET  /v1/skills/frt-1910/runs/{run_id}
POST /v1/skills/frt-1910/runs/{run_id}/verify
GET  /v1/skills/frt-1910/runs/{run_id}/findings
GET  /v1/skills/frt-1910/runs/{run_id}/evidence
```
