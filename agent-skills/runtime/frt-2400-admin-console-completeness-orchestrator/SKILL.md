...
description: "Run FRT-2400 Admin Console Completeness Orchestrator for FRT G24 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-2400"
  source_name: "frt-2400-admin-console-completeness-orchestrator"
  source_sha256: "sha256:cef408c1dd6bc0c5a0ee528b509e11e543b27a3e6e5a0733106aeda13517c863"
  batch: "G24"
...
- Runtime catalog key: `FRT-2400` / `frt-2400-admin-console-completeness-orchestrator`.
...
在G24范围内实现 **Admin Console Completeness Orchestrator**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：Admin Object × Action × Role × Scope × Environment矩阵; 高风险操作Impact Preview、Reason、Approval、Typed Command、Audit; Manual Case、Data Repair、Reconciliation正式后台流程。

## When to Use

- 实现或修改与 **Admin Console Completeness Orchestrator** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-2400Result`：版本化、带状态和诊断的结构化结果。
- `FRT-2400EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-2400FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-2400CertificateFragment`：供G24批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g24/frt-2400-admin-console-completeness-orchestrator/
packages/runtime/g24/frt-2400-admin-console-completeness-orchestrator/
services/control-plane/g24/frt-2400-admin-console-completeness-orchestrator/
apps/web-console/src/features/g24/frt-2400-admin-console-completeness-orchestrator/
apps/admin-console/src/features/g24/frt-2400-admin-console-completeness-orchestrator/
tests/g24/frt-2400-admin-console-completeness-orchestrator/
```
...
```text
POST /v1/skills/frt-2400/runs
GET  /v1/skills/frt-2400/runs/{run_id}
POST /v1/skills/frt-2400/runs/{run_id}/verify
GET  /v1/skills/frt-2400/runs/{run_id}/findings
GET  /v1/skills/frt-2400/runs/{run_id}/evidence
```
