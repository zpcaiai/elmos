...
description: "Run FRT-0104 Plugin and Extension Protocol for FRT G01 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-0104"
  source_name: "frt-0104-plugin-and-extension-protocol"
  source_sha256: "sha256:607040fe424e9e97507bc2883a612ae0213355193a791714935f428b2efa796c"
  batch: "G01"
...
- Runtime catalog key: `FRT-0104` / `frt-0104-plugin-and-extension-protocol`.
...
# FRT-0104 — Plugin and Extension Protocol

## Objective

在G01范围内实现 **Plugin and Extension Protocol**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：定义系统不可妥协语义与风险等级; 建立可扩展Monorepo与稳定模块边界; 统一Skill Manifest、输入输出Schema、权限、版本与证据协议。

## When to Use

- 实现或修改与 **Plugin and Extension Protocol** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-0104Result`：版本化、带状态和诊断的结构化结果。
- `FRT-0104EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-0104FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-0104CertificateFragment`：供G01批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g01/frt-0104-plugin-and-extension-protocol/
packages/runtime/g01/frt-0104-plugin-and-extension-protocol/
services/control-plane/g01/frt-0104-plugin-and-extension-protocol/
apps/web-console/src/features/g01/frt-0104-plugin-and-extension-protocol/
apps/admin-console/src/features/g01/frt-0104-plugin-and-extension-protocol/
tests/g01/frt-0104-plugin-and-extension-protocol/
```
...
```text
POST /v1/skills/frt-0104/runs
GET  /v1/skills/frt-0104/runs/{run_id}
POST /v1/skills/frt-0104/runs/{run_id}/verify
GET  /v1/skills/frt-0104/runs/{run_id}/findings
GET  /v1/skills/frt-0104/runs/{run_id}/evidence
```
