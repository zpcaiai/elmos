...
description: "Run FRT-1700 Mobile Triangle Route Orchestrator for FRT G17 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-1700"
  source_name: "frt-1700-mobile-triangle-route-orchestrator"
  source_sha256: "sha256:4978267144c6e9f5065f5dd582b3cd2bede70614b91c51cfb7cff0cf9f4d4864"
  batch: "G17"
...
- Runtime catalog key: `FRT-1700` / `frt-1700-mobile-triangle-route-orchestrator`.
...
# FRT-1700 — Mobile Triangle Route Orchestrator

## Objective

在G17范围内实现 **Mobile Triangle Route Orchestrator**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：补齐小程序/ArkUI/Flutter六个方向; 统一移动Navigation、Lifecycle、Permission、Storage和Native能力; 验证直接与间接三角路径。

## When to Use

- 实现或修改与 **Mobile Triangle Route Orchestrator** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-1700Result`：版本化、带状态和诊断的结构化结果。
- `FRT-1700EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-1700FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-1700CertificateFragment`：供G17批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g17/frt-1700-mobile-triangle-route-orchestrator/
packages/runtime/g17/frt-1700-mobile-triangle-route-orchestrator/
services/control-plane/g17/frt-1700-mobile-triangle-route-orchestrator/
apps/web-console/src/features/g17/frt-1700-mobile-triangle-route-orchestrator/
apps/admin-console/src/features/g17/frt-1700-mobile-triangle-route-orchestrator/
tests/g17/frt-1700-mobile-triangle-route-orchestrator/
```
...
```text
POST /v1/skills/frt-1700/runs
GET  /v1/skills/frt-1700/runs/{run_id}
POST /v1/skills/frt-1700/runs/{run_id}/verify
GET  /v1/skills/frt-1700/runs/{run_id}/findings
GET  /v1/skills/frt-1700/runs/{run_id}/evidence
```
