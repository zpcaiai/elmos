...
# Repository And Orm Ir
## ELMOS runtime binding

- Invoke this repository Skill as `$pm-b10-repository-and-orm-ir`.
- Immutable source identity: `repository-and-orm-ir` in `precision-migration-b01-44` (B10).
- Runtime adapter: `semantic-recovery-and-ir`; binding state: `UNAVAILABLE`.
- Resolve and plan with `python3 scripts/precision_migration/runtime.py plan --skill pm-b10-repository-and-orm-ir`.
...
- Phase: `C 精密语义表示`
- Version: `1.0.0`
- Tags: backend-frontend-semantic-ir, batch-10

## Purpose

表达实体、映射、查询、Tracking、Lazy load、事务、并发控制和迁移。

本 Skill 属于精密语言转换系统。它必须以**可执行证据**为准，不得用模型自评、源码相似度或“能编译”替代语义与行为验证。

## Use when

- 当前任务直接涉及 `repository-and-orm-ir` 所描述的能力。
...
1. 定义可观察边界
2. 执行本 Skill 的核心职责：表达实体、映射、查询、Tracking、Lazy load、事务、并发控制和迁移。
3. 将源语义归一化到受限模型
4. 保留不可丢失信息
5. 标记近似与不支持语义
