...
description: "Run FRT-1800 Pack Composition Orchestrator for FRT G18 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-1800"
  source_name: "frt-1800-pack-composition-orchestrator"
  source_sha256: "sha256:a215a79aa5c9a89360159bfb2bc3eabed30d7a67beaa00428fb8b8a2afd0ebf6"
  batch: "G18"
...
- Runtime catalog key: `FRT-1800` / `frt-1800-pack-composition-orchestrator`.
...
# FRT-1800 — Pack Composition Orchestrator

## Objective

在G18范围内实现 **Pack Composition Orchestrator**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：将真实项目差异封装为可组合Overlay而非硬编码核心; Pack声明Provides/Requires/Conflicts/Permissions/Versions; 确定性解析优先级和冲突。

## When to Use

- 实现或修改与 **Pack Composition Orchestrator** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-1800Result`：版本化、带状态和诊断的结构化结果。
- `FRT-1800EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-1800FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-1800CertificateFragment`：供G18批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g18/frt-1800-pack-composition-orchestrator/
packages/runtime/g18/frt-1800-pack-composition-orchestrator/
services/control-plane/g18/frt-1800-pack-composition-orchestrator/
apps/web-console/src/features/g18/frt-1800-pack-composition-orchestrator/
apps/admin-console/src/features/g18/frt-1800-pack-composition-orchestrator/
tests/g18/frt-1800-pack-composition-orchestrator/
```
...
```text
POST /v1/skills/frt-1800/runs
GET  /v1/skills/frt-1800/runs/{run_id}
POST /v1/skills/frt-1800/runs/{run_id}/verify
GET  /v1/skills/frt-1800/runs/{run_id}/findings
GET  /v1/skills/frt-1800/runs/{run_id}/evidence
```
