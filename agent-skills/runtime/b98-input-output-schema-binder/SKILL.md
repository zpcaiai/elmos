---
name: b98-input-output-schema-binder
description: "Use when ELMOS must Bind every input and output to exact schemas, content types, ownership and retention rules. Triggers: Input and Output Schema Binder, Executable Skill Contract and Registry Pack, schema binding, artifact typing, content validation, retention."
metadata:
  id: B98-S03
...
# B98-S03 — Input and Output Schema Binder

## Objective

Bind every input and output to exact schemas, content types, ownership and retention rules.
...
Use this Skill when the approved task explicitly requires **Input and Output Schema Binder** or when the capability graph selects it as part of a minimal dependency closure.
...
- all consequential IO typed.
- schema versions pinned.
- sensitive outputs classified.
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b98-input-output-schema-binder/SKILL.md --out /tmp/input_output_schema_binder.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B98-S03
```
...
- Reject or expose schema substitution.
- Reject or expose content-type confusion.
- Reject or expose unbounded artifact size.
