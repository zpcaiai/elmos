---
name: b101-java-route-verification-pr
description: "Use when ELMOS must Build, test, run differential journeys, generate an evidence-backed PR and prove rollback for the Java route. Triggers: Java Route Verification and PR, Three Golden Route Packs, Java verification, PR, evidence pack, rollback."
metadata:
  id: B101-S06
...
# B101-S06 — Java Route Verification and PR

## Objective

Build, test, run differential journeys, generate an evidence-backed PR and prove rollback for the Java route.
...
Use this Skill when the approved task explicitly requires **Java Route Verification and PR** or when the capability graph selects it as part of a minimal dependency closure.
...
- business journeys compared.
- PR traceability complete.
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b101-java-route-verification-pr/SKILL.md --out /tmp/java_route_verification_pr.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B101-S06
```
...
- Reject or expose tests weakened.
- Reject or expose PR based on dirty snapshot.
- Reject or expose rollback not executable.
