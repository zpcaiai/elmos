...
description: "Run FRT-1600 Web Flutter Route Orchestrator for FRT G16 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-1600"
  source_name: "frt-1600-web-flutter-route-orchestrator"
  source_sha256: "sha256:e14e0aab63587005276a2abbebe48ebd37713ee8127f7895cc940a0fa2245341"
  batch: "G16"
...
- Runtime catalog key: `FRT-1600` / `frt-1600-web-flutter-route-orchestrator`.
...
# FRT-1600 — Web Flutter Route Orchestrator

## Objective

在G16范围内实现 **Web Flutter Route Orchestrator**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：映射DOM/组件到Widget树而非逐元素复制; 处理Key、BuildContext、Controller、State和Navigator; 处理Plugin、Platform Channel、Web/Mobile能力差异。

## When to Use

- 实现或修改与 **Web Flutter Route Orchestrator** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-1600Result`：版本化、带状态和诊断的结构化结果。
- `FRT-1600EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-1600FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-1600CertificateFragment`：供G16批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g16/frt-1600-web-flutter-route-orchestrator/
packages/runtime/g16/frt-1600-web-flutter-route-orchestrator/
services/control-plane/g16/frt-1600-web-flutter-route-orchestrator/
apps/web-console/src/features/g16/frt-1600-web-flutter-route-orchestrator/
apps/admin-console/src/features/g16/frt-1600-web-flutter-route-orchestrator/
tests/g16/frt-1600-web-flutter-route-orchestrator/
```
...
```text
POST /v1/skills/frt-1600/runs
GET  /v1/skills/frt-1600/runs/{run_id}
POST /v1/skills/frt-1600/runs/{run_id}/verify
GET  /v1/skills/frt-1600/runs/{run_id}/findings
GET  /v1/skills/frt-1600/runs/{run_id}/evidence
```
