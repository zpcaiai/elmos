---
name: b98-contract-compatibility-deprecation
description: "Use when ELMOS must Validate contract schema evolution, behavior compatibility, replacement paths and deprecation windows. Triggers: Contract Compatibility and Deprecation, Executable Skill Contract and Registry Pack, schema evolution, behavior compatibility, replacement, deprecation."
metadata:
  id: B98-S15
...
# B98-S15 — Contract Compatibility and Deprecation

## Objective

Validate contract schema evolution, behavior compatibility, replacement paths and deprecation windows.
...
Use this Skill when the approved task explicitly requires **Contract Compatibility and Deprecation** or when the capability graph selects it as part of a minimal dependency closure.
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b98-contract-compatibility-deprecation/SKILL.md --out /tmp/contract_compatibility_deprecation.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B98-S15
```
...
- Reject or expose silent breaking change.
- Reject or expose expired compatibility waiver.
- Reject or expose replacement loop.
