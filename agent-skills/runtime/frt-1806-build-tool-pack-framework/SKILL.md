...
description: "Run FRT-1806 Build Tool Pack Framework for FRT G18 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-1806"
  source_name: "frt-1806-build-tool-pack-framework"
  source_sha256: "sha256:1552c1581a5b1023f94a42a67e5b7de5b1a07eabd28f76afce52128f76227cd1"
  batch: "G18"
...
- Runtime catalog key: `FRT-1806` / `frt-1806-build-tool-pack-framework`.
...
# FRT-1806 — Build Tool Pack Framework

## Objective

在G18范围内实现 **Build Tool Pack Framework**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：将真实项目差异封装为可组合Overlay而非硬编码核心; Pack声明Provides/Requires/Conflicts/Permissions/Versions; 确定性解析优先级和冲突。

## When to Use

- 实现或修改与 **Build Tool Pack Framework** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-1806Result`：版本化、带状态和诊断的结构化结果。
- `FRT-1806EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-1806FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-1806CertificateFragment`：供G18批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g18/frt-1806-build-tool-pack-framework/
packages/runtime/g18/frt-1806-build-tool-pack-framework/
services/control-plane/g18/frt-1806-build-tool-pack-framework/
apps/web-console/src/features/g18/frt-1806-build-tool-pack-framework/
apps/admin-console/src/features/g18/frt-1806-build-tool-pack-framework/
tests/g18/frt-1806-build-tool-pack-framework/
```
...
```text
POST /v1/skills/frt-1806/runs
GET  /v1/skills/frt-1806/runs/{run_id}
POST /v1/skills/frt-1806/runs/{run_id}/verify
GET  /v1/skills/frt-1806/runs/{run_id}/findings
GET  /v1/skills/frt-1806/runs/{run_id}/evidence
```
