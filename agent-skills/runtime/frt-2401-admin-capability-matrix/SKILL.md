...
description: "Run FRT-2401 Admin Capability Matrix for FRT G24 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-2401"
  source_name: "frt-2401-admin-capability-matrix"
  source_sha256: "sha256:5986b931debdc5b59b3449107140e82d021d044eb888de5f110f848aed169fa7"
  batch: "G24"
...
- Runtime catalog key: `FRT-2401` / `frt-2401-admin-capability-matrix`.
...
在G24范围内实现 **Admin Capability Matrix**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：Admin Object × Action × Role × Scope × Environment矩阵; 高风险操作Impact Preview、Reason、Approval、Typed Command、Audit; Manual Case、Data Repair、Reconciliation正式后台流程。

## When to Use

- 实现或修改与 **Admin Capability Matrix** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-2401Result`：版本化、带状态和诊断的结构化结果。
- `FRT-2401EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-2401FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-2401CertificateFragment`：供G24批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g24/frt-2401-admin-capability-matrix/
packages/runtime/g24/frt-2401-admin-capability-matrix/
services/control-plane/g24/frt-2401-admin-capability-matrix/
apps/web-console/src/features/g24/frt-2401-admin-capability-matrix/
apps/admin-console/src/features/g24/frt-2401-admin-capability-matrix/
tests/g24/frt-2401-admin-capability-matrix/
```
...
```text
POST /v1/skills/frt-2401/runs
GET  /v1/skills/frt-2401/runs/{run_id}
POST /v1/skills/frt-2401/runs/{run_id}/verify
GET  /v1/skills/frt-2401/runs/{run_id}/findings
GET  /v1/skills/frt-2401/runs/{run_id}/evidence
```
