...
description: "Run FRT-2706 Load Test Coordinator for FRT G27 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-2706"
  source_name: "frt-2706-load-test-coordinator"
  source_sha256: "sha256:0cd957a43803ae50d10472c1636789520f165baf462f2baa768bf64be08c6cf5"
  batch: "G27"
...
- Runtime catalog key: `FRT-2706` / `frt-2706-load-test-coordinator`.
...
在G27范围内实现 **Load Test Coordinator**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：业务化Workload Model; 并发线性化点、Race、Lost Update、Idempotency Under Load; Load/Stress/Spike/Soak与Recovery。

## When to Use

- 实现或修改与 **Load Test Coordinator** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-2706Result`：版本化、带状态和诊断的结构化结果。
- `FRT-2706EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-2706FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-2706CertificateFragment`：供G27批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g27/frt-2706-load-test-coordinator/
packages/runtime/g27/frt-2706-load-test-coordinator/
services/control-plane/g27/frt-2706-load-test-coordinator/
apps/web-console/src/features/g27/frt-2706-load-test-coordinator/
apps/admin-console/src/features/g27/frt-2706-load-test-coordinator/
tests/g27/frt-2706-load-test-coordinator/
```
...
```text
POST /v1/skills/frt-2706/runs
GET  /v1/skills/frt-2706/runs/{run_id}
POST /v1/skills/frt-2706/runs/{run_id}/verify
GET  /v1/skills/frt-2706/runs/{run_id}/findings
GET  /v1/skills/frt-2706/runs/{run_id}/evidence
```
