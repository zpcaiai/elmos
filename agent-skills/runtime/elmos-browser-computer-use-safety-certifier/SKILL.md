---
name: elmos-browser-computer-use-safety-certifier
description: Certify perception, action bounding, confirmation, domain allowlists, sensitive-field handling, rollback and visual evidence for browser/computer-use agents.
version: 4.0.0
...
# Browser and Computer-Use Safety Certifier
...
- `BrowserAndComputerUseSafetyCertifierProfile`
- `BrowserAndComputerUseSafetyCertifierPlan`
- `BrowserAndComputerUseSafetyCertifierRun`
- `BrowserAndComputerUseSafetyCertifierDecision`

Required semantic capabilities:

- classify read, write, destructive and open-world actions
- bind action coordinates/selectors to fresh observations
- require confirmation for high-risk effects
- test prompt injection and UI deception
- record replayable screenshots/action traces with redaction
...
- `elmos-ai-prompt-tool-security-redteam` from `elmos-ai-capability-enhancement-skills` — required base evidence producer.
- `elmos-human-oversight-consent-contract-compiler` from `elmos-ai-capability-enhancement-skills` — required base evidence producer.

## Primary Outputs

- `artifacts/elmos-browser-computer-use-safety-certifier/profile.yaml`
- `artifacts/elmos-browser-computer-use-safety-certifier/plan.json`
- `artifacts/elmos-browser-computer-use-safety-certifier/result.json`
- `artifacts/elmos-browser-computer-use-safety-certifier/evidence/`
...
- **stale screenshot causes wrong click** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **hidden overlay captures credentials** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **agent bypasses confirmation** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **website content injects policy** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **visual evidence exposes PII** — must have a preventive control, a detective signal, a negative fixture and a response action.
...
- native scenario: classify read, write, destructive and open-world actions
- native scenario: bind action coordinates/selectors to fresh observations
- native scenario: require confirmation for high-risk effects
- native scenario: test prompt injection and UI deception
- native scenario: record replayable screenshots/action traces with redaction
