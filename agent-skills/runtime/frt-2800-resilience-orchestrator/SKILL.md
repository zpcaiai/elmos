...
description: "Run FRT-2800 Resilience Orchestrator for FRT G28 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-2800"
  source_name: "frt-2800-resilience-orchestrator"
  source_sha256: "sha256:44a15c2e7b69da4b6639f38d6ed61d4759816f6bd2368ca1012ccf24ed433336"
  batch: "G28"
...
- Runtime catalog key: `FRT-2800` / `frt-2800-resilience-orchestrator`.
...
在G28范围内实现 **Resilience Orchestrator**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：Dependency Failure Model与Common Cause分析; 端到端Deadline、Retry Budget、Circuit Breaker、Bulkhead; Queue Durability、Replay、Stateful Failover、Fence Token。

## When to Use

- 实现或修改与 **Resilience Orchestrator** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-2800Result`：版本化、带状态和诊断的结构化结果。
- `FRT-2800EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-2800FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-2800CertificateFragment`：供G28批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g28/frt-2800-resilience-orchestrator/
packages/runtime/g28/frt-2800-resilience-orchestrator/
services/control-plane/g28/frt-2800-resilience-orchestrator/
apps/web-console/src/features/g28/frt-2800-resilience-orchestrator/
apps/admin-console/src/features/g28/frt-2800-resilience-orchestrator/
tests/g28/frt-2800-resilience-orchestrator/
```
...
```text
POST /v1/skills/frt-2800/runs
GET  /v1/skills/frt-2800/runs/{run_id}
POST /v1/skills/frt-2800/runs/{run_id}/verify
GET  /v1/skills/frt-2800/runs/{run_id}/findings
GET  /v1/skills/frt-2800/runs/{run_id}/evidence
```
