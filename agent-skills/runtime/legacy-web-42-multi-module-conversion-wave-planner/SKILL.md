...
---
id: 42-multi-module-conversion-wave-planner
title: 多模块转换波次规划
version: 1.0.0
phase: planning
priority: P0
...
按模块 DAG、路由簇、数据库边界、共享库和风险切分可独立构建、验证、回滚的 transformation units。
...
producerSkill: 42-multi-module-conversion-wave-planner
...
5. 输出允许差异和回滚设计。
6. 以 SCC 压缩模块 DAG，识别必须同批转换的循环依赖。
7. 按 route cluster、共享 session、DB transaction 和 view dependency 形成 transformation unit。
8. 优先低风险 leaf 与基础 compatibility module，核心写路径后置。
9. 每波都必须可构建、可测试、可回滚。
