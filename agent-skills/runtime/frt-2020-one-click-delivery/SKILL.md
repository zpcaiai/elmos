...
description: "Run FRT-2020 One-Click Delivery for FRT G20 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-2020"
  source_name: "frt-2020-one-click-delivery"
  source_sha256: "sha256:7bfcb475eeecb765a5834030e504938470e0dc3919aa3b145d7d15f0d11f34dd"
  batch: "G20"
...
- Runtime catalog key: `FRT-2020` / `frt-2020-one-click-delivery`.
...
# FRT-2020 — One-Click Delivery

## Objective

在G20范围内实现 **One-Click Delivery**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：提供Skill SDK、Runtime、Registry和Marketplace; 实现CLI、API、IDE、Web Console和Multi-Tenant Workspace; Durable Workflow与分布式Worker支持暂停、恢复、取消和幂等。

## When to Use

- 实现或修改与 **One-Click Delivery** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-2020Result`：版本化、带状态和诊断的结构化结果。
- `FRT-2020EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-2020FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-2020CertificateFragment`：供G20批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g20/frt-2020-one-click-delivery/
packages/runtime/g20/frt-2020-one-click-delivery/
services/control-plane/g20/frt-2020-one-click-delivery/
apps/web-console/src/features/g20/frt-2020-one-click-delivery/
apps/admin-console/src/features/g20/frt-2020-one-click-delivery/
tests/g20/frt-2020-one-click-delivery/
```
...
```text
POST /v1/skills/frt-2020/runs
GET  /v1/skills/frt-2020/runs/{run_id}
POST /v1/skills/frt-2020/runs/{run_id}/verify
GET  /v1/skills/frt-2020/runs/{run_id}/findings
GET  /v1/skills/frt-2020/runs/{run_id}/evidence
```
