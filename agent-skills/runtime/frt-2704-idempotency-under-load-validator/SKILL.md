...
description: "Run FRT-2704 Idempotency Under Load Validator for FRT G27 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-2704"
  source_name: "frt-2704-idempotency-under-load-validator"
  source_sha256: "sha256:6e2cb1df99193eaa96663309058b8b5e9000fb83b5a8e6e448e25eb6cd05be86"
  batch: "G27"
...
- Runtime catalog key: `FRT-2704` / `frt-2704-idempotency-under-load-validator`.
...
在G27范围内实现 **Idempotency Under Load Validator**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：业务化Workload Model; 并发线性化点、Race、Lost Update、Idempotency Under Load; Load/Stress/Spike/Soak与Recovery。

## When to Use

- 实现或修改与 **Idempotency Under Load Validator** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-2704Result`：版本化、带状态和诊断的结构化结果。
- `FRT-2704EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-2704FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-2704CertificateFragment`：供G27批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g27/frt-2704-idempotency-under-load-validator/
packages/runtime/g27/frt-2704-idempotency-under-load-validator/
services/control-plane/g27/frt-2704-idempotency-under-load-validator/
apps/web-console/src/features/g27/frt-2704-idempotency-under-load-validator/
apps/admin-console/src/features/g27/frt-2704-idempotency-under-load-validator/
tests/g27/frt-2704-idempotency-under-load-validator/
```
...
```text
POST /v1/skills/frt-2704/runs
GET  /v1/skills/frt-2704/runs/{run_id}
POST /v1/skills/frt-2704/runs/{run_id}/verify
GET  /v1/skills/frt-2704/runs/{run_id}/findings
GET  /v1/skills/frt-2704/runs/{run_id}/evidence
```
