---
name: b97-dependency-edge-resolver
description: "Use when ELMOS must Resolve explicit and inferred dependencies with confidence, evidence, direction and blocking semantics. Triggers: Dependency Edge Resolver, Canonical Capability Graph Consolidation Pack, depends-on edges, requires-runtime edges, produces-artifact edges, confidence model."
metadata:
  id: B97-S05
...
# B97-S05 — Dependency Edge Resolver

## Objective

Resolve explicit and inferred dependencies with confidence, evidence, direction and blocking semantics.
...
Use this Skill when the approved task explicitly requires **Dependency Edge Resolver** or when the capability graph selects it as part of a minimal dependency closure.
...
- depends-on edges.
- requires-runtime edges.
- produces-artifact edges.
- confidence model.
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b97-dependency-edge-resolver/SKILL.md --out /tmp/dependency_edge_resolver.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B97-S05
```
...
- Reject or expose cycle injection.
- Reject or expose weak evidence promoted to hard dependency.
- Reject or expose missing direction.
