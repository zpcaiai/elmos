---
name: spring-baseline-preflight
description: 在冻结环境运行 compile/test/startup，区分历史失败和迁移回归。 Use for this exact ELMOS Spring Golden Route v2 contract; imported specifications are not runtime or certification evidence.
...
metadata:
  ...
  source_id: ELMOS-GR-027
  source_name: spring-baseline-preflight
  source_path: skills/spring-baseline-preflight/SKILL.md
  source_sha256: sha256:5d64bb7bb2bda31bf7fa2c1a7a2bcb8829672829bd3a165bffc304b5daa7799c
  source_contract_sha256: sha256:48c3ef86f73a87949bb3d8e9167f23ebb2bfb15d34bf62a114d2e4e82829ee16
...
# Baseline Build Preflight

## Objective

在冻结环境运行 compile/test/startup，区分历史失败和迁移回归。

## Scope and capabilities

- JDK/Maven/container 固定
- Maven profiles
- known failing tests ledger
- startup/health
- clean-room 重复
...
- [ ] 定义 `spring-baseline-preflight` 的 public interface、输入/输出/Error Schema 和版本策略。
- [ ] 实现 `spring-baseline-preflight` 的 production adapter/service，不把业务状态只保存在 Prompt 或内存对象中。
...
- [ ] 至少生成：baseline-build.json, known-failures.json, startup-evidence.json。
...
- [ ] 历史失败与迁移回归边界冻结
- [ ] 报价、ETA、假设和风险进入 SOW
- [ ] 本 Skill 的 required outputs 均由真实目标仓库执行产生：baseline-build.json, known-failures.json, startup-evidence.json。
...
- Machine contract: `references/contract.json` (`sha256:48c3ef86f73a87949bb3d8e9167f23ebb2bfb15d34bf62a114d2e4e82829ee16`).
