---
name: legacy-web-22-servlet-container-semantics-recovery
description: "Repository-owned exact runtime interface for Servlet 容器语义恢复; bounded semantic analysis and evidence generation for Java legacy web modernization."
metadata:
  source_package: elmos.java-legacy-web.repository-modernization
  source_version: 1.0.0
  source_id: 22-servlet-container-semantics-recovery
  source_digest: sha256:fd9340a93e4acc9b23f57dbad9c888fc05313e7bb96408aaf99f34b1cdb3b862
  phase: semantic-recovery
  runtime_state: CODE_COMPLETE_LOCAL
  capability_state: LOCAL_EXECUTED
  operation_code: SERVLET_SEMANTICS_RECOVERED
  runtime_handler_id: legacy-web-handler:22-servlet-container-semantics-recovery
---
...
---
id: 22-servlet-container-semantics-recovery
title: Servlet 容器语义恢复
version: 1.0.0
phase: semantic-recovery
priority: P0
requires:
...
恢复 servlet/filter/listener 生命周期、mapping precedence、dispatcher types、async、error dispatch、multipart、session 和安全约束。
...
producerSkill: 22-servlet-container-semantics-recovery
...
6. 恢复 servlet/filter/listener 初始化、销毁、load-on-startup 和实例共享模型。
7. 区分 REQUEST/FORWARD/INCLUDE/ERROR/ASYNC dispatcher type 与 filter 顺序。
8. 建模 sync/async、response commit、streaming、include/forward attribute 和 error page attribute。
9. 处理 multipart、session cookie/timeout/tracking mode、security-constraint 和程序化注册。
