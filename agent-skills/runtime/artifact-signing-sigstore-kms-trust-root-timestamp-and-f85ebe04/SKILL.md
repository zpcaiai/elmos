---
name: artifact-signing-sigstore-kms-trust-root-timestamp-and-f85ebe04
description: 实现 Sigstore Keyless/Private、KMS/HSM、Timestamp、Transparency、Trust Root、轮换与撤销。
metadata:
  elmos_batch: "37A"
  elmos_skill_id: "ART37A006"
  pack_version: "3.7-complete"
---

# ART37A006 — Artifact Signing、Sigstore、KMS、Trust Root 与 Key Lifecycle

## Objective

实现 Sigstore Keyless/Private、KMS/HSM、Timestamp、Transparency、Trust Root、轮换与撤销。
...
- 支持 Purpose-bound Signer：Artifact、Attestation、VSA、Evidence Pack、Revocation。
- Keyless 验证 OIDC Issuer 与 Certificate Identity；KMS 验证 Key ID/Version。
- 保存完整 Offline Verification Bundle 和版本化 Trust Root。
- 轮换先验证 Next Key，撤销后阻止新签名并重评历史签名。
...
- privateKeyIsNotStored
- keylessWrongOidcIssuerIsRejected
- revokedSignerCannotCreateNewSignature
- trustRootVersionIsImmutable
