---
name: elmos-project-generation-file-staging
description: Provide a durable, recoverable workspace and explicit lifecycle for every file created while ELMOS generates a converted project, including fragments, patches, source maps, manifests, compiler outputs, and final publish candidates.
version: 1.2.0
...
# Project Generation File Staging

## Outcome

Provide a durable, recoverable workspace and explicit lifecycle for every file created while ELMOS generates a converted project, including fragments, patches, source maps, manifests, compiler outputs, and final publish candidates.
...
- Dependency skills have passed: **elmos-cache-system-architecture, elmos-cache-metadata-database, elmos-content-addressable-storage**.
...
- No generator writes directly to the source repository or live final output.
- Every generated file has logical path, producer node, attempt, ActionKey, lease epoch, status, digest, file class, and provenance.
- Scratch files are disposable; staged files are recoverable; sealed files are immutable; published files are visible only after whole-tree validation.
- File existence never equals completion.
- Workspace paths are tenant/project/run scoped and resistant to traversal, case collision, device-name abuse, and symlink escape.
...
1. Allocate control, source, overlay, scratch, generated/pending, generated/sealed, artifacts, checkpoints, quarantine, publish, and logs directories.
2. Classify outputs as SCRATCH, STAGED_INTERMEDIATE, SEALED_ARTIFACT, PUBLISH_CANDIDATE, or QUARANTINED.
3. Reserve each logical output path transactionally before writing and enforce explicit overwrite or merge policy.
4. Track RESERVED, WRITING, SEALED, CAS_PROMOTED, TREE_INCLUDED, PUBLISHED, ABORTED, and QUARANTINED states.
5. Recover stale WRITING files conservatively, verify SEALED files, resume CAS promotion, reconstruct publish candidates, and quarantine undeclared output.
6. Prevent cleanup from deleting files or artifacts referenced by checkpoints, pins, active runs, or published trees.
