---
name: b101-dotnet-route-verification-pr
description: "Use when ELMOS must Execute Windows/Linux-aware build, tests, runtime probes, differential journeys, PR and rollback evidence. Triggers: .NET Route Verification and PR, Three Golden Route Packs, .NET verification, platform matrix, PR, rollback."
metadata:
  id: B101-S10
...
# B101-S10 — .NET Route Verification and PR

## Objective

Execute Windows/Linux-aware build, tests, runtime probes, differential journeys, PR and rollback evidence.
...
Use this Skill when the approved task explicitly requires **.NET Route Verification and PR** or when the capability graph selects it as part of a minimal dependency closure.
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b101-dotnet-route-verification-pr/SKILL.md --out /tmp/dotnet_route_verification_pr.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B101-S10
```
...
- Reject or expose Windows test skipped as pass.
- Reject or expose migration irreversible.
- Reject or expose signing identity spoofed.
