---
name: b104-cost-unit-economics-certification
description: "Use when ELMOS must Measure per-repository, per-LOC, per-runner-hour, per-model and support cost with margin scenarios. Triggers: Cost and Unit Economics Certification, Real World Product Certification Pack, cost, unit economics, model usage, capacity."
metadata:
  id: B104-S13
...
# B104-S13 — Cost and Unit Economics Certification

## Objective

Measure per-repository, per-LOC, per-runner-hour, per-model and support cost with margin scenarios.
...
Use this Skill when the approved task explicitly requires **Cost and Unit Economics Certification** or when the capability graph selects it as part of a minimal dependency closure.
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b104-cost-unit-economics-certification/SKILL.md --out /tmp/cost_unit_economics_certification.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B104-S13
```
...
- Reject or expose free-tier assumption.
- Reject or expose support labor omitted.
- Reject or expose failed runs excluded.
