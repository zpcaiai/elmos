...
description: "Run FRT-1010 Authorization and Tenant Scope Mapper for FRT G10 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-1010"
  source_name: "frt-1010-authorization-and-tenant-scope-mapper"
  source_sha256: "sha256:8709551b1400f5fcd13b58c24c4219d68eb704b328bbb537186ca19d196ed21e"
  batch: "G10"
...
- Runtime catalog key: `FRT-1010` / `frt-1010-authorization-and-tenant-scope-mapper`.
...
# FRT-1010 — Authorization and Tenant Scope Mapper

## Objective

在G10范围内实现 **Authorization and Tenant Scope Mapper**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：保持Route、History、Guard和Deep Link语义; 表单Validation、错误、草稿和提交幂等完整; API、Event、Storage和Realtime边界显式。

## When to Use

- 实现或修改与 **Authorization and Tenant Scope Mapper** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-1010Result`：版本化、带状态和诊断的结构化结果。
- `FRT-1010EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-1010FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-1010CertificateFragment`：供G10批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g10/frt-1010-authorization-and-tenant-scope-mapper/
packages/runtime/g10/frt-1010-authorization-and-tenant-scope-mapper/
services/control-plane/g10/frt-1010-authorization-and-tenant-scope-mapper/
apps/web-console/src/features/g10/frt-1010-authorization-and-tenant-scope-mapper/
apps/admin-console/src/features/g10/frt-1010-authorization-and-tenant-scope-mapper/
tests/g10/frt-1010-authorization-and-tenant-scope-mapper/
```
...
```text
POST /v1/skills/frt-1010/runs
GET  /v1/skills/frt-1010/runs/{run_id}
POST /v1/skills/frt-1010/runs/{run_id}/verify
GET  /v1/skills/frt-1010/runs/{run_id}/findings
GET  /v1/skills/frt-1010/runs/{run_id}/evidence
```
