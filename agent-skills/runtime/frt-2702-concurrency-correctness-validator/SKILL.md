...
description: "Run FRT-2702 Concurrency Correctness Validator for FRT G27 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-2702"
  source_name: "frt-2702-concurrency-correctness-validator"
  source_sha256: "sha256:58b3c3a608c78e9149890ecac9c9e681dc583003840c709db4f12f15115dd5a4"
  batch: "G27"
...
- Runtime catalog key: `FRT-2702` / `frt-2702-concurrency-correctness-validator`.
...
在G27范围内实现 **Concurrency Correctness Validator**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：业务化Workload Model; 并发线性化点、Race、Lost Update、Idempotency Under Load; Load/Stress/Spike/Soak与Recovery。

## When to Use

- 实现或修改与 **Concurrency Correctness Validator** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-2702Result`：版本化、带状态和诊断的结构化结果。
- `FRT-2702EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-2702FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-2702CertificateFragment`：供G27批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g27/frt-2702-concurrency-correctness-validator/
packages/runtime/g27/frt-2702-concurrency-correctness-validator/
services/control-plane/g27/frt-2702-concurrency-correctness-validator/
apps/web-console/src/features/g27/frt-2702-concurrency-correctness-validator/
apps/admin-console/src/features/g27/frt-2702-concurrency-correctness-validator/
tests/g27/frt-2702-concurrency-correctness-validator/
```
...
```text
POST /v1/skills/frt-2702/runs
GET  /v1/skills/frt-2702/runs/{run_id}
POST /v1/skills/frt-2702/runs/{run_id}/verify
GET  /v1/skills/frt-2702/runs/{run_id}/findings
GET  /v1/skills/frt-2702/runs/{run_id}/evidence
```
