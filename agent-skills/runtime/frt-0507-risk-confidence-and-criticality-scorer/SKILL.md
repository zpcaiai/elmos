...
description: "Run FRT-0507 Risk Confidence and Criticality Scorer for FRT G05 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-0507"
  source_name: "frt-0507-risk-confidence-and-criticality-scorer"
  source_sha256: "sha256:47670935a9cf7ed0c14b0fe2a4b6d6596cadcba5ca39d28dc6933fbe97acb2af"
  batch: "G05"
...
- Runtime catalog key: `FRT-0507` / `frt-0507-risk-confidence-and-criticality-scorer`.
...
# FRT-0507 — Risk Confidence and Criticality Scorer

## Objective

在G05范围内实现 **Risk Confidence and Criticality Scorer**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：比较源语义与目标能力并生成Gap Register; 区分Native、Shim、Product Decision、Manual与Unsupported; 估算正确率、耗时、风险、数据和测试影响。

## When to Use

- 实现或修改与 **Risk Confidence and Criticality Scorer** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-0507Result`：版本化、带状态和诊断的结构化结果。
- `FRT-0507EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-0507FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-0507CertificateFragment`：供G05批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g05/frt-0507-risk-confidence-and-criticality-scorer/
packages/runtime/g05/frt-0507-risk-confidence-and-criticality-scorer/
services/control-plane/g05/frt-0507-risk-confidence-and-criticality-scorer/
apps/web-console/src/features/g05/frt-0507-risk-confidence-and-criticality-scorer/
apps/admin-console/src/features/g05/frt-0507-risk-confidence-and-criticality-scorer/
tests/g05/frt-0507-risk-confidence-and-criticality-scorer/
```
...
```text
POST /v1/skills/frt-0507/runs
GET  /v1/skills/frt-0507/runs/{run_id}
POST /v1/skills/frt-0507/runs/{run_id}/verify
GET  /v1/skills/frt-0507/runs/{run_id}/findings
GET  /v1/skills/frt-0507/runs/{run_id}/evidence
```
