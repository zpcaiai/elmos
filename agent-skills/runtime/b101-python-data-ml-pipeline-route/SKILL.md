---
name: b101-python-data-ml-pipeline-route
description: "Use when ELMOS must Modernize batch, notebook, orchestration, numerical and ML pipelines with data and model behavior evidence. Triggers: Python Data and ML Pipeline Route, Three Golden Route Packs, data pipelines, numerical tolerance, model artifacts, schedules."
metadata:
  id: B101-S14
...
# B101-S14 — Python Data and ML Pipeline Route

## Objective

Modernize batch, notebook, orchestration, numerical and ML pipelines with data and model behavior evidence.
...
Use this Skill when the approved task explicitly requires **Python Data and ML Pipeline Route** or when the capability graph selects it as part of a minimal dependency closure.
...
- dtype/null/schema compared.
- random seeds and tolerances explicit.
- artifact environment bound.
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b101-python-data-ml-pipeline-route/SKILL.md --out /tmp/python_data_ml_pipeline_route.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B101-S14
```
...
- Reject or expose metric threshold invented.
- Reject or expose pickle unsafe load.
- Reject or expose schedule retry lost.
