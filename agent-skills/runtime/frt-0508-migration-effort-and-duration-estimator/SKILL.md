...
description: "Run FRT-0508 Migration Effort and Duration Estimator for FRT G05 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-0508"
  source_name: "frt-0508-migration-effort-and-duration-estimator"
  source_sha256: "sha256:a006132d05f2128e9905b1eb4f50ecc3365f1ed43c6e1a00e77fd866ec703359"
  batch: "G05"
  source_version: "1.0.0"
  source_risk: "critical-by-default"
  source_certificate_family: "MP"
...
- Runtime catalog key: `FRT-0508` / `frt-0508-migration-effort-and-duration-estimator`.
...
# FRT-0508 — Migration Effort and Duration Estimator

## Objective

在G05范围内实现 **Migration Effort and Duration Estimator**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：比较源语义与目标能力并生成Gap Register; 区分Native、Shim、Product Decision、Manual与Unsupported; 估算正确率、耗时、风险、数据和测试影响。

## When to Use

- 实现或修改与 **Migration Effort and Duration Estimator** 对应的Contract、Runtime、API、UI、测试或证据链时。
- 相关代码、Schema、Pack、Policy、环境或依赖变化导致既有Evidence失效时。
- G05 Orchestrator生成该Skill的执行义务时。
...
- `SourceArtifactSet`：G3 IR、G4 Adapter结果、目标Profile和Pack候选。
- `SkillPolicy`：权限、网络、Sandbox、Toolchain、Evidence和停止规则。

## Outputs

- `FRT-0508Result`：版本化、带状态和诊断的结构化结果。
- `FRT-0508EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-0508FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-0508CertificateFragment`：供G05批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g05/frt-0508-migration-effort-and-duration-estimator/
packages/runtime/g05/frt-0508-migration-effort-and-duration-estimator/
services/control-plane/g05/frt-0508-migration-effort-and-duration-estimator/
apps/web-console/src/features/g05/frt-0508-migration-effort-and-duration-estimator/
apps/admin-console/src/features/g05/frt-0508-migration-effort-and-duration-estimator/
tests/g05/frt-0508-migration-effort-and-duration-estimator/
```
...
```text
POST /v1/skills/frt-0508/runs
GET  /v1/skills/frt-0508/runs/{run_id}
POST /v1/skills/frt-0508/runs/{run_id}/verify
GET  /v1/skills/frt-0508/runs/{run_id}/findings
GET  /v1/skills/frt-0508/runs/{run_id}/evidence
```
...
- The parent G05 Orchestrator can consume the result and independently decide certification.
