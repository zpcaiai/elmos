---
name: b100-runner-mtls-channel
description: "Use when ELMOS must Establish mutually authenticated, rotated and policy-bound runner control channels. Triggers: Runner mTLS Channel, Hardened Runner Fabric Pack, mTLS, certificate rotation, trust bundles, channel policy."
metadata:
  id: B100-S02
...
# B100-S02 — Runner mTLS Channel

## Objective

Establish mutually authenticated, rotated and policy-bound runner control channels.
...
Use this Skill when the approved task explicitly requires **Runner mTLS Channel** or when the capability graph selects it as part of a minimal dependency closure.
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b100-runner-mtls-channel/SKILL.md --out /tmp/runner_mtls_channel.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B100-S02
```
...
- Reject or expose trust-all fallback.
- Reject or expose hostname bypass.
- Reject or expose revoked cert accepted.
