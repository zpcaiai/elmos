---
name: b100-runner-fabric-certification-gate
description: "Use when ELMOS must Certify runner profiles only after identity, attestation, isolation, malicious workload, cancellation and recovery tests pass. Triggers: Runner Fabric Certification Gate, Hardened Runner Fabric Pack, runner gate, adversarial corpus, independent verifier, profile certification."
metadata:
  id: B100-S16
  ...
  category: "Hardened Runner Fabric Pack"
---

# B100-S16 — Runner Fabric Certification Gate

## Objective

Certify runner profiles only after identity, attestation, isolation, malicious workload, cancellation and recovery tests pass.
...
The objective belongs to **Hardened Runner Fabric Pack**, whose system-level purpose is to secure runner enrollment, workload identity, mTLS, attestation, task signatures, sandboxing, isolation, egress, secrets, quotas, signed artifacts, fleet routing, revocation and certification.

## When to Use

Use this Skill when the approved task explicitly requires **Runner Fabric Certification Gate** or when the capability graph selects it as part of a minimal dependency closure.
...
Package dependencies: `B100-S15`, `B99-S16`
...
- `runner_certification_result.json`
- `certified_profiles.json`
...
- Preferred implementation/tooling context: `containers and/or microVMs`, `mTLS PKI`, `workload identity`, `cgroups`, `artifact signing`. Alternatives are allowed only when their versions, behavior differences and rationale are recorded.
...
- zero critical sandbox escapes.
- exact profile scope.
- signed independent result.
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b100-runner-fabric-certification-gate/SKILL.md --out /tmp/runner_fabric_certification_gate.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B100-S16
```
...
- Reject or expose self-certifying runner.
- Reject or expose mock isolation evidence.
- Reject or expose expired posture waiver.
