---
name: durable-session-inbox
description: 用户输入先持久化 admission，再由 runner 安全 promotion 到模型历史。 Use for this exact ELMOS Spring Golden Route v2 contract; imported specifications are not runtime or certification evidence.
...
metadata:
  ...
  source_id: FOUNDATION-07-durable-session-inbox
  source_name: durable-session-inbox
  source_path: skills/durable-session-inbox/SKILL.md
  source_sha256: sha256:b51048aff6efec89a26fbd903b97d18d4a9e2a524e1f0c99a84a4f939d2b168c
  source_contract_sha256: sha256:8f8c7620a2b82a913dfc62b9c64afdc18cf75d20bd16f5631563b0fe251dd3e6
...
用户输入先持久化 admission，再由 runner 安全 promotion 到模型历史。

## Use when

在 Elmos 需要实现或升级 **Durable Session Inbox** 相关能力时使用；它不是仅供解释的提示词，而是实现、测试与验收契约。

## Capabilities to implement

- message id 幂等，精确重试返回同一 admission
- admission 与 model-visible history 分离
- pending input 在 crash/reconnect 后仍存在
- promotion 与可见消息写入原子提交
...
- [ ] 定义 `durable-session-inbox` 的 public interface、input/output/error schemas。
- [ ] 建立 `durable-session-inbox` 的 capability/feature flags 与版本字段。
...
- [ ] 至少覆盖并验收：message id 幂等，精确重试返回同一 admission；以及 admission 与 model-visible history 分离。
...
- Machine contract: `references/contract.json` (`sha256:8f8c7620a2b82a913dfc62b9c64afdc18cf75d20bd16f5631563b0fe251dd3e6`).
