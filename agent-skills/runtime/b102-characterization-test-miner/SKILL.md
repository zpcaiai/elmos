---
name: b102-characterization-test-miner
description: "Use when ELMOS must Mine observed source behavior into stable tests without freezing known defects uncritically. Triggers: Characterization Test Miner, Semantic Equivalence Lab Pack, characterization, coverage mining, behavior capture, defect labeling."
metadata:
  id: B102-S02
...
# B102-S02 — Characterization Test Miner

## Objective

Mine observed source behavior into stable tests without freezing known defects uncritically.
...
Use this Skill when the approved task explicitly requires **Characterization Test Miner** or when the capability graph selects it as part of a minimal dependency closure.
...
- source behavior traceable.
- known defects labeled.
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b102-characterization-test-miner/SKILL.md --out /tmp/characterization_test_miner.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B102-S02
```
...
- Reject or expose production secret captured.
- Reject or expose flaky snapshot.
- Reject or expose defect blessed silently.
