...
description: "Run FRT-0506 Manual Boundary Planner for FRT G05 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-0506"
  source_name: "frt-0506-manual-boundary-planner"
  source_sha256: "sha256:b43a2f5902169c18db472803d7bf8a9e32eddf8be1db34f49a3d8b3aea5c4afc"
  batch: "G05"
...
- Runtime catalog key: `FRT-0506` / `frt-0506-manual-boundary-planner`.
...
# FRT-0506 — Manual Boundary Planner

## Objective

在G05范围内实现 **Manual Boundary Planner**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：比较源语义与目标能力并生成Gap Register; 区分Native、Shim、Product Decision、Manual与Unsupported; 估算正确率、耗时、风险、数据和测试影响。

## When to Use

- 实现或修改与 **Manual Boundary Planner** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-0506Result`：版本化、带状态和诊断的结构化结果。
- `FRT-0506EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-0506FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-0506CertificateFragment`：供G05批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g05/frt-0506-manual-boundary-planner/
packages/runtime/g05/frt-0506-manual-boundary-planner/
services/control-plane/g05/frt-0506-manual-boundary-planner/
apps/web-console/src/features/g05/frt-0506-manual-boundary-planner/
apps/admin-console/src/features/g05/frt-0506-manual-boundary-planner/
tests/g05/frt-0506-manual-boundary-planner/
```
...
```text
POST /v1/skills/frt-0506/runs
GET  /v1/skills/frt-0506/runs/{run_id}
POST /v1/skills/frt-0506/runs/{run_id}/verify
GET  /v1/skills/frt-0506/runs/{run_id}/findings
GET  /v1/skills/frt-0506/runs/{run_id}/evidence
```
