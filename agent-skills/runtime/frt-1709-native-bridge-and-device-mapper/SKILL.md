...
description: "Run FRT-1709 Native Bridge and Device Mapper for FRT G17 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-1709"
  source_name: "frt-1709-native-bridge-and-device-mapper"
  source_sha256: "sha256:bed5a6cf9a4e58f5dfec93dcf6d5d9b6edbec251e2cdc3ea9bc2927f7ebb1d16"
  batch: "G17"
...
- Runtime catalog key: `FRT-1709` / `frt-1709-native-bridge-and-device-mapper`.
...
# FRT-1709 — Native Bridge and Device Mapper

## Objective

在G17范围内实现 **Native Bridge and Device Mapper**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：补齐小程序/ArkUI/Flutter六个方向; 统一移动Navigation、Lifecycle、Permission、Storage和Native能力; 验证直接与间接三角路径。

## When to Use

- 实现或修改与 **Native Bridge and Device Mapper** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-1709Result`：版本化、带状态和诊断的结构化结果。
- `FRT-1709EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-1709FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-1709CertificateFragment`：供G17批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g17/frt-1709-native-bridge-and-device-mapper/
packages/runtime/g17/frt-1709-native-bridge-and-device-mapper/
services/control-plane/g17/frt-1709-native-bridge-and-device-mapper/
apps/web-console/src/features/g17/frt-1709-native-bridge-and-device-mapper/
apps/admin-console/src/features/g17/frt-1709-native-bridge-and-device-mapper/
tests/g17/frt-1709-native-bridge-and-device-mapper/
```
...
```text
POST /v1/skills/frt-1709/runs
GET  /v1/skills/frt-1709/runs/{run_id}
POST /v1/skills/frt-1709/runs/{run_id}/verify
GET  /v1/skills/frt-1709/runs/{run_id}/findings
GET  /v1/skills/frt-1709/runs/{run_id}/evidence
```
