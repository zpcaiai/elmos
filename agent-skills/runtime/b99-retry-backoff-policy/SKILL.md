---
name: b99-retry-backoff-policy
description: "Use when ELMOS must Apply bounded, classified and budget-aware retries without converting deterministic failures into loops. Triggers: Retry and Backoff Policy, Durable Runtime Kernel Pack, retry classification, backoff, budgets, jitter."
metadata:
  id: B99-S05
...
# B99-S05 — Retry and Backoff Policy

## Objective

Apply bounded, classified and budget-aware retries without converting deterministic failures into loops.
...
Use this Skill when the approved task explicitly requires **Retry and Backoff Policy** or when the capability graph selects it as part of a minimal dependency closure.
...
- non-retryable failures stop.
- retry budget enforced.
- attempt lineage retained.
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b99-retry-backoff-policy/SKILL.md --out /tmp/retry_backoff_policy.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B99-S05
```
...
- Reject or expose infinite backoff reset.
- Reject or expose security denial retried.
