---
name: pm-b25-postgresql-logical-replication
description: "迁移 Publication、Subscription、Slot、冲突和 DDL 管理. Precision Migration B25 contract; use for this exact assessment, transformation, validation, repair, evidence, or cutover scope."
---

# Postgresql Logical Replication
## ELMOS runtime binding

- Invoke this repository Skill as `$pm-b25-postgresql-logical-replication`.
- Immutable source identity: `postgresql-logical-replication` in `precision-migration-b01-44` (B25).
- Runtime adapter: `database-and-data-route`; binding state: `DECLARED`.
- Resolve and plan with `python3 scripts/precision_migration/runtime.py plan --skill pm-b25-postgresql-logical-replication`.
...
迁移 Publication、Subscription、Slot、冲突和 DDL 管理。

本 Skill 属于精密语言转换系统。它必须以**可执行证据**为准，不得用模型自评、源码相似度或“能编译”替代语义与行为验证。

## Use when

- 当前任务直接涉及 `postgresql-logical-replication` 所描述的能力。
...
1. 盘点对象与专有能力
2. 执行本 Skill 的核心职责：迁移 Publication、Subscription、Slot、冲突和 DDL 管理。
...
```yaml
skill: postgresql-logical-replication
version: 1.0.0
