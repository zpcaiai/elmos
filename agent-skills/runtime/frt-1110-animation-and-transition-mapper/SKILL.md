...
description: "Run FRT-1110 Animation and Transition Mapper for FRT G11 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-1110"
  source_name: "frt-1110-animation-and-transition-mapper"
  source_sha256: "sha256:869e2370f38c012fb307ba421a72799d0a70c725c624d9fc098a8bab185fab28"
  batch: "G11"
...
- Runtime catalog key: `FRT-1110` / `frt-1110-animation-and-transition-mapper`.
...
# FRT-1110 — Animation and Transition Mapper

## Objective

在G11范围内实现 **Animation and Transition Mapper**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：恢复布局约束、Design Token和响应式Size Class; 生成资产、字体、Icon、Theme和动画映射; 保持Locale、Plural、Date/Number和RTL语义。

## When to Use

- 实现或修改与 **Animation and Transition Mapper** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-1110Result`：版本化、带状态和诊断的结构化结果。
- `FRT-1110EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-1110FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-1110CertificateFragment`：供G11批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g11/frt-1110-animation-and-transition-mapper/
packages/runtime/g11/frt-1110-animation-and-transition-mapper/
services/control-plane/g11/frt-1110-animation-and-transition-mapper/
apps/web-console/src/features/g11/frt-1110-animation-and-transition-mapper/
apps/admin-console/src/features/g11/frt-1110-animation-and-transition-mapper/
tests/g11/frt-1110-animation-and-transition-mapper/
```
...
```text
POST /v1/skills/frt-1110/runs
GET  /v1/skills/frt-1110/runs/{run_id}
POST /v1/skills/frt-1110/runs/{run_id}/verify
GET  /v1/skills/frt-1110/runs/{run_id}/findings
GET  /v1/skills/frt-1110/runs/{run_id}/evidence
```
