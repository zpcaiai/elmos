---
name: pm-b21-trigger-semantics-converter
description: "转换触发器时机、粒度、顺序、递归、OLD/NEW 和副作用. Precision Migration B21 contract; use for this exact assessment, transformation, validation, repair, evidence, or cutover scope."
---

# Trigger Semantics Converter
...
转换触发器时机、粒度、顺序、递归、OLD/NEW 和副作用。

本 Skill 属于精密语言转换系统。它必须以**可执行证据**为准，不得用模型自评、源码相似度或“能编译”替代语义与行为验证。

## Use when

- 当前任务直接涉及 `trigger-semantics-converter` 所描述的能力。
...
1. 盘点对象与专有能力
2. 执行本 Skill 的核心职责：转换触发器时机、粒度、顺序、递归、OLD/NEW 和副作用。
...
```yaml
skill: trigger-semantics-converter
version: 1.0.0
