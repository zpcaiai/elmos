---
name: pm-b43-rule-performance-monitor
description: "跟踪规则命中、构建、验证、误报、漏报、修复轮数和成本. Precision Migration B43 contract; use for this exact assessment, transformation, validation, repair, evidence, or cutover scope."
---

# Rule Performance Monitor
## ELMOS runtime binding

- Invoke this repository Skill as `$pm-b43-rule-performance-monitor`.
- Immutable source identity: `rule-performance-monitor` in `precision-migration-b01-44` (B43).
- Runtime adapter: `continuous-modernization-learning`; binding state: `DECLARED`.
- Resolve and plan with `python3 scripts/precision_migration/runtime.py plan --skill pm-b43-rule-performance-monitor`.
...
跟踪规则命中、构建、验证、误报、漏报、修复轮数和成本。

本 Skill 属于精密语言转换系统。它必须以**可执行证据**为准，不得用模型自评、源码相似度或“能编译”替代语义与行为验证。

## Use when

- 当前任务直接涉及 `rule-performance-monitor` 所描述的能力。
...
1. 汇总证据与未解决项
2. 执行本 Skill 的核心职责：跟踪规则命中、构建、验证、误报、漏报、修复轮数和成本。
3. 执行硬性发布门禁
4. 影子/Canary/渐进切换
5. 监控并自动回滚
6. 沉淀反例、规则和企业交付能力
...
```yaml
skill: rule-performance-monitor
version: 1.0.0
