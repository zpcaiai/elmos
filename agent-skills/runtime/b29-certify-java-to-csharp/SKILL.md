---
name: b29-certify-java-to-csharp
description: "Implement or certify the directed Java-to-C# migration route, including source semantics, target lowering, compatibility strategy, corpus, real builds, behavior evidence, and route manifest. Use only for Java source and C# target work."
---
...
Create a customer-usable, independently certified Java → C# route. The reverse direction is not covered by this skill.

## Use this skill when

Use when adding, repairing, benchmarking, or certifying the `java-to-csharp` route.

## Route-specific semantic hazards

- Java signed byte versus C# byte/sbyte
- type erasure versus reified generics
- checked exceptions and suppressed exceptions
- BigDecimal precision, scale, equality, and rounding
- object identity versus overloaded C# equality operators
- ThreadLocal, monitor, interrupt, and async-context differences

## Workflow

1. Inspect `engines/java-engine`, `engines/csharp-engine`, shared PSP/UIR contracts, and any existing `routes/java-to-csharp` package.
2. Run `python3 scripts/batch29/scaffold_route.py --source java --target csharp` if the route package is absent.
...
- `routes/java-to-csharp/route.json`
- `routes/java-to-csharp/support-matrix.json`
- `routes/java-to-csharp/lowering/`
- `routes/java-to-csharp/mappings/`
- `routes/java-to-csharp/compat-runtime/manifest.json`
- `routes/java-to-csharp/corpus/development/`
- `routes/java-to-csharp/corpus/holdout/`
- `routes/java-to-csharp/certification/evidence.json`

## Verification

- Run the real Java source compiler/runtime and the real C# target compiler/runtime.
...
- Run `python3 scripts/batch29/validate_route.py routes/java-to-csharp` and the route gate.
- Verify critical behavior differences and unknown critical semantics are zero for certified scope.

## Stop and escalate when

- A listed Java signed byte versus C# byte/sbyte cannot be preserved, adapted, or explicitly blocked.
- A listed type erasure versus reified generics cannot be preserved, adapted, or explicitly blocked.
- A listed checked exceptions and suppressed exceptions cannot be preserved, adapted, or explicitly blocked.
