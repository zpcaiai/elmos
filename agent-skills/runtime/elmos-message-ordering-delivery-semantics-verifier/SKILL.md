---
name: elmos-message-ordering-delivery-semantics-verifier
description: Verify at-most-once, at-least-once, effectively-once, partition ordering, redelivery, dead-letter and backpressure behavior.
version: 4.0.0
...
# Message Ordering and Delivery Semantics Verifier
...
- `MessageOrderingAndDeliverySemanticsVerifierProfile`
- `MessageOrderingAndDeliverySemanticsVerifierPlan`
- `MessageOrderingAndDeliverySemanticsVerifierRun`
- `MessageOrderingAndDeliverySemanticsVerifierDecision`

Required semantic capabilities:

- inject duplicate, delayed and reordered messages
- verify partition-key and causal ordering
- exercise acknowledgment and redelivery windows
- validate DLQ/retry and poison-message handling
- measure backpressure and consumer recovery
...
- `artifacts/elmos-message-ordering-delivery-semantics-verifier/profile.yaml`
- `artifacts/elmos-message-ordering-delivery-semantics-verifier/plan.json`
- `artifacts/elmos-message-ordering-delivery-semantics-verifier/result.json`
- `artifacts/elmos-message-ordering-delivery-semantics-verifier/evidence/`
...
- **exactly-once marketing claim unsupported** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **DLQ loses tenant context** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **retry reorders dependent events** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **ack before side effect settles** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **backpressure becomes unbounded memory** — must have a preventive control, a detective signal, a negative fixture and a response action.
...
- native scenario: inject duplicate, delayed and reordered messages
- native scenario: verify partition-key and causal ordering
- native scenario: exercise acknowledgment and redelivery windows
- native scenario: validate DLQ/retry and poison-message handling
- native scenario: measure backpressure and consumer recovery
