...
description: "Run FRT-0807 Controlled and Uncontrolled State Mapper for FRT G08 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-0807"
  source_name: "frt-0807-controlled-and-uncontrolled-state-mapper"
  source_sha256: "sha256:1223b81600b071ee9279e211cbff4d5a2c196eaa29699d29435c5e9fd5eb9984"
  batch: "G08"
  source_version: "1.0.0"
  source_risk: "critical-by-default"
  source_certificate_family: "CS"
...
- Runtime catalog key: `FRT-0807` / `frt-0807-controlled-and-uncontrolled-state-mapper`.
...
# FRT-0807 — Controlled and Uncontrolled State Mapper

## Objective

在G08范围内实现 **Controlled and Uncontrolled State Mapper**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：保持Props默认值、事件时机、Slot/Children/Builder结构; 保持Key、Ref、Widget Identity和状态保留语义; 正确降低Hooks、Composable、Context和局部状态。

## When to Use

- 实现或修改与 **Controlled and Uncontrolled State Mapper** 对应的Contract、Runtime、API、UI、测试或证据链时。
- 相关代码、Schema、Pack、Policy、环境或依赖变化导致既有Evidence失效时。
- G08 Orchestrator生成该Skill的执行义务时。
...
- `SourceArtifactSet`：Component Semantic IR、Target Component Profile。
- `SkillPolicy`：权限、网络、Sandbox、Toolchain、Evidence和停止规则。

## Outputs

- `FRT-0807Result`：版本化、带状态和诊断的结构化结果。
- `FRT-0807EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-0807FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-0807CertificateFragment`：供G08批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g08/frt-0807-controlled-and-uncontrolled-state-mapper/
packages/runtime/g08/frt-0807-controlled-and-uncontrolled-state-mapper/
services/control-plane/g08/frt-0807-controlled-and-uncontrolled-state-mapper/
apps/web-console/src/features/g08/frt-0807-controlled-and-uncontrolled-state-mapper/
apps/admin-console/src/features/g08/frt-0807-controlled-and-uncontrolled-state-mapper/
tests/g08/frt-0807-controlled-and-uncontrolled-state-mapper/
```
...
```text
POST /v1/skills/frt-0807/runs
GET  /v1/skills/frt-0807/runs/{run_id}
POST /v1/skills/frt-0807/runs/{run_id}/verify
GET  /v1/skills/frt-0807/runs/{run_id}/findings
GET  /v1/skills/frt-0807/runs/{run_id}/evidence
```
...
- The parent G08 Orchestrator can consume the result and independently decide certification.
