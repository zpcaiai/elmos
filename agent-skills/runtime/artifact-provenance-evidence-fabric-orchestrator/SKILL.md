---
name: artifact-provenance-evidence-fabric-orchestrator
description: 端到端编排统一 Artifact、Producer SDK、CAS、Attestation、Signing、SLSA、SBOM、OCI、Evidence Graph、Verification、Retention、Offline Pack 与 DR。
metadata:
  elmos_batch: "37A"
  elmos_skill_id: "ART37A001"
  pack_version: "3.7-complete"
---

# ART37A001 — Artifact、Provenance 与 Evidence Fabric 总编排

## Objective

端到端编排统一 Artifact、Producer SDK、CAS、Attestation、Signing、SLSA、SBOM、OCI、Evidence Graph、Verification、Retention、Offline Pack 与 DR。
...
- 执行 Batch 37A 基线盘点并生成 docs/evidence/batch-37a-baseline.md。
- 按 Domain→Producer→Storage→Attestation→Verification→Retention→Pack→Gate 顺序实施。
- 要求每个子 Skill 输出迁移、API、UI、测试、Evidence 与限制。
- 协调事务数据库与对象存储的 Outbox/Reconciliation。
...
- orchestratorLoadsAllDependentSkills
- hardStopOnMutableEvidence
- allReleaseGatesAreReferenced
- completionReportListsUnexecutedTests
