---
name: unsafe-native-ffi-abi-bridge
description: Use this skill when Elmos must 转换 Unsafe、Pointer、Native、C ABI、Struct Layout、Calling Convention 和 FFI。 The execution must use typed contracts, version-pinned environments, independent verification, evidence capture, and rollback.
license: Proprietary-Elmos-Commercial
compatibility: Elmos Proof-Driven Agentic Harness v3+; K3 Repository Semantic Compiler + K6 Equivalence; policy, evidence, workspace and verification services required.
metadata:
  version: "3.0.0"
...
allowed-tools: repo.analyze language.parse semantic.ir code.generate build.execute differential.run evidence.write
---

# unsafe-native-ffi-abi-bridge

## 能力目标

转换 Unsafe、Pointer、Native、C ABI、Struct Layout、Calling Convention 和 FFI。

该能力属于 **Cross-Language Repository Semantic Compiler / 跨语言整库转换**，必须作为仓库级、可重放、可度量、可回滚的生产动作执行，不得降级为一次性 Prompt 技巧。
...
1. `profile-source-target`：记录输入、决定、工具、环境、产物和失败语义。
2. `extract-semantic-ir`：记录输入、决定、工具、环境、产物和失败语义。
3. `measure-loss-and-plan`：记录输入、决定、工具、环境、产物和失败语义。
4. `generate-and-link`：记录输入、决定、工具、环境、产物和失败语义。
5. `compile-and-differential-test`：记录输入、决定、工具、环境、产物和失败语义。
6. `dual-run-and-cutover`：记录输入、决定、工具、环境、产物和失败语义。
...
最低证据级别：**E2**。硬门：semantic-coverage-complete, target-build-passes, behavior-equivalent, interop-boundary-safe, rollback-ready。
