---
name: pm-b21-stored-procedure-parser
description: "解析过程、函数、Package、变量、游标、异常、事务和动态 SQL. Precision Migration B21 contract; use for this exact assessment, transformation, validation, repair, evidence, or cutover scope."
---

# Stored Procedure Parser
## ELMOS runtime binding

- Invoke this repository Skill as `$pm-b21-stored-procedure-parser`.
- Immutable source identity: `stored-procedure-parser` in `precision-migration-b01-44` (B21).
- Runtime adapter: `database-and-data-route`; binding state: `DECLARED`.
- Resolve and plan with `python3 scripts/precision_migration/runtime.py plan --skill pm-b21-stored-procedure-parser`.
...
- Phase: `G 数据库精密互转`
- Version: `1.0.0`
- Tags: batch-21, stored-procedure-conversion

## Purpose

解析过程、函数、Package、变量、游标、异常、事务和动态 SQL。

本 Skill 属于精密语言转换系统。它必须以**可执行证据**为准，不得用模型自评、源码相似度或“能编译”替代语义与行为验证。

## Use when

- 当前任务直接涉及 `stored-procedure-parser` 所描述的能力。
...
1. 盘点对象与专有能力
2. 执行本 Skill 的核心职责：解析过程、函数、Package、变量、游标、异常、事务和动态 SQL。
