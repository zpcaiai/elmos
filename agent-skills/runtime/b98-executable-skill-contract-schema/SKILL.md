...
description: "Use when ELMOS must Define the authoritative versioned contract that makes a Skill executable rather than prose-only. Triggers: Executable Skill Contract Schema, Executable Skill Contract and Registry Pack, contract schema, typed inputs, typed steps, verification states."
metadata:
  id: B98-S01
...
# B98-S01 — Executable Skill Contract Schema

## Objective

Define the authoritative versioned contract that makes a Skill executable rather than prose-only.
...
Use this Skill when the approved task explicitly requires **Executable Skill Contract Schema** or when the capability graph selects it as part of a minimal dependency closure.
...
- `executable_skill_contract.schema.json`
- `schema_examples/`
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b98-executable-skill-contract-schema/SKILL.md --out /tmp/executable_skill_contract_schema.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B98-S01
```
...
- Reject or expose missing rollback contract.
- Reject or expose untyped external effect.
- Reject or expose unknown verification state.
