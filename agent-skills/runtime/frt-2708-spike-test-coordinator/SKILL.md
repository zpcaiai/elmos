...
description: "Run FRT-2708 Spike Test Coordinator for FRT G27 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-2708"
  source_name: "frt-2708-spike-test-coordinator"
  source_sha256: "sha256:684cc2473dec3f775b8a1966727c6516080f4a19edd4e830ad5ea952aadc0add"
  batch: "G27"
...
- Runtime catalog key: `FRT-2708` / `frt-2708-spike-test-coordinator`.
...
在G27范围内实现 **Spike Test Coordinator**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：业务化Workload Model; 并发线性化点、Race、Lost Update、Idempotency Under Load; Load/Stress/Spike/Soak与Recovery。

## When to Use

- 实现或修改与 **Spike Test Coordinator** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-2708Result`：版本化、带状态和诊断的结构化结果。
- `FRT-2708EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-2708FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-2708CertificateFragment`：供G27批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g27/frt-2708-spike-test-coordinator/
packages/runtime/g27/frt-2708-spike-test-coordinator/
services/control-plane/g27/frt-2708-spike-test-coordinator/
apps/web-console/src/features/g27/frt-2708-spike-test-coordinator/
apps/admin-console/src/features/g27/frt-2708-spike-test-coordinator/
tests/g27/frt-2708-spike-test-coordinator/
```
...
```text
POST /v1/skills/frt-2708/runs
GET  /v1/skills/frt-2708/runs/{run_id}
POST /v1/skills/frt-2708/runs/{run_id}/verify
GET  /v1/skills/frt-2708/runs/{run_id}/findings
GET  /v1/skills/frt-2708/runs/{run_id}/evidence
```
