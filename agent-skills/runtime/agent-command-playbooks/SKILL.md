---
name: agent-command-playbooks
description: 把重复开发流程包装成带参数、模型、Agent 和上下文引用的命令宏。 Use for this exact ELMOS Spring Golden Route v2 contract; imported specifications are not runtime or certification evidence.
...
metadata:
  ...
  source_id: FOUNDATION-06-agent-command-playbooks
  source_name: agent-command-playbooks
  source_path: skills/agent-command-playbooks/SKILL.md
  source_sha256: sha256:570a4f17eb231febfa67ed349a5eecd085d5593a25a8b95283c8e8ec56601948
  source_contract_sha256: sha256:a1e56e39363ba2f62f3d66321e92b854c5e41b433553c4a8e59e0764a345a583
...
把重复开发流程包装成带参数、模型、Agent 和上下文引用的命令宏。

## Use when

在 Elmos 需要实现或升级 **Agent Command Playbooks** 相关能力时使用；它不是仅供解释的提示词，而是实现、测试与验收契约。
...
- 可绑定特定 Agent/model
- 可引用文件、测试输出和预计算上下文
- 高风险 shell interpolation 仍经过 permission engine
...
- [ ] 定义 `agent-command-playbooks` 的 public interface、input/output/error schemas。
- [ ] 建立 `agent-command-playbooks` 的 capability/feature flags 与版本字段。
...
- [ ] 至少覆盖并验收：模板支持位置参数和结构化输入；以及 可绑定特定 Agent/model。
...
- Machine contract: `references/contract.json` (`sha256:a1e56e39363ba2f62f3d66321e92b854c5e41b433553c4a8e59e0764a345a583`).
