---
name: b97-tool-runtime-requirement-graph
description: "Use when ELMOS must Map Skills to exact compilers, SDKs, services, runner profiles, licenses, devices and cloud capabilities. Triggers: Tool and Runtime Requirement Graph, Canonical Capability Graph Consolidation Pack, tool requirements, runner matching, license constraints, platform capabilities."
metadata:
  id: B97-S11
...
# B97-S11 — Tool and Runtime Requirement Graph

## Objective

Map Skills to exact compilers, SDKs, services, runner profiles, licenses, devices and cloud capabilities.
...
Use this Skill when the approved task explicitly requires **Tool and Runtime Requirement Graph** or when the capability graph selects it as part of a minimal dependency closure.
...
- `runtime_requirement_graph.json`
- `runner_match_report.json`
...
- unavailable tools block execution.
- license restrictions visible.
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b97-tool-runtime-requirement-graph/SKILL.md --out /tmp/tool_runtime_requirement_graph.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B97-S11
```
...
- Reject or expose phantom tool availability.
- Reject or expose platform spoofing.
- Reject or expose license omission.
