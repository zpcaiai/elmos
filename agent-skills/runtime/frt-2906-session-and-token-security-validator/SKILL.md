...
description: "Run FRT-2906 Session and Token Security Validator for FRT G29 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-2906"
  source_name: "frt-2906-session-and-token-security-validator"
  source_sha256: "sha256:5df02f8b75f2e32fde8ea90f94c81019ca5b1ad8df1f6cab7ed45b23209fc7ab"
  batch: "G29"
...
- Runtime catalog key: `FRT-2906` / `frt-2906-session-and-token-security-validator`.
...
在G29范围内实现 **Session and Token Security Validator**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：Asset、Threat、Trust Boundary、Attack Surface; Authentication、Authorization、Object/Field Scope、Tenant Isolation; Session/Token、API、Input、File、SSRF/XSS/Command/Prompt Injection。

## When to Use

- 实现或修改与 **Session and Token Security Validator** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-2906Result`：版本化、带状态和诊断的结构化结果。
- `FRT-2906EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-2906FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-2906CertificateFragment`：供G29批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g29/frt-2906-session-and-token-security-validator/
packages/runtime/g29/frt-2906-session-and-token-security-validator/
services/control-plane/g29/frt-2906-session-and-token-security-validator/
apps/web-console/src/features/g29/frt-2906-session-and-token-security-validator/
apps/admin-console/src/features/g29/frt-2906-session-and-token-security-validator/
tests/g29/frt-2906-session-and-token-security-validator/
```
...
```text
POST /v1/skills/frt-2906/runs
GET  /v1/skills/frt-2906/runs/{run_id}
POST /v1/skills/frt-2906/runs/{run_id}/verify
GET  /v1/skills/frt-2906/runs/{run_id}/findings
GET  /v1/skills/frt-2906/runs/{run_id}/evidence
```
