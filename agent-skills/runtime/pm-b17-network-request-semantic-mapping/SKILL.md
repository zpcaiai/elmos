---
name: pm-b17-network-request-semantic-mapping
description: "映射请求序列化、取消、重试、缓存、错误、竞态和 Latest-wins. Precision Migration B17 contract; use for this exact assessment, transformation, validation, repair, evidence, or cutover scope."
---

# Network Request Semantic Mapping
## ELMOS runtime binding

- Invoke this repository Skill as `$pm-b17-network-request-semantic-mapping`.
- Immutable source identity: `network-request-semantic-mapping` in `precision-migration-b01-44` (B17).
- Runtime adapter: `frontend-client-route`; binding state: `UNAVAILABLE`.
- Resolve and plan with `python3 scripts/precision_migration/runtime.py plan --skill pm-b17-network-request-semantic-mapping`.
...
- Phase: `F 前端与多端互转`
- Version: `1.0.0`
- Tags: batch-17, frontend-semantic-conversion

## Purpose

映射请求序列化、取消、重试、缓存、错误、竞态和 Latest-wins。

本 Skill 属于精密语言转换系统。它必须以**可执行证据**为准，不得用模型自评、源码相似度或“能编译”替代语义与行为验证。

## Use when

- 当前任务直接涉及 `network-request-semantic-mapping` 所描述的能力。
...
1. 恢复组件契约和UI状态机
2. 执行本 Skill 的核心职责：映射请求序列化、取消、重试、缓存、错误、竞态和 Latest-wins。
3. 映射事件、路由、布局和平台能力
4. 生成目标项目
...
```yaml
skill: network-request-semantic-mapping
version: 1.0.0
