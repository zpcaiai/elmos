---
name: elmos-dependency-update-renovation-controller
description: Continuously propose, test and promote dependency, framework and toolchain upgrades with semantic impact, security and rollback evidence.
version: 4.0.0
...
# Dependency Update and Renovation Controller
...
- `DependencyUpdateAndRenovationControllerProfile`
- `DependencyUpdateAndRenovationControllerPlan`
- `DependencyUpdateAndRenovationControllerRun`
- `DependencyUpdateAndRenovationControllerDecision`

Required semantic capabilities:

- detect releases, advisories and EOL
- compute repository and generated-code impact
- open bounded upgrade changes
- run native, differential and security suites
- promote, defer or rollback with rationale
...
- `elmos-vulnerability-vex-kev-response-governor` from `elmos-functional-assurance-certification-skills` — optional companion; required only for E4/E5/P05 certification.

## Primary Outputs

- `artifacts/elmos-dependency-update-renovation-controller/profile.yaml`
- `artifacts/elmos-dependency-update-renovation-controller/plan.json`
- `artifacts/elmos-dependency-update-renovation-controller/result.json`
- `artifacts/elmos-dependency-update-renovation-controller/evidence/`
...
- **automated major upgrade bypasses review** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **lockfile not updated coherently** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **security fix changes semantics** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **dependency source compromised** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **upgrade fatigue causes blanket auto-merge** — must have a preventive control, a detective signal, a negative fixture and a response action.
...
- native scenario: detect releases, advisories and EOL
- native scenario: compute repository and generated-code impact
- native scenario: open bounded upgrade changes
- native scenario: run native, differential and security suites
- native scenario: promote, defer or rollback with rationale
