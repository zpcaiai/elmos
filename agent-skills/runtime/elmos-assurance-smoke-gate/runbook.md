...
## Verify
执行本skill的acceptance.yaml对应真实测试，并检查原始报告而非Agent总结。关联输出：SmokeManifest, SmokeExecutionEvidence, SmokeGateDecision。
至少演练：预算不足覆盖关键义务；必须观察：报告uncovered而不是小suite PASS。
