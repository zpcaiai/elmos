---
name: dynamic-template-builder
description: 支持运行时条件、循环和可复用片段构造模板。 Use for this exact ELMOS Spring Golden Route v2 contract; imported specifications are not runtime or certification evidence.
...
metadata:
  ...
  source_id: FOUNDATION-03-dynamic-template-builder
  source_name: dynamic-template-builder
  source_path: skills/dynamic-template-builder/SKILL.md
  source_sha256: sha256:c74d444bf8a9fe9c4a44731b8252f8dfad3dfd96e26d0e2c4bc117b19db3ce9e
  source_contract_sha256: sha256:8cdf284213103d8ae57c783afff47a3575792f4ec189703a96eb8da08b903bfb
...
支持运行时条件、循环和可复用片段构造模板。

## Use when

在 Elmos 需要实现或升级 **Dynamic Template Builder** 相关能力时使用；它不是仅供解释的提示词，而是实现、测试与验收契约。

## Capabilities to implement

- Builder API 与 template literal 共用底层解析
- 静态片段与动态 code fragment 时间语义明确
- 允许基于 capture 结果生成不同代码结构
- 动态生成仍必须通过 parser 和 schema 校验
...
- [ ] 定义 `dynamic-template-builder` 的 public interface、input/output/error schemas。
- [ ] 建立 `dynamic-template-builder` 的 capability/feature flags 与版本字段。
...
- [ ] 至少覆盖并验收：Builder API 与 template literal 共用底层解析；以及 静态片段与动态 code fragment 时间语义明确。
...
- Machine contract: `references/contract.json` (`sha256:8cdf284213103d8ae57c783afff47a3575792f4ec189703a96eb8da08b903bfb`).
