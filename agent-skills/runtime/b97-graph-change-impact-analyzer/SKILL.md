---
name: b97-graph-change-impact-analyzer
description: "Use when ELMOS must Calculate downstream compatibility, route, policy, evidence and certification impact for every graph change. Triggers: Graph Change Impact Analyzer, Canonical Capability Graph Consolidation Pack, impact analysis, blast radius, evidence invalidation, owner notification."
metadata:
  id: B97-S15
...
# B97-S15 — Graph Change Impact Analyzer

## Objective

Calculate downstream compatibility, route, policy, evidence and certification impact for every graph change.
...
Use this Skill when the approved task explicitly requires **Graph Change Impact Analyzer** or when the capability graph selects it as part of a minimal dependency closure.
...
- `impact_report.json`
- `invalidated_claims.json`
- `notification_plan.json`
...
- transitive impact bounded.
- critical downstream owners notified.
- stale certifications revoked.
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b97-graph-change-impact-analyzer/SKILL.md --out /tmp/graph_change_impact_analyzer.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B97-S15
```
...
- Reject or expose missed transitive dependency.
- Reject or expose impact truncation.
- Reject or expose silent certification retention.
