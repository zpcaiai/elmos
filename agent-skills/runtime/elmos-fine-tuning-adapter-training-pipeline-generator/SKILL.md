---
name: elmos-fine-tuning-adapter-training-pipeline-generator
description: Generate governed SFT, preference, distillation and PEFT/LoRA pipelines with data lineage, evaluation, safety, reproducibility and promotion gates.
version: 4.0.0
...
# Fine-Tuning and Adapter Training Pipeline Generator
...
- `FineTuningAndAdapterTrainingPipelineGeneratorProfile`
- `FineTuningAndAdapterTrainingPipelineGeneratorPlan`
- `FineTuningAndAdapterTrainingPipelineGeneratorRun`
- `FineTuningAndAdapterTrainingPipelineGeneratorDecision`

Required semantic capabilities:

- compile dataset, split and consent contracts
- generate reproducible training and checkpoint pipeline
- track base model, adapter and tokenizer lineage
- run capability, safety and regression evaluation
- promote via registry, canary and rollback
...
- `artifacts/elmos-fine-tuning-adapter-training-pipeline-generator/profile.yaml`
- `artifacts/elmos-fine-tuning-adapter-training-pipeline-generator/plan.json`
- `artifacts/elmos-fine-tuning-adapter-training-pipeline-generator/result.json`
- `artifacts/elmos-fine-tuning-adapter-training-pipeline-generator/evidence/`
...
- **training data lacks rights** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **holdout leaks into train** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **adapter/base mismatch** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **checkpoint cannot reproduce** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **safety regression averaged away** — must have a preventive control, a detective signal, a negative fixture and a response action.
...
- native scenario: compile dataset, split and consent contracts
- native scenario: generate reproducible training and checkpoint pipeline
- native scenario: track base model, adapter and tokenizer lineage
- native scenario: run capability, safety and regression evaluation
- native scenario: promote via registry, canary and rollback
