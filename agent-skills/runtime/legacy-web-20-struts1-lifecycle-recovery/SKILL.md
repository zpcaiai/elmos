---
name: legacy-web-20-struts1-lifecycle-recovery
description: "Repository-owned exact runtime interface for Struts 1 请求生命周期恢复; bounded semantic analysis and evidence generation for Java legacy web modernization."
metadata:
  source_package: elmos.java-legacy-web.repository-modernization
  source_version: 1.0.0
  source_id: 20-struts1-lifecycle-recovery
  source_digest: sha256:dd4ef6e131a5078cf904a662b7cb72d7f0202d67a6cd0ea668a53f88abf6d75b
  phase: semantic-recovery
  runtime_state: CODE_COMPLETE_LOCAL
  capability_state: LOCAL_EXECUTED
  operation_code: STRUTS1_PIPELINES_RECOVERED
  runtime_handler_id: legacy-web-handler:20-struts1-lifecycle-recovery
---
...
---
id: 20-struts1-lifecycle-recovery
title: Struts 1 请求生命周期恢复
version: 1.0.0
phase: semantic-recovery
priority: P0
requires:
...
恢复 RequestProcessor、ActionServlet、ActionMapping、ActionForm、验证、角色、forward/include、异常和模块配置的真实执行序。
...
producerSkill: 20-struts1-lifecycle-recovery
...
6. 按 RequestProcessor 实际顺序展开 multipart、path、locale、content/no-cache、preprocess、mapping、roles、form、populate、validate、forward/include、Action、exception、forward config。
7. 恢复自定义 RequestProcessor 覆写点及其短路行为。
8. 识别 Action 实例缓存/共享字段，避免把线程不安全状态原样注入 Spring singleton。
9. 精确恢复 ActionForm scope、reset-before-populate、validate-before-execute 和一次性 messages。
