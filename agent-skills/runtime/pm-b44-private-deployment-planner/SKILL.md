...
# Private Deployment Planner
## ELMOS runtime binding

- Invoke this repository Skill as `$pm-b44-private-deployment-planner`.
- Immutable source identity: `private-deployment-planner` in `precision-migration-b01-44` (B44).
- Runtime adapter: `enterprise-private-commercialization`; binding state: `UNAVAILABLE`.
- Resolve and plan with `python3 scripts/precision_migration/runtime.py plan --skill pm-b44-private-deployment-planner`.
...
根据网络、数据、模型、工具链、规模和合规规划私有部署拓扑。

本 Skill 属于精密语言转换系统。它必须以**可执行证据**为准，不得用模型自评、源码相似度或“能编译”替代语义与行为验证。

## Use when

- 当前任务直接涉及 `private-deployment-planner` 所描述的能力。
...
1. 汇总证据与未解决项
2. 执行本 Skill 的核心职责：根据网络、数据、模型、工具链、规模和合规规划私有部署拓扑。
3. 执行硬性发布门禁
4. 影子/Canary/渐进切换
5. 监控并自动回滚
6. 沉淀反例、规则和企业交付能力
...
```yaml
skill: private-deployment-planner
version: 1.0.0
