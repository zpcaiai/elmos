---
name: b97-capability-graph-query-api
description: "Use when ELMOS must Expose bounded, tenant-safe graph queries for discovery, dependency closure, impact analysis and execution planning. Triggers: Capability Graph Query API, Canonical Capability Graph Consolidation Pack, graph API, authorization, pagination, query budgets."
metadata:
  id: B97-S13
...
# B97-S13 — Capability Graph Query API

## Objective

Expose bounded, tenant-safe graph queries for discovery, dependency closure, impact analysis and execution planning.
...
Use this Skill when the approved task explicitly requires **Capability Graph Query API** or when the capability graph selects it as part of a minimal dependency closure.
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b97-capability-graph-query-api/SKILL.md --out /tmp/capability_graph_query_api.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B97-S13
```
...
- Reject or expose graph exfiltration.
- Reject or expose unbounded traversal.
- Reject or expose inference leak.
