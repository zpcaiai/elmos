...
# Protobuf Api Test Generator
## ELMOS runtime binding

- Invoke this repository Skill as `$pm-b29-protobuf-api-test-generator`.
- Immutable source identity: `protobuf-api-test-generator` in `precision-migration-b01-44` (B29).
- Runtime adapter: `differential-test-and-repair`; binding state: `DECLARED`.
- Resolve and plan with `python3 scripts/precision_migration/runtime.py plan --skill pm-b29-protobuf-api-test-generator`.
...
从 Protobuf/gRPC Schema、Field rule 和流式接口生成测试。

本 Skill 属于精密语言转换系统。它必须以**可执行证据**为准，不得用模型自评、源码相似度或“能编译”替代语义与行为验证。

## Use when

- 当前任务直接涉及 `protobuf-api-test-generator` 所描述的能力。
...
1. 评估或生成测试
2. 执行本 Skill 的核心职责：从 Protobuf/gRPC Schema、Field rule 和流式接口生成测试。
3. 同步环境并双运行
4. 比较输出、状态和副作用
5. 搜索最小反例
6. 驱动修复并执行全量回归
...
```yaml
skill: protobuf-api-test-generator
version: 1.0.0
