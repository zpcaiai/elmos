---
name: b102-golden-master-capture
description: "Use when ELMOS must Capture canonical outputs, state transitions and side effects with normalization and approval. Triggers: Golden Master Capture, Semantic Equivalence Lab Pack, golden masters, normalization, state capture, approval."
metadata:
  id: B102-S03
...
# B102-S03 — Golden Master Capture

## Objective

Capture canonical outputs, state transitions and side effects with normalization and approval.
...
Use this Skill when the approved task explicitly requires **Golden Master Capture** or when the capability graph selects it as part of a minimal dependency closure.
...
- content-addressed masters.
- volatile fields normalized explicitly.
- reviewed scope.
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b102-golden-master-capture/SKILL.md --out /tmp/golden_master_capture.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B102-S03
```
...
- Reject or expose overbroad snapshot.
- Reject or expose secret capture.
- Reject or expose normalization hides regression.
