---
name: offline-evidence-pack-export-import-and-airgap-verification
description: 实现可离线验证的规范化、签名、加密 Evidence Pack 与隔离导入。
metadata:
  elmos_batch: "37A"
  elmos_skill_id: "ART37A013"
  pack_version: "3.7-complete"
---

# ART37A013 — Offline Evidence Pack、Export、Import 与 Air-gap Verification

## Objective

实现可离线验证的规范化、签名、加密 Evidence Pack 与隔离导入。
...
- 定义 root manifest、trust、policies、artifacts、attestations、sboms、provenance、verification、audit 布局。
- 每项包含 Digest、Media Type、Classification、Required/Optional 与 Redaction 状态。
- 离线验证顺序覆盖 Verifier、Root Signature、Manifest、Item、Trust、Policy 和 VSA。
- Redacted Pack 创建新 Pack 并重新签名；Import 先 Quarantine。
...
- packManifestIsCanonical
- offlineVerificationWorksWithNetworkDisabled
- tamperedArtifactFailsVerification
- redactedPackIsNewSignedPack
