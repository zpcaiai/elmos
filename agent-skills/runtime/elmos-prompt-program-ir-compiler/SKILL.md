---
name: elmos-prompt-program-ir-compiler
description: Compile prompts into typed, versioned programs with parameters, roles, examples, tool visibility, output schemas,
  safety policies, lineage and tests.
version: 4.0.0
...
- `DeploymentUnit`
- `ConformanceRun`
- `PromptProgramIRCompilerRun`
- `PromptProgramIRCompilerDecision`

Required semantic capabilities:

- Typed parameters and role segments
- Few-shot example provenance
- Tool visibility and authority binding
- Output schema and refusal policy
- Versioning, inheritance and target lowering
...
- `prompt-test-contract.json`
- `prompt-loss-register.json`
...
- **template injection** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **secret in example** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **tool exposure widening** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **role confusion** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **unreviewed inherited prompt** — must have a preventive control, a detective signal, a negative fixture and a response action.
