---
name: b101-spring-boot-modernization-route
description: "Use when ELMOS must Migrate Spring Boot, Security, Data, MVC/WebFlux, configuration and observability with endpoint-level fidelity. Triggers: Spring Boot Modernization Route, Three Golden Route Packs, Spring Boot, Spring Security, configuration, Actuator."
metadata:
  id: B101-S04
...
# B101-S04 — Spring Boot Modernization Route

## Objective

Migrate Spring Boot, Security, Data, MVC/WebFlux, configuration and observability with endpoint-level fidelity.
...
Use this Skill when the approved task explicitly requires **Spring Boot Modernization Route** or when the capability graph selects it as part of a minimal dependency closure.
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b101-spring-boot-modernization-route/SKILL.md --out /tmp/spring_boot_modernization_route.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B101-S04
```
...
- Reject or expose 401/403 drift.
- Reject or expose middleware order change.
- Reject or expose insecure management endpoint.
