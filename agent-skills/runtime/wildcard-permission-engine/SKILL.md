---
name: wildcard-permission-engine
description: action × resource × ruleset 使用 allow/ask/deny 和 last-match 规则。 Use for this exact ELMOS Spring Golden Route v2 contract; imported specifications are not runtime or certification evidence.
...
metadata:
  ...
  source_id: FOUNDATION-08-wildcard-permission-engine
  source_name: wildcard-permission-engine
  source_path: skills/wildcard-permission-engine/SKILL.md
  source_sha256: sha256:08f9c25c94b723c698c42fd490bdf2e617af1886a33dd56bff636d4452d5fd37
  source_contract_sha256: sha256:c090bd759a2b5ebec2337d42989d00e6ef6deae7955046643ca9c94995caf2c6
...
# Wildcard Permission Engine

## Goal

action × resource × ruleset 使用 allow/ask/deny 和 last-match 规则。

## Use when

在 Elmos 需要实现或升级 **Wildcard Permission Engine** 相关能力时使用；它不是仅供解释的提示词，而是实现、测试与验收契约。

## Capabilities to implement

- 全局/项目/Agent/会话/保存规则分层
- 默认缺失 Agent 权限可配置 deny-by-default
- 通配模式支持 tool、文件、命令和外部目录
- permission decision 记录命中规则与来源
...
- [ ] 定义 `wildcard-permission-engine` 的 public interface、input/output/error schemas。
- [ ] 建立 `wildcard-permission-engine` 的 capability/feature flags 与版本字段。
...
- [ ] 至少覆盖并验收：全局/项目/Agent/会话/保存规则分层；以及 默认缺失 Agent 权限可配置 deny-by-default。
...
- Machine contract: `references/contract.json` (`sha256:c090bd759a2b5ebec2337d42989d00e6ef6deae7955046643ca9c94995caf2c6`).
