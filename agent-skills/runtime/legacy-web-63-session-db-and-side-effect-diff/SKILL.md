---
name: legacy-web-63-session-db-and-side-effect-diff
description: "Repository-owned exact runtime interface for Session、数据库与副作用差分; bounded semantic analysis and evidence generation for Java legacy web modernization."
metadata:
  source_package: elmos.java-legacy-web.repository-modernization
  source_version: 1.0.0
  source_id: 63-session-db-and-side-effect-diff
  source_digest: sha256:79001e4d68400b781e48cfa3d6db6cbff3b2c1c50cdf17f865c4cccf194ce058
  phase: verification
  runtime_state: CODE_COMPLETE_LOCAL
  capability_state: LOCAL_EXECUTED
  operation_code: STATE_EFFECT_ORACLE_EVALUATED
  runtime_handler_id: legacy-web-handler:63-session-db-and-side-effect-diff
---
...
---
id: 63-session-db-and-side-effect-diff
title: Session、数据库与副作用差分
version: 1.0.0
phase: verification
priority: P0
requires:
...
produces:
- state-side-effect-equivalence-report
idempotent: true
resumable: true
---
...
比较跨请求 session、DB 行/事务、消息、文件、缓存、邮件和审计副作用，识别顺序、次数和回滚差异。
...
producerSkill: 63-session-db-and-side-effect-diff
...
6. 请求前后快照 session；对对象做稳定序列化和敏感字段脱敏。
7. 数据库比较 row set、字段、事务 commit/rollback、sequence 与触发器效果。
8. 外部 effect 比较 endpoint/topic/path、payload、order、count、idempotency key。
9. 写路径要求隔离环境或 effect capture，禁止对真实生产产生重复副作用。
