---
name: deterministic-recipe-engine
description: 把可重复迁移从 LLM 写代码下沉为类型感知、可测试的确定性 Recipe。 Use for this exact ELMOS Spring Golden Route v2 contract; imported specifications are not runtime or certification evidence.
...
metadata:
  ...
  source_id: FOUNDATION-02-deterministic-recipe-engine
  source_name: deterministic-recipe-engine
  source_path: skills/deterministic-recipe-engine/SKILL.md
  source_sha256: sha256:756da828081694a09d962ee6ec0a66b347a0cc49c27989fb63d9a5b40311132e
  source_contract_sha256: sha256:a9f7a9663ab22bb1948cb5f1733e9a53c54c66632d18d83419ceef727de3e0ff
...
把可重复迁移从 LLM 写代码下沉为类型感知、可测试的确定性 Recipe。

## Use when

在 Elmos 需要实现或升级 **Deterministic Recipe Engine** 相关能力时使用；它不是仅供解释的提示词，而是实现、测试与验收契约。

## Capabilities to implement

- Recipe 具名、可描述、可版本化、可配置
- Recipe 返回确定 changeset，不依赖模型随机性
- 优先把机械性迁移、API 替换、安全修复交给 Recipe
- LLM 仅处理规则无法覆盖的语义歧义
...
- [ ] 定义 `deterministic-recipe-engine` 的 public interface、input/output/error schemas。
- [ ] 建立 `deterministic-recipe-engine` 的 capability/feature flags 与版本字段。
...
- [ ] 至少覆盖并验收：Recipe 具名、可描述、可版本化、可配置；以及 Recipe 返回确定 changeset，不依赖模型随机性。
...
- Machine contract: `references/contract.json` (`sha256:a9f7a9663ab22bb1948cb5f1733e9a53c54c66632d18d83419ceef727de3e0ff`).
