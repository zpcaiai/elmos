---
name: agent-budget-cost-and-quota-controller
description: Reserve, meter and enforce hard cost, token, time and quota limits for Agent work. Use before and after every provider attempt.
---

# Agent Budget Cost And Quota Controller
...
Require task/provider estimates, tenant/project quota, cost registry version, token and wall-clock limits.

## Workflow

1. Validate schema version, stable identities, immutable hashes and policy prerequisites.
2. Reserve before execution; reject if any bound cannot be covered; settle reported usage; charge missing usage conservatively; prevent concurrent overbooking.
...
budget-reservation.json, provider-usage.json and settlement status.

## Fail-closed rules

No reservation means no execution; unknown usage is never zero and limit breach stops the loop.
