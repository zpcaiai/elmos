...
description: "Run FRT-0701 Typed Semantic Lowering Engine for FRT G07 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-0701"
  source_name: "frt-0701-typed-semantic-lowering-engine"
  source_sha256: "sha256:faff44c3be5725a6764b796253b3652e5ecec58ec7b0e61674f7662b04528fba"
  batch: "G07"
...
- Runtime catalog key: `FRT-0701` / `frt-0701-typed-semantic-lowering-engine`.
...
# FRT-0701 — Typed Semantic Lowering Engine

## Objective

在G07范围内实现 **Typed Semantic Lowering Engine**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：Typed IR经规则Pass降低到目标原生AST; Gap生成Typed Hole而非空实现; Format/Analyze/Build驱动有界确定性修复。

## When to Use

- 实现或修改与 **Typed Semantic Lowering Engine** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-0701Result`：版本化、带状态和诊断的结构化结果。
- `FRT-0701EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-0701FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-0701CertificateFragment`：供G07批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g07/frt-0701-typed-semantic-lowering-engine/
packages/runtime/g07/frt-0701-typed-semantic-lowering-engine/
services/control-plane/g07/frt-0701-typed-semantic-lowering-engine/
apps/web-console/src/features/g07/frt-0701-typed-semantic-lowering-engine/
apps/admin-console/src/features/g07/frt-0701-typed-semantic-lowering-engine/
tests/g07/frt-0701-typed-semantic-lowering-engine/
```
...
```text
POST /v1/skills/frt-0701/runs
GET  /v1/skills/frt-0701/runs/{run_id}
POST /v1/skills/frt-0701/runs/{run_id}/verify
GET  /v1/skills/frt-0701/runs/{run_id}/findings
GET  /v1/skills/frt-0701/runs/{run_id}/evidence
```
