...
description: "Run FRT-0800 Component Semantics Orchestrator for FRT G08 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-0800"
  source_name: "frt-0800-component-semantics-orchestrator"
  source_sha256: "sha256:fe62bab5652ae05559fd1ed2398eaa3fdaca035e83688b978a773cdb5d224736"
  batch: "G08"
...
- Runtime catalog key: `FRT-0800` / `frt-0800-component-semantics-orchestrator`.
...
# FRT-0800 — Component Semantics Orchestrator

## Objective

在G08范围内实现 **Component Semantics Orchestrator**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：保持Props默认值、事件时机、Slot/Children/Builder结构; 保持Key、Ref、Widget Identity和状态保留语义; 正确降低Hooks、Composable、Context和局部状态。

## When to Use

- 实现或修改与 **Component Semantics Orchestrator** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-0800Result`：版本化、带状态和诊断的结构化结果。
- `FRT-0800EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-0800FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-0800CertificateFragment`：供G08批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g08/frt-0800-component-semantics-orchestrator/
packages/runtime/g08/frt-0800-component-semantics-orchestrator/
services/control-plane/g08/frt-0800-component-semantics-orchestrator/
apps/web-console/src/features/g08/frt-0800-component-semantics-orchestrator/
apps/admin-console/src/features/g08/frt-0800-component-semantics-orchestrator/
tests/g08/frt-0800-component-semantics-orchestrator/
```
...
```text
POST /v1/skills/frt-0800/runs
GET  /v1/skills/frt-0800/runs/{run_id}
POST /v1/skills/frt-0800/runs/{run_id}/verify
GET  /v1/skills/frt-0800/runs/{run_id}/findings
GET  /v1/skills/frt-0800/runs/{run_id}/evidence
```
