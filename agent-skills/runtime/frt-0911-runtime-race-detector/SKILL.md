...
description: "Run FRT-0911 Runtime Race Detector for FRT G09 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-0911"
  source_name: "frt-0911-runtime-race-detector"
  source_sha256: "sha256:9e1b165b17842725455658e88ea43293762e71d8249fc9a0c3af9f3908f51a29"
  batch: "G09"
...
- Runtime catalog key: `FRT-0911` / `frt-0911-runtime-race-detector`.
...
# FRT-0911 — Runtime Race Detector

## Objective

在G09范围内实现 **Runtime Race Detector**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：区分Local、Server、Global、Persistent和Derived State; 保持Effect触发、Cleanup、Lifecycle和Evaluation Order; 传播Cancellation并防止Stale Result Commit。

## When to Use

- 实现或修改与 **Runtime Race Detector** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-0911Result`：版本化、带状态和诊断的结构化结果。
- `FRT-0911EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-0911FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-0911CertificateFragment`：供G09批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g09/frt-0911-runtime-race-detector/
packages/runtime/g09/frt-0911-runtime-race-detector/
services/control-plane/g09/frt-0911-runtime-race-detector/
apps/web-console/src/features/g09/frt-0911-runtime-race-detector/
apps/admin-console/src/features/g09/frt-0911-runtime-race-detector/
tests/g09/frt-0911-runtime-race-detector/
```
...
```text
POST /v1/skills/frt-0911/runs
GET  /v1/skills/frt-0911/runs/{run_id}
POST /v1/skills/frt-0911/runs/{run_id}/verify
GET  /v1/skills/frt-0911/runs/{run_id}/findings
GET  /v1/skills/frt-0911/runs/{run_id}/evidence
```
