...
# Table And Constraint Converter
## ELMOS runtime binding

- Invoke this repository Skill as `$pm-b20-table-and-constraint-converter`.
- Immutable source identity: `table-and-constraint-converter` in `precision-migration-b01-44` (B20).
- Runtime adapter: `database-and-data-route`; binding state: `DECLARED`.
- Resolve and plan with `python3 scripts/precision_migration/runtime.py plan --skill pm-b20-table-and-constraint-converter`.
...
转换表、列、主外键、唯一、检查、默认值和依赖顺序。

本 Skill 属于精密语言转换系统。它必须以**可执行证据**为准，不得用模型自评、源码相似度或“能编译”替代语义与行为验证。

## Use when

- 当前任务直接涉及 `table-and-constraint-converter` 所描述的能力。
...
1. 盘点对象与专有能力
2. 执行本 Skill 的核心职责：转换表、列、主外键、唯一、检查、默认值和依赖顺序。
...
```yaml
skill: table-and-constraint-converter
version: 1.0.0
