---
name: b98-contract-signing-provenance
description: "Use when ELMOS must Sign executable contracts and bind them to source Skill, compiler, policy and dependency snapshots. Triggers: Contract Signing and Provenance, Executable Skill Contract and Registry Pack, content addressing, signatures, compiler provenance, dependency snapshot."
metadata:
  id: B98-S10
...
# B98-S10 — Contract Signing and Provenance

## Objective

Sign executable contracts and bind them to source Skill, compiler, policy and dependency snapshots.
...
Use this Skill when the approved task explicitly requires **Contract Signing and Provenance** or when the capability graph selects it as part of a minimal dependency closure.
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b98-contract-signing-provenance/SKILL.md --out /tmp/contract_signing_provenance.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B98-S10
```
...
- Reject or expose signature stripping.
- Reject or expose dependency swap.
- Reject or expose replay of revoked contract.
