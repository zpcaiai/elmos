...
metadata:
  ...
  expected_success_code: "ANALYSIS_CACHE_KEY_DERIVED"
...
- This Skill is bound exactly to repository-owned handler `cache_analysis_stage` with bounded capability state `PARTIAL`, expected success code `ANALYSIS_CACHE_KEY_DERIVED`, and local result state `PARTIAL_LOCAL_EXECUTED`. Dispatch is allowlisted; no fallback or name-derived handler exists.
