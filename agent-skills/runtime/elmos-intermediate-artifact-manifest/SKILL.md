---
name: elmos-intermediate-artifact-manifest
description: Standardize manifests for CST/AST, symbol tables, type/call/dataflow graphs, Semantic IR, mapping plans, generated fragments, patches, build outputs, tests, repair candidates, and certification evidence.
version: 1.0.0
...
# Intermediate Artifact Manifest

## Outcome

Standardize manifests for CST/AST, symbol tables, type/call/dataflow graphs, Semantic IR, mapping plans, generated fragments, patches, build outputs, tests, repair candidates, and certification evidence.
...
- Dependency skills have passed: **elmos-content-addressable-storage, elmos-project-generation-file-staging, elmos-cache-metadata-database**.
...
- Manifest identity is content-addressed after canonical serialization.
- Every artifact declares digest, size, media type, schema, role, producer, ActionKey, dependencies, and validation level.
- Required and optional outputs are distinguishable.
- File-tree manifests reject duplicate, traversal, symlink-escape, reserved-name, and case-colliding paths.
...
1. Define a common envelope and typed payloads for blobs, structured records, file trees, partitions, and evidence bundles.
2. Link each artifact to source snapshot, stage/version, rule pack, target profile, toolchain, and parent artifacts.
3. Record source maps from generated files or symbols back to source AST/IR nodes.
4. Validate completeness, path safety, schema compatibility, provenance, and validation level before sealing.
5. Store manifests in CAS and register searchable metadata and reference edges.
