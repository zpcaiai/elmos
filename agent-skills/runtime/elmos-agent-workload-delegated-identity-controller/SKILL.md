---
name: elmos-agent-workload-delegated-identity-controller
description: Separate human, logical agent, runtime workload and downstream delegated identities using short-lived workload
  credentials and policy-bound token exchange.
version: 4.0.0
...
# Agent Workload Delegated Identity Controller
...
- `Approval`
- `ResidualRisk`
- `AgentWorkloadDelegatedIdentityControllerRun`
- `AgentWorkloadDelegatedIdentityControllerDecision`

Required semantic capabilities:

- Human-to-agent delegation chain
- Workload attestation and short-lived identity
- OAuth token exchange with audience binding
- Environment/attachment authority binding
...
- `identity-topology.json`
- `workload-attestation-policy.yaml`
...
- **shared static credential** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **delegation laundering** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **workload impersonation** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **token exchange escalation** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **stale credential after revocation** — must have a preventive control, a detective signal, a negative fixture and a response action.
...
- valid attestation
- unattested workload denial
- delegation depth limit
- wrong audience denial
- credential rotation
- revoked workload termination
