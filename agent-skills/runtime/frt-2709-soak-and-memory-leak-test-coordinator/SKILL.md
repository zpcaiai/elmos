...
description: "Run FRT-2709 Soak and Memory Leak Test Coordinator for FRT G27 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-2709"
  source_name: "frt-2709-soak-and-memory-leak-test-coordinator"
  source_sha256: "sha256:af716acae43be71f653938c322372b8a5514ef0ca0e7d0ec2a9ff3bc8b387b7b"
  batch: "G27"
...
- Runtime catalog key: `FRT-2709` / `frt-2709-soak-and-memory-leak-test-coordinator`.
...
在G27范围内实现 **Soak and Memory Leak Test Coordinator**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：业务化Workload Model; 并发线性化点、Race、Lost Update、Idempotency Under Load; Load/Stress/Spike/Soak与Recovery。

## When to Use

- 实现或修改与 **Soak and Memory Leak Test Coordinator** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-2709Result`：版本化、带状态和诊断的结构化结果。
- `FRT-2709EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-2709FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-2709CertificateFragment`：供G27批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g27/frt-2709-soak-and-memory-leak-test-coordinator/
packages/runtime/g27/frt-2709-soak-and-memory-leak-test-coordinator/
services/control-plane/g27/frt-2709-soak-and-memory-leak-test-coordinator/
apps/web-console/src/features/g27/frt-2709-soak-and-memory-leak-test-coordinator/
apps/admin-console/src/features/g27/frt-2709-soak-and-memory-leak-test-coordinator/
tests/g27/frt-2709-soak-and-memory-leak-test-coordinator/
```
...
```text
POST /v1/skills/frt-2709/runs
GET  /v1/skills/frt-2709/runs/{run_id}
POST /v1/skills/frt-2709/runs/{run_id}/verify
GET  /v1/skills/frt-2709/runs/{run_id}/findings
GET  /v1/skills/frt-2709/runs/{run_id}/evidence
```
