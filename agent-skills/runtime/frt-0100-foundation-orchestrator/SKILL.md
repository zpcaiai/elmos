...
description: "Run FRT-0100 Foundation Orchestrator for FRT G01 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-0100"
  source_name: "frt-0100-foundation-orchestrator"
  source_sha256: "sha256:4001d4c4d59d10443f8b2355bdb2fe085a1e56e82af01461efd7b9ec03d0414f"
  batch: "G01"
...
- Runtime catalog key: `FRT-0100` / `frt-0100-foundation-orchestrator`.
...
# FRT-0100 — Foundation Orchestrator

## Objective

在G01范围内实现 **Foundation Orchestrator**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：定义系统不可妥协语义与风险等级; 建立可扩展Monorepo与稳定模块边界; 统一Skill Manifest、输入输出Schema、权限、版本与证据协议。

## When to Use

- 实现或修改与 **Foundation Orchestrator** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-0100Result`：版本化、带状态和诊断的结构化结果。
- `FRT-0100EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-0100FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-0100CertificateFragment`：供G01批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g01/frt-0100-foundation-orchestrator/
packages/runtime/g01/frt-0100-foundation-orchestrator/
services/control-plane/g01/frt-0100-foundation-orchestrator/
apps/web-console/src/features/g01/frt-0100-foundation-orchestrator/
apps/admin-console/src/features/g01/frt-0100-foundation-orchestrator/
tests/g01/frt-0100-foundation-orchestrator/
```
...
```text
POST /v1/skills/frt-0100/runs
GET  /v1/skills/frt-0100/runs/{run_id}
POST /v1/skills/frt-0100/runs/{run_id}/verify
GET  /v1/skills/frt-0100/runs/{run_id}/findings
GET  /v1/skills/frt-0100/runs/{run_id}/evidence
```
