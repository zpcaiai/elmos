...
description: "Run FRT-2211 Inventory Procure-to-Availability Closure Pack for FRT G22 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-2211"
  source_name: "frt-2211-inventory-procure-to-availability-closure-pack"
  source_sha256: "sha256:2d8aa85814c39c1f2d391168c3024a458e7b930017e6fffa4dd34e6edbc0dcca"
  batch: "G22"
...
- Runtime catalog key: `FRT-2211` / `frt-2211-inventory-procure-to-availability-closure-pack`.
...
在G22范围内实现 **Inventory Procure-to-Availability Closure Pack**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：Value Stream 与业务Outcome建模; 状态机可达性、终态、Timeout、Cancel、Unknown、Recovery完整性; 跨域Saga、幂等、补偿和Reconciliation。

## When to Use

- 实现或修改与 **Inventory Procure-to-Availability Closure Pack** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-2211Result`：版本化、带状态和诊断的结构化结果。
- `FRT-2211EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-2211FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-2211CertificateFragment`：供G22批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g22/frt-2211-inventory-procure-to-availability-closure-pack/
packages/runtime/g22/frt-2211-inventory-procure-to-availability-closure-pack/
services/control-plane/g22/frt-2211-inventory-procure-to-availability-closure-pack/
apps/web-console/src/features/g22/frt-2211-inventory-procure-to-availability-closure-pack/
apps/admin-console/src/features/g22/frt-2211-inventory-procure-to-availability-closure-pack/
tests/g22/frt-2211-inventory-procure-to-availability-closure-pack/
```
...
```text
POST /v1/skills/frt-2211/runs
GET  /v1/skills/frt-2211/runs/{run_id}
POST /v1/skills/frt-2211/runs/{run_id}/verify
GET  /v1/skills/frt-2211/runs/{run_id}/findings
GET  /v1/skills/frt-2211/runs/{run_id}/evidence
```
