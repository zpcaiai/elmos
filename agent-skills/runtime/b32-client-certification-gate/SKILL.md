...
## Skill 1222: Batch 32 client modernization certification gate

## Use this skill when

- A client pack is being promoted, released, sold, or represented as certified.
- A framework, browser, device, rendering, design-system, security, or quality-profile version changed.
- An independent reviewer needs a deterministic result rather than manually edited status.

## Client-specific risks and invariants

- A green build, screenshots, component tests, or one browser are insufficient evidence for client behavior.
- Certification can be falsified if status is trusted without exact versions, metrics, independent corpora, evidence, and zero-critical checks.
- Accessibility, permission, form, route, hydration, visual, privacy, and test-integrity failures cannot be averaged away.

## Workflow

1. Run skill, pack, and UI IR validators and fail on schema, owner, exact-version, source-map, reference, or critical-unknown errors.
2. Verify pack, certification, target profile, fingerprint, acceptance profile, ownership, corpus separation, baseline governance, and evidence references.
3. Run repository-native source and target build, startup, runtime, browser or device, P0 journey, route, form, identity, accessibility, visual, cross-browser, performance, security, privacy, and supply-chain checks.
4. Verify holdout and representative journeys are populated and independent of transformation development.
5. Evaluate all thresholds and zero-critical fields and write `gate-result.json` plus a human-readable report.
6. Keep or downgrade status to the strongest evidence-backed level and never raise it by editing JSON alone.

## Required repository outputs

- `certification/gate-result.json` with deterministic failures
- `certification/gate-report.md` and evidence references
- A defensible status of certified, limited, experimental, or blocked
- Recertification triggers and customer-facing support profile

## Verification

- Run `python3 scripts/batch32/run_client_gate.py client-packs/<pack-key>`.
- Verify exact tuples, fingerprint digest, nonempty UI IR, exact target profile, approved acceptance profile, holdout, representative journeys, and real evidence files.
- Require all certified thresholds and zero-critical fields encoded by the gate.
- Run a negative test that sets certified status without evidence and confirm rejection.

## Stop and escalate when

- Exact source or target versions, owners, runnable environments, UI IR, target profile, acceptance profile, holdout, representative journey, or evidence is missing.
- Any critical unknown, silent drop, route, form, permission, hydration, accessibility, visual, i18n, security, privacy, or test-integrity regression remains.
- Baselines, masks, tolerances, permissions, dependencies, or tests changed after failure without approved decisions.
- The requested claim exceeds the exact stack, provider, browser, device, locale, theme, rendering, route, component, or journey scope.

## Definition of done

The deterministic gate validates the exact tuple, runtime fingerprint, typed UI IR, real source and target execution, independent corpora, quality matrix, zero-critical requirements, provenance, and evidence, and emits only the strongest defensible status.
