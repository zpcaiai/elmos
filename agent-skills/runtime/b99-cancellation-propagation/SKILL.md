---
name: b99-cancellation-propagation
description: "Use when ELMOS must Propagate user, policy, budget and incident cancellation through workflows, runners and external effects. Triggers: Cancellation Propagation, Durable Runtime Kernel Pack, cancellation tokens, safe points, child propagation, cleanup."
metadata:
  id: B99-S07
...
# B99-S07 — Cancellation Propagation

## Objective

Propagate user, policy, budget and incident cancellation through workflows, runners and external effects.
...
Use this Skill when the approved task explicitly requires **Cancellation Propagation** or when the capability graph selects it as part of a minimal dependency closure.
...
- `cancellation_protocol.json`
- `cancellation_test_report.json`
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b99-cancellation-propagation/SKILL.md --out /tmp/cancellation_propagation.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B99-S07
```
...
- Reject or expose cancel lost at boundary.
- Reject or expose unsafe mid-transaction termination.
- Reject or expose post-cancel artifact publish.
