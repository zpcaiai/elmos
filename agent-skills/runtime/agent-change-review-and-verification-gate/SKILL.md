---
name: agent-change-review-and-verification-gate
description: Apply patch scope and anti-cheating policy, then require independent build/test verification. Use on every Agent patch before it can advance.
---

# Agent Change Review And Verification Gate
...
Require task scope, base/result tree hashes, changed files/lines, commands, test/build changes, patch artifact and required validators.

## Workflow

1. Validate schema version, stable identities, immutable hashes and policy prerequisites.
2. Reject out-of-scope or denied paths, SCM mutation, non-allowlisted commands, forbidden test/build changes, deletion and no-op patches; invoke fresh independent validators.
...
patch-review.json with findings, manual-review flag and validation references.

## Fail-closed rules

Agent-reported tests do not count; high-risk, test/build changes or deletion require human review.
