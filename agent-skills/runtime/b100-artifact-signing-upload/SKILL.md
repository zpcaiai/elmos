---
name: b100-artifact-signing-upload
description: "Use when ELMOS must Hash, sign, scan and upload artifacts through content-addressed, tenant-isolated channels. Triggers: Artifact Signing and Upload, Hardened Runner Fabric Pack, artifact hashing, signing, malware scan, upload."
metadata:
  id: B100-S11
...
# B100-S11 — Artifact Signing and Upload

## Objective

Hash, sign, scan and upload artifacts through content-addressed, tenant-isolated channels.
...
Use this Skill when the approved task explicitly requires **Artifact Signing and Upload** or when the capability graph selects it as part of a minimal dependency closure.
...
- signing key scope exact.
- partial upload not published.
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b100-artifact-signing-upload/SKILL.md --out /tmp/artifact_signing_upload.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B100-S11
```
...
- Reject or expose artifact swap.
- Reject or expose signature after upload mismatch.
- Reject or expose cross-tenant object key.
