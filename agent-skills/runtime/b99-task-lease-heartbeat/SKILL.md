---
name: b99-task-lease-heartbeat
description: "Use when ELMOS must Implement exclusive task claims, heartbeat renewal, expiry and safe reclaim without duplicate effects. Triggers: Task Lease and Heartbeat, Durable Runtime Kernel Pack, leases, heartbeats, expiry, reclaim."
metadata:
  id: B99-S04
...
# B99-S04 — Task Lease and Heartbeat

## Objective

Implement exclusive task claims, heartbeat renewal, expiry and safe reclaim without duplicate effects.
...
Use this Skill when the approved task explicitly requires **Task Lease and Heartbeat** or when the capability graph selects it as part of a minimal dependency closure.
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b99-task-lease-heartbeat/SKILL.md --out /tmp/task_lease_heartbeat.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B99-S04
```
...
- Reject or expose split brain.
- Reject or expose lease extension after cancellation.
- Reject or expose stale completion accepted.
