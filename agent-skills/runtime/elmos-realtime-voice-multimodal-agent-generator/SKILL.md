---
name: elmos-realtime-voice-multimodal-agent-generator
description: Generate low-latency audio/video/text agents with session, turn-taking, interruption, tool use, consent, safety and fallback contracts.
version: 4.0.0
...
# Realtime Voice and Multimodal Agent Generator
...
- `RealtimeVoiceAndMultimodalAgentGeneratorProfile`
- `RealtimeVoiceAndMultimodalAgentGeneratorPlan`
- `RealtimeVoiceAndMultimodalAgentGeneratorRun`
- `RealtimeVoiceAndMultimodalAgentGeneratorDecision`

Required semantic capabilities:

- compile realtime session and media state machine
- generate WebRTC/SIP/streaming transport integration
- handle barge-in, VAD, turn and tool synchronization
- apply consent, redaction and recording policy
- measure latency, recovery and degraded text fallback
...
- `artifacts/elmos-realtime-voice-multimodal-agent-generator/profile.yaml`
- `artifacts/elmos-realtime-voice-multimodal-agent-generator/plan.json`
- `artifacts/elmos-realtime-voice-multimodal-agent-generator/result.json`
- `artifacts/elmos-realtime-voice-multimodal-agent-generator/evidence/`
...
- **recording without consent** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **audio tool result attached to wrong turn** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **barge-in leaves side effect running** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **media stream exfiltrates data** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **latency hides safety classifier timeout** — must have a preventive control, a detective signal, a negative fixture and a response action.
...
- native scenario: compile realtime session and media state machine
- native scenario: generate WebRTC/SIP/streaming transport integration
- native scenario: handle barge-in, VAD, turn and tool synchronization
- native scenario: apply consent, redaction and recording policy
- native scenario: measure latency, recovery and degraded text fallback
