...
description: "Run FRT-0409 Reactive State Extractor for FRT G04 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-0409"
  source_name: "frt-0409-reactive-state-extractor"
  source_sha256: "sha256:59a5448788df9137b13cd349f70c1925f6b23e8043e05f1b075f6c212d98b935"
  batch: "G04"
...
- Runtime catalog key: `FRT-0409` / `frt-0409-reactive-state-extractor`.
...
# FRT-0409 — Reactive State Extractor

## Objective

在G04范围内实现 **Reactive State Extractor**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：每个Adapter只解析源，不生成目标代码; 恢复组件、状态、事件、生命周期、路由、样式和平台能力; 统一动态语法、宏、装饰器和模板表达式。

## When to Use

- 实现或修改与 **Reactive State Extractor** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-0409Result`：版本化、带状态和诊断的结构化结果。
- `FRT-0409EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-0409FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-0409CertificateFragment`：供G04批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g04/frt-0409-reactive-state-extractor/
packages/runtime/g04/frt-0409-reactive-state-extractor/
services/control-plane/g04/frt-0409-reactive-state-extractor/
apps/web-console/src/features/g04/frt-0409-reactive-state-extractor/
apps/admin-console/src/features/g04/frt-0409-reactive-state-extractor/
tests/g04/frt-0409-reactive-state-extractor/
```
...
```text
POST /v1/skills/frt-0409/runs
GET  /v1/skills/frt-0409/runs/{run_id}
POST /v1/skills/frt-0409/runs/{run_id}/verify
GET  /v1/skills/frt-0409/runs/{run_id}/findings
GET  /v1/skills/frt-0409/runs/{run_id}/evidence
```
