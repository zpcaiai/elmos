...
description: "Run FRT-0907 Resource Ownership and Cleanup Mapper for FRT G09 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-0907"
  source_name: "frt-0907-resource-ownership-and-cleanup-mapper"
  source_sha256: "sha256:cff7702f6b4cc7109021e2dc355b32b72e978c16fac6a7cde9b6c019c1d9361f"
  batch: "G09"
...
- Runtime catalog key: `FRT-0907` / `frt-0907-resource-ownership-and-cleanup-mapper`.
...
# FRT-0907 — Resource Ownership and Cleanup Mapper

## Objective

在G09范围内实现 **Resource Ownership and Cleanup Mapper**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：区分Local、Server、Global、Persistent和Derived State; 保持Effect触发、Cleanup、Lifecycle和Evaluation Order; 传播Cancellation并防止Stale Result Commit。

## When to Use

- 实现或修改与 **Resource Ownership and Cleanup Mapper** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-0907Result`：版本化、带状态和诊断的结构化结果。
- `FRT-0907EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-0907FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-0907CertificateFragment`：供G09批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g09/frt-0907-resource-ownership-and-cleanup-mapper/
packages/runtime/g09/frt-0907-resource-ownership-and-cleanup-mapper/
services/control-plane/g09/frt-0907-resource-ownership-and-cleanup-mapper/
apps/web-console/src/features/g09/frt-0907-resource-ownership-and-cleanup-mapper/
apps/admin-console/src/features/g09/frt-0907-resource-ownership-and-cleanup-mapper/
tests/g09/frt-0907-resource-ownership-and-cleanup-mapper/
```
...
```text
POST /v1/skills/frt-0907/runs
GET  /v1/skills/frt-0907/runs/{run_id}
POST /v1/skills/frt-0907/runs/{run_id}/verify
GET  /v1/skills/frt-0907/runs/{run_id}/findings
GET  /v1/skills/frt-0907/runs/{run_id}/evidence
```
