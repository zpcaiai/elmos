---
name: session-retry-run-state
description: 把 retry/backoff、运行状态和 terminal reason 从 prompt 逻辑下沉为 runtime。 Use for this exact ELMOS Spring Golden Route v2 contract; imported specifications are not runtime or certification evidence.
...
metadata:
  ...
  source_id: FOUNDATION-08-session-retry-run-state
  source_name: session-retry-run-state
  source_path: skills/session-retry-run-state/SKILL.md
  source_sha256: sha256:94397a6b3f0b481c173300816bd8bd43f3ea186dae64dba2a1e42ed95b127898
  source_contract_sha256: sha256:128f1874fd77d5f5d8dca330192d32247328f3c63bb3c9f82a0e924146e66a9d
...
# Session Retry & Run State

## Goal

把 retry/backoff、运行状态和 terminal reason 从 prompt 逻辑下沉为 runtime。

## Use when

在 Elmos 需要实现或升级 **Session Retry & Run State** 相关能力时使用；它不是仅供解释的提示词，而是实现、测试与验收契约。

## Capabilities to implement

- 区分可重试 provider 错误、tool 错误、用户取消和不可恢复错误
- 重试 attempt 有独立 ID 与原因
- 绝不跨 durable side-effect boundary 盲目重试
- run state 可在 UI/API/恢复逻辑中统一读取
...
- [ ] 定义 `session-retry-run-state` 的 public interface、input/output/error schemas。
- [ ] 建立 `session-retry-run-state` 的 capability/feature flags 与版本字段。
...
- [ ] 至少覆盖并验收：区分可重试 provider 错误、tool 错误、用户取消和不可恢复错误；以及 重试 attempt 有独立 ID 与原因。
...
- Machine contract: `references/contract.json` (`sha256:128f1874fd77d5f5d8dca330192d32247328f3c63bb3c9f82a0e924146e66a9d`).
