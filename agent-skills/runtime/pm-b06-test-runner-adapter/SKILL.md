...
- Invoke this repository Skill as `$pm-b06-test-runner-adapter`.
- Immutable source identity: `test-runner-adapter` in `precision-migration-b01-44` (B06).
- Runtime adapter: `semantic-recovery-and-ir`; binding state: `UNAVAILABLE`.
- Resolve and plan with `python3 scripts/precision_migration/runtime.py plan --skill pm-b06-test-runner-adapter`.
...
1. 发现仓库与环境
2. 执行本 Skill 的核心职责：统一发现、选择、执行和解析不同语言及框架测试运行器。
3. 使用原生工具提取语义
4. 建立可复现工具链和沙箱
5. 执行最小验证任务
6. 持久化摘要、哈希和证据
