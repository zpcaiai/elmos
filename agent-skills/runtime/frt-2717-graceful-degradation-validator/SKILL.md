...
description: "Run FRT-2717 Graceful Degradation Validator for FRT G27 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-2717"
  source_name: "frt-2717-graceful-degradation-validator"
  source_sha256: "sha256:618c6cb3a1668adcee3594d28d74827f78be83b53ce0b105faa3ecb59649d43f"
  batch: "G27"
...
- Runtime catalog key: `FRT-2717` / `frt-2717-graceful-degradation-validator`.
...
在G27范围内实现 **Graceful Degradation Validator**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：业务化Workload Model; 并发线性化点、Race、Lost Update、Idempotency Under Load; Load/Stress/Spike/Soak与Recovery。

## When to Use

- 实现或修改与 **Graceful Degradation Validator** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-2717Result`：版本化、带状态和诊断的结构化结果。
- `FRT-2717EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-2717FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-2717CertificateFragment`：供G27批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g27/frt-2717-graceful-degradation-validator/
packages/runtime/g27/frt-2717-graceful-degradation-validator/
services/control-plane/g27/frt-2717-graceful-degradation-validator/
apps/web-console/src/features/g27/frt-2717-graceful-degradation-validator/
apps/admin-console/src/features/g27/frt-2717-graceful-degradation-validator/
tests/g27/frt-2717-graceful-degradation-validator/
```
...
```text
POST /v1/skills/frt-2717/runs
GET  /v1/skills/frt-2717/runs/{run_id}
POST /v1/skills/frt-2717/runs/{run_id}/verify
GET  /v1/skills/frt-2717/runs/{run_id}/findings
GET  /v1/skills/frt-2717/runs/{run_id}/evidence
```
