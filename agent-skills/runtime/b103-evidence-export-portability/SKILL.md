---
name: b103-evidence-export-portability
description: "Use when ELMOS must Export redacted, signed and independently verifiable evidence packs for customers, auditors and air-gapped review. Triggers: Evidence Export and Portability, Evidence and Certification Fabric Pack, evidence export, redaction, portable verification, air gap."
metadata:
  id: B103-S14
...
# B103-S14 — Evidence Export and Portability

## Objective

Export redacted, signed and independently verifiable evidence packs for customers, auditors and air-gapped review.
...
Use this Skill when the approved task explicitly requires **Evidence Export and Portability** or when the capability graph selects it as part of a minimal dependency closure.
...
- offline hash/signature verification.
- secrets absent.
- scope preserved.
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b103-evidence-export-portability/SKILL.md --out /tmp/evidence_export_portability.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B103-S14
```
...
- Reject or expose raw secret export.
- Reject or expose missing artifact.
- Reject or expose signature verifier downloads network dependency.
