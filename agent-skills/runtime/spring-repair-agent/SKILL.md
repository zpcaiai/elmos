---
name: spring-repair-agent
description: 只处理确定性规则无法覆盖的业务特定缺口，生成最小可回滚 Patch。 Use for this exact ELMOS Spring Golden Route v2 contract; imported specifications are not runtime or certification evidence.
...
metadata:
  ...
  source_id: ELMOS-GR-050
  source_name: spring-repair-agent
  source_path: skills/spring-repair-agent/SKILL.md
  source_sha256: sha256:6be81ce803dadf2cb1efd0b08d07d2ddfe05bcf27a5bda1ff94f823fa93acc90
  source_contract_sha256: sha256:621fb56e543420b48e1c6fa3518cf7f6e20848781c0fcea9d22296aa6bef044e
...
只处理确定性规则无法覆盖的业务特定缺口，生成最小可回滚 Patch。
...
- [ ] 定义 `spring-repair-agent` 的 public interface、输入/输出/Error Schema 和版本策略。
- [ ] 实现 `spring-repair-agent` 的 production adapter/service，不把业务状态只保存在 Prompt 或内存对象中。
...
- [ ] 至少生成：agent-patch.json, repair-rationale.md, uncertainty.json。
...
- [ ] 本 Skill 的 required outputs 均由真实目标仓库执行产生：agent-patch.json, repair-rationale.md, uncertainty.json。
- [ ] 所有 mandatory tests 和 security gates 通过，且无 critical unknown。

## Dependencies

- `spring-diagnostic-classifier`
...
- Machine contract: `references/contract.json` (`sha256:621fb56e543420b48e1c6fa3518cf7f6e20848781c0fcea9d22296aa6bef044e`).
