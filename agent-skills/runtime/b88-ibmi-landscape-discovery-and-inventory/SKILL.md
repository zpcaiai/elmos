---
name: b88-ibmi-landscape-discovery-and-inventory
description: "Use when ELMOS must run ibmi landscape discovery and inventory for Batch 88 IBM i RPG / CL / DDS / DB2 for i Pack. Preserve native semantics, safety boundaries, ownership, traceability, and fail-closed evidence; require real vendor toolchain execution for runtime claims."
metadata:
  source_package: "elmos-language-packs-batch81-95-complete"
  source_id: "PG307"
  source_name: "ibmi-landscape-discovery-and-inventory"
  source_sha256: "sha256:34b1f2e63bcdbd58dcedbd6ee7d5011e272cd328f9333521b8029391767ebd5a"
  batch: 88
  source_engine: "elmos.language-packs"
  source_status: "proposed"
  normalized_namespace: "language-pack"
---
# Batch 88 Language Pack / source PG307 — Ibmi Landscape Discovery And Inventory

## 1. Objective

Discover and inventory ibmi landscape discovery and inventory within the IBM i RPG / CL / DDS / DB2 for i Pack, preserving platform semantics, traceability, security, and reproducible evidence.

## 2. Scope

This Skill is part of **Batch 88: IBM i RPG / CL / DDS / DB2 for i Pack**.

### In scope

- Process RPG III/IV.
- Process free-form RPG.
- Process CL.
- Process DDS.
- Process display/printer files.
- Process DB2 for i.
- Process data queues.
- Process service programs.
- Produce RPG IR.
- Produce Java/.NET services.
- Produce SQL DDL.
- Produce web UI.
- Produce workflow/batch jobs.
...
2. Process the relevant artifacts for Ibmi Landscape Discovery And Inventory using the approved enterprise-ibmi semantic contracts and toolchain versions.
3. Preserve source provenance and create Artifact Graph links across inputs, outputs, tests, approvals, and evidence.
4. Detect and classify zoned/packed decimal drift, record-format mismatch, activation-group semantics; never convert uncertainty or unsupported behavior into a silent success.
...
7. Apply the safety boundary: Record formats, decimal semantics, commitment control, and operational job behavior require production-representative parallel validation.
...
- `ZONED_PACKED_DECIMAL_DRIFT` — stop unsafe continuation and emit source-linked diagnostic evidence.
- `RECORD_FORMAT_MISMATCH` — stop unsafe continuation and emit source-linked diagnostic evidence.
- `ACTIVATION_GROUP_SEMANTICS` — stop unsafe continuation and emit source-linked diagnostic evidence.
- `COMMITMENT_CONTROL_DRIFT` — stop unsafe continuation and emit source-linked diagnostic evidence.
- `JOB_QUEUE_ORDERING` — stop unsafe continuation and emit source-linked diagnostic evidence.
- `5250_FIELD_BEHAVIOR` — stop unsafe continuation and emit source-linked diagnostic evidence.
...
- Apply this Batch safety boundary: **Record formats, decimal semantics, commitment control, and operational job behavior require production-representative parallel validation.**

## 14. Unit Tests

- A representative valid ibmi-rpg input produces all declared outputs for ibmi-landscape-discovery-and-inventory.
...
- At least one batch-specific risk (zoned/packed decimal drift) is detected by a negative fixture.

## 15. Integration Tests

- Consume approved Project Synthesis or Modernization baselines and write complete lineage into the Artifact Graph.
- Integrate with at least one representative toolchain from: IBM i, ILE RPG, ILE CL, DB2 for i.
...
- Reject or block zoned/packed decimal drift without weakening the expected result.
- Reject or block record-format mismatch without weakening the expected result.
- Reject or block activation-group semantics without weakening the expected result.
- Reject or block commitment-control drift without weakening the expected result.
- Reject or block job queue ordering without weakening the expected result.
- Reject or block 5250 field behavior without weakening the expected result.
