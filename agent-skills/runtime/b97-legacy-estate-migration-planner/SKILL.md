---
name: b97-legacy-estate-migration-planner
description: "Use when ELMOS must Plan non-destructive migration from Batch 1–96 manifests into the canonical graph with rollback and reconciliation. Triggers: Legacy Estate Migration Planner, Canonical Capability Graph Consolidation Pack, migration plan, identity mapping, rollback, reconciliation."
metadata:
  id: B97-S14
...
# B97-S14 — Legacy Estate Migration Planner

## Objective

Plan non-destructive migration from Batch 1–96 manifests into the canonical graph with rollback and reconciliation.
...
Use this Skill when the approved task explicitly requires **Legacy Estate Migration Planner** or when the capability graph selects it as part of a minimal dependency closure.
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b97-legacy-estate-migration-planner/SKILL.md --out /tmp/legacy_estate_migration_planner.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B97-S14
```
...
- Reject or expose ID overwrite.
- Reject or expose partial migration presented as complete.
- Reject or expose unmapped critical Skill.
