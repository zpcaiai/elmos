...
description: "Run FRT-2415 Audit and Security Administration for FRT G24 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-2415"
  source_name: "frt-2415-audit-and-security-administration"
  source_sha256: "sha256:d61f0c620e35f49efe09bccd78656bf4abf82cc69ee347429a16f2b5b14a6f3e"
  batch: "G24"
...
- Runtime catalog key: `FRT-2415` / `frt-2415-audit-and-security-administration`.
...
在G24范围内实现 **Audit and Security Administration**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：Admin Object × Action × Role × Scope × Environment矩阵; 高风险操作Impact Preview、Reason、Approval、Typed Command、Audit; Manual Case、Data Repair、Reconciliation正式后台流程。

## When to Use

- 实现或修改与 **Audit and Security Administration** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-2415Result`：版本化、带状态和诊断的结构化结果。
- `FRT-2415EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-2415FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-2415CertificateFragment`：供G24批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g24/frt-2415-audit-and-security-administration/
packages/runtime/g24/frt-2415-audit-and-security-administration/
services/control-plane/g24/frt-2415-audit-and-security-administration/
apps/web-console/src/features/g24/frt-2415-audit-and-security-administration/
apps/admin-console/src/features/g24/frt-2415-audit-and-security-administration/
tests/g24/frt-2415-audit-and-security-administration/
```
...
```text
POST /v1/skills/frt-2415/runs
GET  /v1/skills/frt-2415/runs/{run_id}
POST /v1/skills/frt-2415/runs/{run_id}/verify
GET  /v1/skills/frt-2415/runs/{run_id}/findings
GET  /v1/skills/frt-2415/runs/{run_id}/evidence
```
