---
name: pm-b34-semantic-core-lean-model
description: "把 Semantic Core、类型、Effect、状态和求值关系编码为 Lean 定义. Precision Migration B34 contract; use for this exact assessment, transformation, validation, repair, evidence, or cutover scope."
---

# Semantic Core Lean Model
## ELMOS runtime binding

- Invoke this repository Skill as `$pm-b34-semantic-core-lean-model`.
- Immutable source identity: `semantic-core-lean-model` in `precision-migration-b01-44` (B34).
- Runtime adapter: `formal-and-advanced-verification`; binding state: `UNAVAILABLE`.
- Resolve and plan with `python3 scripts/precision_migration/runtime.py plan --skill pm-b34-semantic-core-lean-model`.
...
把 Semantic Core、类型、Effect、状态和求值关系编码为 Lean 定义。

本 Skill 属于精密语言转换系统。它必须以**可执行证据**为准，不得用模型自评、源码相似度或“能编译”替代语义与行为验证。

## Use when

- 当前任务直接涉及 `semantic-core-lean-model` 所描述的能力。
...
1. 固定语义与定理陈述
2. 执行本 Skill 的核心职责：把 Semantic Core、类型、Effect、状态和求值关系编码为 Lean 定义。
3. 生成或编码证明义务
4. 调用Lean/SMT/模型检查
5. 最小化反例并回写测试
6. 由可信内核/求解器签发结果
...
```yaml
skill: semantic-core-lean-model
version: 1.0.0
