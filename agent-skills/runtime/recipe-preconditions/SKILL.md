---
name: recipe-preconditions
description: 变换前先做类型、框架、文件和版本前置判断，降低误改。 Use for this exact ELMOS Spring Golden Route v2 contract; imported specifications are not runtime or certification evidence.
...
metadata:
  ...
  source_id: FOUNDATION-02-recipe-preconditions
  source_name: recipe-preconditions
  source_path: skills/recipe-preconditions/SKILL.md
  source_sha256: sha256:3fe294e5a4ab31f4be009b13c368b3f506f468ddb4119ef4753d450e2b03d1d0
  source_contract_sha256: sha256:4cf7867978f073133724027ab42d636bac90010c687a5dfd2eba186fdecf41fa
...
变换前先做类型、框架、文件和版本前置判断，降低误改。

## Use when

在 Elmos 需要实现或升级 **Recipe Preconditions** 相关能力时使用；它不是仅供解释的提示词，而是实现、测试与验收契约。

## Capabilities to implement

- 前置条件与真正 edit visitor 分离
- precondition 可组合 AND/OR/NOT
- 未满足时保持源码零修改
- 输出为何跳过的结构化证据
...
- [ ] 定义 `recipe-preconditions` 的 public interface、input/output/error schemas。
- [ ] 建立 `recipe-preconditions` 的 capability/feature flags 与版本字段。
...
- [ ] 至少覆盖并验收：前置条件与真正 edit visitor 分离；以及 precondition 可组合 AND/OR/NOT。
...
- Machine contract: `references/contract.json` (`sha256:4cf7867978f073133724027ab42d636bac90010c687a5dfd2eba186fdecf41fa`).
