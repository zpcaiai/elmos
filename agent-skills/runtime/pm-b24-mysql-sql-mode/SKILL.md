---
name: pm-b24-mysql-sql-mode
description: "迁移 STRICT、ONLY_FULL_GROUP_BY、NO_ZERO_DATE 等 SQL Mode 影响. Precision Migration B24 contract; use for this exact assessment, transformation, validation, repair, evidence, or cutover scope."
---

# Mysql Sql Mode
## ELMOS runtime binding

- Invoke this repository Skill as `$pm-b24-mysql-sql-mode`.
- Immutable source identity: `mysql-sql-mode` in `precision-migration-b01-44` (B24).
- Runtime adapter: `database-and-data-route`; binding state: `DECLARED`.
- Resolve and plan with `python3 scripts/precision_migration/runtime.py plan --skill pm-b24-mysql-sql-mode`.
...
迁移 STRICT、ONLY_FULL_GROUP_BY、NO_ZERO_DATE 等 SQL Mode 影响。

本 Skill 属于精密语言转换系统。它必须以**可执行证据**为准，不得用模型自评、源码相似度或“能编译”替代语义与行为验证。

## Use when

- 当前任务直接涉及 `mysql-sql-mode` 所描述的能力。
...
1. 盘点对象与专有能力
2. 执行本 Skill 的核心职责：迁移 STRICT、ONLY_FULL_GROUP_BY、NO_ZERO_DATE 等 SQL Mode 影响。
