...
metadata:
  ...
  runtime_handler_id: "validate_artifact_version_proposal"
  capability_state: "PARTIAL"
  expected_success_code: "ARTIFACT_VERSION_PROPOSAL_VALIDATED"
...
- This Skill is bound exactly to repository-owned handler `validate_artifact_version_proposal` with bounded capability state `PARTIAL`, expected success code `ARTIFACT_VERSION_PROPOSAL_VALIDATED`, and local result state `PARTIAL_LOCAL_EXECUTED`. Dispatch is allowlisted; no fallback or name-derived handler exists.
