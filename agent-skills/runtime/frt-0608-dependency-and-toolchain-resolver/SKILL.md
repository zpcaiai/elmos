...
description: "Run FRT-0608 Dependency and Toolchain Resolver for FRT G06 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-0608"
  source_name: "frt-0608-dependency-and-toolchain-resolver"
  source_sha256: "sha256:8b17d8aa5c29e3e82320e6c96ea69cde0d6bcbb7d4dcc4dcc5c443f953067fa0"
  batch: "G06"
...
- Runtime catalog key: `FRT-0608` / `frt-0608-dependency-and-toolchain-resolver`.
...
# FRT-0608 — Dependency and Toolchain Resolver

## Objective

在G06范围内实现 **Dependency and Toolchain Resolver**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：按目标生态生成模块、路由、状态、DI、配置和Build边界; 禁止机械复制源目录结构; 锁定目标版本、依赖和工具链。

## When to Use

- 实现或修改与 **Dependency and Toolchain Resolver** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-0608Result`：版本化、带状态和诊断的结构化结果。
- `FRT-0608EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-0608FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-0608CertificateFragment`：供G06批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g06/frt-0608-dependency-and-toolchain-resolver/
packages/runtime/g06/frt-0608-dependency-and-toolchain-resolver/
services/control-plane/g06/frt-0608-dependency-and-toolchain-resolver/
apps/web-console/src/features/g06/frt-0608-dependency-and-toolchain-resolver/
apps/admin-console/src/features/g06/frt-0608-dependency-and-toolchain-resolver/
tests/g06/frt-0608-dependency-and-toolchain-resolver/
```
...
```text
POST /v1/skills/frt-0608/runs
GET  /v1/skills/frt-0608/runs/{run_id}
POST /v1/skills/frt-0608/runs/{run_id}/verify
GET  /v1/skills/frt-0608/runs/{run_id}/findings
GET  /v1/skills/frt-0608/runs/{run_id}/evidence
```
