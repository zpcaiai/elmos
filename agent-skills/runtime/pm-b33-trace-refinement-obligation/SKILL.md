---
name: pm-b33-trace-refinement-obligation
description: "生成目标可观察 Trace 对源合法 Trace 的精化或等价义务. Precision Migration B33 contract; use for this exact assessment, transformation, validation, repair, evidence, or cutover scope."
---

# Trace Refinement Obligation
...
生成目标可观察 Trace 对源合法 Trace 的精化或等价义务。

本 Skill 属于精密语言转换系统。它必须以**可执行证据**为准，不得用模型自评、源码相似度或“能编译”替代语义与行为验证。

## Use when

- 当前任务直接涉及 `trace-refinement-obligation` 所描述的能力。
...
1. 固定语义与定理陈述
2. 执行本 Skill 的核心职责：生成目标可观察 Trace 对源合法 Trace 的精化或等价义务。
3. 生成或编码证明义务
4. 调用Lean/SMT/模型检查
5. 最小化反例并回写测试
6. 由可信内核/求解器签发结果
...
```yaml
skill: trace-refinement-obligation
version: 1.0.0
