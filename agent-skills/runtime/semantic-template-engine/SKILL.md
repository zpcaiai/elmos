---
name: semantic-template-engine
description: 用可解析代码模板完成语义替换，而不是字符串拼接。 Use for this exact ELMOS Spring Golden Route v2 contract; imported specifications are not runtime or certification evidence.
...
metadata:
  ...
  source_id: FOUNDATION-03-semantic-template-engine
  source_name: semantic-template-engine
  source_path: skills/semantic-template-engine/SKILL.md
  source_sha256: sha256:a6fc71921d04469af8bb4c539be86b324c7476c3b4669f91ef9fcd6fbd035c70
  source_contract_sha256: sha256:7e654b115cbee10049f30414c6b41b9556986ba499ec6349d56983c0bb794800
...
# Semantic Template Engine

## Goal

用可解析代码模板完成语义替换，而不是字符串拼接。

## Use when

在 Elmos 需要实现或升级 **Semantic Template Engine** 相关能力时使用；它不是仅供解释的提示词，而是实现、测试与验收契约。

## Capabilities to implement

- pattern 和 template 均先解析为语义树
- 模板参数保留 AST 类型和格式语义
- 支持上下文声明辅助 type attribution
- 替换后再次走 parser/printer 质量门禁
...
- [ ] 定义 `semantic-template-engine` 的 public interface、input/output/error schemas。
- [ ] 建立 `semantic-template-engine` 的 capability/feature flags 与版本字段。
...
- [ ] 至少覆盖并验收：pattern 和 template 均先解析为语义树；以及 模板参数保留 AST 类型和格式语义。
...
- Machine contract: `references/contract.json` (`sha256:7e654b115cbee10049f30414c6b41b9556986ba499ec6349d56983c0bb794800`).
