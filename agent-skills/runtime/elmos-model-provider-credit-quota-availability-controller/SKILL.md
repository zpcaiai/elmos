---
name: elmos-model-provider-credit-quota-availability-controller
description: Track subscription/credit/quota, real-time usage, rate limits, regional availability and failover capacity across model providers.
version: 4.0.0
...
# Model Provider Credit, Quota and Availability Controller
...
- `ModelProviderCreditQuotaAndAvailabilityControllerProfile`
- `ModelProviderCreditQuotaAndAvailabilityControllerPlan`
- `ModelProviderCreditQuotaAndAvailabilityControllerRun`
- `ModelProviderCreditQuotaAndAvailabilityControllerDecision`

Required semantic capabilities:

- ingest quota, credit and rate-limit state
- reserve capacity per run and tenant
- forecast depletion and reset windows
- coordinate safe provider fallback
- reconcile provider usage and invoices
...
- `artifacts/elmos-model-provider-credit-quota-availability-controller/profile.yaml`
- `artifacts/elmos-model-provider-credit-quota-availability-controller/plan.json`
- `artifacts/elmos-model-provider-credit-quota-availability-controller/result.json`
- `artifacts/elmos-model-provider-credit-quota-availability-controller/evidence/`
...
- **quota API stale** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **credits double-reserved** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **reset timezone wrong** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **fallback capacity not real** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **billing usage sign interpreted incorrectly** — must have a preventive control, a detective signal, a negative fixture and a response action.
...
- native scenario: ingest quota, credit and rate-limit state
- native scenario: reserve capacity per run and tenant
- native scenario: forecast depletion and reset windows
- native scenario: coordinate safe provider fallback
- native scenario: reconcile provider usage and invoices
