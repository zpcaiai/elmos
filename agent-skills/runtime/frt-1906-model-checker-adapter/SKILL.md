...
description: "Run FRT-1906 Model Checker Adapter for FRT G19 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-1906"
  source_name: "frt-1906-model-checker-adapter"
  source_sha256: "sha256:12494a3039daf96760e33e6094c7dd871f57b8226285f90a0c6901ad0c38e1dd"
  batch: "G19"
...
- Runtime catalog key: `FRT-1906` / `frt-1906-model-checker-adapter`.
...
# FRT-1906 — Model Checker Adapter

## Objective

在G19范围内实现 **Model Checker Adapter**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：从IR、Rule和业务不变量生成Proof Obligation; Leanstral仅提出证明候选，Lean Kernel为可信根; 组合SMT、Model Checking、Symbolic和Differential Evidence。

## When to Use

- 实现或修改与 **Model Checker Adapter** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-1906Result`：版本化、带状态和诊断的结构化结果。
- `FRT-1906EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-1906FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-1906CertificateFragment`：供G19批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g19/frt-1906-model-checker-adapter/
packages/runtime/g19/frt-1906-model-checker-adapter/
services/control-plane/g19/frt-1906-model-checker-adapter/
apps/web-console/src/features/g19/frt-1906-model-checker-adapter/
apps/admin-console/src/features/g19/frt-1906-model-checker-adapter/
tests/g19/frt-1906-model-checker-adapter/
```
...
```text
POST /v1/skills/frt-1906/runs
GET  /v1/skills/frt-1906/runs/{run_id}
POST /v1/skills/frt-1906/runs/{run_id}/verify
GET  /v1/skills/frt-1906/runs/{run_id}/findings
GET  /v1/skills/frt-1906/runs/{run_id}/evidence
```
