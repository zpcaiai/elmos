...
description: "Run FRT-2816 Safe Mode and Read-Only Mode Generator for FRT G28 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-2816"
  source_name: "frt-2816-safe-mode-and-read-only-mode-generator"
  source_sha256: "sha256:057834bbb87dd3a44eae085649342f70ea168c9e212cf323ab56a11c7f99fcc7"
  batch: "G28"
...
- Runtime catalog key: `FRT-2816` / `frt-2816-safe-mode-and-read-only-mode-generator`.
...
在G28范围内实现 **Safe Mode and Read-Only Mode Generator**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：Dependency Failure Model与Common Cause分析; 端到端Deadline、Retry Budget、Circuit Breaker、Bulkhead; Queue Durability、Replay、Stateful Failover、Fence Token。

## When to Use

- 实现或修改与 **Safe Mode and Read-Only Mode Generator** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-2816Result`：版本化、带状态和诊断的结构化结果。
- `FRT-2816EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-2816FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-2816CertificateFragment`：供G28批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g28/frt-2816-safe-mode-and-read-only-mode-generator/
packages/runtime/g28/frt-2816-safe-mode-and-read-only-mode-generator/
services/control-plane/g28/frt-2816-safe-mode-and-read-only-mode-generator/
apps/web-console/src/features/g28/frt-2816-safe-mode-and-read-only-mode-generator/
apps/admin-console/src/features/g28/frt-2816-safe-mode-and-read-only-mode-generator/
tests/g28/frt-2816-safe-mode-and-read-only-mode-generator/
```
...
```text
POST /v1/skills/frt-2816/runs
GET  /v1/skills/frt-2816/runs/{run_id}
POST /v1/skills/frt-2816/runs/{run_id}/verify
GET  /v1/skills/frt-2816/runs/{run_id}/findings
GET  /v1/skills/frt-2816/runs/{run_id}/evidence
```
