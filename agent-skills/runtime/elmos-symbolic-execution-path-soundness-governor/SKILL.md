---
name: elmos-symbolic-execution-path-soundness-governor
description: Implement and independently certify symbolic execution path soundness governor, including model path constraints, environment, native calls, concurrency and solver limitations, measure path coverage, concretization and infeasible-path handling and cross-check generated counterexamples on native runtime.
version: 4.0.0
...
# Symbolic Execution Path Soundness Governor

## Objective

Implement and independently certify symbolic execution path soundness governor, including model path constraints, environment, native calls, concurrency and solver limitations, measure path coverage, concretization and infeasible-path handling and cross-check generated counterexamples on native runtime.
...
- **Kernel authorities used:** K3, K4, K6, K8
...
- `SymbolicExecutionPathSoundnessGovernorProfile`
- `SymbolicExecutionPathSoundnessGovernorRun`
- `SymbolicExecutionPathSoundnessGovernorFinding`
- `SymbolicExecutionPathSoundnessGovernorDecision`
- `SymbolicExecutionPathSoundnessGovernorEvidence`

Required semantic capabilities:

- model path constraints, environment, native calls, concurrency and solver limitations
- measure path coverage, concretization and infeasible-path handling
- cross-check generated counterexamples on native runtime
...
- No local Skill dependency.

### External Package Dependencies

- `elmos-verification-tool-validation-corpus-governor` from `elmos-functional-assurance-certification-skills` — optional companion; required only for E4/E5/P05 certification.

## Primary Outputs

- `artifacts/elmos-symbolic-execution-path-soundness-governor/profile.yaml`
- `artifacts/elmos-symbolic-execution-path-soundness-governor/plan.json`
- `artifacts/elmos-symbolic-execution-path-soundness-governor/result.json`
- `artifacts/elmos-symbolic-execution-path-soundness-governor/evidence/`
- `artifacts/elmos-symbolic-execution-path-soundness-governor/assurance-case.json`
...
- **unsupported instruction is modeled optimistically** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **path explosion truncation is hidden** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **counterexample cannot reproduce concretely** — must have a preventive control, a detective signal, a negative fixture and a response action.
...
- native scenario: model path constraints, environment, native calls, concurrency and solver limitations
- native scenario: measure path coverage, concretization and infeasible-path handling
- native scenario: cross-check generated counterexamples on native runtime
