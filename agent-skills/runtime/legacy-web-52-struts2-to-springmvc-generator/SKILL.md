---
name: legacy-web-52-struts2-to-springmvc-generator
description: "Repository-owned exact runtime interface for Struts 2→Spring MVC 生成器; bounded semantic analysis and evidence generation for Java legacy web modernization."
metadata:
  source_package: elmos.java-legacy-web.repository-modernization
  source_version: 1.0.0
  source_id: 52-struts2-to-springmvc-generator
  source_digest: sha256:1fb70788b21050779c3dac41c8e05e20aa3201681ae1b277dee43ce201a08495
  phase: transformation
  runtime_state: CODE_COMPLETE_LOCAL
  capability_state: LOCAL_EXECUTED
  operation_code: STRUTS2_TARGET_GENERATED
  runtime_handler_id: legacy-web-handler:52-struts2-to-springmvc-generator
---

# Struts 2→Spring MVC 生成器
...
---
id: 52-struts2-to-springmvc-generator
title: Struts 2→Spring MVC 生成器
version: 1.0.0
phase: transformation
priority: P0
requires:
...
将 interceptor stack 的前置/后置/短路语义、OGNL/ValueStack、Result、异常和 action chain 映射到 Spring 扩展点。
...
producerSkill: 52-struts2-to-springmvc-generator
...
5. 以幂等 change set 发布。
6. 把 interceptor around chain 拆成可组合执行模型，保留 after-action unwind 顺序。
7. ValueStack 数据源转为显式 model/request/session contract，禁止未审计的广泛属性暴露。
8. Result type 映射到 view/redirect/stream/json/chain 等目标构造。
9. custom interceptor 无可靠映射时生成 shim + differential contract。
