---
name: parser-capability-contract
description: 把每个语言适配器的语法、类型、打印、模板、版本能力显式声明。 Use for this exact ELMOS Spring Golden Route v2 contract; imported specifications are not runtime or certification evidence.
...
metadata:
  ...
  source_id: FOUNDATION-01-parser-capability-contract
  source_name: parser-capability-contract
  source_path: skills/parser-capability-contract/SKILL.md
  source_sha256: sha256:e12cb1d5c21ee63911e7b0edb041812a48a3ac5bfcee08e0d01473520ce80f17
  source_contract_sha256: sha256:97fc61a1b3607090f5a1c0ee2ff261a5052e2c0f440fcfecddef33bdf13802ec
...
把每个语言适配器的语法、类型、打印、模板、版本能力显式声明。

## Use when

在 Elmos 需要实现或升级 **Parser Capability Contract** 相关能力时使用；它不是仅供解释的提示词，而是实现、测试与验收契约。

## Capabilities to implement

- 声明 syntax/type/printer/template/refactor 支持级别
- 运行前 capability negotiation，禁止调用不支持能力
- 按语言版本和框架版本选择 parser
- 能力缺失返回结构化原因和替代路径
...
- [ ] 定义 `parser-capability-contract` 的 public interface、input/output/error schemas。
- [ ] 建立 `parser-capability-contract` 的 capability/feature flags 与版本字段。
...
- [ ] 至少覆盖并验收：声明 syntax/type/printer/template/refactor 支持级别；以及 运行前 capability negotiation，禁止调用不支持能力。
...
- Machine contract: `references/contract.json` (`sha256:97fc61a1b3607090f5a1c0ee2ff261a5052e2c0f440fcfecddef33bdf13802ec`).
