...
description: "Run FRT-0307 UI Layout and Style IR for FRT G03 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-0307"
  source_name: "frt-0307-ui-layout-and-style-ir"
  source_sha256: "sha256:d922cc4d1d0200d63c2cc3fc8e8a36f07d0ba5e3f2aef146ed1ea1aa107608f7"
  batch: "G03"
...
- Runtime catalog key: `FRT-0307` / `frt-0307-ui-layout-and-style-ir`.
...
# FRT-0307 — UI Layout and Style IR

## Objective

在G03范围内实现 **UI Layout and Style IR**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：建立跨Vue/React/小程序/ArkUI/Flutter的类型和语义中间层; 保留Null、Identity、Effect、Lifecycle、Navigation、Data Authority和UI语义; 每个IR节点绑定Source Range和稳定Semantic ID。

## When to Use

- 实现或修改与 **UI Layout and Style IR** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-0307Result`：版本化、带状态和诊断的结构化结果。
- `FRT-0307EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-0307FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-0307CertificateFragment`：供G03批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g03/frt-0307-ui-layout-and-style-ir/
packages/runtime/g03/frt-0307-ui-layout-and-style-ir/
services/control-plane/g03/frt-0307-ui-layout-and-style-ir/
apps/web-console/src/features/g03/frt-0307-ui-layout-and-style-ir/
apps/admin-console/src/features/g03/frt-0307-ui-layout-and-style-ir/
tests/g03/frt-0307-ui-layout-and-style-ir/
```
...
```text
POST /v1/skills/frt-0307/runs
GET  /v1/skills/frt-0307/runs/{run_id}
POST /v1/skills/frt-0307/runs/{run_id}/verify
GET  /v1/skills/frt-0307/runs/{run_id}/findings
GET  /v1/skills/frt-0307/runs/{run_id}/evidence
```
