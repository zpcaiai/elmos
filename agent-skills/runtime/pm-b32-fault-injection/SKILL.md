...
- Invoke this repository Skill as `$pm-b32-fault-injection`.
- Immutable source identity: `fault-injection` in `precision-migration-b01-44` (B32).
- Runtime adapter: `differential-test-and-repair`; binding state: `DECLARED`.
- Resolve and plan with `python3 scripts/precision_migration/runtime.py plan --skill pm-b32-fault-injection`.
...
1. 评估或生成测试
2. 执行本 Skill 的核心职责：注入网络、数据库、消息、磁盘、依赖、权限和资源故障。
3. 同步环境并双运行
4. 比较输出、状态和副作用
5. 搜索最小反例
6. 驱动修复并执行全量回归
