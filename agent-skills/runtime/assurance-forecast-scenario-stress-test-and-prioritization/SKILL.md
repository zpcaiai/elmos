---
name: assurance-forecast-scenario-stress-test-and-prioritization
description: 实现Evidence Expiry、Audit Readiness、Remediation Backlog、Risk等可解释预测、情景、敏感性与行动优先级。
metadata:
  elmos_batch: "37C"
  elmos_skill_id: "ANA37C015"
  pack_version: "3.7-complete"
---

# ANA37C015 — Assurance Forecast、Scenario、Stress Test 与 Prioritization

## Objective

实现Evidence Expiry、Audit Readiness、Remediation Backlog、Risk等可解释预测、情景、敏感性与行动优先级。
...
- 优先使用Run-rate、Aging Buckets、Lead-time、Capacity-constrained Queue、Seasonal Baseline。
- Scenario覆盖Signer/Builder/Component/Provider/Deadline/Scope变化。
- 结果保存Projects/Controls/Gaps/Readiness/Load/Recovery影响。
- 每个Forecast必须显示Assumption、Confidence、Interval和Sensitivity；History不足不输出假精度。
...
- forecastStoresAssumptions
- scenarioIsNotReportedAsActual
- insufficientHistoryDoesNotProduceFalsePrecision
- signerRevocationScenarioUsesBlastRadius
