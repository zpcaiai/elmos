...
description: "Run FRT-2913 SAST Coordinator for FRT G29 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-2913"
  source_name: "frt-2913-sast-coordinator"
  source_sha256: "sha256:1692424fa5f53a0bc13f216885dbd8ae3240e51f491408d55d432d074681d5d2"
  batch: "G29"
...
- Runtime catalog key: `FRT-2913` / `frt-2913-sast-coordinator`.
...
在G29范围内实现 **SAST Coordinator**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：Asset、Threat、Trust Boundary、Attack Surface; Authentication、Authorization、Object/Field Scope、Tenant Isolation; Session/Token、API、Input、File、SSRF/XSS/Command/Prompt Injection。

## When to Use

- 实现或修改与 **SAST Coordinator** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-2913Result`：版本化、带状态和诊断的结构化结果。
- `FRT-2913EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-2913FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-2913CertificateFragment`：供G29批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g29/frt-2913-sast-coordinator/
packages/runtime/g29/frt-2913-sast-coordinator/
services/control-plane/g29/frt-2913-sast-coordinator/
apps/web-console/src/features/g29/frt-2913-sast-coordinator/
apps/admin-console/src/features/g29/frt-2913-sast-coordinator/
tests/g29/frt-2913-sast-coordinator/
```
...
```text
POST /v1/skills/frt-2913/runs
GET  /v1/skills/frt-2913/runs/{run_id}
POST /v1/skills/frt-2913/runs/{run_id}/verify
GET  /v1/skills/frt-2913/runs/{run_id}/findings
GET  /v1/skills/frt-2913/runs/{run_id}/evidence
```
