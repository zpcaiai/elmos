...
description: "Run FRT-0612 Target Architecture Certification for FRT G06 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-0612"
  source_name: "frt-0612-target-architecture-certification"
  source_sha256: "sha256:4a516599efe8eac5e2a31f8db8bb99ad840745d5d7cf91e6a1d28641582bf99d"
  batch: "G06"
...
- Runtime catalog key: `FRT-0612` / `frt-0612-target-architecture-certification`.
...
# FRT-0612 — Target Architecture Certification

## Objective

在G06范围内实现 **Target Architecture Certification**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：按目标生态生成模块、路由、状态、DI、配置和Build边界; 禁止机械复制源目录结构; 锁定目标版本、依赖和工具链。

## When to Use

- 实现或修改与 **Target Architecture Certification** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-0612Result`：版本化、带状态和诊断的结构化结果。
- `FRT-0612EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-0612FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-0612CertificateFragment`：供G06批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g06/frt-0612-target-architecture-certification/
packages/runtime/g06/frt-0612-target-architecture-certification/
services/control-plane/g06/frt-0612-target-architecture-certification/
apps/web-console/src/features/g06/frt-0612-target-architecture-certification/
apps/admin-console/src/features/g06/frt-0612-target-architecture-certification/
tests/g06/frt-0612-target-architecture-certification/
```
...
```text
POST /v1/skills/frt-0612/runs
GET  /v1/skills/frt-0612/runs/{run_id}
POST /v1/skills/frt-0612/runs/{run_id}/verify
GET  /v1/skills/frt-0612/runs/{run_id}/findings
GET  /v1/skills/frt-0612/runs/{run_id}/evidence
```
