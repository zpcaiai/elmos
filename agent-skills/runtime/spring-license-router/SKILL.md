---
name: spring-license-router
description: 按许可、能力、质量、成本和客户 entitlement 选择 Native/OpenRewrite/Licensed/Agent。 Use for this exact ELMOS Spring Golden Route v2 contract; imported specifications are not runtime or certification evidence.
...
metadata:
  ...
  source_id: ELMOS-GR-044
  source_name: spring-license-router
  source_path: skills/spring-license-router/SKILL.md
  source_sha256: sha256:544ca185dce03605c2efddae401facaeec62e20277d039c416b1c1911e5ab726
  source_contract_sha256: sha256:18d4ed5fbf97b7bd4f1f8596d76c3397bda91a3c0ffe22e43aad7a7920b0ac86
...
# License-aware Transformation Router

## Objective

按许可、能力、质量、成本和客户 entitlement 选择 Native/OpenRewrite/Licensed/Agent。

## Scope and capabilities

- 默认拒绝未知许可
- 客户侧执行模式
- Fallback 与 rejected reasons
- 执行许可证据
...
- [ ] 定义 `spring-license-router` 的 public interface、输入/输出/Error Schema 和版本策略。
- [ ] 实现 `spring-license-router` 的 production adapter/service，不把业务状态只保存在 Prompt 或内存对象中。
...
- [ ] 至少生成：transformation-route.json, license-evidence.json。
...
- [ ] 全量 Recipe 最终收敛
- [ ] Agent 只接收剩余业务歧义
- [ ] 本 Skill 的 required outputs 均由真实目标仓库执行产生：transformation-route.json, license-evidence.json。
- [ ] 所有 mandatory tests 和 security gates 通过，且无 critical unknown。

## Dependencies

- `spring-native-recipes`
...
- Machine contract: `references/contract.json` (`sha256:18d4ed5fbf97b7bd4f1f8596d76c3397bda91a3c0ffe22e43aad7a7920b0ac86`).
