---
name: b104-fault-injection-resilience-campaign
description: "Use when ELMOS must Inject runner death, network loss, provider limits, registry failure, database locks, queue duplication and budget exhaustion. Triggers: Fault Injection and Resilience Campaign, Real World Product Certification Pack, fault injection, recovery, data integrity, retry control."
metadata:
  id: B104-S06
...
# B104-S06 — Fault Injection and Resilience Campaign

## Objective

Inject runner death, network loss, provider limits, registry failure, database locks, queue duplication and budget exhaustion.
...
Use this Skill when the approved task explicitly requires **Fault Injection and Resilience Campaign** or when the capability graph selects it as part of a minimal dependency closure.
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b104-fault-injection-resilience-campaign/SKILL.md --out /tmp/fault_injection_resilience_campaign.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B104-S06
```
...
- Reject or expose fault not actually injected.
- Reject or expose manual database repair hidden.
- Reject or expose retry storm.
