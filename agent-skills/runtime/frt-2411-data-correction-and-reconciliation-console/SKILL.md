...
description: "Run FRT-2411 Data Correction and Reconciliation Console for FRT G24 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-2411"
  source_name: "frt-2411-data-correction-and-reconciliation-console"
  source_sha256: "sha256:cc1f4f03c487bddfa93e304694afd9e94ad6af81a10c26902fc7f0edbbaf8168"
  batch: "G24"
...
- Runtime catalog key: `FRT-2411` / `frt-2411-data-correction-and-reconciliation-console`.
...
在G24范围内实现 **Data Correction and Reconciliation Console**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：Admin Object × Action × Role × Scope × Environment矩阵; 高风险操作Impact Preview、Reason、Approval、Typed Command、Audit; Manual Case、Data Repair、Reconciliation正式后台流程。

## When to Use

- 实现或修改与 **Data Correction and Reconciliation Console** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-2411Result`：版本化、带状态和诊断的结构化结果。
- `FRT-2411EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-2411FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-2411CertificateFragment`：供G24批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g24/frt-2411-data-correction-and-reconciliation-console/
packages/runtime/g24/frt-2411-data-correction-and-reconciliation-console/
services/control-plane/g24/frt-2411-data-correction-and-reconciliation-console/
apps/web-console/src/features/g24/frt-2411-data-correction-and-reconciliation-console/
apps/admin-console/src/features/g24/frt-2411-data-correction-and-reconciliation-console/
tests/g24/frt-2411-data-correction-and-reconciliation-console/
```
...
```text
POST /v1/skills/frt-2411/runs
GET  /v1/skills/frt-2411/runs/{run_id}
POST /v1/skills/frt-2411/runs/{run_id}/verify
GET  /v1/skills/frt-2411/runs/{run_id}/findings
GET  /v1/skills/frt-2411/runs/{run_id}/evidence
```
