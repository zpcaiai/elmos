...
description: "Run FRT-2808 Worker Pool High Availability for FRT G28 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-2808"
  source_name: "frt-2808-worker-pool-high-availability"
  source_sha256: "sha256:d91cda4a2eeceb668b24ca952ba981405896478a1ddedbe9cdb50adc55134aab"
  batch: "G28"
...
- Runtime catalog key: `FRT-2808` / `frt-2808-worker-pool-high-availability`.
...
在G28范围内实现 **Worker Pool High Availability**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：Dependency Failure Model与Common Cause分析; 端到端Deadline、Retry Budget、Circuit Breaker、Bulkhead; Queue Durability、Replay、Stateful Failover、Fence Token。

## When to Use

- 实现或修改与 **Worker Pool High Availability** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-2808Result`：版本化、带状态和诊断的结构化结果。
- `FRT-2808EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-2808FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-2808CertificateFragment`：供G28批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g28/frt-2808-worker-pool-high-availability/
packages/runtime/g28/frt-2808-worker-pool-high-availability/
services/control-plane/g28/frt-2808-worker-pool-high-availability/
apps/web-console/src/features/g28/frt-2808-worker-pool-high-availability/
apps/admin-console/src/features/g28/frt-2808-worker-pool-high-availability/
tests/g28/frt-2808-worker-pool-high-availability/
```
...
```text
POST /v1/skills/frt-2808/runs
GET  /v1/skills/frt-2808/runs/{run_id}
POST /v1/skills/frt-2808/runs/{run_id}/verify
GET  /v1/skills/frt-2808/runs/{run_id}/findings
GET  /v1/skills/frt-2808/runs/{run_id}/evidence
```
