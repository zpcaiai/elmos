---
name: external-evidence-connector-contract-producer-identity-ddf55e1e
description: 实现 Provider-neutral External Evidence Connector、稳定ID、能力快照、分页、错误和TCK。
metadata:
  elmos_batch: "37B"
  elmos_skill_id: "EVI37B002"
  pack_version: "3.7-complete"
---

# EVI37B002 — 统一 Connector Contract、Producer Identity 与 Capability

## Objective

实现 Provider-neutral External Evidence Connector、稳定ID、能力快照、分页、错误和TCK。
...
- 定义 discover/capabilities/listRuns/getRun/listJobs/listEvidence/fetchEvidence/reconcile 接口。
- 稳定身份由 Provider Instance + Native Project/Run/Attempt/Job/Artifact/Report ID 组成。
- 能力状态区分 Verified/Permission/Version/Plan/Unsupported/Unknown。
- 错误归一为 Auth、Rate Limit、Schema Change、Artifact Expired、Unknown Result 等。
...
- stableNativeRunIdentity
- retryAttemptsRemainDistinct
- connectorCannotChooseTenant
- unknownSchemaIsQuarantined
