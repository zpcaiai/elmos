...
description: "Run FRT-2618 Release Qualification Engine for FRT G26 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-2618"
  source_name: "frt-2618-release-qualification-engine"
  source_sha256: "sha256:232ee73c74545fda7b5a372a64c3a12bda4ccd624e5cc678d1e8ebbebbdc3a11"
  batch: "G26"
...
- Runtime catalog key: `FRT-2618` / `frt-2618-release-qualification-engine`.
...
在G26范围内实现 **Release Qualification Engine**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：Requirement/Acceptance/State Machine派生Test Obligation; Unit/Component/Contract/Integration/Journey分层协调; Differential、Mutation、Visual/Semantic、Security和Performance Regression。

## When to Use

- 实现或修改与 **Release Qualification Engine** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-2618Result`：版本化、带状态和诊断的结构化结果。
- `FRT-2618EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-2618FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-2618CertificateFragment`：供G26批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g26/frt-2618-release-qualification-engine/
packages/runtime/g26/frt-2618-release-qualification-engine/
services/control-plane/g26/frt-2618-release-qualification-engine/
apps/web-console/src/features/g26/frt-2618-release-qualification-engine/
apps/admin-console/src/features/g26/frt-2618-release-qualification-engine/
tests/g26/frt-2618-release-qualification-engine/
```
...
```text
POST /v1/skills/frt-2618/runs
GET  /v1/skills/frt-2618/runs/{run_id}
POST /v1/skills/frt-2618/runs/{run_id}/verify
GET  /v1/skills/frt-2618/runs/{run_id}/findings
GET  /v1/skills/frt-2618/runs/{run_id}/evidence
```
