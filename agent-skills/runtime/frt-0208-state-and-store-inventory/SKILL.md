...
description: "Run FRT-0208 State and Store Inventory for FRT G02 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-0208"
  source_name: "frt-0208-state-and-store-inventory"
  source_sha256: "sha256:0fd53bc0decc700840e07805da4e43ab02539e3ea401703ea01bd49d8ba06c29"
  batch: "G02"
...
- Runtime catalog key: `FRT-0208` / `frt-0208-state-and-store-inventory`.
...
# FRT-0208 — State and Store Inventory

## Objective

在G02范围内实现 **State and Store Inventory**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：识别Monorepo、Package、Framework、Runtime和Build链; 建立模块、依赖、路由、组件、状态、API、存储和资产清单; 检测动态代码、原生能力、平台限制和迁移风险。

## When to Use

- 实现或修改与 **State and Store Inventory** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-0208Result`：版本化、带状态和诊断的结构化结果。
- `FRT-0208EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-0208FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-0208CertificateFragment`：供G02批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g02/frt-0208-state-and-store-inventory/
packages/runtime/g02/frt-0208-state-and-store-inventory/
services/control-plane/g02/frt-0208-state-and-store-inventory/
apps/web-console/src/features/g02/frt-0208-state-and-store-inventory/
apps/admin-console/src/features/g02/frt-0208-state-and-store-inventory/
tests/g02/frt-0208-state-and-store-inventory/
```
...
```text
POST /v1/skills/frt-0208/runs
GET  /v1/skills/frt-0208/runs/{run_id}
POST /v1/skills/frt-0208/runs/{run_id}/verify
GET  /v1/skills/frt-0208/runs/{run_id}/findings
GET  /v1/skills/frt-0208/runs/{run_id}/evidence
```
