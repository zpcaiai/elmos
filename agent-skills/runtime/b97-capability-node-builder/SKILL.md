---
name: b97-capability-node-builder
description: "Use when ELMOS must Build typed capability, Skill, tool, runtime, artifact, test, evidence and policy nodes from the normalized estate. Triggers: Capability Node Builder, Canonical Capability Graph Consolidation Pack, typed graph nodes, stable node IDs, node lifecycle, provenance."
metadata:
  id: B97-S04
...
# B97-S04 — Capability Node Builder

## Objective

Build typed capability, Skill, tool, runtime, artifact, test, evidence and policy nodes from the normalized estate.
...
Use this Skill when the approved task explicitly requires **Capability Node Builder** or when the capability graph selects it as part of a minimal dependency closure.
...
- `capability_nodes.json`
- `node_build_report.json`
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b97-capability-node-builder/SKILL.md --out /tmp/capability_node_builder.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B97-S04
```
...
- Reject or expose dangling source references.
- Reject or expose type confusion.
- Reject or expose hash instability.
