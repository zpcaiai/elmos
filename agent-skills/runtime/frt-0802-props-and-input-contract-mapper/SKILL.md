...
description: "Run FRT-0802 Props and Input Contract Mapper for FRT G08 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-0802"
  source_name: "frt-0802-props-and-input-contract-mapper"
  source_sha256: "sha256:ac9876ae373a674db5507ff3ca43ef35a4a9b50eddbae4df38c6972416532e4a"
  batch: "G08"
...
- Runtime catalog key: `FRT-0802` / `frt-0802-props-and-input-contract-mapper`.
...
# FRT-0802 — Props and Input Contract Mapper

## Objective

在G08范围内实现 **Props and Input Contract Mapper**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：保持Props默认值、事件时机、Slot/Children/Builder结构; 保持Key、Ref、Widget Identity和状态保留语义; 正确降低Hooks、Composable、Context和局部状态。

## When to Use

- 实现或修改与 **Props and Input Contract Mapper** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-0802Result`：版本化、带状态和诊断的结构化结果。
- `FRT-0802EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-0802FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-0802CertificateFragment`：供G08批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g08/frt-0802-props-and-input-contract-mapper/
packages/runtime/g08/frt-0802-props-and-input-contract-mapper/
services/control-plane/g08/frt-0802-props-and-input-contract-mapper/
apps/web-console/src/features/g08/frt-0802-props-and-input-contract-mapper/
apps/admin-console/src/features/g08/frt-0802-props-and-input-contract-mapper/
tests/g08/frt-0802-props-and-input-contract-mapper/
```
...
```text
POST /v1/skills/frt-0802/runs
GET  /v1/skills/frt-0802/runs/{run_id}
POST /v1/skills/frt-0802/runs/{run_id}/verify
GET  /v1/skills/frt-0802/runs/{run_id}/findings
GET  /v1/skills/frt-0802/runs/{run_id}/evidence
```
