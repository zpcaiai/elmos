...
description: "Run FRT-2307 Cache Coherence Validator for FRT G23 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-2307"
  source_name: "frt-2307-cache-coherence-validator"
  source_sha256: "sha256:bce411d1523cc3fb2f22bfc36b7ee44d51fa90f0fa6aa2e0334d0dd7972c670e"
  batch: "G23"
...
- Runtime catalog key: `FRT-2307` / `frt-2307-cache-coherence-validator`.
...
在G23范围内实现 **Cache Coherence Validator**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：字段级Data Contract、Authority和Owner; Source-to-Sink与字段级Lineage; Write→Transaction→Outbox→Event→Projection→Cache→Read完整路径。

## When to Use

- 实现或修改与 **Cache Coherence Validator** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-2307Result`：版本化、带状态和诊断的结构化结果。
- `FRT-2307EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-2307FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-2307CertificateFragment`：供G23批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g23/frt-2307-cache-coherence-validator/
packages/runtime/g23/frt-2307-cache-coherence-validator/
services/control-plane/g23/frt-2307-cache-coherence-validator/
apps/web-console/src/features/g23/frt-2307-cache-coherence-validator/
apps/admin-console/src/features/g23/frt-2307-cache-coherence-validator/
tests/g23/frt-2307-cache-coherence-validator/
```
...
```text
POST /v1/skills/frt-2307/runs
GET  /v1/skills/frt-2307/runs/{run_id}
POST /v1/skills/frt-2307/runs/{run_id}/verify
GET  /v1/skills/frt-2307/runs/{run_id}/findings
GET  /v1/skills/frt-2307/runs/{run_id}/evidence
```
