---
name: elmos-nosql-document-keyvalue-ir-compiler
description: Compile document, key-value, wide-column and aggregate-store schemas, access paths, consistency, TTL, indexing and update semantics into a store-neutral persistence IR.
version: 4.0.0
...
# NoSQL Document and Key-Value IR Compiler
...
- `NoSQLDocumentAndKeyValueIRCompilerProfile`
- `NoSQLDocumentAndKeyValueIRCompilerPlan`
- `NoSQLDocumentAndKeyValueIRCompilerRun`
- `NoSQLDocumentAndKeyValueIRCompilerDecision`

Required semantic capabilities:

- model documents, partitions, keys and secondary indexes
- represent atomic update and conditional-write semantics
- capture TTL, tombstone and compaction behavior
- encode consistency and quorum requirements
- emit semantic gaps for cross-store migration
...
- `artifacts/elmos-nosql-document-keyvalue-ir-compiler/profile.yaml`
- `artifacts/elmos-nosql-document-keyvalue-ir-compiler/plan.json`
- `artifacts/elmos-nosql-document-keyvalue-ir-compiler/result.json`
- `artifacts/elmos-nosql-document-keyvalue-ir-compiler/evidence/`
...
- **partition-key loss** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **atomic update widened** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **TTL semantics change** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **eventual consistency presented as strong** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **tombstone resurrection** — must have a preventive control, a detective signal, a negative fixture and a response action.
...
- native scenario: model documents, partitions, keys and secondary indexes
- native scenario: represent atomic update and conditional-write semantics
- native scenario: capture TTL, tombstone and compaction behavior
- native scenario: encode consistency and quorum requirements
- native scenario: emit semantic gaps for cross-store migration
