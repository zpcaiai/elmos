---
name: b98-precondition-guard-compiler
description: "Use when ELMOS must Compile preconditions, stop conditions and approval requirements into fail-closed executable guards. Triggers: Precondition and Guard Compiler, Executable Skill Contract and Registry Pack, preconditions, policy guards, approval gates, stop conditions."
metadata:
  id: B98-S04
...
# B98-S04 — Precondition and Guard Compiler

## Objective

Compile preconditions, stop conditions and approval requirements into fail-closed executable guards.
...
Use this Skill when the approved task explicitly requires **Precondition and Guard Compiler** or when the capability graph selects it as part of a minimal dependency closure.
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b98-precondition-guard-compiler/SKILL.md --out /tmp/precondition_guard_compiler.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B98-S04
```
...
- Reject or expose TOCTOU guard bypass.
- Reject or expose unknown-as-allow.
- Reject or expose post-write approval.
