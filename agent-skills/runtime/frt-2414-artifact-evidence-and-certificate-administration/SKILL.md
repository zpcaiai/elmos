...
description: "Run FRT-2414 Artifact, Evidence and Certificate Administration for FRT G24 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-2414"
  source_name: "frt-2414-artifact-evidence-and-certificate-administration"
  source_sha256: "sha256:5924d1c4c1a3027f420509760c2c9d6d1835a2c6a3a10984c41628c536da2a8c"
  batch: "G24"
  source_version: "1.0.0"
  source_risk: "critical-by-default"
  source_certificate_family: "AC"
...
- Runtime catalog key: `FRT-2414` / `frt-2414-artifact-evidence-and-certificate-administration`.
...
在G24范围内实现 **Artifact, Evidence and Certificate Administration**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：Admin Object × Action × Role × Scope × Environment矩阵; 高风险操作Impact Preview、Reason、Approval、Typed Command、Audit; Manual Case、Data Repair、Reconciliation正式后台流程。

## When to Use

- 实现或修改与 **Artifact, Evidence and Certificate Administration** 对应的Contract、Runtime、API、UI、测试或证据链时。
- 相关代码、Schema、Pack、Policy、环境或依赖变化导致既有Evidence失效时。
- G24 Orchestrator生成该Skill的执行义务时。
...
- `SourceArtifactSet`：G21 Admin Journey与Requirement, G22 Manual Case、Saga、Reconciliation。
- `SkillPolicy`：权限、网络、Sandbox、Toolchain、Evidence和停止规则。

## Outputs

- `FRT-2414Result`：版本化、带状态和诊断的结构化结果。
- `FRT-2414EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-2414FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-2414CertificateFragment`：供G24批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g24/frt-2414-artifact-evidence-and-certificate-administration/
packages/runtime/g24/frt-2414-artifact-evidence-and-certificate-administration/
services/control-plane/g24/frt-2414-artifact-evidence-and-certificate-administration/
apps/web-console/src/features/g24/frt-2414-artifact-evidence-and-certificate-administration/
apps/admin-console/src/features/g24/frt-2414-artifact-evidence-and-certificate-administration/
tests/g24/frt-2414-artifact-evidence-and-certificate-administration/
```
...
```text
POST /v1/skills/frt-2414/runs
GET  /v1/skills/frt-2414/runs/{run_id}
POST /v1/skills/frt-2414/runs/{run_id}/verify
GET  /v1/skills/frt-2414/runs/{run_id}/findings
GET  /v1/skills/frt-2414/runs/{run_id}/evidence
```
...
- The parent G24 Orchestrator can consume the result and independently decide certification.
