---
name: b104-clean-room-end-to-end-benchmark
description: "Use when ELMOS must Run repository-to-report, patch, build, test, PR, evidence and rollback from empty trusted environments. Triggers: Clean Room End to End Benchmark, Real World Product Certification Pack, clean room, end-to-end, time to success, artifact verification."
metadata:
  id: B104-S04
...
# B104-S04 — Clean Room End to End Benchmark

## Objective

Run repository-to-report, patch, build, test, PR, evidence and rollback from empty trusted environments.
...
Use this Skill when the approved task explicitly requires **Clean Room End to End Benchmark** or when the capability graph selects it as part of a minimal dependency closure.
...
Package dependencies: `B104-S03`, `B101-S16`, `B102-S16`, `B103-S16`
...
- no ambient caches unless declared.
- all stages timed.
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b104-clean-room-end-to-end-benchmark/SKILL.md --out /tmp/clean_room_end_to_end_benchmark.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B104-S04
```
...
- Reject or expose pre-seeded workspace.
- Reject or expose manual hidden fix.
- Reject or expose partial run averaged.
