...
description: "Run FRT-2312 Sensitive Data Label Propagator for FRT G23 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-2312"
  source_name: "frt-2312-sensitive-data-label-propagator"
  source_sha256: "sha256:7097b343c408eca6288683c40114906c673755410a25e0e8e6d9cd27ce957365"
  batch: "G23"
...
- Runtime catalog key: `FRT-2312` / `frt-2312-sensitive-data-label-propagator`.
...
在G23范围内实现 **Sensitive Data Label Propagator**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：字段级Data Contract、Authority和Owner; Source-to-Sink与字段级Lineage; Write→Transaction→Outbox→Event→Projection→Cache→Read完整路径。

## When to Use

- 实现或修改与 **Sensitive Data Label Propagator** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-2312Result`：版本化、带状态和诊断的结构化结果。
- `FRT-2312EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-2312FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-2312CertificateFragment`：供G23批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g23/frt-2312-sensitive-data-label-propagator/
packages/runtime/g23/frt-2312-sensitive-data-label-propagator/
services/control-plane/g23/frt-2312-sensitive-data-label-propagator/
apps/web-console/src/features/g23/frt-2312-sensitive-data-label-propagator/
apps/admin-console/src/features/g23/frt-2312-sensitive-data-label-propagator/
tests/g23/frt-2312-sensitive-data-label-propagator/
```
...
```text
POST /v1/skills/frt-2312/runs
GET  /v1/skills/frt-2312/runs/{run_id}
POST /v1/skills/frt-2312/runs/{run_id}/verify
GET  /v1/skills/frt-2312/runs/{run_id}/findings
GET  /v1/skills/frt-2312/runs/{run_id}/evidence
```
