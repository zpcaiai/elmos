...
- Invoke this repository Skill as `$pm-b39-admin-generator`.
- Immutable source identity: `admin-generator` in `precision-migration-b01-44` (B39).
- Runtime adapter: `skill-and-project-synthesis`; binding state: `UNAVAILABLE`.
- Resolve and plan with `python3 scripts/precision_migration/runtime.py plan --skill pm-b39-admin-generator`.
...
2. 执行本 Skill 的核心职责：生成管理端、权限、审计、运营、配置和数据治理功能。
3. 编译Skills与架构蓝图
4. 生成/复用全部构件
5. 构建并执行验收/安全/完整性门禁
6. 输出项目、Runbook和证据
