...
description: "Run FRT-2811 Region Failure Validator for FRT G28 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-2811"
  source_name: "frt-2811-region-failure-validator"
  source_sha256: "sha256:69aafd9bdb072d9bc304c3e96af7fa1a7a91b51abaf9a6416018b233aafb29c2"
  batch: "G28"
...
- Runtime catalog key: `FRT-2811` / `frt-2811-region-failure-validator`.
...
在G28范围内实现 **Region Failure Validator**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：Dependency Failure Model与Common Cause分析; 端到端Deadline、Retry Budget、Circuit Breaker、Bulkhead; Queue Durability、Replay、Stateful Failover、Fence Token。

## When to Use

- 实现或修改与 **Region Failure Validator** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-2811Result`：版本化、带状态和诊断的结构化结果。
- `FRT-2811EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-2811FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-2811CertificateFragment`：供G28批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g28/frt-2811-region-failure-validator/
packages/runtime/g28/frt-2811-region-failure-validator/
services/control-plane/g28/frt-2811-region-failure-validator/
apps/web-console/src/features/g28/frt-2811-region-failure-validator/
apps/admin-console/src/features/g28/frt-2811-region-failure-validator/
tests/g28/frt-2811-region-failure-validator/
```
...
```text
POST /v1/skills/frt-2811/runs
GET  /v1/skills/frt-2811/runs/{run_id}
POST /v1/skills/frt-2811/runs/{run_id}/verify
GET  /v1/skills/frt-2811/runs/{run_id}/findings
GET  /v1/skills/frt-2811/runs/{run_id}/evidence
```
