---
name: external-evidence-producer-integration-orchestrator
description: 端到端编排外部生产者连接、CI平台、质量/安全/测试/性能报告、审计报告、关联、质量与发布门禁。
metadata:
  elmos_batch: "37B"
  elmos_skill_id: "EVI37B001"
  pack_version: "3.7-complete"
---

# EVI37B001 — External Evidence Producer Integration 总编排

## Objective

端到端编排外部生产者连接、CI平台、质量/安全/测试/性能报告、审计报告、关联、质量与发布门禁。
...
- 生成 docs/evidence/batch-37b-baseline.md 并盘点 Token、Webhook、API、Artifact 和直接信任路径。
- 按 Contract→Identity→CI→Quality/Security→Test/Performance→Audit→Reconciliation→Correlation→Quality→Gate 实施。
- 所有 Connector 通过统一 TCK，并标记真实平台验证级别。
- 最终报告列出未验证 Provider Capability。
...
- orchestratorRequiresBatch37AGates
- allConnectorsImplementTck
- nativeAndNormalizedEvidenceCoexist
- externalSuccessNeverMapsDirectlyToPass
