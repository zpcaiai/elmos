...
description: "Run FRT-0505 Shim Requirement Planner for FRT G05 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-0505"
  source_name: "frt-0505-shim-requirement-planner"
  source_sha256: "sha256:52e30a54fe19df0979ed469d373c8af5e0e9bb63c0cbc8c774db26036eff19f4"
  batch: "G05"
...
- Runtime catalog key: `FRT-0505` / `frt-0505-shim-requirement-planner`.
...
# FRT-0505 — Shim Requirement Planner

## Objective

在G05范围内实现 **Shim Requirement Planner**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：比较源语义与目标能力并生成Gap Register; 区分Native、Shim、Product Decision、Manual与Unsupported; 估算正确率、耗时、风险、数据和测试影响。

## When to Use

- 实现或修改与 **Shim Requirement Planner** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-0505Result`：版本化、带状态和诊断的结构化结果。
- `FRT-0505EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-0505FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-0505CertificateFragment`：供G05批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g05/frt-0505-shim-requirement-planner/
packages/runtime/g05/frt-0505-shim-requirement-planner/
services/control-plane/g05/frt-0505-shim-requirement-planner/
apps/web-console/src/features/g05/frt-0505-shim-requirement-planner/
apps/admin-console/src/features/g05/frt-0505-shim-requirement-planner/
tests/g05/frt-0505-shim-requirement-planner/
```
...
```text
POST /v1/skills/frt-0505/runs
GET  /v1/skills/frt-0505/runs/{run_id}
POST /v1/skills/frt-0505/runs/{run_id}/verify
GET  /v1/skills/frt-0505/runs/{run_id}/findings
GET  /v1/skills/frt-0505/runs/{run_id}/evidence
```
