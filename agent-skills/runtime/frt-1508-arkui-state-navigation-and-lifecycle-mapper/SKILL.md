...
description: "Run FRT-1508 ArkUI State Navigation and Lifecycle Mapper for FRT G15 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-1508"
  source_name: "frt-1508-arkui-state-navigation-and-lifecycle-mapper"
  source_sha256: "sha256:43b1d30b2b6859634b424ba57e8e55d27d1c0eb00a6b157fd7d4330a0f599f68"
  batch: "G15"
  source_version: "1.0.0"
  source_risk: "critical-by-default"
  source_certificate_family: "WA"
...
- Runtime catalog key: `FRT-1508` / `frt-1508-arkui-state-navigation-and-lifecycle-mapper`.
...
# FRT-1508 — ArkUI State Navigation and Lifecycle Mapper

## Objective

在G15范围内实现 **ArkUI State Navigation and Lifecycle Mapper**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：映射ArkUI状态装饰器、Builder、Navigation和生命周期; 处理Web DOM能力与ArkUI Native/Device差异; 生成真机/模拟器和Accessibility验证。

## When to Use

- 实现或修改与 **ArkUI State Navigation and Lifecycle Mapper** 对应的Contract、Runtime、API、UI、测试或证据链时。
- 相关代码、Schema、Pack、Policy、环境或依赖变化导致既有Evidence失效时。
- G15 Orchestrator生成该Skill的执行义务时。
...
- `SourceArtifactSet`：G1–G12、ArkUI/Web Target Profiles和设备矩阵。
- `SkillPolicy`：权限、网络、Sandbox、Toolchain、Evidence和停止规则。

## Outputs

- `FRT-1508Result`：版本化、带状态和诊断的结构化结果。
- `FRT-1508EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-1508FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-1508CertificateFragment`：供G15批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g15/frt-1508-arkui-state-navigation-and-lifecycle-mapper/
packages/runtime/g15/frt-1508-arkui-state-navigation-and-lifecycle-mapper/
services/control-plane/g15/frt-1508-arkui-state-navigation-and-lifecycle-mapper/
apps/web-console/src/features/g15/frt-1508-arkui-state-navigation-and-lifecycle-mapper/
apps/admin-console/src/features/g15/frt-1508-arkui-state-navigation-and-lifecycle-mapper/
tests/g15/frt-1508-arkui-state-navigation-and-lifecycle-mapper/
```
...
```text
POST /v1/skills/frt-1508/runs
GET  /v1/skills/frt-1508/runs/{run_id}
POST /v1/skills/frt-1508/runs/{run_id}/verify
GET  /v1/skills/frt-1508/runs/{run_id}/findings
GET  /v1/skills/frt-1508/runs/{run_id}/evidence
```
...
- The parent G15 Orchestrator can consume the result and independently decide certification.
