...
# Skipped And Flaky Test Detector
## ELMOS runtime binding

- Invoke this repository Skill as `$pm-b28-skipped-and-flaky-test-detector`.
- Immutable source identity: `skipped-and-flaky-test-detector` in `precision-migration-b01-44` (B28).
- Runtime adapter: `differential-test-and-repair`; binding state: `DECLARED`.
- Resolve and plan with `python3 scripts/precision_migration/runtime.py plan --skill pm-b28-skipped-and-flaky-test-detector`.
...
- Phase: `H 测试、验证与自动修复`
- Version: `1.0.0`
- Tags: batch-28, source-test-assessment

## Purpose

识别跳过、条件跳过、重试掩盖、时间依赖和环境不稳定测试。

本 Skill 属于精密语言转换系统。它必须以**可执行证据**为准，不得用模型自评、源码相似度或“能编译”替代语义与行为验证。

## Use when

- 当前任务直接涉及 `skipped-and-flaky-test-detector` 所描述的能力。
...
1. 评估或生成测试
2. 执行本 Skill 的核心职责：识别跳过、条件跳过、重试掩盖、时间依赖和环境不稳定测试。
3. 同步环境并双运行
4. 比较输出、状态和副作用
5. 搜索最小反例
6. 驱动修复并执行全量回归
...
```yaml
skill: skipped-and-flaky-test-detector
version: 1.0.0
