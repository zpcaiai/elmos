---
name: b100-sandbox-profile-selector
description: "Use when ELMOS must Select container, microVM, VM, device or isolated-host profiles based on risk and tool requirements. Triggers: Sandbox Profile Selector, Hardened Runner Fabric Pack, sandbox tiers, risk routing, tool compatibility, fallback policy."
metadata:
  id: B100-S05
...
# B100-S05 — Sandbox Profile Selector

## Objective

Select container, microVM, VM, device or isolated-host profiles based on risk and tool requirements.
...
Use this Skill when the approved task explicitly requires **Sandbox Profile Selector** or when the capability graph selects it as part of a minimal dependency closure.
...
- `sandbox_selection.json`
- `profile_matrix.json`
- `selection_test_report.json`
...
- higher risk never downgraded silently.
- profile capability proven.
- fallback approved.
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b100-sandbox-profile-selector/SKILL.md --out /tmp/sandbox_profile_selector.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B100-S05
```
...
- Reject or expose sandbox downgrade.
- Reject or expose host execution fallback.
- Reject or expose incompatible device claim.
