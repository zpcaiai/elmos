...
description: "Run FRT-2815 RTO and RPO Compliance Validator for FRT G28 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-2815"
  source_name: "frt-2815-rto-and-rpo-compliance-validator"
  source_sha256: "sha256:f5fbdc1b9ac1d10ed90d98474b2faf0f81d038290e7c75fb05891e3c12603592"
  batch: "G28"
...
- Runtime catalog key: `FRT-2815` / `frt-2815-rto-and-rpo-compliance-validator`.
...
在G28范围内实现 **RTO and RPO Compliance Validator**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：Dependency Failure Model与Common Cause分析; 端到端Deadline、Retry Budget、Circuit Breaker、Bulkhead; Queue Durability、Replay、Stateful Failover、Fence Token。

## When to Use

- 实现或修改与 **RTO and RPO Compliance Validator** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-2815Result`：版本化、带状态和诊断的结构化结果。
- `FRT-2815EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-2815FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-2815CertificateFragment`：供G28批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g28/frt-2815-rto-and-rpo-compliance-validator/
packages/runtime/g28/frt-2815-rto-and-rpo-compliance-validator/
services/control-plane/g28/frt-2815-rto-and-rpo-compliance-validator/
apps/web-console/src/features/g28/frt-2815-rto-and-rpo-compliance-validator/
apps/admin-console/src/features/g28/frt-2815-rto-and-rpo-compliance-validator/
tests/g28/frt-2815-rto-and-rpo-compliance-validator/
```
...
```text
POST /v1/skills/frt-2815/runs
GET  /v1/skills/frt-2815/runs/{run_id}
POST /v1/skills/frt-2815/runs/{run_id}/verify
GET  /v1/skills/frt-2815/runs/{run_id}/findings
GET  /v1/skills/frt-2815/runs/{run_id}/evidence
```
