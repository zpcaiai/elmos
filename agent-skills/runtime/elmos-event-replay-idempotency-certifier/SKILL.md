---
name: elmos-event-replay-idempotency-certifier
description: Certify deterministic replay, deduplication, snapshot restore, side-effect suppression and audit equivalence for event-sourced and stream-processing systems.
version: 4.0.0
...
# Event Replay and Idempotency Certifier

## Objective

Certify deterministic replay, deduplication, snapshot restore, side-effect suppression and audit equivalence for event-sourced and stream-processing systems.
...
- **Kernel authorities used:** K1, K3, K5, K6, K7
...
- `EventReplayAndIdempotencyCertifierProfile`
- `EventReplayAndIdempotencyCertifierPlan`
- `EventReplayAndIdempotencyCertifierRun`
- `EventReplayAndIdempotencyCertifierDecision`

Required semantic capabilities:

- rebuild state from immutable event history
- verify idempotency keys and dedup windows
- suppress or reconcile external side effects
- compare replayed and live projections
- exercise versioned event upcasters
...
- `elmos-deterministic-record-replay-model-tool-harness` from `elmos-ai-capability-enhancement-skills` — required base evidence producer.
- `elmos-ai-durable-runtime-integrator` from `elmos-ai-capability-enhancement-skills` — required base evidence producer.

## Primary Outputs

- `artifacts/elmos-event-replay-idempotency-certifier/profile.yaml`
- `artifacts/elmos-event-replay-idempotency-certifier/plan.json`
- `artifacts/elmos-event-replay-idempotency-certifier/result.json`
- `artifacts/elmos-event-replay-idempotency-certifier/evidence/`
...
- **replay sends external message again** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **dedup state expires too early** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **event upcaster changes meaning** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **snapshot hides corrupted history** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **clock/randomness makes replay diverge** — must have a preventive control, a detective signal, a negative fixture and a response action.
...
- native scenario: rebuild state from immutable event history
- native scenario: verify idempotency keys and dedup windows
- native scenario: suppress or reconcile external side effects
- native scenario: compare replayed and live projections
- native scenario: exercise versioned event upcasters
