...
description: "Run FRT-1917 Proof Evidence Graph for FRT G19 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-1917"
  source_name: "frt-1917-proof-evidence-graph"
  source_sha256: "sha256:ddb9aedd44a683cc60d274ed25d973b28cd36161d3e6ec7b33e3e694b4886488"
  batch: "G19"
...
- Runtime catalog key: `FRT-1917` / `frt-1917-proof-evidence-graph`.
...
# FRT-1917 — Proof Evidence Graph

## Objective

在G19范围内实现 **Proof Evidence Graph**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：从IR、Rule和业务不变量生成Proof Obligation; Leanstral仅提出证明候选，Lean Kernel为可信根; 组合SMT、Model Checking、Symbolic和Differential Evidence。

## When to Use

- 实现或修改与 **Proof Evidence Graph** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-1917Result`：版本化、带状态和诊断的结构化结果。
- `FRT-1917EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-1917FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-1917CertificateFragment`：供G19批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g19/frt-1917-proof-evidence-graph/
packages/runtime/g19/frt-1917-proof-evidence-graph/
services/control-plane/g19/frt-1917-proof-evidence-graph/
apps/web-console/src/features/g19/frt-1917-proof-evidence-graph/
apps/admin-console/src/features/g19/frt-1917-proof-evidence-graph/
tests/g19/frt-1917-proof-evidence-graph/
```
...
```text
POST /v1/skills/frt-1917/runs
GET  /v1/skills/frt-1917/runs/{run_id}
POST /v1/skills/frt-1917/runs/{run_id}/verify
GET  /v1/skills/frt-1917/runs/{run_id}/findings
GET  /v1/skills/frt-1917/runs/{run_id}/evidence
```
