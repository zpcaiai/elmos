...
description: "Run FRT-1805 Router Pack Framework for FRT G18 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-1805"
  source_name: "frt-1805-router-pack-framework"
  source_sha256: "sha256:4bbb51facba162935157678dd8e359aa680fc183b898ebe2d2eeb680b5c93f3d"
  batch: "G18"
...
- Runtime catalog key: `FRT-1805` / `frt-1805-router-pack-framework`.
...
# FRT-1805 — Router Pack Framework

## Objective

在G18范围内实现 **Router Pack Framework**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：将真实项目差异封装为可组合Overlay而非硬编码核心; Pack声明Provides/Requires/Conflicts/Permissions/Versions; 确定性解析优先级和冲突。

## When to Use

- 实现或修改与 **Router Pack Framework** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-1805Result`：版本化、带状态和诊断的结构化结果。
- `FRT-1805EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-1805FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-1805CertificateFragment`：供G18批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g18/frt-1805-router-pack-framework/
packages/runtime/g18/frt-1805-router-pack-framework/
services/control-plane/g18/frt-1805-router-pack-framework/
apps/web-console/src/features/g18/frt-1805-router-pack-framework/
apps/admin-console/src/features/g18/frt-1805-router-pack-framework/
tests/g18/frt-1805-router-pack-framework/
```
...
```text
POST /v1/skills/frt-1805/runs
GET  /v1/skills/frt-1805/runs/{run_id}
POST /v1/skills/frt-1805/runs/{run_id}/verify
GET  /v1/skills/frt-1805/runs/{run_id}/findings
GET  /v1/skills/frt-1805/runs/{run_id}/evidence
```
