---
name: pm-b23-sqlserver-clr-stored-procedure
description: "迁移 SQL CLR、程序集、权限集和外部代码依赖. Precision Migration B23 contract; use for this exact assessment, transformation, validation, repair, evidence, or cutover scope."
---

# Sqlserver Clr Stored Procedure
...
迁移 SQL CLR、程序集、权限集和外部代码依赖。

本 Skill 属于精密语言转换系统。它必须以**可执行证据**为准，不得用模型自评、源码相似度或“能编译”替代语义与行为验证。

## Use when

- 当前任务直接涉及 `sqlserver-clr-stored-procedure` 所描述的能力。
...
1. 盘点对象与专有能力
2. 执行本 Skill 的核心职责：迁移 SQL CLR、程序集、权限集和外部代码依赖。
...
```yaml
skill: sqlserver-clr-stored-procedure
version: 1.0.0
