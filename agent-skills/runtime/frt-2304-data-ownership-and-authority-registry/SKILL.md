...
description: "Run FRT-2304 Data Ownership and Authority Registry for FRT G23 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-2304"
  source_name: "frt-2304-data-ownership-and-authority-registry"
  source_sha256: "sha256:af9315f5b5c98127887ff33c88a9f48157df066f7a9892be1dbd0aafabe92541"
  batch: "G23"
...
- Runtime catalog key: `FRT-2304` / `frt-2304-data-ownership-and-authority-registry`.
...
在G23范围内实现 **Data Ownership and Authority Registry**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：字段级Data Contract、Authority和Owner; Source-to-Sink与字段级Lineage; Write→Transaction→Outbox→Event→Projection→Cache→Read完整路径。

## When to Use

- 实现或修改与 **Data Ownership and Authority Registry** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-2304Result`：版本化、带状态和诊断的结构化结果。
- `FRT-2304EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-2304FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-2304CertificateFragment`：供G23批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g23/frt-2304-data-ownership-and-authority-registry/
packages/runtime/g23/frt-2304-data-ownership-and-authority-registry/
services/control-plane/g23/frt-2304-data-ownership-and-authority-registry/
apps/web-console/src/features/g23/frt-2304-data-ownership-and-authority-registry/
apps/admin-console/src/features/g23/frt-2304-data-ownership-and-authority-registry/
tests/g23/frt-2304-data-ownership-and-authority-registry/
```
...
```text
POST /v1/skills/frt-2304/runs
GET  /v1/skills/frt-2304/runs/{run_id}
POST /v1/skills/frt-2304/runs/{run_id}/verify
GET  /v1/skills/frt-2304/runs/{run_id}/findings
GET  /v1/skills/frt-2304/runs/{run_id}/evidence
```
