...
description: "Run FRT-2019 One-Click Verification for FRT G20 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-2019"
  source_name: "frt-2019-one-click-verification"
  source_sha256: "sha256:a611d51a367f168727fefda33ee2742c31e843ac110e502f5955e4c962c3f8f7"
  batch: "G20"
...
- Runtime catalog key: `FRT-2019` / `frt-2019-one-click-verification`.
...
# FRT-2019 — One-Click Verification

## Objective

在G20范围内实现 **One-Click Verification**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：提供Skill SDK、Runtime、Registry和Marketplace; 实现CLI、API、IDE、Web Console和Multi-Tenant Workspace; Durable Workflow与分布式Worker支持暂停、恢复、取消和幂等。

## When to Use

- 实现或修改与 **One-Click Verification** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-2019Result`：版本化、带状态和诊断的结构化结果。
- `FRT-2019EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-2019FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-2019CertificateFragment`：供G20批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g20/frt-2019-one-click-verification/
packages/runtime/g20/frt-2019-one-click-verification/
services/control-plane/g20/frt-2019-one-click-verification/
apps/web-console/src/features/g20/frt-2019-one-click-verification/
apps/admin-console/src/features/g20/frt-2019-one-click-verification/
tests/g20/frt-2019-one-click-verification/
```
...
```text
POST /v1/skills/frt-2019/runs
GET  /v1/skills/frt-2019/runs/{run_id}
POST /v1/skills/frt-2019/runs/{run_id}/verify
GET  /v1/skills/frt-2019/runs/{run_id}/findings
GET  /v1/skills/frt-2019/runs/{run_id}/evidence
```
