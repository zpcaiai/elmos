...
description: "Run FRT-2801 Dependency Failure Model Registry for FRT G28 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-2801"
  source_name: "frt-2801-dependency-failure-model-registry"
  source_sha256: "sha256:571dfc276913ee2e8d7d428e963a7f0f519e2880e4f3deea79f97c155cbb36cf"
  batch: "G28"
...
- Runtime catalog key: `FRT-2801` / `frt-2801-dependency-failure-model-registry`.
...
在G28范围内实现 **Dependency Failure Model Registry**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：Dependency Failure Model与Common Cause分析; 端到端Deadline、Retry Budget、Circuit Breaker、Bulkhead; Queue Durability、Replay、Stateful Failover、Fence Token。

## When to Use

- 实现或修改与 **Dependency Failure Model Registry** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-2801Result`：版本化、带状态和诊断的结构化结果。
- `FRT-2801EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-2801FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-2801CertificateFragment`：供G28批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g28/frt-2801-dependency-failure-model-registry/
packages/runtime/g28/frt-2801-dependency-failure-model-registry/
services/control-plane/g28/frt-2801-dependency-failure-model-registry/
apps/web-console/src/features/g28/frt-2801-dependency-failure-model-registry/
apps/admin-console/src/features/g28/frt-2801-dependency-failure-model-registry/
tests/g28/frt-2801-dependency-failure-model-registry/
```
...
```text
POST /v1/skills/frt-2801/runs
GET  /v1/skills/frt-2801/runs/{run_id}
POST /v1/skills/frt-2801/runs/{run_id}/verify
GET  /v1/skills/frt-2801/runs/{run_id}/findings
GET  /v1/skills/frt-2801/runs/{run_id}/evidence
```
