...
description: "Run FRT-1808 Enterprise Pack Framework for FRT G18 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-1808"
  source_name: "frt-1808-enterprise-pack-framework"
  source_sha256: "sha256:4dd69bdbfa480afced238a319d56013cd7a457cd6a2613a0cf8e5b7f769841af"
  batch: "G18"
...
- Runtime catalog key: `FRT-1808` / `frt-1808-enterprise-pack-framework`.
...
# FRT-1808 — Enterprise Pack Framework

## Objective

在G18范围内实现 **Enterprise Pack Framework**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：将真实项目差异封装为可组合Overlay而非硬编码核心; Pack声明Provides/Requires/Conflicts/Permissions/Versions; 确定性解析优先级和冲突。

## When to Use

- 实现或修改与 **Enterprise Pack Framework** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-1808Result`：版本化、带状态和诊断的结构化结果。
- `FRT-1808EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-1808FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-1808CertificateFragment`：供G18批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g18/frt-1808-enterprise-pack-framework/
packages/runtime/g18/frt-1808-enterprise-pack-framework/
services/control-plane/g18/frt-1808-enterprise-pack-framework/
apps/web-console/src/features/g18/frt-1808-enterprise-pack-framework/
apps/admin-console/src/features/g18/frt-1808-enterprise-pack-framework/
tests/g18/frt-1808-enterprise-pack-framework/
```
...
```text
POST /v1/skills/frt-1808/runs
GET  /v1/skills/frt-1808/runs/{run_id}
POST /v1/skills/frt-1808/runs/{run_id}/verify
GET  /v1/skills/frt-1808/runs/{run_id}/findings
GET  /v1/skills/frt-1808/runs/{run_id}/evidence
```
