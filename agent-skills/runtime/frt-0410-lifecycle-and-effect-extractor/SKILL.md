...
description: "Run FRT-0410 Lifecycle and Effect Extractor for FRT G04 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-0410"
  source_name: "frt-0410-lifecycle-and-effect-extractor"
  source_sha256: "sha256:94b6e46e54d1af7460365b515a92575e5bee0f7a787adcad3174e002358d0120"
  batch: "G04"
...
- Runtime catalog key: `FRT-0410` / `frt-0410-lifecycle-and-effect-extractor`.
...
# FRT-0410 — Lifecycle and Effect Extractor

## Objective

在G04范围内实现 **Lifecycle and Effect Extractor**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：每个Adapter只解析源，不生成目标代码; 恢复组件、状态、事件、生命周期、路由、样式和平台能力; 统一动态语法、宏、装饰器和模板表达式。

## When to Use

- 实现或修改与 **Lifecycle and Effect Extractor** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-0410Result`：版本化、带状态和诊断的结构化结果。
- `FRT-0410EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-0410FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-0410CertificateFragment`：供G04批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g04/frt-0410-lifecycle-and-effect-extractor/
packages/runtime/g04/frt-0410-lifecycle-and-effect-extractor/
services/control-plane/g04/frt-0410-lifecycle-and-effect-extractor/
apps/web-console/src/features/g04/frt-0410-lifecycle-and-effect-extractor/
apps/admin-console/src/features/g04/frt-0410-lifecycle-and-effect-extractor/
tests/g04/frt-0410-lifecycle-and-effect-extractor/
```
...
```text
POST /v1/skills/frt-0410/runs
GET  /v1/skills/frt-0410/runs/{run_id}
POST /v1/skills/frt-0410/runs/{run_id}/verify
GET  /v1/skills/frt-0410/runs/{run_id}/findings
GET  /v1/skills/frt-0410/runs/{run_id}/evidence
```
