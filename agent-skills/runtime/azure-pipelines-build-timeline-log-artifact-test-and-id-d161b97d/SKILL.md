---
name: azure-pipelines-build-timeline-log-artifact-test-and-id-d161b97d
description: 接入 Azure Build 7.1、Timeline、Logs、Artifacts、Test Run/Result/Attachments和Entra Identity。
metadata:
  elmos_batch: "37B"
  elmos_skill_id: "EVI37B006"
  pack_version: "3.7-complete"
---

# EVI37B006 — Azure Pipelines Build、Timeline、Artifact、Test 与 Identity Connector

## Objective

接入 Azure Build 7.1、Timeline、Logs、Artifacts、Test Run/Result/Attachments和Entra Identity。
...
- 稳定与 Preview API Profile 分离，Preview Schema Change 触发 Quarantine。
- Build ID、Project GUID、Timeline Record ID、Test Run/Result ID 分别建模。
- 保留 Stage/Phase/Job/Task hierarchy 和 attempt。
- Artifact/Test Attachment 独立下载、摘要、分类并关联。
...
- buildNumberIsNotNativeIdentity
- timelineHierarchyIsPreserved
- previewSchemaChangeQuarantinesConnector
- testAttachmentIsIndependentArtifact
