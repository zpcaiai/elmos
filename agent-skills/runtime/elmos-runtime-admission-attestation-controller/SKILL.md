---
name: elmos-runtime-admission-attestation-controller
description: Admit only signed, policy-conformant, vulnerability-acceptable and attested artifacts/configurations into execution environments.
version: 4.0.0
...
# Runtime Admission and Attestation Controller
...
- Check certificate, SBOM/VEX and policy state
- Bind configuration and runtime identity
- Deny mutable tags and unpinned dependencies
- Continuously re-evaluate on revocation
...
- `elmos-ai-tool-authority-policy-generator` from `elmos-ai-capability-enhancement-skills` — required base evidence producer.

## Primary Outputs

- `admission-policy.rego`
- `admission-decisions.jsonl`
- `runtime-attestation-results.json`
- `admitted-subject-lock.json`
...
- **admission bypass through alternate path** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **tag-to-digest race** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **policy controller fail-open** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **attestation subject mismatch** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **revocation not delivered** — must have a preventive control, a detective signal, a negative fixture and a response action.
...
- signed digest admitted
- unsigned image denied
- revoked certificate denied
- critical KEV denied
- config digest mismatch
- post-admission revocation eviction
