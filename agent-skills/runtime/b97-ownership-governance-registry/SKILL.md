---
name: b97-ownership-governance-registry
description: "Use when ELMOS must Assign accountable owners, reviewers, security contacts and change authorities to every governed asset. Triggers: Ownership and Governance Registry, Canonical Capability Graph Consolidation Pack, ownership, review authority, security stewardship, escalation."
metadata:
  id: B97-S09
...
# B97-S09 — Ownership and Governance Registry

## Objective

Assign accountable owners, reviewers, security contacts and change authorities to every governed asset.
...
Use this Skill when the approved task explicitly requires **Ownership and Governance Registry** or when the capability graph selects it as part of a minimal dependency closure.
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b97-ownership-governance-registry/SKILL.md --out /tmp/ownership_governance_registry.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B97-S09
```
...
- Reject or expose self-approval.
- Reject or expose orphan critical Skill.
- Reject or expose cross-tenant owner reference.
