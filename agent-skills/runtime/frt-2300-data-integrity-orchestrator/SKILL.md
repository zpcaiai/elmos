...
description: "Run FRT-2300 Data Integrity Orchestrator for FRT G23 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-2300"
  source_name: "frt-2300-data-integrity-orchestrator"
  source_sha256: "sha256:441fe37be835332942363425f88fccb5c5882b5aee39a42da489611f8b8f52a6"
  batch: "G23"
...
- Runtime catalog key: `FRT-2300` / `frt-2300-data-integrity-orchestrator`.
...
在G23范围内实现 **Data Integrity Orchestrator**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：字段级Data Contract、Authority和Owner; Source-to-Sink与字段级Lineage; Write→Transaction→Outbox→Event→Projection→Cache→Read完整路径。

## When to Use

- 实现或修改与 **Data Integrity Orchestrator** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-2300Result`：版本化、带状态和诊断的结构化结果。
- `FRT-2300EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-2300FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-2300CertificateFragment`：供G23批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g23/frt-2300-data-integrity-orchestrator/
packages/runtime/g23/frt-2300-data-integrity-orchestrator/
services/control-plane/g23/frt-2300-data-integrity-orchestrator/
apps/web-console/src/features/g23/frt-2300-data-integrity-orchestrator/
apps/admin-console/src/features/g23/frt-2300-data-integrity-orchestrator/
tests/g23/frt-2300-data-integrity-orchestrator/
```
...
```text
POST /v1/skills/frt-2300/runs
GET  /v1/skills/frt-2300/runs/{run_id}
POST /v1/skills/frt-2300/runs/{run_id}/verify
GET  /v1/skills/frt-2300/runs/{run_id}/findings
GET  /v1/skills/frt-2300/runs/{run_id}/evidence
```
