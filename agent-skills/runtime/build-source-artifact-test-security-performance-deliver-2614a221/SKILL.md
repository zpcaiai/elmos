---
name: build-source-artifact-test-security-performance-deliver-2614a221
description: 使用Commit、Digest和Native ID关联外部运行、Artifact、报告、PR/MR与ELMOS项目，保留置信度和歧义。
metadata:
  elmos_batch: "37B"
  elmos_skill_id: "EVI37B014"
  pack_version: "3.7-complete"
---

# EVI37B014 — Build、Source、Artifact、Test、Security、Performance 与 Delivery Correlation

## Objective

使用Commit、Digest和Native ID关联外部运行、Artifact、报告、PR/MR与ELMOS项目，保留置信度和歧义。
...
- 定义 Verified/High/Medium/Low/Ambiguous/Rejected 置信度。
- 强键包括 Provider Repository ID、Commit SHA、Native Run/Job ID、Artifact Digest、Attestation Subject。
- Branch/Artifact Name/Free-text Version 仅弱证据。
- 支持 Multi-repository Build 和同Commit多Build/多Digest。
...
- branchNameAloneDoesNotCreateHighConfidence
- artifactDigestCreatesVerifiedArtifactCorrelation
- multiRepositoryPipelinePreservesAllSources
- conflictingCommitClaimsCreateAmbiguity
