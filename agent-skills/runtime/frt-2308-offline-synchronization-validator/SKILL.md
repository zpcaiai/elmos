...
description: "Run FRT-2308 Offline Synchronization Validator for FRT G23 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-2308"
  source_name: "frt-2308-offline-synchronization-validator"
  source_sha256: "sha256:3633cdca7ab5a560ed1cab92eca613b968cda9ee78f30e63241cb4e70df890ec"
  batch: "G23"
...
- Runtime catalog key: `FRT-2308` / `frt-2308-offline-synchronization-validator`.
...
在G23范围内实现 **Offline Synchronization Validator**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：字段级Data Contract、Authority和Owner; Source-to-Sink与字段级Lineage; Write→Transaction→Outbox→Event→Projection→Cache→Read完整路径。

## When to Use

- 实现或修改与 **Offline Synchronization Validator** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-2308Result`：版本化、带状态和诊断的结构化结果。
- `FRT-2308EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-2308FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-2308CertificateFragment`：供G23批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g23/frt-2308-offline-synchronization-validator/
packages/runtime/g23/frt-2308-offline-synchronization-validator/
services/control-plane/g23/frt-2308-offline-synchronization-validator/
apps/web-console/src/features/g23/frt-2308-offline-synchronization-validator/
apps/admin-console/src/features/g23/frt-2308-offline-synchronization-validator/
tests/g23/frt-2308-offline-synchronization-validator/
```
...
```text
POST /v1/skills/frt-2308/runs
GET  /v1/skills/frt-2308/runs/{run_id}
POST /v1/skills/frt-2308/runs/{run_id}/verify
GET  /v1/skills/frt-2308/runs/{run_id}/findings
GET  /v1/skills/frt-2308/runs/{run_id}/evidence
```
