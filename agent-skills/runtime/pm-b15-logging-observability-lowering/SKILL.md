---
name: pm-b15-logging-observability-lowering
description: "迁移日志、指标、Trace、Correlation、错误报告和语义约定. Precision Migration B15 contract; use for this exact assessment, transformation, validation, repair, evidence, or cutover scope."
---

# Logging Observability Lowering
## ELMOS runtime binding

- Invoke this repository Skill as `$pm-b15-logging-observability-lowering`.
- Immutable source identity: `logging-observability-lowering` in `precision-migration-b01-44` (B15).
- Runtime adapter: `directed-backend-route`; binding state: `DECLARED`.
- Resolve and plan with `python3 scripts/precision_migration/runtime.py plan --skill pm-b15-logging-observability-lowering`.
...
- Phase: `E 后端语言互转`
- Version: `1.0.0`
- Tags: backend-semantic-lowering, batch-15

## Purpose

迁移日志、指标、Trace、Correlation、错误报告和语义约定。

本 Skill 属于精密语言转换系统。它必须以**可执行证据**为准，不得用模型自评、源码相似度或“能编译”替代语义与行为验证。

## Use when

- 当前任务直接涉及 `logging-observability-lowering` 所描述的能力。
...
1. 使用语言原生前端恢复语义
2. 执行本 Skill 的核心职责：迁移日志、指标、Trace、Correlation、错误报告和语义约定。
...
```yaml
skill: logging-observability-lowering
version: 1.0.0
