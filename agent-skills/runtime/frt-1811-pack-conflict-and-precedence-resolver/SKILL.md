...
description: "Run FRT-1811 Pack Conflict and Precedence Resolver for FRT G18 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-1811"
  source_name: "frt-1811-pack-conflict-and-precedence-resolver"
  source_sha256: "sha256:cfdf4445ce7b5052cb8ad6ebe83c4eed732c24653b99d07608d4c9f262227066"
  batch: "G18"
...
- Runtime catalog key: `FRT-1811` / `frt-1811-pack-conflict-and-precedence-resolver`.
...
# FRT-1811 — Pack Conflict and Precedence Resolver

## Objective

在G18范围内实现 **Pack Conflict and Precedence Resolver**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：将真实项目差异封装为可组合Overlay而非硬编码核心; Pack声明Provides/Requires/Conflicts/Permissions/Versions; 确定性解析优先级和冲突。

## When to Use

- 实现或修改与 **Pack Conflict and Precedence Resolver** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-1811Result`：版本化、带状态和诊断的结构化结果。
- `FRT-1811EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-1811FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-1811CertificateFragment`：供G18批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g18/frt-1811-pack-conflict-and-precedence-resolver/
packages/runtime/g18/frt-1811-pack-conflict-and-precedence-resolver/
services/control-plane/g18/frt-1811-pack-conflict-and-precedence-resolver/
apps/web-console/src/features/g18/frt-1811-pack-conflict-and-precedence-resolver/
apps/admin-console/src/features/g18/frt-1811-pack-conflict-and-precedence-resolver/
tests/g18/frt-1811-pack-conflict-and-precedence-resolver/
```
...
```text
POST /v1/skills/frt-1811/runs
GET  /v1/skills/frt-1811/runs/{run_id}
POST /v1/skills/frt-1811/runs/{run_id}/verify
GET  /v1/skills/frt-1811/runs/{run_id}/findings
GET  /v1/skills/frt-1811/runs/{run_id}/evidence
```
