---
name: elmos-product-analytics-telemetry-experiment-governor
description: Generate privacy-safe product analytics, feature flags and experiments with metric definitions, guardrails, exposure logging and decision evidence.
version: 4.0.0
...
# Product Analytics, Telemetry and Experiment Governor

## Objective

Generate privacy-safe product analytics, feature flags and experiments with metric definitions, guardrails, exposure logging and decision evidence.
...
- **Kernel authorities used:** K1, K2, K4, K5, K6, K7, K8
...
- `ProductAnalyticsTelemetryAndExperimentGovernorProfile`
- `ProductAnalyticsTelemetryAndExperimentGovernorPlan`
- `ProductAnalyticsTelemetryAndExperimentGovernorRun`
- `ProductAnalyticsTelemetryAndExperimentGovernorDecision`

Required semantic capabilities:

- compile metric and event contracts
- generate feature-flag and cohort assignment
- log exposure and guardrail outcomes
- run sequential/statistical analysis
- archive decisions and prevent metric drift
...
- `elmos-ai-quality-model-evaluation-compiler` from `elmos-functional-assurance-certification-skills` — optional companion; required only for E4/E5/P05 certification.
- `elmos-privacy-impact-consent-purpose-verifier` from `elmos-functional-assurance-certification-skills` — optional companion; required only for E4/E5/P05 certification.

## Primary Outputs

- `artifacts/elmos-product-analytics-telemetry-experiment-governor/profile.yaml`
- `artifacts/elmos-product-analytics-telemetry-experiment-governor/plan.json`
- `artifacts/elmos-product-analytics-telemetry-experiment-governor/result.json`
- `artifacts/elmos-product-analytics-telemetry-experiment-governor/evidence/`
...
- **metric definition changes mid-test** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **cohort assignment not stable** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **experiment includes prohibited users** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **guardrail omitted** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **p-hacking** — must have a preventive control, a detective signal, a negative fixture and a response action.
...
- native scenario: compile metric and event contracts
- native scenario: generate feature-flag and cohort assignment
- native scenario: log exposure and guardrail outcomes
- native scenario: run sequential/statistical analysis
- native scenario: archive decisions and prevent metric drift
