...
description: "Run FRT-0604 WeChat Mini Program Architecture Synthesizer for FRT G06 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-0604"
  source_name: "frt-0604-wechat-mini-program-architecture-synthesizer"
  source_sha256: "sha256:ab4ada21baa48868e2b5fe4947e58f88e222f3d9912ae2a442b685f5c4234d5a"
  batch: "G06"
  source_version: "1.0.0"
  source_risk: "critical-by-default"
  source_certificate_family: "TA"
...
- Runtime catalog key: `FRT-0604` / `frt-0604-wechat-mini-program-architecture-synthesizer`.
...
# FRT-0604 — WeChat Mini Program Architecture Synthesizer

## Objective

在G06范围内实现 **WeChat Mini Program Architecture Synthesizer**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：按目标生态生成模块、路由、状态、DI、配置和Build边界; 禁止机械复制源目录结构; 锁定目标版本、依赖和工具链。

## When to Use

- 实现或修改与 **WeChat Mini Program Architecture Synthesizer** 对应的Contract、Runtime、API、UI、测试或证据链时。
- 相关代码、Schema、Pack、Policy、环境或依赖变化导致既有Evidence失效时。
- G06 Orchestrator生成该Skill的执行义务时。
...
- `SourceArtifactSet`：G5 Migration Plan、Target Profile、Framework/Enterprise Packs。
- `SkillPolicy`：权限、网络、Sandbox、Toolchain、Evidence和停止规则。

## Outputs

- `FRT-0604Result`：版本化、带状态和诊断的结构化结果。
- `FRT-0604EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-0604FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-0604CertificateFragment`：供G06批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g06/frt-0604-wechat-mini-program-architecture-synthesizer/
packages/runtime/g06/frt-0604-wechat-mini-program-architecture-synthesizer/
services/control-plane/g06/frt-0604-wechat-mini-program-architecture-synthesizer/
apps/web-console/src/features/g06/frt-0604-wechat-mini-program-architecture-synthesizer/
apps/admin-console/src/features/g06/frt-0604-wechat-mini-program-architecture-synthesizer/
tests/g06/frt-0604-wechat-mini-program-architecture-synthesizer/
```
...
```text
POST /v1/skills/frt-0604/runs
GET  /v1/skills/frt-0604/runs/{run_id}
POST /v1/skills/frt-0604/runs/{run_id}/verify
GET  /v1/skills/frt-0604/runs/{run_id}/findings
GET  /v1/skills/frt-0604/runs/{run_id}/evidence
```
...
- The parent G06 Orchestrator can consume the result and independently decide certification.
