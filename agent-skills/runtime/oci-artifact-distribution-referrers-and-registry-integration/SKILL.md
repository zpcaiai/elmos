---
name: oci-artifact-distribution-referrers-and-registry-integration
description: 实现 OCI 1.1 Subject/Referrer 发布、Registry Capability、Fallback、认证与对账。
metadata:
  elmos_batch: "37A"
  elmos_skill_id: "ART37A009"
  pack_version: "3.7-complete"
---

# ART37A009 — OCI Artifact Distribution、Referrers 与 Registry Integration

## Objective

实现 OCI 1.1 Subject/Referrer 发布、Registry Capability、Fallback、认证与对账。
...
- 发布 Manifest 时绑定 artifactType、subject descriptor、blobs 和 annotations。
- 优先使用 Referrers API，不支持时记录 Tag Schema Fallback。
- 认证使用短期 Credential/Workload Identity，Annotation 不含凭证。
- Push 后验证 Manifest Digest 并建立 ELMOS Artifact Relationship。
...
- publicationUsesSubjectDigest
- mutableTagIsNotPrimaryRelationship
- fallbackModeIsRecorded
- publicationTimeoutIsReconciled
