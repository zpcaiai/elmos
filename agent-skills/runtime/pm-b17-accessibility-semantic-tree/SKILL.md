...
- Invoke this repository Skill as `$pm-b17-accessibility-semantic-tree`.
- Immutable source identity: `accessibility-semantic-tree` in `precision-migration-b01-44` (B17).
- Runtime adapter: `frontend-client-route`; binding state: `UNAVAILABLE`.
- Resolve and plan with `python3 scripts/precision_migration/runtime.py plan --skill pm-b17-accessibility-semantic-tree`.
...
提取并比较 Role、Name、State、Value、Focus 和可操作性语义树。

本 Skill 属于精密语言转换系统。它必须以**可执行证据**为准，不得用模型自评、源码相似度或“能编译”替代语义与行为验证。

## Use when

- 当前任务直接涉及 `accessibility-semantic-tree` 所描述的能力。
...
1. 恢复组件契约和UI状态机
2. 执行本 Skill 的核心职责：提取并比较 Role、Name、State、Value、Focus 和可操作性语义树。
3. 映射事件、路由、布局和平台能力
4. 生成目标项目
...
```yaml
skill: accessibility-semantic-tree
version: 1.0.0
