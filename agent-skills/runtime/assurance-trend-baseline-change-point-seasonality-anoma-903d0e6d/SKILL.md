---
name: assurance-trend-baseline-change-point-seasonality-anoma-903d0e6d
description: 实现时间语义、Gap/Reset、鲁棒Baseline、Change Point、Anomaly、Segment Driver与Restatement处理。
metadata:
  elmos_batch: "37C"
  elmos_skill_id: "ANA37C009"
  pack_version: "3.7-complete"
---

# ANA37C009 — Trend、Baseline、Change Point、Seasonality、Anomaly 与 Diagnostics

## Objective

实现时间语义、Gap/Reset、鲁棒Baseline、Change Point、Anomaly、Segment Driver与Restatement处理。
...
- 保存Event/Observed/Processed/As-of与Interval、Temporality、Reset、Gap。
- 支持Prior/Rolling/Seasonal/Release/Project/Peer/Target Baseline。
- 优先IQR/MAD/Seasonal/Rate-of-change等可解释方法，非默认Z-score。
- Backfill与Restatement重新计算Anomaly但保留原Alert并标注Data Correction。
...
- missingPointIsNotZero
- seasonalMetricUsesSeasonalBaseline
- backfillDoesNotCreateFalseBusinessAnomaly
- anomalyExplanationLinksSourceSnapshots
