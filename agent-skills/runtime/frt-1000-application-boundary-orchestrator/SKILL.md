...
description: "Run FRT-1000 Application Boundary Orchestrator for FRT G10 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-1000"
  source_name: "frt-1000-application-boundary-orchestrator"
  source_sha256: "sha256:12e88c054ab2e64e774e02f2391c88ea219f5917516362a9f2595208e31a1b22"
  batch: "G10"
...
- Runtime catalog key: `FRT-1000` / `frt-1000-application-boundary-orchestrator`.
...
# FRT-1000 — Application Boundary Orchestrator

## Objective

在G10范围内实现 **Application Boundary Orchestrator**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：保持Route、History、Guard和Deep Link语义; 表单Validation、错误、草稿和提交幂等完整; API、Event、Storage和Realtime边界显式。

## When to Use

- 实现或修改与 **Application Boundary Orchestrator** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-1000Result`：版本化、带状态和诊断的结构化结果。
- `FRT-1000EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-1000FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-1000CertificateFragment`：供G10批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g10/frt-1000-application-boundary-orchestrator/
packages/runtime/g10/frt-1000-application-boundary-orchestrator/
services/control-plane/g10/frt-1000-application-boundary-orchestrator/
apps/web-console/src/features/g10/frt-1000-application-boundary-orchestrator/
apps/admin-console/src/features/g10/frt-1000-application-boundary-orchestrator/
tests/g10/frt-1000-application-boundary-orchestrator/
```
...
```text
POST /v1/skills/frt-1000/runs
GET  /v1/skills/frt-1000/runs/{run_id}
POST /v1/skills/frt-1000/runs/{run_id}/verify
GET  /v1/skills/frt-1000/runs/{run_id}/findings
GET  /v1/skills/frt-1000/runs/{run_id}/evidence
```
