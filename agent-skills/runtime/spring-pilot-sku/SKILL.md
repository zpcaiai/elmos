---
name: spring-pilot-sku
description: 选择代表性服务/垂直切片执行完整 Route、staging、rollback 和 Completion Proof。 Use for this exact ELMOS Spring Golden Route v2 contract; imported specifications are not runtime or certification evidence.
...
metadata:
  ...
  source_id: ELMOS-GR-082
  source_name: spring-pilot-sku
  source_path: skills/spring-pilot-sku/SKILL.md
  source_sha256: sha256:6da2146c2ef0e16711201f9149cd38e0b9ffbc2df08139d9c430bb98112ffd1d
  source_contract_sha256: sha256:b2b65e26cd570988418a08683a908b3d6d4e0024b863078d8784edbaf4e67f84
...
# Modernization Pilot SKU

## Objective

选择代表性服务/垂直切片执行完整 Route、staging、rollback 和 Completion Proof。
...
- [ ] 定义 `spring-pilot-sku` 的 public interface、输入/输出/Error Schema 和版本策略。
- [ ] 实现 `spring-pilot-sku` 的 production adapter/service，不把业务状态只保存在 Prompt 或内存对象中。
...
- [ ] 至少生成：pilot-delivery, pilot-proof.json, pilot-scorecard.json。
...
- [ ] 本 Skill 的 required outputs 均由真实目标仓库执行产生：pilot-delivery, pilot-proof.json, pilot-scorecard.json。
- [ ] 所有 mandatory tests 和 security gates 通过，且无 critical unknown。

## Dependencies

- `spring-assessment-sku`
...
- Machine contract: `references/contract.json` (`sha256:b2b65e26cd570988418a08683a908b3d6d4e0024b863078d8784edbaf4e67f84`).
