...
description: "Run FRT-1903 Leanstral Candidate Adapter for FRT G19 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-1903"
  source_name: "frt-1903-leanstral-candidate-adapter"
  source_sha256: "sha256:3e57fc01ca8a592406af3fb0bd826c87d8b9b4ed09f648a2e3ebbf38055a9edd"
  batch: "G19"
...
- Runtime catalog key: `FRT-1903` / `frt-1903-leanstral-candidate-adapter`.
...
# FRT-1903 — Leanstral Candidate Adapter

## Objective

在G19范围内实现 **Leanstral Candidate Adapter**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：从IR、Rule和业务不变量生成Proof Obligation; Leanstral仅提出证明候选，Lean Kernel为可信根; 组合SMT、Model Checking、Symbolic和Differential Evidence。

## When to Use

- 实现或修改与 **Leanstral Candidate Adapter** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-1903Result`：版本化、带状态和诊断的结构化结果。
- `FRT-1903EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-1903FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-1903CertificateFragment`：供G19批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g19/frt-1903-leanstral-candidate-adapter/
packages/runtime/g19/frt-1903-leanstral-candidate-adapter/
services/control-plane/g19/frt-1903-leanstral-candidate-adapter/
apps/web-console/src/features/g19/frt-1903-leanstral-candidate-adapter/
apps/admin-console/src/features/g19/frt-1903-leanstral-candidate-adapter/
tests/g19/frt-1903-leanstral-candidate-adapter/
```
...
```text
POST /v1/skills/frt-1903/runs
GET  /v1/skills/frt-1903/runs/{run_id}
POST /v1/skills/frt-1903/runs/{run_id}/verify
GET  /v1/skills/frt-1903/runs/{run_id}/findings
GET  /v1/skills/frt-1903/runs/{run_id}/evidence
```
