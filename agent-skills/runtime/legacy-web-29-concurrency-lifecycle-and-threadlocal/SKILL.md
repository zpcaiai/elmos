---
name: legacy-web-29-concurrency-lifecycle-and-threadlocal
description: "Repository-owned exact runtime interface for 并发、实例生命周期与 ThreadLocal 恢复; bounded semantic analysis and evidence generation for Java legacy web modernization."
metadata:
...
---
id: 29-concurrency-lifecycle-and-threadlocal
title: 并发、实例生命周期与 ThreadLocal 恢复
version: 1.0.0
phase: semantic-recovery
priority: P0
requires:
...
识别 Action/Servlet/Filter/Listener/Bean 的实例策略、共享字段、同步、ThreadLocal、异步 dispatch 和资源释放风险。
...
producerSkill: 29-concurrency-lifecycle-and-threadlocal
...
6. 比较 legacy 实例模型与 Spring singleton/request/session bean 语义。
7. 识别 mutable fields、static cache、synchronized、ThreadLocal set/remove、executor 上下文传播。
8. 对 async dispatch、streaming 和 listener callback 建立线程切换图。
9. 生成并发探针和泄漏检测测试。
