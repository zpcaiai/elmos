...
# Dynamic Sql Converter
## ELMOS runtime binding

- Invoke this repository Skill as `$pm-b21-dynamic-sql-converter`.
- Immutable source identity: `dynamic-sql-converter` in `precision-migration-b01-44` (B21).
- Runtime adapter: `database-and-data-route`; binding state: `DECLARED`.
- Resolve and plan with `python3 scripts/precision_migration/runtime.py plan --skill pm-b21-dynamic-sql-converter`.
...
解析、枚举、参数化和重写动态 SQL，并检测注入与不可解析路径。

本 Skill 属于精密语言转换系统。它必须以**可执行证据**为准，不得用模型自评、源码相似度或“能编译”替代语义与行为验证。

## Use when

- 当前任务直接涉及 `dynamic-sql-converter` 所描述的能力。
...
1. 盘点对象与专有能力
2. 执行本 Skill 的核心职责：解析、枚举、参数化和重写动态 SQL，并检测注入与不可解析路径。
