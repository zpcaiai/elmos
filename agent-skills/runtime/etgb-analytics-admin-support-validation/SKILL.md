---
name: etgb-analytics-admin-support-validation
implementation_state: "VERIFIED"
external_evidence_status: "LOCAL_EXECUTED"
production_certification: "NOT_CERTIFIED"
description: Validate all Analytics, Administration, Support and Business Operations capabilities through the production adapter, independent Oracles and fail-closed release evidence. Repository-owned ETGB execution is available through the local runtime; external production evidence remains explicit.
metadata:
  implementation_state: "VERIFIED"
  external_evidence_status: "LOCAL_EXECUTED"
  production_certification: "NOT_CERTIFIED"
  ...
  source_skill: analytics-admin-support-validation
...
---
name: analytics-admin-support-validation
description: Validate all declared Analytics, Administration, Support and Business Operations capabilities with executable positive, boundary, adversarial, concurrency, recovery and evidence-backed release tests.
---

# Analytics, Administration, Support and Business Operations Validation

## Purpose

Use this Skill to plan, execute and certify the `analytics-admin-support` product domain. It owns **63 declared capabilities** (33 P0, 25 P1, 5 P2) across 4 execution contexts and four mandatory variants.

## Inputs

- Frozen release candidate and feature-registry revision.
- Tenant, project, environment authority, budget and fencing context.
- Concrete cases from `suites/full-product.jsonl` whose business line is `analytics-admin-support`.
...
2. Provision an isolated context for: tenant-admin, platform-admin, support-agent, finance-ops.
...
5. Apply the domain Oracle `analytics-data-quality-admin-authorization-and-audit` plus security, state, audit and unsupported-disclosure Oracles.
...
Representative capabilities include: Task Volume And Status Metrics, Success Failure Unavailable Separation, Business Line Quality Metrics, Sser And Human Intervention Metrics, Token Credit Cost Metrics, Machine Wall Clock Eta Accuracy, Cache Hit And Savings, Model Provider Performance, Tenant Usage And Quota, Billing Reconciliation Dashboard, Metric Definition Versioning, Data Freshness And As Of. The complete source of truth is `matrices/full-product.yaml`; no prose list may override it.
...
Route execution through `external-analytics-admin-harness` under `integrations/harness/full-product-adapters.yaml`. An adapter stub or skipped case is implementation progress, not product certification.
