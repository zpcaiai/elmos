---
name: multi-cycle-fixpoint
description: Recipe 多轮执行直到不再产生变化或显式达到收敛条件。 Use for this exact ELMOS Spring Golden Route v2 contract; imported specifications are not runtime or certification evidence.
...
metadata:
  ...
  source_id: FOUNDATION-02-multi-cycle-fixpoint
  source_name: multi-cycle-fixpoint
  source_path: skills/multi-cycle-fixpoint/SKILL.md
  source_sha256: sha256:43f1d3ee89165fe7ade34d89aef625a65d3d340a9a0d07784c7ce10af8ec6d48
  source_contract_sha256: sha256:ad8bffa2a8b8806f848d6ed5546544822578d3b5bb0895c04d47dc46fe2ba8ec
...
# Multi-cycle Fixpoint

## Goal

Recipe 多轮执行直到不再产生变化或显式达到收敛条件。

## Use when

在 Elmos 需要实现或升级 **Multi-cycle Fixpoint** 相关能力时使用；它不是仅供解释的提示词，而是实现、测试与验收契约。

## Capabilities to implement

- 支持 minCycles/maxCycles
- Recipe 可声明变更是否触发下一轮
- 每轮记录变化集合和统计
- 到达上限仍未收敛时失败并输出振荡证据
...
- [ ] 定义 `multi-cycle-fixpoint` 的 public interface、input/output/error schemas。
- [ ] 建立 `multi-cycle-fixpoint` 的 capability/feature flags 与版本字段。
...
- [ ] 至少覆盖并验收：支持 minCycles/maxCycles；以及 Recipe 可声明变更是否触发下一轮。
...
- Machine contract: `references/contract.json` (`sha256:ad8bffa2a8b8806f848d6ed5546544822578d3b5bb0895c04d47dc46fe2ba8ec`).
