---
name: elmos-mcp-tasks-durable-bridge
description: Bridge MCP Tasks to Elmos durable runs with monotonic state, checkpointing, pause/resume/cancel, lease fencing,
  mid-flight input and side-effect reconciliation.
version: 4.0.0
...
- **Kernel authorities used:** K3, K6, K7, K8
...
- `DeploymentUnit`
- `ConformanceRun`
- `MCPTasksDurableBridgeRun`
- `MCPTasksDurableBridgeDecision`

Required semantic capabilities:

- Task handle to Run/Step binding
- Monotonic transition validator
- Idempotent polling and updates
- Checkpoint/recovery after client or worker loss
- Cancellation and side-effect settlement
...
- `mcp-task-mapping.json`
- `task-state-journal.jsonl`
- `task-recovery-plan.json`
- `task-settlement-report.json`
...
- **task handle hijack** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **state regression** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **duplicate external write** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **ghost completion** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **cancellation without reconciliation** — must have a preventive control, a detective signal, a negative fixture and a response action.
