...
description: "Run FRT-2719 Performance and Capacity Certificate for FRT G27 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-2719"
  source_name: "frt-2719-performance-and-capacity-certificate"
  source_sha256: "sha256:1b8976b76a2efe2163541b6aafb3722eb124fc436495da5538a12d027c7732c9"
  batch: "G27"
...
- Runtime catalog key: `FRT-2719` / `frt-2719-performance-and-capacity-certificate`.
...
在G27范围内实现 **Performance and Capacity Certificate**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：业务化Workload Model; 并发线性化点、Race、Lost Update、Idempotency Under Load; Load/Stress/Spike/Soak与Recovery。

## When to Use

- 实现或修改与 **Performance and Capacity Certificate** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-2719Result`：版本化、带状态和诊断的结构化结果。
- `FRT-2719EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-2719FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-2719CertificateFragment`：供G27批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g27/frt-2719-performance-and-capacity-certificate/
packages/runtime/g27/frt-2719-performance-and-capacity-certificate/
services/control-plane/g27/frt-2719-performance-and-capacity-certificate/
apps/web-console/src/features/g27/frt-2719-performance-and-capacity-certificate/
apps/admin-console/src/features/g27/frt-2719-performance-and-capacity-certificate/
tests/g27/frt-2719-performance-and-capacity-certificate/
```
...
```text
POST /v1/skills/frt-2719/runs
GET  /v1/skills/frt-2719/runs/{run_id}
POST /v1/skills/frt-2719/runs/{run_id}/verify
GET  /v1/skills/frt-2719/runs/{run_id}/findings
GET  /v1/skills/frt-2719/runs/{run_id}/evidence
```
