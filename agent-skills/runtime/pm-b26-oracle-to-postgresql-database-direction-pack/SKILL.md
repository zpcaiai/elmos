---
name: pm-b26-oracle-to-postgresql-database-direction-pack
description: "提供从 Oracle 到 PostgreSQL 的数据库专用迁移包，覆盖 DDL、DML、过程代码、专有能力、数据迁移、性能、切换和回滚. Precision Migration B26 contract; use for this exact assessment, transformation, validation, repair, evidence, or cutover scope."
---

# Oracle To Postgresql Database Direction Pack
## ELMOS runtime binding

- Invoke this repository Skill as `$pm-b26-oracle-to-postgresql-database-direction-pack`.
- Immutable source identity: `oracle-to-postgresql-database-direction-pack` in `precision-migration-b01-44` (B26).
- Runtime adapter: `database-and-data-route`; binding state: `UNAVAILABLE`.
- Resolve and plan with `python3 scripts/precision_migration/runtime.py plan --skill pm-b26-oracle-to-postgresql-database-direction-pack`.
...
提供从 Oracle 到 PostgreSQL 的数据库专用迁移包，覆盖 DDL、DML、过程代码、专有能力、数据迁移、性能、切换和回滚。

本 Skill 属于精密语言转换系统。它必须以**可执行证据**为准，不得用模型自评、源码相似度或“能编译”替代语义与行为验证。

## Use when

- 当前任务直接涉及 `oracle-to-postgresql-database-direction-pack` 所描述的能力。
...
1. 盘点对象与专有能力
2. 执行本 Skill 的核心职责：提供从 Oracle 到 PostgreSQL 的数据库专用迁移包，覆盖 DDL、DML、过程代码、专有能力、数据迁移、性能、切换和回滚。
...
```yaml
skill: oracle-to-postgresql-database-direction-pack
version: 1.0.0
