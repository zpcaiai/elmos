---
name: pm-b25-postgresql-custom-operator-and-type
description: "迁移自定义 Type、Operator、Cast、Aggregate 和 Operator Class. Precision Migration B25 contract; use for this exact assessment, transformation, validation, repair, evidence, or cutover scope."
---

# Postgresql Custom Operator And Type
## ELMOS runtime binding

- Invoke this repository Skill as `$pm-b25-postgresql-custom-operator-and-type`.
- Immutable source identity: `postgresql-custom-operator-and-type` in `precision-migration-b01-44` (B25).
- Runtime adapter: `database-and-data-route`; binding state: `DECLARED`.
- Resolve and plan with `python3 scripts/precision_migration/runtime.py plan --skill pm-b25-postgresql-custom-operator-and-type`.
...
- Phase: `G 数据库精密互转`
- Version: `1.0.0`
- Tags: batch-25, postgresql-proprietary-pack

## Purpose

迁移自定义 Type、Operator、Cast、Aggregate 和 Operator Class。

本 Skill 属于精密语言转换系统。它必须以**可执行证据**为准，不得用模型自评、源码相似度或“能编译”替代语义与行为验证。

## Use when

- 当前任务直接涉及 `postgresql-custom-operator-and-type` 所描述的能力。
...
1. 盘点对象与专有能力
2. 执行本 Skill 的核心职责：迁移自定义 Type、Operator、Cast、Aggregate 和 Operator Class。
...
```yaml
skill: postgresql-custom-operator-and-type
version: 1.0.0
