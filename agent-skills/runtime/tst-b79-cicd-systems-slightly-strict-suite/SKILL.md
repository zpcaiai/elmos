---
name: tst-b79-cicd-systems-slightly-strict-suite
description: "Run the slightly strict test suite for Batch 79: GitHub Actions GitLab CI and Jenkins, covering every source Skill with positive and negative executable cases."
metadata:
  id: TB79
...
---
    # TB79 — Batch 79 GitHub Actions GitLab CI and Jenkins 略严苛测试套件

    ## Objective

    对 Batch 79 的全部 15 个Source Skills执行可重放的正向、边界、负向、故障、安全和证据完整性验证。范围为：GitHub Actions、GitLab CI、Jenkins、Runner、OIDC、权限、缓存与发布。
    ...
    |---|---|---|---|
    | PG391 | `b79-cicd-pipeline-profile-detector` | detector | `B79-PG391-RUN, B79-PG391-NEG` |
| PG392 | `b79-github-actions-workflow-generator` | cicd | `B79-PG392-RUN, B79-PG392-NEG` |
| PG393 | `b79-github-reusable-workflow-action-generator` | cicd | `B79-PG393-RUN, B79-PG393-NEG` |
| PG394 | `b79-github-oidc-permission-hardening` | cicd | `B79-PG394-RUN, B79-PG394-NEG` |
| PG395 | `b79-github-matrix-cache-artifact-manager` | cicd | `B79-PG395-RUN, B79-PG395-NEG` |
| PG396 | `b79-gitlab-ci-pipeline-generator` | cicd | `B79-PG396-RUN, B79-PG396-NEG` |
| PG397 | `b79-gitlab-child-pipeline-component-generator` | cicd | `B79-PG397-RUN, B79-PG397-NEG` |
| PG398 | `b79-gitlab-runner-secret-hardening` | cicd | `B79-PG398-RUN, B79-PG398-NEG` |
| PG399 | `b79-gitlab-environment-release-manager` | cicd | `B79-PG399-RUN, B79-PG399-NEG` |
| PG400 | `b79-jenkins-declarative-pipeline-generator` | cicd | `B79-PG400-RUN, B79-PG400-NEG` |
| PG401 | `b79-jenkins-shared-library-generator` | cicd | `B79-PG401-RUN, B79-PG401-NEG` |
| PG402 | `b79-jenkins-agent-credential-hardening` | cicd | `B79-PG402-RUN, B79-PG402-NEG` |
| PG403 | `b79-cross-ci-release-promotion` | cicd | `B79-PG403-RUN, B79-PG403-NEG` |
| PG404 | `b79-pipeline-security-provenance-gate` | cicd | `B79-PG404-RUN, B79-PG404-NEG` |
| PG405 | `b79-cicd-cross-system-certifier` | cicd | `B79-PG405-RUN, B79-PG405-NEG` |
...
- `verify provenance and OIDC claims`
    ...
    - 本Batch全部30项Source-specific Cases进入真实最终状态；
