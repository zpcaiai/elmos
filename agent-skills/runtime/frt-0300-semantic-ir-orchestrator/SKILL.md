...
description: "Run FRT-0300 Semantic IR Orchestrator for FRT G03 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-0300"
  source_name: "frt-0300-semantic-ir-orchestrator"
  source_sha256: "sha256:e61147224a366c1673907364fb831da1efe07df57cb80de9edf3835fb2e1c47d"
  batch: "G03"
...
- Runtime catalog key: `FRT-0300` / `frt-0300-semantic-ir-orchestrator`.
...
# FRT-0300 — Semantic IR Orchestrator

## Objective

在G03范围内实现 **Semantic IR Orchestrator**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：建立跨Vue/React/小程序/ArkUI/Flutter的类型和语义中间层; 保留Null、Identity、Effect、Lifecycle、Navigation、Data Authority和UI语义; 每个IR节点绑定Source Range和稳定Semantic ID。

## When to Use

- 实现或修改与 **Semantic IR Orchestrator** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-0300Result`：版本化、带状态和诊断的结构化结果。
- `FRT-0300EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-0300FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-0300CertificateFragment`：供G03批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g03/frt-0300-semantic-ir-orchestrator/
packages/runtime/g03/frt-0300-semantic-ir-orchestrator/
services/control-plane/g03/frt-0300-semantic-ir-orchestrator/
apps/web-console/src/features/g03/frt-0300-semantic-ir-orchestrator/
apps/admin-console/src/features/g03/frt-0300-semantic-ir-orchestrator/
tests/g03/frt-0300-semantic-ir-orchestrator/
```
...
```text
POST /v1/skills/frt-0300/runs
GET  /v1/skills/frt-0300/runs/{run_id}
POST /v1/skills/frt-0300/runs/{run_id}/verify
GET  /v1/skills/frt-0300/runs/{run_id}/findings
GET  /v1/skills/frt-0300/runs/{run_id}/evidence
```
