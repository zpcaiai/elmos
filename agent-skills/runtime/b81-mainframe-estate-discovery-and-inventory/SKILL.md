---
name: b81-mainframe-estate-discovery-and-inventory
description: "Use when ELMOS must run mainframe estate discovery and inventory for Batch 81 COBOL / JCL / REXX / CICS / IMS Mainframe Pack. Preserve native semantics, safety boundaries, ownership, traceability, and fail-closed evidence; require real vendor toolchain execution for runtime claims."
metadata:
  source_package: "elmos-language-packs-batch81-95-complete"
  source_id: "PG223"
  source_name: "mainframe-estate-discovery-and-inventory"
  source_sha256: "sha256:3a81258e8e34a5cd2b8e528e59111f3137a26637808753f7e58f171e00a70b17"
  batch: 81
  source_engine: "elmos.language-packs"
  source_status: "proposed"
  normalized_namespace: "language-pack"
---
# Batch 81 Language Pack / source PG223 — Mainframe Estate Discovery And Inventory

## 1. Objective

Discover and inventory mainframe estate discovery and inventory within the COBOL / JCL / REXX / CICS / IMS Mainframe Pack, preserving platform semantics, traceability, security, and reproducible evidence.

## 2. Scope

This Skill is part of **Batch 81: COBOL / JCL / REXX / CICS / IMS Mainframe Pack**.

### In scope

- Process COBOL programs.
- Process copybooks.
- Process JCL procedures.
- Process REXX scripts.
- Process CICS transactions.
- Process IMS definitions.
- Process VSAM files.
- Process DB2 schemas.
- Produce canonical mainframe IR.
- Produce business-rule graph.
- Produce batch DAG.
- Produce REST/event adapters.
- Produce Java/.NET modernization output.
- Produce parallel-run evidence.
...
2. Process the relevant artifacts for Mainframe Estate Discovery And Inventory using the approved enterprise-mainframe semantic contracts and toolchain versions.
3. Preserve source provenance and create Artifact Graph links across inputs, outputs, tests, approvals, and evidence.
4. Detect and classify packed-decimal drift, EBCDIC/Unicode conversion, transaction-boundary loss; never convert uncertainty or unsupported behavior into a silent success.
...
7. Apply the safety boundary: Financial and transaction semantics must be proven by record-level and monetary parallel-run evidence before cutover.
...
- `PACKED_DECIMAL_DRIFT` — stop unsafe continuation and emit source-linked diagnostic evidence.
- `EBCDIC_UNICODE_CONVERSION` — stop unsafe continuation and emit source-linked diagnostic evidence.
- `TRANSACTION_BOUNDARY_LOSS` — stop unsafe continuation and emit source-linked diagnostic evidence.
- `BATCH_RESTART_CORRUPTION` — stop unsafe continuation and emit source-linked diagnostic evidence.
- `COPYBOOK_MISMATCH` — stop unsafe continuation and emit source-linked diagnostic evidence.
- `HIDDEN_CICS_STATE` — stop unsafe continuation and emit source-linked diagnostic evidence.
...
- Apply this Batch safety boundary: **Financial and transaction semantics must be proven by record-level and monetary parallel-run evidence before cutover.**

## 14. Unit Tests

- A representative valid cobol-mainframe input produces all declared outputs for mainframe-estate-discovery-and-inventory.
...
- At least one batch-specific risk (packed-decimal drift) is detected by a negative fixture.

## 15. Integration Tests

- Consume approved Project Synthesis or Modernization baselines and write complete lineage into the Artifact Graph.
- Integrate with at least one representative toolchain from: Enterprise COBOL, z/OS, CICS, IMS.
...
- Reject or block packed-decimal drift without weakening the expected result.
- Reject or block EBCDIC/Unicode conversion without weakening the expected result.
- Reject or block transaction-boundary loss without weakening the expected result.
- Reject or block batch restart corruption without weakening the expected result.
- Reject or block copybook mismatch without weakening the expected result.
- Reject or block hidden CICS state without weakening the expected result.
