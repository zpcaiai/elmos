...
description: "Run FRT-2514 Visual Consistency Validator for FRT G25 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-2514"
  source_name: "frt-2514-visual-consistency-validator"
  source_sha256: "sha256:b476ae84e9b156c6cc945f462394b0c6ee426ebeca27b637f611418a8606b8cb"
  batch: "G25"
...
- Runtime catalog key: `FRT-2514` / `frt-2514-visual-consistency-validator`.
...
在G25范围内实现 **Visual Consistency Validator**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：Actor/Goal驱动Task Contract与Completion Oracle; Loading/Empty/Error/Retry/Unsupported完整状态矩阵; Form Validation、Data Preservation、Single Flight。

## When to Use

- 实现或修改与 **Visual Consistency Validator** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-2514Result`：版本化、带状态和诊断的结构化结果。
- `FRT-2514EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-2514FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-2514CertificateFragment`：供G25批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g25/frt-2514-visual-consistency-validator/
packages/runtime/g25/frt-2514-visual-consistency-validator/
services/control-plane/g25/frt-2514-visual-consistency-validator/
apps/web-console/src/features/g25/frt-2514-visual-consistency-validator/
apps/admin-console/src/features/g25/frt-2514-visual-consistency-validator/
tests/g25/frt-2514-visual-consistency-validator/
```
...
```text
POST /v1/skills/frt-2514/runs
GET  /v1/skills/frt-2514/runs/{run_id}
POST /v1/skills/frt-2514/runs/{run_id}/verify
GET  /v1/skills/frt-2514/runs/{run_id}/findings
GET  /v1/skills/frt-2514/runs/{run_id}/evidence
```
