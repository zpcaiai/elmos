...
description: "Run FRT-0809 Portal Teleport and Overlay Mapper for FRT G08 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-0809"
  source_name: "frt-0809-portal-teleport-and-overlay-mapper"
  source_sha256: "sha256:1e8af722ece8a734cd8a484bd990a1672ff413c7267ac7486c0323059fd71aa1"
  batch: "G08"
...
- Runtime catalog key: `FRT-0809` / `frt-0809-portal-teleport-and-overlay-mapper`.
...
# FRT-0809 — Portal Teleport and Overlay Mapper

## Objective

在G08范围内实现 **Portal Teleport and Overlay Mapper**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：保持Props默认值、事件时机、Slot/Children/Builder结构; 保持Key、Ref、Widget Identity和状态保留语义; 正确降低Hooks、Composable、Context和局部状态。

## When to Use

- 实现或修改与 **Portal Teleport and Overlay Mapper** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-0809Result`：版本化、带状态和诊断的结构化结果。
- `FRT-0809EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-0809FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-0809CertificateFragment`：供G08批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g08/frt-0809-portal-teleport-and-overlay-mapper/
packages/runtime/g08/frt-0809-portal-teleport-and-overlay-mapper/
services/control-plane/g08/frt-0809-portal-teleport-and-overlay-mapper/
apps/web-console/src/features/g08/frt-0809-portal-teleport-and-overlay-mapper/
apps/admin-console/src/features/g08/frt-0809-portal-teleport-and-overlay-mapper/
tests/g08/frt-0809-portal-teleport-and-overlay-mapper/
```
...
```text
POST /v1/skills/frt-0809/runs
GET  /v1/skills/frt-0809/runs/{run_id}
POST /v1/skills/frt-0809/runs/{run_id}/verify
GET  /v1/skills/frt-0809/runs/{run_id}/findings
GET  /v1/skills/frt-0809/runs/{run_id}/evidence
```
