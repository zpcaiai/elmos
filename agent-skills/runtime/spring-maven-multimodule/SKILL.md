---
name: spring-maven-multimodule
description: 迁移 parent、BOM、plugin management、profiles 和 reactor build。 Use for this exact ELMOS Spring Golden Route v2 contract; imported specifications are not runtime or certification evidence.
...
metadata:
  ...
  source_id: ELMOS-GR-012
  source_name: spring-maven-multimodule
  source_path: skills/spring-maven-multimodule/SKILL.md
  source_sha256: sha256:ad43339278c25423c2d1bb8f16f9f2c249d5d4b17b68b42d3a899f70cd0ccae7
  source_contract_sha256: sha256:13c6f2c8cf44d6115a63e7836c8bf2b728abe4d6f6ac61eb2b9d43a3e10ad39f
...
# Maven Multi-module Route

## Objective

迁移 parent、BOM、plugin management、profiles 和 reactor build。
...
- [ ] 定义 `spring-maven-multimodule` 的 public interface、输入/输出/Error Schema 和版本策略。
- [ ] 实现 `spring-maven-multimodule` 的 production adapter/service，不把业务状态只保存在 Prompt 或内存对象中。
...
- [ ] 至少生成：maven-plan.json, pom-patches, reactor-evidence.json。
...
- [ ] 本 Skill 的 required outputs 均由真实目标仓库执行产生：maven-plan.json, pom-patches, reactor-evidence.json。
- [ ] 所有 mandatory tests 和 security gates 通过，且无 critical unknown。

## Dependencies

- `spring-target-profile`
...
- Machine contract: `references/contract.json` (`sha256:13c6f2c8cf44d6115a63e7836c8bf2b728abe4d6f6ac61eb2b9d43a3e10ad39f`).
