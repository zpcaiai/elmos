...
description: "Run FRT-2909 File Upload and Content Security Validator for FRT G29 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-2909"
  source_name: "frt-2909-file-upload-and-content-security-validator"
  source_sha256: "sha256:2d7d947dfdadce844df996c18898d116a46736b4ae99da145c00a63c2ef048d3"
  batch: "G29"
  source_version: "1.0.0"
  source_risk: "critical-by-default"
  source_certificate_family: "SC"
...
- Runtime catalog key: `FRT-2909` / `frt-2909-file-upload-and-content-security-validator`.
...
在G29范围内实现 **File Upload and Content Security Validator**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：Asset、Threat、Trust Boundary、Attack Surface; Authentication、Authorization、Object/Field Scope、Tenant Isolation; Session/Token、API、Input、File、SSRF/XSS/Command/Prompt Injection。

## When to Use

- 实现或修改与 **File Upload and Content Security Validator** 对应的Contract、Runtime、API、UI、测试或证据链时。
- 相关代码、Schema、Pack、Policy、环境或依赖变化导致既有Evidence失效时。
- G29 Orchestrator生成该Skill的执行义务时。
...
- `SourceArtifactSet`：G21–G28资产、数据流、权限、服务、依赖、运行和恢复模型, Source、Generated Code、Pack/Skill、Container、Dependency、Model和Environment。
- `SkillPolicy`：权限、网络、Sandbox、Toolchain、Evidence和停止规则。

## Outputs

- `FRT-2909Result`：版本化、带状态和诊断的结构化结果。
- `FRT-2909EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-2909FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-2909CertificateFragment`：供G29批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g29/frt-2909-file-upload-and-content-security-validator/
packages/runtime/g29/frt-2909-file-upload-and-content-security-validator/
services/control-plane/g29/frt-2909-file-upload-and-content-security-validator/
apps/web-console/src/features/g29/frt-2909-file-upload-and-content-security-validator/
apps/admin-console/src/features/g29/frt-2909-file-upload-and-content-security-validator/
tests/g29/frt-2909-file-upload-and-content-security-validator/
```
...
```text
POST /v1/skills/frt-2909/runs
GET  /v1/skills/frt-2909/runs/{run_id}
POST /v1/skills/frt-2909/runs/{run_id}/verify
GET  /v1/skills/frt-2909/runs/{run_id}/findings
GET  /v1/skills/frt-2909/runs/{run_id}/evidence
```
...
- The parent G29 Orchestrator can consume the result and independently decide certification.
