---
name: elmos-schema-registry-compatibility-controller
description: Govern Avro, Protobuf, JSON Schema and event schema registration, subject naming, compatibility modes, migration and rollback.
version: 4.0.0
...
# Schema Registry Compatibility Controller
...
- `SchemaRegistryCompatibilityControllerProfile`
- `SchemaRegistryCompatibilityControllerPlan`
- `SchemaRegistryCompatibilityControllerRun`
- `SchemaRegistryCompatibilityControllerDecision`

Required semantic capabilities:

- fingerprint registry and subject policies
- evaluate backward/forward/full compatibility
- coordinate producer and consumer rollout
- generate transcoders and upcasters
- prevent incompatible deletion or mode changes
...
- `artifacts/elmos-schema-registry-compatibility-controller/profile.yaml`
- `artifacts/elmos-schema-registry-compatibility-controller/plan.json`
- `artifacts/elmos-schema-registry-compatibility-controller/result.json`
- `artifacts/elmos-schema-registry-compatibility-controller/evidence/`
...
- **subject naming collision** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **compatibility mode weakened** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **consumer uses stale schema** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **schema ID reused incorrectly** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **registry unavailable during rollback** — must have a preventive control, a detective signal, a negative fixture and a response action.
...
- native scenario: fingerprint registry and subject policies
- native scenario: evaluate backward/forward/full compatibility
- native scenario: coordinate producer and consumer rollout
- native scenario: generate transcoders and upcasters
- native scenario: prevent incompatible deletion or mode changes
