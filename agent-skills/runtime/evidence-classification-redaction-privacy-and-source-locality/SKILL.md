---
name: evidence-classification-redaction-privacy-and-source-locality
description: 实现证据分类、Source Locality、Secret/PII/Source 扫描、结构化脱敏和披露审批。
metadata:
  elmos_batch: "37A"
  elmos_skill_id: "ART37A014"
  pack_version: "3.7-complete"
---

# ART37A014 — Evidence Classification、Redaction、Privacy 与 Source Locality

## Objective

实现证据分类、Source Locality、Secret/PII/Source 扫描、结构化脱敏和披露审批。
...
- 分类 Source/Patch/Path/Log/PII/Secret/Business Confidential/Security Finding。
- 脱敏创建新 Content Object 和 REDACTED_FROM 关系，不修改 sealed object。
- 提供 Patch、SBOM、Log、Public/Internal Pack 披露 Profile。
- Export 前执行第二次 Privacy Scan，并支持 Pack/Project/Tenant Scoped Pseudonym。
...
- secretBlocksArtifactExport
- redactionCreatesNewArtifact
- globalCrossTenantPseudonymIsNotUsed
- exportRunsSecondPrivacyScan
