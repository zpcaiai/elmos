---
name: spring-async-graph
description: 抽取 Kafka/Rabbit、Redis、Scheduler/Batch、retry/DLQ、TTL 和幂等副作用。 Use for this exact ELMOS Spring Golden Route v2 contract; imported specifications are not runtime or certification evidence.
...
metadata:
  ...
  source_id: ELMOS-GR-039
  source_name: spring-async-graph
  source_path: skills/spring-async-graph/SKILL.md
  source_sha256: sha256:a7c53a44ca8adbd39bae1faa46f6c9ec39078417b978af0fc426262ad46a6e5d
  source_contract_sha256: sha256:8e8ef565ffbeb4b51f70a99ef5314126a2660447a409eeff33feafa143371030
...
# Messaging Cache Scheduler Graph

## Objective

抽取 Kafka/Rabbit、Redis、Scheduler/Batch、retry/DLQ、TTL 和幂等副作用。

## Scope and capabilities

- Producer/consumer schema
- Consumer group/order/retry
- Cache key/TTL/session
- Cron/job
...
- messaging-graph.json
- cache-contract.json
- scheduler-inventory.json
...
- [ ] 定义 `spring-async-graph` 的 public interface、输入/输出/Error Schema 和版本策略。
- [ ] 实现 `spring-async-graph` 的 production adapter/service，不把业务状态只保存在 Prompt 或内存对象中。
...
- [ ] 至少生成：messaging-graph.json, cache-contract.json, scheduler-inventory.json。
...
- [ ] 本 Skill 的 required outputs 均由真实目标仓库执行产生：messaging-graph.json, cache-contract.json, scheduler-inventory.json。
...
- Machine contract: `references/contract.json` (`sha256:8e8ef565ffbeb4b51f70a99ef5314126a2660447a409eeff33feafa143371030`).
