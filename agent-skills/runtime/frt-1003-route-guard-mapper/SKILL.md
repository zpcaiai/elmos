...
description: "Run FRT-1003 Route Guard Mapper for FRT G10 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-1003"
  source_name: "frt-1003-route-guard-mapper"
  source_sha256: "sha256:be06f41b6581101f162994f52cccaa85d0200d684717fda75f28f747a6288a3e"
  batch: "G10"
...
- Runtime catalog key: `FRT-1003` / `frt-1003-route-guard-mapper`.
...
# FRT-1003 — Route Guard Mapper

## Objective

在G10范围内实现 **Route Guard Mapper**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：保持Route、History、Guard和Deep Link语义; 表单Validation、错误、草稿和提交幂等完整; API、Event、Storage和Realtime边界显式。

## When to Use

- 实现或修改与 **Route Guard Mapper** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-1003Result`：版本化、带状态和诊断的结构化结果。
- `FRT-1003EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-1003FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-1003CertificateFragment`：供G10批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g10/frt-1003-route-guard-mapper/
packages/runtime/g10/frt-1003-route-guard-mapper/
services/control-plane/g10/frt-1003-route-guard-mapper/
apps/web-console/src/features/g10/frt-1003-route-guard-mapper/
apps/admin-console/src/features/g10/frt-1003-route-guard-mapper/
tests/g10/frt-1003-route-guard-mapper/
```
...
```text
POST /v1/skills/frt-1003/runs
GET  /v1/skills/frt-1003/runs/{run_id}
POST /v1/skills/frt-1003/runs/{run_id}/verify
GET  /v1/skills/frt-1003/runs/{run_id}/findings
GET  /v1/skills/frt-1003/runs/{run_id}/evidence
```
