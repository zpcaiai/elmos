...
# Natural Language Transformation Definition
## ELMOS runtime binding

- Invoke this repository Skill as `$pm-b11-natural-language-transformation-definition`.
- Immutable source identity: `natural-language-transformation-definition` in `precision-migration-b01-44` (B11).
- Runtime adapter: `semantic-recovery-and-ir`; binding state: `UNAVAILABLE`.
- Resolve and plan with `python3 scripts/precision_migration/runtime.py plan --skill pm-b11-natural-language-transformation-definition`.
...
将目标、范围、前置条件、例外、禁止项和验收要求结构化为转换定义。

本 Skill 属于精密语言转换系统。它必须以**可执行证据**为准，不得用模型自评、源码相似度或“能编译”替代语义与行为验证。

## Use when

- 当前任务直接涉及 `natural-language-transformation-definition` 所描述的能力。
...
1. 验证规则前置条件
2. 执行本 Skill 的核心职责：将目标、范围、前置条件、例外、禁止项和验收要求结构化为转换定义。
3. 优先使用确定性规则
4. 对长尾生成多个候选
5. 按可验证性和成本排序
6. 运行最小构建/静态验证并输出结果集
...
```yaml
skill: natural-language-transformation-definition
version: 1.0.0
