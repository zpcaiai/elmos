---
name: legacy-web-43-compatibility-shim-synthesis
description: "Repository-owned exact runtime interface for 兼容层合成规划; bounded semantic analysis and evidence generation for Java legacy web modernization."
metadata:
  source_package: elmos.java-legacy-web.repository-modernization
  source_version: 1.0.0
  source_id: 43-compatibility-shim-synthesis
  source_digest: sha256:a2fcdcca0ec7370ce2a4673395f17eb88730d8b2b5a36447a9855f75bec13fc4
  phase: planning
  runtime_state: CODE_COMPLETE_LOCAL
  capability_state: LOCAL_EXECUTED
  operation_code: COMPATIBILITY_SHIMS_COMPILED
  runtime_handler_id: legacy-web-handler:43-compatibility-shim-synthesis
---
...
---
id: 43-compatibility-shim-synthesis
title: 兼容层合成规划
version: 1.0.0
phase: planning
priority: P0
...
当 Spring 无直接等价物时生成最小、可测试、可移除的 compatibility shim，而非改变业务行为或留下隐式 TODO。
...
producerSkill: 43-compatibility-shim-synthesis
...
5. 输出允许差异和回滚设计。
6. 适用于 legacy message/forward/form lifecycle/value stack/custom tag 等无直接一对一映射语义。
7. shim 必须小于明确复杂度阈值、无业务逻辑、具备 contract tests。
8. 记录 owner、使用点、性能成本、移除 gate 和 target-native replacement。
9. 不得用 shim 隐藏 critical unknown。
