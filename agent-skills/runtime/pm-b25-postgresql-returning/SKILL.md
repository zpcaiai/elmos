---
name: pm-b25-postgresql-returning
description: "迁移 INSERT/UPDATE/DELETE RETURNING 和应用消费方式. Precision Migration B25 contract; use for this exact assessment, transformation, validation, repair, evidence, or cutover scope."
---

# Postgresql Returning
## ELMOS runtime binding

- Invoke this repository Skill as `$pm-b25-postgresql-returning`.
- Immutable source identity: `postgresql-returning` in `precision-migration-b01-44` (B25).
- Runtime adapter: `database-and-data-route`; binding state: `DECLARED`.
- Resolve and plan with `python3 scripts/precision_migration/runtime.py plan --skill pm-b25-postgresql-returning`.
...
迁移 INSERT/UPDATE/DELETE RETURNING 和应用消费方式。

本 Skill 属于精密语言转换系统。它必须以**可执行证据**为准，不得用模型自评、源码相似度或“能编译”替代语义与行为验证。

## Use when

- 当前任务直接涉及 `postgresql-returning` 所描述的能力。
...
1. 盘点对象与专有能力
2. 执行本 Skill 的核心职责：迁移 INSERT/UPDATE/DELETE RETURNING 和应用消费方式。
