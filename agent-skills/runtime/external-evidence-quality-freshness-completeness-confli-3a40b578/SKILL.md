---
name: external-evidence-quality-freshness-completeness-confli-3a40b578
description: 计算Authenticity、Integrity、Correlation、Completeness、Freshness、Scope、Reproducibility与Producer Assurance。
metadata:
  elmos_batch: "37B"
  elmos_skill_id: "EVI37B015"
  pack_version: "3.7-complete"
---

# EVI37B015 — External Evidence Quality、Freshness、Completeness、Conflict 与 Gating

## Objective

计算Authenticity、Integrity、Correlation、Completeness、Freshness、Scope、Reproducibility与Producer Assurance。
...
- 每个维度使用 PASS/PARTIAL/FAIL/UNKNOWN/NA。
- Completeness Profile按项目类型定义必需Build/Test/Security/SBOM/Artifact/Producer证据。
- 保存 Pipeline/Test/Sonar/SAST/SBOM/Performance/Audit 冲突。
- Gate结果区分 PASSED/FAILED/INCONCLUSIVE/REQUIRES_REVIEW。
...
- missingRequiredReportMakesGateIncomplete
- pipelineSuccessDoesNotOverrideTestFailure
- unknownDimensionDoesNotPass
- qualityScoreCannotHideFailedCriticalDimension
