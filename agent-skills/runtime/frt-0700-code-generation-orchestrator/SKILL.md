...
description: "Run FRT-0700 Code Generation Orchestrator for FRT G07 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-0700"
  source_name: "frt-0700-code-generation-orchestrator"
  source_sha256: "sha256:e3ba50b1622d722a4c552796c08f839cc57c9eaa7482d1d8a1e773f88c31cf7a"
  batch: "G07"
...
- Runtime catalog key: `FRT-0700` / `frt-0700-code-generation-orchestrator`.
...
# FRT-0700 — Code Generation Orchestrator

## Objective

在G07范围内实现 **Code Generation Orchestrator**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：Typed IR经规则Pass降低到目标原生AST; Gap生成Typed Hole而非空实现; Format/Analyze/Build驱动有界确定性修复。

## When to Use

- 实现或修改与 **Code Generation Orchestrator** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-0700Result`：版本化、带状态和诊断的结构化结果。
- `FRT-0700EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-0700FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-0700CertificateFragment`：供G07批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g07/frt-0700-code-generation-orchestrator/
packages/runtime/g07/frt-0700-code-generation-orchestrator/
services/control-plane/g07/frt-0700-code-generation-orchestrator/
apps/web-console/src/features/g07/frt-0700-code-generation-orchestrator/
apps/admin-console/src/features/g07/frt-0700-code-generation-orchestrator/
tests/g07/frt-0700-code-generation-orchestrator/
```
...
```text
POST /v1/skills/frt-0700/runs
GET  /v1/skills/frt-0700/runs/{run_id}
POST /v1/skills/frt-0700/runs/{run_id}/verify
GET  /v1/skills/frt-0700/runs/{run_id}/findings
GET  /v1/skills/frt-0700/runs/{run_id}/evidence
```
