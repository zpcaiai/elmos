...
description: 所有内建、MCP、CLI、数据库、部署工具统一成强类型 Tool Definition。 Use for this exact ELMOS Spring Golden Route v2 contract; imported specifications are not runtime or certification evidence.
...
metadata:
  ...
  source_id: FOUNDATION-08-typed-tool-registry
  source_name: typed-tool-registry
  source_path: skills/typed-tool-registry/SKILL.md
  source_sha256: sha256:edd544104b04b0f618308f69533a77df67bd3c8f6bda93ae6d3772fd6780d88b
  source_contract_sha256: sha256:5b0104f9bd21718deb4920b970ee92df719b8e3406e2da22a7a2cadaa3b45edf
...
所有内建、MCP、CLI、数据库、部署工具统一成强类型 Tool Definition。

## Use when

在 Elmos 需要实现或升级 **Typed Tool Registry** 相关能力时使用；它不是仅供解释的提示词，而是实现、测试与验收契约。

## Capabilities to implement

- 定义 name/description/input/output/structured output/permission
- definition 与 executable handler 分离
- tool name/schema 注册时验证
- registry 支持按 Agent/policy/context 动态 materialize
...
- [ ] 定义 `typed-tool-registry` 的 public interface、input/output/error schemas。
- [ ] 建立 `typed-tool-registry` 的 capability/feature flags 与版本字段。
...
- [ ] 至少覆盖并验收：定义 name/description/input/output/structured output/permission；以及 definition 与 executable handler 分离。
...
- Machine contract: `references/contract.json` (`sha256:5b0104f9bd21718deb4920b970ee92df719b8e3406e2da22a7a2cadaa3b45edf`).
