---
name: third-party-audit-penetration-test-compliance-assessmen-f8b2a7d4
description: 安全接收外部审计、渗透测试、认证与评估报告，验证Issuer、Signature、Scope、Period和修订。
metadata:
  elmos_batch: "37B"
  elmos_skill_id: "EVI37B011"
  pack_version: "3.7-complete"
---

# EVI37B011 — 第三方审计、渗透测试与合规报告 Intake

## Objective

安全接收外部审计、渗透测试、认证与评估报告，验证Issuer、Signature、Scope、Period和修订。
...
- 支持 Signed PDF/JSON/XML/Attestation/Certificate/Portal API/Secure Upload。
- 机器抽取标记 MACHINE_EXTRACTED_CLAIM，高风险使用前人工复核。
- 结论只适用于精确 Tenant/Product/Repository/Version/Environment/Period/Control Scope。
- Revision 使用 SUPERSEDES，Withdrawal 不删除历史。
...
- unsignedReportIsNotTrusted
- reportClaimCannotExtendBeyondScope
- machineExtractedClaimRequiresHumanReview
- withdrawalInvalidatesCurrentUse
