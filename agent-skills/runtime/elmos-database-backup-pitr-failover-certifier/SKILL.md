---
name: elmos-database-backup-pitr-failover-certifier
description: Certify backups, point-in-time recovery, replica promotion, consistency, encryption, retention and application reconnect behavior for every stateful target.
version: 4.0.0
...
# Database Backup, PITR and Failover Certifier
...
- `DatabaseBackupPITRAndFailoverCertifierProfile`
- `DatabaseBackupPITRAndFailoverCertifierPlan`
- `DatabaseBackupPITRAndFailoverCertifierRun`
- `DatabaseBackupPITRAndFailoverCertifierDecision`

Required semantic capabilities:

- verify full and incremental backup chains
- perform isolated PITR to selected timestamp
- promote replica under failure injection
- validate application reconnect and fencing
- prove encryption, retention and deletion policy
...
- `artifacts/elmos-database-backup-pitr-failover-certifier/profile.yaml`
- `artifacts/elmos-database-backup-pitr-failover-certifier/plan.json`
- `artifacts/elmos-database-backup-pitr-failover-certifier/result.json`
- `artifacts/elmos-database-backup-pitr-failover-certifier/evidence/`
...
- **backup exists but cannot restore** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **replica promotion causes split brain** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **PITR misses external side effects** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **encryption key unavailable** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **restore evidence uses different schema** — must have a preventive control, a detective signal, a negative fixture and a response action.
...
- native scenario: verify full and incremental backup chains
- native scenario: perform isolated PITR to selected timestamp
- native scenario: promote replica under failure injection
- native scenario: validate application reconnect and fencing
- native scenario: prove encryption, retention and deletion policy
