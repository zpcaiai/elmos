---
name: rewrite-rpc-runtime
description: 让不同语言实现的 parser/recipe 通过统一协议参与 Elmos 变换。 Use for this exact ELMOS Spring Golden Route v2 contract; imported specifications are not runtime or certification evidence.
...
metadata:
  ...
  source_id: FOUNDATION-05-rewrite-rpc-runtime
  source_name: rewrite-rpc-runtime
  source_path: skills/rewrite-rpc-runtime/SKILL.md
  source_sha256: sha256:3a8ff1c173fe7e94cbfe55ebe565abfd0d0d7a47d1c3a0e09b07661173630b19
  source_contract_sha256: sha256:37876e2f8f0538905a54c528dc1687bf5603cfff13f4c2af406cf5dc01fabc19
...
# Cross-language Rewrite RPC Runtime

## Goal

让不同语言实现的 parser/recipe 通过统一协议参与 Elmos 变换。

## Use when

在 Elmos 需要实现或升级 **Cross-language Rewrite RPC Runtime** 相关能力时使用；它不是仅供解释的提示词，而是实现、测试与验收契约。

## Capabilities to implement

- Host 与语言 runtime 分离进程生命周期
- 协议覆盖 prepare/visit/batch/generate/print 等语义操作
- 消息 schema 可版本协商
- 远端失败转换为结构化错误而非死锁
...
- [ ] 定义 `rewrite-rpc-runtime` 的 public interface、input/output/error schemas。
- [ ] 建立 `rewrite-rpc-runtime` 的 capability/feature flags 与版本字段。
...
- [ ] 至少覆盖并验收：Host 与语言 runtime 分离进程生命周期；以及 协议覆盖 prepare/visit/batch/generate/print 等语义操作。
...
- Machine contract: `references/contract.json` (`sha256:37876e2f8f0538905a54c528dc1687bf5603cfff13f4c2af406cf5dc01fabc19`).
