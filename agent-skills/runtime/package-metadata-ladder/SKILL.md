---
name: package-metadata-ladder
description: 按低成本到高成本逐级获取包依赖和哈希元数据。 Use for this exact ELMOS Spring Golden Route v2 contract; imported specifications are not runtime or certification evidence.
...
metadata:
  ...
  source_id: FOUNDATION-04-package-metadata-ladder
  source_name: package-metadata-ladder
  source_path: skills/package-metadata-ladder/SKILL.md
  source_sha256: sha256:d485d6f7ecce8ec64bcc28aaeb818192f263349d26694f0dfd5f8c13898c1287
  source_contract_sha256: sha256:36807bb3c8ccc7fcf1f52856d6ce7444c3157993e13b7b06ad7c14a6e917216b
...
# Package Metadata Ladder

## Goal

按低成本到高成本逐级获取包依赖和哈希元数据。

## Use when

在 Elmos 需要实现或升级 **Package Metadata Ladder** 相关能力时使用；它不是仅供解释的提示词，而是实现、测试与验收契约。

## Capabilities to implement

- 优先标准 metadata sidecar/API
- 其次 Range read 包内 metadata
- 再退到完整包下载
- 无法可信获取时 fail loud，绝不猜测依赖
...
- [ ] 定义 `package-metadata-ladder` 的 public interface、input/output/error schemas。
- [ ] 建立 `package-metadata-ladder` 的 capability/feature flags 与版本字段。
...
- [ ] 至少覆盖并验收：优先标准 metadata sidecar/API；以及 其次 Range read 包内 metadata。
...
- Machine contract: `references/contract.json` (`sha256:36807bb3c8ccc7fcf1f52856d6ce7444c3157993e13b7b06ad7c14a6e917216b`).
