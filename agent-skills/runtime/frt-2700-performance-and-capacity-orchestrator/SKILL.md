...
description: "Run FRT-2700 Performance and Capacity Orchestrator for FRT G27 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-2700"
  source_name: "frt-2700-performance-and-capacity-orchestrator"
  source_sha256: "sha256:15c6dd9d3edea83ac0df66870c8aa97fddb8115c824a4ff82cdd5c622f70702c"
  batch: "G27"
...
- Runtime catalog key: `FRT-2700` / `frt-2700-performance-and-capacity-orchestrator`.
...
在G27范围内实现 **Performance and Capacity Orchestrator**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：业务化Workload Model; 并发线性化点、Race、Lost Update、Idempotency Under Load; Load/Stress/Spike/Soak与Recovery。

## When to Use

- 实现或修改与 **Performance and Capacity Orchestrator** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-2700Result`：版本化、带状态和诊断的结构化结果。
- `FRT-2700EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-2700FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-2700CertificateFragment`：供G27批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g27/frt-2700-performance-and-capacity-orchestrator/
packages/runtime/g27/frt-2700-performance-and-capacity-orchestrator/
services/control-plane/g27/frt-2700-performance-and-capacity-orchestrator/
apps/web-console/src/features/g27/frt-2700-performance-and-capacity-orchestrator/
apps/admin-console/src/features/g27/frt-2700-performance-and-capacity-orchestrator/
tests/g27/frt-2700-performance-and-capacity-orchestrator/
```
...
```text
POST /v1/skills/frt-2700/runs
GET  /v1/skills/frt-2700/runs/{run_id}
POST /v1/skills/frt-2700/runs/{run_id}/verify
GET  /v1/skills/frt-2700/runs/{run_id}/findings
GET  /v1/skills/frt-2700/runs/{run_id}/evidence
```
