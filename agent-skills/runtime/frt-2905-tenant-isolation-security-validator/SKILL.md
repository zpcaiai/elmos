...
description: "Run FRT-2905 Tenant Isolation Security Validator for FRT G29 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-2905"
  source_name: "frt-2905-tenant-isolation-security-validator"
  source_sha256: "sha256:babecf394b19dcd5b55437c736bca02b6ace0d4d2a210de1dd68427b1bb5ec1a"
  batch: "G29"
...
- Runtime catalog key: `FRT-2905` / `frt-2905-tenant-isolation-security-validator`.
...
在G29范围内实现 **Tenant Isolation Security Validator**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：Asset、Threat、Trust Boundary、Attack Surface; Authentication、Authorization、Object/Field Scope、Tenant Isolation; Session/Token、API、Input、File、SSRF/XSS/Command/Prompt Injection。

## When to Use

- 实现或修改与 **Tenant Isolation Security Validator** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-2905Result`：版本化、带状态和诊断的结构化结果。
- `FRT-2905EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-2905FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-2905CertificateFragment`：供G29批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g29/frt-2905-tenant-isolation-security-validator/
packages/runtime/g29/frt-2905-tenant-isolation-security-validator/
services/control-plane/g29/frt-2905-tenant-isolation-security-validator/
apps/web-console/src/features/g29/frt-2905-tenant-isolation-security-validator/
apps/admin-console/src/features/g29/frt-2905-tenant-isolation-security-validator/
tests/g29/frt-2905-tenant-isolation-security-validator/
```
...
```text
POST /v1/skills/frt-2905/runs
GET  /v1/skills/frt-2905/runs/{run_id}
POST /v1/skills/frt-2905/runs/{run_id}/verify
GET  /v1/skills/frt-2905/runs/{run_id}/findings
GET  /v1/skills/frt-2905/runs/{run_id}/evidence
```
