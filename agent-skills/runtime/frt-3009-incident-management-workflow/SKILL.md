...
description: "Run FRT-3009 Incident Management Workflow for FRT G30 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-3009"
  source_name: "frt-3009-incident-management-workflow"
  source_sha256: "sha256:0a22248c4af07d47a4212f19d6ca4f7b50389e44dc29a395c27827763a0a1a22"
  batch: "G30"
...
- Runtime catalog key: `FRT-3009` / `frt-3009-incident-management-workflow`.
...
在G30范围内实现 **Incident Management Workflow**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：九类Certificate聚合与唯一Production Authority; Service Catalog、Owner、Tier、Dependency; SLI/SLO、Business SLI、Error Budget和Burn Rate。

## When to Use

- 实现或修改与 **Incident Management Workflow** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-3009Result`：版本化、带状态和诊断的结构化结果。
- `FRT-3009EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-3009FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-3009CertificateFragment`：供G30批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g30/frt-3009-incident-management-workflow/
packages/runtime/g30/frt-3009-incident-management-workflow/
services/control-plane/g30/frt-3009-incident-management-workflow/
apps/web-console/src/features/g30/frt-3009-incident-management-workflow/
apps/admin-console/src/features/g30/frt-3009-incident-management-workflow/
tests/g30/frt-3009-incident-management-workflow/
```
...
```text
POST /v1/skills/frt-3009/runs
GET  /v1/skills/frt-3009/runs/{run_id}
POST /v1/skills/frt-3009/runs/{run_id}/verify
GET  /v1/skills/frt-3009/runs/{run_id}/findings
GET  /v1/skills/frt-3009/runs/{run_id}/evidence
```
