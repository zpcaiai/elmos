...
description: "Run FRT-3005 Observability Coverage Validator for FRT G30 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-3005"
  source_name: "frt-3005-observability-coverage-validator"
  source_sha256: "sha256:600d4d5c06eac23fd2bd98dcc6a1df2d2ad541f08d0ad56312931d8622993649"
  batch: "G30"
...
- Runtime catalog key: `FRT-3005` / `frt-3005-observability-coverage-validator`.
...
在G30范围内实现 **Observability Coverage Validator**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：九类Certificate聚合与唯一Production Authority; Service Catalog、Owner、Tier、Dependency; SLI/SLO、Business SLI、Error Budget和Burn Rate。

## When to Use

- 实现或修改与 **Observability Coverage Validator** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-3005Result`：版本化、带状态和诊断的结构化结果。
- `FRT-3005EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-3005FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-3005CertificateFragment`：供G30批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g30/frt-3005-observability-coverage-validator/
packages/runtime/g30/frt-3005-observability-coverage-validator/
services/control-plane/g30/frt-3005-observability-coverage-validator/
apps/web-console/src/features/g30/frt-3005-observability-coverage-validator/
apps/admin-console/src/features/g30/frt-3005-observability-coverage-validator/
tests/g30/frt-3005-observability-coverage-validator/
```
...
```text
POST /v1/skills/frt-3005/runs
GET  /v1/skills/frt-3005/runs/{run_id}
POST /v1/skills/frt-3005/runs/{run_id}/verify
GET  /v1/skills/frt-3005/runs/{run_id}/findings
GET  /v1/skills/frt-3005/runs/{run_id}/evidence
```
