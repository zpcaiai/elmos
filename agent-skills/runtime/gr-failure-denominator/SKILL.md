---
name: gr-failure-denominator
description: 所有已接受正式迁移的项目留在分母，区分 Assessment no-go、执行失败、取消与 partial。 Use for this exact ELMOS Spring Golden Route v2 contract; imported specifications are not runtime or certification evidence.
...
metadata:
  ...
  source_id: ELMOS-GR-078
  source_name: gr-failure-denominator
  source_path: skills/gr-failure-denominator/SKILL.md
  source_sha256: sha256:dd405a8cc2a13eb272aceb54955a03d238370330e596b783c59381f7a7003c82
  source_contract_sha256: sha256:236cac7cbd7db8348f43ea09100bd56e58ec126b9165b27136082fadb54e6005
...
所有已接受正式迁移的项目留在分母，区分 Assessment no-go、执行失败、取消与 partial。

## Scope and capabilities

- Immutable acceptance event
- Terminal taxonomy
- No retroactive exclusion
- Success/failure/no-go rates
...
- [ ] 定义 `gr-failure-denominator` 的 public interface、输入/输出/Error Schema 和版本策略。
- [ ] 实现 `gr-failure-denominator` 的 production adapter/service，不把业务状态只保存在 Prompt 或内存对象中。
...
- [ ] 至少生成：denominator-report.json, failure-taxonomy.json。
...
- [ ] 本 Skill 的 required outputs 均由真实目标仓库执行产生：denominator-report.json, failure-taxonomy.json。
...
- Machine contract: `references/contract.json` (`sha256:236cac7cbd7db8348f43ea09100bd56e58ec126b9165b27136082fadb54e6005`).
