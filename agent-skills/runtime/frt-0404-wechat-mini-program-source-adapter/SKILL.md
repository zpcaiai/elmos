...
description: "Run FRT-0404 WeChat Mini Program Source Adapter for FRT G04 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-0404"
  source_name: "frt-0404-wechat-mini-program-source-adapter"
  source_sha256: "sha256:30fcd8a810e11488e698cd4866dc5fd6f4526a6d3885b06b8a2a82a462c8c4e5"
  batch: "G04"
...
- Runtime catalog key: `FRT-0404` / `frt-0404-wechat-mini-program-source-adapter`.
...
# FRT-0404 — WeChat Mini Program Source Adapter

## Objective

在G04范围内实现 **WeChat Mini Program Source Adapter**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：每个Adapter只解析源，不生成目标代码; 恢复组件、状态、事件、生命周期、路由、样式和平台能力; 统一动态语法、宏、装饰器和模板表达式。

## When to Use

- 实现或修改与 **WeChat Mini Program Source Adapter** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-0404Result`：版本化、带状态和诊断的结构化结果。
- `FRT-0404EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-0404FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-0404CertificateFragment`：供G04批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g04/frt-0404-wechat-mini-program-source-adapter/
packages/runtime/g04/frt-0404-wechat-mini-program-source-adapter/
services/control-plane/g04/frt-0404-wechat-mini-program-source-adapter/
apps/web-console/src/features/g04/frt-0404-wechat-mini-program-source-adapter/
apps/admin-console/src/features/g04/frt-0404-wechat-mini-program-source-adapter/
tests/g04/frt-0404-wechat-mini-program-source-adapter/
```
...
```text
POST /v1/skills/frt-0404/runs
GET  /v1/skills/frt-0404/runs/{run_id}
POST /v1/skills/frt-0404/runs/{run_id}/verify
GET  /v1/skills/frt-0404/runs/{run_id}/findings
GET  /v1/skills/frt-0404/runs/{run_id}/evidence
```
