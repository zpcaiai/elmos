...
description: "Run FRT-1607 Flutter Capability Gap Planner for FRT G16 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-1607"
  source_name: "frt-1607-flutter-capability-gap-planner"
  source_sha256: "sha256:1927ccba2884e3b2f14e533d79909a9d8e23d92d5c0e07e2e4696df35709dd16"
  batch: "G16"
...
- Runtime catalog key: `FRT-1607` / `frt-1607-flutter-capability-gap-planner`.
...
# FRT-1607 — Flutter Capability Gap Planner

## Objective

在G16范围内实现 **Flutter Capability Gap Planner**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：映射DOM/组件到Widget树而非逐元素复制; 处理Key、BuildContext、Controller、State和Navigator; 处理Plugin、Platform Channel、Web/Mobile能力差异。

## When to Use

- 实现或修改与 **Flutter Capability Gap Planner** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-1607Result`：版本化、带状态和诊断的结构化结果。
- `FRT-1607EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-1607FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-1607CertificateFragment`：供G16批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g16/frt-1607-flutter-capability-gap-planner/
packages/runtime/g16/frt-1607-flutter-capability-gap-planner/
services/control-plane/g16/frt-1607-flutter-capability-gap-planner/
apps/web-console/src/features/g16/frt-1607-flutter-capability-gap-planner/
apps/admin-console/src/features/g16/frt-1607-flutter-capability-gap-planner/
tests/g16/frt-1607-flutter-capability-gap-planner/
```
...
```text
POST /v1/skills/frt-1607/runs
GET  /v1/skills/frt-1607/runs/{run_id}
POST /v1/skills/frt-1607/runs/{run_id}/verify
GET  /v1/skills/frt-1607/runs/{run_id}/findings
GET  /v1/skills/frt-1607/runs/{run_id}/evidence
```
