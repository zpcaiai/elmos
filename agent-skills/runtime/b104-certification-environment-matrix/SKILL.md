---
name: b104-certification-environment-matrix
description: "Use when ELMOS must Define and provision supported OS, architecture, runtime, database, container, cluster, CI and cloud matrices. Triggers: Certification Environment Matrix, Real World Product Certification Pack, environment matrix, provisioning, support claims, fingerprinting."
metadata:
  id: B104-S03
...
# B104-S03 — Certification Environment Matrix

## Objective

Define and provision supported OS, architecture, runtime, database, container, cluster, CI and cloud matrices.
...
Use this Skill when the approved task explicitly requires **Certification Environment Matrix** or when the capability graph selects it as part of a minimal dependency closure.
...
- every support claim mapped.
- versions exact.
- unavailable cells blocked.
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b104-certification-environment-matrix/SKILL.md --out /tmp/certification_environment_matrix.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B104-S03
```
...
- Reject or expose paper support.
- Reject or expose latest wildcard.
- Reject or expose emulator presented as hardware.
