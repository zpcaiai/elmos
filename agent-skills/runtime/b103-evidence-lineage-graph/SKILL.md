---
name: b103-evidence-lineage-graph
description: "Use when ELMOS must Link requirements, Skills, contracts, runs, steps, tool calls, patches, tests, artifacts, approvals and gates. Triggers: Evidence Lineage Graph, Evidence and Certification Fabric Pack, lineage graph, causal edges, source maps, traceability."
metadata:
  id: B103-S03
...
# B103-S03 — Evidence Lineage Graph

## Objective

Link requirements, Skills, contracts, runs, steps, tool calls, patches, tests, artifacts, approvals and gates.
...
Use this Skill when the approved task explicitly requires **Evidence Lineage Graph** or when the capability graph selects it as part of a minimal dependency closure.
...
Package dependencies: `B103-S02`, `B97-S10`, `B99-S15`
...
- no dangling critical edges.
- causal direction explicit.
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b103-evidence-lineage-graph/SKILL.md --out /tmp/evidence_lineage_graph.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B103-S03
```
...
- Reject or expose fabricated lineage edge.
- Reject or expose cycle hides source.
- Reject or expose orphan approval.
