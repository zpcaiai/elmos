...
description: "Run FRT-2217 Energy Dispatch-to-Reconciliation Closure Pack for FRT G22 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-2217"
  source_name: "frt-2217-energy-dispatch-to-reconciliation-closure-pack"
  source_sha256: "sha256:64b389c91bb12717e41bac816b2586e6ff79d5d785643860209d84b6a7a0f6bb"
  batch: "G22"
...
- Runtime catalog key: `FRT-2217` / `frt-2217-energy-dispatch-to-reconciliation-closure-pack`.
...
在G22范围内实现 **Energy Dispatch-to-Reconciliation Closure Pack**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：Value Stream 与业务Outcome建模; 状态机可达性、终态、Timeout、Cancel、Unknown、Recovery完整性; 跨域Saga、幂等、补偿和Reconciliation。

## When to Use

- 实现或修改与 **Energy Dispatch-to-Reconciliation Closure Pack** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-2217Result`：版本化、带状态和诊断的结构化结果。
- `FRT-2217EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-2217FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-2217CertificateFragment`：供G22批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g22/frt-2217-energy-dispatch-to-reconciliation-closure-pack/
packages/runtime/g22/frt-2217-energy-dispatch-to-reconciliation-closure-pack/
services/control-plane/g22/frt-2217-energy-dispatch-to-reconciliation-closure-pack/
apps/web-console/src/features/g22/frt-2217-energy-dispatch-to-reconciliation-closure-pack/
apps/admin-console/src/features/g22/frt-2217-energy-dispatch-to-reconciliation-closure-pack/
tests/g22/frt-2217-energy-dispatch-to-reconciliation-closure-pack/
```
...
```text
POST /v1/skills/frt-2217/runs
GET  /v1/skills/frt-2217/runs/{run_id}
POST /v1/skills/frt-2217/runs/{run_id}/verify
GET  /v1/skills/frt-2217/runs/{run_id}/findings
GET  /v1/skills/frt-2217/runs/{run_id}/evidence
```
