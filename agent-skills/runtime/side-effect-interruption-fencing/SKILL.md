---
name: side-effect-interruption-fencing
description: 任何有副作用 tool 在 crash/interrupt 后必须进入不确定状态隔离。 Use for this exact ELMOS Spring Golden Route v2 contract; imported specifications are not runtime or certification evidence.
...
metadata:
  ...
  source_id: FOUNDATION-08-side-effect-interruption-fencing
  source_name: side-effect-interruption-fencing
  source_path: skills/side-effect-interruption-fencing/SKILL.md
  source_sha256: sha256:a00a2f068a072f02bc9cfe8770001379929c2b8a200ee2bb41a083ebcc5fb033
  source_contract_sha256: sha256:6fe483c9656b9fb649929545ca2ca4bc30896cc8e7b3cce162331fa4a3948221
...
# Side-effect Interruption Fencing

## Goal

任何有副作用 tool 在 crash/interrupt 后必须进入不确定状态隔离。

## Use when

在 Elmos 需要实现或升级 **Side-effect Interruption Fencing** 相关能力时使用；它不是仅供解释的提示词，而是实现、测试与验收契约。

## Capabilities to implement

- 执行前持久化 idempotency key 与 side-effect class
- 进程丢失时默认 interrupted/unknown，而非自动 replay
- 支持外部系统 reconciliation 检查是否已成功
- 只有证明安全才允许 retry
...
- [ ] 定义 `side-effect-interruption-fencing` 的 public interface、input/output/error schemas。
- [ ] 建立 `side-effect-interruption-fencing` 的 capability/feature flags 与版本字段。
...
- [ ] 至少覆盖并验收：执行前持久化 idempotency key 与 side-effect class；以及 进程丢失时默认 interrupted/unknown，而非自动 replay。
...
- Machine contract: `references/contract.json` (`sha256:6fe483c9656b9fb649929545ca2ca4bc30896cc8e7b3cce162331fa4a3948221`).
