...
description: "Run FRT-0600 Target Architecture Orchestrator for FRT G06 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-0600"
  source_name: "frt-0600-target-architecture-orchestrator"
  source_sha256: "sha256:4d4a44d99ebe9678d45b8d31a6ae571dac6356f7d3e7294e5b47d40e54765f22"
  batch: "G06"
...
- Runtime catalog key: `FRT-0600` / `frt-0600-target-architecture-orchestrator`.
...
# FRT-0600 — Target Architecture Orchestrator

## Objective

在G06范围内实现 **Target Architecture Orchestrator**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：按目标生态生成模块、路由、状态、DI、配置和Build边界; 禁止机械复制源目录结构; 锁定目标版本、依赖和工具链。

## When to Use

- 实现或修改与 **Target Architecture Orchestrator** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-0600Result`：版本化、带状态和诊断的结构化结果。
- `FRT-0600EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-0600FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-0600CertificateFragment`：供G06批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g06/frt-0600-target-architecture-orchestrator/
packages/runtime/g06/frt-0600-target-architecture-orchestrator/
services/control-plane/g06/frt-0600-target-architecture-orchestrator/
apps/web-console/src/features/g06/frt-0600-target-architecture-orchestrator/
apps/admin-console/src/features/g06/frt-0600-target-architecture-orchestrator/
tests/g06/frt-0600-target-architecture-orchestrator/
```
...
```text
POST /v1/skills/frt-0600/runs
GET  /v1/skills/frt-0600/runs/{run_id}
POST /v1/skills/frt-0600/runs/{run_id}/verify
GET  /v1/skills/frt-0600/runs/{run_id}/findings
GET  /v1/skills/frt-0600/runs/{run_id}/evidence
```
