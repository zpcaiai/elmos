---
name: elmos-model-serving-runtime-profile-compiler
description: Compile exact model-server, accelerator, protocol, batching, cache, scheduler, autoscaling and observability capabilities for self-hosted and managed runtimes.
version: 4.0.0
...
# Model Serving Runtime Profile Compiler

## Objective

Compile exact model-server, accelerator, protocol, batching, cache, scheduler, autoscaling and observability capabilities for self-hosted and managed runtimes.
...
- `ModelServingRuntimeProfileCompilerProfile`
- `ModelServingRuntimeProfileCompilerPlan`
- `ModelServingRuntimeProfileCompilerRun`
- `ModelServingRuntimeProfileCompilerDecision`

Required semantic capabilities:

- fingerprint server, backend and hardware versions
- model inference protocol and endpoint semantics
- capture batching, streaming and scheduler behavior
- represent cache, adapter and model repository lifecycle
- emit capacity, security and conformance obligations
...
- `artifacts/elmos-model-serving-runtime-profile-compiler/profile.yaml`
- `artifacts/elmos-model-serving-runtime-profile-compiler/plan.json`
- `artifacts/elmos-model-serving-runtime-profile-compiler/result.json`
- `artifacts/elmos-model-serving-runtime-profile-compiler/evidence/`
...
- **runtime alias hides version** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **protocol extension assumed portable** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **batching changes latency semantics** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **model repository mutable** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **hardware capability overclaimed** — must have a preventive control, a detective signal, a negative fixture and a response action.
...
- native scenario: fingerprint server, backend and hardware versions
- native scenario: model inference protocol and endpoint semantics
- native scenario: capture batching, streaming and scheduler behavior
- native scenario: represent cache, adapter and model repository lifecycle
- native scenario: emit capacity, security and conformance obligations
