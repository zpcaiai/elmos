---
name: usage-metering-and-slo
description: Use this skill when Elmos must perform usage metering and slo as part of the 10-serving-routing-inference production
  workflow, with typed contracts, policy enforcement, evidence capture, and rollback.
license: Proprietary-Elmos-Commercial
compatibility: Elmos v3 harness; K8 Serving and Runtime Control; policy and evidence services required.
metadata:
  version: 3.0.0
  pack: 10-serving-routing-inference
  priority: P0
  exposure: atomic-registry-only
  business-line: model-serving
allowed-tools: gateway.route model.invoke adapter.resolve quota.check deployment.rollout
---
# usage-metering-and-slo

## 能力目标

记录 Token、缓存、GPU、工具、延迟、错误和可用性并计算 SLA。
...
- 必须门：authorized-route, version-pinned, quota-respected, telemetry-emitted, fallback-defined；
...
- 业务线：`model-serving`；包：`10-serving-routing-inference`。
