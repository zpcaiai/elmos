...
description: "Run FRT-1713 Thirty-Path Certification for FRT G17 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-1713"
  source_name: "frt-1713-thirty-path-certification"
  source_sha256: "sha256:0e117d6a549c737fa611246046658dcc1f93e2d349cfb39c2f4742a02721c197"
  batch: "G17"
...
- Runtime catalog key: `FRT-1713` / `frt-1713-thirty-path-certification`.
...
# FRT-1713 — Thirty-Path Certification

## Objective

在G17范围内实现 **Thirty-Path Certification**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：补齐小程序/ArkUI/Flutter六个方向; 统一移动Navigation、Lifecycle、Permission、Storage和Native能力; 验证直接与间接三角路径。

## When to Use

- 实现或修改与 **Thirty-Path Certification** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-1713Result`：版本化、带状态和诊断的结构化结果。
- `FRT-1713EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-1713FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-1713CertificateFragment`：供G17批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g17/frt-1713-thirty-path-certification/
packages/runtime/g17/frt-1713-thirty-path-certification/
services/control-plane/g17/frt-1713-thirty-path-certification/
apps/web-console/src/features/g17/frt-1713-thirty-path-certification/
apps/admin-console/src/features/g17/frt-1713-thirty-path-certification/
tests/g17/frt-1713-thirty-path-certification/
```
...
```text
POST /v1/skills/frt-1713/runs
GET  /v1/skills/frt-1713/runs/{run_id}
POST /v1/skills/frt-1713/runs/{run_id}/verify
GET  /v1/skills/frt-1713/runs/{run_id}/findings
GET  /v1/skills/frt-1713/runs/{run_id}/evidence
```
