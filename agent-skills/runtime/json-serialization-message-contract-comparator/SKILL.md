---
name: json-serialization-message-contract-comparator
description: Compare JSON, serialization and message contracts using golden samples and bidirectional reads. Use for DTOs, events and external payloads.
---

# Json Serialization Message Contract Comparator
...
Require schemas/samples, required fields, formats, enum values, discriminators, date/time rules and Avro/Protobuf descriptors when applicable.

## Workflow

1. Validate schema version, stable identities, immutable hashes and policy prerequisites.
2. Normalize nondeterministic values; serialize and deserialize both directions; compare formats, requiredness, enums, polymorphism and message compatibility.
...
serialization-message-differences.json and sample evidence.

## Fail-closed rules

New required fields, removed enum values, changed dates/discriminators or unreadable old messages block.
