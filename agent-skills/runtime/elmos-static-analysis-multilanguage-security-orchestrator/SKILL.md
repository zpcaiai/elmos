---
name: elmos-static-analysis-multilanguage-security-orchestrator
description: Route compiler diagnostics, SAST, dependency, secret, IaC and custom semantic analyses across generated polyglot repositories with normalized evidence.
version: 4.0.0
...
# Multi-Language Static Analysis and Security Orchestrator
...
- `MultiLanguageStaticAnalysisAndSecurityOrchestratorProfile`
- `MultiLanguageStaticAnalysisAndSecurityOrchestratorPlan`
- `MultiLanguageStaticAnalysisAndSecurityOrchestratorRun`
- `MultiLanguageStaticAnalysisAndSecurityOrchestratorDecision`

Required semantic capabilities:

- select analyzers by language/framework/risk
- pin rule packs and tool digests
- normalize findings without losing provenance
- deduplicate and correlate cross-language flows
- gate suppressions and false-positive waivers
...
- `artifacts/elmos-static-analysis-multilanguage-security-orchestrator/profile.yaml`
- `artifacts/elmos-static-analysis-multilanguage-security-orchestrator/plan.json`
- `artifacts/elmos-static-analysis-multilanguage-security-orchestrator/result.json`
- `artifacts/elmos-static-analysis-multilanguage-security-orchestrator/evidence/`
...
- **analyzer coverage falsely assumed** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **suppression without expiry** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **normalization merges distinct vulnerabilities** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **untrusted analyzer executes code** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **generated code excluded by default** — must have a preventive control, a detective signal, a negative fixture and a response action.
...
- native scenario: select analyzers by language/framework/risk
- native scenario: pin rule packs and tool digests
- native scenario: normalize findings without losing provenance
- native scenario: deduplicate and correlate cross-language flows
- native scenario: gate suppressions and false-positive waivers
