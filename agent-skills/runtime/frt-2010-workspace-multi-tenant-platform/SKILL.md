...
description: "Run FRT-2010 Workspace Multi-Tenant Platform for FRT G20 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-2010"
  source_name: "frt-2010-workspace-multi-tenant-platform"
  source_sha256: "sha256:8d5f495969c02559b6cd2ef978053bfd85a932dbc34a99d7235fe1ecef9d6644"
  batch: "G20"
...
- Runtime catalog key: `FRT-2010` / `frt-2010-workspace-multi-tenant-platform`.
...
# FRT-2010 — Workspace Multi-Tenant Platform

## Objective

在G20范围内实现 **Workspace Multi-Tenant Platform**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：提供Skill SDK、Runtime、Registry和Marketplace; 实现CLI、API、IDE、Web Console和Multi-Tenant Workspace; Durable Workflow与分布式Worker支持暂停、恢复、取消和幂等。

## When to Use

- 实现或修改与 **Workspace Multi-Tenant Platform** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-2010Result`：版本化、带状态和诊断的结构化结果。
- `FRT-2010EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-2010FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-2010CertificateFragment`：供G20批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g20/frt-2010-workspace-multi-tenant-platform/
packages/runtime/g20/frt-2010-workspace-multi-tenant-platform/
services/control-plane/g20/frt-2010-workspace-multi-tenant-platform/
apps/web-console/src/features/g20/frt-2010-workspace-multi-tenant-platform/
apps/admin-console/src/features/g20/frt-2010-workspace-multi-tenant-platform/
tests/g20/frt-2010-workspace-multi-tenant-platform/
```
...
```text
POST /v1/skills/frt-2010/runs
GET  /v1/skills/frt-2010/runs/{run_id}
POST /v1/skills/frt-2010/runs/{run_id}/verify
GET  /v1/skills/frt-2010/runs/{run_id}/findings
GET  /v1/skills/frt-2010/runs/{run_id}/evidence
```
