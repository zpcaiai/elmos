---
name: elmos-language-runtime-performance-portability-profiler
description: Characterize latency, throughput, startup, memory, GC, scheduling and native-image behavior so route selection reflects runtime economics rather than syntax alone.
version: 4.0.0
...
# Language Runtime Performance Portability Profiler
...
- `LanguageRuntimePerformancePortabilityProfilerProfile`
- `LanguageRuntimePerformancePortabilityProfilerPlan`
- `LanguageRuntimePerformancePortabilityProfilerRun`
- `LanguageRuntimePerformancePortabilityProfilerDecision`

Required semantic capabilities:

- benchmark representative workload envelopes
- separate cold start from steady state
- profile GC, allocator and scheduler effects
- compare native/AOT/JIT deployment modes
- produce capacity and cost portability model
...
- `elmos-polyglot-route-certifier` from `elmos-functional-assurance-certification-skills` — optional companion; required only for E4/E5/P05 certification.

## Primary Outputs

- `artifacts/elmos-language-runtime-performance-portability-profiler/profile.yaml`
- `artifacts/elmos-language-runtime-performance-portability-profiler/plan.json`
- `artifacts/elmos-language-runtime-performance-portability-profiler/result.json`
- `artifacts/elmos-language-runtime-performance-portability-profiler/evidence/`
...
- **microbenchmark substituted for workload** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **JIT warmup ignored** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **hardware profile not pinned** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **performance variance cherry-picked** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **cost advantage hides correctness loss** — must have a preventive control, a detective signal, a negative fixture and a response action.
...
- native scenario: benchmark representative workload envelopes
- native scenario: separate cold start from steady state
- native scenario: profile GC, allocator and scheduler effects
- native scenario: compare native/AOT/JIT deployment modes
- native scenario: produce capacity and cost portability model
