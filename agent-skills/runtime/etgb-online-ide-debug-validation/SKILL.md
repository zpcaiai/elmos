---
name: etgb-online-ide-debug-validation
description: Validate all Online IDE, Build, Debug and Record-Replay capabilities through the production adapter, independent Oracles and fail-closed release evidence. Repository-owned ETGB execution is available through the local runtime; external production evidence remains explicit.
metadata:
  ...
  source_skill: online-ide-debug-validation
...
---
name: online-ide-debug-validation
description: Validate all declared Online IDE, Build, Debug and Record-Replay capabilities with executable positive, boundary, adversarial, concurrency, recovery and evidence-backed release tests.
---

# Online IDE, Build, Debug and Record-Replay Validation

## Purpose

Use this Skill to plan, execute and certify the `online-ide-debug` product domain. It owns **65 declared capabilities** (37 P0, 23 P1, 5 P2) across 4 execution contexts and four mandatory variants.

## Inputs

- Frozen release candidate and feature-registry revision.
- Tenant, project, environment authority, budget and fencing context.
- Concrete cases from `suites/full-product.jsonl` whose business line is `online-ide-debug`.
...
2. Provision an isolated context for: browser-ide, sandbox-executor, remote-debug-adapter, distributed-system.
...
5. Apply the domain Oracle `ide-debug-state-causality-sandbox-and-user-content-integrity` plus security, state, audit and unsupported-disclosure Oracles.
...
Representative capabilities include: File Tree Open Create Rename Delete, Editor Read Write And Autosave, Encoding Line Ending Preservation, Search And Replace, Symbol Navigation, Source Diff And Merge, User Owned Region Protection, Terminal Command Sandbox, Build Command, Unit Test Run, Integration Test Run, Debug Adapter Capability Negotiation. The complete source of truth is `matrices/full-product.yaml`; no prose list may override it.
...
Route execution through `external-online-ide-debug-harness` under `integrations/harness/full-product-adapters.yaml`. An adapter stub or skipped case is implementation progress, not product certification.
