...
description: "Run FRT-2107 Critical Admin Journey Registry for FRT G21 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-2107"
  source_name: "frt-2107-critical-admin-journey-registry"
  source_sha256: "sha256:89ef4849af67b64daa7d8c80ad6aa95f440038fe1365b67a3bee202a9aafaad2"
  batch: "G21"
...
- Runtime catalog key: `FRT-2107` / `frt-2107-critical-admin-journey-registry`.
...
在G21范围内实现 **Critical Admin Journey Registry**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：统一 Product Closure Graph; Requirement、Capability、Journey、Command、API、Data、Permission、Test、Monitoring、Runbook、Certificate 双向追踪; 孤儿 UI/API/Command/Event/Data Write 检测。

## When to Use

- 实现或修改与 **Critical Admin Journey Registry** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-2107Result`：版本化、带状态和诊断的结构化结果。
- `FRT-2107EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-2107FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-2107CertificateFragment`：供G21批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g21/frt-2107-critical-admin-journey-registry/
packages/runtime/g21/frt-2107-critical-admin-journey-registry/
services/control-plane/g21/frt-2107-critical-admin-journey-registry/
apps/web-console/src/features/g21/frt-2107-critical-admin-journey-registry/
apps/admin-console/src/features/g21/frt-2107-critical-admin-journey-registry/
tests/g21/frt-2107-critical-admin-journey-registry/
```
...
```text
POST /v1/skills/frt-2107/runs
GET  /v1/skills/frt-2107/runs/{run_id}
POST /v1/skills/frt-2107/runs/{run_id}/verify
GET  /v1/skills/frt-2107/runs/{run_id}/findings
GET  /v1/skills/frt-2107/runs/{run_id}/evidence
```
