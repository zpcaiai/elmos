...
description: "Run FRT-0711 Restricted Agent Repair Envelope for FRT G07 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-0711"
  source_name: "frt-0711-restricted-agent-repair-envelope"
  source_sha256: "sha256:3db5919c769eda0702be45ff80884a984f9596408811942ab6ea49c32ce9ec0f"
  batch: "G07"
...
- Runtime catalog key: `FRT-0711` / `frt-0711-restricted-agent-repair-envelope`.
...
# FRT-0711 — Restricted Agent Repair Envelope

## Objective

在G07范围内实现 **Restricted Agent Repair Envelope**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：Typed IR经规则Pass降低到目标原生AST; Gap生成Typed Hole而非空实现; Format/Analyze/Build驱动有界确定性修复。

## When to Use

- 实现或修改与 **Restricted Agent Repair Envelope** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-0711Result`：版本化、带状态和诊断的结构化结果。
- `FRT-0711EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-0711FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-0711CertificateFragment`：供G07批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g07/frt-0711-restricted-agent-repair-envelope/
packages/runtime/g07/frt-0711-restricted-agent-repair-envelope/
services/control-plane/g07/frt-0711-restricted-agent-repair-envelope/
apps/web-console/src/features/g07/frt-0711-restricted-agent-repair-envelope/
apps/admin-console/src/features/g07/frt-0711-restricted-agent-repair-envelope/
tests/g07/frt-0711-restricted-agent-repair-envelope/
```
...
```text
POST /v1/skills/frt-0711/runs
GET  /v1/skills/frt-0711/runs/{run_id}
POST /v1/skills/frt-0711/runs/{run_id}/verify
GET  /v1/skills/frt-0711/runs/{run_id}/findings
GET  /v1/skills/frt-0711/runs/{run_id}/evidence
```
