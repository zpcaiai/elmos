---
name: b101-python-route-verification-pr
description: "Use when ELMOS must Run environment, test, web/data/ML journey, package, PR and rollback validation for the Python route. Triggers: Python Route Verification and PR, Three Golden Route Packs, Python verification, wheel matrix, PR, rollback."
metadata:
  id: B101-S15
...
# B101-S15 — Python Route Verification and PR

## Objective

Run environment, test, web/data/ML journey, package, PR and rollback validation for the Python route.
...
Use this Skill when the approved task explicitly requires **Python Route Verification and PR** or when the capability graph selects it as part of a minimal dependency closure.
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b101-python-route-verification-pr/SKILL.md --out /tmp/python_route_verification_pr.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B101-S15
```
...
- Reject or expose notebook output-only pass.
- Reject or expose xfail counted as pass.
- Reject or expose rollback cannot restore data.
