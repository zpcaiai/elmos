...
description: "Run FRT-0403 React Source Adapter for FRT G04 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-0403"
  source_name: "frt-0403-react-source-adapter"
  source_sha256: "sha256:7639e4e21aaecb1beea6695b51bce907de6a3dd758672a995bf9d91b8a484f93"
  batch: "G04"
...
- Runtime catalog key: `FRT-0403` / `frt-0403-react-source-adapter`.
...
# FRT-0403 — React Source Adapter

## Objective

在G04范围内实现 **React Source Adapter**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：每个Adapter只解析源，不生成目标代码; 恢复组件、状态、事件、生命周期、路由、样式和平台能力; 统一动态语法、宏、装饰器和模板表达式。

## When to Use

- 实现或修改与 **React Source Adapter** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-0403Result`：版本化、带状态和诊断的结构化结果。
- `FRT-0403EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-0403FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-0403CertificateFragment`：供G04批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g04/frt-0403-react-source-adapter/
packages/runtime/g04/frt-0403-react-source-adapter/
services/control-plane/g04/frt-0403-react-source-adapter/
apps/web-console/src/features/g04/frt-0403-react-source-adapter/
apps/admin-console/src/features/g04/frt-0403-react-source-adapter/
tests/g04/frt-0403-react-source-adapter/
```
...
```text
POST /v1/skills/frt-0403/runs
GET  /v1/skills/frt-0403/runs/{run_id}
POST /v1/skills/frt-0403/runs/{run_id}/verify
GET  /v1/skills/frt-0403/runs/{run_id}/findings
GET  /v1/skills/frt-0403/runs/{run_id}/evidence
```
