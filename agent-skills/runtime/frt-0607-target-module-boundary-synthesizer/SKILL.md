...
description: "Run FRT-0607 Target Module Boundary Synthesizer for FRT G06 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-0607"
  source_name: "frt-0607-target-module-boundary-synthesizer"
  source_sha256: "sha256:d0e327ad7cddcf475bcc3bd8e719fec85681f8cbfcf93cb84d3c4e40d4e18adb"
  batch: "G06"
...
- Runtime catalog key: `FRT-0607` / `frt-0607-target-module-boundary-synthesizer`.
...
# FRT-0607 — Target Module Boundary Synthesizer

## Objective

在G06范围内实现 **Target Module Boundary Synthesizer**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：按目标生态生成模块、路由、状态、DI、配置和Build边界; 禁止机械复制源目录结构; 锁定目标版本、依赖和工具链。

## When to Use

- 实现或修改与 **Target Module Boundary Synthesizer** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-0607Result`：版本化、带状态和诊断的结构化结果。
- `FRT-0607EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-0607FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-0607CertificateFragment`：供G06批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g06/frt-0607-target-module-boundary-synthesizer/
packages/runtime/g06/frt-0607-target-module-boundary-synthesizer/
services/control-plane/g06/frt-0607-target-module-boundary-synthesizer/
apps/web-console/src/features/g06/frt-0607-target-module-boundary-synthesizer/
apps/admin-console/src/features/g06/frt-0607-target-module-boundary-synthesizer/
tests/g06/frt-0607-target-module-boundary-synthesizer/
```
...
```text
POST /v1/skills/frt-0607/runs
GET  /v1/skills/frt-0607/runs/{run_id}
POST /v1/skills/frt-0607/runs/{run_id}/verify
GET  /v1/skills/frt-0607/runs/{run_id}/findings
GET  /v1/skills/frt-0607/runs/{run_id}/evidence
```
