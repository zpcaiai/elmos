...
description: "Run FRT-2617 Test Worker Failover for FRT G26 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-2617"
  source_name: "frt-2617-test-worker-failover"
  source_sha256: "sha256:6a65e8e34aa9152893e4621c0d7c4831f691ddb0a49ff426152ef3a374fd601d"
  batch: "G26"
...
- Runtime catalog key: `FRT-2617` / `frt-2617-test-worker-failover`.
...
在G26范围内实现 **Test Worker Failover**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：Requirement/Acceptance/State Machine派生Test Obligation; Unit/Component/Contract/Integration/Journey分层协调; Differential、Mutation、Visual/Semantic、Security和Performance Regression。

## When to Use

- 实现或修改与 **Test Worker Failover** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-2617Result`：版本化、带状态和诊断的结构化结果。
- `FRT-2617EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-2617FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-2617CertificateFragment`：供G26批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g26/frt-2617-test-worker-failover/
packages/runtime/g26/frt-2617-test-worker-failover/
services/control-plane/g26/frt-2617-test-worker-failover/
apps/web-console/src/features/g26/frt-2617-test-worker-failover/
apps/admin-console/src/features/g26/frt-2617-test-worker-failover/
tests/g26/frt-2617-test-worker-failover/
```
...
```text
POST /v1/skills/frt-2617/runs
GET  /v1/skills/frt-2617/runs/{run_id}
POST /v1/skills/frt-2617/runs/{run_id}/verify
GET  /v1/skills/frt-2617/runs/{run_id}/findings
GET  /v1/skills/frt-2617/runs/{run_id}/evidence
```
