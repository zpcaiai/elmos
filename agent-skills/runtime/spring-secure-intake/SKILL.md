---
name: spring-secure-intake
description: 安全接入 Git/压缩包/目录，建立不可变 snapshot 和客户授权范围。 Use for this exact ELMOS Spring Golden Route v2 contract; imported specifications are not runtime or certification evidence.
...
metadata:
  ...
  source_id: ELMOS-GR-025
  source_name: spring-secure-intake
  source_path: skills/spring-secure-intake/SKILL.md
  source_sha256: sha256:d7f6d6ce65f928d9e3d1a6d36f28f60b37c2da095b93e7c5b4b3ed9889a20a8d
  source_contract_sha256: sha256:acc8bc7ec37e87a41d8d7eb5282a8053e483f2fc7d54a80fd6204aa3a55e4b89
...
# Secure Repository Intake

## Objective

安全接入 Git/压缩包/目录，建立不可变 snapshot 和客户授权范围。

## Scope and capabilities

- 不执行仓库脚本
- archive bomb/Secret/恶意文件检查
- submodule/LFS policy
- 短期凭据 Broker
- 内容哈希恢复
...
- [ ] 定义 `spring-secure-intake` 的 public interface、输入/输出/Error Schema 和版本策略。
- [ ] 实现 `spring-secure-intake` 的 production adapter/service，不把业务状态只保存在 Prompt 或内存对象中。
...
- [ ] 至少生成：repository-snapshot.json, intake-audit.json。
...
- [ ] 历史失败与迁移回归边界冻结
- [ ] 报价、ETA、假设和风险进入 SOW
- [ ] 本 Skill 的 required outputs 均由真实目标仓库执行产生：repository-snapshot.json, intake-audit.json。
...
- Machine contract: `references/contract.json` (`sha256:acc8bc7ec37e87a41d8d7eb5282a8053e483f2fc7d54a80fd6204aa3a55e4b89`).
