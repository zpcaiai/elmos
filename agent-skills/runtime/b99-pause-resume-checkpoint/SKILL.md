---
name: b99-pause-resume-checkpoint
description: "Use when ELMOS must Persist resumable checkpoints and approval pauses without replaying completed side effects. Triggers: Pause Resume and Checkpoint, Durable Runtime Kernel Pack, checkpoints, pause, resume, approval wait."
metadata:
  id: B99-S08
...
# B99-S08 — Pause Resume and Checkpoint

## Objective

Persist resumable checkpoints and approval pauses without replaying completed side effects.
...
Use this Skill when the approved task explicitly requires **Pause Resume and Checkpoint** or when the capability graph selects it as part of a minimal dependency closure.
...
- resume from exact version.
- completed effects not replayed.
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b99-pause-resume-checkpoint/SKILL.md --out /tmp/pause_resume_checkpoint.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B99-S08
```
...
- Reject or expose tampered checkpoint.
- Reject or expose resume with incompatible workflow.
- Reject or expose approval bypass.
