---
name: spring-side-effect-ledger
description: 对 Git push、DB migration、deploy、publish 等先登记、授权、执行、settle 和对账。 Use for this exact ELMOS Spring Golden Route v2 contract; imported specifications are not runtime or certification evidence.
...
metadata:
  ...
  source_id: ELMOS-GR-053
  source_name: spring-side-effect-ledger
  source_path: skills/spring-side-effect-ledger/SKILL.md
  source_sha256: sha256:f7bc1fe4feeb68f5eca414c9f2e4ea9228c01825c3df8071a2451b15407fe193
  source_contract_sha256: sha256:7c3acffca4be4982bbb160b54fd2ab1c9192e9d04047e63199785bc76142e6d7
...
对 Git push、DB migration、deploy、publish 等先登记、授权、执行、settle 和对账。
...
- [ ] 定义 `spring-side-effect-ledger` 的 public interface、输入/输出/Error Schema 和版本策略。
- [ ] 实现 `spring-side-effect-ledger` 的 production adapter/service，不把业务状态只保存在 Prompt 或内存对象中。
...
- [ ] 至少生成：side-effect-ledger.json, settlement-events, reconciliation.json。
...
- [ ] 本 Skill 的 required outputs 均由真实目标仓库执行产生：side-effect-ledger.json, settlement-events, reconciliation.json。
- [ ] 所有 mandatory tests 和 security gates 通过，且无 critical unknown。

## Dependencies

- `spring-durable-session`
...
- Machine contract: `references/contract.json` (`sha256:7c3acffca4be4982bbb160b54fd2ab1c9192e9d04047e63199785bc76142e6d7`).
