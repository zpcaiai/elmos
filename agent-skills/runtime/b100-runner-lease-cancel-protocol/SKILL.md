---
name: b100-runner-lease-cancel-protocol
description: "Use when ELMOS must Coordinate task leases, heartbeat, cancellation, result fencing and stale runner behavior. Triggers: Runner Lease and Cancel Protocol, Hardened Runner Fabric Pack, lease protocol, cancel, result fencing, stale work."
metadata:
  id: B100-S12
...
# B100-S12 — Runner Lease and Cancel Protocol

## Objective

Coordinate task leases, heartbeat, cancellation, result fencing and stale runner behavior.
...
Use this Skill when the approved task explicitly requires **Runner Lease and Cancel Protocol** or when the capability graph selects it as part of a minimal dependency closure.
...
Package dependencies: `B100-S11`, `B99-S04`, `B99-S07`
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b100-runner-lease-cancel-protocol/SKILL.md --out /tmp/runner_lease_cancel_protocol.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B100-S12
```
...
- Reject or expose zombie task publish.
- Reject or expose heartbeat spoofing.
- Reject or expose cancel acknowledgement without stop.
