---
name: native-parser-adapter
description: 允许 ANTLR、编译器 API、Prism、Tree-sitter 等不同原生解析器映射到统一 IR。 Use for this exact ELMOS Spring Golden Route v2 contract; imported specifications are not runtime or certification evidence.
...
metadata:
  ...
  source_id: FOUNDATION-01-native-parser-adapter
  source_name: native-parser-adapter
  source_path: skills/native-parser-adapter/SKILL.md
  source_sha256: sha256:1034d56831dd69d4196fb3fae75f71a4fc7b89b23afac637522abc5f80651c21
  source_contract_sha256: sha256:2dafd212528d61061380d51216a43c7c15f48fcf7fddf35da31ca814a09a4d4c
...
允许 ANTLR、编译器 API、Prism、Tree-sitter 等不同原生解析器映射到统一 IR。

## Use when

在 Elmos 需要实现或升级 **Native Parser Adapter** 相关能力时使用；它不是仅供解释的提示词，而是实现、测试与验收契约。

## Capabilities to implement

- 定义 Parser→IR、IR→Printer、Type Attribution 三个独立适配面
- 优先利用语言官方/成熟 parser 的准确性
- 适配器可独立升级和做兼容性测试
- parser 失败保留原始源码和可诊断证据
...
- [ ] 定义 `native-parser-adapter` 的 public interface、input/output/error schemas。
- [ ] 建立 `native-parser-adapter` 的 capability/feature flags 与版本字段。
...
- [ ] 至少覆盖并验收：定义 Parser→IR、IR→Printer、Type Attribution 三个独立适配面；以及 优先利用语言官方/成熟 parser 的准确性。
...
- Machine contract: `references/contract.json` (`sha256:2dafd212528d61061380d51216a43c7c15f48fcf7fddf35da31ca814a09a4d4c`).
