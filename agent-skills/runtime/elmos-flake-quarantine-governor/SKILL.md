---
name: elmos-flake-quarantine-governor
description: Detect, classify, quarantine and remediate flaky tests without allowing retries or quarantine to make critical release gates falsely green.
version: 4.0.0
...
- Track first failure and every retry
- Classify environment, order, timing and test defects
- Require owner, expiry and remediation SLA
- Keep critical quarantined tests blocking
- Measure flake rate as release quality signal
...
- **retry laundering** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **permanent quarantine** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **failure log discarded** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **flaky production defect mislabeled test issue** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **quarantine scope hides critical path** — must have a preventive control, a detective signal, a negative fixture and a response action.
...
- intermittent timing test
- order-dependent test
- environment capacity flake
- retry-success remains flaky
- quarantine expiry
- critical flake blocks release
