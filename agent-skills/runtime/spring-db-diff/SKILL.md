---
name: spring-db-diff
description: 比较 schema、查询结果、写集合、事务、隔离、锁、retry、rollback 和错误。 Use for this exact ELMOS Spring Golden Route v2 contract; imported specifications are not runtime or certification evidence.
...
metadata:
  ...
  source_id: ELMOS-GR-059
  source_name: spring-db-diff
  source_path: skills/spring-db-diff/SKILL.md
  source_sha256: sha256:bb06510f43b451a1e44db317b83fa7b606649c3e08502d986efae776894c529f
  source_contract_sha256: sha256:71b7e825e9ac3df526623bc6995bd158467e8597e4c728289642875d0813dd3e
...
# Database & Transaction Differential

## Objective

比较 schema、查询结果、写集合、事务、隔离、锁、retry、rollback 和错误。

## Scope and capabilities

- Logical data compare
- Transaction trace
...
- [ ] 定义 `spring-db-diff` 的 public interface、输入/输出/Error Schema 和版本策略。
- [ ] 实现 `spring-db-diff` 的 production adapter/service，不把业务状态只保存在 Prompt 或内存对象中。
...
- [ ] 至少生成：db-diff-results.json, transaction-traces, schema-diff.json。
...
- [ ] 本 Skill 的 required outputs 均由真实目标仓库执行产生：db-diff-results.json, transaction-traces, schema-diff.json。
...
- Machine contract: `references/contract.json` (`sha256:71b7e825e9ac3df526623bc6995bd158467e8597e4c728289642875d0813dd3e`).
