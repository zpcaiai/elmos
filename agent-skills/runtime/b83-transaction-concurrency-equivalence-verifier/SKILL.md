---
name: b83-transaction-concurrency-equivalence-verifier
description: "Use when ELMOS must run transaction concurrency equivalence verifier for Batch 83 PL/SQL / T-SQL / PL/pgSQL / SQL PL Pack. Preserve native semantics, safety boundaries, ownership, traceability, and fail-closed evidence; require real vendor toolchain execution for runtime claims."
metadata:
  source_package: "elmos-language-packs-batch81-95-complete"
  source_id: "PG257"
  source_name: "transaction-concurrency-equivalence-verifier"
  source_sha256: "sha256:7ee2e3f01c883356742291ea30b738ec57b74992434f36e9a9ba27ffe8789d7a"
  batch: 83
  source_engine: "elmos.language-packs"
  source_status: "proposed"
  normalized_namespace: "language-pack"
---
# Batch 83 Language Pack / source PG257 — Transaction Concurrency Equivalence Verifier

## 1. Objective

Verify transaction concurrency equivalence verifier within the PL/SQL / T-SQL / PL/pgSQL / SQL PL Pack, preserving platform semantics, traceability, security, and reproducible evidence.

## 2. Scope

This Skill is part of **Batch 83: PL/SQL / T-SQL / PL/pgSQL / SQL PL Pack**.

### In scope

- Process PL/SQL packages.
- Process T-SQL modules.
- Process PL/pgSQL functions.
- Process SQL PL routines.
- Process triggers.
- Process jobs.
- Process dynamic SQL.
- Process database links.
- Produce database program IR.
- Produce decision records.
- Produce refactored SQL.
- Produce service extraction code.
- Produce transaction equivalence tests.
...
1. Establish independent expected results, representative data, tolerances, environments, and non-compensable critical assertions.
2. Process the relevant artifacts for Transaction Concurrency Equivalence Verifier using the approved database-programming semantic contracts and toolchain versions.
3. Preserve source provenance and create Artifact Graph links across inputs, outputs, tests, approvals, and evidence.
4. Detect and classify transaction semantic drift, trigger side effects, dynamic SQL injection; never convert uncertainty or unsupported behavior into a silent success.
...
7. Apply the safety boundary: Logic must remain in the database when atomicity, data locality, or performance evidence requires it; extraction cannot be automatic dogma.
...
- `TRANSACTION_SEMANTIC_DRIFT` — stop unsafe continuation and emit source-linked diagnostic evidence.
- `TRIGGER_SIDE_EFFECTS` — stop unsafe continuation and emit source-linked diagnostic evidence.
- `DYNAMIC_SQL_INJECTION` — stop unsafe continuation and emit source-linked diagnostic evidence.
- `CURSOR_BEHAVIOR_MISMATCH` — stop unsafe continuation and emit source-linked diagnostic evidence.
- `ISOLATION_LEVEL_DRIFT` — stop unsafe continuation and emit source-linked diagnostic evidence.
- `NUMERIC_DATE_CONVERSION` — stop unsafe continuation and emit source-linked diagnostic evidence.
...
- Apply this Batch safety boundary: **Logic must remain in the database when atomicity, data locality, or performance evidence requires it; extraction cannot be automatic dogma.**

## 14. Unit Tests

- A representative valid database-procedural input produces all declared outputs for transaction-concurrency-equivalence-verifier.
...
- At least one batch-specific risk (transaction semantic drift) is detected by a negative fixture.

## 15. Integration Tests

- Consume approved Project Synthesis or Modernization baselines and write complete lineage into the Artifact Graph.
- Integrate with at least one representative toolchain from: Oracle, SQL Server, PostgreSQL, DB2.
...
- Reject or block transaction semantic drift without weakening the expected result.
- Reject or block trigger side effects without weakening the expected result.
- Reject or block dynamic SQL injection without weakening the expected result.
- Reject or block cursor behavior mismatch without weakening the expected result.
- Reject or block isolation-level drift without weakening the expected result.
- Reject or block numeric/date conversion without weakening the expected result.
