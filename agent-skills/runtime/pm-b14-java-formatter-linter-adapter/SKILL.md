...
# Java Formatter Linter Adapter
## ELMOS runtime binding

- Invoke this repository Skill as `$pm-b14-java-formatter-linter-adapter`.
- Immutable source identity: `java-formatter-linter-adapter` in `precision-migration-b01-44` (B14).
- Runtime adapter: `directed-backend-route`; binding state: `DECLARED`.
- Resolve and plan with `python3 scripts/precision_migration/runtime.py plan --skill pm-b14-java-formatter-linter-adapter`.
...
接入 Java 格式化、Lint 和静态分析工具，并规范化诊断结果。

本 Skill 属于精密语言转换系统。它必须以**可执行证据**为准，不得用模型自评、源码相似度或“能编译”替代语义与行为验证。

## Use when

- 当前任务直接涉及 `java-formatter-linter-adapter` 所描述的能力。
...
1. 使用语言原生前端恢复语义
2. 执行本 Skill 的核心职责：接入 Java 格式化、Lint 和静态分析工具，并规范化诊断结果。
...
```yaml
skill: java-formatter-linter-adapter
version: 1.0.0
