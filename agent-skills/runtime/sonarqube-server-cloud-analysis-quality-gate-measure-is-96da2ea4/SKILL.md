---
name: sonarqube-server-cloud-analysis-quality-gate-measure-is-96da2ea4
description: 接入SonarQube Server/Cloud、HMAC Webhook、Background Task、Analysis、Gate、Measure、Issue与Profile Drift。
metadata:
  elmos_batch: "37B"
  elmos_skill_id: "EVI37B007"
  pack_version: "3.7-complete"
---

# EVI37B007 — SonarQube Analysis、Quality Gate、Measure、Issue 与 Webhook Connector

## Objective

接入SonarQube Server/Cloud、HMAC Webhook、Background Task、Analysis、Gate、Measure、Issue与Profile Drift。
...
- Webhook 使用原始 Body 验证 HMAC，并以 Task ID 拉取完整数据。
- 分析必须关联 Project、Branch/PR、Revision/Commit、Analysis UUID。
- 保留 Gate Conditions、Overall/New Code Measures 和 Profile/Rule/New Code Definition。
- Sonar Resolution 与 ELMOS Suppression/Risk Acceptance 分离。
...
- invalidWebhookHmacIsRejected
- analysisRequiresRevisionForStrongCorrelation
- newCodeMetricIsNotMixedWithOverall
- sonarGatePassDoesNotCreateElmosPass
