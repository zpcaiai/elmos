---
name: spring-openrewrite-adapter
description: 通过可替换 SPI 接入 parser、Recipe、DataTable、Diff、统计和错误。 Use for this exact ELMOS Spring Golden Route v2 contract; imported specifications are not runtime or certification evidence.
...
metadata:
  ...
  source_id: ELMOS-GR-042
  source_name: spring-openrewrite-adapter
  source_path: skills/spring-openrewrite-adapter/SKILL.md
  source_sha256: sha256:7ed3971dee2c45abd4f112f17dc511072c931c858ec353deb7803f3a41d8342a
  source_contract_sha256: sha256:2a160214e7610ebadcefbc8a7a7dcb4901e6c85150831857ea14d51ce8d95dda
...
通过可替换 SPI 接入 parser、Recipe、DataTable、Diff、统计和错误。

## Scope and capabilities

- 版本/capability negotiation
- 进程隔离/timeout
- 结果规范化 Semantic Patch
- DataTable ingestion
- Conformance corpus
...
- [ ] 定义 `spring-openrewrite-adapter` 的 public interface、输入/输出/Error Schema 和版本策略。
- [ ] 实现 `spring-openrewrite-adapter` 的 production adapter/service，不把业务状态只保存在 Prompt 或内存对象中。
...
- [ ] 至少生成：recipe-result.json, semantic-patch.json, data-tables。
...
- [ ] 全量 Recipe 最终收敛
- [ ] Agent 只接收剩余业务歧义
- [ ] 本 Skill 的 required outputs 均由真实目标仓库执行产生：recipe-result.json, semantic-patch.json, data-tables。
- [ ] 所有 mandatory tests 和 security gates 通过，且无 critical unknown。

## Dependencies

- `spring-migration-dag`
- `elmos-recipe-license-gate`
...
- Machine contract: `references/contract.json` (`sha256:2a160214e7610ebadcefbc8a7a7dcb4901e6c85150831857ea14d51ce8d95dda`).
