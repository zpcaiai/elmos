---
name: b104-upgrade-rollback-certification
description: "Use when ELMOS must Certify ELMOS Core, Skill, schema, workflow, runner and route upgrades with rollback and in-flight compatibility. Triggers: Upgrade and Rollback Certification, Real World Product Certification Pack, upgrade, rollback, schema migration, in-flight runs."
metadata:
  id: B104-S08
...
# B104-S08 — Upgrade and Rollback Certification

## Objective

Certify ELMOS Core, Skill, schema, workflow, runner and route upgrades with rollback and in-flight compatibility.
...
Use this Skill when the approved task explicitly requires **Upgrade and Rollback Certification** or when the capability graph selects it as part of a minimal dependency closure.
...
- zero irreversible loss.
- in-flight workflow preserved.
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b104-upgrade-rollback-certification/SKILL.md --out /tmp/upgrade_rollback_certification.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B104-S08
```
...
- Reject or expose forward-only migration.
- Reject or expose old runner stranded.
- Reject or expose downgrade corrupts evidence.
