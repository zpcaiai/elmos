...
description: "Run FRT-2505 Loading, Empty, Error and Retry State Validator for FRT G25 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-2505"
  source_name: "frt-2505-loading-empty-error-and-retry-state-validator"
  source_sha256: "sha256:86f1c0e1058bd1266880f3563e4802e68054b5f1d0d33523ccac8fddbe00c69c"
  batch: "G25"
  source_version: "1.0.0"
  source_risk: "critical-by-default"
  source_certificate_family: "UX"
...
- Runtime catalog key: `FRT-2505` / `frt-2505-loading-empty-error-and-retry-state-validator`.
...
在G25范围内实现 **Loading, Empty, Error and Retry State Validator**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：Actor/Goal驱动Task Contract与Completion Oracle; Loading/Empty/Error/Retry/Unsupported完整状态矩阵; Form Validation、Data Preservation、Single Flight。

## When to Use

- 实现或修改与 **Loading, Empty, Error and Retry State Validator** 对应的Contract、Runtime、API、UI、测试或证据链时。
- 相关代码、Schema、Pack、Policy、环境或依赖变化导致既有Evidence失效时。
- G25 Orchestrator生成该Skill的执行义务时。
...
- `SourceArtifactSet`：G21 User/Admin Journey, G24 Admin Console与Permission Matrix。
- `SkillPolicy`：权限、网络、Sandbox、Toolchain、Evidence和停止规则。

## Outputs

- `FRT-2505Result`：版本化、带状态和诊断的结构化结果。
- `FRT-2505EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-2505FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-2505CertificateFragment`：供G25批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g25/frt-2505-loading-empty-error-and-retry-state-validator/
packages/runtime/g25/frt-2505-loading-empty-error-and-retry-state-validator/
services/control-plane/g25/frt-2505-loading-empty-error-and-retry-state-validator/
apps/web-console/src/features/g25/frt-2505-loading-empty-error-and-retry-state-validator/
apps/admin-console/src/features/g25/frt-2505-loading-empty-error-and-retry-state-validator/
tests/g25/frt-2505-loading-empty-error-and-retry-state-validator/
```
...
```text
POST /v1/skills/frt-2505/runs
GET  /v1/skills/frt-2505/runs/{run_id}
POST /v1/skills/frt-2505/runs/{run_id}/verify
GET  /v1/skills/frt-2505/runs/{run_id}/findings
GET  /v1/skills/frt-2505/runs/{run_id}/evidence
```
...
- The parent G25 Orchestrator can consume the result and independently decide certification.
