---
name: b104-security-red-team-campaign
description: "Use when ELMOS must Attack repository ingestion, prompts, sandboxes, secrets, supply chain, tenancy, APIs, CI and evidence gates. Triggers: Security Red Team Campaign, Real World Product Certification Pack, red team, prompt injection, sandbox escape, tenant isolation."
metadata:
  id: B104-S07
...
# B104-S07 — Security Red Team Campaign

## Objective

Attack repository ingestion, prompts, sandboxes, secrets, supply chain, tenancy, APIs, CI and evidence gates.
...
Use this Skill when the approved task explicitly requires **Security Red Team Campaign** or when the capability graph selects it as part of a minimal dependency closure.
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b104-security-red-team-campaign/SKILL.md --out /tmp/security_red_team_campaign.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B104-S07
```
...
- Reject or expose known exploit excluded.
- Reject or expose severity downgraded without basis.
- Reject or expose test credentials are production.
