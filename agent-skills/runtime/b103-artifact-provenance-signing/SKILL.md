---
name: b103-artifact-provenance-signing
description: "Use when ELMOS must Generate signed provenance for patches, binaries, images, reports and route packs. Triggers: Artifact Provenance and Signing, Evidence and Certification Fabric Pack, artifact provenance, signatures, SBOM linkage, build identity."
metadata:
  id: B103-S05
...
# B103-S05 — Artifact Provenance and Signing

## Objective

Generate signed provenance for patches, binaries, images, reports and route packs.
...
Use this Skill when the approved task explicitly requires **Artifact Provenance and Signing** or when the capability graph selects it as part of a minimal dependency closure.
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b103-artifact-provenance-signing/SKILL.md --out /tmp/artifact_provenance_signing.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B103-S05
```
...
- Reject or expose artifact substitution.
- Reject or expose unsigned secondary artifact.
- Reject or expose self-signed trust.
