---
name: slsa-source-build-provenance-vsa-generation-and-verification
description: 实现 SLSA 1.2 Source/Build Provenance、Builder、参数、依赖、Level Assessment 与 VSA。
metadata:
  elmos_batch: "37A"
  elmos_skill_id: "ART37A007"
  pack_version: "3.7-complete"
---

# ART37A007 — SLSA Source／Build Provenance、VSA 生成与验证

## Objective

实现 SLSA 1.2 Source/Build Provenance、Builder、参数、依赖、Level Assessment 与 VSA。
...
- 分离 Source Provenance 与 Build Provenance。
- Builder ID 表示受信任构建平台边界，并保存 Runner/Sandbox/Workflow Version。
- 记录外部参数、Resolved Dependencies、Retry/Resume 与 producing attempt。
- VSA 绑定 Verifier、Policy Digest、Input Attestations 与 verified levels。
...
- provenanceSubjectMustMatchArtifactDigest
- signatureAloneDoesNotClaimBuildLevel3
- resumeAttemptIsRepresented
- vsaBindsExactPolicyDigest
