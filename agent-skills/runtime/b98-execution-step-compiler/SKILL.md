---
name: b98-execution-step-compiler
description: "Use when ELMOS must Compile workflows into typed, idempotent and resumable execution steps with explicit inputs, outputs and effects. Triggers: Execution Step Compiler, Executable Skill Contract and Registry Pack, step IR, idempotency, effect typing, resume points."
metadata:
  id: B98-S06
...
# B98-S06 — Execution Step Compiler

## Objective

Compile workflows into typed, idempotent and resumable execution steps with explicit inputs, outputs and effects.
...
Use this Skill when the approved task explicitly requires **Execution Step Compiler** or when the capability graph selects it as part of a minimal dependency closure.
...
- acyclic step graph or explicit loop policy.
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b98-execution-step-compiler/SKILL.md --out /tmp/execution_step_compiler.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B98-S06
```
...
- Reject or expose implicit side effect.
- Reject or expose unbounded loop.
- Reject or expose unstable step identity.
