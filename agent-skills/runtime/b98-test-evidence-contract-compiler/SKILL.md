---
name: b98-test-evidence-contract-compiler
description: "Use when ELMOS must Compile required tests and evidence into machine-enforced obligations bound to exact states and scopes. Triggers: Test and Evidence Contract Compiler, Executable Skill Contract and Registry Pack, test obligations, evidence obligations, freshness, state promotion."
metadata:
  id: B98-S08
...
# B98-S08 — Test and Evidence Contract Compiler

## Objective

Compile required tests and evidence into machine-enforced obligations bound to exact states and scopes.
...
Use this Skill when the approved task explicitly requires **Test and Evidence Contract Compiler** or when the capability graph selects it as part of a minimal dependency closure.
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b98-test-evidence-contract-compiler/SKILL.md --out /tmp/test_evidence_contract_compiler.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B98-S08
```
...
- Reject or expose fabricated evidence.
- Reject or expose stale result.
- Reject or expose mock-only promoted to runtime.
