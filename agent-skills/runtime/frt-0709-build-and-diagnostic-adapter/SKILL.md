...
description: "Run FRT-0709 Build and Diagnostic Adapter for FRT G07 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-0709"
  source_name: "frt-0709-build-and-diagnostic-adapter"
  source_sha256: "sha256:d29075cb5e7efdaabebefc927483796411a26a4fe9d0cc644af7ae83c752b372"
  batch: "G07"
...
- Runtime catalog key: `FRT-0709` / `frt-0709-build-and-diagnostic-adapter`.
...
# FRT-0709 — Build and Diagnostic Adapter

## Objective

在G07范围内实现 **Build and Diagnostic Adapter**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：Typed IR经规则Pass降低到目标原生AST; Gap生成Typed Hole而非空实现; Format/Analyze/Build驱动有界确定性修复。

## When to Use

- 实现或修改与 **Build and Diagnostic Adapter** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-0709Result`：版本化、带状态和诊断的结构化结果。
- `FRT-0709EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-0709FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-0709CertificateFragment`：供G07批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g07/frt-0709-build-and-diagnostic-adapter/
packages/runtime/g07/frt-0709-build-and-diagnostic-adapter/
services/control-plane/g07/frt-0709-build-and-diagnostic-adapter/
apps/web-console/src/features/g07/frt-0709-build-and-diagnostic-adapter/
apps/admin-console/src/features/g07/frt-0709-build-and-diagnostic-adapter/
tests/g07/frt-0709-build-and-diagnostic-adapter/
```
...
```text
POST /v1/skills/frt-0709/runs
GET  /v1/skills/frt-0709/runs/{run_id}
POST /v1/skills/frt-0709/runs/{run_id}/verify
GET  /v1/skills/frt-0709/runs/{run_id}/findings
GET  /v1/skills/frt-0709/runs/{run_id}/evidence
```
