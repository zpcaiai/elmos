...
description: "Run FRT-1107 Internationalization Mapper for FRT G11 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-1107"
  source_name: "frt-1107-internationalization-mapper"
  source_sha256: "sha256:779c6231d4da8e6c06ab97b3417c1f871a5fcf8a0096519dff12c809b974b1f0"
  batch: "G11"
...
- Runtime catalog key: `FRT-1107` / `frt-1107-internationalization-mapper`.
...
# FRT-1107 — Internationalization Mapper

## Objective

在G11范围内实现 **Internationalization Mapper**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：恢复布局约束、Design Token和响应式Size Class; 生成资产、字体、Icon、Theme和动画映射; 保持Locale、Plural、Date/Number和RTL语义。

## When to Use

- 实现或修改与 **Internationalization Mapper** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-1107Result`：版本化、带状态和诊断的结构化结果。
- `FRT-1107EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-1107FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-1107CertificateFragment`：供G11批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g11/frt-1107-internationalization-mapper/
packages/runtime/g11/frt-1107-internationalization-mapper/
services/control-plane/g11/frt-1107-internationalization-mapper/
apps/web-console/src/features/g11/frt-1107-internationalization-mapper/
apps/admin-console/src/features/g11/frt-1107-internationalization-mapper/
tests/g11/frt-1107-internationalization-mapper/
```
...
```text
POST /v1/skills/frt-1107/runs
GET  /v1/skills/frt-1107/runs/{run_id}
POST /v1/skills/frt-1107/runs/{run_id}/verify
GET  /v1/skills/frt-1107/runs/{run_id}/findings
GET  /v1/skills/frt-1107/runs/{run_id}/evidence
```
