---
name: stream-usage-cost-cache-runtime
description: 统一 provider stream 事件、token/cost、超时和 cache key 行为。 Use for this exact ELMOS Spring Golden Route v2 contract; imported specifications are not runtime or certification evidence.
...
metadata:
  ...
  source_id: FOUNDATION-09-stream-usage-cost-cache-runtime
  source_name: stream-usage-cost-cache-runtime
  source_path: skills/stream-usage-cost-cache-runtime/SKILL.md
  source_sha256: sha256:1eb433c4019c4ad75e3a721838088cc69207be2469e18842b28727da15d1c626
...
# Stream / Usage / Cost / Cache Runtime

## Goal

统一 provider stream 事件、token/cost、超时和 cache key 行为。

## Use when

在 Elmos 需要实现或升级 **Stream / Usage / Cost / Cache Runtime** 相关能力时使用；它不是仅供解释的提示词，而是实现、测试与验收契约。

## Capabilities to implement

- normalize text/reasoning/tool/usage/finish event
- input/output/cache-read/cache-write token 分项
- provider/request/chunk timeout 分离
- cache policy/fingerprint 可观测并进入成本路由
...
- [ ] 定义 `stream-usage-cost-cache-runtime` 的 public interface、input/output/error schemas。
- [ ] 建立 `stream-usage-cost-cache-runtime` 的 capability/feature flags 与版本字段。
...
- [ ] 至少覆盖并验收：normalize text/reasoning/tool/usage/finish event；以及 input/output/cache-read/cache-write token 分项。
...
- Machine contract: `references/contract.json` (`sha256:9a5e4fc42a2b64f82c37b84c0633a5b433170160c871f48104192f9788628643`).
