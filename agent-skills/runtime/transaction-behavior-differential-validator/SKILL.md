---
name: transaction-behavior-differential-validator
description: Compare real commit, rollback, propagation and isolation behavior outside test-managed transactions. Use for transactional services.
---

# Transaction Behavior Differential Validator
...
Require versioned scenarios, separate databases, traces, expected commit/rollback, propagation, isolation, timeout, async and multi-resource context.

## Workflow

1. Validate schema version, stable identities, immutable hashes and policy prerequisites.
2. Run baseline and migrated scenarios; inspect durable data and transaction traces; flag self-invocation, async and boundary changes.
...
transaction-differences.json with observations and evidence.

## Fail-closed rules

A rollback becoming commit is critical; test-managed transaction evidence alone is insufficient.
