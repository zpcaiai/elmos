---
name: spring-private-deps
description: 验证内部 Maven 仓库、代理、证书、私有 starter、源码和商业许可。 Use for this exact ELMOS Spring Golden Route v2 contract; imported specifications are not runtime or certification evidence.
...
metadata:
  ...
  source_id: ELMOS-GR-029
  source_name: spring-private-deps
  source_path: skills/spring-private-deps/SKILL.md
  source_sha256: sha256:f1e603a21ec32dca5d1b175dd3f2f537efa767bcfa45d7248810334643e6283e
  source_contract_sha256: sha256:1045812e3f2aa997a4a9987116844b344201cbd71052654dc1a57e0d5d6e772c
...
# Private Dependency Preflight

## Objective

验证内部 Maven 仓库、代理、证书、私有 starter、源码和商业许可。

## Scope and capabilities

- Scoped credentials
- Artifact hash/cache
- Source/Javadoc availability
- Custom CA/proxy
- Secret 不落盘
...
- dependency-access-report.json
- offline-cache-plan.json
...
- [ ] 定义 `spring-private-deps` 的 public interface、输入/输出/Error Schema 和版本策略。
- [ ] 实现 `spring-private-deps` 的 production adapter/service，不把业务状态只保存在 Prompt 或内存对象中。
...
- [ ] 至少生成：dependency-access-report.json, offline-cache-plan.json。
...
- [ ] 历史失败与迁移回归边界冻结
- [ ] 报价、ETA、假设和风险进入 SOW
- [ ] 本 Skill 的 required outputs 均由真实目标仓库执行产生：dependency-access-report.json, offline-cache-plan.json。
...
- Machine contract: `references/contract.json` (`sha256:1045812e3f2aa997a4a9987116844b344201cbd71052654dc1a57e0d5d6e772c`).
