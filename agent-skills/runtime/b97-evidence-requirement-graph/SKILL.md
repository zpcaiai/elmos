---
name: b97-evidence-requirement-graph
description: "Use when ELMOS must Link capabilities and Skills to the exact evidence types, freshness windows and independent approvals required for each verification state. Triggers: Evidence Requirement Graph, Canonical Capability Graph Consolidation Pack, evidence requirements, freshness policy, state promotion, independent approval."
metadata:
  id: B97-S10
...
# B97-S10 — Evidence Requirement Graph

## Objective

Link capabilities and Skills to the exact evidence types, freshness windows and independent approvals required for each verification state.
...
Use this Skill when the approved task explicitly requires **Evidence Requirement Graph** or when the capability graph selects it as part of a minimal dependency closure.
...
- `evidence_requirement_graph.json`
...
- state-specific requirements complete.
- expired evidence invalidates claims.
- mock evidence distinguished.
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b97-evidence-requirement-graph/SKILL.md --out /tmp/evidence_requirement_graph.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B97-S10
```
...
- Reject or expose LLM-only evidence.
- Reject or expose stale evidence reuse.
- Reject or expose scope mismatch.
