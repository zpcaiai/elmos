---
name: elmos-api-authz-rbac-abac-rebac-equivalence-verifier
description: Verify authorization semantics across source/target frameworks and policy engines, including field, row, relationship and contextual decisions.
version: 4.0.0
...
# API Authorization RBAC/ABAC/ReBAC Equivalence Verifier
...
- `APIAuthorizationRBACABACReBACEquivalenceVerifierProfile`
- `APIAuthorizationRBACABACReBACEquivalenceVerifierPlan`
- `APIAuthorizationRBACABACReBACEquivalenceVerifierRun`
- `APIAuthorizationRBACABACReBACEquivalenceVerifierDecision`

Required semantic capabilities:

- compile authorization decision graph
- generate positive and negative identity/context matrix
- compare source and target decisions and explanations
- verify cache and policy-update invalidation
- test row/field/tool and relationship authorization
...
- `artifacts/elmos-api-authz-rbac-abac-rebac-equivalence-verifier/profile.yaml`
- `artifacts/elmos-api-authz-rbac-abac-rebac-equivalence-verifier/plan.json`
- `artifacts/elmos-api-authz-rbac-abac-rebac-equivalence-verifier/result.json`
- `artifacts/elmos-api-authz-rbac-abac-rebac-equivalence-verifier/evidence/`
...
- **default allow introduced** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **policy cache stale** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **relationship traversal crosses tenant** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **field auth lost in gateway** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **test identities omit privileged edge case** — must have a preventive control, a detective signal, a negative fixture and a response action.
...
- native scenario: compile authorization decision graph
- native scenario: generate positive and negative identity/context matrix
- native scenario: compare source and target decisions and explanations
- native scenario: verify cache and policy-update invalidation
- native scenario: test row/field/tool and relationship authorization
