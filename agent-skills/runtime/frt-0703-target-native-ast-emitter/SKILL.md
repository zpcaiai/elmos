...
description: "Run FRT-0703 Target Native AST Emitter for FRT G07 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-0703"
  source_name: "frt-0703-target-native-ast-emitter"
  source_sha256: "sha256:a6a7b53bbd8ff47f4a37d7d63f428784e8383d74616a683cb3b0e58fc99ce23b"
  batch: "G07"
...
- Runtime catalog key: `FRT-0703` / `frt-0703-target-native-ast-emitter`.
...
# FRT-0703 — Target Native AST Emitter

## Objective

在G07范围内实现 **Target Native AST Emitter**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：Typed IR经规则Pass降低到目标原生AST; Gap生成Typed Hole而非空实现; Format/Analyze/Build驱动有界确定性修复。

## When to Use

- 实现或修改与 **Target Native AST Emitter** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-0703Result`：版本化、带状态和诊断的结构化结果。
- `FRT-0703EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-0703FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-0703CertificateFragment`：供G07批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g07/frt-0703-target-native-ast-emitter/
packages/runtime/g07/frt-0703-target-native-ast-emitter/
services/control-plane/g07/frt-0703-target-native-ast-emitter/
apps/web-console/src/features/g07/frt-0703-target-native-ast-emitter/
apps/admin-console/src/features/g07/frt-0703-target-native-ast-emitter/
tests/g07/frt-0703-target-native-ast-emitter/
```
...
```text
POST /v1/skills/frt-0703/runs
GET  /v1/skills/frt-0703/runs/{run_id}
POST /v1/skills/frt-0703/runs/{run_id}/verify
GET  /v1/skills/frt-0703/runs/{run_id}/findings
GET  /v1/skills/frt-0703/runs/{run_id}/evidence
```
