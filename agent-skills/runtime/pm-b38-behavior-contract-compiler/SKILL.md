...
# Behavior Contract Compiler
## ELMOS runtime binding

- Invoke this repository Skill as `$pm-b38-behavior-contract-compiler`.
- Immutable source identity: `behavior-contract-compiler` in `precision-migration-b01-44` (B38).
- Runtime adapter: `skill-and-project-synthesis`; binding state: `UNAVAILABLE`.
- Resolve and plan with `python3 scripts/precision_migration/runtime.py plan --skill pm-b38-behavior-contract-compiler`.
...
把 Skill 业务规则编译为类型、Effect、状态、接口和验证契约。

本 Skill 属于精密语言转换系统。它必须以**可执行证据**为准，不得用模型自评、源码相似度或“能编译”替代语义与行为验证。

## Use when

- 当前任务直接涉及 `behavior-contract-compiler` 所描述的能力。
...
2. 执行本 Skill 的核心职责：把 Skill 业务规则编译为类型、Effect、状态、接口和验证契约。
3. 编译Skills与架构蓝图
4. 生成/复用全部构件
5. 构建并执行验收/安全/完整性门禁
6. 输出项目、Runbook和证据
...
```yaml
skill: behavior-contract-compiler
version: 1.0.0
