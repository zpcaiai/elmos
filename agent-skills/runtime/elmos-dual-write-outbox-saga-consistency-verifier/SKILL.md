---
name: elmos-dual-write-outbox-saga-consistency-verifier
description: Verify atomic publication, outbox relay, inbox deduplication, saga compensation and cross-store business invariants during migration and steady-state operation.
version: 4.0.0
...
# Dual-Write, Outbox and Saga Consistency Verifier
...
- `DualWriteOutboxAndSagaConsistencyVerifierProfile`
- `DualWriteOutboxAndSagaConsistencyVerifierPlan`
- `DualWriteOutboxAndSagaConsistencyVerifierRun`
- `DualWriteOutboxAndSagaConsistencyVerifierDecision`

Required semantic capabilities:

- prove or test database/outbox atomicity
- verify relay ordering and deduplication
- exercise compensation and irreversible steps
- reconcile cross-store invariants
- detect orphaned or ghost side effects
...
- `artifacts/elmos-dual-write-outbox-saga-consistency-verifier/profile.yaml`
- `artifacts/elmos-dual-write-outbox-saga-consistency-verifier/plan.json`
- `artifacts/elmos-dual-write-outbox-saga-consistency-verifier/result.json`
- `artifacts/elmos-dual-write-outbox-saga-consistency-verifier/evidence/`
...
- **outbox write separates from business commit** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **dedup key reused across tenant** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **compensation is not inverse** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **relay reorders dependent events** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **ghost completion after partial saga** — must have a preventive control, a detective signal, a negative fixture and a response action.
...
- native scenario: prove or test database/outbox atomicity
- native scenario: verify relay ordering and deduplication
- native scenario: exercise compensation and irreversible steps
- native scenario: reconcile cross-store invariants
- native scenario: detect orphaned or ghost side effects
