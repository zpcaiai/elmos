...
description: "Run FRT-2314 Cross-Tenant Data Isolation Validator for FRT G23 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-2314"
  source_name: "frt-2314-cross-tenant-data-isolation-validator"
  source_sha256: "sha256:1f03dcc7d42ff67cc5d7c21f40c8da6f45c4c5841c6d29bba6d127781a20f2f9"
  batch: "G23"
...
- Runtime catalog key: `FRT-2314` / `frt-2314-cross-tenant-data-isolation-validator`.
...
在G23范围内实现 **Cross-Tenant Data Isolation Validator**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：字段级Data Contract、Authority和Owner; Source-to-Sink与字段级Lineage; Write→Transaction→Outbox→Event→Projection→Cache→Read完整路径。

## When to Use

- 实现或修改与 **Cross-Tenant Data Isolation Validator** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-2314Result`：版本化、带状态和诊断的结构化结果。
- `FRT-2314EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-2314FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-2314CertificateFragment`：供G23批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g23/frt-2314-cross-tenant-data-isolation-validator/
packages/runtime/g23/frt-2314-cross-tenant-data-isolation-validator/
services/control-plane/g23/frt-2314-cross-tenant-data-isolation-validator/
apps/web-console/src/features/g23/frt-2314-cross-tenant-data-isolation-validator/
apps/admin-console/src/features/g23/frt-2314-cross-tenant-data-isolation-validator/
tests/g23/frt-2314-cross-tenant-data-isolation-validator/
```
...
```text
POST /v1/skills/frt-2314/runs
GET  /v1/skills/frt-2314/runs/{run_id}
POST /v1/skills/frt-2314/runs/{run_id}/verify
GET  /v1/skills/frt-2314/runs/{run_id}/findings
GET  /v1/skills/frt-2314/runs/{run_id}/evidence
```
