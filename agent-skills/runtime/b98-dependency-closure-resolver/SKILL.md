---
name: b98-dependency-closure-resolver
description: "Use when ELMOS must Resolve the minimal compatible dependency closure for a selected Skill or route pack. Triggers: Dependency Closure Resolver, Executable Skill Contract and Registry Pack, dependency SAT, minimal closure, version selection, cycle policy."
metadata:
  id: B98-S13
...
# B98-S13 — Dependency Closure Resolver

## Objective

Resolve the minimal compatible dependency closure for a selected Skill or route pack.
...
Use this Skill when the approved task explicitly requires **Dependency Closure Resolver** or when the capability graph selects it as part of a minimal dependency closure.
...
Package dependencies: `B98-S12`, `B97-S05`
...
- deterministic lock.
- minimal closure.
- unsatisfied constraints explained.
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b98-dependency-closure-resolver/SKILL.md --out /tmp/dependency_closure_resolver.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B98-S13
```
...
- Reject or expose dependency confusion.
- Reject or expose version backtracking explosion.
- Reject or expose optional dependency silently required.
