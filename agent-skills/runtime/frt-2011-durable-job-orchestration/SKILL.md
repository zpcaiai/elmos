...
description: "Run FRT-2011 Durable Job Orchestration for FRT G20 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-2011"
  source_name: "frt-2011-durable-job-orchestration"
  source_sha256: "sha256:b4a6e50b2530e6afb7eb14b1c8b0a554972e15aaf486efbb7adb5baf0db3ace1"
  batch: "G20"
...
- Runtime catalog key: `FRT-2011` / `frt-2011-durable-job-orchestration`.
...
# FRT-2011 — Durable Job Orchestration

## Objective

在G20范围内实现 **Durable Job Orchestration**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：提供Skill SDK、Runtime、Registry和Marketplace; 实现CLI、API、IDE、Web Console和Multi-Tenant Workspace; Durable Workflow与分布式Worker支持暂停、恢复、取消和幂等。

## When to Use

- 实现或修改与 **Durable Job Orchestration** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-2011Result`：版本化、带状态和诊断的结构化结果。
- `FRT-2011EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-2011FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-2011CertificateFragment`：供G20批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g20/frt-2011-durable-job-orchestration/
packages/runtime/g20/frt-2011-durable-job-orchestration/
services/control-plane/g20/frt-2011-durable-job-orchestration/
apps/web-console/src/features/g20/frt-2011-durable-job-orchestration/
apps/admin-console/src/features/g20/frt-2011-durable-job-orchestration/
tests/g20/frt-2011-durable-job-orchestration/
```
...
```text
POST /v1/skills/frt-2011/runs
GET  /v1/skills/frt-2011/runs/{run_id}
POST /v1/skills/frt-2011/runs/{run_id}/verify
GET  /v1/skills/frt-2011/runs/{run_id}/findings
GET  /v1/skills/frt-2011/runs/{run_id}/evidence
```
