---
name: jenkins-controller-pipeline-build-artifact-test-fingerp-83b8545a
description: 接入 Jenkins Controller版本、Plugin能力、Job/Build/Pipeline、Artifact、Fingerprint、JUnit与Progressive Log。
metadata:
  elmos_batch: "37B"
  elmos_skill_id: "EVI37B003"
  pack_version: "3.7-complete"
---

# EVI37B003 — Jenkins Controller、Pipeline、Artifact、Test 与 Plugin Connector

## Objective

接入 Jenkins Controller版本、Plugin能力、Job/Build/Pipeline、Artifact、Fingerprint、JUnit与Progressive Log。
...
- 记录 X-Jenkins 版本和 Plugin Catalog，并用其派生 Capability Snapshot。
- 使用 Controller ID + Job Full Name + Build Number 标识 Build。
- 下载 Artifact 后安全解压并计算 SHA-256；Fingerprint 只作为 Provider Evidence。
- 导入 JUnit/TestNG，缺失报告或 allowEmptyResults 产生 Completeness Finding。
...
- controllerVersionIsRecorded
- jenkinsFingerprintIsNotContentDigest
- missingJUnitReportIsNotZeroTests
- pluginUpgradeInvalidatesCapabilitySnapshot
