...
description: "Run FRT-0402 Vue 3 Source Adapter for FRT G04 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-0402"
  source_name: "frt-0402-vue-3-source-adapter"
  source_sha256: "sha256:dcb0039eeaa93e3cb77e30aaa9f9451a67b12f733e9624887b3d081b3e24477a"
  batch: "G04"
...
- Runtime catalog key: `FRT-0402` / `frt-0402-vue-3-source-adapter`.
...
# FRT-0402 — Vue 3 Source Adapter

## Objective

在G04范围内实现 **Vue 3 Source Adapter**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：每个Adapter只解析源，不生成目标代码; 恢复组件、状态、事件、生命周期、路由、样式和平台能力; 统一动态语法、宏、装饰器和模板表达式。

## When to Use

- 实现或修改与 **Vue 3 Source Adapter** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-0402Result`：版本化、带状态和诊断的结构化结果。
- `FRT-0402EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-0402FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-0402CertificateFragment`：供G04批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g04/frt-0402-vue-3-source-adapter/
packages/runtime/g04/frt-0402-vue-3-source-adapter/
services/control-plane/g04/frt-0402-vue-3-source-adapter/
apps/web-console/src/features/g04/frt-0402-vue-3-source-adapter/
apps/admin-console/src/features/g04/frt-0402-vue-3-source-adapter/
tests/g04/frt-0402-vue-3-source-adapter/
```
...
```text
POST /v1/skills/frt-0402/runs
GET  /v1/skills/frt-0402/runs/{run_id}
POST /v1/skills/frt-0402/runs/{run_id}/verify
GET  /v1/skills/frt-0402/runs/{run_id}/findings
GET  /v1/skills/frt-0402/runs/{run_id}/evidence
```
