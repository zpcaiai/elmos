...
description: "Run FRT-2803 Bulkhead and Failure Isolation Validator for FRT G28 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-2803"
  source_name: "frt-2803-bulkhead-and-failure-isolation-validator"
  source_sha256: "sha256:a10588c4825af07b6189bee27919677f30118cf4aa7d03eb33216ec25dcef3d9"
  batch: "G28"
...
- Runtime catalog key: `FRT-2803` / `frt-2803-bulkhead-and-failure-isolation-validator`.
...
在G28范围内实现 **Bulkhead and Failure Isolation Validator**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：Dependency Failure Model与Common Cause分析; 端到端Deadline、Retry Budget、Circuit Breaker、Bulkhead; Queue Durability、Replay、Stateful Failover、Fence Token。

## When to Use

- 实现或修改与 **Bulkhead and Failure Isolation Validator** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-2803Result`：版本化、带状态和诊断的结构化结果。
- `FRT-2803EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-2803FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-2803CertificateFragment`：供G28批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g28/frt-2803-bulkhead-and-failure-isolation-validator/
packages/runtime/g28/frt-2803-bulkhead-and-failure-isolation-validator/
services/control-plane/g28/frt-2803-bulkhead-and-failure-isolation-validator/
apps/web-console/src/features/g28/frt-2803-bulkhead-and-failure-isolation-validator/
apps/admin-console/src/features/g28/frt-2803-bulkhead-and-failure-isolation-validator/
tests/g28/frt-2803-bulkhead-and-failure-isolation-validator/
```
...
```text
POST /v1/skills/frt-2803/runs
GET  /v1/skills/frt-2803/runs/{run_id}
POST /v1/skills/frt-2803/runs/{run_id}/verify
GET  /v1/skills/frt-2803/runs/{run_id}/findings
GET  /v1/skills/frt-2803/runs/{run_id}/evidence
```
