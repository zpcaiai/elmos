---
name: spring-eligibility
description: 返回 eligible、eligible_with_warnings、custom_assessment 或 ineligible。 Use for this exact ELMOS Spring Golden Route v2 contract; imported specifications are not runtime or certification evidence.
...
metadata:
  ...
  source_id: ELMOS-GR-026
  source_name: spring-eligibility
  source_path: skills/spring-eligibility/SKILL.md
  source_sha256: sha256:993f3ceeccea65a835dc52dede5e1e62f58abdea86d305e5a4c9dcf8ba9910bf
  source_contract_sha256: sha256:ede971c28212aac6cb759d7bfa1f9828254974c625e5236a254952d28027b74c
...
# Golden Route Eligibility

## Objective

返回 eligible、eligible_with_warnings、custom_assessment 或 ineligible。
...
- [ ] 定义 `spring-eligibility` 的 public interface、输入/输出/Error Schema 和版本策略。
- [ ] 实现 `spring-eligibility` 的 production adapter/service，不把业务状态只保存在 Prompt 或内存对象中。
...
- [ ] 至少生成：eligibility-report.json, blocking-findings.json。
...
- [ ] 历史失败与迁移回归边界冻结
- [ ] 报价、ETA、假设和风险进入 SOW
- [ ] 本 Skill 的 required outputs 均由真实目标仓库执行产生：eligibility-report.json, blocking-findings.json。
...
- Machine contract: `references/contract.json` (`sha256:ede971c28212aac6cb759d7bfa1f9828254974c625e5236a254952d28027b74c`).
