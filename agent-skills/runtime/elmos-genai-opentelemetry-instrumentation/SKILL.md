---
name: genai-opentelemetry-instrumentation
description: Use this skill when Elmos must perform genai opentelemetry instrumentation as part of the 12-observability-lineage-finops
...
compatibility: Elmos v3 harness; Cross-Cutting Observability and Economics Plane; policy and evidence services required.
metadata:
  version: 3.0.0
  pack: 12-observability-lineage-finops
  priority: P0
  exposure: atomic-registry-only
  business-line: observability-finops
allowed-tools: otel.emit lineage.emit metric.record cost.allocate trace.replay
---
# genai-opentelemetry-instrumentation

## 能力目标

为模型、Agent、Tool、MCP、检索和记忆输出统一 Trace、Metric 和 Event。
...
- 必须门：correlation-id-complete, content-capture-policy, cost-reconciled, slo-observable；
...
- 业务线：`observability-finops`；包：`12-observability-lineage-finops`。
