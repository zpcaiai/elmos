---
name: b100-runner-enrollment-workload-identity
description: "Use when ELMOS must Enroll runners with unique short-lived workload identities and explicit tenant or fleet ownership. Triggers: Runner Enrollment and Workload Identity, Hardened Runner Fabric Pack, enrollment, workload identity, ownership, rotation."
metadata:
  id: B100-S01
...
# B100-S01 — Runner Enrollment and Workload Identity

## Objective

Enroll runners with unique short-lived workload identities and explicit tenant or fleet ownership.
...
Use this Skill when the approved task explicitly requires **Runner Enrollment and Workload Identity** or when the capability graph selects it as part of a minimal dependency closure.
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b100-runner-enrollment-workload-identity/SKILL.md --out /tmp/runner_enrollment_workload_identity.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B100-S01
```
...
- Reject or expose shared credential.
- Reject or expose identity cloning.
- Reject or expose orphan enrollment.
