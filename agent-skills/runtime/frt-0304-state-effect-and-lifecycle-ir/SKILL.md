...
description: "Run FRT-0304 State Effect and Lifecycle IR for FRT G03 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-0304"
  source_name: "frt-0304-state-effect-and-lifecycle-ir"
  source_sha256: "sha256:a48740639cca9f6f2354bafa60b6007863782fc83c014a653220aa41e8608eb9"
  batch: "G03"
...
- Runtime catalog key: `FRT-0304` / `frt-0304-state-effect-and-lifecycle-ir`.
...
# FRT-0304 — State Effect and Lifecycle IR

## Objective

在G03范围内实现 **State Effect and Lifecycle IR**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：建立跨Vue/React/小程序/ArkUI/Flutter的类型和语义中间层; 保留Null、Identity、Effect、Lifecycle、Navigation、Data Authority和UI语义; 每个IR节点绑定Source Range和稳定Semantic ID。

## When to Use

- 实现或修改与 **State Effect and Lifecycle IR** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-0304Result`：版本化、带状态和诊断的结构化结果。
- `FRT-0304EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-0304FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-0304CertificateFragment`：供G03批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g03/frt-0304-state-effect-and-lifecycle-ir/
packages/runtime/g03/frt-0304-state-effect-and-lifecycle-ir/
services/control-plane/g03/frt-0304-state-effect-and-lifecycle-ir/
apps/web-console/src/features/g03/frt-0304-state-effect-and-lifecycle-ir/
apps/admin-console/src/features/g03/frt-0304-state-effect-and-lifecycle-ir/
tests/g03/frt-0304-state-effect-and-lifecycle-ir/
```
...
```text
POST /v1/skills/frt-0304/runs
GET  /v1/skills/frt-0304/runs/{run_id}
POST /v1/skills/frt-0304/runs/{run_id}/verify
GET  /v1/skills/frt-0304/runs/{run_id}/findings
GET  /v1/skills/frt-0304/runs/{run_id}/evidence
```
