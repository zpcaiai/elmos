---
name: b97-legacy-skill-estate-inventory
description: "Use when ELMOS must Inventory every installed Skill, schema, tool, runtime, route, policy, test and evidence asset from Batch 1–96 without treating filenames as authoritative identity. Triggers: Legacy Skill Estate Inventory, Canonical Capability Graph Consolidation Pack, skill inventory, source provenance, duplicate candidates, missing ownership."
metadata:
  id: B97-S01
  ...
  category: "Canonical Capability Graph Consolidation Pack"
---

# B97-S01 — Legacy Skill Estate Inventory

## Objective

Inventory every installed Skill, schema, tool, runtime, route, policy, test and evidence asset from Batch 1–96 without treating filenames as authoritative identity.
...
Consolidates the existing estate before any further execution. It treats the installed Batch 1–96 manifests as evidence sources, not unquestioned truth.

The objective belongs to **Canonical Capability Graph Consolidation Pack**, whose system-level purpose is to normalize Batch 1–96 into a governed capability ontology, dependency graph, conflict model, compatibility matrix, and conservative graph certification gate.

## When to Use

Use this Skill when the approved task explicitly requires **Legacy Skill Estate Inventory** or when the capability graph selects it as part of a minimal dependency closure.
...
Package dependencies: None inside this package; use the installed Batch 1–96 capability graph as upstream authority.
...
- Preferred implementation/tooling context: `Python 3.12+`, `JSON Schema 2020-12`, `graph database or typed adjacency store`, `content hashing`. Alternatives are allowed only when their versions, behavior differences and rationale are recorded.
...
- all installed manifests reconciled.
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b97-legacy-skill-estate-inventory/SKILL.md --out /tmp/legacy_skill_estate_inventory.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B97-S01
```
...
- Reject or expose hidden duplicate identifiers.
- Reject or expose stale copied Skills.
- Reject or expose malformed frontmatter.
