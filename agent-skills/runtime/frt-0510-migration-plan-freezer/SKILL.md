...
description: "Run FRT-0510 Migration Plan Freezer for FRT G05 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-0510"
  source_name: "frt-0510-migration-plan-freezer"
  source_sha256: "sha256:ae17012e87d04dda6fbf83b375964a2a77e9de1ee494f0d7d746fe21085ff3a0"
  batch: "G05"
...
- Runtime catalog key: `FRT-0510` / `frt-0510-migration-plan-freezer`.
...
# FRT-0510 — Migration Plan Freezer

## Objective

在G05范围内实现 **Migration Plan Freezer**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：比较源语义与目标能力并生成Gap Register; 区分Native、Shim、Product Decision、Manual与Unsupported; 估算正确率、耗时、风险、数据和测试影响。

## When to Use

- 实现或修改与 **Migration Plan Freezer** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-0510Result`：版本化、带状态和诊断的结构化结果。
- `FRT-0510EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-0510FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-0510CertificateFragment`：供G05批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g05/frt-0510-migration-plan-freezer/
packages/runtime/g05/frt-0510-migration-plan-freezer/
services/control-plane/g05/frt-0510-migration-plan-freezer/
apps/web-console/src/features/g05/frt-0510-migration-plan-freezer/
apps/admin-console/src/features/g05/frt-0510-migration-plan-freezer/
tests/g05/frt-0510-migration-plan-freezer/
```
...
```text
POST /v1/skills/frt-0510/runs
GET  /v1/skills/frt-0510/runs/{run_id}
POST /v1/skills/frt-0510/runs/{run_id}/verify
GET  /v1/skills/frt-0510/runs/{run_id}/findings
GET  /v1/skills/frt-0510/runs/{run_id}/evidence
```
