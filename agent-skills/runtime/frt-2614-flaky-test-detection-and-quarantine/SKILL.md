...
description: "Run FRT-2614 Flaky Test Detection and Quarantine for FRT G26 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-2614"
  source_name: "frt-2614-flaky-test-detection-and-quarantine"
  source_sha256: "sha256:3e01614e404d3c3ce784ec427f5074bb055f42d463bb2e73c5bf47716c3b757d"
  batch: "G26"
...
- Runtime catalog key: `FRT-2614` / `frt-2614-flaky-test-detection-and-quarantine`.
...
在G26范围内实现 **Flaky Test Detection and Quarantine**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：Requirement/Acceptance/State Machine派生Test Obligation; Unit/Component/Contract/Integration/Journey分层协调; Differential、Mutation、Visual/Semantic、Security和Performance Regression。

## When to Use

- 实现或修改与 **Flaky Test Detection and Quarantine** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-2614Result`：版本化、带状态和诊断的结构化结果。
- `FRT-2614EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-2614FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-2614CertificateFragment`：供G26批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g26/frt-2614-flaky-test-detection-and-quarantine/
packages/runtime/g26/frt-2614-flaky-test-detection-and-quarantine/
services/control-plane/g26/frt-2614-flaky-test-detection-and-quarantine/
apps/web-console/src/features/g26/frt-2614-flaky-test-detection-and-quarantine/
apps/admin-console/src/features/g26/frt-2614-flaky-test-detection-and-quarantine/
tests/g26/frt-2614-flaky-test-detection-and-quarantine/
```
...
```text
POST /v1/skills/frt-2614/runs
GET  /v1/skills/frt-2614/runs/{run_id}
POST /v1/skills/frt-2614/runs/{run_id}/verify
GET  /v1/skills/frt-2614/runs/{run_id}/findings
GET  /v1/skills/frt-2614/runs/{run_id}/evidence
```
