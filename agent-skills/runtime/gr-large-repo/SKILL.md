---
name: gr-large-repo
description: 为 300k–3M+ MRLOC、多模块、长构建和大测试集实施增量、分片、背压和资源治理。 Use for this exact ELMOS Spring Golden Route v2 contract; imported specifications are not runtime or certification evidence.
...
metadata:
  ...
  source_id: ELMOS-GR-089
  source_name: gr-large-repo
  source_path: skills/gr-large-repo/SKILL.md
  source_sha256: sha256:f98f2c2dc40718275844bb3e9365cec1a4b0b9a8af0c0acaa500d18eb307b9b5
  source_contract_sha256: sha256:b47ee8b6211cd073d079ce8a9c1064210bffbcffad85cf8ad51239f22fc20c85
...
为 300k–3M+ MRLOC、多模块、长构建和大测试集实施增量、分片、背压和资源治理。
...
- [ ] 定义 `gr-large-repo` 的 public interface、输入/输出/Error Schema 和版本策略。
- [ ] 实现 `gr-large-repo` 的 production adapter/service，不把业务状态只保存在 Prompt 或内存对象中。
...
- [ ] 至少生成：partition-plan.json, resource-budget.json, scale-results.json。
...
- [ ] 本 Skill 的 required outputs 均由真实目标仓库执行产生：partition-plan.json, resource-budget.json, scale-results.json。
...
- Machine contract: `references/contract.json` (`sha256:b47ee8b6211cd073d079ce8a9c1064210bffbcffad85cf8ad51239f22fc20c85`).
