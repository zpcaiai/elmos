---
name: elmos-native-build-cache-adapters
description: Integrate ecosystem-native incremental caches while keeping ELMOS ActionResults, manifests, and evidence authoritative.
version: 1.0.0
...
Integrate ecosystem-native incremental caches while keeping ELMOS ActionResults, manifests, and evidence authoritative.
...
- Dependency skills have passed: **elmos-action-cache, elmos-cache-key-fingerprinting, elmos-sandbox-overlay-workspaces**.
...
- Native caches accelerate builds but never replace ELMOS provenance or certification.
- Compiler, SDK, target triple, lockfiles, flags, and native-cache configuration participate in fingerprints.
- Writable native caches are isolated by trust domain and toolchain.
- A clean-room rebuild without native cache remains a required correctness check.
...
1. Implement adapters for Gradle/Maven, MSBuild/NuGet, Cargo/sccache, CMake/Ninja/ccache, TypeScript/pnpm/Vite, Python wheel caches, Xcode/Swift caches, and Flutter/Dart pub caches.
2. Redirect all cache paths into sandbox-approved volumes.
3. Capture native hit/miss diagnostics and import build outputs into ELMOS manifests and CAS.
4. Invalidate incompatible caches on toolchain or lockfile change.
5. Degrade safely to clean build when an adapter fails.
