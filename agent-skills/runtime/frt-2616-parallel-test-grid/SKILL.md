...
description: "Run FRT-2616 Parallel Test Grid for FRT G26 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-2616"
  source_name: "frt-2616-parallel-test-grid"
  source_sha256: "sha256:40cf395fd2ba1322e13e0d75446e81eda58a5acf2a779dca5bae413fd0c9b984"
  batch: "G26"
...
- Runtime catalog key: `FRT-2616` / `frt-2616-parallel-test-grid`.
...
在G26范围内实现 **Parallel Test Grid**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：Requirement/Acceptance/State Machine派生Test Obligation; Unit/Component/Contract/Integration/Journey分层协调; Differential、Mutation、Visual/Semantic、Security和Performance Regression。

## When to Use

- 实现或修改与 **Parallel Test Grid** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-2616Result`：版本化、带状态和诊断的结构化结果。
- `FRT-2616EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-2616FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-2616CertificateFragment`：供G26批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g26/frt-2616-parallel-test-grid/
packages/runtime/g26/frt-2616-parallel-test-grid/
services/control-plane/g26/frt-2616-parallel-test-grid/
apps/web-console/src/features/g26/frt-2616-parallel-test-grid/
apps/admin-console/src/features/g26/frt-2616-parallel-test-grid/
tests/g26/frt-2616-parallel-test-grid/
```
...
```text
POST /v1/skills/frt-2616/runs
GET  /v1/skills/frt-2616/runs/{run_id}
POST /v1/skills/frt-2616/runs/{run_id}/verify
GET  /v1/skills/frt-2616/runs/{run_id}/findings
GET  /v1/skills/frt-2616/runs/{run_id}/evidence
```
