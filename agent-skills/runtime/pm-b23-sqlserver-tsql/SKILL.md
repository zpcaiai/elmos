---
name: pm-b23-sqlserver-tsql
description: "迁移 T-SQL 语法、批次、变量、函数、错误和系统对象. Precision Migration B23 contract; use for this exact assessment, transformation, validation, repair, evidence, or cutover scope."
---

# Sqlserver Tsql
## ELMOS runtime binding

- Invoke this repository Skill as `$pm-b23-sqlserver-tsql`.
- Immutable source identity: `sqlserver-tsql` in `precision-migration-b01-44` (B23).
- Runtime adapter: `database-and-data-route`; binding state: `DECLARED`.
- Resolve and plan with `python3 scripts/precision_migration/runtime.py plan --skill pm-b23-sqlserver-tsql`.
...
1. 盘点对象与专有能力
2. 执行本 Skill 的核心职责：迁移 T-SQL 语法、批次、变量、函数、错误和系统对象。
