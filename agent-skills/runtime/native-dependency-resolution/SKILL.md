---
name: native-dependency-resolution
description: 尽量在 Elmos 内原生解析依赖，而不是依赖目标机器安装包管理器。 Use for this exact ELMOS Spring Golden Route v2 contract; imported specifications are not runtime or certification evidence.
...
metadata:
  ...
  source_id: FOUNDATION-04-native-dependency-resolution
  source_name: native-dependency-resolution
  source_path: skills/native-dependency-resolution/SKILL.md
  source_sha256: sha256:b581276a4e116505b45509e20a71ab17f5750a10f2d95bab31c2ed94a11c1542
  source_contract_sha256: sha256:bee3c00b94c42d3f8a6a39e644080cf94f7b55746dd48ec255899a2efb82172d
...
尽量在 Elmos 内原生解析依赖，而不是依赖目标机器安装包管理器。

## Use when

在 Elmos 需要实现或升级 **Native Dependency Resolution** 相关能力时使用；它不是仅供解释的提示词，而是实现、测试与验收契约。

## Capabilities to implement

- 通过 HTTP/index metadata 完成解析
- 宿主统一注入代理、TLS、凭据和缓存
- 避免临时目录丢失项目级 package manager 配置
- 解析失败必须给出包/索引/版本级结构化原因
...
- [ ] 定义 `native-dependency-resolution` 的 public interface、input/output/error schemas。
- [ ] 建立 `native-dependency-resolution` 的 capability/feature flags 与版本字段。
...
- [ ] 至少覆盖并验收：通过 HTTP/index metadata 完成解析；以及 宿主统一注入代理、TLS、凭据和缓存。
...
- Machine contract: `references/contract.json` (`sha256:bee3c00b94c42d3f8a6a39e644080cf94f7b55746dd48ec255899a2efb82172d`).
