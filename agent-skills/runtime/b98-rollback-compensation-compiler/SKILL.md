---
name: b98-rollback-compensation-compiler
description: "Use when ELMOS must Compile reversible actions, compensations and irreversible decision gates into runtime-ready plans. Triggers: Rollback and Compensation Compiler, Executable Skill Contract and Registry Pack, rollback, compensation, irreversibility, reconciliation."
metadata:
  id: B98-S07
...
# B98-S07 — Rollback and Compensation Compiler

## Objective

Compile reversible actions, compensations and irreversible decision gates into runtime-ready plans.
...
Use this Skill when the approved task explicitly requires **Rollback and Compensation Compiler** or when the capability graph selects it as part of a minimal dependency closure.
...
- rollback order valid.
- irreversible steps gated.
- partial compensation reported.
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b98-rollback-compensation-compiler/SKILL.md --out /tmp/rollback_compensation_compiler.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B98-S07
```
...
- Reject or expose destructive rollback.
- Reject or expose compensation recursion.
- Reject or expose false full recovery.
