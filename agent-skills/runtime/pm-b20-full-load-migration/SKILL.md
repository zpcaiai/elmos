...
- Invoke this repository Skill as `$pm-b20-full-load-migration`.
- Immutable source identity: `full-load-migration` in `precision-migration-b01-44` (B20).
- Runtime adapter: `database-and-data-route`; binding state: `UNAVAILABLE`.
- Resolve and plan with `python3 scripts/precision_migration/runtime.py plan --skill pm-b20-full-load-migration`.
...
1. 盘点对象与专有能力
2. 执行本 Skill 的核心职责：规划和执行可恢复、可分片、可校验的全量数据迁移。
