---
name: b100-fleet-routing-capability-match
description: "Use when ELMOS must Route tasks to compatible, healthy and authorized runners by platform, toolchain, trust and locality. Triggers: Fleet Routing and Capability Match, Hardened Runner Fabric Pack, fleet routing, capability match, health, data locality."
metadata:
  id: B100-S13
...
# B100-S13 — Fleet Routing and Capability Match

## Objective

Route tasks to compatible, healthy and authorized runners by platform, toolchain, trust and locality.
...
Use this Skill when the approved task explicitly requires **Fleet Routing and Capability Match** or when the capability graph selects it as part of a minimal dependency closure.
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b100-fleet-routing-capability-match/SKILL.md --out /tmp/fleet_routing_capability_match.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B100-S13
```
...
- Reject or expose label spoofing.
- Reject or expose unhealthy runner selection.
- Reject or expose cross-region data violation.
