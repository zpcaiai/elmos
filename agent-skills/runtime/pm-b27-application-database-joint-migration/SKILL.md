...
- Invoke this repository Skill as `$pm-b27-application-database-joint-migration`.
- Immutable source identity: `batch-27-application-database-joint-migration` in `precision-migration-b01-44` (B27).
- Runtime adapter: `database-and-data-route`; binding state: `DECLARED`.
- Resolve and plan with `python3 scripts/precision_migration/runtime.py plan --skill pm-b27-application-database-joint-migration`.
...
- 当任务涉及 **embedded-sql-discovery** 时，调用 `../pm-b27-embedded-sql-discovery/SKILL.md`。
- 当任务涉及 **orm-dialect-migration** 时，调用 `../pm-b27-orm-dialect-migration/SKILL.md`。
- 当任务涉及 **jdbc-ado-odbc-driver-migration** 时，调用 `../pm-b27-jdbc-ado-odbc-driver-migration/SKILL.md`。
- 当任务涉及 **query-builder-migration** 时，调用 `../pm-b27-query-builder-migration/SKILL.md`。
- 当任务涉及 **native-query-rewriter** 时，调用 `../pm-b27-native-query-rewriter/SKILL.md`。
- 当任务涉及 **stored-procedure-call-rewriter** 时，调用 `../pm-b27-stored-procedure-call-rewriter/SKILL.md`。
- 当任务涉及 **transaction-api-rewriter** 时，调用 `../pm-b27-transaction-api-rewriter/SKILL.md`。
- 当任务涉及 **pagination-and-locking-rewriter** 时，调用 `../pm-b27-pagination-and-locking-rewriter/SKILL.md`。
- 当任务涉及 **database-error-code-mapping** 时，调用 `../pm-b27-database-error-code-mapping/SKILL.md`。
- 当任务涉及 **application-database-dual-validation** 时，调用 `../pm-b27-application-database-dual-validation/SKILL.md`。
- 当任务涉及 **query-plan-regression-analysis** 时，调用 `../pm-b27-query-plan-regression-analysis/SKILL.md`。
- 当任务涉及 **cross-layer-performance-tuning** 时，调用 `../pm-b27-cross-layer-performance-tuning/SKILL.md`。
