...
description: "Run FRT-2109 Missing Implementation Link Detector for FRT G21 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-2109"
  source_name: "frt-2109-missing-implementation-link-detector"
  source_sha256: "sha256:8480f218bcde54171aff747ec26cdde4eb720c0f34d93a34ebaee6ff53b3229a"
  batch: "G21"
...
- Runtime catalog key: `FRT-2109` / `frt-2109-missing-implementation-link-detector`.
...
在G21范围内实现 **Missing Implementation Link Detector**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：统一 Product Closure Graph; Requirement、Capability、Journey、Command、API、Data、Permission、Test、Monitoring、Runbook、Certificate 双向追踪; 孤儿 UI/API/Command/Event/Data Write 检测。

## When to Use

- 实现或修改与 **Missing Implementation Link Detector** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-2109Result`：版本化、带状态和诊断的结构化结果。
- `FRT-2109EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-2109FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-2109CertificateFragment`：供G21批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g21/frt-2109-missing-implementation-link-detector/
packages/runtime/g21/frt-2109-missing-implementation-link-detector/
services/control-plane/g21/frt-2109-missing-implementation-link-detector/
apps/web-console/src/features/g21/frt-2109-missing-implementation-link-detector/
apps/admin-console/src/features/g21/frt-2109-missing-implementation-link-detector/
tests/g21/frt-2109-missing-implementation-link-detector/
```
...
```text
POST /v1/skills/frt-2109/runs
GET  /v1/skills/frt-2109/runs/{run_id}
POST /v1/skills/frt-2109/runs/{run_id}/verify
GET  /v1/skills/frt-2109/runs/{run_id}/findings
GET  /v1/skills/frt-2109/runs/{run_id}/evidence
```
