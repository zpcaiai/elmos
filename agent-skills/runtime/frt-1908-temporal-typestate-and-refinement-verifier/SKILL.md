...
description: "Run FRT-1908 Temporal Typestate and Refinement Verifier for FRT G19 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-1908"
  source_name: "frt-1908-temporal-typestate-and-refinement-verifier"
  source_sha256: "sha256:1e0c404e26e9c7d627d35bfac19073c9102a6520eb3404acf57cc43701a38006"
  batch: "G19"
...
- Runtime catalog key: `FRT-1908` / `frt-1908-temporal-typestate-and-refinement-verifier`.
...
# FRT-1908 — Temporal Typestate and Refinement Verifier

## Objective

在G19范围内实现 **Temporal Typestate and Refinement Verifier**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：从IR、Rule和业务不变量生成Proof Obligation; Leanstral仅提出证明候选，Lean Kernel为可信根; 组合SMT、Model Checking、Symbolic和Differential Evidence。

## When to Use

- 实现或修改与 **Temporal Typestate and Refinement Verifier** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-1908Result`：版本化、带状态和诊断的结构化结果。
- `FRT-1908EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-1908FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-1908CertificateFragment`：供G19批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g19/frt-1908-temporal-typestate-and-refinement-verifier/
packages/runtime/g19/frt-1908-temporal-typestate-and-refinement-verifier/
services/control-plane/g19/frt-1908-temporal-typestate-and-refinement-verifier/
apps/web-console/src/features/g19/frt-1908-temporal-typestate-and-refinement-verifier/
apps/admin-console/src/features/g19/frt-1908-temporal-typestate-and-refinement-verifier/
tests/g19/frt-1908-temporal-typestate-and-refinement-verifier/
```
...
```text
POST /v1/skills/frt-1908/runs
GET  /v1/skills/frt-1908/runs/{run_id}
POST /v1/skills/frt-1908/runs/{run_id}/verify
GET  /v1/skills/frt-1908/runs/{run_id}/findings
GET  /v1/skills/frt-1908/runs/{run_id}/evidence
```
