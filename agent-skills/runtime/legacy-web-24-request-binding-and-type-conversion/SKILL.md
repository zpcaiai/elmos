...
metadata:
  ...
  source_id: 24-request-binding-and-type-conversion
  source_digest: sha256:30bbdba351da1342a8b3032d5a7ec9feab487bd46cda477c42d1f52e0f49a173
  phase: semantic-recovery
  runtime_state: CODE_COMPLETE_LOCAL
  capability_state: LOCAL_EXECUTED
  operation_code: BINDING_CONVERSION_RECOVERED
  runtime_handler_id: legacy-web-handler:24-request-binding-and-type-conversion
---
...
---
id: 24-request-binding-and-type-conversion
title: 请求绑定与类型转换恢复
version: 1.0.0
phase: semantic-recovery
priority: P0
...
恢复参数来源、别名、集合/嵌套属性、缺省值、checkbox reset、conversion error、locale/date/number 和绑定白名单。
...
producerSkill: 24-request-binding-and-type-conversion
...
6. 记录每个字段的来源、名称、别名、scope、类型、null/empty 规则、collection index 和 nested path。
7. 保留 Struts1 checkbox/reset、Struts2 conversion error/OGNL、Servlet getParameterValues 的差异。
8. 生成显式 allowlist，避免 Spring mass assignment 扩大攻击面。
9. 将 locale/timezone/charset 纳入转换上下文。
