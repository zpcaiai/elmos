---
name: provider-certification-factory
description: Turn provider-neutral contracts into supportable SCM, LLM, CI, cloud,
  artifact and identity adapters with runtime evidence.
metadata:
  source_package: elmos-codex-skills-batch56a-product-closure
  source_id: CLO56A005
  source_batch: 56A
  source_maturity: reviewed-design
  source_sha256: sha256:ae1744a2828358c53d156a002fbf5ccab9d3c8ea0b33f64252ed1625d6c05a31
...
Turn provider-neutral contracts into supportable SCM, LLM, CI, cloud, artifact and identity adapters with runtime evidence.
...
1. Prioritize the first supported provider matrix.
2. Pin API and protocol versions.
3. Implement test-tenant and sandbox connectors.
4. Run auth, capability, rate-limit and failure tests.
5. Validate webhook, polling, backfill and reconciliation.
6. Publish supported, conditional or blocked status.

## Required Tests

- `providerCapabilityMatchesRuntime`
- `expiredCredentialFailsClosed`
- `missedWebhookRecoveredByPolling`
- `revokedProviderStopsNewWork`
...
```yaml
skill_id: CLO56A005
skill_name: provider-certification-factory
