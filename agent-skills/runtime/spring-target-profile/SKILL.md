---
name: spring-target-profile
description: 用认证 Target Profile 锁定 Java、Spring Boot、Framework、Security、Hibernate、Maven 和镜像版本。 Use for this exact ELMOS Spring Golden Route v2 contract; imported specifications are not runtime or certification evidence.
...
metadata:
  ...
  source_id: ELMOS-GR-011
  source_name: spring-target-profile
  source_path: skills/spring-target-profile/SKILL.md
  source_sha256: sha256:cc6d4b23625d258508e57c9846646f0fb5c41c3ffa374db65cf10b2766154553
  source_contract_sha256: sha256:befe92b6ebce98633c7cf054a7b222c843290b369f8e0b6167cff6dd1f77dc99
...
# Spring Target Profile Policy

## Objective

用认证 Target Profile 锁定 Java、Spring Boot、Framework、Security、Hibernate、Maven 和镜像版本。

## Scope and capabilities

- exact patch freeze
- certified line 与 latest patch policy
- 中间升级路径
- 安全公告与支持策略
- Target fingerprint
...
- [ ] 定义 `spring-target-profile` 的 public interface、输入/输出/Error Schema 和版本策略。
- [ ] 实现 `spring-target-profile` 的 production adapter/service，不把业务状态只保存在 Prompt 或内存对象中。
...
- [ ] 至少生成：target-profile.json, dependency-policy.json, upgrade-path.json。
...
- [ ] 本 Skill 的 required outputs 均由真实目标仓库执行产生：target-profile.json, dependency-policy.json, upgrade-path.json。
...
- Machine contract: `references/contract.json` (`sha256:befe92b6ebce98633c7cf054a7b222c843290b369f8e0b6167cff6dd1f77dc99`).
