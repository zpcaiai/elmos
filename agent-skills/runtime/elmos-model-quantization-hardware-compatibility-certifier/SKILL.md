---
name: elmos-model-quantization-hardware-compatibility-certifier
description: Certify precision conversion, kernel availability, numerical quality, latency, memory, portability and fallback across accelerator targets.
version: 4.0.0
...
# Model Quantization and Hardware Compatibility Certifier
...
- `ModelQuantizationAndHardwareCompatibilityCertifierProfile`
- `ModelQuantizationAndHardwareCompatibilityCertifierPlan`
- `ModelQuantizationAndHardwareCompatibilityCertifierRun`
- `ModelQuantizationAndHardwareCompatibilityCertifierDecision`

Required semantic capabilities:

- profile FP32/BF16/FP16/INT8/low-bit variants
- verify kernel and architecture support
- measure task and safety quality deltas
- benchmark memory, throughput and latency
- gate fallback and dequantization paths
...
- `artifacts/elmos-model-quantization-hardware-compatibility-certifier/profile.yaml`
- `artifacts/elmos-model-quantization-hardware-compatibility-certifier/plan.json`
- `artifacts/elmos-model-quantization-hardware-compatibility-certifier/result.json`
- `artifacts/elmos-model-quantization-hardware-compatibility-certifier/evidence/`
...
- **benchmark uses different tokenizer** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **quality loss hidden by aggregate score** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **unsupported kernel silently falls back CPU** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **calibration data contaminated** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **quantized safety behavior drifts** — must have a preventive control, a detective signal, a negative fixture and a response action.
...
- native scenario: profile FP32/BF16/FP16/INT8/low-bit variants
- native scenario: verify kernel and architecture support
- native scenario: measure task and safety quality deltas
- native scenario: benchmark memory, throughput and latency
- native scenario: gate fallback and dequantization paths
