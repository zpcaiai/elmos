---
name: b104-product-certification-board-gate
description: "Use when ELMOS must Issue certified, limited, experimental or blocked status for an exact ELMOS release and support matrix. Triggers: Product Certification Board Gate, Real World Product Certification Pack, product gate, board decision, support matrix, public claims."
metadata:
  id: B104-S16
  ...
  category: "Real World Product Certification Pack"
---

# B104-S16 — Product Certification Board Gate

## Objective

Issue certified, limited, experimental or blocked status for an exact ELMOS release and support matrix.
...
The objective belongs to **Real World Product Certification Pack**, whose system-level purpose is to certify ELMOS as a product using governed corpora, representative environments, clean-room and scale benchmarks, fault injection, red-team security, rollback, DR, customer pilots, SLOs, cost, support and release board evidence.

## When to Use

Use this Skill when the approved task explicitly requires **Product Certification Board Gate** or when the capability graph selects it as part of a minimal dependency closure.
...
- `product_certification_result.json`
- `certified_support_matrix.json`
...
- Preferred implementation/tooling context: `certification lab`, `multi-OS runners`, `fault injection`, `security testing`, `customer pilot workflow`. Alternatives are allowed only when their versions, behavior differences and rationale are recorded.
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b104-product-certification-board-gate/SKILL.md --out /tmp/product_certification_board_gate.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B104-S16
```
...
- Reject or expose marketing claim exceeds scope.
- Reject or expose demo-only certification.
- Reject or expose conditional presented as certified.
