---
name: pm-b31-missing-feature-detector
description: "识别源有目标无、目标仅占位、部分实现和未接线功能. Precision Migration B31 contract; use for this exact assessment, transformation, validation, repair, evidence, or cutover scope."
---

# Missing Feature Detector
## ELMOS runtime binding

- Invoke this repository Skill as `$pm-b31-missing-feature-detector`.
- Immutable source identity: `missing-feature-detector` in `precision-migration-b01-44` (B31).
- Runtime adapter: `differential-test-and-repair`; binding state: `UNAVAILABLE`.
- Resolve and plan with `python3 scripts/precision_migration/runtime.py plan --skill pm-b31-missing-feature-detector`.
...
- Phase: `H 测试、验证与自动修复`
- Version: `1.0.0`
- Tags: batch-31, failed-conversion-repair

## Purpose

识别源有目标无、目标仅占位、部分实现和未接线功能。

本 Skill 属于精密语言转换系统。它必须以**可执行证据**为准，不得用模型自评、源码相似度或“能编译”替代语义与行为验证。

## Use when

- 当前任务直接涉及 `missing-feature-detector` 所描述的能力。
...
1. 评估或生成测试
2. 执行本 Skill 的核心职责：识别源有目标无、目标仅占位、部分实现和未接线功能。
3. 同步环境并双运行
4. 比较输出、状态和副作用
5. 搜索最小反例
6. 驱动修复并执行全量回归
...
```yaml
skill: missing-feature-detector
version: 1.0.0
