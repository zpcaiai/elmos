---
name: pm-b20-view-and-materialized-view-converter
description: "转换普通视图、物化视图、刷新策略、权限和依赖. Precision Migration B20 contract; use for this exact assessment, transformation, validation, repair, evidence, or cutover scope."
---

# View And Materialized View Converter
## ELMOS runtime binding

- Invoke this repository Skill as `$pm-b20-view-and-materialized-view-converter`.
- Immutable source identity: `view-and-materialized-view-converter` in `precision-migration-b01-44` (B20).
- Runtime adapter: `database-and-data-route`; binding state: `DECLARED`.
- Resolve and plan with `python3 scripts/precision_migration/runtime.py plan --skill pm-b20-view-and-materialized-view-converter`.
...
- Phase: `G 数据库精密互转`
- Version: `1.0.0`
- Tags: batch-20, database-schema-data-migration

## Purpose

转换普通视图、物化视图、刷新策略、权限和依赖。

本 Skill 属于精密语言转换系统。它必须以**可执行证据**为准，不得用模型自评、源码相似度或“能编译”替代语义与行为验证。

## Use when

- 当前任务直接涉及 `view-and-materialized-view-converter` 所描述的能力。
...
1. 盘点对象与专有能力
2. 执行本 Skill 的核心职责：转换普通视图、物化视图、刷新策略、权限和依赖。
...
```yaml
skill: view-and-materialized-view-converter
version: 1.0.0
