...
description: "Run FRT-0603 React Architecture Synthesizer for FRT G06 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-0603"
  source_name: "frt-0603-react-architecture-synthesizer"
  source_sha256: "sha256:faa0b672870ff211a39fc3ae7e9a24e142db4571f9a0a7608f5ae3432a7c96a3"
  batch: "G06"
...
- Runtime catalog key: `FRT-0603` / `frt-0603-react-architecture-synthesizer`.
...
# FRT-0603 — React Architecture Synthesizer

## Objective

在G06范围内实现 **React Architecture Synthesizer**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：按目标生态生成模块、路由、状态、DI、配置和Build边界; 禁止机械复制源目录结构; 锁定目标版本、依赖和工具链。

## When to Use

- 实现或修改与 **React Architecture Synthesizer** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-0603Result`：版本化、带状态和诊断的结构化结果。
- `FRT-0603EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-0603FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-0603CertificateFragment`：供G06批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g06/frt-0603-react-architecture-synthesizer/
packages/runtime/g06/frt-0603-react-architecture-synthesizer/
services/control-plane/g06/frt-0603-react-architecture-synthesizer/
apps/web-console/src/features/g06/frt-0603-react-architecture-synthesizer/
apps/admin-console/src/features/g06/frt-0603-react-architecture-synthesizer/
tests/g06/frt-0603-react-architecture-synthesizer/
```
...
```text
POST /v1/skills/frt-0603/runs
GET  /v1/skills/frt-0603/runs/{run_id}
POST /v1/skills/frt-0603/runs/{run_id}/verify
GET  /v1/skills/frt-0603/runs/{run_id}/findings
GET  /v1/skills/frt-0603/runs/{run_id}/evidence
```
