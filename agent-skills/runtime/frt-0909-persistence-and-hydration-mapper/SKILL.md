...
description: "Run FRT-0909 Persistence and Hydration Mapper for FRT G09 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-0909"
  source_name: "frt-0909-persistence-and-hydration-mapper"
  source_sha256: "sha256:00a69ab7b0b51884cc893ccfddd6e0f8e9a39d514cc3cfb151dcd137251eca71"
  batch: "G09"
...
- Runtime catalog key: `FRT-0909` / `frt-0909-persistence-and-hydration-mapper`.
...
# FRT-0909 — Persistence and Hydration Mapper

## Objective

在G09范围内实现 **Persistence and Hydration Mapper**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：区分Local、Server、Global、Persistent和Derived State; 保持Effect触发、Cleanup、Lifecycle和Evaluation Order; 传播Cancellation并防止Stale Result Commit。

## When to Use

- 实现或修改与 **Persistence and Hydration Mapper** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-0909Result`：版本化、带状态和诊断的结构化结果。
- `FRT-0909EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-0909FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-0909CertificateFragment`：供G09批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g09/frt-0909-persistence-and-hydration-mapper/
packages/runtime/g09/frt-0909-persistence-and-hydration-mapper/
services/control-plane/g09/frt-0909-persistence-and-hydration-mapper/
apps/web-console/src/features/g09/frt-0909-persistence-and-hydration-mapper/
apps/admin-console/src/features/g09/frt-0909-persistence-and-hydration-mapper/
tests/g09/frt-0909-persistence-and-hydration-mapper/
```
...
```text
POST /v1/skills/frt-0909/runs
GET  /v1/skills/frt-0909/runs/{run_id}
POST /v1/skills/frt-0909/runs/{run_id}/verify
GET  /v1/skills/frt-0909/runs/{run_id}/findings
GET  /v1/skills/frt-0909/runs/{run_id}/evidence
```
