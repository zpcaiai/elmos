...
- Invoke this repository Skill as `$pm-b09-ui-state-machine-ir`.
- Immutable source identity: `ui-state-machine-ir` in `precision-migration-b01-44` (B09).
- Runtime adapter: `semantic-recovery-and-ir`; binding state: `UNAVAILABLE`.
- Resolve and plan with `python3 scripts/precision_migration/runtime.py plan --skill pm-b09-ui-state-machine-ir`.
...
表达 Loading、Success、Failure、Refresh、Submitting 等 UI 状态与事件转换。

本 Skill 属于精密语言转换系统。它必须以**可执行证据**为准，不得用模型自评、源码相似度或“能编译”替代语义与行为验证。

## Use when

- 当前任务直接涉及 `ui-state-machine-ir` 所描述的能力。
...
1. 定义可观察边界
2. 执行本 Skill 的核心职责：表达 Loading、Success、Failure、Refresh、Submitting 等 UI 状态与事件转换。
3. 将源语义归一化到受限模型
4. 保留不可丢失信息
5. 标记近似与不支持语义
