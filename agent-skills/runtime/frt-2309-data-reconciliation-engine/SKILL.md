...
description: "Run FRT-2309 Data Reconciliation Engine for FRT G23 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-2309"
  source_name: "frt-2309-data-reconciliation-engine"
  source_sha256: "sha256:663858dd34e901f368437d1df5523909e6019ba0f156f545f5cf1d8c5df282f1"
  batch: "G23"
...
- Runtime catalog key: `FRT-2309` / `frt-2309-data-reconciliation-engine`.
...
在G23范围内实现 **Data Reconciliation Engine**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：字段级Data Contract、Authority和Owner; Source-to-Sink与字段级Lineage; Write→Transaction→Outbox→Event→Projection→Cache→Read完整路径。

## When to Use

- 实现或修改与 **Data Reconciliation Engine** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-2309Result`：版本化、带状态和诊断的结构化结果。
- `FRT-2309EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-2309FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-2309CertificateFragment`：供G23批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g23/frt-2309-data-reconciliation-engine/
packages/runtime/g23/frt-2309-data-reconciliation-engine/
services/control-plane/g23/frt-2309-data-reconciliation-engine/
apps/web-console/src/features/g23/frt-2309-data-reconciliation-engine/
apps/admin-console/src/features/g23/frt-2309-data-reconciliation-engine/
tests/g23/frt-2309-data-reconciliation-engine/
```
...
```text
POST /v1/skills/frt-2309/runs
GET  /v1/skills/frt-2309/runs/{run_id}
POST /v1/skills/frt-2309/runs/{run_id}/verify
GET  /v1/skills/frt-2309/runs/{run_id}/findings
GET  /v1/skills/frt-2309/runs/{run_id}/evidence
```
