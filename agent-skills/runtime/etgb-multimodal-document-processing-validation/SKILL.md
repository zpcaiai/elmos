---
name: etgb-multimodal-document-processing-validation
description: Validate all Multimodal File and Document Processing capabilities through the production adapter, independent Oracles and fail-closed release evidence. Repository-owned ETGB execution is available through the local runtime; external production evidence remains explicit.
metadata:
  ...
  source_skill: multimodal-document-processing-validation
...
---
name: multimodal-document-processing-validation
description: Validate all declared Multimodal File and Document Processing capabilities with executable positive, boundary, adversarial, concurrency, recovery and evidence-backed release tests.
---

# Multimodal File and Document Processing Validation

## Purpose

Use this Skill to plan, execute and certify the `multimodal-document-processing` product domain. It owns **55 declared capabilities** (30 P0, 20 P1, 5 P2) across 4 execution contexts and four mandatory variants.

## Inputs

- Frozen release candidate and feature-registry revision.
- Tenant, project, environment authority, budget and fencing context.
- Concrete cases from `suites/full-product.jsonl` whose business line is `multimodal-document-processing`.
...
2. Provision an isolated context for: pdf-word-text, image, audio, mixed-bundle.
...
5. Apply the domain Oracle `multimodal-extraction-provenance-safety-and-quality` plus security, state, audit and unsupported-disclosure Oracles.
...
Representative capabilities include: Pdf Text Extraction, Pdf Page Anchor Preservation, Pdf Table Extraction, Pdf Code Block Preservation, Docx Text And Heading Extraction, Markdown Structure Preservation, Plain Text Encoding, Image Metadata And Content Analysis, Audio Transcription, Audio Language Detection, Mime Sniffing And Extension Mismatch, Virus And Malware Scan. The complete source of truth is `matrices/full-product.yaml`; no prose list may override it.
...
Route execution through `external-multimodal-processing-harness` under `integrations/harness/full-product-adapters.yaml`. An adapter stub or skipped case is implementation progress, not product certification.
