---
name: elmos-polyglot-build-toolchain-orchestrator
description: Provision and execute hermetic Maven, Gradle, pip/uv, npm/pnpm, dotnet, Go and Cargo toolchains with exact locks, caches, sandboxes and reproducible logs.
version: 4.0.0
...
# Polyglot Build Toolchain Orchestrator
...
- Resolve exact compiler/build/package-manager versions
- Run builds in isolated reproducible environments
- Use content-addressed caches with semantic invalidation
- Capture network and dependency acquisition evidence
- Resume failed multi-module builds safely
...
- `toolchain-lock.json`
- `build-execution-journal.jsonl`
- `toolchain-cache-manifest.json`
- `build-reproducibility-report.json`
...
- **dependency confusion** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **mutable registry artifact** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **cache cross-tenant contamination** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **ambient host tool used** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **network fetch not captured in provenance** — must have a preventive control, a detective signal, a negative fixture and a response action.
