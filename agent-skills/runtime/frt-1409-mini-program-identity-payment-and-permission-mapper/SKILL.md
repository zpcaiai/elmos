...
description: "Run FRT-1409 Mini Program Identity Payment and Permission Mapper for FRT G14 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-1409"
  source_name: "frt-1409-mini-program-identity-payment-and-permission-mapper"
  source_sha256: "sha256:b84488d6fd99b7b2bc2725052e50d47957eee015dd31c33d9c666367164497de"
  batch: "G14"
  source_version: "1.0.0"
  source_risk: "critical-by-default"
  source_certificate_family: "WM"
...
- Runtime catalog key: `FRT-1409` / `frt-1409-mini-program-identity-payment-and-permission-mapper`.
...
# FRT-1409 — Mini Program Identity Payment and Permission Mapper

## Objective

在G14范围内实现 **Mini Program Identity Payment and Permission Mapper**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：处理WXML/WXSS/setData/Page/Component与Web组件模型; 处理分包、基础库、授权、支付、Storage和页面栈限制; Web缺失能力和小程序限制显式Gap/Unsupported。

## When to Use

- 实现或修改与 **Mini Program Identity Payment and Permission Mapper** 对应的Contract、Runtime、API、UI、测试或证据链时。
- 相关代码、Schema、Pack、Policy、环境或依赖变化导致既有Evidence失效时。
- G14 Orchestrator生成该Skill的执行义务时。
...
- `SourceArtifactSet`：G1–G12、Web/Mini Program Profiles和设备Fixture。
- `SkillPolicy`：权限、网络、Sandbox、Toolchain、Evidence和停止规则。

## Outputs

- `FRT-1409Result`：版本化、带状态和诊断的结构化结果。
- `FRT-1409EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-1409FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-1409CertificateFragment`：供G14批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g14/frt-1409-mini-program-identity-payment-and-permission-mapper/
packages/runtime/g14/frt-1409-mini-program-identity-payment-and-permission-mapper/
services/control-plane/g14/frt-1409-mini-program-identity-payment-and-permission-mapper/
apps/web-console/src/features/g14/frt-1409-mini-program-identity-payment-and-permission-mapper/
apps/admin-console/src/features/g14/frt-1409-mini-program-identity-payment-and-permission-mapper/
tests/g14/frt-1409-mini-program-identity-payment-and-permission-mapper/
```
...
```text
POST /v1/skills/frt-1409/runs
GET  /v1/skills/frt-1409/runs/{run_id}
POST /v1/skills/frt-1409/runs/{run_id}/verify
GET  /v1/skills/frt-1409/runs/{run_id}/findings
GET  /v1/skills/frt-1409/runs/{run_id}/evidence
```
...
- The parent G14 Orchestrator can consume the result and independently decide certification.
