---
name: elmos-inference-cache-prefix-kv-consistency-controller
description: Govern prompt/result/prefix/KV caches with exact key semantics, tenant isolation, model/version binding, invalidation and quality evidence.
version: 4.0.0
...
# Inference Cache, Prefix and KV Consistency Controller
...
- `InferenceCachePrefixAndKVConsistencyControllerProfile`
- `InferenceCachePrefixAndKVConsistencyControllerPlan`
- `InferenceCachePrefixAndKVConsistencyControllerRun`
- `InferenceCachePrefixAndKVConsistencyControllerDecision`

Required semantic capabilities:

- derive canonical cache keys and context boundaries
- bind cache entries to model, prompt, tool and policy digests
- verify tenant and residency isolation
- coordinate KV offload and eviction
- measure hit rate without stale semantic reuse
...
- `artifacts/elmos-inference-cache-prefix-kv-consistency-controller/profile.yaml`
- `artifacts/elmos-inference-cache-prefix-kv-consistency-controller/plan.json`
- `artifacts/elmos-inference-cache-prefix-kv-consistency-controller/result.json`
- `artifacts/elmos-inference-cache-prefix-kv-consistency-controller/evidence/`
...
- **cross-tenant cache hit** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **model drift reuses old completion** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **tool authorization omitted from key** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **KV cache restored to wrong session** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **cache optimization changes answer semantics** — must have a preventive control, a detective signal, a negative fixture and a response action.
...
- native scenario: derive canonical cache keys and context boundaries
- native scenario: bind cache entries to model, prompt, tool and policy digests
- native scenario: verify tenant and residency isolation
- native scenario: coordinate KV offload and eviction
- native scenario: measure hit rate without stale semantic reuse
