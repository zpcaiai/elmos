---
name: elmos-rollback-dual-run-planner
description: "Design reversible cutover, shadow, canary, dual-read/write, CDC, comparison, rollback, and decommission workflows."
version: 1.0.0
skill_id: ELMOS-POLY-045
layer: release
risk: critical
readiness: not-run
dependencies:
  - "elmos-security-supply-chain-validator"
triggers:
  - "Use when implementing or executing `elmos-rollback-dual-run-planner`."
  - "Use when the current DAG node requires the release capability."
outputs:
  - "cutover-plan.yaml"
  - "dual-run-plan.yaml"
  - "rollback-plan.yaml"
  - "decommission-gates.json"
---

# Rollback Dual Run Planner

## Objective

Design reversible cutover, shadow, canary, dual-read/write, CDC, comparison, rollback, and decommission workflows.
...
1. Define traffic, data, event, job, and UI cutover units.
2. Select shadow, mirror, canary, strangler, blue-green, or direct cutover based on side effects and rollback.
3. Prevent duplicate external side effects during replay and dual run.
4. Define compatibility windows for schemas, events, caches, sessions, and clients.
5. Set measurable promotion and rollback triggers.
6. Test rollback with representative data and in-flight work.
7. Define ownership, communications, freeze, recovery, and decommission steps.
8. Require approval for irreversible data or client changes.

## Implementation Contract

- Cutover and rollback triggers are measurable and machine-readable.
- Dual-run plans suppress duplicate external side effects.
...
- [ ] Rollback drill completes within declared objectives..
- [ ] Promotion and rollback triggers are machine-evaluable..
- [ ] Decommission waits for compatibility evidence..
