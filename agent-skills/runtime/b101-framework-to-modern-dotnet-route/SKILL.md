---
name: b101-framework-to-modern-dotnet-route
description: "Use when ELMOS must Plan and implement staged migration from .NET Framework to supported modern .NET targets. Triggers: .NET Framework to Modern .NET Route, Three Golden Route Packs, framework migration, project format, API compatibility, Windows dependencies."
metadata:
  id: B101-S08
...
# B101-S08 — .NET Framework to Modern .NET Route

## Objective

Plan and implement staged migration from .NET Framework to supported modern .NET targets.
...
Use this Skill when the approved task explicitly requires **.NET Framework to Modern .NET Route** or when the capability graph selects it as part of a minimal dependency closure.
...
- Windows-only dependencies explicit.
- multi-target bridge supported.
- API gaps tracked.
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b101-framework-to-modern-dotnet-route/SKILL.md --out /tmp/framework_to_modern_dotnet_route.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B101-S08
```
...
- Reject or expose silent platform change.
- Reject or expose unsupported API stubbed.
- Reject or expose binding redirect loss.
