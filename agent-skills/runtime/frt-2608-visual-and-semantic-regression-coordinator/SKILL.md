...
description: "Run FRT-2608 Visual and Semantic Regression Coordinator for FRT G26 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-2608"
  source_name: "frt-2608-visual-and-semantic-regression-coordinator"
  source_sha256: "sha256:2a5f870d71519d4ea3c22d8414fc7ac58bba8e735f4b6fb7708b9fb143321fcb"
  batch: "G26"
  source_version: "1.0.0"
  source_risk: "critical-by-default"
  source_certificate_family: "RQ"
...
- Runtime catalog key: `FRT-2608` / `frt-2608-visual-and-semantic-regression-coordinator`.
...
在G26范围内实现 **Visual and Semantic Regression Coordinator**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：Requirement/Acceptance/State Machine派生Test Obligation; Unit/Component/Contract/Integration/Journey分层协调; Differential、Mutation、Visual/Semantic、Security和Performance Regression。

## When to Use

- 实现或修改与 **Visual and Semantic Regression Coordinator** 对应的Contract、Runtime、API、UI、测试或证据链时。
- 相关代码、Schema、Pack、Policy、环境或依赖变化导致既有Evidence失效时。
- G26 Orchestrator生成该Skill的执行义务时。
...
- `SourceArtifactSet`：G21–G25所有Contract、Journey、Certificate和Evidence, Release Diff、Semantic Impact Graph、Dependency/Data Lineage。
- `SkillPolicy`：权限、网络、Sandbox、Toolchain、Evidence和停止规则。

## Outputs

- `FRT-2608Result`：版本化、带状态和诊断的结构化结果。
- `FRT-2608EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-2608FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-2608CertificateFragment`：供G26批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g26/frt-2608-visual-and-semantic-regression-coordinator/
packages/runtime/g26/frt-2608-visual-and-semantic-regression-coordinator/
services/control-plane/g26/frt-2608-visual-and-semantic-regression-coordinator/
apps/web-console/src/features/g26/frt-2608-visual-and-semantic-regression-coordinator/
apps/admin-console/src/features/g26/frt-2608-visual-and-semantic-regression-coordinator/
tests/g26/frt-2608-visual-and-semantic-regression-coordinator/
```
...
```text
POST /v1/skills/frt-2608/runs
GET  /v1/skills/frt-2608/runs/{run_id}
POST /v1/skills/frt-2608/runs/{run_id}/verify
GET  /v1/skills/frt-2608/runs/{run_id}/findings
GET  /v1/skills/frt-2608/runs/{run_id}/evidence
```
...
- The parent G26 Orchestrator can consume the result and independently decide certification.
