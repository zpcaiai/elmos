---
name: cursor-execution-context
description: 提供树遍历上下文、跨 visitor 状态共享和生命周期消息总线。 Use for this exact ELMOS Spring Golden Route v2 contract; imported specifications are not runtime or certification evidence.
...
metadata:
  ...
  source_id: FOUNDATION-01-cursor-execution-context
  source_name: cursor-execution-context
  source_path: skills/cursor-execution-context/SKILL.md
  source_sha256: sha256:a9bf6360048ab3ce2681adfade9eafe1f6329ba29f1fa42ad9f034a291cf2be5
  source_contract_sha256: sha256:8921f093cfee57138733df422efd5e0e579dcd69f267ad84d2b90d32ccd6bbb8
...
提供树遍历上下文、跨 visitor 状态共享和生命周期消息总线。

## Use when

在 Elmos 需要实现或升级 **Cursor & Execution Context** 相关能力时使用；它不是仅供解释的提示词，而是实现、测试与验收契约。

## Capabilities to implement

- Cursor 可访问父链、根级共享状态和当前节点语义
- ExecutionContext 管理运行级状态、错误、缓存和数据表 sink
- 每 cycle 创建安全 root context，避免静态 visitor 泄漏状态
- 共享 type/template cache 但显式清理运行期消息
...
- [ ] 定义 `cursor-execution-context` 的 public interface、input/output/error schemas。
- [ ] 建立 `cursor-execution-context` 的 capability/feature flags 与版本字段。
...
- [ ] 至少覆盖并验收：Cursor 可访问父链、根级共享状态和当前节点语义；以及 ExecutionContext 管理运行级状态、错误、缓存和数据表 sink。
...
- Machine contract: `references/contract.json` (`sha256:8921f093cfee57138733df422efd5e0e579dcd69f267ad84d2b90d32ccd6bbb8`).
