---
name: b99-plan-to-dag-compiler
description: "Use when ELMOS must Compile selected executable Skill contracts into a deterministic dependency-aware execution DAG. Triggers: Plan to DAG Compiler, Durable Runtime Kernel Pack, DAG compilation, data dependencies, approval nodes, effect boundaries."
metadata:
  id: B99-S02
...
# B99-S02 — Plan to DAG Compiler

## Objective

Compile selected executable Skill contracts into a deterministic dependency-aware execution DAG.
...
Use this Skill when the approved task explicitly requires **Plan to DAG Compiler** or when the capability graph selects it as part of a minimal dependency closure.
...
Package dependencies: `B99-S01`, `B98-S06`, `B98-S13`
...
- acyclic unless bounded loop construct.
- stable node IDs.
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b99-plan-to-dag-compiler/SKILL.md --out /tmp/plan_to_dag_compiler.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B99-S02
```
...
- Reject or expose cycle smuggling.
- Reject or expose missing approval node.
- Reject or expose effect dependency omitted.
