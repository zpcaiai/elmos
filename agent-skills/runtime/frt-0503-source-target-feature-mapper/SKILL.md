...
description: "Run FRT-0503 Source Target Feature Mapper for FRT G05 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-0503"
  source_name: "frt-0503-source-target-feature-mapper"
  source_sha256: "sha256:3156767b24e3d590e45be082d2cc5ef78ecef16060104f5d1b70e77f5911c870"
  batch: "G05"
...
- Runtime catalog key: `FRT-0503` / `frt-0503-source-target-feature-mapper`.
...
# FRT-0503 — Source Target Feature Mapper

## Objective

在G05范围内实现 **Source Target Feature Mapper**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：比较源语义与目标能力并生成Gap Register; 区分Native、Shim、Product Decision、Manual与Unsupported; 估算正确率、耗时、风险、数据和测试影响。

## When to Use

- 实现或修改与 **Source Target Feature Mapper** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-0503Result`：版本化、带状态和诊断的结构化结果。
- `FRT-0503EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-0503FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-0503CertificateFragment`：供G05批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g05/frt-0503-source-target-feature-mapper/
packages/runtime/g05/frt-0503-source-target-feature-mapper/
services/control-plane/g05/frt-0503-source-target-feature-mapper/
apps/web-console/src/features/g05/frt-0503-source-target-feature-mapper/
apps/admin-console/src/features/g05/frt-0503-source-target-feature-mapper/
tests/g05/frt-0503-source-target-feature-mapper/
```
...
```text
POST /v1/skills/frt-0503/runs
GET  /v1/skills/frt-0503/runs/{run_id}
POST /v1/skills/frt-0503/runs/{run_id}/verify
GET  /v1/skills/frt-0503/runs/{run_id}/findings
GET  /v1/skills/frt-0503/runs/{run_id}/evidence
```
