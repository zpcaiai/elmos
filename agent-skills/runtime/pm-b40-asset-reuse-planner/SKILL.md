...
- Invoke this repository Skill as `$pm-b40-asset-reuse-planner`.
- Immutable source identity: `asset-reuse-planner` in `precision-migration-b01-44` (B40).
- Runtime adapter: `skill-and-project-synthesis`; binding state: `UNAVAILABLE`.
- Resolve and plan with `python3 scripts/precision_migration/runtime.py plan --skill pm-b40-asset-reuse-planner`.
...
- Version: `1.0.0`
- Tags: batch-40, product-entry-modes

## Purpose

决定源码、测试、Schema、资源、协议、组件和部署资产的复用、适配或重建。

本 Skill 属于精密语言转换系统。它必须以**可执行证据**为准，不得用模型自评、源码相似度或“能编译”替代语义与行为验证。

## Use when

- 当前任务直接涉及 `asset-reuse-planner` 所描述的能力。
...
2. 执行本 Skill 的核心职责：决定源码、测试、Schema、资源、协议、组件和部署资产的复用、适配或重建。
3. 编译Skills与架构蓝图
4. 生成/复用全部构件
5. 构建并执行验收/安全/完整性门禁
6. 输出项目、Runbook和证据
