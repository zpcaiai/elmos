---
name: b101-python2-legacy-upgrade-route
description: "Use when ELMOS must Upgrade Python 2 or legacy Python code, packaging and syntax with semantic and encoding fidelity. Triggers: Python 2 and Legacy Upgrade Route, Three Golden Route Packs, Python 2 to 3, encoding, packaging, stdlib changes."
metadata:
  id: B101-S12
...
# B101-S12 — Python 2 and Legacy Upgrade Route

## Objective

Upgrade Python 2 or legacy Python code, packaging and syntax with semantic and encoding fidelity.
...
Use this Skill when the approved task explicitly requires **Python 2 and Legacy Upgrade Route** or when the capability graph selects it as part of a minimal dependency closure.
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b101-python2-legacy-upgrade-route/SKILL.md --out /tmp/python2_legacy_upgrade_route.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B101-S12
```
...
- Reject or expose decode errors hidden.
- Reject or expose integer division drift.
- Reject or expose 2to3 output treated as certified.
