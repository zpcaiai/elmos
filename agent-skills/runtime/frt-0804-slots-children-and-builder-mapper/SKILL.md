...
description: "Run FRT-0804 Slots Children and Builder Mapper for FRT G08 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-0804"
  source_name: "frt-0804-slots-children-and-builder-mapper"
  source_sha256: "sha256:d21e00285fde6e6ee59831ad7ee76a79ad5ca908b4702f5a043daa568d6d18bd"
  batch: "G08"
...
- Runtime catalog key: `FRT-0804` / `frt-0804-slots-children-and-builder-mapper`.
...
# FRT-0804 — Slots Children and Builder Mapper

## Objective

在G08范围内实现 **Slots Children and Builder Mapper**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：保持Props默认值、事件时机、Slot/Children/Builder结构; 保持Key、Ref、Widget Identity和状态保留语义; 正确降低Hooks、Composable、Context和局部状态。

## When to Use

- 实现或修改与 **Slots Children and Builder Mapper** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-0804Result`：版本化、带状态和诊断的结构化结果。
- `FRT-0804EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-0804FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-0804CertificateFragment`：供G08批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g08/frt-0804-slots-children-and-builder-mapper/
packages/runtime/g08/frt-0804-slots-children-and-builder-mapper/
services/control-plane/g08/frt-0804-slots-children-and-builder-mapper/
apps/web-console/src/features/g08/frt-0804-slots-children-and-builder-mapper/
apps/admin-console/src/features/g08/frt-0804-slots-children-and-builder-mapper/
tests/g08/frt-0804-slots-children-and-builder-mapper/
```
...
```text
POST /v1/skills/frt-0804/runs
GET  /v1/skills/frt-0804/runs/{run_id}
POST /v1/skills/frt-0804/runs/{run_id}/verify
GET  /v1/skills/frt-0804/runs/{run_id}/findings
GET  /v1/skills/frt-0804/runs/{run_id}/evidence
```
