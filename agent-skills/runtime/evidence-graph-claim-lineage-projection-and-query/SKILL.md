---
name: evidence-graph-claim-lineage-projection-and-query
description: 构建可重建证据图，区分 Claim、Observation、Verification 和 Inference，并支持影响分析。
metadata:
  elmos_batch: "37A"
  elmos_skill_id: "ART37A010"
  pack_version: "3.7-complete"
---

# ART37A010 — Evidence Graph、Claim、Lineage、Projection 与 Query

## Objective

构建可重建证据图，区分 Claim、Observation、Verification 和 Inference，并支持影响分析。
...
- 建立 Artifact、Attestation、Producer、Runner、Sandbox、Task、PR、Check、Pack 等节点。
- 所有 Edge 必须带 Supporting Evidence；Inference 保存 Rule/Confidence。
- 通过 Outbox 投影至 Neo4j/RDF/PostgreSQL Graph。
- 提供 Lineage、PR Evidence、Revoked Signer/Corrupted Object Impact 查询。
...
- graphIsRebuildable
- everyEdgeHasSupportingEvidence
- inferenceCannotBecomeProducerSignedClaim
- crossTenantGraphTraversalIsRejected
