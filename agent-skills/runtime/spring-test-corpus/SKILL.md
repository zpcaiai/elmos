---
name: spring-test-corpus
description: 映射现有测试到能力并建立独立 holdout、negative 和业务场景。 Use for this exact ELMOS Spring Golden Route v2 contract; imported specifications are not runtime or certification evidence.
...
metadata:
  ...
  source_id: ELMOS-GR-040
  source_name: spring-test-corpus
  source_path: skills/spring-test-corpus/SKILL.md
  source_sha256: sha256:6a8ed3e5d5fb469c575e1f35150632a0230fb5e79ea76060ac550a6783f81d13
  source_contract_sha256: sha256:ed90c021a74dd9a60dc951d0f6292b50d2fb3f94e849e1215d3316cc5d112757
...
# Test Mapping & Behavior Corpus

## Objective

映射现有测试到能力并建立独立 holdout、negative 和业务场景。

## Scope and capabilities

- disabled/skipped/flaky 识别
- Implementation-test map
- Holdout 不对 Repair Agent 可见
- 独立 oracle
- P0 覆盖
...
- [ ] 定义 `spring-test-corpus` 的 public interface、输入/输出/Error Schema 和版本策略。
- [ ] 实现 `spring-test-corpus` 的 production adapter/service，不把业务状态只保存在 Prompt 或内存对象中。
...
- [ ] 至少生成：test-map.json, behavior-corpus, holdout-set.json, test-integrity.json。
...
- [ ] 本 Skill 的 required outputs 均由真实目标仓库执行产生：test-map.json, behavior-corpus, holdout-set.json, test-integrity.json。
...
- Machine contract: `references/contract.json` (`sha256:ed90c021a74dd9a60dc951d0f6292b50d2fb3f94e849e1215d3316cc5d112757`).
