---
name: b97-conflict-overlap-detector
description: "Use when ELMOS must Detect duplicate, overlapping, contradictory and mutually exclusive Skills or policies before runtime selection. Triggers: Conflict and Overlap Detector, Canonical Capability Graph Consolidation Pack, semantic overlap, write-set conflict, policy contradiction, mutual exclusion."
metadata:
  id: B97-S06
...
# B97-S06 — Conflict and Overlap Detector

## Objective

Detect duplicate, overlapping, contradictory and mutually exclusive Skills or policies before runtime selection.
...
Use this Skill when the approved task explicitly requires **Conflict and Overlap Detector** or when the capability graph selects it as part of a minimal dependency closure.
...
- write-set collisions visible.
- unresolved conflicts block composition.
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b97-conflict-overlap-detector/SKILL.md --out /tmp/conflict_overlap_detector.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B97-S06
```
...
- Reject or expose false merge.
- Reject or expose hidden output collision.
- Reject or expose conflicting security rules.
