---
name: elmos-adaptive-cache-policy-orchestrator
description: Select and tune cache policies off the hot path using workload fingerprints, objective profiles, hysteresis, shadow evaluation, and safe rollback.
version: 1.1.0
...
# Adaptive Cache Policy Orchestrator

## Outcome

Choose the best deployable cache policy per tier, artifact class, and workload regime using measured ELMOS traces, while avoiding oscillation and maintaining a strong fixed-policy fallback.
...
- Dependency skills have passed: **elmos-sota-cache-policy-portfolio, elmos-cost-aware-cache-admission, elmos-cache-trace-replay-simulator**.
...
- Policy selection runs off the hit critical path and is bounded by explicit CPU and memory budgets.
- Switching occurs only at safe epochs or namespace boundaries with hysteresis and state-migration rules.
- Every selected policy has a reason code, objective profile, trace/model digest, confidence, and fallback.
- Low-confidence, out-of-distribution, drifting, or telemetry-degraded workloads fall back to a pinned fixed policy.
- Selector improvements must hold on a final untouched trace window and across important tenant/stage cohorts.
...
1. Compute compact workload fingerprints: one-hit ratio, reuse-distance quantiles, object-size distribution, size/reuse correlation, cost skew, stage mix, cacheability, concurrency, and remote-latency regime.
2. Implement a rule-based selector first, then an optional SCION-style off-path linear or tree selector among SIEVE, S3-FIFO, W-TinyLFU, size-aware TinyLFU, and GDSF.
3. Add Merlin-inspired fine-grained locality characterization only when its overhead and state bounds pass benchmark gates.
4. Use multi-objective profiles for development speed, token-cost minimization, byte/network minimization, and production certification retention.
5. Add shadow policies fed from the same request stream, periodic counterfactual replay, hysteresis, minimum dwell time, and automatic rollback.
6. Persist policy epochs and migrate or intentionally reset policy state with auditable reason codes.
