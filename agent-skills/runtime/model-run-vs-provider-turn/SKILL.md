---
name: model-run-vs-provider-turn
description: 高层完整 tool loop 与低层单 provider turn 两套 API 明确分工。 Use for this exact ELMOS Spring Golden Route v2 contract; imported specifications are not runtime or certification evidence.
...
metadata:
  ...
  source_id: FOUNDATION-09-model-run-vs-provider-turn
  source_name: model-run-vs-provider-turn
  source_path: skills/model-run-vs-provider-turn/SKILL.md
  source_sha256: sha256:795779992854ad0cb9ca37f060f1459bbf18f6f7282df98ded021e58e9b78a9e
  source_contract_sha256: sha256:2050402b7968008fde76fe2ce17c2f8c1da2fac2734903d4deeae43169ea680c
...
# Model Run vs Provider Turn

## Goal

高层完整 tool loop 与低层单 provider turn 两套 API 明确分工。

## Use when

在 Elmos 需要实现或升级 **Model Run vs Provider Turn** 相关能力时使用；它不是仅供解释的提示词，而是实现、测试与验收契约。

## Capabilities to implement

- 普通短任务可用 complete run
- Elmos durable runtime 使用 one-turn API 自己持久化/settle/continue
- stop condition 和 turn count 显式
- tool 并发有界且 settlement 顺序确定
...
- [ ] 定义 `model-run-vs-provider-turn` 的 public interface、input/output/error schemas。
- [ ] 建立 `model-run-vs-provider-turn` 的 capability/feature flags 与版本字段。
...
- [ ] 至少覆盖并验收：普通短任务可用 complete run；以及 Elmos durable runtime 使用 one-turn API 自己持久化/settle/continue。
...
- Machine contract: `references/contract.json` (`sha256:2050402b7968008fde76fe2ce17c2f8c1da2fac2734903d4deeae43169ea680c`).
