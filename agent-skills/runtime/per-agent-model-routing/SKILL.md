---
name: per-agent-model-routing
description: 不同角色使用不同模型、temperature、reasoning 与成本预算。 Use for this exact ELMOS Spring Golden Route v2 contract; imported specifications are not runtime or certification evidence.
...
metadata:
  ...
  source_id: FOUNDATION-06-per-agent-model-routing
  source_name: per-agent-model-routing
  source_path: skills/per-agent-model-routing/SKILL.md
  source_sha256: sha256:45079ac5a3f42eb3fce8864923e79275a5bb6045610a5b208b2768523e854a1c
  source_contract_sha256: sha256:44072229e0ebb323775cca923194754ff900b37fb3a17cdc6cd4a300cf17ab60
...
不同角色使用不同模型、temperature、reasoning 与成本预算。

## Use when

在 Elmos 需要实现或升级 **Per-Agent Model Routing** 相关能力时使用；它不是仅供解释的提示词，而是实现、测试与验收契约。

## Capabilities to implement

- 模型选择从 Agent 配置继承并可 task override
- 探索/摘要优先便宜模型，架构/复杂修复可升级模型
- 模型切换在 provider-turn safe boundary 生效
- 记录每次路由理由和实际成本
...
- [ ] 定义 `per-agent-model-routing` 的 public interface、input/output/error schemas。
- [ ] 建立 `per-agent-model-routing` 的 capability/feature flags 与版本字段。
...
- [ ] 至少覆盖并验收：模型选择从 Agent 配置继承并可 task override；以及 探索/摘要优先便宜模型，架构/复杂修复可升级模型。
...
- Machine contract: `references/contract.json` (`sha256:44072229e0ebb323775cca923194754ff900b37fb3a17cdc6cd4a300cf17ab60`).
