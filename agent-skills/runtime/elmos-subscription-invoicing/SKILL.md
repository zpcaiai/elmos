---
name: elmos-subscription-invoicing
description: "Implement billing periods, subscription charges, included-credit grants, proration, invoice lines, taxes, credit notes, renewal, dunning handoff, and immutable invoice snapshots. Use for recurring plan billing and invoice generation; not for payment gateway callbacks or task usage capture."
metadata:
  source_package: "elmos-pricing-billing-skills"
  source_version: "1.0.0"
  source_skill_sha256: "sha256:d782897b265fec0364f4bc937553f5a6ba75f2bd3be14b701c337ce322e085a9"
  package_namespace: "elmos.pricing-billing.v1"
  package_local_batches: "B24,B25,B26"
...
将套餐周期、席位、包含额度和按量超额形成可审计发票，同时保持历史价格与税务快照不可变。
...
`elmos-plan-catalog-entitlements`, `elmos-credit-wallet-ledger`
...
1. 定义 billing anchor、周期、时区、账期边界和续费触发。
2. 生成订阅、席位、超额用量、项目里程碑、折扣和税费发票行。
3. 使用版本化价格、税务输入和合同快照计算 proration。
4. 发票 draft 可重算，finalized 后不可改；修正使用 credit note 或 replacement。
5. 包含额度发放与续费扣款使用不同幂等键并确保 exactly-once business effect。
6. 支持月付、年付、试用转正、周期中升级和周期末降级。
7. 对企业 postpaid 支持账期与信用限额。
8. 将应收账款、递延收入和收入确认事件送入账本/财务适配器。
9. 失败付款进入 dunning，不直接破坏历史发票。
10. 运行时区、月末、闰年、税舍入和重跑测试。
...
- finalized invoice 不可原地修改；所有修正必须通过新的财务文档。
- 续费重试不得重复扣款或重复发放包含额度。
...
- All assigned requirements (EB-09-001, EB-09-002, EB-09-003, EB-09-004, EB-09-005, EB-09-006, EB-09-007, EB-09-008, EB-09-009, EB-09-010) have a five-state classification.
...
This is imported guidance from `elmos-pricing-billing-skills` `1.0.0`. Its source Skill SHA-256 is `d782897b265fec0364f4bc937553f5a6ba75f2bd3be14b701c337ce322e085a9`.
