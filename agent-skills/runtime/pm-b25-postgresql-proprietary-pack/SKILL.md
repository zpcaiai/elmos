...
- Invoke this repository Skill as `$pm-b25-postgresql-proprietary-pack`.
- Immutable source identity: `batch-25-postgresql-proprietary-pack` in `precision-migration-b01-44` (B25).
- Runtime adapter: `database-and-data-route`; binding state: `DECLARED`.
- Resolve and plan with `python3 scripts/precision_migration/runtime.py plan --skill pm-b25-postgresql-proprietary-pack`.
...
- 当任务涉及 **postgresql-plpgsql** 时，调用 `../pm-b25-postgresql-plpgsql/SKILL.md`。
- 当任务涉及 **postgresql-extension** 时，调用 `../pm-b25-postgresql-extension/SKILL.md`。
- 当任务涉及 **postgresql-array-range-composite** 时，调用 `../pm-b25-postgresql-array-range-composite/SKILL.md`。
- 当任务涉及 **postgresql-jsonb** 时，调用 `../pm-b25-postgresql-jsonb/SKILL.md`。
- 当任务涉及 **postgresql-returning** 时，调用 `../pm-b25-postgresql-returning/SKILL.md`。
- 当任务涉及 **postgresql-on-conflict** 时，调用 `../pm-b25-postgresql-on-conflict/SKILL.md`。
- 当任务涉及 **postgresql-sequence-and-identity** 时，调用 `../pm-b25-postgresql-sequence-and-identity/SKILL.md`。
- 当任务涉及 **postgresql-partial-expression-index** 时，调用 `../pm-b25-postgresql-partial-expression-index/SKILL.md`。
- 当任务涉及 **postgresql-materialized-view** 时，调用 `../pm-b25-postgresql-materialized-view/SKILL.md`。
- 当任务涉及 **postgresql-listen-notify** 时，调用 `../pm-b25-postgresql-listen-notify/SKILL.md`。
- 当任务涉及 **postgresql-logical-replication** 时，调用 `../pm-b25-postgresql-logical-replication/SKILL.md`。
- 当任务涉及 **postgresql-logical-decoding** 时，调用 `../pm-b25-postgresql-logical-decoding/SKILL.md`。
- 当任务涉及 **postgresql-row-level-security** 时，调用 `../pm-b25-postgresql-row-level-security/SKILL.md`。
- 当任务涉及 **postgresql-foreign-data-wrapper** 时，调用 `../pm-b25-postgresql-foreign-data-wrapper/SKILL.md`。
- 当任务涉及 **postgresql-event-trigger** 时，调用 `../pm-b25-postgresql-event-trigger/SKILL.md`。
- 当任务涉及 **postgresql-custom-operator-and-type** 时，调用 `../pm-b25-postgresql-custom-operator-and-type/SKILL.md`。
