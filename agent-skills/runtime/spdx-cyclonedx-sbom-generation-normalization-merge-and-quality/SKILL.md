---
name: spdx-cyclonedx-sbom-generation-normalization-merge-and-quality
description: 支持 SPDX 3.0.1/2.3 Export、CycloneDX 1.7、生命周期、完整性、合并冲突和质量维度。
metadata:
  elmos_batch: "37A"
  elmos_skill_id: "ART37A008"
  pack_version: "3.7-complete"
---

# ART37A008 — SPDX、CycloneDX SBOM 生成、归一、合并与质量

## Objective

支持 SPDX 3.0.1/2.3 Export、CycloneDX 1.7、生命周期、完整性、合并冲突和质量维度。
...
- 实现 Maven/Gradle/JAR/OCI/JDK/OS/External Vendor SBOM Generator。
- 保存 PURL、Native ID、Version、Hash、Supplier、License、Relationship。
- 区分 Design/Source/Build/Analyzed/Deployed/Runtime Observed Stage。
- 合并保留 Source Provenance，转换输出 Loss Report。
...
- spdx301IsDefaultStableProfile
- missingDependencyIsNotZeroDependency
- mergePreservesSourceProvenance
- partialSbomCannotClaimComplete
