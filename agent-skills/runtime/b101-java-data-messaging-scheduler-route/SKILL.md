---
name: b101-java-data-messaging-scheduler-route
description: "Use when ELMOS must Preserve JPA/JDBC transactions, schema, Kafka/JMS acknowledgment, cache and scheduler semantics. Triggers: Java Data Messaging and Scheduler Route, Three Golden Route Packs, transactions, messaging, cache, scheduler."
metadata:
  id: B101-S05
...
# B101-S05 — Java Data Messaging and Scheduler Route

## Objective

Preserve JPA/JDBC transactions, schema, Kafka/JMS acknowledgment, cache and scheduler semantics.
...
Use this Skill when the approved task explicitly requires **Java Data Messaging and Scheduler Route** or when the capability graph selects it as part of a minimal dependency closure.
...
- transaction propagation compared.
- ack/retry semantics tested.
- cron timezone preserved.
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b101-java-data-messaging-scheduler-route/SKILL.md --out /tmp/java_data_messaging_scheduler_route.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B101-S05
```
...
- Reject or expose lost rollback.
- Reject or expose duplicate message effect.
- Reject or expose in-memory durable job replacement.
