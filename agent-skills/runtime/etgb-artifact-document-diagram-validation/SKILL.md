---
name: etgb-artifact-document-diagram-validation
description: Validate all Diagrams, Documents, Presentations and Delivery Artifacts capabilities through the production adapter, independent Oracles and fail-closed release evidence. Repository-owned ETGB execution is available through the local runtime; external production evidence remains explicit.
metadata:
  ...
  source_skill: artifact-document-diagram-validation
...
---
name: artifact-document-diagram-validation
description: Validate all declared Diagrams, Documents, Presentations and Delivery Artifacts capabilities with executable positive, boundary, adversarial, concurrency, recovery and evidence-backed release tests.
---

# Diagrams, Documents, Presentations and Delivery Artifacts Validation

## Purpose

Use this Skill to plan, execute and certify the `artifact-document-diagram` product domain. It owns **65 declared capabilities** (36 P0, 24 P1, 5 P2) across 4 execution contexts and four mandatory variants.

## Inputs

- Frozen release candidate and feature-registry revision.
- Tenant, project, environment authority, budget and fencing context.
- Concrete cases from `suites/full-product.jsonl` whose business line is `artifact-document-diagram`.
...
2. Provision an isolated context for: analyzed-repository, generated-project, converted-project, modernized-project.
...
5. Apply the domain Oracle `artifact-content-rendering-provenance-version-and-accessibility` plus security, state, audit and unsupported-disclosure Oracles.
...
Representative capabilities include: Architecture Document Generation, Module Documentation, Api Documentation, Data Dictionary, Runbook Generation, Adr Generation, C4 Context Container Component Diagrams, Er Diagram, Data Flow Diagram, Sequence Diagram, Activity And Flowchart, Deployment Diagram. The complete source of truth is `matrices/full-product.yaml`; no prose list may override it.
...
Route execution through `external-artifact-render-harness` under `integrations/harness/full-product-adapters.yaml`. An adapter stub or skipped case is implementation progress, not product certification.
