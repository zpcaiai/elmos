---
name: elmos-transformation-rule-dsl-author
description: "Author versioned, reviewable transformation rules with preconditions, semantic constraints, fixtures, provenance, and rollback metadata."
version: 1.0.0
skill_id: ELMOS-POLY-017
...
triggers:
  - "Use when implementing or executing `elmos-transformation-rule-dsl-author`."
...
# Transformation Rule Dsl Author

## Objective

Author versioned, reviewable transformation rules with preconditions, semantic constraints, fixtures, provenance, and rollback metadata.
...
1. Define match conditions over source AST/CST, Semantic IR, Framework IR, dependencies, and target profile.
2. Specify deterministic rewrite actions and preserved trivia policy.
3. Declare semantic preconditions, forbidden contexts, confidence, and expected losses.
4. Attach positive, negative, idempotency, and conflict fixtures.
5. Define dependency and configuration edits separately from source edits.
6. Version rules and record author, source rationale, applicable versions, and deprecation.
7. Require human review for security-, data-, or behavior-sensitive rules.
8. Publish rules only after fixture and baseline validation.
...
- [ ] Rules are idempotent..
- [ ] Negative fixtures remain unchanged..
- [ ] Rule conflicts are detected..
- [ ] A rule cannot claim semantics it does not verify..
