...
description: "Run FRT-2921 Secure Admin Console Validator for FRT G29 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-2921"
  source_name: "frt-2921-secure-admin-console-validator"
  source_sha256: "sha256:8cbffbcd37ce50f192b9473c3ecceb018a559eb3a224b1cc605a0847462a1a64"
  batch: "G29"
...
- Runtime catalog key: `FRT-2921` / `frt-2921-secure-admin-console-validator`.
...
在G29范围内实现 **Secure Admin Console Validator**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：Asset、Threat、Trust Boundary、Attack Surface; Authentication、Authorization、Object/Field Scope、Tenant Isolation; Session/Token、API、Input、File、SSRF/XSS/Command/Prompt Injection。

## When to Use

- 实现或修改与 **Secure Admin Console Validator** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-2921Result`：版本化、带状态和诊断的结构化结果。
- `FRT-2921EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-2921FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-2921CertificateFragment`：供G29批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g29/frt-2921-secure-admin-console-validator/
packages/runtime/g29/frt-2921-secure-admin-console-validator/
services/control-plane/g29/frt-2921-secure-admin-console-validator/
apps/web-console/src/features/g29/frt-2921-secure-admin-console-validator/
apps/admin-console/src/features/g29/frt-2921-secure-admin-console-validator/
tests/g29/frt-2921-secure-admin-console-validator/
```
...
```text
POST /v1/skills/frt-2921/runs
GET  /v1/skills/frt-2921/runs/{run_id}
POST /v1/skills/frt-2921/runs/{run_id}/verify
GET  /v1/skills/frt-2921/runs/{run_id}/findings
GET  /v1/skills/frt-2921/runs/{run_id}/evidence
```
