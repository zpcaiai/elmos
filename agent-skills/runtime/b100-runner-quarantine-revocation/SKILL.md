---
name: b100-runner-quarantine-revocation
description: "Use when ELMOS must Quarantine compromised or unhealthy runners, revoke credentials and preserve forensic evidence. Triggers: Runner Quarantine and Revocation, Hardened Runner Fabric Pack, quarantine, revocation, forensics, fleet recovery."
metadata:
  id: B100-S15
...
# B100-S15 — Runner Quarantine and Revocation

## Objective

Quarantine compromised or unhealthy runners, revoke credentials and preserve forensic evidence.
...
Use this Skill when the approved task explicitly requires **Runner Quarantine and Revocation** or when the capability graph selects it as part of a minimal dependency closure.
...
- `revocation_receipt.json`
- `forensic_bundle.json`
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b100-runner-quarantine-revocation/SKILL.md --out /tmp/runner_quarantine_revocation.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B100-S15
```
...
- Reject or expose quarantined runner reclaim.
- Reject or expose forensic evidence mutation.
- Reject or expose revocation partial.
