...
description: "Run FRT-2809 Artifact and Evidence Store Resilience for FRT G28 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-2809"
  source_name: "frt-2809-artifact-and-evidence-store-resilience"
  source_sha256: "sha256:2ef0d4f935004226456f2dc326019d8ac9cf4ce5e700559679aca6b7a98f7715"
  batch: "G28"
...
- Runtime catalog key: `FRT-2809` / `frt-2809-artifact-and-evidence-store-resilience`.
...
在G28范围内实现 **Artifact and Evidence Store Resilience**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：Dependency Failure Model与Common Cause分析; 端到端Deadline、Retry Budget、Circuit Breaker、Bulkhead; Queue Durability、Replay、Stateful Failover、Fence Token。

## When to Use

- 实现或修改与 **Artifact and Evidence Store Resilience** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-2809Result`：版本化、带状态和诊断的结构化结果。
- `FRT-2809EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-2809FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-2809CertificateFragment`：供G28批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g28/frt-2809-artifact-and-evidence-store-resilience/
packages/runtime/g28/frt-2809-artifact-and-evidence-store-resilience/
services/control-plane/g28/frt-2809-artifact-and-evidence-store-resilience/
apps/web-console/src/features/g28/frt-2809-artifact-and-evidence-store-resilience/
apps/admin-console/src/features/g28/frt-2809-artifact-and-evidence-store-resilience/
tests/g28/frt-2809-artifact-and-evidence-store-resilience/
```
...
```text
POST /v1/skills/frt-2809/runs
GET  /v1/skills/frt-2809/runs/{run_id}
POST /v1/skills/frt-2809/runs/{run_id}/verify
GET  /v1/skills/frt-2809/runs/{run_id}/findings
GET  /v1/skills/frt-2809/runs/{run_id}/evidence
```
