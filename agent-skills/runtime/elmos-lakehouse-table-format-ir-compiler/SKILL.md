---
name: elmos-lakehouse-table-format-ir-compiler
description: Compile snapshots, manifests, partition evolution, row-level deletes, schema evolution, transaction logs and reader/writer feature compatibility across Iceberg, Delta and Hudi.
version: 4.0.0
...
# Lakehouse Table Format IR Compiler
...
- `LakehouseTableFormatIRCompilerProfile`
- `LakehouseTableFormatIRCompilerPlan`
- `LakehouseTableFormatIRCompilerRun`
- `LakehouseTableFormatIRCompilerDecision`

Required semantic capabilities:

- model snapshot and commit semantics
- represent schema and partition evolution
- capture delete/update/merge features
- negotiate reader/writer feature compatibility
- preserve time travel and retention invariants
...
- `artifacts/elmos-lakehouse-table-format-ir-compiler/profile.yaml`
- `artifacts/elmos-lakehouse-table-format-ir-compiler/plan.json`
- `artifacts/elmos-lakehouse-table-format-ir-compiler/result.json`
- `artifacts/elmos-lakehouse-table-format-ir-compiler/evidence/`
...
- **reader lacks required table feature** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **snapshot lineage truncated** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **partition evolution misread** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **vacuum removes rollback data** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **concurrent commit semantics changed** — must have a preventive control, a detective signal, a negative fixture and a response action.
...
- native scenario: model snapshot and commit semantics
- native scenario: represent schema and partition evolution
- native scenario: capture delete/update/merge features
- native scenario: negotiate reader/writer feature compatibility
- native scenario: preserve time travel and retention invariants
