---
name: elmos-adapter-powerscript
description: Implement PowerScript/PowerBuilder language parsing, semantic lowering,
  framework/runtime mapping, target emission and verification for repository-scale
  modernization.
version: 1.0.0
skill_id: ELMOS-POLY-092
...
triggers:
- Use when implementing or executing `elmos-adapter-powerscript`.
- Use when the migration DAG requires `technology-adapter` capability.
- Use when `powerscript` is detected as source, target or repository support surface.
outputs:
- adapters/powerscript/adapter-manifest.json
- adapters/powerscript/semantic-mapping.yaml
- adapters/powerscript/verification-profile.yaml
---
...
Implement PowerScript/PowerBuilder language parsing, semantic lowering, framework/runtime mapping, target emission and verification for repository-scale modernization.
...
- Technology ID: `powerscript`
- Display: PowerScript/PowerBuilder
- Kind: language
- File/build markers: .srw, .sru, .srd, .srm, .pbl
- Build systems: PowerBuilder
- Preferred analyzers: PowerScript parser, PBL export parser, DataWindow syntax parser
- Runtime/framework surface: DataWindow, PowerBuilder VM, ODBC/OLE DB
- Native verification: PB build, DataWindow SQL/validation tests, UI replay

### Semantic risk register

- DataWindow implicit data behavior.
- transaction object semantics.
- event lifecycle.
- PBL binary assets.
- dynamic SQL and painter metadata.
