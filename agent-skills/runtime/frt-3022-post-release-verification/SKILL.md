...
description: "Run FRT-3022 Post-Release Verification for FRT G30 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-3022"
  source_name: "frt-3022-post-release-verification"
  source_sha256: "sha256:ffb9e2743b4df655a81faeafa74be4b6f4e812a1dcd8ffb0efd902c15bb6af11"
  batch: "G30"
...
- Runtime catalog key: `FRT-3022` / `frt-3022-post-release-verification`.
...
在G30范围内实现 **Post-Release Verification**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：九类Certificate聚合与唯一Production Authority; Service Catalog、Owner、Tier、Dependency; SLI/SLO、Business SLI、Error Budget和Burn Rate。

## When to Use

- 实现或修改与 **Post-Release Verification** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-3022Result`：版本化、带状态和诊断的结构化结果。
- `FRT-3022EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-3022FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-3022CertificateFragment`：供G30批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g30/frt-3022-post-release-verification/
packages/runtime/g30/frt-3022-post-release-verification/
services/control-plane/g30/frt-3022-post-release-verification/
apps/web-console/src/features/g30/frt-3022-post-release-verification/
apps/admin-console/src/features/g30/frt-3022-post-release-verification/
tests/g30/frt-3022-post-release-verification/
```
...
```text
POST /v1/skills/frt-3022/runs
GET  /v1/skills/frt-3022/runs/{run_id}
POST /v1/skills/frt-3022/runs/{run_id}/verify
GET  /v1/skills/frt-3022/runs/{run_id}/findings
GET  /v1/skills/frt-3022/runs/{run_id}/evidence
```
