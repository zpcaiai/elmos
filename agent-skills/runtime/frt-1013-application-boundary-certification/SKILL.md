...
description: "Run FRT-1013 Application Boundary Certification for FRT G10 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-1013"
  source_name: "frt-1013-application-boundary-certification"
  source_sha256: "sha256:2daad579c6bccc99a80a80bcea8f580e51e0d163fefc6c47a4a8fcee063d5519"
  batch: "G10"
...
- Runtime catalog key: `FRT-1013` / `frt-1013-application-boundary-certification`.
...
# FRT-1013 — Application Boundary Certification

## Objective

在G10范围内实现 **Application Boundary Certification**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：保持Route、History、Guard和Deep Link语义; 表单Validation、错误、草稿和提交幂等完整; API、Event、Storage和Realtime边界显式。

## When to Use

- 实现或修改与 **Application Boundary Certification** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-1013Result`：版本化、带状态和诊断的结构化结果。
- `FRT-1013EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-1013FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-1013CertificateFragment`：供G10批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g10/frt-1013-application-boundary-certification/
packages/runtime/g10/frt-1013-application-boundary-certification/
services/control-plane/g10/frt-1013-application-boundary-certification/
apps/web-console/src/features/g10/frt-1013-application-boundary-certification/
apps/admin-console/src/features/g10/frt-1013-application-boundary-certification/
tests/g10/frt-1013-application-boundary-certification/
```
...
```text
POST /v1/skills/frt-1013/runs
GET  /v1/skills/frt-1013/runs/{run_id}
POST /v1/skills/frt-1013/runs/{run_id}/verify
GET  /v1/skills/frt-1013/runs/{run_id}/findings
GET  /v1/skills/frt-1013/runs/{run_id}/evidence
```
