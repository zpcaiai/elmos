...
description: "Run FRT-2014 Artifact and Evidence Store for FRT G20 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-2014"
  source_name: "frt-2014-artifact-and-evidence-store"
  source_sha256: "sha256:7d890f0e9dd83b656cf89c4680807c21d9ca4d3ec82a752b2b966f4809407ed2"
  batch: "G20"
...
- Runtime catalog key: `FRT-2014` / `frt-2014-artifact-and-evidence-store`.
...
# FRT-2014 — Artifact and Evidence Store

## Objective

在G20范围内实现 **Artifact and Evidence Store**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：提供Skill SDK、Runtime、Registry和Marketplace; 实现CLI、API、IDE、Web Console和Multi-Tenant Workspace; Durable Workflow与分布式Worker支持暂停、恢复、取消和幂等。

## When to Use

- 实现或修改与 **Artifact and Evidence Store** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-2014Result`：版本化、带状态和诊断的结构化结果。
- `FRT-2014EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-2014FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-2014CertificateFragment`：供G20批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g20/frt-2014-artifact-and-evidence-store/
packages/runtime/g20/frt-2014-artifact-and-evidence-store/
services/control-plane/g20/frt-2014-artifact-and-evidence-store/
apps/web-console/src/features/g20/frt-2014-artifact-and-evidence-store/
apps/admin-console/src/features/g20/frt-2014-artifact-and-evidence-store/
tests/g20/frt-2014-artifact-and-evidence-store/
```
...
```text
POST /v1/skills/frt-2014/runs
GET  /v1/skills/frt-2014/runs/{run_id}
POST /v1/skills/frt-2014/runs/{run_id}/verify
GET  /v1/skills/frt-2014/runs/{run_id}/findings
GET  /v1/skills/frt-2014/runs/{run_id}/evidence
```
