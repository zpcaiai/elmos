...
- Invoke this repository Skill as `$pm-b37-test-migration-agent`.
- Immutable source identity: `test-migration-agent` in `precision-migration-b01-44` (B37).
- Runtime adapter: `model-routing-and-agent-harness`; binding state: `DECLARED`.
- Resolve and plan with `python3 scripts/precision_migration/runtime.py plan --skill pm-b37-test-migration-agent`.
...
迁移源测试、保留 Oracle、补充目标测试并阻止弱化断言。

本 Skill 属于精密语言转换系统。它必须以**可执行证据**为准，不得用模型自评、源码相似度或“能编译”替代语义与行为验证。

## Use when

- 当前任务直接涉及 `test-migration-agent` 所描述的能力。
...
1. 计算风险并拆分任务
2. 执行本 Skill 的核心职责：迁移源测试、保留 Oracle、补充目标测试并阻止弱化断言。
3. 选择模型与隔离工作区
4. 执行并收集客观反馈
5. 独立审查和升级
6. 持久化状态、证据、成本与审批
