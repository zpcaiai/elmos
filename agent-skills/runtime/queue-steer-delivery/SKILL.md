---
name: queue-steer-delivery
description: 区分排队输入和下一安全边界立即 steer 的交互语义。 Use for this exact ELMOS Spring Golden Route v2 contract; imported specifications are not runtime or certification evidence.
...
metadata:
  ...
  source_id: FOUNDATION-07-queue-steer-delivery
  source_name: queue-steer-delivery
  source_path: skills/queue-steer-delivery/SKILL.md
  source_sha256: sha256:5e6652b00857e4e599a85c367bb1ad0391ba20e59cb9c9a667b889056646e740
  source_contract_sha256: sha256:fdaad32eeee54d98613932bcdfd254b55ce833b05191f35437f7b7b2f6ca45a0
...
区分排队输入和下一安全边界立即 steer 的交互语义。

## Use when

在 Elmos 需要实现或升级 **Queue / Steer Delivery** 相关能力时使用；它不是仅供解释的提示词，而是实现、测试与验收契约。

## Capabilities to implement

- steer 在下一 provider-turn safe boundary 合并
- queue FIFO，在当前 drain 结束时逐条提升
- 交付模式持久化并可观察
- 设置 backlog/batch 上限防止无界输入
...
- [ ] 定义 `queue-steer-delivery` 的 public interface、input/output/error schemas。
- [ ] 建立 `queue-steer-delivery` 的 capability/feature flags 与版本字段。
...
- [ ] 至少覆盖并验收：steer 在下一 provider-turn safe boundary 合并；以及 queue FIFO，在当前 drain 结束时逐条提升。
...
- Machine contract: `references/contract.json` (`sha256:fdaad32eeee54d98613932bcdfd254b55ce833b05191f35437f7b7b2f6ca45a0`).
