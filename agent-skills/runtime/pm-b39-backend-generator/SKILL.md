---
name: pm-b39-backend-generator
description: "生成 API、领域、数据、消息、任务、安全、测试和运行配置. Precision Migration B39 contract; use for this exact assessment, transformation, validation, repair, evidence, or cutover scope."
---

# Backend Generator
## ELMOS runtime binding

- Invoke this repository Skill as `$pm-b39-backend-generator`.
- Immutable source identity: `backend-generator` in `precision-migration-b01-44` (B39).
- Runtime adapter: `skill-and-project-synthesis`; binding state: `UNAVAILABLE`.
- Resolve and plan with `python3 scripts/precision_migration/runtime.py plan --skill pm-b39-backend-generator`.
...
生成 API、领域、数据、消息、任务、安全、测试和运行配置。

本 Skill 属于精密语言转换系统。它必须以**可执行证据**为准，不得用模型自评、源码相似度或“能编译”替代语义与行为验证。

## Use when

- 当前任务直接涉及 `backend-generator` 所描述的能力。
...
2. 执行本 Skill 的核心职责：生成 API、领域、数据、消息、任务、安全、测试和运行配置。
3. 编译Skills与架构蓝图
4. 生成/复用全部构件
5. 构建并执行验收/安全/完整性门禁
6. 输出项目、Runbook和证据
