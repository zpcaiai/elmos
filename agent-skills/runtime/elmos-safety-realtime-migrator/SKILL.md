---
name: elmos-safety-realtime-migrator
description: Preserve Ada/SPARK/C real-time scheduling, deadlines, boundedness, representation
  and safety contracts when moving to Rust/C++/Ada targets.
version: 1.0.0
skill_id: ELMOS-POLY-122
layer: safety-transformation
risk: high
readiness: not-run
dependencies:
- elmos-semantic-loss-risk-estimator
triggers:
- Use when implementing or executing `elmos-safety-realtime-migrator`.
- Use when the migration DAG requires `safety-transformation` capability.
outputs:
- transforms/elmos-safety-realtime-migrator/plan.json
- transforms/elmos-safety-realtime-migrator/semantic-loss.json
- transforms/elmos-safety-realtime-migrator/evidence.json
---

# Safety Realtime Migrator

## Objective

Preserve Ada/SPARK/C real-time scheduling, deadlines, boundedness, representation and safety contracts when moving to Rust/C++/Ada targets.
