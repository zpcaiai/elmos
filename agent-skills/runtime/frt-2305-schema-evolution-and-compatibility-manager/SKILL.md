...
description: "Run FRT-2305 Schema Evolution and Compatibility Manager for FRT G23 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-2305"
  source_name: "frt-2305-schema-evolution-and-compatibility-manager"
  source_sha256: "sha256:d6269865ad81eae92ce767d6fdd16f0ed25b6a9fb4a5cc344f2ad8251c8f47f3"
  batch: "G23"
  source_version: "1.0.0"
  source_risk: "critical-by-default"
  source_certificate_family: "DC"
...
- Runtime catalog key: `FRT-2305` / `frt-2305-schema-evolution-and-compatibility-manager`.
...
在G23范围内实现 **Schema Evolution and Compatibility Manager**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：字段级Data Contract、Authority和Owner; Source-to-Sink与字段级Lineage; Write→Transaction→Outbox→Event→Projection→Cache→Read完整路径。

## When to Use

- 实现或修改与 **Schema Evolution and Compatibility Manager** 对应的Contract、Runtime、API、UI、测试或证据链时。
- 相关代码、Schema、Pack、Policy、环境或依赖变化导致既有Evidence失效时。
- G23 Orchestrator生成该Skill的执行义务时。
...
- `SourceArtifactSet`：G22 Business Process、Command、Event、State Machine, 数据库DDL、API/Event Schema、Cache、Search、Offline Store。
- `SkillPolicy`：权限、网络、Sandbox、Toolchain、Evidence和停止规则。

## Outputs

- `FRT-2305Result`：版本化、带状态和诊断的结构化结果。
- `FRT-2305EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-2305FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-2305CertificateFragment`：供G23批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g23/frt-2305-schema-evolution-and-compatibility-manager/
packages/runtime/g23/frt-2305-schema-evolution-and-compatibility-manager/
services/control-plane/g23/frt-2305-schema-evolution-and-compatibility-manager/
apps/web-console/src/features/g23/frt-2305-schema-evolution-and-compatibility-manager/
apps/admin-console/src/features/g23/frt-2305-schema-evolution-and-compatibility-manager/
tests/g23/frt-2305-schema-evolution-and-compatibility-manager/
```
...
```text
POST /v1/skills/frt-2305/runs
GET  /v1/skills/frt-2305/runs/{run_id}
POST /v1/skills/frt-2305/runs/{run_id}/verify
GET  /v1/skills/frt-2305/runs/{run_id}/findings
GET  /v1/skills/frt-2305/runs/{run_id}/evidence
```
...
- The parent G23 Orchestrator can consume the result and independently decide certification.
