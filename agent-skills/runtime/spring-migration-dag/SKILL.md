---
name: spring-migration-dag
description: 将迁移拆成 Recipe/Agent/Tool/Human/Verify 节点，声明依赖、权限、副作用、回滚和验收。 Use for this exact ELMOS Spring Golden Route v2 contract; imported specifications are not runtime or certification evidence.
...
metadata:
  ...
  source_id: ELMOS-GR-041
  source_name: spring-migration-dag
  source_path: skills/spring-migration-dag/SKILL.md
  source_sha256: sha256:1fee2f0324c7fd567aef5112c41e5ce22aa167cb20f6a0aac608b6dcb40503db
  source_contract_sha256: sha256:1c7d4e257850268739830cc03b66d6f43cbdbe8dc13e7b3caa9268883c7ca316
...
将迁移拆成 Recipe/Agent/Tool/Human/Verify 节点，声明依赖、权限、副作用、回滚和验收。
...
- [ ] 定义 `spring-migration-dag` 的 public interface、输入/输出/Error Schema 和版本策略。
- [ ] 实现 `spring-migration-dag` 的 production adapter/service，不把业务状态只保存在 Prompt 或内存对象中。
...
- [ ] 至少生成：migration-dag.json, work-packages, checkpoint-plan.json。
...
- [ ] 全量 Recipe 最终收敛
- [ ] Agent 只接收剩余业务歧义
- [ ] 本 Skill 的 required outputs 均由真实目标仓库执行产生：migration-dag.json, work-packages, checkpoint-plan.json。
...
- Machine contract: `references/contract.json` (`sha256:1c7d4e257850268739830cc03b66d6f43cbdbe8dc13e7b3caa9268883c7ca316`).
