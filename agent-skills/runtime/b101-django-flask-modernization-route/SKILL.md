---
name: b101-django-flask-modernization-route
description: "Use when ELMOS must Modernize Django or Flask routing, middleware, auth, ORM, sessions, migrations and deployment behavior. Triggers: Django and Flask Modernization Route, Three Golden Route Packs, Django, Flask, auth/session, ORM migrations."
metadata:
  id: B101-S13
...
# B101-S13 — Django and Flask Modernization Route

## Objective

Modernize Django or Flask routing, middleware, auth, ORM, sessions, migrations and deployment behavior.
...
Use this Skill when the approved task explicitly requires **Django and Flask Modernization Route** or when the capability graph selects it as part of a minimal dependency closure.
...
- `python_web_mapping.json`
- `python_web_patch_set/`
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b101-django-flask-modernization-route/SKILL.md --out /tmp/django_flask_modernization_route.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B101-S13
```
...
- Reject or expose CSRF disabled.
- Reject or expose session format drift.
- Reject or expose migration squash data loss.
