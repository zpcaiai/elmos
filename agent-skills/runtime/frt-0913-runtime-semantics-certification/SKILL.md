...
description: "Run FRT-0913 Runtime Semantics Certification for FRT G09 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-0913"
  source_name: "frt-0913-runtime-semantics-certification"
  source_sha256: "sha256:7bd960371dfd97548980cf3c6764f84ae256878e910363d69c317f28166e6639"
  batch: "G09"
...
- Runtime catalog key: `FRT-0913` / `frt-0913-runtime-semantics-certification`.
...
# FRT-0913 — Runtime Semantics Certification

## Objective

在G09范围内实现 **Runtime Semantics Certification**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：区分Local、Server、Global、Persistent和Derived State; 保持Effect触发、Cleanup、Lifecycle和Evaluation Order; 传播Cancellation并防止Stale Result Commit。

## When to Use

- 实现或修改与 **Runtime Semantics Certification** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-0913Result`：版本化、带状态和诊断的结构化结果。
- `FRT-0913EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-0913FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-0913CertificateFragment`：供G09批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g09/frt-0913-runtime-semantics-certification/
packages/runtime/g09/frt-0913-runtime-semantics-certification/
services/control-plane/g09/frt-0913-runtime-semantics-certification/
apps/web-console/src/features/g09/frt-0913-runtime-semantics-certification/
apps/admin-console/src/features/g09/frt-0913-runtime-semantics-certification/
tests/g09/frt-0913-runtime-semantics-certification/
```
...
```text
POST /v1/skills/frt-0913/runs
GET  /v1/skills/frt-0913/runs/{run_id}
POST /v1/skills/frt-0913/runs/{run_id}/verify
GET  /v1/skills/frt-0913/runs/{run_id}/findings
GET  /v1/skills/frt-0913/runs/{run_id}/evidence
```
