...
description: "Run FRT-1500 Web ArkUI Route Orchestrator for FRT G15 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-1500"
  source_name: "frt-1500-web-arkui-route-orchestrator"
  source_sha256: "sha256:76a14af4b109b85cb1b4d5cbc43deb0c91ca0c637651a2f40646ccdd6313d68f"
  batch: "G15"
...
- Runtime catalog key: `FRT-1500` / `frt-1500-web-arkui-route-orchestrator`.
...
# FRT-1500 — Web ArkUI Route Orchestrator

## Objective

在G15范围内实现 **Web ArkUI Route Orchestrator**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：映射ArkUI状态装饰器、Builder、Navigation和生命周期; 处理Web DOM能力与ArkUI Native/Device差异; 生成真机/模拟器和Accessibility验证。

## When to Use

- 实现或修改与 **Web ArkUI Route Orchestrator** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-1500Result`：版本化、带状态和诊断的结构化结果。
- `FRT-1500EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-1500FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-1500CertificateFragment`：供G15批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g15/frt-1500-web-arkui-route-orchestrator/
packages/runtime/g15/frt-1500-web-arkui-route-orchestrator/
services/control-plane/g15/frt-1500-web-arkui-route-orchestrator/
apps/web-console/src/features/g15/frt-1500-web-arkui-route-orchestrator/
apps/admin-console/src/features/g15/frt-1500-web-arkui-route-orchestrator/
tests/g15/frt-1500-web-arkui-route-orchestrator/
```
...
```text
POST /v1/skills/frt-1500/runs
GET  /v1/skills/frt-1500/runs/{run_id}
POST /v1/skills/frt-1500/runs/{run_id}/verify
GET  /v1/skills/frt-1500/runs/{run_id}/findings
GET  /v1/skills/frt-1500/runs/{run_id}/evidence
```
