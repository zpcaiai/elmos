...
description: "Run FRT-3015 Legacy Coexistence and Cutover Validator for FRT G30 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-3015"
  source_name: "frt-3015-legacy-coexistence-and-cutover-validator"
  source_sha256: "sha256:c81ae4c479c14ebb83be57b3ad1cc7d1c46a2e1ff0a089bab0cfcd1097e21ec6"
  batch: "G30"
  source_version: "1.0.0"
  source_risk: "critical-by-default"
  source_certificate_family: "PR"
...
- Runtime catalog key: `FRT-3015` / `frt-3015-legacy-coexistence-and-cutover-validator`.
...
在G30范围内实现 **Legacy Coexistence and Cutover Validator**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：九类Certificate聚合与唯一Production Authority; Service Catalog、Owner、Tier、Dependency; SLI/SLO、Business SLI、Error Budget和Burn Rate。

## When to Use

- 实现或修改与 **Legacy Coexistence and Cutover Validator** 对应的Contract、Runtime、API、UI、测试或证据链时。
- 相关代码、Schema、Pack、Policy、环境或依赖变化导致既有Evidence失效时。
- G30 Orchestrator生成该Skill的执行义务时。
...
- `SourceArtifactSet`：G21–G29所有Certificate和Evidence, Production Environment、Service/Deployment/Config/Policy/Flag Manifest。
- `SkillPolicy`：权限、网络、Sandbox、Toolchain、Evidence和停止规则。

## Outputs

- `FRT-3015Result`：版本化、带状态和诊断的结构化结果。
- `FRT-3015EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-3015FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-3015CertificateFragment`：供G30批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g30/frt-3015-legacy-coexistence-and-cutover-validator/
packages/runtime/g30/frt-3015-legacy-coexistence-and-cutover-validator/
services/control-plane/g30/frt-3015-legacy-coexistence-and-cutover-validator/
apps/web-console/src/features/g30/frt-3015-legacy-coexistence-and-cutover-validator/
apps/admin-console/src/features/g30/frt-3015-legacy-coexistence-and-cutover-validator/
tests/g30/frt-3015-legacy-coexistence-and-cutover-validator/
```
...
```text
POST /v1/skills/frt-3015/runs
GET  /v1/skills/frt-3015/runs/{run_id}
POST /v1/skills/frt-3015/runs/{run_id}/verify
GET  /v1/skills/frt-3015/runs/{run_id}/findings
GET  /v1/skills/frt-3015/runs/{run_id}/evidence
```
...
- The parent G30 Orchestrator can consume the result and independently decide certification.
