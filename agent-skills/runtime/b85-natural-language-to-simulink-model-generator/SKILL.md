---
name: b85-natural-language-to-simulink-model-generator
description: "Use when ELMOS must run natural language to simulink model generator for Batch 85 MATLAB / Simulink / Stateflow Pack. Preserve native semantics, safety boundaries, ownership, traceability, and fail-closed evidence; require real vendor toolchain execution for runtime claims."
metadata:
  source_package: "elmos-language-packs-batch81-95-complete"
  source_id: "PG277"
  source_name: "natural-language-to-simulink-model-generator"
  source_sha256: "sha256:f2caab978b769d1f9b14072021c4dcc58686546624a33007e8764cd1f2dd9581"
  batch: 85
  source_engine: "elmos.language-packs"
  source_status: "proposed"
  normalized_namespace: "language-pack"
---
# Batch 85 Language Pack / source PG277 — Natural Language To Simulink Model Generator

## 1. Objective

Model natural language to simulink model generator within the MATLAB / Simulink / Stateflow Pack, preserving platform semantics, traceability, security, and reproducible evidence.

## 2. Scope

This Skill is part of **Batch 85: MATLAB / Simulink / Stateflow Pack**.

### In scope

- Process MATLAB files.
- Process projects.
- Process Simulink models.
- Process Stateflow charts.
- Process Simscape models.
- Process data dictionaries.
- Process test harnesses.
- Process S-functions.
- Produce MATLAB projects.
- Produce Simulink/Stateflow models.
- Produce generated C/C++.
- Produce SIL/PIL/HIL tests.
...
2. Process the relevant artifacts for Natural Language To Simulink Model Generator using the approved model-based-engineering semantic contracts and toolchain versions.
3. Preserve source provenance and create Artifact Graph links across inputs, outputs, tests, approvals, and evidence.
4. Detect and classify floating-point drift, solver mismatch, sample-time conflict; never convert uncertainty or unsupported behavior into a silent success.
...
7. Apply the safety boundary: Safety-critical generated code is not certified by model tests alone; target compiler, hardware, and applicable lifecycle standards remain mandatory.
...
- `FLOATING_POINT_DRIFT` — stop unsafe continuation and emit source-linked diagnostic evidence.
- `SOLVER_MISMATCH` — stop unsafe continuation and emit source-linked diagnostic evidence.
- `SAMPLE_TIME_CONFLICT` — stop unsafe continuation and emit source-linked diagnostic evidence.
- `ALGEBRAIC_LOOP` — stop unsafe continuation and emit source-linked diagnostic evidence.
- `STATE_TRANSITION_DRIFT` — stop unsafe continuation and emit source-linked diagnostic evidence.
- `CODE_GENERATION_UNSUPPORTED_BLOCK` — stop unsafe continuation and emit source-linked diagnostic evidence.
...
- Apply this Batch safety boundary: **Safety-critical generated code is not certified by model tests alone; target compiler, hardware, and applicable lifecycle standards remain mandatory.**

## 14. Unit Tests

- A representative valid matlab-simulink input produces all declared outputs for natural-language-to-simulink-model-generator.
...
- At least one batch-specific risk (floating-point drift) is detected by a negative fixture.

## 15. Integration Tests

- Consume approved Project Synthesis or Modernization baselines and write complete lineage into the Artifact Graph.
- Integrate with at least one representative toolchain from: MATLAB, Simulink, Stateflow, Simscape.
...
- Reject or block floating-point drift without weakening the expected result.
- Reject or block solver mismatch without weakening the expected result.
- Reject or block sample-time conflict without weakening the expected result.
- Reject or block algebraic loop without weakening the expected result.
- Reject or block state transition drift without weakening the expected result.
- Reject or block code-generation unsupported block without weakening the expected result.
