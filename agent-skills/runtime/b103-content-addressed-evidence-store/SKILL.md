---
name: b103-content-addressed-evidence-store
description: "Use when ELMOS must Store immutable evidence and artifacts by digest with tenant isolation, retention and legal-hold support. Triggers: Content Addressed Evidence Store, Evidence and Certification Fabric Pack, content addressing, immutability, retention, tenant isolation."
metadata:
  id: B103-S02
...
# B103-S02 — Content Addressed Evidence Store

## Objective

Store immutable evidence and artifacts by digest with tenant isolation, retention and legal-hold support.
...
Use this Skill when the approved task explicitly requires **Content Addressed Evidence Store** or when the capability graph selects it as part of a minimal dependency closure.
...
- `evidence_store_schema.sql`
- `storage_policy.json`
- `store_validation.json`
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b103-content-addressed-evidence-store/SKILL.md --out /tmp/content_addressed_evidence_store.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B103-S02
```
...
- Reject or expose object overwrite.
- Reject or expose cross-tenant digest inference.
- Reject or expose garbage collection of legal hold.
