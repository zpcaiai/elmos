...
description: "Run FRT-2813 Backup and Restore Validator for FRT G28 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-2813"
  source_name: "frt-2813-backup-and-restore-validator"
  source_sha256: "sha256:34a740d6284019f5150eac835dac9bc09ce7923b48a6277e12278f128f4f8045"
  batch: "G28"
...
- Runtime catalog key: `FRT-2813` / `frt-2813-backup-and-restore-validator`.
...
在G28范围内实现 **Backup and Restore Validator**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：Dependency Failure Model与Common Cause分析; 端到端Deadline、Retry Budget、Circuit Breaker、Bulkhead; Queue Durability、Replay、Stateful Failover、Fence Token。

## When to Use

- 实现或修改与 **Backup and Restore Validator** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-2813Result`：版本化、带状态和诊断的结构化结果。
- `FRT-2813EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-2813FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-2813CertificateFragment`：供G28批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g28/frt-2813-backup-and-restore-validator/
packages/runtime/g28/frt-2813-backup-and-restore-validator/
services/control-plane/g28/frt-2813-backup-and-restore-validator/
apps/web-console/src/features/g28/frt-2813-backup-and-restore-validator/
apps/admin-console/src/features/g28/frt-2813-backup-and-restore-validator/
tests/g28/frt-2813-backup-and-restore-validator/
```
...
```text
POST /v1/skills/frt-2813/runs
GET  /v1/skills/frt-2813/runs/{run_id}
POST /v1/skills/frt-2813/runs/{run_id}/verify
GET  /v1/skills/frt-2813/runs/{run_id}/findings
GET  /v1/skills/frt-2813/runs/{run_id}/evidence
```
