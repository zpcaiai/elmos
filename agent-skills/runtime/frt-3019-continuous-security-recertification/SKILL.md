...
description: "Run FRT-3019 Continuous Security Recertification for FRT G30 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-3019"
  source_name: "frt-3019-continuous-security-recertification"
  source_sha256: "sha256:9e8ce9e18c080e7604163addc8202aa8042e4823bb1361b85d2664ba92cc0bc7"
  batch: "G30"
...
- Runtime catalog key: `FRT-3019` / `frt-3019-continuous-security-recertification`.
...
在G30范围内实现 **Continuous Security Recertification**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：九类Certificate聚合与唯一Production Authority; Service Catalog、Owner、Tier、Dependency; SLI/SLO、Business SLI、Error Budget和Burn Rate。

## When to Use

- 实现或修改与 **Continuous Security Recertification** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-3019Result`：版本化、带状态和诊断的结构化结果。
- `FRT-3019EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-3019FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-3019CertificateFragment`：供G30批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g30/frt-3019-continuous-security-recertification/
packages/runtime/g30/frt-3019-continuous-security-recertification/
services/control-plane/g30/frt-3019-continuous-security-recertification/
apps/web-console/src/features/g30/frt-3019-continuous-security-recertification/
apps/admin-console/src/features/g30/frt-3019-continuous-security-recertification/
tests/g30/frt-3019-continuous-security-recertification/
```
...
```text
POST /v1/skills/frt-3019/runs
GET  /v1/skills/frt-3019/runs/{run_id}
POST /v1/skills/frt-3019/runs/{run_id}/verify
GET  /v1/skills/frt-3019/runs/{run_id}/findings
GET  /v1/skills/frt-3019/runs/{run_id}/evidence
```
