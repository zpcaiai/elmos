...
description: "Run FRT-2710 Queue and Backpressure Validator for FRT G27 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-2710"
  source_name: "frt-2710-queue-and-backpressure-validator"
  source_sha256: "sha256:a27bd3e78ef42ffe6b69e1e5ec571337f2789a24d5d47ab5ce8ce12e3fd8d912"
  batch: "G27"
...
- Runtime catalog key: `FRT-2710` / `frt-2710-queue-and-backpressure-validator`.
...
在G27范围内实现 **Queue and Backpressure Validator**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：业务化Workload Model; 并发线性化点、Race、Lost Update、Idempotency Under Load; Load/Stress/Spike/Soak与Recovery。

## When to Use

- 实现或修改与 **Queue and Backpressure Validator** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-2710Result`：版本化、带状态和诊断的结构化结果。
- `FRT-2710EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-2710FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-2710CertificateFragment`：供G27批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g27/frt-2710-queue-and-backpressure-validator/
packages/runtime/g27/frt-2710-queue-and-backpressure-validator/
services/control-plane/g27/frt-2710-queue-and-backpressure-validator/
apps/web-console/src/features/g27/frt-2710-queue-and-backpressure-validator/
apps/admin-console/src/features/g27/frt-2710-queue-and-backpressure-validator/
tests/g27/frt-2710-queue-and-backpressure-validator/
```
...
```text
POST /v1/skills/frt-2710/runs
GET  /v1/skills/frt-2710/runs/{run_id}
POST /v1/skills/frt-2710/runs/{run_id}/verify
GET  /v1/skills/frt-2710/runs/{run_id}/findings
GET  /v1/skills/frt-2710/runs/{run_id}/evidence
```
