---
name: pm-b06-compiler-diagnostic-normalizer
description: "将编译、链接、类型、Lint 和构建诊断规范化为结构化问题. Precision Migration B06 contract; use for this exact assessment, transformation, validation, repair, evidence, or cutover scope."
---

# Compiler Diagnostic Normalizer
## ELMOS runtime binding

- Invoke this repository Skill as `$pm-b06-compiler-diagnostic-normalizer`.
- Immutable source identity: `compiler-diagnostic-normalizer` in `precision-migration-b01-44` (B06).
- Runtime adapter: `semantic-recovery-and-ir`; binding state: `DECLARED`.
- Resolve and plan with `python3 scripts/precision_migration/runtime.py plan --skill pm-b06-compiler-diagnostic-normalizer`.
...
- Phase: `B 源码理解与可信执行底座`
- Version: `1.0.0`
- Tags: batch-06, toolchain-registry

## Purpose

将编译、链接、类型、Lint 和构建诊断规范化为结构化问题。

本 Skill 属于精密语言转换系统。它必须以**可执行证据**为准，不得用模型自评、源码相似度或“能编译”替代语义与行为验证。

## Use when

- 当前任务直接涉及 `compiler-diagnostic-normalizer` 所描述的能力。
...
1. 发现仓库与环境
2. 执行本 Skill 的核心职责：将编译、链接、类型、Lint 和构建诊断规范化为结构化问题。
3. 使用原生工具提取语义
4. 建立可复现工具链和沙箱
5. 执行最小验证任务
6. 持久化摘要、哈希和证据
...
```yaml
skill: compiler-diagnostic-normalizer
version: 1.0.0
