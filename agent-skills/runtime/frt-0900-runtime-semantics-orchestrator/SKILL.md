...
description: "Run FRT-0900 Runtime Semantics Orchestrator for FRT G09 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-0900"
  source_name: "frt-0900-runtime-semantics-orchestrator"
  source_sha256: "sha256:d0189053a64d9ce16bcdd806427a9c6da64b76c5030a9452e87e14eeb617806b"
  batch: "G09"
...
- Runtime catalog key: `FRT-0900` / `frt-0900-runtime-semantics-orchestrator`.
...
# FRT-0900 — Runtime Semantics Orchestrator

## Objective

在G09范围内实现 **Runtime Semantics Orchestrator**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：区分Local、Server、Global、Persistent和Derived State; 保持Effect触发、Cleanup、Lifecycle和Evaluation Order; 传播Cancellation并防止Stale Result Commit。

## When to Use

- 实现或修改与 **Runtime Semantics Orchestrator** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-0900Result`：版本化、带状态和诊断的结构化结果。
- `FRT-0900EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-0900FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-0900CertificateFragment`：供G09批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g09/frt-0900-runtime-semantics-orchestrator/
packages/runtime/g09/frt-0900-runtime-semantics-orchestrator/
services/control-plane/g09/frt-0900-runtime-semantics-orchestrator/
apps/web-console/src/features/g09/frt-0900-runtime-semantics-orchestrator/
apps/admin-console/src/features/g09/frt-0900-runtime-semantics-orchestrator/
tests/g09/frt-0900-runtime-semantics-orchestrator/
```
...
```text
POST /v1/skills/frt-0900/runs
GET  /v1/skills/frt-0900/runs/{run_id}
POST /v1/skills/frt-0900/runs/{run_id}/verify
GET  /v1/skills/frt-0900/runs/{run_id}/findings
GET  /v1/skills/frt-0900/runs/{run_id}/evidence
```
