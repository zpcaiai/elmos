...
description: "Run FRT-0211 Repository Risk Classifier for FRT G02 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-0211"
  source_name: "frt-0211-repository-risk-classifier"
  source_sha256: "sha256:5e84da20a6b84ef8708c48ee4e538f8dda89cf39759dae7cf52f46b7a38e7c54"
  batch: "G02"
...
- Runtime catalog key: `FRT-0211` / `frt-0211-repository-risk-classifier`.
...
# FRT-0211 — Repository Risk Classifier

## Objective

在G02范围内实现 **Repository Risk Classifier**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：识别Monorepo、Package、Framework、Runtime和Build链; 建立模块、依赖、路由、组件、状态、API、存储和资产清单; 检测动态代码、原生能力、平台限制和迁移风险。

## When to Use

- 实现或修改与 **Repository Risk Classifier** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-0211Result`：版本化、带状态和诊断的结构化结果。
- `FRT-0211EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-0211FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-0211CertificateFragment`：供G02批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g02/frt-0211-repository-risk-classifier/
packages/runtime/g02/frt-0211-repository-risk-classifier/
services/control-plane/g02/frt-0211-repository-risk-classifier/
apps/web-console/src/features/g02/frt-0211-repository-risk-classifier/
apps/admin-console/src/features/g02/frt-0211-repository-risk-classifier/
tests/g02/frt-0211-repository-risk-classifier/
```
...
```text
POST /v1/skills/frt-0211/runs
GET  /v1/skills/frt-0211/runs/{run_id}
POST /v1/skills/frt-0211/runs/{run_id}/verify
GET  /v1/skills/frt-0211/runs/{run_id}/findings
GET  /v1/skills/frt-0211/runs/{run_id}/evidence
```
