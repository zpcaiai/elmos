...
description: "Run FRT-2100 Product Closure Orchestrator for FRT G21 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-2100"
  source_name: "frt-2100-product-closure-orchestrator"
  source_sha256: "sha256:ca55624d43274ebce217501f0d8c1cfac0cc05918b3ad85386f9abab8b1e7ad9"
  batch: "G21"
...
- Runtime catalog key: `FRT-2100` / `frt-2100-product-closure-orchestrator`.
...
在G21范围内实现 **Product Closure Orchestrator**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：统一 Product Closure Graph; Requirement、Capability、Journey、Command、API、Data、Permission、Test、Monitoring、Runbook、Certificate 双向追踪; 孤儿 UI/API/Command/Event/Data Write 检测。

## When to Use

- 实现或修改与 **Product Closure Orchestrator** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-2100Result`：版本化、带状态和诊断的结构化结果。
- `FRT-2100EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-2100FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-2100CertificateFragment`：供G21批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g21/frt-2100-product-closure-orchestrator/
packages/runtime/g21/frt-2100-product-closure-orchestrator/
services/control-plane/g21/frt-2100-product-closure-orchestrator/
apps/web-console/src/features/g21/frt-2100-product-closure-orchestrator/
apps/admin-console/src/features/g21/frt-2100-product-closure-orchestrator/
tests/g21/frt-2100-product-closure-orchestrator/
```
...
```text
POST /v1/skills/frt-2100/runs
GET  /v1/skills/frt-2100/runs/{run_id}
POST /v1/skills/frt-2100/runs/{run_id}/verify
GET  /v1/skills/frt-2100/runs/{run_id}/findings
GET  /v1/skills/frt-2100/runs/{run_id}/evidence
```
