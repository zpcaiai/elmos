---
name: database-schema-and-query-graph
description: Use this skill when Elmos must perform database schema and query graph as part of the 02-repository-semantic-intelligence
...
compatibility: Elmos v3 harness; K2 Repository Semantic Compiler; policy and evidence services required.
metadata:
  version: 3.0.0
  pack: 02-repository-semantic-intelligence
  priority: P0
  exposure: atomic-registry-only
  business-line: repository-intelligence
allowed-tools: parser.run compiler.query graph.write symbol.resolve trace.correlate
---
# database-schema-and-query-graph

## 能力目标

关联表、列、约束、Routine、ORM 映射、SQL 与业务调用。
...
- 必须门：parse-coverage, symbol-resolution-threshold, graph-consistency, source-map-complete；
...
- 业务线：`repository-intelligence`；包：`02-repository-semantic-intelligence`。
