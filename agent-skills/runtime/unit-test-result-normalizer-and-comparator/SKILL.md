---
name: unit-test-result-normalizer-and-comparator
description: Normalize test identities and compare baseline with migrated results without allowing count manipulation. Use after both test suites run.
---

# Unit Test Result Normalizer And Comparator
...
Require engine, class, method, parameter identity, status, failure fingerprint and duration for both sides.

## Workflow

1. Validate schema version, stable identities, immutable hashes and policy prerequisites.
2. Normalize statuses; match exact identities; detect missing, new skip, new failure, changed failure and improvement; record flakiness separately.
...
test-result-comparison.json with every identity and classification.

## Fail-closed rules

Removed/not-run tests and new skips never pass; stable pre-existing failures are preserved facts, not new regressions.
