---
name: spring-crash-recovery
description: 使用 lease、fencing token、checkpoint 和 external reconciliation 恢复 Worker/网络故障。 Use for this exact ELMOS Spring Golden Route v2 contract; imported specifications are not runtime or certification evidence.
...
metadata:
  ...
  source_id: ELMOS-GR-054
  source_name: spring-crash-recovery
  source_path: skills/spring-crash-recovery/SKILL.md
  source_sha256: sha256:5d1d7beb7aeed8d43ecfc1c30040d224354b15436b0543e62b423e169b86531e
  source_contract_sha256: sha256:96ac5f85ace627908e4b385ba421f18ef815526ed240f30597c5e715ba589c6d
...
使用 lease、fencing token、checkpoint 和 external reconciliation 恢复 Worker/网络故障。

## Scope and capabilities

- Stale owner detection
- 双执行防护
- Provider/tool ambiguity matrix
- RTO
- Chaos tests
...
- [ ] 定义 `spring-crash-recovery` 的 public interface、输入/输出/Error Schema 和版本策略。
- [ ] 实现 `spring-crash-recovery` 的 production adapter/service，不把业务状态只保存在 Prompt 或内存对象中。
...
- [ ] 至少生成：recovery-decision.json, ownership-events, manual-reconcile-queue。
...
- [ ] 本 Skill 的 required outputs 均由真实目标仓库执行产生：recovery-decision.json, ownership-events, manual-reconcile-queue。
- [ ] 所有 mandatory tests 和 security gates 通过，且无 critical unknown。

## Dependencies

- `spring-side-effect-ledger`
...
- Machine contract: `references/contract.json` (`sha256:96ac5f85ace627908e4b385ba421f18ef815526ed240f30597c5e715ba589c6d`).
