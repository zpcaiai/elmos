---
name: spring-native-recipes
description: 独立实现可商业使用的高频 Spring 确定性规则资产。 Use for this exact ELMOS Spring Golden Route v2 contract; imported specifications are not runtime or certification evidence.
...
metadata:
  ...
  source_id: ELMOS-GR-043
  source_name: spring-native-recipes
  source_path: skills/spring-native-recipes/SKILL.md
  source_sha256: sha256:ca365e2005da7a4fbe585a7b53547590109581a3f3beb9c91d7fdb0108e1819a
  source_contract_sha256: sha256:3c47906167ace2535cdbc9736fa8cc23ae085de068ceec309bf7ecdfbc546e88
...
# Elmos Native Spring Recipes

## Objective

独立实现可商业使用的高频 Spring 确定性规则资产。

## Scope and capabilities

- Jakarta/Build/Starter/Properties
- MVC/Security/Data 常见迁移
- Before/After/Negative/Idempotency
- Candidate→Certified
- 许可 provenance
...
- [ ] 定义 `spring-native-recipes` 的 public interface、输入/输出/Error Schema 和版本策略。
- [ ] 实现 `spring-native-recipes` 的 production adapter/service，不把业务状态只保存在 Prompt 或内存对象中。
...
- [ ] 至少生成：native-recipe-pack, fixtures, coverage-report.json。
...
- [ ] 全量 Recipe 最终收敛
- [ ] Agent 只接收剩余业务歧义
- [ ] 本 Skill 的 required outputs 均由真实目标仓库执行产生：native-recipe-pack, fixtures, coverage-report.json。
- [ ] 所有 mandatory tests 和 security gates 通过，且无 critical unknown。

## Dependencies

- `spring-openrewrite-adapter`
...
- Machine contract: `references/contract.json` (`sha256:3c47906167ace2535cdbc9736fa8cc23ae085de068ceec309bf7ecdfbc546e88`).
