...
description: "Run FRT-2615 Impacted Test Selection for FRT G26 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-2615"
  source_name: "frt-2615-impacted-test-selection"
  source_sha256: "sha256:d45ed2db547c0fe93a4afca8d97d34e65541da0b56b6df1e0eb12bd771bac3a8"
  batch: "G26"
...
- Runtime catalog key: `FRT-2615` / `frt-2615-impacted-test-selection`.
...
在G26范围内实现 **Impacted Test Selection**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：Requirement/Acceptance/State Machine派生Test Obligation; Unit/Component/Contract/Integration/Journey分层协调; Differential、Mutation、Visual/Semantic、Security和Performance Regression。

## When to Use

- 实现或修改与 **Impacted Test Selection** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-2615Result`：版本化、带状态和诊断的结构化结果。
- `FRT-2615EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-2615FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-2615CertificateFragment`：供G26批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g26/frt-2615-impacted-test-selection/
packages/runtime/g26/frt-2615-impacted-test-selection/
services/control-plane/g26/frt-2615-impacted-test-selection/
apps/web-console/src/features/g26/frt-2615-impacted-test-selection/
apps/admin-console/src/features/g26/frt-2615-impacted-test-selection/
tests/g26/frt-2615-impacted-test-selection/
```
...
```text
POST /v1/skills/frt-2615/runs
GET  /v1/skills/frt-2615/runs/{run_id}
POST /v1/skills/frt-2615/runs/{run_id}/verify
GET  /v1/skills/frt-2615/runs/{run_id}/findings
GET  /v1/skills/frt-2615/runs/{run_id}/evidence
```
