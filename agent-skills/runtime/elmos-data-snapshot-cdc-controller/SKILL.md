---
name: elmos-data-snapshot-cdc-controller
description: Execute initial snapshots, change data capture, ordering, deduplication, tombstones, gap detection and final reconciliation for database migration.
version: 4.0.0
...
# Data Snapshot and CDC Controller
...
- Create consistent snapshots with source position
- Persist resumable CDC offsets and schema history
- Deduplicate and order changes idempotently
- Propagate deletes/tombstones
- Detect gaps and block cutover
...
- **lost CDC segment** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **duplicate apply corrupts target** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **source position not bound to snapshot** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **tombstone omitted** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **schema evolution breaks decoder** — must have a preventive control, a detective signal, a negative fixture and a response action.
