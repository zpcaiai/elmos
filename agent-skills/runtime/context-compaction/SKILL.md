...
description: 上下文超预算前生成结构化 checkpoint，而完整 transcript 仍永久保留。 Use for this exact ELMOS Spring Golden Route v2 contract; imported specifications are not runtime or certification evidence.
...
metadata:
  ...
  source_id: FOUNDATION-07-context-compaction
  source_name: context-compaction
  source_path: skills/context-compaction/SKILL.md
  source_sha256: sha256:086885846aa2955c254854f98a04a5e6cad7338e1719e68944c76ba6642fca3c
  source_contract_sha256: sha256:34777edf823782f6de7e9aa61d3a2969000de192e90e4a5ce22f932b9a9ffaf8
...
上下文超预算前生成结构化 checkpoint，而完整 transcript 仍永久保留。

## Use when

在 Elmos 需要实现或升级 **Context Compaction** 相关能力时使用；它不是仅供解释的提示词，而是实现、测试与验收契约。

## Capabilities to implement

- 按 context window - reserved headroom 预算触发
- rolling summary + bounded recent context
- 只有 compaction 完成事件才切换 active boundary
- provider overflow 可触发一次受控补救 compaction
...
- [ ] 定义 `context-compaction` 的 public interface、input/output/error schemas。
- [ ] 建立 `context-compaction` 的 capability/feature flags 与版本字段。
...
- [ ] 至少覆盖并验收：按 context window - reserved headroom 预算触发；以及 rolling summary + bounded recent context。
...
- Machine contract: `references/contract.json` (`sha256:34777edf823782f6de7e9aa61d3a2969000de192e90e4a5ce22f932b9a9ffaf8`).
