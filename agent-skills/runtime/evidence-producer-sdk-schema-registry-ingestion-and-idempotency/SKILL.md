---
name: evidence-producer-sdk-schema-registry-ingestion-and-idempotency
description: 建立统一 Evidence Producer 注册、Schema Registry、SDK、幂等摄取和生产者一致性测试。
metadata:
  elmos_batch: "37A"
  elmos_skill_id: "ART37A003"
  pack_version: "3.7-complete"
---

# ART37A003 — Evidence Producer SDK、Schema Registry、Ingestion 与 Idempotency

## Objective

建立统一 Evidence Producer 注册、Schema Registry、SDK、幂等摄取和生产者一致性测试。
...
- 将 Workload Identity 绑定 Producer ID、Version、Tenant Scope 与 Artifact Type。
- 实现 JSON Schema 主路径及兼容性策略。
- 摄取流程验证身份、Scope、Schema、Payload Digest、Classification 后注册 Artifact。
- 提供 Java/Go SDK、CLI Helper、Batch 与 Source-local Metadata-only 模式。
...
- producerCannotImpersonateAnotherProducer
- unknownSchemaIsRejected
- sameKeySameDigestIsIdempotent
- sameKeyDifferentDigestCreatesConflict
