...
metadata:
  ...
  source_skill: project-intelligence-validation
...
---
name: project-intelligence-validation
description: Validate all declared Repository Intelligence, Code Reading and Architecture Understanding capabilities with executable positive, boundary, adversarial, concurrency, recovery and evidence-backed release tests.
---

# Repository Intelligence, Code Reading and Architecture Understanding Validation

## Purpose

Use this Skill to plan, execute and certify the `project-intelligence` product domain. It owns **71 declared capabilities** (39 P0, 27 P1, 5 P2) across 4 execution contexts and four mandatory variants.

## Inputs

- Frozen release candidate and feature-registry revision.
- Tenant, project, environment authority, budget and fencing context.
- Concrete cases from `suites/full-product.jsonl` whose business line is `project-intelligence`.
...
2. Provision an isolated context for: single-repository, monorepo, multi-repository-system, runtime-trace-fused.
...
5. Apply the domain Oracle `project-intelligence-evidence-coverage-traceability-and-accuracy` plus security, state, audit and unsupported-disclosure Oracles.
...
Representative capabilities include: Repository Manifest And Fingerprint, Multilingual Parser Coverage, Symbol Index, Definition Reference Navigation, Call Graph, Dependency Graph, Module And Boundary Discovery, Entrypoint Discovery, Configuration And Profile Discovery, Database Schema And Orm Map, Api Route And Contract Map, Event Message Topology. The complete source of truth is `matrices/full-product.yaml`; no prose list may override it.
...
Route execution through `external-project-intelligence-harness` under `integrations/harness/full-product-adapters.yaml`. An adapter stub or skipped case is implementation progress, not product certification.
