...
description: "Run FRT-2703 Race and Lost-Update Detector for FRT G27 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-2703"
  source_name: "frt-2703-race-and-lost-update-detector"
  source_sha256: "sha256:6d9dc191aff3f1091bc7bc60f6f1ced6398ff25816c28a112d223e3501a4c2e4"
  batch: "G27"
...
- Runtime catalog key: `FRT-2703` / `frt-2703-race-and-lost-update-detector`.
...
在G27范围内实现 **Race and Lost-Update Detector**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：业务化Workload Model; 并发线性化点、Race、Lost Update、Idempotency Under Load; Load/Stress/Spike/Soak与Recovery。

## When to Use

- 实现或修改与 **Race and Lost-Update Detector** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-2703Result`：版本化、带状态和诊断的结构化结果。
- `FRT-2703EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-2703FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-2703CertificateFragment`：供G27批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g27/frt-2703-race-and-lost-update-detector/
packages/runtime/g27/frt-2703-race-and-lost-update-detector/
services/control-plane/g27/frt-2703-race-and-lost-update-detector/
apps/web-console/src/features/g27/frt-2703-race-and-lost-update-detector/
apps/admin-console/src/features/g27/frt-2703-race-and-lost-update-detector/
tests/g27/frt-2703-race-and-lost-update-detector/
```
...
```text
POST /v1/skills/frt-2703/runs
GET  /v1/skills/frt-2703/runs/{run_id}
POST /v1/skills/frt-2703/runs/{run_id}/verify
GET  /v1/skills/frt-2703/runs/{run_id}/findings
GET  /v1/skills/frt-2703/runs/{run_id}/evidence
```
