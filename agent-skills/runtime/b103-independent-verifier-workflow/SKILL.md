---
name: b103-independent-verifier-workflow
description: "Use when ELMOS must Run evidence review and selected re-execution by an identity independent from implementation. Triggers: Independent Verifier Workflow, Evidence and Certification Fabric Pack, independent verification, re-execution, review sampling, decision."
metadata:
  id: B103-S13
...
# B103-S13 — Independent Verifier Workflow

## Objective

Run evidence review and selected re-execution by an identity independent from implementation.
...
Use this Skill when the approved task explicitly requires **Independent Verifier Workflow** or when the capability graph selects it as part of a minimal dependency closure.
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b103-independent-verifier-workflow/SKILL.md --out /tmp/independent_verifier_workflow.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B103-S13
```
...
- Reject or expose same agent self-verifies.
- Reject or expose evidence inaccessible.
- Reject or expose failed re-run ignored.
