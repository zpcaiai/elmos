---
name: b98-skill-registry-service
description: "Use when ELMOS must Implement tenant-aware storage, retrieval, lifecycle and immutable versioning for executable Skill contracts. Triggers: Skill Registry Service, Executable Skill Contract and Registry Pack, registry API, immutable versions, tenant isolation, lifecycle."
metadata:
  id: B98-S11
...
# B98-S11 — Skill Registry Service

## Objective

Implement tenant-aware storage, retrieval, lifecycle and immutable versioning for executable Skill contracts.
...
Use this Skill when the approved task explicitly requires **Skill Registry Service** or when the capability graph selects it as part of a minimal dependency closure.
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b98-skill-registry-service/SKILL.md --out /tmp/skill_registry_service.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B98-S11
```
...
- Reject or expose version overwrite.
- Reject or expose cross-tenant read.
- Reject or expose deleted contract still selectable.
