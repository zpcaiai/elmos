...
description: "Run FRT-1011 Realtime Webhook and Subscription Mapper for FRT G10 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-1011"
  source_name: "frt-1011-realtime-webhook-and-subscription-mapper"
  source_sha256: "sha256:7244c83d903f7863de6c2df55de4aa7715d251d0fed93e2a9d64eb3dea1c8eeb"
  batch: "G10"
  source_version: "1.0.0"
  source_risk: "critical-by-default"
  source_certificate_family: "AB"
...
- Runtime catalog key: `FRT-1011` / `frt-1011-realtime-webhook-and-subscription-mapper`.
...
# FRT-1011 — Realtime Webhook and Subscription Mapper

## Objective

在G10范围内实现 **Realtime Webhook and Subscription Mapper**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：保持Route、History、Guard和Deep Link语义; 表单Validation、错误、草稿和提交幂等完整; API、Event、Storage和Realtime边界显式。

## When to Use

- 实现或修改与 **Realtime Webhook and Subscription Mapper** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-1011Result`：版本化、带状态和诊断的结构化结果。
- `FRT-1011EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-1011FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-1011CertificateFragment`：供G10批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g10/frt-1011-realtime-webhook-and-subscription-mapper/
packages/runtime/g10/frt-1011-realtime-webhook-and-subscription-mapper/
services/control-plane/g10/frt-1011-realtime-webhook-and-subscription-mapper/
apps/web-console/src/features/g10/frt-1011-realtime-webhook-and-subscription-mapper/
apps/admin-console/src/features/g10/frt-1011-realtime-webhook-and-subscription-mapper/
tests/g10/frt-1011-realtime-webhook-and-subscription-mapper/
```
...
```text
POST /v1/skills/frt-1011/runs
GET  /v1/skills/frt-1011/runs/{run_id}
POST /v1/skills/frt-1011/runs/{run_id}/verify
GET  /v1/skills/frt-1011/runs/{run_id}/findings
GET  /v1/skills/frt-1011/runs/{run_id}/evidence
```
