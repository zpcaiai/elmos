---
name: b98-permission-capability-declaration
description: "Use when ELMOS must Declare least-privilege process, filesystem, network, repository, secret, cloud and signing capabilities for each Skill. Triggers: Permission and Capability Declaration, Executable Skill Contract and Registry Pack, capability manifest, least privilege, egress policy, secret scopes."
metadata:
  id: B98-S05
...
# B98-S05 — Permission and Capability Declaration

## Objective

Declare least-privilege process, filesystem, network, repository, secret, cloud and signing capabilities for each Skill.
...
Use this Skill when the approved task explicitly requires **Permission and Capability Declaration** or when the capability graph selects it as part of a minimal dependency closure.
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b98-permission-capability-declaration/SKILL.md --out /tmp/permission_capability_declaration.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B98-S05
```
...
- Reject or expose wildcard egress.
- Reject or expose ambient credentials.
- Reject or expose hidden subprocess capability.
