...
description: "Run FRT-1407 Mini Program Capability Gap Planner for FRT G14 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-1407"
  source_name: "frt-1407-mini-program-capability-gap-planner"
  source_sha256: "sha256:a74b11af3120e2d722b5a1e6d0e7f54fbca84be2c6c33d0c7b22dfbcdb5ecf6f"
  batch: "G14"
...
- Runtime catalog key: `FRT-1407` / `frt-1407-mini-program-capability-gap-planner`.
...
# FRT-1407 — Mini Program Capability Gap Planner

## Objective

在G14范围内实现 **Mini Program Capability Gap Planner**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：处理WXML/WXSS/setData/Page/Component与Web组件模型; 处理分包、基础库、授权、支付、Storage和页面栈限制; Web缺失能力和小程序限制显式Gap/Unsupported。

## When to Use

- 实现或修改与 **Mini Program Capability Gap Planner** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-1407Result`：版本化、带状态和诊断的结构化结果。
- `FRT-1407EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-1407FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-1407CertificateFragment`：供G14批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g14/frt-1407-mini-program-capability-gap-planner/
packages/runtime/g14/frt-1407-mini-program-capability-gap-planner/
services/control-plane/g14/frt-1407-mini-program-capability-gap-planner/
apps/web-console/src/features/g14/frt-1407-mini-program-capability-gap-planner/
apps/admin-console/src/features/g14/frt-1407-mini-program-capability-gap-planner/
tests/g14/frt-1407-mini-program-capability-gap-planner/
```
...
```text
POST /v1/skills/frt-1407/runs
GET  /v1/skills/frt-1407/runs/{run_id}
POST /v1/skills/frt-1407/runs/{run_id}/verify
GET  /v1/skills/frt-1407/runs/{run_id}/findings
GET  /v1/skills/frt-1407/runs/{run_id}/evidence
```
