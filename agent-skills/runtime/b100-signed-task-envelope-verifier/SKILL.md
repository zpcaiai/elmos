---
name: b100-signed-task-envelope-verifier
description: "Use when ELMOS must Verify task signature, nonce, audience, scope, expiry and capability manifest before claim. Triggers: Signed Task Envelope Verifier, Hardened Runner Fabric Pack, task envelope, signature, anti-replay, audience."
metadata:
  id: B100-S04
...
# B100-S04 — Signed Task Envelope Verifier

## Objective

Verify task signature, nonce, audience, scope, expiry and capability manifest before claim.
...
Use this Skill when the approved task explicitly requires **Signed Task Envelope Verifier** or when the capability graph selects it as part of a minimal dependency closure.
...
Package dependencies: `B100-S03`, `B98-S05`
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b100-signed-task-envelope-verifier/SKILL.md --out /tmp/signed_task_envelope_verifier.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B100-S04
```
...
- Reject or expose signature confusion.
- Reject or expose cross-fleet replay.
- Reject or expose audience wildcard.
