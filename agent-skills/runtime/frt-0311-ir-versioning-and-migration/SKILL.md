...
description: "Run FRT-0311 IR Versioning and Migration for FRT G03 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-0311"
  source_name: "frt-0311-ir-versioning-and-migration"
  source_sha256: "sha256:7bfe0426cecb4bef5f348b0e70664094c62c5ce725fd9008af640a81f2bee241"
  batch: "G03"
...
- Runtime catalog key: `FRT-0311` / `frt-0311-ir-versioning-and-migration`.
...
# FRT-0311 — IR Versioning and Migration

## Objective

在G03范围内实现 **IR Versioning and Migration**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：建立跨Vue/React/小程序/ArkUI/Flutter的类型和语义中间层; 保留Null、Identity、Effect、Lifecycle、Navigation、Data Authority和UI语义; 每个IR节点绑定Source Range和稳定Semantic ID。

## When to Use

- 实现或修改与 **IR Versioning and Migration** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-0311Result`：版本化、带状态和诊断的结构化结果。
- `FRT-0311EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-0311FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-0311CertificateFragment`：供G03批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g03/frt-0311-ir-versioning-and-migration/
packages/runtime/g03/frt-0311-ir-versioning-and-migration/
services/control-plane/g03/frt-0311-ir-versioning-and-migration/
apps/web-console/src/features/g03/frt-0311-ir-versioning-and-migration/
apps/admin-console/src/features/g03/frt-0311-ir-versioning-and-migration/
tests/g03/frt-0311-ir-versioning-and-migration/
```
...
```text
POST /v1/skills/frt-0311/runs
GET  /v1/skills/frt-0311/runs/{run_id}
POST /v1/skills/frt-0311/runs/{run_id}/verify
GET  /v1/skills/frt-0311/runs/{run_id}/findings
GET  /v1/skills/frt-0311/runs/{run_id}/evidence
```
