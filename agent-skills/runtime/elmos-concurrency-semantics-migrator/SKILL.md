---
name: elmos-concurrency-semantics-migrator
description: "Map threads, tasks, coroutines, actors, event loops, goroutines, channels, locks, cancellation, and backpressure into target semantics."
version: 1.0.0
skill_id: ELMOS-POLY-024
layer: transformation
risk: critical
readiness: not-run
dependencies:
  - "elmos-data-access-transaction-migrator"
triggers:
  - "Use when implementing or executing `elmos-concurrency-semantics-migrator`."
  - "Use when the current DAG node requires the transformation capability."
outputs:
  - "concurrency-mapping.json"
  - "target-concurrency-layer/"
  - "race-test-plan.json"
  - "deadlock-risk-register.json"
---

# Concurrency Semantics Migrator

## Objective

Map threads, tasks, coroutines, actors, event loops, goroutines, channels, locks, cancellation, and backpressure into target semantics.
...
1. Inventory concurrency entry points, shared state, synchronization, queues, async boundaries, cancellation, and thread affinity.
2. Map source constructs to target primitives using happens-before and ownership requirements.
3. Preserve ordering, at-most/at-least-once, timeout, cancellation, and backpressure semantics.
4. Handle UI main-thread and event-loop affinity explicitly.
5. Replace unsafe shared mutation with target-safe patterns where approved.
6. Instrument correlation IDs and deterministic schedulers for tests.
7. Run race, deadlock, starvation, leak, and cancellation tests.
8. Escalate semantics that cannot be preserved mechanically.
...
- [ ] Race detectors or equivalent tests run where supported..
- [ ] Cancellation propagates across translated boundaries..
- [ ] Ordering and backpressure fixtures pass..
- [ ] Thread-affinity violations are detected..
