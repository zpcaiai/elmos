...
description: "Run FRT-3021 Production Closure Certificate for FRT G30 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-3021"
  source_name: "frt-3021-production-closure-certificate"
  source_sha256: "sha256:af733480c9a79875ae540acdfa155993fa771fb73957baeb4bf1c960345cf312"
  batch: "G30"
...
- Runtime catalog key: `FRT-3021` / `frt-3021-production-closure-certificate`.
...
在G30范围内实现 **Production Closure Certificate**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：九类Certificate聚合与唯一Production Authority; Service Catalog、Owner、Tier、Dependency; SLI/SLO、Business SLI、Error Budget和Burn Rate。

## When to Use

- 实现或修改与 **Production Closure Certificate** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-3021Result`：版本化、带状态和诊断的结构化结果。
- `FRT-3021EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-3021FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-3021CertificateFragment`：供G30批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g30/frt-3021-production-closure-certificate/
packages/runtime/g30/frt-3021-production-closure-certificate/
services/control-plane/g30/frt-3021-production-closure-certificate/
apps/web-console/src/features/g30/frt-3021-production-closure-certificate/
apps/admin-console/src/features/g30/frt-3021-production-closure-certificate/
tests/g30/frt-3021-production-closure-certificate/
```
...
```text
POST /v1/skills/frt-3021/runs
GET  /v1/skills/frt-3021/runs/{run_id}
POST /v1/skills/frt-3021/runs/{run_id}/verify
GET  /v1/skills/frt-3021/runs/{run_id}/findings
GET  /v1/skills/frt-3021/runs/{run_id}/evidence
```
