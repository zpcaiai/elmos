...
description: "Run FRT-2108 Orphan Feature and Dead Flow Detector for FRT G21 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-2108"
  source_name: "frt-2108-orphan-feature-and-dead-flow-detector"
  source_sha256: "sha256:de7b4c30d3c0869da7d7c19fe309625184cf9186490a7e098131ed3f525449fc"
  batch: "G21"
  source_version: "1.0.0"
  source_risk: "critical-by-default"
  source_certificate_family: "FC"
...
- Runtime catalog key: `FRT-2108` / `frt-2108-orphan-feature-and-dead-flow-detector`.
...
在G21范围内实现 **Orphan Feature and Dead Flow Detector**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：统一 Product Closure Graph; Requirement、Capability、Journey、Command、API、Data、Permission、Test、Monitoring、Runbook、Certificate 双向追踪; 孤儿 UI/API/Command/Event/Data Write 检测。

## When to Use

- 实现或修改与 **Orphan Feature and Dead Flow Detector** 对应的Contract、Runtime、API、UI、测试或证据链时。
- 相关代码、Schema、Pack、Policy、环境或依赖变化导致既有Evidence失效时。
- G21 Orchestrator生成该Skill的执行义务时。
...
- `SourceArtifactSet`：PRD、用户故事、ADR、API Contract、安全和运营政策, G1–G20 生成的 Semantic IR、代码、测试、Proof 与 Artifact。
- `SkillPolicy`：权限、网络、Sandbox、Toolchain、Evidence和停止规则。

## Outputs

- `FRT-2108Result`：版本化、带状态和诊断的结构化结果。
- `FRT-2108EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-2108FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-2108CertificateFragment`：供G21批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g21/frt-2108-orphan-feature-and-dead-flow-detector/
packages/runtime/g21/frt-2108-orphan-feature-and-dead-flow-detector/
services/control-plane/g21/frt-2108-orphan-feature-and-dead-flow-detector/
apps/web-console/src/features/g21/frt-2108-orphan-feature-and-dead-flow-detector/
apps/admin-console/src/features/g21/frt-2108-orphan-feature-and-dead-flow-detector/
tests/g21/frt-2108-orphan-feature-and-dead-flow-detector/
```
...
```text
POST /v1/skills/frt-2108/runs
GET  /v1/skills/frt-2108/runs/{run_id}
POST /v1/skills/frt-2108/runs/{run_id}/verify
GET  /v1/skills/frt-2108/runs/{run_id}/findings
GET  /v1/skills/frt-2108/runs/{run_id}/evidence
```
...
- The parent G21 Orchestrator can consume the result and independently decide certification.
