---
name: b99-concurrency-fairness-controller
description: "Use when ELMOS must Control global, tenant, repository and resource-key concurrency with anti-starvation behavior. Triggers: Concurrency and Fairness Controller, Durable Runtime Kernel Pack, concurrency keys, fairness, rate limits, resource locks."
metadata:
  id: B99-S13
...
# B99-S13 — Concurrency and Fairness Controller

## Objective

Control global, tenant, repository and resource-key concurrency with anti-starvation behavior.
...
Use this Skill when the approved task explicitly requires **Concurrency and Fairness Controller** or when the capability graph selects it as part of a minimal dependency closure.
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b99-concurrency-fairness-controller/SKILL.md --out /tmp/concurrency_fairness_controller.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B99-S13
```
...
- Reject or expose lock leak.
- Reject or expose tenant starvation.
- Reject or expose resource-key collision.
