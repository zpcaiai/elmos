---
name: b99-external-effect-outbox-inbox
description: "Use when ELMOS must Provide durable outbox/inbox patterns for messages, webhooks and provider calls with deduplication. Triggers: External Effect Outbox and Inbox, Durable Runtime Kernel Pack, outbox, inbox, delivery state, dedupe."
metadata:
  id: B99-S11
...
# B99-S11 — External Effect Outbox and Inbox

## Objective

Provide durable outbox/inbox patterns for messages, webhooks and provider calls with deduplication.
...
Use this Skill when the approved task explicitly requires **External Effect Outbox and Inbox** or when the capability graph selects it as part of a minimal dependency closure.
...
- `outbox_schema.sql`
- `delivery_protocol.json`
- `delivery_test_report.json`
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b99-external-effect-outbox-inbox/SKILL.md --out /tmp/external_effect_outbox_inbox.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B99-S11
```
...
- Reject or expose message loss window.
- Reject or expose duplicate side effect.
- Reject or expose poison message infinite loop.
