...
metadata:
  ...
  source_digest: sha256:80a00fc0b0937e296e3b263506bf84606f3fc21d7c2f47cade3babfd537ded85
  phase: semantic-recovery
  runtime_state: CODE_COMPLETE_LOCAL
  capability_state: LOCAL_EXECUTED
  operation_code: STRUTS2_PIPELINES_RECOVERED
  runtime_handler_id: legacy-web-handler:21-struts2-interceptor-pipeline-recovery
---
...
---
id: 21-struts2-interceptor-pipeline-recovery
title: Struts 2 拦截器流水线恢复
version: 1.0.0
phase: semantic-recovery
priority: P0
requires:
...
展开 package 继承、interceptor stack、条件/短路、参数覆盖、ValueStack/OGNL、PreResultListener、Result 与 action chain。
...
producerSkill: 21-struts2-interceptor-pipeline-recovery
...
6. 展开 package 继承和 interceptor-ref 参数覆盖，保留 stack 中的顺序与递归 around-invocation 语义。
7. 识别可短路 interceptor、PreResultListener、Result 执行、ActionChainResult 和 async action。
8. 恢复 ValueStack/OGNL 的读写对象、参数白名单、类型转换和错误处理。
9. 对 custom interceptor 生成 before/action/after/short-circuit 四段契约。
