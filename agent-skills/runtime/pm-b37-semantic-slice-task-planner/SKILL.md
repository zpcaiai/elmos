...
# Semantic Slice Task Planner
## ELMOS runtime binding

- Invoke this repository Skill as `$pm-b37-semantic-slice-task-planner`.
- Immutable source identity: `semantic-slice-task-planner` in `precision-migration-b01-44` (B37).
- Runtime adapter: `model-routing-and-agent-harness`; binding state: `UNAVAILABLE`.
- Resolve and plan with `python3 scripts/precision_migration/runtime.py plan --skill pm-b37-semantic-slice-task-planner`.
...
- Version: `1.0.0`
- Tags: agentic-migration-harness, batch-37

## Purpose

为每个 Agent 准备最小语义切片、契约、规则、工具和验收条件。

本 Skill 属于精密语言转换系统。它必须以**可执行证据**为准，不得用模型自评、源码相似度或“能编译”替代语义与行为验证。

## Use when

- 当前任务直接涉及 `semantic-slice-task-planner` 所描述的能力。
...
1. 计算风险并拆分任务
2. 执行本 Skill 的核心职责：为每个 Agent 准备最小语义切片、契约、规则、工具和验收条件。
3. 选择模型与隔离工作区
4. 执行并收集客观反馈
5. 独立审查和升级
6. 持久化状态、证据、成本与审批
...
```yaml
skill: semantic-slice-task-planner
version: 1.0.0
