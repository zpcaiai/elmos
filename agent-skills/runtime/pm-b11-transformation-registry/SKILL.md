...
- Invoke this repository Skill as `$pm-b11-transformation-registry`.
- Immutable source identity: `transformation-registry` in `precision-migration-b01-44` (B11).
- Runtime adapter: `semantic-recovery-and-ir`; binding state: `UNAVAILABLE`.
- Resolve and plan with `python3 scripts/precision_migration/runtime.py plan --skill pm-b11-transformation-registry`.
...
登记、发现、签名、授权、发布和撤回转换技能与方向包。

本 Skill 属于精密语言转换系统。它必须以**可执行证据**为准，不得用模型自评、源码相似度或“能编译”替代语义与行为验证。

## Use when

- 当前任务直接涉及 `transformation-registry` 所描述的能力。
...
1. 验证规则前置条件
2. 执行本 Skill 的核心职责：登记、发现、签名、授权、发布和撤回转换技能与方向包。
3. 优先使用确定性规则
4. 对长尾生成多个候选
5. 按可验证性和成本排序
6. 运行最小构建/静态验证并输出结果集
