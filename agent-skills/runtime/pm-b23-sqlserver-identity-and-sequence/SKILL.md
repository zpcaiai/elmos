---
name: pm-b23-sqlserver-identity-and-sequence
description: "迁移 IDENTITY、Sequence、SET IDENTITY_INSERT 和取号行为. Precision Migration B23 contract; use for this exact assessment, transformation, validation, repair, evidence, or cutover scope."
---

# Sqlserver Identity And Sequence
## ELMOS runtime binding

- Invoke this repository Skill as `$pm-b23-sqlserver-identity-and-sequence`.
- Immutable source identity: `sqlserver-identity-and-sequence` in `precision-migration-b01-44` (B23).
- Runtime adapter: `database-and-data-route`; binding state: `DECLARED`.
- Resolve and plan with `python3 scripts/precision_migration/runtime.py plan --skill pm-b23-sqlserver-identity-and-sequence`.
...
- Phase: `G 数据库精密互转`
- Version: `1.0.0`
- Tags: batch-23, sqlserver-proprietary-pack

## Purpose

迁移 IDENTITY、Sequence、SET IDENTITY_INSERT 和取号行为。

本 Skill 属于精密语言转换系统。它必须以**可执行证据**为准，不得用模型自评、源码相似度或“能编译”替代语义与行为验证。

## Use when

- 当前任务直接涉及 `sqlserver-identity-and-sequence` 所描述的能力。
...
1. 盘点对象与专有能力
2. 执行本 Skill 的核心职责：迁移 IDENTITY、Sequence、SET IDENTITY_INSERT 和取号行为。
...
```yaml
skill: sqlserver-identity-and-sequence
version: 1.0.0
