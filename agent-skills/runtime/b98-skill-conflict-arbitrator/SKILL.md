---
name: b98-skill-conflict-arbitrator
description: "Use when ELMOS must Arbitrate overlapping write sets, contradictory policies and competing implementations before execution. Triggers: Skill Conflict Arbitrator, Executable Skill Contract and Registry Pack, write-set analysis, policy precedence, human decision, merge strategy."
metadata:
  id: B98-S14
...
# B98-S14 — Skill Conflict Arbitrator

## Objective

Arbitrate overlapping write sets, contradictory policies and competing implementations before execution.
...
Use this Skill when the approved task explicitly requires **Skill Conflict Arbitrator** or when the capability graph selects it as part of a minimal dependency closure.
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b98-skill-conflict-arbitrator/SKILL.md --out /tmp/skill_conflict_arbitrator.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B98-S14
```
...
- Reject or expose last-writer-wins.
- Reject or expose self-approved conflict.
- Reject or expose hidden generated-file collision.
