---
name: elmos-database-performance-plan-certifier
description: Certify representative query plans, latency, throughput, resource use, statistics sensitivity and capacity under target database versions and data distributions.
version: 4.0.0
...
# Database Performance and Plan Certifier
...
- **Kernel authorities used:** K2, K6, K8
...
- Capture normalized and native plans
- Test representative cardinality/data skew
- Use statistical latency and throughput gates
- Detect plan instability after version/statistics change
- Bind certificate to workload and parameter envelope
...
- `elmos-sql-dual-execution-oracle` from `elmos-ai-capability-enhancement-skills` — required base evidence producer.
- `elmos-performance-statistical-gate` from `elmos-ai-capability-enhancement-skills` — required base evidence producer.

## Primary Outputs

- `database-performance-certificate.json`
- `query-plan-baselines/`
- `plan-regression-report.json`
- `database-capacity-model.json`
...
- **benchmark data unrepresentative** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **warm cache hides regression** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **plan hash stable but behavior worse** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **optimizer hint copied incorrectly** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **performance claim extrapolated beyond capacity** — must have a preventive control, a detective signal, a negative fixture and a response action.
...
- TPC-style representative query set
- data skew plan choice
- index usage
- parameter sniffing/bind peeking
- statistics refresh
- P95/P99 regression
