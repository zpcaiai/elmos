...
description: "Run FRT-3012 Feature Flag Safety Validator for FRT G30 with typed frontend contracts, tenant scope, immutable evidence, and fail-closed certification boundaries."
metadata:
  source_package: "FRT_G01_G30_Complete_Skills_Pack"
  source_skill_id: "FRT-3012"
  source_name: "frt-3012-feature-flag-safety-validator"
  source_sha256: "sha256:9827c2b648ed3b9f7aee509c8c3565331511f65419e8f831704acfca9529e39c"
  batch: "G30"
...
- Runtime catalog key: `FRT-3012` / `frt-3012-feature-flag-safety-validator`.
...
在G30范围内实现 **Feature Flag Safety Validator**，并确保其结果可以被后续Batch、测试、管理端和证书系统稳定消费。该Skill服务于：九类Certificate聚合与唯一Production Authority; Service Catalog、Owner、Tier、Dependency; SLI/SLO、Business SLI、Error Budget和Burn Rate。

## When to Use

- 实现或修改与 **Feature Flag Safety Validator** 对应的Contract、Runtime、API、UI、测试或证据链时。
...
- `FRT-3012Result`：版本化、带状态和诊断的结构化结果。
- `FRT-3012EvidenceBundle`：输入、执行、测试、Finding和输出Digest。
- `FRT-3012FindingSet`：error、warning、gap、manual-decision与blocker。
- `FRT-3012CertificateFragment`：供G30批次证书聚合，不得自行宣告全批通过。

## Required Implementation Surfaces

```text
packages/contracts/g30/frt-3012-feature-flag-safety-validator/
packages/runtime/g30/frt-3012-feature-flag-safety-validator/
services/control-plane/g30/frt-3012-feature-flag-safety-validator/
apps/web-console/src/features/g30/frt-3012-feature-flag-safety-validator/
apps/admin-console/src/features/g30/frt-3012-feature-flag-safety-validator/
tests/g30/frt-3012-feature-flag-safety-validator/
```
...
```text
POST /v1/skills/frt-3012/runs
GET  /v1/skills/frt-3012/runs/{run_id}
POST /v1/skills/frt-3012/runs/{run_id}/verify
GET  /v1/skills/frt-3012/runs/{run_id}/findings
GET  /v1/skills/frt-3012/runs/{run_id}/evidence
```
