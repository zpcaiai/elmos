---
name: b100-process-execution-allowlist
description: "Use when ELMOS must Execute only inspected, declared and policy-authorized commands without arbitrary shell escape. Triggers: Process Execution Allowlist, Hardened Runner Fabric Pack, process policy, argument arrays, child process control, lifecycle scripts."
metadata:
  id: B100-S07
...
# B100-S07 — Process Execution Allowlist

## Objective

Execute only inspected, declared and policy-authorized commands without arbitrary shell escape.
...
Use this Skill when the approved task explicitly requires **Process Execution Allowlist** or when the capability graph selects it as part of a minimal dependency closure.
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b100-process-execution-allowlist/SKILL.md --out /tmp/process_execution_allowlist.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B100-S07
```
...
- Reject or expose shell injection.
- Reject or expose PATH hijack.
- Reject or expose interpreter escape.
