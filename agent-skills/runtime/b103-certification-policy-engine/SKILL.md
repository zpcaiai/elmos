---
name: b103-certification-policy-engine
description: "Use when ELMOS must Evaluate versioned certification rules deterministically against exact evidence scopes. Triggers: Certification Policy Engine, Evidence and Certification Fabric Pack, policy engine, rule versions, deterministic evaluation, explanations."
metadata:
  id: B103-S11
...
# B103-S11 — Certification Policy Engine

## Objective

Evaluate versioned certification rules deterministically against exact evidence scopes.
...
Use this Skill when the approved task explicitly requires **Certification Policy Engine** or when the capability graph selects it as part of a minimal dependency closure.
...
Package dependencies: `B103-S10`, `B98-S08`
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b103-certification-policy-engine/SKILL.md --out /tmp/certification_policy_engine.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B103-S11
```
...
- Reject or expose policy hot-swap.
- Reject or expose unknown-as-pass.
- Reject or expose hidden manual override.
