---
name: elmos-multi-agent-budget-scheduler
description: Allocate model, tool, latency, fanout and cost budgets across agent teams using priority, uncertainty, critical
  path and verified task-fit.
version: 4.0.0
...
- `DeploymentUnit`
- `ConformanceRun`
- `MultiAgentBudgetSchedulerRun`
- `MultiAgentBudgetSchedulerDecision`

Required semantic capabilities:

- Per-agent and shared budget envelopes
- Critical-path and slack-aware scheduling
- Dynamic reallocation under uncertainty
- Priority/fairness and tenant quota
- Budget-aware stop and degraded mode
...
- `agent-budget-plan.json`
- `critical-path-schedule.json`
- `budget-events.jsonl`
- `scheduler-performance-report.json`
...
- **starvation** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **budget theft by subagent** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **priority inversion** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **fanout explosion** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **unbounded speculative work** — must have a preventive control, a detective signal, a negative fixture and a response action.
