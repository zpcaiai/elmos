---
name: bulk-backfill-planner
description: "Use when Batch 11 must plan partitioned, throttled, checkpointed and side-effect-free historical data backfill."
---
...
2. Plan partitioned, throttled, checkpointed and side-effect-free historical data backfill.
...
- Block on source mutation, user side effect, non-idempotent target write or unbounded production load.
...
Produce approved per-asset partition, idempotency, validation, failure and duration plan. Bind it to the source/target artifacts, phase, Wave, segment, Authority, production window and approvals. Only the final conformance controller may declare completion, and only from C-G evidence.
