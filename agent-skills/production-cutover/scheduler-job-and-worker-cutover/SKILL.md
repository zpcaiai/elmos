---
name: scheduler-job-and-worker-cutover
description: "Use when Batch 11 must transfer scheduler, job and worker authority with checkpoint, lock and misfire controls."
---

# Scheduler Job And Worker Cutover
...
2. Transfer scheduler, job and worker authority with checkpoint, lock and misfire controls.
...
- Block on simultaneous real execution, lost checkpoint, timezone drift, missed run or duplicate restart.
...
Produce job-authority handoff and first/next-run evidence. Bind it to the source/target artifacts, phase, Wave, segment, Authority, production window and approvals. Only the final conformance controller may declare completion, and only from C-G evidence.
