---
name: read-traffic-routing-and-fallback-controller
description: "Use when Batch 11 must move stable read cohorts through shadow, canary, primary and target-only with bounded fallback."
---

# Read Traffic Routing And Fallback Controller
...
2. Move stable read cohorts through shadow, canary, primary and target-only with bounded fallback.
...
- Block on 403 fallback, unstable cohort, hidden target failure or insufficient source capacity.
...
Produce versioned read routes, fallback telemetry and automatic stop evidence. Bind it to the source/target artifacts, phase, Wave, segment, Authority, production window and approvals. Only the final conformance controller may declare completion, and only from C-G evidence.
