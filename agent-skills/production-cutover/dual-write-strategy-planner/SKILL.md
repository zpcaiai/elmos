---
name: dual-write-strategy-planner
description: "Use when Batch 11 must select a bounded CDC, outbox, primary-secondary, reverse-sync or last-resort dual-write strategy."
---

# Dual Write Strategy Planner
...
2. Select a bounded CDC, outbox, primary-secondary, reverse-sync or last-resort dual-write strategy.
...
- Block on independent writers, silent secondary failure, no termination date or non-idempotent retry.
...
Produce time-limited primary/secondary contract with idempotency, ordering and reconciliation. Bind it to the source/target artifacts, phase, Wave, segment, Authority, production window and approvals. Only the final conformance controller may declare completion, and only from C-G evidence.
