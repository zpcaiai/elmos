---
name: schema-expand-migrate-contract-planner
description: "Use when Batch 11 must plan compatible database, API, message and configuration schema evolution across Expand, Migrate and Contract."
---

# Schema Expand Migrate Contract Planner
...
2. Plan compatible database, API, message and configuration schema evolution across Expand, Migrate and Contract.
...
- Block on early destructive Contract, type narrowing, incompatible enum or unsupported rollback window.
...
Produce compatibility matrix, versioned steps and guarded Contract criteria. Bind it to the source/target artifacts, phase, Wave, segment, Authority, production window and approvals. Only the final conformance controller may declare completion, and only from C-G evidence.
