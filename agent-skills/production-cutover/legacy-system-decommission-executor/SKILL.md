---
name: legacy-system-decommission-executor
description: "Use when Batch 11 must execute an approved, pausable legacy decommission sequence through an external Authority."
---

# Legacy System Decommission Executor
...
2. Execute an approved, pausable legacy decommission sequence through an external Authority.
...
- Block on database deletion before archive, secret source write, missing dual approval or control-plane destruction.
...
Produce asset-by-asset shutdown, credential, network and destruction evidence. Bind it to the source/target artifacts, phase, Wave, segment, Authority, production window and approvals. Only the final conformance controller may declare completion, and only from C-G evidence.
