---
name: final-delta-catchup-write-freeze-and-sync-closure
description: "Use when Batch 11 must coordinate bounded write freeze, in-flight drain, final positions, delta and synchronization closure."
---

# Final Delta Catchup Write Freeze And Sync Closure
...
2. Coordinate bounded write freeze, in-flight drain, final positions, delta and synchronization closure.
...
- Block on freeze timeout, unapplied delta, deleted CDC log, unhandled queued write or position mismatch.
...
Produce final source/target position, reconciliation, freeze and remaining reverse-sync evidence. Bind it to the source/target artifacts, phase, Wave, segment, Authority, production window and approvals. Only the final conformance controller may declare completion, and only from C-G evidence.
