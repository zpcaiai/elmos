---
name: data-rollback-compensation-and-forward-fix-manager
description: "Use when Batch 11 must plan and evidence reverse sync, event replay, compensation and forward-fix for target writes."
---

# Data Rollback Compensation And Forward Fix Manager
...
2. Plan and evidence reverse sync, event replay, compensation and forward-fix for target writes.
...
- Block on deleted audit, broken financial chain, untracked manual fix or unsafe physical rollback.
...
Produce idempotent recovery ledger spanning data, messages, cache, index, file and external effects. Bind it to the source/target artifacts, phase, Wave, segment, Authority, production window and approvals. Only the final conformance controller may declare completion, and only from C-G evidence.
