---
name: session-websocket-and-long-lived-connection-migrator
description: "Use when Batch 11 must migrate session formats and drain WebSocket, SSE and other long-lived connections."
---

# Session Websocket And Long Lived Connection Migrator
...
2. Migrate session formats and drain WebSocket, SSE and other long-lived connections.
...
- Block on unplanned forced logout, duplicate reconnect command, lost event or unbounded drain.
...
Produce session compatibility, connection drain and idempotent reconnect evidence. Bind it to the source/target artifacts, phase, Wave, segment, Authority, production window and approvals. Only the final conformance controller may declare completion, and only from C-G evidence.
