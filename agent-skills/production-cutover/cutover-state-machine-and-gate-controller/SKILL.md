---
name: cutover-state-machine-and-gate-controller
description: "Use when Batch 11 must define and evaluate PCCM phases, allowed operations, entry, exit, rollback and duration controls."
---

# Cutover State Machine And Gate Controller
...
2. Define and evaluate PCCM phases, allowed operations, entry, exit, rollback and duration controls.
...
- Block on phase skipping, routing-state mismatch, missing exit evidence or unapproved irreversibility.
...
Produce machine-queryable phase and transition decisions that never execute production changes. Bind it to the source/target artifacts, phase, Wave, segment, Authority, production window and approvals. Only the final conformance controller may declare completion, and only from C-G evidence.
