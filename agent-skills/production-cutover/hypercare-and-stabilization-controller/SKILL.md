---
name: hypercare-and-stabilization-controller
description: "Use when Batch 11 must govern the post-cutover stability window, change freeze, extended on-call and exit criteria."
---

# Hypercare And Stabilization Controller
...
2. Govern the post-cutover stability window, change freeze, extended on-call and exit criteria.
...
- Block on shortened window, open critical incident, data drift, unplanned fallback or early source deletion.
...
Produce daily Hypercare evidence, incident/fallback/SLO/data status and signed exit. Bind it to the source/target artifacts, phase, Wave, segment, Authority, production window and approvals. Only the final conformance controller may declare completion, and only from C-G evidence.
