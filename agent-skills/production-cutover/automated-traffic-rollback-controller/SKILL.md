---
name: automated-traffic-rollback-controller
description: "Use when Batch 11 must evaluate SLO, data, security, CDC and divergence triggers for automatic stop or bounded rollback."
---

# Automated Traffic Rollback Controller
...
2. Evaluate SLO, data, security, CDC and divergence triggers for automatic stop or bounded rollback.
...
- Block on metric oscillation, irreversible rollback, unverified data prerequisite or automatic waiver.
...
Produce tested stop/rollback policy and audited external-Authority action request. Bind it to the source/target artifacts, phase, Wave, segment, Authority, production window and approvals. Only the final conformance controller may declare completion, and only from C-G evidence.
