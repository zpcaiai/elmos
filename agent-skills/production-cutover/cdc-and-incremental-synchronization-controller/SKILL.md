---
name: cdc-and-incremental-synchronization-controller
description: "Use when Batch 11 must control CDC or outbox positions, lag, ordering, duplicates, deletes and final delta through an external Authority."
---

# Cdc And Incremental Synchronization Controller
...
2. Control CDC or outbox positions, lag, ordering, duplicates, deletes and final delta through an external Authority.
...
- Block on lost delete, guessed position, unknown event skip, excessive lag or source transaction blockage.
...
Produce recoverable change-stream evidence from snapshot position to applied target position. Bind it to the source/target artifacts, phase, Wave, segment, Authority, production window and approvals. Only the final conformance controller may declare completion, and only from C-G evidence.
