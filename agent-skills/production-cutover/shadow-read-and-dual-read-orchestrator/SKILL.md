---
name: shadow-read-and-dual-read-orchestrator
description: "Use when Batch 11 must orchestrate isolated target Shadow Reads and bounded dual reads without user-visible side effects."
---

# Shadow Read And Dual Read Orchestrator
...
2. Orchestrate isolated target Shadow Reads and bounded dual reads without user-visible side effects.
...
- Block on target side effect, user response from Shadow, secret exposure or source SLO impact.
...
Produce sampled request/result pairs tied to identity, tenant, time and data position. Bind it to the source/target artifacts, phase, Wave, segment, Authority, production window and approvals. Only the final conformance controller may declare completion, and only from C-G evidence.
