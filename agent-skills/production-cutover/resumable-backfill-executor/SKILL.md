---
name: resumable-backfill-executor
description: "Use when Batch 11 must execute approved backfill through an external data Authority with durable checkpoints and migration ledger."
---

# Resumable Backfill Executor
...
2. Execute approved backfill through an external data Authority with durable checkpoints and migration ledger.
...
- Block on checkpoint before commit, silent record skip, duplicate side effect or unrecoverable partition.
...
Produce partition execution, checkpoint, retry and verification evidence. Bind it to the source/target artifacts, phase, Wave, segment, Authority, production window and approvals. Only the final conformance controller may declare completion, and only from C-G evidence.
