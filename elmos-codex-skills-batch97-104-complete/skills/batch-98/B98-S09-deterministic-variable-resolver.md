---
name: b98-deterministic-variable-resolver
description: "Use when ELMOS must Resolve parameters, defaults, environment references and secrets without hidden nondeterminism or plaintext exposure. Triggers: Deterministic Variable Resolver, Executable Skill Contract and Registry Pack, parameter resolution, secret references, default policy, normalization."
metadata:
  id: B98-S09
...
# B98-S09 — Deterministic Variable Resolver

## Objective

Resolve parameters, defaults, environment references and secrets without hidden nondeterminism or plaintext exposure.
...
Use this Skill when the approved task explicitly requires **Deterministic Variable Resolver** or when the capability graph selects it as part of a minimal dependency closure.
...
- same inputs produce same non-secret output.
- secret values never persisted.
- ambiguous defaults block.
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b98-deterministic-variable-resolver/SKILL.md --out /tmp/deterministic_variable_resolver.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B98-S09
```
...
- Reject or expose environment drift.
- Reject or expose secret interpolation leak.
- Reject or expose locale-dependent resolution.
