---
name: b104-representative-repository-selection
description: "Use when ELMOS must Select repositories by language, framework, size, topology, dependency, quality and failure characteristics. Triggers: Representative Repository Selection, Real World Product Certification Pack, sampling, stratification, coverage, bias."
metadata:
  id: B104-S02
...
# B104-S02 — Representative Repository Selection

## Objective

Select repositories by language, framework, size, topology, dependency, quality and failure characteristics.
...
Use this Skill when the approved task explicitly requires **Representative Repository Selection** or when the capability graph selects it as part of a minimal dependency closure.
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b104-representative-repository-selection/SKILL.md --out /tmp/representative_repository_selection.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B104-S02
```
...
- Reject or expose demo-only corpus.
- Reject or expose easy-case cherry-pick.
- Reject or expose duplicate forks.
