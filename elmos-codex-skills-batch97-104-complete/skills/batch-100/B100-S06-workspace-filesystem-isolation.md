---
name: b100-workspace-filesystem-isolation
description: "Use when ELMOS must Create per-task isolated workspaces with safe mounts, path controls, cleanup and residue detection. Triggers: Workspace and Filesystem Isolation, Hardened Runner Fabric Pack, workspace isolation, mount policy, path safety, cleanup."
metadata:
  id: B100-S06
...
# B100-S06 — Workspace and Filesystem Isolation

## Objective

Create per-task isolated workspaces with safe mounts, path controls, cleanup and residue detection.
...
Use this Skill when the approved task explicitly requires **Workspace and Filesystem Isolation** or when the capability graph selects it as part of a minimal dependency closure.
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b100-workspace-filesystem-isolation/SKILL.md --out /tmp/workspace_filesystem_isolation.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B100-S06
```
...
- Reject or expose symlink escape.
- Reject or expose shared writable cache poisoning.
- Reject or expose cleanup bypass.
