---
name: b103-tool-model-invocation-record
description: "Use when ELMOS must Record every consequential tool and model invocation with inputs, outputs, policy, cost and redaction. Triggers: Tool and Model Invocation Record, Evidence and Certification Fabric Pack, tool calls, model calls, parameters, cost."
metadata:
  id: B103-S06
...
# B103-S06 — Tool and Model Invocation Record

## Objective

Record every consequential tool and model invocation with inputs, outputs, policy, cost and redaction.
...
Use this Skill when the approved task explicitly requires **Tool and Model Invocation Record** or when the capability graph selects it as part of a minimal dependency closure.
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b103-tool-model-invocation-record/SKILL.md --out /tmp/tool_model_invocation_record.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B103-S06
```
...
- Reject or expose hidden retry.
- Reject or expose prompt missing.
- Reject or expose provider response fabricated.
