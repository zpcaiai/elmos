---
name: b103-test-result-normalizer
description: "Use when ELMOS must Normalize unit, integration, E2E, security, performance, device, infrastructure and manual results without erasing skips or flakes. Triggers: Test Result Normalizer, Evidence and Certification Fabric Pack, test normalization, case identity, skip/xfail, flaky history."
metadata:
  id: B103-S07
...
# B103-S07 — Test Result Normalizer

## Objective

Normalize unit, integration, E2E, security, performance, device, infrastructure and manual results without erasing skips or flakes.
...
Use this Skill when the approved task explicitly requires **Test Result Normalizer** or when the capability graph selects it as part of a minimal dependency closure.
...
- `normalized_test_results.json`
- `test_identity_map.json`
...
- source/discovered/executed counts reconciled.
- not-run visible.
- case IDs stable.
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b103-test-result-normalizer/SKILL.md --out /tmp/test_result_normalizer.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B103-S07
```
...
- Reject or expose skip as pass.
- Reject or expose duplicate case aggregation.
- Reject or expose failed shard missing.
