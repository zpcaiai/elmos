---
name: b103-scoped-certification-state
description: "Use when ELMOS must Represent certification by exact repository, commit, route, environment, journey, artifact and time scope. Triggers: Scoped Certification State, Evidence and Certification Fabric Pack, certification scope, state machine, limitations, revocation."
metadata:
  id: B103-S12
...
# B103-S12 — Scoped Certification State

## Objective

Represent certification by exact repository, commit, route, environment, journey, artifact and time scope.
...
Use this Skill when the approved task explicitly requires **Scoped Certification State** or when the capability graph selects it as part of a minimal dependency closure.
...
- no project-wide claim from partial scope.
- limitations machine-readable.
- revocation state terminal.
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b103-scoped-certification-state/SKILL.md --out /tmp/scoped_certification_state.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B103-S12
```
...
- Reject or expose scope inflation.
- Reject or expose commit mismatch.
- Reject or expose partial platform matrix hidden.
