---
name: b103-canonical-evidence-record-schema
description: "Use when ELMOS must Define one versioned evidence record for source, execution, tests, artifacts, approvals, limitations and state claims. Triggers: Canonical Evidence Record Schema, Evidence and Certification Fabric Pack, evidence schema, scope binding, hashes, verification state."
metadata:
  id: B103-S01
...
# B103-S01 — Canonical Evidence Record Schema

## Objective

Define one versioned evidence record for source, execution, tests, artifacts, approvals, limitations and state claims.
...
Use this Skill when the approved task explicitly requires **Canonical Evidence Record Schema** or when the capability graph selects it as part of a minimal dependency closure.
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b103-canonical-evidence-record-schema/SKILL.md --out /tmp/canonical_evidence_record_schema.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B103-S01
```
...
- Reject or expose free-text-only evidence.
- Reject or expose missing subject hash.
- Reject or expose unknown state.
