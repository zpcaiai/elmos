---
name: b99-execution-budget-quota
description: "Use when ELMOS must Enforce time, token, compute, storage, network and external-operation budgets at run and tenant scope. Triggers: Execution Budget and Quota, Durable Runtime Kernel Pack, budgets, quotas, cost accounting, hard limits."
metadata:
  id: B99-S12
...
# B99-S12 — Execution Budget and Quota

## Objective

Enforce time, token, compute, storage, network and external-operation budgets at run and tenant scope.
...
Use this Skill when the approved task explicitly requires **Execution Budget and Quota** or when the capability graph selects it as part of a minimal dependency closure.
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b99-execution-budget-quota/SKILL.md --out /tmp/execution_budget_quota.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B99-S12
```
...
- Reject or expose counter reset abuse.
- Reject or expose unmetered child task.
- Reject or expose negative usage.
