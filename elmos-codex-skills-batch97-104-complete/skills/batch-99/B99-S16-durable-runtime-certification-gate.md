---
name: b99-durable-runtime-certification-gate
description: "Use when ELMOS must Certify runtime behavior only after recovery, retry, cancellation, replay, budget, fairness and failure-injection evidence passes. Triggers: Durable Runtime Certification Gate, Durable Runtime Kernel Pack, runtime gate, chaos tests, replay proof, signed release."
metadata:
  id: B99-S16
  ...
  category: "Durable Runtime Kernel Pack"
---

# B99-S16 — Durable Runtime Certification Gate

## Objective

Certify runtime behavior only after recovery, retry, cancellation, replay, budget, fairness and failure-injection evidence passes.
...
The objective belongs to **Durable Runtime Kernel Pack**, whose system-level purpose is to execute compiled Skills through durable state machines, DAG planning, scheduling, leases, retries, cancellation, compensation, replay, budgets, observability and conservative runtime certification.

## When to Use

Use this Skill when the approved task explicitly requires **Durable Runtime Certification Gate** or when the capability graph selects it as part of a minimal dependency closure.
...
Package dependencies: `B99-S15`, `B98-S16`
...
- `runtime_certification_result.json`
- `runtime_release_manifest.json`
...
- Preferred implementation/tooling context: `Temporal or equivalent durable workflow engine`, `PostgreSQL`, `message queue`, `OpenTelemetry`. Alternatives are allowed only when their versions, behavior differences and rationale are recorded.
...
- P0 failure injection passes.
- independent replay.
- no fabricated terminal state.
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b99-durable-runtime-certification-gate/SKILL.md --out /tmp/durable_runtime_certification_gate.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B99-S16
```
...
- Reject or expose synthetic-only recovery claim.
- Reject or expose lost effect journal.
- Reject or expose waived cancellation safety.
