---
name: b99-saga-compensation-engine
description: "Use when ELMOS must Coordinate compensations for partial cross-system failure while preserving audit and residual-risk truth. Triggers: Saga Compensation Engine, Durable Runtime Kernel Pack, saga, compensation order, partial recovery, manual repair."
metadata:
  id: B99-S09
...
# B99-S09 — Saga Compensation Engine

## Objective

Coordinate compensations for partial cross-system failure while preserving audit and residual-risk truth.
...
Use this Skill when the approved task explicitly requires **Saga Compensation Engine** or when the capability graph selects it as part of a minimal dependency closure.
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b99-saga-compensation-engine/SKILL.md --out /tmp/saga_compensation_engine.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B99-S09
```
...
- Reject or expose compensation loop.
- Reject or expose destructive overcompensation.
- Reject or expose hidden residual state.
