---
name: b99-durable-run-state-model
description: "Use when ELMOS must Define the authoritative run, task, step and effect state machines with legal transitions and terminal semantics. Triggers: Durable Run State Model, Durable Runtime Kernel Pack, state machine, terminal states, transition reasons, scope identity."
metadata:
  id: B99-S01
...
# B99-S01 — Durable Run State Model

## Objective

Define the authoritative run, task, step and effect state machines with legal transitions and terminal semantics.
...
Use this Skill when the approved task explicitly requires **Durable Run State Model** or when the capability graph selects it as part of a minimal dependency closure.
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b99-durable-run-state-model/SKILL.md --out /tmp/durable_run_state_model.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B99-S01
```
...
- Reject or expose state skipping.
- Reject or expose terminal resurrection.
- Reject or expose scope confusion.
