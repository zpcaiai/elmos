---
name: b99-runtime-observability-tracing
description: "Use when ELMOS must Instrument runs, DAG nodes, runner tasks, tool calls, effects and evidence with correlated telemetry. Triggers: Runtime Observability and Tracing, Durable Runtime Kernel Pack, traces, metrics, logs, correlation IDs."
metadata:
  id: B99-S15
...
# B99-S15 — Runtime Observability and Tracing

## Objective

Instrument runs, DAG nodes, runner tasks, tool calls, effects and evidence with correlated telemetry.
...
Use this Skill when the approved task explicitly requires **Runtime Observability and Tracing** or when the capability graph selects it as part of a minimal dependency closure.
...
- `observability_verification.json`
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b99-runtime-observability-tracing/SKILL.md --out /tmp/runtime_observability_tracing.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B99-S15
```
...
- Reject or expose trace ID spoofing.
- Reject or expose secret in span.
- Reject or expose unbounded labels.
