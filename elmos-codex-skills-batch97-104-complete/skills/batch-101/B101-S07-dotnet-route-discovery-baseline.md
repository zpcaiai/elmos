---
name: b101-dotnet-route-discovery-baseline
description: "Use when ELMOS must Discover solutions, projects, packages, frameworks, workloads and reproduce the source baseline. Triggers: .NET Route Discovery and Baseline, Three Golden Route Packs, .NET discovery, MSBuild, NuGet, baseline."
metadata:
  id: B101-S07
...
# B101-S07 — .NET Route Discovery and Baseline

## Objective

Discover solutions, projects, packages, frameworks, workloads and reproduce the source baseline.
...
Use this Skill when the approved task explicitly requires **.NET Route Discovery and Baseline** or when the capability graph selects it as part of a minimal dependency closure.
...
- `dotnet_route_profile.json`
- `dotnet_baseline_result.json`
...
- SDK pinning exact.
- solution graph complete.
- baseline diagnostics retained.
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b101-dotnet-route-discovery-baseline/SKILL.md --out /tmp/dotnet_route_discovery_baseline.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B101-S07
```
...
- Reject or expose global.json ignored.
- Reject or expose package source credential leak.
- Reject or expose unsupported workload guessed.
