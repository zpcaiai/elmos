---
name: b102-equivalence-evidence-freshness
description: "Use when ELMOS must Invalidate equivalence results when code, tests, data, environment, dependency, tool or policy changes. Triggers: Equivalence Evidence Freshness, Semantic Equivalence Lab Pack, evidence freshness, invalidation, dependency hashes, revalidation."
metadata:
  id: B102-S15
...
# B102-S15 — Equivalence Evidence Freshness

## Objective

Invalidate equivalence results when code, tests, data, environment, dependency, tool or policy changes.
...
Use this Skill when the approved task explicitly requires **Equivalence Evidence Freshness** or when the capability graph selects it as part of a minimal dependency closure.
...
- all causal inputs hashed.
- invalidations transitive.
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b102-equivalence-evidence-freshness/SKILL.md --out /tmp/equivalence_evidence_freshness.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B102-S15
```
...
- Reject or expose commit-only freshness.
- Reject or expose tool update ignored.
- Reject or expose partial retest presented as full.
