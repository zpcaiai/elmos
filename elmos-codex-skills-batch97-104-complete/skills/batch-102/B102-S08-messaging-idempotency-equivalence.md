---
name: b102-messaging-idempotency-equivalence
description: "Use when ELMOS must Compare delivery, acknowledgment, retry, ordering, deduplication and side-effect idempotency. Triggers: Messaging and Idempotency Equivalence, Semantic Equivalence Lab Pack, messaging, ack semantics, retry, idempotency."
metadata:
  id: B102-S08
...
# B102-S08 — Messaging and Idempotency Equivalence

## Objective

Compare delivery, acknowledgment, retry, ordering, deduplication and side-effect idempotency.
...
Use this Skill when the approved task explicitly requires **Messaging and Idempotency Equivalence** or when the capability graph selects it as part of a minimal dependency closure.
...
- `messaging_equivalence.json`
- `duplicate_delivery_results.json`
...
- duplicate inputs safe.
- retry timing bounded.
- dead-letter behavior compared.
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b102-messaging-idempotency-equivalence/SKILL.md --out /tmp/messaging_idempotency_equivalence.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B102-S08
```
...
- Reject or expose at-most-once substituted.
- Reject or expose dedupe key changed.
- Reject or expose poison message ignored.
