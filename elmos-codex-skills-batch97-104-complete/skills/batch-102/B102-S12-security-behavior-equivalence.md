---
name: b102-security-behavior-equivalence
description: "Use when ELMOS must Compare identity, authentication, authorization, tenant isolation, validation, secrets and audit behavior. Triggers: Security Behavior Equivalence, Semantic Equivalence Lab Pack, security behavior, tenant isolation, input validation, audit."
metadata:
  id: B102-S12
...
# B102-S12 — Security Behavior Equivalence

## Objective

Compare identity, authentication, authorization, tenant isolation, validation, secrets and audit behavior.
...
Use this Skill when the approved task explicitly requires **Security Behavior Equivalence** or when the capability graph selects it as part of a minimal dependency closure.
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b102-security-behavior-equivalence/SKILL.md --out /tmp/security_behavior_equivalence.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B102-S12
```
...
- Reject or expose security test mocked.
- Reject or expose admin-only happy path.
- Reject or expose audit omission.
