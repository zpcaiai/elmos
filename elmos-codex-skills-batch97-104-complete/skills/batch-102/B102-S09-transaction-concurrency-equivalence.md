---
name: b102-transaction-concurrency-equivalence
description: "Use when ELMOS must Compare transaction boundaries, isolation, locking, deadlocks, retries and race-sensitive outcomes. Triggers: Transaction and Concurrency Equivalence, Semantic Equivalence Lab Pack, transactions, isolation, locking, concurrency."
metadata:
  id: B102-S09
...
# B102-S09 — Transaction and Concurrency Equivalence

## Objective

Compare transaction boundaries, isolation, locking, deadlocks, retries and race-sensitive outcomes.
...
Use this Skill when the approved task explicitly requires **Transaction and Concurrency Equivalence** or when the capability graph selects it as part of a minimal dependency closure.
...
- boundary and propagation preserved.
- race tests deterministic enough.
- deadlocks classified.
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b102-transaction-concurrency-equivalence/SKILL.md --out /tmp/transaction_concurrency_equivalence.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B102-S09
```
...
- Reject or expose single-thread test only.
- Reject or expose isolation downgrade.
- Reject or expose retry duplicates effect.
