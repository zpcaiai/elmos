---
name: b102-oracle-registry-governance
description: "Use when ELMOS must Register authoritative, supporting and prohibited oracle types with ownership and confidence. Triggers: Oracle Registry and Governance, Semantic Equivalence Lab Pack, oracle registry, authority levels, ownership, confidence."
metadata:
  id: B102-S01
...
# B102-S01 — Oracle Registry and Governance

## Objective

Register authoritative, supporting and prohibited oracle types with ownership and confidence.
...
Use this Skill when the approved task explicitly requires **Oracle Registry and Governance** or when the capability graph selects it as part of a minimal dependency closure.
...
- LLM-only never authoritative.
- oracle scope explicit.
- owner approval.
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b102-oracle-registry-governance/SKILL.md --out /tmp/oracle_registry_governance.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B102-S01
```
...
- Reject or expose circular oracle.
- Reject or expose stale oracle.
- Reject or expose self-generated expected output.
