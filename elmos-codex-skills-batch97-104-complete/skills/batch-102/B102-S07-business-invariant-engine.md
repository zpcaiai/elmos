---
name: b102-business-invariant-engine
description: "Use when ELMOS must Define and evaluate business invariants across state, balance, inventory, permissions, audit and reporting. Triggers: Business Invariant Engine, Semantic Equivalence Lab Pack, business invariants, state transitions, cross-system assertions, criticality."
metadata:
  id: B102-S07
...
# B102-S07 — Business Invariant Engine

## Objective

Define and evaluate business invariants across state, balance, inventory, permissions, audit and reporting.
...
Use this Skill when the approved task explicitly requires **Business Invariant Engine** or when the capability graph selects it as part of a minimal dependency closure.
...
- invariants independently reviewed.
- violations block.
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b102-business-invariant-engine/SKILL.md --out /tmp/business_invariant_engine.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B102-S07
```
...
- Reject or expose invented invariant.
- Reject or expose aggregate hides account error.
- Reject or expose authorization absent.
