---
name: b100-network-egress-control
description: "Use when ELMOS must Apply default-deny DNS, IP, port, protocol and identity-aware egress controls per task. Triggers: Network Egress Control, Hardened Runner Fabric Pack, egress allowlist, DNS policy, proxy enforcement, network evidence."
metadata:
  id: B100-S08
...
# B100-S08 — Network Egress Control

## Objective

Apply default-deny DNS, IP, port, protocol and identity-aware egress controls per task.
...
Use this Skill when the approved task explicitly requires **Network Egress Control** or when the capability graph selects it as part of a minimal dependency closure.
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b100-network-egress-control/SKILL.md --out /tmp/network_egress_control.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B100-S08
```
...
- Reject or expose wildcard Internet.
- Reject or expose IPv6 bypass.
- Reject or expose proxy credential leak.
