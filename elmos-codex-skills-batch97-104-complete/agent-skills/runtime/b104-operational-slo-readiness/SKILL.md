---
name: b104-operational-slo-readiness
description: "Use when ELMOS must Define and validate availability, latency, durability, recovery, support and evidence freshness SLOs. Triggers: Operational SLO and Readiness, Real World Product Certification Pack, SLOs, alerting, error budgets, runbooks."
metadata:
  id: B104-S12
...
# B104-S12 — Operational SLO and Readiness

## Objective

Define and validate availability, latency, durability, recovery, support and evidence freshness SLOs.
...
Use this Skill when the approved task explicitly requires **Operational SLO and Readiness** or when the capability graph selects it as part of a minimal dependency closure.
...
- SLIs measured in representative load.
- alerts actionable.
- error budget policy.
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b104-operational-slo-readiness/SKILL.md --out /tmp/operational_slo_readiness.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B104-S12
```
...
- Reject or expose uptime from health endpoint only.
- Reject or expose no paging drill.
- Reject or expose SLO without owner.
