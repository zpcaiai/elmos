---
name: b101-golden-route-common-contract
description: "Use when ELMOS must Define the common repository-to-PR lifecycle, evidence, state and rollback contract shared by Java, .NET and Python routes. Triggers: Golden Route Common Contract, Three Golden Route Packs, route lifecycle, common states, evidence, rollback."
metadata:
  id: B101-S01
...
# B101-S01 — Golden Route Common Contract

## Objective

Define the common repository-to-PR lifecycle, evidence, state and rollback contract shared by Java, .NET and Python routes.
...
Use this Skill when the approved task explicitly requires **Golden Route Common Contract** or when the capability graph selects it as part of a minimal dependency closure.
...
Package dependencies: `B100-S16`, `B97-S12`, `B98-S16`, `B99-S16`
...
- `golden_route_contract.json`
- `route_state_model.json`
- `common_validation.json`
...
- same lifecycle across routes.
- route-specific extensions bounded.
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b101-golden-route-common-contract/SKILL.md --out /tmp/golden_route_common_contract.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B101-S01
```
...
- Reject or expose route bypass of common gate.
- Reject or expose state name drift.
- Reject or expose missing rollback.
