---
name: b103-assurance-cockpit-read-model
description: "Use when ELMOS must Build evidence-backed read models for progress, coverage, risk, exceptions, freshness and certification state. Triggers: Assurance Cockpit Read Model, Evidence and Certification Fabric Pack, read models, risk views, freshness, drill-down."
metadata:
  id: B103-S15
...
# B103-S15 — Assurance Cockpit Read Model

## Objective

Build evidence-backed read models for progress, coverage, risk, exceptions, freshness and certification state.
...
Use this Skill when the approved task explicitly requires **Assurance Cockpit Read Model** or when the capability graph selects it as part of a minimal dependency closure.
...
- `assurance_read_model.json`
- `dashboard_contract.json`
- `read_model_tests.json`
...
- every metric drillable to evidence.
- stale data flagged.
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b103-assurance-cockpit-read-model/SKILL.md --out /tmp/assurance_cockpit_read_model.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B103-S15
```
...
- Reject or expose dashboard-only truth.
- Reject or expose aggregate hides P0.
- Reject or expose cross-tenant count leak.
