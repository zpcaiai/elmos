---
name: b103-run-environment-fingerprint
description: "Use when ELMOS must Fingerprint source state, runner, OS, toolchains, dependencies, services, test data and policies for reproducibility. Triggers: Run and Environment Fingerprint, Evidence and Certification Fabric Pack, environment fingerprint, tool versions, dependency lock, data version."
metadata:
  id: B103-S04
...
# B103-S04 — Run and Environment Fingerprint

## Objective

Fingerprint source state, runner, OS, toolchains, dependencies, services, test data and policies for reproducibility.
...
Use this Skill when the approved task explicitly requires **Run and Environment Fingerprint** or when the capability graph selects it as part of a minimal dependency closure.
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b103-run-environment-fingerprint/SKILL.md --out /tmp/run_environment_fingerprint.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B103-S04
```
...
- Reject or expose ambient dependency omitted.
- Reject or expose mutable latest tag.
- Reject or expose timezone missing.
