---
name: b101-golden-route-certification-gate
description: "Use when ELMOS must Certify Java, .NET and Python route packs against representative holdout repositories and exact evidence. Triggers: Golden Route Certification Gate, Three Golden Route Packs, route gate, holdout corpus, acceptance rates, independent verification."
metadata:
  id: B101-S16
...
# B101-S16 — Golden Route Certification Gate

## Objective

Certify Java, .NET and Python route packs against representative holdout repositories and exact evidence.
...
Use this Skill when the approved task explicitly requires **Golden Route Certification Gate** or when the capability graph selects it as part of a minimal dependency closure.
...
- `route_certification_result.json`
- `route_support_matrix.json`
...
- no route-certified from demo only.
- scope and limitations signed.
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b101-golden-route-certification-gate/SKILL.md --out /tmp/golden_route_certification_gate.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B101-S16
```
...
- Reject or expose training corpus leakage.
- Reject or expose unsupported profile averaged away.
- Reject or expose manual edits hidden.
