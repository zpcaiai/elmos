---
name: b101-java-route-discovery-baseline
description: "Use when ELMOS must Discover Maven and Gradle Java estates and reproduce a clean immutable baseline build and test profile. Triggers: Java Route Discovery and Baseline, Three Golden Route Packs, Java discovery, build reproduction, JDK matrix, baseline."
metadata:
  id: B101-S02
...
# B101-S02 — Java Route Discovery and Baseline

## Objective

Discover Maven and Gradle Java estates and reproduce a clean immutable baseline build and test profile.
...
Use this Skill when the approved task explicitly requires **Java Route Discovery and Baseline** or when the capability graph selects it as part of a minimal dependency closure.
...
- `java_route_profile.json`
- `java_baseline_result.json`
...
- source and generated code separated.
- exact JDK and build tool.
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b101-java-route-discovery-baseline/SKILL.md --out /tmp/java_route_discovery_baseline.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B101-S02
```
...
- Reject or expose wrapper tampering.
- Reject or expose private dependency leakage.
- Reject or expose generated code treated as source.
