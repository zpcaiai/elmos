---
name: b99-failure-classification-diagnostics
description: "Use when ELMOS must Classify model, contract, dependency, build, test, runtime, policy, infrastructure and user failures before repair. Triggers: Failure Classification and Diagnostics, Durable Runtime Kernel Pack, failure taxonomy, fingerprints, root layer, repair eligibility."
metadata:
  id: B99-S14
...
# B99-S14 — Failure Classification and Diagnostics

## Objective

Classify model, contract, dependency, build, test, runtime, policy, infrastructure and user failures before repair.
...
Use this Skill when the approved task explicitly requires **Failure Classification and Diagnostics** or when the capability graph selects it as part of a minimal dependency closure.
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b99-failure-classification-diagnostics/SKILL.md --out /tmp/failure_classification_diagnostics.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B99-S14
```
...
- Reject or expose blame shifting to model.
- Reject or expose same failure fragmented.
- Reject or expose security incident auto-repaired.
