---
name: b102-performance-resource-equivalence
description: "Use when ELMOS must Compare latency, throughput, memory, CPU, allocation, artifact size and capacity under representative workloads. Triggers: Performance and Resource Equivalence, Semantic Equivalence Lab Pack, performance, resource budgets, capacity, regression."
metadata:
  id: B102-S11
...
# B102-S11 — Performance and Resource Equivalence

## Objective

Compare latency, throughput, memory, CPU, allocation, artifact size and capacity under representative workloads.
...
Use this Skill when the approved task explicitly requires **Performance and Resource Equivalence** or when the capability graph selects it as part of a minimal dependency closure.
...
- `performance_equivalence.json`
- `benchmark_results.json`
- `budget_findings.json`
...
- baseline statistically valid.
- warm/cold separated.
- budgets owner-approved.
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b102-performance-resource-equivalence/SKILL.md --out /tmp/performance_resource_equivalence.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B102-S11
```
...
- Reject or expose microbenchmark only.
- Reject or expose hardware mismatch hidden.
- Reject or expose average masks P99.
