---
name: b104-portfolio-scale-benchmark
description: "Use when ELMOS must Measure thousand-repository and million-line scheduling, storage, runner, graph, evidence and control-plane behavior. Triggers: Portfolio Scale Benchmark, Real World Product Certification Pack, scale, throughput, backpressure, capacity."
metadata:
  id: B104-S05
...
# B104-S05 — Portfolio Scale Benchmark

## Objective

Measure thousand-repository and million-line scheduling, storage, runner, graph, evidence and control-plane behavior.
...
Use this Skill when the approved task explicitly requires **Portfolio Scale Benchmark** or when the capability graph selects it as part of a minimal dependency closure.
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b104-portfolio-scale-benchmark/SKILL.md --out /tmp/portfolio_scale_benchmark.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B104-S05
```
...
- Reject or expose toy files counted as LOC.
- Reject or expose queue drops hidden.
- Reject or expose single-tenant only.
