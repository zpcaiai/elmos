---
name: b100-secret-broker-redaction
description: "Use when ELMOS must Inject task-scoped secrets by reference, redact all surfaces and revoke immediately after use. Triggers: Secret Broker and Redaction, Hardened Runner Fabric Pack, secret broker, ephemeral injection, redaction, revocation."
metadata:
  id: B100-S09
...
# B100-S09 — Secret Broker and Redaction

## Objective

Inject task-scoped secrets by reference, redact all surfaces and revoke immediately after use.
...
Use this Skill when the approved task explicitly requires **Secret Broker and Redaction** or when the capability graph selects it as part of a minimal dependency closure.
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b100-secret-broker-redaction/SKILL.md --out /tmp/secret_broker_redaction.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B100-S09
```
...
- Reject or expose secret in environment dump.
- Reject or expose prompt exposure.
- Reject or expose post-task token reuse.
