---
name: b101-python-route-discovery-environment
description: "Use when ELMOS must Discover Python versions, dependency managers, native extensions, notebooks, web apps, data and ML workloads and reproduce environments. Triggers: Python Route Discovery and Environment, Three Golden Route Packs, Python discovery, environment lock, native extensions, notebooks."
metadata:
  id: B101-S11
  ...
  category: "Three Golden Route Packs"
---

# B101-S11 — Python Route Discovery and Environment

## Objective

Discover Python versions, dependency managers, native extensions, notebooks, web apps, data and ML workloads and reproduce environments.
...
Implements three representative repository-to-PR routes that prove the platform on real technology estates.

The objective belongs to **Three Golden Route Packs**, whose system-level purpose is to deliver real Java, .NET and Python modernization routes from repository connection and baseline through deterministic transformation, constrained repair, verification, PR, rollback and route certification.

## When to Use

Use this Skill when the approved task explicitly requires **Python Route Discovery and Environment** or when the capability graph selects it as part of a minimal dependency closure.
...
- `python_route_profile.json`
- `python_environment_result.json`
...
- Preferred implementation/tooling context: `JDK/Maven/Gradle`, `.NET SDK/MSBuild`, `Python/uv/pip`, `GitHub/GitLab connectors`. Alternatives are allowed only when their versions, behavior differences and rationale are recorded.
...
- interpreter exact.
- dependency sources recorded.
- clean-kernel notebook test.
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b101-python-route-discovery-environment/SKILL.md --out /tmp/python_route_discovery_environment.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B101-S11
```
...
- Reject or expose ambient site-packages.
- Reject or expose untrusted pickle load.
- Reject or expose GPU assumed.
