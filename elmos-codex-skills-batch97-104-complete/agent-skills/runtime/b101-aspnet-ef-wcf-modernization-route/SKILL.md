---
name: b101-aspnet-ef-wcf-modernization-route
description: "Use when ELMOS must Modernize ASP.NET, authentication, middleware, Entity Framework and WCF boundaries with contract preservation. Triggers: ASP.NET EF and WCF Modernization Route, Three Golden Route Packs, ASP.NET Core, EF Core, WCF alternatives, auth."
metadata:
  id: B101-S09
...
# B101-S09 — ASP.NET EF and WCF Modernization Route

## Objective

Modernize ASP.NET, authentication, middleware, Entity Framework and WCF boundaries with contract preservation.
...
Use this Skill when the approved task explicitly requires **ASP.NET EF and WCF Modernization Route** or when the capability graph selects it as part of a minimal dependency closure.
...
- `dotnet_framework_mapping.json`
- `dotnet_service_patch_set/`
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b101-aspnet-ef-wcf-modernization-route/SKILL.md --out /tmp/aspnet_ef_wcf_modernization_route.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B101-S09
```
...
- Reject or expose authorization attr loss.
- Reject or expose lazy-loading behavior drift.
- Reject or expose WCF callback ignored.
