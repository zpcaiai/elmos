---
name: b99-idempotency-effect-journal
description: "Use when ELMOS must Record external effects and enforce idempotency keys, deduplication and reconciliation. Triggers: Idempotency and Effect Journal, Durable Runtime Kernel Pack, effect journal, idempotency, deduplication, reconciliation."
metadata:
  id: B99-S06
...
# B99-S06 — Idempotency and Effect Journal

## Objective

Record external effects and enforce idempotency keys, deduplication and reconciliation.
...
Use this Skill when the approved task explicitly requires **Idempotency and Effect Journal** or when the capability graph selects it as part of a minimal dependency closure.
...
Package dependencies: `B99-S05`, `B98-S07`
...
- effect result content-addressed.
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b99-idempotency-effect-journal/SKILL.md --out /tmp/idempotency_effect_journal.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B99-S06
```
...
- Reject or expose key collision.
- Reject or expose effect without journal.
- Reject or expose duplicate completion.
