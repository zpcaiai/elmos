---
name: b99-durable-scheduler-queue
description: "Use when ELMOS must Schedule runnable DAG nodes with persistence, priorities, fairness, backpressure and tenant isolation. Triggers: Durable Scheduler and Queue, Durable Runtime Kernel Pack, scheduler, priority, fairness, backpressure."
metadata:
  id: B99-S03
...
# B99-S03 — Durable Scheduler and Queue

## Objective

Schedule runnable DAG nodes with persistence, priorities, fairness, backpressure and tenant isolation.
...
Use this Skill when the approved task explicitly requires **Durable Scheduler and Queue** or when the capability graph selects it as part of a minimal dependency closure.
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b99-durable-scheduler-queue/SKILL.md --out /tmp/durable_scheduler_queue.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B99-S03
```
...
- Reject or expose priority inversion.
- Reject or expose cross-tenant dequeue.
- Reject or expose unbounded queue growth.
