---
name: b97-skill-metadata-normalizer
description: "Use when ELMOS must Normalize heterogeneous Skill metadata into one versioned machine-readable representation while preserving source identity and uncertainty. Triggers: Skill Metadata Normalizer, Canonical Capability Graph Consolidation Pack, frontmatter normalization, stable identity, version metadata, ownership."
metadata:
  id: B97-S03
...
# B97-S03 — Skill Metadata Normalizer

## Objective

Normalize heterogeneous Skill metadata into one versioned machine-readable representation while preserving source identity and uncertainty.
...
Use this Skill when the approved task explicitly requires **Skill Metadata Normalizer** or when the capability graph selects it as part of a minimal dependency closure.
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b97-skill-metadata-normalizer/SKILL.md --out /tmp/skill_metadata_normalizer.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B97-S03
```
...
- Reject or expose silent data loss.
- Reject or expose identifier collision.
- Reject or expose fabricated defaults.
