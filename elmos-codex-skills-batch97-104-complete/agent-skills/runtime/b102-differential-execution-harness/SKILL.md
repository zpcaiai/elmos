---
name: b102-differential-execution-harness
description: "Use when ELMOS must Run source and target against identical inputs, environments and controlled dependencies and compare observations. Triggers: Differential Execution Harness, Semantic Equivalence Lab Pack, differential execution, input replay, observation model, comparison."
metadata:
  id: B102-S04
...
# B102-S04 — Differential Execution Harness

## Objective

Run source and target against identical inputs, environments and controlled dependencies and compare observations.
...
Use this Skill when the approved task explicitly requires **Differential Execution Harness** or when the capability graph selects it as part of a minimal dependency closure.
...
Package dependencies: `B102-S03`, `B99-S08`, `B100-S12`
...
- same input and dependency behavior.
- differences classified.
- source/target versions bound.
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b102-differential-execution-harness/SKILL.md --out /tmp/differential_execution_harness.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B102-S04
```
...
- Reject or expose different test data.
- Reject or expose clock drift.
- Reject or expose one side mocked.
