---
name: b97-route-domain-pack-composer
description: "Use when ELMOS must Compose reusable core primitives, route packs and domain policies without duplicating implementation logic. Triggers: Route and Domain Pack Composer, Canonical Capability Graph Consolidation Pack, pack composition, inheritance boundaries, policy overlays, minimal closure."
metadata:
  id: B97-S12
...
# B97-S12 — Route and Domain Pack Composer

## Objective

Compose reusable core primitives, route packs and domain policies without duplicating implementation logic.
...
Use this Skill when the approved task explicitly requires **Route and Domain Pack Composer** or when the capability graph selects it as part of a minimal dependency closure.
...
- `composed_pack_manifest.json`
- `composition_findings.json`
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b97-route-domain-pack-composer/SKILL.md --out /tmp/route_domain_pack_composer.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B97-S12
```
...
- Reject or expose diamond conflict.
- Reject or expose policy shadowing.
- Reject or expose unbounded Skill loading.
