---
name: b102-api-contract-equivalence
description: "Use when ELMOS must Compare routes, methods, auth, validation, error models, schemas, ordering, pagination and compatibility. Triggers: API Contract Equivalence, Semantic Equivalence Lab Pack, API contracts, auth, validation, error semantics."
metadata:
  id: B102-S05
...
# B102-S05 — API Contract Equivalence

## Objective

Compare routes, methods, auth, validation, error models, schemas, ordering, pagination and compatibility.
...
Use this Skill when the approved task explicitly requires **API Contract Equivalence** or when the capability graph selects it as part of a minimal dependency closure.
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b102-api-contract-equivalence/SKILL.md --out /tmp/api_contract_equivalence.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B102-S05
```
...
- Reject or expose HTTP 200 only.
- Reject or expose error body drift.
- Reject or expose authorization weakened.
