---
name: b100-runner-upgrade-drain
description: "Use when ELMOS must Patch runners safely through staged rollout, drain, rollback and version compatibility checks. Triggers: Runner Upgrade and Drain, Hardened Runner Fabric Pack, upgrade, drain, canary, rollback."
metadata:
  id: B100-S14
...
# B100-S14 — Runner Upgrade and Drain

## Objective

Patch runners safely through staged rollout, drain, rollback and version compatibility checks.
...
Use this Skill when the approved task explicitly requires **Runner Upgrade and Drain** or when the capability graph selects it as part of a minimal dependency closure.
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b100-runner-upgrade-drain/SKILL.md --out /tmp/runner_upgrade_drain.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B100-S14
```
...
- Reject or expose forced upgrade during task.
- Reject or expose mixed protocol corruption.
- Reject or expose rollback image missing.
