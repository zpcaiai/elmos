---
name: b98-capability-search-ranking
description: "Use when ELMOS must Rank candidate Skills by capability fit, evidence, compatibility, cost, risk and historical outcomes without loading the full estate. Triggers: Capability Search and Ranking, Executable Skill Contract and Registry Pack, candidate retrieval, ranking, fit explanation, budget."
metadata:
  id: B98-S12
...
# B98-S12 — Capability Search and Ranking

## Objective

Rank candidate Skills by capability fit, evidence, compatibility, cost, risk and historical outcomes without loading the full estate.
...
Use this Skill when the approved task explicitly requires **Capability Search and Ranking** or when the capability graph selects it as part of a minimal dependency closure.
...
Package dependencies: `B98-S11`, `B97-S12`
...
- relevant recall measured.
- ranking features versioned.
- security incompatibility hard-blocks.
...
```bash
python3 scripts/compile_skill_contract.py agent-skills/runtime/b98-capability-search-ranking/SKILL.md --out /tmp/capability_search_ranking.contract.json
```

```bash
python3 scripts/run_certification_gate.py --package . --scope B98-S12
```
...
- Reject or expose popularity overrides safety.
- Reject or expose prompt keyword gaming.
- Reject or expose tenant data leakage.
