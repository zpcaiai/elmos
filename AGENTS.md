...
- Every atomic binding is exact and allowlisted. Exactly 45 provider-free Skills have repository-owned semantic handlers and may report bounded `LOCAL_EXECUTED_SELF_ATTESTED` evidence after their exact handler runs; the other 1,265 remain `PREPARE_ONLY`. External effects require a host-owned Broker route whose operation, effect class, request, permit, receipt and declared outputs all match exactly. External and independent evidence remains `NOT_RUN`, and certification remains `NOT_CERTIFIED`.
