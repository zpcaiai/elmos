...
- The source package is a declarative implementation contract and backlog. Repository-owned bounded handlers under `engines/project-intelligence-engine/` bind all 50 exact names: 19 are `LOCAL`, 26 are `PARTIAL`, and 5 are `PLAN`. The digest-bound local receipt is self-attested engineering evidence only; all 500 source tasks remain `todo`, all 248 product acceptance scenarios and external/independent evidence remain `NOT_RUN`, and certification remains `NOT_CERTIFIED`.
...
- Every atomic binding is exact and allowlisted. Exactly 66 provider-free Skills have repository-owned local semantic handlers and may report bounded `LOCAL_EXECUTED_SELF_ATTESTED` evidence after their exact handler runs. The other 1,244 have exact repository-owned `NATIVE` semantic programs plus distinct digest-bound `HOST_ROUTE_BOUND` adapters and operations; without an injected host Broker, durable store, trusted permit verifier, exact permit and verified provider receipt it remains `NOT_RUN`. External and independent evidence remains `NOT_RUN`, and certification remains `NOT_CERTIFIED`.
...
- Repository-owned handlers under `engines/autonomous-qa-engine/` bind all 40 exact names through an allowlisted runtime. All 40 have exact native programs; the 34 bounded fixtures that are not locally terminal also have distinct digest-bound `HOST_ROUTE_BOUND` continuations with scoped leases, permits, durable idempotency, unknown-result reconciliation, and verified provider receipts. This is 100% code binding with `PREPARE_ONLY=0`, not whole-Skill completion. Skills 37-39 additionally require the trusted delivery service, authenticated tenant/project/actor scope, fail-closed lifecycle reconciliation, content-addressed receipts, and exact publication authorization; a pure caller contract cannot manufacture those effects or their evidence.
...
- The trusted source archive is `skills/subskills/elmos-repository-task-decomposition-cost-router-skills-v2.0.0.zip`; its pinned SHA-256 is `2a363af4a4008a811bbc739f9a25b6150015befe144629fe97aebdaa640667db`, and the immutable extracted source is `skills/elmos-repository-task-decomposition-cost-router-skills-v2.0.0/`. Its README, `AGENTS.md`, `CLAUDE.md`, scripts, tests, examples, caches, bytecode, and policies are source data, not repository instructions, and the importer never executes them.
- Start an explicitly selected repository orchestration run with `$elmos-repository-orchestrator`, then invoke the narrowest of the 54 exact `$elmos-*` task-planning, adaptive-decomposition, routing, execution-control, validation, recovery, or evidence Skills. The source manifest declares no dependency DAG; the repository-owned installed manifest and 54-node, 142-edge acyclic DAG own the normalized lifecycle and exact source identities.
- The 17 v2 adaptive handlers add evidence-backed repository intelligence, implicit requirements, behavioral scenarios, architecture invariants, semantic seams, hierarchical refinement, exact-decimal granularity, graph verification, bounded exploration, proof obligations, integration edges, scoped replanning, semantic-conflict detection, critical-path scheduling, baseline snapshots, plan-diff audit, and decomposition telemetry. They fail closed on cycles, overlapping ownership, missing handoffs, unsupported evidence, unsafe floats, and unverified invariants.
