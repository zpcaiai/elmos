...
class IdentityTests(unittest.TestCase):
    def test_oidc_jwks_tls_configuration_fails_closed(self) -> None:
        issuer = "https://id.example.test"
        with tempfile.TemporaryDirectory(prefix="pi-oidc-ca-") as raw_root:
            ...
            configured = OIDCConfig(
                ...
                "pi-api",
                issuer + "/jwks",
                jwks_ca_file=str(ca_file),
                jwks_timeout_seconds=5,
            )
            self.assertEqual(configured.jwks_ca_file, str(ca_file))
            ...
            with self.assertRaises(ValueError):
                OIDCConfig(
                    ...
                    "pi-api",
                    issuer + "/jwks",
                    jwks_ca_file=str(root / "missing.pem"),
                )
            ...
            with self.assertRaises(ValueError):
                OIDCConfig(
                    ...
                    "pi-api",
                    issuer + "/jwks",
                    jwks_ca_file=str(symlink),
                )

        with self.assertRaises(ValueError):
            OIDCConfig("http://id.example.test", "pi-api", issuer + "/jwks")
        with self.assertRaises(ValueError):
            OIDCConfig(
                issuer,
                "pi-api",
