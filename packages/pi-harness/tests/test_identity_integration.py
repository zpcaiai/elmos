...
class IdentityIntegrationTests(unittest.TestCase):
    ...
    def test_real_jwks_tls_mtls_spiffe_and_crl_revocation(self) -> None:
        ...
        # Loopback TLS qualification must be isolated from workstation proxy
        # configuration; otherwise a local handshake can be routed externally.
        with patch.dict(os.environ, proxy_environment):
            ...
            with opener.open(
                f"https://localhost:{mtls_server.server_port}/probe", timeout=5
            ) as response:
