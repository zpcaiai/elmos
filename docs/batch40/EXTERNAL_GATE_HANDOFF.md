...
- Direct dependency inventory: 427 external components have versions, but none
  ...
  license blocks rather than being silently treated as acceptable.
- Gap inventory: 14 blocking / 13 open, split into 1 repository blocking, 12
...
1. Resolve license identity and policy decisions for all 427 external direct
   components. Unknown, conflicting, prohibited, or context-dependent licenses
   remain blocking.
2. Review the 18 recorded patch-SLA breaches and all 105 dismissed Dependabot
   ...
   risk-decision record; do not rewrite the observed historical result.
3. Implement and execute the 11 still-experimental capability rows recorded in
   `support-matrix.json`. A local contract, template, or scanner declaration is
   insufficient to promote a row.
