...
| 7. 自主 QA、情报与核心技能 (Autonomous QA, Project Intelligence & Core Skills) | `engines/autonomous-qa-engine`、`engines/project-intelligence-engine`、`engines/knowledge-skill-model-foundry-engine`、`scripts/run_business_line_7_gate.py` | 仓库自有代码已建立精确、失败关闭的执行边界：自主 QA 的 40 个精确 Handler 已用受控本地夹具逐一调度并生成摘要绑定回执，结果为 6 `SUCCEEDED`、24 `PARTIAL`、10 `BLOCKED`；Project Intelligence 50 个 Handler 为 19 `LOCAL`、26 `PARTIAL`、5 `PLAN`，500 项源任务保持 `todo`，任务执行必须经过租户/项目/修订绑定的持久服务与幂等恢复，248 个验收场景必须同时具备作用域证据、holdout 和独立验证回执才可通过；10 阶段流水线只接受精确 Host Adapter，缺失、异常、部分结果或未知结果均停止并要求对账；Foundry 66 个 Provider-free Skill 具有本地语义 Handler，另外 1,244 个 Skill 具有精确 `NATIVE` 语义程序和 `HOST_ROUTE_BOUND` 适配器，完整 Skill 仍为 0/1,310；Provider、许可、持久化、成本、恢复和回执边界均显式校验。业务线门禁直接核对三套权威矩阵和资格回执，不再把目录校验、准备计划或自签摘要当作外部执行与验收 | `READY_FOR_EXTERNAL_GATE / NOT_CERTIFIED` | 真实 LLM/SCM/Runner 与 Provider 执行、多租户代表性环境、可信成本账单、故障恢复演练、500 项任务执行、248 个场景及独立 holdout、客户验收、独立第三方验证和认证仍为 `NOT_RUN / NOT_CERTIFIED`；必须由获授权的外部执行与证据链闭环 |
