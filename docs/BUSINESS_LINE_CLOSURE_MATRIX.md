...
| 全库跨语言转换 M29 | `/translation`、156 个受治理 Route Pack、13 语言 polyglot-route engine、持久受控 Runner | Java、Python、C#、TypeScript、Go、Rust、C++、Objective-C、Swift、PHP、Kotlin、React、Flutter 组成显式 13×12 活动矩阵；JavaScript 仅保留在 deprecated 历史分区。精确运行时、13 个源分析面与仓库 inventory/discovery/placement/build/relift 接缝已接入；Kotlin 绑定编译器/JAR/JDK/完整目录摘要，React 绑定 Node/TypeScript/包闭包，Flutter 仅绑定完整 Dart SDK 上的 import-free pure-Dart 分析、kernel 编译与执行。`repository-pipeline` 对内容寻址清单、项目图、逐单元重验、命名空间装配、目标构建和 ZIP 失败关闭。当前 90 条既有路线为 `limited`，66 条新增路线为 `research`；156 条路线及仓库执行证据均保持 `NOT_RUN`（`38` manifest 漂移未重放 + `52` 未执行 + `66` V3 campaign 未运行） | `TOOLCHAIN_READY / NOT_CERTIFIED` | 312 个 SMALL/MEDIUM 独立仓库工作负载、真实客户整库、独立验证与外部认证保持 `NOT_RUN`；React UI 与 Flutter framework/UI/插件/模拟器/设备语义，以及对象图、异常、异步、I/O、数据库、并发、依赖/资源/配置/测试迁移未闭合 |
| 多语言项目生成 B46-B95 | `/generation`、生成 API、本地 Runner、project-synthesis engine、Batch 66-95 Skills | 冻结的 P0 为 API/starter-v1、8 个精确语言目标、PostgreSQL 17.5 本地一次性环境与 JWT/RS256 OIDC；仓库实现包含草稿、分析、显式批准、生成、精确工具链验证、摘要复算、归档、启动/健康/停止、一次性服务凭证、供给链清单和 10 门保守上线聚合器；`run_production_matrix.py` 覆盖 8 × JWT/OIDC 共 16 个本地 Profile | `LOCAL_ENGINEERING_IMPLEMENTED_EXTERNAL_BLOCKED` | 当前 Provider 观察为 PostgreSQL 17.11 且 IdP 为 OKP/EdDSA，与冻结的 RSA/RS256 路径不兼容；托管库迁移/写读/回滚、真实 RS256 IdP、Linux systemd/rootless Runner、Cloud Run apply/rollback/destroy、可信签名根、独立 UAT/安全评估、发布批准与外部生产认证均保持 `NOT_RUN` / `NOT_CERTIFIED` |
| ChinaDB 商业 SQL 迁移扩展 M31 | `/migration`、`/api/capabilities/database-sql`、`elmos-sql-transpiler commercial-*`、47 个 `$chinadb-*` Skill | 规范包经受信 checksum 根、精确库存、双 Skill 根和内容摘要校验接入；登记 13 个独立国产目标适配器身份与 6×13 共 78 条规划路线；预检绑定源 Profile 及目标 version/edition/mode/driver/charset/collation/time zone/capability snapshot，解析为 typed AST 与义务后，仅在显式兼容模式 allow-list 下由本地查询适配器发射目标 SQL（`LOCAL_EMITTED`）；精确产品版本 Profile、实库执行、结果等价与认证仍强制 `NOT_RUN` / `NOT_CERTIFIED` | `LOCAL_ADAPTER / LOCAL_EMITTED` | 真实源/目标执行、值/类型/顺序/错误/事务等价、独立 holdout、性能、安全、切换及 Batch 31 认证证据均保持 `NOT_RUN / NOT_CERTIFIED` |
...
| SQL 方言与本地 routine 转写 `certified-ddl-v1` + `certified-alter-v1` + `certified-insert-v1` + `certified-dml-v1` + `certified-routine-v1` + typed object routes | `engines/sql-dialect-engine` CLI、`make sql-dialect` | PostgreSQL/MySQL/Oracle/SQL Server 四方言 12 条方向；DDL、ALTER、固定列纯字面量 INSERT、受限单源/等值 INNER JOIN `INSERT ... SELECT`、单表 `UPDATE`、有源目录主键/唯一键和同类型证明的受限 `UPDATE ... FROM`、显式 namespace mapping、视图、表/列/函数/约束注释、保留完整签名的 routine 权限、表权限、标量函数、受限 OUT/INOUT 过程、简单 `RETURNS TABLE` 函数与 PostgreSQL trigger metadata 通过真实 sqlglot AST、typed IR 和逐厂商 emitter 转写；SQL Server 表/列注释在显式 schema profile 下使用 `sys.sp_addextendedproperty`，固定边界 ASCII regex 在 SQL Server 使用 binary-collation lowering；MySQL 表注释使用 `ALTER TABLE ... COMMENT`，完整源列 catalogue 下列注释使用 `ALTER TABLE ... MODIFY COLUMN ... COMMENT`，缺少完整列定义时失败关闭；JSON/plain binary、无界 PostgreSQL `BYTEA` 到各目标 LOB、SERIAL/BIGSERIAL、DECIMAL 精度与索引语义按目标能力保守映射；相邻 PostgreSQL 注释字面量和 `COMMENT ON CONSTRAINT` 语法缺口仅做严格词法合并/兼容解析后重新走 typed IR；PL/pgSQL 副作用、动态 SQL、事务控制、未证明的联接回填与冲突策略、RLS、JSONB/数组和未闭合目标路线均返回机器可读 blocker；扫描器实测自动候选 **1173/1485 = 79.0%**，默认无 namespace profile 的四目标 emitter reachability **343/1173 = 29.2%**，显式 `{"": "dbo"}` profile 为 **393/1173 = 33.5%**，源 SQL 处置覆盖 **1485/1485 = 100.0%**；新增 13 个国产数据库目标（DM8、KingbaseES、openGauss、TiDB、GBase 8s/8c/8a、HighGo/HGDB、OceanBase 双模式、GaussDB 双模式、GoldenDB）逐目标 route ledger 为 **19305/19305 = 100.0%**，扫描器不自动发射（`automaticTargetEmissions = 0`）；仅在显式 `--compatibility-mode` allow-list 下把 DDL 映射到已有 postgres/mysql/oracle emitter（`LOCAL_ADAPTER` / `LOCAL_EMITTED`），不是原生国产 DDL、实库执行或认证；完整 SQL 工程测试、ruff/mypy 通过 | `REPOSITORY_CLOSED`（仅限明确 profile 的已证明子集；国产目标为 route accounting） | Oracle/SQL Server 的 routine 语法仍需真实目标执行，Postgres/MySQL routine 也尚未做外部数据库执行；国产目标 renderer、实库执行、独立验证与认证均保持 `NOT_RUN` / `NOT_CERTIFIED` |
