...
| SQL 方言与本地 routine 转写、ChinaDB 本地适配与 Batch 31 门禁 | `engines/sql-dialect-engine`、`engines/database-data-engine/sql-transpiler`、`scripts/batch31/run_chinadb_qualification.py` | P0 高优先级语义阻断为 0；四目标共同可达 1,189/1,365，仍有 365 个 P1/P2 路由单元显式阻断；ChinaDB 真实厂商执行 0/13，DM8 试点缺外部版本/许可沙箱/端点/凭据/授权与回执，专用 Runner 75ms 为 `NOT_RUN_ENVIRONMENT_INVALID`；本地结果仅为自证工程证据 | `PREFLIGHT_BETA / NOT_CERTIFIED` | 依次完成 DM8 真实端到端、专用 Runner 75ms、独立验证，再复制到其余 12 目标；不得以仓库自签或模拟回执关闭门禁 |
