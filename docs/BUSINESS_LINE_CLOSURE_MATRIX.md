...
| 用户操作日志与运营管理端 | 根布局采集器、`/api/telemetry/events`、control-plane observability API、V50 `audit_events`、`/admin` | 页面访问、语义操作、API 成败/耗时、浏览器错误、页面加载与 FCP 批量采集；不记录输入值、Token、查询参数、请求体或错误原文；服务端 HMAC 会话、绑定租户/Actor、RLS、追加式与幂等写入；用户可见关闭开关；管理端短期令牌读取事件量、会话、失败率、P95、业务线与错误码 | `REPOSITORY_CLOSED` | 真实生产量级、30 天保留/归档任务、告警路由与负责人、SLO/成本、隐私评审和故障演练保持 `NOT_RUN` |
