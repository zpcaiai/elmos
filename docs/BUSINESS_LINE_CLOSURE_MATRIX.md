...
源格式复核或引擎缺陷四类之一，2026-09-04 最新实测为
**1739/1739 = 100.0% disposition coverage**。
因此 100% 表示没有 SQL 单元从审计与后续动作中丢失，不表示未获证明的跨方言
语义已经自动转换；外部执行、独立验证和认证继续保持 `NOT_RUN` / `NOT_CERTIFIED`。

截至 2026-09-04，对当前 81 个真实迁移文件的最新复测为：自动转写候选
**1302/1739 = 74.9%**（源侧上界），处置覆盖 **1739/1739 = 100.0%**；本轮新增的
...
当前 81 个迁移文件包含 1739 个处置单元；显式 namespace profile 下有 1302 个
自动候选、435 个人工迁移、2 个源格式复核和 0 个引擎缺陷。将 1302 条源侧候选
逐条通过四个目标 emitter 的最新复测为：四目标交集 **363/1302 = 27.9%**，
PostgreSQL/MySQL/Oracle/SQL Server 分别为 1302/411/435/525。SQL Server 表/列注释可走
`sys.sp_addextendedproperty`，MySQL 在完整源列 catalogue 下使用
`ALTER TABLE ... MODIFY COLUMN ... COMMENT`；缺少完整列定义或 JSONB 列注释仍失败关闭。固定列清单、纯字面量
