...
Pinned ZIP (untrusted data)
  ...
  -> authenticated policy gate
       -> 45 exact local semantic handlers
       -> 1,265 allowlisted prepare-only handlers (no semantic effect)
...
2. The 45 local semantic handlers reject missing or empty declared inputs and
