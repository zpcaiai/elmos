 ...
 *
 * <p>NOTE: this file depends on Spring's {@code JdbcClient} and was therefore not
 * compiled in the authoring sandbox (no Maven Central access). Its SQL, however,
 * targets the V52/V54 objects that were executed and verified there.</p>
 *
...
public final class JdbcObjectStorageStore implements S3ObjectStore.ObjectStorageMetadata {
    ...
    private SigV4Presigner.Credentials credentials(String reference) {
        ...
        String material = secrets.resolve(reference);
        String[] parts = material.split(":", 3);
        if (parts.length < 2) {
            throw new S3ObjectStore.ObjectStorageException("OBJECT_STORAGE_CREDENTIAL_MALFORMED");
        }
        return new SigV4Presigner.Credentials(parts[0], parts[1], parts.length > 2 ? parts[2] : null);
    }
