...
- 新增目录 `2026-09-08.1`：保留三档订阅套餐的精确价格/额度快照，并增加
  500 Credits（¥99）和单项目生成一次（¥39）。V93 前向追加该版本并重绑定
  试用和账期函数；既有 V49 迁移与 `2026-07-28.2` 历史保持不变。
...
- 迁移后创建的 `elmos_billing_runtime` 角色会恢复 V2/商业函数白名单与只读商品、
  Credit、权益、Token 事实权限；不授予 Credit 表的直接写权限。
- V94 前向修复 V84 ELMPay 目录触发器中 `encode` 的 schema 绑定；V84 保持原校验和。
