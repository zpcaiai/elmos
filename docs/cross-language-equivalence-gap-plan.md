...
已交付的只是边界值差分测试的第一片（Python × TypeScript，进 CI 无需额外工具链；
Java/Go/Rust/C++ 的比对是本次一次性验证，未固化）。**仍然缺**：

- **随机属性测试**：每条路线 ≥ 10,000 组生成输入，而非 16 组边界值；
- **变形测试**：`clamp` 的 `0 ≤ result ≤ u`、幂等等不变式，两侧同时校验；
- **突变测试**：对 emitter 注入突变（例如去掉 `Math.trunc`）验证用例集能杀死它。
  **这是唯一能给"用例够不够"一个数字的手段**，建议作为门禁指标，起步阈值 80%。

### 需要在有全部九套工具链的机器上补做

1. `scripts/batch29/run_polyglot_routes.py` 重新生成 30 条路线的证据——
   `calculate` 与 `difference` 的发射结果已变，`certification/*.json` 里记录的
   `target.sha256` 全部过期。
2. C#、Swift、Objective-C 三个目标的编译与执行验证（本次环境无 dotnet / swiftc / macOS SDK）。
3. `routes/*/lowering/profile.json` 增记 R1 / R2 两条规则——本次未改，
   以免影响 `validate_translation_route_matrix.py` 的既有校验。
...
### 6.4 有界形式化验证 —— 只对 L0–L3 的纯函数子集

在纯函数、无循环或循环有界展开的子集上，把源 IR 与目标 IR 编码成 SMT 公式，
用求解器证明"对全部输入等价"（而非抽样等价）。这是唯一能称为"证明"的一层。

- 适用范围：整数/浮点算术、比较、布尔逻辑、`if`、有界循环 —— **正好覆盖 L0–L2**；
- 不适用：递归、无界循环、堆分配、并发；
- 现有资产：`docs/frt-g01-g30` 里有 `frt-1905-smt-solver-adapter` 的 Skill 契约，
  但状态是未配置的规划件，需要真实实现。
...
| 阶段 | 内容 | 前置 | 产出的可信断言 |
| --- | --- | --- | --- |
| **S0** | 统一语言集（6 还是 9），补齐或删除 cpp/objc/swift 路线 | — | 路线计数与证据自洽 |
| **S1** | P0-4：随机属性、变形、突变测试（P0-0 ~ P0-3 已修复） | S0 | L0 子集内**首次**有强度可度量的等价证据 |
| **S2** | SMT 适配器落地，对 L0 子集做有界形式化验证 | S1 | L0 子集内的**真正等价证明** |
...
**S1 和 S2 应当优先于任何范围扩展。** P0-0 ~ P0-3 已修复，但用例强度仍未度量。 理由：现在扩范围，是在一个未经强度验证的
...
**用例强度仍未度量，L0 仍无形式化证明。先把这两件做完，再谈扩范围。**
