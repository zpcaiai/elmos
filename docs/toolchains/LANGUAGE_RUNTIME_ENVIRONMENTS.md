...
| `routes-windows-vcpp6` | Licensed Microsoft Visual C++ 6.0 SP6 `CL.EXE`, `LINK.EXE`, and `MSVCP60.DLL`, all x86 PE files and SHA-256 bound by an external manifest | Windows x86-compatible only; use `schemas/batch29/vcpp6-toolchain-binding.schema.json` and the cross-host campaign; evidence is self-attested until a distinct verifier checks it |
...
| Flutter | Darwin arm64 exact tuple at `/opt/homebrew/share/flutter`: Flutter 3.44.1 revision 924134a44c with its bundled Dart 3.12.1; ambient or environment-selected Dart is not accepted. The read-only doctor invokes only bundled Dart; the exact receipt binds the Flutter launcher/version metadata and complete bundled Dart SDK tree to the authenticated local or GitHub-hosted macOS image profile without calling Flutter's mutable cache updater. Unknown images remain probe-only and fail closed; unobserved tree variants are rejected. The repository gate analyzes, compiles to a linked kernel, and executes dependency-free import-free pure-Dart modules. Flutter framework/UI, plugins, assets, platform builds, emulators and devices remain `UNSUPPORTED` / `NOT_RUN`. |
| VB6 | Microsoft Visual Basic 6.0 SP6 on an authorized Windows x86-compatible runner; exact `VB6.EXE` and `MSVBVM60.DLL` bytes are externally manifest-bound before the 28-route cross-host campaign |
| VC++6 | Microsoft Visual C++ 6.0 SP6 on an authorized Windows x86-compatible runner; exact `CL.EXE`, `LINK.EXE`, and `MSVCP60.DLL` bytes are externally manifest-bound before the 28-route cross-host campaign |
