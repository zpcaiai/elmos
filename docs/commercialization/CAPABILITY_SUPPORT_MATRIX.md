...
| 维度 | 状态 |
|---|---|
| Boot 2.7.18 / Java 17 / Maven | ✅ 有端到端本地执行证据（`PASSED_LOCAL`） |
| Boot 1.5.x、2.0–2.7 其余版本、3.0–3.5 区间 | ⚠️ `NOT_RUN`，需显式开实验路线才执行 |
| **Gradle** | ❌ **`NOT_IMPLEMENTED`** |
| XML 配置 / WebFlux / 部分 Jakarta 迁移 | 指纹阶段会明确阻断 |

**"目录覆盖 ≠ 证据覆盖"**：路线目录里声明了四条 Maven 路线，但只有一个元组有证据。
不要把目录当支持范围报出去。
...
**Q：Spring 升级支持哪些版本？**
A：有完整验证证据的是 Boot 2.7.18 / Java 17 / Maven。其他版本区间在目录中声明了，
但没有执行证据；Gradle 明确未实现。建议先确认贵方仓库的构建工具与 Boot 版本。
