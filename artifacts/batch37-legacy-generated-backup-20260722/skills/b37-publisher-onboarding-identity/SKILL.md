...
description: "Implement and verify Batch 37 Skill 1320 for publisher onboarding identity. Use when work requires Publisher\u5165\u9a7b\u548c\u8eab\u4efd\u9a8c\u8bc1, typed evidence, negative and holdout validation, and a fail-closed certification decision."
---
...
1. Inspect the current implementation, runtime facts, policies, incidents, decisions, and prior evidence relevant to `b37-publisher-onboarding-identity`.
...
4. Implement the smallest production-shaped capability that exercises `Publisher入驻和身份验证` without bypassing existing control planes.
...
- Capability-specific implementation, tests, run logs, evidence references, and residual-risk records for `b37-publisher-onboarding-identity`
