...
description: "Implement and verify Batch 36 Skill 1299 for local eval affected tests. Use when work requires Local Eval\u548c\u53d7\u5f71\u54cd\u6d4b\u8bd5\u8fd0\u884c, typed evidence, negative and holdout validation, and a fail-closed certification decision."
---
...
1. Inspect the current implementation, runtime facts, policies, incidents, decisions, and prior evidence relevant to `b36-local-eval-affected-tests`.
...
4. Implement the smallest production-shaped capability that exercises `Local Eval和受影响测试运行` without bypassing existing control planes.
...
- Capability-specific implementation, tests, run logs, evidence references, and residual-risk records for `b36-local-eval-affected-tests`
