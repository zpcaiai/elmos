---
name: b36-ide-protocol-agent-bridge
description: "Implement and verify Batch 36 Skill 1288 for ide protocol agent bridge. Use when work requires IDE\u534f\u8bae\u3001Language Server\u548c\u672c\u5730Agent Bridge, typed evidence, negative and holdout validation, and a fail-closed certification decision."
---

# IDE协议、Language Server和本地Agent Bridge

## Operating mode

Work directly in the repository and preserve existing customer, tenant, security, and evidence boundaries. Read the shared Batch 36 contracts before changing code:

- `../../../docs/batch36/AUTHORITY.md`
- `../../../docs/batch36/IMPLEMENTATION_CONTRACT.md`
- `../../../docs/batch36/QUALITY_GATES.md`
- `../../../docs/batch36/EVIDENCE_BOUNDARY.md`

Use `python scripts/mature_product_toolkit.py validate --batch 36` for structural checks. Use the scaffold and gate commands only against an explicit pack key and approved scope.
...
## Skill 1288: IDE协议、Language Server和本地Agent Bridge

Implement this capability as part of **IDE CLI and pull-request native developer experience**. The Batch objective is: Expose safe migration workflows inside the tools customer engineers use every day.

## Workflow

1. Inspect the current implementation, runtime facts, policies, incidents, decisions, and prior evidence relevant to `b36-ide-protocol-agent-bridge`.
...
4. Implement the smallest production-shaped capability that exercises `IDE协议、Language Server和本地Agent Bridge` without bypassing existing control planes.
...
7. Run the Batch 36 conservative gate and accept only the strongest status supported by actual evidence.

## Required repository outputs

- `mature-product-packs/batch36/<pack-key>/program.json`
- `mature-product-packs/batch36/<pack-key>/evidence.json`
- `mature-product-packs/batch36/<pack-key>/certification.json`
- Capability-specific implementation, tests, run logs, evidence references, and residual-risk records for `b36-ide-protocol-agent-bridge`
- `mature-product-packs/batch36/<pack-key>/gate-result.json` and `gate-report.md`

## Verification

- Validate all Batch 36 Skills, Schemas, templates, and local references.
...
The capability has typed artifacts, executable implementation or an explicit `BLOCKED` boundary, deterministic and negative tests, independent evidence, residual-risk ownership, and a conservative Batch 36 gate result. Unexecuted field evidence remains `NOT_RUN`; no package-level test may be represented as production certification.
