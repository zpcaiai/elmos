...
public record AgentConfig(
    ...
    public static AgentConfig fromEnvironment(Map<String, String> env) {
        ...
        String enrolment = resolveSecret(
                env, "ELMOS_RUNNER_ENROLMENT_TOKEN", isProductionLike(env));
    ...
    /**
     * Production enrolment is file-backed only. Inline environment secrets leak
     * through process inspection and deployment manifests; writable, world-readable
     * or symlinked files let another workload replace or read the runner identity.
     * Group-read is accepted for Kubernetes projected secrets only when the pod
     * uses a dedicated runtime group.
     */
    private static String resolveSecret(
            Map<String, String> env, String name, boolean productionLike) {
        ...
        if (file != null && !file.isBlank()) {
            ...
            try {
                ...
                if (byteSize < 32 || byteSize > 4096) {
                    throw new ConfigException(name + "_FILE has an unsafe size");
                }
                try {
                    ...
                    if (permissions.stream().anyMatch(UNSAFE_SECRET_PERMISSIONS::contains)) {
                        throw new ConfigException(
                                name + "_FILE must not be group-writable or world-accessible");
                    }
                } catch (UnsupportedOperationException ignored) {
                    // A non-POSIX production platform must enforce an equivalent ACL.
                }
                String value = Files.readString(path).trim();
                if (value.length() < 32 || value.length() > 4096
                        || value.chars().anyMatch(character ->
                        character == 0 || character == '\r' || character == '\n')) {
                    throw new ConfigException(name + "_FILE contains an invalid secret");
                }
                return value;
            } catch (Exception ex) {
                if (ex instanceof ConfigException configException) {
                    throw configException;
                }
                throw new ConfigException("cannot read " + name + "_FILE");
            }
        }
        if (productionLike && inline != null && !inline.isBlank()) {
            throw new ConfigException(name + "_FILE is required in production");
        }
        if (inline == null || inline.isBlank()) {
