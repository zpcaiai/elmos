      ...
      {view === "routes" ? (
        <section id="migration-routes-panel" className="studio-layout" role="tabpanel" aria-labelledby="migration-routes-tab">
          ...
          {selectedCapability && <aside className="detail-panel">
            ...
            {selectedCapability.id === "M30" && <div className="feature-callout"><Icon name="workflow" size={17} /><div><strong>Spring 现代化升级专属流程已接入</strong><small>支持经典 Spring XML、注解与旧 Boot 画像；外部运行与升级认证仍为 NOT_RUN。</small><a className="text-link" href="/spring">打开 Spring 现代化升级 <Icon name="arrow" size={13} /></a></div></div>}
