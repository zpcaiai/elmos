...
export function PrecisionMigrationJobs() {
  ...
  return <section className="surface-card precision-job-card" aria-labelledby="precision-job-title">
    ...
    <div className="form-stack precision-job-form">
      <label><span>运行时功能标识</span><input value={skill} onChange={(event) => setSkill(event.target.value)} required pattern="pm-[a-z0-9-]+" autoComplete="off" spellCheck={false} /></label>
      <div className="form-grid">
        ...
        <label><span>只读工作区路径</span><input value={workspacePath} onChange={(event) => setWorkspacePath(event.target.value)} placeholder="评估 handler 必填；必须位于批准根目录" autoComplete="off" spellCheck={false} /></label>
      </div>
