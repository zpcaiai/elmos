...
export const generationTargets: GenerationTarget[] = [
  ...
  { id: "csharp", language: "C#", runtime: ".NET 10.0.301", framework: "ASP.NET Core 10", port: 8083, sourceSkill: "PG101–PG112", verificationCommand: "dotnet test -c Release", verificationStatus: "NOT_RUN", maturity: "limited", productionProfiles: ["postgresql+jwt", "postgresql+oidc"], productionEntityScope: "multi-entity", accent: "violet", icon: "layers" },
  { id: "typescript", language: "TypeScript", runtime: "Node 26.0.0", framework: "NestJS 11.1.6 / Fastify 5.6.1", port: 8084, sourceSkill: "PG226", verificationCommand: "pnpm check && pnpm test && pnpm build", verificationStatus: "NOT_RUN", maturity: "limited", productionProfiles: ["postgresql+jwt", "postgresql+oidc"], productionEntityScope: "multi-entity", accent: "cyan", icon: "code" },
  { id: "go", language: "Go", runtime: "1.25.0", framework: "net/http", port: 8085, sourceSkill: "PG237", verificationCommand: "go vet ./... && go test -race ./...", verificationStatus: "NOT_RUN", maturity: "limited", productionProfiles: ["postgresql+jwt", "postgresql+oidc"], productionEntityScope: "multi-entity", accent: "green", icon: "code" },
  { id: "kotlin", language: "Kotlin", runtime: "2.2.20 / JVM 21", framework: "Ktor 3.2.3", port: 8086, sourceSkill: "PG250", verificationCommand: "gradle --no-daemon test build", verificationStatus: "NOT_RUN", maturity: "limited", productionProfiles: ["postgresql+jwt", "postgresql+oidc"], productionEntityScope: "multi-entity", accent: "violet", icon: "layers" },
  { id: "php", language: "PHP", runtime: "8.4.12", framework: "Native HTTP modular service", port: 8087, sourceSkill: "PG263", verificationCommand: "php -l public/index.php && php tests/run.php", verificationStatus: "NOT_RUN", maturity: "limited", productionProfiles: ["postgresql+jwt", "postgresql+oidc"], productionEntityScope: "multi-entity", accent: "blue", icon: "spark" },
  { id: "rust", language: "Rust", runtime: "1.89.0", framework: "Axum 0.8.4", port: 8088, sourceSkill: "PG287", verificationCommand: "cargo clippy && cargo test --locked", verificationStatus: "NOT_RUN", maturity: "limited", productionProfiles: ["postgresql+jwt", "postgresql+oidc"], productionEntityScope: "multi-entity", accent: "amber", icon: "code" },
];
...
export const installedSkillInventory = {
  codexSkillCount: 4677,
  runtimeSkillCount: 7022,
  countingRule: "directories-with-SKILL.md",
} as const;

export const migrationCapabilities: MigrationCapability[] = [
  { id: "M29", batch: 29, title: "跨语言路由", domain: "Directed language routes", description: "15 种活动语言组成 210 条精确方向路线；本地通过 Profile 为 0，210 条路线的本地执行、仓库执行、独立验证与外部认证全部保持 NOT_RUN。", skillCount: 20, schemaCount: 3, gateCommand: "scripts/batch29/run_route_gate.py", status: "LIMITED", icon: "code", accent: "cyan" },
  ...
  { id: "M31", batch: 31, title: "数据库与数据平台", domain: "Database & data", description: "类型、约束、SQL、ETL 与逐项对账的精确迁移；ChinaDB 扩展已登记 13 个国产目标，仅在显式兼容模式下有限发射 SQL，不代表厂商原生适配，实库执行与认证保持 NOT_RUN。", skillCount: 69, schemaCount: 9, gateCommand: "scripts/batch31/run_database_gate.py", status: "READY", icon: "database", accent: "violet" },
