...
export const generationTargets: GenerationTarget[] = [
  { id: "java", language: "Java", runtime: "21", framework: "Spring Boot 3.5.3", port: 8081, sourceSkill: "PG077–PG088", verificationCommand: "mvn -B package", verificationStatus: "NOT_RUN", maturity: "limited", productionProfiles: ["postgresql+jwt", "postgresql+oidc"], accent: "amber", icon: "code" },
  ...
  { id: "csharp", language: "C#", runtime: ".NET 10.0.301", framework: "ASP.NET Core 10", port: 8083, sourceSkill: "PG101–PG112", verificationCommand: "dotnet test -c Release", verificationStatus: "NOT_RUN", maturity: "limited", productionProfiles: ["postgresql+jwt", "postgresql+oidc"], accent: "violet", icon: "layers" },
  { id: "typescript", language: "TypeScript", runtime: "Node 26.0.0", framework: "NestJS 11.1.6 / Fastify 5.6.1", port: 8084, sourceSkill: "PG226", verificationCommand: "pnpm check && pnpm test && pnpm build", verificationStatus: "NOT_RUN", maturity: "limited", productionProfiles: ["postgresql+jwt", "postgresql+oidc"], accent: "cyan", icon: "code" },
  { id: "go", language: "Go", runtime: "1.25.0", framework: "net/http", port: 8085, sourceSkill: "PG237", verificationCommand: "go vet ./... && go test -race ./...", verificationStatus: "NOT_RUN", maturity: "limited", productionProfiles: ["postgresql+jwt", "postgresql+oidc"], accent: "green", icon: "code" },
  { id: "kotlin", language: "Kotlin", runtime: "2.2.20 / JVM 21", framework: "Ktor 3.2.3", port: 8086, sourceSkill: "PG250", verificationCommand: "gradle --no-daemon test build", verificationStatus: "NOT_RUN", maturity: "limited", productionProfiles: ["postgresql+jwt", "postgresql+oidc"], accent: "violet", icon: "layers" },
  { id: "php", language: "PHP", runtime: "8.4.12", framework: "Native HTTP modular service", port: 8087, sourceSkill: "PG263", verificationCommand: "php -l public/index.php && php tests/run.php", verificationStatus: "NOT_RUN", maturity: "limited", productionProfiles: ["postgresql+jwt", "postgresql+oidc"], accent: "blue", icon: "spark" },
  { id: "rust", language: "Rust", runtime: "1.89.0", framework: "Axum 0.8.4", port: 8088, sourceSkill: "PG287", verificationCommand: "cargo clippy && cargo test --locked", verificationStatus: "NOT_RUN", maturity: "limited", productionProfiles: ["postgresql+jwt", "postgresql+oidc"], accent: "amber", icon: "code" },
];
