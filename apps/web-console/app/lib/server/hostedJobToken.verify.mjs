...
assert.throws(
  () => issueHostedJobToken({ ...claims, exp: now }, privateKey, "dispatcher-1"),
  (error) => error instanceof HostedJobTokenError && error.code === "JOB_TOKEN_LIFETIME_INVALID",
);
assert.throws(
  () => issueHostedJobToken({ ...claims, scope: ["generate", "admin"] }, privateKey, "dispatcher-1"),
  (error) => error instanceof HostedJobTokenError && error.code === "JOB_TOKEN_SCOPE_INVALID",
);
assert.throws(
  () => issueHostedJobToken({ ...claims, limits: { ...claims.limits, pids: 0 } }, privateKey, "dispatcher-1"),
  (error) => error instanceof HostedJobTokenError && error.code === "JOB_TOKEN_LIMITS_INVALID",
);
assert.throws(
  () => verifyHostedJobToken(token, publicKey, "dispatcher-1", "generate", new Set(), now + 1, "tenant-other"),
  (error) => error instanceof HostedJobTokenError && error.code === "JOB_TOKEN_TENANT_MISMATCH",
);
assert.throws(
  () => verifyHostedJobToken(token, publicKey, "dispatcher-1", "generate", new Set(), now + 1, "tenant-alpha", "job-other"),
  (error) => error instanceof HostedJobTokenError && error.code === "JOB_TOKEN_JOB_MISMATCH",
);
assert.throws(
  () => verifyHostedJobToken("not.a.token", publicKey, "dispatcher-1", "generate", new Set(), now + 1),
  (error) => error instanceof HostedJobTokenError,
);
assert.throws(
  () => rejectLocalRunnerInProduction({
