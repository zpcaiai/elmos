---
name: b42-agent-tool-permissions
description: "Implement and verify Batch 42 Skill 1422 for agent tool permissions. Use when work requires Agent Tool Registry和权限控制, typed evidence, negative and holdout validation, and a fail-closed certification decision."
---

# Agent Tool Registry和权限控制
...
## Skill 1422: Agent Tool Registry和权限控制
...
1. Inspect the current implementation, runtime facts, policies, incidents, decisions, and prior evidence relevant to `b42-agent-tool-permissions`.
...
4. Implement the smallest production-shaped capability that exercises `Agent Tool Registry和权限控制` without bypassing existing control planes.
...
- Capability-specific implementation, tests, run logs, evidence references, and residual-risk records for `b42-agent-tool-permissions`
