---
name: b40-independent-security-assessment
description: "Implement and verify Batch 40 Skill 1391 for independent security assessment. Use when work requires 独立Pen Test、代码审计和认证, typed evidence, negative and holdout validation, and a fail-closed certification decision."
---
...
1. Inspect the current implementation, runtime facts, policies, incidents, decisions, and prior evidence relevant to `b40-independent-security-assessment`.
...
4. Implement the smallest production-shaped capability that exercises `独立Pen Test、代码审计和认证` without bypassing existing control planes.
...
- Capability-specific implementation, tests, run logs, evidence references, and residual-risk records for `b40-independent-security-assessment`
