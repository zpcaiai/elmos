---
name: b44-provider-resource-routing
description: "Implement and verify Batch 44 Skill 1470 for provider resource routing. Use when work requires 模型、Runner和Provider智能路由, typed evidence, negative and holdout validation, and a fail-closed certification decision."
---

# 模型、Runner和Provider智能路由
...
## Skill 1470: 模型、Runner和Provider智能路由
...
1. Inspect the current implementation, runtime facts, policies, incidents, decisions, and prior evidence relevant to `b44-provider-resource-routing`.
...
4. Implement the smallest production-shaped capability that exercises `模型、Runner和Provider智能路由` without bypassing existing control planes.
...
- Capability-specific implementation, tests, run logs, evidence references, and residual-risk records for `b44-provider-resource-routing`
