...
description: "Implement and verify Batch 39 Skill 1354 for multiregion failover. Use when work requires 多区域故障转移, typed evidence, negative and holdout validation, and a fail-closed certification decision."
---
...
1. Inspect the current implementation, runtime facts, policies, incidents, decisions, and prior evidence relevant to `b39-multiregion-failover`.
...
4. Implement the smallest production-shaped capability that exercises `多区域故障转移` without bypassing existing control planes.
...
- Capability-specific implementation, tests, run logs, evidence references, and residual-risk records for `b39-multiregion-failover`
