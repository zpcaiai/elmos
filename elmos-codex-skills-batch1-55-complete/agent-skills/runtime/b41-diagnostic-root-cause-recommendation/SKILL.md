...
description: "Implement and verify Batch 41 Skill 1404 for diagnostic root cause recommendation. Use when work requires Diagnostic Root Cause推荐, typed evidence, negative and holdout validation, and a fail-closed certification decision."
---

# Diagnostic Root Cause推荐
...
## Skill 1404: Diagnostic Root Cause推荐
...
1. Inspect the current implementation, runtime facts, policies, incidents, decisions, and prior evidence relevant to `b41-diagnostic-root-cause-recommendation`.
...
4. Implement the smallest production-shaped capability that exercises `Diagnostic Root Cause推荐` without bypassing existing control planes.
...
- Capability-specific implementation, tests, run logs, evidence references, and residual-risk records for `b41-diagnostic-root-cause-recommendation`
