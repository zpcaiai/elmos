---
name: b41-knowledge-freshness-versioning
description: "Implement and verify Batch 41 Skill 1406 for knowledge freshness versioning. Use when work requires 知识新鲜度、衰减和版本, typed evidence, negative and holdout validation, and a fail-closed certification decision."
---
...
1. Inspect the current implementation, runtime facts, policies, incidents, decisions, and prior evidence relevant to `b41-knowledge-freshness-versioning`.
...
4. Implement the smallest production-shaped capability that exercises `知识新鲜度、衰减和版本` without bypassing existing control planes.
...
- Capability-specific implementation, tests, run logs, evidence references, and residual-risk records for `b41-knowledge-freshness-versioning`
