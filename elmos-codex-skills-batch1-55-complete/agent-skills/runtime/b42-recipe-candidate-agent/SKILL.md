---
name: b42-recipe-candidate-agent
description: "Implement and verify Batch 42 Skill 1431 for recipe candidate agent. Use when work requires 自动生成Recipe Candidate, typed evidence, negative and holdout validation, and a fail-closed certification decision."
---

# 自动生成Recipe Candidate
...
## Skill 1431: 自动生成Recipe Candidate
...
1. Inspect the current implementation, runtime facts, policies, incidents, decisions, and prior evidence relevant to `b42-recipe-candidate-agent`.
...
4. Implement the smallest production-shaped capability that exercises `自动生成Recipe Candidate` without bypassing existing control planes.
...
- Capability-specific implementation, tests, run logs, evidence references, and residual-risk records for `b42-recipe-candidate-agent`
