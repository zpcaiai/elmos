---
name: b39-scheduled-restore-dr-exercise
description: "Implement and verify Batch 39 Skill 1366 for scheduled restore dr exercise. Use when work requires 定期数据恢复和DR演练, typed evidence, negative and holdout validation, and a fail-closed certification decision."
---
...
1. Inspect the current implementation, runtime facts, policies, incidents, decisions, and prior evidence relevant to `b39-scheduled-restore-dr-exercise`.
...
4. Implement the smallest production-shaped capability that exercises `定期数据恢复和DR演练` without bypassing existing control planes.
...
- Capability-specific implementation, tests, run logs, evidence references, and residual-risk records for `b39-scheduled-restore-dr-exercise`
