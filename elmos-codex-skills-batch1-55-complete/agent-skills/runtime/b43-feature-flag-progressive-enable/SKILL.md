---
name: b43-feature-flag-progressive-enable
description: "Implement and verify Batch 43 Skill 1449 for feature flag progressive enable. Use when work requires Feature Flag和渐进启用, typed evidence, negative and holdout validation, and a fail-closed certification decision."
---
...
1. Inspect the current implementation, runtime facts, policies, incidents, decisions, and prior evidence relevant to `b43-feature-flag-progressive-enable`.
...
4. Implement the smallest production-shaped capability that exercises `Feature Flag和渐进启用` without bypassing existing control planes.
...
- Capability-specific implementation, tests, run logs, evidence references, and residual-risk records for `b43-feature-flag-progressive-enable`
