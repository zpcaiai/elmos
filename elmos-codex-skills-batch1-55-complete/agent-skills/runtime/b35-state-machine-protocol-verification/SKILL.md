---
name: b35-state-machine-protocol-verification
description: "Verify state machines protocols temporal ordering invalid transitions recovery and liveness for migrated services jobs messages sessions and cutover workflows."
---
...
## Skill 1273: b35-state-machine-protocol-verification

## Use this skill when

- Services, jobs, messages, sessions, or cutover workflows have explicit lifecycle rules.

## Domain-specific risks and invariants

- Happy-path transition tests miss illegal or temporal behavior.

## Workflow

1. Extract states, events, guards, effects, timers, retries, compensation, and forbidden transitions.
2. Create an executable state/protocol specification independent from source implementation.
3. Generate positive, negative, timeout, duplicate, reorder, restart, and recovery traces.
4. Run source and target conformance and runtime monitoring.
5. Measure state, transition, pair, and forbidden-path coverage.
6. Turn violations into minimal replay traces.
...
- Build explicit states, events, guards, effects, invariants, forbidden transitions, timeouts, and recovery transitions.
...
- State/protocol specs, generated traces, transition coverage, forbidden-path evidence, and runtime monitors.
