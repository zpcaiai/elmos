---
name: b35-counterexample-shrinking-replay
description: "Implement uniform counterexample capture shrinking minimization deduplication provenance and deterministic replay across all verification techniques."
---
...
## Skill 1284: b35-counterexample-shrinking-replay

## Use this skill when

- Any advanced verification failure needs a minimal durable reproducer.

## Domain-specific risks and invariants

- Large unreproducible failures cannot become regression assets.

## Workflow

1. Capture exact input, state, seed, schedule, model, solver, environment, artifact, and oracle versions.
2. Deduplicate by semantic fingerprint.
3. Apply type-aware, state-aware, trace-aware, and schedule-aware shrinking.
4. Verify the minimized case still fails and the original root cause remains.
5. Generate one-command replay and regression fixture.
6. Track owner, fix, and closure.
...
- Convert property failures, fuzz crashes, symbolic witnesses, model traces, query differences, and concurrency schedules into durable regression assets.
...
- Counterexample manifests, minimized inputs/traces, environment snapshots, replay commands, ownership, and regression links.
