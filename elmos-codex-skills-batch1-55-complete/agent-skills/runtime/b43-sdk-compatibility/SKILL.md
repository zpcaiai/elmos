...
description: "Implement and verify Batch 43 Skill 1439 for sdk compatibility. Use when work requires SDK兼容治理, typed evidence, negative and holdout validation, and a fail-closed certification decision."
---
...
1. Inspect the current implementation, runtime facts, policies, incidents, decisions, and prior evidence relevant to `b43-sdk-compatibility`.
...
4. Implement the smallest production-shaped capability that exercises `SDK兼容治理` without bypassing existing control planes.
...
- Capability-specific implementation, tests, run logs, evidence references, and residual-risk records for `b43-sdk-compatibility`
