---
name: b38-plane-topology-governance
description: "Implement and verify Batch 38 Skill 1336 for plane topology governance. Use when work requires 数据面、控制面和执行面拓扑治理, typed evidence, negative and holdout validation, and a fail-closed certification decision."
---
...
1. Inspect the current implementation, runtime facts, policies, incidents, decisions, and prior evidence relevant to `b38-plane-topology-governance`.
...
4. Implement the smallest production-shaped capability that exercises `数据面、控制面和执行面拓扑治理` without bypassing existing control planes.
...
- Capability-specific implementation, tests, run logs, evidence references, and residual-risk records for `b38-plane-topology-governance`
