---
name: b45-independent-expert-validation
description: "Implement and verify Batch 45 Skill 1490 for independent expert validation. Use when work requires 第三方独立验证和专家Review, typed evidence, negative and holdout validation, and a fail-closed certification decision."
---
...
1. Inspect the current implementation, runtime facts, policies, incidents, decisions, and prior evidence relevant to `b45-independent-expert-validation`.
...
4. Implement the smallest production-shaped capability that exercises `第三方独立验证和专家Review` without bypassing existing control planes.
...
- Capability-specific implementation, tests, run logs, evidence references, and residual-risk records for `b45-independent-expert-validation`
