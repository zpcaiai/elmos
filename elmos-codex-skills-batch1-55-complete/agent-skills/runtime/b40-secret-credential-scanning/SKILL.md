---
name: b40-secret-credential-scanning
description: "Implement and verify Batch 40 Skill 1377 for secret credential scanning. Use when work requires Secret扫描和Credential泄漏防护, typed evidence, negative and holdout validation, and a fail-closed certification decision."
---

# Secret扫描和Credential泄漏防护
...
## Skill 1377: Secret扫描和Credential泄漏防护
...
1. Inspect the current implementation, runtime facts, policies, incidents, decisions, and prior evidence relevant to `b40-secret-credential-scanning`.
...
4. Implement the smallest production-shaped capability that exercises `Secret扫描和Credential泄漏防护` without bypassing existing control planes.
...
- Capability-specific implementation, tests, run logs, evidence references, and residual-risk records for `b40-secret-credential-scanning`
