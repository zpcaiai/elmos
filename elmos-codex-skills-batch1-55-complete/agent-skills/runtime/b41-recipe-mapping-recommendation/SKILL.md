...
description: "Implement and verify Batch 41 Skill 1398 for recipe mapping recommendation. Use when work requires Recipe和Mapping推荐, typed evidence, negative and holdout validation, and a fail-closed certification decision."
---
...
1. Inspect the current implementation, runtime facts, policies, incidents, decisions, and prior evidence relevant to `b41-recipe-mapping-recommendation`.
...
4. Implement the smallest production-shaped capability that exercises `Recipe和Mapping推荐` without bypassing existing control planes.
...
- Capability-specific implementation, tests, run logs, evidence references, and residual-risk records for `b41-recipe-mapping-recommendation`
