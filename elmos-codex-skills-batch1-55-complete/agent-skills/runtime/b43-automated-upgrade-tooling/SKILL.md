---
name: b43-automated-upgrade-tooling
description: "Implement and verify Batch 43 Skill 1448 for automated upgrade tooling. Use when work requires 自动升级和迁移工具, typed evidence, negative and holdout validation, and a fail-closed certification decision."
---
...
1. Inspect the current implementation, runtime facts, policies, incidents, decisions, and prior evidence relevant to `b43-automated-upgrade-tooling`.
...
4. Implement the smallest production-shaped capability that exercises `自动升级和迁移工具` without bypassing existing control planes.
...
- Capability-specific implementation, tests, run logs, evidence references, and residual-risk records for `b43-automated-upgrade-tooling`
