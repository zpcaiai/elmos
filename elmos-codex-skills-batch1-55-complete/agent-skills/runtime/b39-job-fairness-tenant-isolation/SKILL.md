---
name: b39-job-fairness-tenant-isolation
description: "Implement and verify Batch 39 Skill 1352 for job fairness tenant isolation. Use when work requires Job公平调度和租户资源隔离, typed evidence, negative and holdout validation, and a fail-closed certification decision."
---
...
1. Inspect the current implementation, runtime facts, policies, incidents, decisions, and prior evidence relevant to `b39-job-fairness-tenant-isolation`.
...
4. Implement the smallest production-shaped capability that exercises `Job公平调度和租户资源隔离` without bypassing existing control planes.
...
- Capability-specific implementation, tests, run logs, evidence references, and residual-risk records for `b39-job-fairness-tenant-isolation`
