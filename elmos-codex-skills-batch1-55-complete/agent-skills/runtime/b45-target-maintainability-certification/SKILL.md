---
name: b45-target-maintainability-certification
description: "Implement and verify Batch 45 Skill 1483 for target maintainability certification. Use when work requires 目标代码维护性认证, typed evidence, negative and holdout validation, and a fail-closed certification decision."
---
...
1. Inspect the current implementation, runtime facts, policies, incidents, decisions, and prior evidence relevant to `b45-target-maintainability-certification`.
...
4. Implement the smallest production-shaped capability that exercises `目标代码维护性认证` without bypassing existing control planes.
...
- Capability-specific implementation, tests, run logs, evidence references, and residual-risk records for `b45-target-maintainability-certification`
