...
description: "Implement and verify Batch 39 Skill 1362 for platform cost anomaly monitoring. Use when work requires 平台成本和异常消耗监控, typed evidence, negative and holdout validation, and a fail-closed certification decision."
---
...
1. Inspect the current implementation, runtime facts, policies, incidents, decisions, and prior evidence relevant to `b39-platform-cost-anomaly-monitoring`.
...
4. Implement the smallest production-shaped capability that exercises `平台成本和异常消耗监控` without bypassing existing control planes.
...
- Capability-specific implementation, tests, run logs, evidence references, and residual-risk records for `b39-platform-cost-anomaly-monitoring`
