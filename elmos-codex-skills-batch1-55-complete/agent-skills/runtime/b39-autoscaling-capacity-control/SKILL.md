...
description: "Implement and verify Batch 39 Skill 1353 for autoscaling capacity control. Use when work requires 自动扩容和容量控制, typed evidence, negative and holdout validation, and a fail-closed certification decision."
---
...
1. Inspect the current implementation, runtime facts, policies, incidents, decisions, and prior evidence relevant to `b39-autoscaling-capacity-control`.
...
4. Implement the smallest production-shaped capability that exercises `自动扩容和容量控制` without bypassing existing control planes.
...
- Capability-specific implementation, tests, run logs, evidence references, and residual-risk records for `b39-autoscaling-capacity-control`
