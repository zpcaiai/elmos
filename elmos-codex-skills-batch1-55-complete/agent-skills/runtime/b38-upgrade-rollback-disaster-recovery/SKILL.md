---
name: b38-upgrade-rollback-disaster-recovery
description: "Implement and verify Batch 38 Skill 1345 for upgrade rollback disaster recovery. Use when work requires 升级失败回滚和灾难恢复, typed evidence, negative and holdout validation, and a fail-closed certification decision."
---
...
1. Inspect the current implementation, runtime facts, policies, incidents, decisions, and prior evidence relevant to `b38-upgrade-rollback-disaster-recovery`.
...
4. Implement the smallest production-shaped capability that exercises `升级失败回滚和灾难恢复` without bypassing existing control planes.
...
- Capability-specific implementation, tests, run logs, evidence references, and residual-risk records for `b38-upgrade-rollback-disaster-recovery`
