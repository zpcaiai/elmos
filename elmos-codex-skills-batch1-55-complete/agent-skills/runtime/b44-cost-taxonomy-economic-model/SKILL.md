---
name: b44-cost-taxonomy-economic-model
description: "Implement and verify Batch 44 Skill 1456 for cost taxonomy economic model. Use when work requires 成本分类、成本中心和经济模型, typed evidence, negative and holdout validation, and a fail-closed certification decision."
---
...
1. Inspect the current implementation, runtime facts, policies, incidents, decisions, and prior evidence relevant to `b44-cost-taxonomy-economic-model`.
...
4. Implement the smallest production-shaped capability that exercises `成本分类、成本中心和经济模型` without bypassing existing control planes.
...
- Capability-specific implementation, tests, run logs, evidence references, and residual-risk records for `b44-cost-taxonomy-economic-model`
