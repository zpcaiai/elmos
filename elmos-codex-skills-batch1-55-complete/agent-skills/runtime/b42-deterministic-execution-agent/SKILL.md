...
description: "Implement and verify Batch 42 Skill 1416 for deterministic execution agent. Use when work requires Deterministic Execution Agent, typed evidence, negative and holdout validation, and a fail-closed certification decision."
---
...
## Skill 1416: Deterministic Execution Agent
...
1. Inspect the current implementation, runtime facts, policies, incidents, decisions, and prior evidence relevant to `b42-deterministic-execution-agent`.
...
4. Implement the smallest production-shaped capability that exercises `Deterministic Execution Agent` without bypassing existing control planes.
...
- Capability-specific implementation, tests, run logs, evidence references, and residual-risk records for `b42-deterministic-execution-agent`
