---
name: b44-customer-roi-tco-value
description: "Implement and verify Batch 44 Skill 1472 for customer roi tco value. Use when work requires 客户ROI、TCO和业务价值模型, typed evidence, negative and holdout validation, and a fail-closed certification decision."
---
...
1. Inspect the current implementation, runtime facts, policies, incidents, decisions, and prior evidence relevant to `b44-customer-roi-tco-value`.
...
4. Implement the smallest production-shaped capability that exercises `客户ROI、TCO和业务价值模型` without bypassing existing control planes.
...
- Capability-specific implementation, tests, run logs, evidence references, and residual-risk records for `b44-customer-roi-tco-value`
