---
name: github-scm-connector-security-compatibility-tests-and-r-bd8c3157
description: "Implement GitHub connector contract tests, GHES version matrix, webhook attacks, credential revocation, rate-limit chaos, tenant isolation, and release gates."
---
...
Implement GitHub connector contract tests, GHES version matrix, webhook attacks, credential revocation, rate-limit chaos, tenant isolation, and release gates.

This Skill belongs to **Batch 35A** and must extend the approved ELMOS domain rather than create parallel Tenant, Identity, Artifact, Policy, Workflow, Audit, Contract or Finding aggregates.
...
4. Record unresolved provider/version assumptions in `docs/implementation/github-scm-connector-security-compatibility-tests-and-release-gates-baseline.md`.
...
```text
github_scm_connector_security_compatibility_tests_and_release_gates_manifest.json
github_scm_connector_security_compatibility_tests_and_release_gates_decision.json
github_scm_connector_security_compatibility_tests_and_release_gates_evidence.json
```
...
- Use provider-native stable identifiers plus provider-instance identity.
- Preserve immutable commits and exact workspace manifests.
- Use short-lived credentials and fail closed on TLS, scope or integrity errors.
- Reconcile webhook/polling/provider unknown results before retrying side effects.
