---
name: salesforce-hubspot-dataverse-zendesk-crm-connector-sync-15b4ac40
description: "Implement provider-neutral Salesforce, HubSpot, Dataverse and Zendesk connectors with object mapping, versioned capabilities, webhooks, journals, polling, backfill, tombstones and conflict reconciliation."
---
...
Implement provider-neutral Salesforce, HubSpot, Dataverse and Zendesk connectors with object mapping, versioned capabilities, webhooks, journals, polling, backfill, tombstones and conflict reconciliation.

This Skill follows the approved Batch 40A design from the conversation.
...
Implement the domain model, lifecycle, APIs, events, workflows, policy controls, provider adapters, evidence, analytics, operations and release gates required for **Batch 40A: Customer、CRM与Revenue Operations**.

Theme:

```text
Account／Contact、Lead、Opportunity、Pipeline、Territory、Quota、Forecast、Customer Success、Renewal、Support、Customer 360与RevOps治理
```
...
5. Write unresolved domain assumptions to `docs/implementation/salesforce-hubspot-dataverse-zendesk-crm-connector-sync-and-reconciliation-baseline.md`.

## Required Capabilities

- Implement Account／Contact、Lead、Opportunity、Pipeline、Territory、Quota、Forecast、Customer Success、Renewal、Support、Customer 360与RevOps治理 without weakening earlier identity, evidence, policy, finance, privacy or tenant-isolation controls.
...
```text
identity→consent→lead→opportunity→contract handoff→success→renewal→support→Customer 360
```
...
```text
salesforce_hubspot_dataverse_zendesk_crm_connector_sync_and_reconciliation_manifest.json
salesforce_hubspot_dataverse_zendesk_crm_connector_sync_and_reconciliation_state.json
salesforce_hubspot_dataverse_zendesk_crm_connector_sync_and_reconciliation_decision.json
salesforce_hubspot_dataverse_zendesk_crm_connector_sync_and_reconciliation_reconciliation.json
salesforce_hubspot_dataverse_zendesk_crm_connector_sync_and_reconciliation_evidence.json
```
...
- Account、Legal Entity、Billing Account与User分离
- Consent按Purpose和Channel治理
- Closed Won不等于合同
- Forecast Snapshot不可变
- Customer 360可重建且用途受控
...
```text
customerAccountIdentityIsStable
customerAccountLifecycleIsVersioned
contactIdentityIsStable
contactLifecycleIsVersioned
leadIdentityIsStable
leadLifecycleIsVersioned
opportunityIdentityIsStable
opportunityLifecycleIsVersioned
salesforceHubspotDataverseZendeskCrmConnectorSyncAndReconciliationPreservesSourceLineage
