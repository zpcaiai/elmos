---
name: b41-holdout-feedback-calibration
description: "Implement and verify Batch 41 Skill 1409 for holdout feedback calibration. Use when work requires Holdout、反馈和预测校准, typed evidence, negative and holdout validation, and a fail-closed certification decision."
---
...
1. Inspect the current implementation, runtime facts, policies, incidents, decisions, and prior evidence relevant to `b41-holdout-feedback-calibration`.
...
4. Implement the smallest production-shaped capability that exercises `Holdout、反馈和预测校准` without bypassing existing control planes.
...
- Capability-specific implementation, tests, run logs, evidence references, and residual-risk records for `b41-holdout-feedback-calibration`
