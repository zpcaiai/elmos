...
description: "Implement and verify Batch 41 Skill 1402 for effort duration cost prediction. Use when work requires 工时、周期和成本预测, typed evidence, negative and holdout validation, and a fail-closed certification decision."
---
...
1. Inspect the current implementation, runtime facts, policies, incidents, decisions, and prior evidence relevant to `b41-effort-duration-cost-prediction`.
...
4. Implement the smallest production-shaped capability that exercises `工时、周期和成本预测` without bypassing existing control planes.
...
- Capability-specific implementation, tests, run logs, evidence references, and residual-risk records for `b41-effort-duration-cost-prediction`
