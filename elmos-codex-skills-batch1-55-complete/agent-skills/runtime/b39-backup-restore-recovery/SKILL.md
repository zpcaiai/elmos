---
name: b39-backup-restore-recovery
description: "Implement and verify Batch 39 Skill 1355 for backup restore recovery. Use when work requires Backup、Restore和数据恢复, typed evidence, negative and holdout validation, and a fail-closed certification decision."
---
...
## Skill 1355: Backup、Restore和数据恢复
...
1. Inspect the current implementation, runtime facts, policies, incidents, decisions, and prior evidence relevant to `b39-backup-restore-recovery`.
...
4. Implement the smallest production-shaped capability that exercises `Backup、Restore和数据恢复` without bypassing existing control planes.
...
- Capability-specific implementation, tests, run logs, evidence references, and residual-risk records for `b39-backup-restore-recovery`
