---
name: b05-core-language-translation-type-control-flow-null-se-0a94dd44
description: "Implement security privacy and tenant isolation for Batch 5: Core Language Translation、Type／Control-flow／Null Semantics."
---

# Objective

Enforce least privilege, tenant isolation, secret exclusion, privacy scope and negative authorization.

Apply this Skill to **Batch 5: Core Language Translation、Type／Control-flow／Null Semantics**. Treat the installed Batch manifest and repository evidence as authoritative; this normalized Skill must not invent unsupported provider, regulatory or runtime capabilities.

# Scope

- Batch: `5`
- Domain: `Core Language Translation、Type／Control-flow／Null Semantics`
- Role: `security-privacy-and-tenant-isolation`
...
2. Resolve Batch 5 dependencies and verify prerequisite gates.
3. Capture a no-change baseline and record pre-existing failures.
4. Build the smallest end-to-end vertical slice for `security-privacy-and-tenant-isolation`.
...
- evaluation order, null, numeric and exception semantics are preserved.
- agent-generated code is separately identified.
- unsafe lowering blocks instead of degrading silently.
