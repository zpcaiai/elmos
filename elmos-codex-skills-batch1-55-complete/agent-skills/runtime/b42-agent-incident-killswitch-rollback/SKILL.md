---
name: b42-agent-incident-killswitch-rollback
description: "Implement and verify Batch 42 Skill 1430 for agent incident killswitch rollback. Use when work requires Agent Incident、Kill Switch和Rollback, typed evidence, negative and holdout validation, and a fail-closed certification decision."
---

# Agent Incident、Kill Switch和Rollback
...
## Skill 1430: Agent Incident、Kill Switch和Rollback
...
1. Inspect the current implementation, runtime facts, policies, incidents, decisions, and prior evidence relevant to `b42-agent-incident-killswitch-rollback`.
...
4. Implement the smallest production-shaped capability that exercises `Agent Incident、Kill Switch和Rollback` without bypassing existing control planes.
...
- Capability-specific implementation, tests, run logs, evidence references, and residual-risk records for `b42-agent-incident-killswitch-rollback`
