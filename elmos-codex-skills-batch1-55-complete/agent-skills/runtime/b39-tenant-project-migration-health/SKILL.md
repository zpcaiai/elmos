...
description: "Implement and verify Batch 39 Skill 1351 for tenant project migration health. Use when work requires Tenant、Project和Migration健康模型, typed evidence, negative and holdout validation, and a fail-closed certification decision."
---

# Tenant、Project和Migration健康模型
...
## Skill 1351: Tenant、Project和Migration健康模型
...
1. Inspect the current implementation, runtime facts, policies, incidents, decisions, and prior evidence relevant to `b39-tenant-project-migration-health`.
...
4. Implement the smallest production-shaped capability that exercises `Tenant、Project和Migration健康模型` without bypassing existing control planes.
...
- Capability-specific implementation, tests, run logs, evidence references, and residual-risk records for `b39-tenant-project-migration-health`
