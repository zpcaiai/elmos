---
name: b45-deployment-matrix-certification
description: "Implement and verify Batch 45 Skill 1485 for deployment matrix certification. Use when work requires 企业部署矩阵认证, typed evidence, negative and holdout validation, and a fail-closed certification decision."
---
...
1. Inspect the current implementation, runtime facts, policies, incidents, decisions, and prior evidence relevant to `b45-deployment-matrix-certification`.
...
4. Implement the smallest production-shaped capability that exercises `企业部署矩阵认证` without bypassing existing control planes.
...
- Capability-specific implementation, tests, run logs, evidence references, and residual-risk records for `b45-deployment-matrix-certification`
