---
name: b45-customer-value-certification
description: "Implement and verify Batch 45 Skill 1488 for customer value certification. Use when work requires 客户结果和价值实现认证, typed evidence, negative and holdout validation, and a fail-closed certification decision."
---
...
1. Inspect the current implementation, runtime facts, policies, incidents, decisions, and prior evidence relevant to `b45-customer-value-certification`.
...
4. Implement the smallest production-shaped capability that exercises `客户结果和价值实现认证` without bypassing existing control planes.
...
- Capability-specific implementation, tests, run logs, evidence references, and residual-risk records for `b45-customer-value-certification`
