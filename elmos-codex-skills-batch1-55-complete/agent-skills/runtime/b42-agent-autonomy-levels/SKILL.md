---
name: b42-agent-autonomy-levels
description: "Implement and verify Batch 42 Skill 1432 for agent autonomy levels. Use when work requires Agent自主等级提升和降级, typed evidence, negative and holdout validation, and a fail-closed certification decision."
---
...
1. Inspect the current implementation, runtime facts, policies, incidents, decisions, and prior evidence relevant to `b42-agent-autonomy-levels`.
...
4. Implement the smallest production-shaped capability that exercises `Agent自主等级提升和降级` without bypassing existing control planes.
...
- Capability-specific implementation, tests, run logs, evidence references, and residual-risk records for `b42-agent-autonomy-levels`
