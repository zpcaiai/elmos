---
name: b43-deprecation-removal
description: "Implement and verify Batch 43 Skill 1446 for deprecation removal. Use when work requires Deprecation和移除流程, typed evidence, negative and holdout validation, and a fail-closed certification decision."
---
...
1. Inspect the current implementation, runtime facts, policies, incidents, decisions, and prior evidence relevant to `b43-deprecation-removal`.
...
4. Implement the smallest production-shaped capability that exercises `Deprecation和移除流程` without bypassing existing control planes.
...
- Capability-specific implementation, tests, run logs, evidence references, and residual-risk records for `b43-deprecation-removal`
