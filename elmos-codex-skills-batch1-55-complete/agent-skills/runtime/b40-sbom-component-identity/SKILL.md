...
description: "Implement and verify Batch 40 Skill 1379 for sbom component identity. Use when work requires SBOM生成和组件身份, typed evidence, negative and holdout validation, and a fail-closed certification decision."
---
...
1. Inspect the current implementation, runtime facts, policies, incidents, decisions, and prior evidence relevant to `b40-sbom-component-identity`.
...
4. Implement the smallest production-shaped capability that exercises `SBOM生成和组件身份` without bypassing existing control planes.
...
- Capability-specific implementation, tests, run logs, evidence references, and residual-risk records for `b40-sbom-component-identity`
