---
name: b44-cost-scenario-forecast
description: "Implement and verify Batch 44 Skill 1471 for cost scenario forecast. Use when work requires 成本Scenario和滚动预测, typed evidence, negative and holdout validation, and a fail-closed certification decision."
---
...
1. Inspect the current implementation, runtime facts, policies, incidents, decisions, and prior evidence relevant to `b44-cost-scenario-forecast`.
...
4. Implement the smallest production-shaped capability that exercises `成本Scenario和滚动预测` without bypassing existing control planes.
...
- Capability-specific implementation, tests, run logs, evidence references, and residual-risk records for `b44-cost-scenario-forecast`
