---
name: b40-slsa-provenance
description: "Implement and verify Batch 40 Skill 1383 for slsa provenance. Use when work requires SLSA Provenance和构建证明, typed evidence, negative and holdout validation, and a fail-closed certification decision."
---

# SLSA Provenance和构建证明
...
## Skill 1383: SLSA Provenance和构建证明
...
1. Inspect the current implementation, runtime facts, policies, incidents, decisions, and prior evidence relevant to `b40-slsa-provenance`.
...
4. Implement the smallest production-shaped capability that exercises `SLSA Provenance和构建证明` without bypassing existing control planes.
...
- Capability-specific implementation, tests, run logs, evidence references, and residual-risk records for `b40-slsa-provenance`
