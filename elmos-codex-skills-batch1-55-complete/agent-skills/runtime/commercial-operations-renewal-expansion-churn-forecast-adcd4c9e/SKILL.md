---
name: commercial-operations-renewal-expansion-churn-forecast-adcd4c9e
description: "Implement commercial operations for renewals, expansions, contraction, churn, commitments, forecasts, leakage detection, revenue assurance and action workflows."
---
...
Implement commercial operations for renewals, expansions, contraction, churn, commitments, forecasts, leakage detection, revenue assurance and action workflows.

This Skill belongs to **Batch 39A** and must extend the approved ELMOS financial domain rather than create parallel Tenant, Legal Entity, Contract, Invoice, Payment, Journal, Metric, Artifact, Policy, Case or Audit aggregates.
...
4. Write unresolved accounting, tax, payment-provider, banking, metric or planning assumptions to `docs/implementation/commercial-operations-renewal-expansion-churn-forecast-and-revenue-assurance-baseline.md`.
...
```text
commercial_operations_renewal_expansion_churn_forecast_and_revenue_assurance_manifest.json
commercial_operations_renewal_expansion_churn_forecast_and_revenue_assurance_decision.json
commercial_operations_renewal_expansion_churn_forecast_and_revenue_assurance_reconciliation.json
commercial_operations_renewal_expansion_churn_forecast_and_revenue_assurance_evidence.json
```
...
- Keep usage facts, rated charges, invoices, payments, revenue and journals as separate immutable or versioned records.
- Use exact decimal money and quantity types; never use binary floating point for authoritative values.
- Issued invoices and posted journals are immutable; corrections use legal documents or balanced adjustment entries.
- Revenue recognition follows versioned accounting policies and never equates invoice or cash timing with revenue.
...
```text
invoiceDateDoesNotDriveRevenueAlone
allocationUsesRelativeSsp
postedRevenueIsNotOverwritten
metricDefinitionIsVersioned
revenueLeakageTracesMissingLink
forecastDoesNotModifyContract
publishedForecastIsImmutable
scenarioDoesNotOverwriteForecast
