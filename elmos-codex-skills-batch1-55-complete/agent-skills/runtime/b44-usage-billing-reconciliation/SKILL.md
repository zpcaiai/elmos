---
name: b44-usage-billing-reconciliation
description: "Implement and verify Batch 44 Skill 1473 for usage billing reconciliation. Use when work requires 用量、账单和财务对账, typed evidence, negative and holdout validation, and a fail-closed certification decision."
---
...
1. Inspect the current implementation, runtime facts, policies, incidents, decisions, and prior evidence relevant to `b44-usage-billing-reconciliation`.
...
4. Implement the smallest production-shaped capability that exercises `用量、账单和财务对账` without bypassing existing control planes.
...
- Capability-specific implementation, tests, run logs, evidence references, and residual-risk records for `b44-usage-billing-reconciliation`
