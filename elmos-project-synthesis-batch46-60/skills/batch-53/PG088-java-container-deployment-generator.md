---
id: PG088
name: java-container-deployment-generator
version: 1.0.0
batch: 53
engine: elmos.project-synthesis
schema: elmos.project-synthesis.v1
status: proposed
summary: non-root Java container, Compose and Kubernetes assets with probes, resources and JVM runtime policy
depends_on: ["PG053", "PG062", "PG070", "PG086"]
capabilities:
  - workspace:read
...
# PG088 — Java Container Deployment Generator

## 1. Objective

Implement non-root Java container, Compose and Kubernetes assets with probes, resources and JVM runtime policy as a governed ELMOS Project Synthesis capability.

## 2. Scope

### In scope
- non-root Java container, Compose and Kubernetes assets with probes, resources and JVM runtime policy.
...
- `local_composition_assets`
- `orchestrator_assets`
- `deployment_validation_plan`
...
3. Map stable semantic symbols and generation units to non-root Java container, Compose and Kubernetes assets with probes, resources and JVM runtime policy.
...
6. Generate orchestrator assets, deployment validation plan from acceptance criteria and failure scenarios.
...
- `ROOT_RUNTIME` — emit structured evidence and stop unsafe continuation.
- `SECRET_EMBEDDED_IN_IMAGE` — emit structured evidence and stop unsafe continuation.
...
- Consume certified outputs from PG053, PG062, PG070, PG086.
...
- Reject `root_runtime` without inventing success.
- Reject `secret_embedded_in_image` without inventing success.

## 17. Acceptance Criteria

- The generated result implements only the approved responsibility: non-root Java container, Compose and Kubernetes assets with probes, resources and JVM runtime policy.
...
- The runtime container executes as non-root and contains no secret material.
