---
id: PG084
name: java-security-authz-generator
version: 1.0.0
batch: 53
engine: elmos.project-synthesis
schema: elmos.project-synthesis.v1
status: proposed
summary: Spring Security authentication, tenant context, authorization policy and negative security tests
depends_on: ["PG041", "PG050", "PG062", "PG077"]
capabilities:
  - workspace:read
...
# PG084 — Java Security Authz Generator

## 1. Objective

Implement Spring Security authentication, tenant context, authorization policy and negative security tests as a governed ELMOS Project Synthesis capability.

## 2. Scope

### In scope
- Spring Security authentication, tenant context, authorization policy and negative security tests.
...
- object and tenant scoped access checks.
...
- `threat_model_and_security_control_plan`
- `permission_matrix`
...
- `identity/tenant_adapters`
- `authorization_rules`
- `positive_and_negative_security_tests`
...
3. Map stable semantic symbols and generation units to Spring Security authentication, tenant context, authorization policy and negative security tests.
...
6. Generate authorization rules, positive and negative security tests from acceptance criteria and failure scenarios.
...
- `IMPLICIT_ALLOW` — emit structured evidence and stop unsafe continuation.
- `TENANT_CONTEXT_LOSS` — emit structured evidence and stop unsafe continuation.
...
- Consume certified outputs from PG041, PG050, PG062, PG077.
...
- Reject `implicit_allow` without inventing success.
- Reject `tenant_context_loss` without inventing success.

## 17. Acceptance Criteria

- The generated result implements only the approved responsibility: Spring Security authentication, tenant context, authorization policy and negative security tests.
