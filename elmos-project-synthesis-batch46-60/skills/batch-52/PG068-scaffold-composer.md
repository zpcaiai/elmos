---
id: PG068
name: scaffold-composer
version: 1.0.0
batch: 52
engine: elmos.project-synthesis
schema: elmos.project-synthesis.v1
status: proposed
summary: Composes repositories, applications, modules, directories, baseline files, tests, docs, and deployment roots.
depends_on: ["PG059", "PG064", "PG065", "PG067"]
capabilities:
  - workspace:read
...
# PG068 — Scaffold Composer

## 1. Objective

Create the complete initial project skeleton according to the approved Project Blueprint.
...
- Produce and validate scaffold_plan.
- Produce and validate directory_tree.
- Produce and validate generation_unit_graph.
- Produce and validate scaffold_artifacts.
...
1. Resolve required repository, application, module, test, docs, and deployment templates.
2. Build the directory, project, package, and generation-unit graph.
3. Create baseline build, source, test, documentation, configuration, and deployment locations.
4. Order generation units by dependencies and required tooling.
5. Detect path, project, artifact, ownership, and naming collisions before any write.
...
- MISSING_REQUIRED_TEMPLATE — emit structured diagnostics and stop unsafe continuation.
- GENERATION_CYCLE — emit structured diagnostics and stop unsafe continuation.
- UNSUPPORTED_REPOSITORY_LAYOUT — emit structured diagnostics and stop unsafe continuation.
...
- Valid inputs produce every declared output for `scaffold-composer`.
...
- Reject or escalate `missing_required_template` without inventing success.
- Reject or escalate `generation_cycle` without inventing success.
- Reject or escalate `unsupported_repository_layout` without inventing success.
...
- The scaffold matches the approved repository layout.
- All required application, test, documentation, and deployment roots exist.
- No file write occurs before collision and ownership checks.
