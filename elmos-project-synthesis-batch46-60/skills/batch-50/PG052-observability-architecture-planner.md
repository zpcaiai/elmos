---
id: PG052
name: observability-architecture-planner
version: 1.0.0
batch: 50
engine: elmos.project-synthesis
schema: elmos.project-synthesis.v1
status: proposed
summary: Plans logs, metrics, traces, audits, correlation, dashboards, alerts, SLOs, and diagnostic retention.
depends_on: ["PG027", "PG038", "PG044", "PG045", "PG050"]
capabilities:
  - workspace:read
...
# PG052 — Observability Architecture Planner

## 1. Objective

Make runtime behavior diagnosable before source generation.
...
- Produce and validate observability_plan.
- Produce and validate telemetry_contract.
- Produce and validate sli_slo_catalog.
- Produce and validate diagnostic_playbooks.
...
1. Identify critical user journeys, workflows, dependencies, and failure paths.
2. Define structured log events, metrics, trace spans, baggage, and correlation identifiers.
3. Define SLIs, SLOs, error budgets, burn alerts, dashboards, and ownership.
4. Separate security audit, business audit, and operational telemetry.
5. Define redaction, sampling, retention, cardinality limits, and diagnostic playbooks.
...
- MISSING_CORRELATION — emit structured diagnostics and stop unsafe continuation.
- SENSITIVE_LOG_DATA — emit structured diagnostics and stop unsafe continuation.
- METRIC_CARDINALITY_EXPLOSION — emit structured diagnostics and stop unsafe continuation.
- ALERT_WITHOUT_OWNER — emit structured diagnostics and stop unsafe continuation.
...
- Valid inputs produce every declared output for `observability-architecture-planner`.
...
- Reject or escalate `missing_correlation` without inventing success.
- Reject or escalate `sensitive_log_data` without inventing success.
- Reject or escalate `metric_cardinality_explosion` without inventing success.
- Reject or escalate `alert_without_owner` without inventing success.
...
- Critical journeys have end-to-end traceability.
- Telemetry excludes secrets and prohibited personal data.
- Each SLO has measurable indicators and an alert owner.
