---
id: PG140
name: e2e-journey-test-generator
version: 1.0.0
batch: 58
engine: elmos.project-synthesis
schema: elmos.project-synthesis.v1
status: proposed
summary: Generates end-to-end tests for critical user and system journeys across frontend, backend, identity, data, and messaging.
depends_on: ["PG033", "PG134", "PG135"]
capabilities:
  - workspace:read
...
# PG140 — E2e Journey Test Generator

## 1. Objective

Verify business outcomes through public boundaries rather than internal implementation.
...
- Validate and consume fullstack_wiring.
- Validate and consume deployment_topology.
- Generate and validate e2e_test_sources.
- Generate and validate journey_fixtures.
- Generate and validate environment_requirements.
- Generate and validate e2e_coverage_map.
...
1. Rank critical journeys by value and risk.
2. Generate setup through supported public interfaces.
3. Exercise authentication, navigation, API, data, and async outcomes.
4. Capture trace and diagnostic identifiers.
5. Generate cleanup and retry-safe test behavior.
...
- `INTERNAL_SHORTCUT_USED` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
- `FLAKY_WAIT` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
- `SHARED_USER_COLLISION` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
- `BUSINESS_OUTCOME_NOT_ASSERTED` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
...
- Valid approved inputs produce all declared outputs for `e2e-journey-test-generator`.
...
- Consume compatible certified outputs from PG033, PG134, PG135.
...
- Reject or escalate `internal_shortcut_used` without inventing a successful result.
- Reject or escalate `flaky_wait` without inventing a successful result.
- Reject or escalate `shared_user_collision` without inventing a successful result.
- Reject or escalate `business_outcome_not_asserted` without inventing a successful result.
...
- Critical journeys use public system interfaces.
- Async outcomes use observable conditions rather than fixed sleeps.
- Tests assert business outcomes and authorization boundaries.
