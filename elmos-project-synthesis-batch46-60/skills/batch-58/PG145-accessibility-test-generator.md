...
status: proposed
summary: Generates automated and manual accessibility checks for keyboard, semantics, focus, contrast, forms, charts, and dynamic content.
depends_on: ["PG128", "PG130", "PG132", "PG135"]
capabilities:
  - workspace:read
...
# PG145 — Accessibility Test Generator

## 1. Objective

Verify frontend accessibility against the approved policy and user journeys.
...
- Validate and consume accessibility_policy.
- Validate and consume user_journeys.
- Validate and consume component_mapping.
- Generate and validate accessibility_test_sources.
- Generate and validate manual_checklist.
- Generate and validate violation_mapping.
- Generate and validate accessibility_evidence_plan.
...
- `accessibility_test_sources`
- `manual_checklist`
- `violation_mapping`
- `accessibility_evidence_plan`
...
1. Generate semantic, label, keyboard, focus, landmark, and automated rule checks.
2. Generate dynamic error, modal, notification, and route-change checks.
3. Generate chart alternatives and color-independent assertions.
4. Create manual checks for behavior automation cannot prove.
5. Map failures to components and journeys.
...
- `AUTOMATION_ONLY_CLAIM` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
- `KEYBOARD_TRAP_UNTESTED` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
- `FOCUS_LOSS` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
- `CHART_ALTERNATIVE_MISSING` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
...
- Valid approved inputs produce all declared outputs for `accessibility-test-generator`.
...
- Consume compatible certified outputs from PG128, PG130, PG132, PG135.
...
- Reject or escalate `automation_only_claim` without inventing a successful result.
- Reject or escalate `keyboard_trap_untested` without inventing a successful result.
- Reject or escalate `focus_loss` without inventing a successful result.
- Reject or escalate `chart_alternative_missing` without inventing a successful result.
...
- Automated results are not claimed as complete accessibility certification.
- Critical journeys have keyboard and focus checks.
- Manual verification obligations are explicit.
