---
id: PG142
name: mutation-test-planner
version: 1.0.0
batch: 58
engine: elmos.project-synthesis
schema: elmos.project-synthesis.v1
status: proposed
summary: Plans and executes mutation testing for high-value domain and security logic under cost controls.
depends_on: ["PG026", "PG136"]
capabilities:
  - workspace:read
...
# PG142 — Mutation Test Planner

## 1. Objective

Detect weak assertions and false test confidence without imposing uncontrolled execution cost.
...
- Validate and consume unit_test_sources.
- Validate and consume risk_register.
- Validate and consume semantic_code_model.
- Validate and consume budget_policy.
- Generate and validate mutation_scope.
- Generate and validate mutation_operator_plan.
- Generate and validate survivor_report.
- Generate and validate test_improvement_actions.
...
1. Select high-risk and high-value modules.
2. Choose safe language-specific mutation operators.
3. Define budgets, timeouts, and exclusions.
4. Classify surviving mutants as equivalent, uncovered, or weakly asserted.
5. Generate targeted test-improvement actions.
...
- `MUTATION_BUDGET_EXCEEDED` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
- `EQUIVALENT_MUTANT_MISCLASSIFIED` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
- `GENERATED_CODE_NOISE` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
- `SECURITY_LOGIC_EXCLUDED` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
...
- Valid approved inputs produce all declared outputs for `mutation-test-planner`.
...
- Consume compatible certified outputs from PG026, PG136.
...
- Reject or escalate `mutation_budget_exceeded` without inventing a successful result.
- Reject or escalate `equivalent_mutant_misclassified` without inventing a successful result.
- Reject or escalate `generated_code_noise` without inventing a successful result.
- Reject or escalate `security_logic_excluded` without inventing a successful result.
...
- Critical domain and authorization logic is included.
- Surviving non-equivalent mutants produce actionable gaps.
- Execution respects configured cost and time budgets.
