...
status: proposed
summary: Generates property-based and generative tests for invariants, parsers, transformations, state machines, and contracts.
depends_on: ["PG034", "PG038", "PG039", "PG135"]
capabilities:
  - workspace:read
...
# PG141 — Property Based Test Generator

## 1. Objective

Explore broad input spaces and discover edge cases beyond example-based tests.
...
- Validate and consume state_machines.
- Validate and consume data_contracts.
- Validate and consume language_profile.
- Generate and validate property_test_sources.
- Generate and validate generators_shrinkers.
- Generate and validate property_catalog.
- Generate and validate property_test_evidence.
...
- `property_test_sources`
- `generators_shrinkers`
- `property_catalog`
- `property_test_evidence`
...
1. Identify stable algebraic, ordering, round-trip, invariant, and transition properties.
2. Generate valid and invalid data generators.
3. Generate shrinking and reproducible seed capture.
4. Constrain generation to meaningful domains.
5. Map discovered counterexamples to requirements and diagnostics.
...
- `INVALID_PROPERTY` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
- `UNBOUNDED_GENERATOR` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
- `NONREPRODUCIBLE_COUNTEREXAMPLE` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
- `GENERATOR_MISSES_DOMAIN` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
...
- Valid approved inputs produce all declared outputs for `property-based-test-generator`.
...
- Consume compatible certified outputs from PG034, PG038, PG039, PG135.
...
- Reject or escalate `invalid_property` without inventing a successful result.
- Reject or escalate `unbounded_generator` without inventing a successful result.
- Reject or escalate `nonreproducible_counterexample` without inventing a successful result.
- Reject or escalate `generator_misses_domain` without inventing a successful result.
...
- Every property states the business meaning it protects.
- Failures record reproducible seeds and minimal counterexamples.
- Generators cover meaningful boundary classes.
