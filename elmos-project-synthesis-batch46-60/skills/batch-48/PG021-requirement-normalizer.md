...
status: proposed
summary: Normalizes requirement wording, structure, identifiers, modality, and verification fields.
depends_on: ["PG014", "PG016", "PG017", "PG020"]
capabilities:
  - workspace:read
...
# PG021 — Requirement Normalizer

## 1. Objective

Create consistent, atomic, reviewable requirements without changing user intent.
...
- Validate and process `normalization_policy`.
- Produce and validate `normalized_requirements`.
- Produce and validate `normalization_diff`.
- Produce and validate `quality_warnings`.
...
1. Normalize actors, actions, objects, conditions, and expected outcomes.
2. Replace vague pronouns with resolved terms where evidence supports it.
3. Separate mandatory, optional, prohibited, and informational statements.
4. Preserve original wording and produce a semantic diff.
5. Validate stable identifiers and references.
...
- `MEANING_DRIFT` — emit a structured diagnostic with supporting evidence.
- `UNRESOLVED_PRONOUN` — emit a structured diagnostic with supporting evidence.
- `INVALID_MODALITY` — emit a structured diagnostic with supporting evidence.
- `IDENTIFIER_COLLISION` — emit a structured diagnostic with supporting evidence.
...
- Valid input produces all declared outputs for `requirement-normalizer`.
...
- Consume valid outputs from PG014, PG016, PG017, PG020.
...
- Reject or escalate `meaning_drift` without inventing a successful result.
- Reject or escalate `unresolved_pronoun` without inventing a successful result.
- Reject or escalate `invalid_modality` without inventing a successful result.
- Reject or escalate `identifier_collision` without inventing a successful result.
...
- Normalized text is semantically equivalent to approved source intent.
- Every change is visible in a normalization diff.
- Ambiguous pronouns are not guessed silently.
