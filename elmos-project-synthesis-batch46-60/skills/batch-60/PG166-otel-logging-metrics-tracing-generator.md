---
id: PG166
name: otel-logging-metrics-tracing-generator
version: 1.0.0
batch: 60
engine: elmos.project-synthesis
schema: elmos.project-synthesis.v1
status: proposed
summary: Generates OpenTelemetry instrumentation, structured logging, metrics, trace propagation, exporters, and collection configuration.
depends_on: ["PG052", "PG085", "PG097", "PG109", "PG164"]
capabilities:
  - workspace:read
...
# PG166 — Otel Logging Metrics Tracing Generator

## 1. Objective

Implement the approved observability architecture consistently across language packs and infrastructure.
...
- Validate and consume communication_contracts.
- Validate and consume deployment_topology.
- Generate and validate instrumentation_sources.
- Generate and validate telemetry_configuration.
- Generate and validate collector_assets.
- Generate and validate telemetry_tests.
...
1. Generate service/resource identity and correlation conventions.
2. Generate inbound, outbound, database, broker, task, and domain instrumentation.
3. Generate structured log fields and redaction.
4. Generate metrics with bounded cardinality.
5. Generate collector/exporter configuration and telemetry tests.
...
- `TRACE_CONTEXT_LOST` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
- `SECRET_LOGGED` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
- `METRIC_CARDINALITY_EXPLOSION` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
- `SAMPLING_HIDES_CRITICAL_TRACE` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
...
- Valid approved inputs produce all declared outputs for `otel-logging-metrics-tracing-generator`.
...
- Consume compatible certified outputs from PG052, PG085, PG097, PG109, PG164.
...
- Reject or escalate `trace_context_lost` without inventing a successful result.
- Reject or escalate `secret_logged` without inventing a successful result.
- Reject or escalate `metric_cardinality_explosion` without inventing a successful result.
- Reject or escalate `sampling_hides_critical_trace` without inventing a successful result.
...
- Trace context crosses HTTP, RPC, broker, and worker boundaries.
- Telemetry contains no prohibited secret or personal data.
- Critical health and business signals remain observable under sampling.
