---
id: PG169
name: canary-bluegreen-rollout-generator
version: 1.0.0
batch: 60
engine: elmos.project-synthesis
schema: elmos.project-synthesis.v1
status: proposed
summary: Generates rolling, canary, blue-green, feature-flag, metric-gated promotion, and automated abort procedures.
depends_on: ["PG026", "PG053", "PG167", "PG168"]
capabilities:
  - workspace:read
...
# PG169 — Canary Bluegreen Rollout Generator

## 1. Objective

Deploy releases gradually with observable safety gates and controlled exposure.
...
- Validate and consume sli_slo_definitions.
- Validate and consume risk_register.
- Generate and validate rollout_plan.
- Generate and validate promotion_gates.
- Generate and validate abort_rules.
- Generate and validate rollout_evidence_plan.
...
1. Select rollout strategy by statefulness, compatibility, risk, and platform capability.
2. Define cohorts, traffic steps, duration, and observations.
3. Generate schema and contract compatibility prerequisites.
4. Generate promotion and automated abort thresholds.
5. Generate operator approval and evidence capture.
...
- `ROLLOUT_INCOMPATIBLE_WITH_SCHEMA` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
- `METRIC_GATE_MISSING` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
- `CANARY_NOT_REPRESENTATIVE` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
- `ABORT_NOT_AUTOMATIC` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
...
- Valid approved inputs produce all declared outputs for `canary-bluegreen-rollout-generator`.
...
- Consume compatible certified outputs from PG026, PG053, PG167, PG168.
...
- Reject or escalate `rollout_incompatible_with_schema` without inventing a successful result.
- Reject or escalate `metric_gate_missing` without inventing a successful result.
- Reject or escalate `canary_not_representative` without inventing a successful result.
- Reject or escalate `abort_not_automatic` without inventing a successful result.
...
- Promotion requires passing defined technical and business signals.
- Abort restores safe traffic without data corruption.
- Stateful compatibility is proven before mixed-version operation.
