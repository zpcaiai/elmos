---
id: PG162
name: sast-sca-license-gate
version: 1.0.0
batch: 60
engine: elmos.project-synthesis
schema: elmos.project-synthesis.v1
status: proposed
summary: Generates and executes static analysis, dependency vulnerability, provenance, SBOM, and license gates.
depends_on: ["PG066", "PG076", "PG144", "PG158"]
capabilities:
  - workspace:read
...
# PG162 — Sast Sca License Gate

## 1. Objective

Block unsafe source and supply-chain content before packaging and deployment.
...
- Validate and consume sbom.
- Validate and consume security_policy.
- Validate and consume license_policy.
- Generate and validate security_gate_config.
- Generate and validate sast_results.
- Generate and validate sca_results.
- Generate and validate license_results.
- Generate and validate gate_decision.
...
1. Select approved language-specific scanners.
2. Scan source, configuration, dependencies, containers, and generated artifacts.
3. Normalize severity, exploitability, reachability, confidence, and ownership.
4. Apply license and provenance policy.
5. Require explicit exception for allowed residual findings.
...
- `SCANNER_NOT_RUN` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
- `CRITICAL_VULNERABILITY_IGNORED` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
- `LICENSE_VIOLATION` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
- `FALSE_CLEAN_DUE_TO_EXCLUSION` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
...
- Valid approved inputs produce all declared outputs for `sast-sca-license-gate`.
...
- Consume compatible certified outputs from PG066, PG076, PG144, PG158.
...
- Reject or escalate `scanner_not_run` without inventing a successful result.
- Reject or escalate `critical_vulnerability_ignored` without inventing a successful result.
- Reject or escalate `license_violation` without inventing a successful result.
- Reject or escalate `false_clean_due_to_exclusion` without inventing a successful result.
...
- Critical policy-blocking findings prevent delivery.
- Exclusions are narrow, justified, versioned, and reviewed.
- Results link to SBOM components and source artifacts.
