---
id: PG168
name: release-versioning-changelog-generator
version: 1.0.0
batch: 60
engine: elmos.project-synthesis
schema: elmos.project-synthesis.v1
status: proposed
summary: Generates versions, release notes, changelogs, contract compatibility summaries, migration instructions, and provenance.
depends_on: ["PG124", "PG158", "PG162"]
capabilities:
  - workspace:read
...
# PG168 — Release Versioning Changelog Generator

## 1. Objective

Create an auditable release artifact tied to exact source, Blueprint, build, and evidence.
...
- Validate and consume generation_manifest.
- Validate and consume build_green_evidence.
- Validate and consume contract_compatibility.
- Validate and consume release_policy.
- Generate and validate release_version.
- Generate and validate release_notes.
- Generate and validate changelog.
- Generate and validate release_manifest.
...
1. Determine version change from product and contract changes.
2. Compile user-visible and operator-visible changes.
3. Generate database, configuration, API, and deployment migration instructions.
4. Attach source, SBOM, image, signature, and evidence references.
5. Validate release completeness.
...
- `VERSION_UNDERSTATES_BREAKING_CHANGE` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
- `MIGRATION_NOTE_MISSING` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
- `ARTIFACT_REFERENCE_GAP` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
- `UNAPPROVED_RELEASE` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
...
- Valid approved inputs produce all declared outputs for `release-versioning-changelog-generator`.
...
- Consume compatible certified outputs from PG124, PG158, PG162.
...
- Reject or escalate `version_understates_breaking_change` without inventing a successful result.
- Reject or escalate `migration_note_missing` without inventing a successful result.
- Reject or escalate `artifact_reference_gap` without inventing a successful result.
- Reject or escalate `unapproved_release` without inventing a successful result.
...
- Breaking public changes cannot receive a patch version.
- Release notes identify required operator actions.
- Release manifest resolves immutable artifacts and evidence.
