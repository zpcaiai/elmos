---
id: PG161
name: secret-config-policy-generator
version: 1.0.0
batch: 60
engine: elmos.project-synthesis
schema: elmos.project-synthesis.v1
status: proposed
summary: Generates configuration validation, secret references, rotation, environment separation, and leak-prevention policy.
depends_on: ["PG050", "PG062"]
capabilities:
  - workspace:read
...
# PG161 — Secret Config Policy Generator

## 1. Objective

Keep credentials and environment-sensitive values outside source, images, logs, and generated examples.
...
- Validate and consume configuration_catalog.
- Validate and consume deployment_topology.
- Validate and consume secret_store_profile.
- Validate and consume security_policy.
- Generate and validate secret_reference_contract.
- Generate and validate config_validation.
- Generate and validate rotation_plan.
- Generate and validate secret_scan_rules.
...
1. Classify configuration and secret values.
2. Generate external secret references by environment.
3. Generate startup validation and safe example files.
4. Generate rotation and revocation hooks.
5. Generate repository, build, image, and log secret scans.
...
- `SECRET_MATERIALIZED` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
- `UNSAFE_EXAMPLE_VALUE` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
- `ROTATION_UNSUPPORTED` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
- `ENVIRONMENT_SECRET_COLLISION` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
...
- Valid approved inputs produce all declared outputs for `secret-config-policy-generator`.
...
- Consume compatible certified outputs from PG050, PG062.
...
- Reject or escalate `secret_materialized` without inventing a successful result.
- Reject or escalate `unsafe_example_value` without inventing a successful result.
- Reject or escalate `rotation_unsupported` without inventing a successful result.
- Reject or escalate `environment_secret_collision` without inventing a successful result.
...
- No real secret value appears in generated artifacts.
- Required secret references fail fast when absent.
