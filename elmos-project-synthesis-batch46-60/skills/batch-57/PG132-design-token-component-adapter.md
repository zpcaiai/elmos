---
id: PG132
name: design-token-component-adapter
version: 1.0.0
batch: 57
engine: elmos.project-synthesis
schema: elmos.project-synthesis.v1
status: proposed
summary: Maps generated UI to approved design tokens and component-library primitives.
depends_on: ["PG063", "PG125"]
capabilities:
  - workspace:read
...
# PG132 — Design Token Component Adapter

## 1. Objective

Keep generated interfaces consistent, themeable, accessible, and independent of arbitrary inline styling.
...
- Validate and consume design_token_contract.
- Validate and consume component_library.
- Validate and consume accessibility_policy.
- Generate and validate theme_adapter.
- Generate and validate component_mapping.
- Generate and validate design_token_usage_report.
- Generate and validate component_tests.
...
1. Import approved tokens and component metadata.
2. Map semantic UI roles to approved components.
3. Generate responsive and theme variants.
4. Detect unsupported component needs and local one-off styling.
5. Generate token and accessibility checks.
...
- `HARDCODED_STYLE_SPRAWL` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
- `TOKEN_MISSING` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
- `COMPONENT_SEMANTIC_MISMATCH` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
- `CONTRAST_VIOLATION` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
...
- Valid approved inputs produce all declared outputs for `design-token-component-adapter`.
...
- Consume compatible certified outputs from PG063, PG125.
...
- Reject or escalate `hardcoded_style_sprawl` without inventing a successful result.
- Reject or escalate `token_missing` without inventing a successful result.
- Reject or escalate `component_semantic_mismatch` without inventing a successful result.
- Reject or escalate `contrast_violation` without inventing a successful result.
...
- Generated UI uses approved tokens for supported properties.
- Interactive elements use accessible component primitives.
- Unsupported design needs are explicit extension points.
