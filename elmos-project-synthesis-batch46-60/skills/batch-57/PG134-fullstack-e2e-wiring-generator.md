---
id: PG134
name: fullstack-e2e-wiring-generator
version: 1.0.0
batch: 57
engine: elmos.project-synthesis
schema: elmos.project-synthesis.v1
status: proposed
summary: Wires frontend, generated SDK, backend, identity, database, messaging, configuration, local composition, and end-to-end journeys.
depends_on: ["PG076", "PG088", "PG100", "PG112", "PG124", "PG125", "PG126", "PG127", "PG133"]
capabilities:
  - workspace:read
...
# PG134 — Fullstack E2e Wiring Generator

## 1. Objective

Produce an integrated full-stack repository slice that can be exercised as one system.
...
- Validate and consume frontend_artifacts.
- Validate and consume backend_generation_manifest.
- Validate and consume integration_contracts.
- Validate and consume deployment_topology.
- Validate and consume acceptance_criteria.
- Generate and validate fullstack_wiring.
- Generate and validate local_compose.
- Generate and validate e2e_environment.
- Generate and validate fullstack_smoke_specs.
...
1. Resolve application URLs, API base paths, identity callbacks, CORS, and environment settings.
2. Generate local service composition and dependency readiness.
3. Generate frontend-to-backend contract verification.
4. Generate critical authenticated end-to-end journeys.
5. Publish integrated artifact and evidence lineage.
...
- `CORS_CONTRACT_MISMATCH` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
- `IDENTITY_CALLBACK_MISMATCH` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
- `SERVICE_READINESS_RACE` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
- `FRONTEND_BACKEND_VERSION_DRIFT` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
...
- Valid approved inputs produce all declared outputs for `fullstack-e2e-wiring-generator`.
...
- Consume compatible certified outputs from PG076, PG088, PG100, PG112, PG124, PG125, PG126, PG127, PG133.
...
- Reject or escalate `cors_contract_mismatch` without inventing a successful result.
- Reject or escalate `identity_callback_mismatch` without inventing a successful result.
- Reject or escalate `service_readiness_race` without inventing a successful result.
- Reject or escalate `frontend_backend_version_drift` without inventing a successful result.
...
- A clean local environment can start the defined full stack.
- Critical user journeys traverse real frontend and backend boundaries.
- Frontend and backend contract versions are compatible.
