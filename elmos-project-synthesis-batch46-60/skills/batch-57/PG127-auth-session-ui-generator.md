---
id: PG127
name: auth-session-ui-generator
version: 1.0.0
batch: 57
engine: elmos.project-synthesis
schema: elmos.project-synthesis.v1
status: proposed
summary: Generates sign-in, sign-out, session, token refresh, tenant selection, and authorization-aware UI flows.
depends_on: ["PG041", "PG050", "PG096", "PG108", "PG125"]
capabilities:
  - workspace:read
...
# PG127 — Auth Session Ui Generator

## 1. Objective

Implement approved identity journeys while keeping server-side authorization authoritative.
...
- Validate and consume identity_contracts.
- Generate and validate auth_ui_components.
- Generate and validate session_runtime.
- Generate and validate route_guards.
- Generate and validate auth_ui_tests.
...
1. Select redirect, popup, cookie-session, or token-mediated interaction from approved identity architecture.
2. Generate login, logout, callback, expiry, refresh, and access-denied states.
3. Propagate tenant and organization context safely.
4. Generate route visibility and capability checks without treating them as security enforcement.
5. Generate authentication journey tests.
...
- `TOKEN_STORAGE_UNSAFE` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
- `AUTHORIZATION_ONLY_IN_UI` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
- `TENANT_CONTEXT_SPOOF` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
- `REFRESH_LOOP` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
...
- Valid approved inputs produce all declared outputs for `auth-session-ui-generator`.
...
- Consume compatible certified outputs from PG041, PG050, PG096, PG108, PG125.
...
- Reject or escalate `token_storage_unsafe` without inventing a successful result.
- Reject or escalate `authorization_only_in_ui` without inventing a successful result.
- Reject or escalate `tenant_context_spoof` without inventing a successful result.
- Reject or escalate `refresh_loop` without inventing a successful result.
...
- Sensitive tokens are not persisted in prohibited browser storage.
- Server authorization remains mandatory.
- Expired and revoked sessions fail safely and visibly.
