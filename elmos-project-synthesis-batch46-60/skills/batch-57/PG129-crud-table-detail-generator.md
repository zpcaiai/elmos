---
id: PG129
name: crud-table-detail-generator
version: 1.0.0
batch: 57
engine: elmos.project-synthesis
schema: elmos.project-synthesis.v1
status: proposed
summary: Generates list, filter, sort, page, create, view, edit, and delete interfaces from approved resource contracts.
depends_on: ["PG033", "PG040", "PG041", "PG125", "PG126"]
capabilities:
  - workspace:read
...
# PG129 — Crud Table Detail Generator

## 1. Objective

Create standard business screens without reducing domain workflows to unsafe generic CRUD.
...
- Validate and consume api_contracts.
- Validate and consume user_journeys.
- Validate and consume permission_matrix.
- Validate and consume frontend_profile.
- Generate and validate list_pages.
- Generate and validate detail_pages.
- Generate and validate resource_actions.
- Generate and validate crud_ui_tests.
...
1. Identify resources whose approved behavior fits CRUD interaction.
2. Generate tables, filters, sorting, pagination, empty, loading, and error states.
3. Generate detail and mutation actions with permissions and confirmations.
4. Exclude actions requiring richer domain workflow.
5. Generate API-backed UI tests.
...
- `GENERIC_CRUD_OVERREACH` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
- `HIDDEN_DESTRUCTIVE_ACTION` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
- `PERMISSION_VISIBILITY_DRIFT` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
- `PAGINATION_CONTRACT_GAP` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
...
- Valid approved inputs produce all declared outputs for `crud-table-detail-generator`.
...
- Consume compatible certified outputs from PG033, PG040, PG041, PG125, PG126.
...
- Reject or escalate `generic_crud_overreach` without inventing a successful result.
- Reject or escalate `hidden_destructive_action` without inventing a successful result.
- Reject or escalate `permission_visibility_drift` without inventing a successful result.
- Reject or escalate `pagination_contract_gap` without inventing a successful result.
...
- Only approved CRUD-like behavior is generated.
- Destructive operations require explicit confirmation and server authorization.
- Filtering and pagination match server contract semantics.
