---
id: PG126
name: typescript-client-sdk-generator
version: 1.0.0
batch: 57
engine: elmos.project-synthesis
schema: elmos.project-synthesis.v1
status: proposed
summary: Generates typed TypeScript clients from approved REST, RPC, event, or webhook contracts.
depends_on: ["PG115", "PG116", "PG124", "PG125"]
capabilities:
  - workspace:read
...
# PG126 — Typescript Client Sdk Generator

## 1. Objective

Provide stable, traceable frontend and Node.js clients without duplicating server contract definitions.
...
- Validate and consume compatibility_policy.
- Generate and validate typescript_sdk.
- Generate and validate generated_types.
- Generate and validate client_runtime.
- Generate and validate sdk_contract_tests.
...
1. Import authoritative contract versions.
2. Generate request, response, error, pagination, auth, timeout, and cancellation types.
3. Generate runtime adapters and serialization rules.
4. Generate compatibility and consumer contract tests.
5. Publish SDK package metadata and provenance.
...
- `CONTRACT_DRIFT` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
- `UNSAFE_ANY_TYPE` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
- `AUTH_HEADER_GAP` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
- `SDK_VERSION_COLLISION` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
...
- Valid approved inputs produce all declared outputs for `typescript-client-sdk-generator`.
...
- Consume compatible certified outputs from PG115, PG116, PG124, PG125.
...
- Reject or escalate `contract_drift` without inventing a successful result.
- Reject or escalate `unsafe_any_type` without inventing a successful result.
- Reject or escalate `auth_header_gap` without inventing a successful result.
- Reject or escalate `sdk_version_collision` without inventing a successful result.
...
- Generated types contain no unapproved any escapes.
- SDK operations map one-to-one to registered contracts.
- Breaking contract changes produce a new compatible SDK version or block generation.
