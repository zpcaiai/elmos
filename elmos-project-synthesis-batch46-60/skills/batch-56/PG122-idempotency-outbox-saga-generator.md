---
id: PG122
name: idempotency-outbox-saga-generator
version: 1.0.0
batch: 56
engine: elmos.project-synthesis
schema: elmos.project-synthesis.v1
status: proposed
summary: idempotency keys, inbox/outbox, transactional publication, sagas, compensation and reconciliation
depends_on: ["PG038", "PG047", "PG048", "PG117"]
capabilities:
  - workspace:read
...
# PG122 — Idempotency Outbox Saga Generator

## 1. Objective

Implement idempotency keys, inbox/outbox, transactional publication, sagas, compensation and reconciliation as a governed ELMOS Project Synthesis capability.

## 2. Scope

### In scope
- idempotency keys, inbox/outbox, transactional publication, sagas, compensation and reconciliation.
...
- idempotency and bounded failure behavior.
- target-compatible infrastructure tests.
- duplicate delivery safety.
- ordering, retry and dead-letter semantics.
...
- `canonical_data_and_integration_contracts`
- `selected_infrastructure_profiles`
...
- `inbox/outbox_components`
- `saga/compensation_components`
- `crash_and_replay_tests`
...
2. Resolve the supported Database, API and External Integration capability profile and reject unsupported or unapproved behavior.
3. Map stable semantic symbols and generation units to idempotency keys, inbox/outbox, transactional publication, sagas, compensation and reconciliation.
...
6. Generate saga/compensation components, crash and replay tests from acceptance criteria and failure scenarios.
...
- `DUPLICATE_SIDE_EFFECT` — emit structured evidence and stop unsafe continuation.
- `UNBOUNDED_RETRY_OR_STUCK_WORKFLOW` — emit structured evidence and stop unsafe continuation.
...
- Inbox/outbox components.
- Saga/compensation components.
...
- Consume certified outputs from PG038, PG047, PG048, PG117.
...
- Reject `duplicate_side_effect` without inventing success.
- Reject `unbounded_retry_or_stuck_workflow` without inventing success.

## 17. Acceptance Criteria

- The generated result implements only the approved responsibility: idempotency keys, inbox/outbox, transactional publication, sagas, compensation and reconciliation.
...
- Compatibility, timeout, retry, security and failure semantics are explicit and testable.
- At-least-once delivery or repeated requests cannot duplicate protected side effects.
