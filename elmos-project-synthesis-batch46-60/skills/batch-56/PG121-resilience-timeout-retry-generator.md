---
id: PG121
name: resilience-timeout-retry-generator
version: 1.0.0
batch: 56
engine: elmos.project-synthesis
schema: elmos.project-synthesis.v1
status: proposed
summary: cross-language timeout, retry, backoff, circuit, bulkhead, rate-limit and semantically safe fallback policies
depends_on: ["PG048", "PG049", "PG077", "PG089", "PG101"]
capabilities:
  - workspace:read
...
# PG121 — Resilience Timeout Retry Generator

## 1. Objective

Implement cross-language timeout, retry, backoff, circuit, bulkhead, rate-limit and semantically safe fallback policies as a governed ELMOS Project Synthesis capability.

## 2. Scope

### In scope
- cross-language timeout, retry, backoff, circuit, bulkhead, rate-limit and semantically safe fallback policies.
...
- `resilience_policy_model`
- `language-specific_policy_sources`
- `failure_tests`
...
3. Map stable semantic symbols and generation units to cross-language timeout, retry, backoff, circuit, bulkhead, rate-limit and semantically safe fallback policies.
...
6. Generate failure tests, observability evidence from acceptance criteria and failure scenarios.
...
- Language-specific policy sources.
- Failure tests.
...
- Consume certified outputs from PG048, PG049, PG077, PG089, PG101.
...
- The generated result implements only the approved responsibility: cross-language timeout, retry, backoff, circuit, bulkhead, rate-limit and semantically safe fallback policies.
