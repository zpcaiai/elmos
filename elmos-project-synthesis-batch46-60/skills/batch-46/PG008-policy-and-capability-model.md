---
id: PG008
name: policy-and-capability-model
version: 1.0.0
batch: 46
engine: elmos.project-synthesis
schema: elmos.project-synthesis.v1
status: proposed
summary: Defines policy evaluation and capability grants for agents, skills, tools, runners, repositories, networks, and secrets.
depends_on: ["PG001", "PG006"]
capabilities:
  - workspace:read
...
# PG008 — Policy And Capability Model

## 1. Objective

Apply least privilege to every synthesis action.
...
- Validate and process `skill_manifest`.
- Validate and process `tool_registry`.
- Validate and process `runner_profile`.
- Produce and validate `capability_model`.
- Produce and validate `policy_decision_contract`.
- Produce and validate `deny_reasons`.
- Produce and validate `approval_escalation_rules`.
...
1. Model capabilities for file, process, network, repository, secret, deployment, and approval operations.
2. Resolve platform, tenant, workspace, and run-level policy precedence.
3. Issue short-lived capability grants.
4. Deny undeclared tool use and record structured reasons.
5. Define approval escalation for high-impact capabilities.
...
- `CAPABILITY_ESCALATION` — emit a structured diagnostic with supporting evidence.
- `POLICY_SHADOWING` — emit a structured diagnostic with supporting evidence.
- `STALE_GRANT` — emit a structured diagnostic with supporting evidence.
- `UNDECLARED_TOOL_USE` — emit a structured diagnostic with supporting evidence.
...
- Valid input produces all declared outputs for `policy-and-capability-model`.
...
- Consume valid outputs from PG001, PG006.
...
- Reject or escalate `capability_escalation` without inventing a successful result.
- Reject or escalate `policy_shadowing` without inventing a successful result.
- Reject or escalate `stale_grant` without inventing a successful result.
- Reject or escalate `undeclared_tool_use` without inventing a successful result.
...
- A skill cannot invoke a tool absent from its declared capabilities.
- High-impact actions require explicit policy authorization.
- Every denial is machine-readable and auditable.
