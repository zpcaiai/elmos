---
id: PG006
name: synthesis-scope-and-engine-extension
version: 1.0.0
batch: 46
engine: elmos.project-synthesis
schema: elmos.project-synthesis.v1
status: proposed
summary: Extends ELMOS scope resolution so new-project synthesis and legacy modernization share platform objects without mixing semantics.
...
# PG006 — Synthesis Scope And Engine Extension

## 1. Objective

Register Project Synthesis as a first-class engine while preserving repository, tenant, policy, and delivery interoperability.
...
- Validate and process `elmos_scope_model`.
- Validate and process `organization_context`.
- Validate and process `workspace_context`.
- Validate and process `repository_context`.
- Produce and validate `scope_extension`.
- Produce and validate `engine_registration`.
- Produce and validate `interop_mapping`.
- Produce and validate `scope_resolution_rules`.
...
1. Register the project-synthesis engine and its supported scopes.
2. Map organization, workspace, repository, system, and delivery objects.
3. Define when a generated project becomes eligible for modernization workflows.
4. Prevent accidental reuse of legacy migration semantics inside greenfield synthesis.
5. Publish scope compatibility tests.
...
- `SCOPE_LEAKAGE` — emit a structured diagnostic with supporting evidence.
- `ENGINE_MISROUTING` — emit a structured diagnostic with supporting evidence.
- `TENANT_BOUNDARY_VIOLATION` — emit a structured diagnostic with supporting evidence.
- `REPOSITORY_IDENTITY_COLLISION` — emit a structured diagnostic with supporting evidence.
...
- Valid input produces all declared outputs for `synthesis-scope-and-engine-extension`.
...
- Reject or escalate `scope_leakage` without inventing a successful result.
- Reject or escalate `engine_misrouting` without inventing a successful result.
- Reject or escalate `tenant_boundary_violation` without inventing a successful result.
- Reject or escalate `repository_identity_collision` without inventing a successful result.
...
- A generated repository can later enter modernization through an explicit handoff.
- Tenant and workspace boundaries remain intact across engines.
- Engine routing is deterministic for every supported scope.
