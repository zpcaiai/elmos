---
id: PG001
name: project-synthesis-product-constitution
version: 1.0.0
batch: 46
engine: elmos.project-synthesis
schema: elmos.project-synthesis.v1
status: proposed
summary: Defines the product-level constitution, invariants, boundaries, and non-negotiable safety rules for ELMOS Project Synthesis.
depends_on: []
capabilities:
  - workspace:read
...
# PG001 — Project Synthesis Product Constitution

## 1. Objective

Establish a single authoritative policy for what the Project Synthesis Engine may generate, how it proves correctness, and when it must stop or request approval.
...
- Validate and process `product_strategy`.
- Validate and process `enterprise_policy`.
- Validate and process `supported_project_profiles`.
- Validate and process `existing_elmos_contracts`.
- Produce and validate `product_constitution`.
- Produce and validate `engine_invariants`.
- Produce and validate `prohibited_behaviors`.
- Produce and validate `approval_matrix`.
...
1. Load applicable ELMOS platform policies and existing modernization contracts.
2. Resolve conflicts between product goals, tenant policy, security policy, and delivery guarantees.
3. Declare mandatory invariants for traceability, reproducibility, evidence, and human approval.
4. Define prohibited shortcuts such as deleting tests, weakening assertions, hiding failures, or embedding secrets.
5. Publish a versioned constitution and compatibility declaration.
...
- `POLICY_CONFLICT` — emit a structured diagnostic with supporting evidence.
- `MISSING_PLATFORM_CONTRACT` — emit a structured diagnostic with supporting evidence.
- `UNSUPPORTED_DELIVERY_CLAIM` — emit a structured diagnostic with supporting evidence.
- `UNRESOLVED_TENANT_OVERRIDE` — emit a structured diagnostic with supporting evidence.
...
- Valid input produces all declared outputs for `project-synthesis-product-constitution`.
...
- Consume valid outputs from the Project Synthesis platform foundation.
...
- Reject or escalate `policy_conflict` without inventing a successful result.
- Reject or escalate `missing_platform_contract` without inventing a successful result.
- Reject or escalate `unsupported_delivery_claim` without inventing a successful result.
- Reject or escalate `unresolved_tenant_override` without inventing a successful result.
...
- Every generated project claim maps to an explicit invariant.
- No downstream skill can override a constitutional prohibition.
- The constitution is versioned and included in every synthesis run evidence bundle.
