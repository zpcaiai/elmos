...
status: proposed
summary: Classifies assertion, fixture, environment, contract, timing, isolation, product, and flaky test failures.
depends_on: ["PG135", "PG147"]
capabilities:
  - workspace:read
...
# PG150 — Test Failure Classifier

## 1. Objective

Determine whether failure belongs to implementation, test, environment, specification, or infrastructure.
...
- Validate and consume test_execution.
- Validate and consume test_intent_graph.
- Validate and consume traceability_graph.
- Validate and consume environment_fingerprint.
- Generate and validate test_failure_groups.
- Generate and validate failure_ownership.
- Generate and validate flake_probability.
...
1. Parse test reports and logs.
2. Map each failure to requirement, criterion, test, and generated artifacts.
3. Classify product defect, test defect, environment issue, contract drift, or flake.
4. Detect common cascading setup failures.
5. Prevent changing tests merely because implementation failed.
...
- `PRODUCT_DEFECT_CALLED_TEST_DEFECT` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
- `ASSERTION_WEAKENED` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
- `ENVIRONMENT_FAILURE_HIDDEN` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
- `TRACE_LINK_MISSING` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
...
- Valid approved inputs produce all declared outputs for `test-failure-classifier`.
...
- Consume compatible certified outputs from PG135, PG147.
...
- Reject or escalate `product_defect_called_test_defect` without inventing a successful result.
- Reject or escalate `assertion_weakened` without inventing a successful result.
- Reject or escalate `environment_failure_hidden` without inventing a successful result.
- Reject or escalate `trace_link_missing` without inventing a successful result.
...
- Acceptance-derived assertions cannot be weakened automatically.
- Product and test defects are distinguished with evidence.
- Environment failures do not count as passing tests.
