---
id: PG149
name: compile-error-classifier
version: 1.0.0
batch: 59
engine: elmos.project-synthesis
schema: elmos.project-synthesis.v1
status: proposed
summary: Classifies Java, Python type/import, C# compiler, generated-source, and configuration compile-time failures.
depends_on: ["PG069", "PG076", "PG147"]
capabilities:
  - workspace:read
...
# PG149 — Compile Error Classifier

## 1. Objective

Convert noisy compiler output into source-grounded, actionable failure groups.
...
- Validate and consume symbol_location_map.
- Validate and consume generation_manifest.
- Validate and consume language_profile.
- Generate and validate compile_failure_groups.
- Generate and validate root_cause_candidates.
- Generate and validate affected_symbols.
...
1. Parse language and build-tool diagnostics.
2. Normalize file, line, symbol, code, and severity.
3. Group cascading errors under likely root causes.
4. Map failures to semantic symbols and Blueprint sources.
5. Identify generated, protected, or user-owned repair boundaries.
...
- `COMPILER_OUTPUT_PARSE_FAILURE` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
- `CASCADE_MISGROUPED` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
- `USER_OWNED_ERROR_TARGETED` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
- `SYMBOL_MAPPING_GAP` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
...
- Valid approved inputs produce all declared outputs for `compile-error-classifier`.
...
- Consume compatible certified outputs from PG069, PG076, PG147.
...
- Reject or escalate `compiler_output_parse_failure` without inventing a successful result.
- Reject or escalate `cascade_misgrouped` without inventing a successful result.
- Reject or escalate `user_owned_error_targeted` without inventing a successful result.
- Reject or escalate `symbol_mapping_gap` without inventing a successful result.
...
- Diagnostics map to concrete source and semantic identifiers where available.
- User-owned failures are not automatically modified.
- Likely root causes are separated from cascades.
