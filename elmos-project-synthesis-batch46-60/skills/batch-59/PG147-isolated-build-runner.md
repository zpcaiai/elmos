---
id: PG147
name: isolated-build-runner
version: 1.0.0
batch: 59
engine: elmos.project-synthesis
schema: elmos.project-synthesis.v1
status: proposed
summary: Executes generated projects in clean, policy-controlled build environments.
depends_on: ["PG009", "PG076"]
capabilities:
  - workspace:read
  - sandbox:create
  - sandbox:execute
...
# PG147 — Isolated Build Runner

## 1. Objective

Produce trustworthy build evidence independent of developer machines and hidden local state.
...
- Validate and consume runner_profile.
- Validate and consume capability_grant.
- Generate and validate build_execution.
- Generate and validate build_artifacts.
- Generate and validate runner_fingerprint.
- Generate and validate build_evidence.
...
1. Provision a clean isolated workspace and approved toolchain.
2. Materialize only declared source and dependency inputs.
3. Run generated wrapper or build commands with network and resource policy.
4. Capture stdout, stderr, exit codes, timing, artifacts, and environment fingerprint.
5. Destroy or quarantine the workspace according to evidence policy.
...
- `RUNNER_PROVISION_FAILURE` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
- `UNDECLARED_NETWORK_ACCESS` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
- `HIDDEN_LOCAL_DEPENDENCY` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
- `RESOURCE_LIMIT_EXCEEDED` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
...
- Valid approved inputs produce all declared outputs for `isolated-build-runner`.
...
- Consume compatible certified outputs from PG009, PG076.
...
- Reject or escalate `runner_provision_failure` without inventing a successful result.
- Reject or escalate `undeclared_network_access` without inventing a successful result.
- Reject or escalate `hidden_local_dependency` without inventing a successful result.
- Reject or escalate `resource_limit_exceeded` without inventing a successful result.
...
- Builds run without developer-machine state.
- All toolchain and environment facts are recorded.
- Undeclared network or secret access is denied and evidenced.
