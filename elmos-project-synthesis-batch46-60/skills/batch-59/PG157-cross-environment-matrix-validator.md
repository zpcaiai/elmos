---
id: PG157
name: cross-environment-matrix-validator
version: 1.0.0
batch: 59
engine: elmos.project-synthesis
schema: elmos.project-synthesis.v1
status: proposed
summary: Validates generated projects across supported runtime, OS, database, browser, architecture, and deployment matrices.
depends_on: ["PG057", "PG076", "PG147"]
capabilities:
  - workspace:read
...
# PG157 — Cross Environment Matrix Validator

## 1. Objective

Detect environment-specific defects and unsupported combinations before delivery.
...
- Validate and consume supported_environment_matrix.
- Validate and consume generation_manifest.
- Validate and consume execution_specs.
- Validate and consume budget_policy.
- Generate and validate matrix_results.
- Generate and validate compatibility_report.
- Generate and validate unsupported_combinations.
- Generate and validate environment_evidence.
...
1. Resolve mandatory and optional matrix cells.
2. Provision clean environments for each selected cell.
3. Run build, tests, startup, and smoke checks.
4. Compare behavior and artifacts across environments.
5. Publish supported and explicitly unsupported combinations.
...
- `MATRIX_CELL_SKIPPED_SILENTLY` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
- `ENVIRONMENT_DRIFT` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
- `PLATFORM_SPECIFIC_FAILURE_HIDDEN` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
- `BUDGET_OVERRUN` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
...
- Valid approved inputs produce all declared outputs for `cross-environment-matrix-validator`.
...
- Consume compatible certified outputs from PG057, PG076, PG147.
...
- Reject or escalate `matrix_cell_skipped_silently` without inventing a successful result.
- Reject or escalate `environment_drift` without inventing a successful result.
- Reject or escalate `platform_specific_failure_hidden` without inventing a successful result.
- Reject or escalate `budget_overrun` without inventing a successful result.
...
- Every mandatory matrix cell has a result.
- Unsupported combinations are explicit.
- Environment differences are recorded in evidence.
