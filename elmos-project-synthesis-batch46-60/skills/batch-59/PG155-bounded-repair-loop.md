...
status: proposed
summary: Applies, verifies, limits, and records automated repair attempts.
depends_on: ["PG073", "PG074", "PG154"]
capabilities:
  - workspace:read
...
# PG155 — Bounded Repair Loop

## 1. Objective

Achieve Build Green only through policy-compliant, minimal, evidence-backed repairs.
...
- Validate and consume repair_plan.
- Validate and consume repair_policy.
- Validate and consume repository_state.
- Validate and consume execution_specs.
- Generate and validate repair_attempts.
- Generate and validate patched_repository.
- Generate and validate verification_results.
- Generate and validate repair_evidence.
...
1. Check attempt, file-count, change-ratio, time, and cost limits.
2. Apply one bounded repair set through ownership-aware merge.
3. Re-run the smallest proving checks, then required regression checks.
4. Stop on success, repeated root cause, limit exhaustion, or approval boundary.
5. Record every diff, result, and rollback.
...
- `REPAIR_LIMIT_EXCEEDED` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
- `LOOP_WITHOUT_PROGRESS` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
- `TEST_DELETED` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
- `SECURITY_CONTROL_WEAKENED` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
...
- Valid approved inputs produce all declared outputs for `bounded-repair-loop`.
...
- Consume compatible certified outputs from PG073, PG074, PG154.
...
- Reject or escalate `repair_limit_exceeded` without inventing a successful result.
- Reject or escalate `loop_without_progress` without inventing a successful result.
- Reject or escalate `test_deleted` without inventing a successful result.
- Reject or escalate `security_control_weakened` without inventing a successful result.
...
- The loop has hard attempt and change limits.
- No repair may delete or disable required tests.
- Successful repair passes all impacted regression gates.
