---
id: PG148
name: dependency-resolution-diagnoser
version: 1.0.0
batch: 59
engine: elmos.project-synthesis
schema: elmos.project-synthesis.v1
status: proposed
summary: Diagnoses package, repository, lockfile, version, platform, and provenance resolution failures.
depends_on: ["PG058", "PG098", "PG110", "PG147"]
capabilities:
  - workspace:read
...
# PG148 — Dependency Resolution Diagnoser

## 1. Objective

Identify dependency failures without replacing approved dependencies with untrusted or incompatible alternatives.
...
- Validate and consume dependency_catalog.
- Validate and consume lockfiles.
- Validate and consume registry_policy.
- Generate and validate dependency_diagnosis.
- Generate and validate resolution_candidates.
- Generate and validate policy_findings.
- Generate and validate repair_constraints.
...
1. Parse resolver and package-manager diagnostics.
2. Classify missing, conflicting, unavailable, incompatible, vulnerable, untrusted, or authentication-related failures.
3. Compare against Blueprint and lock policy.
4. Propose bounded approved corrections.
5. Block silent repository or version substitution.
...
- `DEPENDENCY_CONFLICT_MISDIAGNOSED` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
- `UNTRUSTED_REGISTRY_FALLBACK` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
- `LOCKFILE_BYPASS` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
- `PLATFORM_MARKER_IGNORED` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
...
- Valid approved inputs produce all declared outputs for `dependency-resolution-diagnoser`.
...
- Consume compatible certified outputs from PG058, PG098, PG110, PG147.
...
- Reject or escalate `dependency_conflict_misdiagnosed` without inventing a successful result.
- Reject or escalate `untrusted_registry_fallback` without inventing a successful result.
- Reject or escalate `lockfile_bypass` without inventing a successful result.
- Reject or escalate `platform_marker_ignored` without inventing a successful result.
...
- Every proposed correction remains within approved policy or requests approval.
- Lockfiles are not silently discarded.
- Registry trust and provenance remain enforced.
