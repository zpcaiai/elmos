---
id: PG151
name: runtime-log-trace-diagnoser
version: 1.0.0
batch: 59
engine: elmos.project-synthesis
schema: elmos.project-synthesis.v1
status: proposed
summary: Analyzes startup and runtime logs, metrics, traces, health events, and exception chains.
depends_on: ["PG052", "PG147"]
capabilities:
  - workspace:read
...
# PG151 — Runtime Log Trace Diagnoser

## 1. Objective

Identify runtime root causes across modules, services, dependencies, and asynchronous flows.
...
- Validate and consume runtime_execution.
- Validate and consume observability_contract.
- Validate and consume deployment_topology.
- Validate and consume source_trace_map.
- Generate and validate runtime_diagnosis.
- Generate and validate trace_timeline.
- Generate and validate root_cause_candidates.
- Generate and validate affected_components.
...
1. Normalize logs, spans, metrics, and health events by correlation.
2. Construct timelines across process and async boundaries.
3. Identify first failure, propagated failures, saturation, timeout, or configuration causes.
4. Map frames and spans to generated symbols and contracts.
5. Produce redacted diagnostic evidence.
...
- `CORRELATION_MISSING` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
- `SENSITIVE_LOG_EXPOSURE` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
- `SYMPTOM_AS_ROOT_CAUSE` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
- `CLOCK_SKEW_UNHANDLED` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
...
- Valid approved inputs produce all declared outputs for `runtime-log-trace-diagnoser`.
...
- Consume compatible certified outputs from PG052, PG147.
...
- Reject or escalate `correlation_missing` without inventing a successful result.
- Reject or escalate `sensitive_log_exposure` without inventing a successful result.
- Reject or escalate `symptom_as_root_cause` without inventing a successful result.
- Reject or escalate `clock_skew_unhandled` without inventing a successful result.
...
- Root-cause candidates cite timeline evidence.
- Secrets and regulated data remain redacted.
- Cross-service and async causality is preserved where telemetry permits.
