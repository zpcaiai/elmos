---
id: PG057
name: runtime-version-policy-resolver
version: 1.0.0
batch: 51
engine: elmos.project-synthesis
schema: elmos.project-synthesis.v1
status: proposed
summary: Resolves runtime, SDK, compiler, framework, operating-system, database, and container versions.
depends_on: ["PG056"]
capabilities:
  - workspace:read
...
# PG057 — Runtime Version Policy Resolver

## 1. Objective

Pin supported versions without floating latest tags or hidden compatibility assumptions.
...
- Validate and process runtime_policy.
- Validate and process support_matrix.
- Validate and process deployment_environment.
- Produce and validate runtime_version_lock.
- Produce and validate compatibility_report.
- Produce and validate upgrade_policy.
- Produce and validate version_exceptions.
...
1. Resolve minimum, preferred, prohibited, and end-of-support versions.
2. Check framework, compiler, SDK, OS, container, database, and driver compatibility.
3. Prefer actively supported LTS versions unless requirements justify another choice.
4. Record exact constraints, support windows, patch policy, and upgrade cadence.
5. Block end-of-life, untrusted, floating, or incompatible selections.
...
- EOL_RUNTIME — emit structured diagnostics and stop unsafe continuation.
- INCOMPATIBLE_VERSION — emit structured diagnostics and stop unsafe continuation.
- FLOATING_VERSION — emit structured diagnostics and stop unsafe continuation.
- MISSING_UPGRADE_OWNER — emit structured diagnostics and stop unsafe continuation.
...
- Valid inputs produce every declared output for `runtime-version-policy-resolver`.
...
- Reject or escalate `eol_runtime` without inventing success.
- Reject or escalate `incompatible_version` without inventing success.
- Reject or escalate `floating_version` without inventing success.
- Reject or escalate `missing_upgrade_owner` without inventing success.
...
- All runtime and framework versions are pinned or constrained by an approved lock policy.
- End-of-life versions require explicit exception and mitigation.
- The complete version set passes compatibility validation.
