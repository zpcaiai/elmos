...
class SpringBoot4ExternalCertificationTests(TestCase):
    def test_all_seven_packs_fail_closed_without_external_evidence(self) -> None:
        for pack_key in SPRING_BOOT_4_PACK_KEYS:
            with self.subTest(pack=pack_key):
                ...
                self.assertEqual("experimental", certification["status"])
    ...
    def test_repository_does_not_hold_a_boot4_self_signed_dossier(self) -> None:
        self.assertFalse(
            (ROOT / "certification/dossiers/spring-boot-4-modernization-v1").exists()
        )
        self.assertFalse(
