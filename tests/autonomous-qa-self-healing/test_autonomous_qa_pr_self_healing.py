...
class GitHubIntegrationTests(unittest.TestCase):
    ...
    def test_create_review_comment_and_status_check(self) -> None:
        ...
        self.assertEqual(comment.path, "src/billing/service.py")
...
class GitLabIntegrationTests(unittest.TestCase):
    ...
    def test_create_and_fetch_mr(self) -> None:
        ...
        self.assertEqual(mr.project_id, "group/subgroup/project-x")
        self.assertEqual(mr.mr_iid, 201)

        fetched = self.client.get_merge_request("group/subgroup/project-x", 201)
        self.assertEqual(fetched.mr_iid, 201)
        self.assertEqual(fetched.source_branch, "elmos/heal-mr-2")

    def test_mr_notes_and_pipeline_status(self) -> None:
        note = self.client.add_mr_note(
            project_id="group/subgroup/project-x",
            mr_iid=201,
...
class SafeCodeFixerTests(unittest.TestCase):
    def test_anti_cheating_rejects_tautology_or_test_skip(self) -> None:
        bad_patch = PatchProposal(
            ...
            original_content="def test_eval(): assert False\n",
            patched_content="def test_eval(): assert True\n",
            diff="--- a\n+++ b\n- assert False\n+ assert True\n",
            content_sha256="sha256:1",
            patch_sha256="sha256:2",
            changes_count=1,
            is_test_file=True,
        )
        with self.assertRaises(PatchSafetyViolation):
            SafeCodeFixer.validate_safe_patch(bad_patch)

    def test_anti_cheating_rejects_sleep_injection(self) -> None:
        bad_patch = PatchProposal(
            file_path="src/service.py",
            original_content="def run(): pass\n",
            patched_content="import time\ndef run(): time.sleep(5)\n",
            diff="--- a\n+++ b\n+ time.sleep(5)\n",
            content_sha256="sha256:3",
            patch_sha256="sha256:4",
            changes_count=2,
            is_test_file=False,
        )
        with self.assertRaises(PatchSafetyViolation):
...
class SandboxedVerifierTests(unittest.TestCase):
    def test_sandbox_merkle_root_and_receipt(self) -> None:
        ...
        self.assertTrue(receipt.merkle_root_sha256.startswith("sha256:"))
        self.assertTrue(receipt.verification_hash.startswith("sha256:"))
...
class EndToEndPRSelfHealingOrchestratorTests(unittest.TestCase):
    ...
    def test_github_pr_self_healing_closed_loop(self) -> None:
        event = WebhookEvent(
            ...
            event_kind=CIEventKind.CHECK_SUITE_COMPLETED,
...
"""
    ...
    def test_gitlab_mr_self_healing_closed_loop(self) -> None:
        event = WebhookEvent(
            ...
            event_kind=CIEventKind.PIPELINE_FAILED,
