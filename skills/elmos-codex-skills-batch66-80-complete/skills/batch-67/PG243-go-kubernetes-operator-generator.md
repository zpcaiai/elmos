---
name: b67-go-kubernetes-operator-generator
description: "Use when ELMOS must generate runnable and maintainable Go Kubernetes Operator Generator for Go modules, HTTP/gRPC services, CLI tools, workers, concurrency, operators, MCP servers, tests, and cross-compilation, preserving ownership, compatibility, security, reproducibility, and evidence boundaries. Triggers: Go Kubernetes Operator Generator, Batch 67, Go modules, HTTP/gRPC services, CLI tools, workers, concurrency, operators, MCP servers, tests, and cross-compilation."
metadata:
  id: PG243
  batch: 67
  version: 1.0.0
  ...
  category: "Go Cloud-Native Project Pack"
---

# PG243 — Go Kubernetes Operator Generator

## Objective

Generate runnable and maintainable Go Kubernetes Operator Generator for Go modules, HTTP/gRPC services, CLI tools, workers, concurrency, operators, MCP servers, tests, and cross-compilation, preserving ownership, compatibility, security, reproducibility, and evidence boundaries.
...
Use for `go-kubernetes-operator-generator` tasks involving Go modules, HTTP/gRPC services, CLI tools, workers, concurrency, operators, MCP servers, tests, and cross-compilation.
...
- Inspect source markers such as `go.mod`, `go.sum`, `go.work`, `cmd/`, `internal/`, `api/` and resolve effective includes, generated files, workspaces, modules, targets, and entrypoints.
...
Upstream dependencies: `PG183`, `PG184`, `PG185`, `PG191`, `PG234`, `PG242`.
...
- `go_kubernetes_operator_generator_plan.json`
- `go_kubernetes_operator_generator_artifacts/`
- `go_kubernetes_operator_generator_verification.json`
...
- Preferred tools: `go`, `gofmt`, `go vet`, `staticcheck`, `golangci-lint`, `go test -race`, `govulncheck`. Record alternate tools, versions, and rationale.
...
- Validate selectors, probes, resources, RBAC, network policy, security context, and shutdown.
- Use server-side or cluster-aware validation when available.
- Do not embed plaintext Secret values.
- Use contexts for cancellation and I/O.
- Run race tests for supported code paths.
- Verify graceful shutdown and goroutine ownership.
