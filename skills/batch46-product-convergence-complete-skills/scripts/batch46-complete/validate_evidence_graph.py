...
def validate(path):
 ...
 for n in d.get("nodes",[]):
  if n["id"] in ids: fail("duplicate evidence node")
  ids.add(n["id"])
  if not valid_digest(n.get("digest")): fail("invalid evidence digest")
  for k in ["producer","tenant_id","project_id","version","signature"]:
   if not non_placeholder(n.get(k)): fail("invalid evidence field "+k)
 for e in d.get("edges",[]):
  if e["from"] not in ids or e["to"] not in ids: fail("unknown evidence edge")
 return d
def main():validate(sys.argv[1]);print("evidence graph ok")
