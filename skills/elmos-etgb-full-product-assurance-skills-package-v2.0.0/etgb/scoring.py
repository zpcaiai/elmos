...
def score_results(results: list[dict[str, Any]], root: Path) -> dict[str, Any]:
    ...
    for result in results:
        ...
        if result["status"] in {"skipped", "unavailable"}:
            counts["unavailable_case"] += 1
            adapter_id = result.get("evidence", {}).get("adapter")
            if adapter_id:
                unavailable_adapters.add(str(adapter_id))
        if line == "product-journey":
            journey_total += 1
            if result["status"] == "passed":
                journey_passed += 1
        if line == "standards-assurance" and result["status"] != "passed":
            counts["failed_mandatory_control"] += 1
        if line == "ui-accessibility-localization" and result["status"] not in {"passed", "skipped", "unavailable"}:
            counts["critical_accessibility_failure"] += 1
        if result["status"] not in {"skipped", "unavailable"}:
            ...
            for oracle in result.get("oracle_results", []):
                ...
                if failed:
                    ...
                    if any(x in text for x in ["cross-tenant leak", "tenant leakage", "tenant isolation failure"]):
                        counts["cross_tenant_leak"] += 1
                    if any(x in text for x in ["financial delta", "ledger reconciliation", "provider reconciliation"]):
                        counts["unexplained_financial_delta"] += 1
                    if any(x in text for x in ["duplicate charge", "double charge", "duplicate credit", "double credit"]):
                        counts["duplicate_charge_or_credit"] += 1
                    if any(x in text for x in ["undeclared feature", "feature registry gap"]):
                        counts["undeclared_feature"] += 1
                    if any(x in text for x in ["hidden test authority", "hidden-test authority", "hidden test exfiltration"]):
                        counts["hidden_test_authority_violation"] += 1
                    if any(x in text for x in ["stale evidence", "expired evidence"]):
    ...
    metrics = {
        ...
        "undeclared_feature_count": counts["undeclared_feature"],
        "unavailable_adapter_count": len(unavailable_adapters),
        "unavailable_case_count": counts["unavailable_case"],
        "cross_tenant_leak_count": counts["cross_tenant_leak"],
        "unexplained_financial_delta_count": counts["unexplained_financial_delta"],
        "duplicate_charge_or_credit_count": counts["duplicate_charge_or_credit"],
        "p0_journey_pass_rate": journey_passed / journey_total if journey_total else None,
        "failed_mandatory_control_count": counts["failed_mandatory_control"],
        "critical_accessibility_failure_count": counts["critical_accessibility_failure"],
        "hidden_test_authority_violation_count": counts["hidden_test_authority_violation"],
