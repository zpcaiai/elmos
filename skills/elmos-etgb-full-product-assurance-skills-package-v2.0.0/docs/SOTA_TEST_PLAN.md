# Elmos ETGB SOTA Comprehensive Test Plan — v2.0

ETGB v2.0 extends the original four-line SOTA plan to the full declared product. Current inventory: **75,419 cases, 1,452 product features, 23 product domains, 41 journeys, 100 assurance controls and 50 Skills**. The authoritative full-product plan is `docs/FULL_PRODUCT_TEST_PLAN.md`; the material below remains the detailed engineering plan for Spring modernization, repository translation, project generation and SQL conversion.
...
ETGB v1.1 把这个原则落实为 **46,664 条具体用例**：四条业务线覆盖 694 个领域能力 ID，横切生产矩阵覆盖 100 个能力 ID，共 794 个能力 ID；并把恢复、安全、隔离、成本、权限、证据、供应链、性能和基准治理落实为可调用 Skills、参考代码与集成契约。
...
- 不宣称覆盖未来未知技术；“全覆盖”严格限定为声明的 ETGB v1.1 capability model。
...
- 将包内统一 Harness Adapter Contract 接入四条业务线的生产执行器；
