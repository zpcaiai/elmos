---
name: proof-critic-model-training
description: Use this skill when Elmos must perform proof critic model training as part of the 07-private-model-foundry production
  workflow, with typed contracts, policy enforcement, evidence capture, and rollback.
license: Proprietary-Elmos-Commercial
compatibility: Elmos v3 harness; K7 Private Model Foundry; policy and evidence services required.
metadata:
  version: 3.0.0
  pack: 07-private-model-foundry
  priority: P1
  exposure: atomic-registry-only
  business-line: private-model
allowed-tools: train.launch checkpoint.save registry.log eval.run artifact.sign
---
# proof-critic-model-training

## 能力目标

训练不变量、证明义务、反例和 Evidence Contract 完整性审查。
...
- 必须门：license-approved, dataset-frozen, reproducible-run, eval-regression-pass, signed；
...
- 业务线：`private-model`；包：`07-private-model-foundry`。
