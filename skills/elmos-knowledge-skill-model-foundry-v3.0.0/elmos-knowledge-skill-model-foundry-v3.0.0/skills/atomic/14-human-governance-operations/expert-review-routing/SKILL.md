---
name: expert-review-routing
description: Use this skill when Elmos must perform expert review routing as part of the 14-human-governance-operations production
  workflow, with typed contracts, policy enforcement, evidence capture, and rollback.
license: Proprietary-Elmos-Commercial
compatibility: Elmos v3 harness; Human Assurance Plane; policy and evidence services required.
metadata:
  version: 3.0.0
  pack: 14-human-governance-operations
  priority: P0
  exposure: atomic-registry-only
  business-line: human-governance
allowed-tools: review.assign approval.record annotation.write waiver.issue incident.escalate
---
# expert-review-routing

## 能力目标

按语言、框架、数据库、安全、业务和风险把问题分配给合适专家。
...
- 必须门：reviewer-qualified, conflict-disclosed, decision-reasoned, reversible；
...
- 业务线：`human-governance`；包：`14-human-governance-operations`。
