---
name: graph-path-retrieval
description: Use this skill when Elmos must perform graph path retrieval as part of the 03-retrieval-context-engineering production
  workflow, with typed contracts, policy enforcement, evidence capture, and rollback.
license: Proprietary-Elmos-Commercial
compatibility: Elmos v3 harness; K3 Retrieval and Context Kernel; policy and evidence services required.
metadata:
  version: 3.0.0
  pack: 03-retrieval-context-engineering
  priority: P0
  exposure: atomic-registry-only
  business-line: retrieval-context
allowed-tools: search.lexical search.vector graph.query reranker.score context.pack
---
# graph-path-retrieval

## 能力目标

按调用、数据流、事务、安全或部署路径检索跨文件证据链。
...
- 必须门：provenance-bound, tenant-authorized, version-compatible, budget-respected；
...
- 业务线：`retrieval-context`；包：`03-retrieval-context-engineering`。
