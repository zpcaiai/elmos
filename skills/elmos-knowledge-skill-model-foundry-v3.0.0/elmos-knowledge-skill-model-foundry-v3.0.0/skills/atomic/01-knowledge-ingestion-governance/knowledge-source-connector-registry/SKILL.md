---
name: knowledge-source-connector-registry
description: Use this skill when Elmos must perform knowledge source connector registry as part of the 01-knowledge-ingestion-governance
...
compatibility: Elmos v3 harness; K1 Knowledge Fabric; policy and evidence services required.
metadata:
  version: 3.0.0
  pack: 01-knowledge-ingestion-governance
  priority: P0
  exposure: atomic-registry-only
  business-line: knowledge-ingestion
allowed-tools: source.connect artifact.parse secret.scan license.scan object.store
---
# knowledge-source-connector-registry

## 能力目标

注册并治理 Git、对象存储、Wiki、Issue、PR、CI、数据库和日志等知识源连接器。
...
- 必须门：source-authorized, provenance-complete, sensitive-data-classified, parse-quality-pass；
...
- 业务线：`knowledge-ingestion`；包：`01-knowledge-ingestion-governance`。
