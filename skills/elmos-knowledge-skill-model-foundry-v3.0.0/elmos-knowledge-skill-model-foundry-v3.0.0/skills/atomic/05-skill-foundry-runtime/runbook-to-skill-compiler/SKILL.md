---
name: runbook-to-skill-compiler
description: Use this skill when Elmos must perform runbook to skill compiler as part of the 05-skill-foundry-runtime production
  workflow, with typed contracts, policy enforcement, evidence capture, and rollback.
license: Proprietary-Elmos-Commercial
compatibility: Elmos v3 harness; K5 Skill Foundry and Runtime; policy and evidence services required.
metadata:
  version: 3.0.0
  pack: 05-skill-foundry-runtime
  priority: P1
  exposure: atomic-registry-only
  business-line: skill-runtime
allowed-tools: skill.registry workflow.execute sandbox.run policy.evaluate eval.run
---
# runbook-to-skill-compiler

## 能力目标

把人工 Runbook 转换为带条件、分支、工具、证据和异常处理的工作流。
...
- 必须门：trigger-eval-pass, tool-permission-bounded, evidence-complete, rollback-tested；
...
- 业务线：`skill-runtime`；包：`05-skill-foundry-runtime`。
