...
description: Use this skill when an Elmos task requires the AI Agent, RAG & ML Engineering / AI 工具项目生成与迁移 capability pack. It routes to compatible atomic skills, enforces policy and evidence gates, and builds a durable production workflow.
...
生成、迁移和认证 RAG、Agent、多 Agent、模型服务、私有训练和 AI 应用项目，保持供应商可替换与生产安全。
...
本包包含 **46** 个原子 Skill。启动时只暴露 Meta-Skill；默认最多返回 16 个候选、激活 8 个，超过时按阶段拆分执行。

## 硬门

task-and-tool-contract-valid, grounding-and-quality-pass, tool-permission-safe, data-rights-compliant, shadow-and-rollback-tested。
