...
description: Use this skill when an Elmos task requires the Java & Spring Enterprise Modernization / Java 企业现代化 capability pack. It routes to compatible atomic skills, enforces policy and evidence gates, and builds a durable production workflow.
...
把 Struts、Servlet、JSP、Java EE、应用服务器和旧 Spring 仓库语义正确、行为等价地迁移到现代 Spring Boot 目标栈。
...
本包包含 **48** 个原子 Skill。启动时只暴露 Meta-Skill；默认最多返回 16 个候选、激活 8 个，超过时按阶段拆分执行。

## 硬门

legacy-inventory-complete, target-build-passes, behavior-equivalent, security-not-weakened, migration-reversible。
