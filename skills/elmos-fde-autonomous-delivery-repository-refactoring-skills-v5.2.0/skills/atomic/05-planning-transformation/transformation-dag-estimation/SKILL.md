---
name: transformation-dag-estimation
description: Convert approved target architecture into a dependency-aware executable Transformation DAG with gates, rollback, machine wall-clock, and cost estimates.
---

# Mission

Convert approved target architecture into a dependency-aware executable Transformation DAG with gates, rollback, machine wall-clock, and cost estimates.
...
- one giant patch
- human-day estimate presented as machine runtime
...
1. Decompose target-state deltas into atomic transformation objectives.
2. Add dependency, compatibility, data, side-effect, and approval edges.
3. Attach verification and rollback obligations to each step.
4. Estimate tool, model, compute, storage, and network use with confidence ranges.
5. Simulate failure, cancellation, resume, and partial-completion paths.
6. Publish the signed plan and initial execution horizon.

## Required outputs

- Transformation DAG
- critical path
- machine wall-clock estimate
- cost estimate
- approval and rollback map
