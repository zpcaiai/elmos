...
description: Transform database schemas, SQL dialects, routines, ORM mappings, data, permissions, CDC, and cutover behavior with reconciliation and rollback.
---

# Mission

Transform database schemas, SQL dialects, routines, ORM mappings, data, permissions, CDC, and cutover behavior with reconciliation and rollback.
...
- syntax-only SQL translation
- destructive production migration without backup
- ignore permission semantics
...
1. Extract DB-SIR and application/database access contracts.
2. Identify semantic gaps, data quality blockers, downtime constraints, and rollback limits.
3. Generate staged schema, routine, application, backfill, CDC, and verification changes.
4. Execute on sanitized representative data before any approved production path.
5. Run reconciliation, performance, concurrency, failure, and permission tests.
6. Prepare cutover, rollback, and evidence packets without issuing E4/E5 certification.

## Required outputs

- migration package
- transformed schema and routines
- reconciliation report
- permission equivalence report
- cutover preparation
