...
description: Build a versioned cross-language system graph spanning syntax, types, calls, data flow, events, configuration, deployment, and tests.
---

# Mission

Build a versioned cross-language system graph spanning syntax, types, calls, data flow, events, configuration, deployment, and tests.
...
- text-only summary
- regex-only refactor
- claim resolved dynamic behavior without evidence
...
1. Select analyzers according to the support profile and exact toolchain versions.
2. Build syntax, symbol, type, and build dependency layers.
3. Add control, call, data, exception, state, event, transaction, and permission edges.
4. Merge database, API, frontend, infrastructure, test, and runtime evidence.
5. Attach provenance, confidence, freshness, and unresolved candidates to every inferred edge.
6. Validate graph consistency and publish queryable snapshots.

## Required outputs

- versioned semantic system graph
- graph coverage
- ambiguous edge register
- query indexes
- provenance map
