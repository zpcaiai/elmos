# Runbook: Business Baseline Roi

## Entry conditions

- An existing canonical route owner has selected `business-baseline-roi`.
...
1. Select business outcomes from the approved discovery brief.
2. Identify authoritative baseline sources and time windows.
3. Normalize units, volumes, seasonality, and confidence.
4. Model conservative, expected, and upside value scenarios.
5. Estimate machine execution and operating cost separately from human review.
6. Publish a measurable value contract with stop conditions.
