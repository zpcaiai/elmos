...
class Toolkit(unittest.TestCase):
 ...
 def test_positive_gate(self):
  with tempfile.TemporaryDirectory() as td:
   ...
   metrics={k:1.0 for k in ['secure_sdlc_control_coverage', 'threat_model_coverage', 'sbom_coverage', 'signature_verification_rate', 'provenance_coverage', 'vulnerability_sla_compliance', 'audit_evidence_freshness_rate', 'independent_assessment_closure_rate', 'evidence_trace_coverage']}; zero={k:0 for k in ['critical_open_vulnerabilities', 'secret_leaks', 'unsigned_production_artifacts', 'tampered_artifacts_accepted', 'unresolved_license_blocks', 'builder_attestation_failures', 'runner_downgrade_acceptances', 'cross_tenant_evidence_leaks', 'test_integrity_violations']}
