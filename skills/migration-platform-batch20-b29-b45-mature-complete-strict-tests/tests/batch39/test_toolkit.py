...
class Toolkit(unittest.TestCase):
 ...
 def test_positive_gate(self):
  with tempfile.TemporaryDirectory() as td:
   ...
   metrics={k:1.0 for k in ['service_catalog_coverage', 'slo_compliance_rate', 'incident_response_exercise_pass_rate', 'restore_pass_rate', 'multiregion_failover_pass_rate', 'fair_scheduling_pass_rate', 'support_sla_pass_rate', 'production_readiness_pass_rate', 'evidence_trace_coverage']}; zero={k:0 for k in ['unresolved_sev1_incidents', 'rpo_breaches', 'rto_breaches', 'tenant_starvation_events', 'unowned_critical_alerts', 'missing_critical_runbooks', 'cross_tenant_observability_leaks', 'test_integrity_violations']}
