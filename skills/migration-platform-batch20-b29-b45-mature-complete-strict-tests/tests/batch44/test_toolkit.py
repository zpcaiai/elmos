...
class Toolkit(unittest.TestCase):
 ...
 def test_positive_gate(self):
  with tempfile.TemporaryDirectory() as td:
   ...
   metrics={k:1.0 for k in ['metering_completeness', 'cost_allocation_reconciliation_rate', 'unit_cost_trace_coverage', 'billing_reconciliation_rate', 'margin_model_accuracy_rate', 'budget_guardrail_pass_rate', 'forecast_interval_coverage', 'customer_value_evidence_coverage', 'evidence_trace_coverage']}; zero={k:0 for k in ['duplicate_meter_events', 'unallocated_material_costs', 'nonzero_billing_reconciliation_difference', 'security_gate_bypasses_for_cost', 'unapproved_negative_margin_deals', 'customer_data_loss_from_budget_stops', 'unexplained_price_overrides', 'test_integrity_violations']}
