...
class Toolkit(unittest.TestCase):
 ...
 def test_positive_gate(self):
  with tempfile.TemporaryDirectory() as td:
   ...
   metrics={k:1.0 for k in ['versioned_surface_coverage', 'compatibility_matrix_pass_rate', 'upgrade_pass_rate', 'rollback_pass_rate', 'mixed_version_pass_rate', 'backport_validation_rate', 'deprecation_owner_coverage', 'migration_guide_accuracy_rate', 'evidence_trace_coverage']}; zero={k:0 for k in ['unannounced_breaking_changes', 'unsupported_version_promotions', 'irreversible_upgrade_without_approval', 'runner_protocol_safety_regressions', 'schema_data_loss', 'expired_feature_flags', 'unpatched_supported_critical_vulnerabilities', 'test_integrity_violations']}
