...
class Toolkit(unittest.TestCase):
 ...
 def test_positive_gate(self):
  with tempfile.TemporaryDirectory() as td:
   ...
   metrics={k:1.0 for k in ['edition_coverage', 'portable_control_plane_pass_rate', 'mixed_version_compatibility_pass_rate', 'zero_downtime_upgrade_pass_rate', 'rollback_pass_rate', 'airgap_update_pass_rate', 'tenant_migration_pass_rate', 'dr_recovery_pass_rate', 'evidence_trace_coverage']}; zero={k:0 for k in ['critical_data_loss', 'cross_tenant_access', 'unsigned_updates_accepted', 'unresolved_schema_incompatibilities', 'incompatible_runner_leases', 'orphaned_workflows', 'residency_violations', 'test_integrity_violations']}
