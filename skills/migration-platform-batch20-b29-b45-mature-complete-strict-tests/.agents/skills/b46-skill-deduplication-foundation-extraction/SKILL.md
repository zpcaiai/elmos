---
name: b46-skill-deduplication-foundation-extraction
description: 识别重复Orchestrator、Evidence、Policy、Version、Rollback和Gate，抽取共享Kernel与领域Profile。
---

# Skill 1509：Skill去重与共享基础能力抽取
...
识别重复Orchestrator、Evidence、Policy、Version、Rollback和Gate，抽取共享Kernel与领域Profile。
...
- 共享基础能力只能保留一个权威实现；删除重复Skill前必须提供引用迁移与兼容适配。
...
- `reports/skill-duplication/`
- `shared/skill-foundations/`
