---
name: b46-global-migration-lifecycle-state-machine
description: 把各Batch局部状态机映射为唯一客户项目主状态机及异常分支。
---

# Skill 1500：全局迁移项目生命周期状态机
...
把各Batch局部状态机映射为唯一客户项目主状态机及异常分支。
...
- 同一项目只能有一个全局业务状态；局部状态必须映射；P0状态不允许无Evidence跳转。
...
- `contracts/product-kernel/project-lifecycle.json`
- `services/control-plane/projects/lifecycle/`
