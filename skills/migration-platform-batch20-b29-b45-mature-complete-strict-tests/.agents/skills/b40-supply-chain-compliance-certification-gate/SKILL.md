---
name: b40-supply-chain-compliance-certification-gate
description: Certify secure development and supply-chain controls only from signed artifacts, fresh findings and independent evidence.
---

# Skill 1392: b40-supply-chain-compliance-certification-gate

## Use this skill when

- Implementing or reviewing **Supply-chain and compliance certification gate** within Batch 40: 安全供应链与合规认证.
...
- Security evidence must be produced by the real scanner, builder, signer or independent assessor; self-attestation is insufficient.
- Never weaken tenant isolation, cryptographic verification, policy or tests to obtain a green result.
...
1. Inspect current implementation and create a gap inventory mapped to the Batch 40 support matrix and strict-test coverage.
2. Define the typed contract, state machine, ownership, policy decisions, failure modes, compatibility boundaries and lifecycle for **Supply-chain and compliance certification gate**.
...
7. Update evidence and run the conservative Batch 40 gate. Keep the strongest status actually supported by evidence.

## Required repository outputs

- `supply-chain-compliance-packs/<pack-key>/pack.json` and versioned support matrix
...
- Run `python3 scripts/batch40/validate_skill_bundle.py .`.
- Run the Batch 40 pack validator and final gate against the exact pack directory.
...
- The Batch 40 gate emits only the strongest evidence-supported status and produces a reviewable failure report otherwise.
