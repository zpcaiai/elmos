---
name: b46-maintainability-idiomaticity-gate
description: 在Build、Behavior、Security、Production之外独立验证复杂度、重复、依赖、兼容层和可接管性。
---

# Skill 1512：Maintainability与Target Idiomaticity Gate
...
在Build、Behavior、Security、Production之外独立验证复杂度、重复、依赖、兼容层和可接管性。
...
- Build Green不能抵消维护性失败；Security、Transaction、Money领域限制Compatibility Runtime。
...
- `services/validation/maintainability/`
- `contracts/maintainability/`
