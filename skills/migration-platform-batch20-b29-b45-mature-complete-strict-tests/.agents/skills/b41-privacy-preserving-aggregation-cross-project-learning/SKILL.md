---
name: b41-privacy-preserving-aggregation-cross-project-learning
description: Learn aggregate patterns across projects using minimization, thresholds, privacy review and non-reconstruction tests.
---

# Skill 1408: b41-privacy-preserving-aggregation-cross-project-learning

## Use this skill when

- Implementing or reviewing **Privacy-preserving aggregation and cross-project learning** within Batch 41: 迁移知识图谱与智能飞轮.
...
- Do not declare support from configuration files alone when real runtime, customer, operational or independent evidence is required.
- Unknown P0 obligations, missing owners or stale evidence block certification.
...
1. Inspect current implementation and create a gap inventory mapped to the Batch 41 support matrix and strict-test coverage.
2. Define the typed contract, state machine, ownership, policy decisions, failure modes, compatibility boundaries and lifecycle for **Privacy-preserving aggregation and cross-project learning**.
...
7. Update evidence and run the conservative Batch 41 gate. Keep the strongest status actually supported by evidence.

## Required repository outputs

- `knowledge-flywheel-packs/<pack-key>/pack.json` and versioned support matrix
...
- Run `python3 scripts/batch41/validate_skill_bundle.py .`.
- Run the Batch 41 pack validator and final gate against the exact pack directory.
...
- The Batch 41 gate emits only the strongest evidence-supported status and produces a reviewable failure report otherwise.
