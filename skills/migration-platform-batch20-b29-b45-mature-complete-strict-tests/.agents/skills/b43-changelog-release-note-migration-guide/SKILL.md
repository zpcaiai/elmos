---
name: b43-changelog-release-note-migration-guide
description: Generate accurate changes, impacts, actions, risks, compatibility and rollback guidance from release evidence.
---

# Skill 1453: b43-changelog-release-note-migration-guide

## Use this skill when

- Implementing or reviewing **Changelog, release note and migration guide** within Batch 43: 产品版本、兼容与LTS生命周期.
...
1. Inspect current implementation and create a gap inventory mapped to the Batch 43 support matrix and strict-test coverage.
2. Define the typed contract, state machine, ownership, policy decisions, failure modes, compatibility boundaries and lifecycle for **Changelog, release note and migration guide**.
...
7. Update evidence and run the conservative Batch 43 gate. Keep the strongest status actually supported by evidence.

## Required repository outputs

- `product-lifecycle-packs/<pack-key>/pack.json` and versioned support matrix
...
- Run `python3 scripts/batch43/validate_skill_bundle.py .`.
- Run the Batch 43 pack validator and final gate against the exact pack directory.
...
- The Batch 43 gate emits only the strongest evidence-supported status and produces a reviewable failure report otherwise.
