---
name: b35-race-deadlock-liveness-verification
description: Implement race deadlock starvation livelock and liveness verification using static dynamic stress and model techniques with reproducible evidence.
---
...
## Skill 1278: b35-race-deadlock-liveness-verification

## Use this skill when

- The system needs evidence against races, deadlocks, starvation, livelock, or shutdown hangs.

## Domain-specific risks and invariants

- Static findings can be false positives; stress passes are not proofs.

## Workflow

1. Build lock, wait-for, task, queue, and shared-state models.
2. Run static race/lock analysis, dynamic instrumentation, controlled stress, and schedule exploration.
3. Define liveness and bounded progress obligations.
4. Capture thread/task dumps and exact schedules for failures.
5. Validate mitigations and runtime monitors.
6. Report unsupported native or lock-free paths explicitly.
...
- Analyze shared state, lock graphs, wait-for graphs, async tasks, queues, retry loops, and shutdown/recovery behavior.
...
- Race findings, lock graphs, liveness obligations, stress results, counterexamples, mitigations, and monitors.
