---
name: b45-security-data-governance-certification
description: Verify tenant isolation, runner security, supply chain, privacy, residency, retention and deletion from adversarial evidence.
---

# Skill 1481: b45-security-data-governance-certification

## Use this skill when

- Implementing or reviewing **Security and data-governance certification** within Batch 45: 成熟产品综合认证.
...
1. Inspect current implementation and create a gap inventory mapped to the Batch 45 support matrix and strict-test coverage.
2. Define the typed contract, state machine, ownership, policy decisions, failure modes, compatibility boundaries and lifecycle for **Security and data-governance certification**.
...
7. Update evidence and run the conservative Batch 45 gate. Keep the strongest status actually supported by evidence.

## Required repository outputs

- `mature-product-certification-packs/<pack-key>/pack.json` and versioned support matrix
...
- Run `python3 scripts/batch45/validate_skill_bundle.py .`.
- Run the Batch 45 pack validator and final gate against the exact pack directory.
...
- The Batch 45 gate emits only the strongest evidence-supported status and produces a reviewable failure report otherwise.
