---
name: b39-global-observability-business-telemetry
description: Implement correlated metrics, logs, traces, profiles and business outcomes with tenant safety, retention and cost control.
---

# Skill 1350: b39-global-observability-business-telemetry

## Use this skill when

- Implementing or reviewing **Global observability and business telemetry** within Batch 39: 全球SRE与平台运营.
...
1. Inspect current implementation and create a gap inventory mapped to the Batch 39 support matrix and strict-test coverage.
2. Define the typed contract, state machine, ownership, policy decisions, failure modes, compatibility boundaries and lifecycle for **Global observability and business telemetry**.
...
7. Update evidence and run the conservative Batch 39 gate. Keep the strongest status actually supported by evidence.

## Required repository outputs

- `sre-operations-packs/<pack-key>/pack.json` and versioned support matrix
...
- Run `python3 scripts/batch39/validate_skill_bundle.py .`.
- Run the Batch 39 pack validator and final gate against the exact pack directory.
...
- The Batch 39 gate emits only the strongest evidence-supported status and produces a reviewable failure report otherwise.
