---
name: b42-multiagent-conflict-consensus-arbitration
description: Resolve conflicting plans or findings using evidence, independent verification and human arbitration for critical decisions.
---

# Skill 1433: b42-multiagent-conflict-consensus-arbitration

## Use this skill when

- Implementing or reviewing **Multi-agent conflict, consensus and arbitration** within Batch 42: 成熟Agent迁移工厂.
...
1. Inspect current implementation and create a gap inventory mapped to the Batch 42 support matrix and strict-test coverage.
2. Define the typed contract, state machine, ownership, policy decisions, failure modes, compatibility boundaries and lifecycle for **Multi-agent conflict, consensus and arbitration**.
...
7. Update evidence and run the conservative Batch 42 gate. Keep the strongest status actually supported by evidence.

## Required repository outputs

- `agent-factory-packs/<pack-key>/pack.json` and versioned support matrix
...
- Run `python3 scripts/batch42/validate_skill_bundle.py .`.
- Run the Batch 42 pack validator and final gate against the exact pack directory.
...
- The Batch 42 gate emits only the strongest evidence-supported status and produces a reviewable failure report otherwise.
