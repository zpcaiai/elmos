...
def main():
 ...
 if requested:
  ...
  for rel in ['corpus/negative','corpus/holdout','corpus/representative-extensions']:
   if not real_files(pack/rel): failures.append(f'{rel} empty')
 ...
 lines=[f'# Batch 37 gate: {manifest.get("pack_key")}', '', f'- Pack status: `{manifest.get("status")}`', f'- Gate status: `{status}`','']
