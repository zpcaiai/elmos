...
def main() -> int:
    ...
    lines = [
        ...
        f'- Closure gate status: `{status}`', '',
    ]
    lines += (
        ['## Failures'] + [f'- {item}' for item in failures]
        if failures else ['Core and closure lifecycle checks passed.']
    )
    (pack / 'certification/closure-gate-report.md').write_text('\n'.join(lines) + '\n')
    if failures:
        print('\n'.join('CLOSURE GATE FAIL: ' + item for item in failures), file=sys.stderr)
        return 2
    print(f'CLOSURE GATE PASS: {manifest.get("pack_key")} status={manifest.get("status")}')
    return 0
