...
EXPECTED=22
def main():
 root=Path(sys.argv[1] if len(sys.argv)>1 else '.agents/skills'); errors=[]; names={}; files=sorted(root.glob('*/SKILL.md')); batch=[f for f in files if f.parent.name.startswith('b34-')]
 if not batch: errors.append(f'no Batch 34 skills found under {root}')
 for path in batch:
 ...
 if len(batch)!=EXPECTED: errors.append(f'expected {EXPECTED} Batch 34 skills, found {len(batch)}')
 if errors:
  print('\n'.join('ERROR: '+e for e in errors),file=sys.stderr); return 1
 print(f'OK: {len(batch)} Batch 34 skills'); return 0
