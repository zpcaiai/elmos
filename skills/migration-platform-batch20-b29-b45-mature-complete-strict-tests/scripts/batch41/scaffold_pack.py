...
HERE=Path(__file__).resolve(); ROOT=HERE.parents[2]; T=ROOT/'templates/batch41'
p=argparse.ArgumentParser(); p.add_argument('--pack-key',required=True); p.add_argument('--output-root',default='knowledge-flywheel-packs'); a=p.parse_args()
out=Path(a.output_root)/a.pack_key; out.mkdir(parents=True,exist_ok=True)
for src in T.glob('*.json'):
 obj=json.loads(src.read_text()); raw=json.dumps(obj).replace('example-knowledge-flywheel',a.pack_key); obj=json.loads(raw)
