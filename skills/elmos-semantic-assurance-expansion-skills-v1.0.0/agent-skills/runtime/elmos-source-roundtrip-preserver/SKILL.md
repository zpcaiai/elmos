...
triggers:
  - "Use when implementing or executing `elmos-source-roundtrip-preserver`."
  - "Use when a migration/certification DAG requires `frontend-semantics` capability."
outputs:
  - "semantic-assurance/elmos-source-roundtrip-preserver/model.json"
  - "semantic-assurance/elmos-source-roundtrip-preserver/evidence.json"
  - "semantic-assurance/elmos-source-roundtrip-preserver/diagnostics.json"
---

# Source Round-Trip Preserver

## Objective

Guarantee parse→model→print round trips preserve source meaning and intentionally preserved lexical structure before semantic transformation begins.
...
- **token-equivalent roundtrip.**
- **stable formatting zones.**
- **formatter isolation.**
...
- [ ] byte/token round-trip classes.
- [ ] formatter opt-in fixture.
- [ ] source-map hash test.
