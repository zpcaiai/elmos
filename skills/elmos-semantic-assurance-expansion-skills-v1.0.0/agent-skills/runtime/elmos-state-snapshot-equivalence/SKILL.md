...
triggers:
  - "Use when implementing or executing `elmos-state-snapshot-equivalence`."
  - "Use when a migration/certification DAG requires `behavior-oracle` capability."
outputs:
  - "semantic-assurance/elmos-state-snapshot-equivalence/model.json"
  - "semantic-assurance/elmos-state-snapshot-equivalence/evidence.json"
  - "semantic-assurance/elmos-state-snapshot-equivalence/diagnostics.json"
---

# In-Memory State Snapshot Equivalence

## Objective

Compare selected object/heap/session state under canonical schemas while ignoring approved representation-only differences.
...
- **object graph canonicalization.**
- **session/cache state.**
- **representation adapters.**
- **alias-sensitive state.**
...
- [ ] object graph cycle fixture.
- [ ] cache normalization fixture.
- [ ] alias mismatch fixture.
