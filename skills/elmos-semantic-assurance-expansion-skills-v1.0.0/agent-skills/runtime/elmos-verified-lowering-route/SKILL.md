...
triggers:
  - "Use when implementing or executing `elmos-verified-lowering-route`."
  - "Use when a migration/certification DAG requires `formal-assurance` capability."
outputs:
  - "semantic-assurance/elmos-verified-lowering-route/model.json"
  - "semantic-assurance/elmos-verified-lowering-route/evidence.json"
  - "semantic-assurance/elmos-verified-lowering-route/diagnostics.json"
---

# Verified Lowering Route

## Objective

Define high-assurance lowering paths where a verified compiler/intermediate target or proof-producing step can reduce the trusted computing base.
...
- [ ] CompCert-like C subset profile.
- [ ] unsupported construct fallback.
- [ ] proof artifact hash.
