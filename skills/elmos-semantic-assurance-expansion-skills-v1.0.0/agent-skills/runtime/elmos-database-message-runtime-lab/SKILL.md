---
name: elmos-database-message-runtime-lab
description: "Provision real compatible database/message broker matrices for transaction, collation, delivery and retry equivalence instead of mocks-only certification."
version: 1.0.0
skill_id: ELMOS-POLY-273
layer: runtime-lab
risk: critical
readiness: not-run
dependencies:
...
triggers:
  - "Use when implementing or executing `elmos-database-message-runtime-lab`."
  - "Use when a migration/certification DAG requires `runtime-lab` capability."
outputs:
  - "semantic-assurance/elmos-database-message-runtime-lab/model.json"
  - "semantic-assurance/elmos-database-message-runtime-lab/evidence.json"
  - "semantic-assurance/elmos-database-message-runtime-lab/diagnostics.json"
---

# Database & Messaging Runtime Lab

## Objective

Provision real compatible database/message broker matrices for transaction, collation, delivery and retry equivalence instead of mocks-only certification.
...
- **database versions/dialects.**
- **isolation levels.**
- **queue/topic semantics.**
- **failure/retry injection.**
