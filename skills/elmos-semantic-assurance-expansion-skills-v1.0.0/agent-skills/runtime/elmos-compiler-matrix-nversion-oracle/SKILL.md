...
triggers:
  - "Use when implementing or executing `elmos-compiler-matrix-nversion-oracle`."
  - "Use when a migration/certification DAG requires `fuzzing` capability."
outputs:
  - "semantic-assurance/elmos-compiler-matrix-nversion-oracle/model.json"
  - "semantic-assurance/elmos-compiler-matrix-nversion-oracle/evidence.json"
  - "semantic-assurance/elmos-compiler-matrix-nversion-oracle/diagnostics.json"
---

# Compiler Matrix & N-Version Oracle

## Objective

Compile/run fixtures with multiple independent compilers/runtimes to detect implementation-specific behavior and strengthen differential oracles.
...
- **multiple compilers.**
- **optimization modes.**
- **major runtimes/engines.**
- **majority/consensus is not automatic truth.**
...
- [ ] two compiler agreement.
- [ ] one compiler divergence classified.
- [ ] spec oracle overrides consensus.
