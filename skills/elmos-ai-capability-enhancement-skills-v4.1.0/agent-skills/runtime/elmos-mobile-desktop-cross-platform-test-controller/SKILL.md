---
name: elmos-mobile-desktop-cross-platform-test-controller
description: Test generated iOS, Android, Flutter, macOS, Windows and web clients across device, OS, network, permission and lifecycle matrices.
version: 4.0.0
...
# Mobile and Desktop Cross-Platform Test Controller
...
- `MobileAndDesktopCrossPlatformTestControllerProfile`
- `MobileAndDesktopCrossPlatformTestControllerPlan`
- `MobileAndDesktopCrossPlatformTestControllerRun`
- `MobileAndDesktopCrossPlatformTestControllerDecision`

Required semantic capabilities:

- select representative device/OS matrix
- exercise install, upgrade and deep links
- test permissions, backgrounding and offline mode
- verify accessibility and localization
- capture crash, performance and battery evidence
...
- `elmos-ui-visual-regression-journey-certifier` from `elmos-functional-assurance-certification-skills` — optional companion; required only for E4/E5/P05 certification.
- `elmos-i18n-l10n-conformance-verifier` from `elmos-functional-assurance-certification-skills` — optional companion; required only for E4/E5/P05 certification.

## Primary Outputs

- `artifacts/elmos-mobile-desktop-cross-platform-test-controller/profile.yaml`
- `artifacts/elmos-mobile-desktop-cross-platform-test-controller/plan.json`
- `artifacts/elmos-mobile-desktop-cross-platform-test-controller/result.json`
- `artifacts/elmos-mobile-desktop-cross-platform-test-controller/evidence/`
...
- **emulator pass hides device issue** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **permission state not reset** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **upgrade loses local data** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **background task violates policy** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **device farm logs leak user data** — must have a preventive control, a detective signal, a negative fixture and a response action.
...
- native scenario: select representative device/OS matrix
- native scenario: exercise install, upgrade and deep links
- native scenario: test permissions, backgrounding and offline mode
- native scenario: verify accessibility and localization
- native scenario: capture crash, performance and battery evidence
