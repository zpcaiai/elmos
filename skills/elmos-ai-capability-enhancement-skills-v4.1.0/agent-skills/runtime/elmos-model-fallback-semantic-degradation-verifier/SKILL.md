---
name: elmos-model-fallback-semantic-degradation-verifier
description: Verify that provider/model fallback preserves required schemas, tool behavior, safety, language, memory and task quality or blocks explicitly.
version: 4.0.0
...
# Model Fallback Semantic Degradation Verifier
...
- `ModelFallbackSemanticDegradationVerifierProfile`
- `ModelFallbackSemanticDegradationVerifierPlan`
- `ModelFallbackSemanticDegradationVerifierRun`
- `ModelFallbackSemanticDegradationVerifierDecision`

Required semantic capabilities:

- compile per-task minimum capability contract
- run primary/fallback paired scenarios
- verify structured output and tool-call semantics
- measure safety and multilingual degradation
- gate cascade depth and user-visible disclosure
...
- `elmos-ai-provider-behavior-fingerprint-recertifier` from `elmos-functional-assurance-certification-skills` — optional companion; required only for E4/E5/P05 certification.

## Primary Outputs

- `artifacts/elmos-model-fallback-semantic-degradation-verifier/profile.yaml`
- `artifacts/elmos-model-fallback-semantic-degradation-verifier/plan.json`
- `artifacts/elmos-model-fallback-semantic-degradation-verifier/result.json`
- `artifacts/elmos-model-fallback-semantic-degradation-verifier/evidence/`
...
- **fallback lacks required tool calling** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **schema parser coerces invalid output** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **fallback bypasses residency** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **cascade causes economic DoS** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **quality degradation not disclosed** — must have a preventive control, a detective signal, a negative fixture and a response action.
...
- native scenario: compile per-task minimum capability contract
- native scenario: run primary/fallback paired scenarios
- native scenario: verify structured output and tool-call semantics
- native scenario: measure safety and multilingual degradation
- native scenario: gate cascade depth and user-visible disclosure
