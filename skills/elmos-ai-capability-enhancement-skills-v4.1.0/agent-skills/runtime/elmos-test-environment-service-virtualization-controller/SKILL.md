---
name: elmos-test-environment-service-virtualization-controller
description: Provision reproducible ephemeral environments, real infrastructure where required, bounded simulators where allowed and explicit fidelity evidence.
version: 4.0.0
...
# Test Environment and Service Virtualization Controller
...
- `TestEnvironmentAndServiceVirtualizationControllerProfile`
- `TestEnvironmentAndServiceVirtualizationControllerPlan`
- `TestEnvironmentAndServiceVirtualizationControllerRun`
- `TestEnvironmentAndServiceVirtualizationControllerDecision`

Required semantic capabilities:

- compile environment topology and version lock
- provision databases, brokers, caches and services
- separate native, emulated and simulated components
- inject network and dependency faults
- destroy safely with evidence and cost accounting
...
- `elmos-native-conformance-lab-controller` from `elmos-functional-assurance-certification-skills` — optional companion; required only for E4/E5/P05 certification.

## Primary Outputs

- `artifacts/elmos-test-environment-service-virtualization-controller/profile.yaml`
- `artifacts/elmos-test-environment-service-virtualization-controller/plan.json`
- `artifacts/elmos-test-environment-service-virtualization-controller/result.json`
- `artifacts/elmos-test-environment-service-virtualization-controller/evidence/`
...
- **shared test environment contaminates result** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **simulator silently replaces native service** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **cleanup deletes another tenant** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **version drift between runs** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **test credential becomes persistent** — must have a preventive control, a detective signal, a negative fixture and a response action.
...
- native scenario: compile environment topology and version lock
- native scenario: provision databases, brokers, caches and services
- native scenario: separate native, emulated and simulated components
- native scenario: inject network and dependency faults
- native scenario: destroy safely with evidence and cost accounting
