---
name: elmos-temporal-knowledge-memory-conflict-resolver
description: Resolve contradictory, stale and time-scoped facts across documents, tools, user memory and organizational knowledge with provenance and policy.
version: 4.0.0
...
# Temporal Knowledge and Memory Conflict Resolver
...
- `TemporalKnowledgeAndMemoryConflictResolverProfile`
- `TemporalKnowledgeAndMemoryConflictResolverPlan`
- `TemporalKnowledgeAndMemoryConflictResolverRun`
- `TemporalKnowledgeAndMemoryConflictResolverDecision`

Required semantic capabilities:

- represent valid-time and transaction-time facts
- score source authority and freshness
- retain disagreement and counterevidence
- apply user/organization scope and consent
- generate abstention or review when unresolved
...
- `artifacts/elmos-temporal-knowledge-memory-conflict-resolver/profile.yaml`
- `artifacts/elmos-temporal-knowledge-memory-conflict-resolver/plan.json`
- `artifacts/elmos-temporal-knowledge-memory-conflict-resolver/result.json`
- `artifacts/elmos-temporal-knowledge-memory-conflict-resolver/evidence/`
...
- **newer low-trust source overwrites authority** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **tenant memory crosses scope** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **conflict hidden in summary** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **expired fact remains active** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **resolver fabricates compromise fact** — must have a preventive control, a detective signal, a negative fixture and a response action.
...
- native scenario: represent valid-time and transaction-time facts
- native scenario: score source authority and freshness
- native scenario: retain disagreement and counterevidence
- native scenario: apply user/organization scope and consent
- native scenario: generate abstention or review when unresolved
