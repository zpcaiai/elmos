---
name: elmos-runtime-sandbox-ebpf-behavior-verifier
description: Enforce and observe filesystem, process, network, syscall, device and resource behavior of untrusted generators, adapters and tools.
version: 4.0.0
...
# Runtime Sandbox and eBPF Behavior Verifier
...
- `RuntimeSandboxAndEBPFBehaviorVerifierProfile`
- `RuntimeSandboxAndEBPFBehaviorVerifierPlan`
- `RuntimeSandboxAndEBPFBehaviorVerifierRun`
- `RuntimeSandboxAndEBPFBehaviorVerifierDecision`

Required semantic capabilities:

- compile syscall/network/path policy
- run in gVisor/Kata/Firecracker-class isolation
- observe behavior with eBPF/Falco-class telemetry
- detect policy escape and covert egress
- seal violations and terminate safely
...
- `artifacts/elmos-runtime-sandbox-ebpf-behavior-verifier/profile.yaml`
- `artifacts/elmos-runtime-sandbox-ebpf-behavior-verifier/plan.json`
- `artifacts/elmos-runtime-sandbox-ebpf-behavior-verifier/result.json`
- `artifacts/elmos-runtime-sandbox-ebpf-behavior-verifier/evidence/`
...
- **sandbox profile broader than declared** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **host socket exposed** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **telemetry blind spot** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **eBPF probe leaks tenant data** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **kill leaves external side effect unsettled** — must have a preventive control, a detective signal, a negative fixture and a response action.
...
- native scenario: compile syscall/network/path policy
- native scenario: run in gVisor/Kata/Firecracker-class isolation
- native scenario: observe behavior with eBPF/Falco-class telemetry
- native scenario: detect policy escape and covert egress
- native scenario: seal violations and terminate safely
