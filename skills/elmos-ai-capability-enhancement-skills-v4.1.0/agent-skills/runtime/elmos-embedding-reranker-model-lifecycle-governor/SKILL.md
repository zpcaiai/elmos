---
name: elmos-embedding-reranker-model-lifecycle-governor
description: Govern embedding/reranker selection, versioning, dimensionality, index compatibility, re-embedding, evaluation and rollback.
version: 4.0.0
...
# Embedding and Reranker Model Lifecycle Governor
...
- `EmbeddingAndRerankerModelLifecycleGovernorProfile`
- `EmbeddingAndRerankerModelLifecycleGovernorPlan`
- `EmbeddingAndRerankerModelLifecycleGovernorRun`
- `EmbeddingAndRerankerModelLifecycleGovernorDecision`

Required semantic capabilities:

- register exact embedding and reranker profiles
- validate dimension, metric and tokenizer compatibility
- plan incremental re-embedding and dual indexes
- benchmark retrieval and grounding impact
- coordinate rollback and cache invalidation
...
- `artifacts/elmos-embedding-reranker-model-lifecycle-governor/profile.yaml`
- `artifacts/elmos-embedding-reranker-model-lifecycle-governor/plan.json`
- `artifacts/elmos-embedding-reranker-model-lifecycle-governor/result.json`
- `artifacts/elmos-embedding-reranker-model-lifecycle-governor/evidence/`
...
- **model alias changes dimensions** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **mixed embeddings share index** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **reranker score distribution shifts** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **re-embedding misses tombstones** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **rollback index already deleted** — must have a preventive control, a detective signal, a negative fixture and a response action.
...
- native scenario: register exact embedding and reranker profiles
- native scenario: validate dimension, metric and tokenizer compatibility
- native scenario: plan incremental re-embedding and dual indexes
- native scenario: benchmark retrieval and grounding impact
- native scenario: coordinate rollback and cache invalidation
