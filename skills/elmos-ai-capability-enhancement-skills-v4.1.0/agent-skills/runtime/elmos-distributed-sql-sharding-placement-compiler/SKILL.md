---
name: elmos-distributed-sql-sharding-placement-compiler
description: Compile sharding, locality, replica, consistency, rebalance, global index and distributed transaction semantics for distributed SQL targets.
version: 4.0.0
...
# Distributed SQL Sharding and Placement Compiler
...
- `DistributedSQLShardingAndPlacementCompilerProfile`
- `DistributedSQLShardingAndPlacementCompilerPlan`
- `DistributedSQLShardingAndPlacementCompilerRun`
- `DistributedSQLShardingAndPlacementCompilerDecision`

Required semantic capabilities:

- derive shard and locality keys from workload evidence
- model replica placement and failure domains
- compile global index and cross-shard transaction rules
- plan online rebalance and hotspot mitigation
- verify consistency under partition and failover
...
- `artifacts/elmos-distributed-sql-sharding-placement-compiler/profile.yaml`
- `artifacts/elmos-distributed-sql-sharding-placement-compiler/plan.json`
- `artifacts/elmos-distributed-sql-sharding-placement-compiler/result.json`
- `artifacts/elmos-distributed-sql-sharding-placement-compiler/evidence/`
...
- **hot key amplified** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **data sovereignty placement violated** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **cross-shard atomicity lost** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **rebalancing duplicates writes** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **replica lag hidden from certification** — must have a preventive control, a detective signal, a negative fixture and a response action.
...
- native scenario: derive shard and locality keys from workload evidence
- native scenario: model replica placement and failure domains
- native scenario: compile global index and cross-shard transaction rules
- native scenario: plan online rebalance and hotspot mitigation
- native scenario: verify consistency under partition and failover
