---
name: elmos-distributed-invariant-tla-alloy-verifier
description: Encode leases, fencing, outbox, replication, saga and multi-agent coordination models for bounded exhaustive verification.
version: 4.0.0
...
# Distributed Invariant TLA+/Alloy Verifier
...
- `DistributedInvariantTLAAlloyVerifierProfile`
- `DistributedInvariantTLAAlloyVerifierPlan`
- `DistributedInvariantTLAAlloyVerifierRun`
- `DistributedInvariantTLAAlloyVerifierDecision`

Required semantic capabilities:

- generate TLA+/Alloy model from runtime IR
- declare explicit assumptions and fairness
- check split-brain, stale write and ghost completion
- explore partition, retry and failover schedules
- translate traces into regression tests
...
- `artifacts/elmos-distributed-invariant-tla-alloy-verifier/profile.yaml`
- `artifacts/elmos-distributed-invariant-tla-alloy-verifier/plan.json`
- `artifacts/elmos-distributed-invariant-tla-alloy-verifier/result.json`
- `artifacts/elmos-distributed-invariant-tla-alloy-verifier/evidence/`
...
- **model diverges from implementation** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **bound too small for bug** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **fairness masks starvation** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **generated invariant tautological** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **model checker output misinterpreted** — must have a preventive control, a detective signal, a negative fixture and a response action.
...
- native scenario: generate TLA+/Alloy model from runtime IR
- native scenario: declare explicit assumptions and fairness
- native scenario: check split-brain, stale write and ghost completion
- native scenario: explore partition, retry and failover schedules
- native scenario: translate traces into regression tests
