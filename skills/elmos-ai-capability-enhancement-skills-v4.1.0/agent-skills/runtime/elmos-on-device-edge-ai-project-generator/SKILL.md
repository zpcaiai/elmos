---
name: elmos-on-device-edge-ai-project-generator
description: Generate Core ML, TensorFlow Lite, ONNX Runtime, llama.cpp and edge deployments with resource, privacy, update and offline behavior contracts.
version: 4.0.0
...
# On-Device and Edge AI Project Generator
...
- `OnDeviceAndEdgeAIProjectGeneratorProfile`
- `OnDeviceAndEdgeAIProjectGeneratorPlan`
- `OnDeviceAndEdgeAIProjectGeneratorRun`
- `OnDeviceAndEdgeAIProjectGeneratorDecision`

Required semantic capabilities:

- profile device CPU/GPU/NPU and memory
- convert and package portable model artifacts
- generate offline-first data and tool behavior
- implement signed staged model updates
- test thermal, battery and degraded network envelopes
...
- `elmos-model-quantization-hardware-compatibility-certifier` from `elmos-functional-assurance-certification-skills` — optional companion; required only for E4/E5/P05 certification.

## Primary Outputs

- `artifacts/elmos-on-device-edge-ai-project-generator/profile.yaml`
- `artifacts/elmos-on-device-edge-ai-project-generator/plan.json`
- `artifacts/elmos-on-device-edge-ai-project-generator/result.json`
- `artifacts/elmos-on-device-edge-ai-project-generator/evidence/`
...
- **device model differs from certified artifact** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **update rollback absent** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **sensitive data syncs unexpectedly** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **thermal throttling breaks SLO** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **unsupported operator silently approximated** — must have a preventive control, a detective signal, a negative fixture and a response action.
...
- native scenario: profile device CPU/GPU/NPU and memory
- native scenario: convert and package portable model artifacts
- native scenario: generate offline-first data and tool behavior
- native scenario: implement signed staged model updates
- native scenario: test thermal, battery and degraded network envelopes
