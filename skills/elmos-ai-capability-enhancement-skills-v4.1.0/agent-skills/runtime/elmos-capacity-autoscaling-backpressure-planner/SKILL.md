---
name: elmos-capacity-autoscaling-backpressure-planner
description: Plan end-to-end queues, concurrency, rate limits, autoscaling, load shedding and admission control across agents, tools, stores and models.
version: 4.0.0
...
# Capacity, Autoscaling and Backpressure Planner
...
- `CapacityAutoscalingAndBackpressurePlannerProfile`
- `CapacityAutoscalingAndBackpressurePlannerPlan`
- `CapacityAutoscalingAndBackpressurePlannerRun`
- `CapacityAutoscalingAndBackpressurePlannerDecision`

Required semantic capabilities:

- build queueing and dependency capacity model
- set per-tenant concurrency and rate limits
- coordinate scale signals and cooldown
- propagate backpressure and cancellation
- verify overload degradation and recovery
...
- `artifacts/elmos-capacity-autoscaling-backpressure-planner/profile.yaml`
- `artifacts/elmos-capacity-autoscaling-backpressure-planner/plan.json`
- `artifacts/elmos-capacity-autoscaling-backpressure-planner/result.json`
- `artifacts/elmos-capacity-autoscaling-backpressure-planner/evidence/`
...
- **autoscalers fight each other** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **queue grows without bound** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **load shedding drops committed work** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **tenant noisy neighbor** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **scale-to-zero breaks SLA** — must have a preventive control, a detective signal, a negative fixture and a response action.
...
- native scenario: build queueing and dependency capacity model
- native scenario: set per-tenant concurrency and rate limits
- native scenario: coordinate scale signals and cooldown
- native scenario: propagate backpressure and cancellation
- native scenario: verify overload degradation and recovery
