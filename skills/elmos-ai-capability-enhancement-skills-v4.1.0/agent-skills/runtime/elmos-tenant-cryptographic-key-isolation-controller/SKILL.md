---
name: elmos-tenant-cryptographic-key-isolation-controller
description: Generate and verify per-tenant envelope encryption, KMS/HSM policy, rotation, revocation, backup and cryptographic erasure.
version: 4.0.0
...
# Tenant Cryptographic Key Isolation Controller

## Objective

Generate and verify per-tenant envelope encryption, KMS/HSM policy, rotation, revocation, backup and cryptographic erasure.
...
- `TenantCryptographicKeyIsolationControllerProfile`
- `TenantCryptographicKeyIsolationControllerPlan`
- `TenantCryptographicKeyIsolationControllerRun`
- `TenantCryptographicKeyIsolationControllerDecision`

Required semantic capabilities:

- derive key hierarchy and purpose separation
- enforce tenant-scoped encrypt/decrypt authorization
- rotate keys and rewrap data online
- verify backup restore and key recovery
- support cryptographic erasure with evidence
...
- `artifacts/elmos-tenant-cryptographic-key-isolation-controller/profile.yaml`
- `artifacts/elmos-tenant-cryptographic-key-isolation-controller/plan.json`
- `artifacts/elmos-tenant-cryptographic-key-isolation-controller/result.json`
- `artifacts/elmos-tenant-cryptographic-key-isolation-controller/evidence/`
...
- **shared data key** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **KMS policy wildcard** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **rotation leaves stale plaintext** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **backup depends on deleted key** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **key ID leaks tenant relationship** — must have a preventive control, a detective signal, a negative fixture and a response action.
...
- native scenario: derive key hierarchy and purpose separation
- native scenario: enforce tenant-scoped encrypt/decrypt authorization
- native scenario: rotate keys and rewrap data online
- native scenario: verify backup restore and key recovery
- native scenario: support cryptographic erasure with evidence
