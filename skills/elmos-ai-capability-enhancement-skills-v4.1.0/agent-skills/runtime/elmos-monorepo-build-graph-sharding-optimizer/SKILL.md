---
name: elmos-monorepo-build-graph-sharding-optimizer
description: Partition million-file polyglot repositories into dependency-safe analysis, generation, build and verification shards with cache-aware critical-path scheduling.
version: 4.0.0
...
# Monorepo Build-Graph Sharding Optimizer
...
- `MonorepoBuildGraphShardingOptimizerProfile`
- `MonorepoBuildGraphShardingOptimizerPlan`
- `MonorepoBuildGraphShardingOptimizerRun`
- `MonorepoBuildGraphShardingOptimizerDecision`

Required semantic capabilities:

- construct cross-language build and ownership graph
- derive deterministic shard boundaries
- schedule critical path under tenant quotas
- reuse content-addressed artifacts safely
- merge shard evidence without coverage gaps
...
- `artifacts/elmos-monorepo-build-graph-sharding-optimizer/profile.yaml`
- `artifacts/elmos-monorepo-build-graph-sharding-optimizer/plan.json`
- `artifacts/elmos-monorepo-build-graph-sharding-optimizer/result.json`
- `artifacts/elmos-monorepo-build-graph-sharding-optimizer/evidence/`
...
- **hidden cross-shard dependency** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **cache poisoning across tenants** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **shard evidence double-counted** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **hotspot starves critical path** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **partial shard success reported complete** — must have a preventive control, a detective signal, a negative fixture and a response action.
...
- native scenario: construct cross-language build and ownership graph
- native scenario: derive deterministic shard boundaries
- native scenario: schedule critical path under tenant quotas
- native scenario: reuse content-addressed artifacts safely
- native scenario: merge shard evidence without coverage gaps
