---
name: elmos-query-plan-regression-autotuning-governor
description: Detect plan regressions and propose bounded statistics, index, partition or query changes under proof, rollback and workload constraints.
version: 4.0.0
...
# Query Plan Regression and Autotuning Governor
...
- `QueryPlanRegressionAndAutotuningGovernorProfile`
- `QueryPlanRegressionAndAutotuningGovernorPlan`
- `QueryPlanRegressionAndAutotuningGovernorRun`
- `QueryPlanRegressionAndAutotuningGovernorDecision`

Required semantic capabilities:

- capture normalized plan fingerprints
- detect cardinality and statistics sensitivity
- simulate bounded index/query alternatives
- shadow candidate plans on representative workload
- promote only with correctness and rollback evidence
...
- `elmos-database-performance-plan-certifier` from `elmos-functional-assurance-certification-skills` — optional companion; required only for E4/E5/P05 certification.

## Primary Outputs

- `artifacts/elmos-query-plan-regression-autotuning-governor/profile.yaml`
- `artifacts/elmos-query-plan-regression-autotuning-governor/plan.json`
- `artifacts/elmos-query-plan-regression-autotuning-governor/result.json`
- `artifacts/elmos-query-plan-regression-autotuning-governor/evidence/`
...
- **autotune changes query semantics** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **benchmark cache biases result** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **index write cost ignored** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **plan pin blocks future improvement** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **production statistics exfiltrated** — must have a preventive control, a detective signal, a negative fixture and a response action.
...
- native scenario: capture normalized plan fingerprints
- native scenario: detect cardinality and statistics sensitivity
- native scenario: simulate bounded index/query alternatives
- native scenario: shadow candidate plans on representative workload
- native scenario: promote only with correctness and rollback evidence
