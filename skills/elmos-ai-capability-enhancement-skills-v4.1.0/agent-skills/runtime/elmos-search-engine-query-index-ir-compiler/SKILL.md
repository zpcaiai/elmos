---
name: elmos-search-engine-query-index-ir-compiler
description: Compile analyzer, tokenizer, mapping, relevance, aggregation, vector-hybrid, alias and shard semantics across Elasticsearch/OpenSearch-class systems.
version: 4.0.0
...
# Search Engine Query and Index IR Compiler
...
- `SearchEngineQueryAndIndexIRCompilerProfile`
- `SearchEngineQueryAndIndexIRCompilerPlan`
- `SearchEngineQueryAndIndexIRCompilerRun`
- `SearchEngineQueryAndIndexIRCompilerDecision`

Required semantic capabilities:

- model mappings, analyzers and token filters
- compile query DSL and scoring semantics
- represent index aliases and zero-downtime reindex
- capture shard, replica and refresh behavior
- verify hybrid lexical/vector retrieval
...
- `artifacts/elmos-search-engine-query-index-ir-compiler/profile.yaml`
- `artifacts/elmos-search-engine-query-index-ir-compiler/plan.json`
- `artifacts/elmos-search-engine-query-index-ir-compiler/result.json`
- `artifacts/elmos-search-engine-query-index-ir-compiler/evidence/`
...
- **analyzer drift** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **mapping coercion hides data loss** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **relevance score compared as absolute** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **alias cutover races writes** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **refresh interval changes consistency** — must have a preventive control, a detective signal, a negative fixture and a response action.
...
- native scenario: model mappings, analyzers and token filters
- native scenario: compile query DSL and scoring semantics
- native scenario: represent index aliases and zero-downtime reindex
- native scenario: capture shard, replica and refresh behavior
- native scenario: verify hybrid lexical/vector retrieval
