---
name: elmos-confidential-computing-attestation-controller
description: Verify enclave/TEE identity, measurement, nonce freshness, workload/image binding, key release and evidence appraisal for sensitive execution.
version: 4.0.0
...
# Confidential Computing Attestation Controller
...
- `ConfidentialComputingAttestationControllerProfile`
- `ConfidentialComputingAttestationControllerPlan`
- `ConfidentialComputingAttestationControllerRun`
- `ConfidentialComputingAttestationControllerDecision`

Required semantic capabilities:

- request and validate remote attestation evidence
- bind measurement to signed workload and policy
- release secrets only after appraisal
- rotate trust roots and handle revocation
- record TCB and degraded non-TEE path
...
- `elmos-runtime-admission-attestation-controller` from `elmos-functional-assurance-certification-skills` — optional companion; required only for E4/E5/P05 certification.
- `elmos-customer-vpc-runner-attestation-certifier` from `elmos-functional-assurance-certification-skills` — optional companion; required only for E4/E5/P05 certification.

## Primary Outputs

- `artifacts/elmos-confidential-computing-attestation-controller/profile.yaml`
- `artifacts/elmos-confidential-computing-attestation-controller/plan.json`
- `artifacts/elmos-confidential-computing-attestation-controller/result.json`
- `artifacts/elmos-confidential-computing-attestation-controller/evidence/`
...
- **replayed attestation** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **measurement allowlist stale** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **secret released before appraisal** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **debug mode weakens TEE** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **vendor attestation outage bypassed** — must have a preventive control, a detective signal, a negative fixture and a response action.
...
- native scenario: request and validate remote attestation evidence
- native scenario: bind measurement to signed workload and policy
- native scenario: release secrets only after appraisal
- native scenario: rotate trust roots and handle revocation
- native scenario: record TCB and degraded non-TEE path
