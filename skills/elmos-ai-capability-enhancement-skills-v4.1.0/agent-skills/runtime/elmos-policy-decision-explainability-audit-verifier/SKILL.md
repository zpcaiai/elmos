---
name: elmos-policy-decision-explainability-audit-verifier
description: Verify that authorization, safety, routing and certification decisions expose policy version, inputs, rationale and counterfactuals without leaking sensitive rules.
version: 4.0.0
...
# Policy Decision Explainability and Audit Verifier
...
- `PolicyDecisionExplainabilityAndAuditVerifierProfile`
- `PolicyDecisionExplainabilityAndAuditVerifierPlan`
- `PolicyDecisionExplainabilityAndAuditVerifierRun`
- `PolicyDecisionExplainabilityAndAuditVerifierDecision`

Required semantic capabilities:

- capture policy input and bundle digest
- produce human/machine explanation
- verify deterministic replay of decision
- redact sensitive attributes and rules
- link recourse and appeal workflow
...
- `artifacts/elmos-policy-decision-explainability-audit-verifier/profile.yaml`
- `artifacts/elmos-policy-decision-explainability-audit-verifier/plan.json`
- `artifacts/elmos-policy-decision-explainability-audit-verifier/result.json`
- `artifacts/elmos-policy-decision-explainability-audit-verifier/evidence/`
...
- **explanation differs from evaluated rule** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **secret attribute exposed** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **policy version missing** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **counterfactual suggests prohibited action** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **audit log mutable** — must have a preventive control, a detective signal, a negative fixture and a response action.
...
- native scenario: capture policy input and bundle digest
- native scenario: produce human/machine explanation
- native scenario: verify deterministic replay of decision
- native scenario: redact sensitive attributes and rules
- native scenario: link recourse and appeal workflow
