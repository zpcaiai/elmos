---
name: elmos-probabilistic-reliability-statistical-model-checker
description: Verify probabilistic availability, retry, queue, provider and agent success claims with stochastic models and simulation evidence.
version: 4.0.0
...
# Probabilistic Reliability and Statistical Model Checker
...
- `ProbabilisticReliabilityAndStatisticalModelCheckerProfile`
- `ProbabilisticReliabilityAndStatisticalModelCheckerPlan`
- `ProbabilisticReliabilityAndStatisticalModelCheckerRun`
- `ProbabilisticReliabilityAndStatisticalModelCheckerDecision`

Required semantic capabilities:

- construct Markov/queue/reliability model
- estimate rare-event and tail probabilities
- calibrate model against observed telemetry
- run sensitivity and uncertainty analysis
- bind bounded claims to operating envelope
...
- `elmos-capacity-soak-efficiency-certifier` from `elmos-functional-assurance-certification-skills` — optional companion; required only for E4/E5/P05 certification.

## Primary Outputs

- `artifacts/elmos-probabilistic-reliability-statistical-model-checker/profile.yaml`
- `artifacts/elmos-probabilistic-reliability-statistical-model-checker/plan.json`
- `artifacts/elmos-probabilistic-reliability-statistical-model-checker/result.json`
- `artifacts/elmos-probabilistic-reliability-statistical-model-checker/evidence/`
...
- **independence assumption false** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **rare event under-sampled** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **stationary model used during incident** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **telemetry selection bias** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **probability shown as guarantee** — must have a preventive control, a detective signal, a negative fixture and a response action.
...
- native scenario: construct Markov/queue/reliability model
- native scenario: estimate rare-event and tail probabilities
- native scenario: calibrate model against observed telemetry
- native scenario: run sensitivity and uncertainty analysis
- native scenario: bind bounded claims to operating envelope
