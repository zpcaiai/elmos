---
name: elmos-protocol-conformance-fuzzing-controller
description: Generate grammar, stateful and adversarial fuzz campaigns for MCP, A2A, ACP, HTTP, gRPC, WebSocket and event protocols.
version: 4.0.0
...
# Protocol Conformance Fuzzing Controller
...
- `ProtocolConformanceFuzzingControllerProfile`
- `ProtocolConformanceFuzzingControllerPlan`
- `ProtocolConformanceFuzzingControllerRun`
- `ProtocolConformanceFuzzingControllerDecision`

Required semantic capabilities:

- derive generators from protocol schemas and state machines
- fuzz framing, negotiation, cancellation and extensions
- inject malformed and oversized payloads
- minimize crashes and semantic violations
- promote protocol counterexamples to regression
...
- `artifacts/elmos-protocol-conformance-fuzzing-controller/profile.yaml`
- `artifacts/elmos-protocol-conformance-fuzzing-controller/plan.json`
- `artifacts/elmos-protocol-conformance-fuzzing-controller/result.json`
- `artifacts/elmos-protocol-conformance-fuzzing-controller/evidence/`
...
- **fuzzer escapes sandbox** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **valid extension rejected** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **resource exhaustion not bounded** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **security crash loses evidence** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **minimizer changes causal sequence** — must have a preventive control, a detective signal, a negative fixture and a response action.
...
- native scenario: derive generators from protocol schemas and state machines
- native scenario: fuzz framing, negotiation, cancellation and extensions
- native scenario: inject malformed and oversized payloads
- native scenario: minimize crashes and semantic violations
- native scenario: promote protocol counterexamples to regression
