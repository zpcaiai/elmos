---
name: elmos-cdc-exactly-once-ordering-verifier
description: Verify source log capture, snapshot boundary, schema change, ordering, deduplication, tombstone and offset recovery for CDC pipelines.
version: 4.0.0
...
# CDC Exactly-Once and Ordering Verifier
...
- `CDCExactlyOnceAndOrderingVerifierProfile`
- `CDCExactlyOnceAndOrderingVerifierPlan`
- `CDCExactlyOnceAndOrderingVerifierRun`
- `CDCExactlyOnceAndOrderingVerifierDecision`

Required semantic capabilities:

- align snapshot and log positions
- inject duplicate, late and out-of-order events
- verify DDL and schema-change handling
- recover offsets after connector failure
- reconcile deletes and tombstones
...
- `artifacts/elmos-cdc-exactly-once-ordering-verifier/profile.yaml`
- `artifacts/elmos-cdc-exactly-once-ordering-verifier/plan.json`
- `artifacts/elmos-cdc-exactly-once-ordering-verifier/result.json`
- `artifacts/elmos-cdc-exactly-once-ordering-verifier/evidence/`
...
- **snapshot/stream gap** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **offset committed before sink** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **DDL event ignored** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **delete becomes update** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **connector failover replays non-idempotent effect** — must have a preventive control, a detective signal, a negative fixture and a response action.
...
- native scenario: align snapshot and log positions
- native scenario: inject duplicate, late and out-of-order events
- native scenario: verify DDL and schema-change handling
- native scenario: recover offsets after connector failure
- native scenario: reconcile deletes and tombstones
