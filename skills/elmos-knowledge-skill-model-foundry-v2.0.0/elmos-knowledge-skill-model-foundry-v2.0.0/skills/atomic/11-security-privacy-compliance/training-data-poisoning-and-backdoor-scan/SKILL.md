---
name: training-data-poisoning-and-backdoor-scan
description: Use this skill when Elmos must perform training data poisoning and backdoor scan as part of the 11-security-privacy-compliance production workflow, with typed contracts, policy enforcement, evidence capture, and rollback.
license: Proprietary-Elmos-Commercial
compatibility: Elmos v3 harness; Cross-Cutting Trust Plane; policy and evidence services required.
metadata:
  version: "2.0.0"
  pack: 11-security-privacy-compliance
  priority: P0
  exposure: atomic-registry-only
allowed-tools: identity.verify policy.evaluate sandbox.enforce artifact.verify audit.write
---

# training-data-poisoning-and-backdoor-scan

## 能力目标

检测异常模式、触发器、标签投毒、来源集中和行为后门。
...
- 必须门：least-privilege, tenant-isolation, signed-artifact, audit-complete, critical-risk-zero；
