...
triggers:
  - "Use when implementing or executing `elmos-failure-reducer-minimizer`."
  - "Use when a migration/certification DAG requires `fuzzing` capability."
outputs:
  - "semantic-assurance/elmos-failure-reducer-minimizer/model.json"
  - "semantic-assurance/elmos-failure-reducer-minimizer/evidence.json"
  - "semantic-assurance/elmos-failure-reducer-minimizer/diagnostics.json"
---

# Failure Reducer & Minimizer

## Objective

Automatically minimize failing source programs, inputs, repository slices and traces while preserving the semantic mismatch oracle.
...
- [ ] failure preserved after reduction.
- [ ] dependency-aware repo slice.
- [ ] minimal input seed.
