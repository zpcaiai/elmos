...
description: Replay success, failure, retry and partial-update scenarios to prove
  equivalent commit, rollback, locking and idempotency behavior.
version: 1.0.0
skill_id: ELMOS-POLY-156
layer: validation
risk: high
readiness: not-run
dependencies:
...
triggers:
- Use when implementing or executing `elmos-transaction-commit-rollback-validator`.
- Use when the migration DAG requires `validation` capability.
outputs:
- validation/elmos-transaction-commit-rollback-validator/result.json
- validation/elmos-transaction-commit-rollback-validator/evidence/
---

# Transaction Commit Rollback Validator

## Objective

Replay success, failure, retry and partial-update scenarios to prove equivalent commit, rollback, locking and idempotency behavior.
