---
name: elmos-batch-scheduler-migrator
description: Convert JCL/CL/legacy schedulers into workflow/job definitions while
  preserving dependencies, datasets, return codes, checkpoints and restart.
version: 1.0.0
skill_id: ELMOS-POLY-113
...
triggers:
- Use when implementing or executing `elmos-batch-scheduler-migrator`.
- Use when the migration DAG requires `runtime-transformation` capability.
outputs:
- transforms/elmos-batch-scheduler-migrator/plan.json
- transforms/elmos-batch-scheduler-migrator/semantic-loss.json
- transforms/elmos-batch-scheduler-migrator/evidence.json
---

# Batch Scheduler Migrator

## Objective

Convert JCL/CL/legacy schedulers into workflow/job definitions while preserving dependencies, datasets, return codes, checkpoints and restart.
