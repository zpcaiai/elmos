---
name: elmos-adapter-ada-spark
description: Implement Ada/SPARK language parsing, semantic lowering, framework/runtime
  mapping, target emission and verification for repository-scale modernization.
version: 1.0.0
skill_id: ELMOS-POLY-097
...
triggers:
- Use when implementing or executing `elmos-adapter-ada-spark`.
- Use when the migration DAG requires `technology-adapter` capability.
- Use when `ada-spark` is detected as source, target or repository support surface.
outputs:
- adapters/ada-spark/adapter-manifest.json
- adapters/ada-spark/semantic-mapping.yaml
- adapters/ada-spark/verification-profile.yaml
---
...
Implement Ada/SPARK language parsing, semantic lowering, framework/runtime mapping, target emission and verification for repository-scale modernization.
...
- Technology ID: `ada-spark`
- Display: Ada/SPARK
- Kind: language
- File/build markers: .adb, .ads, .gpr
- Build systems: GNAT, GPRbuild, Alire
- Preferred analyzers: Libadalang, GNAT semantic info
- Runtime/framework surface: Ada tasking, Ravenscar/Jorvik, SPARK contracts
- Native verification: gprbuild, GNATtest, GNATprove

### Semantic risk register

- tasking/rendezvous/protected objects.
- representation clauses.
- range/subtype constraints.
- real-time scheduling.
- proof obligations and contract strength.
