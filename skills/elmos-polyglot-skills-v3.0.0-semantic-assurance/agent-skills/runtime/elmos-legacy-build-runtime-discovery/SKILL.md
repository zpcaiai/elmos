---
name: elmos-legacy-build-runtime-discovery
description: Recover unavailable or implicit compilers, linkers, preprocessors, subsystem
  versions, copy/include paths, runtime libraries, database clients, job schedulers,
  and deployment assumptions.
version: 1.0.0
skill_id: ELMOS-POLY-069
layer: build-analysis
risk: high
readiness: not-run
dependencies:
- elmos-fixed-format-source-normalizer
triggers:
- Use when implementing or executing `elmos-legacy-build-runtime-discovery`.
- Use when the migration DAG requires `build-analysis` capability.
outputs:
- legacy-toolchain-inventory.json
- runtime-requirements.json
- reproduction-plan.md
---

# Legacy Build Runtime Discovery

## Objective

Recover unavailable or implicit compilers, linkers, preprocessors, subsystem versions, copy/include paths, runtime libraries, database clients, job schedulers, and deployment assumptions.
