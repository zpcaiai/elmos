---
name: elmos-native-abi-behavior-validator
description: Verify exported symbols, calling conventions, struct layout, callbacks,
  FFI ownership and binary interop across source/target builds.
version: 1.0.0
skill_id: ELMOS-POLY-162
layer: validation
risk: high
readiness: not-run
dependencies:
...
triggers:
- Use when implementing or executing `elmos-native-abi-behavior-validator`.
- Use when the migration DAG requires `validation` capability.
outputs:
- validation/elmos-native-abi-behavior-validator/result.json
- validation/elmos-native-abi-behavior-validator/evidence/
---

# Native Abi Behavior Validator

## Objective

Verify exported symbols, calling conventions, struct layout, callbacks, FFI ownership and binary interop across source/target builds.
