---
name: elmos-adapter-jcl
description: Implement JCL repository-dsl parsing, semantic lowering, framework/runtime
  mapping, target emission and verification for repository-scale modernization.
version: 1.0.0
skill_id: ELMOS-POLY-099
...
triggers:
- Use when implementing or executing `elmos-adapter-jcl`.
- Use when the migration DAG requires `technology-adapter` capability.
- Use when `jcl` is detected as source, target or repository support surface.
outputs:
- adapters/jcl/adapter-manifest.json
- adapters/jcl/semantic-mapping.yaml
- adapters/jcl/verification-profile.yaml
---
...
Implement JCL repository-dsl parsing, semantic lowering, framework/runtime mapping, target emission and verification for repository-scale modernization.
...
- Display: JCL
- Kind: repository-dsl
- File/build markers: .jcl
- Build systems: owner runtime/toolchain
- Preferred analyzers: JES/JCL parser
...
- job step ordering.
- DISP/dataset lifecycle.
- return-code conditionals.
- restart points.
