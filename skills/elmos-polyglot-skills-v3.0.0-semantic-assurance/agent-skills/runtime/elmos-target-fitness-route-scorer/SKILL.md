---
name: elmos-target-fitness-route-scorer
description: Rank destination languages/stacks by semantic fit, performance, operations,
  ecosystem, team constraints, coexistence and verification cost; reject low-fitness
  blind conversions.
version: 1.0.0
skill_id: ELMOS-POLY-083
layer: planning
risk: high
readiness: not-run
dependencies:
...
triggers:
- Use when implementing or executing `elmos-target-fitness-route-scorer`.
- Use when the migration DAG requires `planning` capability.
outputs:
- target-fitness.json
- route-scorecard.json
---

# Target Fitness Route Scorer

## Objective

Rank destination languages/stacks by semantic fit, performance, operations, ecosystem, team constraints, coexistence and verification cost; reject low-fitness blind conversions.
