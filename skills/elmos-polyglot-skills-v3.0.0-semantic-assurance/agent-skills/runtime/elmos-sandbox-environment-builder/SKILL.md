---
name: elmos-sandbox-environment-builder
description: "Create isolated, reproducible, disposable workspaces for untrusted source, generated code, builds, tests, and migration tools."
version: 1.0.0
skill_id: ELMOS-POLY-034
layer: runner
risk: critical
readiness: not-run
dependencies:
  - "elmos-trusted-runner-policy-controller"
triggers:
  - "Use when implementing or executing `elmos-sandbox-environment-builder`."
...
Create isolated, reproducible, disposable workspaces for untrusted source, generated code, builds, tests, and migration tools.
...
1. Select process, container, microVM, or private runner isolation based on risk.
2. Build from pinned base images or attested host toolchains.
3. Mount the immutable snapshot read-only and create an isolated writable worktree.
4. Apply user namespace, filesystem, device, syscall, capability, and network restrictions.
5. Configure package mirrors and deterministic clocks/locales where required.
6. Separate secrets, caches, outputs, and evidence mounts.
7. Collect resource and process telemetry.
8. Destroy writable state after artifact export unless checkpoint policy retains it.
...
- [ ] Sandbox escape regression tests pass for configured isolation..
- [ ] Outputs cannot write outside authorized mounts..
- [ ] A clean rebuild reproduces the environment identity..
