---
name: elmos-fortran-to-python-numerical-route
description: 'Execute the reference route: Fortran to Python scientific stack while
  retaining native kernels where required.'
version: 1.0.0
skill_id: ELMOS-POLY-149
layer: route-execution
risk: critical
readiness: not-run
dependencies:
- elmos-legacy-p0-orchestrator
- elmos-adapter-fortran
...
triggers:
- Use when implementing or executing `elmos-fortran-to-python-numerical-route`.
- Use when the migration DAG requires `route-execution` capability.
outputs:
- routes/elmos-fortran-to-python-numerical-route/run.json
- routes/elmos-fortran-to-python-numerical-route/migration-plan.json
- routes/elmos-fortran-to-python-numerical-route/evidence/
---

# Fortran To Python Numerical Route

## Objective

Execute the reference route: Fortran to Python scientific stack while retaining native kernels where required.
...
- Source: `fortran`
- Target: `python`
- Intent: Fortran to Python scientific stack while retaining native kernels where required
