---
name: elmos-safety-contract-ir-builder
description: Extract range constraints, contracts, assertions, real-time deadlines,
  tasking assumptions and proof obligations from C/Ada/SPARK and safety-critical code.
version: 1.0.0
skill_id: ELMOS-POLY-078
layer: ir
risk: critical
readiness: not-run
dependencies:
...
triggers:
- Use when implementing or executing `elmos-safety-contract-ir-builder`.
- Use when the migration DAG requires `ir` capability.
outputs:
- safety-contract-ir.json
- proof-obligations.json
---

# Safety Contract Ir Builder

## Objective

Extract range constraints, contracts, assertions, real-time deadlines, tasking assumptions and proof obligations from C/Ada/SPARK and safety-critical code.
