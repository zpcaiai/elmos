---
name: elmos-adapter-cobol-copybook
description: Implement COBOL Copybook repository-dsl parsing, semantic lowering, framework/runtime
  mapping, target emission and verification for repository-scale modernization.
version: 1.0.0
skill_id: ELMOS-POLY-102
...
triggers:
- Use when implementing or executing `elmos-adapter-cobol-copybook`.
- Use when the migration DAG requires `technology-adapter` capability.
- Use when `cobol-copybook` is detected as source, target or repository support surface.
outputs:
- adapters/cobol-copybook/adapter-manifest.json
- adapters/cobol-copybook/semantic-mapping.yaml
- adapters/cobol-copybook/verification-profile.yaml
---

# Adapter Cobol Copybook

## Objective

Implement COBOL Copybook repository-dsl parsing, semantic lowering, framework/runtime mapping, target emission and verification for repository-scale modernization.
...
- Technology ID: `cobol-copybook`
- Display: COBOL Copybook
- Kind: repository-dsl
- File/build markers: .cpy, .copy
- Build systems: owner runtime/toolchain
- Preferred analyzers: copybook parser
