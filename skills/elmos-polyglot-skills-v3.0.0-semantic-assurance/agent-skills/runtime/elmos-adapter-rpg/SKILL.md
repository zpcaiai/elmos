---
name: elmos-adapter-rpg
description: Implement RPG/RPGLE language parsing, semantic lowering, framework/runtime
  mapping, target emission and verification for repository-scale modernization.
version: 1.0.0
skill_id: ELMOS-POLY-087
...
triggers:
- Use when implementing or executing `elmos-adapter-rpg`.
- Use when the migration DAG requires `technology-adapter` capability.
- Use when `rpg` is detected as source, target or repository support surface.
outputs:
- adapters/rpg/adapter-manifest.json
- adapters/rpg/semantic-mapping.yaml
- adapters/rpg/verification-profile.yaml
---
...
Implement RPG/RPGLE language parsing, semantic lowering, framework/runtime mapping, target emission and verification for repository-scale modernization.
...
- Display: RPG/RPGLE
- Kind: language
- File/build markers: .rpg, .rpgle, .sqlrpgle
- Build systems: IBM i ILE, RDi builds
- Preferred analyzers: RPGLE parser, IBM i compiler diagnostics, tree-sitter/custom grammar
- Runtime/framework surface: DB2 for i, DDS, CL/CLLE, ILE service programs
- Native verification: IBM i compile, DB2i integration, job log comparison

### Semantic risk register

- fixed/free-form coexistence.
- indicator semantics.
- packed/zoned decimal.
- record-level IO.
- activation groups and commitment control.
