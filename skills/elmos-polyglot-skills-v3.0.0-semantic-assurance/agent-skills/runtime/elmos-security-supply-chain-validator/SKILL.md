---
name: elmos-security-supply-chain-validator
description: "Validate source and target code, dependencies, images, build provenance, licenses, secrets, and high-risk security behavior."
version: 1.0.0
skill_id: ELMOS-POLY-044
layer: validation
risk: critical
readiness: not-run
dependencies:
  - "elmos-performance-equivalence-validator"
triggers:
  - "Use when implementing or executing `elmos-security-supply-chain-validator`."
...
# Security Supply Chain Validator

## Objective

Validate source and target code, dependencies, images, build provenance, licenses, secrets, and high-risk security behavior.
...
1. Generate SBOMs for source, target, images, and deployable artifacts.
2. Scan dependencies, lockfiles, containers, infrastructure, and licenses.
3. Run secret, SAST, configuration, and targeted dynamic tests.
4. Correlate vulnerabilities with reachability and runtime exposure where evidence exists.
5. Verify artifact provenance, checksums, signatures, and build identity.
6. Test identity, authorization, tenant isolation, injection, SSRF, redirects, deserialization, upload, and sensitive logging according to threat model.
7. Track accepted risks with owner and expiry.
8. Do not equate scanner absence with security certification.
...
- [ ] Seeded secret and vulnerable dependency fixtures are found..
- [ ] Accepted risks expire and reopen..
- [ ] SBOM covers deployable artifacts..
- [ ] Scanner errors produce blocked/not-run, not pass..
