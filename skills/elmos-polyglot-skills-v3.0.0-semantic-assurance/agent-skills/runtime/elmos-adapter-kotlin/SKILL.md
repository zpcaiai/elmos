---
name: elmos-adapter-kotlin
description: "Implement Kotlin semantic and framework support for JVM, Android, Ktor, Spring, coroutines, Compose, and multiplatform boundaries."
version: 1.0.0
skill_id: ELMOS-POLY-050
...
triggers:
  - "Use when implementing or executing `elmos-adapter-kotlin`."
  - "Use when the current DAG node requires the technology-adapter capability."
  - "Use when `kotlin` is a source, target, interoperability, or modernization technology."
  - "Use when files or build roots matching .kt, .kts are detected."
outputs:
  - "adapters/kotlin/adapter-manifest.json"
  - "adapters/kotlin/parser-profile.yaml"
  - "adapters/kotlin/semantic-mapping.yaml"
  - "adapters/kotlin/verification-profile.yaml"
---
...
Implement Kotlin semantic and framework support for JVM, Android, Ktor, Spring, coroutines, Compose, and multiplatform boundaries.
...
- Technology registry entry `kotlin` and the detected native toolchain profile.
...
| Display name | Kotlin |
| Kind | language |
| File/build markers | `.kt`, `.kts` |
| Build systems | Gradle, Maven, Bazel |
| Preferred analyzers | Kotlin PSI, K2/FIR, tree-sitter-kotlin |
| Framework coverage | Spring Boot, Ktor, Android, Jetpack Compose, Kotlin Multiplatform |
| Native verification | `gradle test`, `kotlinc`, `detekt`, `binary compatibility validator` |
...
- extension and delegated properties.
- sealed/data/value classes.
- Kotlin-Java interop and default arguments.
- Compose lifecycle and state.
...
1. Detect Kotlin/JVM, Android, Kotlin Multiplatform, Gradle plugins, compiler version, source sets, and generated code.
2. Prefer Kotlin PSI and K2/FIR for typed analysis; retain syntax-only fallback with confidence loss.
3. Lower nullability, platform types, data/sealed/value classes, extension functions, delegates, lambdas with receiver, contracts, and generics into Semantic IR.
4. Represent suspend functions, coroutine scopes, dispatchers, flows, channels, cancellation, and structured concurrency explicitly.
5. Lower Spring/Ktor routing, serialization, DI, persistence, and Android/Compose lifecycle/state/navigation into Framework IR.
6. Preserve Java interoperability, synthetic methods, default arguments, annotations, and JVM signature constraints.
7. Emit target mappings only after resolving platform types and coroutine semantics.
8. Verify with Gradle, kotlinc, unit/instrumentation tests, Detekt, ABI tools, and coroutine virtual-time tests.
...
- [ ] Kotlin-Java mixed module.
- [ ] nullable/platform type boundary.
- [ ] coroutine cancellation and dispatcher behavior.
- [ ] sealed/data/value class mapping.
- [ ] Ktor and Spring endpoint contracts.
- [ ] Compose recomposition/state/navigation fixture.
