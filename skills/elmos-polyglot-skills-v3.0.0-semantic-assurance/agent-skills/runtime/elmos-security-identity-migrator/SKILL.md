---
name: elmos-security-identity-migrator
description: "Translate identity, authentication, authorization, session, token, cryptography, secret, and audit behavior under current security policy."
version: 1.0.0
skill_id: ELMOS-POLY-027
layer: security
risk: critical
readiness: not-run
dependencies:
  - "elmos-serialization-timezone-migrator"
triggers:
  - "Use when implementing or executing `elmos-security-identity-migrator`."
  - "Use when the current DAG node requires the security capability."
outputs:
  - "security-capability-map.json"
  - "target-security-layer/"
  - "threat-model.md"
  - "security-tests/"
---

# Security Identity Migrator

## Objective

Translate identity, authentication, authorization, session, token, cryptography, secret, and audit behavior under current security policy.
...
1. Inventory identities, trust boundaries, roles, claims, policies, sessions, tokens, keys, secrets, and privileged operations.
2. Map framework-specific security filters and middleware to target equivalents.
3. Preserve authorization semantics at endpoint, service, data, and field levels.
4. Replace obsolete cryptography only through approved migration and compatibility plans.
5. Keep secret values outside generated code, prompts, logs, diffs, and evidence.
6. Add CSRF, CORS, cookie, redirect, SSRF, injection, deserialization, and tenant-isolation controls where applicable.
7. Generate abuse, negative, and privilege-escalation tests.
8. Require security review for trust-boundary or cryptographic changes.

## Implementation Contract

- Security policy denies by default and uses least privilege.
- Secret values never enter source, prompts, logs, diffs, or evidence.
- Trust-boundary and cryptographic changes require security review.
...
- [ ] Cryptographic compatibility and rotation are tested..
