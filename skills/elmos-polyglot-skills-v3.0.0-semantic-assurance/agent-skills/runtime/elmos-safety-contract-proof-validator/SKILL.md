---
name: elmos-safety-contract-proof-validator
description: Check SPARK/GNATprove or equivalent contracts, assertions, ranges, WCET/deadline
  evidence and approved proof waivers for safety routes.
version: 1.0.0
skill_id: ELMOS-POLY-164
layer: validation
risk: high
readiness: not-run
dependencies:
...
triggers:
- Use when implementing or executing `elmos-safety-contract-proof-validator`.
- Use when the migration DAG requires `validation` capability.
outputs:
- validation/elmos-safety-contract-proof-validator/result.json
- validation/elmos-safety-contract-proof-validator/evidence/
---

# Safety Contract Proof Validator

## Objective

Check SPARK/GNATprove or equivalent contracts, assertions, ranges, WCET/deadline evidence and approved proof waivers for safety routes.
