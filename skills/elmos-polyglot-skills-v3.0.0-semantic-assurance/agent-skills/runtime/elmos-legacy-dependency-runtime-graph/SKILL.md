---
name: elmos-legacy-dependency-runtime-graph
description: Build symbol, include/copy, call, job-step, data, transaction, screen,
  dataset, program-to-program, remote-call, and runtime integration graphs.
version: 1.0.0
skill_id: ELMOS-POLY-070
...
triggers:
- Use when implementing or executing `elmos-legacy-dependency-runtime-graph`.
- Use when the migration DAG requires `semantic-analysis` capability.
outputs:
- legacy-dependency-graph.json
- job-call-graph.json
- data-flow-graph.json
---

# Legacy Dependency Runtime Graph

## Objective

Build symbol, include/copy, call, job-step, data, transaction, screen, dataset, program-to-program, remote-call, and runtime integration graphs.
