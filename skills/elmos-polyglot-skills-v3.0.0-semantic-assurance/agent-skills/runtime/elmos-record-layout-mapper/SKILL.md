---
name: elmos-record-layout-mapper
description: Map overlays, REDEFINES, unions, packed records, variable arrays and
  alignment into safe target schemas without silently changing bytes.
version: 1.0.0
skill_id: ELMOS-POLY-109
...
triggers:
- Use when implementing or executing `elmos-record-layout-mapper`.
- Use when the migration DAG requires `semantic-transformation` capability.
outputs:
- transforms/elmos-record-layout-mapper/plan.json
- transforms/elmos-record-layout-mapper/semantic-loss.json
- transforms/elmos-record-layout-mapper/evidence.json
---

# Record Layout Mapper

## Objective

Map overlays, REDEFINES, unions, packed records, variable arrays and alignment into safe target schemas without silently changing bytes.
