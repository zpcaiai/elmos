---
name: elmos-adapter-vb6
description: Implement Visual Basic 6 language parsing, semantic lowering, framework/runtime
  mapping, target emission and verification for repository-scale modernization.
version: 1.0.0
skill_id: ELMOS-POLY-089
...
triggers:
- Use when implementing or executing `elmos-adapter-vb6`.
- Use when the migration DAG requires `technology-adapter` capability.
- Use when `vb6` is detected as source, target or repository support surface.
outputs:
- adapters/vb6/adapter-manifest.json
- adapters/vb6/semantic-mapping.yaml
- adapters/vb6/verification-profile.yaml
---
...
Implement Visual Basic 6 language parsing, semantic lowering, framework/runtime mapping, target emission and verification for repository-scale modernization.
...
- Technology ID: `vb6`
- Display: Visual Basic 6
- Kind: language
- File/build markers: .vbp, .frm, .bas, .cls, .ctl
- Build systems: VB6 toolchain, MSBuild bridge scripts
- Preferred analyzers: VB6 grammar, form/resource parser, COM type library reader
- Runtime/framework surface: COM, ActiveX, ADO, DAO, Win32
- Native verification: legacy executable smoke, COM contract tests, UI workflow replay

### Semantic risk register

- default properties.
- Variants and coercion.
- On Error Resume Next.
- COM apartment/lifetime.
- binary form/resource dependencies.
