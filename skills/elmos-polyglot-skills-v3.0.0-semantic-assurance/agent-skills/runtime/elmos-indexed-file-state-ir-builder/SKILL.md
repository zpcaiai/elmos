---
name: elmos-indexed-file-state-ir-builder
description: Model VSAM, DBF/CDX, keyed/relative files, DDS physical/logical files
  and other indexed storage as logical state plus physical semantics.
version: 1.0.0
skill_id: ELMOS-POLY-076
layer: ir
risk: critical
readiness: not-run
dependencies:
...
triggers:
- Use when implementing or executing `elmos-indexed-file-state-ir-builder`.
- Use when the migration DAG requires `ir` capability.
outputs:
- indexed-state-ir.json
- access-path-ir.json
---

# Indexed File State Ir Builder

## Objective

Model VSAM, DBF/CDX, keyed/relative files, DDS physical/logical files and other indexed storage as logical state plus physical semantics.
