---
name: elmos-differential-behavior-validator
description: "Compare source and target behavior over the same scenarios with typed comparators, tolerances, causal attribution, and evidence."
version: 1.0.0
skill_id: ELMOS-POLY-038
layer: validation
risk: critical
readiness: not-run
dependencies:
  - "elmos-golden-master-recorder"
triggers:
  - "Use when implementing or executing `elmos-differential-behavior-validator`."
...
# Differential Behavior Validator

## Objective

Compare source and target behavior over the same scenarios with typed comparators, tolerances, causal attribution, and evidence.
...
1. Execute source and target against equivalent isolated fixtures.
2. Compare API, event, data, file, CLI, library, UI, side-effect, error, ordering, and timing observations.
3. Use comparator types for exact, unordered, numeric tolerance, temporal tolerance, semantic JSON, images, and domain invariants.
4. Separate expected target changes from regressions using approved decisions.
5. Cluster mismatches by likely root cause and affected contracts.
6. Re-run suspicious nondeterministic cases.
7. Calculate scenario and behavior coverage without inflating unobserved paths.
8. Emit pass, fail, blocked, or not-run per contract.
...
- [ ] Injected semantic differences are detected..
- [ ] Approved differences are traceable to decisions..
- [ ] Flaky comparisons do not become passes..
- [ ] Coverage excludes blocked and unobserved scenarios..
