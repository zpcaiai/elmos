---
name: elmos-long-task-checkpoint-recovery
description: "Make long-running transformation jobs resumable after client disconnects, process crashes, worker loss, and controlled cancellation."
version: 1.0.0
skill_id: ELMOS-POLY-036
layer: runner
risk: critical
readiness: not-run
dependencies:
  - "elmos-toolchain-provisioner"
triggers:
  - "Use when implementing or executing `elmos-long-task-checkpoint-recovery`."
...
# Long Task Checkpoint Recovery

## Objective

Make long-running transformation jobs resumable after client disconnects, process crashes, worker loss, and controlled cancellation.
...
1. Assign durable run and stage identities independent of client connections.
2. Persist input hashes, policies, toolchain identity, worktree state, outputs, and side-effect journal at checkpoints.
3. Use leases and fencing tokens to prevent concurrent duplicate workers.
4. Separate retryable computation from external side effects.
5. Require idempotency or compensation before retrying side effects.
6. Resume only when snapshot, policy, toolchain, and prerequisite evidence still match.
7. Support pause, cancel, retry, and operator recovery.
8. Store final artifacts server-side and return them when the client reconnects.
...
- [ ] Worker termination at every checkpoint resumes correctly..
- [ ] Duplicate workers cannot commit the same stage..
- [ ] Cancellation reaches a stable terminal state..
- [ ] Changed prerequisites invalidate unsafe resume..
