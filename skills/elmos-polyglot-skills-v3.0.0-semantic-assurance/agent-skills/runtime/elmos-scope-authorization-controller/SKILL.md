---
name: elmos-scope-authorization-controller
description: "Define exactly which repositories, modules, data, environments, routes, and tool permissions a run may use."
version: 1.0.0
skill_id: ELMOS-POLY-002
layer: governance
risk: critical
readiness: not-run
dependencies:
  []
triggers:
  - "Use when implementing or executing `elmos-scope-authorization-controller`."
  - "Use when the current DAG node requires the governance capability."
outputs:
  - "scope-boundary.yaml"
  - "authorization-manifest.json"
  - "data-handling-plan.json"
  - "execution-policy.json"
---

# Scope Authorization Controller

## Objective

Define exactly which repositories, modules, data, environments, routes, and tool permissions a run may use.
...
- No hard Skill dependency. The scope and runner policy are still mandatory at execution time.
...
1. Capture business scope, repositories, branches, modules, environments, and explicit exclusions.
2. Resolve read, write, network, secret, package-registry, database, and deployment permissions.
3. Classify source and runtime data by sensitivity and residency constraints.
4. Define model-routing policy for source code and confidential artifacts.
5. Bind approvals to run, snapshot, actor, scope, expiry, and reason.
6. Reject implicit privilege expansion by tools or generated commands.
7. Record every scope change as a new signed decision rather than mutating history.
8. Emit machine-readable policy for the trusted runner.

## Implementation Contract

- Policy evaluation must be deterministic and deny by default.
- Never persist secret values in manifests, prompts, traces, or evidence.
- Authorization changes create new versioned artifacts; they do not rewrite previous decisions.
...
- [ ] Expired approvals do not authorize execution..
- [ ] Secret values never appear in generated evidence..
- [ ] A changed repository or target environment requires a new authorization binding..
