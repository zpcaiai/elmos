---
name: elmos-adapter-rexx
description: Implement REXX repository-dsl parsing, semantic lowering, framework/runtime
  mapping, target emission and verification for repository-scale modernization.
version: 1.0.0
skill_id: ELMOS-POLY-100
...
triggers:
- Use when implementing or executing `elmos-adapter-rexx`.
- Use when the migration DAG requires `technology-adapter` capability.
- Use when `rexx` is detected as source, target or repository support surface.
outputs:
- adapters/rexx/adapter-manifest.json
- adapters/rexx/semantic-mapping.yaml
- adapters/rexx/verification-profile.yaml
---
...
Implement REXX repository-dsl parsing, semantic lowering, framework/runtime mapping, target emission and verification for repository-scale modernization.
...
- Display: REXX
- Kind: repository-dsl
- File/build markers: .rexx, .rex, .exec
- Build systems: owner runtime/toolchain
- Preferred analyzers: REXX interpreter/parser
