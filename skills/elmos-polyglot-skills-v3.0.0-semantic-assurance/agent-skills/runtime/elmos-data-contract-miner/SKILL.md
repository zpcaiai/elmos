---
name: elmos-data-contract-miner
description: "Recover schemas, invariants, ownership, transactions, migrations, caches, files, and message state required for equivalent migration."
version: 1.0.0
skill_id: ELMOS-POLY-009
layer: data-analysis
risk: critical
readiness: not-run
dependencies:
...
triggers:
  - "Use when implementing or executing `elmos-data-contract-miner`."
  - "Use when the current DAG node requires the data-analysis capability."
outputs:
  - "data-contract-catalog.json"
  - "schema-model.json"
  - "transaction-boundary-map.json"
...
Recover schemas, invariants, ownership, transactions, migrations, caches, files, and message state required for equivalent migration.
...
1. Parse database migrations, ORM mappings, queries, stored procedures, schema files, cache keys, file formats, and event payloads.
2. Model primary keys, constraints, defaults, collation, precision, time zone, nullability, ordering, and generated values.
3. Recover transaction boundaries, isolation assumptions, retry behavior, outbox/inbox patterns, and side effects.
4. Map state ownership and shared-database coupling.
5. Identify personally identifiable, regulated, encrypted, and residency-bound data by metadata and policy.
6. Generate read-only invariants and comparison queries.
7. Record unsupported database-specific behavior.
8. Produce a migration and rollback observability surface.

## Implementation Contract

- Schema and data inspection is read-only unless an approved migration stage is active.
- Sensitive values remain masked, hashed, tokenized, or enclave-bound.
- Transaction and state-ownership assumptions require evidence.
...
- [ ] Precision, nullability, collation, and timezone fixtures are preserved..
- [ ] Shared-table ownership conflicts are reported..
- [ ] Transaction and retry boundaries are not inferred as identical without evidence..
- [ ] Comparison queries are read-only by default..
