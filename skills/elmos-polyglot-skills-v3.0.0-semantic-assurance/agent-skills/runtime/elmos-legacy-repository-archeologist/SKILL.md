---
name: elmos-legacy-repository-archeologist
description: Recover code, copybooks, job control, screens, binary assets, schemas,
  generated artifacts, runtime dependencies, and operational knowledge from legacy
  repositories.
version: 1.0.0
skill_id: ELMOS-POLY-066
layer: discovery
risk: high
readiness: not-run
dependencies: []
triggers:
- Use when implementing or executing `elmos-legacy-repository-archeologist`.
- Use when the migration DAG requires `discovery` capability.
outputs:
- legacy-repository-inventory.json
- legacy-surface-map.json
- opaque-assets.json
---

# Legacy Repository Archeologist

## Objective

Recover code, copybooks, job control, screens, binary assets, schemas, generated artifacts, runtime dependencies, and operational knowledge from legacy repositories.
