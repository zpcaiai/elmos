---
name: elmos-target-profile-designer
description: "Define an explicit, testable target architecture and technology profile rather than letting generated code choose architecture implicitly."
version: 1.0.0
skill_id: ELMOS-POLY-014
layer: target-design
risk: critical
readiness: not-run
dependencies:
  - "elmos-framework-ir-builder"
triggers:
  - "Use when implementing or executing `elmos-target-profile-designer`."
  - "Use when the current DAG node requires the target-design capability."
outputs:
  - "target-profile.yaml"
  - "architecture-decisions/"
  - "target-capability-map.json"
  - "target-constraints.json"
---

# Target Profile Designer

## Objective

Define an explicit, testable target architecture and technology profile rather than letting generated code choose architecture implicitly.
...
1. Capture target technologies, versions, frameworks, package layout, deployment platform, data stores, observability, and security constraints.
2. Define module boundaries, API style, error model, nullability, async model, transaction policy, dependency policy, and coding standards.
3. Choose modernization, interoperability, incremental replacement, or full conversion boundaries.
4. Map source capabilities to target capabilities and identify gaps.
5. Declare non-functional budgets and compatibility tolerances.
6. Define generated-code ownership and regeneration policy.
7. Record alternatives and rejected options as ADRs.
8. Validate the target profile before planning work.

## Implementation Contract

- Target versions and framework choices must be explicit.
- Architecture decisions are recorded as ADRs.
- Capability gaps become mitigations, blockers, or approved scope changes.
...
- [ ] No target version is implicit..
- [ ] Capability gaps have mitigation or blocker status..
- [ ] Non-functional budgets are measurable..
- [ ] Rejected alternatives remain auditable..
