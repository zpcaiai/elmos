---
name: elmos-adapter-ibmi-cl
description: Implement IBM i CL/CLLE repository-dsl parsing, semantic lowering, framework/runtime
  mapping, target emission and verification for repository-scale modernization.
version: 1.0.0
skill_id: ELMOS-POLY-101
...
triggers:
- Use when implementing or executing `elmos-adapter-ibmi-cl`.
- Use when the migration DAG requires `technology-adapter` capability.
- Use when `ibmi-cl` is detected as source, target or repository support surface.
outputs:
- adapters/ibmi-cl/adapter-manifest.json
- adapters/ibmi-cl/semantic-mapping.yaml
- adapters/ibmi-cl/verification-profile.yaml
---
...
Implement IBM i CL/CLLE repository-dsl parsing, semantic lowering, framework/runtime mapping, target emission and verification for repository-scale modernization.
...
- Technology ID: `ibmi-cl`
- Display: IBM i CL/CLLE
- Kind: repository-dsl
- File/build markers: .cl, .clle
- Build systems: owner runtime/toolchain
- Preferred analyzers: IBM i CL compiler/parser
