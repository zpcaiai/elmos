---
name: elmos-coexistence-strangler-planner
description: Plan API wrapping, dual run, data replication, batch coexistence, module
  strangulation, rollback, cutover and decommissioning for incremental legacy modernization.
version: 1.0.0
skill_id: ELMOS-POLY-084
layer: planning
risk: high
readiness: not-run
dependencies:
...
triggers:
- Use when implementing or executing `elmos-coexistence-strangler-planner`.
- Use when the migration DAG requires `planning` capability.
outputs:
- coexistence-plan.json
- cutover-waves.json
- rollback-plan.json
---

# Coexistence Strangler Planner

## Objective

Plan API wrapping, dual run, data replication, batch coexistence, module strangulation, rollback, cutover and decommissioning for incremental legacy modernization.
