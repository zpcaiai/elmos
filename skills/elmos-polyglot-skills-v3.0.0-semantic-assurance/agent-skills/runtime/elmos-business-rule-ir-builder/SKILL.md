---
name: elmos-business-rule-ir-builder
description: Extract auditable business rules from procedural, 4GL, screen, job and
  database logic with source spans, predicates, effects, priority and exception semantics.
version: 1.0.0
skill_id: ELMOS-POLY-071
layer: ir
risk: critical
readiness: not-run
dependencies:
...
triggers:
- Use when implementing or executing `elmos-business-rule-ir-builder`.
- Use when the migration DAG requires `ir` capability.
outputs:
- business-rule-ir.json
- rule-provenance.json
---

# Business Rule Ir Builder

## Objective

Extract auditable business rules from procedural, 4GL, screen, job and database logic with source spans, predicates, effects, priority and exception semantics.
