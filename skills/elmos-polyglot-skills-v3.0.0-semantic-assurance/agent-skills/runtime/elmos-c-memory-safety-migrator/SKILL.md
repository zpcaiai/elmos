---
name: elmos-c-memory-safety-migrator
description: Transform C ownership, pointer arithmetic, allocation and lifetime patterns
  toward Rust/C++ safety models using evidence-backed unsafe boundaries.
version: 1.0.0
skill_id: ELMOS-POLY-118
layer: systems-transformation
risk: high
readiness: not-run
dependencies:
- elmos-semantic-loss-risk-estimator
triggers:
- Use when implementing or executing `elmos-c-memory-safety-migrator`.
- Use when the migration DAG requires `systems-transformation` capability.
outputs:
- transforms/elmos-c-memory-safety-migrator/plan.json
- transforms/elmos-c-memory-safety-migrator/semantic-loss.json
- transforms/elmos-c-memory-safety-migrator/evidence.json
---

# C Memory Safety Migrator

## Objective

Transform C ownership, pointer arithmetic, allocation and lifetime patterns toward Rust/C++ safety models using evidence-backed unsafe boundaries.
