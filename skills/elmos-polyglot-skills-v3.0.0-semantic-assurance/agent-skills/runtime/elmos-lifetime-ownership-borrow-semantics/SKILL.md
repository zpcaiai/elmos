...
version: 1.0.0
skill_id: ELMOS-POLY-194
layer: memory-type-semantics
risk: critical
readiness: not-run
dependencies:
  - "elmos-canonical-type-algebra"
triggers:
  - "Use when implementing or executing `elmos-lifetime-ownership-borrow-semantics`."
  - "Use when a migration/certification DAG requires `memory-type-semantics` capability."
outputs:
  - "semantic-assurance/elmos-lifetime-ownership-borrow-semantics/model.json"
  - "semantic-assurance/elmos-lifetime-ownership-borrow-semantics/evidence.json"
  - "semantic-assurance/elmos-lifetime-ownership-borrow-semantics/diagnostics.json"
---

# Lifetime, Ownership & Borrow Semantics

## Objective

Model ownership, borrowing, lifetimes, aliasing and move/copy semantics for safe translation between managed and native languages.
...
- [ ] Rust borrow fixture.
- [ ] C pointer ownership fixture.
- [ ] Swift ARC ownership fixture.
