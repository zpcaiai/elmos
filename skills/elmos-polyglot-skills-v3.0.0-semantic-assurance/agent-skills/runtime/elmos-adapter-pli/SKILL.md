---
name: elmos-adapter-pli
description: Implement PL/I language parsing, semantic lowering, framework/runtime
  mapping, target emission and verification for repository-scale modernization.
version: 1.0.0
skill_id: ELMOS-POLY-088
...
triggers:
- Use when implementing or executing `elmos-adapter-pli`.
- Use when the migration DAG requires `technology-adapter` capability.
- Use when `pli` is detected as source, target or repository support surface.
outputs:
- adapters/pli/adapter-manifest.json
- adapters/pli/semantic-mapping.yaml
- adapters/pli/verification-profile.yaml
---
...
Implement PL/I language parsing, semantic lowering, framework/runtime mapping, target emission and verification for repository-scale modernization.
...
- Display: PL/I
- Kind: language
- File/build markers: .pli, .pl1
- Build systems: IBM Enterprise PL/I, mainframe build pipelines
- Preferred analyzers: vendor compiler frontend, ANTLR/custom PL/I grammar
- Runtime/framework surface: CICS, IMS, DB2, MQ, batch
- Native verification: native compile, record/decimal equivalence, transaction fixtures

### Semantic risk register

- based/controlled storage.
- condition handling.
- picture/decimal semantics.
- preprocessor and include behavior.
- mainframe linkage/ABI.
