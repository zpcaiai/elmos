---
name: elmos-memory-safety-regression-validator
description: Run sanitizers, Miri/fuzzing/static analysis and leak/use-after-free/double-free
  fixtures for C/C++/Rust conversion routes.
version: 1.0.0
skill_id: ELMOS-POLY-163
layer: validation
risk: high
readiness: not-run
dependencies:
...
triggers:
- Use when implementing or executing `elmos-memory-safety-regression-validator`.
- Use when the migration DAG requires `validation` capability.
outputs:
- validation/elmos-memory-safety-regression-validator/result.json
- validation/elmos-memory-safety-regression-validator/evidence/
---

# Memory Safety Regression Validator

## Objective

Run sanitizers, Miri/fuzzing/static analysis and leak/use-after-free/double-free fixtures for C/C++/Rust conversion routes.
