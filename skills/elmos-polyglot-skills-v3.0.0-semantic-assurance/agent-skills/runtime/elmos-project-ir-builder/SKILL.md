---
name: elmos-project-ir-builder
description: "Construct a versioned Project IR describing modules, services, interfaces, state, workflows, deployment, tests, and ownership."
version: 1.0.0
skill_id: ELMOS-POLY-011
layer: ir
risk: critical
readiness: not-run
dependencies:
  - "elmos-behavior-contract-miner"
triggers:
  - "Use when implementing or executing `elmos-project-ir-builder`."
  - "Use when the current DAG node requires the ir capability."
outputs:
  - "project-ir.json"
  - "project-ir-index/"
  - "project-ir-diagnostics.json"
---
...
Construct a versioned Project IR describing modules, services, interfaces, state, workflows, deployment, tests, and ownership.
...
1. Create stable identities for repositories, modules, packages, services, endpoints, stores, queues, jobs, screens, and tests.
2. Link source, build, runtime, API, data, and behavior evidence.
3. Represent generated code, vendored code, opaque components, and unresolved relationships.
4. Preserve source spans and provenance for every IR node.
5. Validate references against the Project IR schema.
6. Compute coverage and confidence by subsystem.
7. Store extensions without breaking schema compatibility.
8. Version the IR and emit deterministic serialization.
...
- [ ] Every reference resolves or appears in diagnostics..
- [ ] Coverage and confidence are reported..
- [ ] Snapshot identity is embedded in the IR..
