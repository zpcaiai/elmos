---
name: elmos-adapter-ibmi-dds
description: Implement IBM i DDS repository-dsl parsing, semantic lowering, framework/runtime
  mapping, target emission and verification for repository-scale modernization.
version: 1.0.0
skill_id: ELMOS-POLY-105
...
triggers:
- Use when implementing or executing `elmos-adapter-ibmi-dds`.
- Use when the migration DAG requires `technology-adapter` capability.
- Use when `ibmi-dds` is detected as source, target or repository support surface.
outputs:
- adapters/ibmi-dds/adapter-manifest.json
- adapters/ibmi-dds/semantic-mapping.yaml
- adapters/ibmi-dds/verification-profile.yaml
---
...
Implement IBM i DDS repository-dsl parsing, semantic lowering, framework/runtime mapping, target emission and verification for repository-scale modernization.
...
- Technology ID: `ibmi-dds`
- Display: IBM i DDS
- Kind: repository-dsl
- File/build markers: .dds
- Build systems: owner runtime/toolchain
- Preferred analyzers: DDS parser
