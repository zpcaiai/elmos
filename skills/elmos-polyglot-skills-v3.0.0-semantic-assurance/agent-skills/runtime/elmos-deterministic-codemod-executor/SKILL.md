---
name: elmos-deterministic-codemod-executor
description: "Apply approved AST/CST/IR codemods reproducibly while preserving formatting, comments, source maps, and change provenance."
version: 1.0.0
skill_id: ELMOS-POLY-018
...
triggers:
  - "Use when implementing or executing `elmos-deterministic-codemod-executor`."
  - "Use when the current DAG node requires the transformation capability."
outputs:
  - "patches/"
  - "codemod-run.json"
  - "change-map.json"
  - "unmatched-register.json"
---

# Deterministic Codemod Executor

## Objective

Apply approved AST/CST/IR codemods reproducibly while preserving formatting, comments, source maps, and change provenance.
...
1. Load only rules compatible with source/target versions and current snapshot.
2. Run precondition queries and calculate the exact candidate set.
3. Apply edits transactionally in an isolated worktree.
4. Preserve comments, formatting intent, encoding, line endings, and generated-code boundaries.
5. Record every changed span, rule ID, before/after hash, and confidence.
6. Reparse changed files and reject syntactically invalid output.
7. Run idempotency and overlap checks.
8. Commit an auditable patch or roll back the worktree.
...
- [ ] Second application produces no additional diff..
- [ ] Changed files parse successfully..
- [ ] Generated or vendored paths are untouched unless explicitly authorized..
- [ ] Every change maps to an approved rule..
