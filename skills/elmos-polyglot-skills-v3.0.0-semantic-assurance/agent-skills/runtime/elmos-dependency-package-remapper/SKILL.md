---
name: elmos-dependency-package-remapper
description: "Map source libraries and package ecosystems to target equivalents using capability, license, security, maintenance, and exit-risk evidence."
version: 1.0.0
skill_id: ELMOS-POLY-020
layer: transformation
risk: critical
readiness: not-run
dependencies:
  - "elmos-bounded-agent-patch-executor"
triggers:
  - "Use when implementing or executing `elmos-dependency-package-remapper`."
...
# Dependency Package Remapper

## Objective

Map source libraries and package ecosystems to target equivalents using capability, license, security, maintenance, and exit-risk evidence.
...
1. Inventory direct and transitive dependencies by capability rather than package name alone.
2. Classify keep, upgrade, replace, wrap, reimplement, isolate, or block decisions.
3. Evaluate API fit, semantic gaps, license, CVEs, maintenance, platform support, and lock-in.
4. Prefer standard-library and well-supported target capabilities when requirements permit.
5. Generate compatibility wrappers where direct replacement would leak target details.
6. Update package manifests and lockfiles through native package managers.
7. Run vulnerability and license checks against the resolved target graph.
8. Record rejected alternatives and unresolved licensing decisions.
...
- [ ] Target builds use resolved lockfiles..
- [ ] License blockers remain blockers..
- [ ] Replacement capability tests cover critical behavior..
- [ ] No dependency is replaced solely because names look similar..
