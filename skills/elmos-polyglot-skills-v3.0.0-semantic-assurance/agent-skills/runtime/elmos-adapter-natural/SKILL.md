---
name: elmos-adapter-natural
description: Implement Natural language parsing, semantic lowering, framework/runtime
  mapping, target emission and verification for repository-scale modernization.
version: 1.0.0
skill_id: ELMOS-POLY-094
...
triggers:
- Use when implementing or executing `elmos-adapter-natural`.
- Use when the migration DAG requires `technology-adapter` capability.
- Use when `natural` is detected as source, target or repository support surface.
outputs:
- adapters/natural/adapter-manifest.json
- adapters/natural/semantic-mapping.yaml
- adapters/natural/verification-profile.yaml
---
...
Implement Natural language parsing, semantic lowering, framework/runtime mapping, target emission and verification for repository-scale modernization.
...
- Technology ID: `natural`
- Display: Natural
- Kind: language
- File/build markers: .nsn, .nsp, .nsg
- Build systems: Natural compiler
- Preferred analyzers: Natural source parser, map/DDM parser
- Runtime/framework surface: Adabas, Natural Security, EntireX
- Native verification: Natural compile, Adabas result comparison, transaction replay

### Semantic risk register

- Adabas DDM/multivalue fields.
- implicit database loop semantics.
- maps and terminal state.
- system variables.
- transaction/backout behavior.
