---
name: elmos-legacy-control-flow-ir-builder
description: Normalize GO TO, PERFORM ranges, ALTER, computed branches, error labels,
  event callbacks and irreducible control flow into explicit CFG/state-machine representations.
version: 1.0.0
skill_id: ELMOS-POLY-079
layer: ir
risk: critical
readiness: not-run
dependencies:
...
triggers:
- Use when implementing or executing `elmos-legacy-control-flow-ir-builder`.
- Use when the migration DAG requires `ir` capability.
outputs:
- legacy-control-flow-ir.json
- irreducible-regions.json
---

# Legacy Control Flow Ir Builder

## Objective

Normalize GO TO, PERFORM ranges, ALTER, computed branches, error labels, event callbacks and irreducible control flow into explicit CFG/state-machine representations.
