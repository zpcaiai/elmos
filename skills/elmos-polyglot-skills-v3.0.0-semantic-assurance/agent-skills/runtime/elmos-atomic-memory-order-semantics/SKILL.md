...
triggers:
  - "Use when implementing or executing `elmos-atomic-memory-order-semantics`."
  - "Use when a migration/certification DAG requires `concurrency-semantics` capability."
outputs:
  - "semantic-assurance/elmos-atomic-memory-order-semantics/model.json"
  - "semantic-assurance/elmos-atomic-memory-order-semantics/evidence.json"
  - "semantic-assurance/elmos-atomic-memory-order-semantics/diagnostics.json"
---
...
Map relaxed/acquire/release/seq-cst atomics and language-specific volatile primitives without strengthening/weakening silently.
...
- [ ] store-buffer litmus.
- [ ] CAS loop fixture.
- [ ] Java volatile→C++ atomic fixture.
