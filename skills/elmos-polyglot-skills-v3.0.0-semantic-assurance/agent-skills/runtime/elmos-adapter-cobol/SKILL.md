---
name: elmos-adapter-cobol
description: Implement COBOL language parsing, semantic lowering, framework/runtime
  mapping, target emission and verification for repository-scale modernization.
version: 1.0.0
skill_id: ELMOS-POLY-086
...
triggers:
- Use when implementing or executing `elmos-adapter-cobol`.
- Use when the migration DAG requires `technology-adapter` capability.
- Use when `cobol` is detected as source, target or repository support surface.
outputs:
- adapters/cobol/adapter-manifest.json
- adapters/cobol/semantic-mapping.yaml
- adapters/cobol/verification-profile.yaml
---
...
Implement COBOL language parsing, semantic lowering, framework/runtime mapping, target emission and verification for repository-scale modernization.
...
- Display: COBOL
- Kind: language
- File/build markers: .cbl, .cob, .cpy
- Build systems: Enterprise COBOL, GnuCOBOL, mainframe build pipelines
- Preferred analyzers: vendor compiler frontend, ANTLR/tree-sitter COBOL, copybook-aware parser
- Runtime/framework surface: CICS, IMS, DB2, VSAM, MQ, batch
- Native verification: native compile, record-layout byte tests, CICS/DB2 integration fixtures

### Semantic risk register

- fixed-format and dialect variance.
- EBCDIC/ASCII conversion.
- COMP/COMP-3 arithmetic.
- REDEFINES/OCCURS DEPENDING ON.
