---
name: elmos-source-encoding-normalizer
description: Detect and normalize EBCDIC/code pages, newline modes, tab/column rules,
  source encodings, and binary/text boundaries without losing byte provenance.
version: 1.0.0
skill_id: ELMOS-POLY-067
layer: normalization
risk: high
readiness: not-run
dependencies:
- elmos-legacy-repository-archeologist
triggers:
- Use when implementing or executing `elmos-source-encoding-normalizer`.
- Use when the migration DAG requires `normalization` capability.
outputs:
- encoding-inventory.json
- normalized-source-map.json
- byte-provenance.json
---

# Source Encoding Normalizer

## Objective

Detect and normalize EBCDIC/code pages, newline modes, tab/column rules, source encodings, and binary/text boundaries without losing byte provenance.
