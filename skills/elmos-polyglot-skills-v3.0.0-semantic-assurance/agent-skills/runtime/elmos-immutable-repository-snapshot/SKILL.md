---
name: elmos-immutable-repository-snapshot
description: "Create a content-addressed, replayable source snapshot that prevents analysis and transformation from drifting across revisions."
version: 1.0.0
skill_id: ELMOS-POLY-003
layer: discovery
risk: critical
readiness: not-run
dependencies:
  - "elmos-scope-authorization-controller"
triggers:
  - "Use when implementing or executing `elmos-immutable-repository-snapshot`."
...
Create a content-addressed, replayable source snapshot that prevents analysis and transformation from drifting across revisions.
...
1. Resolve repository URL or local root, commit, branch, submodules, sparse paths, and Git LFS objects.
2. Refuse dirty working trees unless the uncommitted diff is explicitly captured.
3. Hash source, build files, lockfiles, generated-code declarations, and tool configuration.
4. Capture line-ending, executable-bit, symlink, and case-sensitivity metadata.
5. Record unavailable objects and shallow-history limitations.
6. Create a read-only snapshot workspace.
7. Verify the snapshot hash before every downstream stage.
8. Emit a provenance record suitable for evidence signing.
...
- [ ] Two snapshots of the same commit produce the same identity..
- [ ] Dirty changes change the snapshot identity..
- [ ] Submodule drift is detected..
- [ ] Tampering with a snapshotted file blocks downstream execution..
