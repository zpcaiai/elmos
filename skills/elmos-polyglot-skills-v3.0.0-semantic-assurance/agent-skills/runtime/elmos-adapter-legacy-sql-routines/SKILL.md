---
name: elmos-adapter-legacy-sql-routines
description: Implement Legacy SQL/Routine Surface repository-dsl parsing, semantic
  lowering, framework/runtime mapping, target emission and verification for repository-scale
  modernization.
version: 1.0.0
skill_id: ELMOS-POLY-106
...
triggers:
- Use when implementing or executing `elmos-adapter-legacy-sql-routines`.
- Use when the migration DAG requires `technology-adapter` capability.
- Use when `legacy-sql-routines` is detected as source, target or repository support
  surface.
outputs:
- adapters/legacy-sql-routines/adapter-manifest.json
- adapters/legacy-sql-routines/semantic-mapping.yaml
- adapters/legacy-sql-routines/verification-profile.yaml
---

# Adapter Legacy Sql Routines

## Objective

Implement Legacy SQL/Routine Surface repository-dsl parsing, semantic lowering, framework/runtime mapping, target emission and verification for repository-scale modernization.
...
- Technology ID: `legacy-sql-routines`
- Display: Legacy SQL/Routine Surface
- Kind: repository-dsl
- File/build markers: .sql, .ddl, .proc
- Build systems: owner runtime/toolchain
- Preferred analyzers: dialect parser, vendor catalog introspection
