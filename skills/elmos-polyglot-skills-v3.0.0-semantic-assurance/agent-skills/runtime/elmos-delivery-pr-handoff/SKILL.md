---
name: elmos-delivery-pr-handoff
description: "Deliver reviewable commits, pull requests, release artifacts, documentation, ownership, and evidence without hiding residual work."
version: 1.0.0
skill_id: ELMOS-POLY-048
layer: delivery
risk: high
readiness: not-run
dependencies:
...
triggers:
  - "Use when implementing or executing `elmos-delivery-pr-handoff`."
  - "Use when the current DAG node requires the delivery capability."
outputs:
...
Deliver reviewable commits, pull requests, release artifacts, documentation, ownership, and evidence without hiding residual work.
...
1. Partition changes into reviewable commits or pull requests aligned with the migration DAG.
2. Include objective, scope, architecture decisions, generated/manual code boundaries, tests, evidence, risks, and rollback.
3. Avoid committing secrets, local caches, binaries, or unrelated formatting.
4. Generate setup, build, test, deploy, operate, and troubleshoot documentation.
5. Assign owners for target code, generated rules, tests, dependencies, runbooks, and residual risks.
6. Verify clean checkout and reproducible build.
7. Attach readiness decision without overstating certification.
8. Produce customer or team acceptance checklist.

## Implementation Contract

- Delivery must be reproducible from a clean checkout.
- Residual risks and not-run checks remain visible.
- No unrelated formatting, cache, binary, or secret is committed.
...
- [ ] Clean checkout reproduces build and tests..
- [ ] PR contains no out-of-scope changes..
- [ ] Residual risks and not-run checks are visible..
- [ ] Ownership and operational handoff are complete..
