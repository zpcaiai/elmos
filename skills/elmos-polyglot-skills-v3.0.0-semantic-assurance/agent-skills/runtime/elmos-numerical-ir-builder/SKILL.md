---
name: elmos-numerical-ir-builder
description: Extract numerical kernels, array/storage order, precision, BLAS/LAPACK
  calls, loop transformations and tolerated error envelopes for scientific conversion.
version: 1.0.0
skill_id: ELMOS-POLY-077
layer: ir
risk: critical
readiness: not-run
dependencies:
...
triggers:
- Use when implementing or executing `elmos-numerical-ir-builder`.
- Use when the migration DAG requires `ir` capability.
outputs:
- numerical-ir.json
- numerical-baseline.json
---

# Numerical Ir Builder

## Objective

Extract numerical kernels, array/storage order, precision, BLAS/LAPACK calls, loop transformations and tolerated error envelopes for scientific conversion.
