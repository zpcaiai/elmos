---
name: elmos-adapter-c
description: Implement C language parsing, semantic lowering, framework/runtime mapping,
  target emission and verification for repository-scale modernization.
version: 1.0.0
skill_id: ELMOS-POLY-085
...
triggers:
- Use when implementing or executing `elmos-adapter-c`.
- Use when the migration DAG requires `technology-adapter` capability.
- Use when `c` is detected as source, target or repository support surface.
outputs:
- adapters/c/adapter-manifest.json
- adapters/c/semantic-mapping.yaml
- adapters/c/verification-profile.yaml
---
...
Implement C language parsing, semantic lowering, framework/runtime mapping, target emission and verification for repository-scale modernization.
...
- Display: C
- Kind: language
- File/build markers: .c, .h
- Build systems: CMake, Make, Meson, Bazel
- Preferred analyzers: Clang AST, libclang, tree-sitter-c
- Runtime/framework surface: POSIX, Win32, embedded RTOS, native libraries
- Native verification: clang/gcc build, ctest, ASan, UBSan

### Semantic risk register

- pointer aliasing and provenance.
- manual memory ownership.
- undefined/implementation-defined behavior.
- preprocessor and conditional compilation.
- ABI and struct layout.
