---
name: elmos-transaction-monitor-migrator
description: Map CICS/IMS/SAP LUW/OpenEdge/IBM i transaction behavior to explicit
  modern units of work, retries, outbox/saga patterns and rollback.
version: 1.0.0
skill_id: ELMOS-POLY-112
layer: runtime-transformation
risk: high
readiness: not-run
dependencies:
- elmos-semantic-loss-risk-estimator
triggers:
- Use when implementing or executing `elmos-transaction-monitor-migrator`.
- Use when the migration DAG requires `runtime-transformation` capability.
outputs:
- transforms/elmos-transaction-monitor-migrator/plan.json
- transforms/elmos-transaction-monitor-migrator/semantic-loss.json
- transforms/elmos-transaction-monitor-migrator/evidence.json
---

# Transaction Monitor Migrator

## Objective

Map CICS/IMS/SAP LUW/OpenEdge/IBM i transaction behavior to explicit modern units of work, retries, outbox/saga patterns and rollback.
