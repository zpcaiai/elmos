---
name: elmos-adapter-abap
description: Implement ABAP language parsing, semantic lowering, framework/runtime
  mapping, target emission and verification for repository-scale modernization.
version: 1.0.0
skill_id: ELMOS-POLY-093
...
triggers:
- Use when implementing or executing `elmos-adapter-abap`.
- Use when the migration DAG requires `technology-adapter` capability.
- Use when `abap` is detected as source, target or repository support surface.
outputs:
- adapters/abap/adapter-manifest.json
- adapters/abap/semantic-mapping.yaml
- adapters/abap/verification-profile.yaml
---
...
Implement ABAP language parsing, semantic lowering, framework/runtime mapping, target emission and verification for repository-scale modernization.
...
- Display: ABAP
- Kind: language
- File/build markers: .abap, .asddls, .clas, .prog
- Build systems: ABAP Platform/ADT, ATC
- Preferred analyzers: ADT/ABAP parser, CDS DDL parser
- Runtime/framework surface: Open SQL, CDS, BAPI/RFC, IDoc, BAdI, Dynpro
- Native verification: ABAP Unit, ATC, contract replay

### Semantic risk register

- SAP LUW and update task.
- internal table semantics.
- implicit enhancements/user exits.
- authorization checks.
- dictionary/CDS coupling.
