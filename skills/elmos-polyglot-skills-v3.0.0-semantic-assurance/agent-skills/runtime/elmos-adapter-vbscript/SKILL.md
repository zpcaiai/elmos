---
name: elmos-adapter-vbscript
description: Implement VBScript language parsing, semantic lowering, framework/runtime
  mapping, target emission and verification for repository-scale modernization.
version: 1.0.0
skill_id: ELMOS-POLY-090
...
triggers:
- Use when implementing or executing `elmos-adapter-vbscript`.
- Use when the migration DAG requires `technology-adapter` capability.
- Use when `vbscript` is detected as source, target or repository support surface.
outputs:
- adapters/vbscript/adapter-manifest.json
- adapters/vbscript/semantic-mapping.yaml
- adapters/vbscript/verification-profile.yaml
---
...
Implement VBScript language parsing, semantic lowering, framework/runtime mapping, target emission and verification for repository-scale modernization.
...
- Technology ID: `vbscript`
- Display: VBScript
- Kind: language
- File/build markers: .vbs, .asp
- Build systems: Windows Script Host, IIS Classic ASP
- Preferred analyzers: VBScript parser, ASP mixed-template parser
- Runtime/framework surface: WSH, Classic ASP, COM/ADO
- Native verification: cscript/wscript fixtures, IIS characterization, HTTP differential tests

### Semantic risk register

- late binding and Variant coercion.
- global scope/include order.
- COM automation.
- ASP session/application state.
- error swallowing.
