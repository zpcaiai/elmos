---
name: elmos-legacy-performance-capacity-validator
description: Compare batch elapsed time, throughput, latency, memory, CPU, IO and
  peak-volume behavior against declared SLO/budget envelopes.
version: 1.0.0
skill_id: ELMOS-POLY-165
layer: validation
risk: high
readiness: not-run
dependencies:
...
triggers:
- Use when implementing or executing `elmos-legacy-performance-capacity-validator`.
- Use when the migration DAG requires `validation` capability.
outputs:
- validation/elmos-legacy-performance-capacity-validator/result.json
- validation/elmos-legacy-performance-capacity-validator/evidence/
---

# Legacy Performance Capacity Validator

## Objective

Compare batch elapsed time, throughput, latency, memory, CPU, IO and peak-volume behavior against declared SLO/budget envelopes.
