---
name: elmos-deployment-runtime-migrator
description: "Generate target runtime, container, infrastructure, configuration, release, rollback, and operations assets."
version: 1.0.0
skill_id: ELMOS-POLY-029
layer: deployment
risk: critical
readiness: not-run
dependencies:
  - "elmos-observability-logging-migrator"
triggers:
  - "Use when implementing or executing `elmos-deployment-runtime-migrator`."
  - "Use when the current DAG node requires the deployment capability."
outputs:
  - "deployment/"
  - "runtime-config-map.json"
  - "release-plan.yaml"
  - "rollback-runbook.md"
---

# Deployment Runtime Migrator

## Objective

Generate target runtime, container, infrastructure, configuration, release, rollback, and operations assets.
...
1. Map processes, ports, filesystems, schedules, queues, dependencies, probes, resource limits, and platform requirements.
2. Generate minimal, pinned, non-root runtime images where containers are used.
3. Separate build-time and runtime secrets and configuration.
4. Create health, readiness, startup, shutdown, drain, and migration hooks.
5. Define deployment strategy, canary metrics, rollback triggers, and data compatibility windows.
6. Generate infrastructure policy and least-privilege identities.
7. Run local or ephemeral environment deployment tests.
8. Capture operational handoff and disaster-recovery assumptions.

## Implementation Contract

- Runtime artifacts are pinned, least-privilege, and reproducible.
- Build and runtime secrets are separated.
- Deployment has readiness, shutdown, rollback, and recovery behavior.
...
- [ ] Target starts from a clean environment..
- [ ] Graceful shutdown and readiness tests pass..
- [ ] Rollback triggers are measurable..
- [ ] Images and dependencies are scanned and pinned..
