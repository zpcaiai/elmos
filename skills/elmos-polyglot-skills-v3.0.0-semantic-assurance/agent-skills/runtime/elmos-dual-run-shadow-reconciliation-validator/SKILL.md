---
name: elmos-dual-run-shadow-reconciliation-validator
description: Operate source and target in shadow/dual-run, normalize nondeterminism,
  reconcile outputs/state and quantify divergence before cutover.
version: 1.0.0
skill_id: ELMOS-POLY-167
layer: validation
risk: high
readiness: not-run
dependencies:
...
triggers:
- Use when implementing or executing `elmos-dual-run-shadow-reconciliation-validator`.
- Use when the migration DAG requires `validation` capability.
outputs:
- validation/elmos-dual-run-shadow-reconciliation-validator/result.json
- validation/elmos-dual-run-shadow-reconciliation-validator/evidence/
---

# Dual Run Shadow Reconciliation Validator

## Objective

Operate source and target in shadow/dual-run, normalize nondeterminism, reconcile outputs/state and quantify divergence before cutover.
