---
name: elmos-unified-evidence-packager
description: "Package source, decisions, transformations, tests, security, performance, approvals, and limitations into a tamper-evident evidence graph."
version: 1.0.0
skill_id: ELMOS-POLY-046
layer: evidence
risk: critical
readiness: not-run
dependencies:
  - "elmos-rollback-dual-run-planner"
triggers:
  - "Use when implementing or executing `elmos-unified-evidence-packager`."
  - "Use when the current DAG node requires the evidence capability."
outputs:
  - "evidence-bundle/"
  - "evidence-index.json"
  - "evidence-graph.json"
...
# Unified Evidence Packager

## Objective

Package source, decisions, transformations, tests, security, performance, approvals, and limitations into a tamper-evident evidence graph.
...
1. Assign stable evidence IDs and types.
2. Bind evidence to snapshot, target profile, route, toolchain, runner policy, run, stage, actor, and time.
3. Hash artifacts and record provenance and generation commands.
4. Link findings, decisions, approvals, tests, patches, builds, deployments, and residual risks.
5. Mark stale evidence when source, target, dependencies, tools, policies, or tests change.
6. Redact or encrypt sensitive payloads.
7. Generate human and machine-readable indexes.
8. Verify bundle integrity before handoff.

## Implementation Contract

- Evidence is content-addressed and bound to exact run inputs.
- Stale evidence cannot satisfy a gate.
- Sensitive artifacts use redaction, encryption, or restricted references.
...
- [ ] Stale evidence cannot satisfy a gate..
- [ ] Every gate claim points to supporting evidence..
