---
name: elmos-contract-test-generator
description: "Generate maintainable provider, consumer, schema, event, compatibility, and public API tests from recovered contracts."
version: 1.0.0
skill_id: ELMOS-POLY-039
layer: validation
risk: critical
readiness: not-run
dependencies:
...
triggers:
  - "Use when implementing or executing `elmos-contract-test-generator`."
  - "Use when the current DAG node requires the validation capability."
outputs:
  - "contract-tests/"
  - "contract-test-index.json"
...
Generate maintainable provider, consumer, schema, event, compatibility, and public API tests from recovered contracts.
...
1. Select high-confidence recovered API and data contracts.
2. Generate provider and consumer tests with stable fixtures.
3. Cover validation, defaults, optionality, unknown fields, errors, authentication, idempotency, pagination, and version negotiation.
4. For events, test schema, key, ordering, delivery, deduplication, and compatibility.
5. For libraries, test public symbols, signatures, generic constraints, and binary/source compatibility where relevant.
6. Keep generated tests readable and owned by a declared module.
7. Run fail-before-implementation where possible.
8. Record contracts that cannot be safely generated.
...
- [ ] Generated tests fail against an intentionally incompatible target..
- [ ] Fixtures are deterministic..
- [ ] Contract IDs map to recovered evidence..
- [ ] Tests do not assert undocumented guesses as facts..
