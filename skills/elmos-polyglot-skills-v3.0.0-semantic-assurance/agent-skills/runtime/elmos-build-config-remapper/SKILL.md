---
name: elmos-build-config-remapper
description: "Generate reproducible target build, workspace, package, code-generation, test, and CI configuration."
version: 1.0.0
skill_id: ELMOS-POLY-021
layer: transformation
risk: critical
readiness: not-run
dependencies:
  - "elmos-dependency-package-remapper"
triggers:
  - "Use when implementing or executing `elmos-build-config-remapper`."
...
# Build Config Remapper

## Objective

Generate reproducible target build, workspace, package, code-generation, test, and CI configuration.
...
1. Map source modules and generated-code steps into target build units.
2. Pin compiler, SDK, package manager, plugin, image, and system dependency versions.
3. Preserve build variants, feature flags, platform targets, resources, and native dependencies.
4. Separate production, test, development, and code-generation dependencies.
5. Create hermetic or mirrored dependency options for restricted environments.
6. Generate local and CI commands from the same build graph.
7. Add cache keys that include lockfiles and toolchain identity.
8. Run clean-room build reproduction.
...
- [ ] Clean build succeeds without undeclared local files..
- [ ] CI and local build graphs agree..
- [ ] Generated sources are reproducible..
- [ ] Lockfiles and toolchain versions are committed..
