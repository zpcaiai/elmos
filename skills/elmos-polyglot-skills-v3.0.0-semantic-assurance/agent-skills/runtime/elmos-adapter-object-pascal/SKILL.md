---
name: elmos-adapter-object-pascal
description: Implement Object Pascal/Delphi language parsing, semantic lowering, framework/runtime
  mapping, target emission and verification for repository-scale modernization.
version: 1.0.0
skill_id: ELMOS-POLY-091
...
triggers:
- Use when implementing or executing `elmos-adapter-object-pascal`.
- Use when the migration DAG requires `technology-adapter` capability.
- Use when `object-pascal` is detected as source, target or repository support surface.
outputs:
- adapters/object-pascal/adapter-manifest.json
- adapters/object-pascal/semantic-mapping.yaml
- adapters/object-pascal/verification-profile.yaml
---

# Adapter Object Pascal

## Objective

Implement Object Pascal/Delphi language parsing, semantic lowering, framework/runtime mapping, target emission and verification for repository-scale modernization.
...
- Technology ID: `object-pascal`
- Display: Object Pascal/Delphi
- Kind: language
- File/build markers: .pas, .dpr, .dfm, .fmx
- Build systems: Delphi compiler, MSBuild
- Preferred analyzers: Delphi compiler AST where available, Pascal parser, DFM/FMX resource parser
- Runtime/framework surface: VCL, FMX, RTL, COM, FireDAC
- Native verification: native build, form workflow tests, ABI/DB fixtures

### Semantic risk register

- managed/unmanaged string lifetime.
- properties/events/RTTI.
- VCL message loop.
- packed records and calling conventions.
- resource/form serialization.
