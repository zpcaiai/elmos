---
name: elmos-byte-record-layout-validator
description: Verify record sizes, offsets, overlays, packed fields, alignment, encodings
  and round-trip bytes between source and target representations.
version: 1.0.0
skill_id: ELMOS-POLY-154
layer: validation
risk: high
readiness: not-run
dependencies:
...
triggers:
- Use when implementing or executing `elmos-byte-record-layout-validator`.
- Use when the migration DAG requires `validation` capability.
outputs:
- validation/elmos-byte-record-layout-validator/result.json
- validation/elmos-byte-record-layout-validator/evidence/
---

# Byte Record Layout Validator

## Objective

Verify record sizes, offsets, overlays, packed fields, alignment, encodings and round-trip bytes between source and target representations.
