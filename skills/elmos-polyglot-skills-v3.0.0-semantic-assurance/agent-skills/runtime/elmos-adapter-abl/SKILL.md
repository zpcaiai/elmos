---
name: elmos-adapter-abl
description: Implement Progress OpenEdge ABL language parsing, semantic lowering,
  framework/runtime mapping, target emission and verification for repository-scale
  modernization.
version: 1.0.0
skill_id: ELMOS-POLY-095
...
triggers:
- Use when implementing or executing `elmos-adapter-abl`.
- Use when the migration DAG requires `technology-adapter` capability.
- Use when `abl` is detected as source, target or repository support surface.
outputs:
- adapters/abl/adapter-manifest.json
- adapters/abl/semantic-mapping.yaml
- adapters/abl/verification-profile.yaml
---
...
Implement Progress OpenEdge ABL language parsing, semantic lowering, framework/runtime mapping, target emission and verification for repository-scale modernization.
...
- Technology ID: `abl`
- Display: Progress OpenEdge ABL
- Kind: language
- File/build markers: .p, .w, .cls, .i
- Build systems: OpenEdge compiler, PCT/ABL build
- Preferred analyzers: ABL parser, schema/propath resolver
- Runtime/framework surface: OpenEdge DB, AppServer/PASOE, ADM2
- Native verification: ABL compile, OpenEdge transaction fixtures, API replay

### Semantic risk register

- record buffers and temp-tables.
- NO-ERROR/ERROR-STATUS semantics.
- transaction scoping.
- PROPATH/includes.
- persistent procedures and handles.
