---
name: elmos-adapter-python
description: "Implement Python analysis and conversion support that distinguishes proven semantics from dynamic runtime behavior."
version: 1.0.0
skill_id: ELMOS-POLY-051
...
triggers:
  - "Use when implementing or executing `elmos-adapter-python`."
  - "Use when the current DAG node requires the technology-adapter capability."
  - "Use when `python` is a source, target, interoperability, or modernization technology."
  - "Use when files or build roots matching .py, .pyi are detected."
outputs:
  - "adapters/python/adapter-manifest.json"
  - "adapters/python/parser-profile.yaml"
  - "adapters/python/semantic-mapping.yaml"
  - "adapters/python/verification-profile.yaml"
---
...
Implement Python analysis and conversion support that distinguishes proven semantics from dynamic runtime behavior.
...
- Technology registry entry `python` and the detected native toolchain profile.
...
| Display name | Python |
| Kind | language |
| File/build markers | `.py`, `.pyi` |
| Build systems | uv, Poetry, pip, PDM, setuptools |
| Preferred analyzers | CPython ast, LibCST, astroid, tree-sitter-python |
| Framework coverage | Django, FastAPI, Flask, Celery, SQLAlchemy |
| Native verification | `pytest`, `mypy`, `pyright`, `ruff` |

### Semantic risk register

- dynamic typing and monkey patching.
- metaclasses and descriptors.
- decorators and dependency injection.
- asyncio/event-loop semantics.
- import-time side effects.
- multiple packaging layouts.
- ORM/query behavior and implicit serialization.
...
1. Detect Python versions, package layout, virtual environments, uv/Poetry/PDM/pip metadata, lockfiles, native extensions, and test configuration.
2. Parse with CPython AST and LibCST; use mypy/pyright/type stubs and runtime traces to improve symbol and type confidence.
3. Lower functions, classes, protocols, dataclasses, generators, context managers, exceptions, decorators, descriptors, and type hints into Semantic IR.
4. Represent dynamic imports, monkey patches, metaclasses, eval/exec, import hooks, and native extensions as confidence-scored or blocked behavior.
5. Lower Django/FastAPI/Flask routes, middleware, dependencies, ORM models, migrations, validation, tasks, and serialization into Framework IR.
6. Map asyncio tasks, cancellation, contextvars, generators, and resource lifetime explicitly.
7. Generate characterization tests before translating untyped high-impact code.
8. Verify with uv/pytest, mypy or pyright, ruff, framework tests, and golden behavior comparisons.
...
- [ ] decorator/metaclass/descriptor fixtures.
- [ ] asyncio cancellation/contextvars.
- [ ] Django ORM transaction behavior.
- [ ] FastAPI schema/error contracts.
- [ ] native extension and eval blocker detection.
