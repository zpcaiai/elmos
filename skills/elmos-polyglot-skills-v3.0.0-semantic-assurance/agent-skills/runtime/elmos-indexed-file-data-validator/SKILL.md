---
name: elmos-indexed-file-data-validator
description: Reconcile source indexed files and target stores for key semantics, collation,
  duplicates, ordering, tombstones and full dataset checksums.
version: 1.0.0
skill_id: ELMOS-POLY-159
layer: validation
risk: high
readiness: not-run
dependencies:
...
triggers:
- Use when implementing or executing `elmos-indexed-file-data-validator`.
- Use when the migration DAG requires `validation` capability.
outputs:
- validation/elmos-indexed-file-data-validator/result.json
- validation/elmos-indexed-file-data-validator/evidence/
---

# Indexed File Data Validator

## Objective

Reconcile source indexed files and target stores for key semantics, collation, duplicates, ordering, tombstones and full dataset checksums.
