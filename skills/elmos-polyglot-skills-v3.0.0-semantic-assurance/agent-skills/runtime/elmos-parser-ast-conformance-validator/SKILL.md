---
name: elmos-parser-ast-conformance-validator
description: Compare native compiler/parser results, symbols and source spans against
  adapter output on dialect/version fixture matrices.
version: 1.0.0
skill_id: ELMOS-POLY-153
layer: validation
risk: high
readiness: not-run
dependencies:
...
triggers:
- Use when implementing or executing `elmos-parser-ast-conformance-validator`.
- Use when the migration DAG requires `validation` capability.
outputs:
- validation/elmos-parser-ast-conformance-validator/result.json
- validation/elmos-parser-ast-conformance-validator/evidence/
---

# Parser Ast Conformance Validator

## Objective

Compare native compiler/parser results, symbols and source spans against adapter output on dialect/version fixture matrices.
