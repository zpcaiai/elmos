...
req=['README.md','manifest.json','technology-registry.json','repository-surface-registry.json','route-registry.json','route-certification-registry.json','route-matrix.csv','SKILL_INDEX.md','certification-corpus/corpus-registry.json','native-runtime-lab/lab-registry.json']
for x in req:
 if not (R/x).exists(): err('missing '+x)
m=json.loads((R/'manifest.json').read_text()); sk=m['skills']; names={x['name'] for x in sk}
if len(sk)!=300 or m['package'].get('skill_count')!=300: err('skill count')
if len(names)!=300 or len({x['id'] for x in sk})!=300: err('duplicate skill')
if len(m.get('technologies',[]))!=28 or len(m.get('repository_surfaces',[]))!=8: err('technology/surface count')
ids=sorted(int(x['id'].split('-')[-1]) for x in sk)
if ids!=list(range(1,301)): err('skill id continuity')
...
for s in sk:
 for d in s.get('dependencies',[]):
  if d not in names: err('unknown dep '+s['name']+'->'+d)
g={s['name']:s.get('dependencies',[]) for s in sk}; state={}
def dfs(n,path):
...
if len(rows)!=784 or len({(x['source'],x['target']) for x in rows})!=784: err('route matrix')
if any(x['readiness']!='not-run' for x in rows): err('route readiness')
rr=json.loads((R/'route-registry.json').read_text()); rc=json.loads((R/'route-certification-registry.json').read_text())
if len(rr['spec']['profiles'])!=40 or len(rc['spec']['routes'])!=40: err('route profile/cert count')
...
print('PASS: 300 Skills (168 base + 132 semantic assurance)')
print('PASS: IDs 001..300, frontmatter, required sections, dependency DAG')
print('PASS: 28 technologies + 8 repository surfaces')
print('PASS: 784 route cells + 40 reference profiles + 40 certification plans')
