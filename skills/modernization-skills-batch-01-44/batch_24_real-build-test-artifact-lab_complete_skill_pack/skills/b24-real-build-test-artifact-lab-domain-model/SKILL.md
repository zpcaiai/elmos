---
name: b24-real-build-test-artifact-lab-domain-model
description: "定义稳定 ID、版本化 Schema、状态、输入输出、Unknown 保留、摘要和兼容策略。 Scope: Real Build, Test, Package and Artifact Execution Lab."
version: 1.0.0
batch: batch-24
risk: high
...
本 Skill 属于 **Batch 24: Real Build, Test, Package and Artifact Execution Lab**。Batch 总目标：在受控工具链和隔离 Runner 中执行真实 Analyze/Compile/Test/Package，保存依赖、日志、制品、签名和可重复性证据，为后续行为与发布认证提供真实 Artifact。
...
- 对 JDK/Maven/Gradle 的正常路径产生可重放证据。
- 缺少 .NET SDK 证据时必须降级或阻断。
