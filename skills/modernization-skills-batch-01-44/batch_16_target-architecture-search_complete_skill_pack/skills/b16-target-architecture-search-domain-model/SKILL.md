---
name: b16-target-architecture-search-domain-model
description: "定义稳定 ID、版本化 Schema、状态、输入输出、Unknown 保留、摘要和兼容策略。 Scope: Target Architecture Search and Migration Planning."
version: 1.0.0
batch: batch-16
risk: high
...
本 Skill 属于 **Batch 16: Target Architecture Search and Migration Planning**。Batch 总目标：从源架构和约束出发搜索语言、框架、数据和部署组合，通过多目标优化、原型和仿真生成 Retain/Rewrite/Wrap/Strangler 决策、迁移波次、ADR 和 Target Blueprint。
...
- 对 Source Architecture Recovery 的正常路径产生可重放证据。
- 缺少 Hard/Soft Constraint Registry 证据时必须降级或阻断。
