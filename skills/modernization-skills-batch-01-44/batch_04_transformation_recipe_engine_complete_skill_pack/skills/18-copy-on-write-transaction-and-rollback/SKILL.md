---
name: b04-copy-on-write-transaction-and-rollback
description: "保证原始 Snapshot 不变，并在验证失败时恢复精确状态。 Scope: Batch 4：跨语言语义映射、Transformation Rule DSL 与 Deterministic Recipe Engine."
version: 1.0.0
batch: batch-04
archetype: failure-recovery
risk: critical
status: implementation-ready
---

# Copy On Write Transaction And Rollback

## Objective

保证原始 Snapshot 不变，并在验证失败时恢复精确状态。

本 Skill 属于 **Batch 04: Batch 4：跨语言语义映射、Transformation Rule DSL 与 Deterministic Recipe Engine**,承担运行时原型 `failure-recovery` 的职责,并由
`scripts/modernization_b01_44` 中的同名执行路径实际驱动。

## Scope

- 运行时原型:`failure-recovery`
...
- `transformation-transaction.json`
- `workspace-snapshots/`
- `patch-journal.json`
