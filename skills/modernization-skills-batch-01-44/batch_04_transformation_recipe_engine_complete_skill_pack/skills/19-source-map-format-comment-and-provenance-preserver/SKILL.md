---
name: b04-source-map-format-comment-and-provenance-preserver
description: "让评审者理解每个 Patch 来源，并防止 Formatter 噪声掩盖真实变更。 Scope: Batch 4：跨语言语义映射、Transformation Rule DSL 与 Deterministic Recipe Engine."
version: 1.0.0
batch: batch-04
archetype: lineage-reconciliation
risk: high
status: implementation-ready
---

# Source Map Format Comment And Provenance Preserver

## Objective

让评审者理解每个 Patch 来源，并防止 Formatter 噪声掩盖真实变更。

本 Skill 属于 **Batch 04: Batch 4：跨语言语义映射、Transformation Rule DSL 与 Deterministic Recipe Engine**,承担运行时原型 `lineage-reconciliation` 的职责,并由
`scripts/modernization_b01_44` 中的同名执行路径实际驱动。

## Scope

- 运行时原型:`lineage-reconciliation`
...
- `formatting-diff-report.json`
- `comment-attachment-report.json`
