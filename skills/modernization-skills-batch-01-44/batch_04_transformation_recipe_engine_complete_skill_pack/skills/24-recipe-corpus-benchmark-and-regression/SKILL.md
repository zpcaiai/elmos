---
name: b04-recipe-corpus-benchmark-and-regression
description: "防止只测试成功案例、自动更新 Golden 或用格式变化冒充语义变换。 Scope: Batch 4：跨语言语义映射、Transformation Rule DSL 与 Deterministic Recipe Engine."
version: 1.0.0
batch: batch-04
archetype: corpus-benchmark
risk: critical
status: implementation-ready
---

# Recipe Corpus Benchmark And Regression

## Objective

防止只测试成功案例、自动更新 Golden 或用格式变化冒充语义变换。

本 Skill 属于 **Batch 04: Batch 4：跨语言语义映射、Transformation Rule DSL 与 Deterministic Recipe Engine**,承担运行时原型 `corpus-benchmark` 的职责,并由
`scripts/modernization_b01_44` 中的同名执行路径实际驱动。

## Scope

- 运行时原型:`corpus-benchmark`
