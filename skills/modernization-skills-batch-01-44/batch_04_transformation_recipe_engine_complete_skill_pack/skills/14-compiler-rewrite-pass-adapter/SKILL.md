---
name: b04-compiler-rewrite-pass-adapter
description: "利用 Compiler Fact 与 Pass Manager，同时避免无 Source Map 的 Compiler IR 结果冒充源码补丁。 Scope: Batch 4：跨语言语义映射、Transformation Rule DSL 与 Deterministic Recipe Engine."
version: 1.0.0
batch: batch-04
archetype: adapter-provider
risk: critical
status: implementation-ready
---

# Compiler Rewrite Pass Adapter

## Objective

利用 Compiler Fact 与 Pass Manager，同时避免无 Source Map 的 Compiler IR 结果冒充源码补丁。

本 Skill 属于 **Batch 04: Batch 4：跨语言语义映射、Transformation Rule DSL 与 Deterministic Recipe Engine**,承担运行时原型 `adapter-provider` 的职责,并由
`scripts/modernization_b01_44` 中的同名执行路径实际驱动。

## Scope

- 运行时原型:`adapter-provider`
- 上游契约:Batch 03 认证输出(Batch 01 为 `genesis`)
- 下游消费者:Batch 05
...
- `compiler-pass-result.json`
- `compiler-source-edits.json`
- `analysis-preservation-report.json`
- `EvidenceRefs`
- `KnownLimitations`
- `CompletionReport`
