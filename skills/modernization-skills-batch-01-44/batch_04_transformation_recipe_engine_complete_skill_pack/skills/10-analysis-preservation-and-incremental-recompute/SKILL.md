---
name: b04-analysis-preservation-and-incremental-recompute
description: "防止变换后使用 Stale Analysis，同时避免无必要全量重算。 Scope: Batch 4：跨语言语义映射、Transformation Rule DSL 与 Deterministic Recipe Engine."
version: 1.0.0
batch: batch-04
archetype: lifecycle-recertification
risk: critical
status: implementation-ready
---

# Analysis Preservation And Incremental Recompute

## Objective

防止变换后使用 Stale Analysis，同时避免无必要全量重算。

本 Skill 属于 **Batch 04: Batch 4：跨语言语义映射、Transformation Rule DSL 与 Deterministic Recipe Engine**,承担运行时原型 `lifecycle-recertification` 的职责,并由
`scripts/modernization_b01_44` 中的同名执行路径实际驱动。

## Scope

- 运行时原型:`lifecycle-recertification`
...
- `analysis-invalidation-plan.json`
- `recomputed-ir-bundle.json`
- `stale-analysis-register.json`
- `EvidenceRefs`
- `KnownLimitations`
- `CompletionReport`
