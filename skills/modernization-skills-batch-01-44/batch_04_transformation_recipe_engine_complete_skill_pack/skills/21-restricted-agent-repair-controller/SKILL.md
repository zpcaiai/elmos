---
name: b04-restricted-agent-repair-controller
description: "利用 Agent 处理局部胶水问题，但不让其成为可信根或扩大范围。 Scope: Batch 4：跨语言语义映射、Transformation Rule DSL 与 Deterministic Recipe Engine."
version: 1.0.0
batch: batch-04
...
# Restricted Agent Repair Controller

## Objective

利用 Agent 处理局部胶水问题，但不让其成为可信根或扩大范围。

本 Skill 属于 **Batch 04: Batch 4：跨语言语义映射、Transformation Rule DSL 与 Deterministic Recipe Engine**,承担运行时原型 `human-approval` 的职责,并由
