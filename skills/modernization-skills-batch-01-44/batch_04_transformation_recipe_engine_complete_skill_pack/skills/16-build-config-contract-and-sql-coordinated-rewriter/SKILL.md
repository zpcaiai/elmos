---
name: b04-build-config-contract-and-sql-coordinated-rewriter
description: "避免只改源码而遗漏依赖、连接、事务、契约和部署。 Scope: Batch 4：跨语言语义映射、Transformation Rule DSL 与 Deterministic Recipe Engine."
version: 1.0.0
batch: batch-04
...
# Build Config Contract And Sql Coordinated Rewriter

## Objective

避免只改源码而遗漏依赖、连接、事务、契约和部署。

本 Skill 属于 **Batch 04: Batch 4：跨语言语义映射、Transformation Rule DSL 与 Deterministic Recipe Engine**,承担运行时原型 `deterministic-engine` 的职责,并由
...
- `coordinated-patch-set.json`
- `cross-domain-impact-report.json`
- `contract-drift-after-rewrite.json`
- `EvidenceRefs`
- `KnownLimitations`
- `CompletionReport`
