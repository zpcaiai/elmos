---
name: b04-deterministic-recipe-runtime
description: "提供确定性可信根，并明确 Agent-assisted 运行不具生成确定性。 Scope: Batch 4：跨语言语义映射、Transformation Rule DSL 与 Deterministic Recipe Engine."
version: 1.0.0
batch: batch-04
archetype: workflow-runtime
risk: critical
status: implementation-ready
---

# Deterministic Recipe Runtime

## Objective

提供确定性可信根，并明确 Agent-assisted 运行不具生成确定性。

本 Skill 属于 **Batch 04: Batch 4：跨语言语义映射、Transformation Rule DSL 与 Deterministic Recipe Engine**,承担运行时原型 `workflow-runtime` 的职责,并由
`scripts/modernization_b01_44` 中的同名执行路径实际驱动。

## Scope

- 运行时原型:`workflow-runtime`
...
- `deterministic-output-digest.json`
- `cycle-report.json`
- `EvidenceRefs`
- `KnownLimitations`
- `CompletionReport`
