---
name: b01-database-route-competitive-analyzer
description: "区分 Schema、SQL、Routine、Data、CDC、Dual Run、Cutover 和 Rollback，形成数据库路线竞争地图。 Scope: Batch 1：竞争格局、产品定位与持续竞争情报."
version: 1.0.0
batch: batch-01
...
# Database Route Competitive Analyzer

## Objective

区分 Schema、SQL、Routine、Data、CDC、Dual Run、Cutover 和 Rollback，形成数据库路线竞争地图。

本 Skill 属于 **Batch 01: Batch 1：竞争格局、产品定位与持续竞争情报**,承担运行时原型 `deterministic-engine` 的职责,并由
`scripts/modernization_b01_44` 中的同名执行路径实际驱动。

## Scope

- 运行时原型:`deterministic-engine`
- 上游契约:Batch 00 认证输出(Batch 01 为 `genesis`)
- 下游消费者:Batch 02
...
- `database-route-matrix.json`
- `database-feature-coverage.json`
- `compatibility-layer-risk.json`
- `cutover-capability-map.json`
- `EvidenceRefs`
- `KnownLimitations`
- `CompletionReport`
