---
name: b01-deployment-sovereignty-lockin-analyzer
description: "让客户明确源码去向、模型调用、遥测、密钥、网络出口、区域和退出成本。 Scope: Batch 1：竞争格局、产品定位与持续竞争情报."
version: 1.0.0
batch: batch-01
...
# Deployment Sovereignty Lockin Analyzer

## Objective

让客户明确源码去向、模型调用、遥测、密钥、网络出口、区域和退出成本。

本 Skill 属于 **Batch 01: Batch 1：竞争格局、产品定位与持续竞争情报**,承担运行时原型 `security-policy` 的职责,并由
`scripts/modernization_b01_44` 中的同名执行路径实际驱动。

## Scope

- 运行时原型:`security-policy`
...
- `data-boundary-map.json`
- `lockin-scorecard.json`
- `exit-capability-report.json`
- `EvidenceRefs`
- `KnownLimitations`
- `CompletionReport`
