---
name: b38-platform-convergence-domain-model
description: "定义稳定 ID、版本化 Schema、状态、输入输出、Unknown 保留、摘要和兼容策略。 Scope: Canonical CapabilityPackage, Dependency Graph and Platform Convergence."
version: 1.0.0
batch: batch-38
risk: high
...
本 Skill 属于 **Batch 38: Canonical CapabilityPackage, Dependency Graph and Platform Convergence**。Batch 总目标：统一 Route、Framework、Database、Client、Cloud、Verification、Extension 等 Pack 的父模型、跨 Batch 依赖图、状态、Owner、策略、证据、经济性和 UI/API 生命周期。
...
- 对 CapabilityPackage Meta-model 的正常路径产生可重放证据。
- 缺少 Cross-batch Dependency Graph 证据时必须降级或阻断。
