---
name: b34-portfolio-scale-migration-factory-domain-model
description: "定义稳定 ID、版本化 Schema、状态、输入输出、Unknown 保留、摘要和兼容策略。 Scope: Portfolio-Scale Multi-Repository Migration Factory."
version: 1.0.0
batch: batch-34
risk: high
...
本 Skill 属于 **Batch 34: Portfolio-Scale Multi-Repository Migration Factory**。Batch 总目标：把单仓库迁移扩展到千仓库、百万行、多个团队和 Runner Fleet，管理 Capability Graph、依赖波次、配额、缓存、批量 PR、灾备和组合级认证。
...
- 对 Portfolio Inventory 的正常路径产生可重放证据。
- 缺少 Capability/Dependency Graph 证据时必须降级或阻断。
