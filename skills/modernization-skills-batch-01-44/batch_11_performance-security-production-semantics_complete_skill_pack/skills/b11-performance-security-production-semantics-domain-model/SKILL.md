---
name: b11-performance-security-production-semantics-domain-model
description: "定义稳定 ID、版本化 Schema、状态、输入输出、Unknown 保留、摘要和兼容策略。 Scope: Performance, Security and Production Semantics Validation."
version: 1.0.0
batch: batch-11
risk: high
...
本 Skill 属于 **Batch 11: Performance, Security and Production Semantics Validation**。Batch 总目标：验证并发、事务、锁、精度、时区、序列化、身份权限、加密、性能、资源生命周期和生产配置语义，阻止只通过功能测试的错误迁移。
...
- 对 Concurrency 与 Scheduling 的正常路径产生可重放证据。
- 缺少 Transaction/Isolation/Lock 证据时必须降级或阻断。
