---
name: b25-behavioral-equivalence-gate-domain-model
description: "定义稳定 ID、版本化 Schema、状态、输入输出、Unknown 保留、摘要和兼容策略。 Scope: Behavioral Equivalence Gate and Independent Oracle System."
version: 1.0.0
batch: batch-25
risk: high
...
本 Skill 属于 **Batch 25: Behavioral Equivalence Gate and Independent Oracle System**。Batch 总目标：汇聚真实仓库、强化语义、框架执行和真实构建证据，使用独立 Oracle、可控调度和状态/副作用差分签发或拒绝行为等价证书。
...
- 对 Oracle Registry 的正常路径产生可重放证据。
- 缺少 Scenario Runtime 证据时必须降级或阻断。
