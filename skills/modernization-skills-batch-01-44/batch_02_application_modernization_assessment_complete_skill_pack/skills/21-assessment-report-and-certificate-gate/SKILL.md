---
name: b02-assessment-report-and-certificate-gate
description: "确保 Assessment Certificate 只证明评估覆盖与证据等级，不冒充迁移成功证书。 Scope: Batch 2：应用现代化自动评估."
version: 1.0.0
batch: batch-02
...
# Assessment Report And Certificate Gate

## Objective

确保 Assessment Certificate 只证明评估覆盖与证据等级，不冒充迁移成功证书。

本 Skill 属于 **Batch 02: Batch 2：应用现代化自动评估**,承担运行时原型 `certification-gate` 的职责,并由
`scripts/modernization_b01_44` 中的同名执行路径实际驱动。

## Scope

- 运行时原型:`certification-gate`
- 上游契约:Batch 01 认证输出(Batch 01 为 `genesis`)
- 下游消费者:Batch 03
