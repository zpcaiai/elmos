---
name: b02-source-repository-discovery
description: "建立 Repository→Module→Build Root 的完整、可重放源码清单。 Scope: Batch 2：应用现代化自动评估."
version: 1.0.0
batch: batch-02
archetype: discovery-inventory
risk: high
...
建立 Repository→Module→Build Root 的完整、可重放源码清单。

本 Skill 属于 **Batch 02: Batch 2：应用现代化自动评估**,承担运行时原型 `discovery-inventory` 的职责,并由
`scripts/modernization_b01_44` 中的同名执行路径实际驱动。

## Scope

- 运行时原型:`discovery-inventory`
...
- `repository-snapshot-manifest.json`
- `module-root-map.json`
- `repository-access-failures.json`
- `EvidenceRefs`
- `KnownLimitations`
- `CompletionReport`
