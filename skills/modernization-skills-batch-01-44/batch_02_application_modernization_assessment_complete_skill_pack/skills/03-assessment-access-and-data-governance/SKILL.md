---
name: b02-assessment-access-and-data-governance
description: "确保评估只在被授权的数据和执行边界内运行。 Scope: Batch 2：应用现代化自动评估."
version: 1.0.0
batch: batch-02
...
# Assessment Access And Data Governance

## Objective

确保评估只在被授权的数据和执行边界内运行。

本 Skill 属于 **Batch 02: Batch 2：应用现代化自动评估**,承担运行时原型 `security-policy` 的职责,并由
...
- `access-grant-manifest.json`
- `data-handling-plan.json`
- `model-data-routing-policy.json`
- `sandbox-execution-policy.json`
- `EvidenceRefs`
- `KnownLimitations`
- `CompletionReport`
