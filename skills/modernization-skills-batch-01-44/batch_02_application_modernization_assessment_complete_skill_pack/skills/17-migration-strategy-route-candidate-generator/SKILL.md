---
name: b02-migration-strategy-route-candidate-generator
description: "输出 Pareto 候选，而不是单一 AI 推荐，并显式保留被淘汰方案。 Scope: Batch 2：应用现代化自动评估."
version: 1.0.0
batch: batch-02
...
# Migration Strategy Route Candidate Generator

## Objective

输出 Pareto 候选，而不是单一 AI 推荐，并显式保留被淘汰方案。

本 Skill 属于 **Batch 02: Batch 2：应用现代化自动评估**,承担运行时原型 `deterministic-engine` 的职责,并由
...
- `candidate-pareto-front.json`
- `rejected-candidate-register.json`
- `required-validation-plan.json`
- `EvidenceRefs`
- `KnownLimitations`
- `CompletionReport`
