---
name: b26-dual-run-shadow-reconciliation-domain-model
description: "定义稳定 ID、版本化 Schema、状态、输入输出、Unknown 保留、摘要和兼容策略。 Scope: Dual Run, Shadow Execution and State Reconciliation."
version: 1.0.0
batch: batch-26
risk: high
...
本 Skill 属于 **Batch 26: Dual Run, Shadow Execution and State Reconciliation**。Batch 总目标：建立源目标并行运行、流量捕获与回放、影子执行、副作用隔离、状态和 CDC 对账、分歧定位、Canary 和自动停止回滚闭环。
...
- 对 Traffic Capture/Replay 的正常路径产生可重放证据。
- 缺少 Route Matching 证据时必须降级或阻断。
