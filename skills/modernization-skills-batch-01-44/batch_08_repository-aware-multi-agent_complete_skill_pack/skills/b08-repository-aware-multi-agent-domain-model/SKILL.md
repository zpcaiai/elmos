---
name: b08-repository-aware-multi-agent-domain-model
description: "定义稳定 ID、版本化 Schema、状态、输入输出、Unknown 保留、摘要和兼容策略。 Scope: Repository-Aware Multi-Agent, Context and RAG System."
version: 1.0.0
batch: batch-08
risk: high
...
本 Skill 属于 **Batch 08: Repository-Aware Multi-Agent, Context and RAG System**。Batch 总目标：建立受证据约束的仓库级 Agent 系统，统一 Context、RAG、模型路由、工具权限、成本预算和人工升级，并确保 Agent 不能成为产品可信根。
...
- 对 Repository Context Graph 的正常路径产生可重放证据。
- 缺少 语义检索与 RAG 证据时必须降级或阻断。
