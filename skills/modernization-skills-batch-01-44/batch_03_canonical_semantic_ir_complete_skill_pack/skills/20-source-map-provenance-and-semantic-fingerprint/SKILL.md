---
name: b03-source-map-provenance-and-semantic-fingerprint
description: "提供一对多、多对一、Tombstone 和跨快照语义候选匹配。 Scope: Batch 3：统一源码摄取、解析前端与 Canonical Semantic IR Foundation."
version: 1.0.0
batch: batch-03
...
# Source Map Provenance And Semantic Fingerprint

## Objective

提供一对多、多对一、Tombstone 和跨快照语义候选匹配。

本 Skill 属于 **Batch 03: Batch 3：统一源码摄取、解析前端与 Canonical Semantic IR Foundation**,承担运行时原型 `lineage-reconciliation` 的职责,并由
`scripts/modernization_b01_44` 中的同名执行路径实际驱动。

## Scope

- 运行时原型:`lineage-reconciliation`
- 上游契约:Batch 02 认证输出(Batch 01 为 `genesis`)
- 下游消费者:Batch 04
...
- `source-map-graph.json`
- `provenance-graph.json`
- `semantic-fingerprint-index.json`
- `tombstone-register.json`
- `EvidenceRefs`
- `KnownLimitations`
- `CompletionReport`
