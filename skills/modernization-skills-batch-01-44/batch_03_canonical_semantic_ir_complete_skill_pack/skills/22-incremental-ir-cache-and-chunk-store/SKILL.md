---
name: b03-incremental-ir-cache-and-chunk-store
description: "避免全仓重算，同时确保不同 Build Context、Frontend 和 Schema 之间不误复用。 Scope: Batch 3：统一源码摄取、解析前端与 Canonical Semantic IR Foundation."
version: 1.0.0
batch: batch-03
...
# Incremental Ir Cache And Chunk Store

## Objective

避免全仓重算，同时确保不同 Build Context、Frontend 和 Schema 之间不误复用。

本 Skill 属于 **Batch 03: Batch 3：统一源码摄取、解析前端与 Canonical Semantic IR Foundation**,承担运行时原型 `lifecycle-recertification` 的职责,并由
...
- `bundle-publication-journal.json`
- `EvidenceRefs`
- `KnownLimitations`
- `CompletionReport`
