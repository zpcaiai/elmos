---
name: b19-seventy-two-directional-route-packs-adapter-provider
description: "通过签名适配器连接语言、框架、数据库、云、工具或外部 Provider，保留能力边界。 Scope: 72 Directional Executable Generator Packs."
version: 1.0.0
batch: batch-19
risk: high
...
# Adapter and Provider Layer

## Objective

通过签名适配器连接语言、框架、数据库、云、工具或外部 Provider，保留能力边界。
...
1. 发现 Provider 能力、版本、权限和限制。
2. 通过标准 Adapter Contract 映射输入输出和诊断。
3. 在 Sandbox 中执行 Dry Run 和 Conformance。
4. 捕获外部工具输出、版本、日志、制品和失败。
5. 将 Provider 结果转换为统一 Evidence，不提升其原始可信等级。
