---
name: b09-build-compile-test-repair-domain-model
description: "定义稳定 ID、版本化 Schema、状态、输入输出、Unknown 保留、摘要和兼容策略。 Scope: Build, Compile, Test Repair and Bounded Fix Loop."
version: 1.0.0
batch: batch-09
risk: high
...
本 Skill 属于 **Batch 09: Build, Compile, Test Repair and Bounded Fix Loop**。Batch 总目标：在锁定工具链和隔离环境中复现构建、解析诊断、执行确定性修复和受限 Agent 修复，形成有界、可回滚的 Build/Test 闭环。
...
- 对 Maven/Gradle 的正常路径产生可重放证据。
- 缺少 dotnet 证据时必须降级或阻断。
