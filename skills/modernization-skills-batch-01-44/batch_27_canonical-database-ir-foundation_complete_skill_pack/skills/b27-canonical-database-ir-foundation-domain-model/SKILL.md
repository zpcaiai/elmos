---
name: b27-canonical-database-ir-foundation-domain-model
description: "定义稳定 ID、版本化 Schema、状态、输入输出、Unknown 保留、摘要和兼容策略。 Scope: Canonical Database IR, Data Migration and Cutover Foundation."
version: 1.0.0
batch: batch-27
risk: high
...
本 Skill 属于 **Batch 27: Canonical Database IR, Data Migration and Cutover Foundation**。Batch 总目标：建立数据库资产发现、Canonical Database IR、Schema/Query/Routine/Transaction/ORM/Data Movement/CDC/Cutover 的联合语义底座。
...
- 对 Database Estate Discovery 的正常路径产生可重放证据。
- 缺少 Canonical Database IR 证据时必须降级或阻断。
