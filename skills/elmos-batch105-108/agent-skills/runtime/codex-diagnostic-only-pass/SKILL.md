---
name: codex-diagnostic-only-pass
description: 让Codex在只读工作树中形成根因分析、修复顺序和验证命令，禁止未诊断先大范围改码。
---

# B105-S09 — codex-diagnostic-only-pass

## Objective

让Codex在只读工作树中形成根因分析、修复顺序和验证命令，禁止未诊断先大范围改码。
...
- `services/agent-gateway/.../DiagnosticRunService.java`
- `private-runner/internal/agent/readonly_workspace.go`
- `prompts/codex-diagnostic/`
...
### Inputs
- `MigrationFailures`：必须有版本、租户和主体标识。
- `read-only repository snapshot`：必须有版本、租户和主体标识。
- `AGENTS.md`：必须有版本、租户和主体标识。

### Outputs
- `MIGRATION_FAILURES.md`：必须持久化或内容寻址，并可由下游 Skill 验证。
- `diagnostic plan`：必须持久化或内容寻址，并可由下游 Skill 验证。
- `tool budget report`：必须持久化或内容寻址，并可由下游 Skill 验证。

机器合同：`contracts/batch-105/B105-S09.json`。
...
### Interfaces
- `POST /api/agent-runs/diagnose`
- `event agent.diagnostic.completed`

### Persisted state
- `agent_run`：必须包含 tenant_id、version、created_at/updated_at，并采用服务端解析的租户身份。
- `diagnostic_finding`：必须包含 tenant_id、version、created_at/updated_at，并采用服务端解析的租户身份。
- `tool_budget`：必须包含 tenant_id、version、created_at/updated_at，并采用服务端解析的租户身份。

所有写操作必须具备幂等键或乐观版本；跨进程副作用必须通过Outbox/Workflow receipt记录，不能依赖内存状态。

## Execution workflow

1. 创建只读或discardable workspace。
2. 注入失败分类、仓库规则和预算。
3. 仅允许read/search/build-diagnostic工具。
4. 要求按根因而非错误数量分组。
5. 生成修复顺序、目标文件和验证命令。
6. 校验工作树无修改后签发诊断结果。
