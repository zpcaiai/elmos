---
name: runtime-log-and-event-stream
description: 将构建、启动、探针、验证、到期和清理事件以SSE/WebSocket流式展示并生成不可变摘要。
---

# B106-S14 — runtime-log-and-event-stream

## Objective

将构建、启动、探针、验证、到期和清理事件以SSE/WebSocket流式展示并生成不可变摘要。
...
- `runtime/logging/RuntimeLogService.java`
- `private-runner/internal/logstream/`
- `apps/web/src/features/runtime-logs/`
...
### Inputs
- `runtime events`：必须有版本、租户和主体标识。
- `stdout/stderr streams`：必须有版本、租户和主体标识。
- `redaction policy`：必须有版本、租户和主体标识。

### Outputs
- `LiveEventStream`：必须持久化或内容寻址，并可由下游 Skill 验证。
- `log artifact`：必须持久化或内容寻址，并可由下游 Skill 验证。
- `stream digest`：必须持久化或内容寻址，并可由下游 Skill 验证。

机器合同：`contracts/batch-106/B106-S14.json`。
...
### Interfaces
- `GET /api/runtime-runs/{id}/events`
- `GET /api/runtime-runs/{id}/logs`
- `event runtime-log.segment.closed`

### Persisted state
- `runtime_log_segment`：必须包含 tenant_id、version、created_at/updated_at，并采用服务端解析的租户身份。
- `runtime_event_cursor`：必须包含 tenant_id、version、created_at/updated_at，并采用服务端解析的租户身份。
- `redaction_finding`：必须包含 tenant_id、version、created_at/updated_at，并采用服务端解析的租户身份。

所有写操作必须具备幂等键或乐观版本；跨进程副作用必须通过Outbox/Workflow receipt记录，不能依赖内存状态。

## Execution workflow

1. 接入BuildKit和沙箱stdout/stderr。
2. 统一时间、来源、序号和Run ID。
3. 实时Secret/PII脱敏。
4. 按段压缩存储并计算哈希链。
5. 支持断线后从cursor续传。
