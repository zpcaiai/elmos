---
name: preview-port-exposure-controller
description: 把沙箱内部HTTP/gRPC-Web/WebSocket端口映射为受认证的临时HTTPS地址，并在到期时原子撤销。
---

# B106-S11 — preview-port-exposure-controller

## Objective

把沙箱内部HTTP/gRPC-Web/WebSocket端口映射为受认证的临时HTTPS地址，并在到期时原子撤销。
...
- `runtime/gateway/PreviewEndpointService.java`
- `gateway/preview-routes/`
...
### Inputs
- `SandboxInstance`：必须有版本、租户和主体标识。
- `service port/protocol`：必须有版本、租户和主体标识。
- `access policy`：必须有版本、租户和主体标识。

### Outputs
- `PublicEndpoint`：必须持久化或内容寻址，并可由下游 Skill 验证。
- `access token`：必须持久化或内容寻址，并可由下游 Skill 验证。
- `route receipt`：必须持久化或内容寻址，并可由下游 Skill 验证。

机器合同：`contracts/batch-106/B106-S11.json`。
...
### Interfaces
- `POST /api/runtime-runs/{id}/endpoints`
- `DELETE /api/runtime-runs/{id}/endpoints`
- `event preview.endpoint.created`

### Persisted state
- `preview_endpoint`：必须包含 tenant_id、version、created_at/updated_at，并采用服务端解析的租户身份。
- `endpoint_token`：必须包含 tenant_id、version、created_at/updated_at，并采用服务端解析的租户身份。
- `route_binding`：必须包含 tenant_id、version、created_at/updated_at，并采用服务端解析的租户身份。

所有写操作必须具备幂等键或乐观版本；跨进程副作用必须通过Outbox/Workflow receipt记录，不能依赖内存状态。

## Execution workflow

1. 验证端口属于该实例和Manifest。
2. 创建随机不可猜子域与TLS路由。
3. 配置身份、限流、最大Body和WebSocket升级。
4. 只暴露声明的路径/端口。
5. 返回不含内部拓扑的PublicEndpoint。
6. 销毁时先撤销路由再停止实例。
...
完成时至少产生：
- `endpoint-receipt.json`
- `route-policy.json`
- `access-audit.json`
- `revocation-receipt.json`
