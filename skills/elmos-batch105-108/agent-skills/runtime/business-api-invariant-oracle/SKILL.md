---
name: business-api-invariant-oracle
description: 为CRUD、引用完整性、幂等、分页、排序、认证、授权和事务结果定义跨版本业务不变量。
---

# B107-S07 — business-api-invariant-oracle

## Objective

为CRUD、引用完整性、幂等、分页、排序、认证、授权和事务结果定义跨版本业务不变量。
...
- `validation/oracles/BusinessInvariantService.java`
- `contracts/business-invariant.schema.json`
- `registry/domain-oracles/`
...
### Inputs
- `domain model hints`：必须有版本、租户和主体标识。
- `HTTP scenarios`：必须有版本、租户和主体标识。
- `state observation adapters`：必须有版本、租户和主体标识。

### Outputs
- `InvariantSuite`：必须持久化或内容寻址，并可由下游 Skill 验证。
- `InvariantResults`：必须持久化或内容寻址，并可由下游 Skill 验证。

机器合同：`contracts/batch-107/B107-S07.json`。
...
### Interfaces
- `POST /api/validations/invariants`
- `event invariants.completed`

### Persisted state
- `business_invariant`：必须包含 tenant_id、version、created_at/updated_at，并采用服务端解析的租户身份。
- `invariant_observation`：必须包含 tenant_id、version、created_at/updated_at，并采用服务端解析的租户身份。
- `invariant_result`：必须包含 tenant_id、version、created_at/updated_at，并采用服务端解析的租户身份。
