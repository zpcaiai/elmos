---
name: runtime-manifest-schema-validator
description: 对RuntimeManifest执行结构、语义、权限和兼容性校验，并产生可定位错误。
---

# B106-S03 — runtime-manifest-schema-validator

## Objective

对RuntimeManifest执行结构、语义、权限和兼容性校验，并产生可定位错误。
...
- `runtime/manifest/RuntimeManifestValidator.java`
- `schemas/runtime-manifest.schema.json`
- `policies/runtime-manifest.rego`
...
- `provider capabilities`：必须有版本、租户和主体标识。
- `tenant policy`：必须有版本、租户和主体标识。

### Outputs
- `ManifestValidationResult`：必须持久化或内容寻址，并可由下游 Skill 验证。
- `normalized manifest`：必须持久化或内容寻址，并可由下游 Skill 验证。

机器合同：`contracts/batch-106/B106-S03.json`。
...
### Interfaces
- `POST /api/runtime-manifests:validate`
- `event runtime-manifest.validated`

### Persisted state
- `manifest_validation`：必须包含 tenant_id、version、created_at/updated_at，并采用服务端解析的租户身份。
- `manifest_violation`：必须包含 tenant_id、version、created_at/updated_at，并采用服务端解析的租户身份。
...
1. 执行JSON Schema验证。
2. 检查build/runtime命令、工作目录和artifact路径。
3. 检查端口、协议、健康和preview入口一致性。
4. 检查资源、网络、Secret和文件系统Policy。
5. 检查Provider是否支持所需能力。
6. 返回字段级违规和规范化Manifest。
...
- 缺start command失败。
- public ingress但无端口失败。
- 通配egress默认失败。
- Provider不支持gRPC时显式UNSUPPORTED。
- 错误包含JSON Pointer。
