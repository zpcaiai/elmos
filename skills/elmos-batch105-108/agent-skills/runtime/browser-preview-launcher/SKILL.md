---
name: browser-preview-launcher
description: 在隔离浏览器中打开首页、登录、管理页、Swagger和关键业务页面，建立稳定会话与页面上下文。
---

# B107-S09 — browser-preview-launcher

## Objective

在隔离浏览器中打开首页、登录、管理页、Swagger和关键业务页面，建立稳定会话与页面上下文。
...
- `validation/browser/BrowserLauncherService.java`
- `private-runner/internal/browser/`
- `images/browser/`
...
### Inputs
- `PublicEndpoint`：必须有版本、租户和主体标识。
- `page journey definition`：必须有版本、租户和主体标识。
- `browser profile`：必须有版本、租户和主体标识。

### Outputs
- `BrowserSession`：必须持久化或内容寻址，并可由下游 Skill 验证。
- `page navigation trace`：必须持久化或内容寻址，并可由下游 Skill 验证。

机器合同：`contracts/batch-107/B107-S09.json`。
...
### Interfaces
- `POST /api/runtime-runs/{id}/browser-sessions`
- `event browser.session.started`

### Persisted state
- `browser_session`：必须包含 tenant_id、version、created_at/updated_at，并采用服务端解析的租户身份。
- `browser_navigation`：必须包含 tenant_id、version、created_at/updated_at，并采用服务端解析的租户身份。
...
1. 创建隔离无持久用户Profile。
2. 注入短期Preview访问令牌。
3. 按Journey打开页面并等待可配置稳定条件。
4. 记录导航、Title、URL和主要frame。
5. 处理登录fixture但禁止真实外部账户。
6. 终态关闭浏览器并清理cookie/storage。
