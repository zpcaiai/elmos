---
name: runtime-service-validation-evidence-assembler
description: 把健康、OpenAPI、HTTP、业务不变量、浏览器、截图和流式结果装配为有血缘的LiveValidationBundle。
---

# B107-S15 — runtime-service-validation-evidence-assembler

## Objective

把健康、OpenAPI、HTTP、业务不变量、浏览器、截图和流式结果装配为有血缘的LiveValidationBundle。
...
- `evidence/live/LiveValidationAssembler.java`
- `schemas/live-validation-bundle.schema.json`
...
### Inputs
- `all Batch107 evidence`：必须有版本、租户和主体标识。
- `run/source/image lineage`：必须有版本、租户和主体标识。

### Outputs
- `LiveValidationBundle`：必须持久化或内容寻址，并可由下游 Skill 验证。
- `evidence index`：必须持久化或内容寻址，并可由下游 Skill 验证。
- `freshness status`：必须持久化或内容寻址，并可由下游 Skill 验证。

机器合同：`contracts/batch-107/B107-S15.json`。
...
### Interfaces
- `POST /api/runtime-runs/{id}/evidence:assemble`
- `event live-evidence.assembled`

### Persisted state
- `live_validation_bundle`：必须包含 tenant_id、version、created_at/updated_at，并采用服务端解析的租户身份。
- `evidence_link`：必须包含 tenant_id、version、created_at/updated_at，并采用服务端解析的租户身份。
- `evidence_freshness`：必须包含 tenant_id、version、created_at/updated_at，并采用服务端解析的租户身份。

所有写操作必须具备幂等键或乐观版本；跨进程副作用必须通过Outbox/Workflow receipt记录，不能依赖内存状态。

## Execution workflow

1. 校验所有证据绑定同一Run/Commit/Image。
2. 验证哈希、签名和时间窗口。
3. 建立证据索引与依赖关系。
4. 对缺失/失败/UNKNOWN生成阻断项。
5. 压缩大工件并保留内容寻址。
6. 签名Bundle并写入不可变存储。
