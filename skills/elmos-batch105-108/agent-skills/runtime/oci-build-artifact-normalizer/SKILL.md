---
name: oci-build-artifact-normalizer
description: 将各语言产物统一构建为带SBOM、Provenance和不可变Digest的OCI镜像。
---

# B106-S08 — oci-build-artifact-normalizer

## Objective

将各语言产物统一构建为带SBOM、Provenance和不可变Digest的OCI镜像。
...
- `runtime/build/OciBuildService.java`
- `private-runner/internal/buildkit/client.go`
- `infra/buildkit/`
...
### Inputs
- `validated manifest`：必须有版本、租户和主体标识。
- `language adapter spec`：必须有版本、租户和主体标识。
- `source snapshot`：必须有版本、租户和主体标识。

### Outputs
- `OCIImageRef`：必须持久化或内容寻址，并可由下游 Skill 验证。
- `SBOM`：必须持久化或内容寻址，并可由下游 Skill 验证。
- `BuildProvenance`：必须持久化或内容寻址，并可由下游 Skill 验证。

机器合同：`contracts/batch-106/B106-S08.json`。
...
### Interfaces
- `POST /api/runtime-images:build`
- `event runtime-image.built`

### Persisted state
- `runtime_image`：必须包含 tenant_id、version、created_at/updated_at，并采用服务端解析的租户身份。
- `build_provenance`：必须包含 tenant_id、version、created_at/updated_at，并采用服务端解析的租户身份。
- `sbom_artifact`：必须包含 tenant_id、version、created_at/updated_at，并采用服务端解析的租户身份。

所有写操作必须具备幂等键或乐观版本；跨进程副作用必须通过Outbox/Workflow receipt记录，不能依赖内存状态。

## Execution workflow

1. 生成多阶段Dockerfile/LLB计划。
2. 使用固定digest builder/runtime基础镜像。
3. 以BuildKit隔离构建并应用缓存。
4. 扫描Secret泄露和危险层。
5. 生成CycloneDX/SPDX与SLSA风格Provenance。
6. 推送镜像并只返回digest引用。
...
- 浮动latest基础镜像生产模式拒绝。
- Secret不进入layer/history。
- 相同输入命中缓存且digest稳定。
- 高危恶意Docker指令按Policy阻断。
- SBOM与镜像内容匹配。
