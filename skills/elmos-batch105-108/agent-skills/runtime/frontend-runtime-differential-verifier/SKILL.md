---
name: frontend-runtime-differential-verifier
description: 比较迁移前后页面的语义树、路由、表单、网络、交互结果和可选视觉差异。
---

# B107-S13 — frontend-runtime-differential-verifier

## Objective

比较迁移前后页面的语义树、路由、表单、网络、交互结果和可选视觉差异。
...
- `validation/frontend/RuntimeDifferentialService.java`
- `adapters/visual-diff/`
- `policies/frontend-equivalence.yaml`
...
### Inputs
- `baseline browser evidence`：必须有版本、租户和主体标识。
- `candidate browser evidence`：必须有版本、租户和主体标识。
- `diff policy`：必须有版本、租户和主体标识。

### Outputs
- `FrontendRuntimeDiff`：必须持久化或内容寻址，并可由下游 Skill 验证。
- `semantic/visual deltas`：必须持久化或内容寻址，并可由下游 Skill 验证。

机器合同：`contracts/batch-107/B107-S13.json`。
...
### Interfaces
- `POST /api/validations/frontend-diff`
- `event frontend-diff.completed`

### Persisted state
- `frontend_diff`：必须包含 tenant_id、version、created_at/updated_at，并采用服务端解析的租户身份。
- `semantic_delta`：必须包含 tenant_id、version、created_at/updated_at，并采用服务端解析的租户身份。
- `visual_delta`：必须包含 tenant_id、version、created_at/updated_at，并采用服务端解析的租户身份。
...
3. 比较可访问性树与表单行为。
4. 比较网络调用集合和响应类别。
5. 执行可选像素/布局差异并过滤允许动态区。
6. 综合为功能等价/可接受变化/破坏变化。
...
完成时至少产生：
- `frontend-runtime-diff.json`
- `semantic-deltas.json`
- `visual-diff-images/`
- `network-deltas.json`
