---
name: manual-intervention-ledger
description: 把人工修改、审批、豁免、测试义务和风险到期形成不可变账本。
---

# B108-S05 — manual-intervention-ledger

## Objective

把人工修改、审批、豁免、测试义务和风险到期形成不可变账本。
...
- `evidence/manual/ManualInterventionEvidenceService.java`
...
### Inputs
- `ManualInterventionLedger source`：必须有版本、租户和主体标识。
- `approvals`：必须有版本、租户和主体标识。
- `patches`：必须有版本、租户和主体标识。

### Outputs
- `SignedManualLedger`：必须持久化或内容寻址，并可由下游 Skill 验证。

机器合同：`contracts/batch-108/B108-S05.json`。
...
### Interfaces
- `POST /api/evidence/manual-ledgers`
- `event evidence.manual-ledger.completed`

### Persisted state
- `manual_intervention_evidence`：必须包含 tenant_id、version、created_at/updated_at，并采用服务端解析的租户身份。
- `manual_approval_evidence`：必须包含 tenant_id、version、created_at/updated_at，并采用服务端解析的租户身份。
...
完成时至少产生：
- `manual-intervention-ledger.json`
- `approval-signatures.json`
