---
name: dotnet-preview-runtime-adapter
description: 为Solution/csproj、ASP.NET Core、Minimal API、MVC、Razor和Blazor Server生成publish与运行规格。
---

# B106-S06 — dotnet-preview-runtime-adapter

## Objective

为Solution/csproj、ASP.NET Core、Minimal API、MVC、Razor和Blazor Server生成publish与运行规格。
...
- `runtime/adapters/dotnet/DotnetPreviewAdapter.java`
- `private-runner/internal/runtime/dotnet.go`
- `images/dotnet/`
...
- `.NET repository snapshot`：必须有版本、租户和主体标识。

### Outputs
- `DotnetBuildSpec`：必须持久化或内容寻址，并可由下游 Skill 验证。
- `DotnetRunSpec`：必须持久化或内容寻址，并可由下游 Skill 验证。
- `publish artifact manifest`：必须持久化或内容寻址，并可由下游 Skill 验证。

机器合同：`contracts/batch-106/B106-S06.json`。
...
### Interfaces
- `adapter runtime.dotnet.compile`
- `event runtime.adapter.dotnet.ready`

### Persisted state
- `dotnet_runtime_spec`：必须包含 tenant_id、version、created_at/updated_at，并采用服务端解析的租户身份。
- `dotnet_publish_artifact`：必须包含 tenant_id、version、created_at/updated_at，并采用服务端解析的租户身份。

所有写操作必须具备幂等键或乐观版本；跨进程副作用必须通过Outbox/Workflow receipt记录，不能依赖内存状态。

## Execution workflow

1. 解析global.json、sln和csproj。
2. 选择目标SDK/runtime固定镜像。
3. 执行restore与publish而非dev run。
4. 定位唯一入口DLL。
5. 设置ASPNETCORE_URLS和环境。
6. 发现HealthChecks、Swagger和Razor入口。
...
完成时至少产生：
- `dotnet-build-spec.json`
- `dotnet-run-spec.json`
