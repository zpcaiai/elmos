---
name: openapi-runtime-discovery
description: 从运行中服务发现并规范化OpenAPI，而不是只信任仓库静态文件。
---

# B107-S02 — openapi-runtime-discovery

## Objective

从运行中服务发现并规范化OpenAPI，而不是只信任仓库静态文件。
...
- `validation/openapi/OpenApiDiscoveryService.java`
- `adapters/openapi/normalizer/`
...
### Inputs
- `healthy PublicEndpoint`：必须有版本、租户和主体标识。
- `candidate paths`：必须有版本、租户和主体标识。
- `auth policy`：必须有版本、租户和主体标识。

### Outputs
- `RuntimeOpenAPI`：必须持久化或内容寻址，并可由下游 Skill 验证。
- `discovery trace`：必须持久化或内容寻址，并可由下游 Skill 验证。
- `spec digest`：必须持久化或内容寻址，并可由下游 Skill 验证。

机器合同：`contracts/batch-107/B107-S02.json`。
...
### Interfaces
- `POST /api/runtime-runs/{id}/openapi:discover`
- `event openapi.discovered`

### Persisted state
- `runtime_openapi`：必须包含 tenant_id、version、created_at/updated_at，并采用服务端解析的租户身份。
- `openapi_discovery_attempt`：必须包含 tenant_id、version、created_at/updated_at，并采用服务端解析的租户身份。

所有写操作必须具备幂等键或乐观版本；跨进程副作用必须通过Outbox/Workflow receipt记录，不能依赖内存状态。

## Execution workflow

1. 尝试声明路径和常见OpenAPI路径。
2. 使用临时认证请求。
3. 限制响应大小和重定向。
4. 解析JSON/YAML并执行规范校验。
5. 规范化server URL和无关生成顺序。
6. 保存原始与规范化摘要。
