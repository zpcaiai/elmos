---
name: rewrite-datatable-evidence-collector
description: 收集每阶段Recipe、版本、DataTable、差异、幂等和未解决结果，形成规则自动化证据。
---

# B108-S03 — rewrite-datatable-evidence-collector

## Objective

收集每阶段Recipe、版本、DataTable、差异、幂等和未解决结果，形成规则自动化证据。
...
- `evidence/rewrite/RewriteEvidenceCollector.java`
...
### Inputs
- `RewriteStageResults`：必须有版本、租户和主体标识。
- `EvidenceWorkspace`：必须有版本、租户和主体标识。

### Outputs
- `RewriteEvidenceBundle`：必须持久化或内容寻址，并可由下游 Skill 验证。

机器合同：`contracts/batch-108/B108-S03.json`。
...
### Interfaces
- `POST /api/evidence/rewrite:collect`
- `event evidence.rewrite.collected`

### Persisted state
- `rewrite_evidence`：必须包含 tenant_id、version、created_at/updated_at，并采用服务端解析的租户身份。
- `rewrite_recipe_execution`：必须包含 tenant_id、version、created_at/updated_at，并采用服务端解析的租户身份。

所有写操作必须具备幂等键或乐观版本；跨进程副作用必须通过Outbox/Workflow receipt记录，不能依赖内存状态。

## Execution workflow

1. 验证stage commit和Recipe lock。
2. 采集DataTable和日志。
3. 计算文件/行/AST单元变更。
4. 记录警告和未解决项。
5. 关联幂等第二次运行。
