---
name: python-preview-runtime-adapter
description: 为uv/pip/Poetry及FastAPI、Flask、Django、Gradio、Streamlit生成锁定环境与启动规格。
---

# B106-S05 — python-preview-runtime-adapter

## Objective

为uv/pip/Poetry及FastAPI、Flask、Django、Gradio、Streamlit生成锁定环境与启动规格。
...
- `runtime/adapters/python/PythonPreviewAdapter.java`
- `private-runner/internal/runtime/python.go`
- `images/python/`
...
- `Python environment snapshot`：必须有版本、租户和主体标识。

### Outputs
- `PythonBuildSpec`：必须持久化或内容寻址，并可由下游 Skill 验证。
- `PythonRunSpec`：必须持久化或内容寻址，并可由下游 Skill 验证。
- `dependency cache key`：必须持久化或内容寻址，并可由下游 Skill 验证。

机器合同：`contracts/batch-106/B106-S05.json`。
...
### Interfaces
- `adapter runtime.python.compile`
- `event runtime.adapter.python.ready`

### Persisted state
- `python_runtime_spec`：必须包含 tenant_id、version、created_at/updated_at，并采用服务端解析的租户身份。
- `python_env_lock`：必须包含 tenant_id、version、created_at/updated_at，并采用服务端解析的租户身份。

所有写操作必须具备幂等键或乐观版本；跨进程副作用必须通过Outbox/Workflow receipt记录，不能依赖内存状态。

## Execution workflow

1. 识别pyproject/requirements/lock文件。
2. 保留客户锁文件并固定Python实现/ABI。
3. 选择uv/pip/Poetry适配路径。
4. 检测ASGI/WSGI/Streamlit入口。
5. 注入host 0.0.0.0和PORT。
6. 输出wheel缓存、启动和健康规格。
...
- 无锁文件生成候选但标记风险。
- sdist构建默认无Secret且受网络限制。
- FastAPI入口正确。
- Django settings缺失返回阻断。
- ABI不兼容wheel被拒绝。
...
完成时至少产生：
- `python-build-spec.json`
- `python-run-spec.json`
