---
name: runtime-preview-evidence-schema
description: 定义并验证Run、Commit、Image、Endpoint、Ready/Expire/Destroy、健康、API、浏览器和资源信息的预览证据合同。
---

# B108-S06 — runtime-preview-evidence-schema

## Objective

定义并验证Run、Commit、Image、Endpoint、Ready/Expire/Destroy、健康、API、浏览器和资源信息的预览证据合同。
...
- `evidence/runtime/RuntimePreviewEvidenceService.java`
- `schemas/runtime-preview-evidence.schema.json`
...
- `runtime lifecycle receipts`：必须有版本、租户和主体标识。

### Outputs
- `RuntimePreviewEvidence`：必须持久化或内容寻址，并可由下游 Skill 验证。

机器合同：`contracts/batch-108/B108-S06.json`。
...
### Interfaces
- `POST /api/evidence/runtime-preview`
- `event evidence.runtime-preview.created`

### Persisted state
- `runtime_preview_evidence`：必须包含 tenant_id、version、created_at/updated_at，并采用服务端解析的租户身份。
...
1. 映射Run与Image血缘。
2. 纳入ready/expires/destroyed时间。
3. 仅保存Endpoint hash或受控引用。
4. 关联健康/API/浏览器Claims。
5. 记录资源/Provider/Policy摘要。
6. Schema验证并签名。
...
- destroyed_at早于ready_at拒绝。
- URL明文按Policy禁止。
- Claims无证据链接拒绝。
- Image digest缺失拒绝。
- Provider receipt签名验证。
...
完成时至少产生：
- `runtime-preview-evidence.json`
- `runtime-preview-signature.json`
