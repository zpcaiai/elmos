---
name: manual-fix-regression-obligation
description: 把Agent或人工无法自动证明的修复登记为受审批的干预项，并强制根因、回归测试、Owner和复核证据。
---

# B105-S11 — manual-fix-regression-obligation

## Objective

把Agent或人工无法自动证明的修复登记为受审批的干预项，并强制根因、回归测试、Owner和复核证据。
...
- `services/control-plane/.../governance/ManualFixService.java`
- `apps/web/src/features/manual-fixes/`
- `templates/MANUAL_FIXES.md`
...
- `human edits`：必须有版本、租户和主体标识。
- `policy exceptions`：必须有版本、租户和主体标识。

### Outputs
- `MANUAL_FIXES.md`：必须持久化或内容寻址，并可由下游 Skill 验证。
- `ManualInterventionLedger`：必须持久化或内容寻址，并可由下游 Skill 验证。
- `approval tasks`：必须持久化或内容寻址，并可由下游 Skill 验证。

机器合同：`contracts/batch-105/B105-S11.json`。
...
### Interfaces
- `POST /api/projects/{id}/manual-fixes`
- `POST /api/manual-fixes/{id}:approve`
- `event manual-fix.approved`

### Persisted state
- `manual_fix`：必须包含 tenant_id、version、created_at/updated_at，并采用服务端解析的租户身份。
- `manual_fix_approval`：必须包含 tenant_id、version、created_at/updated_at，并采用服务端解析的租户身份。
- `regression_obligation`：必须包含 tenant_id、version、created_at/updated_at，并采用服务端解析的租户身份。

所有写操作必须具备幂等键或乐观版本；跨进程副作用必须通过Outbox/Workflow receipt记录，不能依赖内存状态。

## Execution workflow

1. 检测非规则/非Agent actor的变更。
2. 要求填写根因、影响、Owner和风险。
3. 关联新增或更新测试。
4. 执行指定Gate并保存结果。
5. 高风险修复要求双人复核。
6. 生成MANUAL_FIXES.md和机器ledger。
