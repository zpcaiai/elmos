---
name: routing-and-trailing-slash-compatibility
description: 专项检查框架升级导致的尾斜杠、大小写、context path、代理前缀、重定向和CORS变化。
---

# B107-S08 — routing-and-trailing-slash-compatibility

## Objective

专项检查框架升级导致的尾斜杠、大小写、context path、代理前缀、重定向和CORS变化。
...
- `validation/http/RoutingCompatibilityService.java`
- `fixtures/routing/`
...
### Inputs
- `route inventory`：必须有版本、租户和主体标识。
- `baseline/candidate endpoints`：必须有版本、租户和主体标识。

### Outputs
- `RoutingCompatibilityResult`：必须持久化或内容寻址，并可由下游 Skill 验证。
- `route deltas`：必须持久化或内容寻址，并可由下游 Skill 验证。

机器合同：`contracts/batch-107/B107-S08.json`。
...
### Interfaces
- `POST /api/validations/routing`
- `event routing-validation.completed`

### Persisted state
- `route_variant`：必须包含 tenant_id、version、created_at/updated_at，并采用服务端解析的租户身份。
- `routing_observation`：必须包含 tenant_id、version、created_at/updated_at，并采用服务端解析的租户身份。
- `routing_delta`：必须包含 tenant_id、version、created_at/updated_at，并采用服务端解析的租户身份。

所有写操作必须具备幂等键或乐观版本；跨进程副作用必须通过Outbox/Workflow receipt记录，不能依赖内存状态。

## Execution workflow

1. 从OpenAPI和Controller构建路由清单。
2. 生成/path与/path/等变体。
3. 测试context path、proxy prefix和redirect。
4. 比较状态、Location、CORS和Body。
5. 识别刻意安全收紧与意外破坏。
6. 输出修复建议或显式兼容决定。
