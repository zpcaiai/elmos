---
name: runtime-health-probe-runner
description: 对真实实例执行HTTP、TCP或gRPC健康与就绪探针，提供退避、超时、响应断言和可复算结果。
---

# B107-S01 — runtime-health-probe-runner

## Objective

对真实实例执行HTTP、TCP或gRPC健康与就绪探针，提供退避、超时、响应断言和可复算结果。
...
- `validation/runtime/HealthProbeService.java`
- `private-runner/internal/probes/`
- `policies/health-probes.yaml`
...
### Inputs
- `READY candidate instance`：必须有版本、租户和主体标识。
- `health probe spec`：必须有版本、租户和主体标识。

### Outputs
- `HealthProbeResult`：必须持久化或内容寻址，并可由下游 Skill 验证。
- `readiness evidence`：必须持久化或内容寻址，并可由下游 Skill 验证。

机器合同：`contracts/batch-107/B107-S01.json`。
...
### Interfaces
- `POST /api/runtime-runs/{id}/probes`
- `event probe.completed`

### Persisted state
- `health_probe_run`：必须包含 tenant_id、version、created_at/updated_at，并采用服务端解析的租户身份。
- `health_probe_attempt`：必须包含 tenant_id、version、created_at/updated_at，并采用服务端解析的租户身份。
...
2. 从沙箱内外两个视角探测。
3. 应用退避、总超时和最大响应大小。
4. 验证状态码/JSON字段/gRPC serving。
5. 记录DNS/TLS/连接/首字节时间。
6. 聚合为PASS/FAIL/UNKNOWN。
