---
name: java-runtime-upgrade-analyzer
description: 分析Java 8/11到17/21的构建、JDK内部API、移除模块、插件、annotation processor和Jakarta风险。
---

# B105-S04 — java-runtime-upgrade-analyzer

## Objective

分析Java 8/11到17/21的构建、JDK内部API、移除模块、插件、annotation processor和Jakarta风险。
...
- `engines/java/src/main/java/.../analysis/JavaRuntimeUpgradeAnalyzer.java`
- `private-runner/internal/java/jdk_matrix.go`
- `rules/java-runtime/`
...
- `target JDK set`：必须有版本、租户和主体标识。

### Outputs
- `JavaUpgradeAnalysis`：必须持久化或内容寻址，并可由下游 Skill 验证。
- `JDK build matrix`：必须持久化或内容寻址，并可由下游 Skill 验证。
- `risk register`：必须持久化或内容寻址，并可由下游 Skill 验证。

机器合同：`contracts/batch-105/B105-S04.json`。
...
### Interfaces
- `POST /api/analyses/java-runtime`
- `event analysis.java-runtime.completed`

### Persisted state
- `java_runtime_analysis`：必须包含 tenant_id、version、created_at/updated_at，并采用服务端解析的租户身份。
- `jdk_matrix_result`：必须包含 tenant_id、version、created_at/updated_at，并采用服务端解析的租户身份。
- `java_risk`：必须包含 tenant_id、version、created_at/updated_at，并采用服务端解析的租户身份。

所有写操作必须具备幂等键或乐观版本；跨进程副作用必须通过Outbox/Workflow receipt记录，不能依赖内存状态。

## Execution workflow

1. 解析source/target/release/toolchain配置。
2. 在允许的JDK矩阵执行compile/test。
3. 运行jdeps、jdeprscan和内部API扫描。
4. 分类javax包为必须迁移与必须保留。
5. 检查Maven/Gradle插件和processor兼容性。
6. 输出P0-P4风险、证据和迁移建议。
...
- JDK11成功/JDK17失败被正确分类。
- javax.sql不得误改为jakarta.sql。
- sun.misc使用产生高风险。
- 插件与processor不兼容有定位。
- 矩阵超时被记录为UNKNOWN而非PASS。
...
- `jdeps.txt`
- `jdeprscan.txt`
- `javax-classification.json`
- `java-upgrade-analysis.md`
