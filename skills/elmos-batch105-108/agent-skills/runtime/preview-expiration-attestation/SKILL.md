---
name: preview-expiration-attestation
description: 独立验证到期路由撤销、沙箱销毁、卷删除、Secret撤销和无孤儿资源，生成销毁证明。
---

# B108-S07 — preview-expiration-attestation

## Objective

独立验证到期路由撤销、沙箱销毁、卷删除、Secret撤销和无孤儿资源，生成销毁证明。
...
- `evidence/runtime/ExpirationAttestationService.java`
...
### Inputs
- `RuntimePreviewEvidence`：必须有版本、租户和主体标识。
- `CleanupReceipt`：必须有版本、租户和主体标识。
- `provider inventory`：必须有版本、租户和主体标识。

### Outputs
- `ExpirationAttestation`：必须持久化或内容寻址，并可由下游 Skill 验证。
- `cleanup certification`：必须持久化或内容寻址，并可由下游 Skill 验证。

机器合同：`contracts/batch-108/B108-S07.json`。
...
### Interfaces
- `POST /api/runtime-runs/{id}/attest-expiration`
- `event expiration.attested`

### Persisted state
- `expiration_attestation`：必须包含 tenant_id、version、created_at/updated_at，并采用服务端解析的租户身份。
- `cleanup_check`：必须包含 tenant_id、version、created_at/updated_at，并采用服务端解析的租户身份。

所有写操作必须具备幂等键或乐观版本；跨进程副作用必须通过Outbox/Workflow receipt记录，不能依赖内存状态。

## Execution workflow

1. 验证Endpoint revocation receipt。
2. 查询Provider确认实例不存在/终止。
3. 验证临时卷与Secret撤销。
4. 在允许窗口后复查孤儿inventory。
5. 核对最终成本停止增长。
6. 签发PASS/FAIL和缺失项。
...
- 仅应用标DESTROYED但Provider仍运行时FAIL。
- 路由仍可访问FAIL。
- 重复Attest幂等。
- Provider不可达标UNKNOWN。
- 成本继续增长FAIL。
...
完成时至少产生：
- `expiration-attestation.json`
- `provider-inventory-check.json`
- `route-recheck.json`
