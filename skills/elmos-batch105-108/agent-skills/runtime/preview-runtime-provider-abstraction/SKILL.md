---
name: preview-runtime-provider-abstraction
description: 定义可替换Sandbox Provider接口、能力协商、错误模型和认证测试，支持云沙箱、Fly、Kata及私有Runner。
---

# B106-S16 — preview-runtime-provider-abstraction

## Objective

定义可替换Sandbox Provider接口、能力协商、错误模型和认证测试，支持云沙箱、Fly、Kata及私有Runner。
...
- `runtime/providers/SandboxProvider.java`
- `runtime/providers/vercel/`
- `runtime/providers/e2b/`
- `runtime/providers/fly/`
- `runtime/providers/kata/`
...
### Inputs
- `provider adapters`：必须有版本、租户和主体标识。
- `capability declarations`：必须有版本、租户和主体标识。
- `certification fixtures`：必须有版本、租户和主体标识。

### Outputs
- `SandboxProvider SPI`：必须持久化或内容寻址，并可由下游 Skill 验证。
- `provider capability matrix`：必须持久化或内容寻址，并可由下游 Skill 验证。
- `certification results`：必须持久化或内容寻址，并可由下游 Skill 验证。

机器合同：`contracts/batch-106/B106-S16.json`。
...
### Interfaces
- `SPI create/exec/exposePort/streamLogs/extend/stop/destroy/snapshot`
- `event provider.certified`

### Persisted state
- `provider_registration`：必须包含 tenant_id、version、created_at/updated_at，并采用服务端解析的租户身份。
- `provider_capability`：必须包含 tenant_id、version、created_at/updated_at，并采用服务端解析的租户身份。
- `provider_certification`：必须包含 tenant_id、version、created_at/updated_at，并采用服务端解析的租户身份。

所有写操作必须具备幂等键或乐观版本；跨进程副作用必须通过Outbox/Workflow receipt记录，不能依赖内存状态。

## Execution workflow

1. 定义稳定Provider接口和统一错误码。
2. 实现能力协商与最小公共语义。
3. 适配身份、超时、网络和端口差异。
4. 运行创建/执行/暴露/延期/销毁契约测试。
5. 运行失败注入和幂等测试。
6. 只有认证Provider进入生产路由。
