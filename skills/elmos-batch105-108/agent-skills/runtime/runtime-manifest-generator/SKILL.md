---
name: runtime-manifest-generator
description: 把检测结果和用户选择编译为语言无关的runtime.yaml，统一构建、启动、端口、健康、预览、资源、网络和TTL。
---

# B106-S02 — runtime-manifest-generator

## Objective

把检测结果和用户选择编译为语言无关的runtime.yaml，统一构建、启动、端口、健康、预览、资源、网络和TTL。
...
- `runtime/manifest/RuntimeManifestService.java`
- `contracts/runtime-manifest.schema.json`
- `apps/web/src/features/runtime-manifest/`
...
### Inputs
- `RuntimeDetectionResult`：必须有版本、租户和主体标识。
- `selected service`：必须有版本、租户和主体标识。
- `tenant policy`：必须有版本、租户和主体标识。

### Outputs
- `RuntimeManifest`：必须持久化或内容寻址，并可由下游 Skill 验证。
- `manifest digest`：必须持久化或内容寻址，并可由下游 Skill 验证。
- `human preview`：必须持久化或内容寻址，并可由下游 Skill 验证。

机器合同：`contracts/batch-106/B106-S02.json`。
...
### Interfaces
- `POST /api/projects/{id}/runtime-manifests`
- `event runtime-manifest.generated`

### Persisted state
- `runtime_manifest`：必须包含 tenant_id、version、created_at/updated_at，并采用服务端解析的租户身份。
- `runtime_command`：必须包含 tenant_id、version、created_at/updated_at，并采用服务端解析的租户身份。
- `runtime_resource_policy`：必须包含 tenant_id、version、created_at/updated_at，并采用服务端解析的租户身份。
...
1. 选择检测候选和适配器。
2. 规范化build/start命令为argv。
3. 注入PORT/host而不覆盖用户Secret。
4. 配置health、homepage、OpenAPI和协议。
5. 应用CPU/内存/磁盘/TTL/网络政策。
6. 验证Schema并计算不可变摘要。
