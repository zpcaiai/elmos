---
name: page-semantic-assertion-runner
description: 通过可访问性树与语义定位验证标题、导航、按钮、表格、表单、登录状态和关键文本。
---

# B107-S11 — page-semantic-assertion-runner

## Objective

通过可访问性树与语义定位验证标题、导航、按钮、表格、表单、登录状态和关键文本。
...
- `validation/browser/PageSemanticAssertionService.java`
- `contracts/page-assertion.schema.json`
- `fixtures/page-assertions/`
...
- `semantic assertion suite`：必须有版本、租户和主体标识。

### Outputs
- `SemanticAssertionResults`：必须持久化或内容寻址，并可由下游 Skill 验证。
- `accessibility snapshots`：必须持久化或内容寻址，并可由下游 Skill 验证。

机器合同：`contracts/batch-107/B107-S11.json`。
...
### Interfaces
- `POST /api/browser-sessions/{id}/assertions`
- `event page-assertions.completed`

### Persisted state
- `page_assertion`：必须包含 tenant_id、version、created_at/updated_at，并采用服务端解析的租户身份。
- `semantic_snapshot`：必须包含 tenant_id、version、created_at/updated_at，并采用服务端解析的租户身份。
- `assertion_result`：必须包含 tenant_id、version、created_at/updated_at，并采用服务端解析的租户身份。
