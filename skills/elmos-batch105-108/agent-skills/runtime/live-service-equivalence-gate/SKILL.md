---
name: live-service-equivalence-gate
description: 对LiveValidationBundle执行独立保守判定，签发HEALTHY、API_COMPATIBLE、PAGE_RENDERED、INTERACTION_PASSED等能力。
---

# B107-S16 — live-service-equivalence-gate

## Objective

对LiveValidationBundle执行独立保守判定，签发HEALTHY、API_COMPATIBLE、PAGE_RENDERED、INTERACTION_PASSED等能力。
...
- `certification/live/LiveServiceGate.java`
- `policies/live-service-gate.yaml`
...
### Inputs
- `LiveValidationBundle`：必须有版本、租户和主体标识。
- `live service policy`：必须有版本、租户和主体标识。

### Outputs
- `LiveServiceDecision`：必须持久化或内容寻址，并可由下游 Skill 验证。
- `capability claims`：必须持久化或内容寻址，并可由下游 Skill 验证。
- `blocking findings`：必须持久化或内容寻址，并可由下游 Skill 验证。

机器合同：`contracts/batch-107/B107-S16.json`。
...
### Interfaces
- `POST /api/runtime-runs/{id}/gates/live-service`
- `event gate.live-service.decided`

### Persisted state
- `live_service_decision`：必须包含 tenant_id、version、created_at/updated_at，并采用服务端解析的租户身份。
- `capability_claim`：必须包含 tenant_id、version、created_at/updated_at，并采用服务端解析的租户身份。
- `live_blocking_finding`：必须包含 tenant_id、version、created_at/updated_at，并采用服务端解析的租户身份。

所有写操作必须具备幂等键或乐观版本；跨进程副作用必须通过Outbox/Workflow receipt记录，不能依赖内存状态。

## Execution workflow

1. 独立读取Bundle而非信任上游状态字段。
2. 验证健康和启动证据。
3. 验证API兼容及业务不变量。
4. 验证页面、Console、交互和截图。
5. 按服务声明决定是否要求Streaming。
6. 签发细粒度Claims和总结果。
