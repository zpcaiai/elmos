---
name: preview-image-cache-and-snapshot-manager
description: 管理语言基础镜像、依赖缓存和沙箱快照，加速Build-to-Ready同时保持租户和供应链隔离。
---

# B106-S09 — preview-image-cache-and-snapshot-manager

## Objective

管理语言基础镜像、依赖缓存和沙箱快照，加速Build-to-Ready同时保持租户和供应链隔离。
...
- `runtime/cache/PreviewCacheService.java`
- `private-runner/internal/cache/manager.go`
- `infra/cache/`
...
### Inputs
- `OCI build specs`：必须有版本、租户和主体标识。
- `cache policy`：必须有版本、租户和主体标识。
- `provider snapshot capabilities`：必须有版本、租户和主体标识。

### Outputs
- `CacheEntry`：必须持久化或内容寻址，并可由下游 Skill 验证。
- `SandboxSnapshot`：必须持久化或内容寻址，并可由下游 Skill 验证。
- `cache metrics`：必须持久化或内容寻址，并可由下游 Skill 验证。

机器合同：`contracts/batch-106/B106-S09.json`。
...
### Interfaces
- `POST /api/runtime-cache:warm`
- `DELETE /api/runtime-cache/{id}`
- `event runtime-cache.updated`

### Persisted state
- `preview_cache_entry`：必须包含 tenant_id、version、created_at/updated_at，并采用服务端解析的租户身份。
- `sandbox_snapshot`：必须包含 tenant_id、version、created_at/updated_at，并采用服务端解析的租户身份。
- `cache_access`：必须包含 tenant_id、version、created_at/updated_at，并采用服务端解析的租户身份。

所有写操作必须具备幂等键或乐观版本；跨进程副作用必须通过Outbox/Workflow receipt记录，不能依赖内存状态。

## Execution workflow

1. 定义JDK/Maven、Python/uv、.NET、Go缓存键。
2. 区分公共只读缓存与租户私有缓存。
3. 构建并签名预热快照。
4. 验证快照无凭证和客户代码。
5. 按TTL/LRU/风险事件失效。
