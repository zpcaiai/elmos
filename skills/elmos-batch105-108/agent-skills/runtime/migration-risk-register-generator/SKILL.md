---
name: migration-risk-register-generator
description: 汇总代码、数据、API、安全、性能、运维、部署、外部消费者和未验证范围的剩余风险。
---

# B108-S10 — migration-risk-register-generator

## Objective

汇总代码、数据、API、安全、性能、运维、部署、外部消费者和未验证范围的剩余风险。
...
- `reporting/risk/MigrationRiskService.java`
- `apps/web/src/features/risk-register/`
...
### Inputs
- `all gate decisions`：必须有版本、租户和主体标识。
- `unverified scope`：必须有版本、租户和主体标识。
- `known failures`：必须有版本、租户和主体标识。

### Outputs
- `MigrationRiskRegister`：必须持久化或内容寻址，并可由下游 Skill 验证。
- `risk heatmap`：必须持久化或内容寻址，并可由下游 Skill 验证。
- `mitigation plan`：必须持久化或内容寻址，并可由下游 Skill 验证。

机器合同：`contracts/batch-108/B108-S10.json`。
...
- `event risk-register.generated`

### Persisted state
- `migration_risk`：必须包含 tenant_id、version、created_at/updated_at，并采用服务端解析的租户身份。
- `risk_mitigation`：必须包含 tenant_id、version、created_at/updated_at，并采用服务端解析的租户身份。
- `risk_acceptance`：必须包含 tenant_id、version、created_at/updated_at，并采用服务端解析的租户身份。

所有写操作必须具备幂等键或乐观版本；跨进程副作用必须通过Outbox/Workflow receipt记录，不能依赖内存状态。

## Execution workflow

1. 从阻断/警告/UNKNOWN提取风险。
2. 按影响、可能性、可检测性评分。
3. 区分已验证与未验证范围。
4. 指定Owner、缓解、截止和证据。
5. 风险接受需审批且有期限。
