---
name: http-scenario-suite-generator
description: 从OpenAPI、Controller、现有测试、Recorded Traffic和Postman生成覆盖关键业务路径的HTTP场景集。
---

# B107-S05 — http-scenario-suite-generator

## Objective

从OpenAPI、Controller、现有测试、Recorded Traffic和Postman生成覆盖关键业务路径的HTTP场景集。
...
- `validation/http/ScenarioGenerator.java`
- `contracts/http-scenario.schema.json`
- `apps/web/src/features/scenario-review/`
...
### Inputs
- `OpenAPI`：必须有版本、租户和主体标识。
- `source/test index`：必须有版本、租户和主体标识。
- `optional traffic/collections`：必须有版本、租户和主体标识。

### Outputs
- `HTTPScenarioSuite`：必须持久化或内容寻址，并可由下游 Skill 验证。
- `coverage map`：必须持久化或内容寻址，并可由下游 Skill 验证。
- `data requirements`：必须持久化或内容寻址，并可由下游 Skill 验证。

机器合同：`contracts/batch-107/B107-S05.json`。
...
### Interfaces
- `POST /api/scenario-suites/http:generate`
- `event scenario-suite.generated`

### Persisted state
- `http_scenario_suite`：必须包含 tenant_id、version、created_at/updated_at，并采用服务端解析的租户身份。
- `http_scenario`：必须包含 tenant_id、version、created_at/updated_at，并采用服务端解析的租户身份。
- `scenario_coverage`：必须包含 tenant_id、version、created_at/updated_at，并采用服务端解析的租户身份。
