---
name: go-preview-runtime-adapter
description: 为Go Modules及net/http、Gin、Fiber、Echo、Chi和gRPC Gateway生成可复现单二进制运行规格。
---

# B106-S07 — go-preview-runtime-adapter

## Objective

为Go Modules及net/http、Gin、Fiber、Echo、Chi和gRPC Gateway生成可复现单二进制运行规格。
...
- `runtime/adapters/go/GoPreviewAdapter.java`
- `private-runner/internal/runtime/go.go`
- `images/go/`
...
- `Go repository snapshot`：必须有版本、租户和主体标识。

### Outputs
- `GoBuildSpec`：必须持久化或内容寻址，并可由下游 Skill 验证。
- `GoRunSpec`：必须持久化或内容寻址，并可由下游 Skill 验证。
- `module cache key`：必须持久化或内容寻址，并可由下游 Skill 验证。

机器合同：`contracts/batch-106/B106-S07.json`。
...
### Interfaces
- `adapter runtime.go.compile`
- `event runtime.adapter.go.ready`

### Persisted state
- `go_runtime_spec`：必须包含 tenant_id、version、created_at/updated_at，并采用服务端解析的租户身份。
- `go_build_artifact`：必须包含 tenant_id、version、created_at/updated_at，并采用服务端解析的租户身份。

所有写操作必须具备幂等键或乐观版本；跨进程副作用必须通过Outbox/Workflow receipt记录，不能依赖内存状态。

## Execution workflow

1. 解析go.mod/toolchain/workspace。
2. 发现cmd目录和main package。
3. 固定GOTOOLCHAIN与module proxy策略。
4. 执行CGO策略明确的构建。
5. 注入PORT并检查监听地址。
6. 识别HTTP、gRPC Gateway和静态文件。
