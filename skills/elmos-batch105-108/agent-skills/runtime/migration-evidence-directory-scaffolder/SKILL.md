---
name: migration-evidence-directory-scaffolder
description: 为每个项目创建before/rewrite/agent/after/runtime/certification标准证据布局和索引。
---

# B108-S01 — migration-evidence-directory-scaffolder

## Objective

为每个项目创建before/rewrite/agent/after/runtime/certification标准证据布局和索引。
...
- `evidence/scaffold/EvidenceWorkspaceService.java`
- `templates/evidence-layout/`
...
### Inputs
- `project/run IDs`：必须有版本、租户和主体标识。
- `evidence policy`：必须有版本、租户和主体标识。

### Outputs
- `EvidenceWorkspace`：必须持久化或内容寻址，并可由下游 Skill 验证。
- `artifact index template`：必须持久化或内容寻址，并可由下游 Skill 验证。

机器合同：`contracts/batch-108/B108-S01.json`。
...
### Interfaces
- `POST /api/projects/{id}/evidence-workspaces`
- `event evidence.workspace.created`

### Persisted state
- `evidence_workspace`：必须包含 tenant_id、version、created_at/updated_at，并采用服务端解析的租户身份。
- `evidence_slot`：必须包含 tenant_id、version、created_at/updated_at，并采用服务端解析的租户身份。
