---
name: desktop-and-mobile-screenshot-capture
description: 按固定viewport、字体、时区和数据种子生成桌面、移动、全页与失败截图，并计算摘要。
---

# B107-S12 — desktop-and-mobile-screenshot-capture

## Objective

按固定viewport、字体、时区和数据种子生成桌面、移动、全页与失败截图，并计算摘要。
...
- `validation/browser/ScreenshotService.java`
- `private-runner/internal/browser/screenshot.go`
...
- `screenshot plan`：必须有版本、租户和主体标识。

### Outputs
- `ScreenshotArtifacts`：必须持久化或内容寻址，并可由下游 Skill 验证。
- `visual metadata`：必须持久化或内容寻址，并可由下游 Skill 验证。

机器合同：`contracts/batch-107/B107-S12.json`。
...
### Interfaces
- `POST /api/browser-sessions/{id}/screenshots`
- `event screenshots.captured`

### Persisted state
- `screenshot_artifact`：必须包含 tenant_id、version、created_at/updated_at，并采用服务端解析的租户身份。
- `visual_capture_metadata`：必须包含 tenant_id、version、created_at/updated_at，并采用服务端解析的租户身份。

所有写操作必须具备幂等键或乐观版本；跨进程副作用必须通过Outbox/Workflow receipt记录，不能依赖内存状态。

## Execution workflow

1. 固定viewport/DPR/locale/timezone/animation策略。
2. 等待字体和网络稳定。
3. 捕获桌面/移动/全页/元素截图。
4. 失败时捕获当前页面和Console摘要。
5. 压缩并计算SHA256。
6. 关联Run、Commit、Journey step和URL hash。
