...
description: 比较基线与候选OpenAPI的path、method、参数、Schema、响应、媒体类型和Security，保守拦截破坏。
---

# B107-S03 — openapi-breaking-change-gate

## Objective

比较基线与候选OpenAPI的path、method、参数、Schema、响应、媒体类型和Security，保守拦截破坏。
...
- `validation/openapi/OpenApiCompatibilityGate.java`
- `adapters/oasdiff/`
- `policies/openapi-compatibility.yaml`
...
### Inputs
- `baseline OpenAPI`：必须有版本、租户和主体标识。
- `runtime candidate OpenAPI`：必须有版本、租户和主体标识。
- `compatibility policy`：必须有版本、租户和主体标识。

### Outputs
- `OpenAPIDiff`：必须持久化或内容寻址，并可由下游 Skill 验证。
- `breaking-change decision`：必须持久化或内容寻址，并可由下游 Skill 验证。

机器合同：`contracts/batch-107/B107-S03.json`。
...
### Interfaces
- `POST /api/validations/openapi-diff`
- `event gate.openapi.decided`

### Persisted state
- `openapi_diff`：必须包含 tenant_id、version、created_at/updated_at，并采用服务端解析的租户身份。
- `api_breaking_change`：必须包含 tenant_id、version、created_at/updated_at，并采用服务端解析的租户身份。
- `api_gate_decision`：必须包含 tenant_id、version、created_at/updated_at，并采用服务端解析的租户身份。
...
- `allowlist-evaluation.json`
- `api-gate-decision.json`
