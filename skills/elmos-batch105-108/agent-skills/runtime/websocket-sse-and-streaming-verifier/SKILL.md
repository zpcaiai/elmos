---
name: websocket-sse-and-streaming-verifier
description: 验证WebSocket握手/消息顺序、SSE事件与重连、流式HTTP首字节、取消和超时。
---

# B107-S14 — websocket-sse-and-streaming-verifier

## Objective

验证WebSocket握手/消息顺序、SSE事件与重连、流式HTTP首字节、取消和超时。
...
- `validation/streaming/StreamingProtocolService.java`
- `private-runner/internal/streaming/`
...
### Inputs
- `streaming endpoint specs`：必须有版本、租户和主体标识。
- `baseline/candidate endpoints`：必须有版本、租户和主体标识。

### Outputs
- `StreamingValidationResult`：必须持久化或内容寻址，并可由下游 Skill 验证。
- `protocol traces`：必须持久化或内容寻址，并可由下游 Skill 验证。

机器合同：`contracts/batch-107/B107-S14.json`。
...
### Interfaces
- `POST /api/validations/streaming`
- `event streaming-validation.completed`

### Persisted state
- `streaming_scenario`：必须包含 tenant_id、version、created_at/updated_at，并采用服务端解析的租户身份。
- `stream_event`：必须包含 tenant_id、version、created_at/updated_at，并采用服务端解析的租户身份。
- `streaming_result`：必须包含 tenant_id、version、created_at/updated_at，并采用服务端解析的租户身份。

所有写操作必须具备幂等键或乐观版本；跨进程副作用必须通过Outbox/Workflow receipt记录，不能依赖内存状态。

## Execution workflow

1. 建立带认证的WS/SSE/HTTP流连接。
2. 验证握手、协议和Header。
3. 发送/接收有序消息并记录时间。
4. 模拟断线、重连、取消和慢消费者。
5. 比较基线候选事件语义。
