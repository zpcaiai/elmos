...
## Verify
执行本skill的acceptance.yaml对应真实测试，并检查原始报告而非Agent总结。关联输出：ContractSnapshot, OpenAPI/AsyncAPI/proto projections, ContractApproval。
至少演练：有状态接口丢失effect投影；必须观察：loss report阻断相应保证。
