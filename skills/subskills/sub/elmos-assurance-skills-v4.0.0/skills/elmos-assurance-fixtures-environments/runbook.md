...
## Verify
执行本skill的acceptance.yaml对应真实测试，并检查原始报告而非Agent总结。关联输出：EnvironmentLock, FixtureSnapshots, CleanupEvidence。
至少演练：没有目标DB运行时；必须观察：native status NOT_RUN，不能标DB回归通过。
