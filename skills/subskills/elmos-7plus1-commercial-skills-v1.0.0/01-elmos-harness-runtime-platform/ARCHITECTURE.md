# P01 架构设计：Elmos 可插拔 Harness 运行时平台
...
flowchart LR
    ...
    P00 --> THIS[P01 Elmos Harness Runtime Platform]
...
```mermaid
flowchart TD
        C1["Harness SPI & Adapter SDK"]
        C2["Reversible Plugin Context"]
        C3["Event-Sourced Session Runtime"]
        C4["Tool Runtime"]
        C5["Async Task Runtime"]
        C6["Subagent Runtime"]
        C7["Permission & Approval Plane"]
        C8["Sandbox & Workspace Runtime"]
        C9["Context & Compaction Runtime"]
        C10["LSP Capability Seam"]
        C11["Runtime Server & SDK"]
        C12["Readiness & Conformance"]
        C1 --> C2
        C2 --> C3
        C3 --> C4
        C4 --> C5
...
| 组件 | 职责 |
| --- | --- |
| Harness SPI & Adapter SDK | 稳定的能力发现、启动、执行、取消、状态、事件和错误合同。 |
| Reversible Plugin Context | Service Definition/Provider/Consumer、可逆注册、作用域与热重载。 |
| Event-Sourced Session Runtime | 追加日志、fork/resume/replay、上下文 Epoch、崩溃闭合与格式版本。 |
| Tool Runtime | 严格 Schema、输出标准化、Hooks、exclusive/parallel、timeouts、approval 和 finalizer。 |
| Async Task Runtime | sync/background/deferred、跨进程 resolve/fail/cancel、task check/steer/result。 |
| Subagent Runtime | one-shot/continuable child、持久 lineage、FIFO、冷恢复、直接父授权。 |
| Permission & Approval Plane | action-resource 策略、硬拒绝、临时/持久授权、可审计决策。 |
| Sandbox & Workspace Runtime | read-only/workspace-write/full access、执行事实、worktree 与临时目录。 |
| Context & Compaction Runtime | prompt admission、safe turn boundary、micro/full compaction、结构化 carryover。 |
| LSP Capability Seam | 按扩展选择 Provider，规范化 definition/reference/implementation/hover。 |
| Runtime Server & SDK | OpenAPI、SSE、Session/Tool/File/LSP/MCP/Permission API、多客户端。 |
| Readiness & Conformance | dry-run 检测模型、工具、沙箱、路径、凭据、网络和 Adapter 能力。 |
