...
| 接口 | 输入 | 输出 | 稳定错误码 |
| --- | --- | --- | --- |
| RequirementEngine.expand | user request + archetype hints + constraints | Requirement Graph + questions/gaps | REQUIREMENT_CONFLICT |
| ArchitectureSynthesizer.design | requirements + target stack + policies | architecture model + ADRs + task DAG | TARGET_INFEASIBLE |
| RuleEngine.transform | source IR node + target profile + rule set | target IR + rule evidence | NO_SAFE_RULE / RULE_CONFLICT |
| Emitter.generate | target IR shard + coding policy | code artifacts + source map | EMISSION_FAILED |
| MigrationController.plan | source/target systems + risk policy | phased migration plan + rollback | NON_REVERSIBLE_CHANGE |
| GapManager.resolve | semantic gap + candidates + constraints | approved decision or blocker | DECISION_REQUIRED |
