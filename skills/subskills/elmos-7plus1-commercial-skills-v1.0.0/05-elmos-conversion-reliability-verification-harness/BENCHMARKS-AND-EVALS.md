...
- 构造“build pass 但漏 MQ/Cron/Auth/Transaction”的陷阱仓库，验证 Gate 阻止完成。
- 对源/目标运行同一场景并注入 DB/cache/MQ/exception/auth 差异。
- 执行 mutation testing，验证测试能够杀死关键业务规则变异。
- 对 UI 关键流程执行 DOM/state/screenshot/video 比较并区分非语义视觉噪声。
- 注入网络延迟、进程崩溃、消息重复、数据库回滚、缓存失效和磁盘压力。
- 故意篡改/过期证据、环境 mismatch 和 waiver，验证 fail closed。
