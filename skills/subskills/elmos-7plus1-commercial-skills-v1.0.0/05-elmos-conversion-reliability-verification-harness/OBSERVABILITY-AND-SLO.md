...
| 指标 | 目标 | 说明 |
| --- | --- | --- |
| Gate evaluation | P95 < 5s | 证据已索引 |
| Evidence traceability | 100% | Gate 中每个结论有 evidence ref |
| Critical test flake rate | <0.5% | 稳定基准环境 |
| Differential scenario reproducibility | >99% | 固定 seed/environment |
| False completion rate | 目标接近 0 | 以事后发现 critical gap 统计 |
| Repair loop boundedness | 100% | 有 max rounds/cost/time/no-progress |
