...
| 接口 | 输入 | 输出 | 稳定错误码 |
| --- | --- | --- | --- |
| PackageRegistry.resolve | package id + requested capability + version range | resolved package graph + compatibility decision | PACKAGE_NOT_FOUND / VERSION_CONFLICT |
| WorkflowCompiler.compile | ELMOS_WORKFLOW.md + repository metadata | immutable execution contract | WORKFLOW_INVALID / UNSAFE_HOOK |
| ReleaseManager.certify | release candidate + evidence bundle refs | certification decision + signed release manifest | GATE_FAILED / EVIDENCE_STALE |
| ControlPlane.createJob | tenant/project/request/budget/policy | job id + initial ETA/cost envelope | QUOTA_EXCEEDED / POLICY_DENIED |
| ConfigManager.publish | versioned config candidate | active config revision or rejection | SCHEMA_INVALID / CANARY_FAILED |
