...
| 指标 | 目标 | 说明 |
| --- | --- | --- |
| Scheduler reconciliation freshness | P95 < 15s | 事件+轮询模式 |
| Ready-task dispatch | P95 < 10s under capacity | 不含 workspace bootstrap |
| Workspace preparation success | >99% | 受支持仓库 |
| Stalled-run detection | < 2 poll intervals | 可配置 |
| Run state recovery | P95 < 60s | 无不可恢复外部 blocker |
| Workpad/evidence consistency | 100% | 状态转移前检查 |
