...
- 机密代码默认只路由到批准的 ZDR/BYOK/私有部署 Provider。
- 模型/Provider 密钥由 Broker 托管，Agent 只获得 scoped invocation capability。
- 路由日志不保存原始 prompt，使用 hash、分类和必要统计；可配置审计采样。
- 第三方 benchmark/health 数据不作为授权依据；授权来自租户策略。
- 跨 Provider fallback 保持或收紧数据政策，绝不放宽。
