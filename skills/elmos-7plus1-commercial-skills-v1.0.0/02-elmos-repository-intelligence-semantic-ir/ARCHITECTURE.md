...
flowchart LR
    ...
    P00 --> THIS[P02 Elmos Repository Intelligence & Semantic IR]
...
```mermaid
flowchart TD
        C1["Repository Inventory Scanner"]
        C2["Language & Framework Detectors"]
        C3["AST & Symbol Index"]
        C4["LSP Semantic Navigation"]
        C5["Program Graph Builder"]
        C6["Platform Graph Builder"]
        C7["Runtime Trace Ingestor"]
        C8["Canonical Semantic IR"]
        C9["Capability Discovery Engine"]
        C10["Provenance & Confidence Engine"]
        C11["Incremental Analysis Cache"]
        C12["Repository Query Service"]
        C1 --> C2
        C2 --> C3
        C3 --> C4
        C4 --> C5
...
| 组件 | 职责 |
| --- | --- |
| Repository Inventory Scanner | 文件、模块、构建系统、依赖、配置、测试、部署与资产清单。 |
| Language & Framework Detectors | 语言版本、框架、插件、代码生成器、运行时和约定识别。 |
| AST & Symbol Index | 多语言解析、符号表、类型、引用、继承/实现和注解。 |
| LSP Semantic Navigation | definition/reference/implementation/hover 及诊断补充。 |
| Program Graph Builder | dependency/call/control/data-flow/side-effect/concurrency graph。 |
| Platform Graph Builder | API/DB/MQ/cache/cron/security/config/infra/deployment graph。 |
| Runtime Trace Ingestor | 请求、SQL、消息、缓存、文件、锁、指标和异常 Trace 对齐。 |
| Canonical Semantic IR | 类型、函数、状态、契约、副作用、事务、并发和平台能力表示。 |
| Capability Discovery Engine | 业务/技术能力识别、聚类、依赖和 Capability Ledger。 |
| Provenance & Confidence Engine | 证据来源、置信度、冲突、blind spot 和 freshness。 |
| Incremental Analysis Cache | 内容寻址、变更影响分析、分区更新和可复现 snapshot。 |
| Repository Query Service | 图查询、语义检索、影响分析和下游批量导出。 |
