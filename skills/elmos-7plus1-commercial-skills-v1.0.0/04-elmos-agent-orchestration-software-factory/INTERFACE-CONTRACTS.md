...
| 接口 | 输入 | 输出 | 稳定错误码 |
| --- | --- | --- | --- |
| TrackerAdapter.fetchCandidates | workflow tracker policy + cursor | normalized issue batch | TRACKER_UNAVAILABLE |
| Scheduler.reconcile | issue snapshot + active run snapshot | start/continue/stop/retry/remove actions | STATE_CONFLICT |
| TaskOrchestrator.dispatch | ready task + resource policy | run assignment | NO_CAPACITY / DEPENDENCY_BLOCKED |
| WorkspaceManager.prepare | task id + source revision + hook policy | workspace lease | WORKSPACE_UNSAFE |
| AgentRegistry.compose | role + task contract + route decision | agent execution profile | ROLE_UNAVAILABLE |
| ReviewCoordinator.sweep | PR ref + policy | resolved/outstanding feedback set | REVIEW_ACCESS_DENIED |
| ProofAssembler.build | task + evidence refs | proof-of-work bundle | EVIDENCE_INCOMPLETE |
