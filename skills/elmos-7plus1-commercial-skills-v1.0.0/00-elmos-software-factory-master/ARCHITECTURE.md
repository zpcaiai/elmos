...
flowchart LR
    ...
    P00 --> THIS[P00 Elmos Software Factory Master Control]
...
flowchart TD
        C1["Package Registry"]
        C2["Workflow Compiler"]
        C3["Architecture Governor"]
        C4["Repository Knowledge System"]
        C5["Commercial Control Plane"]
        C6["Release & Certification Manager"]
        C7["Configuration & Feature Governance"]
        C8["Upstream Intelligence"]
        C1 --> C2
        C2 --> C3
        C3 --> C4
        C4 --> C5
...
| 组件 | 职责 |
| --- | --- |
| Package Registry | 登记包、子技能、依赖、兼容版本、owner、maturity、feature flag 和弃用状态。 |
| Workflow Compiler | 把仓库内工作流合同编译为任务模板、策略快照、Hooks、状态机和验收门。 |
| Architecture Governor | 执行分层、依赖方向、公共 API、Schema/事件兼容和禁止路径等结构测试。 |
| Repository Knowledge System | 维护 AGENTS 地图、产品规范、架构文档、执行计划、运行手册和决策记录。 |
| Commercial Control Plane | 管理 tenant/project/job/quota/cost/revenue/ETA/billing/SLA/support tier。 |
| Release & Certification Manager | 组织版本冻结、迁移、灰度、回滚、E1–E5 认证和审计签名。 |
| Configuration & Feature Governance | 动态配置、灰度、实验、last-known-good、kill switch 和配置审计。 |
| Upstream Intelligence | 跟踪 Harness/SDK breaking changes、许可证、漏洞和适配器兼容性。 |
