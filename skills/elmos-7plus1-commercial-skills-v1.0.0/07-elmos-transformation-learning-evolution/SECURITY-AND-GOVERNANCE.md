...
- 知识 scope 默认为 tenant-private；提升到 organization/global 必须有授权和不可逆脱敏检查。
- 训练数据执行 Secret/PII/license/IP 检测、去重和可删除索引。
- 防止 Agent 通过伪造测试或证据污染规则：只接受 P05 签名 evidence。
- 知识检索结果按调用租户和用途 ACL 过滤。
- 支持客户要求的数据删除、模型再训练影响评估和 corpus tombstone。
