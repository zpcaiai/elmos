...
| 接口 | 输入 | 输出 | 稳定错误码 |
| --- | --- | --- | --- |
| TransformationKB.query | source/target IR pattern + versions + constraints | ranked applicable rules | NO_TRUSTED_RULE |
| RepairCorpus.retrieve | failure signature + context | ranked repair traces | NO_SIMILAR_CASE |
| RuleGovernance.propose | candidate rule + evidence refs | candidate id + required validation plan | PROVENANCE_INSUFFICIENT |
| RuleGovernance.promote | candidate id + cross-project evidence | new maturity or rejection | PROMOTION_GATE_FAILED |
| BenchmarkRunner.replay | release candidate + corpus version | regression report | BENCHMARK_ENV_INVALID |
| LearningQueue.enqueue | gap/failure/uncertainty + value score | learning item | DATA_SCOPE_DENIED |
