...
- 对规则执行跨项目、跨版本、正向/负向/边界/对抗样例验证。
- 将项目特定 workaround 故意提交为通用规则，验证晋升 Gate 拒绝。
- 对 Repair Ranker 测 top-k root-cause/repair success 与错误修复副作用。
- 每个 release 在固定 corpus 与 recent holdout 上比较准确度、完整度、成本和人工介入。
- 模拟知识冲突、证据撤销、框架版本淘汰和依赖安全事件。
- 验证租户删除后检索、训练数据和派生索引不再返回被删除资产。
