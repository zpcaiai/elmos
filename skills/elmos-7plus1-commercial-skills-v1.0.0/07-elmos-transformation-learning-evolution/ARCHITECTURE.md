# P07 架构设计：Elmos 转换知识沉淀、自学习与能力进化
...
flowchart LR
    ...
    P00 --> THIS[P07 Elmos Transformation Learning & Evolution]
...
```mermaid
flowchart TD
        C1["Transformation Knowledge Base"]
        C2["Project Archetype Knowledge Base"]
        C3["Failure & Repair Corpus"]
        C4["Rule Promotion & Governance"]
        C5["Benchmark Corpus"]
        C6["Evidence Corpus"]
        C7["Retrieval & Repair Ranker"]
        C8["Drift & Regression Detector"]
        C9["Active Learning Queue"]
        C10["Specialized Model Pipeline"]
        C11["Tenant/IP Isolation & Consent"]
        C12["Knowledge Quality Auditor"]
        C1 --> C2
        C2 --> C3
        C3 --> C4
        C4 --> C5
...
| 组件 | 职责 |
| --- | --- |
| Transformation Knowledge Base | 语言/框架/API/类型/事务/并发/异常/安全/数据/消息规则。 |
| Project Archetype Knowledge Base | 行业能力基线、架构模式、周边功能和验收模板。 |
| Failure & Repair Corpus | 失败、上下文、根因、错误转换、修复和验证轨迹。 |
| Rule Promotion & Governance | maturity、跨项目证据、版本兼容、owner、expiry、rollback。 |
| Benchmark Corpus | 项目生成、语言转换、框架现代化、前端/小程序和非功能基准。 |
| Evidence Corpus | compile/test/differential/production/rollback 经过签名的结果。 |
| Retrieval & Repair Ranker | 相似失败检索、适用性过滤、修复候选排序和 confidence。 |
| Drift & Regression Detector | 模型/规则/框架/依赖变化导致的质量漂移。 |
| Active Learning Queue | 选择最有信息价值的未知 gap、低置信规则和失败样本。 |
| Specialized Model Pipeline | 数据构造、训练、评测、部署和 shadow。 |
| Tenant/IP Isolation & Consent | 私有/组织/全局 scope、授权、脱敏、删除和审计。 |
| Knowledge Quality Auditor | 重复、冲突、毒化、过期、覆盖盲区和证据完整性。 |
