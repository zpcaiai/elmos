...
| 接口 | 输入 | 输出 | 稳定错误码 |
| --- | --- | --- | --- |
| ModelCatalog.refresh | provider adapters + policy | versioned catalog snapshot | CATALOG_STALE |
| TaskClassifier.classify | task contract + repo/IR metadata | TaskProfile | CLASSIFICATION_LOW_CONFIDENCE |
| RoutePlanner.plan | TaskProfile + constraints + catalog + history | ordered RoutePlan | NO_ELIGIBLE_ROUTE |
| RouteExecutor.execute | RoutePlan + model request | response/run + route trace | ALL_ROUTES_FAILED |
| CostEngine.forecast | task DAG + route candidates + historical usage | cost/ETA distribution | INSUFFICIENT_HISTORY |
| FeedbackStore.record | run outcome + verified quality metrics | updated task-fit stats | OUTCOME_UNVERIFIED |
