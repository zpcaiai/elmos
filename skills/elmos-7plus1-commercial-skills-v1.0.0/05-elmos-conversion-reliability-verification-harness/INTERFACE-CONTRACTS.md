...
| 接口 | 输入 | 输出 | 稳定错误码 |
| --- | --- | --- | --- |
| CoverageLedger.update | entity id + stage + evidence/status | new immutable state | INVALID_TRANSITION |
| VerificationPlanner.plan | change graph + risk profile + policies | test/verification DAG | COVERAGE_UNSATISFIABLE |
| DifferentialRuntime.run | source/target harness + scenario set | normalized diff report | ENVIRONMENT_MISMATCH |
| CompletionGate.evaluate | ledgers + evidence + policy revision | pass/fail/blocked + reasons | EVIDENCE_STALE |
| RepairLoop.submit | failure cluster + scope + budget | repair task or human blocker | NO_PROGRESS / BUDGET_EXHAUSTED |
| CertificationEngine.certify | release candidate + E-level | certificate + residual risk | CERTIFICATION_FAILED |
