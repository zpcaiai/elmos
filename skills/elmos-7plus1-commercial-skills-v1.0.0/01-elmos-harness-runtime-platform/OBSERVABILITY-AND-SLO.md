...
| 指标 | 目标 | 说明 |
| --- | --- | --- |
| Runtime API availability | 99.95% | 企业版目标 |
| Session event durability | RPO ≈ 0 after commit | commit 返回后不得丢失 |
| Crash recovery | P95 < 30s | 恢复可执行状态或明确阻断 |
| Tool dispatch overhead | P95 < 50ms | 不含工具本身 |
| Cancellation propagation | P95 < 2s | 对支持取消的 Provider/Tool |
| Permission decision | P95 < 20ms | 本地缓存策略 |
