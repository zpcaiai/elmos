---
id: PG163
name: container-hardening-generator
version: 1.0.0
batch: 60
engine: elmos.project-synthesis
schema: elmos.project-synthesis.v1
status: proposed
summary: Generates hardened container builds, users, filesystems, capabilities, scanning, signing, and attestations.
depends_on: ["PG088", "PG100", "PG112", "PG162"]
capabilities:
  - workspace:read
...
# PG163 — Container Hardening Generator

## 1. Objective

Produce minimal, non-root, provenance-verifiable runtime images.
...
- Validate and consume container_assets.
- Validate and consume runtime_profile.
- Validate and consume security_policy.
- Validate and consume sbom.
- Generate and validate hardened_container_assets.
- Generate and validate image_scan_results.
- Generate and validate image_signature.
- Generate and validate container_attestation.
...
1. Use minimal approved base images and multi-stage builds.
2. Generate non-root user, readonly filesystem, dropped capabilities, and minimal writable mounts.
3. Remove build tools and caches from runtime layers.
4. Generate vulnerability scan, SBOM attachment, signature, and provenance attestation.
5. Validate startup under hardened settings.
...
- `ROOT_RUNTIME` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
- `MUTABLE_BASE_TAG` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
- `EXCESS_CAPABILITY` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
- `RUNTIME_BREAKS_READONLY_FS` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
...
- Valid approved inputs produce all declared outputs for `container-hardening-generator`.
...
- Consume compatible certified outputs from PG088, PG100, PG112, PG162.
...
- Reject or escalate `root_runtime` without inventing a successful result.
- Reject or escalate `mutable_base_tag` without inventing a successful result.
- Reject or escalate `excess_capability` without inventing a successful result.
- Reject or escalate `runtime_breaks_readonly_fs` without inventing a successful result.
...
- Runtime images operate as non-root.
- Base and application images are pinned and signed according to policy.
- Hardening is validated by startup and smoke tests.
