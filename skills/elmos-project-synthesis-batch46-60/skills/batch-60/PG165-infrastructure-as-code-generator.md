...
status: proposed
summary: Generates approved infrastructure modules for network, compute, databases, brokers, storage, identity integration, and observability.
depends_on: ["PG050", "PG053", "PG117", "PG119"]
capabilities:
  - workspace:read
...
# PG165 — Infrastructure As Code Generator

## 1. Objective

Provision the runtime dependencies declared by the architecture using reviewable and drift-detectable code.
...
- Validate and consume cloud_or_platform_profile.
- Validate and consume security_control_plan.
- Validate and consume data_residency_policy.
- Generate and validate iac_modules.
- Generate and validate state_backend_plan.
- Generate and validate plan_validation.
- Generate and validate infrastructure_test_plan.
...
1. Map approved infrastructure components to supported modules.
2. Generate environment composition and remote state configuration.
3. Generate encryption, network, identity, backup, retention, and tagging controls.
4. Run format, validate, policy, and plan checks.
5. Generate drift, destroy-protection, and recovery tests.
...
- `PUBLIC_EXPOSURE_UNAPPROVED` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
- `UNENCRYPTED_STATE` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
- `DESTRUCTIVE_PLAN` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
- `REGION_RESIDENCY_VIOLATION` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
...
- Valid approved inputs produce all declared outputs for `infrastructure-as-code-generator`.
...
- Consume compatible certified outputs from PG050, PG053, PG117, PG119.
...
- Reject or escalate `public_exposure_unapproved` without inventing a successful result.
- Reject or escalate `unencrypted_state` without inventing a successful result.
- Reject or escalate `destructive_plan` without inventing a successful result.
- Reject or escalate `region_residency_violation` without inventing a successful result.
...
- Infrastructure plans contain no unapproved destructive action.
- State and sensitive outputs are protected.
- Resources satisfy approved region and network boundaries.
