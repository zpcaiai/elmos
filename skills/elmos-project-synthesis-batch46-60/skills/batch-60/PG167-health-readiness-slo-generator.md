---
id: PG167
name: health-readiness-slo-generator
version: 1.0.0
batch: 60
engine: elmos.project-synthesis
schema: elmos.project-synthesis.v1
status: proposed
summary: Generates health/readiness checks, SLIs, SLOs, error budgets, alerts, dashboards, and ownership.
depends_on: ["PG027", "PG052", "PG053", "PG166"]
capabilities:
  - workspace:read
...
# PG167 — Health Readiness Slo Generator

## 1. Objective

Turn runtime quality requirements into actionable operational controls.
...
- Validate and consume observability_plan.
- Validate and consume deployment_topology.
- Validate and consume dependency_map.
- Generate and validate health_checks.
- Generate and validate sli_slo_definitions.
- Generate and validate alert_rules.
- Generate and validate operations_dashboards.
...
1. Classify liveness, readiness, dependency, and business health.
2. Generate checks that avoid cascading overload.
3. Generate SLIs and SLOs from approved measures.
4. Generate multi-window burn alerts and ownership.
5. Generate dashboards and runbook links.
...
- `LIVENESS_CHECKS_DEPENDENCY` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
- `READINESS_FALSE_POSITIVE` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
- `ALERT_WITHOUT_ACTION` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
- `SLO_MEASUREMENT_MISMATCH` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
...
- Valid approved inputs produce all declared outputs for `health-readiness-slo-generator`.
...
- Consume compatible certified outputs from PG027, PG052, PG053, PG166.
...
- Reject or escalate `liveness_checks_dependency` without inventing a successful result.
- Reject or escalate `readiness_false_positive` without inventing a successful result.
- Reject or escalate `alert_without_action` without inventing a successful result.
- Reject or escalate `slo_measurement_mismatch` without inventing a successful result.
...
- Liveness does not fail solely because a dependency is unavailable.
- Readiness reflects ability to serve required work.
