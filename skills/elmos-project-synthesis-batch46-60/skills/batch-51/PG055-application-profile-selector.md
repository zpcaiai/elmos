---
id: PG055
name: application-profile-selector
version: 1.0.0
batch: 51
engine: elmos.project-synthesis
schema: elmos.project-synthesis.v1
status: proposed
summary: Selects project shapes such as API, modular monolith, worker, CLI, batch, event processor, desktop, or full stack.
depends_on: ["PG054"]
capabilities:
  - workspace:read
...
# PG055 — Application Profile Selector

## 1. Objective

Turn the Architecture Baseline into concrete deployable application profiles.
...
- Validate and process delivery_scope.
- Validate and process deployment_topology.
- Validate and process supported_profiles.
- Produce and validate application_profiles.
- Produce and validate profile_rationale.
- Produce and validate profile_constraints.
- Produce and validate unsupported_profile_report.
...
1. Identify required deployable application types from approved containers.
2. Match each deployable to a supported application profile.
3. Prefer fewer deployables and simpler operational shapes when requirements permit.
4. Record profile-specific capabilities, constraints, and generated responsibilities.
5. Reject profiles unsupported by the target environment or ELMOS certification matrix.
...
- PROFILE_MISMATCH — emit structured diagnostics and stop unsafe continuation.
- UNSUPPORTED_PROFILE — emit structured diagnostics and stop unsafe continuation.
- UNNECESSARY_DEPLOYABLE — emit structured diagnostics and stop unsafe continuation.
- MISSING_RUNTIME_CAPABILITY — emit structured diagnostics and stop unsafe continuation.
...
- Valid inputs produce every declared output for `application-profile-selector`.
...
- Reject or escalate `profile_mismatch` without inventing success.
- Reject or escalate `unsupported_profile` without inventing success.
- Reject or escalate `unnecessary_deployable` without inventing success.
- Reject or escalate `missing_runtime_capability` without inventing success.
...
- Each deployable maps to exactly one primary application profile.
- Selected profiles satisfy Architecture Baseline responsibilities.
- Unsupported profiles fail before language-specific generation.
