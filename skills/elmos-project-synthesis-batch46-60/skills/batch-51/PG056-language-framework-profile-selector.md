---
id: PG056
name: language-framework-profile-selector
version: 1.0.0
batch: 51
engine: elmos.project-synthesis
schema: elmos.project-synthesis.v1
status: proposed
summary: Selects language, framework, persistence, messaging, API, testing, and observability profiles.
depends_on: ["PG018", "PG055"]
capabilities:
  - workspace:read
...
# PG056 — Language Framework Profile Selector

## 1. Objective

Choose implementation technology from approved constraints and supported Golden Paths.
...
- Validate and process technical_constraints.
- Validate and process organization_standards.
- Validate and process supported_language_profiles.
- Produce and validate language_framework_profiles.
- Produce and validate selection_rationale.
- Produce and validate compatibility_matrix.
- Produce and validate profile_exceptions.
...
1. Resolve mandatory language and framework constraints.
2. Evaluate supported Golden Paths against application and architecture needs.
3. Choose persistence, messaging, API, serialization, testing, and telemetry adapters.
4. Check license, support, maturity, ecosystem, security, and organizational fit.
5. Require explicit approval and ownership for nonstandard profile exceptions.
...
- UNSUPPORTED_FRAMEWORK — emit structured diagnostics and stop unsafe continuation.
- LICENSE_CONFLICT — emit structured diagnostics and stop unsafe continuation.
- PROFILE_INCOMPATIBILITY — emit structured diagnostics and stop unsafe continuation.
- NONSTANDARD_EXCEPTION_MISSING — emit structured diagnostics and stop unsafe continuation.
...
- Valid inputs produce every declared output for `language-framework-profile-selector`.
...
- Reject or escalate `unsupported_framework` without inventing success.
- Reject or escalate `license_conflict` without inventing success.
- Reject or escalate `profile_incompatibility` without inventing success.
- Reject or escalate `nonstandard_exception_missing` without inventing success.
...
- Every application profile has a compatible language/framework profile.
- Selection favors supported Golden Paths.
- Nonstandard choices include maintenance ownership and exit strategy.
