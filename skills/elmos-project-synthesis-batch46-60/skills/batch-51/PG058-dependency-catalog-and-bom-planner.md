---
id: PG058
name: dependency-catalog-and-bom-planner
version: 1.0.0
batch: 51
engine: elmos.project-synthesis
schema: elmos.project-synthesis.v1
status: proposed
summary: Creates approved dependency catalogs, BOMs, lock strategies, repositories, licenses, and vulnerability policies.
depends_on: ["PG050", "PG056", "PG057"]
capabilities:
  - workspace:read
...
# PG058 — Dependency Catalog And Bom Planner

## 1. Objective

Make third-party dependency selection controlled, minimal, and reproducible.
...
- Validate and process license_policy.
- Produce and validate dependency_catalog.
- Produce and validate bom_plan.
- Produce and validate lockfile_strategy.
- Produce and validate dependency_policy_report.
...
1. Enumerate required functional and infrastructure dependencies.
2. Prefer platform-provided, standard-library, or approved organization dependencies.
3. Resolve direct and managed transitive versions through BOM or lock mechanisms.
4. Check licenses, provenance, repository trust, known vulnerabilities, and maintenance status.
5. Define update, exception, removal, and dependency-minimization policy.
...
- DEPENDENCY_CONFLICT — emit structured diagnostics and stop unsafe continuation.
- UNAPPROVED_REPOSITORY — emit structured diagnostics and stop unsafe continuation.
- LICENSE_VIOLATION — emit structured diagnostics and stop unsafe continuation.
- CRITICAL_VULNERABILITY — emit structured diagnostics and stop unsafe continuation.
...
- Valid inputs produce every declared output for `dependency-catalog-and-bom-planner`.
...
- Reject or escalate `dependency_conflict` without inventing success.
- Reject or escalate `unapproved_repository` without inventing success.
- Reject or escalate `license_violation` without inventing success.
- Reject or escalate `critical_vulnerability` without inventing success.
...
- All direct dependencies have a declared purpose and owner.
- Versions are centrally managed where the ecosystem supports it.
- Critical vulnerabilities block Blueprint compilation unless formally excepted.
