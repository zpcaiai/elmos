...
status: proposed
summary: Detects nondeterministic tests through reruns, order changes, seed control, timing analysis, and shared-state inspection.
depends_on: ["PG150", "PG155"]
capabilities:
  - workspace:read
...
# PG156 — Flaky Test Detector

## 1. Objective

Separate genuine product failures from unstable verification without hiding either.
...
- Validate and consume test_failure_groups.
- Validate and consume test_sources.
- Validate and consume runner_profile.
- Validate and consume flake_policy.
- Generate and validate flake_report.
- Generate and validate reproduction_recipe.
- Generate and validate likely_flake_causes.
- Generate and validate quarantine_decision.
...
1. Rerun suspected tests under controlled seeds and order.
2. Vary scheduling, parallelism, and timing within policy.
3. Inspect shared state, fixed sleeps, external instability, and random data.
4. Calculate evidence-based flake likelihood.
5. Quarantine only under explicit policy with owner and expiry.
...
- `PRODUCT_BUG_MASKED_AS_FLAKE` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
- `INFINITE_RERUN` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
- `QUARANTINE_WITHOUT_OWNER` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
- `NONDETERMINISM_NOT_RECORDED` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
...
- Valid approved inputs produce all declared outputs for `flaky-test-detector`.
...
- Consume compatible certified outputs from PG150, PG155.
...
- Reject or escalate `product_bug_masked_as_flake` without inventing a successful result.
- Reject or escalate `infinite_rerun` without inventing a successful result.
- Reject or escalate `quarantine_without_owner` without inventing a successful result.
- Reject or escalate `nondeterminism_not_recorded` without inventing a successful result.
...
- A passing rerun alone does not prove flakiness.
- Quarantined tests remain visible and blocking according to policy.
- Reproduction data includes seed, order, and environment.
