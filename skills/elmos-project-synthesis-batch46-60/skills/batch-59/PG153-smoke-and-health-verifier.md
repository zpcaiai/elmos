---
id: PG153
name: smoke-and-health-verifier
version: 1.0.0
batch: 59
engine: elmos.project-synthesis
schema: elmos.project-synthesis.v1
status: proposed
summary: Executes health, readiness, authenticated API, worker, event, database, and full-stack smoke checks.
depends_on: ["PG134", "PG140", "PG152"]
capabilities:
  - workspace:read
...
# PG153 — Smoke And Health Verifier

## 1. Objective

Prove the minimum integrated system behavior before deeper certification.
...
- Validate and consume startup_execution.
- Validate and consume smoke_specs.
- Validate and consume health_contracts.
- Validate and consume test_credentials_reference.
- Generate and validate smoke_results.
- Generate and validate health_results.
- Generate and validate journey_trace_refs.
- Generate and validate smoke_evidence.
...
1. Verify liveness and readiness endpoints.
2. Execute one approved authenticated read and write journey.
3. Verify database persistence and async outcome where applicable.
4. Verify unauthorized access fails.
5. Capture trace identifiers and cleanup.
...
- `HEALTH_FALSE_POSITIVE` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
- `UNAUTHORIZED_SMOKE_SUCCESS` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
- `ASYNC_OUTCOME_UNVERIFIED` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
...
- Valid approved inputs produce all declared outputs for `smoke-and-health-verifier`.
...
- Consume compatible certified outputs from PG134, PG140, PG152.
...
- Reject or escalate `health_false_positive` without inventing a successful result.
- Reject or escalate `unauthorized_smoke_success` without inventing a successful result.
- Reject or escalate `async_outcome_unverified` without inventing a successful result.
...
- Readiness represents usable dependency state, not only process existence.
- At least one critical journey succeeds through public boundaries.
