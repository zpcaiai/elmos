...
status: proposed
summary: Generates forms, field schemas, client validation, server-error mapping, accessibility, and optimistic concurrency handling.
depends_on: ["PG034", "PG039", "PG125", "PG126"]
capabilities:
  - workspace:read
...
# PG128 — Form Validation Generator

## 1. Objective

Turn approved commands and data definitions into accessible, contract-aligned forms.
...
- Validate and consume command_contracts.
- Validate and consume data_dictionary.
- Validate and consume frontend_profile.
- Validate and consume validation_rules.
- Generate and validate form_components.
- Generate and validate form_schemas.
- Generate and validate validation_messages.
- Generate and validate form_tests.
...
1. Map command inputs to form controls and semantic field groups.
2. Generate client validation only from approved constraints.
3. Map server validation and concurrency errors to stable UI states.
4. Generate accessible labels, descriptions, focus management, and error summaries.
5. Generate boundary, invalid, and resubmission tests.
...
- `CLIENT_SERVER_VALIDATION_DRIFT` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
- `INACCESSIBLE_ERROR_STATE` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
- `DOUBLE_SUBMIT` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
- `LOST_CONCURRENCY_ERROR` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
...
- Valid approved inputs produce all declared outputs for `form-validation-generator`.
...
- Consume compatible certified outputs from PG034, PG039, PG125, PG126.
...
- Reject or escalate `client_server_validation_drift` without inventing a successful result.
- Reject or escalate `inaccessible_error_state` without inventing a successful result.
- Reject or escalate `double_submit` without inventing a successful result.
- Reject or escalate `lost_concurrency_error` without inventing a successful result.
...
- Every invalid field has an accessible message and focus path.
- Duplicate submission is prevented or idempotently handled.
