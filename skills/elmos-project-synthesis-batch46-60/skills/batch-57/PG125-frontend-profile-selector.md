---
id: PG125
name: frontend-profile-selector
version: 1.0.0
batch: 57
engine: elmos.project-synthesis
schema: elmos.project-synthesis.v1
status: proposed
summary: Selects the supported frontend application and rendering profile.
depends_on: ["PG033", "PG053", "PG055", "PG064"]
capabilities:
  - workspace:read
...
# PG125 — Frontend Profile Selector

## 1. Objective

Choose the simplest supported frontend profile that satisfies approved user journeys, deployment, accessibility, security, and integration constraints.
...
- Validate and consume ui_requirements.
- Validate and consume user_journeys.
- Validate and consume deployment_topology.
- Validate and consume supported_frontend_profiles.
- Generate and validate frontend_profile.
- Generate and validate rendering_mode.
- Generate and validate frontend_capability_map.
- Generate and validate profile_gap_report.
...
1. Classify the frontend as SPA, server-rendered, static, desktop-like web, or embedded administration UI.
2. Evaluate Vue, React, Angular, Blazor, or approved organization profiles without silently changing the Blueprint.
3. Select rendering, routing, state, forms, testing, accessibility, and deployment capabilities.
4. Record unsupported requirements and extension points.
5. Publish a profile decision with alternatives and tradeoffs.
...
- `UNSUPPORTED_FRONTEND_PROFILE` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
- `RENDERING_SECURITY_CONFLICT` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
- `PROFILE_CAPABILITY_GAP` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
- `UNAPPROVED_FRAMEWORK_CHOICE` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
...
- Valid approved inputs produce all declared outputs for `frontend-profile-selector`.
...
- Consume compatible certified outputs from PG033, PG053, PG055, PG064.
...
- Reject or escalate `unsupported_frontend_profile` without inventing a successful result.
- Reject or escalate `rendering_security_conflict` without inventing a successful result.
- Reject or escalate `profile_capability_gap` without inventing a successful result.
- Reject or escalate `unapproved_framework_choice` without inventing a successful result.
...
- Every required UI capability maps to a supported adapter or explicit gap.
- The selected rendering model matches security and deployment constraints.
- The profile is immutable within an approved Blueprint revision.
