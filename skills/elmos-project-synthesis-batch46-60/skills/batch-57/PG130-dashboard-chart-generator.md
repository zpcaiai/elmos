...
status: proposed
summary: Generates KPI cards, charts, filters, drilldowns, freshness indicators, and accessible tabular alternatives.
depends_on: ["PG027", "PG040", "PG125", "PG126"]
capabilities:
  - workspace:read
...
# PG130 — Dashboard Chart Generator

## 1. Objective

Render approved operational and analytical views without inventing metrics or misleading visual encodings.
...
- Validate and consume metric_definitions.
- Validate and consume query_contracts.
- Validate and consume frontend_profile.
- Validate and consume data_freshness_policy.
- Generate and validate dashboard_pages.
- Generate and validate chart_specs.
- Generate and validate metric_state_models.
- Generate and validate dashboard_tests.
...
1. Validate metric definitions, grain, filters, units, and freshness.
2. Select chart forms appropriate to comparison, trend, distribution, or composition.
3. Generate loading, stale, partial, empty, and error states.
4. Generate accessible summaries and tables.
5. Generate metric contract and visual state tests.
...
- `METRIC_DEFINITION_MISSING` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
- `MISLEADING_CHART` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
- `STALE_DATA_HIDDEN` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
- `UNIT_MISMATCH` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
...
- Valid approved inputs produce all declared outputs for `dashboard-chart-generator`.
...
- Consume compatible certified outputs from PG027, PG040, PG125, PG126.
...
- Reject or escalate `metric_definition_missing` without inventing a successful result.
- Reject or escalate `misleading_chart` without inventing a successful result.
- Reject or escalate `stale_data_hidden` without inventing a successful result.
- Reject or escalate `unit_mismatch` without inventing a successful result.
...
- Every displayed metric has an approved definition and unit.
- Freshness and partial-data state are visible.
- Charts include accessible nonvisual alternatives.
