---
id: PG133
name: frontend-state-data-fetch-generator
version: 1.0.0
batch: 57
engine: elmos.project-synthesis
schema: elmos.project-synthesis.v1
status: proposed
summary: Generates query caching, mutations, optimistic updates, invalidation, offline state, and error recovery.
depends_on: ["PG048", "PG126", "PG129"]
capabilities:
  - workspace:read
...
# PG133 — Frontend State Data Fetch Generator

## 1. Objective

Implement predictable client state and server-state synchronization from approved contracts.
...
- Validate and consume typescript_sdk.
- Validate and consume interaction_contracts.
- Validate and consume consistency_policy.
- Generate and validate data_fetch_layer.
- Generate and validate client_state_model.
- Generate and validate cache_invalidation_rules.
- Generate and validate state_tests.
...
1. Classify local, URL, session, and server state.
2. Generate queries, mutations, cancellation, deduplication, and cache keys.
3. Generate invalidation or optimistic update only when consistency semantics permit.
4. Generate retry and offline behavior.
5. Generate race, stale, and rollback tests.
...
- `STALE_AUTHORIZATION_CACHE` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
- `OPTIMISTIC_UPDATE_FALSE_SUCCESS` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
- `CACHE_KEY_COLLISION` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
- `RETRYING_UNSAFE_MUTATION` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
...
- Valid approved inputs produce all declared outputs for `frontend-state-data-fetch-generator`.
...
- Consume compatible certified outputs from PG048, PG126, PG129.
...
- Reject or escalate `stale_authorization_cache` without inventing a successful result.
- Reject or escalate `optimistic_update_false_success` without inventing a successful result.
- Reject or escalate `cache_key_collision` without inventing a successful result.
- Reject or escalate `retrying_unsafe_mutation` without inventing a successful result.
...
- Server state has explicit freshness and invalidation policy.
