---
id: PG131
name: navigation-routing-permission-generator
version: 1.0.0
batch: 57
engine: elmos.project-synthesis
schema: elmos.project-synthesis.v1
status: proposed
summary: Generates application shell, route tree, breadcrumbs, navigation, deep links, and permission-aware visibility.
depends_on: ["PG033", "PG041", "PG045", "PG125"]
capabilities:
  - workspace:read
...
# PG131 — Navigation Routing Permission Generator

## 1. Objective

Create coherent frontend information architecture from approved journeys and modules.
...
- Validate and consume module_catalog.
- Validate and consume permission_matrix.
- Generate and validate route_tree.
- Generate and validate navigation_components.
- Generate and validate breadcrumb_model.
- Generate and validate routing_tests.
...
1. Group journeys into route and navigation candidates.
2. Generate stable path, parameter, and deep-link contracts.
3. Generate permission-aware visibility and denied states.
4. Generate not-found, unauthorized, loading, and redirect behavior.
5. Generate route and navigation tests.
...
- `ROUTE_COLLISION` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
- `PERMISSION_VISIBILITY_LEAK` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
- `REDIRECT_LOOP` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
- `UNSTABLE_DEEP_LINK` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
...
- Valid approved inputs produce all declared outputs for `navigation-routing-permission-generator`.
...
- Consume compatible certified outputs from PG033, PG041, PG045, PG125.
...
- Reject or escalate `route_collision` without inventing a successful result.
- Reject or escalate `permission_visibility_leak` without inventing a successful result.
- Reject or escalate `redirect_loop` without inventing a successful result.
- Reject or escalate `unstable_deep_link` without inventing a successful result.
...
- Routes have stable identifiers and no collision.
- Hidden navigation never substitutes for server authorization.
- Deep links restore the correct authorized state.
