---
id: PG036
name: bounded-context-candidate-builder
version: 1.0.0
batch: 49
engine: elmos.project-synthesis
schema: elmos.project-synthesis.v1
status: proposed
summary: Proposes bounded contexts and context relationships from language, rules, ownership, and change patterns.
depends_on: ["PG014", "PG015", "PG034", "PG035"]
capabilities:
  - workspace:read
...
# PG036 — Bounded Context Candidate Builder

## 1. Objective

Create candidate domain boundaries before architecture decomposition.
...
- Produce and validate bounded_context_candidates.
- Produce and validate context_map.
- Produce and validate boundary_rationale.
- Produce and validate context_risks.
...
1. Cluster concepts by language, invariants, ownership, and rate of change.
2. Identify semantic collisions and translation boundaries.
3. Propose upstream/downstream, anti-corruption-layer, conformist, partnership, or shared-kernel relationships.
4. Score cohesion, coupling, organizational fit, and change independence.
5. Publish alternatives instead of forcing premature final boundaries.
...
- CONTEXT_OVERLAP — emit structured diagnostics and stop unsafe continuation.
- SHARED_MODEL_OVERREACH — emit structured diagnostics and stop unsafe continuation.
- ORGANIZATIONAL_MISMATCH — emit structured diagnostics and stop unsafe continuation.
- SEMANTIC_COLLISION — emit structured diagnostics and stop unsafe continuation.
...
- Valid inputs produce every declared output for `bounded-context-candidate-builder`.
...
- Reject or escalate `context_overlap` without inventing success.
- Reject or escalate `shared_model_overreach` without inventing success.
- Reject or escalate `organizational_mismatch` without inventing success.
- Reject or escalate `semantic_collision` without inventing success.
...
- Each candidate boundary has evidence, risks, and tradeoffs.
- A shared kernel requires explicit governance.
- Context candidates remain architecture-neutral until approved.
