---
id: PG035
name: domain-entity-value-object-modeler
version: 1.0.0
batch: 49
engine: elmos.project-synthesis
schema: elmos.project-synthesis.v1
status: proposed
summary: Models aggregates, entities, value objects, identities, invariants, and lifecycle boundaries.
depends_on: ["PG014", "PG033", "PG034"]
capabilities:
  - workspace:read
...
# PG035 — Domain Entity Value Object Modeler

## 1. Objective

Create a language-neutral domain model grounded in business behavior rather than database tables.
...
- Validate and process data_constraints.
- Produce and validate domain_model.
- Produce and validate aggregate_catalog.
- Produce and validate entity_catalog.
- Produce and validate value_object_catalog.
...
1. Identify concepts with identity, lifecycle, and business behavior.
2. Separate mutable entities from immutable value objects.
3. Group consistency rules into aggregate candidates.
4. Assign invariants, creation rules, lifecycle ownership, and identity rules.
5. Validate that the model supports required behavior without becoming an anemic data model.
...
- ENTITY_VALUE_OBJECT_CONFUSION — emit structured diagnostics and stop unsafe continuation.
- OVERSIZED_AGGREGATE — emit structured diagnostics and stop unsafe continuation.
- ANEMIC_DOMAIN_MODEL — emit structured diagnostics and stop unsafe continuation.
- IDENTITY_COLLISION — emit structured diagnostics and stop unsafe continuation.
...
- Valid inputs produce every declared output for `domain-entity-value-object-modeler`.
...
- Reject or escalate `entity_value_object_confusion` without inventing success.
- Reject or escalate `oversized_aggregate` without inventing success.
- Reject or escalate `anemic_domain_model` without inventing success.
- Reject or escalate `identity_collision` without inventing success.
...
- Every aggregate has a consistency boundary and invariant set.
- Value objects are immutable in the semantic model.
- Every modeled concept traces to approved behavior or data semantics.
