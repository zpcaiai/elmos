...
status: proposed
summary: Generates load, stress, endurance, concurrency, backpressure, and capacity tests from measurable quality scenarios.
depends_on: ["PG027", "PG046", "PG053", "PG135"]
capabilities:
  - workspace:read
...
# PG143 — Performance Test Generator

## 1. Objective

Verify latency, throughput, resource, and scaling behavior under defined workloads.
...
- Validate and consume quality_attribute_scenarios.
- Validate and consume deployment_topology.
- Validate and consume critical_operations.
- Validate and consume data_volume_model.
- Generate and validate performance_test_specs.
- Generate and validate workload_models.
- Generate and validate performance_data_generators.
- Generate and validate performance_evidence_plan.
...
- `performance_test_specs`
- `workload_models`
- `performance_data_generators`
- `performance_evidence_plan`
...
1. Translate quality scenarios into workload phases and pass thresholds.
2. Generate realistic data volume and access distributions.
3. Generate load, stress, spike, endurance, and concurrency tests as applicable.
4. Capture latency percentiles, errors, saturation, queues, and resource metrics.
5. Define environment comparability and result validity rules.
...
- `UNREPRESENTATIVE_WORKLOAD` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
- `COORDINATED_OMISSION` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
- `WARMUP_MISSING` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
- `ENVIRONMENT_NOT_COMPARABLE` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
...
- Valid approved inputs produce all declared outputs for `performance-test-generator`.
...
- Consume compatible certified outputs from PG027, PG046, PG053, PG135.
...
- Reject or escalate `unrepresentative_workload` without inventing a successful result.
- Reject or escalate `coordinated_omission` without inventing a successful result.
- Reject or escalate `warmup_missing` without inventing a successful result.
- Reject or escalate `environment_not_comparable` without inventing a successful result.
...
- Every performance claim names workload and environment.
- Latency reporting includes percentiles and error rates.
- Results are invalidated when the environment is not comparable.
