...
status: proposed
summary: Generates focused unit tests for domain rules, value objects, services, transformations, and pure logic.
depends_on: ["PG034", "PG035", "PG067", "PG135"]
capabilities:
  - workspace:read
...
# PG136 — Unit Test Generator

## 1. Objective

Verify behavior at the smallest stable semantic boundary without over-mocking implementation details.
...
- Validate and consume domain_specification.
- Validate and consume semantic_code_model.
- Validate and consume language_profile.
- Generate and validate unit_test_sources.
- Generate and validate unit_fixture_plan.
- Generate and validate unit_coverage_map.
- Generate and validate unit_test_evidence.
...
1. Select stable semantic units from domain and application models.
2. Generate positive, negative, boundary, and invariant cases.
3. Use deterministic data and clocks.
4. Mock only declared ports, not internal implementation details.
5. Link each test to verification obligations.
...
- `BRITTLE_INTERNAL_MOCK` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
- `MISSING_BOUNDARY_CASE` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
- `NONDETERMINISTIC_TIME` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
- `ASSERTION_WITHOUT_BUSINESS_MEANING` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
...
- Valid approved inputs produce all declared outputs for `unit-test-generator`.
...
- Consume compatible certified outputs from PG034, PG035, PG067, PG135.
...
- Reject or escalate `brittle_internal_mock` without inventing a successful result.
- Reject or escalate `missing_boundary_case` without inventing a successful result.
- Reject or escalate `nondeterministic_time` without inventing a successful result.
- Reject or escalate `assertion_without_business_meaning` without inventing a successful result.
...
- Tests assert approved observable behavior.
- Fixtures are deterministic and minimal.
- Domain invariants receive positive and negative coverage.
