...
status: proposed
summary: Generates migration, upgrade, rollback/forward-recovery, restart, lock, and data-preservation tests.
depends_on: ["PG113", "PG114", "PG135"]
capabilities:
  - workspace:read
...
# PG139 — Database Migration Test Generator

## 1. Objective

Prove database evolution works from empty and supported prior states.
...
- Validate and consume supported_previous_versions.
- Validate and consume database_profile.
- Generate and validate migration_test_sources.
- Generate and validate migration_test_matrix.
- Generate and validate data_preservation_assertions.
- Generate and validate migration_evidence_plan.
...
- `migration_test_matrix`
- `data_preservation_assertions`
- `migration_evidence_plan`
...
1. Generate empty-database migration tests.
2. Generate upgrade tests from every supported baseline.
3. Generate schema, constraint, index, and seed assertions.
4. Generate restart and concurrent-access scenarios for online changes.
5. Generate rollback or forward-recovery validation according to the plan.
...
- `MIGRATION_ONLY_TESTED_FROM_EMPTY` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
- `DATA_LOSS_ASSERTION_MISSING` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
- `UNSAFE_ROLLBACK_TEST` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
- `DATABASE_LOCK_IGNORED` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
...
- Valid approved inputs produce all declared outputs for `database-migration-test-generator`.
...
- Consume compatible certified outputs from PG113, PG114, PG135.
...
- Reject or escalate `migration_only_tested_from_empty` without inventing a successful result.
- Reject or escalate `data_loss_assertion_missing` without inventing a successful result.
- Reject or escalate `unsafe_rollback_test` without inventing a successful result.
- Reject or escalate `database_lock_ignored` without inventing a successful result.
...
- Empty and supported-upgrade paths pass.
- Destructive changes prove approved preservation behavior.
- Migration tests use the target-compatible database engine.
