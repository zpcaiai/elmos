...
status: proposed
summary: Generates component integration tests using compatible databases, brokers, caches, storage, identity, and external-service substitutes.
depends_on: ["PG113", "PG117", "PG118", "PG119", "PG123", "PG135"]
capabilities:
  - workspace:read
...
# PG137 — Integration Test Generator

## 1. Objective

Verify generated adapters and infrastructure semantics across real process boundaries where required.
...
- Validate and consume language_pack_artifacts.
- Generate and validate integration_test_sources.
- Generate and validate test_service_topology.
- Generate and validate integration_fixtures.
- Generate and validate integration_evidence_plan.
...
1. Select integration boundaries from contracts and risk.
2. Provision compatible ephemeral services or approved test environments.
3. Generate setup, migration, fixture, execution, and cleanup logic.
4. Verify errors, retries, transactions, tenant isolation, and recovery.
5. Link results to obligations and generated adapters.
...
- `CRITICAL_INTEGRATION_MOCKED_ONLY` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
- `TEST_SERVICE_VERSION_MISMATCH` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
- `SHARED_TEST_STATE` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
- `CLEANUP_FAILURE` — stop unsafe continuation and emit a structured diagnostic with supporting evidence.
...
- Valid approved inputs produce all declared outputs for `integration-test-generator`.
...
- Consume compatible certified outputs from PG113, PG117, PG118, PG119, PG123, PG135.
...
- Reject or escalate `critical_integration_mocked_only` without inventing a successful result.
- Reject or escalate `test_service_version_mismatch` without inventing a successful result.
- Reject or escalate `shared_test_state` without inventing a successful result.
- Reject or escalate `cleanup_failure` without inventing a successful result.
...
- Target-database behavior is tested against a compatible engine.
- Broker delivery and duplication semantics are exercised.
