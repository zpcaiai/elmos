---
id: PG004
name: artifact-graph-schema
version: 1.0.0
batch: 46
engine: elmos.project-synthesis
schema: elmos.project-synthesis.v1
status: proposed
summary: Defines the graph linking requirements, PSIR nodes, architecture decisions, generated files, tests, evidence, and deliveries.
depends_on: ["PG003"]
capabilities:
  - workspace:read
...
# PG004 — Artifact Graph Schema

## 1. Objective

Make every generated artifact traceable and every change impact computable.
...
- Validate and process `psir_schema`.
- Validate and process `artifact_types`.
- Validate and process `evidence_types`.
- Validate and process `repository_model`.
- Produce and validate `artifact_graph_schema`.
- Produce and validate `edge_semantics`.
- Produce and validate `lineage_rules`.
- Produce and validate `impact_query_contract`.
...
1. Define node types for requirements, decisions, modules, files, symbols, tests, builds, vulnerabilities, and releases.
2. Define typed edges such as implements, verifies, generated-from, supersedes, and invalidates.
3. Specify provenance and immutable lineage fields.
4. Define graph integrity constraints.
5. Publish impact-analysis query examples.
...
- `ORPHAN_ARTIFACT` — emit a structured diagnostic with supporting evidence.
- `AMBIGUOUS_LINEAGE` — emit a structured diagnostic with supporting evidence.
- `INVALID_EDGE_TYPE` — emit a structured diagnostic with supporting evidence.
- `EVIDENCE_CYCLE` — emit a structured diagnostic with supporting evidence.
...
- Valid input produces all declared outputs for `artifact-graph-schema`.
...
- Consume valid outputs from PG003.
...
- Reject or escalate `orphan_artifact` without inventing a successful result.
- Reject or escalate `ambiguous_lineage` without inventing a successful result.
- Reject or escalate `invalid_edge_type` without inventing a successful result.
- Reject or escalate `evidence_cycle` without inventing a successful result.
...
- Every managed source file has at least one upstream requirement or architecture node.
- Every acceptance criterion has at least one planned verification edge.
- A changed requirement returns a bounded impacted-artifact set.
