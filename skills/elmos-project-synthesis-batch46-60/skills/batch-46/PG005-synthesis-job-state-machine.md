---
id: PG005
name: synthesis-job-state-machine
version: 1.0.0
batch: 46
engine: elmos.project-synthesis
schema: elmos.project-synthesis.v1
status: proposed
summary: Defines the lifecycle, transitions, guards, compensations, and terminal states of a synthesis run.
...
# PG005 — Synthesis Job State Machine

## 1. Objective

Prevent hidden or inconsistent run progress across agents, workers, and human approvals.
...
- Validate and process `engine_contract`.
- Validate and process `approval_matrix`.
- Validate and process `runner_capabilities`.
- Validate and process `retry_policy`.
- Produce and validate `job_state_machine`.
- Produce and validate `transition_guards`.
- Produce and validate `compensation_map`.
- Produce and validate `timeout_policy`.
...
1. Define all lifecycle states from draft through delivery.
2. Assign transition ownership to user, orchestrator, runner, or policy engine.
3. Define guards for approval, evidence availability, and quality gates.
4. Define pause, resume, cancellation, rollback, and partial-failure behavior.
5. Publish deterministic transition tests.
...
- `ILLEGAL_TRANSITION` — emit a structured diagnostic with supporting evidence.
- `STUCK_STATE` — emit a structured diagnostic with supporting evidence.
- `MISSING_COMPENSATION` — emit a structured diagnostic with supporting evidence.
- `APPROVAL_BYPASS` — emit a structured diagnostic with supporting evidence.
...
- Valid input produces all declared outputs for `synthesis-job-state-machine`.
...
- Reject or escalate `illegal_transition` without inventing a successful result.
- Reject or escalate `stuck_state` without inventing a successful result.
- Reject or escalate `missing_compensation` without inventing a successful result.
- Reject or escalate `approval_bypass` without inventing a successful result.
...
- No transition to DELIVERED is possible without required evidence.
- Cancellation from every active state has a defined compensation behavior.
