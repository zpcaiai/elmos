---
id: PG114
name: migration-seed-data-generator
version: 1.0.0
batch: 56
engine: elmos.project-synthesis
schema: elmos.project-synthesis.v1
status: proposed
summary: ordered forward migrations, safe recovery, deterministic reference/test seeds and migration certification tests
depends_on: ["PG032", "PG113"]
capabilities:
  - workspace:read
...
# PG114 — Migration Seed Data Generator

## 1. Objective

Implement ordered forward migrations, safe recovery, deterministic reference/test seeds and migration certification tests as a governed ELMOS Project Synthesis capability.

## 2. Scope

### In scope
- ordered forward migrations, safe recovery, deterministic reference/test seeds and migration certification tests.
...
3. Map stable semantic symbols and generation units to ordered forward migrations, safe recovery, deterministic reference/test seeds and migration certification tests.
...
6. Generate seed/reference data, upgrade/recovery tests from acceptance criteria and failure scenarios.
...
- `UNSAFE_SCHEMA_CHANGE` — emit structured evidence and stop unsafe continuation.
- `TARGET_DATABASE_SEMANTIC_MISMATCH` — emit structured evidence and stop unsafe continuation.
...
- Consume certified outputs from PG032, PG113.
...
- Reject `unsafe_schema_change` without inventing success.
- Reject `target_database_semantic_mismatch` without inventing success.

## 17. Acceptance Criteria

- The generated result implements only the approved responsibility: ordered forward migrations, safe recovery, deterministic reference/test seeds and migration certification tests.
...
- Certification tests use the selected target-compatible database, not only an in-memory substitute.
