---
id: PG022
name: requirement-deduplicator
version: 1.0.0
batch: 48
engine: elmos.project-synthesis
schema: elmos.project-synthesis.v1
status: proposed
summary: Detects exact, semantic, and partially overlapping duplicate requirements.
depends_on: ["PG021"]
capabilities:
  - workspace:read
...
# PG022 — Requirement Deduplicator

## 1. Objective

Prevent duplicated implementation and contradictory acceptance evidence.
...
- Validate and process `normalized_requirements`.
- Validate and process `product_glossary`.
- Validate and process `source_provenance`.
- Produce and validate `duplicate_clusters`.
- Produce and validate `canonical_requirements`.
- Produce and validate `merge_proposals`.
- Produce and validate `retained_provenance`.
...
2. Detect semantic duplicates with different terms or sources.
3. Detect subset and overlap relationships.
4. Propose canonical wording and provenance union.
5. Require review for merges that could alter scope.
...
- `FALSE_MERGE` — emit a structured diagnostic with supporting evidence.
- `MISSED_DUPLICATE` — emit a structured diagnostic with supporting evidence.
- `PROVENANCE_LOSS` — emit a structured diagnostic with supporting evidence.
- `SCOPE_REDUCTION` — emit a structured diagnostic with supporting evidence.
...
- Valid input produces all declared outputs for `requirement-deduplicator`.
...
- Consume valid outputs from PG021.
...
- Reject or escalate `false_merge` without inventing a successful result.
- Reject or escalate `missed_duplicate` without inventing a successful result.
- Reject or escalate `provenance_loss` without inventing a successful result.
- Reject or escalate `scope_reduction` without inventing a successful result.
...
- Merged requirements retain all source references.
- Partial overlaps are not auto-merged without a safe rule.
- Deduplication does not reduce approved scope.
