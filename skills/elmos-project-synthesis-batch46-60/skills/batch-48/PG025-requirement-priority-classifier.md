---
id: PG025
name: requirement-priority-classifier
version: 1.0.0
batch: 48
engine: elmos.project-synthesis
schema: elmos.project-synthesis.v1
status: proposed
summary: Classifies requirement priority using value, urgency, dependency, risk, and delivery slicing.
depends_on: ["PG024"]
capabilities:
  - workspace:read
...
# PG025 — Requirement Priority Classifier

## 1. Objective

Provide an explainable build order and release sequence.
...
- Validate and process `business_value_inputs`.
- Validate and process `dependency_graph`.
- Produce and validate `priority_model`.
- Produce and validate `release_slices`.
- Produce and validate `priority_rationale`.
- Produce and validate `contention_report`.
...
1. Evaluate business value, urgency, dependency centrality, risk reduction, and effort uncertainty.
2. Apply the configured prioritization method.
3. Identify priority conflicts among stakeholders.
4. Create coherent release slices rather than isolated rankings.
5. Record rationale and confidence.
...
- `MISSING_VALUE_INPUT` — emit a structured diagnostic with supporting evidence.
- `PRIORITY_WITHOUT_RATIONALE` — emit a structured diagnostic with supporting evidence.
- `DEPENDENCY_VIOLATION` — emit a structured diagnostic with supporting evidence.
- `STAKEHOLDER_CONTENTION` — emit a structured diagnostic with supporting evidence.
...
- Valid input produces all declared outputs for `requirement-priority-classifier`.
...
- Consume valid outputs from PG024.
...
- Reject or escalate `missing_value_input` without inventing a successful result.
- Reject or escalate `priority_without_rationale` without inventing a successful result.
- Reject or escalate `dependency_violation` without inventing a successful result.
- Reject or escalate `stakeholder_contention` without inventing a successful result.
...
- Higher-priority requirements do not depend on excluded lower-priority prerequisites.
- Every ranking has an explainable rationale.
- Stakeholder conflicts are visible rather than averaged away.
