---
id: PG027
name: measurable-quality-attribute-builder
version: 1.0.0
batch: 48
engine: elmos.project-synthesis
schema: elmos.project-synthesis.v1
status: proposed
summary: Converts quality expectations into measurable scenarios with thresholds, workload, environment, and observation methods.
depends_on: ["PG017", "PG024"]
capabilities:
  - workspace:read
...
# PG027 — Measurable Quality Attribute Builder

## 1. Objective

Make nonfunctional requirements testable and architecture-relevant.
...
- Validate and process `nonfunctional_requirements`.
...
- Produce and validate `measurement_plan`.
- Produce and validate `threshold_gaps`.
- Produce and validate `derived_test_requirements`.
...
1. Define stimulus, source, environment, artifact, response, and response measure.
2. Add workload and data-volume assumptions.
3. Define measurement tooling and pass/fail thresholds.
4. Identify policy minima and user targets.
5. Create derived performance, reliability, and security test requirements.
...
- `MISSING_THRESHOLD` — emit a structured diagnostic with supporting evidence.
- `UNREALISTIC_TARGET` — emit a structured diagnostic with supporting evidence.
- `UNAVAILABLE_MEASUREMENT_TOOL` — emit a structured diagnostic with supporting evidence.
- `ENVIRONMENT_MISMATCH` — emit a structured diagnostic with supporting evidence.
...
- Valid input produces all declared outputs for `measurable-quality-attribute-builder`.
...
- Consume valid outputs from PG017, PG024.
...
- Reject or escalate `missing_threshold` without inventing a successful result.
- Reject or escalate `unrealistic_target` without inventing a successful result.
- Reject or escalate `unavailable_measurement_tool` without inventing a successful result.
- Reject or escalate `environment_mismatch` without inventing a successful result.
...
- Every accepted quality target is measurable.
- Targets include the environment in which they apply.
- Policy minima cannot be weakened by user preference.
