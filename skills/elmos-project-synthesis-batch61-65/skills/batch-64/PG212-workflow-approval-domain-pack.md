---
id: PG212
name: workflow-approval-domain-pack
version: 1.0.0
batch: 64
engine: elmos.project-synthesis
schema: elmos.project-synthesis.v1
status: proposed
summary: Provides configurable forms, cases, tasks, states, rules, approvals, escalations, SLAs, and audit models.
depends_on: ["PG038", "PG179", "PG188", "PG202"]
capabilities:
  - workspace:read
...
# PG212 — Workflow Approval Domain Pack

## 1. Objective

Accelerate general workflow and approval products without losing decision accountability.
...
- Validate and consume workflow_requirements.
- Validate and consume approval_policy.
- Produce and validate workflow_domain_pack.
- Produce and validate state_machine_templates.
- Produce and validate permission_models.
...
1. Model cases, forms, tasks, actors, queues, states, transitions, rules, decisions, comments, and attachments.
2. Define sequential, parallel, quorum, conditional, delegated, and escalated approvals.
3. Define SLA timers, reminders, cancellation, rework, and compensation.
4. Define separation-of-duty and immutable decision audit.
5. Provide concurrency, delegation, expiry, and reassignment tests.
...
- `APPROVAL_BYPASS` — stop unsafe continuation and emit a structured diagnostic.
- `DELEGATION_SCOPE_ERROR` — stop unsafe continuation and emit a structured diagnostic.
- `SLA_TIMER_DUPLICATION` — stop unsafe continuation and emit a structured diagnostic.
- `AUDIT_MUTATION` — stop unsafe continuation and emit a structured diagnostic.
...
- Reject or escalate `approval_bypass` without inventing success.
- Reject or escalate `delegation_scope_error` without inventing success.
- Reject or escalate `sla_timer_duplication` without inventing success.
- Reject or escalate `audit_mutation` without inventing success.
...
- Decisions bind to exact case state and evidence.
- Separation-of-duty rules are enforceable.
- Timers and retries do not duplicate transitions.
