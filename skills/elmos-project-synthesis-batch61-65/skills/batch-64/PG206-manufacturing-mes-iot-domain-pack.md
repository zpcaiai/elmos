---
id: PG206
name: manufacturing-mes-iot-domain-pack
version: 1.0.0
batch: 64
engine: elmos.project-synthesis
schema: elmos.project-synthesis.v1
status: proposed
summary: Provides plant, equipment, work order, operation, quality, maintenance, telemetry, and traceability models.
depends_on: ["PG117", "PG188", "PG202"]
capabilities:
  - workspace:read
...
# PG206 — Manufacturing Mes Iot Domain Pack

## 1. Objective

Accelerate MES and industrial IoT synthesis with OT-aware safety boundaries.
...
- Validate and consume manufacturing_requirements.
- Validate and consume ot_security_policy.
- Produce and validate manufacturing_domain_pack.
- Produce and validate equipment_models.
- Produce and validate iot_contracts.
- Produce and validate industrial_test_scenarios.
...
1. Model plant hierarchy, assets, recipes, work orders, operations, material genealogy, quality, alarms, and maintenance.
2. Define telemetry, command, acknowledgment, and offline synchronization contracts.
3. Separate monitoring from safety-critical control.
4. Provide OPC UA, MQTT, historian, ERP, and maintenance integration profiles.
5. Provide replay, out-of-order, device-offline, and traceability tests.
...
- `UNSAFE_CONTROL_GENERATION` — stop unsafe continuation and emit a structured diagnostic.
- `DEVICE_IDENTITY_GAP` — stop unsafe continuation and emit a structured diagnostic.
- `TELEMETRY_ORDERING_ERROR` — stop unsafe continuation and emit a structured diagnostic.
- `GENEALOGY_BREAK` — stop unsafe continuation and emit a structured diagnostic.
...
- Reject or escalate `unsafe_control_generation` without inventing success.
- Reject or escalate `device_identity_gap` without inventing success.
- Reject or escalate `telemetry_ordering_error` without inventing success.
- Reject or escalate `genealogy_break` without inventing success.
...
- Safety-critical control requires explicit certified control scope.
- Device and tenant identities are authenticated.
- Material and operation genealogy remains traceable.
