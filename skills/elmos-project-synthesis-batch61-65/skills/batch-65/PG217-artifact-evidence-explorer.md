---
id: PG217
name: artifact-evidence-explorer
version: 1.0.0
batch: 65
engine: elmos.project-synthesis
schema: elmos.project-synthesis.v1
status: proposed
summary: Provides searchable lineage and evidence navigation across requirements, code, tests, builds, security, deployments, and releases.
depends_on: ["PG004", "PG007", "PG197"]
capabilities:
  - workspace:read
...
# PG217 — Artifact Evidence Explorer

## 1. Objective

Make ELMOS delivery claims independently auditable.
...
- Validate and consume evidence_store.
- Validate and consume release_manifests.
- Validate and consume access_policy.
- Produce and validate evidence_explorer_views.
- Produce and validate lineage_queries.
- Produce and validate evidence_exports.
...
1. Provide graph and list navigation by artifact, claim, run, requirement, file, test, or release.
2. Show immutable fingerprints, signatures, versions, and source runner.
3. Support path queries from requirement to release and reverse.
4. Apply redaction and access policy to evidence.
5. Generate integrity, authorization, and large-graph tests.
...
- `EVIDENCE_ACCESS_LEAK` — stop unsafe continuation and emit a structured diagnostic.
- `LINEAGE_QUERY_INCORRECT` — stop unsafe continuation and emit a structured diagnostic.
- `TAMPER_STATUS_HIDDEN` — stop unsafe continuation and emit a structured diagnostic.
- `EXPORT_REDACTION_GAP` — stop unsafe continuation and emit a structured diagnostic.
...
- Reject or escalate `evidence_access_leak` without inventing success.
- Reject or escalate `lineage_query_incorrect` without inventing success.
- Reject or escalate `tamper_status_hidden` without inventing success.
- Reject or escalate `export_redaction_gap` without inventing success.
...
- Every visible claim links to supporting evidence.
- Tampered or untrusted evidence is clearly marked.
- Exports preserve integrity metadata and policy.
