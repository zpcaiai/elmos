---
id: PG213
name: requirement-studio-ui
version: 1.0.0
batch: 65
engine: elmos.project-synthesis
schema: elmos.project-synthesis.v1
status: proposed
summary: Provides the interactive workspace for natural-language discovery, source review, requirements, assumptions, conflicts, scope, and baselines.
depends_on: ["PG011", "PG031", "PG179"]
capabilities:
  - workspace:read
...
# PG213 — Requirement Studio Ui

## 1. Objective

Turn requirement intelligence into a reviewable multi-user product experience.
...
- Validate and consume requirement_workspace.
- Validate and consume source_map.
- Validate and consume approval_state.
- Validate and consume ui_policy.
- Produce and validate requirement_studio_views.
- Produce and validate editing_contracts.
- Produce and validate collaboration_events.
- Produce and validate ui_test_suite.
...
1. Design source-linked conversation and specification views.
2. Support requirement, assumption, question, conflict, scope, priority, risk, and criterion editing.
3. Show provenance, confidence, supersession, and approval state.
4. Support comments, assignments, review, and baseline approval.
5. Generate accessibility, authorization, concurrency, and audit tests.
...
- `SOURCE_LINK_HIDDEN` — stop unsafe continuation and emit a structured diagnostic.
- `STALE_EDIT_OVERWRITE` — stop unsafe continuation and emit a structured diagnostic.
- `APPROVAL_STATE_CONFUSION` — stop unsafe continuation and emit a structured diagnostic.
- `UNAUTHORIZED_REQUIREMENT_EDIT` — stop unsafe continuation and emit a structured diagnostic.
...
- Reject or escalate `source_link_hidden` without inventing success.
- Reject or escalate `stale_edit_overwrite` without inventing success.
- Reject or escalate `approval_state_confusion` without inventing success.
- Reject or escalate `unauthorized_requirement_edit` without inventing success.
...
- Every editable statement shows source and status.
- Concurrent edits use explicit conflict handling.
- Approved baselines cannot be silently mutated.
