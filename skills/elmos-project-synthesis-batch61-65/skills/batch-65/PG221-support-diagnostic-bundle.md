---
id: PG221
name: support-diagnostic-bundle
version: 1.0.0
batch: 65
engine: elmos.project-synthesis
schema: elmos.project-synthesis.v1
status: proposed
summary: Creates privacy-safe diagnostic packages for failed or degraded synthesis runs.
depends_on: ["PG007", "PG009", "PG158", "PG216"]
capabilities:
  - workspace:read
...
# PG221 — Support Diagnostic Bundle

## 1. Objective

Enable support and engineering investigation without exposing unnecessary customer data or secrets.
...
- Validate and consume run_state.
- Validate and consume diagnostics.
- Validate and consume evidence.
- Validate and consume policy_context.
- Validate and consume support_request.
- Produce and validate diagnostic_bundle.
- Produce and validate redaction_report.
- Produce and validate reproduction_recipe.
- Produce and validate support_metadata.
...
1. Collect relevant manifests, versions, state transitions, diagnostics, logs, traces, and policy decisions.
2. Minimize to the requested failure scope.
3. Redact secrets, personal data, source content, and tenant identifiers according to policy.
4. Include reproduction steps or synthetic fixtures when possible.
5. Sign and expire the bundle.
...
- `SECRET_LEAK` — stop unsafe continuation and emit a structured diagnostic.
- `OVERCOLLECTION` — stop unsafe continuation and emit a structured diagnostic.
- `INSUFFICIENT_REPRODUCTION_DATA` — stop unsafe continuation and emit a structured diagnostic.
- `BUNDLE_TAMPERING` — stop unsafe continuation and emit a structured diagnostic.
...
- Reject or escalate `secret_leak` without inventing success.
- Reject or escalate `overcollection` without inventing success.
- Reject or escalate `insufficient_reproduction_data` without inventing success.
- Reject or escalate `bundle_tampering` without inventing success.
...
- Bundle contains no prohibited sensitive data.
- Collected information is sufficient for the declared diagnostic scope or limitations are explicit.
- Integrity and expiry are verifiable.
