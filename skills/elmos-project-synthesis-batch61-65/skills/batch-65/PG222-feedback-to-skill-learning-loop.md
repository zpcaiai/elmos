---
id: PG222
name: feedback-to-skill-learning-loop
version: 1.0.0
batch: 65
engine: elmos.project-synthesis
schema: elmos.project-synthesis.v1
status: proposed
summary: Converts user feedback, defects, review findings, support cases, and evaluation failures into governed Skill improvements.
depends_on: ["PG189", "PG190", "PG202", "PG220", "PG221"]
capabilities:
  - workspace:read
...
# PG222 — Feedback To Skill Learning Loop

## 1. Objective

Enable continuous product learning without uncontrolled self-modification.
...
- Validate and consume feedback_events.
- Validate and consume support_findings.
- Validate and consume certification_failures.
- Validate and consume skill_versions.
- Validate and consume product_metrics.
- Produce and validate improvement_candidates.
- Produce and validate training_and_eval_cases.
- Produce and validate skill_change_proposals.
- Produce and validate learning_evidence.
...
1. Normalize feedback and link it to runs, artifacts, Skills, models, templates, and Domain Packs.
2. Cluster recurring failure patterns while preserving severity and context.
3. Generate candidate prompt, contract, template, test, routing, or product changes.
4. Add regression cases before implementation.
5. Publish changes only through review, certification, versioning, and rollout.
...
- `FEEDBACK_POISONING` — stop unsafe continuation and emit a structured diagnostic.
- `SELF_MODIFICATION_WITHOUT_REVIEW` — stop unsafe continuation and emit a structured diagnostic.
- `PRIVACY_LEAK` — stop unsafe continuation and emit a structured diagnostic.
- `METRIC_GAMING` — stop unsafe continuation and emit a structured diagnostic.
...
- Reject or escalate `feedback_poisoning` without inventing success.
- Reject or escalate `self_modification_without_review` without inventing success.
- Reject or escalate `privacy_leak` without inventing success.
- Reject or escalate `metric_gaming` without inventing success.
...
- No feedback directly changes production behavior.
- Every improvement has a regression test or evaluation case.
- Published changes pass normal certification and rollback controls.
