---
id: PG190
name: skill-version-compatibility-manager
version: 1.0.0
batch: 62
engine: elmos.project-synthesis
schema: elmos.project-synthesis.v1
status: proposed
summary: Manages Skill, schema, template, model adapter, and Domain Pack compatibility across platform releases.
depends_on: ["PG184", "PG188", "PG189"]
capabilities:
  - workspace:read
...
# PG190 — Skill Version Compatibility Manager

## 1. Objective

Prevent silent breakage as the Skills ecosystem evolves.
...
- Validate and consume installed_artifacts.
- Validate and consume compatibility_rules.
- Validate and consume platform_version.
- Validate and consume upgrade_candidates.
- Produce and validate compatibility_matrix.
- Produce and validate upgrade_plan.
- Produce and validate pin_set.
- Produce and validate deprecation_alerts.
...
1. Build a dependency and version graph.
2. Evaluate semantic-version and explicit compatibility rules.
3. Detect conflicts, deprecated interfaces, and vulnerable versions.
4. Plan staged upgrades and compatibility adapters.
5. Pin a coherent version set for each run.
...
- `DEPENDENCY_VERSION_CONFLICT` — stop unsafe continuation and emit a structured diagnostic.
- `HIDDEN_BREAKING_CHANGE` — stop unsafe continuation and emit a structured diagnostic.
- `DEPRECATED_CONTRACT_USED` — stop unsafe continuation and emit a structured diagnostic.
- `INCOHERENT_PIN_SET` — stop unsafe continuation and emit a structured diagnostic.
...
- Reject or escalate `dependency_version_conflict` without inventing success.
- Reject or escalate `hidden_breaking_change` without inventing success.
- Reject or escalate `deprecated_contract_used` without inventing success.
- Reject or escalate `incoherent_pin_set` without inventing success.
...
- Every run uses a coherent pinned Skill set.
- Breaking upgrades require explicit migration.
- Compatibility evidence is retained with the run.
