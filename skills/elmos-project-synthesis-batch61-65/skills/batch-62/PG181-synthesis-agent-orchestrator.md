...
status: proposed
summary: Coordinates specialist agents, skills, tools, approvals, budgets, and state transitions for Project Synthesis.
depends_on: ["PG005", "PG008", "PG009", "PG180"]
capabilities:
  - workspace:read
...
# PG181 — Synthesis Agent Orchestrator

## 1. Objective

Execute complex synthesis workflows as a governed graph rather than an unconstrained autonomous loop.
...
- Validate and consume run_request.
- Validate and consume project_state.
- Validate and consume skill_registry.
- Validate and consume policy_context.
- Validate and consume budget.
- Produce and validate execution_graph.
- Produce and validate agent_assignments.
- Produce and validate run_events.
- Produce and validate final_run_state.
...
1. Compile the requested workflow into a bounded dependency graph.
2. Assign specialist roles and skills using capabilities and confidence.
3. Enforce state, approval, budget, retry, and cancellation rules.
4. Persist checkpoints and Artifact Graph updates after each step.
5. Terminate with delivered, paused, blocked, cancelled, or failed state and evidence.
...
- `AGENT_LOOP` — stop unsafe continuation and emit a structured diagnostic.
- `STATE_DIVERGENCE` — stop unsafe continuation and emit a structured diagnostic.
- `BUDGET_OVERRUN` — stop unsafe continuation and emit a structured diagnostic.
- `CAPABILITY_VIOLATION` — stop unsafe continuation and emit a structured diagnostic.
...
- Reject or escalate `agent_loop` without inventing success.
- Reject or escalate `state_divergence` without inventing success.
- Reject or escalate `budget_overrun` without inventing success.
- Reject or escalate `capability_violation` without inventing success.
...
- No agent can bypass the run state machine.
- Execution is resumable from persisted checkpoints.
- Loops, retries, and total cost are bounded.
