---
id: PG189
name: template-skill-marketplace-publisher
version: 1.0.0
batch: 62
engine: elmos.project-synthesis
schema: elmos.project-synthesis.v1
status: proposed
summary: Publishes governed Skills, templates, profiles, and Domain Packs into an internal or external marketplace.
depends_on: ["PG066", "PG188"]
capabilities:
  - workspace:read
...
# PG189 — Template Skill Marketplace Publisher

## 1. Objective

Create a discoverable supply chain with trust, review, licensing, and lifecycle controls.
...
- Validate and consume publishable_artifact.
- Validate and consume provenance.
- Validate and consume certification_results.
- Validate and consume license_metadata.
- Validate and consume visibility_policy.
- Produce and validate marketplace_entry.
- Produce and validate signed_release.
- Produce and validate search_metadata.
- Produce and validate lifecycle_record.
...
1. Validate identity, version, publisher, license, documentation, tests, and signatures.
2. Verify certification and security results.
3. Generate searchable capability and compatibility metadata.
4. Publish with visibility, deprecation, support, and revocation rules.
5. Record downloads, usage, issues, and upgrade paths.
...
- `PUBLISHER_UNTRUSTED` — stop unsafe continuation and emit a structured diagnostic.
- `LICENSE_MISSING` — stop unsafe continuation and emit a structured diagnostic.
- `CERTIFICATION_INSUFFICIENT` — stop unsafe continuation and emit a structured diagnostic.
- `MALICIOUS_ARTIFACT` — stop unsafe continuation and emit a structured diagnostic.
...
- Reject or escalate `publisher_untrusted` without inventing success.
- Reject or escalate `license_missing` without inventing success.
- Reject or escalate `certification_insufficient` without inventing success.
- Reject or escalate `malicious_artifact` without inventing success.
...
- Only signed and policy-compliant artifacts publish.
- Marketplace entries declare support and deprecation state.
- Revoked artifacts cannot be newly selected by routing.
