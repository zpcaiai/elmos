---
id: PG182
name: planner-coder-reviewer-agent-team
version: 1.0.0
batch: 62
engine: elmos.project-synthesis
schema: elmos.project-synthesis.v1
status: proposed
summary: Defines separated planner, implementer, reviewer, tester, security, and certifier responsibilities.
depends_on: ["PG181"]
capabilities:
  - workspace:read
...
# PG182 — Planner Coder Reviewer Agent Team

## 1. Objective

Reduce self-confirmation bias and enforce independent review of generated work.
...
- Validate and consume execution_graph.
- Validate and consume project_context.
- Validate and consume agent_profiles.
- Validate and consume separation_policy.
- Produce and validate team_plan.
- Produce and validate role_assignments.
- Produce and validate handoff_contracts.
- Produce and validate independence_report.
...
1. Assign planning, coding, review, test, security, and certification roles.
2. Define allowed inputs and outputs for each role.
3. Prevent implementers from self-approving critical claims.
4. Require fresh-context review for configured risk classes.
5. Record disagreements and resolution paths.
...
- `SELF_APPROVAL` — stop unsafe continuation and emit a structured diagnostic.
- `ROLE_CONFLICT` — stop unsafe continuation and emit a structured diagnostic.
- `CONTEXT_CONTAMINATION` — stop unsafe continuation and emit a structured diagnostic.
- `MISSING_INDEPENDENT_REVIEW` — stop unsafe continuation and emit a structured diagnostic.
...
- Reject or escalate `self_approval` without inventing success.
- Reject or escalate `role_conflict` without inventing success.
- Reject or escalate `context_contamination` without inventing success.
- Reject or escalate `missing_independent_review` without inventing success.
...
- Critical delivery claims have an independent reviewer or certifier.
- Role boundaries are machine-enforced.
- Disagreements remain visible in evidence.
