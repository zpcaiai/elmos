---
id: PG186
name: model-routing-and-budget-controller
version: 1.0.0
batch: 62
engine: elmos.project-synthesis
schema: elmos.project-synthesis.v1
status: proposed
summary: Routes tasks across models and controls token, latency, monetary, and retry budgets.
depends_on: ["PG009", "PG181"]
capabilities:
  - workspace:read
...
# PG186 — Model Routing And Budget Controller

## 1. Objective

Use the least costly model path that still satisfies quality and risk requirements.
...
- Validate and consume task_profile.
- Validate and consume model_catalog.
- Validate and consume quality_requirements.
- Validate and consume budget.
- Validate and consume runtime_metrics.
- Produce and validate model_route.
- Produce and validate budget_allocations.
- Produce and validate fallback_route.
- Produce and validate usage_evidence.
...
1. Classify reasoning, coding, extraction, review, and certification demands.
2. Filter models by capability, privacy, region, latency, and policy.
3. Allocate per-step and total budgets.
4. Define fallback and escalation thresholds.
5. Stop or pause before exceeding hard limits.
...
- `INADEQUATE_MODEL` — stop unsafe continuation and emit a structured diagnostic.
- `BUDGET_OVERRUN` — stop unsafe continuation and emit a structured diagnostic.
- `PRIVACY_ROUTE_VIOLATION` — stop unsafe continuation and emit a structured diagnostic.
- `FALLBACK_LOOP` — stop unsafe continuation and emit a structured diagnostic.
...
- Reject or escalate `inadequate_model` without inventing success.
- Reject or escalate `budget_overrun` without inventing success.
- Reject or escalate `privacy_route_violation` without inventing success.
- Reject or escalate `fallback_loop` without inventing success.
...
- Hard budgets cannot be exceeded silently.
- High-risk certification uses approved model classes.
- Every model call records identity, route, usage, and purpose.
