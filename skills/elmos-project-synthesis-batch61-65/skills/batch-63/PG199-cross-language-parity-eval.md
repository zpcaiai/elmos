---
id: PG199
name: cross-language-parity-eval
version: 1.0.0
batch: 63
engine: elmos.project-synthesis
schema: elmos.project-synthesis.v1
status: proposed
summary: Compares Java, Python, and .NET implementations of the same approved specification.
depends_on: ["PG077", "PG089", "PG101", "PG191"]
capabilities:
  - workspace:read
...
# PG199 — Cross Language Parity Eval

## 1. Objective

Measure behavioral and operational parity without forcing identical source structures.
...
- Validate and consume shared_benchmark_spec.
- Validate and consume java_result.
- Validate and consume python_result.
- Validate and consume dotnet_result.
- Validate and consume parity_policy.
- Produce and validate parity_score.
- Produce and validate behavior_delta.
...
1. Run common API, workflow, data, security, and failure scenarios.
2. Compare wire contracts, error semantics, state transitions, and authorization.
3. Compare build, startup, telemetry, and deployment obligations.
4. Allow language-idiomatic internal differences.
5. Publish material parity gaps.
...
- `BEHAVIOR_PARITY_GAP` — stop unsafe continuation and emit a structured diagnostic.
- `ERROR_SEMANTICS_GAP` — stop unsafe continuation and emit a structured diagnostic.
- `SECURITY_PARITY_GAP` — stop unsafe continuation and emit a structured diagnostic.
- `FALSE_SOURCE_SIMILARITY_METRIC` — stop unsafe continuation and emit a structured diagnostic.
...
- Reject or escalate `behavior_parity_gap` without inventing success.
- Reject or escalate `error_semantics_gap` without inventing success.
- Reject or escalate `security_parity_gap` without inventing success.
- Reject or escalate `false_source_similarity_metric` without inventing success.
...
- Externally visible behavior satisfies the shared specification.
- Security and tenant isolation are equivalent.
- Parity does not require identical implementation structure.
