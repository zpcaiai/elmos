---
id: PG192
name: requirements-understanding-eval
version: 1.0.0
batch: 63
engine: elmos.project-synthesis
schema: elmos.project-synthesis.v1
status: proposed
summary: Evaluates extraction, ambiguity handling, scope, conflict detection, assumptions, and acceptance criteria.
depends_on: ["PG011", "PG020", "PG021", "PG031", "PG191"]
capabilities:
  - workspace:read
...
# PG192 — Requirements Understanding Eval

## 1. Objective

Measure whether natural-language intent becomes a faithful executable specification.
...
- Validate and consume benchmark_requirements.
- Validate and consume generated_requirement_workspace.
- Validate and consume source_materials.
- Validate and consume scoring_policy.
- Produce and validate requirements_eval_score.
- Produce and validate error_catalog.
- Produce and validate traceability_score.
...
- `generated_requirement_workspace`
- `source_materials`
- `scoring_policy`
...
1. Compare explicit facts, omissions, additions, conflicts, and supersession handling.
2. Evaluate assumption visibility and question value.
3. Evaluate scope classification and measurable acceptance criteria.
4. Score traceability to source evidence.
5. Publish severity-weighted errors and examples.
...
- `HALLUCINATED_REQUIREMENT` — stop unsafe continuation and emit a structured diagnostic.
- `CRITICAL_OMISSION` — stop unsafe continuation and emit a structured diagnostic.
- `CONFLICT_MISSED` — stop unsafe continuation and emit a structured diagnostic.
- `SOURCE_TRACE_MISSING` — stop unsafe continuation and emit a structured diagnostic.
...
- Reject or escalate `hallucinated_requirement` without inventing success.
- Reject or escalate `critical_omission` without inventing success.
- Reject or escalate `conflict_missed` without inventing success.
- Reject or escalate `source_trace_missing` without inventing success.
...
- Critical hallucination or omission fails certification.
- Scores separate correctness from completeness.
- Human-reviewed reference labels are versioned.
