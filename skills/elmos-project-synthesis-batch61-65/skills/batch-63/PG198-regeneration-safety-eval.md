---
id: PG198
name: regeneration-safety-eval
version: 1.0.0
batch: 63
engine: elmos.project-synthesis
schema: elmos.project-synthesis.v1
status: proposed
summary: Evaluates idempotency, incremental scope, manual-edit preservation, merge behavior, and regression safety.
depends_on: ["PG173", "PG174", "PG180", "PG191"]
capabilities:
  - workspace:read
...
# PG198 — Regeneration Safety Eval

## 1. Objective

Certify that projects can evolve without destructive regeneration.
...
- Validate and consume golden_project_versions.
- Validate and consume change_requests.
- Validate and consume previous_and_current_repositories.
- Validate and consume ownership_manifest.
- Produce and validate regeneration_score.
- Produce and validate preservation_findings.
- Produce and validate diff_quality.
- Produce and validate regression_evidence.
...
- `golden_project_versions`
- `change_requests`
- `previous_and_current_repositories`
- `ownership_manifest`
...
1. Run identical-input idempotency tests.
2. Apply additive, breaking, security, data, and architecture change scenarios.
3. Insert protected and user-owned modifications before regeneration.
4. Evaluate diff scope, merge conflicts, preservation, and regressions.
5. Publish unsafe overwrite and over-generation findings.
...
- `USER_EDIT_LOST` — stop unsafe continuation and emit a structured diagnostic.
- `NON_IDEMPOTENT_DIFF` — stop unsafe continuation and emit a structured diagnostic.
- `UNDER_SCOPED_CHANGE` — stop unsafe continuation and emit a structured diagnostic.
- `OVERBROAD_CHANGE` — stop unsafe continuation and emit a structured diagnostic.
...
- Reject or escalate `user_edit_lost` without inventing success.
- Reject or escalate `non_idempotent_diff` without inventing success.
- Reject or escalate `under_scoped_change` without inventing success.
- Reject or escalate `overbroad_change` without inventing success.
...
- Identical inputs produce no semantic diff.
- Protected and user-owned edits survive.
- Change output stays within justified impact scope.
