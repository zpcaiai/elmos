---
id: PG177
name: database-api-compatibility-check
version: 1.0.0
batch: 61
engine: elmos.project-synthesis
schema: elmos.project-synthesis.v1
status: proposed
summary: Checks schema, migration, REST, RPC, event, and webhook compatibility for a change set.
depends_on: ["PG114", "PG115", "PG116", "PG117", "PG124", "PG175"]
capabilities:
  - workspace:read
...
# PG177 — Database Api Compatibility Check

## 1. Objective

Prevent unsafe consumer, provider, or data evolution during regeneration.
...
- Validate and consume previous_contract_versions.
- Validate and consume proposed_contracts.
- Validate and consume migration_plan.
- Validate and consume consumer_provider_map.
- Validate and consume support_policy.
- Produce and validate compatibility_report.
- Produce and validate breaking_changes.
- Produce and validate migration_requirements.
- Produce and validate consumer_actions.
...
1. Run format-specific backward and forward compatibility checks.
2. Evaluate database expand-migrate-contract sequencing.
3. Map breaking changes to active consumers and supported versions.
4. Require versioning, dual-read/write, adapters, or migration windows where needed.
5. Publish machine-readable and human review results.
...
- `BREAKING_CHANGE_MISSED` — stop unsafe continuation and emit a structured diagnostic.
- `UNSAFE_CONTRACT_REMOVAL` — stop unsafe continuation and emit a structured diagnostic.
- `MIGRATION_WINDOW_GAP` — stop unsafe continuation and emit a structured diagnostic.
- `CONSUMER_UNKNOWN` — stop unsafe continuation and emit a structured diagnostic.
...
- Reject or escalate `breaking_change_missed` without inventing success.
- Reject or escalate `unsafe_contract_removal` without inventing success.
- Reject or escalate `migration_window_gap` without inventing success.
- Reject or escalate `consumer_unknown` without inventing success.
...
- All supported consumers are evaluated.
- Breaking changes cannot publish without explicit approval and migration guidance.
- Database changes include deploy-order and rollback/forward-recovery implications.
