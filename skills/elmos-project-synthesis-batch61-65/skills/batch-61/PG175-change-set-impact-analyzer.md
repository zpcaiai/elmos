---
id: PG175
name: change-set-impact-analyzer
version: 1.0.0
batch: 61
engine: elmos.project-synthesis
schema: elmos.project-synthesis.v1
status: proposed
summary: Analyzes proposed repository changes across behavior, contracts, data, security, operations, and cost.
depends_on: ["PG032", "PG046", "PG050", "PG053", "PG174"]
capabilities:
  - workspace:read
...
# PG175 — Change Set Impact Analyzer

## 1. Objective

Give reviewers a precise explanation of what a generated or manual change will alter.
...
- Validate and consume change_set.
- Validate and consume requirements_baseline.
- Validate and consume architecture_baseline.
- Validate and consume artifact_graph.
- Validate and consume runtime_and_delivery_models.
- Produce and validate impact_report.
- Produce and validate risk_delta.
- Produce and validate contract_delta.
- Produce and validate operational_delta.
...
1. Classify file and symbol changes by upstream intent.
2. Evaluate public API, event, database, permission, workflow, and deployment effects.
3. Estimate revalidation, migration, rollout, rollback, and cost implications.
4. Detect changes whose observed impact exceeds declared scope.
5. Publish reviewer-focused risk and evidence summaries.
...
- `HIDDEN_BREAKING_CHANGE` — stop unsafe continuation and emit a structured diagnostic.
- `RISK_UNDERSTATEMENT` — stop unsafe continuation and emit a structured diagnostic.
- `SCOPE_MISMATCH` — stop unsafe continuation and emit a structured diagnostic.
- `OPERATIONAL_IMPACT_GAP` — stop unsafe continuation and emit a structured diagnostic.
...
- Reject or escalate `hidden_breaking_change` without inventing success.
- Reject or escalate `risk_understatement` without inventing success.
- Reject or escalate `scope_mismatch` without inventing success.
- Reject or escalate `operational_impact_gap` without inventing success.
...
- Public and data breaking changes are explicitly identified.
- Security and tenant-isolation deltas cannot be classified as cosmetic.
- Impact claims link to concrete changed artifacts.
