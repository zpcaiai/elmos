---
id: PG180
name: git-pr-delivery-manager
version: 1.0.0
batch: 61
engine: elmos.project-synthesis
schema: elmos.project-synthesis.v1
status: proposed
summary: Packages approved incremental changes into reviewable commits and pull or merge requests.
depends_on: ["PG158", "PG175", "PG177", "PG179"]
capabilities:
  - workspace:read
...
# PG180 — Git Pr Delivery Manager

## 1. Objective

Turn synthesis updates into auditable source-control deliveries.
...
- Validate and consume approved_merge_result.
- Validate and consume impact_report.
- Validate and consume test_and_build_evidence.
- Validate and consume approval_records.
- Validate and consume repository_policy.
- Produce and validate commit_plan.
- Produce and validate branch.
- Produce and validate pull_request_package.
- Produce and validate delivery_evidence.
...
1. Create an isolated branch from the approved base revision.
2. Group changes into intentional commits with trace identifiers.
3. Generate PR title, description, risk summary, test evidence, migration notes, and rollback guidance.
4. Apply labels, owners, required checks, and generated-file review rules.
5. Publish delivery references without merging unless separately authorized.
...
- `WRONG_BASE_REVISION` — stop unsafe continuation and emit a structured diagnostic.
- `UNREVIEWABLE_COMMIT` — stop unsafe continuation and emit a structured diagnostic.
- `MISSING_EVIDENCE` — stop unsafe continuation and emit a structured diagnostic.
- `UNAUTHORIZED_MERGE` — stop unsafe continuation and emit a structured diagnostic.
...
- Reject or escalate `wrong_base_revision` without inventing success.
- Reject or escalate `unreviewable_commit` without inventing success.
- Reject or escalate `missing_evidence` without inventing success.
- Reject or escalate `unauthorized_merge` without inventing success.
...
- Every commit maps to an approved change scope.
- PR descriptions include compatibility, data, security, and rollout effects.
- The skill does not merge without explicit authority.
