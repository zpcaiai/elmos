---
name: elmos-checkpoint-resume
description: Create durable checkpoints at stage and interval boundaries so service or worker failure resumes from verified state instead of restarting the full conversion.
version: 1.0.0
...
Create durable checkpoints at stage and interval boundaries so service or worker failure resumes from verified state instead of restarting the full conversion.
...
- Dependency skills have passed: **elmos-intermediate-artifact-manifest, elmos-run-journal-state-machine, elmos-content-addressable-storage**.
...
- A checkpoint references only sealed, digest-verified artifacts and committed state.
- Side effects are represented by idempotency receipts or compensation state.
- Resume validates source snapshot, ActionKey, stage/version, rule pack, toolchain, dependent checkpoints, and journal boundary.
- Checkpoint chains are immutable and monotonic per node attempt.
...
1. Choose stage-boundary and timed policies based on recomputation cost, partitionability, and artifact size.
2. Flush staged files, manifests, journal, node progress, and side-effect receipts before checkpoint commit.
3. Store the checkpoint manifest in CAS and atomically attach it to the node version.
4. Validate chain integrity and select the newest compatible checkpoint on recovery.
5. Resume completed file/symbol partitions idempotently and regenerate only incomplete or invalid work.
6. Add kill-point tests before and after every checkpoint commit boundary.
