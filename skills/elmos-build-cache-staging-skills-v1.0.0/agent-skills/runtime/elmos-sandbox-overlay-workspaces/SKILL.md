---
name: elmos-sandbox-overlay-workspaces
description: Create isolated copy-on-write workspaces combining immutable source snapshots, writable conversion overlays, durable staged output, and disposable scratch data.
version: 1.0.0
...
# Sandbox and Overlay Workspaces

## Outcome

Create isolated copy-on-write workspaces combining immutable source snapshots, writable conversion overlays, durable staged output, and disposable scratch data.
...
- Dependency skills have passed: **elmos-project-generation-file-staging, elmos-atomic-file-write-promotion**.
...
- Source snapshot content is read-only inside execution.
- All writes are constrained to declared overlay, scratch, and generated roots.
- Changing one run's overlay cannot mutate CAS, the source snapshot, or another run.
- Byte, file, inode, and process quotas are enforced before host exhaustion.
...
1. Select reflink, overlayfs, sandbox mount, hardlink with write guard, or full copy according to platform capability.
2. Materialize source and dependency artifacts with verified digests and immutable permissions.
3. Mount only approved roots and deny host home, credentials, sockets, and unrelated project paths.
4. Track workspace resource usage and undeclared output.
5. Export only declared outputs; quarantine everything else.
6. Add platform-specific isolation tests and documented fallbacks.
