---
name: elmos-generation-conflict-merge
description: Resolve collisions among generators, retries, framework adapters, previous generated output, and user edits without silent data loss.
version: 1.0.0
...
# Generation Conflict and Merge

## Outcome

Resolve collisions among generators, retries, framework adapters, previous generated output, and user edits without silent data loss.
...
- Dependency skills have passed: **elmos-project-generation-file-staging, elmos-atomic-file-write-promotion, elmos-intermediate-artifact-manifest**.
...
- Generated ownership is explicit at path or protected-region level.
- User-owned content is never overwritten silently.
- Three-way merge uses an immutable base and records all decisions.
...
1. Classify files as fully generated, generated with protected regions, user owned, shared, or externally managed.
2. Detect path, case, module, symbol, dependency-manifest, and protected-region collisions before publication.
3. Implement deterministic mergers for dependency files, routing tables, registrations, configuration, and schema manifests.
4. Create three-way patches for supported text files and preserve all unresolved candidates in CAS.
5. Persist manual and automatic resolution rules so retries remain deterministic.
