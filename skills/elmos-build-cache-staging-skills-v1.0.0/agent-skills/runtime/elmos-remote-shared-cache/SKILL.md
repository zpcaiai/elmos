---
name: elmos-remote-shared-cache
description: Scale reuse across workers and teams with S3/MinIO blobs, PostgreSQL metadata, optional Redis leases/hot indexes, resumable transfer, and trust namespaces.
version: 1.0.0
package: elmos-build-cache-staging-recovery
phase: P5-distributed
...
Scale reuse across workers and teams with S3/MinIO blobs, PostgreSQL metadata, optional Redis leases/hot indexes, resumable transfer, and trust namespaces.
...
- Executing phase `P5-distributed` of the cache/staging capability DAG.
- Diagnosing correctness, recoverability, cache-hit, storage, or publication problems related to this capability.
- Dependency skills have passed: **elmos-content-addressable-storage, elmos-action-cache, elmos-cache-metadata-database, elmos-cache-api-cli-contracts**.
...
- Remote outage cannot corrupt local execution.
- Object identity is verified end to end independently of transport checksums.
- Official, branch, fork, experimental, and quarantined results use separate trust namespaces.
- An ActionResult becomes discoverable only after all referenced blobs and manifests are durable.
...
1. Implement read-through, write-through, and write-behind policies with bounded queues and retry budgets.
2. Use conditional object creation, multipart upload, checksums, and atomic metadata publication.
3. Deduplicate remote misses with leases while permitting bounded speculative execution.
4. Support offline runs and later synchronization without overwriting canonical entries.
5. Add regional replica, prefetch, scrub, repair, bandwidth, and egress controls.
