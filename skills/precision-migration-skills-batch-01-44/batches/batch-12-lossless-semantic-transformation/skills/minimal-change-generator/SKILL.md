---
name: minimal-change-generator
description: 优先生成最小语义变更集，限制无关格式化、重排和风格漂移。
---
...
优先生成最小语义变更集，限制无关格式化、重排和风格漂移。

本 Skill 属于精密语言转换系统。它必须以**可执行证据**为准，不得用模型自评、源码相似度或“能编译”替代语义与行为验证。

## Use when

- 当前任务直接涉及 `minimal-change-generator` 所描述的能力。
...
1. 验证规则前置条件
2. 执行本 Skill 的核心职责：优先生成最小语义变更集，限制无关格式化、重排和风格漂移。
3. 优先使用确定性规则
4. 对长尾生成多个候选
5. 按可验证性和成本排序
6. 运行最小构建/静态验证并输出结果集
...
```yaml
skill: minimal-change-generator
version: 1.0.0
