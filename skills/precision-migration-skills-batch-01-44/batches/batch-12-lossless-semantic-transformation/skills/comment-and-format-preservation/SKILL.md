---
name: comment-and-format-preservation
description: 在转换中保留注释、文档、格式和组织约定，避免无关 Diff。
---
...
在转换中保留注释、文档、格式和组织约定，避免无关 Diff。

本 Skill 属于精密语言转换系统。它必须以**可执行证据**为准，不得用模型自评、源码相似度或“能编译”替代语义与行为验证。

## Use when

- 当前任务直接涉及 `comment-and-format-preservation` 所描述的能力。
...
1. 验证规则前置条件
2. 执行本 Skill 的核心职责：在转换中保留注释、文档、格式和组织约定，避免无关 Diff。
3. 优先使用确定性规则
4. 对长尾生成多个候选
5. 按可验证性和成本排序
6. 运行最小构建/静态验证并输出结果集
...
```yaml
skill: comment-and-format-preservation
version: 1.0.0
