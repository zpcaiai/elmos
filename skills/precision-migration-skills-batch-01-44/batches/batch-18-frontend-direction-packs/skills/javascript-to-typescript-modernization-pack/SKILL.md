---
name: javascript-to-typescript-modernization-pack
description: 将 JavaScript 仓库迁移为严格 TypeScript，恢复类型、对象 Shape、Nullability 和运行时校验。
---

# Javascript To Typescript Modernization Pack
...
- Phase: `F 前端与多端互转`
- Version: `1.0.0`
- Tags: batch-18, frontend, frontend-direction-packs, modernization-pack

## Purpose

将 JavaScript 仓库迁移为严格 TypeScript，恢复类型、对象 Shape、Nullability 和运行时校验。

本 Skill 属于精密语言转换系统。它必须以**可执行证据**为准，不得用模型自评、源码相似度或“能编译”替代语义与行为验证。

## Use when

- 当前任务直接涉及 `javascript-to-typescript-modernization-pack` 所描述的能力。
...
1. 恢复组件契约和UI状态机
2. 执行本 Skill 的核心职责：将 JavaScript 仓库迁移为严格 TypeScript，恢复类型、对象 Shape、Nullability 和运行时校验。
3. 映射事件、路由、布局和平台能力
4. 生成目标项目
...
```yaml
skill: javascript-to-typescript-modernization-pack
version: 1.0.0
