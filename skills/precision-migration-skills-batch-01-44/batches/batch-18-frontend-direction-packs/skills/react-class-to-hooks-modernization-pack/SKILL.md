---
name: react-class-to-hooks-modernization-pack
description: 将 React Class 组件迁移为函数组件与 Hooks，并验证生命周期、闭包、Effect 和取消语义。
---

# React Class To Hooks Modernization Pack
...
将 React Class 组件迁移为函数组件与 Hooks，并验证生命周期、闭包、Effect 和取消语义。

本 Skill 属于精密语言转换系统。它必须以**可执行证据**为准，不得用模型自评、源码相似度或“能编译”替代语义与行为验证。

## Use when

- 当前任务直接涉及 `react-class-to-hooks-modernization-pack` 所描述的能力。
...
1. 恢复组件契约和UI状态机
2. 执行本 Skill 的核心职责：将 React Class 组件迁移为函数组件与 Hooks，并验证生命周期、闭包、Effect 和取消语义。
3. 映射事件、路由、布局和平台能力
4. 生成目标项目
...
```yaml
skill: react-class-to-hooks-modernization-pack
version: 1.0.0
