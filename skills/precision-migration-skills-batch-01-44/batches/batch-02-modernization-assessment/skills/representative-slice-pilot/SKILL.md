---
name: representative-slice-pilot
description: 自动选择代表性语义切片执行试转换，以校准正确率、修复轮数、成本和风险。
---

# Representative Slice Pilot
...
自动选择代表性语义切片执行试转换，以校准正确率、修复轮数、成本和风险。

本 Skill 属于精密语言转换系统。它必须以**可执行证据**为准，不得用模型自评、源码相似度或“能编译”替代语义与行为验证。

## Use when

- 当前任务直接涉及 `representative-slice-pilot` 所描述的能力。
...
1. 确认决策问题与可比较边界
2. 执行本 Skill 的核心职责：自动选择代表性语义切片执行试转换，以校准正确率、修复轮数、成本和风险。
3. 收集可验证证据并标准化
4. 应用评分/比较/试点模型
5. 显式列出未知项和敏感假设
6. 输出推荐、备选与拒绝条件
...
```yaml
skill: representative-slice-pilot
version: 1.0.0
