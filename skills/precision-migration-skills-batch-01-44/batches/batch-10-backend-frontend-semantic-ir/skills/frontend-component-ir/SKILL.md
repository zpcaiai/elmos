---
name: frontend-component-ir
description: 表达组件输入、输出、状态、派生状态、子内容、事件和渲染语义。
---

# Frontend Component Ir
...
表达组件输入、输出、状态、派生状态、子内容、事件和渲染语义。

本 Skill 属于精密语言转换系统。它必须以**可执行证据**为准，不得用模型自评、源码相似度或“能编译”替代语义与行为验证。

## Use when

- 当前任务直接涉及 `frontend-component-ir` 所描述的能力。
...
1. 定义可观察边界
2. 执行本 Skill 的核心职责：表达组件输入、输出、状态、派生状态、子内容、事件和渲染语义。
3. 将源语义归一化到受限模型
4. 保留不可丢失信息
5. 标记近似与不支持语义
