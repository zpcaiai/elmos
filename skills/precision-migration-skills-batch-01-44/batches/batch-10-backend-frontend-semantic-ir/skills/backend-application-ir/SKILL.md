---
name: backend-application-ir
description: 定义应用、模块、端口、服务、命令、查询、实体和部署边界。
---

# Backend Application Ir
...
定义应用、模块、端口、服务、命令、查询、实体和部署边界。

本 Skill 属于精密语言转换系统。它必须以**可执行证据**为准，不得用模型自评、源码相似度或“能编译”替代语义与行为验证。

## Use when

- 当前任务直接涉及 `backend-application-ir` 所描述的能力。
...
1. 定义可观察边界
2. 执行本 Skill 的核心职责：定义应用、模块、端口、服务、命令、查询、实体和部署边界。
3. 将源语义归一化到受限模型
4. 保留不可丢失信息
5. 标记近似与不支持语义
