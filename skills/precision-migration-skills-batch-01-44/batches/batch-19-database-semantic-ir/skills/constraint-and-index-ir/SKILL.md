---
name: constraint-and-index-ir
description: 表达主外键、唯一、检查、索引、分区、聚簇、部分和表达式索引。
---

# Constraint And Index Ir
...
表达主外键、唯一、检查、索引、分区、聚簇、部分和表达式索引。

本 Skill 属于精密语言转换系统。它必须以**可执行证据**为准，不得用模型自评、源码相似度或“能编译”替代语义与行为验证。

## Use when

- 当前任务直接涉及 `constraint-and-index-ir` 所描述的能力。
...
1. 盘点对象与专有能力
2. 执行本 Skill 的核心职责：表达主外键、唯一、检查、索引、分区、聚簇、部分和表达式索引。
