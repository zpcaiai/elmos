---
name: transaction-isolation-ir
description: 表达隔离级别、锁、MVCC、保存点、死锁、重试和可见性。
---

# Transaction Isolation Ir
...
表达隔离级别、锁、MVCC、保存点、死锁、重试和可见性。

本 Skill 属于精密语言转换系统。它必须以**可执行证据**为准，不得用模型自评、源码相似度或“能编译”替代语义与行为验证。

## Use when

- 当前任务直接涉及 `transaction-isolation-ir` 所描述的能力。
...
1. 盘点对象与专有能力
2. 执行本 Skill 的核心职责：表达隔离级别、锁、MVCC、保存点、死锁、重试和可见性。
...
```yaml
skill: transaction-isolation-ir
version: 1.0.0
