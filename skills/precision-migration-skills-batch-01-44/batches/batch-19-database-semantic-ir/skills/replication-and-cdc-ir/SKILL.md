---
name: replication-and-cdc-ir
description: 表达日志、位点、快照、增量、顺序、冲突、Exactly-once 和切换。
---

# Replication And Cdc Ir
...
表达日志、位点、快照、增量、顺序、冲突、Exactly-once 和切换。

本 Skill 属于精密语言转换系统。它必须以**可执行证据**为准，不得用模型自评、源码相似度或“能编译”替代语义与行为验证。

## Use when

- 当前任务直接涉及 `replication-and-cdc-ir` 所描述的能力。
...
1. 盘点对象与专有能力
2. 执行本 Skill 的核心职责：表达日志、位点、快照、增量、顺序、冲突、Exactly-once 和切换。
