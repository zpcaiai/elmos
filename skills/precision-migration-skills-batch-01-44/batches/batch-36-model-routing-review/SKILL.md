---
name: batch-36-model-routing-review
description: 按方向、风险、成本、私有化和历史表现动态选择模型，并用不同模型家族独立审查以降低共因错误。
---
...
- 当任务涉及 **model-capability-registry** 时，调用 `skills/model-capability-registry/SKILL.md`。
- 当任务涉及 **task-risk-scoring** 时，调用 `skills/task-risk-scoring/SKILL.md`。
- 当任务涉及 **language-pair-model-ranking** 时，调用 `skills/language-pair-model-ranking/SKILL.md`。
- 当任务涉及 **cost-aware-model-routing** 时，调用 `skills/cost-aware-model-routing/SKILL.md`。
- 当任务涉及 **private-model-routing** 时，调用 `skills/private-model-routing/SKILL.md`。
- 当任务涉及 **byok-provider-adapter** 时，调用 `skills/byok-provider-adapter/SKILL.md`。
- 当任务涉及 **heterogeneous-model-review** 时，调用 `skills/heterogeneous-model-review/SKILL.md`。
- 当任务涉及 **stronger-model-escalation** 时，调用 `skills/stronger-model-escalation/SKILL.md`。
- 当任务涉及 **model-output-calibration** 时，调用 `skills/model-output-calibration/SKILL.md`。
- 当任务涉及 **cost-per-accepted-module** 时，调用 `skills/cost-per-accepted-module/SKILL.md`。
