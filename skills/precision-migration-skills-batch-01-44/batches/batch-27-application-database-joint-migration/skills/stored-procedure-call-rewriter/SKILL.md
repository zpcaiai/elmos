---
name: stored-procedure-call-rewriter
description: 迁移过程调用、IN/OUT 参数、结果集、返回码、事务和异常。
---

# Stored Procedure Call Rewriter
...
迁移过程调用、IN/OUT 参数、结果集、返回码、事务和异常。

本 Skill 属于精密语言转换系统。它必须以**可执行证据**为准，不得用模型自评、源码相似度或“能编译”替代语义与行为验证。

## Use when

- 当前任务直接涉及 `stored-procedure-call-rewriter` 所描述的能力。
...
1. 盘点对象与专有能力
2. 执行本 Skill 的核心职责：迁移过程调用、IN/OUT 参数、结果集、返回码、事务和异常。
...
```yaml
skill: stored-procedure-call-rewriter
version: 1.0.0
