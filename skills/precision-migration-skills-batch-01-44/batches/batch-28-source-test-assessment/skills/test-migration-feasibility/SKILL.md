---
name: test-migration-feasibility
description: 判断哪些测试可直接迁移、需重写、需双运行或不可复用。
---
...
判断哪些测试可直接迁移、需重写、需双运行或不可复用。

本 Skill 属于精密语言转换系统。它必须以**可执行证据**为准，不得用模型自评、源码相似度或“能编译”替代语义与行为验证。

## Use when

- 当前任务直接涉及 `test-migration-feasibility` 所描述的能力。
...
1. 评估或生成测试
2. 执行本 Skill 的核心职责：判断哪些测试可直接迁移、需重写、需双运行或不可复用。
3. 同步环境并双运行
4. 比较输出、状态和副作用
5. 搜索最小反例
6. 驱动修复并执行全量回归
...
```yaml
skill: test-migration-feasibility
version: 1.0.0
