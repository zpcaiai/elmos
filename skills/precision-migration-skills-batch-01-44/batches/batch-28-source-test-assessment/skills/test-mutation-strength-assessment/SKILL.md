---
name: test-mutation-strength-assessment
description: 通过通用和方向专用变异衡量测试能否发现真实转换错误。
---

# Test Mutation Strength Assessment
...
通过通用和方向专用变异衡量测试能否发现真实转换错误。

本 Skill 属于精密语言转换系统。它必须以**可执行证据**为准，不得用模型自评、源码相似度或“能编译”替代语义与行为验证。

## Use when

- 当前任务直接涉及 `test-mutation-strength-assessment` 所描述的能力。
...
1. 评估或生成测试
2. 执行本 Skill 的核心职责：通过通用和方向专用变异衡量测试能否发现真实转换错误。
3. 同步环境并双运行
4. 比较输出、状态和副作用
5. 搜索最小反例
6. 驱动修复并执行全量回归
...
```yaml
skill: test-mutation-strength-assessment
version: 1.0.0
