...
description: 检测无断言、弱断言、仅状态码、过度快照和未验证副作用等问题。
---
...
检测无断言、弱断言、仅状态码、过度快照和未验证副作用等问题。

本 Skill 属于精密语言转换系统。它必须以**可执行证据**为准，不得用模型自评、源码相似度或“能编译”替代语义与行为验证。

## Use when

- 当前任务直接涉及 `assertion-quality-analyzer` 所描述的能力。
...
1. 评估或生成测试
2. 执行本 Skill 的核心职责：检测无断言、弱断言、仅状态码、过度快照和未验证副作用等问题。
3. 同步环境并双运行
4. 比较输出、状态和副作用
5. 搜索最小反例
6. 驱动修复并执行全量回归
...
```yaml
skill: assertion-quality-analyzer
version: 1.0.0
