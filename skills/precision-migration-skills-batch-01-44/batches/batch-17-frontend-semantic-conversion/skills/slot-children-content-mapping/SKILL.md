---
name: slot-children-content-mapping
description: 映射 Slot、Scoped Slot、Children、Render Props、Builder 和内容投影。
---

# Slot Children Content Mapping
...
映射 Slot、Scoped Slot、Children、Render Props、Builder 和内容投影。

本 Skill 属于精密语言转换系统。它必须以**可执行证据**为准，不得用模型自评、源码相似度或“能编译”替代语义与行为验证。

## Use when

- 当前任务直接涉及 `slot-children-content-mapping` 所描述的能力。
...
1. 恢复组件契约和UI状态机
2. 执行本 Skill 的核心职责：映射 Slot、Scoped Slot、Children、Render Props、Builder 和内容投影。
3. 映射事件、路由、布局和平台能力
4. 生成目标项目
...
```yaml
skill: slot-children-content-mapping
version: 1.0.0
