---
name: batch-17-frontend-semantic-conversion
description: 恢复前端组件、响应式依赖、UI状态机、事件、布局、路由和平台能力，作为跨框架和跨平台转换的共同语义层。
---
...
- 当任务涉及 **component-contract-recovery** 时，调用 `skills/component-contract-recovery/SKILL.md`。
- 当任务涉及 **reactive-dependency-analysis** 时，调用 `skills/reactive-dependency-analysis/SKILL.md`。
- 当任务涉及 **ui-state-machine-recovery** 时，调用 `skills/ui-state-machine-recovery/SKILL.md`。
- 当任务涉及 **event-effect-trace-recovery** 时，调用 `skills/event-effect-trace-recovery/SKILL.md`。
- 当任务涉及 **lifecycle-semantic-mapping** 时，调用 `skills/lifecycle-semantic-mapping/SKILL.md`。
- 当任务涉及 **slot-children-content-mapping** 时，调用 `skills/slot-children-content-mapping/SKILL.md`。
- 当任务涉及 **form-validation-mapping** 时，调用 `skills/form-validation-mapping/SKILL.md`。
- 当任务涉及 **route-navigation-mapping** 时，调用 `skills/route-navigation-mapping/SKILL.md`。
- 当任务涉及 **network-request-semantic-mapping** 时，调用 `skills/network-request-semantic-mapping/SKILL.md`。
- 当任务涉及 **local-storage-mapping** 时，调用 `skills/local-storage-mapping/SKILL.md`。
- 当任务涉及 **layout-constraint-mapping** 时，调用 `skills/layout-constraint-mapping/SKILL.md`。
- 当任务涉及 **accessibility-semantic-tree** 时，调用 `skills/accessibility-semantic-tree/SKILL.md`。
