---
name: transaction-boundary-converter
description: 映射隐式/显式提交、自治事务、保存点、传播和失败可见性。
---

# Transaction Boundary Converter
...
映射隐式/显式提交、自治事务、保存点、传播和失败可见性。

本 Skill 属于精密语言转换系统。它必须以**可执行证据**为准，不得用模型自评、源码相似度或“能编译”替代语义与行为验证。

## Use when

- 当前任务直接涉及 `transaction-boundary-converter` 所描述的能力。
...
1. 盘点对象与专有能力
2. 执行本 Skill 的核心职责：映射隐式/显式提交、自治事务、保存点、传播和失败可见性。
...
```yaml
skill: transaction-boundary-converter
version: 1.0.0
