---
name: procedure-performance-validator
description: 比较执行计划、IO、CPU、临时空间、锁、吞吐和延迟。
---

# Procedure Performance Validator
...
比较执行计划、IO、CPU、临时空间、锁、吞吐和延迟。

本 Skill 属于精密语言转换系统。它必须以**可执行证据**为准，不得用模型自评、源码相似度或“能编译”替代语义与行为验证。

## Use when

- 当前任务直接涉及 `procedure-performance-validator` 所描述的能力。
...
1. 盘点对象与专有能力
2. 执行本 Skill 的核心职责：比较执行计划、IO、CPU、临时空间、锁、吞吐和延迟。
...
```yaml
skill: procedure-performance-validator
version: 1.0.0
