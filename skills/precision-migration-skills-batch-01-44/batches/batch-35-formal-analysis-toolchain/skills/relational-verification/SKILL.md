---
name: relational-verification
description: 验证两个实现之间的输入、状态、输出和 Effect 关系。
---
...
验证两个实现之间的输入、状态、输出和 Effect 关系。

本 Skill 属于精密语言转换系统。它必须以**可执行证据**为准，不得用模型自评、源码相似度或“能编译”替代语义与行为验证。

## Use when

- 当前任务直接涉及 `relational-verification` 所描述的能力。
...
1. 固定语义与定理陈述
2. 执行本 Skill 的核心职责：验证两个实现之间的输入、状态、输出和 Effect 关系。
3. 生成或编码证明义务
4. 调用Lean/SMT/模型检查
5. 最小化反例并回写测试
6. 由可信内核/求解器签发结果
