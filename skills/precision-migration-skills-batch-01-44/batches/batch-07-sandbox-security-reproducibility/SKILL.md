---
name: batch-07-sandbox-security-reproducibility
description: 将客户代码、AI生成代码、构建脚本和测试视为不可信工作负载，确保隔离、最小权限、审计和可重现。
---
...
- 当任务涉及 **untrusted-code-sandbox** 时，调用 `skills/untrusted-code-sandbox/SKILL.md`。
- 当任务涉及 **network-egress-policy** 时，调用 `skills/network-egress-policy/SKILL.md`。
- 当任务涉及 **resource-quota-governor** 时，调用 `skills/resource-quota-governor/SKILL.md`。
- 当任务涉及 **secret-scope-manager** 时，调用 `skills/secret-scope-manager/SKILL.md`。
- 当任务涉及 **ephemeral-workspace-manager** 时，调用 `skills/ephemeral-workspace-manager/SKILL.md`。
- 当任务涉及 **dependency-proxy-and-cache** 时，调用 `skills/dependency-proxy-and-cache/SKILL.md`。
- 当任务涉及 **artifact-signing-and-provenance** 时，调用 `skills/artifact-signing-and-provenance/SKILL.md`。
- 当任务涉及 **reproducible-build-validator** 时，调用 `skills/reproducible-build-validator/SKILL.md`。
- 当任务涉及 **malicious-build-script-detector** 时，调用 `skills/malicious-build-script-detector/SKILL.md`。
- 当任务涉及 **execution-audit-recorder** 时，调用 `skills/execution-audit-recorder/SKILL.md`。
