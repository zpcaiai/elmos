---
name: dual-write-validation
description: 验证源目标双写一致性、冲突、顺序、补偿和回收策略。
---
...
验证源目标双写一致性、冲突、顺序、补偿和回收策略。

本 Skill 属于精密语言转换系统。它必须以**可执行证据**为准，不得用模型自评、源码相似度或“能编译”替代语义与行为验证。

## Use when

- 当前任务直接涉及 `dual-write-validation` 所描述的能力。
...
1. 汇总证据与未解决项
2. 执行本 Skill 的核心职责：验证源目标双写一致性、冲突、顺序、补偿和回收策略。
3. 执行硬性发布门禁
4. 影子/Canary/渐进切换
5. 监控并自动回滚
6. 沉淀反例、规则和企业交付能力
