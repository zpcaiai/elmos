---
name: canary-traffic-planner
description: 选择低风险租户、用户、接口和流量比例设计 Canary 阶段。
---
...
选择低风险租户、用户、接口和流量比例设计 Canary 阶段。

本 Skill 属于精密语言转换系统。它必须以**可执行证据**为准，不得用模型自评、源码相似度或“能编译”替代语义与行为验证。

## Use when

- 当前任务直接涉及 `canary-traffic-planner` 所描述的能力。
...
1. 汇总证据与未解决项
2. 执行本 Skill 的核心职责：选择低风险租户、用户、接口和流量比例设计 Canary 阶段。
3. 执行硬性发布门禁
4. 影子/Canary/渐进切换
5. 监控并自动回滚
6. 沉淀反例、规则和企业交付能力
