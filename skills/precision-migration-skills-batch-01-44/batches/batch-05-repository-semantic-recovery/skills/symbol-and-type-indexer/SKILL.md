---
name: symbol-and-type-indexer
description: 建立跨文件符号、类型、引用、泛型实例化和源码位置索引。
---

# Symbol And Type Indexer
...
建立跨文件符号、类型、引用、泛型实例化和源码位置索引。

本 Skill 属于精密语言转换系统。它必须以**可执行证据**为准，不得用模型自评、源码相似度或“能编译”替代语义与行为验证。

## Use when

- 当前任务直接涉及 `symbol-and-type-indexer` 所描述的能力。
...
1. 发现仓库与环境
2. 执行本 Skill 的核心职责：建立跨文件符号、类型、引用、泛型实例化和源码位置索引。
3. 使用原生工具提取语义
4. 建立可复现工具链和沙箱
5. 执行最小验证任务
6. 持久化摘要、哈希和证据
