---
name: batch-05-repository-semantic-recovery
description: 将源仓库从文件集合恢复为可查询的 Repository Semantic Graph，并产出可供转换、测试和证明复用的语义资产。
---
...
- 当任务涉及 **repository-inventory-scanner** 时，调用 `skills/repository-inventory-scanner/SKILL.md`。
- 当任务涉及 **build-system-detector** 时，调用 `skills/build-system-detector/SKILL.md`。
- 当任务涉及 **dependency-and-plugin-discovery** 时，调用 `skills/dependency-and-plugin-discovery/SKILL.md`。
- 当任务涉及 **symbol-and-type-indexer** 时，调用 `skills/symbol-and-type-indexer/SKILL.md`。
- 当任务涉及 **call-graph-builder** 时，调用 `skills/call-graph-builder/SKILL.md`。
- 当任务涉及 **data-flow-graph-builder** 时，调用 `skills/data-flow-graph-builder/SKILL.md`。
- 当任务涉及 **effect-and-side-effect-discovery** 时，调用 `skills/effect-and-side-effect-discovery/SKILL.md`。
- 当任务涉及 **database-access-discovery** 时，调用 `skills/database-access-discovery/SKILL.md`。
- 当任务涉及 **external-service-discovery** 时，调用 `skills/external-service-discovery/SKILL.md`。
- 当任务涉及 **runtime-trace-fusion** 时，调用 `skills/runtime-trace-fusion/SKILL.md`。
- 当任务涉及 **semantic-slice-extractor** 时，调用 `skills/semantic-slice-extractor/SKILL.md`。
- 当任务涉及 **business-rule-extractor** 时，调用 `skills/business-rule-extractor/SKILL.md`。
