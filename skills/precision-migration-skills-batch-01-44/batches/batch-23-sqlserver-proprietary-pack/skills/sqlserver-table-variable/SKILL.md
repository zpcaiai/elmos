---
name: sqlserver-table-variable
description: 迁移 Table Variable、统计、作用域和性能行为。
---
...
迁移 Table Variable、统计、作用域和性能行为。

本 Skill 属于精密语言转换系统。它必须以**可执行证据**为准，不得用模型自评、源码相似度或“能编译”替代语义与行为验证。

## Use when

- 当前任务直接涉及 `sqlserver-table-variable` 所描述的能力。
...
1. 盘点对象与专有能力
2. 执行本 Skill 的核心职责：迁移 Table Variable、统计、作用域和性能行为。
...
```yaml
skill: sqlserver-table-variable
version: 1.0.0
