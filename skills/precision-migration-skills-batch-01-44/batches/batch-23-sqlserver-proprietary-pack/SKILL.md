---
name: batch-23-sqlserver-proprietary-pack
description: 识别 T-SQL 和 SQL Server 平台能力，形成目标数据库或外部平台的安全迁移策略。
---
...
- 当任务涉及 **sqlserver-tsql** 时，调用 `skills/sqlserver-tsql/SKILL.md`。
- 当任务涉及 **sqlserver-identity-and-sequence** 时，调用 `skills/sqlserver-identity-and-sequence/SKILL.md`。
- 当任务涉及 **sqlserver-merge** 时，调用 `skills/sqlserver-merge/SKILL.md`。
- 当任务涉及 **sqlserver-try-catch** 时，调用 `skills/sqlserver-try-catch/SKILL.md`。
- 当任务涉及 **sqlserver-table-variable** 时，调用 `skills/sqlserver-table-variable/SKILL.md`。
- 当任务涉及 **sqlserver-temporary-table** 时，调用 `skills/sqlserver-temporary-table/SKILL.md`。
- 当任务涉及 **sqlserver-agent** 时，调用 `skills/sqlserver-agent/SKILL.md`。
- 当任务涉及 **sqlserver-service-broker** 时，调用 `skills/sqlserver-service-broker/SKILL.md`。
- 当任务涉及 **sqlserver-clr-stored-procedure** 时，调用 `skills/sqlserver-clr-stored-procedure/SKILL.md`。
- 当任务涉及 **sqlserver-linked-server** 时，调用 `skills/sqlserver-linked-server/SKILL.md`。
- 当任务涉及 **sqlserver-indexed-view** 时，调用 `skills/sqlserver-indexed-view/SKILL.md`。
- 当任务涉及 **sqlserver-temporal-table** 时，调用 `skills/sqlserver-temporal-table/SKILL.md`。
- 当任务涉及 **sqlserver-cdc-and-change-tracking** 时，调用 `skills/sqlserver-cdc-and-change-tracking/SKILL.md`。
- 当任务涉及 **sqlserver-row-level-security** 时，调用 `skills/sqlserver-row-level-security/SKILL.md`。
- 当任务涉及 **sqlserver-always-encrypted** 时，调用 `skills/sqlserver-always-encrypted/SKILL.md`。
- 当任务涉及 **sqlserver-ssis-dependency** 时，调用 `skills/sqlserver-ssis-dependency/SKILL.md`。
- 当任务涉及 **sqlserver-query-hint-and-plan** 时，调用 `skills/sqlserver-query-hint-and-plan/SKILL.md`。
