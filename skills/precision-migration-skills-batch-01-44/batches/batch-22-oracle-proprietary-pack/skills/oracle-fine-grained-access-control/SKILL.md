---
name: oracle-fine-grained-access-control
description: 迁移 VPD/FGAC 策略函数、上下文和行列级安全。
---
...
迁移 VPD/FGAC 策略函数、上下文和行列级安全。

本 Skill 属于精密语言转换系统。它必须以**可执行证据**为准，不得用模型自评、源码相似度或“能编译”替代语义与行为验证。

## Use when

- 当前任务直接涉及 `oracle-fine-grained-access-control` 所描述的能力。
...
1. 盘点对象与专有能力
2. 执行本 Skill 的核心职责：迁移 VPD/FGAC 策略函数、上下文和行列级安全。
...
```yaml
skill: oracle-fine-grained-access-control
version: 1.0.0
