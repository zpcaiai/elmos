---
name: oracle-plsql-package-and-state
description: 迁移 PL/SQL Package、公开/私有成员、初始化和会话级 Package State。
---

# Oracle Plsql Package And State
...
迁移 PL/SQL Package、公开/私有成员、初始化和会话级 Package State。

本 Skill 属于精密语言转换系统。它必须以**可执行证据**为准，不得用模型自评、源码相似度或“能编译”替代语义与行为验证。

## Use when

- 当前任务直接涉及 `oracle-plsql-package-and-state` 所描述的能力。
...
1. 盘点对象与专有能力
2. 执行本 Skill 的核心职责：迁移 PL/SQL Package、公开/私有成员、初始化和会话级 Package State。
...
```yaml
skill: oracle-plsql-package-and-state
version: 1.0.0
