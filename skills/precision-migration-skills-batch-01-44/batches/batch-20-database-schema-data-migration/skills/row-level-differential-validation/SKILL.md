---
name: row-level-differential-validation
description: 对关键表执行行级规范化 Diff，并分类差异。
---

# Row Level Differential Validation
...
对关键表执行行级规范化 Diff，并分类差异。

本 Skill 属于精密语言转换系统。它必须以**可执行证据**为准，不得用模型自评、源码相似度或“能编译”替代语义与行为验证。

## Use when

- 当前任务直接涉及 `row-level-differential-validation` 所描述的能力。
...
1. 盘点对象与专有能力
2. 执行本 Skill 的核心职责：对关键表执行行级规范化 Diff，并分类差异。
...
```yaml
skill: row-level-differential-validation
version: 1.0.0
