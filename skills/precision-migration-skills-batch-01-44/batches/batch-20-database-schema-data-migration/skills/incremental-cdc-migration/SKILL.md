---
name: incremental-cdc-migration
description: 规划和执行日志/触发器/时间戳增量迁移及位点恢复。
---
...
规划和执行日志/触发器/时间戳增量迁移及位点恢复。

本 Skill 属于精密语言转换系统。它必须以**可执行证据**为准，不得用模型自评、源码相似度或“能编译”替代语义与行为验证。

## Use when

- 当前任务直接涉及 `incremental-cdc-migration` 所描述的能力。
...
1. 盘点对象与专有能力
2. 执行本 Skill 的核心职责：规划和执行日志/触发器/时间戳增量迁移及位点恢复。
...
```yaml
skill: incremental-cdc-migration
version: 1.0.0
