---
name: cutover-and-rollback-plan
description: 设计停机、低停机、双写、切流、回滚和数据收敛方案。
---
...
设计停机、低停机、双写、切流、回滚和数据收敛方案。

本 Skill 属于精密语言转换系统。它必须以**可执行证据**为准，不得用模型自评、源码相似度或“能编译”替代语义与行为验证。

## Use when

- 当前任务直接涉及 `cutover-and-rollback-plan` 所描述的能力。
...
1. 盘点对象与专有能力
2. 执行本 Skill 的核心职责：设计停机、低停机、双写、切流、回滚和数据收敛方案。
...
```yaml
skill: cutover-and-rollback-plan
version: 1.0.0
