---
name: null-to-option-pointer-lowering
description: 在 Null、Option、Nullable、Pointer、zero value 和 Missing 之间安全映射。
---

# Null To Option Pointer Lowering
...
在 Null、Option、Nullable、Pointer、zero value 和 Missing 之间安全映射。

本 Skill 属于精密语言转换系统。它必须以**可执行证据**为准，不得用模型自评、源码相似度或“能编译”替代语义与行为验证。

## Use when

- 当前任务直接涉及 `null-to-option-pointer-lowering` 所描述的能力。
...
1. 使用语言原生前端恢复语义
2. 执行本 Skill 的核心职责：在 Null、Option、Nullable、Pointer、zero value 和 Missing 之间安全映射。
...
```yaml
skill: null-to-option-pointer-lowering
version: 1.0.0
