---
name: technical-debt-drift-detection
description: 检测新技术债、已修复问题回归和跨仓库治理漂移。
---

# Technical Debt Drift Detection
...
检测新技术债、已修复问题回归和跨仓库治理漂移。

本 Skill 属于精密语言转换系统。它必须以**可执行证据**为准，不得用模型自评、源码相似度或“能编译”替代语义与行为验证。

## Use when

- 当前任务直接涉及 `technical-debt-drift-detection` 所描述的能力。
...
1. 汇总证据与未解决项
2. 执行本 Skill 的核心职责：检测新技术债、已修复问题回归和跨仓库治理漂移。
3. 执行硬性发布门禁
4. 影子/Canary/渐进切换
5. 监控并自动回滚
6. 沉淀反例、规则和企业交付能力
...
```yaml
skill: technical-debt-drift-detection
version: 1.0.0
