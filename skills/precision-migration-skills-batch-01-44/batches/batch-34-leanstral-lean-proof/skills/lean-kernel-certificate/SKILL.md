---
name: lean-kernel-certificate
description: 记录 Lean 版本、依赖摘要、定理、Axiom 报告和内核验收结果。
---
...
记录 Lean 版本、依赖摘要、定理、Axiom 报告和内核验收结果。

本 Skill 属于精密语言转换系统。它必须以**可执行证据**为准，不得用模型自评、源码相似度或“能编译”替代语义与行为验证。

## Use when

- 当前任务直接涉及 `lean-kernel-certificate` 所描述的能力。
...
1. 固定语义与定理陈述
2. 执行本 Skill 的核心职责：记录 Lean 版本、依赖摘要、定理、Axiom 报告和内核验收结果。
3. 生成或编码证明义务
4. 调用Lean/SMT/模型检查
5. 最小化反例并回写测试
6. 由可信内核/求解器签发结果
