---
name: proof-regression-ci
description: 在 IR、规则、生成器和依赖变化时持续重跑证明回归。
---
...
1. 固定语义与定理陈述
2. 执行本 Skill 的核心职责：在 IR、规则、生成器和依赖变化时持续重跑证明回归。
3. 生成或编码证明义务
4. 调用Lean/SMT/模型检查
5. 最小化反例并回写测试
6. 由可信内核/求解器签发结果
