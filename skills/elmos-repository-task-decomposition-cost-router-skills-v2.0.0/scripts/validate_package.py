...
if len(skill_files) != 54:
    errors.append(f"expected 54 skills, found {len(skill_files)}")
skill_names=set()
...
# Validate adaptive decomposition policy and required schemas.
adaptive=yaml.safe_load((ROOT/'config/adaptive-decomposition-policy.yaml').read_text())
if not adaptive.get('planner',{}).get('coarse_to_fine'):
    errors.append('adaptive planner must use coarse_to_fine')
if adaptive.get('planner',{}).get('max_depth',0) < 4:
    errors.append('adaptive planner hierarchy too shallow')
for schema_name in ['hierarchical-plan.schema.json','scenario-graph.schema.json','repository-intelligence-graph.schema.json','invariant-ledger.schema.json','proof-obligation.schema.json','plan-revision.schema.json']:
    if not (ROOT/'schemas'/schema_name).exists():
        errors.append(f'missing adaptive schema: {schema_name}')
