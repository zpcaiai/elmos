---
name: elmos-certificate-deployment-admission-controller
description: Admit deployment only when artifact, environment, policy, evidence freshness and independent certificate exactly match the requested production envelope.
version: 4.0.0
...
# Certificate-Bound Deployment Admission Controller
...
- `CertificateBoundDeploymentAdmissionControllerProfile`
- `CertificateBoundDeploymentAdmissionControllerPlan`
- `CertificateBoundDeploymentAdmissionControllerRun`
- `CertificateBoundDeploymentAdmissionControllerDecision`

Required semantic capabilities:

- verify certificate signature, expiry and revocation
- bind artifact digest, policy bundle and environment profile
- check E0-E5 and P05 scope for the requested deployment
- deny stale or partial evidence
- emit immutable admission and denial receipts
...
- `artifacts/elmos-certificate-deployment-admission-controller/profile.yaml`
- `artifacts/elmos-certificate-deployment-admission-controller/plan.json`
- `artifacts/elmos-certificate-deployment-admission-controller/result.json`
- `artifacts/elmos-certificate-deployment-admission-controller/evidence/`
...
- **certificate replayed for different artifact** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **environment differs from certified envelope** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **revocation cache stale** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **waiver expired** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **admission webhook fail-open** — must have a preventive control, a detective signal, a negative fixture and a response action.
...
- native scenario: verify certificate signature, expiry and revocation
- native scenario: bind artifact digest, policy bundle and environment profile
- native scenario: check E0-E5 and P05 scope for the requested deployment
- native scenario: deny stale or partial evidence
- native scenario: emit immutable admission and denial receipts
