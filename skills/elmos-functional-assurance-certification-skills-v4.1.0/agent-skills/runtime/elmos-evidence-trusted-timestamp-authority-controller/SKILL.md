---
name: elmos-evidence-trusted-timestamp-authority-controller
description: Implement and independently certify evidence trusted timestamp authority controller, including bind evidence digest to trustworthy time using signed timestamp tokens and redundant time sources, verify TSA policy, chain, nonce, accuracy and revocation and preserve long-term validation data for expired algorithms and certificates.
version: 4.0.0
...
# Evidence Trusted Timestamp Authority Controller

## Objective

Implement and independently certify evidence trusted timestamp authority controller, including bind evidence digest to trustworthy time using signed timestamp tokens and redundant time sources, verify TSA policy, chain, nonce, accuracy and revocation and preserve long-term validation data for expired algorithms and certificates.
...
- `EvidenceTrustedTimestampAuthorityControllerProfile`
- `EvidenceTrustedTimestampAuthorityControllerRun`
- `EvidenceTrustedTimestampAuthorityControllerFinding`
- `EvidenceTrustedTimestampAuthorityControllerDecision`
- `EvidenceTrustedTimestampAuthorityControllerEvidence`

Required semantic capabilities:

- bind evidence digest to trustworthy time using signed timestamp tokens and redundant time sources
- verify TSA policy, chain, nonce, accuracy and revocation
- preserve long-term validation data for expired algorithms and certificates
...
- `artifacts/elmos-evidence-trusted-timestamp-authority-controller/profile.yaml`
- `artifacts/elmos-evidence-trusted-timestamp-authority-controller/plan.json`
- `artifacts/elmos-evidence-trusted-timestamp-authority-controller/result.json`
- `artifacts/elmos-evidence-trusted-timestamp-authority-controller/evidence/`
- `artifacts/elmos-evidence-trusted-timestamp-authority-controller/assurance-case.json`
...
- **local clock is treated as trusted time** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **timestamp is issued after evidence mutation** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **revoked TSA chain remains accepted** — must have a preventive control, a detective signal, a negative fixture and a response action.
...
- native scenario: bind evidence digest to trustworthy time using signed timestamp tokens and redundant time sources
- native scenario: verify TSA policy, chain, nonce, accuracy and revocation
- native scenario: preserve long-term validation data for expired algorithms and certificates
