---
name: elmos-independent-k8-certificate-signer
description: Sign, publish or refuse completion certificates from sealed assurance cases under current certificate policy, authorized signer identity and exact evidence root.
version: 4.0.0
...
# Independent K8 Certificate Signer
...
- `CertificateDecision`
- `SignedCertificate`
- `BlockedCertification`

Required semantic capabilities:

- Verify scope, assurance case and evidence root
- Enforce policy, competence and waiver state
- Use protected short-lived or HSM-backed signer
- Publish certificate plus limitations and revocation endpoint
- Produce typed blocked result on any critical gap
...
- `certificate-signature.bundle`
- `certificate-publication-record.json`
- `blocked-certification-report.json`
...
- **signer key exfiltration** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **sign request points to different revision** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **certificate status inflated** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **signature valid but policy outdated** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **publication log omits certificate** — must have a preventive control, a detective signal, a negative fixture and a response action.
...
- valid certificate signing
- tampered evidence root rejected
- expired waiver blocks
- unauthorized signer rejected
- blocked result generation
- certificate verification and revocation lookup
