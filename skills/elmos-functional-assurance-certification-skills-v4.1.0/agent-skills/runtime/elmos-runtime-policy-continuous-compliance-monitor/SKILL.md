---
name: elmos-runtime-policy-continuous-compliance-monitor
description: Evaluate deployed configuration, identity, network, data, model and evidence state continuously against versioned policies and certification scope.
version: 4.0.0
...
# Runtime Policy Continuous Compliance Monitor
...
- `RuntimePolicyContinuousComplianceMonitorProfile`
- `RuntimePolicyContinuousComplianceMonitorPlan`
- `RuntimePolicyContinuousComplianceMonitorRun`
- `RuntimePolicyContinuousComplianceMonitorDecision`

Required semantic capabilities:

- collect attested runtime state
- evaluate drift and control effectiveness
- distinguish transient from persistent violation
- trigger containment, evidence invalidation and recertification
- report scope and policy version
...
- `elmos-ai-policy-as-code-compiler` from `elmos-ai-capability-enhancement-skills` — required base evidence producer.

## Primary Outputs

- `artifacts/elmos-runtime-policy-continuous-compliance-monitor/profile.yaml`
- `artifacts/elmos-runtime-policy-continuous-compliance-monitor/plan.json`
- `artifacts/elmos-runtime-policy-continuous-compliance-monitor/result.json`
- `artifacts/elmos-runtime-policy-continuous-compliance-monitor/evidence/`
...
- **monitor has read-only blind spot** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **policy bundle stale** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **alert suppression hides drift** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **runtime self-reports false state** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **continuous pass shown as statutory certification** — must have a preventive control, a detective signal, a negative fixture and a response action.
...
- native scenario: collect attested runtime state
- native scenario: evaluate drift and control effectiveness
- native scenario: distinguish transient from persistent violation
- native scenario: trigger containment, evidence invalidation and recertification
- native scenario: report scope and policy version
