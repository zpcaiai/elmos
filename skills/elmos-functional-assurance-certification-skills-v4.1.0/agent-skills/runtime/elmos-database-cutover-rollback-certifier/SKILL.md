---
name: elmos-database-cutover-rollback-certifier
description: Certify snapshot/CDC reconciliation, write freeze or dual-write, final cutover, rollback window and source retirement with no unexplained data loss.
version: 4.0.0
...
# Database Cutover and Rollback Certifier
...
- **Kernel authorities used:** K5, K6, K7, K8
...
- Freeze exact source/target positions
- Require zero unexplained reconciliation differences
- Exercise application cutover and rollback
- Set rollback data capture strategy
- Retire source only after expiry and acceptance
...
- `elmos-data-snapshot-cdc-controller` from `elmos-ai-capability-enhancement-skills` — required base evidence producer.
- `elmos-transaction-locking-equivalence-verifier` from `elmos-ai-capability-enhancement-skills` — required base evidence producer.

## Primary Outputs

- `database-cutover-certificate.json`
- `cutover-runbook.yaml`
- `rollback-drill-report.json`
- `source-retirement-gates.json`
...
- **cutover position ambiguous** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **dual-write divergence** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **rollback drops target-only writes** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **client connection cache bypasses cutover** — must have a preventive control, a detective signal, a negative fixture and a response action.
- **source retired before evidence expiry** — must have a preventive control, a detective signal, a negative fixture and a response action.
...
- read/write cutover
- rollback with new writes
- DNS/connection failover
