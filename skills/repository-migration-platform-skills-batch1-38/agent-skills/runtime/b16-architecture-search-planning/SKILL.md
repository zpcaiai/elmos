---
name: b16-architecture-search-planning
description: >-
  Target Architecture Search与Migration Planning。恢复Source真实架构和硬约束，搜索语言、Framework、数据、消息与部署组合并生成可执行迁移Blueprint。 Use when implementing, debugging, or evaluating Batch 16 of an evidence-governed repository migration.
---
...
Batch 1, Batch 3, Batch 4, Batch 5, Batch 6, Batch 7, Batch 8, Batch 9, Batch 10, Batch 11, Batch 12, Batch 13, Batch 14, Batch 15
...
2. Discover this Batch against an immutable Source fingerprint:
   ...
   ```bash
   python3 "$RMP_RUNTIME" prepare --batch 16 --source "$SOURCE_REPO" --workspace "$EVIDENCE_WORKSPACE" --target-objective "$TARGET_OBJECTIVE"
   ```

3. Read `batches/batch-16/profile.json`, `implementation-plan.json`, and `execution-plan.json`; implement each required output and populate exact argv-only execution steps.
...
6. Evaluate the fail-closed gate:
   ...
   ```bash
   python3 "$RMP_RUNTIME" gate --workspace "$EVIDENCE_WORKSPACE" --batch 16 --mode local
   ```
...
- **AP1–AP5** Gate已通过，或明确报告未达到的原因和Certificate Ceiling。

## Completion Report

```yaml
batch: 16
skill: b16-architecture-search-planning
