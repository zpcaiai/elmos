---
name: b09-concurrency-memory-native-semantics
description: >-
  Concurrency、Async、Memory、Lifetime与Native Semantics。保持线程、任务、Actor、Channel、Cancellation、Ownership、ABI、Buffer与Callback等运行时语义。 Use when implementing, debugging, or evaluating Batch 9 of an evidence-governed repository migration.
---
...
2. Discover this Batch against an immutable Source fingerprint:
   ...
   ```bash
   python3 "$RMP_RUNTIME" prepare --batch 9 --source "$SOURCE_REPO" --workspace "$EVIDENCE_WORKSPACE" --target-objective "$TARGET_OBJECTIVE"
   ```

3. Read `batches/batch-09/profile.json`, `implementation-plan.json`, and `execution-plan.json`; implement each required output and populate exact argv-only execution steps.
...
6. Evaluate the fail-closed gate:
   ...
   ```bash
   python3 "$RMP_RUNTIME" gate --workspace "$EVIDENCE_WORKSPACE" --batch 9 --mode local
   ```
...
- **CM1–CM5** Gate已通过，或明确报告未达到的原因和Certificate Ceiling。

## Completion Report

```yaml
batch: 9
skill: b09-concurrency-memory-native-semantics
