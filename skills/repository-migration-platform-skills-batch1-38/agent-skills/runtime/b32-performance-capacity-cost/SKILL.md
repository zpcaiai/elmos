---
name: b32-performance-capacity-cost
description: >-
  Performance、Capacity、Scalability与Cost Assurance。以生产Workload验证吞吐、尾延迟、资源、扩缩容、Backpressure和单位成本，形成容量安全边际。 Use when implementing, debugging, or evaluating Batch 32 of an evidence-governed repository migration.
---
...
- Batch: `32`
- Risk: `critical`
- Gate: `Performance & Capacity Gate`
...
Batch 9, Batch 10, Batch 12, Batch 16, Batch 18, Batch 29, Batch 30, Batch 31
...
- Gate / Certification：**Performance & Capacity Gate**；
...
2. Discover this Batch against an immutable Source fingerprint:
   ...
   ```bash
   python3 "$RMP_RUNTIME" prepare --batch 32 --source "$SOURCE_REPO" --workspace "$EVIDENCE_WORKSPACE" --target-objective "$TARGET_OBJECTIVE"
   ```

3. Read `batches/batch-32/profile.json`, `implementation-plan.json`, and `execution-plan.json`; implement each required output and populate exact argv-only execution steps.
...
6. Evaluate the fail-closed gate:
   ...
   ```bash
   python3 "$RMP_RUNTIME" gate --workspace "$EVIDENCE_WORKSPACE" --batch 32 --mode local
   ```
...
- **Performance & Capacity Gate** Gate已通过，或明确报告未达到的原因和Certificate Ceiling。

## Completion Report

```yaml
batch: 32
skill: b32-performance-capacity-cost
