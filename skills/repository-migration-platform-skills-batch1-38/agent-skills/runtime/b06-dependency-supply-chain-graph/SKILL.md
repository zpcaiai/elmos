---
name: b06-dependency-supply-chain-graph
description: >-
  Dependency、Native、License与Supply-Chain Graph。按实际Usage Contract迁移依赖，处理组合替代、Native资产、License、SBOM、签名与供应链风险。 Use when implementing, debugging, or evaluating Batch 6 of an evidence-governed repository migration.
---
...
- Gate / Certification：**DA/DR Certification**；
...
2. Discover this Batch against an immutable Source fingerprint:
   ...
   ```bash
   python3 "$RMP_RUNTIME" prepare --batch 6 --source "$SOURCE_REPO" --workspace "$EVIDENCE_WORKSPACE" --target-objective "$TARGET_OBJECTIVE"
   ```

3. Read `batches/batch-06/profile.json`, `implementation-plan.json`, and `execution-plan.json`; implement each required output and populate exact argv-only execution steps.
...
6. Evaluate the fail-closed gate:
   ...
   ```bash
   python3 "$RMP_RUNTIME" gate --workspace "$EVIDENCE_WORKSPACE" --batch 6 --mode local
   ```
...
- **DA/DR Certification** Gate已通过，或明确报告未达到的原因和Certificate Ceiling。

## Completion Report

```yaml
batch: 6
skill: b06-dependency-supply-chain-graph
