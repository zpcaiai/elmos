---
name: elmos-batch-03-slightly-strict-tests
description: "Run the slightly strict Batch 3 test suite for Unified intermediate representation and source mapping."
version: 1.0.0
test_skill_id: T003
...
# T003 — Unified intermediate representation and source mapping Slightly Strict Tests

## 1. Objective

Execute a deterministic, evidence-backed and moderately adversarial certification suite for **Unified intermediate representation and source mapping**. This suite is stricter than a smoke test but does not claim exhaustive formal verification.
...
- `b03-unified-intermediate-representation-source-mappi-capability-matrix-and-planning`
- `b03-unified-intermediate-representation-source-mappi-conformance-security-and-chaos-tests`
- `b03-unified-intermediate-representation-source-mappi-data-lineage-and-reconciliation`
- `b03-unified-intermediate-representation-source-mappi-discovery-and-baseline`
- `b03-unified-intermediate-representation-source-mappi-domain-model-and-contracts`
- `b03-unified-intermediate-representation-source-mappi-evidence-audit-and-provenance`
- `b03-unified-intermediate-representation-source-mappi-execution-engine`
- `b03-unified-intermediate-representation-source-mappi-failure-recovery-and-rollback`
- `b03-unified-intermediate-representation-source-mappi-governance-approval-and-waiver`
- `b03-unified-intermediate-representation-source-mappi-observability-and-operational-readiness`
- `b03-unified-intermediate-representation-source-mappi-orchestrator`
- `b03-unified-intermediate-representation-source-mappi-performance-capacity-and-cost`
- `b03-unified-intermediate-representation-source-mappi-provider-and-tool-adapters`
- `b03-unified-intermediate-representation-source-mappi-release-certification-gate`
- `b03-unified-intermediate-representation-source-mappi-security-privacy-and-tenant-isolation`
- `b03-unified-intermediate-representation-source-mappi-workflow-and-state-machine`
...
### TC-T003-001 — Representative approved workflow completes end to end

- **Category:** `happy_path`
- **Severity:** `HIGH`
- **Setup:** Use a valid representative fixture covering the primary responsibilities of the target Batch. Batch theme: Unified intermediate representation and source mapping.
...
- **Expected result:** All mandatory outputs validate, trace to inputs, and contain no hidden warnings or unsupported claims. Required Batch-specific contracts for Unified intermediate representation and source mapping remain satisfied.
...
### TC-T003-002 — Boundary and moderate scale remain correct

- **Category:** `boundary`
- **Severity:** `MEDIUM`
- **Setup:** Use minimum/maximum supported sizes, empty optional collections, and a moderately large realistic fixture. Batch theme: Unified intermediate representation and source mapping.
...
- **Expected result:** Behavior remains bounded; limits are explicit; no silent truncation, quadratic blow-up, or inconsistent result occurs. Required Batch-specific contracts for Unified intermediate representation and source mapping remain satisfied.
- **Deterministic oracle:** Use schema/parser validation, stable identifiers, stored baseline comparison, explicit state/permission checks, content hashes and recorded runner/tool exit status. LLM interpretation may explain a finding but cannot determine raw pass/fail facts.
- **Required evidence:**
  - input and fixture fingerprints
  - target Skill and tool versions
...
### TC-T003-003 — Malformed or contradictory input fails safely

- **Category:** `negative_input`
- **Severity:** `HIGH`
- **Setup:** Corrupt one required field, introduce one invalid reference, and add one contradictory constraint. Batch theme: Unified intermediate representation and source mapping.
...
- **Expected result:** The workflow rejects or blocks with a structured diagnostic and makes no authoritative partial success claim. Required Batch-specific contracts for Unified intermediate representation and source mapping remain satisfied.
- **Deterministic oracle:** Use schema/parser validation, stable identifiers, stored baseline comparison, explicit state/permission checks, content hashes and recorded runner/tool exit status. LLM interpretation may explain a finding but cannot determine raw pass/fail facts.
- **Required evidence:**
  - input and fixture fingerprints
  - target Skill and tool versions
...
### TC-T003-004 — Dependency failure is contained and recoverable

- **Category:** `dependency_failure`
- **Severity:** `HIGH`
- **Setup:** Make one required provider, registry, model, runner, database, broker, or external dependency unavailable. Batch theme: Unified intermediate representation and source mapping.
...
- **Expected result:** Failure classification is correct, partial writes are journaled, retry is bounded, and resume does not duplicate side effects. Required Batch-specific contracts for Unified intermediate representation and source mapping remain satisfied.
- **Deterministic oracle:** Use schema/parser validation, stable identifiers, stored baseline comparison, explicit state/permission checks, content hashes and recorded runner/tool exit status. LLM interpretation may explain a finding but cannot determine raw pass/fail facts.
- **Required evidence:**
  - input and fixture fingerprints
  - target Skill and tool versions
...
### TC-T003-005 — Security, tenant and permission boundaries cannot be bypassed

- **Category:** `security_isolation`
- **Severity:** `CRITICAL`
- **Setup:** Create two tenants/projects and an actor lacking one required permission; include a malicious embedded instruction. Batch theme: Unified intermediate representation and source mapping.
...
- **Expected result:** All attempts are denied; no sensitive data leaks; the malicious instruction is treated as untrusted content and audited. Required Batch-specific contracts for Unified intermediate representation and source mapping remain satisfied.
- **Deterministic oracle:** Use schema/parser validation, stable identifiers, stored baseline comparison, explicit state/permission checks, content hashes and recorded runner/tool exit status. LLM interpretation may explain a finding but cannot determine raw pass/fail facts.
- **Required evidence:**
  - input and fixture fingerprints
  - target Skill and tool versions
...
### TC-T003-006 — Replay and retry are idempotent

- **Category:** `replay_idempotency`
- **Severity:** `HIGH`
- **Setup:** Capture an accepted request, event, generation unit, migration step, or task with its idempotency identity. Batch theme: Unified intermediate representation and source mapping.
...
- **Expected result:** No duplicate semantic object or side effect is produced; the original result or safe reconciliation is returned. Required Batch-specific contracts for Unified intermediate representation and source mapping remain satisfied.
- **Deterministic oracle:** Use schema/parser validation, stable identifiers, stored baseline comparison, explicit state/permission checks, content hashes and recorded runner/tool exit status. LLM interpretation may explain a finding but cannot determine raw pass/fail facts.
- **Required evidence:**
  - input and fixture fingerprints
  - target Skill and tool versions
...
### TC-T003-007 — Schema, runtime and dependency drift is detected

- **Category:** `version_drift`
- **Severity:** `HIGH`
- **Setup:** Change one upstream schema or tool version compatibly and one incompatibly. Batch theme: Unified intermediate representation and source mapping.
...
- **Expected result:** Compatible evolution succeeds with evidence; incompatible evolution blocks with precise impact and migration guidance. Required Batch-specific contracts for Unified intermediate representation and source mapping remain satisfied.
- **Deterministic oracle:** Use schema/parser validation, stable identifiers, stored baseline comparison, explicit state/permission checks, content hashes and recorded runner/tool exit status. LLM interpretation may explain a finding but cannot determine raw pass/fail facts.
- **Required evidence:**
  - input and fixture fingerprints
  - target Skill and tool versions
...
### TC-T003-008 — Tampered or incomplete evidence cannot produce a pass

- **Category:** `evidence_tamper`
- **Severity:** `CRITICAL`
- **Setup:** Remove one required evidence record and alter one stored output fingerprint. Batch theme: Unified intermediate representation and source mapping.
...
- **Expected result:** Hash/signature or completeness checks fail and release remains blocked; no score averaging can hide the defect. Required Batch-specific contracts for Unified intermediate representation and source mapping remain satisfied.
- **Deterministic oracle:** Use schema/parser validation, stable identifiers, stored baseline comparison, explicit state/permission checks, content hashes and recorded runner/tool exit status. LLM interpretation may explain a finding but cannot determine raw pass/fail facts.
- **Required evidence:**
  - input and fixture fingerprints
  - target Skill and tool versions
