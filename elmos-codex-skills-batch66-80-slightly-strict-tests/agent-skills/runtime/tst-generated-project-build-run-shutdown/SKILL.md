---
name: tst-generated-project-build-run-shutdown
description: "生成项目Build Run Shutdown。验证生成项目能从空目录构建、启动、探测、失败、取消、优雅关闭和清理。 Use for ELMOS Batch 66–80 slightly strict certification tests."
metadata:
  id: X006
...
---
    # X006 — 生成项目Build Run Shutdown

    ## Objective

    验证生成项目能从空目录构建、启动、探测、失败、取消、优雅关闭和清理。 覆盖成功、边界、负向、故障、重放、恢复、篡改和状态冒充。默认不得把未运行、Skipped、Mock-only或陈旧Evidence计为通过。
    ...
    ### X006-01 — 生成项目Build Run Shutdown / 空目录Bootstrap到Runtime

- Priority: `P0`
- Category: `runtime`
- Given: 按文档从空目录生成、安装、构建并启动代表性项目
- When: 执行本Case定义的测试、故障注入和证据收集
- Then: 入口可访问、Readiness成功、Artifact可定位且无手工隐藏步骤
- Evidence: source-snapshot, environment-manifest, raw-command-log, result-json, artifact-manifest, replay-command

### X006-02 — 生成项目Build Run Shutdown / 失败取消优雅关闭

- Priority: `P1`
- Category: `lifecycle`
- Given: 注入依赖失败、Timeout和SIGTERM/取消
- When: 执行本Case定义的测试、故障注入和证据收集
- Then: 进程及时退出，连接与临时资源清理，退出码和诊断正确
- Evidence: source-snapshot, environment-manifest, raw-command-log, result-json, artifact-manifest, replay-command

### X006-03 — 生成项目Build Run Shutdown / 重复生成无破坏

- Priority: `P2`
- Category: `idempotency`
- Given: 对用户修改过的项目再次运行生成器
- When: 执行本Case定义的测试、故障注入和证据收集
- Then: 受保护内容不被覆盖，Diff稳定且冲突被显式报告
- Evidence: source-snapshot, environment-manifest, raw-command-log, result-json, artifact-manifest, replay-command
