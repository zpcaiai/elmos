---
name: tst-cicd-runner-permission-provenance
description: "CI/CD Runner权限与Provenance。验证GitHub/GitLab/Jenkins权限、OIDC、Fork/PR隔离、缓存投毒、环境审批和发布来源。 Use for ELMOS Batch 66–80 slightly strict certification tests."
metadata:
  id: X017
...
---
    # X017 — CI/CD Runner权限与Provenance

    ## Objective

    验证GitHub/GitLab/Jenkins权限、OIDC、Fork/PR隔离、缓存投毒、环境审批和发布来源。 覆盖成功、边界、负向、故障、重放、恢复、篡改和状态冒充。默认不得把未运行、Skipped、Mock-only或陈旧Evidence计为通过。
    ...
    ### X017-01 — CI/CD Runner权限与Provenance / Untrusted PR/Fork Secret隔离

- Priority: `P0`
- Category: `cicd`
- Given: 从Fork或低信任分支触发流水线
- When: 执行本Case定义的测试、故障注入和证据收集
- Then: 高权限Token、环境Secret和持久Runner不可用，日志无泄漏
- Evidence: source-snapshot, environment-manifest, raw-command-log, result-json, artifact-manifest, replay-command

### X017-02 — CI/CD Runner权限与Provenance / OIDC权限和环境审批

- Priority: `P0`
- Category: `cicd`
- Given: 伪造Audience/Subject或跳过生产审批
- When: 执行本Case定义的测试、故障注入和证据收集
- Then: 云身份交换或发布被拒绝，权限为最小范围
- Evidence: source-snapshot, environment-manifest, raw-command-log, result-json, artifact-manifest, replay-command

### X017-03 — CI/CD Runner权限与Provenance / 缓存投毒与跨CI晋级

- Priority: `P2`
- Category: `cicd`
- Given: 污染缓存并从一个CI系统向另一个系统晋级Artifact
- When: 执行本Case定义的测试、故障注入和证据收集
- Then: 缓存有信任边界，Artifact Digest/Provenance在系统间保持一致
- Evidence: source-snapshot, environment-manifest, raw-command-log, result-json, artifact-manifest, replay-command
