---
name: tst-deterministic-dependency-build-reproducibility
description: "依赖与构建可重复性。验证锁文件、离线缓存、生成顺序、构建Hash和两次Clean Build一致性。 Use for ELMOS Batch 66–80 slightly strict certification tests."
metadata:
  id: X005
...
---
    ...
    验证锁文件、离线缓存、生成顺序、构建Hash和两次Clean Build一致性。 覆盖成功、边界、负向、故障、重放、恢复、篡改和状态冒充。默认不得把未运行、Skipped、Mock-only或陈旧Evidence计为通过。
    ...
    ### X005-01 — 依赖与构建可重复性 / 两次Clean Build产物一致

- Priority: `P0`
- Category: `reproducibility`
- Given: 在独立空目录执行两次相同构建
- When: 执行本Case定义的测试、故障注入和证据收集
- Then: 依赖图、生成文件和可重现产物Hash一致或差异有批准解释
- Evidence: source-snapshot, environment-manifest, raw-command-log, result-json, artifact-manifest, replay-command

### X005-02 — 依赖与构建可重复性 / 锁文件与缓存投毒

- Priority: `P0`
- Category: `supply-chain`
- Given: 注入锁文件Hash错误或污染离线缓存
- When: 执行本Case定义的测试、故障注入和证据收集
- Then: 构建Fail Closed且不自动改写锁文件制造通过
- Evidence: source-snapshot, environment-manifest, raw-command-log, result-json, artifact-manifest, replay-command

### X005-03 — 依赖与构建可重复性 / 并行与增量构建一致

- Priority: `P2`
- Category: `build`
- Given: 比较串行、并行、Clean和Incremental构建
- When: 执行本Case定义的测试、故障注入和证据收集
- Then: 输出功能与依赖关系一致，无漏建或陈旧产物
- Evidence: source-snapshot, environment-manifest, raw-command-log, result-json, artifact-manifest, replay-command
