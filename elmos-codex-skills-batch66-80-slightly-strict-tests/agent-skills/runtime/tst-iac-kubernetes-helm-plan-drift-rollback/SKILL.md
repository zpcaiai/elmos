---
name: tst-iac-kubernetes-helm-plan-drift-rollback
description: "IaC Kubernetes Helm Plan Drift Rollback。验证Plan、Policy、Cost、Server Dry-run、Admission、Drift、Upgrade与Rollback。 Use for ELMOS Batch 66–80 slightly strict certification tests."
metadata:
  id: X016
...
---
    # X016 — IaC Kubernetes Helm Plan Drift Rollback

    ## Objective

    验证Plan、Policy、Cost、Server Dry-run、Admission、Drift、Upgrade与Rollback。 覆盖成功、边界、负向、故障、重放、恢复、篡改和状态冒充。默认不得把未运行、Skipped、Mock-only或陈旧Evidence计为通过。
    ...
    ### X016-01 — IaC Kubernetes Helm Plan Drift Rollback / Plan Policy Cost门禁

- Priority: `P0`
- Category: `iac`
- Given: 生成包含公开端口、未加密存储和超预算资源的Plan
- When: 执行本Case定义的测试、故障注入和证据收集
- Then: Policy与Cost Gate拒绝，禁止unattended destructive apply
- Evidence: source-snapshot, environment-manifest, raw-command-log, result-json, artifact-manifest, replay-command

### X016-02 — IaC Kubernetes Helm Plan Drift Rollback / Kubernetes Server Dry-run与Admission

- Priority: `P0`
- Category: `iac`
- Given: 提交特权Pod、明文Secret、无资源限制和宽NetworkPolicy
- When: 执行本Case定义的测试、故障注入和证据收集
- Then: Server Schema/Admission/Policy拒绝并定位字段
- Evidence: source-snapshot, environment-manifest, raw-command-log, result-json, artifact-manifest, replay-command

### X016-03 — IaC Kubernetes Helm Plan Drift Rollback / Helm升级漂移与回滚

- Priority: `P2`
- Category: `iac`
- Given: 制造现网Drift后升级失败并执行Rollback
- When: 执行本Case定义的测试、故障注入和证据收集
- Then: Drift被识别，Rollback恢复已知版本且数据/资源不丢失
- Evidence: source-snapshot, environment-manifest, raw-command-log, result-json, artifact-manifest, replay-command
