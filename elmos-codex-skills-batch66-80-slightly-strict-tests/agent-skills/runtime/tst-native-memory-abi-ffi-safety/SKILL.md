---
name: tst-native-memory-abi-ffi-safety
description: "Native内存ABI与FFI安全。验证C/C++/Rust ABI、FFI、所有权、生命周期、Sanitizer、Unsafe和跨语言异常边界。 Use for ELMOS Batch 66–80 slightly strict certification tests."
metadata:
  id: X012
...
---
    # X012 — Native内存ABI与FFI安全

    ## Objective

    验证C/C++/Rust ABI、FFI、所有权、生命周期、Sanitizer、Unsafe和跨语言异常边界。 覆盖成功、边界、负向、故障、重放、恢复、篡改和状态冒充。默认不得把未运行、Skipped、Mock-only或陈旧Evidence计为通过。
    ...
    ### X012-01 — Native内存ABI与FFI安全 / Sanitizer与生命周期

- Priority: `P0`
- Category: `memory-safety`
- Given: 在ASan/UBSan/TSan或Miri下运行代表性与故障路径
- When: 执行本Case定义的测试、故障注入和证据收集
- Then: 无UAF、越界、UB、数据竞争和未解释泄漏
- Evidence: source-snapshot, environment-manifest, raw-command-log, result-json, artifact-manifest, replay-command

### X012-02 — Native内存ABI与FFI安全 / ABI与FFI错误边界

- Priority: `P0`
- Category: `abi`
- Given: 跨语言传递字符串、Buffer、Null、错误、回调和版本结构
- When: 执行本Case定义的测试、故障注入和证据收集
- Then: 调用约定、对齐、所有权、释放和异常边界符合契约
- Evidence: source-snapshot, environment-manifest, raw-command-log, result-json, artifact-manifest, replay-command

### X012-03 — Native内存ABI与FFI安全 / Unsafe最小化与Fuzz

- Priority: `P2`
- Category: `fuzz`
- Given: 对Unsafe/Parser/FFI入口运行有Seed的Fuzz与静态审查
- When: 执行本Case定义的测试、故障注入和证据收集
- Then: Unsafe范围可解释，发现的Crash可复现且无自动忽略
- Evidence: source-snapshot, environment-manifest, raw-command-log, result-json, artifact-manifest, replay-command
