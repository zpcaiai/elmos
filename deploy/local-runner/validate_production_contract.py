...
def validate_static_contract() -> dict[str, Any]:
    ...
    for binding in bindings:
        ...
        if not source.is_relative_to(REPOSITORY_ROOT) or source.is_symlink() or not source.is_file():
...
def validate_runtime_environment(environment: Mapping[str, str] = os.environ) -> None:
    ...
    for name in ("ELMOS_UV_PATH", "ELMOS_LOCAL_RUNNER_PNPM_PATH", "ELMOS_LOCAL_RUNNER_CONTAINER_ENGINE"):
